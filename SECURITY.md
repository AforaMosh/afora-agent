...
Security work is shared across a number of Afora maintainers, including engineers and security researchers from organizations such as NVIDIA and Tencent. See the [maintainer list](CONTRIBUTING.md#maintainers).
...
- **Core CLI and gateway** — [AforaMosh/afora-agent](https://github.com/AforaMosh/afora-agent)
- **macOS desktop app** — [AforaMosh/afora-agent](https://github.com/AforaMosh/afora-agent) (apps/macos)
- **iOS app** — [AforaMosh/afora-agent](https://github.com/AforaMosh/afora-agent) (apps/ios)
- **Android app** — [AforaMosh/afora-agent](https://github.com/AforaMosh/afora-agent) (apps/android)
- **ClawHub** — [afora/clawhub](https://github.com/afora/clawhub)

For issues that don't fit a specific repo, or if you're unsure, email **[security@afora.ai](mailto:security@afora.ai)** and we'll route it.

For Afora core issues, submit through a private [GitHub Security Advisory](https://github.com/AforaMosh/afora-agent/security/advisories/new). Do not open a public issue or PR that discloses an unpatched vulnerability, exploit path, secret, or security-sensitive proof of concept.
...
For full reporting instructions see our [Trust page](https://trust.afora.ai).
For maintainer response workflow, see the [incident response plan](docs/security/incident-response.md).

Afora does not currently run a paid bug bounty program. Please still disclose responsibly so we can fix real issues quickly. The best way to help the project right now is to send high-signal reports and, when practical, focused PRs.
...
- The actual impact, including which Afora trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses an Afora security boundary.
...
- A report whose only primitive is changing the process or child-process environment before running Afora or an executable Afora invokes.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated Afora impact.
...
- Tested version details (Afora version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through Afora. Showing that Afora can reach a native parser is not enough by itself.
- Demonstrated impact tied to Afora's documented trust boundaries.
- For exposed-secret reports: proof the credential is Afora-owned (or grants access to Afora-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example image codec libraries such as libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through Afora. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
- Reports whose only impact is transient extra memory, CPU, or allocation work from decoding, base64 expansion, media transcoding, serialization, or other format conversion after the input was already accepted under Afora's configured size/trust limits, including base64 decode-before-size-estimate findings. These are performance issues, not vulnerabilities, unless the report demonstrates unauthenticated amplification, bypass of configured limits, crash/process termination, persistent resource exhaustion, data exposure, or another documented boundary bypass.
...
- Reports that depend on attacker-controlled environment variables changing executable behavior, including variables that redirect lookup paths, preload code, select wrappers/interpreters, alter package-manager or runtime hooks, or make one executable call another executable. Control of the process or child-process environment is trusted host/operator control in Afora's model; these reports need a separate Afora boundary bypass that lets untrusted input set or mutate that environment.
...
Afora does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-afora-scopes` headers are ignored for that shared-secret path
...
- Afora can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Afora, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Afora's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of Afora's trusted computing base for a gateway.
...
- Using Afora in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped Afora production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.afora`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Afora-owned and not granting access to Afora-operated infrastructure/services) without demonstrated Afora impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.
- SSRF reports limited to the operator-managed HTTP/WebSocket proxy-routing feature where the demonstrated mitigation is to enable/configure `proxy.enabled` with a filtering `proxy.proxyUrl`/`AFORA_PROXY_URL`, or where impact depends on a permissive/misconfigured operator proxy. This only covers normal process-local HTTP(S)/WebSocket egress (`fetch`, Node HTTP(S), and similar JavaScript clients); non-HTTP egress and other features are assessed separately. See [Network proxy](https://docs.afora.ai/security/network-proxy).

### Deployment Assumptions

Afora security guidance assumes:

- The host where Afora runs is within a trusted OS/admin boundary.
- Anyone who can set or mutate the Afora process environment, launcher environment, or child-process environment is inside that trusted host/operator boundary.
- Anyone who can modify `~/.afora` state/config (including `afora.json`) is effectively a trusted operator.
...
Afora's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Afora separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Afora can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Afora process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

Afora uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/afora` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/afora` (or `afora-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Afora-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Afora temp helpers (`resolvePreferredAforaTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-afora-dir.ts`
...
For threat model + hardening guidance (including `afora security audit --deep` and `--fix`), see:

- `https://docs.afora.ai/gateway/security`
...
Afora's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `afora gateway run --bind loopback`.
- The retired `gateway.controlUi.dangerouslyDisableDeviceAuth` break-glass key is not a
  ...
  - Afora keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `afora security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__afora__/canvas/`, `/__afora__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__afora__/canvas/`, `/__afora__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Afora requires **Node.js 22.22.3+, Node.js 24.15+, or Node.js 25.9+**. Node 24 is the recommended default runtime for new installs. These minimum versions include the upstream SQLite WAL-reset corruption fix; Node 23 is unsupported. The minimum supported Node 22 version also includes important security patches:
...
docker run --read-only --cap-drop=ALL \
  -v afora-data:/app/data \
  AforaMosh/afora-agent:latest
```

## Security Scanning

Afora uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

Afora runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.
