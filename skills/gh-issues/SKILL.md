...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${AFORA_CONFIG_PATH:-${AFORA_STATE_DIR:-$HOME/.afora}/afora.json}"
...
```text
${AFORA_STATE_DIR:-$HOME/.afora}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with afora message send if route provided.
```
