...
The Afora macOS app hosts Peekaboo Bridge at
`~/Library/Application Support/Afora/bridge.sock`. Before running Peekaboo
from Afora, select that socket so the CLI uses the app's Screen Recording
and Accessibility grants instead of starting its standalone daemon:

```bash
export PEEKABOO_BRIDGE_SOCKET="${PEEKABOO_BRIDGE_SOCKET:-$HOME/Library/Application Support/Afora/bridge.sock}"
```

Confirm routing with `peekaboo bridge status --json`; `hostKind` must be `gui`
and the socket path must end in `Afora/bridge.sock`.
...
- In Afora subprocesses, set `PEEKABOO_BRIDGE_SOCKET` as shown above. Do not
