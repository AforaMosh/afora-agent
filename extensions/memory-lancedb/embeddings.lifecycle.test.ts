...
function createApi(): AforaPluginApi {
  const config = {};
  return {
    config,
    runtime: {
      config: { current: () => config },
      agent: { resolveAgentDir: () => "/tmp/afora-agent" },
    },
...
describe("memory-lancedb provider lifecycle", () => {
  ...
  it("rotates actual private auth snapshots without replacing the runtime config", async () => {
    const config = {};
    const agentDir = "/tmp/afora-lancedb-private-auth-rotation";
  ...
  it("invalidates every inheriting agent when the actual main auth snapshot rotates", async () => {
    const config = {};
    const agentDirs = {
      private: "/tmp/afora-lancedb-inherited-private",
      secondary: "/tmp/afora-lancedb-inherited-secondary",
    };
  ...
  it("drains an admitted embedding before retiring a rotated actual auth snapshot", async () => {
    const config = {};
    const agentDir = "/tmp/afora-lancedb-inflight-auth-rotation";
