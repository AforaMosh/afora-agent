...
export function legacyMemorySchemaError(): Error {
  return new Error(
    'memory-lancedb: the existing memory table predates per-agent isolation. Run "afora doctor --fix" to assign legacy rows to the default agent, then restart Afora.',
  );
}
