...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__afora_channel_id";
...
type InteractionCallbackConfig = Pick<AforaConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "afora-mattermost-interactions").update(botToken).digest("hex");
}
