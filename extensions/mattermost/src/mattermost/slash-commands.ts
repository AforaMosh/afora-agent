...
/**
 * Built-in Afora commands to register as native slash commands.
...
/**
 * Register all Afora slash commands for a given team.
...
export async function registerSlashCommands(params: {
  ...
  for (const spec of commands) {
    ...
    if (ownedCommands.length === 0 && foreignCommands.length > 0) {
      log?.(
        `mattermost: trigger /${spec.trigger} already used by non-Afora command(s); skipping to avoid mutating external integrations`,
      );
      continue;
    }
...
/**
 * Map the trigger word back to the original Afora command name.
