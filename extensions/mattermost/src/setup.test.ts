...
import type { AforaConfig, AforaPluginApi } from "../runtime-api.js";
...
vi.mock("./setup.accounts.runtime.js", () => {
  ...
  return {
    listMattermostAccountIds: vi.fn((cfg: AforaConfig) => {
  ...
  it("offers env shortcut only for the default account when env is present and config is empty", () => {
    ...
    expect(
      mattermostSetupWizard.envShortcut?.isAvailable?.({
        cfg: { channels: { mattermost: {} } } as AforaConfig,
        accountId: "default",
      } as never),
    ).toBe(true);
  ...
  it("keeps env shortcut as a no-op patch for the selected account", () => {
    expect(
      mattermostSetupWizard.envShortcut?.apply?.({
        cfg: { channels: { mattermost: { enabled: false } } } as AforaConfig,
        accountId: "default",
      } as never),
    ).toEqual({
      channels: {
