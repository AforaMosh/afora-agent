...
export function createUsageRecorders(runtime: DiagnosticsRecorderRuntime) {
  ...
  const recordModelUsage = (
    ...
    const attrs = {
      "afora.channel": evt.channel ?? "unknown",
      "afora.agent": normalizeDiagnosticValue(evt.agentId),
    ...
    if (usage.input) {
      tokensCounter.add(usage.input, { ...attrs, "afora.token": "input" });
    ...
    if (usage.output) {
      tokensCounter.add(usage.output, { ...attrs, "afora.token": "output" });
    ...
    if (usage.cacheRead) {
      tokensCounter.add(usage.cacheRead, { ...attrs, "afora.token": "cache_read" });
    }
    if (usage.cacheWrite) {
      tokensCounter.add(usage.cacheWrite, { ...attrs, "afora.token": "cache_write" });
    }
    if (usage.promptTokens) {
      tokensCounter.add(usage.promptTokens, { ...attrs, "afora.token": "prompt" });
    }
    if (usage.total) {
      tokensCounter.add(usage.total, { ...attrs, "afora.token": "total" });
    }
    ...
    const spanAttrs: Record<string, string | number> = {
      ...attrs,
      "afora.tokens.input": usage.input ?? 0,
      "afora.tokens.output": usage.output ?? 0,
      "afora.tokens.cache_read": usage.cacheRead ?? 0,
      "afora.tokens.cache_write": usage.cacheWrite ?? 0,
      "afora.tokens.total": usage.total ?? 0,
    };
    if (metadata.trusted && metadata.internal && hostPluginId) {
      spanAttrs["afora.plugin"] = normalizeDiagnosticValue(hostPluginId);
    }
    ...
    const span = spanWithDuration("afora.model.usage", spanAttrs, evt.durationMs, {
  ...
  const recordWebhookReceived = (
    ...
    const attrs = {
      "afora.channel": evt.channel ?? "unknown",
      "afora.webhook": evt.updateType ?? "unknown",
    };
  ...
  const recordWebhookProcessed = (
    ...
    const attrs = {
      "afora.channel": normalizeDiagnosticValue(evt.channel),
      "afora.webhook": normalizeDiagnosticValue(evt.updateType),
    };
    ...
    const span = spanWithDuration("afora.webhook.processed", spanAttrs, evt.durationMs);
    span.end();
  };
  ...
  const recordWebhookError = (evt: Extract<DiagnosticEventPayload, { type: "webhook.error" }>) => {
    ...
    const spanAttrs: Record<string, string | number> = {
      ...attrs,
      "afora.error": redactedError,
    };
    const span = tracer.startSpan("afora.webhook.error", {
  ...
  const recordMessageQueued = (
    ...
    const attrs = {
      "afora.channel": normalizeDiagnosticValue(evt.channel),
      "afora.source": normalizeDiagnosticValue(evt.source),
    };
  ...
  const recordMessageDispatchStarted = (
    ...
    trackInternalOrTrustedSpan(
      evt,
      metadata,
      spanWithDuration("afora.message.processed", attrs, undefined, {
  ...
  const recordMessageProcessed = (
    ...
    const attrs = {
      "afora.channel": normalizeDiagnosticValue(evt.channel),
      "afora.outcome": evt.outcome ?? "unknown",
    };
    ...
    const spanAttrs: Record<string, string | number> = { ...attrs };
    if (evt.reason) {
      spanAttrs["afora.reason"] = normalizeDiagnosticValue(evt.reason, "unknown");
    }
    const trackedSpan = getTrackedInternalOrTrustedSpan(evt, metadata);
    const span =
      trackedSpan ??
      spanWithDuration("afora.message.processed", spanAttrs, evt.durationMs, {
        parentContext: internalOrTrustedExplicitParentContext(evt, metadata),
        endTimeMs: evt.ts,
      });
  ...
  const messageDeliveryAttrs = (evt: MessageDeliveryDiagnosticEvent): Record<string, string> => ({
    "afora.channel": normalizeDiagnosticValue(evt.channel),
    "afora.delivery.kind": normalizeDiagnosticValue(evt.deliveryKind, "other"),
  });
  ...
  const recordMessageDeliveryCompleted = (
    ...
    const span = spanWithDuration(
      ...
      {
        ...attrs,
        "afora.delivery.result_count": evt.resultCount,
      },
      evt.durationMs,
  ...
  const recordMessageDeliveryError = (
    ...
    const span = spanWithDuration("afora.message.delivery", attrs, evt.durationMs, {
  ...
  const recordRunStarted = (
    ...
    const span = trackTrustedSpan(
      evt,
      metadata,
      spanWithDuration("afora.run", spanAttrs, undefined, {
        parentContext: activeTrustedParentContext(evt, metadata),
        startTimeMs: evt.ts,
      }),
    );
