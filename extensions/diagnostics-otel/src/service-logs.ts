...
export function createDiagnosticsLogExporter(params: {
  ...
  if (logsEnabled) {
    ...
    if (logsToOtlp) {
      ...
      otelLogger = logProvider.getLogger("afora");
    }
    ...
    const buildDiagnosticLogRecord = (
      ...
      assignOtelLogAttribute(attributes, "afora.log.level", logLevelName);
      if (evt.loggerName) {
        assignOtelLogAttribute(attributes, "afora.logger", evt.loggerName);
      }
      if (evt.loggerParents?.length) {
        assignOtelLogAttribute(attributes, "afora.logger.parents", evt.loggerParents.join("."));
      }
