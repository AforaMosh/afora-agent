...
import { asFiniteNumber, asFiniteNumberInRange } from "afora-agent/plugin-sdk/string-coerce-runtime";
...
export function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "afora.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "afora.model_call.response_bytes", evt.responseStreamBytes);
...
export function assignModelCallPromptStatsAttrs(
  ...
  for (const [key, value] of [
    ["afora.model_call.prompt.input_messages_count", stats.inputMessagesCount],
    ["afora.model_call.prompt.input_messages_chars", stats.inputMessagesChars],
    ["afora.model_call.prompt.system_prompt_chars", stats.systemPromptChars],
    ["afora.model_call.prompt.tool_definitions_count", stats.toolDefinitionsCount],
    ["afora.model_call.prompt.tool_definitions_chars", stats.toolDefinitionsChars],
    ["afora.model_call.prompt.total_chars", stats.totalChars],
  ] as const) {
...
export function assignModelCallUsageAttrs(
  ...
  for (const [key, value] of [
    ["afora.model_call.usage.input_tokens", usage.input],
    ["afora.model_call.usage.output_tokens", usage.output],
    ["afora.model_call.usage.cache_read_input_tokens", usage.cacheRead],
    ["afora.model_call.usage.cache_creation_input_tokens", usage.cacheWrite],
    ["afora.model_call.usage.reasoning_output_tokens", usage.reasoningTokens],
    ["afora.model_call.usage.prompt_tokens", promptTokens],
    ["afora.model_call.usage.total_tokens", usage.total],
...
export function assignGenAiModelCallAttrs(
  ...
  attrs["afora.model_call.observation_unit"] = modelCallObservationUnit(evt);
}
...
export function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("afora.provider.request", {
    "afora.upstreamRequestIdHash": boundedHash,
  });
}
