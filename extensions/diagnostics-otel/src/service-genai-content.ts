...
// Shared text-part reading for gen_ai message normalization: Afora emits
...
function redactInternalReasoningFromMessage(value: unknown): unknown {
  ...
  // instead of using Afora's normalized thinking blocks. Those fields are
...
export function assignOtelToolIdentityAttributes(
  ...
  // low-cardinality, and unlike the dropped afora.toolCallId passthrough keys
...
export function assignOtelModelContentAttributes(
  ...
  if (policy.toolDefinitions) {
    assignOtelContentAttribute(
      attributes,
      "afora.content.tool_definitions",
  ...
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "afora.content.system_prompt", content?.systemPrompt);
  }
}
...
export function assignOtelToolContentAttributes(
  ...
  // afora.content.* names; normalize once so both copies stay byte-identical.
  if (policy.toolInputs) {
    ...
    if (toolInput) {
      ...
      attributes["afora.content.tool_input"] = toolInput;
    }
  }
  if (policy.toolOutputs) {
    ...
    if (toolOutput) {
      ...
      attributes["afora.content.tool_output"] = toolOutput;
    }
  }
}
