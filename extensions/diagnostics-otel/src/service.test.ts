...
vi.mock("afora-agent/plugin-sdk/fetch-runtime", () => ({
...
const ORIGINAL_AFORA_OTEL_PRELOADED = process.env.AFORA_OTEL_PRELOADED;
...
afterAll(() => {
  ...
  vi.doUnmock("afora-agent/plugin-sdk/fetch-runtime");
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.AFORA_OTEL_PRELOADED;
  ...
  afterEach(async () => {
    ...
    resetDiagnosticEventsForTest();
    if (ORIGINAL_AFORA_OTEL_PRELOADED === undefined) {
      delete process.env.AFORA_OTEL_PRELOADED;
    } else {
      process.env.AFORA_OTEL_PRELOADED = ORIGINAL_AFORA_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["afora.provider"]).toBe("openai");
    for (const key of [
      "afora.callId",
      "afora.call_id",
      "afora.chatId",
  ...
  test.each([
    {
      ...
      expectedTokenName: "afora.tokens",
      expectedDurationName: "afora.run.duration_ms",
    },
    {
    ...
    {
      metricNamePrefix: "acme.afora.",
      expectedTokenName: "acme.afora.tokens",
      expectedDurationName: "acme.afora.run.duration_ms",
    },
  ])(
    "replaces the default Afora metric prefix with $metricNamePrefix",
    async ({ metricNamePrefix, expectedTokenName, expectedDurationName }) => {
      ...
      expect(telemetryState.counters.has("afora.tokens")).toBe(
        expectedTokenName === "afora.tokens",
      );
    },
  );
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("afora.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "afora.channel": "telegram",
      "afora.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("afora.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "afora.channel": "telegram",
      "afora.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("afora.message.queued")?.add).toHaveBeenCalledWith(1, {
      "afora.channel": "telegram",
      "afora.source": "telegram",
    });
    expect(telemetryState.histograms.get("afora.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("afora.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "afora.channel": "telegram",
      "afora.source": "telegram",
    });
    expect(telemetryState.histograms.get("afora.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "afora.lane": "main",
    });
    expect(telemetryState.counters.get("afora.message.processed")?.add).toHaveBeenCalledWith(1, {
      "afora.channel": "telegram",
      "afora.outcome": "completed",
    });
    expect(telemetryState.counters.get("afora.message.received")?.add).toHaveBeenCalledWith(1, {
      "afora.channel": "telegram",
      "afora.source": "webhook",
    });
    expect(telemetryState.counters.get("afora.message.received")?.add).toHaveBeenCalledWith(1, {
      "afora.channel": "unknown",
      "afora.source": "unknown",
    });
    expect(
      telemetryState.counters.get("afora.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("afora.message.dispatch.completed")?.add,
    ...
    expect(
      telemetryState.histograms.get("afora.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "afora.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("afora.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "afora.channel": "telegram",
      "afora.outcome": "completed",
    });
    expect(telemetryState.histograms.get("afora.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "afora.lane": "main",
      },
    );
    expect(telemetryState.counters.get("afora.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("afora.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "afora.state": "processing",
    });
    expect(
      telemetryState.histograms.get("afora.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "afora.state": "processing",
    });
    expect(telemetryState.counters.get("afora.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "afora.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("afora.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "afora.agent": "agent.default",
        "afora.channel": "telegram",
        "afora.trigger": "user",
      },
    );
    ...
    expect(spanNames).toContain("afora.webhook.processed");
    expect(spanNames).toContain("afora.message.processed");
    expect(spanNames).toContain("afora.session.stuck");
    const webhookSpanOptions = startedSpanOptions("afora.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("afora.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("afora.message.processed");
    expect(messageSpanOptions?.attributes?.["afora.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["afora.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["afora.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("afora.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("afora.messageId");
  ...
  test("surfaces bridge cleanup failure after attempting every shutdown phase", async () => {
    ...
    await service.start(ctx);
    emitRunStarted();
    const runSpan = spanByName("afora.run");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.AFORA_OTEL_PRELOADED = "1";
    const { service, ctx } = await startOtelService({ traces: true, metrics: true, logs: true });
    ...
    const runDurationRecordCall = lastHistogramRecord("afora.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["afora.provider"]).toBe("openai");
    expect(runDurationAttributes?.["afora.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("afora.run");
    expect(runSpanOptions?.attributes?.["afora.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("afora.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "afora.exporter": "diagnostics-otel",
      "afora.signal": "logs",
      "afora.status": "started",
      "afora.reason": "configured",
    });
  ...
  test.each([" TRUE ", "TrUe"])(
    "disables every Afora-owned telemetry route for OTEL_SDK_DISABLED=%j",
    async (value) => {
      const events: TelemetryExporterEvent[] = [];
      const unsubscribe = onInternalDiagnosticEvent((event) => {
        if (event.type === "telemetry.exporter") {
  ...
  test("preserves preloaded trace and metric ownership while disabling plugin logs", async () => {
    ...
    expect(meterProviderCtor).not.toHaveBeenCalled();
    expect(unhandledRejectionHandlerState.register).not.toHaveBeenCalled();
    expect(lastHistogramRecord("afora.run.duration_ms")?.[0]).toBe(100);
    expect(startedSpanOptions("afora.run")?.attributes?.["afora.outcome"]).toBe("completed");
    expect(logEmit).not.toHaveBeenCalled();
  ...
  test("retires trace ownership across external, unsupported, and supported restarts", async () => {
    ...
    try {
      ...
      process.env.AFORA_OTEL_PRELOADED = "1";
      await service.start(ctx);
      process.env.AFORA_OTEL_PRELOADED = "0";
  ...
  test("exports trusted security events as bounded OTLP logs", async () => {
    ...
    expect(emitCall.body).toBe("afora.security.event");
    expect(emitCall.severityText).toBe("WARN");
    expect(emitCall.severityNumber).toBe(13);
    expect(emitCall.attributes).toMatchObject({
      "afora.security.event_id": "security-event-1",
      "afora.security.category": "tool",
      "afora.security.action": "tool.execution.blocked",
      "afora.security.outcome": "denied",
      "afora.security.severity": "medium",
      "afora.security.reason": "tools.deny",
      "afora.security.actor.kind": "agent",
      "afora.security.actor.id_hash": "agent-hash-1",
      "afora.security.actor.role": "operator",
      "afora.security.actor.scopes": "operator.read,operator.approvals",
      "afora.security.target.kind": "plugin",
      "afora.security.target.name": "@acme/security-event-plugin",
      "afora.security.target.owner": "plugin-installer",
      "afora.security.policy.id": "tools.exec",
      "afora.security.policy.decision": "deny",
      "afora.security.policy.reason": "allowlist.miss",
      "afora.security.control.id": "exec-approval",
      "afora.security.control.family": "approval",
      "afora.security.attribute.params_kind": "object",
      "afora.security.attribute.secretish": "unknown",
    });
    ...
    expect(Object.hasOwn(emitCall.attributes ?? {}, "afora.security.attribute.__proto__")).toBe(
      false,
    );
  ...
  test("exports trusted security events as stdout JSONL logs", async () => {
    ...
    try {
      ...
      expect(record.body).toBe("afora.security.event");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(record.attributes).toMatchObject({
        "afora.security.event_id": "security-event-stdout",
        "afora.security.category": "tool",
        "afora.security.action": "tool.execution.blocked",
        "afora.security.outcome": "denied",
        "afora.security.severity": "medium",
      ...
      expect(Object.hasOwn(record.attributes ?? {}, "afora.security.attribute.__proto__")).toBe(
        false,
      );
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("afora.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "afora.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("afora.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "afora.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("afora.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "afora.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("afora.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["afora.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["afora.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["afora.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "afora.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("afora.payload.large")?.add).toHaveBeenCalledWith(1, {
      "afora.payload.action": "rejected",
      "afora.payload.surface": "gateway.frame",
      "afora.channel": "web",
      "afora.plugin": "none",
      "afora.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("afora.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "afora.exporter": "diagnostics-otel",
      "afora.signal": "logs",
      "afora.status": "failure",
      "afora.reason": "emit_failed",
      "afora.errorCategory": "TypeError",
    });
    expect(
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    telemetryState.counters.get("afora.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("afora.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["afora.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["afora.blocked_by"]).toBe("policy-plugin");
  ...
  test("run.completed error span carries the redacted message off the metric attrs", async () => {
    ...
    expect(startedSpanOptions("afora.run")?.attributes?.["afora.error"]).toBe(
      "upstream model stream stalled then aborted",
    );
    expect(spanByName("afora.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "upstream model stream stalled then aborted",
    });
    // The raw message must never widen metric cardinality.
    const runDuration = lastHistogramRecord("afora.run.duration_ms");
    expect(runDuration?.[1]?.["afora.outcome"]).toBe("error");
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "afora.error")).toBe(false);
  });
  ...
  test("run.completed bounds sensitive error text before export", async () => {
    ...
    const status = mockCallArg(spanByName("afora.run").setStatus, 0) as {
      message?: string;
    };
  ...
  test("harness.run.completed error span carries the redacted message", async () => {
    ...
    expect(startedSpanOptions("afora.harness.run")?.attributes?.["afora.error"]).toBe(
      "model run failed during resolve phase",
    );
    expect(spanByName("afora.harness.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "model run failed during resolve phase",
    });
    const harnessDuration = lastHistogramRecord("afora.harness.duration_ms");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "afora.error")).toBe(false);
  });
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("afora.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["afora.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const attrs = startedSpanOptions("afora.model.call")?.attributes;
    expect(attrs?.["afora.content.input_messages"]).toBe("user prompt");
  });
  ...
  test("lets explicit Afora sampling override the inherited sampler environment", async () => {
  ...
  test("preserves OTLP TLS env options when passing env proxy agents", async () => {
    const certDir = mkdtempSync(path.join(tmpdir(), "afora-otel-tls-"));
    try {
  ...
  test("pins validated collector TLS material on direct HTTPS exporter agents", async () => {
    const certDir = mkdtempSync(path.join(tmpdir(), "afora-otel-direct-tls-"));
    try {
      const rootCertificatePath = path.join(certDir, "root.pem");
  ...
  test("exports diagnostic logs as stdout JSONL without constructing the OTLP log exporter", async () => {
    await startOtelService({
      ...
      captureContent: true,
      configure: (ctx) => {
        ctx.config.diagnostics!.otel!.serviceName = "rovoclaw-afora";
      },
    });
    const stdout = captureStdoutWrites();
    ...
    try {
      ...
      expect(record.signal).toBe("afora.diagnostic.log");
      expect(record["service.name"]).toBe("rovoclaw-afora");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(String(record.body)).not.toContain("sk-1234567890abcdef1234567890abcdef");
      expect(String(record.body)).toContain("sk-123");
      expect(record.attributes).toMatchObject({
        "afora.log.level": "WARN",
        "afora.subsystem": "diagnostic",
      });
      const tokenAttr = record.attributes?.["afora.token"];
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["afora.token"];
    expect(tokenAttr).not.toBe("ghp_abcdefghijklmnopqrstuvwxyz123456"); // pragma: allowlist secret
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "afora.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "afora.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "afora.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/afora/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/afora/src/private.ts:42",
      },
    ...
    expect(emitCall.attributes["afora.good"]).toBe(`${"y".repeat(4095)}...(truncated)`);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(Object.hasOwn(emitCall.attributes, `afora.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "afora.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "afora.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "afora.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "afora.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "afora.code.location")).toBe(false);
  });
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("afora.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "afora.channel": "webchat",
      "afora.agent": "ops",
      "afora.provider": "openai",
      "afora.model": "gpt-5.4",
      "afora.token": "input",
    });
  ...
  test("advertises explicit duration buckets on the afora run/harness/context histograms", async () => {
    const priorSdkBoundaries = [
      0, 5, 10, 25, 50, 75, 100, 250, 500, 750, 1000, 2500, 5000, 7500, 10000,
    ];
    await startOtelService({ metrics: true });

    const runDurationOptions = histogramCreateOptions("afora.run.duration_ms");
    ...
    const harnessDurationOptions = histogramCreateOptions("afora.harness.duration_ms");
    ...
    const contextOptions = histogramCreateOptions("afora.context.tokens");
  ...
  test.each([
    ...
    expect(telemetryState.counters.get("afora.tokens")?.add).toHaveBeenCalledWith(2, {
      "afora.channel": "unknown",
      "afora.agent": "unknown",
      "afora.provider": "openai",
      "afora.model": "gpt-5.4",
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("afora.tokens")?.add.mock.calls),
  ...
  test.each([
    ...
    expect(telemetryState.counters.get("afora.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "afora.lane": expected,
      },
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("afora.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("afora.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "afora.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "afora.sessionId")).toBe(false);
  ...
  test("separates request and turn GenAI client duration by operation", async () => {
    ...
    const aforaModelCallDuration = telemetryState.histograms.get(
      "afora.model_call.duration_ms",
    );
    expect(aforaModelCallDuration?.record).toHaveBeenCalledTimes(4);
    expect(
      aforaModelCallDuration?.record.mock.calls.map(
        (call) => call[1]?.["afora.model_call.observation_unit"],
      ),
    ).toEqual(["request", "request", "turn", "turn"]);
    const spanObservationUnits = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "afora.model.call")
      .map(
        (call) =>
          (call[1] as { attributes?: Record<string, unknown> }).attributes?.[
            "afora.model_call.observation_unit"
          ],
      );
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "afora.agent": "main",
      "afora.skill.activation": "read",
      "afora.skill.name": "tiny-llm-brainstorm",
      "afora.skill.source": "workspace",
      "afora.toolName": "read",
    };
    expect(telemetryState.counters.get("afora.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "afora.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("afora.run");
    expect(spanNames).toContain("afora.model.call");
    expect(spanNames).toContain("afora.harness.run");
    expect(spanNames).toContain("afora.tool.execution");

    const runOptions = startedSpanOptions("afora.run");
    expect(runOptions?.attributes?.["afora.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["afora.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["afora.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["afora.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "afora.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "afora.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "afora.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("afora.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "afora.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "afora.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "afora.sessionKey")).toBe(false);
    expect(modelOptions?.startTime).toBeTypeOf("number");
    expect(modelOptions?.kind).toBe(2);
    expect(modelCall?.[2]).toBeUndefined();

    const harnessCall = startedSpanCall("afora.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["afora.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["afora.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["afora.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["afora.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["afora.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["afora.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["afora.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["afora.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["afora.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["afora.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["afora.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "afora.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "afora.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "afora.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "afora.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("afora.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["afora.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["afora.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["afora.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["afora.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["afora.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "afora.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "afora.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "afora.sessionKey")).toBe(false);
    ...
    const modelCallDuration = lastHistogramRecord("afora.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["afora.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["afora.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("afora.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["afora.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["afora.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("afora.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["afora.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["afora.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("afora.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["afora.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["afora.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("afora.model.call");
    expect(modelSpanAttributes["afora.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["afora.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["afora.model_call.time_to_first_byte_ms"]).toBe(45);
    expect(modelSpanAttributes["afora.model_call.prompt.input_messages_count"]).toBe(2);
    expect(modelSpanAttributes["afora.model_call.prompt.input_messages_chars"]).toBe(3456);
    expect(modelSpanAttributes["afora.model_call.prompt.system_prompt_chars"]).toBe(789);
    expect(modelSpanAttributes["afora.model_call.prompt.tool_definitions_count"]).toBe(4);
    expect(modelSpanAttributes["afora.model_call.prompt.tool_definitions_chars"]).toBe(2345);
    expect(modelSpanAttributes["afora.model_call.prompt.total_chars"]).toBe(6590);
    expect(modelSpanAttributes["afora.model_call.usage.input_tokens"]).toBe(100);
    expect(modelSpanAttributes["afora.model_call.usage.output_tokens"]).toBe(20);
    expect(modelSpanAttributes["afora.model_call.usage.cache_read_input_tokens"]).toBe(30);
    expect(modelSpanAttributes["afora.model_call.usage.cache_creation_input_tokens"]).toBe(5);
    expect(modelSpanAttributes["afora.model_call.usage.reasoning_output_tokens"]).toBe(8);
    expect(modelSpanAttributes["afora.model_call.usage.prompt_tokens"]).toBe(135);
    expect(modelSpanAttributes["afora.model_call.usage.total_tokens"]).toBe(155);
    ...
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "afora.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("afora.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["afora.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["afora.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["afora.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "afora.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "afora.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("afora.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["afora.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "afora.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "afora.runId")).toBe(false);

    const toolSpan = spanByName("afora.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("afora.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  });
  ...
  test("closes a tracked blocked tool span once without creating a duplicate", async () => {
    ...
    const toolSpans = telemetryState.spans.filter(
      (span) => span.name === "afora.tool.execution",
    );
    expect(toolSpans).toHaveLength(1);
    expect(startedSpanOptions("afora.tool.execution")?.startTime).toBe(1_000);
  ...
  test("uses authoritative source timestamps for terminal-only tool spans", async () => {
    ...
    const toolSpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "afora.tool.execution",
    );
    const toolSpans = telemetryState.spans.filter(
      (span) => span.name === "afora.tool.execution",
    );
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("afora.model.failover");
    expect(failoverOptions?.attributes?.["afora.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["afora.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["afora.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["afora.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["afora.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["afora.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["afora.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["afora.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "afora.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "afora.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("afora.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("afora.model.failover")).toStrictEqual([
      1,
      {
        "afora.failover.reason": "overloaded",
        "afora.failover.suspended": "true",
        "afora.lane": "main",
        "afora.model": "claude-opus-4-6",
        "afora.provider": "anthropic",
        "afora.failover.to_model": "gpt-5.4",
        "afora.failover.to_provider": "openai",
      },
    ]);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("afora.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "afora.toolName": "browser",
        "afora.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "afora.tool.owner": "browser-tools",
        "afora.tool.params.kind": "object",
        "afora.deniedReason": "tools.deny",
      },
    ]);
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalledWith(
      "afora.tool.execution",
      expect.anything(),
      expect.anything(),
    );
  });
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("afora.model.failover");
    expect(failoverOptions?.attributes?.["afora.lane"]).toBe("session");
  ...
  test("maps model call APIs and preserves zero usage on terminal spans", async () => {
    ...
    for (const attrs of [modelCallAttrs[0], modelCallAttrs[2]]) {
      expect(attrs).toMatchObject({
        "afora.model_call.usage.input_tokens": 0,
        "afora.model_call.usage.output_tokens": 0,
        "afora.model_call.usage.cache_read_input_tokens": 0,
        "afora.model_call.usage.cache_creation_input_tokens": 0,
        "afora.model_call.usage.reasoning_output_tokens": 0,
        "afora.model_call.usage.prompt_tokens": 0,
        "afora.model_call.usage.total_tokens": 0,
    ...
    for (const key of [
      "afora.model_call.request_bytes",
      "afora.model_call.response_bytes",
    ...
    expect(
      Object.keys(modelCallAttrs[1] ?? {}).filter(
        (key) => key.startsWith("afora.model_call.usage.") || key.startsWith("gen_ai.usage."),
      ),
    ).toEqual([]);
    expect(modelCallAttrs[3]).toMatchObject({
      "afora.model_call.usage.cache_creation_input_tokens": 7,
      "afora.model_call.usage.prompt_tokens": 7,
      "gen_ai.usage.input_tokens": 7,
      "gen_ai.usage.cache_creation.input_tokens": 7,
    });
    expect(modelCallAttrs[3]).not.toHaveProperty("afora.model_call.usage.input_tokens");
  });
  ...
  test("uses latest GenAI request and agent span shapes only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("afora.model.call")).toBeUndefined();
    ...
    expect(agentTurnOptions?.attributes?.["afora.model_call.observation_unit"]).toBe("turn");
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("afora.model.call");
    expect(modelCallOptions?.attributes?.["afora.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "afora.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "afora.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("afora.provider.request", {
      "afora.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("afora.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["afora.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "afora.upstreamRequestIdHash")).toBe(
      false,
    );
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("afora.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "afora.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["afora.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["afora.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["afora.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["afora.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["afora.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["afora.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["afora.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["afora.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["afora.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["afora.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["afora.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["afora.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["afora.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("afora.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "afora.toolName": "process",
      "afora.loop.level": "critical",
      "afora.loop.action": "block",
      "afora.loop.detector": "known_poll_no_progress",
      "afora.loop.count": 20,
      "afora.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("afora.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["afora.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["afora.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["afora.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["afora.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["afora.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["afora.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "afora.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("afora.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("afora.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "afora.memory.level": "critical",
        "afora.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("afora.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "afora.memory.level": "critical",
      "afora.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("afora.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["afora.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["afora.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["afora.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["afora.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["afora.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["afora.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["afora.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["afora.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["afora.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["afora.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "afora.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("afora.diagnostic.async_queue.dropped");
    expect(counter?.add).toHaveBeenCalledWith(4, {
      "afora.diagnostic.async_queue.drop_class": "total",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "afora.diagnostic.async_queue.drop_class": "trusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(2, {
      "afora.diagnostic.async_queue.drop_class": "untrusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "afora.diagnostic.async_queue.drop_class": "priority",
    });
  });
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "afora.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "afora.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "afora.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["afora.run"]).toBeUndefined();
    expect(parentBySpanName["afora.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["afora.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("afora.message.processed");
    const harnessSpan = spanByName("afora.harness.run");
    const runSpan = spanByName("afora.run");
    const usageSpan = spanByName("afora.model.usage");
    const modelCallSpan = spanByName("afora.model.call");
    ...
    expect(parentBySpanName["afora.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["afora.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["afora.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["afora.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["afora.model.call"]?.spanId).toBe(runSpanContext.spanId);
  });
  ...
  test("prepares model and tool spans synchronously for propagation without duplicate spans", async () => {
    ...
    const modelSpanContext = spanByName("afora.model.call").spanContext();
    const runSpanContext = spanByName("afora.run").spanContext();
    expect(startedSpanParentContexts("afora.model.call")).toEqual([runSpanContext]);
    expect(formatDiagnosticTraceparent(modelTrace)).toBe(
      `00-${modelTrace.traceId}-${modelTrace.spanId}-01`,
    );
    ...
    expect(startedSpanParentContexts("afora.tool.execution")).toEqual([modelSpanContext]);
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("afora.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("afora.message.processed")[0]).toBeUndefined();
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("afora.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("afora.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("afora.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("afora.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("afora.message.processed")["afora.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("afora.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("afora.harness.run")?.[2]).toBeUndefined();
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("afora.run")).toBeTypeOf("number");
  });
  ...
  test.each([
    ...
    const runContexts = startedSpanParentContextsByName("afora.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("afora.run").spanContext();
    const modelParentContext = startedSpanParentContexts("afora.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("afora.model.call");
  ...
  test("bounds retained run contexts by evicting the oldest completed runs", async () => {
    ...
    const newestRunSpan = telemetryState.spans.findLast((span) => span.name === "afora.run");
    telemetryState.tracer.startSpan.mockClear();
    ...
    const usageParents = startedSpanParentContexts("afora.model.usage");
  ...
  test("clears retained run contexts when the service stops", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("afora.model.usage")?.[2]).toBeUndefined();
  });
  ...
  test.each([
    ...
    expect(parentBySpanName["afora.run"]).toBeUndefined();
    expect(parentBySpanName["afora.model.call"]).toBeUndefined();
  });
  ...
  test.each([
    ...
    expect(startedSpanParentContexts("afora.harness.run")[0]).toBeUndefined();
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["afora.run"]).toBeUndefined();
    expect(parentBySpanName["afora.model.call"]).toBeUndefined();
    expect(parentBySpanName["afora.tool.execution"]).toBeUndefined();
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "afora.run"),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "afora.model.call",
  ...
  test("nests exec spans under the run when the trace context is Afora-owned", async () => {
    await startOtelService({ traces: true, metrics: true });
    ...
    const execParents = startedSpanParentContexts("afora.exec");
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("afora.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["afora.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["afora.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["afora.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["afora.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("afora.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["afora.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["afora.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["afora.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["afora.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["afora.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["afora.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["afora.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "afora.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "afora.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "afora.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("afora.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("afora.exec")).toBeTypeOf("number");
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("afora.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "afora.channel": "matrix",
      "afora.delivery.kind": "text",
    });
    ...
    expect(deliveryDurationRecords[0]?.[1]["afora.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["afora.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["afora.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["afora.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["afora.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["afora.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["afora.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "afora.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["afora.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["afora.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["afora.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["afora.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["afora.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["afora.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["afora.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["afora.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "afora.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "afora.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "afora.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "afora.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "afora.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "afora.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "afora.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("afora.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["afora.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["afora.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["afora.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("afora.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["afora.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["afora.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["afora.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["afora.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("afora.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["afora.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["afora.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["afora.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("afora.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["afora.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["afora.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["afora.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("afora.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["afora.status"]).toBe("released");
    expect(telemetryState.counters.get("afora.talk.event")?.add).toHaveBeenCalledWith(1, {
      "afora.talk.brain": "agent-consult",
      "afora.talk.event_type": "input.audio.delta",
      "afora.talk.mode": "realtime",
      "afora.talk.provider": "openai",
      "afora.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("afora.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "afora.talk.brain": "agent-consult",
        "afora.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("afora.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "afora.talk.brain": "agent-consult",
      "afora.talk.event_type": "latency.metrics",
      "afora.talk.mode": "realtime",
      "afora.talk.provider": "openai",
      "afora.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("afora.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("afora.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "afora.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "afora.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "afora.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("afora.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "afora.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "afora.content.tool_output")).toBe(
      false,
    );
  ...
  test("exports bounded redacted content when capture is enabled", async () => {
    ...
    const modelAttrs = startedSpanOptions("afora.model.call")?.attributes;
    const toolAttrs = startedSpanOptions("afora.tool.execution")?.attributes;

    expect(modelAttrs?.["afora.content.output_messages"]).toBe("model reply");
    expect(Object.hasOwn(modelAttrs ?? {}, "afora.content.system_prompt")).toBe(false);
    expect(String(modelAttrs?.["afora.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["afora.content.tool_input"]).toBe("tool input");
    ...
    expect(toolAttrs?.["gen_ai.tool.call.arguments"]).toBe(
      toolAttrs?.["afora.content.tool_input"],
    );
    expect(typeof toolAttrs?.["afora.content.tool_output"]).toBe("string");
    expect(String(toolAttrs?.["afora.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["afora.content.tool_output"])).not.toContain("a".repeat(11));
    expect(toolAttrs?.["gen_ai.tool.call.result"]).toBe(
      toolAttrs?.["afora.content.tool_output"],
    );
  });
  ...
  test("omits absent model content fields when capture is enabled", async () => {
    ...
    const attrs = startedSpanOptions("afora.model.call")?.attributes ?? {};
    expect(attrs["afora.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "afora.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "afora.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "afora.content.tool_definitions")).toBe(false);
  ...
  test("exports Claude CLI turn content through the existing Phoenix GenAI keys", async () => {
    ...
    emitTrustedModelCallCompletedWithContent(
      ...
      },
      {
        ...
        systemPrompt: "Afora appended instructions",
      },
    );
    await flushDiagnosticEvents();

    const attrs = startedSpanOptions("afora.model.call")?.attributes;
    expect(attrs?.["afora.api"]).toBe("claude-code");
    expect(attrs?.["afora.transport"]).toBe("stdio-live");
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.input.messages"))).toEqual([
    ...
    const compatibilityOutput = stringAttribute(attrs, "afora.content.output_messages");
  ...
  test("never exports provider-internal thinking payloads in model message attributes", async () => {
    ...
    for (const key of [
      ...
      "afora.content.input_messages",
      "afora.content.output_messages",
    ]) {
    ...
    expect(
      JSON.parse(stringAttribute(attrs, "afora.content.input_messages"))[0]?.content[0],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "afora.content.input_messages"))[0]?.content[1],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "afora.content.output_messages"))[0]?.content[0],
    ).toEqual({ type: "reasoning", redacted: true });
    expect(
      JSON.parse(stringAttribute(attrs, "afora.content.input_messages"))[0]?.content[2],
    ).toEqual({ type: "text", text: "visible input" });
    expect(
      JSON.parse(stringAttribute(attrs, "afora.content.output_messages"))[0]?.content[2],
    ).toEqual({
      type: "toolCall",
      id: "tool-1",
      name: "lookup",
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("afora.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["afora.reason"])).toContain("…");
    expect(typeof attrs?.["afora.reason"]).toBe("string");
    expect(String(attrs?.["afora.reason"])).not.toContain(
