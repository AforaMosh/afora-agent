...
import type { AforaPluginServiceContext } from "../api.js";
import type { ExporterHealthUpdate } from "./service-exporter-health.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/afora-diagnostics-otel-test";
...
type OtelConfig = NonNullable<
  NonNullable<AforaPluginServiceContext["config"]["diagnostics"]>["otel"]
>;
...
type StartOtelServiceOptions = OtelContextFlags & {
  endpoint?: string;
  configure?: (ctx: AforaPluginServiceContext) => void;
};
type InternalDiagnosticListener = Parameters<
  NonNullable<AforaPluginServiceContext["internalDiagnostics"]>["onEvent"]
>[0];
...
const exporterHealthReports = new WeakMap<AforaPluginServiceContext, ReportedExporterHealth[]>();
...
export function createOtelContext(
  ...
  const ctx: AforaPluginServiceContext = {
    config: {
      diagnostics: {
        enabled: true,
        otel: {
