...
/** Input data used to publish Afora gateway Bonjour records. */
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.AFORA_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Afora"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Afora";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Afora\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.AFORA_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "afora";
    ...
    const hostnameLabel = hostnameWithoutLocal.slice(0, labelEnd).trim() || "afora";
    const hostname = truncateToDnsLabel(hostnameLabel, "afora");
    const instanceName =
