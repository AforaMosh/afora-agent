...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  vi.stubEnv("AFORA_MDNS_HOSTNAME", hostname);
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("afora-gw");
  ...
  it("honors truthy AFORA_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    vi.stubEnv("AFORA_DISABLE_BONJOUR", "true");
  ...
  it("auto-disables Bonjour on Fly Machines without Docker sentinel files", async () => {
    ...
    vi.stubEnv("FLY_APP_NAME", "afora-clawcks-test");
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    vi.stubEnv("AFORA_DISABLE_BONJOUR", "0");
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._afora-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._afora-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("never overlaps ciao lifecycle states or conflict handling with another advertise call", async () => {
    ...
    listenerMap.get("name-change")?.("test-host (Afora) (2)");
    listenerMap.get("hostname-change")?.("test-host-(2)");
    expectWarnContaining('name conflict resolved; newName="test-host (Afora) (2)"');
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("Mac (Afora)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
  ...
  it("falls back to afora when system hostname is invalid for DNS", async () => {
    // Allow advertiser to run in unit tests.
    vi.stubEnv("VITEST", undefined);
    vi.stubEnv("NODE_ENV", "development");
    vi.stubEnv("AFORA_MDNS_HOSTNAME", undefined);
    ...
    const [gatewayCall] = createService.mock.calls as Array<[ServiceCall]>;
    expect(gatewayCall?.[0]?.hostname).toBe("afora");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("afora.local");
  ...
  it("truncates reported Kubernetes service name at the DNS label byte limit", async () => {
    ...
    expectDnsLabelByteLength(`${reportedHostname} (Afora)`, 64);
  ...
  it("truncates multi-byte hostname within DNS label byte limit", async () => {
    // 21 CJK characters = 63 bytes in UTF-8, adding " (Afora)" pushes over
  ...
  it("uses system hostname when AFORA_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    vi.stubEnv("VITEST", undefined);
    vi.stubEnv("NODE_ENV", "development");
    vi.stubEnv("AFORA_MDNS_HOSTNAME", undefined);
