...
function getChannelConfig(cfg: AforaConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: AforaConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: AforaConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: AforaConfig, accountId: string): string[] {
