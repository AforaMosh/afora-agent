...
import { resolveAgentIdFromSessionKey } from "afora-agent/plugin-sdk/routing";
...
const AFORA_MODAL_CALLBACK_PREFIX = "afora:";
...
function resolveSlackModalPluginInteractiveData(params: {
  ...
  if (!params.callbackId.startsWith(AFORA_MODAL_CALLBACK_PREFIX)) {
    return undefined;
  }
  const callbackData = params.callbackId.slice(AFORA_MODAL_CALLBACK_PREFIX.length).trim();
...
function shouldHandleSlackModalLifecycleBody(body: unknown): boolean {
  ...
  if (callbackId.startsWith(AFORA_MODAL_CALLBACK_PREFIX)) {
    return true;
  }
