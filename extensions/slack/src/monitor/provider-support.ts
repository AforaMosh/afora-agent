...
import { channelBlockedPatch, channelReadyPatch } from "afora-agent/plugin-sdk/gateway-runtime";
import { asOptionalRecord as asRecord } from "afora-agent/plugin-sdk/string-coerce-runtime";
...
const AFORA_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
const AFORA_SLACK_SOCKET_START_FAILED_EVENT = "unable_to_socket_mode_start";
const AFORA_SLACK_NATIVE_RECONNECT_OBSERVER_KEY = "__aforaNativeReconnectFailureObserver";
...
function installSlackNativeReconnectFailureObserver(receiver: unknown) {
  ...
  if (Reflect.get(client, AFORA_SLACK_NATIVE_RECONNECT_OBSERVER_KEY)) {
    return;
  }
  ...
  Reflect.set(client, AFORA_SLACK_NATIVE_RECONNECT_OBSERVER_KEY, true);
  Reflect.set(
    client,
    ...
    function patchedDelayReconnectAttempt(this: object, callback: unknown) {
      ...
      const delayMs =
        (Number.isFinite(pingTimeoutMs) && pingTimeoutMs >= 0
          ? pingTimeoutMs
          : AFORA_SLACK_CLIENT_PING_TIMEOUT_MS) * nextFailureCount;
        ...
        const reconnectTimer = setTimeout(() => {
          ...
          Promise.resolve(callback.call(this)).then(resolve, (error: unknown) => {
            if (callback === Reflect.get(this, "start")) {
              emit.call(this, AFORA_SLACK_SOCKET_START_FAILED_EVENT, error);
              resolve(undefined);
              return;
            }
...
function shouldSkipAforaSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout: AFORA_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping Afora's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
    ...(appReceiver ? { receiver: appReceiver } : {}),
  });
  app.use(async (args) => {
    await params.onContextIdentity?.(args.context ?? {});
    if (shouldSkipAforaSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
