...
import { resolveEnvelopeFormatOptions } from "afora-agent/plugin-sdk/channel-inbound";
...
describe("resolveSlackThreadContextData", () => {
  const storeFixture = createSlackSessionStoreFixture("afora-slack-thread-context-");
  ...
  async function resolveAllowlistedThreadContext(params: {
    ...
    const result = await resolveSlackThreadContextData({
      ...
      envelopeOptions: resolveEnvelopeFormatOptions({} as AforaConfig),
