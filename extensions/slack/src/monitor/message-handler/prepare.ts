...
import { resolveChannelMessageSourceReplyDeliveryMode } from "afora-agent/plugin-sdk/channel-outbound";
...
import { shouldHandleTextCommands } from "afora-agent/plugin-sdk/command-surface";
import { ensureConfiguredBindingRouteReady } from "afora-agent/plugin-sdk/conversation-runtime";
import { formatErrorMessage } from "afora-agent/plugin-sdk/error-runtime";
import { mimeTypeFromFilePath } from "afora-agent/plugin-sdk/media-mime";
...
const SLACK_CHANNEL_ACCESS_DOCS_URL =
  "https://docs.afora.ai/channels/slack#access-control-and-routing";
    ...
    if (
      ...
      try {
        await (params.eventScope?.client ?? ctx.app.client).chat.postEphemeral({
          ...
          user: message.user,
          text: `${subject} can’t reply here because this channel isn’t in its Afora channel allowlist. Ask the Afora owner to allow this channel. <${SLACK_CHANNEL_ACCESS_DOCS_URL}|Learn how to configure Slack channel access.>`,
        });
