...
import { installChannelOutboundPayloadContractSuite } from "afora-agent/plugin-sdk/channel-contract-testing";
...
describe("slackOutbound sendPayload", () => {
    ...
    expect(client.chat.postMessage.mock.calls.at(-1)?.[0]).toMatchObject({
      ...
      blocks: [
        ...
        },
        {
          ...
          elements: [expect.objectContaining({ action_id: "afora:reply_select:1" })],
        },
        {
          type: "actions",
        ...
        },
        {
          ...
          elements: [expect.objectContaining({ action_id: "afora:reply_select:2" })],
        },
      ],
    });
  });
            ...
            buttons: [
              {
                ...
                action: {
                  type: "web-app",
                  url: "https://node.tailnet.ts.net/__afora__/mcp-app#opaque-ticket",
                },
              },
      ...
      blocks: [
        expect.objectContaining({
          ...
          elements: [
            ...
            expect.objectContaining({
              type: "button",
              action_id: "afora:reply_link:1:2",
  ...
  it("uses the full ordered table fallback when preserved siblings exceed the block limit", async () => {
    ...
    expect(segments[2]).toMatchObject({
      kind: "blocks",
      blocks: [{ block_id: "afora_reply_buttons_1" }, { block_id: "afora_reply_buttons_2" }],
    });
  ...
  it("offsets presentation controls against native Slack blocks before standalone interactive controls", async () => {
    ...
    expect(blocks?.[2]?.block_id).toBe("afora_reply_buttons_2");
    expect(blocks?.[2]?.elements?.[0]?.action_id).toBe("afora:reply_button:2:1");
    expect(blocks?.[3]?.block_id).toBe("afora_reply_buttons_3");
    expect(blocks?.[3]?.elements?.[0]?.action_id).toBe("afora:reply_button:3:1");
  });
});
