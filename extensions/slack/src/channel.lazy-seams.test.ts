...
vi.mock("afora-agent/plugin-sdk/target-resolver-runtime", async (orig) => {
...
vi.mock("afora-agent/plugin-sdk/extension-shared", async (orig) => {
  const original = (await orig()) as Record<string, unknown>;
  return {
    ...original,
...
function makeMinimalSlackConfig(
  ...
  return { channels: { slack } } as AforaConfig;
}
