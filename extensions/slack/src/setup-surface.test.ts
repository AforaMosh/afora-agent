...
describe("slackSetupWizard.prepare", () => {
  ...
  it("prints the manifest as plain JSON when Slack is not configured", async () => {
    ...
    await runSetupWizardPrepare({
      ...
      cfg: { channels: { slack: {} } } as AforaConfig,
    ...
    expect(JSON.parse(manifest)).toEqual({
      display_information: {
        name: "Afora",
        description: "Afora connector for Afora",
      },
      features: {
        bot_user: {
          display_name: "Afora",
  ...
  it("collects a signing secret instead of an app token for HTTP user identity", async () => {
    ...
    const result = await runSetupWizardConfigure({
      configure,
      cfg: { channels: { slack: { mode: "http" } } } as AforaConfig,
      prompter: queued.prompter,
      options: { secretInputMode: "plaintext" as const },
    });
  ...
  it.each([
    { name: "new setup", cfg: {} as AforaConfig },
    {
      name: "switch from user identity",
      cfg: { channels: { slack: { postAs: "user" } } } as AforaConfig,
    },
