...
const SLACK_API_URL_KEYS = ["SLACK_API_URL", "AFORA_SLACK_API_URL"] as const;
const PROXY_KEYS = [
  "ALL_PROXY",
  "HTTPS_PROXY",
  "HTTP_PROXY",
...
function writeTempCa(contents: string): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "afora-slack-proxy-ca-"));
  tempDirs.push(dir);
...
describe("slack web client config", () => {
  ...
  it("does not read AFORA_SLACK_API_URL as a default Slack Web API root", () => {
    process.env.AFORA_SLACK_API_URL = "http://127.0.0.1:49152/api/";
