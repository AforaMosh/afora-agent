...
function readSlackAforaBlockIndex(blockId: string, prefix: string): number | undefined {
...
export function resolveSlackBlockOffsets(blocks?: readonly SlackBlock[]): SlackBlockRenderOptions {
  ...
  for (const block of blocks ?? []) {
    ...
    buttonIndexOffset = Math.max(
      buttonIndexOffset,
      readSlackAforaBlockIndex(blockId, "afora_reply_buttons_") ?? 0,
    );
    selectIndexOffset = Math.max(
      selectIndexOffset,
      readSlackAforaBlockIndex(blockId, "afora_reply_select_") ?? 0,
    );
  }
  return {
    buttonIndexOffset,
...
export function buildSlackInteractiveBlocks(
  ...
  return reduceLegacyInteractiveReply(interactive, initialState, (state, block) => {
    ...
    if (block.type === "buttons") {
      ...
      state.blocks.push({
        type: "actions",
        block_id: `afora_reply_buttons_${++state.buttonIndex}`,
        elements,
      });
      return state;
    }
    ...
    state.blocks.push({
      type: "actions",
      block_id: `afora_reply_select_${++state.selectIndex}`,
      elements: [
        {
          type: "static_select",
          action_id:
...
function buildSlackPresentationButtonBlock(
  ...
  return elements.length > 0
    ? {
        type: "actions",
        block_id: `afora_reply_buttons_${buttonIndex}`,
        elements,
      }
    : undefined;
}
...
function buildSlackPresentationSelectBlock(
  ...
  return options.length > 0 && optionKinds.size === 1
    ? {
        type: "actions",
        block_id: `afora_reply_select_${selectIndex}`,
        elements: [
          {
            type: "static_select",
            action_id:
