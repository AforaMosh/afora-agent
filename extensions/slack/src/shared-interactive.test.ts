...
describe("buildSlackInteractiveBlocks", () => {
    ...
    expect(
    ...
    ).toEqual([
      {
        ...
        elements: [
          {
            type: "static_select",
            action_id: "afora:reply_select:1",
            placeholder: {
              type: "plain_text",
              text: "Pick one",
              emoji: true,
      ...
      },
      {
        ...
        elements: [
          {
            type: "button",
            action_id: "afora:reply_button:1:1",
            text: {
              type: "plain_text",
              text: "Retry",
              emoji: true,
  ...
  it("preserves original callback payloads for round-tripping", () => {
    ...
    expect(buttonBlock.elements?.[0]?.action_id).toBe("afora:reply_button:1:1");
    expect(buttonBlock.elements?.[0]?.value).toBe("pluginbind:approval-123:o");
    expect(selectBlock.elements?.[0]?.action_id).toBe("afora:reply_select:1");
  ...
  it("drops value-only Slack buttons with values beyond Block Kit limits", () => {
    ...
    expect(buttonBlock.elements?.[1]).toEqual({
      type: "button",
      action_id: "afora:reply_link:1:3",
      text: {
        type: "plain_text",
        text: "Docs",
        emoji: true,
  ...
  it("preserves URL-only buttons as Slack link buttons", () => {
    ...
    expect(buttonBlock.elements?.[0]).toEqual({
      type: "button",
      action_id: "afora:reply_link:1:1",
      text: {
        type: "plain_text",
        text: "Docs",
        emoji: true,
    ...
    ).toEqual([
      {
        ...
        elements: [
          expect.objectContaining({
            action_id: "afora:question_button:1:1",
        ...
        elements: [
          {
            ...
            value:
              'afora:approval:v1:{"approvalId":"plugin:req/😀","approvalKind":"plugin","decision":"allow-once"}',
          },
        ],
      },
    ]);
...
describe("resolveSlackReplyBlocks", () => {
  it("offsets legacy interactive blocks after channel and presentation controls", () => {
    ...
    const legacyButtonBlock = blocks?.[2] as
      | { elements?: Array<{ action_id?: string }> }
      | undefined;
    expect(blocks?.[0]?.block_id).toBe("afora_reply_buttons_1");
    expect(blocks?.[1]?.block_id).toBe("afora_reply_buttons_2");
    expect(presentationButtonBlock?.elements?.[0]?.action_id).toBe("afora:reply_button:2:1");
    expect(blocks?.[2]?.block_id).toBe("afora_reply_buttons_3");
    expect(legacyButtonBlock?.elements?.[0]?.action_id).toBe("afora:reply_button:3:1");
  });
});
