...
metadata: { "afora": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Afora `sessions_spawn`
- **File I/O** == Afora `read`/`write`
- **Remote fetch** == Afora `web_fetch` (or `exec` with curl when POST is required)
