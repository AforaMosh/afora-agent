...
describe("registerPolicyDoctorChecks", () => {
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          kind: "alsoAllow",
          entries: ["exec"],
          source: "oc://afora.config/tools/alsoAllow",
        }),
        expect.objectContaining({
          id: "reviewer-alsoAllow",
          kind: "alsoAllow",
          entries: ["write"],
          source: "oc://afora.config/agents/list/#0/tools/alsoAllow",
        }),
      ]),
    );
    expect(result.findings).toEqual(
  ...
  it("repairs quoted channel ingress paths without splitting slash segments", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ocPath: 'oc://afora.config/channels/"team/sebby"/requireMention',
      }),
    ]);
    expect(result.changes).toEqual([
  ...
  it("skips scoped channel ingress repairs that would mutate inherited defaults", async () => {
    ...
    expect(result.remainingFindings).toEqual([
      expect.objectContaining({
        ...
        ocPath: "oc://afora.config/channels/defaults/groupPolicy",
