...
async function runModelPolicyFixture(providers: object, cfg: AforaConfig) {
...
describe("registerPolicyDoctorChecks", () => {
  ...
  it("blocks governed tool evaluation until TOOLS.md is migrated into AGENTS.md", async () => {
    ...
    expect(beforeMigration.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        message:
          "TOOLS.md contains unmigrated governed tool declarations; run `afora doctor --fix` to migrate them into the AGENTS.md `## Tools` section before policy evaluation can pass.",
        path: "TOOLS.md",
      }),
    ]);
  ...
  it("reports model providers denied by policy", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://afora.config/models/providers/openrouter",
        requirement: "oc://policy.jsonc/models/providers/deny",
      }),
      expect.objectContaining({
        checkId: "policy/models-denied-provider",
        severity: "error",
        ocPath: "oc://afora.config/agents/defaults/model",
        requirement: "oc://policy.jsonc/models/providers/deny",
      }),
    ]);
  });
  ...
  it("compares canonical model provider refs for allow policy checks", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://afora.config/models/providers/aws-bedrock",
  ...
  it("reports model allowlist keys outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-unapproved-provider",
        severity: "error",
        ocPath: 'oc://afora.config/agents/defaults/models/"openrouter/*"',
        requirement: "oc://policy.jsonc/models/providers/allow",
      }),
    ]);
  });
  ...
  it("reports per-agent model allowlist keys outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-unapproved-provider",
        severity: "error",
        ocPath: 'oc://afora.config/agents/list/#0/models/"openrouter/*"',
        requirement: "oc://policy.jsonc/models/providers/allow",
      }),
    ]);
  });
  ...
  it("reports configured model providers outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-unapproved-provider",
        severity: "error",
        ocPath: "oc://afora.config/models/providers/anthropic",
        requirement: "oc://policy.jsonc/models/providers/allow",
      }),
    ]);
  });
  ...
  it("reports non-default agent model refs outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-unapproved-provider",
        severity: "error",
        ocPath: "oc://afora.config/agents/defaults/subagents/model",
        requirement: "oc://policy.jsonc/models/providers/allow",
      }),
    ]);
  });
  ...
  it("reports per-agent model refs outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-denied-provider",
        severity: "error",
        ocPath: "oc://afora.config/agents/list/#0/model/primary",
        requirement: "oc://policy.jsonc/models/providers/deny",
      }),
    ]);
  });
  ...
  it("reports MCP servers denied by policy", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://afora.config/mcp/servers/untrusted",
  ...
  it("preserves MCP server casing for deny rules", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/mcp-denied-server",
        severity: "error",
        ocPath: "oc://afora.config/mcp/servers/DocsServer",
        requirement: "oc://policy.jsonc/mcp/servers/deny",
      }),
    ]);
  });
  ...
  it("quotes MCP server ids with whitespace in policy evidence paths", () => {
    ...
    expect(server).toEqual(
      expect.objectContaining({
        id: "Outlook Graph",
        source: 'oc://afora.config/mcp/servers/"Outlook Graph"',
      }),
    );
  });
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          severity: "error",
          ocPath: "oc://afora.config/session/dmScope",
        ...
        expect.objectContaining({
          ...
          severity: "error",
          ocPath: "oc://afora.config/channels/telegram/groupPolicy",
    ...
    expect(evidence.ingress).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          source: "oc://afora.config/channels/telegram/groupPolicy",
          value: "open",
        }),
      ]),
    );
    ...
    expect(evidence.ingress).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          explicit: false,
          kind: "channelGroupPolicy",
          source: "oc://afora.config/channels/signal/groupPolicy",
          value: "allowlist",
        }),
      ]),
    );
    ...
    expect(evidence.ingress).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          source: "oc://afora.config/channels/slack/dm/enabled",
          value: "disabled",
        }),
      ]),
    );
  ...
  it("records nested ingress mention overrides", () => {
    ...
    expect(scanPolicyIngress(cfg)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          source:
            "oc://afora.config/channels/discord/guilds/ops/channels/releases/requireMention",
          value: false,
        }),
        expect.objectContaining({
          source:
            "oc://afora.config/channels/msteams/teams/engineering/channels/general/requireMention",
          value: false,
        }),
        expect.objectContaining({
          source: "oc://afora.config/channels/matrix/rooms/standup/requireMention",
          value: false,
        }),
        expect.objectContaining({
          source:
  ...
  it("uses effective ingress defaults when policy governs omitted fields", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/ingress-open-groups-denied",
        ocPath: "oc://afora.config/channels/qqbot/groupPolicy",
      }),
    ]);
  });
  ...
  it("infers allowlist group posture from configured groups", async () => {
    ...
    expect(evidence.ingress).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "channelGroupPolicy",
          source: "oc://afora.config/channels/telegram/groups",
          value: "allowlist",
        }),
        expect.objectContaining({
          kind: "channelRequireMention",
          source: "oc://afora.config/channels/telegram/requireMention",
          value: true,
        }),
      ]),
    );
  ...
  it("does not infer allowlist posture from Slack channel entries", async () => {
    ...
    expect(evidence.ingress).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          channel: "slack",
          kind: "channelGroupPolicy",
          source: "oc://afora.config/channels/slack/groupPolicy",
          value: "open",
        }),
      ]),
    );
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/ingress-open-groups-denied",
          ocPath: "oc://afora.config/channels/slack/groupPolicy",
        }),
      ]),
    );
  });
