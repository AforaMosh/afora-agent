...
import type { HealthCheck, HealthFinding } from "afora-agent/plugin-sdk/health";
  ...
  return policyDenied
    ...
    .map((command): HealthFinding => {
      return {
        ...
        severity: "error",
        message: `Gateway node command '${command}' is denied by policy but not denied by Afora config.`,
        source: "policy",
        path: "afora config",
