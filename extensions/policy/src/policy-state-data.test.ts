...
describe("scanPolicyDataHandling", () => {
  it("reports canonical per-agent memory overrides from agents.entries", () => {
    ...
    expect(evidence).toContainEqual(
      expect.objectContaining({
        ...
        source:
          "oc://afora.config/agents/entries/support/memory/search/experimental/sessionMemory",
        scope: "agent",
        agentId: "support",
        value: true,
      }),
  ...
  it("keeps legacy-list policy evidence visible during doctor compatibility", () => {
    ...
    expect(evidence).toContainEqual(
      expect.objectContaining({
        source: "oc://afora.config/agents/list/#0/memory/search/experimental/sessionMemory",
        agentId: "support",
        value: true,
      }),
    );
