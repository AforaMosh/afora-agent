...
import { createUnionActionGate } from "afora-agent/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "afora-agent/plugin-sdk/channel-contract";
import type { DiscordActionConfig, AforaConfig } from "afora-agent/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: AforaConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: AforaConfig) {
