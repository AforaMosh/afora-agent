...
vi.mock("afora-agent/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("afora-agent/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<AforaConfig["channels"]>["discord"]>["execApprovals"],
): AforaConfig {
  return {
    channels: {
      discord: {
