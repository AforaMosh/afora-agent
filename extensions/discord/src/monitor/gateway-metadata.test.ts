...
function createStalledLookup() {
  ...
  return {
    lookupFn: lookupFn as unknown as NonNullable<
      Parameters<
        typeof import("afora-agent/plugin-sdk/ssrf-runtime").fetchWithSsrFGuard
      >[0]["lookupFn"]
    >,
    release: () => release?.(),
  };
...
describe("Discord gateway metadata", () => {
  it("resolves gateway info timeouts from strict integer env values", () => {
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { AFORA_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "90000" },
      }),
    ).toBe(90_000);
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { AFORA_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "0x1000" },
      }),
    ).toBe(30_000);
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { AFORA_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "1e3" },
      }),
    ).toBe(30_000);
  });
