...
vi.mock("afora-agent/plugin-sdk/channel-outbound", async () => {
  const actual = await vi.importActual<typeof import("afora-agent/plugin-sdk/channel-outbound")>(
...
describe("deliverDiscordReply", () => {
      ...
      replies: [
        {
          text: [
            "[tool:exec]",
            "<parameter=command>",
            'cat /proc/mounts 2>/dev/null | grep -i "libra|rav|afora" | head -20',
            "</parameter>",
            "",
            "<function=exec>",
            "<parameter=command>",
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    await deliverDiscordReply({
      ...
      textLimit: 2000,
      replyToMode: "off",
      mediaLocalRoots: ["/tmp/afora-media"],
      kind: "final",
    });
    ...
    expect(params.mediaAccess).toEqual({ localRoots: ["/tmp/afora-media"] });
  });
