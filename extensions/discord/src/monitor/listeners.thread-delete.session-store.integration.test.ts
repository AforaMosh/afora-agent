...
import { withEnvAsync, withStateDirEnv } from "afora-agent/plugin-sdk/test-env";
...
describe("DiscordThreadDeleteListener session-store integration", () => {
  it("deletes matching sessions from every configured agent store", async () => {
    await withStateDirEnv("afora-discord-thread-delete-", async ({ tempRoot, stateDir }) => {
      ...
      await withEnvAsync({ AFORA_STATE_DIR: canonicalStateDir }, async () => {
