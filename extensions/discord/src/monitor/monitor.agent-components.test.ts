...
import { peekSystemEvents, resetSystemEventsForTest } from "afora-agent/plugin-sdk/test-fixtures";
...
describe("agent components", () => {
  ...
  const createCfg = (): AforaConfig => ({}) as AforaConfig;
  ...
  it("sends pairing reply when DM sender is not allowlisted", async () => {
    ...
    expect(pairingText).toContain(`afora pairing approve discord ${code}`);
