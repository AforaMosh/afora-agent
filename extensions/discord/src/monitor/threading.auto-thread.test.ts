...
describe("maybeCreateDiscordAutoThread autoThreadName", () => {
  it("renames created thread when generated mode is enabled", async () => {
    ...
    const cfg = { agents: { defaults: { model: "anthropic/claude-opus-4-6" } } } as AforaConfig;
    const result = await maybeCreateDiscordAutoThread(
      createBaseParams({
        baseText: "Need help with deploy rollout",
        combinedBody: "Need help with deploy rollout",
        channelName: "afora",
        channelDescription: "Afora development coordination and release planning",
    ...
    expectGeneratedTitleField("channelName", "afora");
    expectGeneratedTitleField(
      "channelDescription",
      "Afora development coordination and release planning",
    );
