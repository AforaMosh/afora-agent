...
vi.mock("afora-agent/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("afora-agent/plugin-sdk/reply-dispatch-runtime")>(
    "afora-agent/plugin-sdk/reply-dispatch-runtime",
  );
  return {
    ...actual,
...
vi.mock("afora-agent/plugin-sdk/command-status-runtime", () => ({
...
function createConfig(params?: { requireMention?: boolean }): AforaConfig {
  return {
    commands: {
...
async function createStatusCommand(cfg: AforaConfig, pluginExecute?: ReturnType<typeof vi.fn>) {
