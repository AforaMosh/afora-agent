...
function createCfg(): AforaConfig {
  return {
    channels: {
      discord: {
        enabled: true,
...
function resolveAccount(cfg: AforaConfig, accountId = "default"): ResolvedDiscordAccount {
  return discordPlugin.config.resolveAccount(cfg, accountId);
}

function startDiscordAccount(cfg: AforaConfig, accountId = "default") {
