...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: AforaConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): AforaConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("afora-agent/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("afora-agent/plugin-sdk/native-command-config-runtime")
  >("afora-agent/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("afora-agent/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("afora-agent/plugin-sdk/error-runtime")>(
    "afora-agent/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
