...
describe("discord config schema", () => {
  ...
  it("loads guild map and dm group settings", () => {
    ...
    expect(cfg.dm?.groupChannels).toEqual(["afora-dm"]);
    ...
    expect(cfg.guilds?.["123"]?.slug).toBe("friends-of-afora");
  ...
  it.each([
    ...
    },
    {
      ...
      config: { activity: "Live", activityUrl: "https://twitch.tv/afora" },
    },
    {
