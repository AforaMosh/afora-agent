import { createChannelConfigUiHints } from "afora-agent/plugin-sdk/channel-core";
import type { ChannelConfigUiHint } from "afora-agent/plugin-sdk/channel-core";
...
export const discordChannelConfigUiHints = {
  ...
  ...createChannelConfigUiHints({
    ...
    nativeCommands: true,
    streaming: {
      "": 'Discord preview streaming is off by default. Set mode to "partial", "block", or "progress" to opt in. Run afora doctor --fix to migrate legacy keys.',
  ...
  "voice.mode": {
    ...
    help: "Conversation mode: agent-proxy (default) uses realtime voice as the microphone/speaker for the routed Afora agent, stt-tts uses batch speech-to-text plus TTS, and bidi lets the realtime provider converse directly with the Afora consult tool.",
  },
  "voice.agentSession": {
    label: "Discord Voice Agent Session",
    help: 'Controls which Afora conversation receives voice turns. Leave unset for the voice channel session, or set mode="target" with a Discord target such as channel:123 to make voice an extension of an existing text channel session.',
  },
  ...
  "voice.realtime.model": {
    ...
    help: "Provider realtime session model, such as gpt-realtime-2.1. This is separate from voice.model, which remains the Afora agent brain model.",
  },
  ...
  "voice.realtime.toolPolicy": {
    ...
    help: "Tool policy for the Afora agent consult tool in realtime voice modes: safe-read-only, owner, or none. Default is owner for agent-proxy and safe-read-only for bidi.",
  },
  "voice.realtime.consultPolicy": {
    label: "Discord Realtime Consult Policy",
    help: "Use always to strongly prefer the Afora agent brain for substantive realtime turns. agent-proxy defaults to always.",
  },
  ...
  "voice.reconnectGraceMs": {
    ...
    help: "Grace period for a disconnected Discord voice session to enter Signalling or Connecting before Afora destroys it. Default: 15000.",
  },
  "voice.captureSilenceGraceMs": {
    label: "Discord Voice Capture Silence Grace (ms)",
    help: "Silence window after Discord reports a speaker ended before Afora finalizes the audio segment for transcription. Default: 2000.",
  },
  "voice.tts": {
