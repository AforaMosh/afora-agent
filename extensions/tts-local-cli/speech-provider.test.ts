...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "afora-agent/plugin-sdk/speech-core";
...
function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "afora-cli-tts-test-"));
...
`,
  ...
  it.each(["synthesize", "synthesizeTelephony"] as const)(
    ...
    async (method) => {
      ...
      const providerConfig = { command: "missing-afora-tts-test-command" };
      const run =
        method === "synthesize"
