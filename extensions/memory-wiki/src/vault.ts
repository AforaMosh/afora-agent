...
import { FsSafeError, pathExists, root as fsRoot } from "afora-agent/plugin-sdk/security-runtime";
import {
...
const WIKI_VAULT_SCAFFOLD = ["AGENTS.md", "WIKI.md", "index.md", ".afora-wiki/log.jsonl"];
...
This vault is maintained by the Afora memory-wiki plugin.
...
- Compiled query and prompt snapshots live in Afora plugin state, not vault files.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".afora-wiki/log.jsonl", "", createdFiles);
