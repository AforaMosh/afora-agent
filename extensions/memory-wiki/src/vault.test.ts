...
describe("initializeMemoryWikiVault", () => {
  it("creates the wiki layout and seed files", async () => {
    ...
    await expect(fs.readFile(path.join(rootDir, "WIKI.md"), "utf8")).resolves.toContain(
      "snapshots live in Afora plugin state",
    );
    await expect(fs.access(path.join(rootDir, ".afora-wiki", "cache"))).rejects.toThrow(
      /ENOENT/,
    );
    await expect(fs.access(path.join(rootDir, ".afora-wiki", "state.json"))).rejects.toThrow(
      /ENOENT/,
    );
    await expect(fs.access(path.join(rootDir, ".afora-wiki", "locks"))).rejects.toThrow(
      /ENOENT/,
    );
  });
