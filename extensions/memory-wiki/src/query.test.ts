...
import { filterMemorySearchHitsBySessionVisibility } from "@afora/memory-core/api.js";
...
vi.mock("@afora/memory-core/api.js", { spy: true });

vi.mock("afora-agent/plugin-sdk/memory-host-core", () => ({
...
function createAppConfig(): AforaConfig {
  return {
    agents: {
...
function createSessionVisibilityAppConfig(): AforaConfig {
  return {
    agents: {
...
function createAgentSessionVisibilityAppConfig(): AforaConfig {
  return {
  ...
  loadCombinedSessionStoreForGatewayMock.mockReturnValue({
    ...
    store: {
      "agent:main:child-session": {
        ...
        updatedAt: 1,
        sessionFile: "/tmp/afora/child-session.jsonl",
      },
      "agent:main:sibling-session": {
        sessionId: "sibling-session",
        updatedAt: 2,
        sessionFile: "/tmp/afora/sibling-session.jsonl",
      },
    },
  });
}
...
describe("searchMemoryWiki", () => {
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Related",
          "<!-- afora:wiki:related:start -->",
          "### Related Pages",
          "- [Needle Person](entities/needle-person.md)",
          "<!-- afora:wiki:related:end -->",
          "",
        ].join("\n"),
      }),
      "utf8",
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Agent Card",
          "- Maintainer lane: CEO; Microsoft-facing Afora maintainer",
          "",
          "## AI Notes",
  ...
  it("reports a contract error when the shared manager lacks search()", async () => {
    ...
    // Partial manager as registered by @mem0/afora-mem0 <= 1.0.14.
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:visible-session": {
          ...
          updatedAt: 1,
          sessionFile: "/tmp/afora/visible-session.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:main": {
          sessionId: "main",
          updatedAt: 1,
          sessionFile: "/tmp/afora/main.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
