...
export function registerWikiCli(program: Command, registration: MemoryWikiCliRegistration) {
  ...
  let commandContext:
    | { agentId?: string; appConfig?: AforaConfig; config: ResolvedMemoryWikiConfig }
    | undefined;
  ...
  wiki.hook("preAction", (_thisCommand, actionCommand) => {
    ...
    if (
      ...
      try {
        ...
        throw new Error(
          "No default memory-wiki agent is configured. Pass --agent <id>, or add an agent with `afora agents add`.",
        );
      }
    }
