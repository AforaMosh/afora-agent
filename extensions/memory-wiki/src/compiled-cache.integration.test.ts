...
import type { OpenBlobStoreOptions } from "afora-agent/plugin-sdk/plugin-state-runtime";
import {
...
describe("Memory Wiki compiled cache lifecycle", () => {
  beforeEach(async () => {
    ...
    blobStoreEnv = { ...process.env, AFORA_STATE_DIR: blobStateDir };
    configureMemoryWikiCompiledCacheStore(createCacheStore());
  });
  ...
  it("ignores legacy files and rebuilds only on compile", async () => {
    ...
    const legacyPath = path.join(rootDir, ".afora-wiki", "cache", "agent-digest.json");
  ...
  it("rejects claims newer than a restored vault after lifecycle refresh", async () => {
    ...
    const logPath = path.join(rootDir, ".afora-wiki", "log.jsonl");
  ...
  it("rejects a predecessor snapshot when a vault path is reused", async () => {
    ...
    await fs.mkdir(path.join(rootDir, ".afora-wiki"), { recursive: true });
    await Promise.all([
      fs.writeFile(path.join(rootDir, "WIKI.md"), "# Replacement\n", "utf8"),
      fs.writeFile(
        path.join(rootDir, ".afora-wiki", "log.jsonl"),
        `${JSON.stringify({
