export function zoomMeetingStatusAccessSource(): string {
  ...
  if (identityVerified && !inCall && passcodeRequired) {
    controlManualAction = manualActionFor("zoom-passcode-required", "Enter the Zoom meeting passcode in the Afora browser profile, then retry joining.");
  } else if (identityVerified && !inCall && captchaRequired) {
    controlManualAction = manualActionFor("zoom-captcha-required", "Complete Zoom's security check in the Afora browser profile, then retry joining.");
  }
`;
}
