...
// Nodes without the Afora app (plain `afora node host run`) advertise
...
export async function handleLogbookSnapshot(rawParams: unknown): Promise<LogbookSnapshotPayload> {
  ...
  // keeps captures out of the broader Afora temp namespace.
  const captureDir = path.join(resolvePreferredAforaTmpDir(), "logbook");
