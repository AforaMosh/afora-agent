...
import { withTrustedWebSearchEndpoint } from "afora-agent/plugin-sdk/provider-web-search";
...
// without this, undici sends a generic `node` UA and Afora usage is only
  ...
  const init = await postMcp({
    ...
    body: {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "afora-parallel", version: PLUGIN_VERSION },
      },
    },
  });
  if (!init.ok) {
