...
function resolveMoonshotModels(
  registeredProvider: Awaited<ReturnType<typeof registerSingleProviderPlugin>>,
  modelId: string,
): Model<"openai-completions">[] {
  const catalogProvider = buildMoonshotProvider();
  ...
  return [defaultModel, { ...defaultModel, baseUrl: MOONSHOT_CN_BASE_URL }].map((routeModel) => {
    const prepared = registeredProvider.normalizeResolvedModel?.({
      provider: "moonshot",
      modelId,
      model: routeModel,
    } as never) as Model<"openai-completions"> | undefined;
    if (!prepared) {
      throw new Error(`registered Moonshot provider did not prepare ${modelId}`);
    }
    if (modelId === "kimi-k3") {
      expect(prepared.nativeVideoInput?.wireFamily).toBe("openai-chat-video-url");
    }
    return prepared;
  });
}
...
describeModelLive("moonshot K2.6 replay live", () => {
  it("accepts a cross-model tool-call replay after backfilling reasoning_content", async () => {
    ...
    for (const model of resolveMoonshotModels(provider, "kimi-k2.6")) {
      try {
...
describeModelLive("moonshot K2.7 Code live", () => {
  it("omits thinking controls and completes a replayed tool turn", async () => {
    ...
    let lastAuthError: unknown;
    for (const model of resolveMoonshotModels(provider, "kimi-k2.7-code")) {
      try {
        await runScenario(model);
        return;
      } catch (error) {
...
describeModelLive("moonshot K3 live", () => {
  it("forces max reasoning and completes a required tool call", async () => {
    ...
    let lastAuthError: unknown;
    for (const model of resolveMoonshotModels(provider, "kimi-k3")) {
      try {
