...
function resolveKimiBaseUrl(kimi?: KimiConfig, aforaConfig?: AforaConfig): string {
  ...
  const moonshotBaseUrl = aforaConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: AforaConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.afora.ai/tools/kimi-search",
