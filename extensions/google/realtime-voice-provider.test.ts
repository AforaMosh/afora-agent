...
import type { RealtimeVoiceTool } from "afora-agent/plugin-sdk/realtime-voice";
...
describe("buildGoogleRealtimeVoiceProvider", () => {
  ...
  it("uses Gemini 3.1 Live-compatible defaults", async () => {
    const bridge = createGoogleLiveBridge({
      ...
      tools: [createRealtimeTool("afora_agent_consult")],
    });
  ...
  it("connects with Google Live setup config and tool declarations", async () => {
    ...
    expect(declarations[1]?.name).toBe("afora_agent_consult");
    expect(declarations[1]?.description).toBe("Ask Afora");
  ...
  it("creates constrained browser sessions for Google Live Talk", async () => {
    ...
    expect(declaration?.name).toBe("afora_agent_consult");
  ...
  it("keeps Google Live consult calls open after continuing tool responses", async () => {
    ...
    lastConnectParams().callbacks.onmessage({
      ...
      toolCall: {
        functionCalls: [
          { id: "consult-call", name: "afora_agent_consult", args: { prompt: "hi" } },
        ],
      },
    });
