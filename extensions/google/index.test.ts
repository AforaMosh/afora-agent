...
import { createCapturedThinkingConfigStream } from "afora-agent/plugin-sdk/provider-test-contracts";
import type {
  RealtimeVoiceBridge,
...
describe("google provider plugin hooks", () => {
  ...
  it("keeps the Gemini CLI runtime without Afora-owned OAuth surfaces", async () => {
    const { providers } = await registerProviderPlugin({
      plugin: googleProviderPlugin,
      id: "google",
      name: "Google Provider",
  ...
  it("resolves Google Vertex ADC auth evidence to the config marker", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "afora-google-vertex-config-key-"));
  ...
  it("prefers relocated Google Cloud SDK ADC over the home fallback", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "afora-google-vertex-cloud-sdk-"));
