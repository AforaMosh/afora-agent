...
const AFORA_OPENSHELL_E2E = process.env.AFORA_E2E_OPENSHELL === "1";
const AFORA_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const AFORA_OPENSHELL_COMMAND =
  process.env.AFORA_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
const AFORA_OPENSHELL_CONFIG_HOME =
  process.env.AFORA_E2E_OPENSHELL_CONFIG_HOME?.trim() || null;
const AFORA_OPENSHELL_HOST_IP = process.env.AFORA_E2E_OPENSHELL_HOST_IP?.trim() || null;
...
RUN echo "afora-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
async function resolveOpenShellHostIp(): Promise<string> {
  if (AFORA_OPENSHELL_HOST_IP) {
    return AFORA_OPENSHELL_HOST_IP;
  }
  ...
  throw new Error(
    "OpenShell E2E could not resolve the OpenShell Docker network gateway; set AFORA_E2E_OPENSHELL_HOST_IP",
  );
}
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && AFORA_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: AFORA_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        throw new Error("OpenShell E2E requires a working Docker daemon");
      }
      if (!(await commandAvailable(AFORA_OPENSHELL_COMMAND))) {
        throw new Error(`OpenShell CLI is unavailable: ${AFORA_OPENSHELL_COMMAND}`);
      }
      if (!AFORA_OPENSHELL_CONFIG_HOME) {
        throw new Error(
          "OpenShell E2E requires AFORA_E2E_OPENSHELL_CONFIG_HOME because tests isolate HOME and XDG_CONFIG_HOME",
        );
      }
      const openshellConfigHome = AFORA_OPENSHELL_CONFIG_HOME;
      const hostIp = await resolveOpenShellHostIp();
      const gatewayName = await activeOpenShellGateway(AFORA_OPENSHELL_COMMAND, {
        ...process.env,
      ...
      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-openshell-e2e-"));
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: AFORA_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("afora-openshell-e2e");
