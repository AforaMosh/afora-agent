...
export function inspectCuaDriverArtifacts(
  ...
  if (!accepted) {
    const fixHint = "Reinstall Afora from a complete official package.";
    return failure(
      "COMPUTER_DRIVER_MANIFEST_INVALID",
  ...
  if (!sdkManifestPath || !platformManifestPath) {
    ...
    const fixHint = `Reinstall Afora on this node host so ${DRIVER_PACKAGE} ${accepted.version} and its native platform package are installed together.`;
    return failure(
      "COMPUTER_DRIVER_PACKAGE_MISSING",
  ...
  try {
    ...
    const fixHint =
      "Reinstall Afora on this node host; do not repair native package files by hand.";
    return failure(
      "COMPUTER_DRIVER_PACKAGE_MISSING",
  ...
  if (
    ...
    const fixHint = `Reinstall or update Afora on this node host so both CUA Driver packages resolve to ${accepted.version}.`;
    return failure(
      "COMPUTER_DRIVER_VERSION_MISMATCH",
  ...
  for (const [filename, expectedDigest] of Object.entries(accepted.artifact.files).toSorted(
    ...
    try {
      ...
      const fixHint = `Reinstall Afora on this node host to restore ${platformPackage} ${accepted.version}.`;
      return failure(
        "COMPUTER_DRIVER_PACKAGE_MISSING",
    ...
    if (!stat.isFile() || stat.isSymbolicLink()) {
      const fixHint = "Reinstall Afora; the native driver files must be regular package files.";
      return failure(
        "COMPUTER_DRIVER_DIGEST_MISMATCH",
    ...
    if (actualDigest !== expectedDigest) {
      const fixHint =
        "Reinstall Afora; do not run or replace the mismatched native package files.";
      return failure(
        "COMPUTER_DRIVER_DIGEST_MISMATCH",
