...
vi.mock("afora-agent/plugin-sdk/setup-tools", async (importOriginal) => {
  const actual = await importOriginal<typeof import("afora-agent/plugin-sdk/setup-tools")>();
  return {
    ...actual,
...
vi.mock("afora-agent/plugin-sdk/run-command", () => ({
  runPluginCommandWithTimeout: runPluginCommandWithTimeoutMock,
}));

vi.mock("afora-agent/plugin-sdk/temp-path", async (importOriginal) => {
  const actual = await importOriginal<typeof import("afora-agent/plugin-sdk/temp-path")>();
  return {
    ...actual,
...
async function withTempFile(run: (filePath: string) => Promise<void>) {
  const workDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-signal-download-"));
  try {
...
describe("installSignalCliFromRelease", () => {
  ...
  it("removes the download temp dir on the success path too", async () => {
    setProcessPlatform("linux", "x64");
    const staging = await fs.mkdtemp(path.join(os.tmpdir(), "afora-signal-staging-"));
    try {
...
describe("installSignalCli", () => {
  it("uses Homebrew on macOS instead of downloading the first GitHub release archive", async () => {
    ...
    const brewPrefix = await fs.mkdtemp(path.join(os.tmpdir(), "afora-signal-brew-"));
...
describe("extractSignalCliArchive", () => {
  async function withArchiveWorkspace(run: (workDir: string) => Promise<void>) {
    const workDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-signal-install-"));
    try {
      await run(workDir);
    } finally {
      await fs.rm(workDir, { recursive: true, force: true }).catch(() => undefined);
