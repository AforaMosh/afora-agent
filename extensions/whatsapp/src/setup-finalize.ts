...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: AforaConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: AforaConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: AforaConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: AforaConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("afora channels login"),
      }),
      "WhatsApp",
    );
  }
