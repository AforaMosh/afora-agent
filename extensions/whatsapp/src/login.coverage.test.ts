...
import { sanitizeTerminalText } from "afora-agent/plugin-sdk/test-fixtures";
...
const testState = vi.hoisted(() => ({
  authDir: `${(process.env.TMPDIR ?? "/tmp").replace(/\/+$/, "")}/afora-wa-creds-${process.pid}-${Math.random().toString(16).slice(2)}`,
}));
