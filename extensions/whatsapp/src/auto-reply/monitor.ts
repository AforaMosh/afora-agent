...
import { resolveInboundDebounceMs } from "afora-agent/plugin-sdk/channel-inbound-debounce";
    ...
    while (true) {
        ...
        if (setupDecision) {
          ...
          if (setupDecision.action === "stop") {
            ...
            if (setupDecision.healthState === "logged-out") {
              runtime.error(
                `WhatsApp session logged out during setup. Run \`${formatCliCommand("afora channels login --channel whatsapp")}\` to relink.`,
              );
            } else if (setupDecision.healthState === "conflict") {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then restart the channel. To force a fresh QR, run \`${formatCliCommand("afora channels logout --channel whatsapp")}\` before \`${formatCliCommand("afora channels login --channel whatsapp")}\`. Stopping web monitoring.`,
              );
            } else {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}) after ${setupDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("afora channels login --channel whatsapp")}\` if the issue persists.`,
              );
            }
            await controller.shutdown();
            break;
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("afora channels login --channel whatsapp")}\` to relink.`,
          );
