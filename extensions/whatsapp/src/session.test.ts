...
import { resetLogger, setLoggerOverride } from "afora-agent/plugin-sdk/runtime-env";
import {
  afterEach,
  beforeAll,
  beforeEach,
...
const AFORA_WHATSAPP_WEB_SOCKET_URL_ENV = "AFORA_WHATSAPP_WEB_SOCKET_URL";
...
function createTempCaFile(contents: string): string {
  const dir = createTempAuthDir("afora-wa-proxy-ca");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("afora-wa-creds-test");
  ...
  it("creates standalone directory sockets without inbound message consumers", async () => {
    const authDir = createTempAuthDir("afora-wa-directory-socket");
  ...
  it("prints compact terminal QR output when requested", async () => {
    const authDir = createTempAuthDir("afora-wa-terminal-qr");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("afora-wa-creds-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("afora-wa-authdir-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("afora-wa-auth-parent-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("afora-wa-creds-symlink-save");
      const targetPath = path.join(authDir, "target-creds.json");
      const credsPath = path.join(authDir, "creds.json");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("afora-wa-creds-parent-symlink-save");
      const targetBaseDir = path.join(rootDir, "target-base");
      const linkedBaseDir = path.join(rootDir, "linked-base");
      const authDir = path.join(linkedBaseDir, "default");
      fsSync.mkdirSync(targetBaseDir);
  ...
  it("uses AFORA_WHATSAPP_WEB_SOCKET_URL as the default Baileys WebSocket URL", async () => {
    vi.stubEnv(AFORA_WHATSAPP_WEB_SOCKET_URL_ENV, " ws://127.0.0.1:49153/ws/chat ");
  ...
  it("ignores blank Baileys WebSocket URL environment overrides", async () => {
    vi.stubEnv(AFORA_WHATSAPP_WEB_SOCKET_URL_ENV, " ");
  ...
  it("rejects invalid AFORA_WHATSAPP_WEB_SOCKET_URL values", async () => {
    vi.stubEnv(AFORA_WHATSAPP_WEB_SOCKET_URL_ENV, "http://127.0.0.1:14567/ws");

    await expect(createWaSocket(false, false)).rejects.toThrow(
      "AFORA_WHATSAPP_WEB_SOCKET_URL must use ws:// or wss://.",
    );
  ...
  it("preserves explicit Baileys WebSocket URL options over invalid environment", async () => {
    vi.stubEnv(AFORA_WHATSAPP_WEB_SOCKET_URL_ENV, "http://127.0.0.1:49153/ws/chat");
  ...
  it("logWebSelfId prints cached E.164 when creds exist", () => {
    const authDir = createTempAuthDir("afora-wa-log-self");
    fsSync.writeFileSync(
  ...
  it("logWebSelfId prints cached lid details when creds include a lid", () => {
    const authDir = createTempAuthDir("afora-wa-log-self-lid");
    fsSync.writeFileSync(
      path.join(authDir, "creds.json"),
      JSON.stringify({
        me: {
  ...
  it("does not clobber creds backup when creds.json is corrupted", async () => {
    const authDir = createTempAuthDir("afora-wa-corrupt-backup");
  ...
  it("revalidates setup ownership immediately before a delayed creds.update write", async () => {
    const authDir = createTempAuthDir("afora-wa-guarded-creds");
  ...
  it("revalidates setup ownership before Baileys persists signal keys", async () => {
    const authDir = createTempAuthDir("afora-wa-guarded-keys");
    const guardError = new Error("verified inference route changed");
    let routeOwner = "original";
    const beforeCredentialPersistence = vi.fn(async () => {
      if (routeOwner !== "original") {
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("afora-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("afora-wa-a");
    const authDirB = createTempAuthDir("afora-wa-b");
    const onError = vi.fn();
  ...
  it("rotates creds backup when creds.json is valid JSON", async () => {
    const authDir = createTempAuthDir("afora-wa-rotate-backup");
    const credsPath = path.join(authDir, "creds.json");
    const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("afora-wa-rotate-backup-symlink");
      const credsPath = path.join(authDir, "creds.json");
      const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    const authDir = createTempAuthDir("afora-wa-creds-atomic-write");
    const credsPath = path.join(authDir, "creds.json");
    const openMock = mockFsOpenForCredsWrites();
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("afora-wa-creds-atomic");
    const credsPath = path.join(authDir, "creds.json");
