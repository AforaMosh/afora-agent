...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["AFORA_STATE_DIR", "AFORA_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-wa-profile-"));
  ...
  it("resolves the default web auth dir from AFORA_STATE_DIR at call time", async () => {
