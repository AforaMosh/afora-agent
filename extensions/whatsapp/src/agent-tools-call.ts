...
import { normalizeE164 } from "afora-agent/plugin-sdk/account-resolution";
import { createActionGate, stringEnum } from "afora-agent/plugin-sdk/channel-actions";
import type {
  AnyAgentTool,
  AforaPluginApi,
  AforaPluginToolContext,
} from "afora-agent/plugin-sdk/core";
import { mulawToPcm } from "afora-agent/plugin-sdk/realtime-voice";
import { detectBinary } from "afora-agent/plugin-sdk/setup-tools";
import { resolveOAuthDir } from "afora-agent/plugin-sdk/state-paths";
...
async function resolveRequesterE164(params: {
  accountId: string;
  cfg: NonNullable<AforaPluginToolContext["config"]>;
...
function resolveRuntimeConfig(api: AforaPluginApi, context: AforaPluginToolContext) {
    ...
    async execute(_toolCallId, rawParams, signal) {
      ...
      if (params.action === "status") {
        return jsonResult({
          ...
          note: "MeowCaller uses a separate WhatsApp linked-device session; it cannot reuse Afora's Baileys credentials.",
        });
      }
      ...
      if (linkedSelf === target) {
        throw new Error(
          "WhatsApp cannot call the linked account itself; use a dedicated Afora WhatsApp number",
        );
      }
      ...
      try {
        ...
        const tempDir = await fs.mkdtemp(
          path.join(resolvePreferredAforaTmpDir(), "afora-whatsapp-call-"),
        );
...
export function registerWhatsAppCallTool(api: AforaPluginApi): void {
