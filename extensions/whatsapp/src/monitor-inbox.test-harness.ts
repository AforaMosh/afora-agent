...
import { createChannelIngressQueueForTests } from "afora-agent/plugin-sdk/plugin-state-test-runtime";
...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/afora-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/afora-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("Afora: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "afora-auth-"));
    } else {
      authDir = undefined;
    }
  });
