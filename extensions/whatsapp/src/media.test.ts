...
import { captureEnv, mockPinnedHostnameResolution } from "afora-agent/plugin-sdk/test-env";
import {
  createGrayscaleAlphaPngBuffer,
  createSolidPngBuffer,
} from "afora-agent/plugin-sdk/test-fixtures";
import { withMockedWindowsPlatform, withRestoredMocks } from "afora-agent/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "afora-agent/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredAforaTmpDir(), "afora-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub AFORA_STATE_DIR.
    // Also keep it outside the Afora temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["AFORA_STATE_DIR"]);
    process.env.AFORA_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "afora-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredAforaTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/afora` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredAforaTmpDir();
  ...
  it("allows default Afora state workspace and sandbox roots", async () => {
  ...
  it("rejects default Afora state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
