    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (healthState === "logged-out") {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("afora channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (healthState === "stale") {
        ...
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("afora doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
        return;
      }
      ...
      if (
        ...
        issues.push({
          ...
          fix: `Watch: ${formatCliCommand("afora logs --follow")} and run ${formatCliCommand("afora channels status --probe")} if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.`,
        });
        return;
      }
