...
import { DEFAULT_ACCOUNT_ID, type AforaConfig } from "afora-agent/plugin-sdk/setup";
...
const hoisted = vi.hoisted(() => ({
  detectWhatsAppLinked: vi.fn<(cfg: AforaConfig, accountId: string) => Promise<boolean>>(
    async () => false,
  ),
  ...
  resolveWhatsAppAuthDir: vi.fn<
    (params: { cfg: AforaConfig; accountId: string }) => { authDir: string }
  >(() => ({
    authDir: "/tmp/afora-whatsapp-test",
  })),
}));
...
vi.mock("afora-agent/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("afora-agent/plugin-sdk/setup")>(
    "afora-agent/plugin-sdk/setup",
  );
  return {
    ...actual,
...
function expectFinalizeResult(result: Awaited<ReturnType<typeof runFinalizeWithHarness>>): {
  ...
  return result as { cfg: AforaConfig };
}
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/afora-whatsapp-test" });
  });
  ...
  it("writes named-account DM policy and allowFrom instead of the channel root", async () => {
    ...
    const named = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "work",
        cfg: createWhatsAppWorkAccountConfig() as AforaConfig,
      }),
    );
  ...
  it("uses configured defaultAccount for omitted-account finalize writes", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "",
        cfg: createWhatsAppWorkAccountConfig({ defaultAccount: "work" }) as AforaConfig,
      }),
    );
  ...
  it("forces wildcard allowFrom for open policy without allowFrom follow-up prompts", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        cfg: createWhatsAppRootAllowFromConfig() as AforaConfig,
      }),
    );
