...
} from "afora-agent/plugin-sdk/agent-harness";
import { isRecord } from "afora-agent/plugin-sdk/string-coerce-runtime";
import { createTokenjuiceAforaEmbeddedExtension } from "./runtime-api.js";

type TokenjuiceToolResultHandler = (
  event: {
    toolName: string;
    input: Record<string, unknown>;
    content: AforaAgentToolResult["content"];
    details: unknown;
    isError?: boolean;
  },
  ctx: { cwd: string },
) => Promise<Partial<AforaAgentToolResult> | void> | Partial<AforaAgentToolResult> | void;

function normalizeDetails(
  event: AgentToolResultMiddlewareEvent,
  current: AforaAgentToolResult,
): unknown {
  if (
