...
const CODEX_DIAGNOSTICS_SOURCE = "afora-diagnostics";
...
function formatCodexDiagnosticsTargetBlock(
  ...
  if (target.sessionKey) {
    lines.push(`Afora session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
  }
  if (target.sessionId) {
    lines.push(`Afora session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
  }
...
function formatCodexDiagnosticsTargetLine(target: CodexDiagnosticsTarget): string {
  ...
  if (sessionLabel) {
    parts.push(`Afora session ${formatCodexValueForDisplay(sessionLabel)}`);
  }
