...
/** Resolves the same agent scope Afora uses for transcript/session ownership. */
...
/** Builds the terminal coordination error used when a newer Afora session owns the binding. */
...
const threadBindingSchema = z
  .object({
    ...
    // marker; public Afora session metadata must never authorize user-home access.
    ...
    // this marker across resumes so Afora never substitutes a default or fallback.
    preserveNativeModel: z.literal(true).optional().catch(undefined),
    // Continue creates the Afora Chat before native execution. This closed
...
/** Lets the authoritative Afora session generation claim a stale stable binding row. */
  ...
  return {
            ...
            if (mutation.kind === "reclaim-generation") {
              ...
              if (ownsGeneration) {
                if (
                  ...
                  // Reset boundaries now retain the Afora session id. The
    ...
    async adoptSessionGeneration(identity, expectedPreviousSessionId) {
      return await runBindingMutation(async () => {
        ...
        // Context-engine compaction rotates the physical Afora session before
