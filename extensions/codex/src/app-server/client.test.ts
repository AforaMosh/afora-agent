...
import { embeddedAgentLog, AFORA_VERSION } from "afora-agent/plugin-sdk/agent-harness-runtime";
...
describe("CodexAppServerClient", () => {
  ...
  it("initializes with the required client version", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "afora-agent/0.147.0 (macOS; test)" },
    });
  ...
  it("blocks unsupported app-server versions during initialize", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "afora-agent/0.124.9 (macOS; test)" },
    });
  ...
  it("blocks the previously bundled Codex app-server version", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "afora-agent/0.146.0 (macOS; test)" },
    });
  ...
  it("blocks Codex app-server prereleases of the exact supported version", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "afora-agent/0.147.0-alpha.2 (macOS; test)" },
    });
  ...
  it("blocks Codex app-server prereleases outside generated stable schemas", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "afora-agent/0.144.0-alpha.1 (macOS; test)" },
    });
  ...
  it("blocks Codex app-server custom builds outside generated stable schemas", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "afora-agent/0.144.0+custom (macOS; test)" },
    });
  ...
  it("blocks stable Codex app-server versions newer than generated schemas", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: `afora/${newerVersion} (macOS; test)` },
    });
    ...
    expect(JSON.parse(harness.writes[0] ?? "{}")).toEqual({
      ...
      result: {
        ...
        contentItems: [
          {
            type: "inputText",
            text: `Afora dynamic tool call timed out after ${CODEX_DYNAMIC_TOOL_SERVER_REQUEST_TIMEOUT_MS}ms before sending a response to Codex.`,
          },
        ],
      },
    });
