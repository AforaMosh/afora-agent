 ...
 * access through the active Afora sandbox backend.
 */
...
import { SsrFBlockedError, isBlockedHostnameOrIp } from "afora-agent/plugin-sdk/ssrf-runtime";
...
import type { HttpHeader, AforaExecServer } from "./types.js";
...
async function runStreamingSandboxHttpRequest(
  ...
  try {
    ...
    if (!command) {
      throw new Error("Afora sandbox HTTP exec spec did not provide a command.");
    }
...
tmp=$(mktemp "$TMPDIR/afora-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/afora-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
