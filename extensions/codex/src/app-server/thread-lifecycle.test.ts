...
describe("Codex app-server native code mode config", () => {
  ...
  it("keeps Codex-native subagents primary while limiting Afora spawn to Afora delegation", () => {
    const instructions = buildDeveloperInstructions(createAttemptParams({ provider: "openai" }), {
      dynamicTools: [
        {
          type: "function",
          name: "sessions_spawn",
          description: "Start an Afora session",
    ...
    expect(instructions).toContain(
      "Use Afora `sessions_spawn` only for Afora or ACP delegation, never as a substitute for `spawn_agent`.",
    );
  });
  ...
  it("never advertises unavailable delegation or session tools", () => {
    const restrictedParams = [
      ...
      Object.assign(createAttemptParams({ provider: "openai" }), { toolsAllow: ["afora"] }),
  ...
  it("adds native completion handoff guidance only when sessions_yield is available", () => {
    ...
    expect(withSessionsYield).toContain(
      "end the current turn with `afora_direct.sessions_yield`",
    );
    expect(withSessionsYield).toContain(
    ...
    expect(withoutSessionsYield).not.toContain("`afora_direct.sessions_yield`");
    expect(withoutSessionsYield).not.toContain("native `wait_agent`");
    expect(withWrongNamespace).not.toContain("`afora_direct.sessions_yield`");
    expect(withWrongNamespace).not.toContain("native `wait_agent`");
  });

  it.each([
    { namespace: "afora_direct", exposesNativeYield: true },
    { namespace: "afora", exposesNativeYield: false },
  ])(
    ...
    ({ namespace, exposesNativeYield }) => {
      ...
      expect(instructions.includes("`afora_direct.sessions_yield`")).toBe(exposesNativeYield);
  ...
  it("summarizes deferred dynamic tool names in developer instructions", () => {
    ...
    expect(instructions).toContain(
      "Deferred searchable Afora dynamic tools available: image_generate, music_generate.",
    );
    expect(instructions).toContain("Use `tool_search` when directly callable");
    expect(instructions).toContain(
      "On code-mode-only models, use `exec` instead: filter `ALL_TOOLS` by name and description",
  ...
  it("materializes namespaced prompt tools once while preserving all guidance", () => {
    ...
    expect(namespaceReads).toBe(1);
    expect(instructions).toContain(
      "Deferred searchable Afora dynamic tools available: alpha_tool, skill_workshop, zeta_tool.",
    );
    ...
    expect(instructions).not.toContain("`afora_direct.sessions_yield`");
  });
  ...
  it("keeps developer instructions compact when no dynamic tools are deferred", () => {
    ...
    expect(instructions).not.toContain("Deferred searchable Afora dynamic tools available");
  });
  ...
  it("keeps Afora skill catalogs out of developer instructions", () => {
    const params = createAttemptParams({ provider: "openai" });
  ...
  it("omits Afora model selection when adopting a native Codex thread", () => {
    const request = buildThreadResumeParams(createAttemptParams({ provider: "codex" }), {
      threadId: "thread-adopted",
      model: "afora-model",
      modelProvider: "afora-provider",
      preserveNativeModel: true,
      appServer: createAppServerOptions() as never,
      developerInstructions: "test instructions",
    });
...
describe("Codex app-server turn params", () => {
  it("builds resume and turn params from the currently selected Afora model", () => {
    const params = createAttemptParams({ provider: "codex" });
  ...
  it("uses turn-scoped collaboration instructions for heartbeat Codex turns", () => {
    ...
    expect(heartbeatCollaborationMode.settings.developer_instructions).toContain(
      "This is an Afora heartbeat turn. Apply these instructions only to this heartbeat wake",
    );
    expect(heartbeatCollaborationMode.settings.developer_instructions).toContain(
  ...
  it("uses turn-scoped collaboration instructions for cron Codex turns", () => {
    ...
    expect(cronCollaborationMode.settings.developer_instructions).toContain(
      "This is an Afora cron automation turn",
    );
    expect(cronCollaborationMode.settings.developer_instructions).toContain(
...
describe("Codex plugin binding recovery", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-plugin-recovery-"));
    resetCodexTestBindingStore();
  });
...
describe("Codex thread-effective app attestation", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-plugin-attestation-"));
    resetCodexTestBindingStore();
  });
...
describe("Codex app-server adopted thread lifecycle", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-thread-adoption-"));
    resetCodexTestBindingStore();
  });
  ...
  it("keeps Afora from overriding App Server model selection across resumes", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createThreadLifecycleParams(sessionFile, workspaceDir);
...
describe("Codex app-server supervised branch lifecycle", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-supervision-"));
    resetCodexTestBindingStore();
  });
...
describe("Codex app-server thread lifecycle timing", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-thread-lifecycle-"));
