...
export function mapCodexAppServerRemoteWorkspacePath(
  ...
  if (!normalizedValue.startsWith(prefix)) {
    throw new Error(
      `Codex remoteWorkspaceRoot is configured but cwd ${params.value} is outside Afora workspace root ${params.localWorkspaceRoot}; refusing to send a gateway-local cwd to the remote Codex app-server.`,
    );
  }
