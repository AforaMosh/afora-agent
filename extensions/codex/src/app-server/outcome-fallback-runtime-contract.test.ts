...
import { classifyEmbeddedAgentRunResultForModelFallback } from "afora-agent/plugin-sdk/agent-harness-runtime";
import {
...
type MirrorTaggedMessage = { __afora?: { mirrorIdentity?: string } };

async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-outcome-contract-"));
...
function readMirrorIdentity(message: unknown): string | undefined {
  const meta = (message as MirrorTaggedMessage | undefined)?.["__afora"];
...
describe("Outcome/fallback runtime contract - Codex app-server adapter", () => {
  it("preserves an empty terminal turn for Afora-owned fallback classification", async () => {
  ...
  it("preserves reasoning-only terminal turns for Afora-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
  ...
  it("preserves planning-only terminal turns for Afora-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
