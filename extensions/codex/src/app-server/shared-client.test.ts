...
const mocks = vi.hoisted(() => ({
  ...
  resolveDefaultAgentDir: vi.fn(() => "/tmp/afora-agent"),
}));
...
function configureManagedDesktopFallback(): CodexAppServerStartOptions {
  mocks.resolveManagedCodexAppServerStartOptions.mockImplementation(async (startOptions) => ({
    ...
    managedFallbackCommandPaths: ["/cache/afora/codex"],
  }));
  return {
    transport: "stdio",
    homeScope: "user",
...
describe("shared Codex app-server client", () => {
  ...
  it("closes the shared app-server when the version gate fails", async () => {
    ...
    await sendInitializeResult(harness, "afora-agent/0.117.9 (macOS; test)");
  ...
  it("skips auth-store resolution only while the same config-owned client stays warm", async () => {
    ...
    await sendInitializeResult(first, "afora-agent/0.147.0 (Linux; test)");
    ...
    await sendInitializeResult(replacement, "afora-agent/0.147.0 (Linux; test)");
  ...
  it("rejects an aborted startup acquire while another caller keeps initialization alive", async () => {
    ...
    await sendInitializeResult(harness, "afora-agent/0.147.0 (Linux; test)");
  ...
  it("reuses the successful managed fallback after desktop initialize is unsupported", async () => {
    ...
    await sendInitializeResult(desktop, "afora-agent/0.124.9 (macOS; test)");
    await sendInitializeResult(pluginLocal, "afora-agent/0.147.0 (macOS; test)");
  ...
  it("shares a managed fallback with a waiter that arrives during fallback initialize", async () => {
    ...
    await sendInitializeResult(fallback, "afora-agent/0.147.0 (macOS; test)");
  ...
  it("keeps capture clients separate from ordinary shared clients", async () => {
    await withTempDir("afora-codex-capture-client-", async (root) => {
      ...
      await sendInitializeResult(normal, "afora-agent/0.147.0 (Linux; test)");
      ...
      await sendInitializeResult(captured, "afora-agent/0.147.0 (Linux; test)");
  ...
  it("binds the managed fallback candidate that actually initialized", async () => {
    await withTempDir("afora-codex-capture-fallback-", async (root) => {
  ...
  it("detects persisted Computer Use enabled after managed client startup", async () => {
    await withTempDir("afora-codex-managed-selection-", async (root) => {
      const harness = createClientHarness();
      vi.spyOn(CodexAppServerClient, "start").mockReturnValue(harness.client);
      mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(
        async (startOptions) => ({
      ...
      await sendInitializeResult(harness, "afora-agent/0.147.0 (macOS; test)");
      ...
      expect(readCodexAppServerClientProcessIdentity(client)).toEqual({
        ...
        commandSource: "resolved-managed",
        nativeCommand: "/cache/afora/codex.native",
        serverVersion: "0.147.0",
        userAgent: "afora-agent/0.147.0 (macOS; test)",
      });
  ...
  it.each(["abort", "timeout"] as const)(
    ...
    async (mode) => {
      await withTempDir("afora-codex-guarded-request-cancel-", async (root) => {
        const harness = createClientHarness();
        vi.spyOn(CodexAppServerClient, "start").mockReturnValue(harness.client);
        mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(
          async (startOptions) => ({
  ...
  it("closes and clears a shared app-server when initialize times out", async () => {
    ...
    await sendInitializeResult(second, "afora-agent/0.147.0 (macOS; test)");
  ...
  it("separates prepared profile clients by secret-free account identity", async () => {
    ...
    await sendInitializeResult(firstHarness, "afora-agent/0.147.0 (macOS; test)");
    ...
    await sendInitializeResult(secondHarness, "afora-agent/0.147.0 (macOS; test)");
  ...
  it("registers persisted profile refresh for isolated app-server startup", async () => {
    ...
    const clientPromise = createIsolatedCodexAppServerClient({
      ...
      agentDir: "/tmp/afora-persisted-agent",
    });
    await sendInitializeResult(harness, "afora-agent/0.147.0 (macOS; test)");
  ...
  it("skips target auth resolution when native source auth is requested", async () => {
    ...
    const clientPromise = getSharedCodexAppServerClient({
      timeoutMs: 1000,
      authProfileId: null,
      agentDir: "/tmp/afora-target-agent",
      agentId: "research",
      config,
    });
    await sendInitializeResult(harness, "afora-agent/0.147.0 (macOS; test)");
  ...
  it("uses the selected agent dir for shared app-server auth bridging", async () => {
    ...
    await expect(listPromise).resolves.toEqual({ models: [] });
    const bridgeCall = bridgeStartOptionsCall();
    expect(bridgeCall?.agentDir).toBe("/tmp/afora-agent-nova");
    expect(bridgeCall?.authProfileId).toBe("openai:work");
    const applyCall = applyAuthProfileCall();
    expect(applyCall?.agentDir).toBe("/tmp/afora-agent-nova");
    expect(applyCall?.authProfileId).toBe("openai:work");
  });
  ...
  it("keeps an active shared client alive when another agent dir uses a different key", async () => {
    ...
    const firstList = listCodexAppServerModels({
      timeoutMs: 1000,
      agentDir: "/tmp/afora-agent-one",
    });
    await sendInitializeResult(first, "afora-agent/0.147.0 (macOS; test)");
    await sendEmptyModelList(first);
    await expect(firstList).resolves.toEqual({ models: [] });

    const secondList = listCodexAppServerModels({
      timeoutMs: 1000,
      agentDir: "/tmp/afora-agent-two",
    });
    await sendInitializeResult(second, "afora-agent/0.147.0 (macOS; test)");
    await sendEmptyModelList(second);
    await expect(secondList).resolves.toEqual({ models: [] });
  ...
  it("resolves the managed binary before bridging and spawning the shared client", async () => {
    ...
    const bridgeCall = bridgeStartOptionsCall();
    expect(bridgeCall?.startOptions.command).toBe("/cache/afora/codex");
    expect(bridgeCall?.startOptions.commandSource).toBe("resolved-managed");
    const startCall = clientStartCall(startSpy);
    expect(startCall?.command).toBe("/cache/afora/codex");
  ...
  it("rechecks persisted native Computer Use before managed binary resolution", async () => {
    await withTempDir("afora-codex-shared-native-", async (agentDir) => {
      ...
      await sendInitializeResult(harness, `afora/${CODEX_APP_SERVER_VERSION} (macOS; test)`);
    ...
    server.on("connection", (socket, request) => {
      ...
      socket.on("message", (data) => {
        ...
        if (message.method === "initialize") {
          socket.send(
            JSON.stringify({ id: message.id, result: { userAgent: "afora-agent/0.147.0" } }),
          );
          return;
        }
