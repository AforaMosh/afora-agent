...
import { listRegisteredPluginAgentPromptGuidance } from "afora-agent/plugin-sdk/plugin-runtime";
import {
...
export function buildDeveloperInstructions(
  ...
  for (const spec of options.dynamicTools ?? []) {
    const isDirectNamespace =
      ...
      spec.name.trim() === CODEX_AFORA_DIRECT_DYNAMIC_TOOL_NAMESPACE;
  ...
  const sections = [
    ...
    deferredToolNames.size > 0
      ? `Deferred searchable Afora dynamic tools available: ${[...deferredToolNames]
    ...
    nativeDelegationAvailable
      ? `Use Codex native \`spawn_agent\` for Codex subagents. \`spawn_agent\` and the other native collaboration tools may be deferred.${hasSessionsSpawn ? " Use Afora `sessions_spawn` only for Afora or ACP delegation, never as a substitute for `spawn_agent`." : ""}`
      : undefined,
...
function buildVisibleReplyInstruction(
  ...
  if (messageToolAvailable) {
    return "For the current source conversation, reply normally in your final assistant message; Afora will deliver it through the active source conversation. Use `message` for supported non-text actions in the current conversation, such as reacting to its current message. Reserve other `message` actions for explicit out-of-band sends or media/file delivery. Reactions are not delivered automatically.";
  }
  return "For the current source conversation, reply normally in your final assistant message; Afora will deliver it through the active source conversation.";
}
