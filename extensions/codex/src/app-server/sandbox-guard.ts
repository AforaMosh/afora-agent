/**
 * Blocks direct Codex app-server requests that would bypass Afora sandbox or
 * node-exec routing guarantees.
 */
import { tryResolveDefaultAgentId } from "afora-agent/plugin-sdk/agent-scope-runtime";
import type { AforaConfig } from "afora-agent/plugin-sdk/config-contracts";
import { parseAgentSessionKey } from "afora-agent/plugin-sdk/routing";
import { resolveSandboxRuntimeStatus, type SandboxContext } from "afora-agent/plugin-sdk/sandbox";
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-afora-environment"],
...
/** Returns a block message when a direct app-server method would bypass Afora execution policy. */
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-afora-environment" &&
    hasAforaSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasAforaSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("afora-sandbox-") &&
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because Afora sandboxing is active for this session.`,
    "This mode cannot route execution through the Afora sandbox backend.",
