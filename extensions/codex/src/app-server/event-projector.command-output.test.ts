...
describe("CodexAppServerEventProjector command output projection", () => {
  ...
  it("keeps final command output UTF-16 safe at the transcript limit", async () => {
    ...
    expect(item.content).toBe(
      `${prefix}\n...(Afora truncated Codex native tool output: original 10288 chars, showing 10000; rerun with narrower args.)`,
    );
  ...
  it("keeps streamed command output UTF-16 safe at the transcript limit", async () => {
    ...
    expect(item.content).not.toMatch(/[\uD800-\uDFFF]/);
    expect(item.content).toContain("Afora truncated Codex native tool output");
  ...
  it("keeps streaming after output text includes the truncation notice prefix", async () => {
    ...
    const userOutputWithNotice =
      "...(Afora truncated Codex native tool output is a literal line from the process)\n";
  ...
  it("does not parse user output as a prior truncation notice when streaming crosses the cap", async () => {
    ...
    const userOutputWithNotice =
      "before user marker\n...(Afora truncated Codex native tool output: original literal process text)\nsecond line must survive\n";
    ...
    expect(output).toContain("Afora truncated Codex native tool output");
  ...
  it("caps streamed command output used for replay when snapshots omit aggregated output", async () => {
    ...
    expect(output?.match(/Afora truncated Codex native tool output/g)).toHaveLength(1);
    ...
    expect(toolResultContentItem.content).toContain("Afora truncated Codex native tool output");
  });
