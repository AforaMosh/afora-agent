...
import * as mediaStore from "afora-agent/plugin-sdk/media-store";
...
describe("runCodexAppServerAttempt turn watches", () => {
  ...
  it("releases the session when Codex never completes after a dynamic tool response", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(
      /^(Unknown Afora tool: message|Action send requires a target\.)$/u,
    );
  ...
  it("preserves raw image-generation media when Codex never sends turn completion", async () => {
    const harness = createStartedThreadHarness();
    const params = makeTestParams({ timeoutMs: 200 });
    vi.stubEnv("AFORA_STATE_DIR", path.join(tempDir, "state"));
  ...
  it("keeps waiting after an Afora dynamic tool response before final synthesis", async () => {
    const harness = createStartedThreadHarness();
    const params = createParams(
  ...
  it("clears the thread binding after a completion-idle timeout so the next turn starts fresh", async () => {
    // Regression for afora#89974. Codex writes a generic <turn_aborted>
    ...
    // Turn 2: with no binding, Afora starts a brand-new thread instead of
  ...
  it("releases completion when a projector callback throws during turn/completed", async () => {
    // Regression for AforaMosh/afora-agent#67996: a throw inside the projector's
