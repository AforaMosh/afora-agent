/**
 * Bridges Codex native hook callbacks into Afora's native hook relay so
 * app-server tool events can still run Afora policy and diagnostics.
 */
...
import { registerRetainedNativeHookRelayForBundledRuntime } from "afora-agent/plugin-sdk/native-hook-relay-runtime";
import {
  addTimerTimeoutGraceMs,
  finiteSecondsToTimerSafeMilliseconds,
} from "afora-agent/plugin-sdk/number-runtime";
import type { PluginHookToolContext } from "afora-agent/plugin-sdk/types";
import type { CodexAppServerRuntimeOptions } from "./config.js";
import { resolveCodexToolAbortTerminalReason } from "./dynamic-tool-execution.js";
import { nativeHookRelayUnregisterQueue } from "./native-hook-relay-state.js";
import { isJsonObject, type JsonObject, type JsonValue } from "./protocol.js";

/** Codex hook events that can be registered through Afora's native relay. */
...
/** Registers an Afora native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian Afora plugin approval prompt.
...
function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("afora:codex:native-hook-relay:v1");
  hash.update("\0");
...
function buildCodexNativeToolMatcher(toolNames: readonly string[] | undefined): string | undefined {
  ...
  for (const toolName of toolNames) {
    ...
    if (!canonicalToolName || canonicalToolName === "*") {
      throw new TypeError("Codex native hook matcher requires canonical Afora tool ids");
    }
