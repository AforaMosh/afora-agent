...
import type { CodexSessionTranscriptMirrorWriteLockContext } from "afora-agent/plugin-sdk/codex-session-transcript-runtime";
import {
...
vi.mock("afora-agent/plugin-sdk/codex-session-transcript-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("afora-agent/plugin-sdk/codex-session-transcript-runtime")>();
  return {
    ...actual,
...
it("adopts a competing indexed user without duplicating writes or slowing assistant mirrors", async () => {
  ...
  try {
    const target = {
      ...
      storePath: path.join(root, "afora-agent.sqlite"),
    };
