  ...
  for (const [turnIndex, turn] of params.turns.entries()) {
    ...
    for (const item of turn.items) {
      ...
      if (display.hasUnverifiableInput) {
        return failure(
          "drift-mismatch",
          "A message before the fork point contains images or attachments that cannot be verified across Afora and Codex. Fork from a text-only span instead.",
        );
      }
      if (!display.visible) {
        continue;
...
export async function resolveCodexUpstreamForkBoundary(params: {
  ...
  try {
    ...
    if (thread.historyMode === "paginated") {
      return failure(
        "upstream-unavailable",
        "This Codex thread uses paginated history, which cannot be forked from Afora yet.",
      );
    }
