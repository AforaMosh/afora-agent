...
import { registerSandboxBackend } from "afora-agent/plugin-sdk/sandbox";
import { upsertSessionEntry } from "afora-agent/plugin-sdk/session-store-runtime";
import { formatSqliteSessionFileMarker } from "afora-agent/plugin-sdk/sqlite-runtime-testing";
...
/** Keeps native Codex bindings reusable while omitting Afora tools and search. */
...
describe("runCodexAppServerAttempt context-engine lifecycle", () => {
  ...
  it("bootstraps and assembles non-legacy context before the Codex turn starts", async () => {
    ...
    expectRequestInputTextContains(harness, "Afora assembled context for this turn:");
  ...
  it("projects thread-bootstrap context only once for a matching context-engine epoch", async () => {
    ...
    expectRequestInputTextContains(firstHarness, "Afora assembled context for this turn:");
  ...
  it("fails first-turn Codex context overflow instead of falling back to Afora compaction", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const compact = vi.fn<ContextEngine["compact"]>(async () => ({
      ok: true,
  ...
  it("keeps current inbound context at the front of the Codex context-engine prompt", async () => {
    ...
    params.currentInboundContext = {
      text: [
        ...
        "#6474 Sun 2026-05-10 22:22 GMT+5:30 [reply target] Afora: anchor REPLYCTX this is the old message",
        "#6498 Sun 2026-05-10 22:22 GMT+5:30 Afora: filler REPLYCTX 23",
      ].join("\n"),
    };
    ...
    expect(inputText).toContain("Afora assembled context for this turn:");
    expect(inputText).toContain("Current user request:\nhello");
    expect(inputText).toContain("[reply target] Afora: anchor REPLYCTX");
