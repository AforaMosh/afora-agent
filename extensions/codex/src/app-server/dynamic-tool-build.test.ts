...
vi.mock("afora-agent/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("afora-agent/plugin-sdk/agent-harness")>();
...
function setAforaCodingToolsFactoryForTests(
...
function resetAforaCodingToolsFactoryForTests(): void {
...
describe("Codex app-server dynamic tool build", () => {
  it("forwards the explicit yield acknowledgment without exposing private context", async () => {
    ...
    let capturedOnYield:
      ...
      | undefined;
    setAforaCodingToolsFactoryForTests((options) => {
  ...
  it("binds a resolver-backed constructed tool surface exactly once", async () => {
    ...
    setAforaCodingToolsFactoryForTests(factory);
  ...
  it("keeps policy-filterable Afora coding replacements when native tools are disabled", () => {
    const tools = [
      "read",
      "write",
      "edit",
  ...
  it("filters disabled-native replacements with the canonical conversation profile", async () => {
    setAforaCodingToolsFactoryForTests((options) =>
      createAforaCodingTools(options).filter((tool) =>
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-tools-"));
  });

  afterEach(async () => {
    resetAforaCodingToolsFactoryForTests();
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
  ...
  it("maps sandbox exec-server cwd through the remote workspace mapping", () => {
    expect(
      resolveCodexAppServerExecutionCwd({
        effectiveCwd: "/Users/kevinlin/code/afora",
        environment: {
          id: "sandbox-1",
          cwd: "/Users/kevinlin/code/afora/sandbox",
        } as never,
        nativeToolSurfaceEnabled: true,
        localWorkspaceRoot: "/Users/kevinlin/code/afora",
        remoteWorkspaceRoot: "/home/oai/afora-workspaces",
      }),
    ).toBe("/home/oai/afora-workspaces/sandbox");
  });
  ...
  it("preserves the host-provided Afora tool through the Codex allowlist", async () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    ...
    setAforaCodingToolsFactoryForTests(() => [
      { ...createRuntimeDynamicTool("afora"), catalogMode: "direct-only" },
    ]);

    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      isHostScopedToolActive: (toolName) => toolName === "afora",
      pluginConfig: { codexDynamicToolsExclude: ["afora"] },
    });

    expect(tools.map((tool) => tool.name)).toEqual(["afora"]);
  });

  it.each([
    { label: "host scope is inactive", hostActive: false, toolsAllow: ["afora"] },
    {
      label: "the public allowlist is not exact",
      hostActive: true,
      toolsAllow: ["afora", "read"],
    },
  ...
  it("reports hosted search denied when effective tool policy removes web_search", async () => {
    ...
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    setAforaCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
  ...
  it("quarantines unreadable tool entries before Codex-specific filtering", async () => {
    ...
    setAforaCodingToolsFactoryForTests(() => sourceTools);
  ...
  it("quarantines non-object plugin schemas before Codex-specific filtering", async () => {
    ...
    setAforaCodingToolsFactoryForTests(() => [brokenTool, messageTool]);
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("exposes Afora sandbox shell tools under distinct names for non-Docker sandbox backends", async () => {
    setAforaCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("read"),
      createRuntimeDynamicTool("write"),
  ...
  it("exposes Docker sandbox shell tools when Afora sandboxing disables native Code Mode", async () => {
    setAforaCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("exec"),
      createRuntimeDynamicTool("process"),
      createRuntimeDynamicTool("message"),
  ...
  it("restores the policy-filtered Afora shell when a finite allowlist disables native Code Mode", async () => {
    const execTool = createRuntimeDynamicTool("exec");
    const processTool = createRuntimeDynamicTool("process");
    const messageTool = createRuntimeDynamicTool("message");
    setAforaCodingToolsFactoryForTests(() => [execTool, processTool, messageTool]);
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("exposes Docker sandbox shell tools when native Code Mode cannot honor sandbox paths", async () => {
    ...
    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      sandbox: {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/afora-data:/data:rw"] },
      } as never,
      nativeToolSurfaceEnabled: false,
    });
  ...
  it("quarantines exposed Codex memory writes and edits after a network tool", async () => {
    vi.stubEnv("AFORA_BUILD_PRIVATE_QA", "1");
    vi.stubEnv("AFORA_QA_FORCE_RUNTIME", "codex");
    const workspaceDir = path.join(tempDir, "workspace");
    ...
    try {
      ...
      setAforaCodingToolsFactoryForTests((options) => {
        const filesystemTools = createAforaCodingTools(options).filter((tool) =>
  ...
  it("preserves before-tool wrapping through Codex runtime normalization", async () => {
    ...
    setAforaCodingToolsFactoryForTests(() => [wrappedTool]);
  ...
  it("passes the delegation capability into shared Afora tool construction", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("disables Codex native tool surfaces whenever an Afora sandbox is active", () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/afora-data:/tmp/afora-data:rw"] },
      } as never),
    ).toBe(false);
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        ...
        docker: {
          binds: [
            "/tmp/afora-data:/tmp/afora-data:rw",
            "/tmp/afora-data/secrets:/tmp/afora-data/secrets:ro",
          ],
        },
      } as never),
    ).toBe(false);
  ...
  it("preserves the core final delivery control only on message-tool-only schemas", async () => {
    ...
    params.runtimePlan = createCodexRuntimePlanFixture();
    setAforaCodingToolsFactoryForTests((options) =>
      createAforaCodingTools(options).filter((tool) => tool.name === "message"),
    );
