...
describe("runCodexAppServerAttempt", () => {
  ...
  it("uses a supervised native model for review policy despite an outer Anthropic default", async () => {
    ...
    testing.setOpenClawCodingToolsFactoryForTests(() => []);
    // This test owns effective supervision capability gating, not requester-scoped discovery.
    agentHarnessRuntimeMocks.skipConfiguredMcpMaterialization = true;
    const params = createParams(sessionFile, workspaceDir);
    ...
    expect(agentHarnessRuntimeMocks.materializeConfiguredMcpCalls).toHaveLength(1);
    expect(agentHarnessRuntimeMocks.materializeConfiguredMcpCalls[0]).toMatchObject({
      toolsEnabled: true,
      policyContext: {
