...
describe("maybeCompactCodexAppServerSession", () => {
  beforeEach(async () => {
    resetCodexTestBindingStore();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-compact-"));
  });
  ...
  it("blocks native app-server compaction for configured and remote-exec sandboxes", async () => {
    ...
    for (const result of [
      ...
      expect(result.reason).toContain(
        "Codex-native native compaction is unavailable because Afora sandboxing is active for this session.",
      );
    }
    expect(fake.request).not.toHaveBeenCalled();
  });
  ...
  it("blocks native app-server compaction when exec host=node is active", async () => {
    ...
    expect(result.compacted).toBe(false);
    expect(result.reason).toContain(
      "Codex-native native compaction is unavailable because Afora exec host=node is active for this session.",
    );
    expect(fake.request).not.toHaveBeenCalled();
  });
  ...
  it("warns when stale Afora compaction overrides are ignored", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "ignoring Afora compaction overrides for Codex app-server compaction; Codex uses native server-side compaction",
      {
        sessionId: "session-1",
        sessionKey: "agent:main:session-1",
        ignoredConfig: [
