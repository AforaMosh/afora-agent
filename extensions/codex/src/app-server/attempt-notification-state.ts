...
import {
  ...
  isPendingAforaDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingAforaDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingAforaDynamicToolCompletionNotification(
    notification,
    params.pendingAforaDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after Afora hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served Afora
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingAforaDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }
