...
export class CodexAppServerClient {
  ...
  /** Registers a handler for app-server requests sent back to Afora. */
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "Afora did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
...
class CodexAppServerVersionError extends Error {
  ...
  constructor(detectedVersion: string | undefined) {
    const detected = detectedVersion
      ...
      : "Afora could not determine the running Codex version";
    super(
...
function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // Afora, Codex Desktop, or an env override, so only the slash-delimited
...
/** Returns true for app-server approval request methods Afora can answer. */
