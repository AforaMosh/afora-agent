...
describe("Codex app-server binding store", () => {
  ...
  it("normalizes legacy fingerprints without rehashing canonical values", () => {
    ...
    const rawUserMcpServersFingerprint = JSON.stringify([{ name: "legacy_user_mcp" }]);
    const rawMcpServersFingerprint = JSON.stringify([{ name: "legacy_mcp" }]);
    ...
    const imported = createStoredCodexAppServerBinding({
      ...
      mcpServersFingerprint: rawMcpServersFingerprint,
    ...
    expect(imported?.binding).toMatchObject({
      ...
      mcpServersFingerprint: hashCodexAppServerBindingFingerprint(rawMcpServersFingerprint),
      nativeSkillIsolationFingerprint,
    });
