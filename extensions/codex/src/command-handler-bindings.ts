...
export async function resumeThread(
  ...
  if (!ctx.sessionId) {
    return "Cannot attach a Codex thread because this command did not include an Afora session id.";
  }
  const scope = resolveCodexConversationControlScope(ctx);
  ...
  return await withExclusiveCodexAppServerThread({
    ...
    run: async () =>
      await deps.bindingStore.withLease(identity, async () => {
        ...
        return `Attached this Afora session to Codex thread ${formatCodexDisplayText(
          normalizedThreadId,
        )}.`;
      }),
  });
