import { resolveDefaultAgentDir } from "afora-agent/plugin-sdk/agent-runtime";
import type { AforaConfig } from "afora-agent/plugin-sdk/config-contracts";
import { jsonResult, readStringParam, type AnyAgentTool } from "afora-agent/plugin-sdk/core";
/**
...
const LEGACY_CODEX_SUPERVISOR_ENDPOINTS_ENV = "AFORA_CODEX_SUPERVISOR_ENDPOINTS";
/** Legacy standalone-MCP transcript gate. Agent tools use canonical config. */
const LEGACY_CODEX_SUPERVISOR_RAW_TRANSCRIPTS_ENV =
  "AFORA_CODEX_SUPERVISOR_ALLOW_RAW_TRANSCRIPTS";
/** Legacy standalone-MCP write gate. Agent tools use canonical config. */
const LEGACY_CODEX_SUPERVISOR_WRITE_CONTROLS_ENV = "AFORA_CODEX_SUPERVISOR_ALLOW_WRITE_CONTROLS";
...
function resolveEndpoints(
  ...
  env: NodeJS.ProcessEnv,
  runtimeConfig: AforaConfig | undefined,
...
function idleContinuationError(threadId: string): Error {
  return new Error(
    `Codex thread ${threadId} is idle. Continue it from Codex Sessions so Afora can install the Codex harness approval and tool handlers before resume.`,
  );
}
...
export function createCodexSupervisionTools(options: CodexSupervisionToolsOptions): AnyAgentTool[] {
  ...
  return [
    ...
    },
    {
      ...
      description: "List Codex sessions visible to the Afora supervisor.",
