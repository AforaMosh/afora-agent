...
import { withFileLock, type FileLockOptions } from "afora-agent/plugin-sdk/file-lock";
...
async function collectSessionSurfaces(params: MigrationEnvironment): Promise<SessionSurface[]> {
  ...
  const { resolveStorePath } = await import("afora-agent/plugin-sdk/session-store-runtime");
...
async function recordSessionOwner(
  ...
  const { patchSessionEntry } = await import("afora-agent/plugin-sdk/session-store-runtime");
