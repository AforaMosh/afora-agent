...
import type { MigrationItem, MigrationProviderContext } from "afora-agent/plugin-sdk/plugin-entry";
import {
...
function replaceConfigDraft(draft: AforaConfig, next: AforaConfig): void {
  for (const key of Object.keys(draft) as Array<keyof AforaConfig>) {
    delete draft[key];
  }
...
function existingAuthProfileConfigIsCompatible(
  existing: NonNullable<NonNullable<AforaConfig["auth"]>["profiles"]>[string],
...
function hasCurrentAuthProfileConfigConflict(
  ...
  try {
    config =
      (resolveMigrationConfigRuntime(ctx)?.current?.() as AforaConfig | undefined) ?? config;
  } catch {
...
function applyDefaultModelIfMissing(cfg: AforaConfig): AforaConfig {
...
function applyOAuthConfigToConfig(
  ...
  let next = mergeMigrationConfigValue(cfg, credential.result.configPatch) as AforaConfig;
  const profile = credential.result.profiles[0];
  if (profile) {
...
async function applyCodexAuthProfileConfig(
  ctx: MigrationProviderContext,
  profile: CodexAuthProfileConfig,
  applyConfig: (config: AforaConfig) => AforaConfig,
