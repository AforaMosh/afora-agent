...
import { channelBlockedPatch } from "afora-agent/plugin-sdk/gateway-runtime";
import { createLazyRuntimeModule } from "afora-agent/plugin-sdk/lazy-runtime";
import { resolveAgentRoute, type RoutePeer } from "afora-agent/plugin-sdk/routing";
import {
...
function isTelegramRichMessagesEnabled(cfg: AforaConfig, accountId?: string | null): boolean {
...
const telegramMessageAdapter = createChannelMessageAdapterFromOutbound<AforaConfig>({
  id: "telegram",
  live: {
    capabilities: {
      draftPreview: true,
...
function resolveTelegramNativeTopicThreadId(
  ...
  // Keep the chat-scoped canonical id inside Afora state; translate it back
  ...
  base: {
      ...
      inboundFormattingHints: ({ cfg, accountId }) => {
        ...
        if (richMessages) {
          return {
            ...
            rules: [
              "Telegram rich ON (Bot API 10.2 blocks; Afora maps markdown + these HTML islands to typed blocks).",
    ...
    gateway: {
      ...
      logoutAccount: async ({ accountId, cfg }) => {
        ...
        const nextCfg = { ...cfg } as AforaConfig;
