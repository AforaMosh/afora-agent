...
import { withEnv } from "afora-agent/plugin-sdk/test-env";
...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", AFORA_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): AforaConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): AforaConfig => ({
    channels: {
      telegram: {
