...
import { escapeRegExp, formatEnvelopeTimestamp } from "afora-agent/plugin-sdk/channel-test-helpers";
import type { TelegramGroupConfig } from "afora-agent/plugin-sdk/config-contracts";
import {
...
import type { GetReplyOptions, MsgContext } from "afora-agent/plugin-sdk/reply-runtime";
import { withEnvAsync } from "afora-agent/plugin-sdk/test-env";
import { createRequireRecord, sanitizeTerminalText } from "afora-agent/plugin-sdk/test-fixtures";
...
vi.mock("afora-agent/plugin-sdk/conversation-runtime", { spy: true });

const harness = await import("./bot.create-telegram-bot.test-harness.js");
const pluginStateTestRuntime = await import("afora-agent/plugin-sdk/plugin-state-test-runtime");
const configMutation = await import("afora-agent/plugin-sdk/config-mutation");
const modelSessionRuntime = await import("afora-agent/plugin-sdk/model-session-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "afora-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  beforeEach(async () => {
    ...
    process.env.AFORA_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
    clearPluginInteractiveHandlers();
  ...
  it.each(["stop", "/stop@afora_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "Afora Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
  ...
  it("routes plugin callback_query payloads to plugin handlers without fallback callback_data text", async () => {
    ...
    expect(
      registerPluginInteractiveHandler("afora-code-agent", {
        channel: "telegram",
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("afora pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Afora", username: "afora_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "Afora", username: "afora_bot" },
          text: "Binding: Review pull request 54118 (afora)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "Afora", username: "afora_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "Afora Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Afora", username: "afora_bot" },
  ...
  const groupPolicyCases: MessagePolicyCase[] = [
    ...
    makeMessagePolicyCase({
      ...
      message: { from: { id: 999999, username: "notallowed" }, text: "@afora_bot hello" },
      expectedReplyCount: 0,
    }),
    makeMessagePolicyCase({
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "afora_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("marks explicit Telegram bot-handle mentions in the inbound context", async () => {
    ...
    await dispatchMessage({
      message: {
        chat: { id: 7, type: "group", title: "Test Group" },
        text: "@afora_bot status",
        entities: [{ type: "mention", offset: 0, length: "@afora_bot".length }],
        date: 1736380800,
        message_id: 4,
        from: { id: 9, first_name: "Ada" },
      },
    ...
    expect(payload.BotUsername).toBe("afora_bot");
  });
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/afora-telegram-group-activation.json";
