...
const { clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
  await import("afora-agent/plugin-sdk/runtime-config-snapshot");

let defaultSessionStoreRoot = "";

beforeEach(async () => {
  defaultSessionStoreRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "afora-telegram-message-context-"),
  );
...
describe("buildTelegramMessageContext group sessions without forum", () => {
  ...
  it("does not add a topic-cache store lookup for non-forum group reply threads", async () => {
    const resolveStorePath = vi.fn(() => "/tmp/afora/session-store.json");
  ...
  it("reloads topic name from disk after cache reset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-telegram-topic-name-"));
