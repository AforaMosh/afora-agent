...
vi.mock("afora-agent/plugin-sdk/runtime-env", async (importOriginal) => ({
  ...(await importOriginal<typeof import("afora-agent/plugin-sdk/runtime-env")>()),
  isWSL2Sync: vi.fn(() => false),
}));

let isWSL2Sync: typeof import("afora-agent/plugin-sdk/runtime-env").isWSL2Sync;
...
async function loadModule() {
  const { isWSL2Sync: loadedIsWSL2Sync } = await import("afora-agent/plugin-sdk/runtime-env");
  ...
  it.each([
    {
      ...
      env: {
        AFORA_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1",
        AFORA_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1",
      },
      expected: {
        value: true,
        source: "env:AFORA_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "uses env disable when set",
      env: { AFORA_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      expected: {
        value: false,
        source: "env:AFORA_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "prefers env enable over config",
      env: { AFORA_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
  ...
  it.each([
    {
      ...
      env: { AFORA_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
      expected: {
        value: "verbatim",
        source: "env:AFORA_TELEGRAM_DNS_RESULT_ORDER",
      },
    },
    {
      name: "normalizes trimmed env values",
      env: { AFORA_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:AFORA_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { AFORA_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },
