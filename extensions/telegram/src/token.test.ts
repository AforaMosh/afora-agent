...
describe("resolveTelegramToken", () => {
  ...
  function createUnknownAccountConfig(): AforaConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: AforaConfig) {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as AforaConfig;
  ...
  it("applies account file, account config, channel file, channel config, then env precedence", () => {
    ...
    const resolve = (telegram: Record<string, unknown>) =>
      resolveTelegramToken({ channels: { telegram } } as AforaConfig);
  ...
  // Regression: https://github.com/AforaMosh/afora-agent/issues/53876
