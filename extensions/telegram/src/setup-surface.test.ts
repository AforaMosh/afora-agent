...
import { DEFAULT_ACCOUNT_ID } from "afora-agent/plugin-sdk/setup";
...
describe("telegram DM access warning helpers", () => {
  it("shows global config commands for the default account", () => {
    ...
    expect(lines.join("\n")).toContain(
      'afora config set channels.telegram.dmPolicy "allowlist"',
    );
    expect(lines.join("\n")).toContain(
      `afora config set channels.telegram.allowFrom '["YOUR_USER_ID"]'`,
    );
  });
  ...
  it("shows account-scoped config commands for named accounts", () => {
    ...
    expect(lines.join("\n")).toContain(
      'afora config set channels.telegram.accounts.alerts.dmPolicy "allowlist"',
    );
    expect(lines.join("\n")).toContain(
      `afora config set channels.telegram.accounts.alerts.allowFrom '["YOUR_USER_ID"]'`,
    );
  });
