...
import { createRequireRecord, importFreshModule } from "afora-agent/plugin-sdk/test-fixtures";
...
const { getChildLogger, resetLogger, setLoggerOverride } =
  await import("afora-agent/plugin-sdk/runtime-env");
const {
  buildInlineKeyboard,
...
function captureInfoLogs(): string {
  logCaptureCounter += 1;
  const logFile = `/tmp/afora-telegram-send-log-${process.pid}-${logCaptureCounter}.jsonl`;
...
describe("sent-message-cache", () => {
  ...
  it("keeps sent-message ownership across restart", async () => {
    const persistedStorePath = `/tmp/afora-telegram-send-tests-${process.pid}-restart.json`;
  ...
  it("keeps expired custom-store cleanup away from the default store", () => {
    const customStorePath = `/tmp/afora-telegram-send-tests-${process.pid}-custom-cleanup.json`;
  ...
  it("keeps default and custom stores isolated while both are loaded", () => {
    const customStorePath = `/tmp/afora-telegram-send-tests-${process.pid}-custom-isolated.json`;
    const customCfg = { session: { store: customStorePath } };
  ...
  it("keeps sent-message ownership isolated across differently routed accounts", () => {
    const multiAgentCfg = {
      ...
      session: {
        store: "/tmp/afora-telegram-sent-owner/{agentId}/sessions.json",
      },
    } as AforaConfig;
...
describe("sendMessageTelegram", () => {
  ...
  it("records sent text messages into the Telegram prompt context cache", async () => {
    const storePath = `/tmp/afora-telegram-send-context-${process.pid}-${Date.now()}.json`;
  ...
  it("records a successful General-topic send when the response omits the thread id", async () => {
    const storePath = `/tmp/afora-telegram-general-context-${process.pid}-${Date.now()}.json`;
    const chatId = "-1003966283270";
    botApi.sendMessage.mockResolvedValueOnce({
      message_id: 1498,
      date: 1_779_394_741,
      chat: { id: chatId, type: "supergroup", title: "QA forum" },
      from: { id: 42, is_bot: true, first_name: "Afora" },
  ...
  it("records transcript projection metadata without replacing Telegram time", async () => {
    const storePath = `/tmp/afora-telegram-send-context-override-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
  ...
  it("invalidates the projection cursor when a later chunk fails terminally", async () => {
    const storePath = `/tmp/afora-telegram-projection-terminal-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
    const cursor = createTelegramPromptContextProjectionCursor({
      transcriptMessageId: "assistant-final",
    });
  ...
  it("records transcript projection metadata for native locations", async () => {
    const storePath = `/tmp/afora-telegram-location-context-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it("continues rich plain-fallback chunks after a middle rejection", async () => {
    ...
    const storePath = `/tmp/afora-telegram-projection-rich-fallback-${process.pid}-${Date.now()}.json`;
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it.each([
    {
      name: "local path",
      markdown:
        "See [scripts/yougile.py](/home/user/.afora/workspace/scripts/yougile.py#L41) and [docs](https://example.com/docs)",
    },
    {
      name: "relative path",
      markdown: "Edit [config](./afora.json) or see [docs](https://example.com/docs)",
    },
  ...
  it("indexes every successful text chunk and marks only the last one final", async () => {
    const storePath = `/tmp/afora-telegram-projection-chunks-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it("records each HTML chunk using that message's visible text", async () => {
    const storePath = `/tmp/afora-telegram-projection-html-chunks-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it("does not consume a projection part for a rejected HTML attempt", async () => {
    const storePath = `/tmp/afora-telegram-projection-html-fallback-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it("continues after a rejected middle chunk and reports incomplete delivery", async () => {
    const storePath = `/tmp/afora-telegram-projection-partial-${process.pid}-${Date.now()}.json`;
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it("finalizes the visible chunk when Telegram rejects an invisible tail", async () => {
    const storePath = `/tmp/afora-telegram-empty-tail-${process.pid}-${Date.now()}.json`;
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it("reports partial delivery when the final keyboard retrofit fails", async () => {
    const storePath = `/tmp/afora-telegram-empty-tail-keyboard-failure-${process.pid}-${Date.now()}.json`;
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it("finalizes media when its separate text renders empty", async () => {
    const storePath = `/tmp/afora-telegram-empty-media-follow-up-${process.pid}-${Date.now()}.json`;
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it("chunks long default markdown media follow-up text", async () => {
    ...
    const storePath = `/tmp/afora-telegram-projection-media-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
    const cursor = createTelegramPromptContextProjectionCursor({
  ...
  it.each([false, true])(
    ...
    async (richMessages) => {
      const chatId = "-1001234567890";
      const storePath = `/tmp/afora-telegram-voice-fallback-${process.pid}-${richMessages}.json`;
      const onDeliveryResult = vi.fn();
      const cursor = createTelegramPromptContextProjectionCursor({
...
describe("sendStickerTelegram", () => {
  ...
  it("records a successful topic sticker for later message mutations", async () => {
    const storePath = `/tmp/afora-telegram-sticker-context-${process.pid}-${Date.now()}.json`;
    const chatId = "-100123";
    const sendSticker = vi.fn().mockResolvedValue({
      message_id: 107,
      chat: { id: chatId, type: "supergroup" },
...
describe("shared send behaviors", () => {
  ...
  it("retries media native quotes with a legacy reply before recording projection", async () => {
    const chatId = "123";
    const storePath = `/tmp/afora-telegram-media-quote-${process.pid}-${Date.now()}.json`;
    const cursor = createTelegramPromptContextProjectionCursor({
...
describe("editMessageTelegram", () => {
  ...
  it.each([
    ...
    const storePath = `/tmp/afora-telegram-edited-context-${process.pid}-${Date.now()}-${testCase.name}.json`;
    const cfg = { session: { store: storePath } };
    const chat = {
      id: -100123,
      type: "supergroup" as const,
  ...
  it("refreshes edited group messages without duplicating self history or hiding later replies", async () => {
    const storePath = `/tmp/afora-telegram-edit-history-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
...
describe("sendPollTelegram", () => {
  ...
  it("records a successful General-topic poll for later message mutations", async () => {
    const storePath = `/tmp/afora-telegram-poll-context-${process.pid}-${Date.now()}.json`;
    const chatId = "-100123";
