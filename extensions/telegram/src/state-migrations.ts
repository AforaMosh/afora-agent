...
import { listAgentIds } from "afora-agent/plugin-sdk/agent-scope-runtime";
...
import { resolveStorePath } from "afora-agent/plugin-sdk/session-store-paths";
...
function resolveTelegramLegacyStateOwnerAgentId(cfg: AforaConfig): string {
...
function listTelegramLegacyMessageCacheEntries(persistedPath: string) {
  ...
  for (const value of readLegacyMessageCacheValues(raw)) {
    ...
    const { afora_prompt_context_projection: _projection, ...canonicalSourceMessage } =
      sourceMessage as PersistedTelegramMessageCacheValue["sourceMessage"] & {
        afora_prompt_context_projection?: unknown;
      };
