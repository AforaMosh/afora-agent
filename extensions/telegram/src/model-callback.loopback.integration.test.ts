// Real grammY and Bot API sockets prove long model callbacks survive the full loop.
...
describe("Telegram model callback loopback", () => {
  ...
  it("sends, authorizes, resolves, persists, answers, and edits an opaque callback", async () => {
    const stateDir = tempDirs.make("openclaw-telegram-model-loopback-");
    ...
    try {
      ...
      await within(
        "sendMessage",
        sendMessageTelegram(String(CHAT_ID), "Select a model:", {
          cfg: config,
          token: TOKEN,
          buttons,
        }),
      ...
      } as unknown as TelegramHandlerAuthorizationRuntime;
      ...
      } as unknown as TelegramHandlerMessageRuntime;
      registerTelegramCallbackQueryHandler(
        {
          accountId: "default",
          bot,
          runtime: {},
      ...
      expect(callbackSteps).toEqual(["context", "sender", "model", "model", "catalog"]);
