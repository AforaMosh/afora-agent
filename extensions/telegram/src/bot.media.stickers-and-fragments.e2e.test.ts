...
describe("telegram local Bot API media", () => {
  it("reads a container-local file from its trusted host volume mount", async () => {
    ...
    const tempRoot = await realpath(await mkdtemp(path.join(os.tmpdir(), "afora-tg-local-")));
      ...
      try {
        await handler({
          message: {
            ...
            date: 1736380800,
            text: `@afora_bot topic-one ${"A".repeat(4050)}`,
          },
          me: { username: "afora_bot" },
          getFile: async () => ({}),
        });
        ...
        await handler({
          message: {
            ...
            is_topic_message: true,
            date: 1736380801,
            text: `@afora_bot topic-two ${"B".repeat(4050)}`,
          },
          me: { username: "afora_bot" },
          getFile: async () => ({}),
        });
