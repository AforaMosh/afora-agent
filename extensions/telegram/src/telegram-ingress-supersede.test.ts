...
let aforaState: AforaTestState | undefined;

afterEach(async () => {
  closeAforaStateDatabaseForTest();
  await aforaState?.cleanup();
  aforaState = undefined;
});
...
function cfgWithOwner(ownerId = OWNER_ID): AforaConfig {
  return {
    channels: {
      telegram: {
        allowFrom: [ownerId],
...
describe("telegram ingress supersede policy", () => {
  ...
  it("gates command supersede on authorized sender", async () => {
    // /new is a recognized text alias in Afora command set.
