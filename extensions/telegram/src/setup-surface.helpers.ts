...
export function shouldShowTelegramDmAccessWarning(cfg: AforaConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`afora config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`afora config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
