...
- Polling and webhook both: enqueue then pump
  ...
  loops.
- Stall timeout: `AFORA_TELEGRAM_SPOOLED_HANDLER_TIMEOUT_MS` →
...
- Do not reintroduce `sendMessageDraft` for answer streaming. Telegram drafts
  ...
  requires a separate `sendMessage`. Afora uses `sendMessage` plus
...
- Reply context comes from Afora-observed messages. Bot API updates expose
