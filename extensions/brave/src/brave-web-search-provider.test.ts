...
const braveManifest = JSON.parse(
  fs.readFileSync(new URL("../afora.plugin.json", import.meta.url), "utf-8"),
) as {
...
describe("brave web search provider", () => {
  ...
  it("points missing-key users to fetch/browser alternatives", async () => {
    ...
    expect(result).toEqual({
      ...
      message:
        "web_search (brave) needs a Brave Search API key. Run `afora configure --section web` to store it, or set BRAVE_API_KEY in the Gateway environment. If you do not want to configure a search API key, use web_fetch for a specific URL or the browser tool for interactive pages.",
