...
import type { SearchConfigRecord } from "afora-agent/plugin-sdk/provider-web-search";
import {
  buildSearchCacheKey,
...
function missingBraveKeyPayload() {
  return {
    ...
    message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("afora configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment. If you do not want to configure a search API key, use web_fetch for a specific URL or the browser tool for interactive pages.`,
