...
import { createSubsystemLogger, isTruthyEnvValue } from "afora-agent/plugin-sdk/runtime-env";
...
const EAGER_BROWSER_CONTROL_SERVICE_ENV = "AFORA_EAGER_BROWSER_CONTROL_SERVER";
...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage Afora's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
...
function createBrowserToolOptions(ctx: AforaPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
function createBrowserProxyNodeHostCommand(command: string): AforaPluginNodeHostCommand {
  return {
    command,
    cap: "browser",
    isAvailable: ({ config }) =>
...
export const browserPluginNodeHostCommands: AforaPluginNodeHostCommand[] = [
...
export const browserSecurityAuditCollectors: AforaPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): AforaPluginService {
  let service: AforaPluginService | null = null;
...
export function registerBrowserPlugin(api: AforaPluginApi) {
  ...
  api.registerTool(((ctx: AforaPluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as AforaPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
  ...
  api.registerHttpRoute({
    ...
    handler: (_req: IncomingMessage, res: ServerResponse) => {
      ...
      res.end("Upgrade Required: connect the Afora Chrome extension over WebSocket.");
    },
