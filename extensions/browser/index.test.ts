...
type BrowserAutoEnableProbe = Parameters<AforaPluginApi["registerAutoEnableProbe"]>[0];
...
describe("browser plugin", () => {
  ...
  it("bundles the browser automation skill with the plugin", () => {
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, "afora.plugin.json"), "utf8"),
    ) as { skills?: string[] };
  ...
  it("registers a lazy browser control service", async () => {
    ...
    await service.start({ config: {}, stateDir: "/tmp/afora", logger: { warn: vi.fn() } });
    expect(runtimeApiMocks.createBrowserPluginService).not.toHaveBeenCalled();

    await service.stop({ config: {}, stateDir: "/tmp/afora", logger: { warn: vi.fn() } });
  ...
  it("eager-loads the browser control service when explicitly requested", async () => {
    vi.stubEnv("AFORA_EAGER_BROWSER_CONTROL_SERVER", "1");
    const { api, registerService } = createApi();
    registerBrowserPlugin(api);
  ...
  for (const value of ["false", "", "disabled"]) {
    it(`keeps browser control service env value ${JSON.stringify(value)} lazy`, async () => {
      vi.stubEnv("AFORA_EAGER_BROWSER_CONTROL_SERVER", value);
      const { api, registerService } = createApi();
      registerBrowserPlugin(api);
  ...
  it("declares setup auto-enable reasons for browser config surfaces", () => {
    ...
    expect(probe({ config: { browser: { defaultProfile: "afora" } }, env: {} })).toBe(
      "browser configured",
    );
    ...
    expect(
      probe({ config: { browser: { defaultProfile: "afora", enabled: false } }, env: {} }),
    ).toBeNull();
  });
});
