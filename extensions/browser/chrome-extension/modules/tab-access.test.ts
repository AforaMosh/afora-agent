...
describe("tab access policy", () => {
  it("allows ungrouped eligible tabs in all mode and only grouped tabs in selected mode", async () => {
    ...
    await expect(harness.policy.requireTab(1)).rejects.toThrow("not in the Afora tab group");
  ...
  it("invalidates captured authority across mode and per-tab deny changes", async () => {
    ...
    await expect(harness.policy.requireTab(1)).rejects.toThrow("paused for Afora");
  });
  ...
  it("restores existing-tab session denies across worker instances and prunes closed ids", async () => {
    ...
    await expect(harness.policy.requireTab(2)).rejects.toThrow("paused for Afora");
    ...
    await expect(reloaded.requireTab(1)).rejects.toThrow("paused for Afora");
