#!/usr/bin/env node
/**
 * Copies the unpacked Afora Chrome extension into the browser plugin dist so
 * `afora browser extension path` resolves a stable location for
...
const srcDir = process.env.AFORA_CHROME_EXT_SRC_DIR ?? path.join(pluginDir, "chrome-extension");
const outDir =
  process.env.AFORA_CHROME_EXT_OUT_DIR ??
...
async function main() {
  if (!(await pathExists(srcDir))) {
    if (
      process.env.AFORA_SPARSE_PROFILE ||
      process.env.AFORA_CHROME_EXT_SKIP_MISSING === "1"
    ) {
      return;
    }
