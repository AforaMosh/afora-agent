...
export function buildBrowserDoctorReport(params: {
  ...
  checks.push({
    ...
    label: "Profile",
    status: "pass",
    summary: `${status.profile ?? "afora"} via ${transport}`,
  });
  ...
  if (transport === "chrome-mcp") {
    ...
    checks.push({
      ...
      status: status.running ? "pass" : "fail",
      summary: status.running
        ? "Afora Chrome extension is connected"
        : "Afora Chrome extension is not connected",
      ...(status.running
        ? {}
        : {
            fixHint:
              "Install the Afora Chrome extension (afora browser extension path), run afora browser extension pair, and paste the pairing string into the extension popup.",
          }),
    });
  } else {
    checks.push({
    ...
    if (missingDisplay) {
      checks.push({
        ...
        fixHint:
          "Use a desktop session, Xvfb, set AFORA_BROWSER_HEADLESS=1, or remove the headed override.",
      });
    }
    ...
    checks.push({
      ...
      ...(status.cdpHttp || !status.running
        ? {}
        : {
            fixHint: "Run afora browser start or inspect browser.cdpUrl/CDP port reachability.",
          }),
    });
