...
function setupEnsureBrowserAvailableHarness() {
  ...
  const launchAforaChrome = vi.mocked(chromeModule.launchAforaChrome);
  const stopAforaChrome = vi.mocked(chromeModule.stopAforaChrome);
  ...
  const profile = ctx.forProfile("afora");
...
function requireFirstLaunchOptions(launchAforaChrome: {
  mock: { calls: unknown[][] };
}): unknown {
  const [call] = launchAforaChrome.mock.calls;
  if (!call) {
...
describe("browser server-context ensureBrowserAvailable", () => {
  it("rejects and cleans a deferred launch before stop returns, then allows restart", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady, profile, state } =
    ...
    expect(launchAforaChrome).toHaveBeenCalledTimes(1);
    ...
    expect(stopAforaChrome).toHaveBeenCalledTimes(1);
    expect(stopAforaChrome).toHaveBeenCalledWith(late);
    expect(state.profiles.get("afora")?.running).toBeNull();
    const runtime = state.profiles.get("afora");
    ...
    expect(state.profiles.get("afora")?.running).toBe(replacement);
  });
  ...
  it("does not count canceled managed starts toward the launch cooldown", async () => {
    ...
    const runtime = state.profiles.get("afora");
    if (!runtime) {
      throw new Error("expected afora runtime");
    }
    ...
    const launchEntered = deferred<void>();
    const deferredLaunch = deferred<RunningChrome>();
    launchAforaChrome.mockImplementationOnce(async () => {
      launchEntered.resolve();
      return await deferredLaunch.promise;
    });
    ...
    launchAforaChrome.mockResolvedValueOnce(replacement);
    await expect(profile.ensureBrowserAvailable()).resolves.toBeUndefined();

    expect(launchAforaChrome).toHaveBeenCalledTimes(2);
    expect(stopAforaChrome).toHaveBeenCalledTimes(1);
    expect(state.profiles.get("afora")?.running).toBe(replacement);
    expect(state.profiles.get("afora")?.managedLaunchFailure).toBeUndefined();
  });

  it("waits for CDP readiness after launching to avoid follow-up PortInUseError races (#21149)", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValueOnce(false).mockResolvedValue(true);
    mockLaunchedChrome(launchAforaChrome, 123);
    ...
    expect(stopAforaChrome).not.toHaveBeenCalled();
  });
  ...
  it("stops launched chrome when CDP readiness never arrives", async () => {
    ...
    mockLaunchedChrome(launchAforaChrome, 321);
  ...
  it("rejects a foreign listener that wins the managed CDP port after spawn", async () => {
    ...
    launchAforaChrome.mockResolvedValue(launched);
    isChromeCdpReady.mockResolvedValue(true);
    ...
    expect(stopAforaChrome).toHaveBeenCalledExactlyOnceWith(launched);
    expect(state.profiles.get("afora")?.running).toBeNull();
  });
  ...
  it("uses configured local CDP readiness timeout after launching", async () => {
    ...
    isChromeCdpReady.mockResolvedValue(false);
    mockLaunchedChrome(launchAforaChrome, 322);
  ...
  it("deduplicates concurrent lazy-start calls to prevent PortInUseError", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchAforaChrome, 456);
  ...
  it("deduplicates concurrent lazy-start calls across fresh profile contexts", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchAforaChrome, 457);

    const firstCtx = createBrowserRouteContext({ getState: () => state });
    const secondCtx = createBrowserRouteContext({ getState: () => state });
    const first = firstCtx.forProfile("afora").ensureBrowserAvailable();
    const second = secondCtx.forProfile("afora").ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(Promise.all([first, second])).resolves.toEqual([undefined, undefined]);
  ...
  it("passes request-local headless override to initial launch", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchAforaChrome, 654);
    ...
    expect(launchAforaChrome).toHaveBeenCalledTimes(1);
    expect(requireFirstLaunchOptions(launchAforaChrome)).toEqual(
  ...
  it("passes request-local headless override to the owned restart path", async () => {
    ...
    mockLaunchedChrome(launchAforaChrome, 987);
  ...
  it("does not share inflight lazy-start promises across different headless overrides", async () => {
    const { launchAforaChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("clears the concurrent lazy-start guard after launch failure", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchAforaChrome.mockRejectedValueOnce(
    ...
    mockLaunchedChrome(launchAforaChrome, 789);
  ...
  it("cools down repeated managed Chrome launch failures across route contexts", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchAforaChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));

    for (let attempt = 0; attempt < 3; attempt += 1) {
      const ctx = createBrowserRouteContext({ getState: () => state });
      await expect(ctx.forProfile("afora").ensureBrowserAvailable()).rejects.toThrow(
        "Failed to start Chrome CDP",
      );
    }

    const cooledDownCtx = createBrowserRouteContext({ getState: () => state });
    await expect(cooledDownCtx.forProfile("afora").ensureBrowserAvailable()).rejects.toThrow(
      'Browser launch for profile "afora" is cooling down after 3 consecutive managed Chrome launch failures.',
    );
    await expect(cooledDownCtx.forProfile("afora").ensureBrowserAvailable()).rejects.toThrow(
      "set browser.enabled=false if the browser tool is not needed",
    );

    expect(launchAforaChrome).toHaveBeenCalledTimes(3);
    expect(stopAforaChrome).not.toHaveBeenCalled();
  });
  ...
  it("does not let no-display preflight failures block explicit headless recovery", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchAforaChrome.mockRejectedValue(
    ...
    const recovery = recoveryCtx.forProfile("afora").ensureBrowserAvailable({ headless: true });
    await vi.advanceTimersByTimeAsync(100);
    await expect(recovery).resolves.toBeUndefined();

    expect(launchAforaChrome).toHaveBeenCalledTimes(4);
    expect(launchAforaChrome.mock.calls.at(-1)?.[2]).toEqual(
      expect.objectContaining({ headlessOverride: true, signal: expect.any(AbortSignal) }),
    );
    expect(state.profiles.get("afora")?.managedLaunchFailure).toBeUndefined();
    expect(stopAforaChrome).not.toHaveBeenCalled();
  ...
  it("allows one managed Chrome launch attempt after the cooldown expires", async () => {
    const { launchAforaChrome, isChromeCdpReady, state } = setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchAforaChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));
    ...
    await expect(retryCtx.forProfile("afora").ensureBrowserAvailable()).rejects.toThrow(
      "Failed to start Chrome CDP",
    );
  ...
  it("reuses a pre-existing loopback browser after an initial short probe miss", async () => {
    ...
    expect(launchAforaChrome).not.toHaveBeenCalled();
    expect(stopAforaChrome).not.toHaveBeenCalled();
  });
  ...
  it("explains attachOnly for externally managed loopback CDP services", async () => {
    ...
    await expect(promise).rejects.toThrow(
      'Port 18800 is in use for profile "afora" but not by afora.',
    );
    await expect(promise).rejects.toThrow(
      "set browser.profiles.afora.attachOnly=true so Afora attaches without trying to manage the local process",
    );
    await expect(promise).rejects.toThrow(
      "For Browserless Docker, set EXTERNAL to the same WebSocket endpoint Afora can reach via browser.profiles.<name>.cdpUrl.",
    );
  ...
  it("retries remote CDP websocket reachability once before failing", async () => {
    const { launchAforaChrome, stopAforaChrome, isChromeCdpReady } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);

    const state = makeBrowserServerState();
    state.resolved.profiles.afora = {
  ...
  it("treats attachOnly loopback CDP as local control with remote-class probe timeouts", async () => {
    const { launchAforaChrome, stopAforaChrome } = setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
    const isChromeCdpReady = vi.mocked(chromeModule.isChromeCdpReady);
