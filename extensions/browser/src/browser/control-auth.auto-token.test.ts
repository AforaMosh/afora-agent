...
const mocks = vi.hoisted(() => ({
  ...
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: AforaConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<AforaConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: AforaConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): AforaConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: AforaConfig) {
