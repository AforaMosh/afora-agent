...
async function runBrowserLiveProbe(profileCtx: ProfileContext, signal: AbortSignal) {
  ...
  try {
    ...
    return {
      ...
      summary: String(err),
      fixHint: "Run afora browser start, then retry with afora browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    params.profileCtx.profile.driver !== "afora" ||
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched afora profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post("/profiles/create", async (req, res) => {
    ...
    if (driver && driver !== "afora" && driver !== "clawd" && driver !== "existing-session") {
      return jsonError(
        res,
        400,
        `unsupported profile driver "${driver}"; use "afora", "clawd", or "existing-session"`,
      );
    }
