...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.AFORA_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "afora",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/afora" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    afora: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/afora.json" } });
...
vi.mock("./chrome.js", () => ({
  ...
  launchAforaChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveAforaUserDataDir: vi.fn(() => chromeUserDataDir.dir),
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.AFORA_GATEWAY_PORT;
  process.env.AFORA_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.AFORA_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.AFORA_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.AFORA_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.AFORA_GATEWAY_PASSWORD;
  } else {
    process.env.AFORA_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
