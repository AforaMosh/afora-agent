...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAforaTmpDir: vi.fn(() => "/tmp/afora"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-afora-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-afora-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredAforaTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredAforaTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAforaTmpDir.mockReturnValue("/tmp/afora");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredAforaTmpDir.mockReturnValue("/tmp/afora-preferred");
    ...
    const expectedRootedDownloadsDir = await fs.realpath(
      path.resolve(path.join(path.sep, "tmp", "afora-preferred", "downloads")),
    );
    const expectedDownloadsTail = `${path.join("tmp", "afora-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAforaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "afora-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredAforaTmpDir.mockReturnValue(tempDir);
