...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const opened = await afora.openTab("about:blank");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    await afora.focusTab("T2");
    await afora.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(afora.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(afora.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(afora.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
