...
describe("browser remote profile fallback and attachOnly behavior", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    const state = deps.makeState("afora");
    ...
    const launchMock = vi.mocked(deps.chromeModule.launchAforaChrome);
  ...
  it("does not enforce managed tab cap for remote afora profiles", async () => {
  ...
  it("uses remote-class tab-open timeouts for attachOnly loopback CDP profiles", async () => {
    ...
    const opened = await ctx.forProfile("afora").openTab("https://example.com");
  ...
  it("keeps managed loopback tab opens on local CDP defaults", async () => {
    ...
    await ctx.forProfile("afora").openTab("http://127.0.0.1:3000");
