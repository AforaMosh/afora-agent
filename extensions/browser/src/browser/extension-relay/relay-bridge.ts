 ...
 * on one side and the Afora Chrome extension's chrome.debugger transport on
...
const BROWSER_TARGET_ID = "afora-extension-relay";
/** Playwright requires every attached page target to identify its browser context. */
const BROWSER_CONTEXT_ID = "afora-extension-context";
...
export class ExtensionRelayBridge {
  ...
  private sendToExtension(msg: RelayToExtensionMessage): void {
    if (!this.extension) {
      throw new Error("Afora Chrome extension is not connected to the relay");
    }
  ...
  private async ensureTabAttached(tabId: number): Promise<{ targetId: string; sessionId: string }> {
    ...
    if (!tab) {
      throw new Error(`tab ${tabId} is not available to Afora`);
    }
    if (tab.attached) {
      return tab.attached;
    }
    ...
    const attaching = (async () => {
      ...
      const sessionId = `afora-tab-${tabId}-${this.nextSessionOrdinal++}`;
   ...
   * "Afora is debugging this browser" infobar only spans active automation.
   */
    ...
    switch (request.method) {
      case "Browser.getVersion": {
        ...
        this.respond(client, request, {
          ...
          revision: "afora-extension-relay",
      ...
      case "Target.attachToBrowserTarget": {
        const sessionId = `afora-browser-${this.nextSessionOrdinal++}`;
      ...
      case "Target.attachToTarget": {
        ...
        if (request.sessionId && this.browserSessions.get(request.sessionId) === client) {
          ...
          const sessionId = `afora-tab-${found.tabId}-${this.nextSessionOrdinal++}`;
      ...
      case "Target.createBrowserContext": {
        this.respondError(
          client,
          request,
          "The Afora extension relay drives the user's real browser profile; isolated browser contexts are not supported.",
        );
        return;
      }
      default: {
