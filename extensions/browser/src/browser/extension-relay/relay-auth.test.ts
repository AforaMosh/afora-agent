...
let stateDir = "";
const prevStateDir = process.env.AFORA_STATE_DIR;

beforeEach(() => {
  stateDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "afora-relay-auth-")));
...
describe("extension relay host-local secret", () => {
  ...
  it("gives different hosts (state dirs) different secrets", async () => {
    ...
    const otherDir = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "afora-relay-auth-2-")),
    );
    try {
      const b = await ensureExtensionRelayToken({ ...process.env, AFORA_STATE_DIR: otherDir });
      expect(b).not.toBe(a);
    } finally {
