...
import { readMemoryHostEventRecords } from "afora-agent/plugin-sdk/memory-host-events";
import {
...
describe("memory-core doctor dreaming migration", () => {
  ...
  beforeEach(async () => {
    resetPluginStateStoreForTests();
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-memory-core-doctor-"));
    workspaceDir = path.join(rootDir, "workspace");
    await fs.mkdir(path.join(workspaceDir, "memory", ".dreams"), { recursive: true });
    env = { ...process.env, AFORA_STATE_DIR: path.join(rootDir, "state") };
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const externalMemoryDir = await fs.mkdtemp(
        path.join(os.tmpdir(), "afora-memory-core-external-events-"),
      );
  ...
  it("uses migration env when resolving default workspaces", async () => {
    env = { ...env, AFORA_WORKSPACE_DIR: workspaceDir };
    const recallPath = path.join(workspaceDir, "memory", ".dreams", "short-term-recall.json");
    await fs.writeFile(
      recallPath,
      JSON.stringify({
  ...
  it("does not infer agent ownership from configured sidecar filenames", async () => {
    ...
    const mainAgentPath = path.join(stateDir, "agents", "main", "agent", "afora-agent.sqlite");
  ...
  it("copies shared retired configured legacy sidecars to each configured agent", async () => {
    ...
    const mainAgentPath = path.join(stateDir, "agents", "main", "agent", "afora-agent.sqlite");
    const workAgentPath = path.join(stateDir, "agents", "work", "agent", "afora-agent.sqlite");
    await writeLegacyMemorySidecar(legacyPath);
    const config = {
      memory: {
        search: {
  ...
  it("copies custom vector sidecars to the canonical retry path when sqlite-vec cannot load", async () => {
    ...
    const result = await legacyMemoryIndexMigration().migrateLegacyState(migrationParams(config));
    const repairedConfig: AforaConfig = {
      agents: {
        list: [{ id: "main", workspace: workspaceDir }],
      },
    };
  ...
  it("removes only exact stale QMD lock sidecars and is idempotent", async () => {
    ...
    await expect(fs.access(path.join(stateDir, "afora.sqlite"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    await expect(
      fs.access(path.join(stateDir, "agents", "main", "agent", "afora-agent.sqlite")),
