...
import type { PluginCommandContext } from "afora-agent/plugin-sdk/core";
...
function resolveStoredDreaming(config: AforaConfig): Record<string, unknown> {
...
function createHarness(initialConfig: AforaConfig = {}) {
  let runtimeConfig: AforaConfig = initialConfig;
  ...
  const runtime = {
    config: {
      ...
      writeConfigFile: vi.fn(async (nextConfig: AforaConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
