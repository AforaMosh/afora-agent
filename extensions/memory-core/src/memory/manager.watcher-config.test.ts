...
const {
  ...
  const chokidarKey = Symbol.for("afora.test.memoryWatchFactory");
  const nativeKey = Symbol.for("afora.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("afora.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("afora.test.memoryNativeWatchFactory");
const originalWatcherStateDir = process.env.AFORA_STATE_DIR;
...
function restoreWatcherStateDir(): void {
  if (originalWatcherStateDir === undefined) {
    ...
    Reflect.set(process.env, "AFORA_STATE_DIR", originalWatcherStateDir);
  }
}

vi.mock("afora-agent/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("afora-agent/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): AforaConfig {
    const defaults: NonNullable<NonNullable<AforaConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: AforaConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. Afora watches directories only so
