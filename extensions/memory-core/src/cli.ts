    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["afora memory status", "Show index and provider status."],
          [
            "afora memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["afora memory status --deep", "Probe embedding provider readiness."],
          ["afora memory index --force", "Force a full reindex."],
          ['afora memory search "meeting notes"', "Quick search using positional query."],
          [
            'afora memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `afora memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
            "Review weighted short-term candidates for long-term memory.",
          ],
          [
            "afora memory promote --apply",
            "Append top-ranked short-term candidates into MEMORY.md.",
          ],
          [
            'afora memory promote-explain "router vlan"',
            "Explain why a specific candidate would or would not promote.",
          ],
          [
            "afora memory rem-harness --json",
            "Preview REM reflections, candidate truths, and deep promotion output.",
          ],
          [
            "afora memory rem-backfill --path ./memory",
          ...
          [
            "afora memory session-backfill --agent main --from 2026-01-01",
            "Preview trusted candidates from retained session history.",
          ],
          ["afora memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.afora.ai/cli/memory")}\n`,
    );
