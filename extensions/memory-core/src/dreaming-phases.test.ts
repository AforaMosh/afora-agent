...
import { RequestScopedSubagentRuntimeError } from "afora-agent/plugin-sdk/error-runtime";
import {
  resolveMemoryDreamingPluginConfig,
  resolveSessionTranscriptsDirForAgent,
} from "afora-agent/plugin-sdk/memory-core-host-runtime-core";
import { clearRuntimeConfigSnapshot } from "afora-agent/plugin-sdk/runtime-config-snapshot";
...
const LIGHT_SLEEP_EVENT_TEXT = "__afora_memory_core_light_sleep__";
const REM_SLEEP_EVENT_TEXT = "__afora_memory_core_rem_sleep__";
const originalDreamingTestFast = process.env.AFORA_TEST_FAST;
const originalDreamingStateDir = process.env.AFORA_STATE_DIR;
const LIGHT_DREAMING_TEST_CONFIG: AforaConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
function setDreamingTestEnv(stateDir: string): void {
  Reflect.set(process.env, "AFORA_TEST_FAST", "1");
...
function restoreDreamingTestEnv(): void {
  if (originalDreamingTestFast === undefined) {
    Reflect.deleteProperty(process.env, "AFORA_TEST_FAST");
  } else {
    Reflect.set(process.env, "AFORA_TEST_FAST", originalDreamingTestFast);
  }
  if (originalDreamingStateDir === undefined) {
    Reflect.deleteProperty(process.env, "AFORA_STATE_DIR");
  } else {
    Reflect.set(process.env, "AFORA_STATE_DIR", originalDreamingStateDir);
  }
  clearRuntimeConfigSnapshot();
}
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("afora-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__afora_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __afora_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__afora_memory_core_light_sleep__",
            "",
  ...
  it("promotes one recurring bullet across three contextualized day files", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__afora_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
  ...
  it("increments dailyCount when the same daily file is re-ingested on a later day", async () => {
    ...
    const configForTest: AforaConfig = {
      plugins: {
        entries: {
          "memory-core": {
            config: {
