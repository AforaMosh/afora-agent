...
function createPolicyContext(
  overrides: Partial<AforaPluginNodeInvokePolicyContext> = {},
): AforaPluginNodeInvokePolicyContext {
  return {
    nodeId: "node-1",
    command: "file.fetch",
    ...
    client: null,
    invokeNode: vi.fn<AforaPluginNodeInvokePolicyContext["invokeNode"]>(async () => ({
      ok: true,
      payload: { ok: true },
      payloadJSON: null,
    })),
...
describe("lazy file-transfer node invoke policy", () => {
  it("exposes command metadata without loading the delegate", () => {
    const loadPolicy = vi.fn<() => Promise<AforaPluginNodeInvokePolicy>>();
  ...
  it("loads and caches the delegate on first handle", async () => {
    const invokeNode = vi.fn<AforaPluginNodeInvokePolicyContext["invokeNode"]>(async () => ({
      ok: true,
      payload: { ok: true },
      payloadJSON: null,
    }));
    const delegateHandle = vi.fn<AforaPluginNodeInvokePolicy["handle"]>(async (ctx) => {
      await ctx.invokeNode();
      return { ok: true, payload: { delegated: true } };
    });
    const loadPolicy = vi.fn<() => Promise<AforaPluginNodeInvokePolicy>>(async () => ({
      commands: ["file.fetch"],
