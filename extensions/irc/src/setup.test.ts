...
describe("irc setup", () => {
  ...
  it("validates required input and applies normalized account config", () => {
    ...
    expect(
      validateInput({
        input: { host: "irc.libera.chat", nick: "afora" },
      } as never),
    ).toBeNull();

    expect(
      validateInput({
        input: { host: "irc.libera.chat", nick: "afora", port: "+07000" },
      } as never),
    ).toBeNull();

    expect(
      validateInput({
        input: { host: "irc.libera.chat", nick: "afora", port: "7000x" },
      } as never),
    ).toBe("IRC port must be between 1 and 65535.");

    expect(
      validateInput({
        input: { host: "irc.libera.chat", nick: "afora", port: "70000" },
      } as never),
    ).toBe("IRC port must be between 1 and 65535.");
  ...
  it("configures host and nick via setup prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("afora-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#afora", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#afora", "#ops"]);
  });
