...
import { collectProviderApiKeys, getShellEnvAppliedKeys } from "afora-agent/plugin-sdk/test-live-auth";
import {
...
} from "afora-agent/plugin-sdk/test-media-generation";
import type {
  GeneratedVideoAsset,
  VideoGenerationMode,
...
const REQUIRE_PROFILE_KEYS =
  isLiveProfileKeyModeEnabled() || isTruthyEnvValue(process.env.AFORA_LIVE_REQUIRE_PROFILE_KEYS);
const describeLive = LIVE ? describe : describe.skip;
const providerFilter = parseCsvFilter(process.env.AFORA_LIVE_VIDEO_GENERATION_PROVIDERS);
const defaultSkippedProviders = providerFilter
  ? null
  : parseCsvFilter(process.env.AFORA_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS ?? "fal");
const envModelMap = parseProviderModelMap(process.env.AFORA_LIVE_VIDEO_GENERATION_MODELS);
const RUN_FULL_VIDEO_MODES = isTruthyEnvValue(
  process.env.AFORA_LIVE_VIDEO_GENERATION_FULL_MODES,
);
const LIVE_VIDEO_REQUESTED_DURATION_SECONDS = 1;
const LIVE_VIDEO_OPERATION_TIMEOUT_MS = readPositiveIntegerEnv(
  process.env.AFORA_LIVE_VIDEO_GENERATION_TIMEOUT_MS,
  180_000,
);
...
function withPluginsEnabled(cfg: AforaConfig): AforaConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
