 ...
 * The AFORA_TWITCH_ACCESS_TOKEN env var is only used for the default account.
 *
 * Token resolution priority:
 * 1. Account access token from merged config (accounts.{id} or base-level for default)
 * 2. Environment variable: AFORA_TWITCH_ACCESS_TOKEN (default account only)
 */
...
export function resolveTwitchToken(
  ...
  const envToken = allowEnv
    ? normalizeTwitchToken(opts.envToken ?? process.env.AFORA_TWITCH_ACCESS_TOKEN)
    : undefined;
  if (envToken) {
