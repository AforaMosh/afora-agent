...
import { getChatChannelMeta, type ChannelPlugin } from "afora-agent/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: AforaConfig, requestedAccountId?: string): string {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.AFORA_TWITCH_ACCESS_TOKEN?.trim();
