...
describe("diffs plugin language-pack discovery", () => {
  it.each(["assets", "dist/assets"])(
    ...
    (assetDir) => {
      ...
      const root = fs.mkdtempSync(join(os.tmpdir(), "afora-diffs-language-pack-"));
      try {
        ...
        fs.writeFileSync(
          join(languagePackRoot, "afora.plugin.json"),
          '{"id":"diffs-language-pack"}\n',
        );
        const config = { plugins: {} } as AforaConfig;
        ...
        } satisfies AforaPluginToolContext;
