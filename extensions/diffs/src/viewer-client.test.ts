...
function renderCard(payloadOverride?: string): void {
  ...
  document.body.insertAdjacentHTML(
    ...
    `<section class="oc-diff-card">
      <div data-afora-diff-host></div>
      <script type="application/json" data-afora-diff-payload>${payload}</script>
    </section>`,
  );
}
...
describe("hydrateViewer", () => {
  beforeEach(() => {
    ...
    delete document.documentElement.dataset.aforaDiffsError;
    delete document.documentElement.dataset.aforaDiffsReady;
    vi.clearAllMocks();
  });
  ...
  it("continues hydrating later cards when one card throws", async () => {
    ...
    expect(document.documentElement.dataset.aforaDiffsError).toBeUndefined();
    warn.mockRestore();
  });
