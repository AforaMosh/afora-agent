...
import type { AforaPluginApi, AforaPluginToolContext } from "../api.js";
...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ...
    } = await createDiffStoreHarness("afora-diffs-tool-"));
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/afora/plugins/diffs/view/",
    );
...
function createApi(pluginConfig?: Record<string, unknown>): AforaPluginApi {
