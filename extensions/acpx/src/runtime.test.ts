...
import { AFORA_CODEX_CONFIG_ARG } from "./codex-adapter.js";
import {
  AFORA_ACPX_LEASE_ID_ARG,
  AFORA_GATEWAY_INSTANCE_ID_ARG,
...
const DOCUMENTED_AFORA_BRIDGE_COMMAND =
  "env AFORA_HIDE_BANNER=1 AFORA_SUPPRESS_NOTES=1 afora acp --url ws://127.0.0.1:18789 --token-file ~/.afora/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @agentclientprotocol/codex-acp@1.1.2";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/afora/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-close ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "afora" ? "afora acp" : agentName),
        list: () => ["codex", "afora"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("adds the Afora session key to both managed tools MCP bridges", () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    const { runtime } = makeRuntime(baseStore, {
      ...
      aforaToolsMcpBridgeEnabled: true,
      mcpServers: [
        {
          name: "afora-plugin-tools",
          command: "node",
        ...
        },
        {
          name: "afora-tools",
          command: "node",
          args: ["dist/mcp/afora-tools-serve.js"],
          env: [],
        },
      ],
    });
    ...
    expect(readScopedMcpEnv("agent:worker:main", "afora-plugin-tools")).toContainEqual({
      name: "AFORA_TOOLS_MCP_AGENT_SESSION_KEY",
      value: "agent:worker:main",
    });
    expect(readScopedMcpEnv("agent:research:main", "afora-tools")).toContainEqual({
      name: "AFORA_TOOLS_MCP_AGENT_SESSION_KEY",
      value: "agent:research:main",
    });
  });

  it("keeps managed Afora tools MCP delegates reachable for fresh sessions", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("uses the no-MCP delegate for startup probes when the Afora tools bridge is enabled", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("leases generated-wrapper probes before delegate entry and retains absent wrappers", async () => {
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        aforaGatewayInstanceId: "gateway-test",
        aforaProcessLeaseStore: leaseStore.store,
        aforaWrapperRoot: "/tmp/afora/acpx",
        agentRegistry: {
    ...
    expect(launchedCommand).toContain(AFORA_ACPX_LEASE_ID_ARG);
    expect(launchedCommand).toContain(`${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`);
  ...
  it("coalesces repeated probe uncertainty before it can evict a live lease", async () => {
    ...
    leaseStore.leases.set("lease-live", {
      ...
      wrapperRoot: "/tmp/afora/acpx",
      wrapperPath: "/tmp/afora/acpx/codex-acp-wrapper.mjs",
      rootPid: 700,
  ...
  it("leases generated-wrapper doctor probes and keeps uncertain failures open", async () => {
    ...
    vi.spyOn(delegate, "doctor").mockImplementation(async () => {
      ...
      expect(command).toContain(AFORA_ACPX_LEASE_ID_ARG);
  ...
  it("normalizes Afora Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it.each([
    {
      name: "strips the Afora Anthropic provider prefix for Claude ACP startup",
  ...
  it.each([
    ...
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-acpx-runtime-"));
    const leaseStore = makeLeaseStore();
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        aforaLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("disables delegate prompt timeout for Afora-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(
      ...
      `npx @agentclientprotocol/codex-acp@1.1.2 ${AFORA_CODEX_CONFIG_ARG} '{"model":"gpt-5.4","model_reasoning_effort":"medium"}'`,
    );
    expect(testing.isCodexAcpCommand("afora acp")).toBe(false);
  ...
  it.each([
    {
      name: "normalizes Afora-qualified Codex ACP model controls",
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/afora/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/afora/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("afora acp")).toBe(false);
  ...
  it("releases managed Afora tools MCP delegates after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("cleans up Afora-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/afora/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 900,
              ppid: 1,
              command: 'node "/tmp/afora/acpx/codex-acp-wrapper.mjs"',
            },
            {
              pid: 901,
              ppid: 900,
              command:
                "node /tmp/afora/plugin-runtime-deps/node_modules/@agentclientprotocol/codex-acp/dist/index.js",
            },
          ]),
          killProcess: vi.fn((pid, signal) => {
            killed.push({ pid, signal });
  ...
  it("persists ACPX process lease identity for later wrapper reconnects", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/afora/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain(AFORA_ACPX_LEASE_ID_ARG);
    expect(launchCommands[0]).toContain(AFORA_GATEWAY_INSTANCE_ID_ARG);
    expect(savedRecords[0]?.agentCommand).toBe(launchCommands[0]);
    expect(savedRecords[0]?.aforaGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.aforaLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("keeps reusable persistent ACP launch commands stable across ensures", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-existing ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
  ...
  it("recreates a missing sidecar with the persisted lease identity", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-missing ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  it("does not reuse commands leased by another gateway instance", async () => {
    const foreignCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-foreign ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-foreign`;
    let savedRecord: Record<string, unknown> = {
      name: "agent:codex:acp:binding:test",
      acpxRecordId: "record-1",
      acpSessionId: "session-1",
    ...
    expect(resolvedCommands[0]).toContain(`${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`);
    expect(savedRecord.pid).toBe(888);
    expect(leaseStore.leases.size).toBe(1);
  });

  it("rejects reconnect operations for commands leased by another gateway", async () => {
    const foreignCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-foreign-operation ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-foreign`;
    const handle = {
      sessionKey: "agent:codex:acp:binding:test",
  ...
  it("adopts legacy persistent commands before their next reconnect", async () => {
    ...
    expect(savedRecord.agentCommand).toContain(AFORA_ACPX_LEASE_ID_ARG);
    expect(savedRecord.agentCommand).toContain(AFORA_GATEWAY_INSTANCE_ID_ARG);
    ...
    expect(lease?.leaseId).toBe(savedRecord.aforaLeaseId);
  ...
  it("restores a pending process lease before runTurn reconnects", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-turn-reconnect ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("restores a missing sidecar from the persisted lease PID", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-live-reconnect ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("retires runTurn pending leases when handle resolution fails", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-turn-resolution ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    let loads = 0;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => {
        loads += 1;
  ...
  it("restores a pending process lease before startTurn reconnects", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-start-reconnect ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("loads one wrapper snapshot per handle operation before mutation", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-control-reconnect ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("preserves a promoted PID when the session record save fails", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-partial-save ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  it("keeps a shared pending lease until the last concurrent operation finishes", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-concurrent-operations ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("retires an old lease after the session record switches identity", async () => {
    const oldCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-old-operation ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const newCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-new-session ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    let savedRecord: Record<string, unknown> = {
      name: "agent:codex:acp:binding:test",
  ...
  it("keeps launch ownership while a concurrent reconnect operation is active", async () => {
    ...
    const leaseId = String(savedRecord?.aforaLeaseId);
  ...
  it("serializes last-owner retirement with the next lease acquisition", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-retirement-race ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("rechecks a reusable sidecar after the prior owner retires it", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-reusable-race ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const sessionKey = "agent:codex:acp:binding:test";
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: sessionKey,
  ...
  it("keeps close pending leases when cleanup fails", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-close-failure ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("preserves PID-bearing close leases when cleanup fails", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-close-live ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it.each([
    ...
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-close-process-list ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    expect(loadedRecord?.aforaGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.aforaLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.aforaGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.aforaLeaseId).toBe("lease-current");
  });
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-old ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} lease-current ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 920,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${AFORA_ACPX_LEASE_ID_ARG} other-lease ${AFORA_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ]),
          killProcess: vi.fn((pid, signal) => {
            killed.push({ pid, signal });
  ...
  it("routes afora ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:afora:acp:test",
      backend: "acpx",
  ...
  it("routes non-afora sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for afora sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:afora:acp:test",
      backend: "acpx",
      runtimeSessionName: "afora-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the afora agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "afora" ? "codex" : agentName),
        list: () => ["codex", "afora"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the afora bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "afora acp" : agentName),
        list: () => ["codex", "afora"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped afora bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "afora" ? DOCUMENTED_AFORA_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "afora"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node afora entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "afora" ? "env AFORA_HIDE_BANNER=1 node afora.mjs acp" : agentName,
        list: () => ["codex", "afora"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:afora:acp:test",
        agentCommand: DOCUMENTED_AFORA_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:afora:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:afora:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to afora bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
