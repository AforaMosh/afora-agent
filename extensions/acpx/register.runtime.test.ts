...
vi.mock("afora-agent/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.AFORA_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.AFORA_SKIP_ACPX_RUNTIME;
  } else {
    process.env.AFORA_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/afora-acpx-register-test",
    stateDir: "/tmp/afora-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
