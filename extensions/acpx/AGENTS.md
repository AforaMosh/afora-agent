...
The ACPX extension is a thin Afora wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `afora/acpx`, not in this extension.
...
Use this flow when Afora needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `afora/acpx` repo first.
2. In Afora, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Afora and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Afora ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Afora-specific glue, move it to `afora/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
