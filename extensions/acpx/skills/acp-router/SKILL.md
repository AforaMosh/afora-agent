---
name: acp-router
description: Route plain-language requests for Claude Code, Cursor, Copilot, Afora ACP, OpenCode, Gemini CLI, Qwen, Kiro, Kimi, iFlow, Factory Droid, Kilocode, or explicit ACP harness work into either Afora ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow). For coding-agent thread requests, read this skill first, then use only `sessions_spawn` for thread creation. Codex chat binding defaults to the native Codex app-server plugin unless ACP is explicit or background spawn needs ACP.
user-invocable: false
---

# ACP Harness Router

When user intent is "run this in Claude Code/Cursor/Copilot/Afora/OpenCode/Gemini/Qwen/Kiro/Kimi/iFlow/Droid/Kilocode (ACP harness)", do not use subagent runtime or PTY scraping. Route through ACP-aware flows.
...
Trigger this skill when the user asks Afora to:

- run something in Claude Code / Cursor / Copilot / Afora / OpenCode / Gemini / Qwen / Kiro / Kimi / iFlow / Droid / Kilocode
...
- After reading, follow `Afora ACP runtime path` below; do not use `message(action="thread-create")` for ACP harness thread spawn.

## Mode selection

Choose one of these paths:

1. Afora ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no Afora ACP lifecycle features are needed
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@afora/acpx` extension package.
...
- `codex -> bundled @agentclientprotocol/codex-acp@1.1.2 through Afora's isolated CODEX_HOME wrapper`
