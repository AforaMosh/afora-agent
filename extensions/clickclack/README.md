# ClickClack Afora channel

Official Afora channel plugin for ClickClack.

## Install

```sh
afora plugins install @afora/clickclack
```
...
`/api/bot-setup-codes/claim#CODE` endpoint. Afora validates the versioned
claim response and saves its canonical API base.

For manual token setup:

```sh
afora channels add clickclack \
...
Run `afora onboard` for guided setup. The workspace value can be a
...
authentication gateway such as Cloudflare Access while the local Afora
...
extension publishes Afora's native commands for composer autocomplete,
...
ClickClack can create one managed channel for each Afora session:
...
Afora Control UI, preserving base paths. Main sessions use `/chat/<agent>`.
...
two Afora gateways using the same ClickClack workspace do not adopt each
...
See `docs/channels/clickclack.md` in the Afora repository, or the published docs at `https://docs.afora.ai/channels/clickclack`.
