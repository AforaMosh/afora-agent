...
import { keepHttpServerTaskAlive, waitUntilAbort } from "afora-agent/plugin-sdk/channel-outbound";
...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. Afora passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error("Raft requires Afora channel runtime support. Update Afora and retry.");
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. Afora has no
      ...
      // Raft owns wake metadata and its schema evolution. Afora accepts only
