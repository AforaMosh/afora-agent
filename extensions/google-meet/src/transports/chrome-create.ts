...
const CREATE_MEET_FROM_BROWSER_SCRIPT = `async () => {
  ...
  if (!current().startsWith("https://meet.google.com/")) {
    return {
      manualAction: manualActionFor("google-login-required", "Sign in to Google in the Afora browser profile, then retry meeting creation."),
      browserUrl: current(),
  ...
  if (/do you want people to hear you in the meeting/i.test(pageText)) {
    return {
      manualAction: manualActionFor("meet-audio-choice-required", "Meet is showing the microphone choice. Click Use microphone in the Afora browser profile, then retry meeting creation."),
      browserUrl: href,
      browserTitle: document.title,
      notes,
    };
  ...
  if (/allow.*(microphone|camera)|blocked.*(microphone|camera)|permission.*(microphone|camera)/i.test(pageText)) {
    return {
      manualAction: manualActionFor("meet-permission-required", "Allow microphone/camera permissions for Meet in the Afora browser profile, then retry meeting creation."),
      browserUrl: href,
      browserTitle: document.title,
      notes,
    };
  ...
  if (/couldn't create|unable to create/i.test(pageText)) {
    return {
      manualAction: manualActionFor("browser-control-unavailable", "Resolve the Google Meet page prompt in the Afora browser profile, then retry meeting creation."),
      browserUrl: href,
      browserTitle: document.title,
      notes,
    };
