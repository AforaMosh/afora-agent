...
import type { RealtimeTranscriptionProviderPlugin } from "afora-agent/plugin-sdk/realtime-transcription";
import type {
  RealtimeVoiceBridge,
...
async function startTestNodeRealtimeAudioBridge(params: TestNodeRealtimeEngineParams) {
  ...
  Reflect.set(transport, Symbol.for("afora.internal.meeting-node-output-generation.v1"), true);
  return {
...
const GOOGLE_MEET_ENV_KEYS = [
  "AFORA_GOOGLE_MEET_CLIENT_ID",
  "GOOGLE_MEET_CLIENT_ID",
  "AFORA_GOOGLE_MEET_CLIENT_SECRET",
  "GOOGLE_MEET_CLIENT_SECRET",
  "AFORA_GOOGLE_MEET_REFRESH_TOKEN",
  "GOOGLE_MEET_REFRESH_TOKEN",
  "AFORA_GOOGLE_MEET_ACCESS_TOKEN",
  "GOOGLE_MEET_ACCESS_TOKEN",
  "AFORA_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT",
  "GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT",
  "AFORA_GOOGLE_MEET_DEFAULT_MEETING",
  "GOOGLE_MEET_DEFAULT_MEETING",
  "AFORA_GOOGLE_MEET_PREVIEW_ACK",
...
function createMockSessionRuntime(sessionStore: Record<string, unknown>) {
  const sessionRoot = createIsolatedTestDir("afora-google-meet-session-");
  return {
...
describe("google-meet plugin", () => {
  ...
  it("defaults to chrome agent mode with safe read-only tools", () => {
    ...
    expect(config.realtime.instructions).toContain("afora_agent_consult");
  ...
  it("keeps advanced config metadata manifest-owned", () => {
    const manifest = JSON.parse(
      readFileSync(new URL("./afora.plugin.json", import.meta.url), "utf8"),
    ) as {
  ...
  it("uses env fallbacks for OAuth, preview, and default meeting values", () => {
    const config = resolveGoogleMeetConfigFromTestEnv({
      AFORA_GOOGLE_MEET_CLIENT_ID: "client-id",
      GOOGLE_MEET_CLIENT_SECRET: "client-secret",
      AFORA_GOOGLE_MEET_REFRESH_TOKEN: "refresh-token",
      GOOGLE_MEET_ACCESS_TOKEN: "access-token",
      AFORA_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: "123456",
      GOOGLE_MEET_DEFAULT_MEETING: MEET_URL,
      AFORA_GOOGLE_MEET_PREVIEW_ACK: "true",
    });
  ...
  it.each(["0x10", "1e3"])("ignores non-decimal env numeric fallbacks: %s", (expiresAt) => {
    const config = resolveGoogleMeetConfigFromTestEnv({
      AFORA_GOOGLE_MEET_ACCESS_TOKEN: "access-token",
      AFORA_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: expiresAt,
    });
  ...
  it("uses a provider-safe flat tool parameter schema", () => {
    ...
    expect(properties.mode).toEqual({
      ...
      description:
        "Join mode. agent uses realtime transcription, the configured Afora agent, and regular TTS. bidi uses the realtime voice model directly. transcribe joins observe-only.",
    });
  });
  ...
  it("reports setup status through the tool", async () => {
    await withPlatform("darwin", async () => {
      const { tools } = setup({
        chrome: {
          audioInputCommand: ["afora-audio-bridge", "capture"],
          audioOutputCommand: ["afora-audio-bridge", "play"],
        },
      });
      const tool = getMeetTool({ tools });
  ...
  it("writes export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "afora-google-meet-tool-export-"));
    const { tools } = setup();
    const tool = getMeetTool({ tools });
  ...
  it("dry-runs export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const parentDir = mkdtempSync(path.join(tmpdir(), "afora-google-meet-tool-dry-run-"));
          ...
          if (request.path === "/act") {
            return {
              result: JSON.stringify({
                ...
                manualAction: {
                  reason: "meet-admission-required",
                  message: "Admit the Afora browser participant in Google Meet.",
                },
                title: "Meet",
                url: MEET_URL,
              }),
  ...
  it("retries caption enable until the captions button is available", async () => {
    ...
    const captionsStateKey = "__aforaMeetCaptions";
  ...
  it("coalesces progressive captions and ignores end phrases while still in call", async () => {
    ...
    await inspect();
    const state = windowState["__aforaMeetCaptions"] as {
      epoch: string;
      lines: Array<{ text: string }>;
    ...
    delete windowState["__aforaMeetCaptions"];
    await inspect();
    const reloadedState = windowState["__aforaMeetCaptions"] as { epoch: string };
    ...
    const nextState = windowState["__aforaMeetCaptions"] as {
      droppedLines: number;
      epoch: string;
      sessionId?: string;
      lines: Array<{ text: string }>;
    ...
    const { tools } = setup(
      ...
      },
      {
        browserActResult: {
          ...
          manualAction: {
            reason: "google-login-required",
            message: "Sign in to Google in the Afora browser profile, then retry the Meet join.",
          },
  ...
  it("explains when chrome-node has no capable paired node", async () => {
    ...
    expect(result.details.error).toContain("afora node run");
  });
  ...
  it("pipes Chrome command-pair audio through the realtime provider", async () => {
    ...
    expect(progressPayload.tool).toBe("afora_agent_consult");
    ...
    expect(callbacks.tools?.map((tool) => tool.name)).toContain("afora_agent_consult");
    await vi.waitFor(() => {
  ...
  it("pipes paired-node command-pair audio through the realtime provider", async () => {
    ...
    const fullConfig = { models: { providers: {} } } as never;
    const sessionStore: Record<string, unknown> = {};
    const testRoot = createIsolatedTestDir("afora-google-meet-node-agent-");
    const runtime = {
      nodes: {
