...
type GatewayHandler = Parameters<AforaPluginApi["registerGatewayMethod"]>[1];
type CliRegistrar = Parameters<AforaPluginApi["registerCli"]>[0];
type ToolFactory = Parameters<AforaPluginApi["registerTool"]>[0];
...
describe("google-meet lazy imports", () => {
  afterEach(() => {
    for (const moduleId of [
      ...
      "./src/cli.js",
      "afora-agent/plugin-sdk/gateway-runtime",
      "afora-agent/plugin-sdk/param-readers",
      "afora-agent/plugin-sdk/routing",
      "afora-agent/plugin-sdk/transcripts",
    ]) {
  ...
  it("loads each runtime owner only on first use", async () => {
    ...
    vi.doMock("afora-agent/plugin-sdk/gateway-runtime", () => {
      gatewayRuntimeImports += 1;
      return {
        callGatewayFromCli: async () => ({ ok: true }),
      };
    });
    vi.doMock("afora-agent/plugin-sdk/param-readers", () => {
      paramReaderImports += 1;
      return {
        readPositiveIntegerParam: () => 2_500,
      };
    });
    vi.doMock("afora-agent/plugin-sdk/routing", () => {
      routingImports += 1;
      return {
    ...
    vi.doMock("afora-agent/plugin-sdk/transcripts", () => {
  ...
  it("loads and caches the node policy delegate", async () => {
    const delegateHandle = vi.fn<AforaPluginNodeInvokePolicy["handle"]>(async () => ({
      ok: true,
    }));
    const loadPolicy = vi.fn(
      async (_config: GoogleMeetConfig): Promise<AforaPluginNodeInvokePolicy> => ({
    ...
    await expect(policy.handle({} as AforaPluginNodeInvokePolicyContext)).resolves.toEqual({
      ok: true,
    });
    await expect(policy.handle({} as AforaPluginNodeInvokePolicyContext)).resolves.toEqual({
      ok: true,
  ...
  it("fails closed when the node policy cannot load", async () => {
    ...
    await expect(policy.handle({} as AforaPluginNodeInvokePolicyContext)).resolves.toMatchObject(
      {
        ok: false,
  ...
  it("does not rewrite node policy delegate failures", async () => {
    ...
    const policy = createLazyGoogleMeetNodeInvokePolicy(
      ...
      async () =>
        ...
        }) satisfies AforaPluginNodeInvokePolicy,
    );

    await expect(policy.handle({} as AforaPluginNodeInvokePolicyContext)).rejects.toBe(
      delegateError,
    );
  });
});
