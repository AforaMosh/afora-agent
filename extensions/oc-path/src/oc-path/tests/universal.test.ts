...
describe("setOcPath — jsonc leaf with coercion", () => {
  ...
  it("resolves slash-deep JSONC paths", () => {
    ...
    const r = setOcPath(
      ast,
      parseOcPath("oc://afora.json/agents/list/0/tools/exec/security"),
      "allowlist",
    );
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  it("keeps JSON-looking strings as strings by default", () => {
    ...
    const r = setOcPath(ast, parseOcPath("oc://afora.json/token"), '{"source":"file"}');
    expect(r.ok).toBe(true);
    if (r.ok) {
      const ast2 = r.ast as Parameters<typeof emitJsonc>[0];
  ...
  it("replaces a JSONC leaf with parsed JSON when requested", () => {
    ...
    const r = setOcPath(
      ast,
      parseOcPath("oc://afora.json/token"),
  ...
  it("rejects non-finite parsed JSON replacement values", () => {
    ...
    const r = setOcPath(ast, parseOcPath("oc://afora.json/limit"), "1e999", {
      valueJson: true,
    });
    expect(r.ok).toBe(false);
    if (!r.ok) {
