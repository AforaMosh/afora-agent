 ...
 * `__AFORA_REDACTED__` literal anywhere in the output throws
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__AFORA_REDACTED__")) {
      guardSentinel("__AFORA_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
