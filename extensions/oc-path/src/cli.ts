/**
 * `afora path` — shell access to the OcPath substrate verbs.
 *
...
export function registerPathCli(program: Command): void {
  const path = program
    ...
    .addHelpText("after", "\nDocs: https://docs.afora.ai/cli/path\n");
  ...
  // Bare `afora path` prints help and exits 0 (matches the core
  // applyParentDefaultHelpAction contract — see afora#73077).
  path.action(() => {
    path.outputHelp();
    process.exitCode = 0;
  });
