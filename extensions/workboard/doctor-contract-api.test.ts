...
import { createPluginStateKeyedStoreForTests as createPluginStateKeyedStore } from "afora-agent/plugin-sdk/plugin-state-test-runtime";
import type {
...
describe("workboard doctor contract", () => {
  it("migrates shipped .28 plugin-state workboard data into sqlite", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-workboard-doctor-"));
