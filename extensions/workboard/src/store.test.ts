...
import { WORKBOARD_STATUSES } from "@afora/workboard-contract";
...
describe("WorkboardStore", () => {
  ...
  it("emits when another sqlite connection commits", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-workboard-change-"));
  ...
  it("persists boards, cards, subscriptions, and attachment blobs in sqlite", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-workboard-sqlite-"));
    const dbPath = path.join(dir, "workboard.sqlite");
  ...
  it("lists sqlite board summaries without hydrating card child rows", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-workboard-summary-"));
    const dbPath = path.join(dir, "workboard.sqlite");
    try {
      let cardId = "";
  ...
  it("migrates a version 2 workboard table to STRICT without losing rows", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-workboard-strict-migration-"));
    const dbPath = path.join(dir, "workboard.sqlite");
  ...
  it("uses rollback journaling on network-backed volumes", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-workboard-sqlite-network-"));
    const dbPath = path.join(dir, "workboard.sqlite");
  ...
  it("persists board metadata and notification subscriptions in separate stores", async () => {
    ...
    const board = await store.upsertBoard({
      ...
      defaultWorkspace: { kind: "dir", path: "/tmp/afora-ops" },
    });
    ...
    expect(board.defaultWorkspace).toEqual({ kind: "dir", path: "/tmp/afora-ops" });
