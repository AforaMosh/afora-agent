...
import type { AforaPluginCommandDefinition } from "afora-agent/plugin-sdk/core";
...
function createApi(run = vi.fn().mockResolvedValue({ runId: "run-1" })): AforaPluginApi {
  return {
...
describe("handleWorkboardCommand", () => {
  ...
  it("attests the default agent for an unassigned slash-command card", async () => {
    ...
    let command: AforaPluginCommandDefinition | undefined;
    const api = {
      registerCommand: vi.fn((definition: AforaPluginCommandDefinition) => {
        command = definition;
      }),
      runtime: {
        subagent: { run },
