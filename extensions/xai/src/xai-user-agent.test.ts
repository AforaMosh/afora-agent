...
describe("xaiUserAgent", () => {
  ...
  it("prefers AFORA_VERSION env over the bundled package version", () => {
    vi.stubEnv("AFORA_VERSION", "2026.3.22");
    expect(xaiUserAgent()).toBe("afora-agent/2026.3.22");
  });

  it("returns the afora/<version> shape", () => {
    vi.stubEnv("AFORA_VERSION", "2026.5.16");
    expect(xaiUserAgent()).toMatch(/^afora\/\d+\.\d+\.\d+$/u);
  });
});
