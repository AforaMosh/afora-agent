...
export async function runXaiSearchProviderSetup(
  ...
  await ctx.prompter.note(
    [
      ...
      `You can change this later with ${formatCliCommand("afora configure --section web")}.`,
    ].join("\n"),
    "X search",
  );
...
async function resolveXaiProviderAuthCredential(params: {
  ...
  try {
    const config = params.config as AforaConfig | undefined;
    const agentDir =
...
export async function executeXaiWebSearchProviderTool(
  ...
  if (!auth) {
    return {
      ...
      message:
        "web_search (grok) needs xAI credentials. Run `afora onboard --auth-choice xai-oauth` to sign in with Grok, run `afora onboard --auth-choice xai-api-key`, set `XAI_API_KEY` in the Gateway environment, or configure `plugins.entries.xai.config.webSearch.apiKey`. If you do not want to configure search credentials, use web_fetch for a specific URL or the browser tool for interactive pages.",
