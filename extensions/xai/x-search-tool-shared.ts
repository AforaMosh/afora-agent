...
export function buildMissingXSearchApiKeyPayload() {
  return {
    ...
    message:
      "x_search needs xAI credentials. Run `afora onboard --auth-choice xai-oauth` to sign in with Grok, run `afora onboard --auth-choice xai-api-key`, set `XAI_API_KEY` in the Gateway environment, or configure `plugins.entries.xai.config.webSearch.apiKey`.",
