...
export function formatMatrixCliCommandParts(parts: string[], accountId?: string): string {
  const normalizedAccountId = normalizeAccountId(accountId);
  const command = ["afora", "matrix", ...parts];
...
export function printMatrixVerificationSummary(summary: MatrixCliVerificationSummary): void {
  ...
  console.log(`Initiated by Afora: ${summary.initiatedByMe ? "yes" : "no"}`);
...
function buildVerificationGuidance(
  ...
  if (status.serverDeviceKnown === false) {
    nextSteps.add(
      `This Matrix device is no longer listed on the homeserver. Create a new Afora Matrix device with ${formatMatrixCliCommand("account add --homeserver <url> --user-id <@user:server> --password <password> --device-name Afora-Gateway", accountId)}. If you use token auth, create a fresh Matrix access token in your Matrix client or admin UI, then run ${formatMatrixCliCommand("account add --homeserver <url> --access-token <token>", accountId)}.`,
    );
  }
