...
describe("matrix CLI verification commands", () => {
  ...
  it("prints recovery-key and identity-trust diagnostics for device verification failures", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Recovery key can unlock the room-key backup, but full Matrix identity trust is still incomplete. Run afora matrix verify self, accept the request in another verified Matrix client, and confirm the SAS only if it matches.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- If you intend to replace the current cross-signing identity, run the shown printf pipeline with the Matrix recovery key env var for this account: printf '%s\\n' \"$MATRIX_RECOVERY_KEY\" | afora matrix verify bootstrap --recovery-key-stdin --force-reset-cross-signing.",
    );
  });
  ...
  it("requests Matrix self-verification and prints the follow-up SAS commands", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Then run afora matrix verify start --account ops -- txn-1 to start SAS verification.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run afora matrix verify sas --account ops -- txn-1 to display the SAS emoji or decimals.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- When the SAS matches, run afora matrix verify confirm-sas --account ops -- txn-1.",
    );
  });
  ...
  it("prints DM lookup details in Matrix verification follow-up commands", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Then run afora matrix verify start --user-id @alice:example.org --room-id '!room-'\\''$(x):example.org' -- txn-dm to start SAS verification.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run afora matrix verify sas --user-id @alice:example.org --room-id '!room-'\\''$(x):example.org' -- txn-dm to display the SAS emoji or decimals.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- When the SAS matches, run afora matrix verify confirm-sas --user-id @alice:example.org --room-id '!room-'\\''$(x):example.org' -- txn-dm.",
    );
  });
  ...
  it("terminates options before remote Matrix verification ids in follow-up commands", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Then run afora matrix verify start --account ops -- --account=evil to start SAS verification.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run afora matrix verify sas --account ops -- --account=evil to display the SAS emoji or decimals.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- When the SAS matches, run afora matrix verify confirm-sas --account ops -- --account=evil.",
    );
  });
  ...
  it("lists Matrix verification requests", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith("Initiated by Afora: no");
  });
  ...
  it("shell-quotes Matrix verification ids in follow-up command guidance", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Then run afora matrix verify start -- 'txn-'\\''$(touch /tmp/pwn)' to start SAS verification.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run afora matrix verify sas -- 'txn-'\\''$(touch /tmp/pwn)' to display the SAS emoji or decimals.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- When the SAS matches, run afora matrix verify confirm-sas -- 'txn-'\\''$(touch /tmp/pwn)'.",
    );
  });
  ...
  it("shows Matrix SAS diagnostics and confirm/mismatch guidance", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- If they match, run afora matrix verify confirm-sas -- self-1.",
    );
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- If they do not match, run afora matrix verify mismatch-sas -- self-1.",
    );
  });
  ...
  it("passes DM lookup details through Matrix verification follow-up commands", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- If they match, run afora matrix verify confirm-sas --user-id @alice:example.org --room-id '!dm:example.org' --account ops -- txn-dm.",
    );
  });
  ...
  it("prints stable transaction ids in follow-up commands after accepting verification", async () => {
    ...
    expect(consoleLogMock).toHaveBeenCalledWith(
      "- Run afora matrix verify start -- txn-stable to start SAS verification.",
    );
  });
  ...
  it("lists matrix devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("- A7hWrQ70ea (current, Afora Gateway)");
    expect(console.log).toHaveBeenCalledWith("  Last IP: 127.0.0.1");
    expect(console.log).toHaveBeenCalledWith("- BritdXC6iL (Afora Gateway)");
  });
  ...
  it("omits invalid matrix device last seen timestamps", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("Account: poe");
    expect(console.log).toHaveBeenCalledWith("- DEVICE123 (current, Afora Gateway)");
    expect(console.log).toHaveBeenCalledWith("  Last IP: 127.0.0.1");
    expect(
  ...
  it("prunes stale matrix gateway devices", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith("Deleted stale Afora devices: BritdXC6iL");
  ...
  it("adds a matrix account and prints a binding hint", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Bind this account to an agent: afora agents bind --agent <id> --bind matrix:ops",
    );
  });
  ...
  it("bootstraps verification for newly added encrypted accounts", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Matrix device hygiene warning: stale Afora devices detected (BritdXC6iL). Run afora matrix devices prune-stale --account ops.",
    );
  });
  ...
  it("uses --name as fallback account id and prints account-scoped config path", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "Bind this account to an agent: afora agents bind --agent <id> --bind matrix:main-bot",
    );
  });
  ...
  for (const scenario of [
    {
      ...
      expectedLogs: [
        ...
        "- Backup key is not loaded on this device. Run afora matrix verify backup restore to load it and restore old room keys. If restore still cannot load the key, run the shown printf pipeline with the Matrix recovery key env var for this account: printf '%s\\n' \"$MATRIX_RECOVERY_KEY\" | afora matrix verify backup restore --recovery-key-stdin.",
      ],
      absentLogs: [
        "- Backup is present but not trusted for this device. Re-run 'afora matrix verify device <key>'.",
      ],
    },
    {
      ...
      expectedLogs: [
        ...
        "- This Matrix device is no longer listed on the homeserver. Create a new Afora Matrix device with afora matrix account add --homeserver '<url>' --user-id '<@user:server>' --password '<password>' --device-name Afora-Gateway. If you use token auth, create a fresh Matrix access token in your Matrix client or admin UI, then run afora matrix account add --homeserver '<url>' --access-token '<token>'.",
      ],
    },
    {
    ...
    },
    {
      ...
      expectedLogs: [
        "- If you want a fresh backup baseline and accept losing unrecoverable history, run afora matrix verify backup reset --yes. Add --rotate-recovery-key only when the old recovery key should stop unlocking the fresh backup.",
      ],
    },
  ]) {
  ...
  it("requires --yes before resetting the Matrix room-key backup", async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(
      "Backup reset failed: Refusing to reset Matrix room-key backup without --yes. If you accept losing unrecoverable history, re-run afora matrix verify backup reset --yes.",
    );
  });
  ...
  it("prints resolved account-aware guidance when a named Matrix account is selected implicitly", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "- Run the shown printf pipeline with the Matrix recovery key env var for this account: printf '%s\\n' \"$MATRIX_RECOVERY_KEY_ASSISTANT\" | afora matrix verify device --recovery-key-stdin --account assistant. If you do not have the recovery key but still have another verified Matrix client, run afora matrix verify self --account assistant instead.",
    );
    expect(console.log).toHaveBeenCalledWith(
      "- Run afora matrix verify bootstrap --account assistant to create a room key backup.",
    );
  });
