...
import { isPrivateNetworkOptInEnabled } from "afora-agent/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "Afora agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "Afora Gateway",
      }),
    ) ?? "";
