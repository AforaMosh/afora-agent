...
describe("ensureMatrixSdkInstalled", () => {
  ...
  it("throws actionable repair error listing every missing package", async () => {
    ...
    await expect(ensureMatrixSdkInstalled({ resolveFn })).rejects.toThrow(
      /Matrix plugin dependencies are missing: matrix-js-sdk, @matrix-org\/matrix-sdk-crypto-nodejs, @matrix-org\/matrix-sdk-crypto-wasm\. Repair this plugin with `afora plugins update matrix` or run `afora doctor --fix`\./,
    );
  });
