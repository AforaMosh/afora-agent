...
function expectFinalizedPreviewEdit(eventId: string, text: string) {
  ...
  expect(options.extraContent).toEqual({ [MATRIX_AFORA_FINALIZED_PREVIEW_KEY]: true });
}
...
describe("matrix monitor handler draft streaming", () => {
  ...
  it("replaces recovered Matrix command progress instead of leaving stale failed text", async () => {
    ...
    await opts.onItemEvent?.({
      ...
      phase: "end",
      status: "failed",
      progressText: "run afora cron -> run jq (agent) failed",
    });
    await opts.onItemEvent?.({
      itemId: "command-1",
      kind: "command",
    ...
    expect(recoveredEdit?.[2]).not.toContain("run afora cron -> run jq");
    vi.useRealTimers();
  });
