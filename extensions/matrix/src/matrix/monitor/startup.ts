...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleAforaDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Afora devices detected for ${params.auth.userId}: ${deviceHealth.staleAforaDevices.map((device) => device.deviceId).join(", ")}. Run 'afora matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
