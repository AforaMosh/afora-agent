...
function createDeps(
  ...
  return {
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleAforaDevices: [] as MatrixManagedDeviceInfo[],
      currentAforaDevices: [] as MatrixManagedDeviceInfo[],
    })),
...
describe("runMatrixStartupMaintenance", () => {
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      currentDeviceId: null,
      staleAforaDevices: [
        { deviceId: "DEV123", displayName: "Afora Device", current: false },
      ],
      currentAforaDevices: [],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Afora devices detected for @bot:example.org: DEV123. Run 'afora matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'afora matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
