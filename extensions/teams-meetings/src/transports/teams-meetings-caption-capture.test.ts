...
describe("Microsoft Teams meeting captions and permissions", () => {
  ...
  it("enables live captions and captures the validated Teams caption row DOM", async () => {
    ...
    expect(window["__aforaTeamsCaptions"]).toMatchObject({
      identity: "teams-work:19:meeting_test@thread.v2",
    });
  });

  it("retries an unverified live-caption activation", async () => {
    const first = await runCaptionRows([captionRow("Afora QA", "Retry captions")], undefined, {
    ...
    const second = await runCaptionRows([captionRow("Afora QA", "Retry captions")], first, {
      captionsInitiallyOn: false,
  ...
  it("preserves valid one-character caption lines", async () => {
    const { result } = await runCaptionRows([captionRow("Afora QA", "I")]);

    expect(result).toMatchObject({
      lastCaptionText: "I",
      recentTranscript: [{ speaker: "Afora QA", text: "I" }],
      transcriptLines: 1,
    });
  });
  ...
  it("bounds visible and committed caption rows together", async () => {
    const { result, window } = await runCaptionStatusScript({
      captionRows: Array.from({ length: 505 }, (_, index) =>
        captionRow("Afora QA", `Bounded caption ${index}`),
      ),
    });
    const state = window["__aforaTeamsCaptions"] as {
      droppedLines: number;
      lines: unknown[];
      visible: unknown[];
    };
  ...
  it("keeps repeated utterances from distinct caption rows", async () => {
    const first = await runCaptionRows([captionRow("Afora QA", "Yes")]);
    const second = await runCaptionRows([captionRow("Afora QA", "Yes")], first, {
      priorMeeting: first.window[MEETING_STATE_KEY] as Record<string, unknown>,
    });
  ...
  it("keeps the latest caption when Teams shortens a provisional row", async () => {
    vi.useFakeTimers();
    const row = captionRow("Afora QA", "We should leave today");
    ...
    expect(second.result.recentTranscript).toMatchObject([
      { speaker: "Afora QA", text: "We should leave" },
    ]);
  });

  it("keeps a mid-sentence caption correction in the same row lifecycle", async () => {
    const row = captionRow("Afora QA", "I like cats");
  ...
  it("keeps one utterance when Teams replaces a logically identical caption row", async () => {
    const first = await runCaptionRows([captionRow("Afora QA", "Logical row", "8")]);
    const second = await runCaptionRows(
      [captionRow("Afora QA", "Logical row replacement", "8")],
      first,
    );
  ...
  it("updates a settled logical row in place when Teams corrects it late", async () => {
    ...
    const first = await runCaptionRows(
      [captionRow("Afora QA", "Late logical", "9")],
      undefined,
      {
        captionsInitiallyOn: true,
      },
    ...
    await vi.advanceTimersByTimeAsync(1_000);
    const second = await runCaptionRows(
      [captionRow("Afora QA", "Late logical correction", "9")],
      first,
    );
  ...
  it("deduplicates a settled logical row after temporary DOM removal", async () => {
    vi.useFakeTimers();
    const first = await runCaptionRows(
      [captionRow("Afora QA", "Virtual row return", "13")],
      undefined,
      {
        captionsInitiallyOn: true,
      },
  ...
  it("commits an utterance when Teams recycles the same virtual-list row", async () => {
    vi.useFakeTimers();
    const row = captionRow("Afora QA", "First recycled-row utterance");
    const first = await runCaptionRows([row], undefined, {
      captionsInitiallyOn: true,
    });
  ...
  it("commits a removed row before Teams rapidly reuses its DOM node", async () => {
    const row = captionRow("Afora QA", "Rapid first utterance");
    const first = await runCaptionRows([row]);
    const caption = row.querySelector('[data-tid="closed-caption-text"]');
    if (!caption) {
      throw new Error("expected caption text control");
  ...
  it("does not merge recycled rows that only share a text prefix", async () => {
    vi.useFakeTimers();
    const row = captionRow("Afora QA", "Thank you");
    const first = await runCaptionRows([row], undefined, {
      captionsInitiallyOn: true,
    });
    const disappeared = await runCaptionRows([], first);
  ...
  it("retains settled markers for older rows while newer captions settle", async () => {
    vi.useFakeTimers();
    const firstRow = captionRow("Afora QA", "First settled row", "11");
    const secondRow = captionRow("Afora QA", "Second settled row", "12");
  ...
  it("updates a caption when its speaker label arrives late", async () => {
    ...
    author.textContent = "Afora QA";
    const second = await runCaptionRows([row], first);

    expect(second.result.transcriptLines).toBe(1);
    expect(second.result.recentTranscript).toMatchObject([
      { speaker: "Afora QA", text: "Late attribution" },
    ]);
  });

  it("updates a corrected speaker on the same logical row before settlement", async () => {
    const first = await runCaptionRows([
      captionRow("Afora Q", "Stable speaker correction", "10"),
    ]);
    const second = await runCaptionRows(
      [captionRow("Afora QA", "Stable speaker correction", "10")],
      first,
    );

    expect(second.result.transcriptLines).toBe(1);
    expect(second.result.recentTranscript).toMatchObject([
      { speaker: "Afora QA", text: "Stable speaker correction" },
    ]);
  });
  ...
  it("disconnects stale caption capture outside transcribe mode", async () => {
    ...
    expect(window).not.toHaveProperty("__aforaTeamsCaptions");
  });
  ...
  it("does not clear captions owned by another tab or meeting session", async () => {
    ...
    expect(wrongTab.window["__aforaTeamsCaptions"]).toBe(active);
    expect(wrongSession.window["__aforaTeamsCaptions"]).toBe(active);
  });
  ...
  it("finalizes same-session captions when meeting identity is lost", async () => {
    ...
    const captions = window["__aforaTeamsCaptions"] as Record<string, unknown>;
    const readTranscript = runInNewContext(
    ...
    expect(window["__aforaTeamsCaptions"]).toBe(captions);
  });

  it("finalizes caption capture before an SPA navigation can mix meetings", async () => {
    const params = {
      captionRows: [captionRow("Afora QA", "Meeting A caption")],
      captureCaptions: true,
      leave: control({ label: "Leave" }),
    };
    const page = await runStatusScript(params);

    params.captionRows = [captionRow("Afora QA", "Meeting B caption")];
    page.triggerCaptionMutation(CONSUMER_URL);

    const captions = page.window["__aforaTeamsCaptions"] as {
      finalized?: boolean;
  ...
  it("returns finalized captions after the tab navigates into another meeting", () => {
    ...
    expect(window["__aforaTeamsCaptions"]).toBeDefined();
  });
  ...
  it("preserves same-session captions during the in-call rerender window", async () => {
    ...
    expect(disconnects).toBe(0);
    expect(window["__aforaTeamsCaptions"]).toBe(active);
  });
  ...
  it("keeps the live caption observer during a bounded in-call control rerender", async () => {
    ...
    const page = await runCaptionStatusScript({
      captionRows: [captionRow("Afora QA", "Caption before control rerender")],
      currentUrl,
      leave,
      priorMeeting: {
        identity: "teams-work:19:meeting_test@thread.v2",
    ...
    expect(page.window["__aforaTeamsCaptions"]).not.toMatchObject({ finalized: true });
  });
  ...
  it("replaces finalized captions for a new verified session", async () => {
    ...
    const { window } = await runCaptionRows([captionRow("Afora QA", "New session")], undefined, {
      priorCaptions: old,
    });
    const current = window["__aforaTeamsCaptions"] as Record<string, unknown>;
  ...
  it("atomically refuses to replace a newer live owner during recovery", async () => {
    ...
    expect(window["__aforaTeamsCaptions"]).toBe(priorCaptions);
    expect(disconnects).toBe(0);
  });
  ...
  it("repairs a stale caption owner for the committed meeting session", async () => {
    ...
    expect(window["__aforaTeamsCaptions"]).toMatchObject({ sessionId: "session-1" });
  });

  it("disconnects caption capture when finalizing a transcript", async () => {
    const first = await runCaptionRows([captionRow("Afora QA", "Final caption")]);
    let disconnects = 0;
    const captions = first.window["__aforaTeamsCaptions"] as Record<string, unknown>;
    ...
    const refreshed = await runCaptionRows([captionRow("Afora QA", "Late caption")], undefined, {
      priorCaptions: captions,
    });
    expect(refreshed.window["__aforaTeamsCaptions"]).toBe(captions);
    expect(captions.observerInstalled).toBe(false);
