...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds Afora attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.["User-Agent"]).toBe("afora-agent/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.AFORA_DEBUG_PROXY_SESSION_ID = "tts-session";
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      proxyReset.captureProxyEnv();
      process.env.AFORA_DEBUG_PROXY_ENABLED = "1";
      process.env.AFORA_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
