...
import type { ProviderDefaultThinkingPolicyContext } from "afora-agent/plugin-sdk/core";
import type { ProviderNormalizeResolvedModelContext } from "afora-agent/plugin-sdk/plugin-entry";
import type {
  ModelApi,
  ModelProviderConfig,
...
} from "afora-agent/plugin-sdk/provider-model-types";
...
const OPENAI_AGENT_RUNTIME_ID = "afora";
const CODEX_AGENT_RUNTIME_ID = "codex";
const AFORA_RUNTIME_COMPATIBLE_IDS = [OPENAI_AGENT_RUNTIME_ID] as const;
...
function resolveSingleObservedModelRoute(
  ...
  // and let the Afora runtime own the request.
  const platformApi =
...
export function resolveModelRoutes(
  ...
  return {
    ...
    defaultRuntimeId: resolutions.some(
      (resolution) => resolution.kind === "routes" && resolution.defaultRuntimeId === "afora",
    )
      ? OPENAI_AGENT_RUNTIME_ID
