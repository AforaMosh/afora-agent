...
describe("probeNextcloudTalkBotResponseFeature", () => {
  ...
  it("passes when the matching bot has the response feature bit", async () => {
    ...
    await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
      ...
      botName: "Afora",
      features: 11,
      message: 'Nextcloud Talk bot "Afora" has the response feature.',
    });
  });
  ...
  it("reports missing response feature for the matching webhook bot", async () => {
    ...
    await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
      ...
      features: 9,
      message:
        'Nextcloud Talk bot "Afora" (7) is missing the response feature (features=9); outbound replies will fail. Run ./occ talk:bot:state --feature webhook --feature response --feature reaction 7 1 or reinstall the bot with --feature response.',
    });
  });
  ...
  it("does not coerce partial bot feature strings", async () => {
    ...
    await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
      ...
      botName: "Afora",
      message:
        'Nextcloud Talk bot "Afora" (7) is missing the response feature; outbound replies will fail. Run ./occ talk:bot:state --feature webhook --feature response --feature reaction 7 1 or reinstall the bot with --feature response.',
    });
  });
