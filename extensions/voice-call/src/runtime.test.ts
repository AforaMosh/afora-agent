...
describe("createVoiceCallRuntime lifecycle", () => {
  ...
  it("selects realtime provider readiness and admission for the routed agent", async () => {
    const config = createBaseConfig();
    config.realtime.enabled = true;
    ...
    mocks.resolveConfiguredRealtimeVoiceProvider.mockImplementation(
      ({ agentId }: { agentId?: string }) => {
        ...
        return {
          ...
          providerConfig: { model: "gpt-live-1-boulder-alpha", owner: agentId },
        };
      },
    );
    ...
    expect(
      mocks.resolveConfiguredRealtimeVoiceProvider.mock.calls.map(
        ([options]) => requireRecord(options, "provider options").agentId,
      ),
    ).toEqual(["main", "support"]);
    ...
    expect(mocks.resolveConfiguredRealtimeVoiceProvider.mock.calls.at(-1)?.[0]).toEqual(
      expect.objectContaining({ agentId: "support" }),
    );
  });

  it("normalizes non-error realtime provider failures after all candidates fail", async () => {
    const sentinel = { reason: "provider unavailable" };
    const config = createBaseConfig();
    config.realtime.enabled = true;
    const fullConfig = {
      agents: { list: [{ id: "main", default: true }, { id: "support" }] },
    ...
    mocks.resolveConfiguredRealtimeVoiceProvider.mockImplementation(() => {
      // oxlint-disable-next-line typescript/only-throw-error -- Deliberately exercises a non-Error throw.
      throw sentinel;
    });
    ...
    expect(failure).toMatchObject({
      message: "No configured realtime voice provider registered",
      cause: sentinel,
    });
  });
