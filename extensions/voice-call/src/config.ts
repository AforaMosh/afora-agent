...
import { mergeDeep } from "afora-agent/plugin-sdk/plugin-config-runtime";
import { REALTIME_VOICE_AGENT_CONSULT_TOOL_POLICIES } from "afora-agent/plugin-sdk/realtime-voice";
...
import { normalizeWebhookPath } from "afora-agent/plugin-sdk/webhook-ingress";
import { z } from "zod";
...
const VoiceCallRealtimeConfigSchema = z
  .object({
    ...
    /** Tool policy for the shared Afora agent consult tool. */
    toolPolicy: VoiceCallRealtimeToolPolicySchema.default("safe-read-only"),
    /** Guidance for when the realtime model should call the Afora agent consult tool. */
