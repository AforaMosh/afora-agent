...
describe("PlivoProvider", () => {
  ...
  it("pins stored callback bases to publicUrl instead of request Host", () => {
    const provider = new PlivoProvider(
      ...
      {
        publicUrl: "https://voice.afora.ai/voice/webhook?provider=plivo",
      },
    );
    ...
    expect(callbackMap.get("call-uuid")).toBe("https://voice.afora.ai/voice/webhook");
  });
    ...
    expect(apiRequest).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        body: expect.objectContaining({
          aleg_url:
            "https://voice.afora.ai/voice/webhook?provider=plivo&flow=xml-speak&callId=internal-call-id",
        }),
      }),
    );
  });
