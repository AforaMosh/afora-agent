...
Official Voice Call plugin for **Afora**.
...
Docs: `https://docs.afora.ai/plugins/voice-call`
Plugin system: `https://docs.afora.ai/tools/plugin`
...
- Runtime accepts canonical config only. If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `afora doctor --fix` to rewrite them.
- advanced webhook, streaming, and tunnel notes: `https://docs.afora.ai/plugins/voice-call`
...
- `realtime.consultThinkingLevel` is optional. When set, it overrides the thinking level used by the model behind realtime `afora_agent_consult` calls.
- `realtime.consultFastMode` is optional. When set, it toggles fast mode for realtime `afora_agent_consult` calls.

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.afora.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.afora.ai/plugins/voice-call#tts-for-calls`
