...
Official MXC sandbox execution plugin for Afora.

This plugin lets Afora run tool execution through MXC on Windows hosts with
ProcessContainer support.

## Install

```bash
afora plugins install @afora/mxc-sandbox
```
...
- Plugin id: `mxc`
- Package: `@afora/mxc-sandbox`
- Minimum Afora host: `2026.6.11`
...
Any other key is rejected. `afora.plugin.json` publishes the same schema
(enums, `minimum`/`maximum` bounds) so `afora config` validation and CLI
...
- `workspaceAccess`:
  ...
  - `rw`: the active agent workspace is mounted read-write. If protected
    Afora skill roots (`skills`, `.agents/skills`, or the materialized
...
- `scope` workspace selection:
  - `session`, `agent`, and `shared` choose the Afora workspace directory
    passed to MXC.
...
- Afora passes per-run command, environment, and filesystem config to the
...
[`afora config patch --stdin`](https://docs.afora.ai/cli/config#config-patch)
...
```powershell
$mxcPolicyPath = Join-Path $env:TEMP "afora-mxc-policy.json"
@'
{
  "filesystem": {
...
$mxcConfigPatch = @"
{
  agents: {
    list: [
      ...
      {
        id: "mxc-test",
        workspace: "~/.afora/workspace-mxc-test",
        sandbox: {
          mode: "all",
          backend: "mxc",
          scope: "agent",
...
$mxcConfigPatch | afora config patch --stdin --dry-run
$mxcConfigPatch | afora config patch --stdin
```
    ...
    "entries": {
      "mxc": {
        ...
        "config": {
          "containment": "process",
          "network": "none",
          "mxcPolicyPaths": ["C:\\Users\\you\\AppData\\Local\\Temp\\afora-mxc-policy.json"],
        },
      },
    },
  },
...
```json
{
  "filesystem": {
    "restrictToProjectDir": true,
    "additionalReadonlyPaths": ["C:\\Tools\\Afora\\shared-readonly"],
    "additionalReadwritePaths": ["D:\\AforaScratch"]
  },
  "process": {
    "timeoutSeconds": 120
  }
...
When multiple configured policy files exist, Afora layers them
...
The filesystem bridge keeps protected Afora skill overlays read-only. For
...
```powershell
afora tui --session agent:mxc-test:main
```

For local embedded testing without a Gateway:

```powershell
afora tui --local --session agent:mxc-test:main
```
...
$mxcCleanupPatch | afora config patch --stdin --dry-run
$mxcCleanupPatch | afora config patch --stdin
