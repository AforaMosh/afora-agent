...
vi.mock("afora-agent/plugin-sdk/sandbox", () => ({
...
const nonFullRegistrationModes = [
  ...
  "setup-runtime",
  "cli-metadata",
] as const satisfies readonly AforaPluginApi["registrationMode"][];
...
function createApi(
  ...
  registrationMode: AforaPluginApi["registrationMode"] = "full",
): {
  api: AforaPluginApi;
  registerService: ReturnType<typeof vi.fn>;
  services: AforaPluginService[];
} {
  const services: AforaPluginService[] = [];
  const registerService = vi.fn((service: AforaPluginService): void => {
  ...
  return {
    api: api as unknown as AforaPluginApi,
    registerService,
    services,
  };
}
...
describe("registerMxcPlugin", () => {
  ...
  test("registers the sandbox backend on Windows when feature probes pass", () => {
    ...
    void services[0]?.stop?.({} as AforaPluginServiceContext);
