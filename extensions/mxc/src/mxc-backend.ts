...
import { runCommandBuffered } from "afora-agent/plugin-sdk/process-runtime";
import { resolvePreferredAforaTmpDir } from "afora-agent/plugin-sdk/sandbox";
import type {
...
function createLauncherPayloadFile(
  ...
  const payloadDir = mkdtempSync(
    path.join(resolvePreferredAforaTmpDir(), "afora-mxc-payload-"),
  );
...
function createSandboxTempDir(hostEnv: BaselineHostEnv): string {
  return mkdtempSync(path.join(resolveSandboxTempDir(hostEnv), "afora-mxc-sandbox-"));
}
...
/** Manager for `afora sandbox list` and `afora sandbox remove`. */
