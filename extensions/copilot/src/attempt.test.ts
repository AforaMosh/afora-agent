...
const gatewayQuestionMock = vi.hoisted(() => ({
  ...
  claimPendingAgentQuestionAnswer: undefined as
    | ((
        ...args: Parameters<
          typeof import("afora-agent/plugin-sdk/agent-harness-runtime").claimPendingAgentQuestionAnswer
        >
      ) => Promise<boolean>)
    | undefined,
...
describe("runCopilotAttempt", () => {
  ...
  it("hydrates offloaded prompt images before creating SDK blob attachments", async () => {
    ...
    const inboundDir = aforaState.statePath("media", "inbound");
  ...
  it("registers ask_user and resolves it from the active Afora queue", async () => {
  ...
  describe("workspace bootstrap (systemMessage)", () => {
    ...
    it("forwards rendered bootstrap instructions into SDK SessionConfig.systemMessage (append mode)", async () => {
      ...
      // while layering Afora context after it. See
    ...
    it("forwards rendered bootstrap instructions to resumeSession on the resume path", async () => {
      ...
      // otherwise resumed sessions would silently lose Afora
  ...
  describe("session-level gitHubToken (independent of client-level)", () => {
    ...
    it("SessionConfig.gitHubToken is omitted when default mode is useLoggedInUser (no auth signal)", async () => {
      ...
      const prevAfora = process.env.AFORA_GITHUB_TOKEN;
      const prevGithub = process.env.GITHUB_TOKEN;
      delete process.env.AFORA_GITHUB_TOKEN;
      ...
      try {
        ...
        if (prevAfora !== undefined) {
          process.env.AFORA_GITHUB_TOKEN = prevAfora;
        }
  ...
  // bypass Afora's wrapped-tool enforcement under any permissive
  ...
  describe("availableTools surface restriction (PR #86155 [P1] round-8)", () => {
    ...
    it("keeps native ask_user when its restricted Afora equivalent remains allowed", async () => {
      const sdk = makeFakeSdk();
      const pool = makeFakePool(sdk);
    ...
    it("keeps a host-scoped Afora create-session surface ring-zero", async () => {
      const sdk = makeFakeSdk();
      const pool = makeFakePool(sdk);
      const sdkTools = [makeFakeSdkTool("afora")];
      const createToolBridge = vi.fn(async () => ({ sdkTools, sourceTools: [] }));

      await runCopilotAttempt(makeParams({ toolsAllow: ["afora"] }), {
        createToolBridge,
        isHostScopedToolActive: (toolName) => toolName === "afora",
        pool,
      });

      expect(readAvailableTools(sdk.createSession.mock.calls[0])).toEqual(["afora"]);
    });
    ...
    it("keeps a host-scoped Afora resume-session surface ring-zero", async () => {
      const sdk = makeFakeSdk({
        onResumeSession: (session) => {
          session.sendAndWait.mockResolvedValueOnce(makeAssistantMessageEvent("resumed"));
        },
      ...
      await runCopilotAttempt(
        makeParams({
          initialReplayState: { sdkSessionId: "sess-afora" },
          toolsAllow: ["afora"],
        } as never),
        {
          createToolBridge,
      ...
      const resumeCall = sdk.resumeSession.mock.calls[0] as unknown[] | undefined;
      const resumeCfg = resumeCall?.[1] as { availableTools?: string[] };
      expect(resumeCfg?.availableTools).toEqual(["afora"]);
    });
