...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { aforaDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { aforaDefaultImporterSentinel?: boolean };

      expect(sdk.aforaDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("afora plugins install @afora/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("afora plugins install @afora/copilot");
  });
  ...
  it("resolves the fallback install from AFORA_STATE_DIR", async () => {
