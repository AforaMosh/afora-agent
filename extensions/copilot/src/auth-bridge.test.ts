...
describe("resolveCopilotAuth - copilotHome resolution", () => {
  ...
  it("synthesises per-agent default from homeDir when no path is given", () => {
    ...
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".afora", "agents", "agent-1", "copilot")),
    );
  });

  it("respects AFORA_HOME env var as the home root", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: { AFORA_HOME: "/custom/afora" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.copilotHome).toBe(
      resolve(join("/custom/afora", ".afora", "agents", "agent-1", "copilot")),
    );
  });
  ...
  it("uses the default agent id when agentId is invalid/missing", () => {
    ...
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".afora", "agents", COPILOT_DEFAULT_AGENT_ID, "copilot")),
    );
  });
...
describe("resolveCopilotAuth - contract-resolved auth (resolvedApiKey + authProfileId)", () => {
  ...
  it("contract resolvedApiKey takes precedence over env fallback", () => {
    const result = resolveCopilotAuth({
      ...
      authProfileId: "p",
      env: {
        AFORA_GITHUB_TOKEN: "env-should-be-ignored",
...
describe("resolveCopilotAuth - env var fallbacks", () => {
  ...
  it("AFORA_GITHUB_TOKEN takes precedence over GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        AFORA_GITHUB_TOKEN: "afora-tok",
        GITHUB_TOKEN: "github-tok",
      } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.gitHubToken).toBe("afora-tok");
    expect(result.authProfileId).toBe("env:AFORA_GITHUB_TOKEN");
    expect(result.authProfileVersion).toBe(tokenFingerprint("afora-tok"));
  });
  ...
  it("AFORA_GITHUB_TOKEN takes precedence over COPILOT_GITHUB_TOKEN, GH_TOKEN and GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        AFORA_GITHUB_TOKEN: "afora-tok",
  ...
  it("explicit auth.useLoggedInUser=true wins over env tokens", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      auth: { useLoggedInUser: true },
      env: { AFORA_GITHUB_TOKEN: "env-tok" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.authMode).toBe("useLoggedInUser");
  });
...
describe("resolveCopilotAuth - defaults wiring", () => {
  ...
  it("uses os.homedir() when homeDir is not injected", () => {
    ...
    expect(result.copilotHome.endsWith(join(".afora", "agents", "agent-1", "copilot"))).toBe(
      true,
    );
  });
  ...
  it("falls back to process.cwd() if homeDir throws", () => {
    ...
    expect(result.copilotHome.includes(join(".afora", "agents", "agent-1", "copilot"))).toBe(
      true,
    );
  });
});
