...
import { createAforaCodingTools as createRealAforaCodingTools } from "afora-agent/plugin-sdk/agent-harness";
import {
  type AnyAgentTool,
  type SandboxContext,
...
async function convertAforaToolToSdkToolForTest(
  ...
  const bridge = await createCopilotToolBridge({
    ...
    createAforaCodingTools: async () => [sourceTool],
    modelId: "gpt-test",
...
describe("createCopilotToolBridge", () => {
  it("rejects a direct caller that omits the required host capability", async () => {
    const createAforaCodingTools = vi.fn(async () => [makeTool()]);
    await expect(
    ...
    expect(createAforaCodingTools).not.toHaveBeenCalled();
  });
  ...
  it("binds every tool surface and retained source/SDK tools fail after capability closure", async () => {
    ...
    const bridge = await createCopilotToolBridge({
      ...
      createAforaCodingTools: async () => [makeTool({ execute })],
      modelId: "gpt-test",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    ...
    expect(createAforaCodingTools).toHaveBeenCalledTimes(0);
  });

  it("allows vetted BYOK providers to expose model tools", async () => {
    const sourceTools = [makeTool()];
    const createAforaCodingTools = vi.fn(async () => sourceTools);
    ...
    expect(createAforaCodingTools).toHaveBeenCalledWith(
  ...
  it("forwards supported fields to injected createAforaCodingTools", async () => {
    ...
    expect(createAforaCodingTools).toHaveBeenCalledTimes(1);
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createAforaCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("preserves direct-only Afora through the exact Copilot allowlist", async () => {
    ...
    const result = await createCopilotToolBridge({
      ...
      createAforaCodingTools: async () => [systemAgentTool],
      modelId: "gpt-4.1",
      modelProvider: "github-copilot",
    ...
    expect(result.sdkTools.map((tool) => tool.name)).toEqual(["afora"]);
  });

  it("compacts the Copilot tool surface behind tool_search controls when enabled", async () => {
    const createAforaCodingTools = vi.fn(async (opts: unknown) => {
  ...
  it("compacts the Copilot tool surface behind code-mode exec/wait when enabled", async () => {
    const createAforaCodingTools = vi.fn(async () => [
      makeTool({ name: "fake_hidden" }),
      makeTool({ name: "read" }),
    ]);
  ...
  it("binds retained code-mode source and SDK controls exactly once", async () => {
    ...
    const bridge = await createCopilotToolBridge({
      ...
      createAforaCodingTools: async () => [makeTool({ execute: hiddenExecute, name: "read" })],
      modelId: "gpt-test",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("reports code mode as disengaged when the config leaves it off", async () => {
    const result = await createCopilotToolBridge({
      ...
      createAforaCodingTools: vi.fn(async () => [makeTool({ name: "read" })]),
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("throws when createAforaCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createAforaCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createAforaCodingTools must return an array");
  });

  it("throws when createAforaCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createAforaCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged Afora tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createAforaCodingTools,
        getOpts: () =>
          (createAforaCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createAforaCodingTools, getOpts } = captureCall();
      const toolBindings = {
    ...
    it("quarantines owner memory writes, edits, and patches after a network tool", async () => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-copilot-memory-"));
      ...
      try {
        ...
        const createTools = (rawOptions: unknown) => {
          const options = rawOptions as NonNullable<
            Parameters<typeof createRealAforaCodingTools>[0]
          >;
          const filesystemTools = createRealAforaCodingTools(options).filter((tool) =>
        ...
        const bridge = await createCopilotToolBridge({
          ...
          createAforaCodingTools: createTools,
          modelId: "gpt-4o",
          modelProvider: "github-copilot",
    ...
    it("prefers the unscoped toolAuthProfileStore when building Afora tools", async () => {
      const { createAforaCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createAforaCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createAforaCodingTools", async () => {
  ...
  // Afora attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("submits the exact conversation-policy-filtered catalog to the SDK", async () => {
      await withTempDir("afora-copilot-policy-", async (workspaceDir) => {
        const result = await createCopilotToolBridge({
          agentId: "agent-1",
          attemptParams: {
          ...
          createAforaCodingTools: createRealAforaCodingTools,
          modelId: "gpt-4o",
          modelProvider: "github-copilot",
    ...
    it("short-circuits when attemptParams.disableTools is true and never calls createAforaCodingTools", async () => {
      const createAforaCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createAforaCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
        attemptParams: {} as never,
        createAforaCodingTools,
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("does not discard lean-mode overrides after tool construction", async () => {
      const result = await createCopilotToolBridge({
        ...
        createAforaCodingTools: async () => [makeTool({ name: "image_generate" })],
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      });
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createAforaCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeAforaTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeAforaTools).toBe(true);
    });
...
describe("createCopilotToolBridge tool conversion", () => {
  it("throws on empty and non-string names", async () => {
    await expect(
      convertAforaToolToSdkToolForTest(makeTool({ name: "" as never }), {}),
    ).rejects.toThrow("tool name must be a non-empty string");
    await expect(
      convertAforaToolToSdkToolForTest(makeTool({ name: 42 as never }), {}),
    ).rejects.toThrow("tool name must be a non-empty string");
  });

  it("throws on non-function execute", async () => {
    await expect(
      convertAforaToolToSdkToolForTest(makeTool({ execute: "nope" as never }), {}),
  ...
  it("preserves name, description, and parameters exactly", async () => {
    ...
    const result = await convertAforaToolToSdkToolForTest(sourceTool, {});
  ...
  it("sets skipPermission: true so Afora's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", async () => {
    // Per the harness docs: every bridged Afora tool comes from
    // `createAforaCodingTools`, which already wraps each tool with
    ...
    // for kind: "custom-tool" would either short-circuit Afora's
    // richer enforcement (allow-all) or block every call (reject-all).
    // Setting `skipPermission: true` lets the wrapped execute() run
    // Afora's hook with the right context — mirrors codex
    // (`extensions/codex/src/app-server/dynamic-tools.ts`).
    const result = (await convertAforaToolToSdkToolForTest(makeTool(), {})) as SdkTool & {
  ...
  it("marks every bridged tool as overridesBuiltInTool so Afora owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", async () => {
    // Real-world dogfood found that afora's createAforaCodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = (await convertAforaToolToSdkToolForTest(
        makeTool({ name }),
        {},
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = await convertAforaToolToSdkToolForTest(sourceTool, {
      abortSignal: controller.signal,
    });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = await convertAforaToolToSdkToolForTest(sourceTool, { beforeExecute });
  ...
  it("calls prepareArguments and passes the prepared args and toolCallId to execute", async () => {
    ...
    const sdkTool = await convertAforaToolToSdkToolForTest(sourceTool, { onToolCompleted });
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = await convertAforaToolToSdkToolForTest(makeTool({}, sourceResult), {
      onAgentToolResult,
    });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = await convertAforaToolToSdkToolForTest(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("surfaces an owner-backed memory delete failure before a false final claim", async () => {
    ...
    const sdkTool = await convertAforaToolToSdkToolForTest(tool, { observeToolTerminal });
  ...
  it("keeps owner-backed failures before execution non-mutating", async () => {
    ...
    const sdkTool = await convertAforaToolToSdkToolForTest(tool, {
      abortSignal: controller.signal,
      observeToolTerminal,
    });
  ...
  it("reports returned Afora error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
  ...
  it("reports owner-backed catalog tool failures to the host terminal observer", async () => {
    ...
    await createCopilotToolBridge({
      ...
      createAforaCodingTools: async (options: unknown) => {
