...
import { createMigrationManualItem } from "afora-agent/plugin-sdk/migration";
import type { MigrationItem } from "afora-agent/plugin-sdk/plugin-entry";
import {
  asBoolean,
  isRecord,
...
export function mcpManualItems(params: {
  ...
  if (
    ...
    add(
      ...
      "Define the missing values in Afora's MCP server environment or headers manually.",
    );
  }
  ...
  if (Array.isArray(cert) && cert.length === 3) {
    add(
      ...
      `Hermes MCP server "${name}" uses a password-protected client key, which Afora cannot represent in MCP config.`,
    ...
    add(
      ...
      "Configure separate Afora clientCert and clientKey file paths manually.",
    );
  }
  if (typeof (raw.sslVerify ?? raw.ssl_verify) === "string") {
    add(
      "tls-ca",
      `Hermes MCP server "${name}" uses a CA bundle path for TLS verification, which Afora MCP config cannot represent.`,
  ...
  if (transport && !["http", "streamable-http", "sse", "stdio"].includes(transport)) {
    add(
      ...
      "Configure an equivalent Afora MCP transport manually.",
    );
  }
  ...
  if (auth && auth !== "oauth") {
    add(
      ...
      "Configure an equivalent Afora MCP authentication mode manually.",
    );
  }
  ...
  if (auth === "oauth" || oauth) {
    add(
      "oauth-login",
      `Hermes MCP server "${name}" requires OAuth login in Afora.`,
      `Run "afora mcp login ${name}" after migration.`,
    );
  }
  if (
    oauth &&
    ...
    add(
      "oauth-client",
      `Hermes MCP server "${name}" uses pre-registered OAuth client settings that were not copied into Afora config.`,
      `Run "afora mcp login ${name}" and configure supported OAuth metadata manually.`,
    );
  }
  ...
  for (const [feature, configured] of unsupported) {
    if (configured) {
      add(
        feature,
        `Hermes MCP server "${name}" uses ${feature} behavior that Afora MCP config does not expose.`,
