    ...
    if (transport && !HERMES_TRANSPORTS[transport]) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Configure an equivalent Afora provider plugin or API adapter manually.",
        }),
      );
    ...
    if (normalizeOptionalString(raw.api_key) && !readEnvReference(raw.api_key)) {
      items.push(
        createMigrationManualItem({
          ...
          message: `Hermes provider "${id}" contains an inline API key that was not copied into Afora config.`,
          recommendation: "Move the key to an environment variable or Afora secret provider.",
        }),
      );
    }
    ...
    if (isRecord(raw.extra_body) && Object.keys(raw.extra_body).length > 0) {
      items.push(
        createMigrationManualItem({
          ...
          message: `Hermes provider "${id}" adds request body fields that Afora cannot import generically.`,
          recommendation:
    ...
    if (apiKeyEnv && !env[apiKeyEnv]?.trim()) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Configure an Afora auth profile for this provider or expose the variable to the Afora runtime.",
        }),
      );
    }
  }
