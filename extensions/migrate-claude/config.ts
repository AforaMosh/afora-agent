    ...
    if (settingsPath && settings.hooks !== undefined) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation: "Review hook commands before recreating equivalent Afora automation.",
        }),
      );
    }
    ...
    if (settingsPath && settings.env !== undefined) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Move non-secret values manually and store credentials through Afora credential flows.",
        }),
      );
    }
  }
