...
const DEEPINFRA_MODELS_URL =
  "https://api.deepinfra.com/v1/openai/models?sort_by=afora&filter=with_meta";
...
describe("DEEPINFRA_MODELS_URL", () => {
  it("points at /v1/openai/models with the afora sort + filter=with_meta gate", () => {
...
describe("hasDeepInfraApiKey", () => {
  ...
  it("falls back to the auth-profile store when no env var is set", () => {
    ...
    expect(hasDeepInfraApiKey({ env: {}, agentDir: "/tmp/afora-agent" })).toBe(true);
  ...
  it("returns false when env is empty and the auth-profile store has no deepinfra profile", () => {
    ...
    expect(hasDeepInfraApiKey({ env: {}, agentDir: "/tmp/afora-agent" })).toBe(false);
...
describe("discoverDeepInfraModels (chat-only shim)", () => {
  ...
  it("fetches the afora-projection endpoint and parses chat-surface entries when an API key is configured", async () => {
