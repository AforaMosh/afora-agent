...
function resolveDeepInfraVideoBaseUrl(req: VideoGenerationRequest): string {
  ...
  // migrated by `afora doctor --fix` (doctor-contract-api.ts), never remapped here.
  ...
  if (baseUrl.includes("/v1/inference")) {
    throw new Error(
      'DeepInfra video generation requires an OpenAI-compatible endpoint, but models.providers.deepinfra.baseUrl targets the retired native /v1/inference surface. Run "afora doctor --fix" (api.deepinfra.com migrates automatically; custom hosts must set baseUrl to an OpenAI-compatible videos endpoint).',
    );
  }
  return baseUrl;
}
