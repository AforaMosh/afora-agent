...
vi.mock("afora-agent/plugin-sdk/media-understanding", async () => {
  const actual = await vi.importActual<typeof import("afora-agent/plugin-sdk/media-understanding")>(
    "afora-agent/plugin-sdk/media-understanding",
  );
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("afora-agent/plugin-sdk/media-understanding");
  vi.resetModules();
});
