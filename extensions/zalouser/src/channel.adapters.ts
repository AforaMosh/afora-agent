...
function resolveZalouserOutboundChunkMode(cfg: AforaConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: AforaConfig, accountId?: string) {
...
export const zalouserSecurityAdapter = {
  ...
  dmRouting: {
    resolveDmScope: ({ cfg }: { cfg: AforaConfig }) => resolveZalouserDmSessionScope(cfg),
  },
...
export const zalouserPairingTextAdapter = {
  ...
  notify: async ({ cfg, id, message }: { cfg: AforaConfig; id: string; message: string }) => {
    const { sendMessageZalouser } = await loadZalouserChannelRuntime();
