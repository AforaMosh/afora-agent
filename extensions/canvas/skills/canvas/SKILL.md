---
name: canvas
description: "Present HTML on connected Afora node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "afora": { "emoji": "🖼️" } }
---
...
4. Present the hosted URL: `/__afora__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__afora__/canvas/index.html
http://<gateway-host>:<gateway.port>/__afora__/canvas/games/snake.html
```

Path mapping:

- `/__afora__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__afora__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
