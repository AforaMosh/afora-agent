...
describe("canvas host", () => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-canvas-fixtures-"));
  });
  ...
  it("injects the Canvas runtime before authored page scripts", () => {
    const authoredScript = "globalThis.bridgeReadyAtStartup = typeof aforaSendUserAction";
    ...
    expect(out).toContain("aforaCanvasA2UIAction");
    expect(out).toContain("aforaSendUserAction");
    expect(out).toContain("crypto?.getRandomValues");
    expect(out).not.toContain("String(Date.now())");
    expect(out.indexOf("globalThis.Afora.postMessage")).toBeGreaterThan(out.indexOf("<head>"));
    expect(out.indexOf("globalThis.Afora.postMessage")).toBeLessThan(
  ...
  it("ignores commented tags and quoted closing brackets when injecting", () => {
    const authoredScript = "globalThis.bridgeReadyAfterComment = typeof aforaSendUserAction";
  ...
  it("reports websocket initialization errors instead of swallowing them silently", () => {
    ...
    expect(consoleError).toHaveBeenCalledWith("Afora canvas live reload unavailable");
  });
  ...
  it("uses a path-scoped capability ahead of a stale query capability", () => {
    ...
    runInjectedScript(
      ...
      {
        pathname: "/__afora__/cap/current-token/__afora__/canvas/",
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(response.body).toContain("aforaSendUserAction");
  ...
  it("watches Canvas content when the state directory is hidden", async () => {
    const dir = path.join(fixtureRoot, ".afora", `case-${fixtureCount++}`);
    await fs.mkdir(dir, { recursive: true });
    const handler = await createTestCanvasHostHandler(dir);
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        ...
        `<afora-a2ui-host></afora-a2ui-host>
<script>aforaCanvasA2UIAction</script>`,
        "utf8",
      );
      await fs.writeFile(bundlePath, "window.aforaA2UI = {};", "utf8");
      ...
      expect(html).toContain("afora-a2ui-host");
      expect(html).toContain("aforaCanvasA2UIAction");

      const noReloadRes = await captureA2uiFixtureResponse(`${A2UI_PATH}/`, "GET", false);
      expect(noReloadRes.body).toContain("aforaCanvasA2UIAction");
