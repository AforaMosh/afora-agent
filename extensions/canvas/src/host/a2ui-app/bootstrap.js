...
import { themeContext } from "@afora/a2ui-theme-context";
...
class AforaA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding: var(--afora-a2ui-inset-top, 0px) var(--afora-a2ui-inset-right, 0px)
        var(--afora-a2ui-inset-bottom, 0px) var(--afora-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--afora-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--afora-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--afora-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--afora-a2ui-empty-top, var(--afora-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.aforaA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["afora:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis["__aforaLastA2UIAction"] = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.aforaCanvasA2UIAction ??
      globalThis.aforaCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.aforaCanvasA2UIAction) {
...
if (!customElements.get("afora-a2ui-host")) {
  customElements.define("afora-a2ui-host", AforaA2UIHost);
}
