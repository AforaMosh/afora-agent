...
export const A2UI_PATH = "/__afora__/a2ui";

/** Hosted path prefix for Canvas document/static assets. */
export const CANVAS_HOST_PATH = "/__afora__/canvas";

/** Hosted WebSocket path for Canvas live reload. */
export const CANVAS_WS_PATH = "/__afora__/ws";
...
export function injectCanvasRuntime(html: string, options: { liveReload?: boolean } = {}): string {
  ...
  function reportCanvasLiveReloadError() {
    ...
    try {
      ...
      console.error("Afora canvas live reload unavailable");
    } catch {}
  }

  try {
    const capMatch = String(location.pathname || "").match(/\\/__afora__\\/cap\\/([^/]+)(?:\\/|$)/);
    let pathCapability = "";
    try {
...
(() => {
  ...
  // Works on:
  // - iOS: window.webkit.messageHandlers.aforaCanvasA2UIAction.postMessage(...)
  // - Android: window.aforaCanvasA2UIAction.postMessage(...)
  const handlerNames = ["aforaCanvasA2UIAction"];
  ...
  globalThis.Afora = globalThis.Afora ?? {};
  globalThis.Afora.postMessage = postToNode;
  globalThis.Afora.sendUserAction = sendUserAction;
  globalThis.aforaPostMessage = postToNode;
  globalThis.aforaSendUserAction = sendUserAction;
${liveReloadSnippet}
})();
</script>
`.trim();
