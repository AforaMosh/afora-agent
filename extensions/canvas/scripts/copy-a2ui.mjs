...
function getA2uiPaths(env = process.env) {
  const srcDir = env.AFORA_A2UI_SRC_DIR ?? path.join(pluginDir, "src", "host", "a2ui");
  const outDir = env.AFORA_A2UI_OUT_DIR ?? path.join(rootDir, "dist", "canvas-host", "a2ui");
  return { srcDir, outDir };
}

function shouldSkipMissingA2uiAssets(env = process.env) {
  return env.AFORA_A2UI_SKIP_MISSING === "1" || Boolean(env.AFORA_SPARSE_PROFILE);
}
...
export async function copyA2uiAssets({ srcDir, outDir }) {
  ...
  try {
    ...
    if (skipMissing) {
      console.warn(
        `${message} Skipping copy because AFORA_A2UI_SKIP_MISSING=1 or AFORA_SPARSE_PROFILE is set.`,
      );
      return;
    }
