...
const ORIGINAL_SKIP_MISSING = process.env.AFORA_A2UI_SKIP_MISSING;
const ORIGINAL_SPARSE_PROFILE = process.env.AFORA_SPARSE_PROFILE;

describe("canvas a2ui copy", () => {
  beforeEach(() => {
    delete process.env.AFORA_A2UI_SKIP_MISSING;
    delete process.env.AFORA_SPARSE_PROFILE;
  });
  ...
  afterEach(() => {
    if (ORIGINAL_SKIP_MISSING === undefined) {
      ...
      process.env.AFORA_A2UI_SKIP_MISSING = ORIGINAL_SKIP_MISSING;
    }

    if (ORIGINAL_SPARSE_PROFILE === undefined) {
      delete process.env.AFORA_SPARSE_PROFILE;
    } else {
      process.env.AFORA_SPARSE_PROFILE = ORIGINAL_SPARSE_PROFILE;
    }
  });

  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    await withTempWorkspace(
      { rootDir: resolvePreferredAforaTmpDir(), prefix: "afora-a2ui-" },
  ...
  it("skips missing assets when AFORA_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.AFORA_A2UI_SKIP_MISSING = "1";
      await expect(
        copyA2uiAssets({ srcDir: path.join(dir, "src"), outDir: path.join(dir, "out") }),
  ...
  it("skips missing assets when AFORA_SPARSE_PROFILE is set", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.AFORA_SPARSE_PROFILE = "core";
      await expect(
        copyA2uiAssets({ srcDir: path.join(dir, "src"), outDir: path.join(dir, "out") }),
      ).resolves.toBeUndefined();
    });
