...
beforeEach(async () => {
  tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "afora-feishu-agent-"));
});
...
function createRuntime(
  ...
  persistedCfg?: AforaConfig,
  mutationCfg?: AforaConfig,
) {
  let runtimeCfg = structuredClone(currentCfg ?? ({} as AforaConfig));
  const commitConfig = vi.fn();
  const mutateConfigFile = vi.fn(
    async (params: {
      mutate: (draft: AforaConfig, context: { snapshot: never; previousHash: null }) => unknown;
    }) => {
...
describe("maybeCreateDynamicAgent", () => {
  ...
  it("rechecks current ingress inside the config mutation lock", async () => {
    ...
    const canCreateForConfig = vi
      .fn<(cfg: AforaConfig) => Promise<boolean>>()
  ...
  it("preserves concurrent runtime config when creating from a stale request snapshot", async () => {
    ...
    const result = await maybeCreateDynamicAgent({
      cfg: { agents: createCanonicalAgentRoster(), bindings: [] } as AforaConfig,
      runtime,
      accountId: "default",
      senderOpenId: "ou_sender",
      canCreateForConfig: async () => true,
