...
import { normalizeAccountId, resolveAgentRoute } from "afora-agent/plugin-sdk/routing";
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: AforaConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: AforaConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as AforaConfig;
  ...
  let skippedCfg: AforaConfig | undefined;
      ...
      mutate: async (draft) => {
        ...
        if (!agentExists) {
          const workspaceTemplate =
            dynamicCfg.workspaceTemplate ?? "~/.afora/workspace-{agentId}";
          const agentDirTemplate =
            dynamicCfg.agentDirTemplate ?? "~/.afora/agents/{agentId}/agent";
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as AforaConfig,
