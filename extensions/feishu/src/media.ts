...
import type { MessageReceipt } from "afora-agent/plugin-sdk/channel-outbound";
import { PlatformMessageNotDispatchedError } from "afora-agent/plugin-sdk/error-runtime";
import { detectMime, mediaKindFromMime } from "afora-agent/plugin-sdk/media-mime";
import {
...
import { saveMediaBuffer, type SavedMedia } from "afora-agent/plugin-sdk/media-store";
import type { ReplyPayloadTtsSupplement } from "afora-agent/plugin-sdk/reply-payload";
import { readRegularFile, writeExternalFileWithinRoot } from "afora-agent/plugin-sdk/security-runtime";
...
async function saveMessageResourceWithType(params: {
  ...
  const saved = await saveFeishuResponseMedia({
    response,
    tmpDirPrefix: "afora-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredAforaTmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
...
async function probeMediaDurationMs(params: {
  ...
  try {
    return await withTempWorkspace(
      { rootDir: resolvePreferredAforaTmpDir(), prefix: "feishu-media-probe-" },
      async (workspace) => {
        const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
        const inferredExt =
          ext && ext.length <= 12
