...
function resolveAforaVersion(): string {
  try {
 ...
 * and appends the Afora version.
 *
 * Format: "teams.ts[apps]/<sdk-version> Afora/<afora-version>"
 * Example: "teams.ts[apps]/2.0.5 Afora/2026.3.22"
 *
...
export function buildUserAgent(): string {
  ...
  cachedUserAgent = `teams.ts[apps]/${resolveTeamsSdkVersion()} Afora/${resolveAforaVersion()}`;
  return cachedUserAgent;
}
 ...
 * only contribute the Afora piece — passing the full `buildUserAgent()`
 * would double-print the SDK token.
 *
 * Format: "Afora/<afora-version>"
 */
export function buildAforaUserAgentFragment(): string {
  return `Afora/${resolveAforaVersion()}`;
}
