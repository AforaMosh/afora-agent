...
import { buildAforaUserAgentFragment } from "./user-agent.js";
...
type MSTeamsAppOn = {
  ...
  // delegates back into the SDK handlers after Afora sender policy passes.
...
async function createMSTeamsApp(
  ...
  // backend can identify Afora traffic for usage telemetry. Teams SDK
  ...
  const appOptions: Record<string, unknown> = {
    client: privateQaRuntime?.client ??
      options?.httpClient ?? {
        headers: { "User-Agent": buildAforaUserAgentFragment() },
