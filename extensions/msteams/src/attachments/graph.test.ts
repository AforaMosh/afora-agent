...
vi.mock("./shared.js", async (importOriginal) => {
  ...
  const { isRecord } = await import("afora-agent/plugin-sdk/string-coerce-runtime");
  return {
    ...actual,
...
describe("downloadMSTeamsGraphMedia hosted content $value fallback", () => {
  ...
  it("adds the Afora User-Agent to guarded Graph attachment fetches", async () => {
  ...
  it("adds the Afora User-Agent to Graph shares downloads for reference attachments", async () => {
    mockGraphMediaFetch({
      messageId: "msg-share",
      messageResponse: {
        body: {},
    ...
    expect(headers.get("User-Agent")).toMatch(/^teams\.ts\[apps\]\/.+ Afora\/.+$/);
  });
});
