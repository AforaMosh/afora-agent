...
export function buildComfyConfig(config: Record<string, unknown>): AforaConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): AforaConfig {
  return {
    models: {
      providers: {
        comfy: config,
