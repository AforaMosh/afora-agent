...
import { buildTypedExecApprovalPendingReplyPayload } from "afora-agent/plugin-sdk/approval-reply-runtime";
import {
...
import { withStateDirEnv } from "afora-agent/plugin-sdk/test-env";
...
describe("imessagePlugin contracts", () => {
  ...
  it.each([
    ...
    async ({ filename, contents, readerCalls }) => {
      await withStateDirEnv("afora-imessage-media-policy-", async ({ stateDir }) => {
  ...
  it("preserves provider-accepted attachment progress through actual durable core without replay", async () => {
    ...
    try {
      await withStateDirEnv("afora-imessage-durable-attachment-", async ({ stateDir }) => {
  ...
  it("halts native caption delivery when actual durable progress custody rejects", async () => {
    ...
    try {
      await withStateDirEnv("afora-imessage-durable-custody-", async ({ stateDir }) => {
        const workspaceDir = fs.realpathSync(stateDir);
