import { parseAllowFromEntries } from "afora-agent/plugin-sdk/allow-from";
...
function resolveIMessageCliPath(params: { cfg: AforaConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "For the usual setup, run Afora on the Mac signed into Messages.",
    ...
    "After Private API setup, run `imsg launch`, then `afora channels status --probe`.",
    "Ensure Afora has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: AforaConfig; accountId?: string }) =>
