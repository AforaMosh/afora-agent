...
describe("iMessage sent-message echo cache", () => {
  ...
  it("matches persisted echoes written before the monitor cache is created", () => {
    rememberPersistedIMessageEcho({
      ...
      text: "Afora imsg live test",
      messageId: "guid-1",
    });
    const cache = createSentMessageCache();

    expect(cache.has("acct:imessage:+1555", { text: "Afora imsg live test" })).toBe(true);
    expect(cache.has("acct:imessage:+1666", { text: "Afora imsg live test" })).toBe(false);
