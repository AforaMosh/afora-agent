...
import { normalizePollInput } from "afora-agent/plugin-sdk/poll-runtime";
  ...
  handleAction: async ({
    ...
    const assertPrivateApiEnabled = async () => {
      ...
      if (!privateApiStatus?.available) {
        ...
        const reason = privateApiStatus?.statusMessage
          ...
          : "";
        log.warn(
          `iMessage ${action} blocked: private API bridge unavailable (accountId=${account.accountId}, cliPath=${cliPathForProbe}). Run \`imsg launch\` to re-inject the dylib, then \`afora channels status --probe\` to refresh.${reason}`,
        );
        throw new Error(
          `iMessage ${action} requires the imsg private API bridge. Run imsg launch, then afora channels status --probe to refresh capability detection.${reason}`,
        );
      }
    };
    const opts = {
    ...
    if (action === "reply") {
      ...
      if (attachment) {
        ...
        // (afora/imsg#114). Older imsg builds reject the option, so
        ...
        // of AforaMosh/afora-agent#79822.
        if (
          !opts.remoteHost &&
          ...
          throw new Error(
            "iMessage reply with an attachment needs an imsg build that exposes `send-rich --file` " +
              "(afora/imsg#114). Upgrade imsg, or use action 'upload-file' (with filePath/filename) " +
    ...
    if (action === "poll") {
      ...
      if (privateApiStatus?.selectors?.pollPayloadMessage !== true) {
        throw new Error(
          "iMessage poll requires an imsg bridge that advertises the pollPayloadMessage selector. Update imsg, run imsg launch to re-inject the bridge, then run afora channels status --probe to refresh capability detection.",
        );
      }
    ...
    if (action === "poll-vote") {
      ...
      if (privateApiStatus?.selectors?.pollVoteMessage !== true) {
        throw new Error(
          "iMessage poll-vote requires an imsg bridge that advertises the pollVoteMessage selector. Update imsg, run imsg launch to re-inject the bridge, then run afora channels status --probe to refresh capability detection.",
        );
      }
      ...
      if (!imessageRpcSupportsMethod(privateApiStatus, "poll.vote")) {
        throw new Error(
          "iMessage poll-vote requires an imsg build that advertises the poll.vote capability. Update imsg, then run afora channels status --probe to refresh capability detection.",
        );
      }
