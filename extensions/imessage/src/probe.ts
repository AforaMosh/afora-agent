...
import type { BaseProbeResult } from "afora-agent/plugin-sdk/channel-contract";
import {
  asDateTimestampMs,
...
import { detectBinary } from "afora-agent/plugin-sdk/setup";
import {
  filterStringEntries,
...
function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run Afora on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by afora/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // afora/imsg#114. We do this even when the bridge is unavailable
    ...
    // Caption suppression is required for approval polls because Afora
