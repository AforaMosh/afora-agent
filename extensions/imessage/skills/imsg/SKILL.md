---
name: imsg
description: "Use the imsg CLI from Afora agents for iMessage/SMS DMs, groups, replies, reactions, polls, watching, and private-API actions."
...
Do not use this skill for Telegram, Signal, WhatsApp, Discord, Slack, or for replying inside the current Afora conversation when the configured channel already routes the reply.
...
- Private API mode is strongly encouraged for Afora iMessage. It unlocks replies, precise tapbacks, effects, polls, attachment replies, read/typing actions, and group management. Basic mode is a fallback for reads plus plain text/file send.
- Full Disk Access for the process context that runs `imsg` or Afora; reads fail without Messages DB access.
...
Use the private API bridge for the native iMessage actions Afora users normally expect:
...
For Afora channel setup, check bridge availability early:
