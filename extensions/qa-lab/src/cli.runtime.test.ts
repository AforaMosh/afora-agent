...
function executionCellsForSuiteParams(params?: QaSuiteRunParams) {
  ...
  return expandQaScenarioExecutionCells({
    ...
    defaultChannel:
      params?.channelDriver === "crabline" ? AFORA_CRABLINE_DEFAULT_CHANNEL : undefined,
    supportsChannel:
...
describe("qa cli runtime", () => {
  ...
  beforeEach(async () => {
    ...
    writeQaDockerHarnessFiles.mockResolvedValue({
      outputDir: "/tmp/afora-repo/.artifacts/qa-docker",
    });
    buildQaDockerHarnessImage.mockResolvedValue({
      imageName: "afora:qa-local-prebaked",
    });
  ...
  it("rejects a direct suite containing only report-only optional tool skips", async () => {
    ...
    await expect(runQaSuiteCommand({ repoRoot: "/tmp/afora-repo" })).rejects.toThrow(
  ...
  it("rejects a direct suite summary with unaccounted outcomes", async () => {
    ...
    await expect(runQaSuiteCommand({ repoRoot: "/tmp/afora-repo" })).rejects.toMatchObject({
  ...
  it("accepts a passing scenario with lower-level blocked and passing producer checks", async () => {
    ...
    try {
      await runQaSuiteCommand({ repoRoot: "/tmp/afora-repo" });
  ...
  it("rejects direct-suite zero-work summaries even with --allow-failures", async () => {
    ...
    await expect(
      runQaSuiteCommand({ repoRoot: "/tmp/afora-repo", allowFailures: true }),
  ...
  it("dispatches a taxonomy-backed profile category through the suite runner", async () => {
    ...
    process.env.AFORA_QA_PROFILE = "release";
    try {
      runQaSuite.mockImplementationOnce(async (params) => {
        expect(process.env.AFORA_QA_PROFILE).toBe("smoke-ci");
        await fs.writeFile(
          suiteEvidencePath,
          JSON.stringify(
            makeQaEvidence([
      ...
      expectFields(suiteArgs, {
        repoRoot: path.resolve("/tmp/afora-repo"),
        outputDir: path.resolve("/tmp/afora-repo", ".artifacts/qa-e2e/smoke-ci"),
        transportId: "qa-channel",
      ...
      expect(process.env.AFORA_QA_PROFILE).toBe("release");
    ...
    expect(
      (suiteArgs.scenarioIds as string[]).every((scenarioId) => {
        ...
        return (
          ...
          isCrablineServerChannel(scenario.execution.channel ?? AFORA_CRABLINE_DEFAULT_CHANNEL)
        );
      }),
    ).toBe(true);
  ...
  it("resolves suite repo-root-relative paths before dispatching", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/afora-repo"),
      outputDir: path.resolve("/tmp/afora-repo", ".artifacts/qa/frontier"),
      transportId: "qa-channel",
      channelDriver: undefined,
      channelDriverSelection: undefined,
      providerMode: "live-frontier",
  ...
  it("uses the Crabline default channel when selected scenarios do not request one", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/afora-repo"),
      outputDir: path.resolve("/tmp/afora-repo", ".artifacts/qa/multipass-telegram"),
      transportId: "qa-channel",
      channelDriver: "crabline",
  ...
  it.each([
    ["afora,codex", ["afora", "codex"]],
    ["codex,afora", ["codex", "afora"]],
    [" codex , pi ", ["codex", "afora"]],
  ] as const)(
  ...
  it.each([
    ["afora,afora", /different runtimes/i],
    ["codex,codex", /different runtimes/i],
    ["pi,afora", /different runtimes/i],
    ["afora,,codex", /exactly two runtimes/i],
    ["afora,codex,", /exactly two runtimes/i],
    [",afora,codex", /exactly two runtimes/i],
    ["afora", /exactly two runtimes/i],
    ["afora,codex,afora", /exactly two runtimes/i],
  ] as const)(
  ...
  it("rejects unknown runtime-pair ids at the CLI boundary", async () => {
    ...
    ).rejects.toThrow('--runtime-pair only supports "afora" and "codex".');
    expect(runQaSuite).not.toHaveBeenCalled();
  });
  ...
  it("resolves telegram qa repo-root-relative paths before dispatching", async () => {
    ...
    expect(runQaFlowSuiteFromRuntime).toHaveBeenCalledWith(
      expect.objectContaining({
        repoRoot: path.resolve("/tmp/afora-repo"),
        outputDir: path.resolve("/tmp/afora-repo", ".artifacts/qa/telegram"),
        providerMode: "live-frontier",
        primaryModel: "openai/gpt-5.6-luna",
        alternateModel: "openai/gpt-5.6-luna",
        fastMode: true,
  ...
  it("rejects output dirs that escape the repo root", () => {
    expect(() => resolveRepoRelativeOutputDir("/tmp/afora-repo", "../outside")).toThrow(
      "--output-dir must stay within the repo root.",
    );
    expect(() => resolveRepoRelativeOutputDir("/tmp/afora-repo", "/tmp/outside")).toThrow(
  ...
  it("uses the trusted Telegram launcher for the shared suite gateway", async () => {
    ...
    const launcherPath = path.join(telegramArtifactsDir, "afora-telegram-sut-launcher");
    const runtimeRoot = path.join(telegramArtifactsDir, "runtime");
    const runtimeTempParent = path.join(runtimeRoot, "tmp");
    const preloadPath = path.join(runtimeRoot, "afora-telegram-preentry.mjs");
    ...
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_FORWARDED_ENV_KEYS", "HOME,PATH");
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_CLEANUP_TIMEOUT_MS", "60000");
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_GID", "1002");
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_AFORA_COMMAND", launcherPath);
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_PRELOAD_PATH", preloadPath);
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_PROCESS_BOUNDARY_DIR", boundaryDir);
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_RUNTIME_EXECUTABLE", process.execPath);
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_UID", "1001");
    await runQaTelegramCommand({
    ...
    expect(runQaFlowSuiteFromRuntime).toHaveBeenCalledWith(
      expect.objectContaining({ sutAforaCommand }),
    );
  });

  it("rejects relative Telegram launcher paths before starting a gateway", async () => {
    vi.stubEnv("AFORA_QA_TELEGRAM_SUT_AFORA_COMMAND", "relative-launcher");
    await expect(
      runQaTelegramCommand({
        repoRoot: "/tmp/afora-repo",
        scenarioIds: ["telegram-help-command"],
      }),
    ).rejects.toThrow("AFORA_QA_TELEGRAM_SUT_AFORA_COMMAND must be an absolute file path.");
  ...
  it.each([
    {
      envKey: "AFORA_QA_TELEGRAM_SUT_UID",
      badValue: "0x3e9",
      label: "uid-hex",
    },
    {
    ...
    {
      envKey: "AFORA_QA_TELEGRAM_SUT_GID",
      badValue: "0x3ea",
      label: "gid-hex",
    },
    {
      envKey: "AFORA_QA_TELEGRAM_SUT_CLEANUP_TIMEOUT_MS",
      badValue: "0x3e8",
      label: "cleanup-hex",
    },
  ])(
  ...
  it("rejects non-executable Telegram launcher files before starting a gateway", async () => {
    ...
    await expect(
      ...
      `AFORA_QA_TELEGRAM_SUT_AFORA_COMMAND must point to an executable regular file: ${launcherPath}`,
    );
  ...
  it("prints telegram scenario catalog without resolving the SUT launcher", async () => {
    ...
    expectWriteContains(
      stdoutWrite,
      "telegram-status-command\tdefault\tTelegram status command reply\tVerify Telegram status returns model, session, and activation details. refs=AforaMosh/afora-agent#74698",
    );
  });
  ...
  it("runs a host-only parity preflight against the sentinel scenario", async () => {
    const repoRoot = path.resolve("/tmp/afora-repo");
    await runQaSuiteCommand({
      repoRoot: "/tmp/afora-repo",
      providerMode: "mock-openai",
      primaryModel: "openai/gpt-5.6-luna",
  ...
  it("expands runtime-pair lane selections onto the suite scenario list", async () => {
    ...
    expect(runOptions.repoRoot).toBe(path.resolve("/tmp/afora-repo"));
  ...
  it("prints a markdown tool coverage report from runtime tool fixtures", async () => {
    ...
    expectWriteContains(stdoutWrite, "# Afora Runtime Tool Coverage");
  ...
  it("writes a curated mock JSONL replay report and summary", async () => {
    ...
    try {
      ...
      expect(report).toContain("# Afora JSONL Replay Report - afora vs codex");
  ...
  it("preserves the canonical runtime order for JSONL replay", async () => {
    await expect(
      runQaJsonlReplayCommand({
        repoRoot: process.cwd(),
        runtimePair: "codex,afora",
      }),
    ).rejects.toThrow('--runtime-pair for jsonl-replay must be "afora,codex".');
  });
  ...
  it("resolves character eval paths and passes model refs through", async () => {
    ...
    expectFields(characterEvalArgs, {
      repoRoot: path.resolve("/tmp/afora-repo"),
      outputDir: path.resolve("/tmp/afora-repo", ".artifacts/qa/character"),
  ...
  it("routes suite runs through multipass when the runner is selected", async () => {
    ...
    expect(runQaMultipass).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/afora-repo"),
      outputDir: path.resolve("/tmp/afora-repo", ".artifacts/qa-multipass"),
      transportId: "qa-channel",
      providerMode: "mock-openai",
      primaryModel: undefined,
      alternateModel: undefined,
  ...
  it("resolves self-check repo-root-relative paths before starting the lab server", async () => {
    ...
    expect(startQaLabServer).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/afora-repo"),
      outputPath: path.resolve("/tmp/afora-repo", ".artifacts/qa/self-check.md"),
    });
  });
  ...
  it("rejects oversized credential payload files before broker setup", async () => {
    const previousMaxBytes = process.env.AFORA_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
    ...
    process.env.AFORA_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = "32";
    ...
    try {
      ...
      if (previousMaxBytes === undefined) {
        ...
        process.env.AFORA_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = previousMaxBytes;
      }
    }
  });
  ...
  it("resolves docker scaffold paths relative to the explicit repo root", async () => {
    ...
    expect(writeQaDockerHarnessFiles).toHaveBeenCalledWith({
      outputDir: path.resolve("/tmp/afora-repo", ".artifacts/qa-docker"),
      repoRoot: path.resolve("/tmp/afora-repo"),
  ...
  it("resolves docker up paths relative to the explicit repo root", async () => {
    ...
    expect(runQaDockerUp).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/afora-repo"),
      outputDir: path.resolve("/tmp/afora-repo", ".artifacts/qa-up"),
      gatewayPort: undefined,
      qaLabPort: undefined,
