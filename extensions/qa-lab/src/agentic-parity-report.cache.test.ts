...
describe("qa runtime parity prompt-cache reporting", () => {
  it("reports the exact turn of a measured cache miss without inventing missing telemetry", () => {
    ...
    expect(report.scenarios[0]?.aforaCacheDiagnostics).toBeUndefined();
    expect(renderQaRuntimeParityMarkdownReport(report)).toContain(
      "post-warm cache misses: afora N/A; codex turn 2 (24448 uncached input)",
    );
  });
  ...
  it("reports unknown post-warm turns without hiding measured cache misses", () => {
    ...
    expect(renderQaRuntimeParityMarkdownReport(report)).toContain(
      "post-warm cache misses: afora N/A; codex turn 2 (1050 uncached input); unmeasured turns 3",
    );
  });
  ...
  it("preserves unknown warm turns when no turn has complete cache telemetry", () => {
    ...
    expect(renderQaRuntimeParityMarkdownReport(report)).toContain(
      "post-warm cache misses: afora N/A; codex N/A (unmeasured turns 2)",
    );
  });
  ...
  it("reports cached, uncached, and cache-write input without counting output as cacheable", () => {
    ...
    scenario.runtimeParity.cells.afora.usage = {
      inputTokens: 10,
      outputTokens: 5,
      totalTokens: 85,
      cacheRead: 60,
    ...
    expect(report.scenarios[0]?.aforaUsage).toEqual({
      totalTokens: 85,
      inputTokens: 10,
      outputTokens: 5,
      grossInputTokens: 80,
    ...
    expect(report.usage.afora).toMatchObject({
      totalTokens: 100,
      grossInputTokens: 90,
    ...
    expect(report.usage.afora?.cacheHitPercent).toBeCloseTo((60 / 90) * 100);
    ...
    expect(markdown).toContain("| afora | 90 | 30 | 60 | 10 | 10 | 100 | 66.7% |");
    expect(markdown).toContain("| codex | 25 | 17 | 8 | 0 | 8 | 33 | 32.0% |");
    expect(markdown).toContain(
      "prompt cache: afora 75.0% (60 cached, 20 uncached input); codex 50.0% (8 cached, 8 uncached input)",
    );
  });
  ...
  it("computes aggregate cache hits from measured input rather than averaging percentages", () => {
    ...
    first.runtimeParity.cells.afora.usage = {
      inputTokens: 10,
      outputTokens: 5,
      totalTokens: 85,
      cacheRead: 60,
    ...
    second.runtimeParity.cells.afora.usage = {
      inputTokens: 10,
      outputTokens: 5,
      totalTokens: 115,
      cacheRead: 100,
    ...
    expect(report.usage.afora?.cacheHitPercent).toBeCloseTo((160 / 190) * 100);
  });
  ...
  it("reports unavailable runtime captures as N/A instead of fabricated zero-token usage", () => {
    const report = buildQaRuntimeParityReport({
      summary: {
        ...
        run: { providerMode: "live-frontier", runtimePair: ["afora", "codex"] },
      },
    });
    expect(report.usage).toEqual({ afora: null, codex: null });
    expect(report.scenarios[0]).toMatchObject({ aforaUsage: null, codexUsage: null });
    const markdown = renderQaRuntimeParityMarkdownReport(report);
    expect(markdown).toContain("| afora | N/A | N/A | N/A | N/A | N/A | N/A | N/A |");
  ...
  it("keeps missing cache telemetry unknown while preserving measured token totals", () => {
    ...
    expect(report.scenarios[0]?.aforaUsage).toMatchObject({
      inputTokens: 10,
      outputTokens: 5,
      totalTokens: 15,
    ...
    expect(renderQaRuntimeParityMarkdownReport(report)).toContain(
      "| afora | N/A | N/A | N/A | N/A | 10 | 30 | N/A |",
    );
  });
