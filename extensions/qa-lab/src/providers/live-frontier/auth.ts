...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "AFORA_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "AFORA_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "AFORA_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["AFORA_LIVE_ANTHROPIC_KEY"],
  gemini: ["AFORA_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "AFORA_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: AforaConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: AforaConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: AforaConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.AFORA_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "afora") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or AFORA_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host Afora OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
