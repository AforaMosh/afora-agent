...
const QA_COMPACTION_RETRY_CODE_MODE_WRITE_RESULT = {
  ...
  telemetry: {
    catalogSize: 32,
    sources: { afora: 32, mcp: 0, client: 0 },
...
const WHATSAPP_GROUP_AGENT_REACT_PROMPT =
  "aforaqa react to this WhatsApp group message with thumbs up for QA action check WHATSAPP_QA_GROUP_AGENT_REACT_TEST.";
...
const WHATSAPP_GROUP_AGENT_UPLOAD_PROMPT =
  `aforaqa use the WhatsApp message tool upload-file action to send a PNG with caption ${WHATSAPP_GROUP_AGENT_UPLOAD_TOKEN}. ` +
  "Do not send any visible text reply after the upload.";
...
const WHATSAPP_PENDING_HISTORY_TRIGGER_PROMPT = [
  "aforaqa pending history context check",
...
const WHATSAPP_BROADCAST_PROMPT = `aforaqa broadcast fanout check ${WHATSAPP_BROADCAST_TOKEN}`;
...
describe("qa mock openai server", () => {
  ...
  it("honors exact replies after QA kickoff reads without marker wording", async () => {
    ...
    const final = await expectNonStreamingResponsesJson<{
      ...
      input: [
        ...
        makeToolOutputWithCallId(
          "call_mock_read_1",
          JSON.stringify({ text: "QA mission: understand this Afora repo." }),
        ),
      ],
    });
  ...
  it.each([
    {
      name: "preview",
      prompt:
        "@afora:matrix-qa.test Tool progress QA check: call the read tool exactly once on `QA_KICKOFF_TASK.md` before answering. After that read completes, reply exactly `CURRENT_PREVIEW_OK`.",
      toolName: "read",
    ...
    },
    {
      ...
      prompt:
        "@afora:matrix-qa.test Tool progress QA check: call the exec tool exactly once with this exact command before answering: `printf 'matrix-command-progress-start\\n'; sleep 2`. After that exec command completes or fails, reply exactly `CURRENT_COMMAND_OK`.",
      toolName: "exec",
    ...
    },
    {
      name: "error",
      prompt:
        "@afora:matrix-qa.test Tool progress error QA check: read `missing-matrix-tool-progress-target.txt` before answering. After the read fails, reply exactly `CURRENT_ERROR_OK`.",
      toolName: "read",
    ...
    },
    {
      name: "mention safety",
      prompt:
        "@afora:matrix-qa.test Tool progress QA check: read the missing workspace file `matrix-progress-@room-@alice:matrix-qa.test-!room:matrix-qa.test.txt` before answering. After that read fails, reply exactly `CURRENT_MENTION_OK`.",
      toolName: "read",
      expectedArgs: {
    ...
    const payload = await expectNonStreamingResponsesJson(server, {
      input: [
        makeUserInput(
          "@afora:matrix-qa.test Quiet streaming QA check: reply exactly `STALE_STREAMING_OK`.",
        ),
  ...
  it("does not let a prior Matrix tool result satisfy the current progress prompt", async () => {
    ...
    const payload = await expectNonStreamingResponsesJson(server, {
      input: [
        makeUserInput(
          "@afora:matrix-qa.test Tool progress QA check: read `stale-progress-target.txt` before answering. Reply exactly `STALE_PROGRESS_OK`.",
        ),
        makeToolOutputWithCallId("call_mock_read_stale_progress", "STALE_PROGRESS_OK"),
        makeUserInput(
          "@afora:matrix-qa.test Tool progress QA check: read `current-progress-target.txt` before answering. Reply exactly `CURRENT_PROGRESS_OK`.",
        ),
      ],
    });
  ...
  it.each([
    {
      ...
      stalePrompt:
        "@afora:matrix-qa.test Quiet streaming QA check: reply exactly `STALE_QUIET_OK`.",
    },
    {
      name: "tool progress",
      stalePrompt:
  ...
  it("uses the current tool result marker after stale streaming history", async () => {
    const server = await startMockServer();
    const currentPrompt =
      "@afora:matrix-qa.test Tool progress QA check: call the read tool exactly once on `QA_KICKOFF_TASK.md` before answering. The only valid final marker is inside that file.";
    const payload = await expectNonStreamingResponsesJson(server, {
      input: [
        makeUserInput(
          "@afora:matrix-qa.test Quiet streaming QA check: reply exactly `STALE_STREAMING_OK`.",
  ...
  it("selects tool progress after quiet streaming in one user envelope", async () => {
    ...
    const currentPrompt =
      "@afora:matrix-qa.test Tool progress QA check: call the read tool exactly once on `QA_KICKOFF_TASK.md` before answering. The only valid final marker is inside that file.";
    const envelope = [
      "@afora:matrix-qa.test Quiet streaming QA check: reply exactly `STALE_ENVELOPE_OK`.",
      currentPrompt,
    ].join("\n");
      ...
      input: [
        makeUserInput(
          [
            ...
            "",
            `[Slack Afora (this assistant) (assistant) Fri 2026-07-31 10:01 UTC] ${botReplyMarker}`,
  ...
  it("answers WhatsApp pending-history prompts only with injected prior group context", async () => {
    ...
    const contextWithoutCurrentTrigger = await expectOpenAiNonStreamingResponsesJson(server, {
      input: [
        makeUserInput(
          [historyContext, "aforaqa pending history context check without current trigger"].join(
            "\n",
          ),
        ),
      ],
  ...
  it("answers reply-to-bot seed and implicit quoted-trigger markers deterministically", async () => {
    ...
    expect(WHATSAPP_REPLY_TO_BOT_TRIGGER_PROMPT).not.toMatch(/\baforaqa\b/iu);
...
Update and merge these partial structured summaries.`,
  ...
  it("plans the write from an Afora compacted retry payload", async () => {
    const server = await startMockServer();
    const runtimeSessionId = "compaction-afora-retry";
    const initial = await postNonStreamingResponses(server, {
      model: "gpt-5.6-luna",
      instructions: `Runtime: embedded | sessionId=${runtimeSessionId}`,
      input: [
  ...
  it("requires memory_get before answering thread recall in Code Mode", async () => {
    const server = await startMockServer();
    const prompt =
      "@afora Thread memory check: what is the hidden thread codename stored only in memory? Use memory tools first and reply only in this thread.";
    const codeModeTools = [
      {
        type: "function",
        name: "exec",
  ...
  it.each([
    ...
    async (instructionSource) => {
      ...
      const instructions = usesCodexDelivery
        ? "Visible source replies are not automatically delivered for this run. Use `message(action=send)` for user-visible source-channel output. For progress, set `final=false`. When the message is the completed reply to the current source conversation, set `final=true`; Afora stops after confirming delivery."
  ...
  it("uses WhatsApp structured markers for metadata-prefixed message bodies", async () => {
    ...
    const contactResponse = await expectNonStreamingResponses(server, {
      input: [
        ...
        previousExactMarkerInput,
        makeUserInput(
          ["Sender: ⟦afora:ctx⟧", "```json", '{"name":"QA"}', "```", "", "<contact>"].join("\n"),
        ),
      ],
    });
    const stickerResponse = await expectNonStreamingResponses(server, {
  ...
  it("plans QA tool-search calls for instruction-declared Codex dynamic tools", async () => {
    ...
    const response = await expectNonStreamingResponses(server, {
      instructions: "Codex dynamic Afora tools available in this turn: web_search.",
      input: [
        makeUserInput(
    ...
    expect(String(toolPlanOutput.arguments)).toContain("Afora runtime parity fixed query");
  });
      ...
      input: [
        ...
        makeToolOutputWithCallId(
          ...
          JSON.stringify({
            ...
            value: {
              tool: {
                id: `afora:tool-search-e2e-fixture:${targetTool}`,
                source: "afora",
  ...
  it("plans QA tool-search failure calls with denied-input args", async () => {
    ...
    expect(toolPlanOutput.type).toBe("function_call");
    expect(toolPlanOutput.name).toBe("web_search");
    expect(String(toolPlanOutput.arguments)).toContain("AFORA_QA_WEB_SEARCH_DENIED_INPUT");
  });
  ...
  it("plans Codex Responses Lite handoff from declared developer additional tools", async () => {
    ...
    expect(toolPlanOutput.namespace).toBe("afora");
  });
  ...
  it("returns continuity language after the model-switch reread completes", async () => {
    ...
    const response = await expectNonStreamingResponses(server, {
      ...
      input: [
        ...
        makeToolOutput(
          "QA mission: Understand this Afora repo from source + docs before acting.",
        ),
      ],
    });
  ...
  it("serves deterministic WhatsApp group audio transcription for the trigger fixture", async () => {
    ...
    await expect(triggered.json()).resolves.toEqual({
      text: "aforaqa reply with only this exact marker after group audio preflight: WHATSAPP_QA_GROUP_AUDIO_TRANSCRIPT_OK",
    });
