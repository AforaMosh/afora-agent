...
export const QA_EVIDENCE_SUMMARY_KIND = "afora.qa.evidence-summary";
...
export function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["AFORA_E2E_PROFILE", params.env?.AFORA_E2E_PROFILE],
    ["AFORA_QA_PROFILE", params.env?.AFORA_QA_PROFILE],
  ] as const;
...
function resolveQaEvidenceRunner(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  return params.env?.AFORA_QA_RUNNER?.trim() || params.fallback || "host";
}

function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.AFORA_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.AFORA_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
  const explicitKind = env?.AFORA_QA_PACKAGE_SOURCE_KIND?.trim();
  const kind =
    explicitKind ||
