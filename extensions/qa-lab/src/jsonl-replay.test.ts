...
describe("jsonl replay", () => {
  ...
  it("reports the earliest divergent turn using runtime parity drift classes", async () => {
      ...
      if (turn.turn === 2) {
        return {
          ...
          cell: makeCell(runtime, {
            toolCalls: [makeToolCall(runtime === "afora" ? {} : { argsHash: "args-codex" })],
          }),
        };
      }
      if (turn.turn === 3) {
        return {
          status: "pass",
          cell: makeCell(runtime, {
            finalText: runtime === "afora" ? "afora wording" : "codex wording",
          }),
        };
      }
      return {
    ...
    expect(result.transcripts[0]?.cells.afora).toHaveLength(3);
