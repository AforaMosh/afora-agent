...
function runtimeParityScenarioResultStatus(result: RuntimeParityResult) {
  const cellStatuses = new Set([
    runtimeParityScenarioStepStatus(result.cells.afora),
...
export function buildRuntimeParityScenarioResult(params: {
  ...
  const aforaCell = params.result.cells.afora;
  ...
  return {
    ...
    steps: [
      {
        name: aforaCell.runtime,
        status: runtimeParityScenarioStepStatus(aforaCell),
        details: formatRuntimeParityScenarioCellDetails(aforaCell),
      },
      {
