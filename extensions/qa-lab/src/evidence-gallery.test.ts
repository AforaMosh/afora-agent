...
describe("evidence gallery", () => {
  ...
  it("sanitizes local roots from gallery failure reasons", async () => {
    ...
    evidence.entries[0] = {
      ...
      result: {
        ...
        failure: {
          class: "blocked",
          reason: `Command failed at ${repoRoot}/afora.mjs and file://${repoRoot}/trace.log`,
        },
      },
    };
    ...
    expect(failureModelEntry.failureReason).toBe(
      "Command failed at <repo-root>/afora.mjs and file://<repo-root>/trace.log",
    );
  ...
  it("resolves evidence and declared artifacts inside the repo root only", async () => {
    ...
    await expect(
      buildQaEvidenceGalleryModel({
        evidencePath: "/tmp/not-afora-evidence.json",
        repoRoot,
      }),
