...
const QA_CREDENTIALS_ALLOW_INSECURE_HTTP_ENV_KEY = "AFORA_QA_ALLOW_INSECURE_HTTP";
...
export function normalizeQaCredentialConvexSiteUrl(params: {
  ...
  try {
    ...
    throw createError(
      `AFORA_QA_CONVEX_SITE_URL must be a valid URL, got "${params.raw || "<empty>"}".`,
    );
  }
  ...
  if (url.protocol !== "http:") {
    throw createError("AFORA_QA_CONVEX_SITE_URL must use https://.");
  }
  ...
  if (!allowInsecureHttp || !isLoopbackHost(url.hostname)) {
    throw createError(
      `AFORA_QA_CONVEX_SITE_URL must use https://. http:// is only allowed for loopback hosts when ${QA_CREDENTIALS_ALLOW_INSECURE_HTTP_ENV_KEY}=1.`,
    );
  }
  const text = url.toString();
  return text.endsWith("/") ? text.slice(0, -1) : text;
