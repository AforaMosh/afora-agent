...
function createSelection(channel: AforaCrablineChannelDriverSelection["channel"] = "telegram") {
  return {
...
describe("crabline transport", () => {
  ...
  it("configures Afora's Telegram plugin against a Crabline local provider server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      const transport = await createQaCrablineTransportAdapter({
        outputDir,
        selection: createSelection(),
  ...
  it("configures Afora's Slack plugin against a Crabline local provider server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      const transport = await createQaCrablineTransportAdapter({
        outputDir,
  ...
  it("configures Afora's WhatsApp plugin against a Crabline Baileys WebSocket server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      const transport = await createQaCrablineTransportAdapter({
        outputDir,
      ...
      try {
        ...
        expect(env).toMatchObject({
          ...
          AFORA_WHATSAPP_WEB_SOCKET_URL: expect.stringMatching(
    ...
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      ...
      try {
        ...
        expect(transport.createRuntimeEnvPatch?.()).toMatchObject({
          ...
          MATRIX_USER_ID: "@afora:matrix.test",
        });
        ...
        expect(() => transport.buildAgentDelivery({ target: "thread:/v1/main/%24event" })).toThrow(
          "Matrix thread targets require Afora QA thread forwarding",
        );
        await expect(
          transport.state.addInboundMessage({
