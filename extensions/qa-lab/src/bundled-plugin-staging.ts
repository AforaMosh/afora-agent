...
function findQaBundledPluginDirsByManifestId(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of readdirSync(sourceRoot, { withFileTypes: true }).toSorted((left, right) =>
      ...
      const manifestId = readQaBundledManifestId(path.join(candidate, "afora.plugin.json"));
...
function resolveQaBundledPluginManifestPath(params: {
  ...
  return manifestDir ? path.join(manifestDir, "afora.plugin.json") : null;
}
...
export async function resolveQaOwnerPluginIdsForProviderIds(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of await fs.readdir(sourceRoot, { withFileTypes: true })) {
      ...
      const manifestPath = path.join(sourceRoot, entry.name, "afora.plugin.json");
...
export async function resolveQaRuntimeHostVersion(params: {
  ...
  for (const pluginId of stagedPluginIds) {
    ...
    const candidate = parseStableSemverFloor(packageJson.afora?.install?.minHostVersion);
...
export async function createQaBundledPluginsDir(params: {
  ...
  const stagedAforaPackageDir = path.join(stagedRoot, "node_modules", "afora");
  await fs.mkdir(stagedAforaPackageDir, { recursive: true });
  await fs.copyFile(
    path.join(params.repoRoot, "package.json"),
    path.join(stagedAforaPackageDir, "package.json"),
  );
  ...
  for (const pluginId of stagedPluginIds) {
    ...
    if (manifestPath) {
      await fs.copyFile(manifestPath, path.join(targetDir, "afora.plugin.json"));
    }
  }
  await symlinkQaStagedDirEntry({
    sourcePath: path.join(stagedRoot, "dist"),
    targetPath: path.join(stagedAforaPackageDir, "dist"),
    directory: true,
  });
  return {
    bundledPluginsDir,
