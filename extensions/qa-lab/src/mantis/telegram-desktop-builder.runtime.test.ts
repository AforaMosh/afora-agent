...
describe("mantis Telegram desktop builder runtime", () => {
  ...
  it("leases a desktop box, installs Telegram Desktop, configures Afora, and keeps the gateway lease", async () => {
    ...
    expect(runCommand?.env?.AFORA_LIVE_OPENAI_KEY).toBe("openai-runtime-key");
    expect(runCommand?.env?.AFORA_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64).toBe(
      "profile-archive",
    );
    expect(runCommand?.env?.AFORA_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN).toBe("driver-token");
    expect(runCommand?.env?.AFORA_MANTIS_TELEGRAM_GROUP_ID).toBe("-1001234567890");
    expect(runCommand?.env?.AFORA_MANTIS_TELEGRAM_SUT_BOT_TOKEN).toBe("sut-token");
    ...
    expect(remoteScript).toContain("AFORA_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64");
    expect(remoteScript).toContain(
  ...
  it("leases Convex Telegram credentials and maps them into the VM env", async () => {
    ...
    expect(runCommand?.env?.AFORA_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN).toBe("driver-leased");
    expect(runCommand?.env?.AFORA_MANTIS_TELEGRAM_GROUP_ID).toBe("-100222333444");
    expect(runCommand?.env?.AFORA_MANTIS_TELEGRAM_SUT_BOT_TOKEN).toBe("sut-leased");
    expect(runCommand?.env?.AFORA_QA_TELEGRAM_DRIVER_BOT_TOKEN).toBe("driver-leased");
    expect(runCommand?.env?.AFORA_QA_TELEGRAM_GROUP_ID).toBe("-100222333444");
    expect(runCommand?.env?.AFORA_QA_TELEGRAM_SUT_BOT_TOKEN).toBe("sut-leased");
    expect(
      commands.some((entry) => entry.command === "/tmp/crabbox" && entry.args[0] === "stop"),
    ).toBe(true);
