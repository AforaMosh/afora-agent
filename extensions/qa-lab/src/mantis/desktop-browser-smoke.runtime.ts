...
const DEFAULT_BROWSER_URL = "https://afora.ai";
...
const BROWSER_PROFILE_ARCHIVE_ENV = "AFORA_MANTIS_BROWSER_PROFILE_TGZ_B64";
const BROWSER_PROFILE_DIR_ENV = "AFORA_MANTIS_BROWSER_PROFILE_DIR";
...
if [ -n "$profile_archive_b64" ]; then
  profile_archive="$profile/afora-mantis-browser-profile.tgz"
...
export async function runMantisDesktopBrowserSmoke(
  ...
  const browserProfileArchiveEnv =
    ...
    trimToValue(env.AFORA_MANTIS_BROWSER_PROFILE_ARCHIVE_ENV) ??
  ...
  const remoteOutputDir = `/tmp/afora-mantis-desktop-${startedAt
    .toISOString()
