...
const DEFAULT_MANTIS_TOKEN_ENV = "AFORA_QA_DISCORD_MANTIS_BOT_TOKEN";
const DEFAULT_MANTIS_TOKEN_FILE_ENV = "AFORA_QA_DISCORD_MANTIS_BOT_TOKEN_FILE";
const DEFAULT_GUILD_ID_ENV = "AFORA_QA_DISCORD_GUILD_ID";
const DEFAULT_CHANNEL_ID_ENV = "AFORA_QA_DISCORD_CHANNEL_ID";
const QA_REDACT_PUBLIC_METADATA_ENV = "AFORA_QA_REDACT_PUBLIC_METADATA";
...
function assertMantisDiscordChannelInGuild(params: {
  ...
  if (!params.guildChannels.some((channel) => channel.id === params.channelId)) {
    throw new Error(
      `AFORA_QA_DISCORD_CHANNEL_ID ${params.channelId} is not in guild ${params.guildId}.`,
    );
  }
  if (params.channel.guild_id && params.channel.guild_id !== params.guildId) {
    throw new Error(
      `AFORA_QA_DISCORD_CHANNEL_ID ${params.channelId} belongs to guild ${params.channel.guild_id}, not ${params.guildId}.`,
    );
  }
}
