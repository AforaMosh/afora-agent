...
const CRABBOX_BIN_ENV = "AFORA_MANTIS_CRABBOX_BIN";
const CRABBOX_PROVIDER_ENV = "AFORA_MANTIS_CRABBOX_PROVIDER";
const CRABBOX_CLASS_ENV = "AFORA_MANTIS_CRABBOX_CLASS";
const CRABBOX_MARKET_ENV = "AFORA_MANTIS_CRABBOX_MARKET";
const CRABBOX_LEASE_ID_ENV = "AFORA_MANTIS_CRABBOX_LEASE_ID";
const CRABBOX_KEEP_ENV = "AFORA_MANTIS_KEEP_VM";
const CRABBOX_IDLE_TIMEOUT_ENV = "AFORA_MANTIS_CRABBOX_IDLE_TIMEOUT";
const CRABBOX_TTL_ENV = "AFORA_MANTIS_CRABBOX_TTL";
const HYDRATE_MODE_ENV = "AFORA_MANTIS_HYDRATE_MODE";
const SLACK_URL_ENV = "AFORA_MANTIS_SLACK_URL";
const SLACK_CHANNEL_ID_ENV = "AFORA_MANTIS_SLACK_CHANNEL_ID";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.AFORA_LIVE_OPENAI_KEY) && trimToValue(next.OPENAI_API_KEY)) {
    next.AFORA_LIVE_OPENAI_KEY = next.OPENAI_API_KEY;
  }
  if (!trimToValue(next.AFORA_MANTIS_SLACK_BOT_TOKEN) && trimToValue(next.SLACK_BOT_TOKEN)) {
    next.AFORA_MANTIS_SLACK_BOT_TOKEN = next.SLACK_BOT_TOKEN;
  }
  if (
    !trimToValue(next.AFORA_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(next.AFORA_QA_SLACK_SUT_BOT_TOKEN)
  ) {
    next.AFORA_MANTIS_SLACK_BOT_TOKEN = next.AFORA_QA_SLACK_SUT_BOT_TOKEN;
  }
  if (!trimToValue(next.AFORA_MANTIS_SLACK_APP_TOKEN) && trimToValue(next.SLACK_APP_TOKEN)) {
    next.AFORA_MANTIS_SLACK_APP_TOKEN = next.SLACK_APP_TOKEN;
  }
  if (
    !trimToValue(next.AFORA_MANTIS_SLACK_APP_TOKEN) &&
    trimToValue(next.AFORA_QA_SLACK_SUT_APP_TOKEN)
  ) {
    next.AFORA_MANTIS_SLACK_APP_TOKEN = next.AFORA_QA_SLACK_SUT_APP_TOKEN;
  }
  if (
    !trimToValue(next.AFORA_MANTIS_SLACK_CHANNEL_ID) &&
    trimToValue(next.AFORA_QA_SLACK_CHANNEL_ID)
  ) {
    next.AFORA_MANTIS_SLACK_CHANNEL_ID = next.AFORA_QA_SLACK_CHANNEL_ID;
  }
  return next;
}

function resolveSlackGatewayEnvPayload(env: NodeJS.ProcessEnv): SlackGatewayCredentialPayload {
  const channelId = trimToValue(env.AFORA_QA_SLACK_CHANNEL_ID);
  const sutBotToken = trimToValue(env.AFORA_QA_SLACK_SUT_BOT_TOKEN);
  const sutAppToken = trimToValue(env.AFORA_QA_SLACK_SUT_APP_TOKEN);
  if (!channelId || !sutBotToken || !sutAppToken) {
    throw new Error(
      "Gateway setup requires AFORA_QA_SLACK_CHANNEL_ID, AFORA_QA_SLACK_SUT_BOT_TOKEN, and AFORA_QA_SLACK_SUT_APP_TOKEN when using --credential-source env.",
    );
  }
  return {
    channelId,
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.AFORA_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(params.env.AFORA_MANTIS_SLACK_APP_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.AFORA_MANTIS_SLACK_BOT_TOKEN = payload.sutBotToken;
  params.env.AFORA_MANTIS_SLACK_APP_TOKEN = payload.sutAppToken;
  params.env.AFORA_MANTIS_SLACK_CHANNEL_ID =
    trimToValue(params.env.AFORA_MANTIS_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.AFORA_QA_SLACK_CHANNEL_ID =
    trimToValue(params.env.AFORA_QA_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.AFORA_QA_SLACK_SUT_BOT_TOKEN =
    trimToValue(params.env.AFORA_QA_SLACK_SUT_BOT_TOKEN) ?? payload.sutBotToken;
  params.env.AFORA_QA_SLACK_SUT_APP_TOKEN =
    trimToValue(params.env.AFORA_QA_SLACK_SUT_APP_TOKEN) ?? payload.sutAppToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
remote_command_timeout_seconds="\${AFORA_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS:-${remoteCommandTimeoutSeconds}}"
if [ -z "\${AFORA_QA_SLACK_CHANNEL_ID:-}" ] && [ -n "$slack_channel_id" ]; then
  export AFORA_QA_SLACK_CHANNEL_ID="$slack_channel_id"
fi
case "$remote_command_timeout_seconds" in
  ''|*[!0-9]*)
    echo "AFORA_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be an integer number of seconds." >&2
    exit 2
    ;;
esac
if [ "$remote_command_timeout_seconds" -le 0 ]; then
  echo "AFORA_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be greater than zero." >&2
  exit 2
fi
rm -rf "$out"
mkdir -p "$out"
export DISPLAY="\${DISPLAY:-:99}"
if [ -n "\${AFORA_LIVE_OPENAI_KEY:-}" ] && [ -z "\${OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$AFORA_LIVE_OPENAI_KEY"
fi
...
team_id="\${AFORA_QA_SLACK_TEAM_ID:-}"
auth_test_token="\${AFORA_QA_SLACK_SUT_BOT_TOKEN:-\${AFORA_MANTIS_SLACK_BOT_TOKEN:-}}"
...
const token = process.env.AFORA_QA_SLACK_SUT_BOT_TOKEN || process.env.AFORA_MANTIS_SLACK_BOT_TOKEN;
...
if [ -z "$slack_url" ] && [ -n "$team_id" ] && [ -n "\${AFORA_QA_SLACK_CHANNEL_ID:-}" ]; then
  slack_url="https://app.slack.com/client/$team_id/$AFORA_QA_SLACK_CHANNEL_ID"
fi
profile="\${AFORA_MANTIS_SLACK_BROWSER_PROFILE_DIR:-$HOME/.config/afora-mantis/slack-chrome-profile}"
mkdir -p "$profile"
if [ "$setup_gateway" = "1" ]; then
  export SLACK_BOT_TOKEN="\${AFORA_MANTIS_SLACK_BOT_TOKEN:-\${SLACK_BOT_TOKEN:-}}"
  export SLACK_APP_TOKEN="\${AFORA_MANTIS_SLACK_APP_TOKEN:-\${SLACK_APP_TOKEN:-}}"
  if [ -z "$SLACK_BOT_TOKEN" ] || [ -z "$SLACK_APP_TOKEN" ]; then
    echo "Gateway setup requires AFORA_MANTIS_SLACK_BOT_TOKEN and AFORA_MANTIS_SLACK_APP_TOKEN." >&2
    exit 2
  fi
...
run_mantis_remote_body() {
  ...
  if ! node_supports_type_stripping; then
    ...
    node_root="$HOME/.cache/afora-mantis/node-v$node_version-linux-$node_arch"
...
')
  ...
  if [ "$setup_gateway" = "1" ]; then
    export AFORA_HOME="$HOME/.afora-mantis/slack-afora"
    mkdir -p "$AFORA_HOME"
...
MANTIS_SLACK_PATCH
    pnpm afora config patch --file "$out/slack.socket.patch.json5" --dry-run
    pnpm afora config patch --file "$out/slack.socket.patch.json5"
    nohup pnpm afora gateway run --dev --allow-unconfigured --port 38973 --cli-backend-logs </dev/null >"$out/afora-gateway.log" 2>&1 &
    gateway_pid="$!"
    echo "$gateway_pid" >"$out/afora-gateway.pid"
    sleep 12
    if ! kill -0 "$gateway_pid" >/dev/null 2>&1; then
      echo "Afora gateway exited during startup." >&2
    ...
    qa_args=(afora qa slack --repo-root . --output-dir "$slack_qa_output_dir" --provider-mode "$provider_mode" --model "$primary_model" --alt-model "$alternate_model" --credential-source "$credential_source" --credential-role "$credential_role")
    if [ "$fast_mode" = "1" ]; then
      qa_args+=(--fast)
    fi
    if [ "$approval_checkpoints" = "1" ]; then
      ...
      export AFORA_QA_SLACK_APPROVAL_CHECKPOINT_DIR="$checkpoint_dir"
      export AFORA_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS="\${AFORA_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS:-${approvalCheckpointTimeoutMs}}"
      export AFORA_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON="$approval_checkpoint_scenarios_json"
      export AFORA_MANTIS_APPROVAL_BROWSER_BIN="$browser_bin"
      cat >"$out/approval-checkpoint-watcher.mjs" <<'MANTIS_APPROVAL_WATCHER'
	import { spawn } from "node:child_process";
	import fs from "node:fs/promises";
	import path from "node:path";

const checkpointDir = process.env.AFORA_QA_SLACK_APPROVAL_CHECKPOINT_DIR;
const timeoutMs = Number.parseInt(
  process.env.AFORA_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS || "${approvalCheckpointTimeoutMs}",
  10,
);
	const scenarioIds = JSON.parse(
	  process.env.AFORA_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON || "[]",
	);
	const browserBin = process.env.AFORA_MANTIS_APPROVAL_BROWSER_BIN;

if (!checkpointDir) {
  throw new Error("AFORA_QA_SLACK_APPROVAL_CHECKPOINT_DIR is required.");
}
if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
  throw new Error("AFORA_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS must be a positive integer.");
}
  ...
  throw new Error(\`Timed out waiting for approval checkpoint: \${filePath}\`);
	...
	function renderCheckpointHtml(checkpoint) {
	  ...
	  return '<!doctype html><html><head><meta charset="utf-8">' +
	    ...
	    '<div class="meta"><span class="name">afora</span><span class="app">APP</span><span class="state">' +
	...
	async function captureScreenshot(screenshotPath, checkpoint) {
	  if (!browserBin) {
	    throw new Error("AFORA_MANTIS_APPROVAL_BROWSER_BIN is required to render approval checkpoint screenshots.");
	  }
...
cat >"$out/remote-metadata.json" <<MANTIS_REMOTE_METADATA
{
  ...
  "gatewayAlive": $(if [ "$setup_gateway" = "1" ] && [ -f "$out/afora-gateway.pid" ] && kill -0 "$(cat "$out/afora-gateway.pid")" >/dev/null 2>&1; then echo true; else echo false; fi),
  "gatewayPid": "$(if [ -f "$out/afora-gateway.pid" ]; then cat "$out/afora-gateway.pid"; fi)",
  "gatewayPort": 38973,
...
export async function runMantisSlackDesktopSmoke(
  ...
  const slackChannelId =
    ...
    trimToValue(env.AFORA_QA_SLACK_CHANNEL_ID) ??
  ...
  const remoteOutputDir = `/tmp/afora-mantis-slack-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Slack desktop gateway setup did not report a live Afora gateway.");
    }
