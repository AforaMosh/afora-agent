...
describe("mantis Slack desktop smoke runtime", () => {
  ...
  it("leases a desktop box, runs Slack QA inside it, copies artifacts, and stops on pass", async () => {
    ...
    const runtimeEnv = {
      ...
      AFORA_QA_SLACK_CHANNEL_ID: "C123",
      AFORA_QA_SLACK_DRIVER_BOT_TOKEN: "driver-token",
      AFORA_QA_SLACK_SUT_APP_TOKEN: "app-token",
      AFORA_QA_SLACK_SUT_BOT_TOKEN: "sut-token",
    };
    const runner = vi.fn(
    ...
    expect(
      commands.every((entry) => entry.env?.AFORA_LIVE_OPENAI_KEY === "openai-runtime-key"),
    ).toBe(true);
    ...
    expect(runArgs).toContain("AforaMosh/afora-agent#85141");
    ...
    expect(remoteScript).toContain("afora qa slack");
    ...
    expect(remoteScript).toContain("AFORA_MANTIS_SLACK_BROWSER_PROFILE_DIR");
    expect(remoteScript)
  ...
  signal: AbortSignal.timeout(15_000),
    ...
    expect(rsyncArgs).toContain(
      "crabbox@203.0.113.10:/tmp/afora-mantis-slack-desktop-2026-05-04T13-00-00-000Z/",
    );
    expect(rsyncArgs).toContain(
      "crabbox@203.0.113.10:/tmp/afora-mantis-slack-desktop-2026-05-04T13-00-00-000Z/slack-qa/",
    );
  ...
  it("runs approval checkpoint mode with default approval scenarios and records screenshots", async () => {
    ...
    expect(remoteScript).toContain('export AFORA_QA_SLACK_CHANNEL_ID="$slack_channel_id"');
    ...
    expect(remoteScript).toContain("AFORA_QA_SLACK_APPROVAL_CHECKPOINT_DIR");
    expect(remoteScript).toContain("AFORA_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS");
    ...
    expect(remoteScript).toContain("AFORA_MANTIS_APPROVAL_BROWSER_BIN");
  ...
  it.each([
    ...
    async ({ expectedScenarioIds, remoteTimeoutSeconds, requestedScenarioIds }) => {
      ...
      expect(remoteScript).toContain("AFORA_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS:-470000");
      expect(remoteScript).toContain(
        `AFORA_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS:-${remoteTimeoutSeconds}`,
      );
    },
  );
  ...
  it("leases Convex Slack credentials for gateway setup and maps them into the VM env", async () => {
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: {
        CI: "1",
        OPENAI_API_KEY: "openai-runtime-key",
        AFORA_QA_CONVEX_SECRET_CI: "convex-secret",
        AFORA_QA_CONVEX_SITE_URL: "https://example.convex.site",
        PATH: process.env.PATH,
      },
      gatewaySetup: true,
      keepLease: false,
    ...
    expect(runCommand?.env?.AFORA_MANTIS_SLACK_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.AFORA_MANTIS_SLACK_BOT_TOKEN).toBe("xoxb-leased");
    expect(runCommand?.env?.AFORA_MANTIS_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.AFORA_QA_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.AFORA_QA_SLACK_SUT_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.AFORA_QA_SLACK_SUT_BOT_TOKEN).toBe("xoxb-leased");
    const remoteScript = runCommand?.args.at(-1);
    expect(remoteScript).toContain("setup_gateway=1");
    expect(remoteScript).toContain("afora gateway run");
    expect(remoteScript).toContain('</dev/null >"$out/afora-gateway.log"');
  ...
  it("passes gateway setup when Crabbox returns non-zero after remote metadata proves success", async () => {
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: {
        OPENAI_API_KEY: "openai-runtime-key",
        AFORA_MANTIS_SLACK_APP_TOKEN: "xapp-direct",
        AFORA_MANTIS_SLACK_BOT_TOKEN: "xoxb-direct",
        PATH: process.env.PATH,
      },
      gatewaySetup: true,
