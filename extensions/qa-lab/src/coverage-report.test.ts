...
describe("qa coverage report", () => {
  ...
  it("renders Playwright matches as qa suite targets", () => {
    ...
    expect(report).toContain(
      "- Suite command: `pnpm afora qa suite --scenario control-ui-chat-flow-playwright`",
    );
    expect(report).toContain(
  ...
  it("includes a runnable channel driver choice in scenario match commands", () => {
    ...
    expect(report).toContain(
      "- Suite command: `pnpm afora qa suite --channel-driver live --channel whatsapp --scenario whatsapp-access-control-group-disabled`",
    );
  });
  ...
  it("keeps qa-channel scenario commands on the default driver", () => {
    ...
    expect(report).toContain(
      "- Suite command: `pnpm afora qa suite --scenario instruction-followthrough-repo-contract`",
    );
  ...
  it("uses the live lane as the coverage-report default for channel scenarios", () => {
    ...
    expect(report).toContain(
      "- Suite command: `pnpm afora qa suite --channel-driver live --channel matrix --scenario dm-per-room-session`",
    );
  });
  ...
  it("splits qa suite targets when matches mix execution kinds", () => {
    ...
    expect(report).toContain("  - flow: `pnpm afora qa suite --scenario flow-proof`");
    expect(report).toContain(
      "  - playwright: `pnpm afora qa suite --scenario playwright-proof`",
    );
  });
    ...
    const report = buildQaScorecardTaxonomyReport({
      ...
      scenarios: [
        scenarioWithCoverage({
          primary: [TEST_EXECUTABLE_COVERAGE_ID],
          executionKind: "playwright",
          executionPath: "../outside-afora.test.ts",
        }),
      ],
    });
