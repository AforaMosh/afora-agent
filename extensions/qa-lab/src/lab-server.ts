...
function createQaLabConfig(baseUrl: string): AforaConfig {
...
export async function startQaLabServer(
  ...
  const resolveServerRunPlan = async (
    ...
    return resolveQaLabRunPlan({
      ...
      scorecardReport,
      defaultChannel:
        crabline?.AFORA_CRABLINE_DEFAULT_CHANNEL ??
        (selection.channelDriver === "qa-channel" ? "qa-channel" : undefined),
      ...(crabline
        ? {
            supportsChannel: (channel: string) => {
              try {
                crabline.resolveAforaCrablineChannelDriverSelection({ channel });
                return true;
              } catch {
                return false;
              }
