...
export const CODEX_PLUGIN_LIFECYCLE_MESSAGES = Object.freeze({
  missingPlugin:
    'Codex plugin is required for Codex runtime. Run "afora doctor --fix" to install @afora/codex, then retry.',
});
...
export async function installCodexPluginFixture(agentDir: string): Promise<void> {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({ name: "@afora/codex" }, null, 2)}\n`,
    "utf8",
  );
  await fs.writeFile(
    path.join(targetDir, "afora.plugin.json"),
