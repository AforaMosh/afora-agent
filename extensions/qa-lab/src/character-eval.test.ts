...
function defaultModelTranscript(model: string) {
  return `USER Alice: hi\n\nASSISTANT afora: reply from ${model}`;
}
...
describe("runQaCharacterEval", () => {
  ...
  beforeEach(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-character-eval-test-"));
  });
  ...
  it("creates a unique default output directory under repo artifacts", async () => {
    const runSuite = vi.fn(async (params: CharacterRunSuiteParams) =>
      makeSuiteResult({
        outputDir: params.outputDir,
        model: params.primaryModel,
        transcript: "USER Alice: hi\n\nASSISTANT afora: default dir reply",
      }),
    );
    const runJudge = makeRunJudge([
      {
  ...
  it("can hide candidate model refs from judge prompts and map rankings back", async () => {
    const runSuite = vi.fn(async (params: CharacterRunSuiteParams) =>
      makeSuiteResult({
        outputDir: params.outputDir,
        model: params.primaryModel,
        transcript: "USER Alice: hi\n\nASSISTANT afora: anonymous reply",
      }),
    );
  ...
  it.each([
    {
      ...
      transcript:
        "USER Alice: Are you awake?\n\nASSISTANT Afora QA: 400 model `qwen3.6-plus` is not supported.",
    ...
    },
    {
      ...
      transcript:
        "ASSISTANT Afora QA: ⚠️ Something went wrong while processing your request. Please try again, or use /new to start a fresh session.",
    ...
    },
    {
      ...
      transcript:
        "ASSISTANT Afora QA: The model did not produce a response before the LLM idle timeout. Please try again, or increase `agents.defaults.llm.idleTimeoutSeconds` in your config.",
    ...
    },
    {
      ...
      transcript:
        "ASSISTANT Afora QA: checking thread context; then post a tight progress reply here.\nQA_LEAK_OK",
  ...
  it("marks candidates failed when the suite summary has failed scenarios", async () => {
    const runSuite = vi.fn(async (params: CharacterRunSuiteParams) =>
      makeSuiteResult({
        outputDir: params.outputDir,
        model: params.primaryModel,
        transcript: "USER Alice: hi\n\nASSISTANT afora: outwardly fine",
        summaryStatus: "fail",
  ...
  it("marks raw tool failure transcripts as failed output", async () => {
    const runSuite = vi.fn(async (params: CharacterRunSuiteParams) =>
      makeSuiteResult({
        outputDir: params.outputDir,
        model: params.primaryModel,
        transcript: "ASSISTANT Afora QA: ⚠️ ✍️ Write: to /tmp/precious.html failed",
      }),
    );
    const runJudge = makeRunJudge([
