...
const runExecSpy = vi.hoisted(() =>
  vi.fn<typeof import("afora-agent/plugin-sdk/process-runtime").runExec>(),
);
...
function createWhatsAppScenarioGateway(
  ...
  return {
    call: async () => ({}),
    restart: async () => {},
    workspaceDir: "/tmp/afora-whatsapp-qa",
    ...overrides,
  };
}
...
function createWhatsAppScenarioContext(
  ...
  const workspaceDir = contextOverrides.gatewayWorkspaceDir ?? "/tmp/afora-whatsapp-qa";
  return {
...
function buildWhatsAppQaConfigFixture(
  ...
  return testing.buildWhatsAppQaConfig(base, {
    ...
    authDir: "/tmp/afora-whatsapp-qa-auth",
    dmPolicy: "allowlist",
...
function createWhatsAppAgentReactionFixture(params: {
  ...
  const context = createWhatsAppScenarioContext({
    ...
    ...(params.groupJid
      ...
      : { gatewayWorkspaceDir: "/tmp/afora-whatsapp-qa-gateway" }),
    recordedMessages,
    scenarioId,
...
describe("WhatsApp QA live runtime", () => {
  ...
  it("unpacks auth archives into a caller-provided temp directory", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-wa-qa-test-"));
    try {
  ...
  it("runs WhatsApp group direct Gateway media, audio, and poll probes against the group target", async () => {
    ...
    const context = createWhatsAppScenarioContext({
      ...
      gateway: createWhatsAppScenarioGateway({
        ...
        workspaceDir: "/tmp/afora-whatsapp-qa-gateway",
      }),
  ...
  it("defines Phase 2 WhatsApp group scenarios as mock-backed user-path scenarios", () => {
    ...
    for (const scenario of scenarios) {
      ...
      expect(run.input).toContain("aforaqa");
    }
  });
  ...
  it("models activation always through visible group behavior and restores mention gating", async () => {
    ...
    const context = createWhatsAppScenarioContext({
      driver,
      gatewayTarget: groupJid,
      gatewayWorkspaceDir: "/tmp/afora-whatsapp-qa-workspace",
      scenario,
    ...
    expect(alwaysProbe?.text).not.toMatch(/\baforaqa\b/i);
    ...
    expect(restoredQuietProbe?.text).not.toMatch(/\baforaqa\b/i);
  ...
  it("quotes the observed SUT reply without an explicit mention for reply-to-bot activation", async () => {
    ...
    expect(run.target).toBe("group");
    expect(run.input).toMatch(/\baforaqa\b/iu);
    ...
    expect(quotedSend?.text).not.toMatch(/\baforaqa\b/i);
  ...
  it("defines quote-reply scenarios for DM and group replies", () => {
    ...
    expect(runs[0]?.run.input).not.toContain("aforaqa");
    expect(runs[1]?.run.input).toMatch(/^aforaqa\b/u);
  ...
  it("defines group audio gating as captionless audio driven by mock transcription sentinel", () => {
    ...
    const triggerSentinel = Buffer.from("AFORA_QA_GROUP_AUDIO_TRIGGER", "utf8");
  ...
  it("uses automatic visible replies for WhatsApp group mention gating", () => {
    ...
    expect(scenarioRun.input).toContain("aforaqa reply with only this exact marker");
    ...
    expect(cfg.messages?.groupChat?.mentionPatterns).toContain("\\baforaqa\\b");
  });
