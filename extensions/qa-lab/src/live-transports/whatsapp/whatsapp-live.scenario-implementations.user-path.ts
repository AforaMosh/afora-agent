...
export const whatsappQaGroupAgentMessageActionReactScenario: WhatsAppQaScenarioImplementation = {
  ...
  buildRun: () => {
    ...
    return {
      ...
      expectReply: false,
      input:
        `aforaqa react to this WhatsApp group message with thumbs up for QA action check ${token}. ` +
        "Do not send any visible text reply after the reaction.",
      matchText: token,
      quietWindowMs: 8_000,
      target: "group",
...
export const whatsappQaGroupAgentMessageActionUploadFileScenario: WhatsAppQaScenarioImplementation =
  {
    ...
    buildRun: () => {
      ...
      return {
        ...
        expectReply: false,
        input:
          `aforaqa use the WhatsApp message tool upload-file action to send a PNG with caption ${token}. ` +
          "Do not send any visible text reply after the upload.",
        matchText: token,
        quietWindowMs: 8_000,
        target: "group",
