...
import {
  ...
  resolveMatrixQaAforaCliEntryPath,
} from "./scenario-runtime-cli-process.js";
import {
  formatMatrixQaCliCommand,
  redactMatrixQaCliOutput,
  runMatrixQaAforaCli,
  startMatrixQaAforaCli,
} from "./scenario-runtime-cli.js";

const testing = { killMatrixQaCliChild, resolveMatrixQaAforaCliEntryPath };
...
describe("Matrix QA CLI runtime", () => {
  it("redacts secret CLI arguments in diagnostic command text", () => {
    ...
    ).toBe("afora matrix verify backup restore --recovery-key [REDACTED]");
    expect(formatMatrixQaCliCommand(["matrix", "account", "add", "--access-token=token-123"])).toBe(
      "afora matrix account add --access-token=[REDACTED]",
    );
    expect(
      formatMatrixQaCliCommand(["matrix", "verify", "device", "abcdef1234567890ghij", "--json"]),
    ).toBe("afora matrix verify device [REDACTED] --json");
  });
  ...
  it("prefers the ESM Afora CLI entrypoint when present", async () => {
    const root = await mkdtemp(path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-entry-"));
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(path.join(root, "dist", "index.mjs"), "");
      expect(testing.resolveMatrixQaAforaCliEntryPath(root)).toBe(
  ...
  it("can preserve expected non-zero CLI output for negative scenarios", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-nonzero-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
      ...
      const result = await runMatrixQaAforaCli({
        allowNonZero: true,
  ...
  it("can pass stdin to CLI commands", async () => {
    const root = await mkdtemp(path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-stdin-"));
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
        path.join(root, "dist", "index.mjs"),
  ...
  it("can close stdin after interactive CLI prompts", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-interactive-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
      ...
      const session = startMatrixQaAforaCli({
  ...
  it("includes timed-out CLI output in diagnostics", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-timeout-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
  ...
  it("kills CLI commands that ignore graceful timeout termination", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-timeout-kill-"),
    );
  ...
  it("preserves timeout diagnostics when wait attaches after timeout", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-late-wait-timeout-"),
    );
    const pidPath = path.join(root, "cli.pid");
    let childPid: number | undefined;
    try {
  ...
  it("settles and kills descendants that keep timed-out CLI stdio open", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-timeout-tree-"),
    );
  ...
  it("kills ignored-stdio descendants after a timed-out CLI exits gracefully", async () => {
    ...
    const root = await mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-timeout-ignored-stdio-"),
    );
    const childPidPath = path.join(root, "child.pid");
    const grandchildPidPath = path.join(root, "grandchild.pid");
    let childPid: number | undefined;
  ...
  it("kills ignored-stdio descendants after manual CLI session kill", async () => {
    ...
    const root = await mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "matrix-qa-cli-session-kill-ignored-stdio-"),
    );
    const childPidPath = path.join(root, "child.pid");
    const grandchildPidPath = path.join(root, "grandchild.pid");
    let childPid: number | undefined;
