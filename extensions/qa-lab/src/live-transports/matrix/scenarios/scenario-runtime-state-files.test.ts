...
import { createClaimableDedupe } from "afora-agent/plugin-sdk/persistent-dedupe";
...
describe("Matrix QA persisted state probes", () => {
  ...
  it("observes inbound dedupe entries committed through the core claimable dedupe", async () => {
    ...
    const guard = createClaimableDedupe({
      ...
      memoryMaxSize: 100,
      stateMaxEntries: 100,
      env: { ...process.env, AFORA_STATE_DIR: accountRoot },
    });
    ...
    ).resolves.toBe(path.join(accountRoot, "state", "afora.sqlite"));
  });
});
