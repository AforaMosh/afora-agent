    ...
    await withStartedMatrixHarness(
      ...
      async ({ outputDir, result }) => {
        expect(calls).toEqual([
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml down --remove-orphans @/repo/afora`,
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml up -d @/repo/afora`,
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml ps --format json matrix-qa-homeserver @/repo/afora`,
        ]);
        ...
        expect(calls).toContain(
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml restart matrix-qa-homeserver @/repo/afora`,
        );
      },
    );
  });
  ...
  it("lets Docker atomically assign an unpinned loopback port", async () => {
    ...
    await withStartedMatrixHarness(
      ...
      async ({ outputDir, result }) => {
        ...
        expect(calls).toContain(
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml port matrix-qa-homeserver 8008 @/repo/afora`,
        );
        const compose = await readFile(result.composeFile, "utf8");
    ...
    await withTempDir("matrix-qa-harness-", async (outputDir) => {
      await expect(
        startMatrixQaHarness(
          { outputDir, repoRoot: "/repo/afora", homeserverPort: 28008 },
          {
            async runCommand(command, args, cwd) {
              calls.push([command, ...args, `@${cwd}`].join(" "));
              if (args.join(" ").includes("ps --format json")) {
  ...
  it("falls back to the container IP when the host port is unreachable", async () => {
    ...
    await withStartedMatrixHarness(
      ...
      ({ outputDir, result }) => {
        ...
        expect(calls).toContain(
          `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml ps -q matrix-qa-homeserver @/repo/afora`,
        );
        expect(calls).toContain(
          "docker inspect --format {{range .NetworkSettings.Networks}}{{println .IPAddress}}{{end}} container-123 @/repo/afora",
        );
      },
    );
  });
