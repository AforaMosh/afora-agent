...
import type { LiveTransportQaCommandOptions } from "afora-agent/plugin-sdk/qa-runtime";
...
const TELEGRAM_QA_SUT_AFORA_COMMAND_ENV = "AFORA_QA_TELEGRAM_SUT_AFORA_COMMAND";
const TELEGRAM_QA_SUT_UID_ENV = "AFORA_QA_TELEGRAM_SUT_UID";
const TELEGRAM_QA_SUT_GID_ENV = "AFORA_QA_TELEGRAM_SUT_GID";
const TELEGRAM_QA_SUT_BOUNDARY_DIR_ENV = "AFORA_QA_TELEGRAM_SUT_PROCESS_BOUNDARY_DIR";
const TELEGRAM_QA_SUT_CLEANUP_TIMEOUT_ENV = "AFORA_QA_TELEGRAM_SUT_CLEANUP_TIMEOUT_MS";
const TELEGRAM_QA_SUT_RUNTIME_EXECUTABLE_ENV = "AFORA_QA_TELEGRAM_SUT_RUNTIME_EXECUTABLE";
const TELEGRAM_QA_SUT_PRELOAD_PATH_ENV = "AFORA_QA_TELEGRAM_SUT_PRELOAD_PATH";
const TELEGRAM_QA_SUT_FORWARDED_ENV_KEYS_ENV = "AFORA_QA_TELEGRAM_SUT_FORWARDED_ENV_KEYS";
...
async function resolveTelegramQaSutAforaCommand(
  repoRoot: string,
  env: NodeJS.ProcessEnv = process.env,
): Promise<QaGatewayChildCommand | undefined> {
  const configuredCommand = env[TELEGRAM_QA_SUT_AFORA_COMMAND_ENV];
  if (configuredCommand === undefined) {
    return undefined;
  }
  const command = await resolveRegularPath({
    env,
    key: TELEGRAM_QA_SUT_AFORA_COMMAND_ENV,
    executable: true,
  });
...
export async function runQaTelegramSuite(opts: TelegramQaSuiteOptions) {
  ...
  const result = await runQaFlowSuiteFromRuntime({
    ...
    scenarioIds,
    sutAforaCommand: opts.sutAforaCommand,
...
export async function runQaTelegramCommand(opts: LiveTransportQaCommandOptions) {
  ...
  const sutAforaCommand = await resolveTelegramQaSutAforaCommand(runOptions.repoRoot);
  return await runQaTelegramSuite({
    ...opts,
    resolvedScenarioIds,
    ...(sutAforaCommand ? { sutAforaCommand } : {}),
  });
}
