...
describe("Slack live QA runtime helpers", () => {
  ...
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        AFORA_QA_SLACK_CHANNEL_ID: "C123456789",
        AFORA_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        AFORA_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        AFORA_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        AFORA_QA_SLACK_CHANNEL_ID: "qa-channel",
        AFORA_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        AFORA_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        AFORA_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("AFORA_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("extracts typed Slack approval button values from blocks", () => {
    const actionValue =
      'afora:approval:v1:{"approvalId":"plugin:abc","approvalKind":"plugin","decision":"allow-once"}';
    expect(
  ...
  it("extracts plugin approval ids from typed Slack approval action values", () => {
    expect(
      testing.extractSlackNativeApprovalId({
        actionValues: [
          'afora:approval:v1:{"approvalId":"plugin:abc123","approvalKind":"plugin","decision":"allow-once"}',
          'afora:approval:v1:{"approvalId":"plugin:abc123","approvalKind":"plugin","decision":"deny"}',
        ],
        decision: "allow-once",
      }),
    ).toBe("plugin:abc123");
  ...
  it("resolves the Codex file approval target path", () => {
    expect(testing.resolveCodexFileApprovalTargetPath("MARKER")).toMatch(
      /\.afora-qa-codex-file-approval-marker\.txt$/u,
    );
  });
