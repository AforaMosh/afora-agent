...
describe("credential lease runtime", () => {
  ...
  it("acquires, heartbeats, and releases convex credentials", async () => {
    ...
    const lease = await acquireQaCredentialLease({
      ...
      role: "maintainer",
      env: {
        AFORA_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        AFORA_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
      },
      fetchImpl,
    ...
    try {
      await expect(
        acquireQaCredentialLease({
          ...
          role: "maintainer",
          env: {
            AFORA_QA_CONVEX_SITE_URL: broker.url,
            AFORA_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
            AFORA_QA_ALLOW_INSECURE_HTTP: "1",
          },
          resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
          parsePayload: (payload) =>
            payload as { groupId: string; driverToken: string; sutToken: string },
  ...
  it("hydrates chunked convex credential payloads after acquire", async () => {
    ...
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          ...
          payload: {
            __aforaQaCredentialPayloadChunksV1: true,
    ...
    const lease = await acquireQaCredentialLease({
      ...
      env: {
        AFORA_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        AFORA_QA_CONVEX_SECRET_CI: "ci-secret",
      },
      fetchImpl,
      resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
      parsePayload: (payload) =>
    ...
    await expect(
      acquireQaCredentialLease({
        ...
        env: {
          AFORA_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
          AFORA_QA_CONVEX_SECRET_CI: "ci-secret",
          AFORA_QA_CREDENTIAL_PAYLOAD_MAX_CHUNKS: "2",
        },
        fetchImpl,
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
    ...
    await expect(
      acquireQaCredentialLease({
        ...
        env: {
          AFORA_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
          AFORA_QA_CONVEX_SECRET_CI: "ci-secret",
          AFORA_QA_CREDENTIAL_PAYLOAD_MAX_BYTES: "32",
        },
        fetchImpl,
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
  ...
  it("retries convex acquire while the pool is exhausted", async () => {
    ...
    const lease = await acquireQaCredentialLease({
      ...
      env: {
        AFORA_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        AFORA_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        AFORA_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "90000",
      },
      fetchImpl,
      randomImpl: () => 0,
      timeImpl: () => nowMs,
  ...
  it("retries transient convex acquire transport failures", async () => {
    ...
    const lease = await acquireQaCredentialLease({
      ...
      env: {
        AFORA_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        AFORA_QA_CONVEX_SECRET_MAINTAINER: "test",
        AFORA_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "90000",
      },
      fetchImpl,
      randomImpl: () => 0,
  ...
  it("rejects non-https convex site URLs unless local insecure opt-in is enabled", async () => {
    await expect(
      acquireQaCredentialLease({
        ...
        source: "convex",
        env: {
          AFORA_QA_CONVEX_SITE_URL: "http://qa-cred.example.convex.site",
          AFORA_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        },
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
  ...
  it("allows loopback http URLs when AFORA_QA_ALLOW_INSECURE_HTTP is enabled", async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValueOnce(
      jsonResponse({
        status: "ok",
    ...
    await acquireQaCredentialLease({
      ...
      role: "maintainer",
      env: {
        AFORA_QA_CONVEX_SITE_URL: "http://127.0.0.1:3210",
        AFORA_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        AFORA_QA_ALLOW_INSECURE_HTTP: "1",
      },
      fetchImpl,
  ...
  it("caps oversized convex HTTP timeouts before creating abort signals", async () => {
    ...
    await acquireQaCredentialLease({
      ...
      env: {
        AFORA_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        AFORA_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        AFORA_QA_CREDENTIAL_HTTP_TIMEOUT_MS: String(Number.MAX_SAFE_INTEGER),
      },
      fetchImpl,
      resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
      parsePayload: (payload) =>
    ...
    await expect(
      acquireQaCredentialLease({
        ...
        env: {
          AFORA_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
          AFORA_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
          AFORA_QA_CONVEX_ENDPOINT_PREFIX: "//evil.example",
        },
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
          payload as { groupId: string; driverToken: string; sutToken: string },
      }),
    ).rejects.toThrow("AFORA_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path");
  });
  ...
  it("fails convex mode when auth secret is missing", async () => {
    ...
    ).rejects.toThrow("AFORA_QA_CONVEX_SECRET_MAINTAINER");
  });
