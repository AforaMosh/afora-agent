...
import { readProviderTextResponse } from "afora-agent/plugin-sdk/provider-http";
import { z } from "zod";
import {
...
const CHUNKED_PAYLOAD_MAX_BYTES_ENV = "AFORA_QA_CREDENTIAL_PAYLOAD_MAX_BYTES";
const CHUNKED_PAYLOAD_MAX_CHUNKS_ENV = "AFORA_QA_CREDENTIAL_PAYLOAD_MAX_CHUNKS";
...
function normalizeEndpointPrefix(value: string | undefined): string {
  return normalizeQaCredentialEndpointPrefix({
    ...
    invalidAbsoluteMessage:
      "AFORA_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path like /qa-credentials/v1.",
    invalidSegmentsMessage:
      "AFORA_QA_CONVEX_ENDPOINT_PREFIX must not contain backslashes or .. path segments.",
  });
}
...
function resolveConvexAuthToken(env: NodeJS.ProcessEnv, role: QaCredentialRole): string {
  const roleToken =
    role === "ci"
      ? env.AFORA_QA_CONVEX_SECRET_CI?.trim()
      : env.AFORA_QA_CONVEX_SECRET_MAINTAINER?.trim();
  const token = roleToken;
  if (token) {
    return token;
  }
  if (role === "ci") {
    throw new Error("Missing AFORA_QA_CONVEX_SECRET_CI for CI credential access.");
  }
  throw new Error("Missing AFORA_QA_CONVEX_SECRET_MAINTAINER for maintainer credential access.");
}
...
function resolveConvexCredentialBrokerConfig(params: {
  ...
  const siteUrl = params.env.AFORA_QA_CONVEX_SITE_URL?.trim();
  if (!siteUrl) {
    throw new Error("Missing AFORA_QA_CONVEX_SITE_URL for --credential-source convex.");
  }
  const baseUrl = normalizeConvexSiteUrl(siteUrl, params.env);
  const endpointPrefix = normalizeEndpointPrefix(params.env.AFORA_QA_CONVEX_ENDPOINT_PREFIX);
  const ownerId =
    params.ownerId?.trim() ||
    params.env.AFORA_QA_CREDENTIAL_OWNER_ID?.trim() ||
...
export async function acquireQaCredentialLease<TPayload>(
  ...
  const source = normalizeQaCredentialSource(opts.source ?? env.AFORA_QA_CREDENTIAL_SOURCE);
  if (source === "env") {
    return {
      source: "env",
      kind: opts.kind,
  ...
  const role = normalizeQaCredentialRole(opts.role ?? env.AFORA_QA_CREDENTIAL_ROLE, env);
