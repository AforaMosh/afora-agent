...
describe("qa suite planning helpers", () => {
  ...
  it("normalizes suite concurrency to a bounded integer", () => {
    const previous = process.env.AFORA_QA_SUITE_CONCURRENCY;
    delete process.env.AFORA_QA_SUITE_CONCURRENCY;
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.AFORA_QA_SUITE_CONCURRENCY = previous;
      }
    }
  });
  ...
  it("resolves a default worker startup stagger for concurrent suite workers", () => {
    ...
    expect(
      resolveQaSuiteWorkerStartStaggerMs(4, {
        AFORA_QA_SUITE_WORKER_START_STAGGER_MS: "0",
      }),
    ).toBe(0);
    expect(
      resolveQaSuiteWorkerStartStaggerMs(4, {
        AFORA_QA_SUITE_WORKER_START_STAGGER_MS: "25",
      }),
    ).toBe(25);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        resolveQaSuiteWorkerStartStaggerMs(4, {
          AFORA_QA_SUITE_WORKER_START_STAGGER_MS: value,
        }),
      ).toBe(1500);
    }
