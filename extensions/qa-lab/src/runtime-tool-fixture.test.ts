...
describe("runtime tool fixture", () => {
  ...
  it("skips Codex-native fixtures when only Afora dynamic exposure evidence is absent", async () => {
    const env = await makeEnv({
      ...
      gateway: {
        ...
        tempRoot: "",
        workspaceDir: "",
        runtimeEnv: { AFORA_QA_FORCE_RUNTIME: "codex" },
        call: vi.fn(),
      },
    });
  ...
  it.each([
    ...
    },
    {
      ...
      toolName: "web_search",
      happyArgs: { query: "Afora runtime parity fixed query" },
      happyOutput: "result",
  ...
  it("still fails required Afora dynamic fixtures when the tool is absent", async () => {
    const env = await makeEnv();
