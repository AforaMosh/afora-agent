...
export function readToolSearchGatewayFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "AFORA_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv("AFORA_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS", 5_000, env),
  };
}
...
async function writeFakePlugin(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        name: "@afora/tool-search-e2e-fixture",
        version: "0.0.0",
        type: "module",
        afora: {
          extensions: ["./index.js"],
...
export async function runToolSearchGatewayLane(params: {
  ...
  const gatewayToken = params.env.gateway.runtimeEnv.AFORA_GATEWAY_TOKEN;
