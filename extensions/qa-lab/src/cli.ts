...
export function registerQaLabCli(program: Command) {
  ...
  qa.command("suite")
    ...
    .option("--runtime-pair <pair>", "Run each scenario under both runtimes, e.g. afora,codex")
    .option(
  ...
  qa.command("jsonl-replay")
    ...
    .option("--runtime-pair <pair>", "Runtime pair label, e.g. afora,codex", "afora,codex")
    .option(
      "--provider-mode <mode>",
  ...
  credentials
    ...
    .option("--site-url <url>", "Override AFORA_QA_CONVEX_SITE_URL")
    .option("--endpoint-prefix <path>", "Override AFORA_QA_CONVEX_ENDPOINT_PREFIX")
  ...
  qa.command("docker-scaffold")
    ...
    .option("--image <name>", "Prebaked image name", "afora:qa-local-prebaked")
  ...
  qa.command("docker-build-image")
    ...
    .option("--repo-root <path>", "Repository root to target when running from a neutral cwd")
    .option("--image <name>", "Image tag", "afora:qa-local-prebaked")
