...
type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { afora: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { afora: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      afora: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.afora.push(parity.cells.afora);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# Afora JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
