...
export function renderCaptureStartupInstructions(status: CaptureStartupStatus | null): string {
  ...
  const gatewayStart = `AFORA_DEBUG_PROXY_ENABLED=1 \\
AFORA_DEBUG_PROXY_REQUIRE=1 \\
AFORA_DEBUG_PROXY_URL=http://127.0.0.1:7799 \\
pnpm afora gateway --port 18789 --bind loopback`;
  const qaStart = "pnpm qa:lab:ui --port 43124 --control-ui-url http://127.0.0.1:18789/";
  const caInstall = "pnpm proxy:install-ca";
  const caTrust =
    'sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain "$HOME/.afora/debug-proxy/certs/root-ca.pem"';
