...
function resolveGoogleChatAccountImpl(params: { cfg: AforaConfig; accountId?: string | null }) {
...
vi.mock("./channel.deps.runtime.js", () => {
  return {
    ...
    listGoogleChatAccountIds: (cfg: AforaConfig) => {
...
function createGoogleChatCfg(): AforaConfig {
  return {
    channels: {
      googlechat: {
        enabled: true,
