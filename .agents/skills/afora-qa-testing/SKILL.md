---
name: afora-qa-testing
description: Run, watch, debug, extend, or explain Afora qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
5. If the user wants to watch the live UI, find the current `afora-qa` listen port and report `http://127.0.0.1:<port>`.
...
- That item is the first place to look for:
  ...
  - `AFORA_QA_PROVIDER_MODE`
  - `AFORA_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `Afora`
  - item: `AFORA_QA_CONVEX_SITE_URL`
  - item: `AFORA_QA_CONVEX_SECRET_MAINTAINER`
  - item: `AFORA_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `AFORA QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `AFORA_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `AFORA_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `AforaMosh/afora-agent_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
AFORA_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
AFORA_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
AFORA_QA_PROVIDER_MODE="mock-openai" \
AFORA_NPM_TELEGRAM_PACKAGE_SPEC="afora@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `AFORA_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `AFORA_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `AFORA_QA_PROVIDER_MODE`, map it explicitly to `AFORA_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `AFORA_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo AforaMosh/afora-agent --ref main \
  -f package_spec=afora@YYYY.M.D-beta.N \
  -f package_label=afora@YYYY.M.D-beta.N \
...
```bash
gh api repos/AforaMosh/afora-agent/actions/runs/<run-id>/artifacts
```
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal Afora WhatsApp accounts in the shared pool.
...
- Validate with `pnpm afora qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- Packaged `pnpm build` omits QA Lab + qa-channel by design (source-checkout
  only). To exercise `afora qa`/qa-channel from a built dist, build with
  `AFORA_BUILD_PRIVATE_QA=1 pnpm build` (emits `dist/plugin-sdk/qa-lab.js`,
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Afora identity combines with the character.
...
```bash
pnpm afora qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `AFORA_HOME` sandboxed.
