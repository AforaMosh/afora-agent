---
name: afora-ci-limits
description: Manage Afora GitHub Actions and Blacksmith CI capacity, runner-registration budgets, fanout caps, main-push single-flight, shard sizing, hosted-runner offload, queue health, and safe ramp-down/ramp-up changes. Use when tuning `.github/workflows/*`, `docs/ci.md`, CI runner labels, matrix `max-parallel`, ClawSweeper/Blacksmith burst protection, CodeQL runner placement, or investigating slow/queued Afora CI.
---

# Afora CI Limits

Use this skill for CI capacity changes, not ordinary test failure triage. The
goal is to keep Afora fast while distinguishing runner registration, runner
...
- GitHub runner registrations for `afora` currently report a 10,000 per
  5-minute bucket in `actions_runner_registration`. Verify the live bucket
  before each tuning pass because GitHub can change it. The `afora`
...
- **Actions-artifact checkout (2026-08-16):** Do not recommend replacing the
  ...
  artifact. [PR #124818](https://github.com/AforaMosh/afora-agent/pull/124818)
  ...
  worse: [run 31971531521](https://github.com/AforaMosh/afora-agent/actions/runs/31971531521)
...
ghx run list -R AforaMosh/afora-agent --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
ghx run list -R afora/clawsweeper --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
ghx api repos/afora/clawsweeper/actions/runs/<run-id>/jobs --paginate --jq '.jobs[] | {id,name,status,conclusion,labels,created_at,started_at,completed_at,runner_name,runner_group_name}'
blacksmith testbox list --all
curl -fsS https://clawsweeper.afora.ai/api/status | jq '{generated_at,fleet,diagnostics:{errors:.diagnostics.errors}}'
curl -fsS https://clawsweeper.afora.ai/api/exact-review-queue | jq '{generated_at,review:.lanes.review,publication:.lanes.publication,state_writer,state_append}'
...
- **Afora test runtime:** jobs start quickly but one lane dominates wall time.
  Use `$afora-test-performance` instead of runner tuning.
- **Real failing CI:** one job fails after starting. Use `$github:gh-fix-ci` or
  `$afora-testing`, not this skill.
...
ClawSweeper, ClawHub, Clownfish, Afora RTT, and Clawbench.
...
7. Split truly slow tests with `$afora-test-performance`; do not hide a slow
...
- `AFORA_CI_RUNNER_BACKEND=github` routes every configurable `ci.yml` job
...
./node_modules/.bin/oxfmt --check .github/workflows/ci.yml .github/workflows/codeql-critical-quality.yml docs/ci.md test/scripts/ci-workflow-guards.test.ts .agents/skills/afora-ci-limits/SKILL.md .agents/skills/afora-ci-limits/agents/openai.yaml
git diff --check
```
...
ghx pr checks <pr> -R AforaMosh/afora-agent --watch --interval 15
```
...
AFORA_TESTBOX=1 scripts/pr prepare-run <pr>
```
...
```bash
ghx run list -R AforaMosh/afora-agent --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
for repo in afora/clawsweeper afora/clawhub afora/clownfish AforaMosh/afora-agent-rtt afora/clawbench; do
  ghx run list -R "$repo" --limit 12 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
done
curl -fsS https://clawsweeper.afora.ai/api/exact-review-queue | jq '.'
```
