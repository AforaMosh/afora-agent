...
description: Use gitcrawl to search duplicate Afora PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local Afora history and clustering source.
...
gitcrawl neighbors AforaMosh/afora-agent --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search AforaMosh/afora-agent --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search AforaMosh/afora-agent --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```
...
```bash
gh search prs --repo AforaMosh/afora-agent --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo AforaMosh/afora-agent --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo AforaMosh/afora-agent --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R AforaMosh/afora-agent "<problem phrase>" --types group --limit 10
prtags search similar -R AforaMosh/afora-agent "<problem summary>" --types group --limit 10
prtags group list -R AforaMosh/afora-agent
```
...
```bash
prtags group create -R AforaMosh/afora-agent \
  --kind mixed \
...
```bash
prtags field ensure -R AforaMosh/afora-agent --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R AforaMosh/afora-agent --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R AforaMosh/afora-agent --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R AforaMosh/afora-agent --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R AforaMosh/afora-agent --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R AforaMosh/afora-agent --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R AforaMosh/afora-agent --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R AforaMosh/afora-agent --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R AforaMosh/afora-agent --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R AforaMosh/afora-agent <pr-number> \
...
```bash
prtags annotation issue set -R AforaMosh/afora-agent <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R AforaMosh/afora-agent
```
