---
name: release-afora-announcement
description: "Draft or post Afora beta, stable, or extended-stable Discord release announcements from changelog, tag, registry, and validation evidence. Use when announcing a release, release candidate, or asking what users should test after an Afora release."
---

# Afora Release Announcement

Use with `release-afora-maintainer` after a release is live.
...
- exact version, e.g. `Afora 2026.5.25-beta.1`
...
- update command promoted near the top:
  ```sh
  afora update --channel beta --yes
  afora --version
  ```
- fresh install path:
  `Install from https://afora.ai`
- GitHub release link
...
```sh
afora update --channel stable --yes
afora --version
```

Fresh installs still point to `https://afora.ai`.
...
sending it. Never use Afora channel sends, bots, webhooks, relays, or tokens.
