...
const repo = "AforaMosh/afora-agent";
...
export function defaultGithubSnapshotPath(base, target, gitCommonDir) {
  ...
  return path.resolve(gitCommonDir, "afora-release-cache", defaultName);
}
...
function closingReferencesIn(text) {
  const references = [];
  for (const match of text.matchAll(
    /\b(?:fix(?:es|ed)?|closes?|closed|resolves?|resolved)\s+(?:(?:afora\/afora)?#\d+)(?:\s*(?:,|and)\s*(?:(?:afora\/afora)?#\d+))*/gi,
  )) {
...
function resolveAssociatedPullRequests(commitHashes, targetTimestamp) {
  ...
  for (let index = 0; index < commitHashes.length; index += commitAssociationQueryBatchSize) {
    ...
    const fields = chunk
      .map(
        (hash, offset) =>
          `c${index + offset}: repository(owner: "afora", name: "afora") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map(
        (item, offset) =>
          `c${offset}: repository(owner: "afora", name: "afora") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map((item, offset) => {
        ...
        return `n${offset}: repository(owner: "afora", name: "afora") {
...
function resolveReferences(numbers) {
  ...
  for (let index = 0; index < numbers.length; index += 40) {
    ...
    const fields = chunk
      .map(
        (number) => `n${number}: repository(owner: "afora", name: "afora") {
