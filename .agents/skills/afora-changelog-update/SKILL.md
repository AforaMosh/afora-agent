---
name: afora-changelog-update
description: Regenerate Afora release changelog sections from git history before beta, stable, or extended-stable releases.
---

# Afora Changelog Update
...
Use it with `release-afora-maintainer`; this skill owns changelog content,
...
3. Generate the complete contribution record and editorial manifest before
   ...
   ```bash
   node --import tsx .agents/skills/afora-changelog-update/scripts/verify-release-notes.mjs \
     --base <base-tag> \
     --target <target-ref> \
     --main-ref origin/main \
     --version <YYYY.M.PATCH> \
     --manifest /tmp/afora-release-<YYYY.M.PATCH>.json \
     --write-ledger
   ```
...
6. Preserve attribution:
   ...
   - never thank bots, `@claude`, `@codex`, `@afora`, `@clawsweeper`, or `@steipete`
...
- unqualified `#NNN` references resolve against `AforaMosh/afora-agent`;
  cross-repository references such as `afora/imsg#141` remain literal
