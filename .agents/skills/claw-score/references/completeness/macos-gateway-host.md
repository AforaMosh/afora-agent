...
- Gateway Service Lifecycle: Per-user Gateway LaunchAgent install, launchctl bootstrap, LaunchAgent labels, Gateway token/env handling, App-managed LaunchAgent handoff, afora update package/git handoff, Managed service refresh, Stale updater launchd job detection, afora uninstall, Stranded service recovery
- Diagnostics and Observability: LaunchAgent log paths, afora gateway status --deep, Gateway silently stops responding, Stale updater jobs
