---
name: release-afora-plugin-testing
description: Plan and run pre-release Afora plugin validation across bundled plugins, package artifacts, lifecycle commands, doctor/fix, config round-trip, gateway startup, SDK compatibility, Docker E2E, Package Acceptance, and Testbox proof.
---

# Afora Pre-Release Plugin Testing
...
release?" It complements `afora-testing`; use that skill too when choosing
...
the main Afora checkout. Do not run `pnpm install` there. For broad or
...
```bash
AFORA_TESTBOX=1 pnpm check:changed
pnpm run test:extensions:package-boundary:canary
pnpm run test:extensions:package-boundary:compile
pnpm test:docker:plugins
AFORA_PLUGINS_E2E_CLAWHUB=0 pnpm test:docker:plugins
...
```bash
AFORA_BUNDLED_PLUGIN_SWEEP_TOTAL=8 \
AFORA_BUNDLED_PLUGIN_SWEEP_INDEX=<0-7> \
...
Use `source=npm -f package_spec=afora@beta` for published beta proof. Keep
...
Start packaged Afora in Docker with clean state:
...
- `afora status --json` includes plugin diagnostics
- `afora plugins inspect --all --json` is parseable
...
`afora/plugin-sdk/*` subpaths. Assert `plugins inspect` sees its tool,
