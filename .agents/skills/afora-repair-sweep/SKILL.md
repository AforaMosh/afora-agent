---
name: afora-repair-sweep
description: "Orchestrate worker fleets over Afora issues and PRs: prove root causes, prefer clean refactors over quick patches, land verified repairs, and close items proven fixed or no longer useful."
---
...
`$gitcrawl` discovery/clusters, `$afora-pr-maintainer` live GitHub mutation
rules, `$github-author-context` for contributor trust, `$afora-testing`
...
- Choose proof with `$afora-testing`; live-test the real user/provider/
...
- Land main-targeted PRs only through the repo-native `scripts/pr` flow
  (review-init, artifacts, `AFORA_TESTBOX=1 scripts/pr prepare-run`,
...
Landed: https://github.com/AforaMosh/afora-agent/pull/123
```
