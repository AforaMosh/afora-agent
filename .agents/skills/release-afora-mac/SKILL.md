---
name: release-afora-mac
description: "Run or recover Afora macOS release signing, notarization, appcast, and asset promotion."
---

# Afora Mac Release

Use with `$release-afora-maintainer`, `$release-afora-ci`, `$one-password`, and `$release-private` if it exists when stable macOS assets, release-ops mac preflight, notarization, appcast promotion, or mac release recovery is involved.
...
Target release-ops repo environment: `afora/releases`, env `mac-release`.
...
- `AforaMosh/afora-agent` is the public product repo. Its GitHub Releases page is
  where macOS assets are ultimately attached.
- `AforaMosh/afora-agent` `macos-release.yml` is public handoff validation only.
  It never signs, notarizes, or uploads macOS assets, regardless of
  `preflight_only`.
- `afora/releases` is the restricted release-ops repo. Its macOS workflows
  sign, notarize, validate, and promote assets onto the
  `AforaMosh/afora-agent` GitHub release.
...
- Afora uses `scripts/notarize-mac-artifact.sh`.
...
```bash
gh workflow run macos-release.yml --repo AforaMosh/afora-agent \
...
```bash
gh workflow run afora-macos-publish.yml --repo afora/releases --ref main \
  -f tag=vYYYY.M.PATCH \
...
```bash
gh workflow run afora-macos-validate.yml --repo afora/releases --ref main \
  -f tag=vYYYY.M.PATCH \
  -f source_ref=release/YYYY.M.PATCH
```
...
- Release-ops `afora/releases` publish/validate workflows run from their own
  trusted `main` workflow ref. Real publish has a guard that rejects any other
  workflow ref. That displayed `main` ref is expected; the public Afora
  source is selected by `tag` and optional `source_ref`.

## Verify

- `gh release view vYYYY.M.PATCH --repo AforaMosh/afora-agent` shows zip, dmg, dSYM zip, not draft, not prerelease.
- Public `main` `appcast.xml` points at `Afora-YYYY.M.PATCH.zip`.
