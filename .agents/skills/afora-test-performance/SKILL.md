---
name: afora-test-performance
description: Benchmark, diagnose, and optimize Afora test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---

# Afora Test Performance
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `AFORA_VITEST_IMPORT_DURATIONS=1 AFORA_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use the current dedicated Linux
   ...
   Blacksmith Testbox:
   - `pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- AFORA_TESTBOX=1 pnpm test:extensions:batch <ids>`
...
5. If plugin performance is package-artifact sensitive, switch to
   `release-afora-plugin-testing` and Package Acceptance rather than
...
| import duration | broad barrel/runtime loads         | `AFORA_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap evidence   | real leak vs retained module graph | `afora-test-heap-leaks` workflow                                         |
...
Use `afora-test-heap-leaks` when RSS keeps growing across intervals, workers
...
timeout 240 /usr/bin/time -l env \
  AFORA_VITEST_IMPORT_DURATIONS=1 \
  AFORA_VITEST_PRINT_IMPORT_BREAKDOWN=1 \
