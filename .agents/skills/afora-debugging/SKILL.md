---
name: afora-debugging
description: Debug Afora model, provider, tool-surface, code-mode, streaming, and live/Crabbox behavior by choosing the right logs, probes, and proof path before changing code, including fetching stored sessions, transcripts, and attachments as evidence.
---

# Afora Debugging

Use this skill when Afora behavior differs between local tests, live models,
providers, code mode, Tool Search, Crabbox, or CI, and the next move should be a
debug signal rather than a guess.

## Read First

- `docs/logging.md` for log files, `afora logs`, and targeted debug flags.
- `docs/reference/test.md` for local test commands.
- `docs/tools/code-mode.md` for code-mode exec/wait and tool catalog rules.
- Use `$afora-testing` for choosing test lanes.
...
AFORA_DEBUG_MODEL_PAYLOAD=full-redacted AFORA_DEBUG_SSE=peek afora gateway
```

Useful flags:

- `AFORA_DEBUG_MODEL_TRANSPORT=1`: request start, fetch response, SDK
  headers, first SSE event, stream done, and transport errors at `info`.
- `AFORA_DEBUG_MODEL_PAYLOAD=summary`: bounded payload summary.
- `AFORA_DEBUG_MODEL_PAYLOAD=tools`: all model-facing tool names.
- `AFORA_DEBUG_MODEL_PAYLOAD=full-redacted`: capped, redacted JSON payload.
  Use only while debugging; prompts/message text may still appear.
- `AFORA_DEBUG_SSE=events`: first-event and stream-completion timing.
- `AFORA_DEBUG_SSE=peek`: first five redacted SSE events.
- `AFORA_DEBUG_CODE_MODE=1`: code-mode tool-surface diagnostics.
...
```bash
afora sessions list --agent <agentId> --json
afora sessions tail
afora sessions export-trajectory
```

Docs: `docs/reference/database-schemas.md` for the store layout,
https://docs.afora.ai/cli/sessions for the CLI.

Raw store (when the CLI is unavailable, e.g. inspecting a remote host over
SSH, or you need event-level detail):

- Per-agent data plane: `~/.afora/agents/<agentId>/agent/afora-agent.sqlite`.
  Canonical schema: `src/state/afora-agent-schema.sql`.
- Hosted/systemd installs keep state under the service user's home (e.g.
  `/home/afora/.afora/...`), not root's — root may carry a separate
  stray install with different agents. If the expected agent dir is missing,
  locate the real DB: `find / -maxdepth 6 -name afora-agent.sqlite`.
...
- Sender provenance: real user messages carry `message.__afora`
...
- User-attached images are not `image` content parts. They ride the message
  envelope: `message.__afora.media[]` entries with `url`
