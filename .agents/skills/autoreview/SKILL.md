...
```bash
# Source checkout of afora/agent-skills:
...
printed before execution. Set `AFORA_TESTBOX=1` on the autoreview process, not
inside the test command, because the environment snapshot and credential staging
happen before the test shell starts:

```bash
AFORA_TESTBOX=1 "$AUTOREVIEW" --parallel-tests "pnpm check:changed"
```
...
| ------------------- | -------------------------------------------------- | ----------------------------------------------------- |
| **codex** (default) | `gpt-5.6-sol` -> `gpt-5.6-terra` on access failure | Afora org review default                           |
