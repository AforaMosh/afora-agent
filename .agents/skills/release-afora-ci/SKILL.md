---
name: release-afora-ci
description: "Run, watch, debug, and summarize Afora full release CI, release checks, live provider gates, install/update proofs, and release-secret preflights."
---

# Afora Release CI

Use this with `$release-afora-maintainer` and `$afora-testing` when a release candidate needs full validation, install/update proof, live provider checks, or CI recovery.
...
- Reused Testboxes are provenance-gated after their first successful run.
  ...
  `AFORA_TESTBOX_ALLOW_STALE=1` for release evidence.
...
```bash
node .agents/skills/release-afora-ci/scripts/verify-provider-secrets.mjs --required openai,anthropic,fireworks
...
gh workflow run afora-performance.yml \
...
Publish with `afora-release-publish.yml` using `release_profile=from-validation`
...
2. List failed jobs only:
   ```bash
   gh run view <child-run-id> --repo AforaMosh/afora-agent --json jobs \
