---
name: release-afora-maintainer
description: Prepare or verify Afora stable, beta, and extended-stable releases, including backport discovery, changelogs, release notes, publish commands, and artifacts.
---

# Afora Release Maintainer
...
- Core `afora` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- As soon as the Code SHA exists, dispatch `Afora Performance`
...
- In maintainer Testbox mode, use `AFORA_TESTBOX=1 scripts/pr prepare-run
<PR>` only after the exact PR head has passed `CI` and every scheduled
  ...
  `AFORA_TESTBOX=1 scripts/pr prepare-sync-head <PR>`, wait for those hosted
...
- If an exact PR-head CI run has no active jobs because Blacksmith capacity is
  ...
  `gh api 'repos/AforaMosh/afora-agent/contents/.github/workflows/ci.yml?ref=<pr-head-branch>'
--jq .content | base64 --decode | rg -q 'pull_request_number:'`. If absent,
  ...
  `gh workflow run ci.yml --repo AforaMosh/afora-agent --ref <pr-head-branch> -f
...
include_android=true -f release_gate=true`.
  ...
  cancelled, or been skipped. Then rerun `AFORA_TESTBOX=1 scripts/pr
prepare-run <PR>`.
- Generate the changelog once after the final Code SHA is fully green. Do not
  ...
  `$afora-changelog-update` for the rewrite. Do not continue release prep if
...
distribution: the `afora` npm package, official npm plugins, and matching
Docker Gateway images. Treat
`docs/reference/RELEASING.md`,
`scripts/afora-npm-extended-stable-release.mjs`, and the release workflows
...
5. Run `pnpm release:generated:check`, `pnpm deps:npm-lock:check`, and
   `AFORA_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main`
...
6. Keep repository variables `RELEASE_ROLLBACK_DRILL_ID` and
   ...
   `afora-stable-main-closeout.yml` starts from the `main` push carrying the
...
- Version locations include:
  ...
  - `apps/macos/Sources/Afora/Resources/Info.plist`
...
- Every stable Afora release ships the npm package, macOS app, and signed
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/AforaMosh/afora-agent/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `afora` repo.
...
- Stable Windows Hub release closeout requires the signed
  `AforaCompanion-Setup-x64.exe`, `AforaCompanion-Setup-arm64.exe`, and
  `AforaCompanion-SHA256SUMS.txt` assets on the canonical
  `AforaMosh/afora-agent` GitHub Release. Pass the exact signed
  `AforaMosh/afora-agent-windows-node` release tag as `windows_node_tag` to
  `Afora Release Publish`, together with the candidate-approved
  ...
  dispatches the public `Windows Node Release` workflow while the Afora
  release is still a draft, carries those pinned source asset digests
  unchanged, verifies the expected Afora Foundation Authenticode signer on
  ...
  `AforaCompanion-*` target asset names, then replaces the expected contract
  assets with the pinned source bytes.
- Website Windows Hub download links should target exact canonical
  `AforaMosh/afora-agent/releases/download/vYYYY.M.PATCH/...` assets for the current
  ...
  is visible from both the GitHub release page and afora.ai.
...
- Generate `$afora-changelog-update`'s full contribution manifest before
...
- Before publishing or closing a release, run
  `$afora-changelog-update`'s `verify-release-notes.mjs` with every stable
...
- To update an existing GitHub Release body, resolve the numeric release id and
  ...
  `gh api repos/AforaMosh/afora-agent/releases/tags/vYYYY.M.PATCH --jq .id`, then
  `gh api -X PATCH repos/AforaMosh/afora-agent/releases/<id> -F body=@/tmp/notes.md`.
  Do not trust `gh release edit --notes-file` or `--input` JSON if verification
  disagrees; verify with `gh api repos/AforaMosh/afora-agent/releases/<id>` because
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `afora YYYY.M.PATCH-beta.N`
...
Use the Afora account's existing release-post style:

- Format: `Afora YYYY.M.PATCH 🦞` or `🦞 Afora YYYY.M.PATCH is live`, blank line,
  then 3-4 emoji-led bullets, blank line, one short punchline, then the release
  link.
- For beta: say `Afora YYYY.M.PATCH-beta.N 🦞` or `Afora YYYY.M.PATCH beta N is
...
- Before tagging, diff publishable plugin package manifests against the last
  ...
  (`afora.release.publishToNpm: true` or `publishToClawHub: true`) whose
...
- Bootstrap a new ClawHub package only from the trusted workflow source:
  `gh workflow run plugin-clawhub-new.yml --ref main -f plugins=@afora/name -f ref=<full-release-sha> -f pretag_validation=true -f dry_run=true`.
...
```bash
  AFORA_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/afora-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `AFORA_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `AFORA_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the Afora npm release path before creating the tag.
...
- Post-published beta verification roster:
  - `node --import tsx scripts/afora-npm-postpublish-verify.ts <beta-version>`
  ...
  - published npm Telegram proof: dispatch Actions > `NPM Telegram Beta E2E`
    from `main` with `package_spec=afora@<beta-version>` and
    ...
    `AFORA_NPM_TELEGRAM_PACKAGE_SPEC` and Convex CI env only as a fallback
    or debugging path.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `AFORA_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  ...
  - `180m` for local full QA live OpenAI + Anthropic rosters when explicitly
    ...
    `QA-Lab - All Lanes`
  - Parallels caps from the `afora-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `afora update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `AforaMosh/afora-agent` and the release-ops mac preflight in
  `afora/releases` for every release.
...
- Afora publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` uses the restricted release-ops
  `afora/releases/.github/workflows/afora-npm-dist-tags.yml` workflow
...
- Prefer fixing the release-ops workflow token path over any local 1Password
  ...
  repo's `NPM_TOKEN` secret, scoped to the `afora` package with read/write
...
- If the release-ops dist-tag workflow cannot promote because `NPM_TOKEN` is
  ...
  - Promote with a fresh OTP:
    `npm dist-tag add afora@YYYY.M.PATCH latest --otp "$OTP"`.
  - Verify with a cache-bypassed registry read, for example:
    `npm view afora dist-tags --json --prefer-online --cache /tmp/afora-npm-cache-verify-$$`
    and `npm view afora@latest version dist.tarball --json --prefer-online`.
...
- npm registry metadata is eventually consistent immediately after trusted
  ...
  steps instead of reading the registry again. If the Afora npm child
...
- `.github/workflows/macos-release.yml` in `AforaMosh/afora-agent` is now a
  ...
  artifacts.
- `afora/releases/.github/workflows/afora-macos-validate.yml` is the
...
- Real mac preflight and real mac publish both use
  `afora/releases/.github/workflows/afora-macos-publish.yml`.
...
- Focused plugin-only repairs use `plugin_publish_scope=selected` with a nonempty
  ...
  is disabled.
- Dispatch regular beta and stable `Afora Release Publish` runs from trusted
...
- The `npm-release` environment must be approved by `@AforaMosh/afora-agent-release-managers` before publish continues.
- Mac publish uses
  `afora/releases/.github/workflows/afora-macos-publish.yml` for
...
- Real release-ops mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `AforaMosh/afora-agent`
  automatically when `AFORA_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the Afora package
...
- `@afora/*` plugin publishes use a separate maintainer-only flow.
...
7. Immediately dispatch Actions > `Afora Performance` from the pinned
...
20. For stable releases, start `.github/workflows/macos-release.yml` in
    `AforaMosh/afora-agent` and wait for the public validation-only run to pass.
21. For stable releases, start
    `afora/releases/.github/workflows/afora-macos-validate.yml` with the
    same tag and wait for the release-ops mac validation lane to pass.
22. For stable releases, start
    `afora/releases/.github/workflows/afora-macos-publish.yml` with
...
23. Classify every failure before changing git state. Confirmed product defects return to
    ...
    rerun `all`.
24. Start `.github/workflows/afora-release-publish.yml` from the exact pinned
    ...
    `AforaMosh/afora-agent-windows-node` tag as `windows_node_tag` and its
    candidate-approved installer digest map as `windows_node_installer_digests`.
25. Wait for `npm-release` approval from `@AforaMosh/afora-agent-release-managers`.
26. Wait for the real publish workflow to run postpublish verification,
    ...
    waited plugin publish or Windows Hub promotion fails after Afora npm
    succeeds, the workflow keeps the release draft with Afora npm evidence
...
32. For stable releases, start
    `afora/releases/.github/workflows/afora-macos-publish.yml` for the
...
- Use `afora-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
