...
`afora` npm package, official npm plugins, and matching Docker Gateway
...
- Read `docs/reference/RELEASING.md`,
  `scripts/afora-npm-extended-stable-release.mjs`, and the relevant release
...
- Cover the core `afora` package and every npm-publishable official plugin
...
- Read `SECURITY.md` and use `$security-triage` for security candidates. Route
  unpublished advisory work through `$afora-ghsa-maintainer`; never expose
  private details before the security owner authorizes disclosure.
- Use `$afora-testing` for proof selection, `$autoreview` before handoff,
  and `$afora-pr-maintainer` for GitHub operations.
...
dist_tags=$(npm view afora dist-tags --json --userconfig "$npm_userconfig")
published_version=$(printf '%s' "$dist_tags" | jq -r '."extended-stable" // empty')
if [[ -n "$published_version" ]]; then
  npm view "afora@${published_version}" version \
...
- mode, published `afora@extended-stable` version or approved bootstrap
