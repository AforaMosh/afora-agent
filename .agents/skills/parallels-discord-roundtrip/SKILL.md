...
pnpm test:parallels:macos \
  --discord-token-env AFORA_PARALLELS_DISCORD_TOKEN \
...
- Use the `afora` wrapper for guest `message send/read`; `node afora.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/afora-parallels-smoke.*`
...
- host inbound nonce appears in `afora message read` output
