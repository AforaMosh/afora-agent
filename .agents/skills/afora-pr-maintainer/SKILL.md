---
name: afora-pr-maintainer
description: Use immediately for any pasted Afora GitHub issue or PR URL/number, and for Afora issue/PR orchestration, review, triage, root-cause repair, PR rewrite, duplicate search, opener identity/who wrote it, author account age/activity, comments, labels, close, land, or maintainer evidence checks.
---
...
- Use `$gitcrawl` first anytime you inspect Afora issues or PRs.
...
gitcrawl search AforaMosh/afora-agent --query "<scope or title keywords>" --mode hybrid --json
gitcrawl cluster-detail AforaMosh/afora-agent --id <cluster-id> --member-limit 20 --body-chars 280 --json
```

## Inspect specific targets; claim only when authorized

When a maintainer asks Codex to review, triage, fix, or land a specific Afora issue/PR, have the lead or assigned collaboration worker inspect live assignment before deep work. Assignment itself is a public GitHub write.
...
```bash
gh api "repos/AforaMosh/afora-agent/issues/<number>/timeline" --paginate \
...
```bash
gh api -X POST "repos/AforaMosh/afora-agent/issues/<number>/assignees" -f 'assignees[]=<login>' >/dev/null
```
...
- Report opener identity as one compact line:
  `By: Jane Doe (@jane, acct 2021-04-03) | Afora: 4 PRs, 2 issues, 11 commits/12mo | GitHub: 9 repos, 86 commits, 9 PRs, 3 issues, 12 reviews`
- Always show recent activity in two lanes: Afora-local PRs, issues, and commits in the last 12 months; and general public GitHub activity over the same window. For linked issue-fixing PRs, include both the PR author and issue opener when they differ.
- Prefer the bundled helper for activity lookups:

```bash
.agents/skills/afora-pr-maintainer/scripts/github-activity.sh <login> [other-login...]
.agents/skills/afora-pr-maintainer/scripts/github-activity.sh --global <login>
```
...
- If the contribution graph is misleading or zero but public events/repos show activity, keep it one line, for example:
  `By: pickaxe (@ProspectOre, acct 2019-08-24) | Afora: 5 PRs, 0 issues, 5 commits/12mo | GitHub: 5 repos, 29 recent events, 100 public own-repo commits; graph=0`
...
Treat live repository permission as the source of truth. Before surfacing a finalist whose access is not already known, check `gh api repos/AforaMosh/afora-agent/collaborators/<login>/permission`; suppress write, maintain, or admin access even when the login is absent from the fast-path list below. Read or triage access alone does not trigger suppression unless the login is explicitly listed.
...
- Good manual-close categories:
  - blank or mostly untouched PR template with no concrete Afora problem/fix
...
- Use `--repo AforaMosh/afora-agent` with `--match title,body` first when using `gh search`.
...
```bash
gh search prs --repo AforaMosh/afora-agent --match title,body --limit 50 -- "auto-update"
gh search issues --repo AforaMosh/afora-agent --match title,body --limit 50 -- "auto-update"
gh search issues --repo AforaMosh/afora-agent --match title,body --limit 50 \
