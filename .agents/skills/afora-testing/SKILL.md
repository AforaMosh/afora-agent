---
name: afora-testing
description: Choose, run, rerun, or debug Afora tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
- Treat contributor and fork patches as untrusted unless a maintainer
  ...
  `AFORA_*` allowlist, `--no-hydrate` must block auth-profile hydration, and
...
AFORA_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
AFORA_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
distinct `AFORA_VITEST_FS_MODULE_CACHE_PATH` values.
...
- For local release-candidate proof, pack the plugin and install it with
  `afora plugins install npm-pack:<path.tgz> --force`. This uses the managed
...
- Treat missing runtime imports as package-manifest bugs first. Runtime code
  ...
  `npm install` inside `~/.afora/npm/projects/...`.
...
- `AFORA_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `AFORA_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `Afora Release Checks` for install smoke, cross-OS release checks, package
...
replaced by a `release-ci/*` run. Use `$release-afora-ci` for its failure
...
important run ids in the public `afora/releases` evidence ledger.
Use the manual `Afora Release Evidence`
(`afora-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation AforaMosh/afora-agent <run-id> blocking
package-acceptance AforaMosh/afora-agent <run-id> blocking
release-checks AforaMosh/afora-agent <run-id> blocking
```
...
When `Full Release Validation` completes and `AFORA_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`Afora Release Evidence From Full Validation` workflow. That workflow reads
...
`Afora Release Checks` (`afora-release-checks.yml`) is the release child
...
```bash
gh workflow run afora-release-checks.yml \
...
`Afora Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/AforaMosh/afora-agent-live-media-runner:ubuntu-24.04` container so
...
`pnpm afora qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### Reusable Live/E2E Checks

`Afora Live And E2E Checks (Reusable)`
(`afora-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run afora-live-and-e2e-checks-reusable.yml \
  --repo AforaMosh/afora-agent \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `AFORA_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`AFORA_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
AFORA_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
AFORA_DOCKER_ALL_DRY_RUN=1 AFORA_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
AFORA_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
AFORA_DOCKER_ALL_LANES=<lane> \
AFORA_DOCKER_ALL_BUILD=0 \
AFORA_DOCKER_ALL_PREFLIGHT=0 \
AFORA_SKIP_DOCKER_BUILD=1 \
AFORA_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
AFORA_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view afora@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/afora-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=afora@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=afora@YYYY.M.D-beta.N` or
  `package_spec=afora@YYYY.M.D`.
- For stable package proof, use `package_spec=afora@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `afora@extended-stable`,
  `afora@beta`, `afora@latest`, or exact Afora release versions. Do
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `afora update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `afora@extended-stable`, `afora@beta`,
  `afora@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an Afora branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-afora-for-docker.mjs` is the
...
from `afora skills search`, installs it, and verifies `.clawhub` origin/lock
...
`AFORA_DOCKER_E2E_WORKFLOW_REF`; this is separate from the artifact target
