---
name: prototype-afora-tui
description: Build throwaway, fixture-driven Afora Clack or Pi TUI prototypes and compare multiple interactive variants side by side in tmux without running the full application or touching live state.
user-invocable: true
...
Use this skill to isolate one Afora terminal surface, render it with the
real Afora TUI stack, and compare two to six design variants in tmux.
...
- Use the real Afora renderer, theme, copy, and component where practical.
...
- Do not run the full Afora application when the isolated surface is enough.
...
- Agent shell, chat, overlay, editor, selector, or `src/tui/**` component:
  use `@earendil-works/pi-tui` and the existing Afora component.
...
```bash
.agents/skills/prototype-afora-tui/scripts/launch-tmux-grid.sh \
  --open \
