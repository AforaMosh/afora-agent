...
const DEFAULT_EXPECTED_ORIGIN = "AforaMosh/afora-agent";
...
const GATEWAY_STARTUP_TRACE_ENV = "AFORA_GATEWAY_STARTUP_TRACE";
...
export function classifyActions(
  ...
  const generatedMacProtocolChanged = changedPaths.some((changedPath) =>
    changedPath.startsWith("apps/shared/AforaKit/Sources/AforaProtocol/"),
  );
  const runMacos =
  ...
  const macUiVerification =
    ...
    changedPaths.some((changedPath) =>
      /^(?:apps\/macos\/Sources\/|apps\/shared\/AforaKit\/Sources\/|apps\/swabble\/Sources\/)/u.test(
        changedPath,
      ),
    );
...
function defaultLockPath(checkout) {
  ...
  return path.join(tmpdir(), `afora-live-updater-${key}.lock`);
}

function defaultStatePath(checkout) {
  return path.join(realpathSync(checkout), ".git", "afora-live-updater-state.json");
}
...
function writeMaintenanceState(statePath, state) {
  ...
  const temporary = path.join(directory, `.afora-live-updater-${process.pid}-${randomUUID()}`);
...
function resolveManagedGatewayCommand(
  ...
  const defaultEnvDir = path.join(stateDir ?? path.join(home, ".afora"), "service-env");
...
function readManagedGatewayLaunchAgent(checkout) {
  ...
  const plistPath = path.join(home, "Library/LaunchAgents/ai.afora.gateway.plist");
  let plistStat;
  try {
  ...
  const stateDir =
    typeof environmentVariables?.AFORA_STATE_DIR === "string"
      ? environmentVariables.AFORA_STATE_DIR
      : path.join(home, ".afora");
  ...
  const configPath =
    typeof plist?.EnvironmentVariables?.AFORA_CONFIG_PATH === "string"
      ? plist.EnvironmentVariables.AFORA_CONFIG_PATH
      : path.join(gatewayCommand.stateDir, "afora.json");
  return {
    configPath,
...
function inspectManagedGatewayDeployment(checkout) {
  ...
  const home = process.env.HOME;
  if (!home || !existsSync(path.join(home, "Library/LaunchAgents/ai.afora.gateway.plist"))) {
    return null;
  }
...
function prepareLaunchAgentEntrypointReplacement(deployment, entrypoint, options = {}) {
  const temporaryPath = `${deployment.plistPath}.afora-live-updater-${randomUUID()}`;
  ...
  try {
    ...
    const restore = () => {
      ...
      const rollbackPath = `${deployment.plistPath}.afora-live-updater-rollback-${randomUUID()}`;
      try {
...
export function runBuiltGatewayCli(checkout, args, deployment, options = {}) {
  ...
  delete baseEnv.AFORA_GATEWAY_URL;
  delete baseEnv.AFORA_GATEWAY_TOKEN;
  delete baseEnv.AFORA_GATEWAY_PASSWORD;
  delete baseEnv.AFORA_CONFIG_PATH;
  delete baseEnv.AFORA_GATEWAY_PORT;
  ...
  if (wrapperPath && envFilePath) {
    ...
    try {
      const configuredPath = execFileSync(
        executable,
        [...wrapperPrefix, "/usr/bin/printenv", "AFORA_CONFIG_PATH"],
        boundedSyncOptions({
          encoding: "utf8",
          env: baseEnv,
  ...
  const overlayPath = path.join(
    ...
    `.afora-live-updater-config-${randomUUID()}.json`,
  );
  writeFileSync(
    overlayPath,
    `${JSON.stringify({
  ...
  const envOverrides = [`AFORA_CONFIG_PATH=${overlayPath}`, `AFORA_GATEWAY_PORT=${port}`];
  const env = Object.assign(baseEnv, {
    AFORA_CONFIG_PATH: overlayPath,
...
export function prepareGatewaySuspension(
  ...
  const requestId = `afora-live-updater-${randomUUID()}`;
...
async function runBuildWithPreservedMacApp(
  ...
  const appBundle = path.join(checkout, "dist/Afora.app");
  ...
  const preservedBundle = path.join(
    checkout,
    ".git",
    `.afora-live-mac-${process.pid}-${randomUUID()}.app`,
  );
...
function armLaunchdEnvironmentRestore(name, previousValue) {
  const markerPath = path.join(
    tmpdir(),
    `.afora-launchd-env-restore-${process.pid}-${randomUUID()}`,
  );
...
async function verifyGatewayDeepRpc(runCommand, checkout, expectedSha, deployment, now) {
  ...
  if (deployment) {
    ...
    await runUpdateCommand(
      ...
      "gateway.status",
      "pnpm",
      ["afora", "gateway", "status", "--deep", "--require-rpc", "--json"],
      checkout,
      {
...
function isCurrentGatewayLogSource(source, sourceRoot, managedSourceRoots) {
  ...
  while (candidate !== path.dirname(candidate)) {
    ...
    if (existsSync(packagePath) && existsSync(gitPath)) {
      try {
        if (JSON.parse(readFileSync(packagePath, "utf8")).name === "afora") {
...
function readFallbackGatewayLogs(sinceMs) {
  ...
  const directories = new Set(["/tmp/afora", path.join(tmpdir(), "afora")]);
  const contents = [];
  for (const directory of directories) {
    for (const date of dates) {
      const logPath = path.join(directory, `afora-${date}.log`);
...
async function defaultVerifyMacTarget(checkout) {
  await delay(10_000);
  const executable = path.join(checkout, "dist/Afora.app/Contents/MacOS/Afora");
