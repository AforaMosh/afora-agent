...
description: "Triage Afora security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof."
---

# Security Triage

Use when reviewing Afora security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/AforaMosh/afora-agent/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view afora version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/AforaMosh/afora-agent/security-advisories/<GHSA>
gh api /repos/AforaMosh/afora-agent/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view afora version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo AforaMosh/afora-agent --match title,body,comments -- "<terms>"
gh search prs --repo AforaMosh/afora-agent --match title,body,comments -- "<terms>"
```
