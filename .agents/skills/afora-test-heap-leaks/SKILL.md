---
name: afora-test-heap-leaks
description: Investigate Afora pnpm test memory growth, Vitest OOMs, RSS spikes, and heap snapshot deltas.
---

# Afora Test Heap Leaks

Use this skill for test-memory investigations. Do not guess from RSS alone when heap snapshots are available. Treat snapshot-name deltas as triage evidence, not proof, until retainers or dominators support the call.

Read `../afora-test-performance/SKILL.md` first for the current test-performance commands and proof routing.
...
2. Collect the strongest available heap evidence.
   ...
   - When a dedicated harness or another known producer emits repeated `.heapsnapshot` files, compare snapshots from the same PID with `.agents/skills/afora-test-heap-leaks/scripts/heapsnapshot-delta.mjs`.
...
- Direct comparison:
  - `node .agents/skills/afora-test-heap-leaks/scripts/heapsnapshot-delta.mjs before.heapsnapshot after.heapsnapshot`
- Auto-select earliest/latest snapshots per PID within one lane:
  - `node .agents/skills/afora-test-heap-leaks/scripts/heapsnapshot-delta.mjs --lane-dir <snapshot-directory>`
- Useful flags:
  - `--top 40`
  - `--min-kb 32`
  - `--pid 16133`
...
```
node .agents/skills/afora-test-heap-leaks/scripts/heapsnapshot-delta.mjs \
