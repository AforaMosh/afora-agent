---
name: clawsweeper
description: "Use for all ClawSweeper work: Afora issue/PR sweep reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one Afora
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `afora/clawsweeper` for comment-router
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/afora-afora/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/afora/inbox/clawsweeper-afora-afora-123.md
pnpm run repair:dispatch -- jobs/afora/inbox/clawsweeper-afora-afora-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo afora/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo afora/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo afora/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo afora/clawsweeper --body 1
```
...
@afora-clawsweeper fix ci
@afora-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo AforaMosh/afora-agent --lookback-minutes 180
pnpm run repair:comment-router -- --repo AforaMosh/afora-agent --execute --wait-for-capacity
```
...
leave the local Afora checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central Afora security handling.
...
```bash
gh run list --repo afora/clawsweeper --workflow sweep.yml \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo afora/clawsweeper --workflow repair-cluster-worker.yml \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo afora/clawsweeper --workflow repair-comment-router.yml \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo afora/clawsweeper --workflow github-activity.yml \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo AforaMosh/afora-agent --workflow "ClawSweeper Dispatch" \
