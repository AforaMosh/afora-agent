...
export async function verifyExtendedStableRegistryReadback({
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    const exactResult = await query(`afora@${expectedVersion}`);
    const extendedStableResult = await query("afora@extended-stable");
  ...
  throw new Error(
    `npm registry did not converge to afora@${expectedVersion} and afora@extended-stable=${expectedVersion} after ${attempts} attempts (exact=${exactVersion}, extended-stable=${extendedStableSelector}).`,
  );
}
...
export function extendedStableSelectorRepairCommand(expectedVersion) {
  ...
  return `npm dist-tag add afora@${parsed.version} extended-stable`;
}
...
async function main() {
  ...
  if (command === "capture-selector") {
    const previous = capturePriorExtendedStableSelector({
      query: () =>
        spawnSync("npm", ["view", "afora", "dist-tags", "--json"], { encoding: "utf8" }),
    });
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error(`afora-npm-extended-stable-release: ${message}`);
    process.exit(1);
  }
}
