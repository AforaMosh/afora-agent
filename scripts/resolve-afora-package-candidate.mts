...
const DEFAULT_OUTPUT_NAME = "afora-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "AFORA_TRUSTED_PACKAGE_TOKEN";
...
export const AFORA_PACKAGE_SPEC_RE =
  /^afora@(alpha|beta|extended-stable|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node --import tsx scripts/resolve-afora-package-candidate.mts --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedAforaTarballFilename(value: unknown) {
...
export function resolveNpmPackageCandidatePackRunner(
  ...
  validateAforaPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref: string) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Afora branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref: string) {
  ...
  const sourceDir = path.join(
    ...
    `afora-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir: string, packOutput: string, outputName: string) {
  ...
  if (parsed !== undefined) {
    ...
    if (packedFilename) {
      filename = resolvePackedAforaTarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        (trimmed.startsWith("afora-") ||
          trimmed.includes(":") ||
          trimmed.includes("/") ||
          trimmed.includes("\\"))
      ) {
        filename = resolvePackedAforaTarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename =
      entries
        .filter((entry) => {
          try {
            return resolvePackedAforaTarballFilename(entry) === entry;
          } catch {
            return false;
          }
        })
  ...
  if (!filename) {
    throw new Error(`npm pack produced no Afora tarball in ${outputDir}`);
  }
...
export const cleanPackedAforaTarballsForTest = cleanPackedAforaTarballs;
...
async function resolveCandidate(options: PackageCandidateOptions) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await cleanPackedAforaTarballs(outputDir);
  ...
  console.error(`Checking Afora package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-afora-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
  ...
  if (pkg.name !== "afora") {
    throw new Error(`package candidate must be named "afora"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
