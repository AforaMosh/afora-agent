#!/usr/bin/env bash
# Reset Afora like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "${ROOT_DIR}/scripts/lib/restart-mac-gateway.sh"
APP_BUNDLE="${AFORA_APP_BUNDLE:-}"
APP_EXECUTABLE_RELATIVE_PATH="Contents/MacOS/Afora"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Afora"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Afora"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Afora"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.afora.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/afora-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
LOCK_HELD=0
WAIT_FOR_LOCK=0
LOG_PATH="${AFORA_RESTART_LOG:-${TMPDIR:-/tmp}/afora-restart-${LOCK_KEY}.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${AFORA_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.afora/disable-launchagent"
ATTACH_ONLY=1
BACKGROUND_ONLY=0
TARGET_ONLY=0
TARGET_APP_BUNDLE="${ROOT_DIR}/dist/Afora.app"
TARGET_EXECUTABLE="${TARGET_APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}"
INSTALLED_EXECUTABLE="/Applications/Afora.app/${APP_EXECUTABLE_RELATIVE_PATH}"
STAGED_APP_DIR="${ROOT_DIR}/dist/.afora-replacement-${LOCK_KEY}-$$"
STAGED_APP_BUNDLE="${STAGED_APP_DIR}/Afora.app"
...
canonicalize_app_bundle() {
  ...
  if [[ ! -d "${APP_BUNDLE}" ]]; then
    fail "AFORA_APP_BUNDLE does not exist: ${APP_BUNDLE}"
  fi
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  --target-only    Restart only this checkout's dist app; fail if another Afora app is active"
      log ""
      log "Env:"
      log "  AFORA_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node afora.mjs daemon install --force --runtime node"
      log "  node afora.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.afora/disable-launchagent"
      log ""
...
if [[ "$TARGET_ONLY" -eq 1 && -n "$APP_BUNDLE" ]]; then
  fail "--target-only does not accept AFORA_APP_BUNDLE"
fi
if [[ -n "${AFORA_PROFILE:-}" ]]; then
  normalized_profile="$(printf '%s' "${AFORA_PROFILE}" | tr '[:upper:]' '[:lower:]')"
...
kill_all_afora() {
  ...
  [[ -z "$(afora_process_pids)" ]]
}

known_afora_executables() {
  if [[ -n "${APP_BUNDLE}" ]]; then
    printf '%s\n' "${APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}"
  fi
  printf '%s\n' \
    "${ROOT_DIR}/dist/Afora.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
    "/Applications/Afora.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
...
afora_process_pids() {
  ...
  done < <(known_afora_executables) | sort -u
}
...
foreign_afora_process_pids() {
  ps axww -o pid=,command= 2>/dev/null \
    | while read -r pid command_line; do
        ...
        if [[ "${executable}" == *"/Afora.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
          || "${executable}" == *"/apps/macos/.build/debug/Afora" \
          || "${executable}" == *"/apps/macos/.build-local/debug/Afora" \
          || "${executable}" == *"/apps/macos/.build/release/Afora" ]]; then
          printf '%s\n' "${pid}"
        fi
      done
}
...
print_managed_afora_supervisor_label() {
  local domain="$1"
  local label="$2"
  local job=""
...
managed_afora_supervisor_labels() {
  local jobs=""
  ...
  while IFS='|' read -r domain label; do
    ...
    print_managed_afora_supervisor_label "${domain}" "${label}" &
...
kill_managed_afora() {
  for _ in {1..10}; do
    local pids=""
    pids="$(managed_afora_process_pids)"
    if [[ -z "${pids}" ]]; then
      return 0
    fi
    while IFS= read -r pid; do
  ...
  remaining_pids="$(managed_afora_process_pids)"
  while IFS= read -r pid; do
    [[ -n "${pid}" ]] || continue
    kill -KILL "${pid}" 2>/dev/null || true
  done <<< "${remaining_pids}"
  sleep 0.3
  [[ -z "$(managed_afora_process_pids)" ]]
}

stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.afora.mac 2>/dev/null || true
}
...
if [[ "$TARGET_ONLY" -eq 1 ]]; then
  if ! managed_supervisors="$(managed_afora_supervisor_labels | sort -u | /usr/bin/paste -sd, -)"; then
    fail "Unable to inspect loaded launchd jobs before target-only restart"
  fi
  if [[ -n "${managed_supervisors}" ]]; then
    fail "Managed Afora app is supervised by launchd job(s): ${managed_supervisors}; stop those jobs before a target-only restart"
  fi
  if [[ -n "$(foreign_afora_process_pids)" ]]; then
    fail "Another Afora app or test process is active; target-only restart deferred"
  fi
  log "==> Keeping managed Afora running while the replacement builds"
else
  stop_launch_agent
  log "==> Killing existing Afora instances"
  if ! kill_all_afora; then
    fail "Afora instances did not exit after cleanup attempts"
  fi
fi
...
choose_app_bundle() {
  ...
  if [[ -d "${ROOT_DIR}/dist/Afora.app" ]]; then
    APP_BUNDLE="$(cd "${ROOT_DIR}/dist/Afora.app" && pwd -P)"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Afora.app missing Sparkle after packaging"
    fi
    return 0
  fi

  if [[ -d "/Applications/Afora.app" ]]; then
    APP_BUNDLE="$(cd "/Applications/Afora.app" && pwd -P)"
    return 0
  fi

  fail "App bundle not found. Set AFORA_APP_BUNDLE to your installed Afora.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -c "cd '${ROOT_DIR}' && node afora.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -c "cd '${ROOT_DIR}' && node afora.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".afora", "afora.json"), "utf8");
...
if [[ "$TARGET_ONLY" -eq 1 ]]; then
  if [[ -n "$(foreign_afora_process_pids)" ]]; then
    fail "Another Afora app or test process appeared during build; target-only restart deferred"
  fi
  log "==> Switching managed installed and exact target Afora instances"
  if ! kill_managed_afora; then
    fail "Managed Afora instances did not exit after cleanup attempts"
  fi
fi
...
if [[ -n "$(process_pids_matching "${APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}")" ]]; then
  log "OK: Afora is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -c "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.afora.gateway.plist' | head -n 40 || true"
fi
