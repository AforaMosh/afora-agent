...
Options:
  ...
  --keep                         Keep the synthetic AFORA_HOME and workspace after the run.
...
export function parseArgs(argv: string[]) {
  ...
  let mode = process.env.AFORA_MEMORY_FD_REPRO_MODE || "fixed";
  let outputDir = path.resolve(".artifacts", "memory-fd-repro", stamp);
  let keep = process.env.AFORA_MEMORY_FD_REPRO_KEEP === "1";
  let allowNonDarwin = process.env.AFORA_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1";
  ...
  fileCount ??= readPositiveNumberEnv("AFORA_MEMORY_FD_REPRO_FILES", DEFAULT_FILE_COUNT);
  maxWorkspaceRegFds ??= readNumberEnv(
    "AFORA_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS",
    DEFAULT_MAX_WORKSPACE_REG_FDS,
  );
  invokeTimeoutMs ??= readTimerTimeoutNumberEnv("AFORA_MEMORY_FD_REPRO_TIMEOUT_MS", 30_000);
  sampleDelayMs ??= readTimerTimeoutNumberEnv("AFORA_MEMORY_FD_REPRO_SAMPLE_DELAY_MS", 1_000, 0);
  settleDelayMs ??= readTimerTimeoutNumberEnv("AFORA_MEMORY_FD_REPRO_SETTLE_DELAY_MS", 5_000, 0);
...
/**
 * Writes isolated Afora config for the synthetic memory workspace.
 */
export function writeConfig({ homeDir, workspaceDir, port, token }: ConfigOptions) {
  const configDir = path.join(homeDir, ".afora");
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    AFORA_STATE_DIR: path.join(homeDir, ".afora"),
