...
BASE_IMAGE="${BASE_IMAGE:-afora-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-afora-sandbox-common:bookworm-slim}"
...
AFORA_DOCKER_BUILD_USE_BUILDX="${AFORA_DOCKER_BUILD_USE_BUILDX:-0}"
AFORA_DOCKER_BUILD_CACHE_FROM="${AFORA_DOCKER_BUILD_CACHE_FROM:-}"
AFORA_DOCKER_BUILD_CACHE_TO="${AFORA_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=afora.sandbox=1)
NOTE
