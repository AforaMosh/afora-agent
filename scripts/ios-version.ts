// Ios Version script supports Afora repository automation.
...
function main(argv = process.argv.slice(2)): number {
  ...
  if (options.format === "shell") {
    process.stdout.write(
      [
        `AFORA_IOS_VERSION=${version.canonicalVersion}`,
        `AFORA_APP_STORE_REVISION=${version.appStoreRevision ?? ""}`,
        `AFORA_APP_STORE_VERSION=${version.appStoreVersion ?? ""}`,
        `AFORA_MARKETING_VERSION=${version.marketingVersion}`,
        `AFORA_BUILD_VERSION=${version.buildVersion}`,
      ].join("\n") + "\n",
    );
  } else {
