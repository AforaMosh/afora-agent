...
// Afora Npm Prepublish Verify script supports Afora repository automation.
...
import { resolveNpmCommandInvocation } from "./afora-npm-release-check.ts";
...
type AforaNpmPrepublishVerifyArgs =
  | {
...
export function aforaNpmPrepublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/afora-npm-prepublish-verify.ts <tarball.tgz> [expected-version] [dependency.tgz ...]";
}

export function parseAforaNpmPrepublishVerifyArgs(
  argv: readonly string[],
): AforaNpmPrepublishVerifyArgs {
  ...
  if (!tarballPath) {
    throw new Error(aforaNpmPrepublishVerifyUsage());
  }
  if (tarballPath.startsWith("-")) {
    throw new Error(`Unknown afora npm prepublish verifier option: ${tarballPath}`);
  }

  const expectedVersion = args[1]?.trim();
  if (expectedVersion?.startsWith("-")) {
    throw new Error(`Unknown afora npm prepublish verifier option: ${expectedVersion}`);
  }
...
export function assertPreparedAforaAiDependency(params: {
  aiManifest: PackedPackageJson;
  rootManifest: PackedPackageJson;
}): void {
  if (params.aiManifest.name !== "@afora/ai" || !params.aiManifest.version) {
    throw new Error("Prepared dependency tarball must contain @afora/ai with a version.");
  }
  if (params.rootManifest.name !== "afora") {
    throw new Error("Prepared root tarball must contain the afora package.");
  }
  if (!params.rootManifest.version || params.rootManifest.version !== params.aiManifest.version) {
    throw new Error(
      `Prepared root and @afora/ai tarballs must both be version ${params.aiManifest.version}.`,
    );
  }
  if (params.rootManifest.dependencies?.["@afora/ai"] !== params.aiManifest.version) {
    throw new Error(
      `Prepared root tarball must depend on exact @afora/ai@${params.aiManifest.version}.`,
    );
  }
}
...
function main(argv = process.argv.slice(2)): void {
  const args = parseAforaNpmPrepublishVerifyArgs(argv);
  if (args.help) {
    console.log(aforaNpmPrepublishVerifyUsage());
    return;
  }

  const workingDir = mkdtempSync(join(tmpdir(), "afora-prepublish-"));
  ...
  try {
    ...
    if (usesPreparedLocalDependencyInstall(args.dependencyTarballPaths.length)) {
      ...
      assertPreparedAforaAiDependency({
        ...
        `${JSON.stringify(
          {
            private: true,
            dependencies: {
              "@afora/ai": pathToFileURL(aiTarballPath).href,
              afora: pathToFileURL(realpathSync(args.tarballPath)).href,
            },
          },
          null,
          2,
      ...
      packageRoot = join(prefixDir, "node_modules", "afora");
      const binaryPath = join(
        prefixDir,
        "node_modules",
        ".bin",
        process.platform === "win32" ? "afora.cmd" : "afora",
      );
      binaryInvocation =
      ...
      packageRoot = join(globalRoot, "afora");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed afora binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `afora-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`afora-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
