...
pin_worktree_bundled_plugins_dir() {
  ...
  # PR branches without the afora-root node_modules-boundary fix still test
  # their own extensions instead of the primary checkout's stale trees.
  export AFORA_BUNDLED_PLUGINS_DIR="${AFORA_BUNDLED_PLUGINS_DIR:-$PWD/extensions}"
}

resolve_pr_gates_remote_mode() {
  case "${AFORA_PR_GATES_REMOTE:-}" in
    "")
      printf 'local\n'
      ;;
    testbox)
      ...
      ;;
    *)
      echo "Unsupported AFORA_PR_GATES_REMOTE=${AFORA_PR_GATES_REMOTE} (supported: testbox)." >&2
      return 1
      ;;
  esac
}
...
run_remote_testbox_full_test_gate() {
  ...
  run_quiet_logged "$label" "$log_file" \
    ...
    --timing-json \
    --label "$lease_label" \
    -- env CI=1 AFORA_TESTBOX_REMOTE_RUN=1 PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN=install corepack pnpm test
}
...
read_remote_testbox_gate_run_url() {
  ...
  if [ -z "$expected_repo" ] || [ "$expected_repo" = "$pr_url" ]; then
    expected_repo="AforaMosh/afora-agent"
  fi
...
prepare_gates() {
  ...
  local gates_remote_mode
  gates_remote_mode=$(resolve_pr_gates_remote_mode)
  if [ "$gates_remote_mode" = "testbox" ] && [ "${AFORA_TESTBOX:-}" = "1" ]; then
    echo "AFORA_PR_GATES_REMOTE=testbox conflicts with AFORA_TESTBOX=1; hosted PR gates already own remote proof."
    exit 2
  fi
  ...
  if [ "$has_changelog_update" = "true" ]; then
    if ! root_changelog_update_allowed_for_pr; then
      ...
      echo "Set AFORA_ALLOW_ROOT_CHANGELOG_PR=1 only for explicit release automation or maintainer release closeout."
      exit 1
    fi
  ...
  if [ "${AFORA_TESTBOX:-}" != "1" ] && [ "$docs_only" = "true" ] && [ -n "$previous_last_verified_head" ] && git merge-base --is-ancestor "$previous_last_verified_head" HEAD 2>/dev/null; then
  ...
  if [ "${AFORA_TESTBOX:-}" = "1" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      gates_mode="remote_testbox"
      echo "Running pnpm test on Blacksmith Testbox (AFORA_PR_GATES_REMOTE=testbox)."
      run_remote_testbox_full_test_gate \
      ...
      gates_mode="full"
      if [ -n "${AFORA_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with AFORA_VITEST_MAX_WORKERS=$AFORA_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env AFORA_VITEST_MAX_WORKERS="$AFORA_VITEST_MAX_WORKERS" pnpm test
      else
