...
func (docFrontmatterTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	replacer := strings.NewReplacer(
		"Step-by-step Fly.io deployment for Afora with persistent storage and HTTPS", "在 Fly.io 上逐步部署 Afora，包含持久化存储和 HTTPS",
		"Deploying Afora on Fly.io", "在 Fly.io 上部署 Afora",
...
func (docFrontmatterFallbackTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	switch text {
	case "Step-by-step Fly.io deployment for Afora with persistent storage and HTTPS":
		return strings.Join([]string{
			"<frontmatter>",
			"title: Fly.io",
			"summary: \"在 Fly.io 上部署 Afora 的逐步指南，包含持久化存储和 HTTPS 设置\"",
			"read_when:",
			"  - 在 Fly.io 上部署 Afora",
			"  - 设置 Fly 卷、机密和初始运行配置",
			"</frontmatter>",
	...
	case "Deploying Afora on Fly.io":
		return "在 Fly.io 上部署 Afora", nil
...
func TestTranslateDocBodyChunkedFallsBackToSmallerChunks(t *testing.T) {
	...
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
...
func TestValidateDocChunkTranslationRejectsChangedCapitalizedPlaceholder(t *testing.T) {
	...
	source := "Run `afora pairing approve sms <CODE>`.\n"
	translated := "Ejecuta `afora pairing approve sms <TOKEN>`.\n"
...
func TestValidateDocChunkTranslationAllowsDedentedCodeInIndentedFence(t *testing.T) {
	...
	source := "<Example>\n" + sourceFence + "    Run `afora doctor` after editing.\n</Example>\n"
	translated := "<Example>\n" + translatedFence + "    Ejecuta `afora doctor` después de editar.\n</Example>\n"
	if err := validateDocChunkTranslation(source, translated); err != nil {
		t.Fatalf("expected translated component fence and preserved trailing inline code to validate, got %v", err)
	}
	changedTrailingCode := strings.Replace(translated, "`afora doctor`", "`afora fix`", 1)
...
func TestTranslateDocBodyChunkedPreservesMarkersAfterSplit(t *testing.T) {
	...
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "32")
...
func TestTranslateDocBodyChunkedPreservesListStructureAcrossSanitizedChunkBoundary(t *testing.T) {
	...
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "20")
...
func TestTranslateDocBodyChunkedPreSplitsOversizedPromptBudget(t *testing.T) {
	...
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", "60")
...
func TestTranslateDocBodyChunkedSplitsOversizedSingletonBlock(t *testing.T) {
	...
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "24")
...
func TestTranslateDocBodyChunkedSplitsSingletonBlockWhenPromptBudgetExceeded(t *testing.T) {
	...
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", strconv.Itoa(budget))
	translator := &oversizedBlockTranslator{}
	translated, err := translateDocBodyChunked(context.Background(), translator, "gateway/configuration-reference.md", body, "en", "zh-CN")
	if err != nil {
		t.Fatalf("translateDocBodyChunked returned error: %v", err)
...
func TestTranslateDocBodyChunkedMasksInlineCodeAndListMarkers(t *testing.T) {
	body := strings.Join([]string{
		"- Visible prose uses `afora config`.",
	...
	for _, input := range translator.rawInputs {
		if strings.Contains(input, "`afora config`") || strings.Contains(input, "``nested `ticks` ``") || strings.Contains(input, "2.3/02") {
	...
	for _, exact := range []string{
		"- Видимый текст uses `afora config`.",
...
func TestTranslateDocBodyChunkedRetriesSingletonFenceAfterValidationFailure(t *testing.T) {
	...
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("AFORA_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", "4096")
...
func TestProcessFileDocUsesFieldLevelFrontmatterTranslation(t *testing.T) {
	...
	text := string(output)
	if !strings.Contains(text, "在 Fly.io 上逐步部署 Afora，包含持久化存储和 HTTPS") {
		t.Fatalf("expected translated summary in output:\n%s", text)
	}
	if !strings.Contains(text, "在 Fly.io 上部署 Afora") {
...
func TestProcessFileDocRejectsSuspiciousFrontmatterScalarExpansion(t *testing.T) {
	...
	if !strings.Contains(text, "summary: Step-by-step Fly.io deployment for Afora with persistent storage and HTTPS") {
...
func TestValidateDocChunkTranslationRejectsMovedProtectedProductLinkLabel(t *testing.T) {
	...
	source := "Deploy Afora on [Render](https://render.com) using the Blueprint.\n"
	translated := "Render पर Blueprint का उपयोग करके [Afora](https://render.com) परिनियोजित करें।\n"
...
func TestValidateDocBodyRejectsMovedProtectedReferenceLinkLabel(t *testing.T) {
	...
	translated := "Auf Render mit [Afora][provider] bereitstellen.\n\n[provider]: https://render.com\n"
	err := validateDocBodyFencedLiterals(source, translated)
	if err == nil || !strings.Contains(err.Error(), "protected link label mismatch") {
