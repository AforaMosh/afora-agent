// Zai Fallback Repro script supports Afora repository automation.
...
export async function runZaiFallbackRepro(deps: RunZaiFallbackReproDeps = {}): Promise<number> {
  ...
  const baseDir = await mkdtemp(path.join(os.tmpdir(), "afora-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "afora.json");
  try {
    ...
    const sessionId = env.AFORA_ZAI_FALLBACK_SESSION_ID ?? createUuid();
    ...
    const run1 = await run(
      "run1",
      ["afora", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
      envValidAnthropic,
    );
    if (run1.code !== 0) {
      return run1.code ?? 1;
    ...
    const run2 = await run(
      "run2",
      ["afora", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
      envInvalidAnthropic,
    );
