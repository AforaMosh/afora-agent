...
profile_path="${1:-${RUNNER_TEMP:-/tmp}/afora-live.profile}"
...
activate_claude_oauth_access_token() {
  local credentials="${AFORA_CLAUDE_CREDENTIALS_JSON:-}"
...
write_secret_file "$HOME/.codex/auth.json" AFORA_CODEX_AUTH_JSON
write_secret_file "$HOME/.codex/config.toml" AFORA_CODEX_CONFIG_TOML
write_secret_file "$HOME/.claude.json" AFORA_CLAUDE_JSON
write_secret_file "$HOME/.claude/.credentials.json" AFORA_CLAUDE_CREDENTIALS_JSON
write_secret_file "$HOME/.claude/settings.json" AFORA_CLAUDE_SETTINGS_JSON
write_secret_file "$HOME/.claude/settings.local.json" AFORA_CLAUDE_SETTINGS_LOCAL_JSON
write_secret_file "$HOME/.gemini/settings.json" AFORA_GEMINI_SETTINGS_JSON

if [[ -n "${GITHUB_ENV:-}" ]]; then
  {
    echo "AFORA_PROFILE_FILE=$profile_path"
  } >>"$GITHUB_ENV"
fi
