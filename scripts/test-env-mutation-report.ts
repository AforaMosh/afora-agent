#!/usr/bin/env node
// Test Env Mutation Report script supports Afora repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/afora-test-state.ts", "canonical Afora test state helper"],
  ...
  ["test/setup-afora-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "Afora test env mutation report",
...
function printHelp(): void {
  process.stdout.write(`Afora test env mutation report
