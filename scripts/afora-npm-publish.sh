...
usage() {
  echo "usage: bash scripts/afora-npm-publish.sh --publish [package.tgz]"
}
...
publish_plan="$(
  PACKAGE_VERSION="${package_version}" REQUESTED_PUBLISH_TAG="${AFORA_NPM_PUBLISH_TAG:-}" \
    BYPASS_EXTENDED_STABLE_GUARD="${BYPASS_EXTENDED_STABLE_GUARD:-}" \
    node scripts/afora-npm-extended-stable-release.mjs publish-plan
)"
