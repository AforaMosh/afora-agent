...
const MACOS_SCRIPT_SCOPE_RE =
  /^(?:scripts\/e2e\/macos-app-bootstrap-ci\.ts|scripts\/e2e\/lib\/plugins\/npm-registry-server\.mjs|scripts\/e2e\/parallels\/(?:env-limits|filesystem|host-command|host-server|package-artifact|types)\.ts|scripts\/(?:npm-runner|pnpm-runner)\.mts|scripts\/windows-cmd-helpers\.mjs|scripts\/(?:check-swift-tools|codesign-mac-app|create-dmg|format-swift|install-cli|install-swift-tools|install-xcodegen|lint-swift|notarize-mac-artifact|package-mac-app|package-mac-dist)\.sh|scripts\/lib\/(?:npm-json-output|windows-cmd-helpers-runtime)\.mts|scripts\/lib\/(?:bounded-response|sleep)\.mjs|scripts\/lib\/(?:plistbuddy|swift-toolchain)\.sh|test\/scripts\/(?:codesign-mac-app|create-dmg|notarize-mac-artifact|package-mac-app|package-mac-dist|parallels-smoke-model)\.test\.ts)$/;
