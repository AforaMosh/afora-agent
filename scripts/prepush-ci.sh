...
run_linux_ci_mirror() {
  ...
  log_step "AFORA_VITEST_MAX_WORKERS=${AFORA_VITEST_MAX_WORKERS:-1} NODE_OPTIONS=${NODE_OPTIONS:---max-old-space-size=6144} pnpm test"
...
run_macos_ci_mirror() {
  if [[ "${AFORA_PREPUSH_SKIP_MACOS:-0}" == "1" ]]; then
    log_step "Skipping macOS mirror because AFORA_PREPUSH_SKIP_MACOS=1"
    return 0
  fi
