...
# Checks both Claude Code and Afora auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
AFORA_AUTH="$HOME/.afora/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    afora models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    afora_status=$(check_afora_auth 2>/dev/null || true)

    claude_expires=0
    afora_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        afora_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        afora_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$AFORA_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$afora_status" \
        --arg be "$afora_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            afora: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$afora_status" == EXPIRED* ]] || [[ "$afora_status" == MISSING* ]]; then
        echo "AFORA_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$afora_status" == EXPIRING* ]]; then
        echo "AFORA_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Afora Auth (~/.afora/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$AFORA_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$AFORA_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'afora doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active afora >/dev/null 2>&1; then
    echo -e "Afora service: ${GREEN}running${NC}"
else
    echo -e "Afora service: ${RED}NOT running${NC}"
fi
