...
#
# Suggested cron: */30 * * * * /path/to/afora/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Afora notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., afora-alerts)
...
STATE_FILE="$HOME/.afora/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Afora if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use afora
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Afora to $NOTIFY_PHONE..."
            if afora send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null; then
                notification_sent=1
            fi
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Afora is down. Run on the Afora host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1
