#!/usr/bin/env node
// Builds the canonical Afora package artifact used by Docker E2E.
...
const AI_RUNTIME_PACKAGE = "@afora/ai";
const AI_RUNTIME_BACKUP_DIR = ".afora-ai-package-backup";
...
function resolvePackedAforaFileName(value: string) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("afora-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  if (
    !/^afora-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
    filename !== path.basename(filename) ||
    filename !== path.win32.basename(filename)
  ) {
    throw new Error(`npm pack reported unsafe Afora tarball filename: ${filename}`);
  }
  return filename;
}
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building Afora package artifacts",
    command: "pnpm",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "AFORA_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestAforaTarball(outputDir: string, packOutput: string) {
  let fromOutput = "";
  try {
    ...
    for (const entry of resolveNpmJsonEntries(parsed)) {
      ...
      const filename = resolvePackedAforaFileName(filenameValue);
      if (filename) {
        fromOutput = filename;
      }
    }
  ...
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedAforaFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedAforaFileName(entry) === entry;
      } catch {
        return false;
      }
    })
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Afora tarball in ${outputDir}`);
  }
...
async function cleanPackedAforaTarballs(outputDir: string) {
  let entries: string[];
  try {
...
function isPackedAiRuntimeTarball(filename: string) {
  return /^afora-ai-[A-Za-z0-9._-]+\.tgz$/u.test(filename);
}
...
export async function prepareBundledAiRuntimePackage(
  ...
  const aiRuntimePath = path.join(sourceDir, "node_modules", "@afora", "ai");
  ...
  const extractAiRuntime =
    ...
    ((tarballPath: string, destination: string) =>
      ...
      run("tar", ["-xzf", tarballPath, "-C", destination, "--strip-components=1"], destination, {
        timeoutMs: resolveTimeoutMs(
          "AFORA_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  if (!hasAiRuntimeWorkspace) {
    throw new Error("@afora/ai dependency requires the packages/ai workspace");
  }
  if (typeof aiRuntimeDependency !== "string") {
    throw new Error("root package.json must declare @afora/ai as a dependency");
  }
  ...
  try {
    ...
    if (packedAiTarballs.length !== 1) {
      throw new Error(
        `expected one packed @afora/ai tarball in ${outputDir}, found ${packedAiTarballs.length}`,
      );
    }
    ...
    if (typeof stagedPackageJson.version !== "string" || !stagedPackageJson.version) {
      throw new Error("packed @afora/ai package must declare a version");
    }
    for (const [name, version] of Object.entries(stagedPackageJson.dependencies ?? {})) {
      if (typeof version !== "string") {
        throw new Error(`packed @afora/ai dependency ${name} must declare a string version`);
      }
      ...
      if (rootVersion !== version && rootVersion !== `workspace:${version}`) {
        throw new Error(
          `root package.json must declare ${name}@${version} to bundle @afora/ai without duplicate dependencies`,
        );
      }
...
export async function packAforaPackageForDocker(
  sourcePath: string,
  outputPath: string,
  packageOptions: PackageOptions = {},
) {
  ...
  console.error("==> Packing Afora package");
  ...
  try {
    ...
    try {
      await cleanPackedAforaTarballs(outputPath);
      ...
      if (packTool === "npm" && packageOptions.packJsonPath) {
        packReceiptDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-npm-pack-receipt-"));
      }
    ...
    let tarball = await newestAforaTarball(
      outputPath,
...
export async function writePackageInventoryForDocker(
  ...
  await runImpl(
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "AFORA_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
...
async function main() {
  ...
  console.error("==> Writing Afora package inventory");
  ...
  console.error("==> Checking Afora package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [
      path.join(ROOT_DIR, "scripts/check-afora-package-tarball.mjs"),
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "AFORA_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
        DEFAULT_PACKAGE_TARBALL_CHECK_TIMEOUT_MS,
      ),
    },
  );
  console.error(
    `==> Afora package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
