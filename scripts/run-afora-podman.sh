#!/usr/bin/env bash
# Rootless Afora in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-afora-podman.sh launch        # Start gateway
#   ./scripts/run-afora-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   afora --container afora dashboard --no-open
#   afora --container afora channels login
#
...
run_podman_detached() {
  afora_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      AFORA_GATEWAY_TOKEN|AFORA_PODMAN_CONTAINER|AFORA_PODMAN_IMAGE|AFORA_IMAGE|AFORA_PODMAN_PULL|AFORA_PODMAN_RUN_TIMEOUT|AFORA_PODMAN_GATEWAY_HOST_PORT|AFORA_GATEWAY_PORT|AFORA_PODMAN_BRIDGE_HOST_PORT|AFORA_BRIDGE_PORT|AFORA_GATEWAY_BIND|AFORA_PODMAN_USERNS|AFORA_BIND_MOUNT_OPTIONS|AFORA_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-afora-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${AFORA_CONFIG_DIR:-$EFFECTIVE_HOME/.afora}"
ENV_FILE="${AFORA_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${AFORA_CONFIG_DIR:-$EFFECTIVE_HOME/.afora}"
ENV_FILE="${AFORA_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${AFORA_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${AFORA_PODMAN_CONTAINER:-afora}"
AFORA_IMAGE="${AFORA_PODMAN_IMAGE:-${AFORA_IMAGE:-afora:local}}"
PODMAN_PULL="${AFORA_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${AFORA_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${AFORA_PODMAN_GATEWAY_HOST_PORT:-${AFORA_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${AFORA_PODMAN_BRIDGE_HOST_PORT:-${AFORA_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${AFORA_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$AFORA_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v afora >/dev/null 2>&1; then
    return 0
  fi
  AFORA_CONTAINER="" AFORA_CONFIG_DIR="$config_dir" \
    afora config get gateway.bind 2>/dev/null || true
}
...
# AFORA_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${AFORA_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v afora >/dev/null 2>&1; then
    echo "Warning: afora not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    AFORA_CONTAINER="" AFORA_CONFIG_DIR="$config_dir" \
      afora config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    AFORA_CONTAINER="" AFORA_CONFIG_DIR="$config_dir" \
      afora config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  AFORA_CONTAINER="" AFORA_CONFIG_DIR="$config_dir" \
    afora config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${AFORA_GATEWAY_TOKEN:-}" ]]; then
  export AFORA_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "AFORA_GATEWAY_TOKEN" "$AFORA_GATEWAY_TOKEN"
  echo "Generated AFORA_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/afora.json"
...
PODMAN_USERNS="${AFORA_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported AFORA_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (AFORA_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${AFORA_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${AFORA_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$AFORA_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.afora/.npm \
    -e AFORA_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.afora:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.afora/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$AFORA_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME afora dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME afora devices approve --latest  # if pairing required"
