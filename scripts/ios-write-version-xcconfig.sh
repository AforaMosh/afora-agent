...
- AFORA_IOS_VERSION = exact canonical iOS version
- AFORA_MARKETING_VERSION = short iOS/App Store version
- AFORA_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
RESOLVED_GIT_COMMIT="$(afora_resolve_git_commit "${ROOT_DIR}")"
RESOLVED_BUILD_TIMESTAMP="$(afora_resolve_build_timestamp)"
...
AFORA_IOS_VERSION = ${IOS_VERSION}
AFORA_MARKETING_VERSION = ${MARKETING_VERSION}
AFORA_BUILD_VERSION = ${BUILD_NUMBER}
AFORA_GIT_COMMIT = ${RESOLVED_GIT_COMMIT}
AFORA_BUILD_TIMESTAMP = ${RESOLVED_BUILD_TIMESTAMP}
EOF
