...
image="$1"
attempts="${AFORA_DOCKER_PULL_ATTEMPTS:-3}"
timeout_seconds="${AFORA_DOCKER_PULL_TIMEOUT_SECONDS:-180}"
retry_delay_seconds="${AFORA_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"

if ! [[ "$attempts" =~ ^[1-9][0-9]*$ ]]; then
  echo "AFORA_DOCKER_PULL_ATTEMPTS must be a positive integer, got: $attempts" >&2
  exit 2
fi

if ! [[ "$timeout_seconds" =~ ^[1-9][0-9]*$ ]]; then
  echo "AFORA_DOCKER_PULL_TIMEOUT_SECONDS must be a positive integer, got: $timeout_seconds" >&2
  exit 2
fi

if ! [[ "$retry_delay_seconds" =~ ^[0-9]+$ ]]; then
  echo "AFORA_DOCKER_PULL_RETRY_DELAY_SECONDS must be a non-negative integer, got: $retry_delay_seconds" >&2
  exit 2
fi
...
run_docker_pull() {
  local timeout_bin
  if ! timeout_bin="$(afora_host_timeout_bin)"; then
