...
process.env.AFORA_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "AFORA_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "AFORA_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";

type PackageManifest = {
  files?: unknown;
  afora?: Partial<Record<"channel" | "extensions" | "setupEntry", unknown>>;
};
...
function packageRootLooksInstalled(root: string) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/afora");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "afora");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const extensionEntries =
      Array.isArray(packageJson.afora.extensions) && packageJson.afora.extensions.length > 0
