...
const DEFAULT_REPO = "AforaMosh/afora-agent";
...
const WINDOWS_NODE_REPO = "AforaMosh/afora-agent-windows-node";
const WINDOWS_NODE_REQUIRED_ASSETS = [
  "AforaCompanion-Setup-x64.exe",
  "AforaCompanion-Setup-arm64.exe",
];
const SHA256_DIGEST_PATTERN = /^sha256:[a-f0-9]{64}$/u;
const SHA256_HEX_PATTERN = /^[a-f0-9]{64}$/u;
const RELEASE_CANDIDATE_STATE_VERSION = 2;
const RELEASE_CANDIDATE_STATE_FILE = "release-candidate-state.json";
const TRUSTED_TOOLING_SHA_ENV = "AFORA_RELEASE_CANDIDATE_TRUSTED_TOOLING_SHA";
...
Afora Release Publish command only after everything is green.
...
Options:
  ...
  --npm-preflight-run <id>            Reuse successful Afora NPM Release preflight run.
...
export function parseArgs(argv: string[]) {
  ...
  if (options.pluginPublishScope === "selected") {
    throw new Error(
      "--plugin-publish-scope selected is only for plugin-only repair publishes; release candidates publish Afora with --plugin-publish-scope all-publishable",
    );
  }
...
export function validateParallelsRegistryPackageArtifact(
  ...
  if (
    manifest.schema !== "afora.plugin-publication-artifact/v1" ||
...
function githubApiTimeoutMs() {
  const raw = process.env.AFORA_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_GITHUB_API_TIMEOUT_MS;
  }
  if (!/^[1-9]\d*$/u.test(raw)) {
    throw new Error("AFORA_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive integer");
  }
...
function runFromTrustedTooling(
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "afora-release-tooling-"));
...
async function runParallelsIfNeeded(
  ...
  const command = candidateParallelsShellCommand(
    ...
    dependencyTarballPaths,
    registryPackageTarballPaths,
    process.env.AFORA_PARALLELS_MACOS_SNAPSHOT_HINT?.trim() ?? "",
  );
  run("bash", ["-lc", command], {
    env: {
      AFORA_PARALLELS_ARTIFACT_ROOT: join(process.cwd(), ".artifacts", "parallels"),
    },
  });
  return {
    status: "passed",
...
async function runTelegramIfNeeded(
  ...
  const runId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
    package_spec: `afora@${options.tag.replace(/^v/u, "")}`,
...
async function main() {
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    const workflowFile = "afora-npm-release.yml";
  ...
  const npmRun = await waitForSuccessfulRun(options.repo, options.npmPreflightRunId, {
    workflowName: "Afora NPM Release",
    workflowRef: options.workflowRef,
  });
  ...
  const npmArtifact = await downloadResolvedArtifact(
    ...
    `afora-npm-preflight-${options.tag}`,
    "afora-npm-preflight-",
    npmDir,
  );
  const npmArtifactName = npmArtifact.name;
  if (!Number.isInteger(npmRun.runAttempt) || npmRun.runAttempt < 1) {
    throw new Error(`Afora npm preflight run ${options.npmPreflightRunId} has invalid attempt.`);
  }
  downloadArtifact(
    options.repo,
    options.npmPreflightRunId,
