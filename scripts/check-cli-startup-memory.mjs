...
function parseArgs(argv) {
  const options = {
    jsonPath:
      readNonEmptyEnv("AFORA_STARTUP_MEMORY_JSON_PATH") ??
      path.join(repoRoot, ".artifacts", "startup-memory", "startup-memory.json"),
    summaryPath:
      readNonEmptyEnv("AFORA_STARTUP_MEMORY_SUMMARY_PATH") ??
...
const cases = [
  {
    ...
    label: "--help",
    args: ["afora.mjs", "--help"],
    limitMb: readPositiveNumberEnv("AFORA_STARTUP_MEMORY_HELP_MB", DEFAULT_LIMITS_MB.help),
  },
  {
    id: "pluginsList",
    ...
    limitMb: readPositiveNumberEnv(
      "AFORA_STARTUP_MEMORY_PLUGINS_LIST_MB",
  ...
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["afora.mjs", "status", "--json"],
    limitMb: readPositiveNumberEnv(
      "AFORA_STARTUP_MEMORY_STATUS_JSON_MB",
  ...
  },
  {
    ...
    args: ["afora.mjs", "gateway", "status"],
    limitMb: readPositiveNumberEnv(
      "AFORA_STARTUP_MEMORY_GATEWAY_STATUS_MB",
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"Afora startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
...
function runStartupMemoryCheck(argv = process.argv.slice(2), params = {}) {
  ...
  tmpHome = mkdtempSync(path.join(os.tmpdir(), "afora-startup-memory-"));
