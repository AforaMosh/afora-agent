...
DOCKER_BUILD_EXTENSIONS="${AFORA_DOCKER_BUILD_EXTENSIONS:-${AFORA_EXTENSIONS:-}}"
LIVE_IMAGE_PULL_ATTEMPTS="${AFORA_LIVE_DOCKER_PULL_ATTEMPTS:-3}"
LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS="${AFORA_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"

if ! [[ "$LIVE_IMAGE_PULL_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
  echo "AFORA_LIVE_DOCKER_PULL_ATTEMPTS must be a positive integer, got: $LIVE_IMAGE_PULL_ATTEMPTS" >&2
  exit 2
fi
if ! [[ "$LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" =~ ^[0-9]+$ ]]; then
  echo "AFORA_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS must be a non-negative integer, got: $LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" >&2
  exit 2
fi
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "AFORA_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi
...
if [[ "${AFORA_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  if ! docker_build_on_missing_enabled; then
    ...
    echo "Build it first or unset AFORA_SKIP_DOCKER_BUILD." >&2
    exit 1
  fi
  echo "==> Live-test image not available; building because AFORA_DOCKER_BUILD_ON_MISSING/AFORA_TESTBOX allows fallback."
fi
