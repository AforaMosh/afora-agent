...
// Afora Prepack script supports Afora repository automation.
...
import { restorePrepackArtifacts } from "./afora-postpack.mjs";
...
const PREPARED_RELEASE_ENV = "AFORA_PREPACK_PREPARED";
const OCM_INTERNAL_NPM_BIN_ENV = "OCM_INTERNAL_NPM_BIN";
const OCM_WORKSPACE_DIRS_ENV = "AFORA_OCM_WORKSPACE_DEPENDENCY_DIRS";
...
const SELF_CONTAINED_SOURCE_PACK_COMMAND =
  "node scripts/package-afora-for-docker.mjs --allow-unreleased-changelog";
...
export function collectSourcePackWorkspaceDependencyErrors(
  ...
  const aiDependency = packageJson.dependencies?.["@afora/ai"];
  if (typeof aiDependency !== "string" || !aiDependency.trim().startsWith("workspace:")) {
    return [];
  }
  if (ocmExternalizesWorkspacePackage("@afora/ai", env)) {
    return [];
  }
  return [
    `plain root packing cannot safely resolve @afora/ai from ${aiDependency}: pnpm rewrites the workspace dependency to an exact version without bundling the package`,
    `use \`${SELF_CONTAINED_SOURCE_PACK_COMMAND}\` for a self-contained source package; official npm release automation prepares and publishes @afora/ai separately`,
  ];
}
...
export function resolvePrepackCommandTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveEnvInt(
    "AFORA_PREPACK_COMMAND_TIMEOUT_MS",
    env,
