...
const REMOTE_CHANGED_GATE_BUNDLE_FILE = ".afora-crabbox-changed-gate.bundle";
...
const ignoreRepoBinary = process.env.AFORA_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
...
function selectedProvider(
  ...
  if (workload === null) {
    return {
      provider: "",
      source: "policy",
      workload: workloadOption ?? process.env.AFORA_CRABBOX_WORKLOAD ?? "",
      chain: [],
      error: `unsupported Crabbox workload ${JSON.stringify(workloadOption ?? process.env.AFORA_CRABBOX_WORKLOAD)}`,
    };
  }
...
function requestedWorkload(commandArgs: string[]) {
  ...
  const raw = workloadOption ?? process.env.AFORA_CRABBOX_WORKLOAD?.trim() ?? "";
  if (!raw) {
    return "";
  }
...
function directCloudOverrideEnabled(providerName: string) {
  return (
    ...
    process.env.AFORA_CRABBOX_ALLOW_DIRECT_CLOUD === "1"
  );
}
...
function enforceBrokeredDaytonaVersion(
  ...
  console.error(
    [
      ...
      "[crabbox] direct Daytona debugging requires an original `--provider daytona`, no `--workload`, and AFORA_CRABBOX_ALLOW_DIRECT_CLOUD=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function enforceBrokeredCloud(
  ...
  if ("brokerAuthFailure" in readiness && readiness.brokerAuthFailure) {
    const instructions = [
      `[crabbox] provider=${canonicalProvider} requires managed Crabbox broker authentication for Afora proof.`,
      `[crabbox] run \`${recoveryCommand(["login", "--url", "https://crabbox.afora.ai"])}\`, then retry.`,
    ];
    if (canonicalProvider !== "aws") {
      instructions.push(
        `[crabbox] direct ${canonicalProvider} debugging requires an original \`--provider ${canonicalProvider}\`, no \`--workload\`, and AFORA_CRABBOX_ALLOW_DIRECT_CLOUD=1.`,
      );
    }
  ...
  if (!("brokerReady" in readiness) || !readiness.brokerReady) {
    console.error(
      [
        `[crabbox] provider=${canonicalProvider} failed readiness for Afora proof: ${readiness.reason}.`,
...
function remoteGitBootstrapForChangedGate(changedGateBase: string, changedGateAlias: string) {
  ...
  return [
    `afora_changed_gate_base=${quotedBase};`,
    `afora_changed_gate_alias=${quotedAlias};`,
    'if ! command -v git >/dev/null 2>&1; then echo "git is required for Afora remote changed-gate sync" >&2; exit 2; fi;',
    `afora_changed_gate_bundle=${quotedBundleFile};`,
    'if [ ! -f "$afora_changed_gate_bundle" ]; then echo "missing changed-gate bundle: $afora_changed_gate_bundle" >&2; exit 2; fi;',
    'afora_changed_gate_bundle_tmp="$(mktemp /tmp/afora-changed-gate.XXXXXX)" || exit 2;',
    "trap 'rm -f \"$afora_changed_gate_bundle_tmp\"' EXIT HUP INT TERM;",
    'cp "$afora_changed_gate_bundle" "$afora_changed_gate_bundle_tmp" || exit 2;',
    ...
    "rm -rf .git || exit 2;",
    "git init -q || exit 2;",
    "git remote add origin https://github.com/AforaMosh/afora-agent.git 2>/dev/null || git remote set-url origin https://github.com/AforaMosh/afora-agent.git || exit 2;",
    'git fetch -q --depth=2 origin "$afora_changed_gate_base:refs/remotes/origin/main" || exit 2;',
    'if [ -n "$afora_changed_gate_alias" ]; then git update-ref "$afora_changed_gate_alias" refs/remotes/origin/main || exit 2; fi;',
    'if [ ! -f "$afora_changed_gate_bundle_tmp" ]; then echo "changed-gate bundle disappeared before import" >&2; exit 2; fi;',
    "afora_changed_gate_target=refs/remotes/origin/main;",
    'if [ -s "$afora_changed_gate_bundle_tmp" ]; then git fetch -q "$afora_changed_gate_bundle_tmp" HEAD:refs/heads/afora-changed-gate-tree || exit 2; afora_changed_gate_tree="$(git rev-parse refs/heads/afora-changed-gate-tree^{tree})" || exit 2; afora_changed_gate_head="$(git -c user.name=Afora -c user.email=ci@afora.local commit-tree "$afora_changed_gate_tree" -p refs/remotes/origin/main -m remote-changed-gate-tree)" || exit 2; git update-ref refs/heads/afora-changed-gate-head "$afora_changed_gate_head" || exit 2; afora_changed_gate_target=refs/heads/afora-changed-gate-head; fi;',
    'rm -f "$afora_changed_gate_bundle_tmp" || exit 2;',
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$aforaModulesDir = if ($env:CRABBOX_PNPM_MODULES_DIR) { $env:CRABBOX_PNPM_MODULES_DIR } else { $env:PNPM_CONFIG_MODULES_DIR }",
    "if ($aforaModulesDir) {",
    'if (-not (Test-Path $aforaModulesDir)) { throw "hydrated pnpm modules directory does not exist: $aforaModulesDir" }',
    '$aforaWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$aforaSelfModules = Join-Path $aforaModulesDir "node_modules"',
    'if (-not (Test-Path $aforaSelfModules)) { cmd /c mklink /J "$aforaSelfModules" "$aforaModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $aforaWorkspaceModules)) { cmd /c mklink /J "$aforaWorkspaceModules" "$aforaModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remotePosixHydratedNodeModulesBootstrap() {
  ...
  return 'afora_modules_dir="${CRABBOX_PNPM_MODULES_DIR:-${PNPM_CONFIG_MODULES_DIR:-}}"; if [ -n "$afora_modules_dir" ] && [ -d "$afora_modules_dir" ] && [ ! -e node_modules ]; then ln -s "$afora_modules_dir" node_modules; fi;';
}
...
function remotePosixJsEnvBootstrap() {
  return [
    ...
    "afora_env_args=();",
    "afora_env_ignore=0;",
    "afora_env_path_seen=0;",
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) afora_env_ignore=1; afora_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${afora_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) afora_env_ignore=1; afora_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) afora_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then afora_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) afora_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$afora_env_ignore" = "1" ]; then afora_env_args+=("PATH=${AFORA_CRABBOX_BOOTSTRAP_PATH:-$PATH}:${1#PATH=}"); else afora_env_args+=("$1"); fi; afora_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) afora_env_args+=("$1"); shift ;;',
    '--) afora_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$afora_env_ignore" = "1" ] && [ "$afora_env_path_seen" = "0" ]; then afora_env_args+=("PATH=${AFORA_CRABBOX_BOOTSTRAP_PATH:-$PATH}"); fi;',
    'command env "${afora_env_args[@]}" "$@";',
    "};",
  ];
}

function remoteAwsMacosJsBootstrap({ packageManager = false, bun = false } = {}) {
  const nodeVersion = process.env.AFORA_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "afora_crabbox_bootstrap_macos_js() {",
    'tool_root="${AFORA_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.afora-crabbox-toolchain}";',
    ...
    'macos_locale="${AFORA_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.afora-crabbox-node-ready";',
  ...
  if (bun) {
    bootstrap.push(
      ...
      'bun_ready_marker="$bun_root/.afora-crabbox-bun-ready";',
  ...
  bootstrap.push('export AFORA_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "afora_crabbox_bootstrap_macos_js");
  return bootstrap.join(" ");
}

function remoteWsl2JsBootstrap({ packageManager = false } = {}) {
  const nodeVersion = process.env.AFORA_CRABBOX_WSL2_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "afora_crabbox_bootstrap_wsl2_js() {",
    'tool_root="${AFORA_CRABBOX_WSL2_TOOLCHAIN_DIR:-$HOME/.afora-crabbox-toolchain}";',
    `node_version=${shellQuote(nodeVersion)};`,
    'arch="$(uname -m)";',
  ...
  bootstrap.push('export AFORA_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "afora_crabbox_bootstrap_wsl2_js");
  return bootstrap.join(" ");
}
...
function scopedAwsMacosEnvCommand(commandArgs: string[]) {
  ...
  return {
    ...
    bun: needsBun,
    shellCommand: `afora_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function prepareRemoteWsl2JsBootstrapScript(
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "afora-crabbox-wsl2-script-"));
...
function remoteAwsMacosSwiftBootstrap() {
  return [
    "afora_crabbox_require_macos_swift_62() {",
    'afora_xcode="";',
    'for afora_candidate in /Applications/Xcode_26.1.app /Applications/Xcode_26*.app /Applications/Xcode-26*.app; do if [ -d "$afora_candidate" ]; then afora_xcode="$afora_candidate"; fi; done;',
    'if [ -n "$afora_xcode" ]; then afora_developer="$afora_xcode/Contents/Developer"; if [ ! -d "$afora_developer" ]; then afora_developer="$afora_xcode"; fi; sudo xcode-select -s "$afora_developer" || return 1; fi;',
    'afora_swift_version="$(swift --version 2>&1)" || { status=$?; printf "%s\\n" "$afora_swift_version" >&2; return "$status"; };',
    'printf "%s\\n" "$afora_swift_version" >&2;',
    'afora_swift_major_minor="$(printf "%s\\n" "$afora_swift_version" | sed -nE "s/.*Apple Swift version ([0-9]+)\\.([0-9]+).*/\\1 \\2/p" | head -n 1)";',
    'if [ -z "$afora_swift_major_minor" ]; then echo "[crabbox] Afora macOS app proof requires Swift tools 6.2+; unable to parse swift --version." >&2; return 2; fi;',
    "set -- $afora_swift_major_minor;",
    'if [ "$1" -lt 6 ] || { [ "$1" -eq 6 ] && [ "$2" -lt 2 ]; }; then',
    'echo "[crabbox] Afora macOS app proof requires Swift tools 6.2+ (Xcode 26.x)." >&2;',
    'echo "[crabbox] current Swift is $1.$2; select/install Xcode 26.x or use a Blacksmith macOS runner with Xcode_26.1.app." >&2;',
    "return 2;",
    "fi;",
    'afora_xcodebuild_version="$(xcodebuild -version 2>&1)" || { printf "%s\\n" "$afora_xcodebuild_version" >&2; echo "[crabbox] Afora macOS app proof requires Xcode 26.x; active developer directory does not provide usable xcodebuild." >&2; return 2; };',
    'printf "%s\\n" "$afora_xcodebuild_version" >&2;',
    'afora_xcode_major="$(printf "%s\\n" "$afora_xcodebuild_version" | sed -nE "s/^Xcode ([0-9]+)(\\..*)?$/\\1/p" | head -n 1)";',
    'if [ "$afora_xcode_major" != "26" ]; then echo "[crabbox] Afora macOS app proof requires Xcode 26.x; current xcodebuild is ${afora_xcode_major:-unknown}." >&2; return 2; fi;',
    "};",
    "afora_crabbox_require_macos_swift_62",
  ].join(" ");
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs: string[], providerName: string) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "afora-crabbox-macos-script-"));
  const scriptPath = resolve(scriptRoot, "script.sh");
...
function createAwsMacosScriptStdinWrapper(script: string) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/afora-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_afora_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_afora_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script: string) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `AFORA_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "afora", "crabbox-sync");
  }
  return resolve(tmpdir(), "afora-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.AFORA_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root: string) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "AFORA_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set AFORA_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower AFORA_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}

function currentWorktreeTree(syncRoot: string) {
  const indexDir = mkdtempSync(resolve(syncRoot, "afora-crabbox-index-"));
...
function prepareFullCheckoutForSync(options: { changedGateBase?: string } = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "afora-crabbox-sync-"));
  let active = false;
  ...
  function create() {
    ...
    if (options.changedGateBase) {
      ...
      let bundleTempDir;
      try {
        bundleTempDir = mkdtempSync(resolve(syncRoot, "afora-crabbox-bundle-"));
...
function fullCheckoutKeepaliveIntervalMs() {
  return parseNonNegativeIntegerEnv(
    "AFORA_CRABBOX_SYNC_KEEPALIVE_MS",
    5000,
...
function injectRemoteTestboxBootstrap(commandArgs: string[], providerName: string) {
  ...
  const snapshot = hasOption(commandArgs, "--no-sync")
    ? ""
    : `if [ -n "$(git status --porcelain=v1)" ]; then git add -A && git -c user.name=Afora -c user.email=ci@afora.local -c commit.gpgsign=false commit --no-verify -qm remote-testbox-sync || exit $?; fi; `;
  return replaceRunCommandWithShell(
    invocation,
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        ...
        "[crabbox] if using ../crabbox, rebuild it: version=$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../crabbox -trimpath -ldflags \"-s -w -X github.com/afora/crabbox/internal/cli.version=${version}\" -o bin/crabbox ./cmd/crabbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    ...
    console.error(
      `[crabbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm/Bun for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for Afora CI-like proof; not switching providers automatically`,
    );
  }
}
...
if (
  ...
  console.error(
    "[crabbox] provider=docker enabling host Docker socket pass-through for Afora Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/afora-crabbox-docker-work";
  console.error(
...
function resolveChildKillGraceMs(env: ProcessEnv) {
  if (!env.VITEST || !env.AFORA_TEST_CRABBOX_CHILD_KILL_GRACE_MS) {
    return 5_000;
  }
  const value = Number.parseInt(env.AFORA_TEST_CRABBOX_CHILD_KILL_GRACE_MS, 10);
  return Number.isFinite(value) && value >= 0 ? value : 5_000;
}

function resolveMetadataProbeTimeoutMs(env: ProcessEnv) {
  if (!env.VITEST || !env.AFORA_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS) {
    return CRABBOX_METADATA_PROBE_TIMEOUT_MS;
  }
  const value = Number.parseInt(env.AFORA_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS, 10);
