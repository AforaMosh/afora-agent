...
import { parseFlagArgs, stringFlag, intFlag } from "./lib/arg-utils.mts";
...
if (process.argv.slice(2).includes("--help")) {
  console.log(
    [
      "Usage: node --import tsx scripts/test-update-cli-startup-bench.mts [options]",
      "",
      "Refresh the checked-in CLI benchmark fixture.",
      "",
      "Options:",
      "  --out <path>          Output path (default: test/fixtures/cli-startup-bench.json)",
      "  --entry <path>        CLI entry to benchmark (default: afora.mjs)",
      ...
      "",
      "Example:",
      "  node --import tsx scripts/test-update-cli-startup-bench.mts --preset all --runs 3 --warmup 1",
    ].join("\n"),
  );
  process.exit(0);
}
