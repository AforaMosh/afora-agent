...
export function parseArgs(argv: string[]) {
  ...
  const options = {
    ...
    pluginIds,
    shardTotal: readOptionalPositiveIntEnv("AFORA_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("AFORA_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.AFORA_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.AFORA_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsvOrLooseStringList(process.env.AFORA_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
Environment:
  AFORA_PLUGIN_GATEWAY_GAUNTLET_IDS   Comma-separated plugin ids to include
  AFORA_PLUGIN_GATEWAY_GAUNTLET_TOTAL Total plugin shards
  AFORA_PLUGIN_GATEWAY_GAUNTLET_INDEX Zero-based shard index
  AFORA_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION=1
  AFORA_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT=1
  AFORA_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES  QA summary read ceiling
`);
}
...
function aforaCommand(repoRoot: string, args: string[]) {
  return {
    command: process.execPath,
...
function sourceAforaCommand(repoRoot: string, args: string[]) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot: string, runRoot: string) {
  ...
  return {
    ...
    AFORA_LOG_DIR: path.join(runRoot, "logs"),
    AFORA_QA_SUITE_PROGRESS: process.env.AFORA_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(
  ...
  return {
    ...
    ...aforaCommand(params.repoRoot, [command, "--help"]),
...
async function runPluginLifecycleCommand(
  ...
  params.rows.push(
    await runMeasuredCommand({
      ...
      ...aforaCommand(params.repoRoot, ["plugins", ...params.args]),
      label: params.label,
...
async function runQaChunks(params: GauntletContext) {
  ...
  for (const [index, chunk] of chunks.entries()) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceAforaCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-plugin-gauntlet-"));
