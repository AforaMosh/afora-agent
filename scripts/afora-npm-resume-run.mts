...
export interface AforaNpmResumeValidationInput {
...
const WORKFLOW_PATH = ".github/workflows/afora-npm-release.yml";
...
function requiredString(value: unknown, label: string): string {
  if (typeof value !== "string" || value.length === 0) {
    fail(`Afora npm resume run is missing ${label}.`);
  }
  return value;
}
...
function requiredSha(value: unknown, label: string): string {
  ...
  if (!SHA_PATTERN.test(sha)) {
    fail(`Afora npm resume run has invalid ${label}.`);
  }
  return sha;
}
...
export function validateAforaNpmResumeRun({
  canonicalWorkflowId,
  compareStatus,
  jobs,
  run,
  ...
  if (!branchMatch) {
    fail(`Afora npm resume run has an untrusted workflow ref: ${url}`);
  }
  ...
  if (
    ...
    fail(`Afora npm resume run has an untrusted workflow identity: ${url}`);
  }

  const tagObjectSha = requiredSha(tagRef?.object?.sha, "tooling tag object SHA");
  if (tagRef?.object?.type !== "tag") {
    fail(`Afora npm resume run tooling ref is not a signed annotated tag: ${url}`);
  }
  ...
  if (
    ...
    fail(
      `Afora npm resume run is not bound to a real, main-reachable protected tooling tag: ${url}`,
    );
  }
  ...
  if (
    ...
    fail(`Afora npm resume run lacks successful parent release approval validation: ${url}`);
  }
...
export function runAforaNpmResumeGh(
  args: string[],
  params: {
...
export function resolveAforaNpmResumeRun({
  repo,
  runId,
  runGh = runAforaNpmResumeGh,
}: {
  repo: string;
  runId: string;
  ...
  if (!/^[1-9][0-9]*$/u.test(runId)) {
    fail("Afora npm resume run id must be a positive integer.");
  }
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(repo)) {
    fail("Afora npm resume repository must be owner/name.");
  }
  ...
  return validateAforaNpmResumeRun({
...
function main(argv: string[] = process.argv.slice(2)): void {
  const result = resolveAforaNpmResumeRun(parseArgs(argv));
