...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/afora/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Afora Re-Auth" -c "After getting key, run: ssh $SERVER '~/afora/scripts/mobile-reauth.sh'" --id afora-auth
        ;;
    *)
