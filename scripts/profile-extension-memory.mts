...
const RSS_MARKER = "__AFORA_MAX_RSS_KB__=";
...
function defaultJsonReportPath(): string {
  return path.join(
    os.tmpdir(),
    `afora-extension-memory-${process.pid}-${Date.now()}-${randomUUID()}.json`,
  );
}
...
async function main(): Promise<void> {
  ...
  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "afora-extension-memory-"));
