...
docker_build_on_missing_enabled() {
  case "${AFORA_DOCKER_BUILD_ON_MISSING:-}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
    0 | false | FALSE | no | NO)
...
docker_build_command() {
  ...
  if [ "${AFORA_DOCKER_BUILD_USE_BUILDX:-0}" = "1" ] || docker_build_args_need_buildx "$@"; then
    build_cmd=(docker buildx build --load)
    if [ -n "${AFORA_DOCKER_BUILD_CACHE_FROM:-}" ]; then
      build_cmd+=(--cache-from "${AFORA_DOCKER_BUILD_CACHE_FROM}")
    fi
    if [ -n "${AFORA_DOCKER_BUILD_CACHE_TO:-}" ]; then
      build_cmd+=(--cache-to "${AFORA_DOCKER_BUILD_CACHE_TO}")
    fi
  fi
...
Try increasing the Docker/BuildKit memory limit, closing other memory-heavy processes, or rebuilding with a smaller Afora build heap, for example:
  AFORA_DOCKER_BUILD_NODE_OPTIONS=--max-old-space-size=4096 AFORA_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=4096 ./scripts/docker/setup.sh
EOF
}

docker_build_retry_count() {
  local configured="${AFORA_DOCKER_BUILD_RETRIES:-2}"
  if [[ "$configured" =~ ^[0-9]+$ ]]; then
    echo "$((10#$configured))"
    return 0
  fi
  echo "invalid AFORA_DOCKER_BUILD_RETRIES: $configured" >&2
  return 2
}

docker_build_timeout_required() {
  case "${AFORA_DOCKER_BUILD_REQUIRE_TIMEOUT:-0}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
  esac
...
docker_build_heartbeat_seconds() {
  local configured="${AFORA_DOCKER_BUILD_HEARTBEAT_SECONDS:-30}"
  if [[ "$configured" =~ ^[0-9]+$ ]] && [ "$configured" -ge 1 ]; then
    echo "$((10#$configured))"
    return 0
  fi
  echo "invalid AFORA_DOCKER_BUILD_HEARTBEAT_SECONDS: $configured" >&2
  return 2
}
...
docker_build_with_retries() {
  ...
  local timeout_value="${AFORA_DOCKER_BUILD_TIMEOUT:-3600s}"
  while true; do
...
docker_build_run() {
  ...
  AFORA_DOCKER_BUILD_REQUIRE_TIMEOUT="${AFORA_DOCKER_BUILD_REQUIRE_TIMEOUT:-1}" \
