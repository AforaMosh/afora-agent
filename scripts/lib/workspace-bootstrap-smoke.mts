// Verifies installed packages can bootstrap the default Afora workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params: { packageRoot: string }): void {
  const tempRoot = mkdtempSync(join(tmpdir(), "afora-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "afora.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".afora", "workspace");
