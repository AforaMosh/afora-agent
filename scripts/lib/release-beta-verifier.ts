...
// Release Beta Verifier script supports Afora repository automation.
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--release-sha SHA] [--workflow-ref REF] [--clawhub-workflow-ref REF] [--full-release-validation-run ID] [--afora-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID] [--plugin-clawhub-bootstrap-run ID --clawhub-bootstrap-plugins NAMES] [--npm-telegram-run ID] [--skip-github-release] [--skip-clawhub]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--afora-npm-run":
        parsed.workflowRuns.aforaNpm = next();
        break;
      case "--plugin-npm-run":
...
export function resolveAforaNpmPostpublishVerifier(rootDir: string, override?: string): string {
  if (override === undefined) {
    return resolve(rootDir, "scripts/afora-npm-postpublish-verify.ts");
  }
  const verifier = resolve(override);
  if (verifier !== resolve(TRUSTED_TOOLING_ROOT, "scripts/afora-npm-postpublish-verify.ts")) {
...
export async function verifyBetaRelease(
  ...
  const aforaNpm = await verifyNpmPackage("afora", args.version, args.distTag);
  lines.push(`afora npm OK: ${args.version} (${args.distTag})`);

  if (!args.skipPostpublish) {
    const postpublishVerifier = resolveAforaNpmPostpublishVerifier(
      rootDir,
      args.postpublishVerifier,
    );
    runCommandInherited("node", ["--import", "tsx", postpublishVerifier, args.version]);
    lines.push("afora postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.aforaNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.aforaNpm,
        label: "Afora NPM Release",
        repo: args.repo,
        expectedWorkflowName: "Afora NPM Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: false,
      }),
    );
      ...
      `${JSON.stringify(
        {
          ...
          aforaNpmIntegrity: aforaNpm.integrity,
          aforaNpmTarball: aforaNpm.tarball,
