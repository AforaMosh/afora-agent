// Plugin Npm Release script supports Afora repository automation.
...
export function isPluginExternalPublicationDeferred(packageJson: {
  afora?: { build?: { bundledDist?: unknown } };
}): boolean {
  return packageJson.afora?.build?.bundledDist === true;
}
...
export const AFORA_PLUGIN_NPM_REPOSITORY_URL = "https://github.com/AforaMosh/afora-agent";
...
export function collectRequiredLatestDependencies(packageJson: PluginPackageJson): {
  ...
  const configured = packageJson.afora?.release?.requireLatestDependencies;
  ...
  if (!Array.isArray(configured)) {
    return {
      dependencies: [],
      errors: ["afora.release.requireLatestDependencies must be an array of package names."],
    };
  }
  ...
  for (const value of configured) {
    if (typeof value !== "string" || !value.trim()) {
      errors.push(
        "afora.release.requireLatestDependencies must contain only non-empty package names.",
      );
      continue;
    }
    ...
    if (seen.has(packageName)) {
      errors.push(
        `afora.release.requireLatestDependencies must not contain duplicate package names; found "${packageName}".`,
      );
      continue;
    }
    ...
    if (typeof version !== "string" || !version.trim()) {
      errors.push(
        `afora.release.requireLatestDependencies must reference package.json dependencies or optionalDependencies; "${packageName}" is not a runtime dependency.`,
      );
      continue;
    }
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.afora?.install?.npmSpec);
  const repositoryUrl =
    typeof packageJson.repository === "string"
      ? packageJson.repository.trim()
      : (packageJson.repository?.url?.trim() ?? "");
  const extensions = packageJson.afora?.extensions ?? [];
  const requiredLatestDependencies = collectRequiredLatestDependencies(packageJson);

  if (!packageName.startsWith("@afora/")) {
    errors.push(
      `package name must start with "@afora/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (repositoryUrl !== AFORA_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${AFORA_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("afora.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("afora.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("afora.install.npmSpec must be a non-empty string for publishable plugins.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.afora?.install?.npmSpec),
...
function runNpmView(args: string[]): string {
  const tempDir = mkdtempSync(join(tmpdir(), "afora-plugin-npm-view-"));
