...
require_swift_toolchain() {
  ...
  if ! xcodebuild_version="$(xcrun xcodebuild -version 2>&1)"; then
    ...
    echo "ERROR: Afora macOS app packaging requires a full Xcode developer directory." >&2
  ...
  if ! swift_version="$(swift --version 2>&1)"; then
    printf '%s\n' "$swift_version" >&2
    echo "ERROR: Afora macOS app packaging requires Swift tools ${REQUIRED_SWIFT_TOOLS_MAJOR}.${REQUIRED_SWIFT_TOOLS_MINOR}+." >&2
  ...
  if [[ -z "$major_minor" ]]; then
    ...
    echo "       Afora macOS app packaging requires Swift tools ${REQUIRED_SWIFT_TOOLS_MAJOR}.${REQUIRED_SWIFT_TOOLS_MINOR}+." >&2
    return 1
  fi
