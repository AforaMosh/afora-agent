...
type KovaWorkflowEvidenceParams = Record<
  "plan" | "profile" | "repeat" | "report" | "target",
  unknown
> & { authMode: unknown; expectedModel: unknown; includeFilters: unknown };
...
function positiveInteger(value: unknown, label: string) {
  check(typeof value === "number" && Number.isSafeInteger(value) && value > 0, `invalid ${label}`);
  return value;
}

function exactStrings(value: unknown, expected: string[], label: string) {
...
function pairKey(scenario: unknown, state: unknown, label: string) {
  return `${text(scenario, `${label} scenario`)}\u0000${text(state, `${label} state`)}`;
}

function displayPair(key: string) {
  return key.replace("\u0000", "/");
}
...
function repeatIndexesFor(selectedPairs: Set<string>) {
  return new Map([...selectedPairs].map((key) => [key, new Set<number>()]));
}

function validateLiveRecord(record: Record<string, unknown>, key: string, expectedModel: string) {
  ...
  check(
    provider.source === "afora-timeline",
...
export function validateKovaWorkflowEvidence({
  ...
  for (const [index, value] of records.entries()) {
    ...
    check(indexes !== undefined, `record ${displayPair(key)} lacked repeat tracking`);
...
function parseCliArgs(argv: string[]) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    check(
      name !== undefined && name.startsWith("--") && value !== undefined,
...
function readJson(pathValue: unknown, label: string): unknown {
...
function runCli() {
  ...
  const include = text(flags.include, "--include");
