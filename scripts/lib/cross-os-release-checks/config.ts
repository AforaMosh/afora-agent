...
export const PUBLISHED_INSTALLER_BASE_URL = "https://afora.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "AFORA_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
export const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "AFORA_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
const providerConfig = {
  openai: {
    ...
    requiredCompanionPackages: ["@afora/codex"],
...
export function resolveProviderConfig(provider: string, env = process.env): ProviderConfig | null {
  ...
  const providerEnvKey = `AFORA_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.AFORA_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
export function buildReleaseProviderConfigOverride(providerMeta: ProviderConfig) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "afora" } } : {}),
    models: [],
...
export const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.afora-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "AFORA_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "AFORA_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("AFORA_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.AFORA_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
...
export function buildRealUpdateEnv(env: NodeJS.ProcessEnv) {
  ...
  delete updateEnv.AFORA_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]afora\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/afora[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec: string) {
  ...
  if (trimmed.startsWith("afora@")) {
    return trimmed.slice("afora@".length);
  }
  return trimmed;
}
