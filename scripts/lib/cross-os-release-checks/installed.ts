...
export function buildInstallerSmokeScript(
  ...
  if ((params.platform ?? process.platform) === "win32") {
    return `
$installerPath = Join-Path ([System.IO.Path]::GetTempPath()) ("afora-installer-" + [guid]::NewGuid().ToString("N") + ".ps1")
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "curl.exe failed to download the Afora installer (exit $LASTEXITCODE)"
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'afora.cmd'),
      (Join-Path $npmPrefix 'afora.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command afora -ErrorAction Stop
...
Write-Output "__AFORA_PATH__=$commandPath"
...
export async function verifyFreshShellCommand(params: {
  ...
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__AFORA_PATH__=") ?? "",
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed afora path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v afora >/dev/null 2>&1",
    'printf "__AFORA_PATH__=%s\\n" "$(command -v afora)"',
    "afora --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__AFORA_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed afora path from fresh POSIX shell.");
  }
...
export async function ensureDevUpdateGitInstall(params: {
  ...
  // The dev-update lane must prove that `afora update --channel dev` landed on
