// Scope script supports Afora repository automation.
...
export function createPluginSdkScope(_repoRoot: string): TopologyScope {
  const entrypoints = publicPluginSdkEntrypoints.map((entrypoint) => ({
    ...
    importSpecifier: `afora/plugin-sdk/${entrypoint}`,
  }));
  return buildScopeFromEntrypoints("plugin-sdk", "Afora plugin-sdk public surface", entrypoints);
}
