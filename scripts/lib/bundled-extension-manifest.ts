// Bundled Extension Manifest script supports Afora repository automation.
...
export function collectBundledExtensionManifestErrors(extensions: BundledExtension[]): string[] {
  ...
  for (const extension of extensions) {
    const install = extension.packageJson.afora?.install;
    if (install !== undefined && !isRecord(install)) {
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | afora.install must be an object`,
      );
      continue;
    }
    ...
    if (
      ...
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | afora.install.npmSpec must be a non-empty string`,
      );
    }
