// Afora release ClawHub plan script supports release workflow routing.
...
type AforaReleaseClawHubPlanArgs = {
...
type AforaReleaseClawHubRuntimeStateArgs = {
  repository: string;
...
type AforaReleaseClawHubRuntimeState = {
  verifierArgs: string[];
  proofLines: {
    normal: string;
    bootstrap: string;
...
export function buildAforaReleaseClawHubRuntimeState(
  args: AforaReleaseClawHubRuntimeStateArgs,
): AforaReleaseClawHubRuntimeState {
...
export function parseAforaReleaseClawHubPlanArgs(
  argv: string[],
): AforaReleaseClawHubPlanArgs {
  const values = [...argv];
  if (values[0] === "--") {
    values.shift();
  }
...
export async function buildAforaReleaseClawHubPlan(
  args: AforaReleaseClawHubPlanArgs,
  options: {
    rootDir?: string;
    fetchImpl?: typeof fetch;
    registryBaseUrl?: string;
  } = {},
): Promise<AforaReleaseClawHubPlan> {
