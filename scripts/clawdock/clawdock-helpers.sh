#!/usr/bin/env bash
# ClawDock - Docker helpers for Afora
# Inspired by Simon Willison's "Running Afora in Docker"
# https://til.simonwillison.net/llms/afora-docker
#
# Installation:
...
# Common paths to check for Afora
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/afora"
  "${HOME}/workspace/afora"
  "${HOME}/projects/afora"
  "${HOME}/dev/afora"
  "${HOME}/code/afora"
  "${HOME}/src/afora"
...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port afora-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Afora at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/afora"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Afora not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/AforaMosh/afora-agent.git ~/afora"
    echo "  cd ~/afora && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^AFORA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d afora-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart afora-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f afora-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.afora"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show afora.json
  if [[ -f "${config_dir}/afora.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/afora.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/afora.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No afora.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec afora-gateway \
    bash -c 'echo "alias afora=\"./afora.mjs\"" > /tmp/.bashrc_afora && bash --rcfile /tmp/.bashrc_afora'
}
...
clawdock-cli() {
  _clawdock_compose run --rm afora-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build afora-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d afora-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build afora-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "AFORA_GATEWAY_TOKEN=$token" afora-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" afora-gateway \
    bash -c './afora.mjs config set gateway.remote.token "$TOKEN" && ./afora.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec afora-gateway \
    bash -c "./afora.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart afora-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps afora-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec afora-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'afora config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Afora${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (afora alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to afora project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.afora)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'afora channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'afora status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.afora.ai${_CLR_RESET}"
  echo ""
}
