...
const PACKAGE_DOCKER_TEST_TARGET =
  "test/e2e/qa-lab/runtime/package-afora-for-docker.e2e.test.ts";
...
const INCLUDE_FILE_ENV_KEY = "AFORA_VITEST_INCLUDE_FILE";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-afora-runtime)?\.ts$/u,
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/afora-test-state.ts", ["src/test-utils/afora-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "AFORA_TEST_CHANGED_BROAD";
...
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "AFORA_VITEST_NO_OUTPUT_RETRY";
...
const npmPostpublish = "test/afora-npm-postpublish-verify.test.ts";
const crossOsReleaseChecks = "afora-cross-os-release-checks";
...
const EXACT_TOOLING_TARGETS = new Map<string, string[]>([
  ...
  [
    ...
    [
      ...
      "test/afora-npm-release-check.test.ts",
      npmPostpublish,
      ...
      "ios-version",
      "afora-npm-extended-stable-release",
      "afora-npm-publish",
      "release-preflight",
      "release-prepare",
  ...
  ["scripts/afora-postpack.mjs", [TOOLING_VITEST_CONFIG]],
  ["scripts/package-manifest.mjs", ["test/afora-prepack.test.ts"]],
  ["scripts/afora-npm-prepublish-verify.ts", ["test/afora-npm-prepublish-verify.test.ts"]],
...
const SEMANTIC_TOOLING_TARGET_PATTERNS: Array<[RegExp, string[]]> = [
  ...
  [
    /^\.github\/workflows\/afora-release-checks\.yml$/u,
  ...
  [/^\.github\/workflows\/afora-performance\.yml$/u, ["afora-performance-workflow"]],
  [/^\.github\/workflows\/plugin-prerelease\.yml$/u, [pluginPrerelease]],
  [/^\.github\/workflows\/tui-pty\.yml$/u, [packageAcceptance]],
  [
    /^\.github\/workflows\/afora-cross-os-release-checks-reusable\.yml$/u,
    [crossOsReleaseChecks, "afora-cross-os-release-workflow", packageAcceptance],
  ],
  [
    /^\.github\/workflows\/(?:afora-release-publish|package-acceptance)\.yml$/u,
    [packageAcceptance],
  ],
  [
  ...
  [
    /^\.github\/workflows\/afora-npm-release\.yml$/u,
    [npmPostpublish, "afora-npm-extended-stable-workflow", packageAcceptance],
  ],
  [
    new RegExp(
      [
  ...
  [
    /^scripts\/package-afora-for-docker\.m[jt]s$/u,
    ["test/e2e/qa-lab/runtime/package-afora-for-docker.e2e.test.ts"],
  ],
  ...
  [/^scripts\/afora-npm-postpublish-verify\.ts$/u, [npmPostpublish]],
  [
  ...
  [
    /^scripts\/github\/run-afora-cross-os-release-checks\.sh$/u,
    ["afora-cross-os-release-workflow"],
  ],
  [
  ...
  [
    new RegExp(
      [
        ...
        "^scripts\\/systemd\\/afora-auth-monitor\\.(?:service|timer)$",
      ].join(""),
      "u",
    ),
    ["auth-monitor"],
  ...
  [
    ...
    [releaseCheck, "test/afora-npm-release-check.test.ts"],
  ],
  ...
  [
    /^\.agents\/skills\/afora-changelog-update\/scripts\/verify-release-notes\.mjs$/u,
  ...
  [/^scripts\/e2e\/onboard-docker\.sh$/u, [dockerBuild, "afora-test-state"]],
  [
    new RegExp(
      [
...
function resolveToolingTestTargets(changedPath: string, cwd = process.cwd()) {
  ...
  const crossOsReleaseTargets =
    implementationPath === "scripts/afora-cross-os-release-checks.ts" ||
...
function resolvePreciseChangedTestTargets(
  ...
  const mappedTargets =
    ...
    (/^extensions\/[^/]+\/afora\.plugin\.json$/u.test(changedPath)
...
export function buildFullSuiteVitestRunPlans(args: string[], cwd = process.cwd()) {
  ...
  const parallelShardCount = parsePositiveInt(
    process.env.AFORA_TEST_PROJECTS_PARALLEL,
    "AFORA_TEST_PROJECTS_PARALLEL",
  );
  ...
  return fullSuiteVitestShards.flatMap((shard) => {
    ...
    const expandShard =
      ...
      (process.env.AFORA_TESTBOX_REMOTE_RUN === "1" &&
...
function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return env.AFORA_TEST_PROJECTS_SERIAL !== "1" && !isCiLikeEnv(env);
}
...
function hasConservativeVitestWorkerBudget(env: NodeJS.ProcessEnv) {
  const workerBudget = parsePositiveInt(
    env.AFORA_VITEST_MAX_WORKERS ?? env.AFORA_TEST_WORKERS,
    env.AFORA_VITEST_MAX_WORKERS === undefined
      ? "AFORA_TEST_WORKERS"
...
export function resolveParallelFullSuiteConcurrency(
  ...
  const override = parsePositiveInt(
    env.AFORA_TEST_PROJECTS_PARALLEL,
    "AFORA_TEST_PROJECTS_PARALLEL",
  );
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.AFORA_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.AFORA_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
...
export function createVitestRunSpecs(
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(os.tmpdir(), `afora-vitest-include-${randomUUID()}-${index}.json`)
      : null;
    return {
      config: plan.config,
      env: includeFilePath
