...
#
# Run the Afora precise OpenGrep rulepack against the local working tree
...
resolve_changed_diff_ref() {
  local diff_ref="${AFORA_OPENGREP_BASE_REF:-origin/main...HEAD}"
  local base_ref
  local head_ref
  local resolved_base
  ...
  if [[ "${AFORA_OPENGREP_MERGE_HEAD_FIRST_PARENT:-0}" != "1" ]]; then
    printf '%s\n' "$diff_ref"
    return 0
  fi
