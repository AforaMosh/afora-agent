// Bench Cli Startup script supports Afora repository automation.
...
const DEFAULT_ENTRY = "afora.mjs";
const MAX_RSS_MARKER = "__AFORA_MAX_RSS_KB__=";

function resolveTimeoutKillGraceMs(env: NodeJS.ProcessEnv): number {
  const raw = env.VITEST ? env.AFORA_TEST_CLI_STARTUP_TIMEOUT_KILL_GRACE_MS : undefined;
  if (!raw || !/^\d+$/u.test(raw)) {
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "AFORA_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "AFORA_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.AFORA_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = params.runRoot ?? mkdtempSync(path.join(os.tmpdir(), "afora-cli-bench-home-"));
  const ownsRunRoot = params.runRoot == null;
  const stateDir = path.join(runRoot, ".afora");
...
async function runCase(params: {
  ...
  const caseRunRoot =
    params.commandCase.stateScope === "case"
      ? mkdtempSync(path.join(os.tmpdir(), "afora-cli-bench-home-"))
      : undefined;
  try {
...
function printUsage(): void {
  console.log(`Afora CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: afora.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "afora-cli-bench-"));
