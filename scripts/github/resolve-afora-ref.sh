...
REMOTE_URL="${AFORA_REF_REMOTE:-https://github.com/AforaMosh/afora-agent.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-afora-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Afora branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve Afora ref: ${REF}" >&2
exit 1
