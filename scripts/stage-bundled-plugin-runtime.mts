...
function ensureAforaExtensionAlias(params: { distExtensionsRoot: string; repoRoot: string }) {
  ...
  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "afora");
...
function shouldCopyRuntimeFile(relativePath: string) {
  return (
    ...
    isPathOrNestedPath(relativePath, "afora.plugin.json") ||
...
export function stageBundledPluginRuntime(params: { cwd?: string; repoRoot?: string } = {}) {
  ...
  ensureAforaExtensionAlias({ repoRoot, distExtensionsRoot });
