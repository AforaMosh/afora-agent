...
const FS_MODULE_CACHE_WRITER_ENV_KEY = "AFORA_VITEST_FS_MODULE_CACHE_WRITER";
const NODE_COMPILE_CACHE_PATH_ENV_KEY = "NODE_COMPILE_CACHE";
const NODE_COMPILE_CACHE_WRITER_ENV_KEY = "AFORA_NODE_COMPILE_CACHE_WRITER";
const VITEST_EXTRA_ARGS_ENV_KEY = "AFORA_NODE_TEST_VITEST_ARGS_JSON";
...
const FS_MODULE_CACHE_GENERATION_FILE = ".afora-transform-generation";
...
export function resolveShardPlans(env: NodeJS.ProcessEnv = process.env): ShardPlan[] {
  const targets = parseJsonEnv(env, "AFORA_NODE_TEST_TARGETS_JSON");
  ...
  const groups = parseJsonEnv(env, "AFORA_NODE_TEST_GROUPS_JSON");
  const groupPlans = Array.isArray(groups) ? groups.filter(isShardGroupConfig) : [];
  const configs = parseJsonEnv(env, "AFORA_NODE_TEST_CONFIGS_JSON", []);
  const groupEnv = parseJsonEnv(env, "AFORA_NODE_TEST_ENV_JSON");
  const includePatterns = parseJsonEnv(env, "AFORA_NODE_TEST_INCLUDE_PATTERNS_JSON");
      ...
      : [
          {
            ...
            shard_name: env.AFORA_VITEST_SHARD_NAME,
          },
        ];
...
export function buildChildEnv(
  ...
  const childEnv: NodeJS.ProcessEnv = {
    ...
    AFORA_TEST_PROJECTS_PARALLEL: "1",
  };
  if (entry.kind === "group") {
    const plan = entry.plan;
    if (plan.shard_name) {
      childEnv.AFORA_VITEST_SHARD_NAME = plan.shard_name;
    }
    ...
    if (Array.isArray(plan.includePatterns) && plan.includePatterns.length > 0) {
      ...
      childEnv.AFORA_VITEST_INCLUDE_FILE = includeFile;
    } else {
      delete childEnv.AFORA_VITEST_INCLUDE_FILE;
    }
  }
  return childEnv;
}
...
export async function runShardPlans(plans: ShardPlan[], options: RunShardOptions = {}) {
  ...
  const scratchDir = options.scratchDir ?? mkdtempSync(join(tmpdir(), "afora-node-shard-"));
  const persistentCacheRoot = baseEnv[FS_MODULE_CACHE_PATH_ENV_KEY]?.trim();
...
if (isDirectRunUrl(process.argv[1], import.meta.url)) {
  ...
  const planConcurrency = Number(process.env.AFORA_NODE_TEST_PLAN_CONCURRENCY) || undefined;
