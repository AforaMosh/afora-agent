...
SERVER="${AFORA_SERVER:-afora-host}"

# Check auth status
termux-toast "Checking Afora auth..."

STATUS=$(ssh "$SERVER" '$HOME/afora/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/afora/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|AFORA_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/afora/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Afora won't work until re-auth"
                ;;
        esac
        ;;
