...
// Afora Npm Release Check script supports Afora repository automation.
...
const skipPackValidationEnv = "AFORA_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return readPositiveEnvInt(
    "AFORA_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "afora") {
    errors.push(`package.json name must be "afora"; found "${pkg.name ?? ""}".`);
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`afora-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `afora-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
