...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-cron-mcp-cleanup-e2e" AFORA_IMAGE)"
PORT="18789"
TOKEN="cron-mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="afora-cron-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t afora-cron-mcp-client-log.XXXXXX)"
...
AFORA_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 cron-mcp-cleanup empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    export SUCCESS_MARKER=AFORA_CRON_MCP_CLEANUP_OK
    export MOCK_REQUEST_LOG=/tmp/afora-cron-mock-openai-requests.jsonl
    export AFORA_DOCKER_OPENAI_BASE_URL=\"http://127.0.0.1:\$MOCK_PORT/v1\"
    mock_pid=\"\$(afora_e2e_start_mock_openai \"\$MOCK_PORT\" /tmp/cron-mcp-cleanup-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(afora_e2e_start_gateway \"\$entry\" $PORT /tmp/cron-mcp-cleanup-gateway.log)\"
    afora_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/cron-mcp-cleanup-gateway.log 300 $PORT
