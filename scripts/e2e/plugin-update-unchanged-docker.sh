#!/usr/bin/env bash
# Verifies `afora plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-plugin-update-e2e" AFORA_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${AFORA_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${AFORA_CURRENT_PACKAGE_TGZ:-}")"
...
AFORA_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
