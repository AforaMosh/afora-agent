...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-mcp-channels-e2e" AFORA_IMAGE)"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="afora-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t afora-mcp-client-log.XXXXXX)"
...
AFORA_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-channels empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    mock_pid=\"\$(afora_e2e_start_mock_openai \"\$mock_port\" /tmp/mcp-channels-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(afora_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-channels-gateway.log)\"
    afora_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-channels-gateway.log 480 $PORT
