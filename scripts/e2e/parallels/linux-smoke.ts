...
// Linux Smoke script supports Afora repository automation.
...
function parseAforaPackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareAforaPackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseAforaPackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke extends SmokeRunController<LinuxOptions> {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("afora-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("afora-parallels-linux-tgz.");
    try {
  ...
  protected async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("afora-main-upgrade.tgz"),
    );
...
npm uninstall -g afora-agent >/dev/null 2>&1 || true
rm -rf /root/.afora /root/.npm/_cacache
rm -f /tmp/afora-parallels-linux-gateway.log`);
  }

  private installLatestRelease(): void {
    this.downloadGuestFile(this.options.installUrl, "/tmp/afora-install.sh");
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "AFORA_NO_ONBOARD=1",
        "bash",
        "/tmp/afora-install.sh",
        "--version",
...
if command -v curl >/dev/null 2>&1; then
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["afora", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.afora/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@afora/test-bad-plugin","version":"1.0.0","afora":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/afora.plugin.json" <<'JSON'
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseAforaPackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareAforaPackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " AFORA_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "afora gateway run" >/dev/null 2>&1 || true
rm -f /tmp/afora-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["afora", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["afora", "gateway", "status", "--deep", "--require-rpc"]
      : ["afora", "gateway", "status", "--deep"];
    const result = run(
      "prlctl",
      ["exec", this.options.vmName, "/usr/bin/env", "HOME=/root", "AFORA_ALLOW_ROOT=1", ...args],
      {
        check: false,
        quiet: true,
        timeoutMs: this.remainingPhaseTimeoutMs(),
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.afora/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.afora/test-bad-plugin`);
  }
  ...
  private verifyLocalTurn(): void {
    this.guestExec(["afora", "models", "set", this.auth.modelId]);
...
JSON
afora config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["afora", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env AFORA_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} afora agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private async extractLastVersion(phaseId: string): Promise<string> {
    return await extractLastAforaVersion(
      this.runDir,
      phaseId,
      /(Afora [^\r\n]+ \([0-9a-f]{7,}\))/g,
    );
