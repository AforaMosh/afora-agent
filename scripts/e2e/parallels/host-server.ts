// Host Server script supports Afora repository automation.
...
export async function startNpmRegistryServer(input: {
  ...
  const portFile = path.join(tmpdir(), `afora-npm-registry-${randomUUID()}.port`);
  ...
  const child = spawn(
    ...
    ["scripts/e2e/lib/plugins/npm-registry-server.mjs", portFile, ...packageArgs],
    {
      env: {
        ...process.env,
        AFORA_NPM_REGISTRY_BIND_HOST: "0.0.0.0",
        AFORA_NPM_REGISTRY_PORT: String(port),
