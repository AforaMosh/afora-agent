// Powershell script supports Afora repository automation.
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:AFORA_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.afora\\afora.json' }
$agentTurnVersionText = Invoke-Afora --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'Afora\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:AFORA_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:AFORA_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:AFORA_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'afora-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.AFORA_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.AFORA_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.AFORA_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@afora", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "afora" };
  } else {
...
Remove-Item Env:AFORA_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:AFORA_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:AFORA_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsAforaResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-AforaCommand {
  if ($script:AforaResolvedCommand) { return $script:AforaResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\afora.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\afora.ps1'
  }
  foreach ($name in @('afora.cmd', 'afora.ps1', 'afora')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'afora.cmd'
    $shimCandidates += Join-Path $npmPrefix 'afora.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:AforaResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:AforaResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\afora\afora.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:AforaResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:AforaResolvedCommand
    }
  }
  throw 'afora command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-Afora {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $AforaArgs)
  $command = Resolve-AforaCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @AforaArgs
    } else {
      & $command.Path @AforaArgs
    }
  } finally {
