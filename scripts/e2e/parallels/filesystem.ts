// Filesystem script supports Afora repository automation.
...
const AFORA_VERSION_PATTERN = /Afora\s+([0-9][^\s]*)/gi;
...
export async function extractLastAforaVersionFromLog(
  logPath: string,
  pattern = AFORA_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.AFORA_PARALLELS_ARTIFACT_ROOT || path.join(repoRoot, ".artifacts", "parallels");
