...
BASE_IMAGE="$(docker_e2e_resolve_image "afora-browser-cdp-base-e2e" AFORA_BROWSER_CDP_BASE_E2E_IMAGE)"
if [ -n "${AFORA_BROWSER_CDP_SNAPSHOT_E2E_IMAGE:-}" ]; then
  IMAGE_NAME="$AFORA_BROWSER_CDP_SNAPSHOT_E2E_IMAGE"
  DERIVED_SHARED_IMAGE="0"
elif [ -n "${AFORA_DOCKER_E2E_IMAGE:-}" ]; then
  IMAGE_NAME="afora-browser-cdp-snapshot-e2e:${AFORA_DOCKER_ALL_LANE_NAME:-shared}"
  DERIVED_SHARED_IMAGE="1"
else
  IMAGE_NAME="afora-browser-cdp-snapshot-e2e"
  DERIVED_SHARED_IMAGE="0"
fi
SKIP_BUILD="${AFORA_BROWSER_CDP_SNAPSHOT_E2E_SKIP_BUILD:-0}"
PORT="18789"
CDP_PORT="19222"
FIXTURE_PORT="18080"
TOKEN="browser-cdp-e2e-token"
CONTAINER_NAME="afora-browser-cdp-e2e-$$"
DOCKER_COMMAND_TIMEOUT="${AFORA_BROWSER_CDP_SNAPSHOT_DOCKER_COMMAND_TIMEOUT:-900s}"
SNAPSHOT_MAX_BYTES="$(docker_e2e_read_positive_int_env AFORA_BROWSER_CDP_SNAPSHOT_MAX_BYTES 524288)"
...
if [ "$SKIP_BUILD" = "1" ] || { [ "$DERIVED_SHARED_IMAGE" = "0" ] && [ "${AFORA_SKIP_DOCKER_BUILD:-0}" = "1" ]; }; then
  ...
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/afora-browser-cdp-build.XXXXXX")"
...
AFORA_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 browser-cdp-snapshot empty)"
...
docker_e2e_docker_cmd run -d \
  ...
  -e "AFORA_BROWSER_CDP_SNAPSHOT_MAX_BYTES=$SNAPSHOT_MAX_BYTES" \
...
mkdir -p /tmp/afora-browser-cdp
find dist -maxdepth 1 -type f -name 'pw-ai-*.js' ! -name 'pw-ai-state-*' -exec mv {} /tmp/afora-browser-cdp/ \;
...
afora_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/browser-cdp-gateway.log" >/dev/null

echo "Waiting for Gateway and fixture server..."
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "
    source scripts/lib/afora-e2e-instance.sh
    afora_e2e_probe_http_status http://127.0.0.1:$FIXTURE_PORT/
    afora_e2e_probe_tcp 127.0.0.1 $PORT
"; then
...
set -euo pipefail
source /tmp/afora-test-state-env
source scripts/lib/afora-e2e-instance.sh
entry=\"\$(afora_e2e_resolve_entrypoint)\"
