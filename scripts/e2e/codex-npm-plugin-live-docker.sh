#!/usr/bin/env bash
# Installs Afora from a prepared package tarball, installs @afora/codex
...
CANDIDATE_ROOT="${AFORA_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-codex-npm-plugin-live-e2e" AFORA_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${AFORA_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${AFORA_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${AFORA_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${AFORA_CODEX_NPM_PLUGIN_PROFILE_FILE:-${AFORA_TESTBOX_PROFILE_FILE:-$HOME/.afora-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${AFORA_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
...
ASSERT_MAX_TEXT_FILE_BYTES="$(
  docker_e2e_read_positive_int_env AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES 1048576
)"
ASSERT_MAX_ERROR_TAIL_BYTES="$(
  docker_e2e_read_positive_int_env AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES 65536
)"
ASSERT_MAX_TRANSCRIPT_FILES="$(
  docker_e2e_read_positive_int_env AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES 64
)"
ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES="$(
  docker_e2e_read_positive_int_env AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES 4096
)"
ASSERT_MAX_TRANSCRIPT_SCAN_BYTES="$(
  docker_e2e_read_positive_int_env AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES 2097152
)"
AGENT_TURN_TIMEOUT_SECONDS="$(
  docker_e2e_read_positive_int_env AFORA_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS 420
)"
SESSION_STORE_CONTRACT="${AFORA_CODEX_NPM_PLUGIN_SESSION_STORE_CONTRACT:-}"
...
BINDING_STORE_CONTRACT="${AFORA_CODEX_NPM_PLUGIN_BINDING_STORE_CONTRACT:-}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${AFORA_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "AFORA_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires AFORA_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/afora-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/afora-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      docker_e2e_print_log /tmp/afora-codex-plugin-pack.log >&2
      exit 1
    fi
...
AFORA_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e AFORA_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${AFORA_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e AFORA_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${AFORA_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e AFORA_CODEX_NPM_PLUGIN_MODEL="${AFORA_CODEX_NPM_PLUGIN_MODEL:-openai/gpt-5.4}" \
  -e AFORA_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e AFORA_CODEX_NPM_PLUGIN_REGISTRY_PACKAGE="$CODEX_PLUGIN_REGISTRY_PACKAGE" \
  -e AFORA_CODEX_NPM_PLUGIN_REGISTRY_TARBALL="$CODEX_PLUGIN_REGISTRY_TARBALL" \
  -e AFORA_CODEX_NPM_PLUGIN_REGISTRY_VERSION="$CODEX_PLUGIN_REGISTRY_VERSION" \
  -e AFORA_CODEX_NPM_PLUGIN_BINDING_STORE_CONTRACT="$BINDING_STORE_CONTRACT" \
  -e AFORA_CODEX_NPM_PLUGIN_SESSION_STORE_CONTRACT="$SESSION_STORE_CONTRACT" \
  -e "AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES=$ASSERT_MAX_TEXT_FILE_BYTES" \
  -e "AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES=$ASSERT_MAX_ERROR_TAIL_BYTES" \
  -e "AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES=$ASSERT_MAX_TRANSCRIPT_FILES" \
  -e "AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES=$ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES" \
  -e "AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES=$ASSERT_MAX_TRANSCRIPT_SCAN_BYTES" \
...
export AFORA_AGENT_HARNESS_FALLBACK=none
...
CODEX_PLUGIN_SPEC="${AFORA_CODEX_NPM_PLUGIN_SPEC:?missing AFORA_CODEX_NPM_PLUGIN_SPEC}"
CODEX_PLUGIN_REGISTRY_PACKAGE="${AFORA_CODEX_NPM_PLUGIN_REGISTRY_PACKAGE:-}"
CODEX_PLUGIN_REGISTRY_TARBALL="${AFORA_CODEX_NPM_PLUGIN_REGISTRY_TARBALL:-}"
CODEX_PLUGIN_REGISTRY_VERSION="${AFORA_CODEX_NPM_PLUGIN_REGISTRY_VERSION:-}"
MODEL_REF="${AFORA_CODEX_NPM_PLUGIN_MODEL:?missing AFORA_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="$MODEL_REF"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="AFORA-CODEX-NPM-PLUGIN-LIVE-OK"
AGENT_TURN_TIMEOUT_SECONDS="${AFORA_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS:-420}"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${AFORA_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  afora_e2e_dump_logs \
    ...
    /tmp/afora-codex-plugin-registry.log \
    /tmp/afora-codex-plugin-install.log \
    /tmp/afora-codex-plugin-enable.log \
    /tmp/afora-codex-plugins-list.json \
    /tmp/afora-codex-plugin-inspect.json \
    /tmp/afora-codex-preflight.log \
    /tmp/afora-codex-agent.json \
    /tmp/afora-codex-agent.err \
    /tmp/afora-codex-agent-turn1.json \
    /tmp/afora-codex-agent-turn1.err \
    /tmp/afora-codex-agent-turn2.json \
    /tmp/afora-codex-agent-turn2.err \
    /tmp/afora-codex-followthrough.json \
    /tmp/afora-codex-followthrough.log \
    /tmp/afora-codex-followthrough.err \
    /tmp/afora-codex-plugin-uninstall.log \
    /tmp/afora-codex-plugins-list-after-uninstall.json \
    /tmp/afora-codex-agent-after-uninstall.json \
    /tmp/afora-codex-agent-after-uninstall.err
}
...
if [ -n "$CODEX_PLUGIN_REGISTRY_TARBALL" ]; then
  registry_port_file=/tmp/afora-codex-plugin-registry.port
  rm -f "$registry_port_file"
  AFORA_NPM_REGISTRY_UPSTREAM="${AFORA_CODEX_NPM_PLUGIN_REGISTRY_UPSTREAM:-https://registry.npmjs.org}" \
    node scripts/e2e/lib/plugins/npm-registry-server.mjs \
      "$registry_port_file" \
      "$CODEX_PLUGIN_REGISTRY_PACKAGE" \
      "$CODEX_PLUGIN_REGISTRY_VERSION" \
      "$CODEX_PLUGIN_REGISTRY_TARBALL" \
      >/tmp/afora-codex-plugin-registry.log 2>&1 &
  registry_pid=$!
  for _ in $(seq 1 100); do
    ...
    if ! kill -0 "$registry_pid" 2>/dev/null; then
      afora_e2e_print_log /tmp/afora-codex-plugin-registry.log >&2
      exit 1
    fi
    sleep 0.1
  done
...
afora plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/afora-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
afora plugins enable codex >/tmp/afora-codex-plugin-enable.log 2>&1

afora plugins list --json >/tmp/afora-codex-plugins-list.json
afora plugins inspect codex --runtime --json >/tmp/afora-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/afora-codex-preflight.log 2>&1 </dev/null
...
echo "Running Afora local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the Afora Codex plugin." \
  /tmp/afora-codex-agent-turn1.json \
  /tmp/afora-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
FOLLOWTHROUGH_WORKSPACE="${AFORA_STATE_DIR:?missing AFORA_STATE_DIR}/workspace"
...
AFORA_PACKAGE_ROOT="$(afora_e2e_package_root "$NPM_CONFIG_PREFIX")"
if node scripts/e2e/lib/codex-npm-plugin-live/followthrough-turn.mjs \
  ...
  >/tmp/afora-codex-followthrough.log \
  2>/tmp/afora-codex-followthrough.err </dev/null; then
  followthrough_status=0
else
  followthrough_status=$?
fi
echo "followthrough_agent_status: $followthrough_status stdout_bytes=$(wc -c </tmp/afora-codex-followthrough.json 2>/dev/null || printf 0) stderr_bytes=$(wc -c </tmp/afora-codex-followthrough.err 2>/dev/null || printf 0)"
...
afora plugins uninstall codex --force >/tmp/afora-codex-plugin-uninstall.log 2>&1
afora plugins list --json >/tmp/afora-codex-plugins-list-after-uninstall.json
...
if afora agent --local \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/afora-codex-agent-after-uninstall.json 2>/tmp/afora-codex-agent-after-uninstall.err; then
