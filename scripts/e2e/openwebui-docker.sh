#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized Afora Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-openwebui-e2e" AFORA_OPENWEBUI_E2E_IMAGE)"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env AFORA_OPENWEBUI_MAX_MEMORY_MIB 8192)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env AFORA_OPENWEBUI_MAX_CPU_PERCENT 1600)"
...
MODEL="${AFORA_OPENWEBUI_MODEL:-openai/gpt-5.6-luna}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${AFORA_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="$(docker_e2e_read_tcp_port_env AFORA_OPENWEBUI_GATEWAY_PORT 18789)"
WEBUI_PORT="$(docker_e2e_read_tcp_port_env AFORA_OPENWEBUI_PORT 8080)"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${AFORA_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${AFORA_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="afora-openwebui-e2e-$$"
GW_NAME="afora-openwebui-gateway-$$"
OW_NAME="afora-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${AFORA_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
DOCKER_PULL_TIMEOUT="${AFORA_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROBE_FETCH_TIMEOUT_MS="${AFORA_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS_DECIMAL * 1000 + 60000))}"
validate_positive_int AFORA_OPENWEBUI_FETCH_TIMEOUT_MS "$PROBE_FETCH_TIMEOUT_MS"
DOCKER_COMMAND_TIMEOUT="${AFORA_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS_DECIMAL + 90))s}"
...
PROFILE_FILE="${AFORA_TESTBOX_PROFILE_FILE:-$HOME/.afora-testbox-live.profile}"
...
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/afora-openwebui-stats.XXXXXX")"
PROBE_LOG="$(mktemp "${TMPDIR:-/tmp}/afora-openwebui-probe.XXXXXX")"
STATS_STOP_FILE="$(mktemp "${TMPDIR:-/tmp}/afora-openwebui-stats-stop.XXXXXX")"
STATS_PIDS=()
...
docker_e2e_docker_cmd run -d \
  ...
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "AFORA_GATEWAY_TOKEN=$TOKEN" \
  -e "AFORA_OPENWEBUI_MODEL=$MODEL" \
  -e "AFORA_SKIP_CHANNELS=1" \
  -e "AFORA_SKIP_GMAIL_WATCHER=1" \
  -e "AFORA_SKIP_CRON=1" \
  -e "AFORA_SKIP_CANVAS_HOST=1" \
  -e "AFORA_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=$PROVIDER_TIMEOUT_SECONDS" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/afora-openwebui-config.XXXXXX.json)"
    AFORA_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    ...
    afora_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null
start_openwebui_stats_sampler

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "AFORA_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Afora E2E" \
  -e WEBUI_SECRET_KEY="afora-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "AFORA_HTTP_PROBE_BEARER='$TOKEN' AFORA_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> Afora smoke..."
set +e
