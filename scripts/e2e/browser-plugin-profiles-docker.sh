...
BASE_IMAGE="$(docker_e2e_resolve_image "afora-browser-plugin-profiles-base-e2e" AFORA_BROWSER_PLUGIN_PROFILES_BASE_IMAGE)"
IMAGE_NAME="${AFORA_BROWSER_PLUGIN_PROFILES_IMAGE:-afora-browser-plugin-profiles-e2e:${AFORA_DOCKER_ALL_LANE_NAME:-local}}"
CONTAINER_NAME="afora-browser-plugin-profiles-e2e-$$"
BUILD_DIR=""
PORT="18789"
TOKEN="browser-plugin-profiles-token"
PROFILE="qa-browser"
DOCKER_COMMAND_TIMEOUT="${AFORA_BROWSER_PLUGIN_PROFILES_DOCKER_TIMEOUT:-1200s}"
...
if [ "${AFORA_BROWSER_PLUGIN_PROFILES_SKIP_BUILD:-0}" = "1" ]; then
  docker_e2e_docker_cmd image inspect "$IMAGE_NAME" >/dev/null
else
  docker_e2e_build_or_reuse "$BASE_IMAGE" browser-plugin-profiles-base
  BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/afora-browser-plugin-profiles-build.XXXXXX")"
...
AFORA_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 browser-plugin-profiles empty)"
...
afora_e2e_eval_test_state_from_b64 \"\${AFORA_TEST_STATE_SCRIPT_B64:?missing test state}\"
afora_e2e_write_state_env
source /tmp/afora-test-state-env
test -z \"\${AFORA_EAGER_BROWSER_CONTROL_SERVER:-}\"
...
afora_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/browser-plugin-profiles-gateway.log" >/dev/null

if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 240 0.5 \
  "source scripts/lib/afora-e2e-instance.sh; afora_e2e_probe_tcp 127.0.0.1 $PORT"; then
...
if [ -z "$BUILD_DIR" ]; then
  BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/afora-browser-plugin-profiles-run.XXXXXX")"
fi
...
for _ in $(seq 1 80); do
  if ! kill -0 "$BROWSER_PID" 2>/dev/null &&
    ! afora_e2e_probe_tcp 127.0.0.1 "$CDP_PORT" 2>/dev/null; then
    break
  fi
  sleep 0.25
done
! kill -0 "$BROWSER_PID" 2>/dev/null
! afora_e2e_probe_tcp 127.0.0.1 "$CDP_PORT" 2>/dev/null
