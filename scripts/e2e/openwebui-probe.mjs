...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.AFORA_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    lastModelsError = `missing afora model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `afora model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
