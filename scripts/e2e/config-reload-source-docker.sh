...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-config-reload-e2e" AFORA_CONFIG_RELOAD_E2E_IMAGE)"
SKIP_BUILD="${AFORA_CONFIG_RELOAD_E2E_SKIP_BUILD:-0}"
PORT="18789"
TOKEN="reload-e2e-token"
CONTAINER_NAME="afora-config-reload-e2e-$$"
...
AFORA_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 config-reload empty)"
...
afora_e2e_exec_gateway \"\$entry\" $PORT loopback /tmp/config-reload-e2e.log" >/dev/null

echo "Waiting for gateway..."
if ! docker_e2e_wait_container_bash "$CONTAINER_NAME" 180 0.5 "source scripts/lib/afora-e2e-instance.sh; afora_e2e_probe_tcp 127.0.0.1 $PORT"; then
...
docker_e2e_docker_cmd exec "$CONTAINER_NAME" bash -lc "source /tmp/afora-test-state-env
