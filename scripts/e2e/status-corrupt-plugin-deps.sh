...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/afora-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.AFORA_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".afora-rename-tmp");
...
run_afora status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run afora doctor --fix" "$BEFORE" >/dev/null

run_afora doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_afora status --all --timeout 1 > "$AFTER"
