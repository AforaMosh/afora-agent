...
import { asRecord, isRecord } from "@afora/normalization-core/record-coerce";
...
const PLUGIN_SPEC =
  process.env.AFORA_KITCHEN_SINK_NPM_SPEC || "npm:@afora/kitchen-sink@latest";
const PLUGIN_ID = process.env.AFORA_KITCHEN_SINK_PLUGIN_ID || "afora-kitchen-sink-fixture";
...
Runs the external Kitchen Sink plugin RPC walk against a built Afora entry.

Environment:
  AFORA_ENTRY                         Built Afora entrypoint. Defaults to dist/index.mjs or dist/index.js.
  AFORA_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@afora/kitchen-sink@latest.
  AFORA_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: afora-kitchen-sink-fixture.
  AFORA_KITCHEN_SINK_PERSONALITY      Plugin fixture personality. Default: conformance.
  AFORA_KITCHEN_SINK_RPC_PORT         Gateway loopback port. Default: OS-selected free port.
  AFORA_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  AFORA_KITCHEN_SINK_RPC_COMMAND_MS   Afora command timeout.
  AFORA_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  AFORA_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  AFORA_KITCHEN_SINK_RPC_FETCH_MS     HTTP readiness probe timeout.
  AFORA_KITCHEN_SINK_RPC_FETCH_BODY_BYTES  HTTP readiness probe response ceiling.
  AFORA_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  AFORA_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  AFORA_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS  Per-command stdout/stderr capture ceiling.
  AFORA_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function resolveKitchenSinkRpcConfig(env: ProcessEnv = process.env) {
  const commandTimeoutMs = readPositiveTimerMs(
    env.AFORA_KITCHEN_SINK_RPC_COMMAND_MS,
    DEFAULT_COMMAND_TIMEOUT_MS,
    "AFORA_KITCHEN_SINK_RPC_COMMAND_MS",
  );
  return {
    commandMaxRssMiB: readPositiveInt(
      env.AFORA_KITCHEN_SINK_COMMAND_MAX_RSS_MIB,
      DEFAULT_MAX_COMMAND_RSS_MIB,
      "AFORA_KITCHEN_SINK_COMMAND_MAX_RSS_MIB",
    ),
    commandTimeoutMs,
    fetchBodyMaxBytes: readPositiveInt(
      env.AFORA_KITCHEN_SINK_RPC_FETCH_BODY_BYTES,
      DEFAULT_FETCH_BODY_MAX_BYTES,
      "AFORA_KITCHEN_SINK_RPC_FETCH_BODY_BYTES",
    ),
    fetchTimeoutMs: readPositiveTimerMs(
      env.AFORA_KITCHEN_SINK_RPC_FETCH_MS,
      DEFAULT_FETCH_TIMEOUT_MS,
      "AFORA_KITCHEN_SINK_RPC_FETCH_MS",
    ),
    installTimeoutMs: readPositiveTimerMs(
      env.AFORA_KITCHEN_SINK_RPC_INSTALL_MS,
      Math.max(commandTimeoutMs, DEFAULT_INSTALL_TIMEOUT_MS),
      "AFORA_KITCHEN_SINK_RPC_INSTALL_MS",
    ),
    maxRssMiB: readPositiveInt(
      env.AFORA_KITCHEN_SINK_MAX_RSS_MIB,
      DEFAULT_MAX_RSS_MIB,
      "AFORA_KITCHEN_SINK_MAX_RSS_MIB",
    ),
    outputCaptureChars: readPositiveInt(
      env.AFORA_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS,
      DEFAULT_OUTPUT_CAPTURE_CHARS,
      "AFORA_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS",
    ),
    readyTimeoutMs: readPositiveTimerMs(
      env.AFORA_KITCHEN_SINK_RPC_READY_MS,
      DEFAULT_READY_TIMEOUT_MS,
      "AFORA_KITCHEN_SINK_RPC_READY_MS",
    ),
    rpcTimeoutMs: readPositiveTimerMs(
      env.AFORA_KITCHEN_SINK_RPC_CALL_MS,
      DEFAULT_RPC_TIMEOUT_MS,
      "AFORA_KITCHEN_SINK_RPC_CALL_MS",
    ),
  };
}
...
export async function resolveKitchenSinkRpcPort(
  ...
  const rawPort = (env.AFORA_KITCHEN_SINK_RPC_PORT || "").trim();
  if (rawPort) {
    const port = readPositiveInt(rawPort, 0, "AFORA_KITCHEN_SINK_RPC_PORT");
    if (port > 65535) {
      throw new Error(
        `AFORA_KITCHEN_SINK_RPC_PORT must be a TCP port from 1 to 65535. Got: ${JSON.stringify(rawPort)}`,
      );
    }
    return port;
  }
...
function resolveAforaRunner(): AforaRunner {
  if (process.env.AFORA_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.AFORA_ENTRY],
      label: process.env.AFORA_ENTRY,
    };
  }
  ...
  return { pnpm: true, baseArgs: ["afora"], label: "pnpm afora" };
}

export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      AFORA_NO_ONBOARD: "1",
      AFORA_SKIP_PROVIDERS: "0",
      AFORA_KITCHEN_SINK_PERSONALITY:
        process.env.AFORA_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
function resolveCommandParentSignalKillGraceMs(env: ProcessEnv) {
  const raw = env.VITEST && env.AFORA_TEST_KITCHEN_SINK_PARENT_SIGNAL_KILL_GRACE_MS;
  if (!raw) {
...
async function runAfora(
  ...
  const command = await resolveAforaCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveAforaCommand(
  runner: AforaRunner,
  args: string[],
  env: ProcessEnv,
...
async function rpcCall(method: string, params: unknown, options: RpcCallOptions) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.AFORA_CONFIG_PATH),
        configPath: options.env.AFORA_CONFIG_PATH,
...
async function loadCallGatewayModule(runner: AforaRunner) {
  if (!usesBuiltAforaEntry(runner)) {
    return null;
  }
...
export function usesBuiltAforaEntry(
  runner: AforaRunner,
  cwd = process.cwd(),
  env: ProcessEnv = process.env,
) {
  ...
  if (env.AFORA_ENTRY && entry === env.AFORA_ENTRY) {
    return true;
  }
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.AFORA_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function startGateway(
  ...
  const command = await resolveAforaCommand(
    runner,
...
async function samplePosixProcessTree(
  ...
  const gatewayTitleMatches = descendants.filter((row) =>
    row.command.toLowerCase().includes("afora-gateway"),
  );
...
async function main() {
  const config = resolveKitchenSinkRpcConfig();
  let runner = resolveAforaRunner();
  const port = await resolveKitchenSinkRpcPort();
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.AFORA_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runAfora(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,
    ...
    assertKitchenSinkUiDescriptors(uiDescriptors, {
      expectDescriptor: env.AFORA_KITCHEN_SINK_PERSONALITY !== "conformance",
    });
