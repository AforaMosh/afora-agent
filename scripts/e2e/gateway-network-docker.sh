...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-gateway-network-e2e" AFORA_GATEWAY_NETWORK_E2E_IMAGE)"
SKIP_BUILD="${AFORA_GATEWAY_NETWORK_E2E_SKIP_BUILD:-0}"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="afora-net-e2e-$$"
GW_NAME="afora-gateway-e2e-$$"
SUSPENSION_STATE_PATH="/tmp/gateway-network-suspension.json"
DOCKER_COMMAND_TIMEOUT="${AFORA_GATEWAY_NETWORK_DOCKER_COMMAND_TIMEOUT:-600s}"
CLIENT_TIMEOUT="${AFORA_GATEWAY_NETWORK_CLIENT_TIMEOUT:-90s}"
CLIENT_LIMIT_ENV_ARGS=()
if [[ -n "${AFORA_GATEWAY_NETWORK_CLIENT_CONNECT_TIMEOUT_MS+x}" ]]; then
...
elif [[ -n "${AFORA_GATEWAY_NETWORK_CONNECT_READY_TIMEOUT_MS+x}" ]]; then
...
docker_e2e_docker_cmd run -d \
  ...
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail; source scripts/lib/afora-e2e-instance.sh; entry=\"\$(afora_e2e_resolve_entrypoint)\"; node \"\$entry\" config set gateway.controlUi.enabled false >/dev/null; if [[ ! -f /tmp/gateway-network-configured ]]; then node \"\$entry\" plugins enable admin-http-rpc >/dev/null; touch /tmp/gateway-network-configured; fi; afora_e2e_exec_gateway \"\$entry\" $PORT lan /tmp/gateway-net-e2e.log" >/dev/null

echo "Waiting for gateway to come up..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 180 0.5 "source scripts/lib/afora-e2e-instance.sh; afora_e2e_probe_tcp 127.0.0.1 $PORT"; then
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 180 0.5 "source scripts/lib/afora-e2e-instance.sh; afora_e2e_probe_http http://127.0.0.1:$PORT/readyz ok 400"; then
