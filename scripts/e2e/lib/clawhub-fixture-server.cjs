...
const packageName = "@afora/kitchen-sink";
  ...
  const artifacts = new Map(
    manifest.packages.map((entry) => {
      ...
      const packedPlugin = JSON.parse(
        execFileSync("tar", ["-xOf", tarballPath, "package/afora.plugin.json"], {
          encoding: "utf8",
        }),
      );
      if (
...
async function buildNpmPackArtifact(fixture) {
  ...
  const packRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "afora-clawhub-fixture-"));
  try {
...
`,
    ...
    packageDetail(artifact) {
      ...
      const packageDetail = {
        package: {
          name: packageName,
          displayName: "Afora Kitchen Sink",
          family: "code-plugin",
          runtimeId: pluginId,
          channel: "official",
          isOfficial: true,
          ...
          verification: {
            tier: "source-linked",
            sourceRepo: "https://github.com/afora/kitchen-sink",
            hasProvenance: false,
            scanStatus: "passed",
          },
...
async function main() {
  ...
  zip.file("package/afora.plugin.json", manifestJson, { date: new Date(0) });
  ...
  const artifactResolverDetail = {
    package: versionDetail.package ?? {
      name: packageName,
      displayName: packageDetail.package?.displayName ?? "Afora Kitchen Sink",
