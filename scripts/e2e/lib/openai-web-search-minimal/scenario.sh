...
dump_debug_logs() {
  ...
  for file in \
    ...
    "$CLIENT_REJECT_LOG" \
    "$MOCK_REQUEST_LOG" \
    "$AFORA_STATE_DIR/afora.json"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
      AFORA_E2E_LOG_TAIL_BYTES=8192 afora_e2e_print_log "$file" >&2
    fi
  done
...
PORT="$PORT" AFORA_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs success >"$CLIENT_SUCCESS_LOG" 2>&1

node scripts/e2e/lib/openai-web-search-minimal/assertions.mjs assert-success-request "$MOCK_REQUEST_LOG"

PORT="$PORT" AFORA_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs reject >"$CLIENT_REJECT_LOG" 2>&1
