...
AFORA_TEST_STATE_SCRIPT_B64="${AFORA_TEST_STATE_SCRIPT_B64:-}"
afora_skill_install_owns_home=0
cleanup_clawhub_skill_install_home() {
  if [ "$afora_skill_install_owns_home" = "1" ] && [ -n "${HOME:-}" ]; then
    rm -rf "$HOME"
  fi
}
trap cleanup_clawhub_skill_install_home EXIT

if [ -n "$AFORA_TEST_STATE_SCRIPT_B64" ]; then
  afora_e2e_eval_test_state_from_b64 "$AFORA_TEST_STATE_SCRIPT_B64"
else
  export HOME="$(mktemp -d "${TMPDIR:-/tmp}/afora-skill-install-home.XXXXXX")"
  afora_skill_install_owns_home=1
  export USERPROFILE="$HOME"
  export AFORA_HOME="$HOME"
...
if [ -n "${AFORA_CURRENT_PACKAGE_TGZ:-}" ]; then
  ...
  afora_e2e_install_package /tmp/afora-skill-install-npm.log
fi

if [ -n "${AFORA_CURRENT_PACKAGE_TGZ:-}" ] && command -v afora >/dev/null 2>&1; then
  AFORA_CMD=(afora)
elif command -v pnpm >/dev/null 2>&1 && [ -f package.json ]; then
  if [ "${AFORA_SKILL_INSTALL_E2E_BUILD_SOURCE:-0}" = "1" ]; then
    pnpm build >/tmp/afora-skill-install-build.log 2>&1
  fi
  AFORA_CMD=(pnpm --silent afora)
elif command -v afora >/dev/null 2>&1; then
  AFORA_CMD=(afora)
else
  echo "afora command not found; install package first or run from repo with pnpm" >&2
  exit 1
fi

mkdir -p "$(dirname "$AFORA_CONFIG_PATH")"
node --input-type=module - "$AFORA_CONFIG_PATH" <<'NODE'
...
query="${AFORA_SKILL_INSTALL_E2E_QUERY:-homeassistant}"
requested_slug="${AFORA_SKILL_INSTALL_E2E_SLUG:-}"
preferred_slug="${AFORA_SKILL_INSTALL_E2E_PREFERRED_SLUG:-homeassistant-skill}"
search_json="/tmp/afora-skill-install-search.json"
resolve_json="/tmp/afora-skill-install-resolved.json"
install_log="/tmp/afora-skill-install.log"
info_json="/tmp/afora-skill-install-info.json"

echo "Searching live ClawHub skills for: $query"
"${AFORA_CMD[@]}" skills search "$query" --limit 8 --json >"$search_json"
...
if ! "${AFORA_CMD[@]}" skills install "$slug" --force >"$install_log" 2>&1; then
  echo "Skill install failed" >&2
  afora_e2e_dump_logs /tmp/afora-skill-install-npm.log "$search_json" "$resolve_json" "$install_log"
  exit 1
fi

workspace_dir="$HOME/.afora/workspace"
skill_dir="$workspace_dir/skills/$slug"
origin_json="$skill_dir/.clawhub/origin.json"
lock_json="$workspace_dir/.clawhub/lock.json"

afora_e2e_assert_file "$skill_dir/SKILL.md"
afora_e2e_assert_file "$origin_json"
afora_e2e_assert_file "$lock_json"

"${AFORA_CMD[@]}" skills info "$slug" --json >"$info_json"

node --input-type=module - "$AFORA_CONFIG_PATH" "$skill_dir" "$origin_json" "$lock_json" "$info_json" "$slug" <<'NODE'
import fs from "node:fs";
