...
AFORA_ONBOARD_SCENARIO_SOURCE_ONLY="${AFORA_ONBOARD_SCENARIO_SOURCE_ONLY:-0}"
if [ "$AFORA_ONBOARD_SCENARIO_SOURCE_ONLY" != "1" ]; then
...
if [ -z "${AFORA_ENTRY:-}" ] && [ "$AFORA_ONBOARD_SCENARIO_SOURCE_ONLY" != "1" ]; then
  AFORA_ENTRY="$(afora_e2e_resolve_entrypoint)"
fi
export AFORA_ENTRY
ONBOARD_TMP_ROOT="${AFORA_ONBOARD_E2E_TMPDIR:-${TMPDIR:-/tmp}}"
ONBOARD_TMP_ROOT="${ONBOARD_TMP_ROOT%/}"
[ -n "$ONBOARD_TMP_ROOT" ] || ONBOARD_TMP_ROOT="/tmp"
mkdir -p "$ONBOARD_TMP_ROOT"
ONBOARD_TMP_DIR="$(mktemp -d "$ONBOARD_TMP_ROOT/afora-onboard.XXXXXX")"
AFORA_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"
GATEWAY_LOG_PATH="$ONBOARD_TMP_DIR/gateway-e2e.log"
export AFORA_E2E_LOG_DIR
export GATEWAY_LOG_PATH
mkdir -p "$AFORA_E2E_LOG_DIR"
cleanup_onboard_artifacts() {
  afora_e2e_stop_process "${GATEWAY_PID:-}"
  afora_e2e_stop_process "${mock_openai_pid:-}"
...
start_gateway() {
  GATEWAY_PID="$(afora_e2e_start_gateway "$AFORA_ENTRY" 18789 "$GATEWAY_LOG_PATH")"
}

wait_for_gateway() {
  local wait_attempts
  wait_attempts="$(afora_e2e_read_positive_int_env AFORA_ONBOARD_GATEWAY_WAIT_ATTEMPTS 20)" || return $?
  local wait_interval_s="${AFORA_ONBOARD_GATEWAY_WAIT_INTERVAL_S:-1}"
  local saw_listening_log="false"
  for _ in $(seq 1 "$wait_attempts"); do
    if afora_e2e_probe_tcp 127.0.0.1 18789 500 >/dev/null 2>&1; then
      return 0
    fi
...
cleanup_wizard_case() {
  ...
  afora_e2e_stop_process "${wizard_pid:-}"
  stop_gateway "${gw_pid:-}"
...
run_wizard_cmd() {
  ...
  set_isolated_afora_env "$state_ref"
  ...
  local log_path="$AFORA_E2E_LOG_DIR/${case_name}.log"
  WIZARD_LOG_PATH="$log_path"
  export WIZARD_LOG_PATH
  # Run under script to keep an interactive TTY for clack prompts.
  afora_e2e_run_script_with_pty "$command" "$log_path" <"$input_fifo" >/dev/null 2>&1 &
  wizard_pid=$!
...
assert_onboard_config() {
  local scenario="$1"
  shift
  afora_e2e_assert_file "$AFORA_CONFIG_PATH"
  node scripts/e2e/lib/onboard/assert-config.mjs "$scenario" "$AFORA_CONFIG_PATH" "$@"
}

set_isolated_afora_env() {
  local state_ref="$1"
  afora_test_state_create "$state_ref" empty
}
...
validate_guided_skip_ui_log() {
  ...
  log_contains "Hi — I'm Afora. I keep this system running. Let's get you set up." || {
    echo "Guided onboarding introduction was not rendered"
    return 1
  }
  log_contains "Afora is ready." || {
...
run_case_guided_skip_ui() {
  ...
  set_isolated_afora_env guided-skip-ui
  export OPENAI_API_KEY="sk-afora-guided-skip-ui-e2e"
  ...
  run_wizard_cmd \
    guided-skip-ui \
    "$HOME" \
    "node \"$AFORA_ENTRY\" onboard --skip-ui" \
    send_guided_skip_ui_flow
...
run_case_local_basic() {
  set_isolated_afora_env local-basic
  afora_e2e_run_logged local-basic node "$AFORA_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
  ...
  validate_local_basic_log "$AFORA_E2E_LAST_LOG_PATH"

  # Assert config + workspace scaffolding.
  workspace_dir="$AFORA_STATE_DIR/workspace"
  sessions_dir="$AFORA_STATE_DIR/agents/main/sessions"

  afora_e2e_assert_dir "$sessions_dir"
  for file in AGENTS.md BOOTSTRAP.md IDENTITY.md SOUL.md USER.md; do
    afora_e2e_assert_file "$workspace_dir/$file"
  done
...
run_case_local_auth_refs() {
  set_isolated_afora_env local-auth-refs
  export OPENAI_API_KEY="sk-afora-onboard-auth-ref-e2e"
  export AFORA_GATEWAY_TOKEN="afora-onboard-gateway-ref-e2e"

  afora_e2e_run_logged local-auth-refs node "$AFORA_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
...
run_case_local_password() {
  set_isolated_afora_env local-password

  afora_e2e_run_logged local-password node "$AFORA_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
    ...
    --gateway-password "afora-onboard-password-e2e" \
    --skip-channels \
    --skip-skills \
    --skip-daemon \
    --skip-ui \
...
run_case_remote_non_interactive() {
  set_isolated_afora_env remote-non-interactive
  # Smoke test non-interactive remote config write.
  afora_e2e_run_logged remote-non-interactive node "$AFORA_ENTRY" onboard --non-interactive --accept-risk \
    --mode remote \
...
run_case_reset() {
  set_isolated_afora_env reset-config
  node scripts/e2e/lib/onboard/write-config.mjs reset "$AFORA_CONFIG_PATH"

  afora_e2e_run_logged reset-config node "$AFORA_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
...
run_case_channels() {
  ...
  run_wizard_cmd channels channels "node \"$AFORA_ENTRY\" configure --section channels" send_channels_flow
...
run_case_skills() {
  local home_dir
  set_isolated_afora_env skills
  home_dir="$HOME"
  node scripts/e2e/lib/onboard/write-config.mjs skills "$AFORA_CONFIG_PATH"

  run_wizard_cmd skills "$home_dir" "node \"$AFORA_ENTRY\" configure --section skills" send_skills_flow
...
validate_local_basic_log() {
  local log_path="$1"
  afora_e2e_assert_log_not_contains "$log_path" "systemctl --user unavailable"
}

run_selected_cases() {
  local selected_cases="${AFORA_ONBOARD_E2E_CASES:-guided-skip-ui,local-basic,remote-non-interactive,reset,channels,skills}"
  local case_name
  local -a cases=()
