...
afora_e2e_install_package /tmp/afora-install.log "mounted Afora package" /tmp/npm-prefix
...
AFORA_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(node "$probe" legacy-compat "$package_version")"
export AFORA_PACKAGE_ACCEPTANCE_LEGACY_COMPAT
export PATH="/tmp/npm-prefix/bin:$PATH"

node "$probe" seed

registry_port_file=/tmp/afora-e2e-registry.port
rm -f "$registry_port_file"
node scripts/e2e/lib/plugin-update/registry-server.mjs "$registry_port_file" >/tmp/afora-e2e-registry.log 2>&1 &
registry_pid=$!
trap 'afora_e2e_stop_process "${registry_pid:-}"' EXIT
...
if [ ! -s "$registry_port_file" ]; then
  ...
  afora_e2e_print_log /tmp/afora-e2e-registry.log
  exit 1
fi
...
before_config_hash=""
if [ "$AFORA_PACKAGE_ACCEPTANCE_LEGACY_COMPAT" != "1" ]; then
  before_config_hash="$(sha256sum "$AFORA_CONFIG_PATH" | awk '{print $1}')"
fi
plugin_update_timeout_seconds="$(afora_e2e_read_positive_int_env AFORA_PLUGIN_UPDATE_TIMEOUT_SECONDS 180)"

node "$probe" snapshot > /tmp/plugin-update-before.json

set +e
afora_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update @example/lossless-claw > /tmp/plugin-update-output.log 2>&1
...
if [ "$plugin_update_status" -ne 0 ]; then
  ...
  afora_e2e_print_log /tmp/plugin-update-output.log
...
if [ -n "$before_config_hash" ]; then
  after_config_hash="$(sha256sum "$AFORA_CONFIG_PATH" | awk '{print $1}')"
