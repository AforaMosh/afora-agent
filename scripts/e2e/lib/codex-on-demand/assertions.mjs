...
const onboard = readJson("/tmp/afora-onboard.json");
const inspect = readJson("/tmp/afora-codex-inspect.json");
...
if (!codexRecord.spec?.includes("@afora/codex")) {
  throw new Error(`expected @afora/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @afora/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@afora/codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@afora/codex");
...
function readAuthProfileStoreText(agentDir) {
  const dbPath = path.join(agentDir, "afora-agent.sqlite");
...
assertOpenAiEnvAuthProfileStore(authRaw, {
  ...
  rawKeyNeedle: "sk-afora-codex-on-demand-e2e",
});
