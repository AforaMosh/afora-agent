...
function assertConfigChannel(channel) {
  const config = readJson(path.join(process.env.HOME, ".afora", "afora.json"));
  if (config.update?.channel === channel) {
    return;
  }
  if (process.env.AFORA_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1") {
    console.log(
...
function assertInstalledVersion(root, expectedVersion) {
  ...
  if (manifest.version !== expectedVersion) {
    throw new Error(
      `expected installed afora ${expectedVersion}, got ${String(manifest.version)}`,
    );
  }
}
...
switch (command) {
  case "prepare-git-fixture":
    prepareGitFixture(args[0] ?? "/tmp/afora-git");
    break;
  case "write-control-ui":
    writeControlUi(args[0] ?? "/tmp/afora-git");
    break;
  case "assert-update":
    assertUpdate(args[0]);
    break;
