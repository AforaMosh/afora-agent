...
export OPENAI_API_KEY="sk-afora-release-upgrade-user-journey"
export CLICKCLACK_BOT_TOKEN="clickclack-release-upgrade-token"

PORT="18789"
MOCK_PORT="44210"
CLICKCLACK_PORT="44211"
SUCCESS_MARKER="AFORA_E2E_OK_RELEASE_UPGRADE"
...
candidate_version="$(
  tar -xOf "${AFORA_CURRENT_PACKAGE_TGZ:?missing AFORA_CURRENT_PACKAGE_TGZ}" package/package.json |
...
if [ -n "${AFORA_RELEASE_UPGRADE_BASELINE_SPEC:-}" ]; then
  BASELINE_SPEC="$AFORA_RELEASE_UPGRADE_BASELINE_SPEC"
else
  BASELINE_SPEC="$(
...
cleanup() {
  ...
  afora_e2e_stop_process "${clickclack_pid:-}"
...
start_gateway() {
  local log_path="$1"
  gateway_pid="$(afora_e2e_start_gateway "$entry" "$PORT" "$log_path")"
  afora_e2e_wait_gateway_ready "$gateway_pid" "$log_path" 300 "$PORT"
}

echo "Installing published baseline $BASELINE_SPEC..."
if ! afora_e2e_maybe_timeout "${AFORA_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v afora >/dev/null
baseline_root="$(afora_e2e_package_root)"
baseline_entry="$(afora_e2e_package_entrypoint "$baseline_root")"
afora_e2e_enable_afora_cli_timeout

mock_pid="$(afora_e2e_start_mock_openai "$MOCK_PORT" "$OPENAI_LOG")"
...
for _ in $(seq 1 100); do
  if afora_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200 >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
afora_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200
...
afora plugins install "$plugin_dir" --force >"$PLUGIN_INSTALL_LOG" 2>&1
afora release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
afora_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate Afora package"
...
afora release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1
...
afora plugins install "$clickclack_plugin_dir" --force >"$CLICKCLACK_PLUGIN_INSTALL_LOG" 2>&1

afora channels status --json >"$STATUS_JSON" 2>"$STATUS_ERR"
