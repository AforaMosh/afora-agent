...
ARTIFACT_ROOT="$(dirname "${AFORA_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/afora-upgrade-survivor-artifacts/summary.json}")"
export AFORA_UPGRADE_SURVIVOR_RUNTIME_ROOT="${AFORA_UPGRADE_SURVIVOR_RUNTIME_ROOT:-/tmp/afora-upgrade-survivor-runtime}"
RUNTIME_ROOT="$AFORA_UPGRADE_SURVIVOR_RUNTIME_ROOT"
STATE_HOME_ROOT="${AFORA_UPGRADE_SURVIVOR_STATE_HOME_ROOT:-$RUNTIME_ROOT/state-home}"
mkdir -p "$ARTIFACT_ROOT"
mkdir -p "$RUNTIME_ROOT"
export TMPDIR="${AFORA_UPGRADE_SURVIVOR_TMPDIR:-$RUNTIME_ROOT/tmp}"
export AFORA_TEST_STATE_TMPDIR="${AFORA_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR:-$RUNTIME_ROOT/state-tmp}"
mkdir -p "$TMPDIR" "$AFORA_TEST_STATE_TMPDIR"
...
SUMMARY_JSON="${AFORA_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${AFORA_UPGRADE_SURVIVOR_BASELINE:?missing AFORA_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${AFORA_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${AFORA_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${AFORA_CURRENT_PACKAGE_TGZ:-}}"
...
PREPUBLISH_AUTHORED_CONFIG="$RUNTIME_ROOT/prepublish-authored-afora.json"
export AFORA_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
...
validate_baseline_package_spec() {
  ...
  echo "AFORA_UPGRADE_SURVIVOR_BASELINE must be afora-agent@latest, afora@beta, afora@alpha, an exact Afora release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "AFORA_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    afora@*)
      baseline_spec="$raw"
      baseline_version="${raw#afora@}"
      ;;
    *@*)
      echo "AFORA_UPGRADE_SURVIVOR_BASELINE must be afora@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="afora@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "AFORA_UPGRADE_SURVIVOR_BASELINE must be afora-agent@latest, afora@beta, afora@alpha, afora@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "AFORA_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.AFORA_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.AFORA_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.AFORA_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      afora_e2e_terminate_gateways "$shim_pid"
    fi
  fi
...
legacy_runtime_deps_symlink_plugin() {
  local plugin="${AFORA_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "AFORA_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@afora-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${AFORA_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$AFORA_STATE_DIR/extensions/opik-afora"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-afora",
...
JSON
  cat >"$shadow_root/afora.plugin.json" <<'JSON'
{
  "id": "opik-afora",
  "activation": {
    "onStartup": false
...
configure_clawhub_fixture() {
  ...
  node "${AFORA_UPGRADE_SURVIVOR_CLAWHUB_FIXTURE_SERVER:-scripts/e2e/lib/clawhub-fixture-server.cjs}" \
...
prepublish_auto_auth_enabled() {
  [ "$UPDATE_RESTART_MODE" = "auto-auth" ] &&
    [ -n "${AFORA_PREPUBLISH_PLUGIN_REGISTRY_DIR:-}" ]
}

park_prepublish_authored_config() {
  prepublish_auto_auth_enabled || return 0
  node "${AFORA_UPGRADE_SURVIVOR_CLAWHUB_FIXTURE_SERVER:-scripts/e2e/lib/clawhub-fixture-server.cjs}" \
    park-prepublish-auth-config "$AFORA_CONFIG_PATH" "$PREPUBLISH_AUTHORED_CONFIG"
}

assert_prepublish_fixture_idle() {
  prepublish_auto_auth_enabled || return 0
  node "${AFORA_UPGRADE_SURVIVOR_CLAWHUB_FIXTURE_SERVER:-scripts/e2e/lib/clawhub-fixture-server.cjs}" \
    assert-no-requests "$AFORA_CLAWHUB_URL"
}

restore_prepublish_authored_config() {
  prepublish_auto_auth_enabled || return 0
  if ! node "${AFORA_UPGRADE_SURVIVOR_CLAWHUB_FIXTURE_SERVER:-scripts/e2e/lib/clawhub-fixture-server.cjs}" \
    restore-prepublish-auth-config "$AFORA_CONFIG_PATH" "$PREPUBLISH_AUTHORED_CONFIG"; then
    return 1
  fi
  if ! cmp -s "$PREPUBLISH_AUTHORED_CONFIG" "$AFORA_CONFIG_PATH"; then
...
NODE
  ...
  mkdir -p "$fixture_root" && rm -f "$port_file"
  AFORA_NPM_REGISTRY_DIST_TAGS="beta=$candidate_version" \
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.afora-runtime-deps.json" \
      "$plugin_dir/.afora-runtime-deps-stamp.json" \
      "$plugin_dir/.afora-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.afora-install-stage-upgrade-survivor" \
      "$plugin_dir/.afora-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$AFORA_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$AFORA_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/afora-upgrade-survivor-dep" \
      "$plugin_dir/.afora-runtime-deps-copy-upgrade-survivor/node_modules/afora-upgrade-survivor-dep" \
      "$plugin_dir/.afora-install-stage-upgrade-survivor" \
      "$plugin_dir/.afora-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/afora-upgrade-survivor-dep" \
      "$AFORA_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/afora-upgrade-survivor-dep" \
      "$AFORA_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/afora-upgrade-survivor-dep"
    printf '{"name":"afora-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/afora-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.afora-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.afora-runtime-deps-stamp.json"
    printf '{"name":"afora-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.afora-runtime-deps-copy-upgrade-survivor/node_modules/afora-upgrade-survivor-dep/package.json"
    printf '{"name":"afora-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/afora-upgrade-survivor-dep/package.json"
    printf '{"name":"afora-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$AFORA_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/afora-upgrade-survivor-dep/package.json"
    printf '{"name":"afora-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$AFORA_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/afora-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"afora-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
reset_run_state() {
  rm_rf_retry "$npm_config_prefix" "$TMPDIR" "$AFORA_TEST_STATE_TMPDIR" "$STATE_HOME_ROOT"
  rm -f "$SYSTEMCTL_SHIM_PID_FILE" "$SYSTEMCTL_SHIM_DAEMON_LOG"
  mkdir -p "$npm_config_prefix" "$npm_config_cache" "$TMPDIR" "$AFORA_TEST_STATE_TMPDIR"
}

install_baseline() {
  normalize_baseline
  echo "Installing baseline package: $baseline_spec"
  if ! afora_e2e_maybe_timeout "${AFORA_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix "$npm_config_prefix" "$baseline_spec" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
    echo "baseline npm install failed" >&2
    afora_e2e_print_log "$BASELINE_INSTALL_LOG" >&2
    return 1
  fi
  if ! command -v afora >/dev/null; then
    echo "baseline install did not expose afora on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora --version 2>&1)"; then
    echo "baseline afora --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline afora --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  local account_home=""
  afora_e2e_eval_test_state_from_b64 "${AFORA_TEST_STATE_FUNCTION_B64:?missing AFORA_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.afora /root/workspace
    afora_test_state_create /root minimal
  else
    afora_test_state_create "$STATE_HOME_ROOT" minimal
  fi
  ...
  export AFORA_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
  node scripts/e2e/lib/upgrade-survivor/assertions.mjs seed
}

apply_baseline_config_recipe() {
  local tsx_import="${AFORA_UPGRADE_SURVIVOR_TSX_IMPORT:-tsx}"
  local recipe_runner=(
...
validate_baseline_config() {
  if ! afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
    echo "generated baseline config failed baseline validation" >&2
    afora_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG" >&2
    return 1
  fi
}
...
log_file="${AFORA_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/afora-systemctl-shim.log}"
pid_file="${AFORA_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/afora-systemctl-shim.pid}"
daemon_log="${AFORA_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/afora-systemctl-shim-gateway.log}"
...
unit_path() {
  printf '%s/.config/systemd/user/afora-gateway.service\n' "${HOME:?missing HOME}"
}
...
const command = process.env.AFORA_SYSTEMCTL_SHIM_EXEC_START;
const daemonLog = process.env.AFORA_SYSTEMCTL_SHIM_DAEMON_LOG;
...
delete childEnv.AFORA_SYSTEMCTL_SHIM_EXEC_START;
delete childEnv.AFORA_SYSTEMCTL_SHIM_DAEMON_LOG;
...
SUPERVISOR
  (
    ...
    AFORA_SYSTEMCTL_SHIM_EXEC_START="$exec_start" \
      AFORA_SYSTEMCTL_SHIM_DAEMON_LOG="$daemon_log" \
...
install_update_restart_service_unit() {
  if ! afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u AFORA_GATEWAY_TOKEN -u AFORA_GATEWAY_PASSWORD afora gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
    echo "baseline gateway service install failed" >&2
    afora_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR" >&2
    afora_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON" >&2
    return 1
  fi
}
...
if (!stateDir) {
  throw new Error("missing AFORA_STATE_DIR");
}
...
write_update_restart_service_env() {
  ...
  local dotenv_path="$AFORA_STATE_DIR/.env"
  local tmp_path="$dotenv_path.tmp.$$"
  if [ -f "$dotenv_path" ]; then
    grep -Ev '^(GATEWAY_AUTH_TOKEN_REF|AFORA_CLAWHUB_URL)=' "$dotenv_path" >"$tmp_path" || true
  else
    : >"$tmp_path"
  fi
  ...
  if [ -n "${AFORA_CLAWHUB_URL:-}" ]; then
    printf 'AFORA_CLAWHUB_URL=%s\n' "$AFORA_CLAWHUB_URL" >>"$tmp_path"
  fi
  chmod 600 "$tmp_path"
...
assert_baseline_state() {
  AFORA_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing AFORA_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(AFORA_ALLOW_ROOT=1)
  fi
  local update_status=0
  afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" afora "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR" || update_status=$?
  if [ "$update_status" -ne 0 ]; then
    echo "afora update failed" >&2
    local validate_status=0
    afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora config validate --json >"$POST_UPDATE_VALIDATE_JSON" 2>"$POST_UPDATE_VALIDATE_ERR" || validate_status=$?
...
assert_root_managed_vps_cli_usable() {
  ...
  afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" afora config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" afora plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
    echo "afora doctor failed" >&2
    afora_e2e_print_log "$DOCTOR_LOG" >&2
    return 1
  fi
}

validate_post_doctor_config() {
  if ! afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${AFORA_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$AFORA_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
...
start_gateway() {
  ...
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u AFORA_GATEWAY_TOKEN -u AFORA_GATEWAY_PASSWORD afora gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_status() {
  ...
  if ! afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
    echo "gateway status failed" >&2
    afora_e2e_print_log "$STATUS_ERR" >&2
    afora_e2e_print_log "$GATEWAY_LOG" >&2
    return 1
  fi
  ...
  if [ "$status_seconds" -gt "$budget" ]; then
    ...
    afora_e2e_print_log "$STATUS_JSON" >&2
    return 1
  fi
...
if [ -n "${AFORA_CLAWHUB_URL:-}" ]; then
  ...
  prepublish_package="@afora/whatsapp"
  if [ "$SCENARIO" = "configured-plugin-installs" ]; then
    prepublish_package="@afora/matrix"
  fi
  ...
  phase assert-prepublish-requests node \
    "${AFORA_UPGRADE_SURVIVOR_CLAWHUB_FIXTURE_SERVER:-scripts/e2e/lib/clawhub-fixture-server.cjs}" \
    assert-prepublish-requests "$AFORA_CLAWHUB_URL" "$prepublish_package" "$candidate_version" "$clawhub_security_mode"
fi
