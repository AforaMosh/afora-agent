...
if [ -f dist/index.mjs ]; then
  AFORA_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  AFORA_ENTRY="dist/index.js"
else
...
sweep_command_timeout="${AFORA_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT:-300s}"
...
run_logged_sweep_command() {
  ...
  local log_file="$2"
  shift 2
  if afora_e2e_maybe_timeout "$sweep_command_timeout" "$@" >"$log_file" 2>&1; then
    return 0
  else
    local status=$?
...
echo "Selected ${#plugin_entries[@]} bundled plugins for shard ${AFORA_BUNDLED_PLUGIN_SWEEP_INDEX:-0}/${AFORA_BUNDLED_PLUGIN_SWEEP_TOTAL:-1}: ${selected_labels[*]}"

plugin_index=0
for plugin_entry in "${plugin_entries[@]}"; do
  IFS=$'\t' read -r plugin_id plugin_dir requires_config plugin_root <<<"$plugin_entry"
  install_log="/tmp/afora-install-${plugin_index}.log"
  uninstall_log="/tmp/afora-uninstall-${plugin_index}.log"
  ...
  if [[ "${AFORA_BUNDLED_PLUGIN_RUNTIME_SMOKE:-1}" != "0" ]]; then
    ...
    if [[ "$plugin_id" == "${AFORA_BUNDLED_PLUGIN_TTS_LIVE_PROVIDER:-openai}" ]]; then
