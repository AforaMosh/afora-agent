...
function resolveStateDir() {
  if (process.env.AFORA_STATE_DIR) {
    return process.env.AFORA_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".afora");
}
...
function resolveAforaEntry() {
  if (process.env.AFORA_ENTRY) {
    return process.env.AFORA_ENTRY;
  }
  ...
  throw new Error("Missing AFORA_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveAforaEntry();
...
async function loadManifestEntries() {
  const explicit = (process.env.AFORA_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `AFORA_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("AFORA_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("AFORA_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `AFORA_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.AFORA_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
