...
export OPENAI_API_KEY="sk-afora-release-user-journey"
export AFORA_GATEWAY_TOKEN="release-user-journey-token"
export CLICKCLACK_BOT_TOKEN="clickclack-release-token"

PORT="18789"
MOCK_PORT="44180"
CLICKCLACK_PORT="44181"
SUCCESS_MARKER="AFORA_E2E_OK_RELEASE_USER_JOURNEY"
...
afora_e2e_install_package "$INSTALL_LOG"
...
afora plugins install "$plugin_a_dir" --force >"$PLUGIN_A_INSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
  ...
  "$plugin_a_dir"
afora journey-a ping >"$PLUGIN_A_CLI_LOG" 2>&1
...
afora plugins uninstall journey-plugin-a --force >"$PLUGIN_A_UNINSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
...
afora plugins install "$plugin_b_dir" --force >"$PLUGIN_B_INSTALL_LOG" 2>&1
afora journey-b ping >"$PLUGIN_B_CLI_LOG" 2>&1
...
afora plugins inspect journey-plugin-b --runtime --json >"$PLUGIN_B_AFTER_RESTART_JSON" 2>&1
afora channels status --json >"$STATUS_AFTER_RESTART_JSON" 2>"$STATUS_AFTER_RESTART_ERR"
...
afora doctor --repair --non-interactive >"$DOCTOR_LOG" 2>&1
