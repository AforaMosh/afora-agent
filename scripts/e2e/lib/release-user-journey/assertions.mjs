...
function clickClackHttpTimeoutMs() {
  return readPositiveInt(
    process.env.AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS,
    5000,
    "AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS",
  );
}

function clickClackHttpBodyMaxBytes() {
  return readPositiveInt(
    process.env.AFORA_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES,
    1024 * 1024,
...
function configPath() {
  return (
    ...
    path.join(process.env.HOME ?? "", ".afora", "afora.json")
  );
}
...
function assertOnboard() {
  ...
  assert(fs.existsSync(configPath()), "onboard did not write afora.json");
  const stateRaw =
    fs.readFileSync(configPath(), "utf8") +
    (fs.existsSync(authPath) ? fs.readFileSync(authPath, "utf8") : "");
  assert(
    !stateRaw.includes("sk-afora-release-user-journey"),
