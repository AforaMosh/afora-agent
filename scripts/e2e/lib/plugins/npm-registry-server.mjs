...
function normalizeUpstreamRegistry(raw) {
  ...
  if (
    ...
    throw new Error("AFORA_NPM_REGISTRY_UPSTREAM must be an HTTP(S) origin");
  }
  return url.origin;
}

const upstreamRegistry = normalizeUpstreamRegistry(process.env.AFORA_NPM_REGISTRY_UPSTREAM);
const distTagOverrides = new Map(
  (process.env.AFORA_NPM_REGISTRY_DIST_TAGS ?? "")
    .split(",")
    ...
    .map((entry) => {
      ...
      if (separator <= 0 || separator === entry.length - 1) {
        throw new Error(
          "AFORA_NPM_REGISTRY_DIST_TAGS must contain comma-separated tag=version entries",
        );
      }
...
function readPackageManifest(tarballPath, packageName) {
  try {
    ...
    return packageName === "@afora/demo-plugin-npm"
...
const bindHost = process.env.AFORA_NPM_REGISTRY_BIND_HOST || "127.0.0.1";
const requestedPort = Number(process.env.AFORA_NPM_REGISTRY_PORT || 0);
