...
function writeConfig(kind) {
  const configPath = requireArg(process.env.AFORA_CONFIG_PATH, "AFORA_CONFIG_PATH");
      ...
      : kind === "browser-cdp"
        ? {
            gateway: {
              ...
              auth: {
                mode: "token",
                token: requireArg(process.env.AFORA_GATEWAY_TOKEN, "AFORA_GATEWAY_TOKEN"),
              },
              controlUi: { enabled: false },
            },
            browser: {
...
function writeOpenAiWebSearchMinimalConfig() {
  writeJson(path.join(process.env.AFORA_STATE_DIR, "afora.json"), {
    agents: {
      defaults: {
    ...
    gateway: { auth: { mode: "token", token: process.env.AFORA_GATEWAY_TOKEN } },
  });
}

function writeOpenWebUiConfig([openaiApiKey]) {
  const batchPath = requireArg(
    process.env.AFORA_CONFIG_BATCH_PATH,
    "AFORA_CONFIG_BATCH_PATH",
  );
  writeJson(batchPath, [
    ...
    { path: "models.providers.openai.models", value: [] },
    {
      ...
      value: readPositiveIntEnv("AFORA_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS", 900),
    },
    { path: "models.providers.openai.agentRuntime", value: { id: "afora" } },
    { path: "gateway.controlUi.enabled", value: false },
    { path: "gateway.mode", value: "local" },
    { path: "gateway.bind", value: "lan" },
    { path: "gateway.auth.mode", value: "token" },
    { path: "gateway.auth.token", value: process.env.AFORA_GATEWAY_TOKEN },
    { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
    { path: "agents.defaults.model.primary", value: process.env.AFORA_OPENWEBUI_MODEL },
  ]);
}
