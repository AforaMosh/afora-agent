...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-cron-cli-e2e" AFORA_IMAGE)"
PORT="18789"
TOKEN="cron-cli-e2e-$(date +%s)-$$"
CONTAINER_NAME="afora-cron-cli-e2e-$$"
CLIENT_LOG="$(mktemp -t afora-cron-cli-log.XXXXXX)"
...
AFORA_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 cron-cli empty)"
...
gateway_pid="$(afora_e2e_start_gateway "$entry" 18789 /tmp/cron-cli-gateway.log)"
afora_e2e_wait_gateway_ready "$gateway_pid" /tmp/cron-cli-gateway.log 300 18789
...
cron_add_args=(
  "cli cron smoke"
  --cron "*/5 * * * *"
  --command "printf afora-cli-cron-ok"
  --no-deliver
  --timeout-seconds 15
  --json
)
...
node --input-type=module -e '
  ...
  const matching = Array.isArray(value.entries)
    ? value.entries.find((entry) => entry.status === "ok" && entry.summary === "afora-cli-cron-ok")
    : undefined;
  if (!matching) {
