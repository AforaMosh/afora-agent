...
# Afora under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `afora update`. Verifies that:
#
...
#
# Requires: Docker, a built afora-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="$(docker_e2e_resolve_image "afora-multi-node-update-e2e" AFORA_MULTI_NODE_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${AFORA_MULTI_NODE_UPDATE_E2E_SKIP_BUILD:-0}"
DOCKER_RUN_TIMEOUT="${AFORA_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${AFORA_CURRENT_PACKAGE_TGZ:-}")"
...
echo ""
echo "── Step 2: Install Afora under node-A ──"

# Use node-A to install afora with npm prefix A.
...
echo "Installing Afora package under node-A prefix: $NPM_PREFIX_A"
afora_e2e_install_package "$ARTIFACTS/install-a.log" "Afora package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking afora location..."

AFORA_A="$(command -v afora)"
echo "afora binary: $AFORA_A"
echo "afora version: $(afora --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/afora"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/afora-gateway.service"
...
if ! afora gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
# but afora is NOT installed there.
...
echo "which afora: $(command -v afora)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run afora update (this is the bug) ──"
...
echo "Running afora update --yes --json..."
UPDATE_EXIT=0
afora update --yes --json \
  --tag /tmp/afora-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: afora update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but Afora package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the Afora package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/afora/package.json" ]; then
  echo "WARNING: Afora was ALSO installed under node-B prefix (split install)"
else
  echo "OK: Afora is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell Afora root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
