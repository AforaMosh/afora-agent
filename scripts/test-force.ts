...
// Test Force script supports Afora repository automation.
...
function usage(): string {
  return [
    ...
    "",
    "Clears the configured Afora gateway port, then runs the local test suite.",
    "",
    "Options:",
...
function runTests() {
  const isolatedLock =
    process.env.AFORA_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `afora-gateway.lock.test.${Date.now()}`);
  const result = spawnSync(process.execPath, ["--import", "tsx", "scripts/test-projects.mts"], {
    stdio: "inherit",
    env: {
      ...process.env,
      AFORA_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
