...
function usage(): string {
  return "Usage: node scripts/check-afora-package-tarball.mjs [--require-bundled-workspace-deps] <afora.tgz>";
}
...
function parseArgs(argv: string[]) {
  ...
  for (const rawArg of args) {
    ...
    if (arg.startsWith("-")) {
      throw new Error(`Unknown Afora package tarball check option: ${arg}`);
    }
    if (tarball) {
      throw new Error(`Unexpected Afora package tarball check argument: ${arg}`);
    }
    tarball = arg;
  }
  if (!tarball) {
...
if (!fs.existsSync(tarball)) {
  fail(`Afora package tarball does not exist: ${tarball}`);
}
...
const REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES = ["@afora/ai"];
...
const REQUIRED_BUNDLED_WORKSPACE_RUNTIME_ENTRIES = new Map([
  [
    "@afora/ai",
    [
      { specifier: "@afora/ai", entry: "dist/index.mjs" },
      { specifier: "@afora/ai/providers", entry: "dist/providers.mjs" },
      {
        specifier: "@afora/ai/transports",
        entry: "dist/transports.mjs",
        whenExported: "./transports",
      },
      {
        specifier: "@afora/ai/internal/openai-responses-payload-policy",
        entry: "dist/internal/openai-responses-payload-policy.mjs",
        whenExported: "./internal/openai-responses-payload-policy",
      },
      {
        specifier: "@afora/ai/internal/runtime",
...
function collectRequiredBundledWorkspaceDependencyErrors(
  ...
  for (const name of REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES) {
    ...
    if (!bundledDependencies.has(name)) {
      errors.push(
        `package.json dependencies.${name} must be listed in bundleDependencies because it is private to the Afora workspace`,
      );
    }
...
const phaseTimingsEnabled = process.env.AFORA_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
...
function runPhase<Result>(label: string, action: () => Result): Result {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-afora-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-package-tarball-"));
try {
...
const PACKAGE_INSTALL_GUARD_RELATIVE_PATH = "dist/afora-install-guard";
...
if (shouldValidateShrinkwrap) {
  try {
    ...
    if (shrinkwrap.name !== "afora") {
      errors.push("npm-shrinkwrap.json root name must be afora");
    }
    ...
    if (!rootPackage || rootPackage.name !== "afora") {
      errors.push("npm-shrinkwrap.json packages root must name afora");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`Afora package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Afora package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("Afora package tarball integrity passed.");
