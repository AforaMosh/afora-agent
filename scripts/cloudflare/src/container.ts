...
interface AforaContainerEnv {
  ...
  AFORA_GATEWAY_TOKEN: string;
  AFORA_WEBHOOK_ONLY: string;
...
function buildContainerEnv(env: AforaContainerEnv): Record<string, string> {
  const containerEnv: Record<string, string> = {
    ...
    AFORA_GATEWAY_TOKEN: env.AFORA_GATEWAY_TOKEN,
  };
...
export class AforaContainer extends Container<AforaContainerEnv> {
  override defaultPort = 8080;
  // /healthz exists in every published Afora image and answers as soon as the
  ...
  constructor(ctx: unknown, env: AforaContainerEnv) {
    super(ctx, env);
    this.envVars = buildContainerEnv(env);
    this.webhookOnly = env.AFORA_WEBHOOK_ONLY === "true";
  }
