# Afora on Cloudflare Containers (experimental)

This template runs one Afora installation behind a Cloudflare Worker and one named Durable Object. The Durable Object starts a `standard-2` Container from a public, digest-pinned Docker Hub image. Litestream continuously replicates the global and per-agent SQLite databases to R2 through its S3-compatible API.
...
Cloudflare Worker
        |
        v
AforaContainer Durable Object (one stable name)
        |
        v
Afora + Litestream container :8080
        |
...
- Node.js and npm
- Model-provider and channel credentials for the Afora setup you choose
...
Choose an immutable, architecture-compatible digest from the official [`AforaMosh/afora-agent`](https://hub.docker.com/r/AforaMosh/afora-agent) Docker Hub repository. Replace `<official-afora-image-digest>` in `Dockerfile`, then build and push the derived image:
...
```bash
docker buildx imagetools inspect docker.io/<docker-hub-user>/afora-cloudflare:<version>
```

Use the resulting immutable `docker.io/<docker-hub-user>/afora-cloudflare@sha256:<digest>` reference. Cloudflare Containers can pull public Docker Hub images, but not GHCR images directly.
...
Keep the exact bootstrap recipe in a private, reproducible runbook. Litestream does not replicate `afora.json`, credential files, installed plugin files, or workspaces.
...
The template defaults `AFORA_WEBHOOK_ONLY` to `false`. This keeps the Container alive across idle periods for Discord, Slack Socket Mode, WhatsApp, and every other channel that maintains a socket or polling process.
...
- **Ephemeral disk:** Every Container restart or sleep starts with a fresh filesystem. The entrypoint lists R2 objects, derives the concrete SQLite restore manifest, restores each database, then starts Afora under Litestream.
- **Partial durability:** Litestream covers `/home/node/.afora/state/*.sqlite` and recursive per-agent SQLite databases only. Use a separate, private [`afora backup create`](https://docs.afora.ai/install/backups#full-archives) workflow for config, credential files, plugins, and workspaces.
...
- **Not a `cloudWorkers` provider:** this is a hosting template. Operator SSH access is enabled for bootstrap, but the template does not implement Afora's SSH-based cloud-worker provider contract.

## Updating

Build a new derived image from a new immutable official Afora digest, push it, replace the derived digest in `wrangler.jsonc`, and run:
...
Full operator guide: <https://docs.afora.ai/install/cloudflare>.
...
- `Dockerfile`: official Afora image plus pinned Litestream for `linux/amd64`
