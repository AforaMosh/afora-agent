...
// Afora Npm Postpublish Verify script supports Afora repository automation.
...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./afora-npm-release-check.ts";
...
const PACKAGED_BUNDLED_PLUGIN_ARTIFACTS = new Set(
  listBundledPluginPackArtifacts({
    env: { ...process.env, AFORA_BUNDLED_PLUGIN_BUILD_IDS: undefined },
  }),
);
...
type AforaNpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function aforaNpmPostpublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/afora-npm-postpublish-verify.ts <version>";
}

export function parseAforaNpmPostpublishVerifyArgs(
  argv: readonly string[],
): AforaNpmPostpublishVerifyArgs {
  ...
  if (!version) {
    throw new Error(aforaNpmPostpublishVerifyUsage());
  }
  if (version.startsWith("-")) {
    throw new Error(`Unknown afora npm postpublish verifier option: ${version}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected afora npm postpublish verifier argument: ${extraArg}`);
  }
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`afora@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
const NPM_PROVENANCE_REPOSITORY = "https://github.com/AforaMosh/afora-agent";
const NPM_PROVENANCE_WORKFLOW_PATH = ".github/workflows/afora-npm-release.yml";
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  if (
    ...
    throw new Error(
      `npm provenance attestation does not bind ${version} to the trusted Afora GitHub release workflow.`,
    );
  }
...
function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@afora/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "afora.cmd")
    : pathPosix.join(prefixDir, "bin", "afora");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  await verifyNpmProvenanceAttestation({
    ...
    integrity,
    attestations,
    expectedWorkflowRef: process.env.AFORA_NPM_EXPECTED_WORKFLOW_REF,
    expectedWorkflowSha: process.env.AFORA_NPM_EXPECTED_WORKFLOW_SHA,
  });
  console.log(
    `afora-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `afora-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "afora");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed afora binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`afora-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  const args = parseAforaNpmPostpublishVerifyArgs(argv);
  if (args.help) {
    console.log(aforaNpmPostpublishVerifyUsage());
    return;
  }
  ...
  console.log(
    `afora-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`afora-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
