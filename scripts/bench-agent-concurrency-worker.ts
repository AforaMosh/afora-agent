...
async function resetRuntime(persist: boolean): Promise<void> {
  const [subagents, tasks, stateDb, agentDb] = await Promise.all([
    ...
    import("../src/state/afora-agent-db.js"),
  ]);
  ...
  agentDb.closeAforaAgentDatabasesForTest();
}
...
type BenchmarkStateDatabase = Pick<AforaStateKyselyDatabase, "subagent_runs" | "task_runs">;
...
async function main(): Promise<void> {
  ...
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-agent-concurrency-worker-"));
