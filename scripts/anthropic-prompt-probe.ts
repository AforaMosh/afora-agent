// Anthropic Prompt Probe script supports Afora repository automation.
import { spawn } from "node:child_process";
// Live prompt probe for Anthropic setup-token and Claude CLI prompt-path debugging.
// Usage:
// AFORA_PROMPT_TRANSPORT=direct|gateway
// AFORA_PROMPT_MODE=extra
// AFORA_PROMPT_TEXT='...'
// AFORA_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.AFORA_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.AFORA_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.AFORA_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.AFORA_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "AFORA_PROMPT_CAPTURE",
  raw: process.env.AFORA_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "AFORA_PROMPT_INCLUDE_RAW",
  raw: process.env.AFORA_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "AFORA_PROMPT_KEEP_TMP",
  raw: process.env.AFORA_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.AFORA_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "AFORA_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.AFORA_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "AFORA_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.AFORA_PROMPT_GATEWAY_TIMEOUT_MS,
});
...
const CAPTURE_PROXY_MAX_BODY_BYTES = parseStrictIntegerOption({
  ...
  label: "AFORA_PROMPT_CAPTURE_MAX_BODY_BYTES",
  min: 1,
  raw: process.env.AFORA_PROMPT_CAPTURE_MAX_BODY_BYTES,
});
const GATEWAY_LOG_TAIL_BYTES = 256 * 1024;
const SETUP_TOKEN_RAW = process.env.AFORA_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.AFORA_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.AFORA_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
async function resolveSetupTokenSource(): Promise<TokenSource> {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set AFORA_LIVE_SETUP_TOKEN_VALUE or AFORA_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["afora.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        ...
        AFORA_AGENT_DIR: params.agentDir,
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-prompt-probe-"));
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing AFORA_PROMPT_TEXT or AFORA_PROMPT_LIST_JSON");
  }
