    ...
    `${JSON.stringify(
      {
        ...
        name: "weather-helper",
        extensions: {
          "ai.afora": { activation: { onStartup: true } },
...
async function main(): Promise<void> {
  ...
  const rootDir = await fs.realpath(
    await fs.mkdtemp(path.join(os.tmpdir(), "afora-agent-plugin-gateway-")),
  );
  const keep = process.env.AFORA_AGENT_PLUGIN_GATEWAY_E2E_KEEP === "1";
