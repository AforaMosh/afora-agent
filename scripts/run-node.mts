#!/usr/bin/env node
// Development runner that rebuilds Afora, runs runtime postbuild steps, and
...
) => env.AFORA_BUILD_PRIVATE_QA === "1" || !NON_PACKAGED_BUNDLED_PLUGIN_DIRS.has(pluginId);
...
const listRequiredBundledPluginMetadataOutputs = (
  ...
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "afora.plugin.json"));
    }
...
const listRequiredAforaExtensionAliasOutputs = (deps: RunNodeRequirementDeps) => {
  const distRoot = deps.distRoot;
  ...
  const aliasDir = path.join(distRoot, "extensions", "node_modules", "afora");
  return [
...
const listRequiredStaticExtensionAssetOutputs = (deps: RunNodeRequirementDeps) => {
  if (deps.env.AFORA_RUNTIME_POSTBUILD_STATIC_ASSETS === "0") {
    return [];
  }
  const distRoot = deps.distRoot;
  const runtimeRoot = path.join(deps.cwd, "dist-runtime");
...
const listRequiredRuntimePostBuildOutputs = (deps: RunNodeRequirementDeps) => {
  ...
  return [
    ...
    ...listRequiredAforaExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps: RunNodeRequirementDeps): BuildRequirement => {
  if (deps.env.AFORA_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.AFORA_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (
  ...
  if (deps.env.AFORA_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by AFORA_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by AFORA_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "AFORA_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "AFORA_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "AFORA_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "AFORA_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "AFORA_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "AFORA_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "AFORA_RUN_NODE_BUILD_LOCK_STALE_MS";
const RUN_NODE_SKIP_DTS_BUILD_ENV = "AFORA_RUN_NODE_SKIP_DTS_BUILD";
...
const logRunner = (message: string, deps: RunNodeLogDeps) => {
  if (deps.env.AFORA_RUNNER_LOG === "0") {
    return;
  }
  const line = `[afora] ${message}\n`;
...
const shouldUseRunNodeProgress = (deps: RunNodeDeps) =>
  ...
  deps.env.AFORA_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const createRunNodeProgress = (label: string, deps: RunNodeDeps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[afora] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const listRunNodeCpuProfiles = (
  ...
  const prefix = `afora-${commandName}-`;
  return entries
    .filter(
      (entry) =>
...
const resolveRunNodeCpuProfileArgs = (deps: RunNodeDeps) => {
  ...
  const profileName = `afora-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps: RunNodeDeps) => {
  ...
  if (deps.env.AFORA_TRACE_SYNC_IO === "1") {
...
const runAfora = (deps: RunNodeDeps) =>
  runNodeChild(deps, [...resolveRunNodeDiagnosticArgs(deps), "afora.mjs", ...deps.args]);
...
const closeRunNodeOutputTee = async (deps: RunNodeDeps, exitCode: number) => {
  ...
  try {
    ...
    deps.stderr.write(`[afora] Failed to write output log: ${getErrorMessage(error)}\n`);
...
const canUseStampedGatewayClientDist = (deps: RunNodeDeps) => {
  ...
  if (
    ...
    deps.env.AFORA_FORCE_RUNTIME_POSTBUILD === "1"
  ) {
    return false;
  }
...
export async function runNodeMain(params: RunNodeMainParams = {}): Promise<number> {
  ...
  if (deps.args[0] === "qa") {
    deps.env.AFORA_BUILD_PRIVATE_QA = "1";
    deps.env.AFORA_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.AFORA_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
