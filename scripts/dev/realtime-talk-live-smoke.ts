// Realtime Talk Live Smoke script supports Afora repository automation.
...
const OPENAI_REALTIME_MODEL =
  process.env.AFORA_REALTIME_OPENAI_MODEL?.trim() || "gpt-realtime-2.1";
const OPENAI_REALTIME_VOICE = process.env.AFORA_REALTIME_OPENAI_VOICE?.trim() || "alloy";
...
const GOOGLE_REALTIME_MODEL =
  process.env.AFORA_REALTIME_GOOGLE_MODEL?.trim() || "gemini-3.1-flash-live-preview";
const GOOGLE_REALTIME_VOICE = process.env.AFORA_REALTIME_GOOGLE_VOICE?.trim() || "Kore";
...
type OpenAIWebRtcSmokeGlobal = typeof globalThis & {
  aforaReadBoundedRealtimeResponseText?: OpenAIRealtimeBrowserResponseReader;
};
...
function resolveOpenAIHttpTimeoutMs(
  raw = process.env.AFORA_REALTIME_OPENAI_HTTP_TIMEOUT_MS,
): number {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_HTTP_TIMEOUT_MS,
    label: "AFORA_REALTIME_OPENAI_HTTP_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
...
function openAIRealtimeBrowserResponseReaderInitScript(): string {
  return `globalThis.aforaReadBoundedRealtimeResponseText = ${readOpenAIRealtimeBrowserResponseText.toString()};`;
}
...
async function smokeOpenAIBackendBridge(apiKey: string): Promise<SmokeResult> {
  ...
  const bridge = provider.createBridge({
    ...
    instructions: "Afora backend realtime live smoke. Do not speak yet.",
    onAudio: () => {},
    onClearAudio: () => {},
    onEvent: (event) => {
      ...
      const result = await page.evaluate(
        async ({ clientSecret: secret, sdpAnswerMaxBytes, timeoutMs }) => {
          const readBoundedTextLocal = (globalThis as OpenAIWebRtcSmokeGlobal)
            .aforaReadBoundedRealtimeResponseText;
...
async function smokeGoogleLiveBrowserWs(browser: Browser, apiKey: string): Promise<SmokeResult> {
  try {
    ...
    const session = await provider.createBrowserSession?.({
      ...
      voice: GOOGLE_REALTIME_VOICE,
      instructions:
        "Afora browser Video Talk live smoke. After receiving a visual frame and request, call describe_view exactly once.",
...
async function smokeGatewayRelayBrowser(browser: Browser): Promise<SmokeResult> {
  ...
  const dir = await mkdtemp(path.join(tmpdir(), "afora-realtime-talk-"));
  try {
