// Gateway Smoke script supports Afora repository automation.
...
function parseGatewaySmokeCli(
  ...
  return {
    help: has("--help") || has("-h"),
    token: getArg("--token") ?? env.AFORA_GATEWAY_TOKEN,
    urlRaw: getArg("--url") ?? env.AFORA_GATEWAY_URL,
  };
}
...
export async function runGatewaySmoke(
  ...
  try {
    ...
    const connectRes = await request("connect", {
      ...
      client: {
        id: "afora-ios",
        displayName: "afora gateway smoke test",
        version: "dev",
        platform: "dev",
        mode: "ui",
        instanceId: "afora-dev-smoke",
      },
      locale: "en-US",
