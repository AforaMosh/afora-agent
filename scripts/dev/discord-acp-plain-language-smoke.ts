#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports Afora repository automation.
...
type DriverMode = "token" | "webhook" | "afora";
...
const DEFAULT_AFORA_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  ...
  const home = process.env.AFORA_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".afora");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "afora") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or afora.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver afora] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Afora spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|afora> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override Afora state dir for plugin-state polling\n" +
    "  --afora-bin <path>        Afora CLI binary for driver=afora (default: afora)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  AFORA_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  AFORA_DISCORD_SMOKE_DRIVER\n" +
    "  AFORA_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  AFORA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  AFORA_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  AFORA_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  AFORA_DISCORD_SMOKE_AGENT\n" +
    "  AFORA_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  AFORA_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  AFORA_DISCORD_SMOKE_POLL_MS\n" +
    "  AFORA_STATE_DIR\n" +
    "  AFORA_DISCORD_SMOKE_AFORA_BIN"
  );
}
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const channelId =
    resolveArg("--channel", argv) || process.env.AFORA_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver", argv) || process.env.AFORA_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token", argv) || process.env.AFORA_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.AFORA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
    resolveArg("--bot-token", argv) ||
    process.env.AFORA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix", argv) ||
    process.env.AFORA_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent", argv) || process.env.AFORA_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention", argv) ||
    process.env.AFORA_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction", argv) ||
    process.env.AFORA_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms", argv) || process.env.AFORA_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.AFORA_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir", argv) || resolveStateDir());
  const aforaBin =
    resolveArg("--afora-bin", argv) ||
    process.env.AFORA_DISCORD_SMOKE_AFORA_BIN ||
    "afora";
...
async function aforaCliJson<T>(params: {
  aforaBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.aforaBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_AFORA_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`afora ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithAfora(params: {
  aforaBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await aforaCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    aforaBin: params.aforaBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "afora") {
    return await readMessagesWithAfora({
      aforaBin: params.args.aforaBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `afora-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("afora message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "afora"
            ? await readMessagesWithAfora({
                aforaBin: args.aforaBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Afora.`,
        diagnostics: {
          bindingCandidates: [
            {
