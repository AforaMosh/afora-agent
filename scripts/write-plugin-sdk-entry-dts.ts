// Write Plugin Sdk Entry Dts script supports Afora repository automation.
...
const USE_CANONICAL_DECLARATIONS = process.env.AFORA_PLUGIN_SDK_CANONICAL_DTS === "1";

function isBareImportSpecifier(id: string): boolean {
  if (
    id === "@afora/llm-core" ||
    id.startsWith("@afora/llm-core/") ||
    id === "@afora/model-catalog-core/model-catalog-types" ||
    id === "@afora/retry" ||
...
if (USE_CANONICAL_DECLARATIONS) {
  ...
  const flatDeclarationTempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-plugin-sdk-dts-"));
  try {
    await build({
      clean: true,
      config: false,
