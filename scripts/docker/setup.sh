...
EXTRA_MOUNTS="${AFORA_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${AFORA_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${AFORA_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${AFORA_DOCKER_SOCKET:-}"
TIMEZONE="${AFORA_TZ:-}"
RAW_SKIP_ONBOARDING="${AFORA_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
DOCKER_PULL_TIMEOUT="${AFORA_DOCKER_SETUP_PULL_TIMEOUT:-600s}"
OFFLINE_MODE=""
DEFAULT_SANDBOX_IMAGE="afora-sandbox:bookworm-slim"
DEFAULT_SANDBOX_BROWSER_IMAGE="afora-sandbox-browser:bookworm-slim"
...
read_config_gateway_token() {
  local config_path="$AFORA_CONFIG_DIR/afora.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == AFORA_GATEWAY_TOKEN=* ]]; then
      token="${line#AFORA_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${AFORA_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$AFORA_GATEWAY_PORT" "$AFORA_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$AFORA_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$AFORA_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network afora-cli service because it
  ...
  # Host AFORA_* paths are Compose bind-mount sources. Setup-time CLI writes
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" afora-cli "$@"
}
...
run_gateway_up() {
  ...
  docker compose "${compose_args[@]}" "${up_args[@]}" afora-gateway
}
...
resolve_offline_sandbox_images() {
  ...
  printf '%s' "$agents_json" | run_prestart_gateway \
    -T --entrypoint node afora-gateway -e '
const fs = require("node:fs");
...
const matchesBrowser = (rawPattern) => {
  ...
  if (pattern === "group:afora" || pattern === "group:ui") {
    return true;
  }
  if (!pattern) {
    return false;
...
validate_offline_sandbox_prerequisites() {
      ...
      browser)
        if ! browser_contract="$(
          docker --host "unix://$DOCKER_SOCKET_PATH" image inspect \
            -f '{{ index .Config.Labels "org.afora.sandbox-browser.contract" }}' \
  ...
  if ! run_compose_one_off --rm --entrypoint docker afora-gateway --version >/dev/null 2>&1; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "AFORA_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "AFORA_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
AFORA_CONFIG_DIR="${AFORA_CONFIG_DIR:-$HOME/.afora}"
AFORA_WORKSPACE_DIR="${AFORA_WORKSPACE_DIR:-$HOME/.afora/workspace}"
AFORA_AUTH_PROFILE_SECRET_DIR="${AFORA_AUTH_PROFILE_SECRET_DIR:-$HOME/.afora-auth-profile-secrets}"

validate_mount_path_value "AFORA_CONFIG_DIR" "$AFORA_CONFIG_DIR"
validate_mount_path_value "AFORA_WORKSPACE_DIR" "$AFORA_WORKSPACE_DIR"
validate_mount_path_value "AFORA_AUTH_PROFILE_SECRET_DIR" "$AFORA_AUTH_PROFILE_SECRET_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "AFORA_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "AFORA_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "AFORA_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "AFORA_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "AFORA_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$AFORA_CONFIG_DIR"
mkdir -p "$AFORA_WORKSPACE_DIR"
mkdir -p "$AFORA_AUTH_PROFILE_SECRET_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$AFORA_CONFIG_DIR/identity"
mkdir -p "$AFORA_CONFIG_DIR/agents/main/agent"
mkdir -p "$AFORA_CONFIG_DIR/agents/main/sessions"

export AFORA_CONFIG_DIR
export AFORA_WORKSPACE_DIR
export AFORA_AUTH_PROFILE_SECRET_DIR
export AFORA_GATEWAY_PORT="${AFORA_GATEWAY_PORT:-18789}"
export AFORA_BRIDGE_PORT="${AFORA_BRIDGE_PORT:-18790}"
export AFORA_GATEWAY_BIND="${AFORA_GATEWAY_BIND:-lan}"
export AFORA_DISABLE_BONJOUR="${AFORA_DISABLE_BONJOUR:-}"
export AFORA_IMAGE="$IMAGE_NAME"
export AFORA_IMAGE_APT_PACKAGES="${AFORA_IMAGE_APT_PACKAGES-${AFORA_DOCKER_APT_PACKAGES:-}}"
export AFORA_IMAGE_PIP_PACKAGES="${AFORA_IMAGE_PIP_PACKAGES:-}"
export AFORA_EXTENSIONS="${AFORA_EXTENSIONS:-}"
export AFORA_DOCKER_BUILD_NODE_OPTIONS="${AFORA_DOCKER_BUILD_NODE_OPTIONS---max-old-space-size=8192}"
export AFORA_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB="${AFORA_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB:-}"
export AFORA_DOCKER_BUILD_SKIP_DTS="${AFORA_DOCKER_BUILD_SKIP_DTS:-1}"
export AFORA_INSTALL_BROWSER="${AFORA_INSTALL_BROWSER:-}"
export AFORA_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export AFORA_HOME_VOLUME="$HOME_VOLUME_NAME"
export AFORA_ALLOW_INSECURE_PRIVATE_WS="${AFORA_ALLOW_INSECURE_PRIVATE_WS:-}"
export AFORA_SANDBOX="$SANDBOX_ENABLED"
export AFORA_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export AFORA_DOCKER_SETUP=1
export AFORA_TZ="$TIMEZONE"
...
export AFORA_OTEL_PRELOADED="${AFORA_OTEL_PRELOADED:-}"
export AFORA_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${AFORA_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    AFORA_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $AFORA_CONFIG_DIR/afora.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      AFORA_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      AFORA_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      AFORA_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${AFORA_CONFIG_DIR}:/home/node/.afora"
    gateway_workspace_mount="${AFORA_WORKSPACE_DIR}:/home/node/.afora/workspace"
    gateway_auth_profile_secret_mount="${AFORA_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/afora"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${AFORA_INSTALL_DOCKER_CLI:-}" ]]; then
    export AFORA_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  AFORA_IMAGE_PIP_PACKAGES \
  AFORA_EXTENSIONS \
  AFORA_DOCKER_BUILD_NODE_OPTIONS \
  AFORA_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB \
  AFORA_DOCKER_BUILD_SKIP_DTS \
  AFORA_INSTALL_BROWSER \
  AFORA_SANDBOX \
  AFORA_DOCKER_SOCKET \
  DOCKER_GID \
  AFORA_INSTALL_DOCKER_CLI \
  ...
  AFORA_OTEL_PRELOADED \
  AFORA_SKIP_ONBOARDING

if [[ -n "$OFFLINE_MODE" ]]; then
  require_local_docker_image "$IMAGE_NAME"
  echo "==> Using preloaded Docker image: $IMAGE_NAME"
elif [[ "$IMAGE_NAME" == "afora:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  BUILD_GIT_COMMIT="$(afora_resolve_git_commit "$ROOT_DIR")"
  BUILD_TIMESTAMP="$(afora_resolve_build_timestamp)"
  PROVENANCE_BUILD_ARGS=(--build-arg "AFORA_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}")
  if [[ "$BUILD_GIT_COMMIT" =~ ^[0-9a-fA-F]{40}$ ]]; then
    PROVENANCE_BUILD_ARGS+=(--build-arg "GIT_COMMIT=${BUILD_GIT_COMMIT}")
  fi
  run_docker_build \
    "${PROVENANCE_BUILD_ARGS[@]}" \
    --build-arg "AFORA_IMAGE_APT_PACKAGES=${AFORA_IMAGE_APT_PACKAGES}" \
    --build-arg "AFORA_IMAGE_PIP_PACKAGES=${AFORA_IMAGE_PIP_PACKAGES}" \
    --build-arg "AFORA_EXTENSIONS=${AFORA_EXTENSIONS}" \
    --build-arg "AFORA_DOCKER_BUILD_NODE_OPTIONS=${AFORA_DOCKER_BUILD_NODE_OPTIONS}" \
    --build-arg "AFORA_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=${AFORA_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB}" \
    --build-arg "AFORA_DOCKER_BUILD_SKIP_DTS=${AFORA_DOCKER_BUILD_SKIP_DTS}" \
    --build-arg "AFORA_INSTALL_BROWSER=${AFORA_INSTALL_BROWSER}" \
    --build-arg "AFORA_INSTALL_DOCKER_CLI=${AFORA_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Afora metadata subdirectory
# (.afora/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh afora-gateway -c \
  'PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; export PATH; \
   /usr/bin/find -P /home/node/.afora -xdev -execdir /usr/bin/chown -h node:node {} +; \
   /usr/bin/chown -h node:node /home/node/.config; \
   /usr/bin/find -P /home/node/.config/afora -xdev -execdir /usr/bin/chown -h node:node {} +; \
   if [ -d /home/node/.afora/workspace/.afora ] && [ ! -L /home/node/.afora/workspace/.afora ]; then \
     /usr/bin/find -P /home/node/.afora/workspace/.afora -xdev -execdir /usr/bin/chown -h node:node {} +; \
   fi || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (AFORA_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from AFORA_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $AFORA_GATEWAY_BIND"
  if is_truthy_value "$AFORA_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (AFORA_DISABLE_BONJOUR=$AFORA_DISABLE_BONJOUR)."
  elif [[ -z "$AFORA_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (AFORA_DISABLE_BONJOUR=$AFORA_DISABLE_BONJOUR)."
  fi
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm afora-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm afora-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm afora-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.afora.ai/channels"
...
# --- Sandbox setup (opt-in via AFORA_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -z "$OFFLINE_MODE" && ! -S "$DOCKER_SOCKET_PATH" ]]; then
    echo "WARNING: AFORA_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
  ...
  if [[ -n "$SANDBOX_ENABLED" && -z "$OFFLINE_MODE" ]] &&
    ! run_compose_one_off --rm --entrypoint docker afora-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg AFORA_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (AFORA_IMAGE=afora:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Afora config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.afora.ai/gateway/sandboxing"
...
echo "Config: $AFORA_CONFIG_DIR"
echo "Workspace: $AFORA_WORKSPACE_DIR"
echo "Token: stored in Docker environment/config (not printed)."
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f afora-gateway"
echo "  ${COMPOSE_HINT} exec afora-gateway sh -lc 'node dist/index.js gateway health --token \"\$AFORA_GATEWAY_TOKEN\"'"
