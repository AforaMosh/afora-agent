...
workflow_sha=""
image_refs=()
shared_package_sha256="${AFORA_SHARED_IMAGE_PACKAGE_SHA256:-}"
shared_archive_sha256="${AFORA_SHARED_IMAGE_ARCHIVE_SHA256:-}"
shared_run_id="${AFORA_SHARED_IMAGE_RUN_ID:-}"
shared_run_attempt="${AFORA_SHARED_IMAGE_RUN_ATTEMPT:-}"
...
if (value.schema !== "afora.shared-docker-image-artifact/v1") fail("schema");
...
case "$command_name" in
  pack | load)
    ...
    if [[ "$command_name" == "pack" ]]; then
      ...
      require_positive_decimal "AFORA_SHARED_IMAGE_RUN_ID" "$shared_run_id"
      require_positive_decimal "AFORA_SHARED_IMAGE_RUN_ATTEMPT" "$shared_run_attempt"
