...
const GIT_TIMEOUT_ENV = "AFORA_RELEASE_METADATA_GIT_TIMEOUT_MS";
...
function normalizeVersionText(raw: string) {
  return raw
    .replace(/\b20\d{2}\.\d{1,2}\.\d{1,2}(?:-beta\.\d+|-\d+)?\b/gu, "<AFORA_VERSION>")
    .replace(/\b20\d{6}(?:\d{2})?\b/gu, "<AFORA_BUILD>");
}
