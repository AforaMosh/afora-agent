...
#   ./scripts/k8s/deploy.sh --delete-resources # Delete Afora resources only
#   ./scripts/k8s/deploy.sh --delete-namespace # Delete the namespace and all resources
#
# Environment:
#   AFORA_NAMESPACE   Kubernetes namespace (default: afora)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MANIFESTS="$SCRIPT_DIR/manifests"
NS="${AFORA_NAMESPACE:-afora}"
...
Usage: ./scripts/k8s/deploy.sh [OPTION]
  ...
  (no args)        Deploy Afora (creates secret from env if needed)
  ...
  --delete-resources
                  Delete Afora resources from the namespace
  --delete-namespace
...
if [[ "$MODE" == "delete-resources" ]]; then
  echo "Deleting Afora resources from namespace '$NS'..."
  kubectl delete -k "$MANIFESTS" -n "$NS" --ignore-not-found
  kubectl delete secret afora-secrets -n "$NS" --ignore-not-found
  echo "Done."
  exit 0
fi
...
_apply_secret() {
  ...
  if kubectl get secret afora-secrets -n "$NS" &>/dev/null; then
    EXISTING_SECRET=true
    EXISTING_TOKEN="$(kubectl get secret afora-secrets -n "$NS" -o jsonpath='{.data.AFORA_GATEWAY_TOKEN}' | base64 -d)"
    ANTHROPIC_VALUE="$(kubectl get secret afora-secrets -n "$NS" -o jsonpath='{.data.ANTHROPIC_API_KEY}' 2>/dev/null | base64 -d)"
    OPENAI_VALUE="$(kubectl get secret afora-secrets -n "$NS" -o jsonpath='{.data.OPENAI_API_KEY}' 2>/dev/null | base64 -d)"
    GEMINI_VALUE="$(kubectl get secret afora-secrets -n "$NS" -o jsonpath='{.data.GEMINI_API_KEY}' 2>/dev/null | base64 -d)"
    OPENROUTER_VALUE="$(kubectl get secret afora-secrets -n "$NS" -o jsonpath='{.data.OPENROUTER_API_KEY}' 2>/dev/null | base64 -d)"
  fi
  ...
  printf '%s' "$TOKEN" > "$TMP_DIR/AFORA_GATEWAY_TOKEN"
  ...
  chmod 600 \
    "$TMP_DIR/AFORA_GATEWAY_TOKEN" \
    "$TMP_DIR/ANTHROPIC_API_KEY" \
    "$TMP_DIR/OPENAI_API_KEY" \
    "$TMP_DIR/GEMINI_API_KEY" \
    "$TMP_DIR/OPENROUTER_API_KEY"

  kubectl create secret generic afora-secrets \
    -n "$NS" \
    --from-file=AFORA_GATEWAY_TOKEN="$TMP_DIR/AFORA_GATEWAY_TOKEN" \
  ...
  kubectl apply --server-side --field-manager=afora -f "$SECRET_MANIFEST" >/dev/null
  # Clean up any annotation left by older client-side apply runs.
  kubectl annotate secret afora-secrets -n "$NS" kubectl.kubernetes.io/last-applied-configuration- >/dev/null 2>&1 || true
  rm -rf "$TMP_DIR"
  trap - EXIT
  ...
  if $SHOW_TOKEN; then
    ...
    echo "  kubectl get secret afora-secrets -n $NS -o jsonpath='{.data.AFORA_GATEWAY_TOKEN}' | base64 -d && echo"
  fi
}
...
kubectl rollout restart deployment/afora -n "$NS" 2>/dev/null || true
echo ""
echo "Waiting for rollout..."
kubectl rollout status deployment/afora -n "$NS" --timeout=300s
echo ""
echo "Done. Access the gateway:"
echo "  kubectl port-forward svc/afora 18789:18789 -n $NS"
echo "  open http://localhost:18789"
echo ""
if $SHOW_TOKEN; then
  echo "Gateway token (paste into Control UI):"
  echo "  $(kubectl get secret afora-secrets -n "$NS" -o jsonpath='{.data.AFORA_GATEWAY_TOKEN}' | base64 -d)"
echo ""
fi
