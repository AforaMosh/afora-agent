...
- regenerates apps/ios/Afora.xcodeproj via xcodegen
EOF
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical Afora Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi

if [[ -n "${AFORA_PUSH_RELAY_BASE_URL:-}" || -n "${IOS_PUSH_RELAY_BASE_URL:-}" ]]; then
...
RELEASE_GIT_COMMIT="$(AFORA_REQUIRE_BUILD_METADATA=1 afora_resolve_git_commit "${ROOT_DIR}")"
...
${RELEASE_SIGNING_XCCONFIG}
AFORA_DEVELOPMENT_TEAM = ${TEAM_ID}
AFORA_IOS_SELECTED_TEAM = ${TEAM_ID}
AFORA_APP_BUNDLE_ID = ai.aforafoundation.app
AFORA_SHARE_BUNDLE_ID = ai.aforafoundation.app.share
AFORA_ACTIVITY_WIDGET_BUNDLE_ID = ai.aforafoundation.app.activitywidget
AFORA_WATCH_APP_BUNDLE_ID = ai.aforafoundation.app.watchkitapp
AFORA_CODE_SIGN_ENTITLEMENTS = Sources/AforaAppAttest.entitlements
AFORA_APNS_ENTITLEMENT_ENVIRONMENT = production
AFORA_APP_ATTEST_ENVIRONMENT = production
AFORA_PUSH_MODE = appStore
AFORA_PUSH_RELAY_BASE_URL =
EOF
