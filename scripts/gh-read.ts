// Gh Read script supports Afora repository automation.
import { spawnSync } from "node:child_process";
import { createPrivateKey, createSign } from "node:crypto";
import { pathToFileURL } from "node:url";
import { readSecretFileSync } from "@afora/fs-safe/secret";
...
const APP_ID_ENV = "AFORA_GH_READ_APP_ID";
const KEY_FILE_ENV = "AFORA_GH_READ_PRIVATE_KEY_FILE";
const INSTALLATION_ID_ENV = "AFORA_GH_READ_INSTALLATION_ID";
const PERMISSIONS_ENV = "AFORA_GH_READ_PERMISSIONS";
...
export function resolveGitHubFetchTimeoutMs(raw = process.env.AFORA_GH_READ_FETCH_TIMEOUT_MS) {
  return parseStrictIntegerOption({
    fallback: DEFAULT_GITHUB_FETCH_TIMEOUT_MS,
    label: "AFORA_GH_READ_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
...
async function main() {
  if (process.argv.length <= 2) {
    fail(
      "usage: scripts/gh-read <gh args...>\nset AFORA_GH_READ_APP_ID and AFORA_GH_READ_PRIVATE_KEY_FILE first",
    );
  }
