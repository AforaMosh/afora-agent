...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.AFORA_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.AFORA_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.AFORA_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.AFORA_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.AFORA_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.AFORA_VITEST_PRINT_IMPORT_BREAKDOWN)) {
