...
import { createAforaTestState } from "../src/test-utils/afora-test-state.js";
...
const replyMarker = "AFORA_OPENAI_ONBOARDING_OK";

async function runAfora(args: string[], env: NodeJS.ProcessEnv): Promise<string> {
  try {
  ...
  it("keeps repeated onboarding secret-safe and runs the actual default model", async () => {
    const state = await createAforaTestState({
      ...
      env: {
        ...
        AFORA_TEST_HOME: undefined,
        AFORA_TEST_MINIMAL_GATEWAY: undefined,
        AFORA_TEST_TRUST_BUNDLED_PLUGINS_DIR: undefined,
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        await runAfora(onboardArgs, state.env);
        ...
        expect(config.agents?.defaults?.workspace).toBe(
          path.join(state.home, ".afora", "workspace"),
        );
