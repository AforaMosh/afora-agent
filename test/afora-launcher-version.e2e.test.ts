...
async function makeLauncherVersionFixture(
  ...
  const fixtureRoot = makeTempDir(fixtureRoots, "afora-launcher-version-");
  await fs.copyFile(
...
function runLauncherVersion(
  ...
  return spawnSync(
    process.execPath,
    [path.join(fixtureRoot, "afora.mjs"), options.flag ?? "--version"],
    {
      cwd: fixtureRoot,
      env: {
        ...process.env,
...
describe("afora launcher version provenance", () => {
  ...
  it.each(["directory", "linked"] as const)(
    ...
    async (checkout) => {
      ...
      expect(result.stdout).toBe(`Afora ${packageVersion} (${buildCommit.slice(0, 7)})\n`);
  ...
  it.each(["directory", "linked"] as const)(
    ...
    async (checkout) => {
      ...
      expect(result.status).toBe(0);
      expect(result.stdout).toBe(`Afora ${packageVersion} (${checkoutCommit.slice(0, 7)})\n`);
      expect(result.stderr).toBe("");
    },
  );
  ...
  it.each([
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toBe(`Afora ${packageVersion} (${expected})\n`);
    expect(result.stderr).toBe("");
  });
  ...
  it("defers container-targeted root version to the runtime entry", async () => {
    ...
    const argumentResult = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "afora.mjs"), "--container", "demo", "--version"],
      {
        cwd: fixtureRoot,
        env: { ...process.env, AFORA_CONTAINER: undefined },
        encoding: "utf8",
      },
    );
