...
// Afora npm postpublish tests validate postpublish verification behavior.
...
import {
  ...
  aforaNpmPostpublishVerifyUsage,
  parseAforaNpmPostpublishVerifyArgs,
...
describe("parseAforaNpmPostpublishVerifyArgs", () => {
  it("keeps trusted release verification independent from target app dependencies", () => {
    const source = readFileSync("scripts/afora-npm-postpublish-verify.ts", "utf8");
  ...
  it("supports help and package-manager separators", () => {
    expect(parseAforaNpmPostpublishVerifyArgs(["--help"])).toEqual({
      help: true,
      version: "",
    });
    expect(parseAforaNpmPostpublishVerifyArgs(["--", "2026.3.23"])).toEqual({
      help: false,
      version: "2026.3.23",
    });
  });
  ...
  it("rejects missing, option-like, and extra arguments before verification", () => {
    expect(() => parseAforaNpmPostpublishVerifyArgs([])).toThrow(
      aforaNpmPostpublishVerifyUsage(),
    );
    expect(() => parseAforaNpmPostpublishVerifyArgs(["--tag"])).toThrow(
      "Unknown afora npm postpublish verifier option: --tag",
    );
    expect(() => parseAforaNpmPostpublishVerifyArgs(["2026.3.23", "extra"])).toThrow(
      "Unexpected afora npm postpublish verifier argument: extra",
    );
  });
});
...
describe("buildPublishedInstallScenarios", () => {
  ...
  it("adds a stable-to-correction upgrade scenario for correction releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23-2")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["afora@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
      {
        name: "upgrade-from-base-stable",
        installSpecs: ["afora@2026.3.23", "afora@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
    ]);
  });
...
describe("npm registry provenance verification", () => {
  ...
  const buildProvenancePayload = (releaseVersion: string, workflowRef: string) => ({
    ...
    predicate: {
      buildDefinition: {
        externalParameters: {
          workflow: {
            repository: "https://github.com/AforaMosh/afora-agent",
            path: ".github/workflows/afora-npm-release.yml",
            ref: workflowRef,
          },
        },
      },
  ...
  it("fetches npm registry JSON with bounded response handling", async () => {
    ...
    await expect(
      fetchRegistryJson("https://registry.example/afora", {
  ...
  it("bounds oversized npm registry response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry https://registry.example/afora response body exceeded 64 bytes",
    );
  });
  ...
  it("keeps npm registry timeouts active while reading response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry request timed out after 5ms: https://registry.example/afora",
    );
  });
  ...
  it("requires a trusted GitHub release identity for the exact SLSA provenance attestation", async () => {
    ...
    expect(verificationPolicy).toEqual({
      ...
      certificateIdentityURI:
        "https://github.com/AforaMosh/afora-agent/.github/workflows/afora-npm-release.yml@refs/heads/release/2026.3.23",
    });
                    ...
                    predicate: {
                      ...
                      buildDefinition: {
                        ...
                        resolvedDependencies: [
                          {
                            uri: `git+https://github.com/AforaMosh/afora-agent@${protectedWorkflowRef}`,
    ...
    expect(verificationPolicy).toEqual({
      certificateIssuer: "https://token.actions.githubusercontent.com",
      certificateIdentityURI: `https://github.com/AforaMosh/afora-agent/.github/workflows/afora-npm-release.yml@${protectedWorkflowRef}`,
    });
            ...
            bundle: {
              dsseEnvelope: {
                payload: Buffer.from(
                  JSON.stringify({
                    ...provenancePayload,
                    subject: [{ name: "pkg:npm/afora@2026.3.24", digest: {} }],
                  }),
                  "utf8",
                ).toString("base64"),
              },
  ...
  it.each([
    ...
    async (extendedStableVersion, workflowRef) => {
      ...
      expect(verificationPolicy).toEqual({
        certificateIssuer: "https://token.actions.githubusercontent.com",
        certificateIdentityURI: `https://github.com/AforaMosh/afora-agent/.github/workflows/afora-npm-release.yml@${workflowRef}`,
      });
    },
  );
  ...
  it.each([
    ...
    async (_label, extendedStableVersion, workflowRef) => {
      ...
      await expect(
        ...
        `does not bind ${extendedStableVersion} to the trusted Afora GitHub release workflow`,
      );
  ...
  it("rejects matching provenance from an untrusted source before Sigstore verification", async () => {
    ...
    ).rejects.toThrow("does not bind 2026.3.23 to the trusted Afora GitHub release workflow");
    expect(verificationCalls).toBe(0);
  });
    ...
    await expect(
      retryNpmRegistryProvenanceRead(
        async () => {
          ...
          if (attempts === 1) {
            throw new Error(
              "npm provenance attestation does not bind 2026.3.23 to the trusted Afora GitHub release workflow.",
            );
          }
          if (attempts === 2) {
            throw new Error(
              "npm registry provenance metadata is incomplete for afora@2026.3.23.",
            );
          }
          return "verified";
        },
...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/afora-prefix", "afora@2026.4.10");
...
describe("collectInstalledPackageErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "afora-postpublish-package-"));
  }
  ...
  it("keeps bundled manifest requirements stable after the build filter changes", () => {
    ...
    try {
      ...
      const filteredErrors = withEnv({ AFORA_BUNDLED_PLUGIN_BUILD_IDS: "ollama" }, () =>
    ...
    try {
      const probe = spawnSync(
        ...
        [
          ...
          "--eval",
          [
            'import { collectInstalledBundledExtensionManifestErrors } from "./scripts/afora-npm-postpublish-verify.ts";',
        ...
        ],
        {
          cwd: process.cwd(),
          encoding: "utf8",
          env: { ...process.env, AFORA_BUNDLED_PLUGIN_BUILD_IDS: "ollama" },
          timeout: 30_000,
        },
      );
...
describe("collectInstalledAlwaysAllowedRuntimeFacadeErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "afora-postpublish-facade-runtime-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "afora-postpublish-context-engine-"));
  }
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("Afora 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("Afora 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
    expect(normalizeInstalledBinaryVersion("Afora 2026.4.8-alpha.1 (9ece252)")).toBe(
      "2026.4.8-alpha.1",
    );
  });
});
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/afora-prefix", "darwin")).toBe(
      "/tmp/afora-prefix/bin/afora",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/afora-prefix", "win32")).toBe(
      "C:\\afora-prefix\\afora.cmd",
    );
  });
});
...
describe("resolveInstalledBinaryCommandInvocation", () => {
  it("runs the Unix installed binary directly", () => {
    expect(
      resolveInstalledBinaryCommandInvocation("/tmp/afora-prefix", ["--version"], {
        platform: "linux",
      }),
    ).toEqual({
      command: "/tmp/afora-prefix/bin/afora",
      args: ["--version"],
    });
  });
  ...
  it("wraps the Windows installed npm shim without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\afora prefix\\afora.cmd" agent --message "hello world""',
      ],
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "afora-postpublish-root-deps-"));
  }
