// Afora test instance helper spawns isolated Afora processes.
...
type AforaTestStateOptions = NonNullable<Parameters<typeof createAforaTestState>[0]>;

type AforaTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<AforaTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

type AforaTestInstanceCommandResult = {
  code: number | null;
...
type AforaTestProcess = ChildProcessByStdio<null, Readable, Readable>;

export type AforaTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  stdout: string[];
  stderr: string[];
  child?: AforaTestProcess;
  env: NodeJS.ProcessEnv;
  entrypoint: () => Promise<string[]>;
  cli: (
    args: string[],
    options?: { timeoutMs?: number },
  ) => Promise<AforaTestInstanceCommandResult>;
...
const GATEWAY_MIGRATION_CONVERGENCE_RESTART_MARKER =
  "[afora-test-instance] restarting gateway after migration convergence refusal\n";
...
type AforaTestChildProcess = Pick<AforaTestProcess, "kill" | "pid">;
type AforaTestProcessReadiness = Pick<AforaTestProcess, "exitCode" | "signalCode"> & {
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseAforaTestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalAforaTestProcess(child, "SIGKILL");
...
async function waitForGatewayReady(
  proc: AforaTestProcessReadiness,
  chunksOut: string[],
  chunksErr: string[],
  port: number,
  timeoutMs: number,
...
function hasGatewayProcessClosed(child: AforaTestProcess): boolean {
...
function hasChildExited(child: Pick<AforaTestProcess, "exitCode" | "signalCode">) {
...
export async function createAforaTestInstance(
  options: AforaTestInstanceOptions,
): Promise<AforaTestInstance> {
  ...
  let child: AforaTestProcess | undefined;
  let cleaned = false;
  const stopTimeoutMs = options.stopTimeoutMs ?? GATEWAY_STOP_TIMEOUT_MS;
  const spawnGatewayProcess = (args: string[], attemptStderr: string[]): AforaTestProcess => {
  ...
  const instance: AforaTestInstance = {
    name: options.name,
    port,
...
function shouldUseAforaTestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalAforaTestProcess(
  child: AforaTestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = (pid, nextSignal) =>
    process.kill(pid, nextSignal),
): void {
  if (shouldUseAforaTestProcessGroup() && typeof child.pid === "number") {
    try {
