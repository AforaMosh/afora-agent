...
import { createAforaTestInstance, type AforaTestInstance } from "./afora-test-instance.js";

export type GatewayInstance = AforaTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createAforaTestInstance({ name });
  try {
