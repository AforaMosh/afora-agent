      ...
      writePersistedAuthProfileStoreRaw(
        {
          ...
          profiles: {
            "openai:test": {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "AFORA_LIVE_OPENAI_KEY" },
            },
          },
        },
        agentDir,
...
describe("stage-live-auth-profiles", () => {
  it.each(["auth_profile_store", "auth_profile_state"] as const)(
    ...
    (missingTable) => {
      const sourceStateDir = createStateDir("afora-live-auth-partial-source-");
      const targetStateDir = createStateDir("afora-live-auth-partial-target-");
  ...
  it("fails closed when both auth tables are absent", () => {
    const sourceStateDir = createStateDir("afora-live-auth-legacy-source-");
    const targetStateDir = createStateDir("afora-live-auth-legacy-target-");
    const sourceAgentDir = createAuthSource(sourceStateDir);
    const database = new DatabaseSync(resolveAuthProfileDatabasePath(sourceAgentDir));
  ...
  it("stages a readable store when the state row is absent", () => {
    const sourceStateDir = createStateDir("afora-live-auth-row-source-");
    const targetStateDir = createStateDir("afora-live-auth-row-target-");
    const sourceAgentDir = createAuthSource(sourceStateDir);
    const database = new DatabaseSync(resolveAuthProfileDatabasePath(sourceAgentDir));
