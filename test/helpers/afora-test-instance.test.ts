// Afora test instance tests cover spawned test instance lifecycle.
...
import { createAforaTestInstance, testing } from "./afora-test-instance.js";
...
const fakeInstances: Awaited<ReturnType<typeof createAforaTestInstance>>[] = [];
...
async function createFakeGateway(sequence: string, startTimeoutMs = 1_000, stopTimeoutMs = 1_500) {
  const cwd = await fs.mkdtemp(path.join(tmpdir(), "afora-test-instance-gateway-"));
  fakeRoots.push(cwd);
...
const tracePath = process.env.AFORA_FAKE_GATEWAY_TRACE;
...
const env = Object.fromEntries(["HOME", "AFORA_CONFIG_PATH", "AFORA_GATEWAY_TOKEN", "AFORA_STATE_DIR"].map((key) => [key, process.env[key]]));
appendFileSync(tracePath, JSON.stringify({ argv, config: JSON.parse(readFileSync(process.env.AFORA_CONFIG_PATH, "utf8")), cwd: process.cwd(), env, pid: process.pid, port }) + "\\n");
const action = (process.env.AFORA_FAKE_GATEWAY_SEQUENCE || "ready").split(",")[attempt - 1] || "ready";
...
`,
  ...
  const instance = await createAforaTestInstance({
    ...
    cwd,
    env: {
      AFORA_FAKE_GATEWAY_SEQUENCE: sequence,
      AFORA_FAKE_GATEWAY_TRACE: tracePath,
    },
    startTimeoutMs,
    stopTimeoutMs,
  });
...
describe("afora test instance", () => {
  ...
  it("signals test instance process groups on POSIX", () => {
    ...
    testing.signalAforaTestProcess(child, "SIGKILL", killProcess);
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".afora"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "afora.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.AFORA_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.AFORA_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.AFORA_SKIP_CRON).toBe("0");
