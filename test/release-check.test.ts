...
import { bundledDistPluginFile, bundledPluginFile } from "afora-agent/plugin-sdk/test-fixtures";
...
} from "../scripts/afora-npm-postpublish-verify.ts";
import {
...
describe("packed CLI smoke", () => {
  ...
  it("builds a packed CLI smoke env with packaged-install guardrails", () => {
    expect(
      createPackedCliSmokeEnv(
        {
          ...
          AFORA_CONFIG_PATH: "/tmp/leaky-config.json",
        },
        { HOME: "/tmp/smoke-home", AFORA_STATE_DIR: "/tmp/smoke-state" },
      ),
    ).toEqual({
      PATH:
      ...
      AFORA_STATE_DIR: "/tmp/smoke-state",
    });
  });
  ...
  it("skips plugin command discovery during packed completion cache smoke", () => {
    expect(
      createPackedCompletionSmokeEnv(
        {
          PATH: "/usr/bin",
          AFORA_COMPLETION_SKIP_PLUGIN_COMMANDS: "0",
        },
        {
          HOME: "/tmp/smoke-home",
          AFORA_STATE_DIR: "/tmp/smoke-state",
...
describe("runReleaseCheckCommand", () => {
  ...
  it("rejects malformed command limit environment values", () => {
    withProcessEnv({ AFORA_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "1e3" }, () => {
      expect(() =>
        runReleaseCheckCommand(
          { command: process.execPath, args: ["--eval", "process.stdout.write('ok')"] },
          { stdio: ["ignore", "pipe", "pipe"] },
        ),
      ).toThrow("invalid AFORA_RELEASE_CHECK_COMMAND_TIMEOUT_MS: 1e3");
    });

    withProcessEnv({ AFORA_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES: "16mb" }, () => {
      expect(() =>
        runReleaseCheckCommand(
          { command: process.execPath, args: ["--eval", "process.stdout.write('ok')"] },
          { stdio: ["ignore", "pipe", "pipe"] },
        ),
      ).toThrow("invalid AFORA_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES: 16mb");
    });
  });
});
...
describe("resolveReleaseNpmCommand", () => {
  ...
  it("rejects bare npm fallback on Windows release checks", () => {
    ...
    ).toThrow("Afora refuses to shell out to bare npm on Windows");
  });
});
...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | afora.install.npmSpec must be a non-empty string",
    ]);
  });
      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            afora: {
              install: { npmSpec: "@afora/broken", minHostVersion: "2026.3.14" },
            },
          },
        },
      ]),
    ).toEqual([
      "bundled extension 'broken' manifest invalid | afora.install.minHostVersion must use a semver floor in the form \">=x.y.z[-prerelease][+build]\"",
    ]);
  });
  ...
  it("flags non-object install metadata instead of throwing", () => {
    ...
    ).toEqual(["bundled extension 'broken' manifest invalid | afora.install must be an object"]);
  });
});
...
describe("bundled plugin package dependency checks", () => {
  ...
  it("does not require root deps for root chunks sourced from the owning installed plugin", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "afora-root-owned-installed-"));

    try {
      mkdirSync(join(tempRoot, "dist", "extensions", "memory-lancedb"), { recursive: true });
      writeFileSync(
        join(tempRoot, "package.json"),
        `{"name":"afora","dependencies":{}}\n`,
        "utf8",
      );
      writeFileSync(
        join(tempRoot, "dist", "extensions", "memory-lancedb", "package.json"),
        `{"name":"@afora/memory-lancedb","dependencies":{"root-owned-test-dep":"^1.0.0"}}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("still requires root deps for root-owned installed chunks", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "afora-root-owned-installed-missing-"));
...
describe.skipIf(process.platform === "win32")("collectSkillShellScriptExecutableErrors", () => {
  it("flags non-executable shell scripts under skills/*/scripts", () => {
    const root = mkdtempSync(join(tmpdir(), "afora-release-check-"));
...
describe("collectForbiddenPackPaths", () => {
  ...
  it("blocks Docker-selected external plugin trees from the core npm pack", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "dist/extensions/msteams/afora.plugin.json",
      ]),
    ).toEqual([
      "dist/extensions/clickclack/index.js",
      "dist/extensions/msteams/afora.plugin.json",
  ...
  it("blocks legacy runtime dependency stamps from npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        "dist/index.js",
        "dist/extensions/browser/.Afora-Install-Stage/package.json",
        "dist/extensions/codex/.afora-runtime-deps-backup-node_modules-old/zod/index.js",
        "dist/extensions/discord/.afora-runtime-deps-stamp.json",
      ]),
    ).toEqual([
      "dist/extensions/browser/.Afora-Install-Stage/package.json",
      "dist/extensions/codex/.afora-runtime-deps-backup-node_modules-old/zod/index.js",
  ...
  it("blocks root dist chunks that still reference private qa lab sources", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "afora-release-private-qa-"));
  ...
  it("blocks private QA paths in the generated dist inventory", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "afora-release-inventory-"));
  ...
  it("blocks root plugin SDK declarations that still reference private test helpers", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "afora-release-private-sdk-"));
...
describe("collectMissingPackPaths", () => {
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    for (const path of [
      ...
      bundledDistPluginFile("telegram", "afora.plugin.json"),
  ...
  it("runs postpublish package integrity checks against the packed install before publish", () => {
    ...
    try {
      const packageRoot = join(root, "afora");
      ...
      writeFileSync(
        ...
        `${JSON.stringify({ name: "afora", version: "2026.5.14-beta.3", dependencies: {} })}\n`,
      );
      ...
      expect(
        collectPackedInstalledPackageVerificationErrors({
          ...
          installedBinaryVersion: "afora 2026.5.14-beta.3",
          packageRoot,
        }),
      ).toEqual([
  ...
  it("writes a consumer project that imports representative public SDK subpaths", () => {
    ...
    try {
      ...
      createPackedPluginSdkTypescriptSmokeProject({
        ...
        aiPackageSpec: "file:/tmp/afora-ai.tgz",
      });
      ...
      expect(packageJson.dependencies?.afora).toBe(`file:${packageRoot}`);
      expect(packageJson.dependencies?.["@afora/ai"]).toBe("file:/tmp/afora-ai.tgz");
      expect(tsconfig.compilerOptions?.skipLibCheck).toBe(true);
      expect(source).toBe(fixtureSource);
      expect(source).toContain('"afora-agent/plugin-sdk/core"');
      expect(source).toContain('"afora-agent/plugin-sdk/plugin-entry"');
      expect(source).toContain('"afora-agent/plugin-sdk/channel-entry-contract"');
      expect(source).toContain('"afora-agent/plugin-sdk/config-contracts"');
      expect(source).toContain('"afora-agent/plugin-sdk/runtime-env"');
      expect(source).toContain("type PublicPluginSdkModules = [");
      expect(source).not.toContain("TelegramAccountConfig");
      expect(source).not.toContain("afora-agent/plugin-sdk/channel-contract-testing");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("afora-2026.3.14.tgz", 120_354_302)]),
    ).toStrictEqual([]);
  });
  ...
  it("accepts npm 12 name-keyed pack results", () => {
    expect(
      collectPackUnpackedSizeErrors({
        afora: makePackResult("afora-2026.3.14.tgz", 120_354_302),
      }),
    ).toStrictEqual([]);
  });
  ...
  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("afora-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "afora-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 211812352 bytes (202.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "afora-2026.3.14.tgz" },
        { filename: "afora-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
...
describe("resolveNpmJsonEntries", () => {
  it("normalizes npm <=11 arrays and npm 12 name-keyed objects", () => {
    const entry = makePackResult("afora-2026.7.2.tgz", 120_354_302);

    expect(resolveNpmJsonEntries([entry])).toEqual([entry]);
    expect(resolveNpmJsonEntries(entry)).toEqual([entry]);
    expect(resolveNpmJsonEntries({ afora: entry })).toEqual([entry]);
    expect(resolveNpmJsonEntries({ "@afora/demo": entry })).toEqual([entry]);
    expect(resolveNpmJsonEntries({ afora: entry })).toEqual(
      resolveRuntimeNpmJsonEntries({ afora: entry }),
    );
  });
});
...
describe("resolvePackedTarballPath", () => {
  it("resolves one local npm pack tarball filename inside the pack destination", () => {
    expect(
      resolvePackedTarballPath("/tmp/afora-pack", [{ filename: "afora-2026.6.17.tgz" }]),
    ).toBe(resolvePath("/tmp/afora-pack", "afora-2026.6.17.tgz"));
    expect(
      resolvePackedTarballPath("/tmp/afora-pack", [
        { filename: "/tmp/afora-pack/afora-2026.6.17.tgz" },
      ]),
    ).toBe(resolvePath("/tmp/afora-pack", "afora-2026.6.17.tgz"));
  });
  ...
  it("rejects path-like npm pack tarball filenames", () => {
    ...
    for (const filename of unsafeFilenames) {
      expect(() => resolvePackedTarballPath("/tmp/afora-pack", [{ filename }])).toThrow(
