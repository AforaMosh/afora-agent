...
function writePublishablePluginPackage(repoDir: string): string {
  ...
  writeJsonFile(join(packageDir, "afora.plugin.json"), { id: "diffs" });
...
describe("plugin npm package manifest staging", () => {
  it("keeps msteams runtime dependencies registry-installed", () => {
    ...
    ) as { afora?: { release?: { bundleRuntimeDependencies?: boolean } } };

    expect(packageJson.afora?.release?.bundleRuntimeDependencies).toBe(false);
  });
  ...
  it("cleans an exhausted timeout before reusing the same package directory", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "afora-plugin-npm-timeout-");
  ...
  it("retries timed-out package-lock generation with a bounded command timeout", () => {
    ...
    expect(generateOptions[0]).toMatchObject({
      env: { AFORA_NPM_LOCK_COMMAND_TIMEOUT_MS: "180000" },
      installStrategy: "shallow",
    });
  ...
  it("overlays generated channel configs while packing and restores source manifest", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "afora-plugin-npm-package-manifest-");
    ...
    writeJsonFile(join(packageDir, "afora.plugin.json"), sourceManifest);
    ...
    const originalText = readFileSync(join(packageDir, "afora.plugin.json"), "utf8");
    withAugmentedPluginNpmManifestForPackage({ repoRoot: repoDir, packageDir }, () => {
      const stagedManifest = JSON.parse(
        readFileSync(join(packageDir, "afora.plugin.json"), "utf8"),
      );
      expect(stagedManifest.channelConfigs.twitch.description).toBe("Twitch chat integration");
    });
    expect(readFileSync(join(packageDir, "afora.plugin.json"), "utf8")).toBe(originalText);
  });

  it("overlays package-local runtime metadata while packing and restores source package json", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "afora-plugin-npm-package-runtime-");
    ...
    sourcePackageJson.afora.channel = {
      id: "diffs",
      configuredState: {
    ...
    expect(resolved.packageJson).toEqual({
      ...
      type: "module",
      bundledDependencies: [],
      files: ["dist/**", "afora.plugin.json", "README.md", "SKILL.md", "skills/**"],
      peerDependencies: {
        afora: ">=2026.4.30",
      },
    ...
    withAugmentedPluginNpmManifestForPackage(
      ...
      () => {
        ...
        expect(stagedPackageJson.afora.extensions).toEqual(["./index.ts"]);
        expect(stagedPackageJson.afora.runtimeExtensions).toEqual(["./dist/index.js"]);
        expect(stagedPackageJson.afora.setupEntry).toBe("./dist/setup-entry.js");
        expect(stagedPackageJson.afora.runtimeSetupEntry).toBe("./dist/setup-entry.js");
        expect(stagedPackageJson.afora.channel.configuredState.specifier).toBe(
        ...
        expect(stagedPackageJson.peerDependencies.afora).toBe(">=2026.4.30");
        expect(stagedPackageJson.peerDependenciesMeta.afora.optional).toBe(true);
      },
    );
  ...
  it("packs and loads both mapped channel-state probes from one package artifact", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "afora-plugin-npm-package-state-runtime-");
    const packageDir = writePublishablePluginPackage(repoDir);
    const sourcePackageJson = JSON.parse(readFileSync(join(packageDir, "package.json"), "utf8"));
    sourcePackageJson.afora.build = { runtimeFormat: "cjs" };
    sourcePackageJson.afora.channel = {
      id: "diffs",
      configuredState: {
        specifier: "./configured-state",
    ...
    withAugmentedPluginNpmManifestForPackage({ repoRoot: repoDir, packageDir }, () => {
      ...
      expect(stagedPackageJson.afora.channel.configuredState).toEqual({
        specifier: "./dist/configured-state.cjs",
        exportName: "hasConfiguredChannelState",
      });
      expect(stagedPackageJson.afora.channel.persistedAuthState).toEqual({
        specifier: "./dist/auth-presence.cjs",
        exportName: "hasPersistedChannelAuth",
      });
...
for (const key of ["configuredState", "persistedAuthState"]) {
  const state = pkg.afora.channel[key];
...
`,
  ...
  it("stages portable bundled dependencies without polluting pack output", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "afora-plugin-npm-package-portable-optional-");
    const packageDir = writePublishablePluginPackage(repoDir);
    writeFileText(join(packageDir, "dist", "index.js"), "export {};\n");
    writeFileText(join(packageDir, "dist", "setup-entry.js"), "export {};\n");
...
`;
  ...
  it("honors plugin package opt-out for bundled runtime dependencies", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "afora-plugin-npm-package-bundle-opt-out-");
    const packageDir = writePublishablePluginPackage(repoDir);
    writeFileText(join(packageDir, "dist", "index.js"), "export {};\n");
    writeFileText(join(packageDir, "dist", "setup-entry.js"), "export {};\n");
    writeLocalDependencyPackage(packageDir);
  ...
  it("refuses to pack publishable plugins before package-local runtime files exist", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "afora-plugin-npm-package-runtime-missing-");
    const packageDir = writePublishablePluginPackage(repoDir);
  ...
  it("refuses package file rules that omit advertised package-local runtime files", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "afora-plugin-npm-package-runtime-excluded-");
    const packageDir = writePublishablePluginPackage(repoDir);
    writeFileText(join(packageDir, "dist", "index.js"), "export {};\n");
    writeFileText(join(packageDir, "dist", "setup-entry.js"), "export {};\n");
    writeJsonFile(join(packageDir, "package.json"), {
