// Afora prepack tests validate package prepack output.
...
function runStandaloneBundledChannelSmoke(entrySource: string) {
  const rootDir = tempDirs.make("afora-prepack-standalone-smoke-");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      name: "@afora/fixture-channel",
      afora: { channel: true, extensions: ["./index.ts"] },
    })}\n`,
  );
  ...
  return spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        AFORA_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT: "1",
      },
...
describe("collectSourcePackWorkspaceDependencyErrors", () => {
  it("rejects the plain source pack that pnpm rewrites without bundling @afora/ai", () => {
    const rootDir = tempDirs.make("afora-source-pack-workspace-");
    ...
    const rootPackageJson = {
      ...
      name: "afora-source-pack-regression",
      version,
    };
    ...
    writeFileSync(
      ...
      `${JSON.stringify({ name: "@afora/ai", version }, null, 2)}\n`,
    );
    ...
    expect(packedPackageJson.dependencies?.["@afora/ai"]).toBe(version);
    expect(packedPackageJson.bundleDependencies).toBeUndefined();
    expect(existsSync(path.join(extractDir, "package", "node_modules", "@afora", "ai"))).toBe(
      false,
    );
    ...
    expect(collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {})).toEqual([
      "plain root packing cannot safely resolve @afora/ai from workspace:*: pnpm rewrites the workspace dependency to an exact version without bundling the package",
      "use `node scripts/package-afora-for-docker.mjs --allow-unreleased-changelog` for a self-contained source package; official npm release automation prepares and publishes @afora/ai separately",
    ]);
    expect(
    ...
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        ...
        AFORA_OCM_WORKSPACE_DEPENDENCY_DIRS: aiDir,
      }),
    ).toEqual([]);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        npm_command: "pack",
        OCM_INTERNAL_NPM_BIN: path.join(rootDir, "scripts", "ocm-npm-workspace-deps.mts"),
        AFORA_OCM_WORKSPACE_DEPENDENCY_DIRS: rootDir,
      }),
    ).toHaveLength(2);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
  ...
  it("omits build-only workspace dependencies from direct pnpm pack manifests", () => {
    const rootDir = tempDirs.make("afora-direct-pack-manifest-");
    const packDir = path.join(rootDir, "pack");
    const extractDir = path.join(rootDir, "extract");
    const scriptsDir = path.join(rootDir, "scripts");
    const originalPackageJson = `${JSON.stringify(
      {
        name: "afora-direct-pack-manifest",
...
describe("resolvePrepackAllowUnreleasedChangelog", () => {
  it("requires an explicit non-publish opt-in", () => {
    for (const raw of [undefined, "", "0", "false"]) {
      expect(
        resolvePrepackAllowUnreleasedChangelog({
          AFORA_PREPACK_ALLOW_UNRELEASED_CHANGELOG: raw,
        }),
      ).toBe(false);
    }
    ...
    expect(() =>
      resolvePrepackAllowUnreleasedChangelog({
        AFORA_PREPACK_ALLOW_UNRELEASED_CHANGELOG: "yes",
      }),
    ).toThrow("invalid AFORA_PREPACK_ALLOW_UNRELEASED_CHANGELOG: yes");
  });
});
...
describe("resolvePrepackBuildEnvironment", () => {
  it("pins one timestamp across package and Control UI builds", () => {
    ...
    expect(
      ...
      GIT_COMMIT: commit,
      AFORA_BUILD_TIMESTAMP: "2026-07-10T12:34:56.000Z",
    });
    expect(
      resolvePrepackBuildEnvironment(
        { AFORA_BUILD_TIMESTAMP: "2026-07-10T01:02:03.7Z" },
...
describe("resolvePrepackCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(resolvePrepackCommandTimeoutMs({ AFORA_PREPACK_COMMAND_TIMEOUT_MS: "" })).toBe(
      30 * 60 * 1000,
    );
    expect(resolvePrepackCommandTimeoutMs({ AFORA_PREPACK_COMMAND_TIMEOUT_MS: "1234" })).toBe(
      1234,
    );

    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolvePrepackCommandTimeoutMs({ AFORA_PREPACK_COMMAND_TIMEOUT_MS: raw }),
      ).toThrow(`invalid AFORA_PREPACK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
