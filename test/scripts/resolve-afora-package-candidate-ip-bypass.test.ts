// Resolve Afora Package Candidate Ip Bypass tests cover resolve afora package candidate ip bypass script behavior.
import { mkdtemp, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import path from "node:path";
import { afterEach, describe, expect, it } from "vitest";
import { downloadUrl } from "../../scripts/resolve-afora-package-candidate.mts";
...
describe("package URL IPv6 transition address blocking", () => {
  it.each([
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-ip-bypass-"));
    tempDirs.push(dir);
  ...
  it.each([
    ["IPv4-mapped loopback dotted", `https://[::ffff:${dotted(127, 0, 0, 1)}]/afora.tgz`],
    ["IPv4-compatible loopback dotted", `https://[::${dotted(127, 0, 0, 1)}]/afora.tgz`],
