...
function createDistFixture() {
  const distDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-control-ui-performance-"));
...
describe("Control UI performance budgets", () => {
  ...
  it("updates the baseline from local or explicit CI metrics", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-control-ui-budget-cli-"));
