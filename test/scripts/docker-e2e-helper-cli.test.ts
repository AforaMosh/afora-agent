...
describe("Docker E2E helper CLIs", () => {
  ...
  it("emits prerelease plugin registry planning outputs", () => {
    const root = tempDirs.make("afora-docker-e2e-helper-plan-");
    const file = path.join(root, "plan.json");
    writeFileSync(
      file,
      `${JSON.stringify({
        requiredPrepublishPluginPackages: ["@afora/discord", "@afora/feishu"],
    ...
    expect(result.stdout).toContain(
      'required_prepublish_plugin_packages=["@afora/discord","@afora/feishu"]',
    );
  });

  it("rejects oversized scheduler helper JSON artifacts without a Node stack trace", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-helper-`);
    try {
      ...
      const result = runHelper("scripts/docker-e2e.mjs", "failed-reruns", file, {
        AFORA_DOCKER_E2E_JSON_ARTIFACT_MAX_BYTES: "64",
      });
  ...
  it("rejects oversized timing JSON artifacts without a Node stack trace", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-timings-`);
    try {
      const file = path.join(root, "summary.json");
      writeFileSync(file, `${JSON.stringify({ filler: "x".repeat(128) })}\n`, "utf8");
  ...
  it("rejects oversized rerun JSON artifacts without a Node stack trace", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-`);
    try {
      const file = path.join(root, "summary.json");
      writeFileSync(file, `${JSON.stringify({ filler: "x".repeat(128) })}\n`, "utf8");
  ...
  it("ignores artifact-provided GitHub rerun commands", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-command-`);
    try {
  ...
  it.each([
    ...
    (name, refData) => {
      const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-ref-`);
      try {
  ...
  it("lets an explicit target ref override artifact refs", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-ref-override-`);
    try {
      ...
      writeFileSync(
        file,
        `${JSON.stringify({
          images: { bare: "ghcr.io/AforaMosh/afora-agent-bare:artifact-a" },
          lanes: [{ name: "gateway-network", status: 1 }],
          ref: artifactRef,
          status: "failed",
        })}\n`,
  ...
  it("requires an artifact target ref when no explicit ref is supplied", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-ref-missing-`);
    try {
      const file = path.join(root, "failures.json");
      writeFileSync(
        file,
  ...
  it("rejects a non-exact artifact target for a targetable rerun", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-ref-artifact-invalid-`);
    try {
      const file = path.join(root, "failures.json");
      writeFileSync(
        file,
  ...
  it.each(["abc123", "A".repeat(40), "main"])(
    ...
    (explicitRef) => {
      const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-ref-invalid-`);
      try {
        const file = path.join(root, "failures.json");
        writeFileSync(
          file,
  ...
  it("preserves declared rerun inputs but ignores package and workflow refs", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-inputs-`);
    try {
      const file = path.join(root, "failures.json");
      writeFileSync(
        file,
        `${JSON.stringify(
          {
            lanes: [
              {
                ghWorkflowCommand:
                  "gh workflow run 'afora-live-and-e2e-checks-reusable.yml' --ref 'full-release-validation-temp-deleted' -f package_artifact_run_id='12345' -f package_artifact_name='docker-e2e-package' -f docker_e2e_bare_image='ghcr.io/AforaMosh/afora-agent-bare:test' -f published_upgrade_survivor_baselines='afora@2026.5.3' -f published_upgrade_survivor_scenarios='plugin-dependency-cleanup' -f allow_unreleased_changelog=true -f unsafe_input='do-not-copy'",
                name: "published-upgrade-survivor-afora-2026-5-3",
                status: 1,
              },
            ],
      ...
      expect(combinedCommand).toContain(
        "docker_e2e_bare_image='ghcr.io/AforaMosh/afora-agent-bare:test'",
      );
      expect(combinedCommand).toContain("shared_image_policy=existing-only");
      expect(combinedCommand).toContain("published_upgrade_survivor_baselines='afora@2026.5.3'");
      expect(combinedCommand).toContain(
      ...
      expect(result.stdout).toContain("published_upgrade_survivor_baselines='afora@2026.5.3'");
      expect(result.stdout).toContain(
        "published_upgrade_survivor_scenarios='plugin-dependency-cleanup'",
      );
  ...
  it("rejects non-boolean unreleased changelog intent from summary artifacts", () => {
    const root = tempDirs.make("afora-docker-e2e-rerun-inputs-");
    const file = path.join(root, "summary.json");
    writeFileSync(
      file,
      `${JSON.stringify({
  ...
  it("groups combined reruns by recovered workflow inputs", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-groups-`);
    try {
      const file = path.join(root, "failures.json");
      writeFileSync(
        file,
        `${JSON.stringify(
          {
            lanes: [
              {
                ghWorkflowCommand:
                  "gh workflow run 'afora-live-and-e2e-checks-reusable.yml' --ref 'release/2026.6' -f published_upgrade_survivor_baselines='afora@2026.5.3' -f allow_unreleased_changelog=1",
                name: "published-upgrade-survivor-afora-2026-5-3",
                status: 1,
              },
              {
                ghWorkflowCommand:
                  "gh workflow run 'afora-live-and-e2e-checks-reusable.yml' --ref 'release/2026.6' -f published_upgrade_survivor_baselines='afora@2026.5.2'",
                name: "published-upgrade-survivor-afora-2026-5-2",
                status: 1,
              },
            ],
            status: "failed",
      ...
      expect(result.stdout).toContain(
        "- published-upgrade-survivor-afora-2026-5-3: gh workflow run",
      );
      expect(result.stdout).toContain(
        "- published-upgrade-survivor-afora-2026-5-2: gh workflow run",
      );
      expect(result.stdout).toContain(
        "docker_lanes='published-upgrade-survivor-afora-2026-5-3'",
      );
      expect(result.stdout).toContain(
        "docker_lanes='published-upgrade-survivor-afora-2026-5-2'",
      );
      expect(result.stdout).not.toContain(
        "docker_lanes='published-upgrade-survivor-afora-2026-5-3 published-upgrade-survivor-afora-2026-5-2'",
      );
      expect(result.stdout).not.toContain("allow_unreleased_changelog");
    } finally {
      rmSync(root, { force: true, recursive: true });
  ...
  it("merges duplicate lane entries before printing reruns", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-merge-`);
    try {
      const file = path.join(root, "failures.json");
      writeFileSync(
        file,
        `${JSON.stringify(
          {
            lanes: [
              ...
              {
                ghWorkflowCommand:
                  "gh workflow run 'afora-live-and-e2e-checks-reusable.yml' --ref 'release/2026.6' -f published_upgrade_survivor_baselines='afora@2026.5.3'",
                name: "published-upgrade-survivor-afora-2026-5-3",
                status: 1,
              },
            ],
  ...
  it("downloads GitHub run artifacts into distinct default directories", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-gh-`);
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "    headSha: 'abc123',",
          "    status: 'completed',",
          "    url: 'https://github.com/AforaMosh/afora-agent/actions/runs/12345',",
          "    workflowName: 'Afora Live and E2E Checks',",
          "  }));",
          "  process.exit(0);",
          "}",
          "if (args[0] === 'api') {",
      ...
      expect(path.basename(firstDir)).toMatch(/^afora-docker-e2e-rerun-12345-/u);
      expect(path.basename(secondDir)).toMatch(/^afora-docker-e2e-rerun-12345-/u);
  ...
  it("isolates files with the same name across downloaded artifacts", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-collisions-`);
    try {
      const binDir = path.join(root, "bin");
  ...
  it("fails closed when downloaded artifacts contain mixed target refs", () => {
    const root = mkdtempSync(`${tmpdir()}/afora-docker-e2e-rerun-mixed-refs-`);
    try {
      const binDir = path.join(root, "bin");
      const outputDir = path.join(root, "artifacts");
      const ghPath = path.join(binDir, "gh");
