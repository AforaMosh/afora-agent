...
const CROSS_OS_RELEASE_CHECKS_REUSABLE_WORKFLOW =
  ".github/workflows/afora-cross-os-release-checks-reusable.yml";
const LIVE_MEDIA_RUNNER_DOCKERFILE = ".github/images/live-media-runner/Dockerfile";
const LIVE_MEDIA_RUNNER_IMAGE = "ghcr.io/AforaMosh/afora-agent-live-media-runner:ubuntu-24.04";
...
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/afora-release-checks.yml";
const RELEASE_TELEGRAM_QA_WORKFLOW = ".github/workflows/afora-release-telegram-qa.yml";
const RELEASE_PUBLISH_WORKFLOW = ".github/workflows/afora-release-publish.yml";
const PLUGIN_PRERELEASE_WORKFLOW = ".github/workflows/plugin-prerelease.yml";
const AFORA_NPM_RELEASE_WORKFLOW = ".github/workflows/afora-npm-release.yml";
...
const STABLE_MAIN_CLOSEOUT_WORKFLOW = ".github/workflows/afora-stable-main-closeout.yml";
const WINDOWS_NODE_RELEASE_WORKFLOW = ".github/workflows/windows-node-release.yml";
const FULL_RELEASE_VALIDATION_WORKFLOW = ".github/workflows/full-release-validation.yml";
const CI_WORKFLOW = ".github/workflows/ci.yml";
const PERFORMANCE_WORKFLOW = ".github/workflows/afora-performance.yml";
const FULL_RELEASE_CHILD_DISPATCHES = [
  ...
  },
  {
    ...
    kind: "performance",
    nonceSuffix: "",
    runName: "Afora Performance",
    stepName: "Dispatch and monitor Afora Performance",
...
const RELEASE_MAINTAINER_SKILL = resolve(
  REPO_ROOT,
  ".agents/skills/release-afora-maintainer/SKILL.md",
);
...
const SCHEDULED_LIVE_CHECKS_WORKFLOW = ".github/workflows/afora-scheduled-live-checks.yml";
...
const VERIFY_PROVIDER_SECRETS_SCRIPT =
  ".agents/skills/release-afora-ci/scripts/verify-provider-secrets.mjs";
...
function createReleaseChecksContextFixture() {
  ...
  git("config", "user.email", "afora-test@example.com");
...
const url = "https://github.com/AforaMosh/afora-agent/actions/runs/101";
function nextStatus() {
...
`,
  ...
  const defaultJobs = [
    {
      conclusion: "success",
      html_url: "https://github.com/AforaMosh/afora-agent/actions/runs/101/job/201",
      name: "Verify release checks",
      status: "completed",
      url: "https://github.com/AforaMosh/afora-agent/actions/runs/101/job/201",
    },
  ];
...
function runNpmTelegramInputValidation(overrides: Record<string, string>) {
  ...
  return spawnSync("bash", ["-c", script], {
    encoding: "utf8",
    env: {
      AFORA_QA_CONVEX_SECRET_CI: "test-secret",
      AFORA_QA_CONVEX_SITE_URL: "https://example.invalid",
...
function runAforaNpmTrustedRefGuard(overrides: Record<string, string>) {
  const job = workflowJob(AFORA_NPM_RELEASE_WORKFLOW, "validate_publish_request");
  const script = workflowStep(job, "Require trusted workflow ref for publish").run;
  if (!script) {
    throw new Error("Expected Afora npm trusted ref guard");
  }
  const binDir = tempDirs.make("afora-npm-trusted-ref-");
...
function runReleaseChecksSummary(params: {
  ...
  const workdir = tempDirs.make("afora-release-check-status-");
...
describe("package acceptance workflow", () => {
  it("forwards Plugin SDK acknowledgement through the canonical publish dispatch", () => {
    ...
    const downloadPreflight = workflowStep(resolveJob, "Download Afora npm preflight manifest");
    const validateEvidence = workflowStep(resolveJob, "Validate Afora npm preflight manifest");
    ...
    expect(readFileSync(RELEASE_PUBLISH_WORKFLOW, "utf8")).toContain(
      "group: afora-release-publish-${{ inputs.npm_dist_tag }}",
    );
    ...
    expect(validateEvidence.run).toContain('npm view "afora@${RELEASE_NPM_DIST_TAG}" version');
    ...
    expect(downloadPreflight.run).toContain(
      'preflight_path" != ".github/workflows/afora-npm-release.yml"',
    );
  ...
  it("requires selected plugin names or complete immutable evidence for broad publication", () => {
    ...
    expect(runReleasePublishInputValidation({ PUBLISH_AFORA_NPM: "false" }).status).toBe(0);

    const invalidResumeRun = runReleasePublishInputValidation({
      AFORA_NPM_RESUME_RUN_ID: "not-a-run-id",
    });
    expect(invalidResumeRun.status).toBe(1);
    expect(invalidResumeRun.stderr).toContain(
      "afora_npm_resume_run_id must be a positive GitHub Actions run id",
    );
  });
  ...
  it("allows Docker-only recovery for beta and extended-stable releases", () => {
    ...
    expect(verifyStep.run).toContain('npm view "afora@${RELEASE_NPM_DIST_TAG}" version');
    expect(verifyStep.run).not.toContain("npm view afora@extended-stable version");
  });
  ...
  it("allows protected SHA-pinned tooling tags through the core npm publish guard", () => {
    ...
    const valid = runAforaNpmTrustedRefGuard({
      WORKFLOW_REF: protectedRef,
      WORKFLOW_SHA: workflowSha,
    });
    expect(valid.status, valid.stderr).toBe(0);

    const mismatchedName = runAforaNpmTrustedRefGuard({
      WORKFLOW_REF: `refs/tags/release-publish/${"b".repeat(12)}-123`,
      WORKFLOW_SHA: workflowSha,
    });
    expect(mismatchedName.status).toBe(1);
    expect(mismatchedName.stderr).toContain(
      "SHA-pinned release-publish tag does not match the Afora npm workflow SHA",
    );

    const unreachable = runAforaNpmTrustedRefGuard({
    ...
    expect(unreachable.status).toBe(1);
    expect(unreachable.stderr).toContain(
      "SHA-pinned Afora npm workflow revision is not reachable from current main",
    );
  });
  ...
  it("resolves broad release evidence and exact-binds every publish child", () => {
    ...
    for (const stepName of [
      "Download Afora npm preflight manifest",
    ...
    for (const stepName of [
      ...
      expect(workflowStep(publishJob, stepName).if).toContain("inputs.publish_afora_npm");
    }
    ...
    expectTextToIncludeAll(publishOrchestration.run, [
      ...
      'wait_for_run_background afora-npm-release.yml "${afora_npm_run_id}" "${PARENT_WORKFLOW_SHA}"',
  ...
  it("verifies immutable postpublish evidence before stable closeout reads it", () => {
    ...
    expect(workflow).toContain("verify_checksum_manifest Afora-Android-SHA256SUMS.txt");
    expect(workflow).toContain("verify_checksum_manifest AforaCompanion-SHA256SUMS.txt");
    ...
    expect(workflow).toContain(
      'awk -v asset="afora-${release_version}-stable-main-closeout.json"',
    );
    ...
    expect(workflow).toContain(
      'release_manifest_asset="afora-${evidence_version}-release-manifest.json"',
    );
    ...
    expect(evidenceStep.run).toContain(
      'manifest_asset="afora-${evidence_version}-release-manifest.json"',
    );
  ...
  it("keeps Crabbox hydration compatible with local Actions replay", () => {
    ...
    expect(hydratePnpm.run).toContain(
      'pnpm_cache_root="${XDG_CACHE_HOME:-$HOME/.cache}/afora/pnpm"',
    );
    ...
    expect(ensureDocker.run).toContain(
      'echo "AFORA_CRABBOX_DOCKER_AVAILABLE=0" >> "$GITHUB_ENV"',
    );
    expect(ensureDocker.run).toContain(
      'echo "AFORA_CRABBOX_DOCKER_AVAILABLE=1" >> "$GITHUB_ENV"',
    );
    ...
    expect(markCrabboxReady).toContain("AFORA_CRABBOX_DOCKER_AVAILABLE");
    ...
    expect(hydrateWindowsPnpm.run).toContain('"afora",');
    expect(hydrateWindowsPnpm.run).toContain(
    ...
    expect(workflowText).toContain("AFORA_CRABBOX_HYDRATE_DOWNLOAD_TIMEOUT_SECONDS:-300");
    expect(workflowText).toContain("AFORA_CRABBOX_HYDRATE_DOWNLOAD_RETRIES:-3");
    ...
    expect(markHydrateGithubReady).toContain("AFORA_CRABBOX_DOCKER_AVAILABLE");
  ...
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-afora-package-candidate.mts");
    ...
    expect(workflow).toContain('node scripts/check-afora-package-tarball.mjs "$package"');
  ...
  it("keeps ref packaging independent of workflow-checkout dependencies", () => {
    ...
    expect(resolveJob).toContain("scripts/resolve-afora-package-candidate.mts");
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: afora@${{ needs.resolve_package.outputs.package_version }}",
    );
    ...
    expect(workflow).toContain('fallback_version="$(npm view afora@latest version)"');
  ...
  it("requires full release child workflows to run at the parent workflow SHA", () => {
    ...
    const performanceJob = workflowStep(
      ...
      "Dispatch and monitor Afora Performance",
    ).run;
    ...
    expect(releaseChecksWorkflow).toContain(
      'codex_plugin_spec="npm:@afora/codex@${BASH_REMATCH[1]}"',
    );
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || needs.resolve_target.outputs.release_package_spec || 'afora@beta' }}",
    );
  });
  ...
  it("keeps beta performance advisory at the publish gate", () => {
    ...
    const npmValidationStep = workflowStep(
      workflowJob(".github/workflows/afora-npm-release.yml", "publish_afora_npm"),
  ...
  it("adopts exact full-release child runs without retrying ambiguous dispatch posts", () => {
    ...
    for (const message of [
      ...
      "unexpected EOF",
      'Post "https://api.github.com/repos/AforaMosh/afora-agent/actions/workflows/ci.yml/dispatches": EOF',
      "EOF",
      "ETIMEDOUT",
      "ECONNRESET",
      "EAI_AGAIN",
    ...
    expect(readFileSync(RELEASE_CHECKS_WORKFLOW, "utf8")).toContain(
      "format('Afora Release Checks {0}', inputs.dispatch_id)",
    );
  ...
  it.each(FULL_RELEASE_CHILD_DISPATCHES)(
    ...
    (child) => {
      const { calls, result } = runFullReleaseChildDispatch(child, {
        MOCK_GH_DISPATCH_OUTPUT: "https://github.com/AforaMosh/afora-agent/actions/runs/101",
      });
  ...
  it.each([
    ...
    const { result } = runFullReleaseChildDispatch(FULL_RELEASE_CHILD_DISPATCHES[2], {
      ...
      MOCK_GH_JOBS: JSON.stringify([
        ...
        {
          conclusion: "failure",
          html_url: "https://github.com/AforaMosh/afora-agent/actions/runs/101/job/202",
          name: failingJob,
          status: "completed",
        },
      ]),
...
describe("package artifact reuse", () => {
  ...
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("AFORA_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "AFORA_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "AFORA_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("AFORA_UPGRADE_SURVIVOR_TARGET_ROOT: ${{ github.workspace }}");
    expect(workflow).toContain(
      "AFORA_ALLOW_FROZEN_TARGET_SCENARIO_OMISSIONS: ${{ inputs.allow_frozen_target_scenario_omissions && '1' || '0' }}",
    );
    expect(workflow).toContain("Download current-run Afora Docker E2E package");
    expect(workflow).toContain("Download previous-run Afora Docker E2E package");
    ...
    expect(workflow).toContain("AFORA_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflow).toContain("AFORA_PREPUBLISH_PLUGIN_REGISTRY_DIR");
    ...
    expect(workflow).toContain(
      "AFORA_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain('["AFORA_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["AFORA_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("AFORA_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("AFORA_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("AFORA_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain("AFORA_CLAWHUB_URL=%s");
    expect(publishedUpgradeSurvivor).toContain("assert-no-requests");
    expect(publishedUpgradeSurvivor).toContain(
      "env -u AFORA_GATEWAY_TOKEN -u AFORA_GATEWAY_PASSWORD afora",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("afora@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-afora"');
  ...
  it("reuses a content-addressed bare image for prepared E2E images", () => {
    ...
    expect(workflow).toContain("--build-context afora_package=.artifacts/docker-e2e-package");
  ...
  it("prepares one immutable candidate for release validation children", () => {
    ...
    expect(prepare.uses).toBe("./.github/workflows/afora-live-and-e2e-checks-reusable.yml");
  ...
  it("gives memory extension shards enough CPU without lowering their planner cost", () => {
    ...
    expect(workflow).toContain("AFORA_VITEST_MAX_WORKERS: ${{ matrix.vitest_max_workers }}");
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("AFORA_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("AFORA_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("AFORA_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("AFORA_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == matrix.suite_group");
    expect(workflow).toContain("AFORA_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6");
    expect(workflow).toContain("AFORA_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).toContain("suite_id: live-cli-cache-docker");
    expect(workflow).toContain("AFORA_LIVE_CLI_BACKEND_CACHE_PROBE=1");
    expect(workflow).toContain('live_image_extensions="matrix,acpx,anthropic"');
    expect(workflow).not.toContain("AFORA_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect(workflow).not.toContain('service_tier=\\"fast\\"');
    expect(workflow).not.toContain("AFORA_LIVE_CLI_BACKEND_ARGS=");
    expect(workflow).not.toContain("AFORA_LIVE_CLI_BACKEND_RESUME_ARGS=");
    expect(workflow).not.toContain(
      'AFORA_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: AFORA_LIVE_CODEX_HARNESS=1 AFORA_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-core",
    );
    expect(workflow).toContain(
      "command: AFORA_LIVE_CODEX_HARNESS=1 AFORA_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-backends",
    );
    expect(workflow).toContain("suite_id: native-live-src-infra");
    expect(workflow).toContain(
      "command: AFORA_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    expect(workflow).toContain("suite_id: native-live-src-gateway-profiles-anthropic-smoke");
    expect(workflow).toContain("AFORA_LIVE_GATEWAY_SETUP_TIMEOUT_MS=300000");
    ...
    expect(workflow).toContain("AFORA_LIVE_GATEWAY_MODELS=anthropic/claude-opus-5");
    ...
    expect(workflow).toContain(
      "command: AFORA_LIVE_GATEWAY_SETUP_TIMEOUT_MS=300000 AFORA_LIVE_GATEWAY_THINKING=off AFORA_LIVE_GATEWAY_PROVIDERS=openai AFORA_LIVE_GATEWAY_MODELS=openai/gpt-5.6-luna AFORA_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000 AFORA_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "AFORA_LIVE_GATEWAY_MODELS=google/gemini-3.1-pro-preview node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
    );
    expect(workflow).toContain(
      "AFORA_LIVE_GATEWAY_MODELS=minimax/MiniMax-M3,minimax-portal/MiniMax-M3 AFORA_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).not.toContain(
      "AFORA_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("AFORA_LIVE_GATEWAY_MAX_MODELS=2");
    expect(workflow).toContain(
      "AFORA_LIVE_GATEWAY_THINKING=off AFORA_LIVE_GATEWAY_PROVIDERS=openai AFORA_LIVE_GATEWAY_MODELS=openai/gpt-5.6-luna AFORA_LIVE_GATEWAY_MAX_MODELS=1 AFORA_LIVE_GATEWAY_STEP_TIMEOUT_MS=90000 AFORA_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "AFORA_LIVE_GATEWAY_MODELS=anthropic/claude-sonnet-4-6,anthropic/claude-haiku-4-5 AFORA_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain("AFORA_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000");
    ...
    expect(workflow).toContain("AFORA_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    expect(workflow).toContain("Advisory live suite failed with exit code");
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?AFORA_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}[\s\S]*?shell: bash[\s\S]*?Advisory live suite failed with exit code/u,
    );
    expect(workflow).toMatch(
      /suite_id: live-gateway-advisory-docker-deepseek-fireworks[\s\S]*?advisory: true/u,
    );
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?AFORA_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}/u,
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain("AFORA_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("AFORA_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    expect(workflow).toContain(
    ...
    expect(retryHelper).toContain("AFORA_LIVE_COMMAND_ATTEMPTS:-2");
    ...
    expect(retryHelper).toContain("AFORA_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS:-60");
  ...
  it("pins DeepSeek live profiles to both current V4 model refs", () => {
    ...
    expect(deepSeek).toMatchObject({
      advisory: true,
      command:
        "AFORA_LIVE_GATEWAY_PROVIDERS=deepseek AFORA_LIVE_GATEWAY_MODELS=deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
      profiles: "full",
    });
    expect(openCodeGo.command).toContain(
      "AFORA_LIVE_GATEWAY_MODELS=opencode-go/deepseek-v4-flash,opencode-go/deepseek-v4-pro",
    );
  });
  ...
  it("pins OpenCode Go MiMo live profiles to both current V2.5 model refs", () => {
    ...
    expect(mimo).toMatchObject({
      advisory: true,
      command:
        "AFORA_LIVE_GATEWAY_PROVIDERS=opencode-go AFORA_LIVE_GATEWAY_MODELS=opencode-go/mimo-v2.5,opencode-go/mimo-v2.5-pro node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
      profiles: "full",
  ...
  it("runs the fresh OpenAI API-key default without hard-coding a model filter", () => {
    ...
    expect(openaiDefault.command).toContain("AFORA_LIVE_GATEWAY_OPENAI_API_DEFAULT=1");
    expect(openaiDefault.command).toContain("AFORA_LIVE_GATEWAY_PROVIDERS=openai");
    expect(openaiDefault.command).not.toContain("AFORA_LIVE_GATEWAY_MODELS=");
  });
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: AFORA_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: AFORA_LIVE_GATEWAY_THINKING=off AFORA_LIVE_GATEWAY_PROVIDERS=openai AFORA_LIVE_GATEWAY_MODELS=openai/gpt-5.6-luna AFORA_LIVE_GATEWAY_MAX_MODELS=1",
    );
    expect(workflow).toContain(
      "command: AFORA_LIVE_GATEWAY_PROVIDERS=minimax,minimax-portal AFORA_LIVE_GATEWAY_MODELS=minimax/MiniMax-M3,minimax-portal/MiniMax-M3 AFORA_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain(
      'command: AFORA_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: AFORA_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: AFORA_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    ...
    expect(codexCompatibility).toMatchObject({
      id: "codex_compat",
      env: {
        AFORA_FROZEN_CODEX_SUITE_ID: "${{ matrix.suite_id }}",
        AFORA_FROZEN_TARGET_ROOT: "${{ github.workspace }}",
        AFORA_SELECTED_SHA: "${{ needs.validate_selected_ref.outputs.selected_sha }}",
        AFORA_WORKFLOW_SHA: "${{ needs.validate_selected_ref.outputs.workflow_sha }}",
      },
    ...
    for (const [model, thinking] of [
      ...
      expect(workflow).toContain(
        `AFORA_LIVE_CODEX_HARNESS_TARGETS=openai/gpt-5.6-${model}=${thinking}`,
      );
    }
    ...
    expect(workflow).toContain(
      'command: AFORA_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 20m bash .release-harness/scripts/test-live-subagent-announce-docker.sh',
    );
    ...
    expect(scenarios).toContain("process.env.AFORA_DOCKER_E2E_REPO_ROOT");
    expect(scenarios).toContain(
      'harness="\\${AFORA_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"',
    );
    ...
    expect(liveDockerAuth).toContain("afora_live_init_docker_run_args()");
    expect(liveDockerAuth).toContain("afora_live_stage_profile_into_home()");
    expect(liveDockerAuth).toContain("afora_live_chown_bind_dirs_for_container_user()");
    expect(liveDockerAuth).toContain("afora_live_uses_managed_bind_dirs()");
    expect(liveDockerAuth).toContain('afora_live_truthy "${AFORA_TESTBOX:-}"');
    expect(liveDockerAuth).toContain('[[ -n "${AFORA_DOCKER_CACHE_HOME_DIR:-}" ]]');
    expect(liveDockerAuth).toContain(
      'timeout_value="${2:-${AFORA_LIVE_DOCKER_RUN_TIMEOUT:-2700s}}"',
    );
    ...
    for (const script of [harness, ...sharedLiveScripts]) {
      ...
      expect(script).toContain("afora_live_init_docker_run_args DOCKER_RUN_ARGS");
      expect(script).toContain("DOCKER_RUN_ARGS+=(--rm -t \\");
      expect(script).not.toContain("DOCKER_RUN_ARGS=(docker run --rm -t \\");
    }
    expect(liveDockerAuth).toContain("afora_live_prepare_bind_dir_for_container_user");
    for (const script of sharedLiveScripts) {
      expect(script).toContain("afora_live_uses_managed_bind_dirs");
    ...
    for (const script of [
      ...
      expect(script).toContain("afora_live_run_setup_command");
      expect(script).not.toContain('timeout --kill-after=30s "${AFORA_LIVE_');
    }
    ...
    expect(readFileSync("scripts/test-live-models-docker.sh", "utf8")).toContain(
      "AFORA_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-gateway-models-docker.sh", "utf8")).toContain(
      "AFORA_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      "AFORA_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'CLI_SETUP_TIMEOUT_SECONDS="$(afora_live_read_positive_int_env AFORA_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      '"${AFORA_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:?missing live CLI backend setup timeout seconds}"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      '"live CLI backend setup"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      "AFORA_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
    ...
    expect(acpBindScript).toContain(
      "AFORA_LIVE_ACP_BIND_CLAUDE_AUTH must be one of: auto, api-key, subscription.",
    );
    expect(acpBindScript).toContain(
    ...
    expect(workflow.match(/AFORA_LIVE_ACP_BIND_CLAUDE_AUTH=subscription/g)).toHaveLength(2);
    expect(workflow.match(/AFORA_LIVE_ACP_BIND_CLAUDE_AUTH=api-key/g)).toHaveLength(2);
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      "AFORA_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-$((2100 * CODEX_HARNESS_TARGET_COUNT))s",
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-subagent-announce-docker.sh", "utf8")).toContain(
      "AFORA_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s",
    );
    expect(build).toContain('ROOT_DIR="${AFORA_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    ...
    expect(build).toContain('LIVE_IMAGE_PULL_ATTEMPTS="${AFORA_LIVE_DOCKER_PULL_ATTEMPTS:-3}"');
    expect(build).toContain('docker_e2e_docker_cmd pull "$LIVE_IMAGE_NAME"');
    expect(build).not.toContain('docker pull "$LIVE_IMAGE_NAME"');
    expect(stage).toContain(
      'local scripts_dir="${AFORA_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("fails Droid ACP Docker live proof when Factory auth is missing", () => {
    ...
    expect(script).toContain("afora_live_acp_bind_load_factory_api_key_from_profile");
    expect(script).not.toContain('source "$PROFILE_FILE"');
    expect(script.indexOf("afora_live_acp_bind_load_factory_api_key_from_profile")).toBeLessThan(
  ...
  it("plumbs live credentials through planned Docker E2E live lanes", () => {
    ...
    const testboxProviderSecretKeys = [
      ...
      "AFORA_LIVE_BROWSER_CDP_URL",
      "AFORA_LIVE_SETUP_TOKEN",
      "AFORA_LIVE_SETUP_TOKEN_MODEL",
      "AFORA_LIVE_SETUP_TOKEN_PROFILE",
      "AFORA_LIVE_SETUP_TOKEN_VALUE",
      "GEMINI_API_KEY",
      "GOOGLE_API_KEY",
      "OPENROUTER_API_KEY",
      "QWEN_API_KEY",
      ...
      "AFORA_CODEX_AUTH_JSON",
      "AFORA_CODEX_CONFIG_TOML",
      "AFORA_CLAUDE_JSON",
      "AFORA_CLAUDE_CREDENTIALS_JSON",
      "AFORA_CLAUDE_SETTINGS_JSON",
      "AFORA_CLAUDE_SETTINGS_LOCAL_JSON",
      "AFORA_GEMINI_SETTINGS_JSON",
      "FIREWORKS_API_KEY",
    ];
    ...
    expectTextToIncludeAll(reusableWorkflow, [
      ...
      "require_any Codex AFORA_CODEX_AUTH_JSON",
      'if [[ "$credentials" == *",gemini,"* ]]; then',
      "require_any Gemini GEMINI_API_KEY GOOGLE_API_KEY AFORA_GEMINI_SETTINGS_JSON",
      'if [[ "$credentials" == *",opencode,"* ]]; then',
      "require_any OpenCode OPENCODE_API_KEY OPENCODE_ZEN_API_KEY",
    ]);
    expect(reusableWorkflow.match(/AFORA_LIVE_CLI_BACKEND_AUTH=subscription/g)).toHaveLength(2);
    expect(
      reusableWorkflow.match(
        /if \[\[ -n "\$\{AFORA_CLAUDE_CREDENTIALS_JSON:-\}" \|\| -n "\$\{CLAUDE_CODE_OAUTH_TOKEN:-\}" \]\]; then/g,
      ),
    ).toHaveLength(4);
  });
  ...
  it("finalizes dispatched Testbox delegation even when setup or the remote command fails", () => {
    ...
    expect(workflow).not.toContain('PNPM_CONFIG_STORE_DIR: "/tmp/afora-pnpm-store"');
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("AFORA_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain("AFORA_RELEASE_QA_*_LIVE_CI_ENABLED");
    expect(workflow).not.toContain(
  ...
  it("routes release Matrix through the QA Lab selector", () => {
    ...
    expect(releaseWorkflow).not.toContain("pnpm afora qa matrix");
    expect(qaWorkflow).toContain("pnpm afora qa matrix");
    ...
    expect(workflowStep(matrixJob, "Run Matrix live lane").env).toEqual({
      ...
      OPENAI_API_KEY: "${{ secrets.OPENAI_API_KEY }}",
      AFORA_LIVE_OPENAI_KEY: "${{ secrets.OPENAI_API_KEY }}",
      AFORA_QA_REDACT_PUBLIC_METADATA: "1",
    });
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.AFORA_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      ...
      expect(releaseWorkflow).not.toContain(
        `vars.AFORA_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`AFORA_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("overlays only trusted Anthropic mock tooling for frozen-target QA parity", () => {
    ...
    for (const contextRef of [
      ...
      expect(result.stderr).toContain(
        "target_context_ref must be a canonical Afora release branch or tag.",
      );
    }
  ...
  it("runs the core gateway restart pair on its pinned live model", () => {
    ...
    expect(runStep.run).toContain("--runtime-pair afora,codex");
    expect(runStep.run).toContain(
  ...
  it("runs canonical runtime-pair lanes in parallel and preserves one gate", () => {
    ...
    const coreRestartRun = workflowStep(laneJob, "Run Afora core restart proof").run;
    expect(coreRestartRun).toContain("--scenario gateway-restart-inflight-run");
    expect(coreRestartRun).toContain('--output-dir ".artifacts/qa-e2e/afora-core-restart"');
  ...
  it("isolates Open WebUI release coverage on a lean large-disk runner", () => {
    ...
    expect(job.env?.AFORA_DOCKER_ALL_RELEASE_PROFILE).toBe("${{ inputs.release_test_profile }}");
  ...
  it("documents the full-release Telegram package path in operator summaries", () => {
    ...
    expectTextToIncludeAll(workflow, [
      ...
      "Package Telegram E2E: deferred by \\`skip_package_telegram_e2e\\`",
      "Package Telegram E2E: Afora Release Checks Package Acceptance",
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(validateStep.run, [
      ...
      "package_spec must be afora@alpha",
    ...
    expectTextToIncludeAll(runStep.run, [
      ...
      "package candidate manifest does not match the Afora tarball",
      ...
      'export AFORA_NPM_TELEGRAM_PACKAGE_DIR="${package_dir}"',
      'export AFORA_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgz}"',
    ]);
  });
  ...
  it("uses bounded Convex lease waits instead of GitHub concurrency for CI Telegram consumers", () => {
    ...
    for (const [workflowPath, jobName, stepName, acquireTimeoutMs] of telegramJobs) {
      ...
      expect(step.env?.AFORA_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS).toBe(acquireTimeoutMs);
    }
  });
  ...
  it("keeps release QA and repo E2E lanes off scarce 32-core runners", () => {
    ...
    expectTextToIncludeAll(liveE2eWorkflow, [
      "AFORA_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000",
      "AFORA_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    ]);
  });
  ...
  it("keeps release QA status artifacts blocking in the verifier", () => {
    ...
    expect(telegramDispatch.run).toContain('workflow="afora-release-telegram-qa.yml"');
  ...
  it("wires evidence attempts into the acceptance gate", () => {
    ...
    const npmPublishJob = workflowJob(AFORA_NPM_RELEASE_WORKFLOW, "publish_afora_npm");
  ...
  it("keeps release publish artifacts and release-note ordering wired", () => {
    ...
    expect(npmTarget.run).not.toContain(
      "node scripts/afora-npm-extended-stable-release.mjs verify-manifest",
    );
    ...
    expect(publishSteps[notesIndex]?.if).toBe("${{ inputs.publish_afora_npm }}");
  ...
  it("accepts tag-matched frozen release branches in Afora npm preflight", () => {
    const preflight = workflowJob(AFORA_NPM_RELEASE_WORKFLOW, "preflight_afora_npm");
  ...
  it("gates stable GitHub publication on the Windows Hub release asset contract", () => {
    ...
    expect(releaseWorkflow).toContain(
      "Stable Afora publish requires an explicit windows_node_tag.",
    );
    expect(releaseWorkflow).toContain(
      "Stable Afora publish requires candidate-approved windows_node_installer_digests.",
    );
    ...
    expect(releaseWorkflow).toContain(
      "Stable release AforaCompanion asset names do not exactly match the current contract",
    );
    expect(releaseWorkflow).toContain('select(.name | startswith("AforaCompanion-"))');
    expect(releaseWorkflow).toContain(
    ...
    expect(windowsWorkflow).toContain("must be an explicit afora-windows-node release tag");
    ...
    expect(windowsWorkflow).toContain(
      "--repo AforaMosh/afora-agent-windows-node --json tagName,isDraft,isPrerelease,assets,url",
    );
    expect(windowsWorkflow).toContain(
    ...
    expect(windowsWorkflow).toContain(
      "CN=Afora Foundation, O=Afora Foundation, L=Mill Valley, S=California, C=US",
    );
    expect(windowsWorkflow).toContain("has unexpected signer subject");
    expect(windowsWorkflow).toContain("AforaCompanion-SHA256SUMS.txt");
    expect(windowsWorkflow).toContain("Verify promoted release asset contract");
    expect(windowsWorkflow).toContain(
      "Promoted AforaCompanion asset names do not exactly match the current contract",
    );
    expect(windowsWorkflow).toContain(
  ...
  it("gates stable GitHub publication on the signed Android APK contract", () => {
    ...
    expect(androidWorkflow).toContain(
      '--signer-workflow "${GITHUB_REPOSITORY}/.github/workflows/afora-release-publish.yml"',
    );
    ...
    expect(androidWorkflow).toContain("Afora-Android.apk");
    expect(androidWorkflow).toContain("Afora-Android-SHA256SUMS.txt");
    ...
    expect(androidWorkflow).toContain(
      "AFORA_BUILD_TIMESTAMP: ${{ steps.release_approval.outputs.build_timestamp }}",
    );
    ...
    expect(androidWorkflow).toContain("requires a final or correction Afora release tag");
    ...
    expect(androidDocs).toContain("github.com/AforaMosh/afora-agent/releases");
    expect(androidDocs).not.toContain("releases/latest/download/Afora-Android.apk");
    expect(androidDocs).toContain("gh attestation verify Afora-Android.apk");
  ...
  it("rejects unsafe direct Windows recovery before uploading assets", () => {
    ...
    const rejectUnexpectedTargetAssetsIndex = windowsWorkflow.indexOf(
      "Target Afora release contains unexpected AforaCompanion assets before upload",
    );
  ...
  it("publish requires the credentialed authorization path", () => {
    ...
    for (const [workflowPath, publishJobName, environment] of [
      ...
      [AFORA_NPM_RELEASE_WORKFLOW, "publish_afora_npm", "npm-release"],
      [
        ".github/workflows/plugin-clawhub-new.yml",
        "publish_bootstrap_plugins",
  ...
  it("keeps release publication ownership and artifact boundaries wired", () => {
    ...
    expect(clawHubPublish.uses).toBe(
      "afora-agent/clawhub/.github/workflows/package-publish.yml@d8096dfc039e86ab942ddf9ef117d04849fd84c1",
    );
    ...
    expect(postpublishEvidence.if).toContain("inputs.publish_afora_npm");
    expect(postpublishEvidence.with).toMatchObject({
      "if-no-files-found": "error",
      name: "afora-release-postpublish-evidence-${{ inputs.tag }}",
      path: "${{ runner.temp }}/afora-release-postpublish-evidence",
    });
  ...
  it("bounds the npm registry tarball download used for release resume", () => {
    ...
    const resolvePublishState = shellFunctionSource(
      publishRun,
      "resolve_afora_npm_publish_state",
    );
...
`,
  ...
  it("keeps release workflow setup aligned", () => {
    ...
    const releaseWorkflowPaths = [
      ...
      ".github/workflows/afora-release-publish.yml",
  ...
  it("keeps known bounded dominant child paths below parent monitors", () => {
    ...
    expect(
      workflowStep(performanceParent, "Dispatch and monitor Afora Performance").run,
  ...
  it("pins every documented raw Full Release Validation caller to one exact SHA", () => {
    const nightly = readFileSync(".agents/skills/release-afora-nightly/SKILL.md", "utf8");
    const liveUpdater = readFileSync(".agents/skills/afora-live-updater/SKILL.md", "utf8");
    const releaseCi = readFileSync(".agents/skills/release-afora-ci/SKILL.md", "utf8");
    const releaseCiNotes = readFileSync(
      ".agents/skills/release-afora-ci/references/release-ci-notes.md",
      "utf8",
    );
    const testing = readFileSync(".agents/skills/afora-testing/SKILL.md", "utf8");
    const parallels = readFileSync(".agents/skills/afora-parallels-smoke/SKILL.md", "utf8");
  ...
  it("executes shared release candidate identity validation with its JSON input", () => {
    ...
    const candidate = {
      ...
      packageFileName: "afora-current.tgz",
...
`,
  ...
  it("keeps release history checks blobless", () => {
    const fullHistoryCheckouts: Array<[string, string, string]> = [
      ...
      [AFORA_NPM_RELEASE_WORKFLOW, "preflight_afora_npm", "Checkout"],
      [AFORA_NPM_RELEASE_WORKFLOW, "validate_publish_request", "Checkout"],
      [
        ".github/workflows/afora-cross-os-release-checks-reusable.yml",
        "prepare",
  ...
  it("validates the macOS release handoff before the GitHub release page exists", () => {
    ...
    expect(buildControlUi?.env?.AFORA_CONTROL_UI_RELEASE_BUILD).toBe("1");
  });
  ...
  it("classifies fast pretag Control UI output as a release artifact", () => {
    ...
    expect(script).toContain("AFORA_CONTROL_UI_RELEASE_BUILD=1 pnpm ui:build");
  });
