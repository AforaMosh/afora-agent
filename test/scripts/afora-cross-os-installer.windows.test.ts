...
function installerTempFiles(dir: string) {
  return readdirSync(dir).filter((entry) => entry.startsWith("afora-installer-"));
}
...
describe("cross-OS installer fetch", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const dir = tempDirs.make("afora-cross-os-installer-");
      ...
      const server = createServer((request, response) => {
        ...
        if (request.url === "/healthy") {
          response.end('printf "%s" "$1|$2|$3" > "$AFORA_PROOF_HEALTHY_MARKER"\n');
          return;
        }
        response.write('printf executed > "$AFORA_PROOF_STALL_MARKER"\n');
      });
      ...
      try {
        ...
        const env = {
          ...process.env,
          TMPDIR: dir,
          AFORA_PROOF_HEALTHY_MARKER: healthyMarker,
          AFORA_PROOF_STALL_MARKER: stalledMarker,
        };
