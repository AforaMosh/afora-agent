...
describe("bench-cli-startup", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const tmpDir = tempDirs.make("afora-cli-startup-timeout-group-");
      ...
      try {
        ...
        const result = spawnSync(
          ...
          ],
          {
            ...
            env: {
              ...process.env,
              AFORA_TEST_CLI_STARTUP_TIMEOUT_KILL_GRACE_MS: "50",
              VITEST: "1",
            },
            timeout: 8_000,
          },
  ...
  it("writes compare-mode JSON output and creates parent directories", () => {
    const tempDirs = createTempDirTracker();
    const tmpDir = tempDirs.make("afora-cli-startup-compare-output-");
    try {
  ...
  it("fails reports with no measured samples", () => {
    ...
    ).toEqual(["afora.mjs version: no measured samples"]);
  });
  ...
  it("fails reports with samples that did not report RSS", () => {
    ...
    ).toEqual(["afora.mjs version sample 1: did not report max RSS"]);
  });
  ...
  it("rejects allowed nonzero exits without their expected clean-state output", () => {
    ...
    expect(
      ...
      "afora.mjs health sample 1: exited with expected code 1 but output did not match expected clean-state markers (Gateway target:)",
    ]);
  });
  ...
  it("parses config fixture gateway ports strictly from env", () => {
    ...
    for (const id of [
      ...
      expect(
        withEnv({ AFORA_GATEWAY_PORT: "45678" }, () =>
    ...
    for (const invalid of ["45678abc", "127.0.0.1:45678abc"]) {
      expect(() =>
        withEnv({ AFORA_GATEWAY_PORT: invalid }, () =>
          testing.buildConfigFixture({
            id: "gatewayHealthJson",
            name: "gateway health --json",
            args: ["gateway", "health", "--json"],
      ...
      ).toThrow("AFORA_GATEWAY_PORT must be an integer >= 1");
    }
  });
});
