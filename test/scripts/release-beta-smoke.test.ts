...
describe("release-beta-smoke", () => {
  ...
  it("rejects malformed positive integer environment limits", () => {
    expect(readPositiveInt(undefined, 60, "AFORA_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(60);
    expect(readPositiveInt("", 60, "AFORA_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(60);
    expect(readPositiveInt("25", 60, "AFORA_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(25);

    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() => readPositiveInt(raw, 60, "AFORA_RELEASE_BETA_SMOKE_COMMAND_MS")).toThrow(
        "AFORA_RELEASE_BETA_SMOKE_COMMAND_MS must be a positive integer",
      );
    }
  });
  ...
  it("parses workflow run urls when gh includes them in dispatch output", () => {
    expect(
      parseWorkflowRunIdFromOutput(
        "Dispatched: https://github.com/AforaMosh/afora-agent/actions/runs/1234567890",
      ),
    ).toBe("1234567890");
  });
  ...
  it("replaces stale Telegram proof placeholders", () => {
    const body = [
      ...
      "",
      "- npm package: https://www.npmjs.com/package/afora/v/2026.5.20-beta.1",
    ...
    const merged = mergeTelegramProofIntoReleaseBody(
      body,
      "- npm Telegram beta E2E: https://github.com/AforaMosh/afora-agent/actions/runs/123",
    );
  ...
  it("stops polling Telegram workflow runs after the timeout budget", async () => {
    ...
    await expect(
      pollRun("AforaMosh/afora-agent", "123", {
        now: () => now,
        pollIntervalMs: 400,
        readRun: () => ({
          conclusion: null,
          html_url: "https://github.com/AforaMosh/afora-agent/actions/runs/123",
          status: "queued",
