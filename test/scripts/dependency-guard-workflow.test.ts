...
describe("dependency guard workflow", () => {
  ...
  it("keeps contents write scoped to the conditional autoscrub job", () => {
    ...
    expect(detectRunStep.env?.AFORA_DEPENDENCY_GUARD_MODE).toBe("detect");
    ...
    expect(autoscrubRunStep.env?.AFORA_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN).toBe(
      "${{ steps.app-token.outputs.token || steps.app-token-fallback.outputs.token }}",
    );
    expect(autoscrubRunStep.env?.AFORA_DEPENDENCY_GUARD_MODE).toBe("autoscrub");
    expect(finalRunStep.env?.AFORA_DEPENDENCY_GUARD_MODE).toBe("enforce");
  });
  ...
  it("uses a dedicated checked-in script and bounded sticky comments", () => {
    ...
    expect(runStep.env?.AFORA_SECURITY_TEAM_SLUG).toBe("afora-secops");
    expect(runStep.env?.AFORA_SECURITY_APPROVERS).toBe("vincentkoc,steipete,joshavant");
  ...
  it("blocks package lockfile and manifest graph changes unless secops approves the current head sha", () => {
    ...
    expect(script).toContain("afora-secops");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/dependency-guard.yml @AforaMosh/afora-agent-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/dependency-guard-workflow.test.ts @AforaMosh/afora-agent-secops",
    );
    expect(codeowners).toContain("/scripts/github/dependency-guard.mjs @AforaMosh/afora-agent-secops");
    expect(codeowners).toContain("/package-lock.json @AforaMosh/afora-agent-secops");
    expect(codeowners).toContain("/extensions/*/package-lock.json @AforaMosh/afora-agent-secops");
    expect(codeowners).toContain("/pnpm-lock.yaml @AforaMosh/afora-agent-secops");
  });
});
