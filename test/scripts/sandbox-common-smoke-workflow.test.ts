...
describe("sandbox common smoke workflow", () => {
  ...
  it("bounds Docker build and run smoke steps", () => {
    expect(workflow).toContain(
      "timeout --kill-after=30s 5m docker build -t afora-sandbox-smoke-base:bookworm-slim -",
    );
    expect(workflow).toContain(
      "timeout --kill-after=30s 2m docker run --rm afora-sandbox-common-smoke:bookworm-slim",
    );
    expect(workflow).toContain("node --version");
    expect(workflow).toContain("pnpm --version");
    expect(workflow).not.toContain("INSTALL_PNPM=0");
    expect(workflow).not.toMatch(/(^|\n)\s+docker build -t afora-sandbox-smoke-base/u);
    expect(workflow).not.toContain(
      'u="$(docker run --rm afora-sandbox-common-smoke:bookworm-slim',
    );
  });
});
