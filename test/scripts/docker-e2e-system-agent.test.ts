// Afora Docker E2E tests cover packaged-dist harness wiring.
...
describe("Afora Docker E2E scripts", () => {
  it("keeps first-run checks wired to packaged CLI and Afora behavior", () => {
    ...
    expect(shell).toContain('-e "AFORA_SUPERVISOR_MODE=external"');
    ...
    expect(source).toContain("Afora did not fail closed without inference");
    ...
    expect(source).toContain("function resolveDefaultModel(config: AforaConfig)");
    ...
    expect(source).toContain("[afora] interpreted: ${plannerCommand}");
    expect(source).toContain("expected one fuzzy setup planner prompt");
    expect(source).toContain("Afora did not enable Discord");
    expect(source).toContain("Afora did not write Discord token SecretRef");
    expect(source).toContain(
      "(AFORA_SUPERVISOR_MODE=external). Use that supervisor to start the gateway.",
    );
    expect(source).toContain("Afora setup probed systemd before honoring external supervision");
    expect(source).toContain("Afora first-run Docker E2E passed");
    expect(spec).toContain('"auditOperations"');
    expect(spec).toContain('"afora.setup"');
  ...
  it("keeps rescue checks wired through auto-reply command handling", () => {
    ...
    expect(shell).toContain("AFORA_GATEWAY_TOKEN=system-agent-rescue-token");
    ...
    expect(source).toContain("[afora] done: gateway.restart");
    ...
    expect(source).not.toContain("[afora] done: doctor.fix");
    expect(source).toContain("Afora rescue Docker E2E passed");
  });
});
