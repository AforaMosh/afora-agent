...
describe("release user journey assertions", () => {
  it("rejects loose mock OpenAI port args", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-release-user-assertions-"));
  ...
  it("fails when uninstall leaves the managed plugin directory behind", () => {
    ...
    const installPath = path.join(home, ".afora", "extensions", pluginId);
    const installPathFile = path.join(root, "install-path.txt");

    try {
      writeJson(path.join(home, ".afora", "afora.json"), {
        plugins: {
          entries: {},
          allow: [],
          deny: [],
  ...
  it("passes after uninstall clears config, records, and managed files", () => {
    ...
    try {
      ...
      writeFileSync(
        installPathFile,
        path.join(home, ".afora", "extensions", "journey-plugin-a"),
        "utf8",
      );
  ...
  it("accepts ready ClickClack fixture state", async () => {
    ...
    try {
      await expect(
        withEnvAsync({ HOME: home, AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000" }, () =>
  ...
  it("bounds stalled ClickClack fixture HTTP probes", async () => {
    ...
    try {
      const startedAt = Date.now();
      await expect(
        withEnvAsync({ HOME: home, AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "100" }, () =>
          waitForClickClackSocket({
  ...
  it("rejects loose HTTP timeout env values instead of parsing prefixes", async () => {
    ...
    try {
      await expect(
        withEnvAsync({ HOME: home, AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "100ms" }, () =>
          runReleaseUserJourneyAssertion("wait-clickclack-socket", [
            `http://127.0.0.1:${server.port}`,
            "1",
          ]),
        ...
        'AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS must be a positive integer. Got: "100ms"',
      );
    } finally {
      await server.stop();
      rmSync(root, { force: true, recursive: true });
    ...
    try {
      await expect(
        withEnvAsync(
          {
            HOME: home,
            AFORA_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES: "16",
            AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000",
          },
          () =>
            runReleaseUserJourneyAssertion("post-clickclack-inbound", [
              `http://127.0.0.1:${server.port}`,
    ...
    try {
      await expect(
        withEnvAsync(
          {
            HOME: home,
            AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "25",
          },
          () =>
            runReleaseUserJourneyAssertion("post-clickclack-inbound", [
              `http://127.0.0.1:${server.port}`,
    ...
    try {
      await expect(
        withEnvAsync(
          {
            HOME: home,
            AFORA_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES: "16bytes",
            AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000",
          },
          () =>
            runReleaseUserJourneyAssertion("post-clickclack-inbound", [
        ...
        'AFORA_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES must be a positive integer. Got: "16bytes"',
      );
    } finally {
      await server.stop();
      rmSync(root, { force: true, recursive: true });
