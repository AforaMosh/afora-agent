...
describe("Mantis Telegram Desktop proof workflow", () => {
  ...
  it("releases Telegram Desktop proof leases left by interrupted agents", () => {
    ...
    expect(codexStep.env?.AFORA_QA_CREDENTIAL_OWNER_ID).toContain(
      "mantis-telegram-desktop-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    expect(workflowStep("Prepare Codex user").run).toContain("AFORA_QA_CREDENTIAL_OWNER_ID");
    ...
    expect(cleanupStep.env?.AFORA_QA_CONVEX_SECRET_CI).toContain(
      "secrets.AFORA_QA_CONVEX_SECRET_CI",
    );
    expect(cleanupStep.env?.AFORA_QA_CONVEX_SITE_URL).toContain(
      "secrets.AFORA_QA_CONVEX_SITE_URL",
    );
  ...
  it("installs local proof tools before the Codex agent runs", () => {
    ...
    expect(install.run).toContain("/usr/local/bin/afora-telegram-user-crabbox-proof");
    ...
    expect(agent.env?.AFORA_TELEGRAM_USER_DRIVER_SCRIPT).toBe(
      "${{ github.workspace }}/scripts/e2e/telegram-user-driver.py",
    );
    expect(agent.env?.AFORA_TELEGRAM_USER_PROOF_CMD).toBe(
      "/usr/local/bin/afora-telegram-user-crabbox-proof",
    );
    expect(agent.env?.AFORA_TELEGRAM_USER_CRABBOX_BIN).toBe("/usr/local/bin/crabbox");
    ...
    expect(agent.env?.CRABBOX_COORDINATOR).toContain(
      "secrets.CRABBOX_COORDINATOR || secrets.AFORA_QA_MANTIS_CRABBOX_COORDINATOR",
    );
    expect(agent.env?.CRABBOX_COORDINATOR_TOKEN).toContain(
      "secrets.CRABBOX_COORDINATOR_TOKEN || secrets.AFORA_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN",
    );

    const prepare = workflowStep("Prepare Codex user");
    expect(prepare.run).toContain(
      "AFORA_TELEGRAM_USER_CRABBOX_BIN AFORA_TELEGRAM_USER_CRABBOX_PROVIDER AFORA_TELEGRAM_USER_DRIVER_SCRIPT AFORA_TELEGRAM_USER_PROOF_CMD",
    );
    ...
    expect(prompt).toContain("$AFORA_TELEGRAM_USER_PROOF_CMD");
  ...
  it("prepares exact proof worktrees before agent secrets are available", () => {
    ...
    expect(run).toContain(
      'sudo /usr/local/sbin/afora-mantis-sut-container build "$candidate_root"',
    );
    ...
    expect(run).not.toContain("AFORA_QA_");
  });
  ...
  it("does not pass the full workflow environment into the local Telegram SUT", () => {
    ...
    expect(workflow).toContain(
      "sudo install -m 0755 scripts/mantis/mantis-sut-container.sh /usr/local/sbin/afora-mantis-sut-container",
    );
    expect(workflow).toContain(
      "codex ALL=(root) NOPASSWD: /usr/local/sbin/afora-mantis-sut-container",
    );
    expect(workflow).not.toContain("NOPASSWD:SETENV:");
    expect(workflow).toContain("/etc/afora-mantis-sut-revisions");
    ...
    expect(wrapper).toContain('readonly network_state_root="/run/afora-mantis-sut-networks"');
