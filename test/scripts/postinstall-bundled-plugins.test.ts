...
describe("bundled plugin postinstall", () => {
  it("resolves TypeScript from NODE_PATH during external modules-dir installs", async () => {
    const packageRoot = await createTempDirAsync("afora-postinstall-node-path-");
    ...
    await fs.writeFile(
      ...
      '{"name":"afora","type":"module","version":"2026.7.2"}\n',
    );
  ...
  it("recognizes direct invocation through symlinked temp prefixes", () => {
    ...
    expect(
      isDirectPostinstallInvocation({
        entryPath: "/var/folders/tmp/afora/scripts/postinstall-bundled-plugins.mjs",
        modulePath: "/private/var/folders/tmp/afora/scripts/postinstall-bundled-plugins.mjs",
        realpathSync,
      }),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    async ({ disableCompileCache }) => {
      const packageRoot = await createTempDirAsync("afora-packaged-compile-cache-");
      const scriptRoot = path.join(packageRoot, "scripts");
      ...
      const result = spawnSync(
        ...
        [path.join(scriptRoot, "postinstall-bundled-plugins.mjs")],
        {
          ...
          env: {
            ...
            AFORA_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: undefined,
            AFORA_HOME: path.join(packageRoot, "home"),
            AFORA_STATE_DIR: path.join(packageRoot, "state"),
  ...
  it("patches the Baileys upload helper dispatcher guard", async () => {
    const packageRoot = await createTempDirAsync("afora-baileys-postinstall-");
  ...
  it("prunes source-checkout bundled plugin node_modules", async () => {
    const packageRoot = await createTempDirAsync("afora-source-checkout-");
  ...
  it("keeps source-checkout prune non-fatal", async () => {
    const packageRoot = await createTempDirAsync("afora-source-checkout-prune-error-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("does not prune user-state legacy runtime deps during source-checkout postinstall", async () => {
    const packageRoot = await createTempDirAsync("afora-source-checkout-state-skip-");
    const home = await createTempDirAsync("afora-source-checkout-home-");
    const legacyRuntimeRoot = path.join(home, ".afora", "plugin-runtime-deps");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("honors disable env before source-checkout pruning", async () => {
    const packageRoot = await createTempDirAsync("afora-source-checkout-disabled-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
    ...
    runBundledPluginPostinstall({
      env: { AFORA_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1" },
      packageRoot,
      log: { log: vi.fn(), warn: vi.fn() },
    });
  ...
  it("migrates the plugin registry during postinstall from built dist contracts", async () => {
    const packageRoot = await createTempDirAsync("afora-postinstall-registry-");
  ...
  it("prunes stale dist files from packaged installs", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-");
  ...
  it("omits unpacked plugin-sdk test helpers from the package dist inventory", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-inventory-");
  ...
  it("keeps legacy plugin runtime deps cleanup non-fatal", () => {
    ...
    expect(warn).toHaveBeenNthCalledWith(
      2,
      "[postinstall] could not prune legacy plugin runtime deps /home/alice/.afora/plugin-runtime-deps: Error: locked",
    );
  });

  it("resolves legacy plugin runtime deps roots from Afora state env", () => {
    expect(
      collectLegacyPluginRuntimeDepsStateRoots({
        env: {
          HOME: "/users/alice",
          AFORA_HOME: "/srv/afora-home",
          AFORA_CONFIG_PATH: "~/profile/afora.json",
          AFORA_STATE_DIR: "~/state",
          STATE_DIRECTORY: "/var/lib/afora",
        },
        homedir: () => "/users/alice",
      }),
    ).toEqual([
      "/srv/afora-home/.clawdbot/plugin-runtime-deps",
      "/srv/afora-home/.afora/plugin-runtime-deps",
      "/srv/afora-home/profile/plugin-runtime-deps",
      "/srv/afora-home/state/plugin-runtime-deps",
      "/var/lib/afora/plugin-runtime-deps",
    ]);
  });

  it("keeps imported dist chunks even when inventory is stale", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-import-");
  ...
  it("keeps named imported chunks without preserving template-literal pseudoimports", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-named-import-");
    const entryFile = path.join(packageRoot, "dist", "cli", "run-main.js");
  ...
  it("does not abort dist pruning when a listed chunk disappears before import expansion", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-missing-chunk-");
  ...
  it("prunes stale private QA files without restoring compat sidecars", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-qa-compat-");
    const currentFile = path.join(packageRoot, "dist", "entry.js");
    ...
    await expectPathMissing(
      path.join(packageRoot, "dist", "extensions", "qa-channel", "afora.plugin.json"),
    );
    await expectPathMissing(
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is missing", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-missing-inventory-");
    const staleFile = path.join(packageRoot, "dist", "channel-CJUAgRQR.js");
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is invalid", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-invalid-inventory-");
    const currentFile = path.join(packageRoot, "dist", "channel-BOa4MfoC.js");
  ...
  it("rejects packaged dist scans that exceed the filesystem entry limit", () => {
    ...
    // several-x headroom fails `npm install -g afora` for upgrading users.
  ...
  it("prunes sibling empty dist directories after closing parent scans", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-empty-dirs-");
  ...
  it("prunes stale bundled plugin dependency debris from packaged dist", async () => {
    const packageRoot = await createTempDirAsync("afora-packaged-install-dist-prune-");
