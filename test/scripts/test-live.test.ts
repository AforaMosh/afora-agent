...
describe("scripts/test-live", () => {
  ...
  posixIt("signals the live pnpm child when the wrapper is terminated", async () => {
    const root = mkdtempSync(join(tmpdir(), "afora-test-live-signal-"));
  ...
  posixIt("kills the live pnpm process group after the no-output timeout", async () => {
    const root = mkdtempSync(join(tmpdir(), "afora-test-live-timeout-"));
    const fakePnpmPath = join(root, "pnpm");
    const childPidPath = join(root, "child.pid");
    const descendantPidPath = join(root, "descendant.pid");
    const stderr: Buffer[] = [];
    ...
    const runner = spawn(
      ...
      ["--import", "tsx", "scripts/test-live.mts", "--", "fake.live.test.ts"],
      {
        env: {
          ...
          AFORA_FAKE_PNPM_PID_PATH: childPidPath,
          AFORA_FAKE_PNPM_DESCENDANT_PID_PATH: descendantPidPath,
          AFORA_LIVE_WRAPPER_HEARTBEAT_MS: "25",
          AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "100",
          npm_execpath: fakePnpmPath,
        },
  ...
  it("rejects loose heartbeat intervals instead of parsing prefixes", () => {
    ...
    expect(resolveTestLiveHeartbeatMs({ AFORA_LIVE_WRAPPER_HEARTBEAT_MS: "2500" })).toBe(2500);
    expect(() => resolveTestLiveHeartbeatMs({ AFORA_LIVE_WRAPPER_HEARTBEAT_MS: "1e3" })).toThrow(
      "invalid AFORA_LIVE_WRAPPER_HEARTBEAT_MS: 1e3",
    );
    expect(() =>
      resolveTestLiveHeartbeatMs({ AFORA_LIVE_WRAPPER_HEARTBEAT_MS: "1000ms" }),
    ).toThrow("invalid AFORA_LIVE_WRAPPER_HEARTBEAT_MS: 1000ms");
    expect(() => resolveTestLiveHeartbeatMs({ AFORA_LIVE_WRAPPER_HEARTBEAT_MS: "0" })).toThrow(
      "invalid AFORA_LIVE_WRAPPER_HEARTBEAT_MS: 0",
    );
  });
