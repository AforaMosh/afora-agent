...
describe("verify-docker-attestations", () => {
  it("bounds docker manifest inspection calls", () => {
    const calls: unknown[] = [];
    const imageRef = "ghcr.io/AforaMosh/afora-agent:test";
  ...
  it("parses required platforms and image refs", () => {
    expect(
      parseArgs(["--platform", "linux/amd64", "--platform", "linux/arm64", "ghcr.io/afora/app"]),
    ).toEqual({
      help: false,
      imageRefs: ["ghcr.io/afora/app"],
      requiredPlatforms: [
  ...
  it("resolves digest refs from tagged image refs", () => {
    expect(imageRefForDigest("ghcr.io/AforaMosh/afora-agent:2026.4.26", imageDigest)).toBe(
      `ghcr.io/AforaMosh/afora-agent@${imageDigest}`,
    );
    expect(imageRefForDigest("localhost:5000/afora:main", imageDigest)).toBe(
      `localhost:5000/afora@${imageDigest}`,
    );
  });

  it("accepts an image index with SBOM and provenance predicates", () => {
    const errors = collectDockerAttestationErrors({
      imageRef: "ghcr.io/AforaMosh/afora-agent:test",
      index: createIndex(),
  ...
  it("reports unexpected attestation artifact types", () => {
    ...
    expect(errors).toEqual([
      `ghcr.io/AforaMosh/afora-agent:test: linux/amd64 attestation ${attestationDigest} has unexpected artifactType "application/vnd.unknown"`,
    ]);
  });
  ...
  it("reports missing attestation manifests", () => {
    ...
    expect(errors).toEqual([
      "ghcr.io/AforaMosh/afora-agent:test: missing attestation manifest for linux/amd64",
    ]);
  });
  ...
  it("reports missing SBOM or provenance predicates", () => {
    ...
    expect(errors).toEqual([
      "ghcr.io/AforaMosh/afora-agent:test: linux/amd64 missing predicate https://slsa.dev/provenance/v1",
    ]);
  });
});
