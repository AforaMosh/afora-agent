...
describe("MCP channel E2E limits", () => {
  ...
  it("accepts strict positive integer overrides", () => {
    expect(
      readMcpChannelLimits({
        AFORA_MCP_CHANNELS_CONNECT_TIMEOUT_MS: "120000",
        AFORA_MCP_CHANNELS_GATEWAY_EVENT_RETAIN_LIMIT: "500",
        AFORA_MCP_CHANNELS_RAW_MESSAGE_RETAIN_LIMIT: "25",
      }),
    ).toEqual({
  ...
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    expect(() =>
      readMcpChannelLimits({
        AFORA_MCP_CHANNELS_CONNECT_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("invalid AFORA_MCP_CHANNELS_CONNECT_TIMEOUT_MS: 1e3");
    expect(() =>
      readMcpChannelLimits({
        AFORA_MCP_CHANNELS_RAW_MESSAGE_RETAIN_LIMIT: "1000ms",
      }),
    ).toThrow("invalid AFORA_MCP_CHANNELS_RAW_MESSAGE_RETAIN_LIMIT: 1000ms");
  });
});
