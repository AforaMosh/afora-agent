...
function runAssertionCommand(command: string, root: string, env: Record<string, string> = {}) {
  return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, command], {
    ...
    env: {
      ...
      AFORA_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH: path.join(root, "agent.err"),
      AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH: path.join(root, "agent.json"),
      AFORA_STATE_DIR: path.join(root, "state"),
      PLUGIN_ID: "e2e-live-plugin-tool",
      PLUGIN_NAME: "@afora/e2e-live-plugin-tool",
...
describe("live plugin tool assertions", () => {
  it("rejects loose timeout env values instead of parsing numeric prefixes", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-live-plugin-tool-"));
    try {
      const result = runAssertionCommand("configure", root, {
        AFORA_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: "1e3",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("invalid AFORA_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: 1e3");
    } finally {
  ...
  it("writes strict positive timeout values into generated config", () => {
    ...
    try {
      const result = runAssertionCommand("configure", root, {
        AFORA_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS: "240",
      });

      expect(result.status, result.stderr).toBe(0);
      const config = JSON.parse(readFileSync(path.join(root, "state", "afora.json"), "utf8"));
  ...
  it("bounds session transcript traversal before scanning unbounded trees", () => {
    ...
    try {
      ...
      const result = runAssertion(root, {
        AFORA_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES: "2",
      });
  ...
  it("rejects oversized agent output before parsing it", () => {
    ...
    try {
      ...
      const result = runAssertion(root, {
        AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES: "1024",
      });
