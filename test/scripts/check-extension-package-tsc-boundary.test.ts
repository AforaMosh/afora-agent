...
function createTempExtensionRoot(extensionId = "demo") {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-boundary-canary-"));
...
describe("check-extension-package-tsc-boundary", () => {
  ...
  it("parses extension boundary compile concurrency strictly", () => {
    expect(resolveCompileConcurrency({ AFORA_EXTENSION_BOUNDARY_CONCURRENCY: "4" }, 32)).toBe(4);
    expect(resolveCompileConcurrency({}, 12)).toBe(6);
    expect(resolveCompileConcurrency({}, 3)).toBe(1);
    for (const value of ["4x", "0", "1e3"]) {
      expect(() =>
        resolveCompileConcurrency({ AFORA_EXTENSION_BOUNDARY_CONCURRENCY: value }, 32),
      ).toThrow("AFORA_EXTENSION_BOUNDARY_CONCURRENCY must be a positive integer");
    }
  });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-extension-tsc-timeout-"));
      tempRoots.add(root);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-extension-tsc-signal-"));
      tempRoots.add(root);
      const childPidPath = path.join(root, "child.pid");
