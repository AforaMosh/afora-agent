...
describe("scripts/run-vitest", () => {
  ...
  it("restores the workspace node_modules link from a hydrated pnpm modules directory", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { PNPM_CONFIG_MODULES_DIR: "/runner/afora-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/afora-pnpm-node-modules/vitest/package.json",
    ...
    expect(symlinks).toEqual([
      {
        target: "/runner/afora-pnpm-node-modules",
        path: "/runner/afora-pnpm-node-modules/node_modules",
        type: "junction",
      },
      {
        target: "/runner/afora-pnpm-node-modules",
  ...
  it("self-links hydrated pnpm modules when pnpm lowercases the env key", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { npm_config_modules_dir: "/runner/afora-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/afora-pnpm-node-modules/vitest/package.json",
        ...
        platform: "win32",
        requireResolve: () => "/runner/afora-pnpm-node-modules/vitest/package.json",
      }),
    ).toBe("/repo/node_modules/vitest/vitest.mjs");
    expect(symlinks).toEqual([
      {
  ...
  it("parses the optional no-output timeout env", () => {
    ...
    expect(resolveVitestNoOutputTimeoutMs({ AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500" })).toBe(
      2500,
    );
    expect(
      resolveVitestNoOutputTimeoutMs({ AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "0" }),
    ).toBeNull();
    expect(
      resolveVitestNoOutputTimeoutMs({ AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "1e3" }),
    ).toBeNull();
    expect(
      resolveVitestNoOutputTimeoutMs({ AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500ms" }),
    ).toBeNull();
  });
  ...
  it("defaults direct non-watch runs to the stall watchdog", () => {
    expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run"])).toEqual({
      PATH: "/usr/bin",
      AFORA_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
      AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "120000",
    });
    ...
    expect(
      resolveRunVitestSpawnEnv({ AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "0", PATH: "/usr/bin" }, [
        "run",
      ]),
    ).toEqual({
  ...
  it("uses a longer default stall watchdog for broad e2e and project shard configs", () => {
    ...
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        AFORA_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: timeout,
      });
    }
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        AFORA_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: extraLongTimeout,
      });
    }
    expect(
      resolveDefaultVitestNoOutputTimeoutMs([
  ...
  it("does not force the stall watchdog into delegated runner environments", () => {
    ...
    expect(
      resolveTestProjectsRunnerEnv({
        AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
        PATH: "/usr/bin",
      }),
    ).toEqual({
      AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
  ...
  posixIt("terminates a silent Vitest child through the watchdog", async () => {
    const watched = spawnWatchedVitestProcess({
      ...
      env: { AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "100" },
    });
  ...
  posixIt("stops residual process-group descendants before completing", async () => {
    const descendantPidPath = nodePath.join(
      os.tmpdir(),
      `afora-run-vitest-residual-${process.pid}-${Date.now()}.pid`,
    );
    const watchedEnv = {
      AFORA_RESIDUAL_PID_PATH: descendantPidPath,
      AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS: "5000",
    };
    const watched = spawnWatchedVitestProcess({
      pnpmArgs: [
        "exec",
        ...
        [
          ...
          "const pidPath = process.env.AFORA_RESIDUAL_PID_PATH;",
  ...
  it("parses the optional watchdog heartbeat interval", () => {
    expect(
      resolveVitestNoOutputHeartbeatMs({ AFORA_VITEST_NO_OUTPUT_HEARTBEAT_MS: "120000" }),
    ).toBe(120000);
    expect(
      resolveVitestNoOutputHeartbeatMs({ AFORA_VITEST_NO_OUTPUT_HEARTBEAT_MS: "0" }),
    ).toBeNull();
  });
});
