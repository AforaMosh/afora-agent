...
function makeRepository(): { root: string; commit: string } {
  const root = mkdtempSync(path.join(tmpdir(), "afora-apple-release-source-"));
  tempDirs.push(root);
  execFileSync("git", ["init", "--quiet"], { cwd: root });
  execFileSync("git", ["config", "user.email", "release-test@afora.test"], { cwd: root });
  execFileSync("git", ["config", "user.name", "Afora Release Test"], { cwd: root });
