...
describe("classifyRunForRevive", () => {
  ...
  const cases: Array<{
    ...
    },
    {
      ...
      runOverrides: { head_repository: { full_name: "fork/afora" } },
      expected: { action: "skip", reason: "fork-head-repository" },
    },
    {
  ...
  it.each(cases)(
    ...
    ({
      ...
      expectedRepoFullName = "AforaMosh/afora-agent",
      expected,
    }) => {
      expect(
        classifyRunForRevive({
...
const context = { repo: { owner: "afora", repo: "afora" } };
...
function githubActionsCheck(runId: number, overrides: Partial<FakeCheckRun> = {}): FakeCheckRun {
  return {
    ...
    details_url: `https://github.com/AforaMosh/afora-agent/actions/runs/${runId}/job/456`,
    ...overrides,
  };
}
...
describe("runPrCiSweeper", () => {
  ...
  it("does not spend the re-fire budget on PRs that change during revalidation", async () => {
    ...
    expect(calls.filter((call) => call.method === "pulls.update").map((call) => call.args)).toEqual(
      [
        { owner: "afora", repo: "afora", pull_number: 210, state: "closed" },
        { owner: "afora", repo: "afora", pull_number: 210, state: "open" },
      ],
    );
    ...
    expect(calls.filter((call) => call.method === "actions.reRunWorkflow")).toEqual([
      {
        ...
        args: { owner: "afora", repo: "afora", run_id: 1234 },
      },
    ]);
  ...
  it("does not let a newer run from another workflow suppress a cancelled run", async () => {
    ...
    expect(calls.filter((call) => call.method === "actions.reRunWorkflow")).toEqual([
      {
        method: "actions.reRunWorkflow",
        args: { owner: "afora", repo: "afora", run_id: 100 },
      },
    ]);
    expect(
  ...
  it.each([
    ...
    { name: "missing head branch", replacement: { head_branch: null } },
    {
      ...
      replacement: { head_repository: { full_name: "someone-else/afora" } },
    },
  ...
  it.each([
    ...
    { name: "malformed", detailsUrl: "https://github.com/AforaMosh/afora-agent/actions/runs/nope" },
