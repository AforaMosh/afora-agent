...
describe("CLI startup benchmark script spawners", () => {
  ...
  it("reuses warmed state for gateway health while isolating first-device samples", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-state-scope-test-"));
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "appendFileSync(process.env.AFORA_BENCH_HOME_LOG, `${process.env.HOME}\\n`);",
          "console.log('{\"ok\":true}');",
          "",
        ].join("\n"),
      );
      ...
      const runCase = (caseId: string) => {
        ...
        execFileSync(
          ...
          ],
          {
            ...
            env: {
              ...process.env,
              AFORA_BENCH_HOME_LOG: homeLogPath,
            },
            stdio: "pipe",
          },
        );
  ...
  it("requires connected gateway health probes to exit successfully", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-connected-test-"));
    try {
  ...
  it("does not require unrelated fixture cases for a narrowed preset", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-budget-test-"));
    try {
      ...
      expect(() =>
        execFileSync(
          ...
          ],
          {
            ...
            env: {
              ...process.env,
              AFORA_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH: "1",
            },
            stdio: "pipe",
          },
        ),
  ...
  it("rejects narrowed preset reports with no matching current cases", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-budget-empty-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
      fs.writeFileSync(
  ...
  it("rejects narrowed preset reports with unrelated current cases when baseline checks run", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-budget-overlap-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
      const makeCase = (id: string, name: string) => ({
  ...
  it("allows skip-baseline reports without fixture case overlap", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-budget-skip-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
      const makeCase = (id: string, name: string) => ({
  ...
  it("skips x64 startup budgets on noncanonical architectures", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-budget-arch-test-"));
    try {
  ...
  it("fails reused reports with timed-out samples", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-budget-timeout-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
      const timedOutCase = {
  ...
  it("fails reused reports with missing RSS samples", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bench-budget-rss-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
  ...
  it("rejects malformed startup budget env vars before reading reports", () => {
    const result = spawnSync(
      ...
      ["--import", "tsx", "scripts/test-cli-startup-bench-budget.mts"],
      {
        ...
        env: {
          ...process.env,
          AFORA_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT: "20pct",
        },
      },
    );
    ...
    expect(result.stderr).toContain(
      "AFORA_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT must be a non-negative number",
    );
