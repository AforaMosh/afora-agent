// Afora Cross Os Release Workflow tests cover afora cross os release workflow script behavior.
import { spawnSync } from "node:child_process";
import { readFileSync } from "node:fs";
import { describe, expect, it } from "vitest";
import { parse } from "yaml";

const WORKFLOW_PATH = ".github/workflows/afora-cross-os-release-checks-reusable.yml";
const RELEASE_CHECKS_PATH = ".github/workflows/afora-release-checks.yml";
const WRAPPER_PATH = "scripts/github/run-afora-cross-os-release-checks.sh";
const SCRIPT_PATH = "scripts/afora-cross-os-release-checks.ts";
const HARNESS = "bash workflow/scripts/github/run-afora-cross-os-release-checks.sh";
...
describe("cross-OS release checks workflow", () => {
  ...
  it("retries only an interrupted Windows dashboard probe", () => {
    ...
    expect(run).toContain('"${AFORA_RELEASE_CHECK_OS}" != "windows"');
  ...
  it("binds the prepared release package to an immutable artifact and package tuple", () => {
    ...
    expect(metadata.run).toContain("file_name=afora-current.tgz");
  ...
  it("downloads and re-exports exact candidate artifacts only by immutable id", () => {
    ...
    expect(resolve.run).toContain("resolve-afora-package-candidate.mts");
    ...
    expect(upload.with?.name).toBe(
      "afora-cross-os-release-checks-candidate-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    ...
    expect(baselineUpload.with?.name).toBe(
      "afora-cross-os-release-checks-baseline-${{ github.run_id }}-${{ github.run_attempt }}",
    );
  ...
  it("passes and consumes the immutable prerelease companion registry generically", () => {
    ...
    expect(run.run).not.toContain("AFORA_PLUGIN_INSTALL_OVERRIDES");
    expect(JSON.stringify(workflow)).not.toContain("@afora/codex");
  });
  ...
  it.each([
    ...
    const result = spawnSync(
      ...
      [WRAPPER_PATH, "--resolve-provider-required-companion-packages", "--provider", provider],
      {
        cwd: process.cwd(),
        encoding: "utf8",
        env: { ...process.env, AFORA_RELEASE_CHECKS_SCRIPT: SCRIPT_PATH },
      },
    );
  ...
  it("executes the release harness directly with Node", () => {
    ...
    expect(windowsCiCoverage).toContain("test/scripts/afora-cross-os-release-workflow.test.ts");
    const result = spawnSync(
      BASH_BIN,
      [
        WRAPPER_PATH,
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          AFORA_RELEASE_CHECKS_SCRIPT: SCRIPT_PATH,
        },
      },
    );
