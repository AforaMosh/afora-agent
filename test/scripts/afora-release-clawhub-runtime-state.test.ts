// Afora release ClawHub runtime-state script tests cover its CLI-only parser.
import { spawnSync } from "node:child_process";
import { describe, expect, it } from "vitest";

const SCRIPT_PATH = "scripts/afora-release-clawhub-runtime-state.ts";
...
describe("scripts/afora-release-clawhub-runtime-state.ts", () => {
  it("emits verifier args and proof lines for awaited ClawHub runs", () => {
    ...
    expect(JSON.parse(result.stdout)).toEqual({
      ...
      proofLines: {
        normal: "- plugin ClawHub publish: https://github.com/AforaMosh/afora-agent/actions/runs/123",
        bootstrap:
          "- plugin ClawHub bootstrap: https://github.com/AforaMosh/afora-agent/actions/runs/456",
      },
    });
