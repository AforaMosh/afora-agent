...
describe("gh-read helpers", () => {
  ...
  it("prints wrapper usage before reading auth env", () => {
    ...
    expect(stderr).toContain("AFORA_GH_READ_APP_ID");
  });

  it("finds repo from gh args", () => {
    expect(parseRepoArg(["pr", "view", "42", "-R", "AforaMosh/afora-agent"])).toBe("AforaMosh/afora-agent");
    expect(parseRepoArg(["run", "list", "--repo=afora/docs"])).toBe("afora-agent/docs");
    expect(parseRepoArg(["pr", "view", "42"])).toBeNull();
  });

  it("normalizes repo strings from common git formats", () => {
    expect(normalizeRepo("AforaMosh/afora-agent")).toBe("AforaMosh/afora-agent");
    expect(normalizeRepo("github.com/AforaMosh/afora-agent")).toBe("AforaMosh/afora-agent");
    expect(normalizeRepo("github:AforaMosh/afora-agent")).toBe("AforaMosh/afora-agent");
    expect(normalizeRepo("https://github.com/AforaMosh/afora-agent.git")).toBe("AforaMosh/afora-agent");
    expect(normalizeRepo("git@github.com:AforaMosh/afora-agent.git")).toBe("AforaMosh/afora-agent");
    expect(normalizeRepo("https://gitlab.com/AforaMosh/afora-agent.git")).toBeNull();
  ...
  it("bounds GitHub App private key files", () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "afora-gh-read-key-"));
  ...
  it("rejects invalid GitHub API timeout values", () => {
    ...
    expect(() => resolveGitHubFetchTimeoutMs("1s")).toThrow(
      /AFORA_GH_READ_FETCH_TIMEOUT_MS must be an integer/u,
    );
  });
});
