...
describe("real-behavior-proof-policy", () => {
  it.each([
    ...
    "Linked artifact: https://github.com/AforaMosh/afora-agent/actions/runs/123456789/artifacts/987654321",
    "Redacted runtime log: gateway connected Discord channel and delivered the reply.",
    ["Terminal transcript:", "```text", "$ afora gateway status", "discord ready", "```"].join(
      "\n",
    ),
    ...
    const evaluation = evaluatePullRequestContext({
      pullRequest: externalPr(
        [
          ...
          "",
          "- Behavior addressed: The gateway dropped the configured Discord channel during startup.",
          "- Real environment tested: macOS 15.4, Node 24, local Afora gateway.",
          "- Exact steps or command run after this patch: pnpm afora gateway restart",
  ...
  it("accepts out-of-scope follow-ups as not-tested proof detail", () => {
    const body = [
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: Afora 2026.5.21`",
      "",
  ...
  it("accepts source PR proof when explicit gaps live in out-of-scope follow-ups", () => {
    const body = [
      ...
      "## Evidence",
      "",
      "- Real environment tested: Local macOS source checkout, Node v24.8.0, Afora 2026.5.21 (c8a35c4), local `afora` shim pointed at the freshly built checkout. No channel credentials or provider API keys were used.",
      "- Exact steps or command run after this patch:",
      "  1. Built the local checkout with `node --import tsx scripts/build-all.mts`.",
      "  2. Updated `/Users/example/.local/bin/afora` to run this checkout's `afora.mjs` and verified `/Users/example/.local/bin/afora --version`.",
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: Afora 2026.5.21 (c8a35c4)`",
      "",
      "## Out-of-scope Follow-ups",
  ...
  it("accepts exact Afora ClawSweeper bot pass verdict markers when GitHub omits the app source", () => {
    const pullRequest = {
      number: 83581,
      head: {
        sha: "06ee95df6608d29a395c52ba8ab53fdd93a9dc4f",
...
describe("isMaintainerTeamMember", () => {
  ...
  it("returns true for active members", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://api.github.com/orgs/afora/teams/maintainer/memberships/private-maint",
