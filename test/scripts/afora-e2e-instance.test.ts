...
const helperPath = path.resolve("scripts/lib/afora-e2e-instance.sh");
const hostPath = [
...
function runHelper(payload: string) {
  return runSourcedHelper(
    [
      `afora_e2e_eval_test_state_from_b64 ${shellQuote(payload)}`,
      'printf "value=%s" "${AFORA_E2E_INSTANCE_TEST:-unset}"',
    ].join("; "),
    null,
  );
}
...
function writePackageFixture(packagePath: string): void {
  withTempDir("afora-e2e-package-", (root) => {
    const packageDir = path.join(root, "package");
    fs.mkdirSync(packageDir);
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "afora-e2e-fixture", version: "0.0.0" }),
    );
...
function createPackageInstallFixture(tempDir: string) {
  const fixture = {
    ...
    packagePath: path.join(tempDir, "afora.tgz"),
...
function runPackageInstall(
  ...
  return runBashWithHelper(
    [
      `afora_e2e_install_package ${shellQuote(fixture.logPath)} ${shellQuote("fixture package")} ${shellQuote(fixture.prefixPath)}`,
    ],
    env,
    undefined,
    "; ",
...
function expectNpmInstallObserved(argsPath: string, expectedArgs: string, prefix: string): void {
  ...
  expect(
    fs.existsSync(path.join(prefix, "lib/node_modules/afora-e2e-fixture/package.json")),
  ).toBe(true);
}

describe("scripts/lib/afora-e2e-instance.sh", () => {
  it("sources decoded test-state scripts", () => {
    const result = runHelper(base64('export AFORA_E2E_INSTANCE_TEST="ok"\n'));
  ...
  it("fails when the test-state payload is not valid base64", () => {
    ...
    expect(result.stderr).toContain("Invalid Afora test-state base64 payload");
  });
  ...
  it("reads positive integer env values without treating decimal input as durations", () => {
    const fallback = runSourcedHelper(
      'printf "%s" "$(afora_e2e_read_positive_int_env AFORA_E2E_SAMPLE_SECONDS 180)"',
    );
    const leadingZero = runSourcedHelper(
      'printf "%s" "$(afora_e2e_read_positive_int_env AFORA_E2E_SAMPLE_SECONDS 180)"',
      { AFORA_E2E_SAMPLE_SECONDS: "008" },
    );
    const duration = runSourcedHelper(
      "afora_e2e_read_positive_int_env AFORA_E2E_SAMPLE_SECONDS 180",
      { AFORA_E2E_SAMPLE_SECONDS: "30s" },
    );
    ...
    expect(duration.stderr).toContain("invalid AFORA_E2E_SAMPLE_SECONDS: 30s");
  });

  it("reads non-negative integer env values without accepting shell-style sizes", () => {
    const fallback = runSourcedHelper(
      'printf "%s" "$(afora_e2e_read_nonnegative_int_env AFORA_E2E_SAMPLE_BYTES 262144)"',
    );
    const zero = runSourcedHelper(
      'printf "%s" "$(afora_e2e_read_nonnegative_int_env AFORA_E2E_SAMPLE_BYTES 262144)"',
      { AFORA_E2E_SAMPLE_BYTES: "0" },
    );
    const size = runSourcedHelper(
      "afora_e2e_read_nonnegative_int_env AFORA_E2E_SAMPLE_BYTES 262144",
      { AFORA_E2E_SAMPLE_BYTES: "64kb" },
    );
    ...
    expect(size.stderr).toContain("invalid AFORA_E2E_SAMPLE_BYTES: 64kb");
  });

  it("probes default and explicit mock OpenAI base URLs", () => {
    withTempDir("afora-e2e-mock-openai-url-", (tempDir) => {
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = runSourcedHelper(
        [
          `afora_e2e_probe_http() { printf "%s\\n" "$1" >>${shellQuote(probePath)}; return 0; }`,
          "afora_e2e_wait_mock_openai 44080 1 400",
          "afora_e2e_wait_mock_openai 443 1 400 https://api.openai.com:443",
        ].join("; "),
      );
  ...
  it("probes /readyz on the explicit gateway port", () => {
    withTempDir("afora-e2e-readyz-port-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = runBashWithHelper(
        [
          ...
          `afora_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456`,
        ],
        {},
        5_000,
      );
  ...
  it("allows explicit legacy ready-log mode without /readyz", () => {
    withTempDir("afora-e2e-readyz-legacy-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const result = runBashWithHelper(
        [
          "afora_e2e_probe_http() { return 1; }",
          ...
          "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
          `printf '[gateway] ready\\n' >${shellQuote(logPath)}`,
          `afora_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456 legacy-ready-log-ok`,
        ],
        {},
        5_000,
      );
  ...
  it("wraps package installs with the configured timeout", () => {
    withTempDir("afora-e2e-instance-", (tempDir) => {
      ...
      const result = runPackageInstall(fixture, {
        ...
        AFORA_E2E_NPM_INSTALL_TIMEOUT: "42s",
        AFORA_TEST_TIMEOUT_ARGS: fixture.timeoutArgsPath,
        AFORA_TEST_NPM_ARGS: fixture.npmArgsPath,
        AFORA_TEST_NPM_BIN: path.join(tempDir, "npm"),
      });
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    withTempDir("afora-e2e-instance-plain-timeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("uses gtimeout when GNU timeout is not on PATH", () => {
    withTempDir("afora-e2e-instance-gtimeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("uses the Node watchdog when timeout is unavailable", () => {
    withTempDir("afora-e2e-instance-no-timeout-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
  ...
  it("bounds npm install failure logs to the configured tail", () => {
    withTempDir("afora-e2e-instance-install-log-", (tempDir) => {
      const fixture = createPackageInstallFixture(tempDir);
      writeFakeTimeout(path.join(tempDir, "timeout"), true);
  ...
  it.each([
    ["bytes", "AFORA_E2E_LOG_TAIL_BYTES", "64kb"],
    ["lines", "AFORA_E2E_LOG_TAIL_LINES", "25 lines"],
  ])("rejects invalid E2E log tail %s before invoking tail", (_label, envName, value) => {
    withTempDir("afora-e2e-instance-log-tail-", (tempDir) => {
      const logPath = path.join(tempDir, "install.log");
      fs.writeFileSync(logPath, "old log\nrecent log\n", "utf8");

      const result = runSourcedHelper(`afora_e2e_print_log ${shellQuote(logPath)}`, {
        [envName]: value,
      });
  ...
  it("bounds commands with the Node watchdog when timeout is unavailable", () => {
    withTempDir("afora-e2e-instance-node-watchdog-", (tempDir) => {
      writeNodeShim(tempDir);
      const startedAt = Date.now();
      const result = runBashWithHelper(
        [
          `afora_e2e_maybe_timeout 200ms ${shellQuote(process.execPath)} -e ${shellQuote("setInterval(() => {}, 1000)")}`,
        ],
        { PATH: tempDir },
        5_000,
        "; ",
      ...
      expect(result.stderr).toContain("Afora E2E command timed out after 200ms");
    });
  });
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Node watchdog children that ignore parent SIG${shellSignal}`, () => {
      withTempDir("afora-e2e-instance-node-watchdog-signal-", (tempDir) => {
        writeNodeShim(tempDir);
...
export AFORA_E2E_TIMEOUT_KILL_GRACE_MS=100
afora_e2e_maybe_timeout 30s node ${shellQuote(childPath)} ${shellQuote(pidPath)} ${shellQuote(watchdogPidPath)} &
wrapper_pid="$!"
...
`;
  ...
  it("terminates only the tracked gateway process", () => {
    withTempDir("afora-e2e-gateway-terminate-", (tempDir) => {
      ...
      writeBashExecutable(path.join(tempDir, "pkill"), [
        'printf "pkill %s\\n" "$*" >>"$AFORA_TEST_FORBIDDEN_PROCESS_TOOL_LOG"',
        "exit 42",
      ]);
...
set -euo pipefail
source ${shellQuote(helperPath)}
afora_e2e_terminate_gateways ""
/bin/sleep 30 &
tracked_pid="$!"
afora_e2e_terminate_gateways "$tracked_pid"
if kill -0 "$tracked_pid" 2>/dev/null; then
  echo "tracked gateway process still alive" >&2
  exit 1
fi
[ ! -s "$AFORA_TEST_FORBIDDEN_PROCESS_TOOL_LOG" ]
`;
      ...
      const result = runBash(
        script,
        {
          ...
          AFORA_TEST_FORBIDDEN_PROCESS_TOOL_LOG: forbiddenToolLog,
        },
        5_000,
      );
  ...
  it("terminates descendants in the tracked process group", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-e2e-process-group-"));
...
set -euo pipefail
source ${shellQuote(helperPath)}
tracked_pid="$(afora_e2e_start_tracked_process ${shellQuote(logPath)} ${shellQuote(process.execPath)} ${shellQuote(parentPath)} ${shellQuote(childPath)} ${shellQuote(parentPidPath)} ${shellQuote(childPidPath)} ${shellQuote(childTermPath)})"
for ((i = 0; i < 100; i += 1)); do
...
afora_e2e_stop_process "$tracked_pid"
for ((i = 0; i < 100; i += 1)); do
...
`;
  ...
  it("bounds HTTP readiness probes when a server accepts connections but never responds", () => {
    withTempDir("afora-e2e-http-probe-", (tempDir) => {
      ...
      const result = runBash(
        [
          ...
          `source ${shellQuote(helperPath)}`,
          'afora_e2e_probe_http_status "http://127.0.0.1:${port}/health" 200 100',
        ],
        undefined,
        3_000,
        "; ",
  ...
  it("does not repeatedly grep the full gateway log while waiting for readiness", () => {
    withTempDir("afora-e2e-readyz-incremental-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const grepArgsPath = path.join(tempDir, "grep-args.txt");
      const result = runBashWithHelper(
        [
          "afora_e2e_probe_http() { return 0; }",
          "grep() {",
          ...
          `printf 'old log line\\n' >${shellQuote(logPath)}`,
          `printf '[gateway] ready ws://127.0.0.1:23456\\n' >>${shellQuote(logPath)}`,
          `afora_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2`,
        ],
        {},
        5_000,
      );
  ...
  it("detects gateway ready markers split across incremental log reads", () => {
    withTempDir("afora-e2e-readyz-split-marker-", (tempDir) => {
      const logPath = path.join(tempDir, "gateway.log");
      const result = runBashWithHelper(
        [
          "afora_e2e_probe_http() { return 0; }",
          ...
          `afora_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 8`,
        ],
        {},
        5_000,
      );
  ...
  it("derives the readiness port only from gateway ready log lines", () => {
    withTempDir("afora-e2e-readyz-port-", (tempDir) => {
      ...
      const result = runBashWithHelper(
        [
          `afora_e2e_probe_http() { printf '%s' "$1" >${shellQuote(probePath)}; [[ "$1" = "http://127.0.0.1:23456/readyz" ]]; }`,
          "sleep 30 &",
          'gateway_pid="$!"',
          "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
  ...
  it("wraps logged Afora E2E commands with the configured timeout", () => {
    withTempDir("afora-e2e-instance-run-logged-", (tempDir) => {
      ...
      writeTimeoutDispatcher(tempDir, "fixture-command", "AFORA_TEST_COMMAND_BIN");
      writeBashExecutable(path.join(tempDir, "fixture-command"), [
        'printf "%s\\n" "$*" >"$AFORA_TEST_COMMAND_ARGS"',
        'printf "fixture output\\n"',
      ]);

      const result = runBashWithHelper(
        [
          `afora_e2e_run_logged ${shellQuote(logLabel)} fixture-command one two`,
          `printf "%s" "$AFORA_E2E_LAST_LOG_PATH" > ${shellQuote(logPathFile)}`,
        ],
        {
          PATH: `${tempDir}:${hostPath}`,
          AFORA_E2E_LOG_DIR: logDir,
          AFORA_E2E_COMMAND_TIMEOUT: "17s",
          AFORA_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          AFORA_TEST_COMMAND_ARGS: commandArgsPath,
          AFORA_TEST_COMMAND_BIN: path.join(tempDir, "fixture-command"),
        },
        undefined,
        "; ",
      );
      ...
      expect(path.basename(logPath)).toMatch(new RegExp(`^afora-${logLabel}\\..+\\.log$`, "u"));
  ...
  it("bounds logged command failure output to the configured tail", () => {
    withTempDir("afora-e2e-instance-run-log-tail-", (tempDir) => {
      const logLabel = path.basename(tempDir);
      const logDir = path.join(tempDir, "logs");
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      writeFakeTimeout(path.join(tempDir, "timeout"), true);
      ...
      const result = runBashWithHelper(
        [`afora_e2e_run_logged ${shellQuote(logLabel)} fixture-command`],
        {
          PATH: `${tempDir}${path.delimiter}${hostPath}`,
          AFORA_E2E_COMMAND_TIMEOUT: "17s",
          AFORA_E2E_LOG_DIR: logDir,
      ...
      const logFile = expectDefined(fs.readdirSync(logDir)[0], "Afora E2E command log file");
  ...
  it("installs the trash shim under isolated test state", () => {
    withTempDir("afora-e2e-trash-shim-", (tempDir) => {
      ...
      const result = runBashWithHelper(
        [
          "afora_e2e_install_trash_shim",
          "afora_e2e_install_trash_shim",
          `printf "%s" "$PATH" > ${shellQuote(pathFile)}`,
          `printf "%s" "$AFORA_E2E_BIN_DIR" > ${shellQuote(binDirFile)}`,
          "command -v trash >/dev/null",
        ],
        { HOME: homeDir, AFORA_STATE_DIR: stateDir, PATH: hostPath },
        undefined,
        "; ",
      );
      ...
      expect(binDir).not.toBe("/tmp/afora-bin");
      expect(pathEntries.filter((entry) => entry === binDir)).toHaveLength(1);
      expect(fs.existsSync(path.join(binDir, "trash"))).toBe(true);
    });
  });

  it("wraps package-installed Afora CLI calls with the configured timeout", () => {
    withTempDir("afora-e2e-instance-afora-cli-", (tempDir) => {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const commandArgsPath = path.join(tempDir, "afora-args.txt");
      writeTimeoutDispatcher(tempDir, path.join(tempDir, "afora"), "AFORA_TEST_AFORA_BIN");
      writeBashExecutable(path.join(tempDir, "afora"), [
        'printf "%s\\n" "$*" >"$AFORA_TEST_COMMAND_ARGS"',
      ]);
      ...
      const result = runBashWithHelper(
        ...
        {
          PATH: `${tempDir}:${hostPath}`,
          AFORA_E2E_COMMAND_TIMEOUT: "23s",
          AFORA_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          AFORA_TEST_COMMAND_ARGS: commandArgsPath,
          AFORA_TEST_AFORA_BIN: path.join(tempDir, "afora"),
        },
        undefined,
        "; ",
      );
      ...
      expect(result.status).toBe(0);
      expect(fs.readFileSync(timeoutArgsPath, "utf8").trim()).toBe(
        `--kill-after=30s 23s ${path.join(tempDir, "afora")} plugins list --json`,
      );
  ...
  it("wraps interactive PTY scripts with the configured timeout", () => {
    withTempDir("afora-e2e-instance-pty-timeout-", (tempDir) => {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const scriptArgsPath = path.join(tempDir, "script-args.txt");
      const logPath = path.join(tempDir, "pty.log");
      writeTimeoutDispatcher(tempDir, "script", "AFORA_TEST_SCRIPT_BIN");
      writeBashExecutable(path.join(tempDir, "script"), [
        'if [ "${1:-}" = "--version" ]; then exit 0; fi',
        'printf "%s\\n" "$*" >"$AFORA_TEST_SCRIPT_ARGS"',
      ]);

      const result = runBashWithHelper(
        [
          `afora_e2e_run_script_with_pty ${shellQuote("node /tmp/entry onboard")} ${shellQuote(logPath)}`,
        ],
        {
          PATH: `${tempDir}:${hostPath}`,
          AFORA_E2E_COMMAND_TIMEOUT: "31s",
          AFORA_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          AFORA_TEST_SCRIPT_ARGS: scriptArgsPath,
          AFORA_TEST_SCRIPT_BIN: path.join(tempDir, "script"),
        },
        undefined,
        "; ",
      );
