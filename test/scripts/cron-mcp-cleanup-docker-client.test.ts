...
describe("cron MCP cleanup docker client", () => {
  it("rejects malformed probe pid wait limits", () => {
    ...
    expect(readCronMcpCleanupProbePidWaitMs({ AFORA_CRON_MCP_CLEANUP_PID_WAIT_MS: "250" })).toBe(
      250,
    );
    for (const value of ["1.5", "1e3", "10ms", "0"]) {
      expect(() =>
        readCronMcpCleanupProbePidWaitMs({
          AFORA_CRON_MCP_CLEANUP_PID_WAIT_MS: value,
        }),
      ).toThrow("invalid AFORA_CRON_MCP_CLEANUP_PID_WAIT_MS");
    }
  });

  it("bounds missing probe pid waits", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-cron-mcp-client-"));
    try {
