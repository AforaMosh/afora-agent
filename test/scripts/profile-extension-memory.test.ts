...
describe("scripts/profile-extension-memory", () => {
  ...
  it("bounds noisy child output without losing RSS samples", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-extension-memory-test-"));
    try {
    ...
    try {
      ...
      for (let index = 0; index < 2; index += 1) {
        ...
        expect(path.basename(reportPath)).toMatch(
          /^afora-extension-memory-\d+-\d+-[0-9a-f-]+\.json$/u,
        );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "afora-extension-memory-timeout-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "afora-extension-memory-parent-signal-"));
      const hookPath = path.join(root, "rss-hook.mjs");
