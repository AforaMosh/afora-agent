...
function makeFixture(bundleId: string): { root: string; script: string; logFile: string } {
  const root = mkdtempSync(path.join(os.tmpdir(), "afora-ios-run-"));
  tempDirs.push(root);
  ...
  writeExecutable(
    ...
    `#!/usr/bin/env bash
set -euo pipefail
if [[ -n "\${AFORA_SIMULATOR_PUSH_PROOF_SECRET:-}" || -n "\${CUSTOM_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
if (process.env.AFORA_SIMULATOR_PUSH_PROOF_SECRET || process.env.CUSTOM_SIMULATOR_PUSH_PROOF_SECRET) {
...
set -euo pipefail
printf 'xcodegen %s\\n' "$*" >>"${logFile}"
if [[ -n "\${AFORA_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
app_dir="$derived/Build/Products/$configuration-iphonesimulator/Afora.app"
mkdir -p "$app_dir"
...
printf 'simctl %s\\n' "$*" >>"${logFile}"
if [[ "$1" == "launch" ]]; then
  if [[ -n "\${SIMCTL_CHILD_AFORA_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
describe("scripts/ios-run.sh", () => {
  ...
  it("installs and launches the configured app bundle identifier", () => {
    const fixture = makeFixture("ai.aforafoundation.app");

    runIosRun(fixture, { SIMCTL_BOOT_MODE: "booted" });

    expect(readFileSync(fixture.logFile, "utf8")).toContain(
      "simctl launch iPhone 17 ai.aforafoundation.app",
    );
  });
  ...
  it("builds simulator sandbox relay mode and injects proof secret only at launch", () => {
    ...
    runIosRun(
      fixture,
      {
        AFORA_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
    ...
    expect(log).toContain("AFORA_PUSH_MODE=simulatorSandbox");
    expect(log).toContain(
      "AFORA_PUSH_RELAY_BASE_URL=https://ios-push-relay-sandbox.afora.ai",
    );
    expect(log).toContain("simctl launch iPhone 17 ai.aforafoundation.app");
  ...
  it("scrubs exported simulator proof secrets from normal build helpers", () => {
    ...
    runIosRun(fixture, {
      AFORA_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
      AFORA_SIMULATOR_PUSH_PROOF_SECRET_ENV: "CUSTOM_SIMULATOR_PUSH_PROOF_SECRET",
