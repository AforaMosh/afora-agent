...
function commandResult() {
  return {
    command: "ocm @env -- afora agent --local",
    status: 0,
    stderr: "",
    stdout: "",
    timedOut: false,
...
function partialReport(): JsonObject {
  return {
    ...
    target: "local-build:/workspace/afora",
...
function writeReport(report: unknown): string {
  const root = mkdtempSync(join(tmpdir(), "afora-kova-report-"));
  tempRoots.push(root);
...
describe("scripts/lib/kova-report-gate.mts", () => {
  ...
  const profiledMutations: ReportMutation[] = [
    ...
    [
      ...
      (report) => (blockingCard(report).failedCommand = "afora agent"),
    ],
    [
    ...
    [
      ...
      (report) => {
        const cards = arrayAt(objectAt(report.gate).cards);
        cards.push({ ...infoCard(), kind: "afora-failure", status: "FAIL" });
