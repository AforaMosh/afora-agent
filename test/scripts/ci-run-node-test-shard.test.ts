...
function makeScratchDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "afora-shard-test-"));
...
describe("scripts/ci-run-node-test-shard.mts", () => {
  ...
  it("prefers explicit targets and keeps one target per child", () => {
    const plans = resolveShardPlans({
      AFORA_NODE_TEST_TARGETS_JSON: JSON.stringify(["a.test.ts", "b.test.ts"]),
      AFORA_NODE_TEST_GROUPS_JSON: JSON.stringify([{ configs: ["c.config.ts"] }]),
    });
  ...
  it("falls back from groups to the single-shard matrix envelope", () => {
    const groupPlans = resolveShardPlans({
      AFORA_NODE_TEST_GROUPS_JSON: JSON.stringify([
    ...
    const singlePlans = resolveShardPlans({
      AFORA_NODE_TEST_CONFIGS_JSON: JSON.stringify(["solo.config.ts"]),
      AFORA_VITEST_SHARD_NAME: "solo",
    });
  ...
  it("builds child env with per-plan cache isolation, includes, and env overlays", () => {
    ...
    const childEnv = buildChildEnv(
      entry,
      { BASE: "1", AFORA_VITEST_INCLUDE_FILE: "stale.json" },
      scratchDir,
      3,
    );
    ...
    expect(childEnv.AFORA_VITEST_SHARD_NAME).toBe("g");
    expect(childEnv.AFORA_TEST_PROJECTS_PARALLEL).toBe("1");
    expect(childEnv.AFORA_VITEST_FS_MODULE_CACHE_PATH).toBe(
      path.join(scratchDir, "vitest-cache-3"),
    );
    expect(childEnv.AFORA_VITEST_INCLUDE_FILE).toBe(
      path.join(scratchDir, "node-test-include-3.json"),
    );
    expect(JSON.parse(readFileSync(childEnv.AFORA_VITEST_INCLUDE_FILE ?? "", "utf8"))).toEqual([
      "src/a.test.ts",
    ]);

    const bare = buildChildEnv(
      { kind: "group" as const, name: "bare", plan: { configs: ["cfg.ts"] } },
      { AFORA_VITEST_INCLUDE_FILE: "stale.json" },
      scratchDir,
      0,
    );
    expect(bare.AFORA_VITEST_INCLUDE_FILE).toBeUndefined();
  });
    ...
    const exitCode = await runShardPlans(
      ...
      }),
      {
        ...
        runChild: async (
          ...
          seen.push({ args, cache: childEnv.AFORA_VITEST_FS_MODULE_CACHE_PATH, label });
          active -= 1;
          return 0;
        },
        scratchDir,
  ...
  it("runs per-config groups serially through one persistent cache slot", async () => {
    ...
    const exitCode = await runShardPlans(
      ...
      {
        concurrency: 1,
        env: { AFORA_VITEST_FS_MODULE_CACHE_PATH: persistentRoot },
        runChild: async (
          args: string[],
          childEnv: Record<string, string | undefined>,
          label: string,
          ...
          seen.push({
            args,
            cache: childEnv.AFORA_VITEST_FS_MODULE_CACHE_PATH,
            label,
          });
          active -= 1;
          return 0;
  ...
  it("forwards trusted Vitest arguments after the target separator", async () => {
    ...
    const exitCode = await runShardPlans(
      resolveShardPlans({
        AFORA_NODE_TEST_CONFIGS_JSON: JSON.stringify(["test/vitest/vitest.unit.config.ts"]),
      }),
      {
        concurrency: 1,
        env: {
          AFORA_NODE_TEST_VITEST_ARGS_JSON: JSON.stringify(["--hookTimeout=300000"]),
        },
    ...
    const exitCode = await runShardPlans(
      ...
      }),
      {
        ...
        runChild: async (_args: string[], childEnv: Record<string, string | undefined>) => {
          const cache = childEnv.AFORA_VITEST_FS_MODULE_CACHE_PATH ?? "";
  ...
  it("prunes oldest transform entries while preserving Vitest metadata", () => {
    ...
    const generation = path.join(persistentRoot, ".afora-transform-generation");
  ...
  it("prunes persistent caches only in the designated writer job", async () => {
    ...
    const run = (writer: string) =>
      runShardPlans(plans, {
        concurrency: 1,
        env: {
          AFORA_VITEST_FS_MODULE_CACHE_PATH: persistentRoot,
          AFORA_VITEST_FS_MODULE_CACHE_WRITER: writer,
        },
