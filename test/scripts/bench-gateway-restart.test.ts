...
describe("gateway restart benchmark script", () => {
  ...
  it("prints help without running benchmark cases", () => {
    ...
    expect(helpResult.stdout).toContain("Afora Gateway restart benchmark");
...
`),
  ...
  it("enables both startup and restart trace in the child gateway environment", () => {
    const env = testing.sanitizedEnv("/tmp/afora-bench", "/tmp/afora-bench/config.json", {
      config: {},
      id: "skipChannels",
      name: "gateway restart, skip channels",
    });

    expect(env.AFORA_GATEWAY_STARTUP_TRACE).toBe("1");
    expect(env.AFORA_GATEWAY_RESTART_TRACE).toBe("1");
    expect(env.AFORA_NO_RESPAWN).toBe("1");
    expect(env.AFORA_LOCAL_CHECK).toBeUndefined();
  });

  it("can pin ACPX startup probe policy per benchmark case", () => {
    const probeOffEnv = testing.sanitizedEnv(
      "/tmp/afora-bench",
      "/tmp/afora-bench/config.json",
      {
        config: {},
        env: { AFORA_ACPX_RUNTIME_STARTUP_PROBE: "0" },
        id: "skipChannelsNoAcpxProbe",
        name: "gateway restart, skip channels, ACPX startup probe off",
      },
    );

    expect(probeOffEnv.AFORA_ACPX_RUNTIME_STARTUP_PROBE).toBe("0");
  });
  ...
  it("writes restart intent files for the target gateway pid", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-restart-bench-test-"));
    try {
      const env = { AFORA_STATE_DIR: path.join(root, "state") };
  ...
  it("writes plugin fixtures as a parent load path with explicit startup activation", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-restart-bench-config-test-"));
    try {
      ...
      const manifest = JSON.parse(
        fs.readFileSync(
          path.join(root, "plugins", "bench-plugin-01", "afora.plugin.json"),
          "utf8",
        ),
