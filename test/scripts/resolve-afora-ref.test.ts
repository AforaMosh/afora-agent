// Resolve Afora ref tests cover the release workflow ref resolver script.
import { execFileSync, spawnSync } from "node:child_process";
import { join } from "node:path";
import { afterAll, beforeAll, describe, expect, it } from "vitest";
import { createTempDirTracker } from "../helpers/temp-dir.js";

const SCRIPT_PATH = "scripts/github/resolve-afora-ref.sh";
...
function createRemoteRepo() {
  const repo = tempDirs.make("afora-ref-remote-");
...
describe("scripts/github/resolve-afora-ref.sh", () => {
  ...
  it("writes fallback outputs for unresolved refs when a caller supplies an expected SHA", () => {
    const outputPath = join(tempDirs.make("afora-ref-output-"), "github-output.txt");
  ...
  it("does not let fallback mode hide remote lookup failures", () => {
    const missingRemote = join(tempDirs.make("afora-ref-missing-"), "missing.git");
