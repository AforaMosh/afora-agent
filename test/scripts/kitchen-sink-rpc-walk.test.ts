...
describe("kitchen-sink RPC isolated state", () => {
  it("prints help without creating temp state or installing the plugin", async () => {
    ...
    expect(result.stdout).toContain("AFORA_KITCHEN_SINK_NPM_SPEC");
    expect(result.stdout).toContain("AFORA_KITCHEN_SINK_PERSONALITY");
    expect(result.stdout).toContain("AFORA_KITCHEN_SINK_RPC_PORT");
    expect(result.stdout).toContain("AFORA_KITCHEN_SINK_RPC_FETCH_MS");
    expect(result.stdout).toContain("AFORA_KITCHEN_SINK_RPC_FETCH_BODY_BYTES");
    expect(result.stdout).toContain("AFORA_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS");
  ...
  it("prints help before parsing malformed runtime guardrails", async () => {
    const result = await runCommand(
      ...
      ["--import", "tsx", "scripts/e2e/kitchen-sink-rpc-walk.mts", "--help"],
      {
        env: {
          ...process.env,
          AFORA_KITCHEN_SINK_MAX_RSS_MIB: "1e3",
        },
      },
    );
  ...
  it("rejects loose numeric env values before they bypass runtime guardrails", () => {
    ...
    expect(readPositiveInt(" 1000 ", 60_000)).toBe(1000);
    expect(() => readPositiveInt("1e3", 60_000, "AFORA_KITCHEN_SINK_MAX_RSS_MIB")).toThrow(
      'AFORA_KITCHEN_SINK_MAX_RSS_MIB must be a positive integer. Got: "1e3"',
    );
    expect(() => readPositiveInt("1000ms", 60_000, "AFORA_KITCHEN_SINK_RPC_READY_MS")).toThrow(
      'AFORA_KITCHEN_SINK_RPC_READY_MS must be a positive integer. Got: "1000ms"',
    );
    expect(() => readPositiveInt("0", 60_000, "AFORA_KITCHEN_SINK_RPC_PORT")).toThrow(
      'AFORA_KITCHEN_SINK_RPC_PORT must be a positive integer. Got: "0"',
    );
  });
  ...
  it("clamps timer env values before they reach Node timers", () => {
    ...
    const config = resolveKitchenSinkRpcConfig({
      AFORA_KITCHEN_SINK_RPC_CALL_MS: oversizedTimerMs,
      AFORA_KITCHEN_SINK_RPC_COMMAND_MS: oversizedTimerMs,
      AFORA_KITCHEN_SINK_RPC_FETCH_MS: oversizedTimerMs,
      AFORA_KITCHEN_SINK_RPC_INSTALL_MS: oversizedTimerMs,
      AFORA_KITCHEN_SINK_RPC_READY_MS: oversizedTimerMs,
    });
    ...
    expect(
      createRpcCliRunOptions("kitchen_sink_text", {
        env: {
          AFORA_KITCHEN_SINK_RPC_CALL_MS: String(MAX_KITCHEN_SINK_TIMER_TIMEOUT_MS),
        },
      }).timeoutMs,
  ...
  it("uses an explicit RPC port or asks the OS for an available fallback", async () => {
    await expect(
      resolveKitchenSinkRpcPort({ AFORA_KITCHEN_SINK_RPC_PORT: "19080" }),
    ).resolves.toBe(19080);
    await expect(
      resolveKitchenSinkRpcPort({ AFORA_KITCHEN_SINK_RPC_PORT: "65535" }),
    ).resolves.toBe(65535);
    await expect(
      resolveKitchenSinkRpcPort({ AFORA_KITCHEN_SINK_RPC_PORT: "65536" }),
    ).rejects.toThrow(
      'AFORA_KITCHEN_SINK_RPC_PORT must be a TCP port from 1 to 65535. Got: "65536"',
    );
    await expect(
  ...
  it("cleans up the generated temporary home tree", async () => {
    ...
    expect(root).toContain("afora-kitchen-sink-rpc-");
    expect(env.HOME).toBe(path.join(root, "home"));
    expect(env.USERPROFILE).toBe(env.HOME);
    expect(env.AFORA_HOME).toBe(env.HOME);
    expect(env.AFORA_STATE_DIR).toBe(path.join(env.HOME, ".afora"));
    expect(env.AFORA_CONFIG_PATH).toBe(path.join(env.AFORA_STATE_DIR, "afora.json"));
    expect(existsSync(env.AFORA_STATE_DIR)).toBe(true);
  ...
  it("can fail the walk when generated temp cleanup cannot remove the root", async () => {
    ...
    try {
      await expect(
        cleanupKitchenSinkEnv("/tmp/afora-kitchen-sink-rpc-stuck", {
          attempts: 3,
          delayMs: 1,
          throwOnFailure: true,
          warn: false,
        ...
        "failed to remove Kitchen Sink RPC temp root: /tmp/afora-kitchen-sink-rpc-stuck",
      );
...
describe("kitchen-sink RPC gateway teardown", () => {
  ...
  it("fails readiness waits before polling after signaled gateway exits", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-signal-ready-"));
    try {
  ...
  it("aborts stalled readiness probes when the gateway exits mid-probe", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-exit-during-ready-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("keeps stalled readiness probes inside the caller deadline", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-stalled-ready-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("requires /readyz body.ready before accepting gateway readiness", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-ready-body-"));
    try {
      const logPath = path.join(root, "gateway.log");
...
describe("kitchen-sink RPC gateway readiness logs", () => {
  it("scans gateway readiness logs incrementally across appended chunks", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-log-scan-"));
    try {
      const logPath = path.join(root, "gateway.log");
      writeFileSync(logPath, "booting\n".repeat(1000));
  ...
  it("resets the readiness scanner after log rotation", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-log-rotate-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("scans gateway error logs incrementally and keeps the latest findings", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-log-errors-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("does not allowlist dirty error lines that mention zero errors", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-log-zero-error-smuggle-"));
    try {
      const logPath = path.join(root, "gateway.log");
  ...
  it("bounds scanner memory for very long log lines", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-log-long-line-"));
    try {
      const logPath = path.join(root, "gateway.log");
...
describe("kitchen-sink RPC command output capture", () => {
  ...
  it("clamps oversized command timeout env values before scheduling timers", async () => {
    const previousTimeout = process.env.AFORA_KITCHEN_SINK_RPC_COMMAND_MS;
    process.env.AFORA_KITCHEN_SINK_RPC_COMMAND_MS = String(Number.MAX_SAFE_INTEGER);
    try {
      await expect(
      ...
      if (previousTimeout === undefined) {
        delete process.env.AFORA_KITCHEN_SINK_RPC_COMMAND_MS;
      } else {
        process.env.AFORA_KITCHEN_SINK_RPC_COMMAND_MS = previousTimeout;
      }
    }
  });

  posixIt("kills timed command process groups", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-timeout-"));
...
`,
  ...
  posixIt("rejects timed commands that exit cleanly after SIGTERM", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-kitchen-rpc-timeout-zero-"));
...
`,
  ...
  it("rejects command spawn failures as Error objects", async () => {
    await expect(runCommand("afora-definitely-missing-command", [])).rejects.toMatchObject({
      code: "ENOENT",
    });
  });
...
describe("kitchen-sink RPC caller loading", () => {
  ...
  it("uses built callGateway chunks for dist and packaged entries", () => {
    expect(usesBuiltAforaEntry({ command: "node", baseArgs: ["dist/index.js"] })).toBe(true);
    expect(
      usesBuiltAforaEntry({ command: "node", baseArgs: ["/app/afora.mjs"] }, "/repo", {
        AFORA_ENTRY: "/app/afora.mjs",
      }),
    ).toBe(true);
  });

  it("does not deep-import gateway TypeScript for source pnpm runners", () => {
    expect(usesBuiltAforaEntry({ pnpm: true, baseArgs: ["afora"] })).toBe(false);
    expect(usesBuiltAforaEntry({ command: "node", baseArgs: ["scripts/dev.mjs"] })).toBe(false);
  });

  it("finds only built callGateway chunks", () => {
    const root = mkdtempSync(path.join(tmpdir(), "afora-rpc-call-chunks-"));
    try {
  ...
  posixIt("kills descendants when timed commands exit cleanly after SIGTERM", async () => {
    ...
    const root = makeTempDir(tempDirs, "afora-kitchen-rpc-timeout-clean-parent-");
...
`,
  ...
  posixIt("cleans active command process groups before parent signal exit", async () => {
    const tempDirs: string[] = [];
    const root = makeTempDir(tempDirs, "afora-kitchen-rpc-parent-signal-");
...
`,
    ...
    try {
      runner = spawn(process.execPath, ["--import", "tsx", runnerPath], {
        ...
        env: {
          ...process.env,
          AFORA_TEST_KITCHEN_SINK_PARENT_SIGNAL_KILL_GRACE_MS: "100",
        },
...
describe("kitchen-sink RPC command catalog assertions", () => {
  ...
  it("requires every expected Kitchen Sink plugin tool", () => {
    expect(() =>
      assertExpectedKitchenSinkToolEntries(
        [{ id: "kitchen_sink_text", source: "plugin", pluginId: "afora-kitchen-sink-fixture" }],
  ...
  it("requires plugin provenance for expected catalog tools", () => {
    expect(() =>
      assertExpectedKitchenSinkToolEntries(
        [
          { id: "kitchen_sink_text", source: "plugin", pluginId: "afora-kitchen-sink-fixture" },
          { id: "kitchen_sink_search", source: "core", pluginId: "afora-kitchen-sink-fixture" },
  ...
  it("requires Kitchen Sink UI descriptor coverage", () => {
    expect(() =>
      assertKitchenSinkUiDescriptors({
        ok: true,
        descriptors: [{ pluginId: "afora-kitchen-sink-fixture", id: "kitchen-sink-panel" }],
      }),
    ).not.toThrow();
  ...
  it("accepts minimally valid gateway health and status RPC payloads", () => {
    expect(() =>
      assertGatewayHealthPayload({
        ...
        agents: [],
        sessions: {
          path: "/tmp/afora-sessions.sqlite",
          count: 0,
          recent: [],
        },
      }),
...
describe("kitchen-sink RPC process sampling", () => {
  ...
  it("samples the POSIX gateway child instead of the pnpm launcher", async () => {
    const sample = await samplePosixSnapshot(
      [
        " 4321     1   16384   0.0 node /usr/local/bin/corepack pnpm afora gateway --port 19080",
  ...
  it("falls back to the POSIX gateway process title when the port arg is rewritten", async () => {
    const sample = await samplePosixSnapshot(
      [
        " 4321     1 1048576   0.0 node /usr/local/bin/corepack pnpm afora gateway --port 19080",
        " 4322  4321  262144  12.5 afora-gateway",
        " 4323  4322   32768   1.5 node helper.js",
      ].join("\n"),
      { commandLineNeedles: ["gateway", "--port", "19080"], platform: "darwin" },
    );
  ...
  it("does not accept a POSIX launcher sample when the gateway child is missing", async () => {
    const sample = await samplePosixSnapshot(
      " 4321     1   16384   0.0 node /usr/local/bin/corepack pnpm afora status\n",
      { commandLineNeedles: ["gateway", "--port", "19080"], platform: "darwin" },
    );
