...
describe("scripts/mantis/publish-pr-evidence", () => {
  it("renders a manifest-driven PR comment with inline screenshots and video links", () => {
    ...
    const body = renderEvidenceComment({
      artifactUrl: "https://github.com/AforaMosh/afora-agent/actions/runs/1/artifacts/2",
      manifest,
      marker: "<!-- mantis-discord-status-reactions -->",
      rawBase: "https://qa.afora.ai/mantis/discord/pr-1/run-1",
      requestSource: "workflow_dispatch",
      runUrl: "https://github.com/AforaMosh/afora-agent/actions/runs/1",
      treeUrl: "https://qa.afora.ai/mantis/discord/pr-1/run-1",
    });
    ...
    expect(body).toContain(
      '<td width="50%" align="center"><img src="https://qa.afora.ai/mantis/discord/pr-1/run-1/baseline.png" width="100%"',
    );
    expect(body).toContain(
      "[Baseline change MP4](https://qa.afora.ai/mantis/discord/pr-1/run-1/baseline-change.mp4)",
    );
  ...
  it("uploads manifest artifacts to R2-compatible object storage", async () => {
    ...
    const published = await publishArtifactFiles({
      ...
      storageConfig: {
        ...
        publicBaseUrl: "https://qa.afora.ai",
        region: "auto",
    ...
    expect(published).toEqual({
      artifactRoot: "mantis/discord/pr-1/run-1",
      rawBase: "https://qa.afora.ai/mantis/discord/pr-1/run-1",
      treeUrl: "https://qa.afora.ai/mantis/discord/pr-1/run-1/index.json",
    });
    ...
    expect(String(requests[4]?.body)).toContain(
      '"url": "https://qa.afora.ai/mantis/discord/pr-1/run-1/baseline.png"',
    );
  });
  ...
  it("allows failure manifests to omit optional visual artifacts", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        scenario: "slack-afora-desktop-smoke",
        comparison: {
          candidate: {
    ...
    const body = renderEvidenceComment({
      ...
      rawBase: "https://qa.afora.ai/mantis/slack/pr-1/run-1",
      requestSource: "workflow_dispatch",
      runUrl: "https://github.com/AforaMosh/afora-agent/actions/runs/1",
      treeUrl: "https://qa.afora.ai/mantis/slack/pr-1/run-1",
    });
  ...
  it("renders a successful no-visual-proof manifest without media tables", () => {
    ...
    const body = renderEvidenceComment({
      ...
      rawBase:
        "https://raw.githubusercontent.com/AforaMosh/afora-agent/qa-artifacts/mantis/telegram-desktop/pr-1/run-1",
      requestSource: "issue_comment",
      runUrl: "https://github.com/AforaMosh/afora-agent/actions/runs/1",
      treeUrl:
        "https://github.com/AforaMosh/afora-agent/tree/qa-artifacts/mantis/telegram-desktop/pr-1/run-1",
    });
  ...
  it("does not publish PR comments for Telegram capture infrastructure failures", () => {
    ...
    const body = renderEvidenceComment({
      manifest,
      marker: "<!-- mantis-telegram-desktop-proof -->",
      rawBase: "https://artifacts.afora.ai/mantis/telegram-desktop/pr-1/run-1",
      requestSource: "pull_request_target",
      runUrl: "https://github.com/AforaMosh/afora-agent/actions/runs/1",
      treeUrl: "https://artifacts.afora.ai/mantis/telegram-desktop/pr-1/run-1/index.json",
    });
