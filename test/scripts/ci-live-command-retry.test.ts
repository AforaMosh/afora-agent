...
function runRetryHelper(commandPath: string, counterPath: string) {
  ...
  delete env.AFORA_LIVE_COMMAND_RETRY_PATTERN;
  delete env.AFORA_LIVE_COMMAND_RATE_LIMIT_PATTERN;
  return spawnSync("/bin/bash", [SCRIPT_PATH], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: {
      ...env,
      AFORA_LIVE_COMMAND: `/bin/bash ${JSON.stringify(commandPath)}`,
      AFORA_LIVE_COMMAND_ATTEMPTS: "2",
      AFORA_LIVE_COMMAND_RETRY_DELAY_SECONDS: "0",
      AFORA_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS: "0",
      AFORA_RETRY_TEST_COUNTER: counterPath,
    },
  });
}
...
describe("scripts/ci-live-command-retry.sh", () => {
  it("retries a provider-internal RPC timeout", () => {
    const { commandPath, counterPath } = writeCommand("afora-ci-live-rpc-timeout-", [
      'attempts="$(cat "$AFORA_RETRY_TEST_COUNTER" 2>/dev/null || printf 0)"',
      'attempts="$((attempts + 1))"',
      'printf "%s" "$attempts" > "$AFORA_RETRY_TEST_COUNTER"',
  ...
  it.each([
    ...
    const { commandPath, counterPath } = writeCommand("afora-ci-live-transient-", [
      'attempts="$(cat "$AFORA_RETRY_TEST_COUNTER" 2>/dev/null || printf 0)"',
      'attempts="$((attempts + 1))"',
      'printf "%s" "$attempts" > "$AFORA_RETRY_TEST_COUNTER"',
      'if [[ "$attempts" -eq 1 ]]; then',
  ...
  it("does not retry a MiniMax authentication failure", () => {
    const { commandPath, counterPath } = writeCommand("afora-ci-live-auth-failure-", [
      'attempts="$(cat "$AFORA_RETRY_TEST_COUNTER" 2>/dev/null || printf 0)"',
      'attempts="$((attempts + 1))"',
      'printf "%s" "$attempts" > "$AFORA_RETRY_TEST_COUNTER"',
