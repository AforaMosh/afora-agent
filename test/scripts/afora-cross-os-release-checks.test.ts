// Afora Cross Os Release Checks tests cover afora cross os release checks script behavior.
...
describe("scripts/afora-cross-os-release-checks", () => {
  it("uses the host account identity for managed installer services", () => {
    const env = resolveManagedGatewayInstallerEnv({
      env: {
        ...
        AFORA_HOME: "C:\\temp\\lane",
        AFORA_PROFILE: "work",
        AFORA_STATE_DIR: "C:\\temp\\lane\\.afora",
        AFORA_CONFIG_PATH: "C:\\temp\\lane\\.afora\\afora.json",
        AFORA_WINDOWS_TASK_NAME: "Afora Gateway (work)",
        AFORA_TASK_SCRIPT_NAME: "work.cmd",
        AFORA_TASK_SCRIPT: "C:\\temp\\work.cmd",
        AFORA_SERVICE_KIND: "node",
        Afora_Home: "C:\\temp\\case-variant",
        afora_config_path: "C:\\temp\\case-variant\\afora.json",
    ...
    expect(env.AFORA_TASK_SCRIPT_NAME).toBeUndefined();
    expect(env.AFORA_TASK_SCRIPT).toBeUndefined();
  ...
  it("keeps isolated installer state when no managed service is used", () => {
    const env = { AFORA_HOME: "/tmp/afora-installer" };
  ...
  it("fails closed before borrowing an occupied managed-service account", () => {
    ...
    expect(() =>
      assertManagedGatewayInstallerHostAvailable({
        ...
        pathExists: (path) => path.endsWith(".afora"),
      }),
    ).toThrow(/pristine host account/);
  });
  ...
  it("holds an exclusive managed-service host lease until release", () => {
    withTempDir("afora-managed-host-", (accountHome) => {
  ...
  it("bounds public installer fetches on Windows and POSIX", () => {
    const windowsScript = buildInstallerSmokeScript({
      installerUrl: "https://afora.ai/install.ps1",
      installTarget: "2026.7.1",
      platform: "win32",
    });
    const posixScript = buildInstallerSmokeScript({
      installerUrl: "https://afora.ai/install.sh",
      installTarget: "2026.7.1",
      platform: "linux",
    });

    expect(windowsScript).toContain(
      "curl.exe -fsSL --connect-timeout 10 --max-time 120 -o $installerPath 'https://afora.ai/install.ps1'",
    );
    expect(windowsScript).toContain("afora-installer-");
    ...
    expect(posixScript).toContain(
      'installer_path="$(mktemp "${TMPDIR:-/tmp}/afora-installer-XXXXXX")"',
    );
    expect(posixScript).toContain("trap 'rm -f \"$installer_path\"' EXIT");
    expect(posixScript).toContain(
      "curl -fsSL --connect-timeout 10 --max-time 120 -o \"$installer_path\" 'https://afora.ai/install.sh'",
    );
  ...
  it("keeps gateway status RPC probing when help probing is unavailable", () => {
    ...
    expect(buildGatewayStatusArgsFromHelpText("Usage: afora gateway status")).toEqual([
      "gateway",
      "status",
    ]);
    expect(
  ...
  it("restarts an exited manual gateway once after the exact startup migration refusal", async () => {
    await withTempDirAsync("afora-cross-os-gateway-restart-", async (dir) => {
      const refusal =
        "Afora plugin migration inputs changed during startup convergence; refusing to report the gateway ready.";
  ...
  it.each([
    ...
    await withTempDirAsync("afora-cross-os-gateway-no-restart-", async (dir) => {
  ...
  it("does not restart a live gateway after a readiness failure", async () => {
    await withTempDirAsync("afora-cross-os-gateway-live-", async (dir) => {
      const gateway = createGatewayHandleFixture({
        dir,
        name: "gateway",
        afterLaunch:
  ...
  it("fails after a second startup migration refusal without looping", async () => {
    await withTempDirAsync("afora-cross-os-gateway-second-refusal-", async (dir) => {
      const refusal =
        "Afora plugin migration inputs changed during startup convergence; refusing to report the gateway ready.\n";
      const first = createGatewayHandleFixture({
        dir,
  ...
  it("routes the Discord manual relaunch through the bounded retry wait", async () => {
    await withTempDirAsync("afora-cross-os-discord-gateway-", async (dir) => {
      ...
      await restartManualGatewayForDiscordSmoke({
        lane,
        cliPath: join(dir, "afora"),
        env: { AFORA_HOME: lane.homeDir },
        gatewayHolder,
        gatewayLogPath,
        statusLogPath,
        operations: {
  ...
  it("gives the Windows packaged updater wrapper enough headroom for Afora timeout output", () => {
  ...
  it("rejects malformed cross-OS positive integer environment values", () => {
    expect(parsePositiveIntegerEnv("AFORA_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {})).toBe(60);
    expect(
      parsePositiveIntegerEnv("AFORA_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
        AFORA_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: "25",
      }),
    ).toBe(25);
    ...
    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() =>
        parsePositiveIntegerEnv("AFORA_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
          AFORA_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: raw,
        }),
      ).toThrow("AFORA_CROSS_OS_COMMAND_HEARTBEAT_SECONDS must be a positive integer");
    }
  });
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    withTempDir("afora-cross-os-agent-output-", (dir) => {
  ...
  it("ignores stale OK markers outside the recent agent log tail", () => {
    withTempDir("afora-cross-os-agent-output-tail-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        [
  ...
  it("requires explicit opt-in before cross-OS agent turns become optional", () => {
    ...
    expect(resolveCrossOsAgentTurnOptional({ AFORA_CROSS_OS_AGENT_TURN_OPTIONAL: "1" })).toBe(
      true,
    );
    expect(
      resolveCrossOsAgentTurnOptional({ AFORA_CROSS_OS_AGENT_TURN_OPTIONAL: "false" }),
    ).toBe(false);
  });

  it("skips optional live agent turns only for model availability failures", () => {
    withTempDir("afora-cross-os-agent-skip-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        JSON.stringify({
  ...
  it("does not classify stale timeout logs as current optional agent-turn failures", () => {
    withTempDir("afora-cross-os-agent-skip-tail-", (dir) => {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
        [
  ...
  it("only skips opted-in cross-OS live agent turns after retry exhaustion", () => {
    withTempDir("afora-cross-os-agent-skip-retry-", (dir) => {
      const logPath = join(dir, "agent.log");
  ...
  it("allows cross-OS provider smoke models to use faster CI overrides", () => {
    expect(
      resolveProviderConfig("openai", {
        AFORA_CROSS_OS_OPENAI_MODEL: "openai/gpt-5.4-mini",
      })?.model,
    ).toBe("openai/gpt-5.4-mini");
    expect(
      resolveProviderConfig("openai", {
        AFORA_CROSS_OS_MODEL: "openai/gpt-5.4-nano",
      })?.model,
  ...
  it("keeps release cross-OS OpenAI smoke on GPT-5.6 Luna", () => {
    ...
    const releaseChecks = readFileSync(".github/workflows/afora-release-checks.yml", "utf8");

    expect(workflow).toContain(
      "AFORA_CROSS_OS_OPENAI_MODEL: ${{ inputs.openai_model || vars.AFORA_CROSS_OS_OPENAI_MODEL || 'openai/gpt-5.6-luna' }}",
    );
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("afora@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "afora@2026.5.2",
      "--omit=dev",
  ...
  it("rejects unsafe npm pack tarball filenames before staging release artifacts", () => {
    expect(resolveNpmPackTarballFileName("afora-2026.6.17.tgz")).toBe("afora-2026.6.17.tgz");
  ...
  it("accepts pnpm pack tarballs reported under the requested destination", () => {
    const packDir = resolvePath("/tmp/afora-pack");

    expect(resolvePackDestinationTarball("afora-2026.6.17.tgz", packDir, "pnpm pack")).toEqual({
      fileName: "afora-2026.6.17.tgz",
      path: resolvePath(packDir, "afora-2026.6.17.tgz"),
    });
    expect(
      resolvePackDestinationTarball(
        resolvePath(packDir, "afora-2026.6.17.tgz"),
        packDir,
        "pnpm pack",
      ),
    ).toEqual({
  ...
  it("rejects pnpm pack tarballs outside the requested destination", () => {
    ...
    const unsafeFilenames = [
      ...
      "nested/afora.tgz",
      "nested\\afora.tgz",
      resolvePath(dirname(packDir), "afora.tgz"),
      resolvePath(packDir, "nested", "afora.tgz"),
      "afora\u0000.tgz",
      "afora.tar.gz",
    ];
  ...
  it("falls back to pnpm pack for historical refs without the Docker package helper", () => {
    withTempDir("afora-cross-os-pack-command-", (dir) => {
      ...
      const helperPath = join(dir, "scripts", "package-afora-for-docker.mjs");
  ...
  it("keeps packaged-upgrade release updates out of service restart flow", () => {
    const args = buildPackagedUpgradeUpdateArgs("http://127.0.0.1:49152/afora-current.tgz");
    expect(args.slice(0, 6)).toEqual([
      "update",
      "--tag",
      "http://127.0.0.1:49152/afora-current.tgz",
      "--yes",
      "--json",
      "--no-restart",
    ]);
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "afora" }');
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'afora.cmd')");
    expect(script).toContain("$cmd = Get-Command afora -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        AFORA_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        AFORA_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        AFORA_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("afora-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("streams release artifacts from the static file server", async () => {
    const dir = mkdtempSync(join(tmpdir(), "afora-cross-os-static-server-"));
    const filePath = join(dir, "afora-2026.4.14.tgz");
  ...
  it("closes static release artifact sockets left by aborted clients", async () => {
    const dir = mkdtempSync(join(tmpdir(), "afora-cross-os-static-server-close-"));
    const filePath = join(dir, "afora-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("flushes static release artifact logs before close resolves", async () => {
    const dir = mkdtempSync(join(tmpdir(), "afora-cross-os-static-server-log-flush-"));
    const filePath = join(dir, "afora-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it.each([
    {
      ...
      expected: [
        "https://afora.ai/install.sh",
        "https://afora.ai/install.sh",
        "https://afora.ai/install.ps1",
      ],
    },
    {
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "afora-agent/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "AFORA_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/afora/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/afora/dist/plugin-sdk/plugin-runtime.js"',
    );
    expect(readFileSync("scripts/lib/cross-os-release-checks/install.ts", "utf8")).toContain(
      "AFORA_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href",
    );
  });
  ...
  it("wraps Windows cmd shims without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\npm.cmd" view afora@latest version"`,
      ],
      shell: false,
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\Afora Prefix`, "afora.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Afora Prefix\afora.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs resolved command invocations and writes command logs", async () => {
    await withTempDirAsync("afora-cross-os-run-command-", async (dir) => {
  ...
  it("bounds retained command output while preserving full command logs", async () => {
    await withTempDirAsync("afora-cross-os-run-command-output-", async (dir) => {
      const logPath = join(dir, "command.log");
  ...
  it("keeps multibyte command output and error tails within the byte budget", async () => {
    await withTempDirAsync("afora-cross-os-run-command-utf8-tail-", async (dir) => {
      const logPath = join(dir, "command.log");
      const result = await runCommand(
        process.execPath,
  ...
  it("keeps rolling multibyte command output and error tails within the byte budget", async () => {
    await withTempDirAsync("afora-cross-os-run-command-utf8-rolling-", async (dir) => {
      const logPath = join(dir, "command.log");
      const script = [
  ...
  it.each(["stdout", "stderr"] as const)(
    ...
    async (stream) => {
      await withTempDirAsync("afora-cross-os-run-command-utf8-split-", async (dir) => {
        const logPath = join(dir, "command.log");
        const script = [
  ...
  it.each([1, 2, 3])(
    ...
    async (maxOutputBytes) => {
      await withTempDirAsync("afora-cross-os-run-command-utf8-budget-", async (dir) => {
        const logPath = join(dir, "command.log");
        const result = await runCommand(
          process.execPath,
  ...
  it.each([
    ...
    async ({ maxOutputBytes, expected }) => {
      await withTempDirAsync("afora-cross-os-run-command-utf8-incomplete-", async (dir) => {
        const logPath = join(dir, "command.log");
        const result = await runCommand(
          process.execPath,
          [
  ...
  it("flushes command logs before resolving", async () => {
    await withTempDirAsync("afora-cross-os-run-command-flush-", async (dir) => {
  ...
  it("reads npm debug logs from the Windows cache root", () => {
    withTempDir("afora-cross-os-npm-debug-", (dir) => {
  ...
  it("prefers npm configured log directories over cache defaults", () => {
    withTempDir("afora-cross-os-npm-logs-dir-", (dir) => {
      const homeDir = join(dir, "home");
  ...
  it("keeps npm debug log collection best-effort", () => {
    withTempDir("afora-cross-os-npm-debug-best-effort-", (dir) => {
      const homeDir = join(dir, "home");
      const logPath = join(dir, "install.log");
  ...
  it("resolves relative npm log config from the install working directory", () => {
    withTempDir("afora-cross-os-npm-relative-logs-", (dir) => {
      const homeDir = join(dir, "home");
  ...
  it("kills timed-out command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "afora-cross-os-run-command-timeout-"));
    ...
    try {
      ...
      const parentScript = [
        ...
        "fs.writeFileSync(process.env.AFORA_TEST_CHILD_PID, String(child.pid));",
        "setInterval(() => {}, 1000);",
      ].join("");

      const command = runCommand(process.execPath, ["-e", parentScript], {
        cwd: dir,
        env: { ...process.env, AFORA_TEST_CHILD_PID: childPidPath },
        logPath,
        timeoutMs: 500,
      });
  ...
  it("forwards external termination to command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "afora-cross-os-run-command-signal-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            AFORA_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "200",
            AFORA_TEST_CHILD_PID: childPidPath,
          },
  ...
  it("exits promptly after externally signaled commands close", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "afora-cross-os-run-command-signal-exit-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            AFORA_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "3000",
            AFORA_TEST_CHILD_PID: childPidPath,
          },
          stdio: ["ignore", "ignore", "pipe"],
        },
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/afora", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "afora-cross-os-linux-home-"));
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "afora");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "afora" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "afora"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "afora"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "afora"));
  ...
  it("drops the bundled plugin postinstall disable flag for real updater calls", () => {
    expect(
      buildRealUpdateEnv({
        FOO: "bar",
        NODE_COMPILE_CACHE: "/tmp/stale-afora-cache",
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[afora] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/afora/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\afora-upgrade-q9DsA7\\prefix\\node_modules\\afora\\afora.mjs update --tag http://127.0.0.1:49951/afora-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\afora\\afora.mjs update --tag http://127.0.0.1:49951/afora-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node afora.mjs update --tag afora@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.afora-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("afora@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("afora@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    withTempDir("afora-cross-os-installed-version-", (prefixDir) => {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "afora")
          : join(prefixDir, "lib", "node_modules", "afora");
  ...
  it("treats missing package scripts as optional in older refs", () => {
    withTempDir("afora-cross-os-scripts-", (packageRoot) => {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
          name: "afora",
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    await withTempDirAsync("afora-cross-os-stage-debris-", async (packageRoot) => {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Afora-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Afora-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    await withTempDirAsync("afora-cross-os-local-stamps-", async (packageRoot) => {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "afora-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );
