...
function createPackageRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-probe-"));
...
function writePluginManifest(root: string, pluginRoot: string, manifest: Record<string, unknown>) {
  ...
  fs.writeFileSync(
    path.join(dir, "afora.plugin.json"),
...
function runProbe(root: string, env: Record<string, string | undefined> = {}) {
  ...
  childEnv.AFORA_ENTRY = path.join(root, "dist", "index.js");
...
function runRuntimeSmoke(root: string, args: string[]) {
  return spawnSync(process.execPath, [runtimeSmokePath, ...args], {
    ...
    env: {
      ...process.env,
      AFORA_ENTRY: path.join(root, "dist", "index.js"),
    },
  });
}
...
describe("bundled plugin install/uninstall probe", () => {
  ...
  it("bounds bundled plugin package lifecycle commands", () => {
    ...
    expect(sweep).toContain("AFORA_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT:-300s");
    expect(sweep.match(/afora_e2e_maybe_timeout/g)).toHaveLength(1);
  ...
  it("rejects unsafe bundled plugin runtime limit env values", async () => {
    await expect(
      importRuntimeSmokeWithEnv({
        AFORA_BUNDLED_PLUGIN_RUNTIME_READY_MS: String(Number.MAX_SAFE_INTEGER + 1),
      }),
    ).rejects.toThrow("invalid AFORA_BUNDLED_PLUGIN_RUNTIME_READY_MS: 9007199254740992");
  });

  it("rejects bundled plugin runtime ports outside the TCP range", async () => {
    const runtimeSmoke = await import(pathToFileURL(runtimeSmokePath).href);
    const env = {
      AFORA_BUNDLED_PLUGIN_RUNTIME_PORT_BASE: "65533",
    };
    ...
    expect(() => runtimeSmoke.resolveRuntimeSmokePort(1, 0, env)).toThrow(
      "AFORA_BUNDLED_PLUGIN_RUNTIME_PORT_BASE with bundled plugin runtime index 1 and offset 0 must resolve to a TCP port from 1 to 65535. Got: 65536",
    );
  });

  it("caps noisy runtime gateway logs", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      AFORA_BUNDLED_PLUGIN_RUNTIME_GATEWAY_LOG_BYTES: "64",
    });
    const root = makePackageRoot();
  ...
  it("rejects loose runtime output limit env values instead of parsing prefixes", async () => {
    await expect(
      importRuntimeSmokeWithEnv({
        AFORA_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS: "5chars",
      }),
    ).rejects.toThrow("invalid AFORA_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS: 5chars");
  });
  ...
  it("rejects loose runtime log scan byte env values instead of parsing prefixes", async () => {
    await expect(
      importRuntimeSmokeWithEnv({
        AFORA_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES: "64bytes",
      }),
    ).rejects.toThrow("invalid AFORA_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES: 64bytes");
  });
  ...
  it.runIf(process.platform !== "win32")("stops runtime gateway process groups", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      AFORA_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS: "50",
  ...
  it("cleans per-call RPC state directories", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        "import fs from 'node:fs';",
        "fs.writeFileSync(process.env.AFORA_TEST_RPC_STATE_PATH, process.env.AFORA_STATE_DIR);",
    ...
    await expect(
      runtimeSmoke.rpcCall(
        ...
        {
          entrypoint,
          env: { AFORA_TEST_RPC_STATE_PATH: statePath },
          port: 19001,
        },
      ),
    ...
    expect(path.basename(rpcStateDir)).toMatch(/^afora-plugin-runtime-rpc-/u);
  ...
  it("allows degraded runtime readiness only for expected channel failures", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      AFORA_BUNDLED_PLUGIN_RUNTIME_HTTP_MS: "100",
      AFORA_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS: "50",
    });
    const server = createHttpServer((_request, response) => {
  ...
  it("keeps stalled runtime readiness probes inside the ready deadline", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      AFORA_BUNDLED_PLUGIN_RUNTIME_HTTP_MS: "1000",
      AFORA_BUNDLED_PLUGIN_RUNTIME_READY_MS: "50",
    });
    const sockets = new Set<Socket>();
    const server = createNetServer((socket) => {
      sockets.add(socket);
  ...
  it("creates runtime smoke state with AFORA_HOME at the test home", async () => {
    const runtimeSmoke = await import(pathToFileURL(runtimeSmokePath).href);
  ...
  it("selects packaged installable bundled sources instead of raw dist extension dirs", () => {
    ...
    fs.writeFileSync(
      path.join(root, "dist", "extensions", "qa-channel", "afora.plugin.json"),
      '{"id":"qa-channel"}\n',
      "utf8",
    );
    ...
    const result = runProbe(root, {
      AFORA_BUNDLED_PLUGIN_SWEEP_IDS: undefined,
    });
  ...
  it("does not select source-only bundled plugins for package-backed sweeps", () => {
    ...
    const result = runProbe(root, {
      AFORA_BUNDLED_PLUGIN_SWEEP_IDS: "qa-channel",
    });

    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "AFORA_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: qa-channel",
    );
  ...
  it("rejects loose packaged plugin list limit env values", () => {
    ...
    const timeout = runProbe(root, {
      AFORA_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: "100ms",
    });
    expect(timeout.status).toBe(1);
    expect(timeout.stderr).toContain("invalid AFORA_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: 100ms");

    const maxBuffer = runProbe(root, {
      AFORA_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES: "64bytes",
    });
    expect(maxBuffer.status).toBe(1);
    expect(maxBuffer.stderr).toContain(
      "invalid AFORA_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES: 64bytes",
    );
  });
  ...
  it("rejects loose bundled plugin sweep shard env values", () => {
    ...
    const total = runProbe(root, {
      AFORA_BUNDLED_PLUGIN_SWEEP_TOTAL: "2shards",
    });
    expect(total.status).toBe(1);
    expect(total.stderr).toContain("invalid AFORA_BUNDLED_PLUGIN_SWEEP_TOTAL: 2shards");

    const index = runProbe(root, {
      AFORA_BUNDLED_PLUGIN_SWEEP_INDEX: "0of2",
    });
    expect(index.status).toBe(1);
    expect(index.stderr).toContain("invalid AFORA_BUNDLED_PLUGIN_SWEEP_INDEX: 0of2");
  });
  ...
  it("bounds plugin list selection when the CLI hangs", () => {
    ...
    const startedAt = Date.now();
    const result = runProbe(root, {
      AFORA_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: "100",
    });
  ...
  it("loads runtime smoke manifests from the selected packaged root", () => {
    ...
    fs.writeFileSync(
      path.join(root, "dist-runtime", "extensions", "runtime-only", "afora.plugin.json"),
      '{"id":"runtime-only"}\n',
      "utf8",
    );
