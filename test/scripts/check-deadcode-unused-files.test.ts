...
describe("check-deadcode-unused-files", () => {
  ...
  it("parses the compact Knip unused-file section", () => {
    expect(
      parseKnipCompactUnusedFiles(`
> afora@2026.4.27 deadcode:knip /repo
...
Delete the files or model their real entrypoints in Knip.`,
  ...
  it("runs Knip through a process-group-aware subprocess", async () => {
    const calls: unknown[] = [];
    const root = mkdtempSync(path.join(os.tmpdir(), "afora-knip-runner-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(os.tmpdir(), "afora-knip-timeout-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(os.tmpdir(), "afora-knip-parent-signal-"));
      const childPidPath = path.join(root, "child.pid");
