// Afora source performance isolation executes the workflow's real shared-shell path.
...
describe("Afora source performance isolation", () => {
  it("isolates the source performance gateway from network and scheduled work", () => {
    ...
    const targetStepName = "Run Afora source performance probes";
    ...
    expect(workflowSlice.match(/AFORA_SKIP_CRON/g) ?? []).toHaveLength(1);
    expect(workflowSlice.match(/AFORA_SKIP_CHANNELS/g) ?? []).toHaveLength(1);

    const fixtureRoot = tempDirs.make("afora-performance-isolation-");
...
record_env() {
  {
    printf 'AFORA_HOME=%s\\n' "\${AFORA_HOME-<unset>}"
    printf 'AFORA_STATE_DIR=%s\\n' "\${AFORA_STATE_DIR-<unset>}"
    printf 'AFORA_CONFIG_PATH=%s\\n' "\${AFORA_CONFIG_PATH-<unset>}"
    printf 'AFORA_GATEWAY_PORT=%s\\n' "\${AFORA_GATEWAY_PORT-<unset>}"
    printf 'AFORA_SKIP_CHANNELS=%s\\n' "\${AFORA_SKIP_CHANNELS-<unset>}"
    printf 'AFORA_SKIP_CRON=%s\\n' "\${AFORA_SKIP_CRON-<unset>}"
  } > "$CAPTURE_DIR/$1.env"
}
record_argv() {
  name="$1"
...
if [ "\${1-}" = "dist/entry.js" ] && [ "\${2-}" = "gateway" ] && [ "\${3-}" = "run" ]; then
  record_env gateway
  record_argv gateway "$@"
  /bin/cp "$AFORA_CONFIG_PATH" "$CAPTURE_DIR/gateway.config"
  printf '%s\\n' "$AFORA_GATEWAY_PORT" > "$CAPTURE_DIR/gateway.port"
...
if [ "\${1-}" = "dist/entry.js" ] && [ "\${2-}" = "gateway" ] && [ "\${3-}" = "health" ]; then
  record_env health
  record_argv health "$@"
  /bin/cp "$AFORA_CONFIG_PATH" "$CAPTURE_DIR/health.config"
...
if [ "\${1-}" = "--import" ] && [ "\${2-}" = "tsx" ]; then
  ...
  /bin/cp "$AFORA_CONFIG_PATH" "$CAPTURE_DIR/benchmark.config"
  printf 'benchmark\\n' >> "$EVENTS_FILE"
  exit 0
fi
...
`,
    ...
    for (const [branch, rgStatus, expectsCatalogRefresh] of [
      ...
      const gatewayHome = expectDefined(gatewayEnv.AFORA_HOME, "gateway home capture");
      const gatewayState = expectDefined(gatewayEnv.AFORA_STATE_DIR, "gateway state capture");
      const gatewayConfigPath = expectDefined(
        gatewayEnv.AFORA_CONFIG_PATH,
        "gateway config capture",
      );
      const healthHome = expectDefined(healthEnv.AFORA_HOME, "readiness home capture");
      const healthState = expectDefined(healthEnv.AFORA_STATE_DIR, "readiness state capture");
      const healthConfigPath = expectDefined(
        healthEnv.AFORA_CONFIG_PATH,
        "readiness config capture",
      );
      expect(gatewayEnv.AFORA_SKIP_CRON).toBe("1");
      expect(gatewayEnv.AFORA_SKIP_CHANNELS).toBe("1");
      expect(benchmarkEnv.AFORA_SKIP_CRON).toBe("<unset>");
      expect(benchmarkEnv.AFORA_SKIP_CHANNELS).toBe("<unset>");
      expect(healthEnv.AFORA_SKIP_CRON).toBe("<unset>");
      expect(healthEnv.AFORA_SKIP_CHANNELS).toBe("<unset>");
      expect(benchmarkEnv.AFORA_HOME).toBe(gatewayHome);
      expect(benchmarkEnv.AFORA_STATE_DIR).toBe(gatewayState);
      expect(benchmarkEnv.AFORA_CONFIG_PATH).toBe(gatewayConfigPath);
      expect(benchmarkEnv.AFORA_GATEWAY_PORT).toBe(gatewayPort);
      expect(gatewayEnv.AFORA_GATEWAY_PORT).toBe(gatewayPort);
      ...
      expect(gatewayState).toBe(join(gatewayHome, ".afora"));
      expect(gatewayConfigPath).toBe(join(gatewayState, "afora.json"));
      expect(healthState).toBe(join(healthHome, ".afora"));
      expect(healthConfigPath).toBe(join(healthState, "afora.json"));
