...
function makeSkipInventoryFixture(): string {
  const repoRoot = createTempDir("afora-test-skip-inventory-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
...
`,
  ...
  writeRepoFile(
    ...
    `
const LIVE = process.env.AFORA_LIVE_TEST === "1";
...
describe("collectTestSkipInventoryReport", () => {
  ...
  it("renders a compact non-blocking text report", () => {
    ...
    expect(rendered).toContain("Afora test skip inventory");
  ...
  it("rejects missing CLI repo roots and loose limits before scanning", () => {
    ...
    expect(() =>
      runTestSkipInventory([
        ...
        "1e3",
        "--repo-root",
        createTempDir("afora-skip-limit-"),
      ]),
