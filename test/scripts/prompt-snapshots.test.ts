...
describe("happy path prompt snapshots", () => {
  ...
  it("reconstructs complete Codex tool catalogs from readable full-tool overrides", async () => {
    ...
    const scenarios = [
      ...
      { name: "heartbeat-turn", replacements: ["afora_direct"] },
    ];
  ...
  it("deletes stale generated snapshot artifacts", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-prompt-snapshot-stale-"));
    try {
  ...
  it("renders the Codex model-bound prompt layers", async () => {
    ...
    expect(telegram).toContain("Afora runtime context for this turn:");
  ...
  it("keeps heartbeat guidance in heartbeat collaboration mode only", async () => {
    ...
    const agentSoulHeading = "## Afora Agent Soul";
    ...
    expect(direct).not.toContain("This is an Afora heartbeat turn.");
    expect(group).not.toContain("This is an Afora heartbeat turn.");

    expect(heartbeat).toContain('"collaborationMode": {');
    expect(heartbeat).toContain('"developer_instructions": "This is an Afora heartbeat turn.');
    expect(heartbeat).toContain(agentSoulHeading);
    const aforaRuntimeInstructions = renderedPromptSection(
      heartbeat,
    ...
    expect(aforaRuntimeInstructions).not.toContain(heartbeatPhrase);
  ...
  it("finds the first available default Codex model catalog source", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-catalog-"));
    try {
  ...
  it("skips Codex model prompt fixture sync when no default catalog exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-catalog-missing-"));
    const chunks: string[] = [];
    try {
  ...
  it("writes Codex model prompt fixtures from an explicit catalog", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-catalog-write-"));
    try {
