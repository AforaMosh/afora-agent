...
const workflowPath = ".github/workflows/afora-npm-release.yml";
...
describe("minimal npm extended-stable workflow", () => {
  ...
  it("allows an explicit default-off GitHub-hosted preflight runner", () => {
    ...
    expect(parsed.jobs?.preflight_afora_npm?.["runs-on"]).toBe(
  ...
  it("binds intentional Plugin SDK release changes to the reported digest", () => {
    ...
    const preflightDiff = step(
      parsed.jobs?.preflight_afora_npm,
      "Verify Plugin SDK API changes",
    );
    const publishProvenance = step(
      parsed.jobs?.publish_afora_npm,
    ...
    expect(preflightDiff.run).toContain('npm view "afora@${RELEASE_NPM_DIST_TAG}" version');
    ...
    expect(publishProvenanceRun).toContain('npm view "afora@${RELEASE_NPM_DIST_TAG}" version');
    ...
    expect(
      ...
      publishProvenanceRun.indexOf('npm view "afora@${RELEASE_NPM_DIST_TAG}" version'),
    );
  ...
  it("reuses the v1 preflight tarball and guards all three extended-stable gates", () => {
    ...
    expect(raw).toContain("afora-npm-preflight-${{ inputs.tag }}");
    expect(raw.match(/afora-npm-extended-stable-release\.mjs validate-request/g)).toHaveLength(
      3,
    );
    expect(step(parsed.jobs?.preflight_afora_npm, "Validate npm release request").run).toContain(
      "afora-npm-extended-stable-release.mjs validate-request",
    );
    expect(
      step(parsed.jobs?.preflight_afora_npm, "Validate npm release request").env?.PREFLIGHT_ONLY,
    ).toBe("${{ inputs.preflight_only }}");
    expect(
      step(parsed.jobs?.validate_publish_request, "Validate npm release request").run,
    ).toContain("afora-npm-extended-stable-release.mjs validate-request");
    expect(step(parsed.jobs?.publish_afora_npm, "Recheck npm release request").run).toContain(
      "afora-npm-extended-stable-release.mjs validate-request",
    );
    expect(
  ...
  it("threads an explicit, default-off extended-stable bypass through every policy gate", () => {
    ...
    const policySteps = [
      step(parsed.jobs?.preflight_afora_npm, "Validate npm release request"),
      step(parsed.jobs?.validate_publish_request, "Validate npm release request"),
      step(parsed.jobs?.publish_afora_npm, "Recheck npm release request"),
      step(parsed.jobs?.publish_afora_npm, "Publish"),
    ];
  ...
  it("lets main promote only the canonical immutable extended-stable candidate", () => {
    ...
    const recheck = step(parsed.jobs?.publish_afora_npm, "Recheck npm release request");
  ...
  it("accepts arbitrary SHA preflight targets and exercises every publishable plugin package", () => {
    const parsed = workflow();
    const preflight = parsed.jobs?.preflight_afora_npm;
    ...
    expect(plugins.run).toContain("AFORA_PLUGIN_NPM_PACK_OUTPUT_DIR");
  ...
  it("restores same-SHA preflight build outputs and keeps validation steps running", () => {
    ...
    expect(build.env?.AFORA_CONTROL_UI_RELEASE_BUILD).toBe("1");
  ...
  it("uses the trusted Full Validation evidence verifier", () => {
    ...
    expect(raw.match(/afora-npm-extended-stable-release\.mjs verify-run/g)).toHaveLength(2);
    expect(raw).not.toContain("afora-npm-extended-stable-release.mjs verify-manifest");
  });
  ...
  it("requires and authenticates the plugin npm run before an extended-stable core publish", () => {
    ...
    expect(verify.run).toContain("afora-npm-extended-stable-release.mjs verify-run");
  });

  it("captures selector fail closed, publishes extended-stable, retries, and summarizes", () => {
    const parsed = workflow();
    const publish = parsed.jobs?.publish_afora_npm;
    ...
    expect(capture.run).toContain("afora-npm-extended-stable-release.mjs capture-selector");
    expect(step(publish, "Publish").run).toContain("afora-npm-publish.sh");
    expect(readback.run).toContain("afora-npm-extended-stable-release.mjs verify-readback");
    expect(summary.if).toContain("always()");
    expect(summary.run).toContain("afora-npm-extended-stable-release.mjs repair-command");
  ...
  it("publishes only the tarball path verified from the preflight manifest", () => {
    const publish = workflow().jobs?.publish_afora_npm;
  ...
  it("publishes gateway packages in manifest order before the root package", () => {
    ...
    const publish = step(parsed.jobs?.publish_afora_npm, "Publish");
    expect(preflightPack.env?.CORE_PACKAGE_DIRS).toBe(
      "packages/ai packages/gateway-protocol packages/gateway-client",
    );
    expect(readFileSync(workflowPath, "utf8")).toContain('packageName: "@afora/gateway-client"');
