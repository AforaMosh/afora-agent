...
function extractWindowsBackgroundControlMarkers(decoded: string): {
  ...
  return {
    done: marker("__AFORA_BACKGROUND_DONE__", false),
    exitPrefix: marker("__AFORA_BACKGROUND_EXIT__", true),
  };
}
...
describe("parallels npm update smoke", () => {
  it("accepts one prepared tarball target for update and fresh install", () => {
    expect(
      parseArgs([
        "--target-tarball",
        "/tmp/afora-candidate.tgz",
        "--dependency-tarball",
        "/tmp/afora-ai-candidate.tgz",
        "--registry-package-tarball",
        "/tmp/afora-codex-candidate.tgz",
      ]),
    ).toMatchObject({
      dependencyTarballs: ["/tmp/afora-ai-candidate.tgz"],
      registryPackageTarballs: ["/tmp/afora-codex-candidate.tgz"],
      targetTarball: "/tmp/afora-candidate.tgz",
      updateTarget: "",
      freshTargetSpec: undefined,
    });
    expect(() =>
      parseArgs(["--target-tarball", "/tmp/afora-candidate.tgz", "--update-target", "beta"]),
    ).toThrow("--target-tarball cannot be combined");
    expect(() => parseArgs(["--dependency-tarball", "/tmp/afora-ai-candidate.tgz"])).toThrow(
      "--dependency-tarball requires --target-tarball",
    );
    expect(() =>
      parseArgs(["--registry-package-tarball", "/tmp/afora-codex-candidate.tgz"]),
  ...
  it("stops the host artifact server when the wrapper fails mid-run", async () => {
    ...
    class FailingNpmUpdateSmoke extends NpmUpdateSmoke {
      protected override async makeRunTempDir(prefix: string): Promise<string> {
        void prefix;
        return tempDirs.make("afora-parallels-npm-update-");
      }
  ...
  it("removes uploaded guest update scripts when chmod fails", () => {
    const root = tempDirs.make("afora-parallels-npm-update-");
...
printf '%s\\n' "$*" >>"$log_path"
args=" $* "
if [[ "$args" == *" /usr/bin/tee /tmp/afora-parallels-npm-update-linux-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" /bin/chmod 755 /tmp/afora-parallels-npm-update-linux-"* ]]; then
  echo "chmod denied" >&2
  exit 7
fi
if [[ "$args" == *" /bin/rm -f /tmp/afora-parallels-npm-update-linux-"* ]]; then
  printf 'cleanup\\n' >>"$log_path"
  exit 0
fi
exit 1
`,
    ...
    expect(log).toContain("/bin/chmod 755 /tmp/afora-parallels-npm-update-linux-");
    expect(log).toContain("/bin/rm -f /tmp/afora-parallels-npm-update-linux-");
...
if [[ "$args" == *" /usr/bin/tee /tmp/afora-parallels-npm-update-macos-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" /bin/chmod 700 /tmp/afora-parallels-npm-update-macos-"* ]]; then
  exit 0
fi
if [[ "$args" == *" /usr/sbin/chown desktop-user /tmp/afora-parallels-npm-update-macos-"* ]]; then
  exit 0
fi
if [[ "$args" == *" /bin/rm -f /tmp/afora-parallels-npm-update-macos-"* ]]; then
  exit 0
fi
exit 1
`,
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(call?.[1]).toEqual([
          ...
          "/bin/bash",
          expect.stringMatching(/^\/tmp\/afora-parallels-npm-update-macos-/),
        ]);
      },
    );
    ...
    expect(log).toContain("/usr/bin/tee /tmp/afora-parallels-npm-update-macos-");
    expect(log).toContain("/bin/chmod 700 /tmp/afora-parallels-npm-update-macos-");
  ...
  it("has a one-command beta validation mode with fresh target coverage", () => {
    ...
    expect(script).toContain("resolveAforaRegistryVersion");
    expect(script).toContain("this.options.updateTarget = version");
    expect(script).toContain("this.options.freshTargetSpec = `afora@${version}`");
  ...
  it("serves a prepared package set for both proof phases", () => {
    ...
    expect(script).toContain("this.freshTargetSpec = `afora@${this.targetTarballVersion}`");
    ...
    expect(script).toContain('readPositiveIntEnv("AFORA_PARALLELS_NPM_UPDATE_TIMEOUT_S", 2700)');
  });
  ...
  it("restarts the Windows gateway only after its current launch exits with the exact refusal", () => {
    ...
    expect(script).not.toContain("Invoke-Afora gateway restart");
  });
  ...
  it("accepts keyed and nested npm metadata for published update targets", () => {
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          ...
          "dist.tarball": "https://registry.example/afora-keyed.tgz",
          gitHead: "abcdef0123456789",
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/afora-keyed.tgz",
      gitHead: "abcdef0123456789",
    });
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          version: "2026.5.20-beta.1",
          dist: { tarball: "https://registry.example/afora-nested.tgz" },
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/afora-nested.tgz",
      gitHead: "",
    });
  });
  ...
  it("guards beta validation against cross-version harness checkouts", () => {
    ...
    expect(script).toContain("aforaVersionFamily");
    expect(script).toContain("AFORA_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH");
  ...
  it("bounds POSIX guest failure logs", () => {
    ...
    expect(scripts).toContain("AFORA_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES");
    ...
    expect(scripts).not.toContain("cat /tmp/afora-parallels-");
  });
  ...
  it("passes platform model timeouts to POSIX update agent turns", () => {
    ...
    withEnv(
      {
        AFORA_PARALLELS_LINUX_MODEL_TIMEOUT_S: undefined,
        AFORA_PARALLELS_MACOS_MODEL_TIMEOUT_S: undefined,
        AFORA_PARALLELS_MODEL_TIMEOUT_S: undefined,
      },
      () => {
    ...
    withEnv(
      {
        AFORA_PARALLELS_LINUX_MODEL_TIMEOUT_S: "321",
        AFORA_PARALLELS_MACOS_MODEL_TIMEOUT_S: "654",
      },
      () => {
  ...
  it("sets platform-aware fresh lane timeouts", () => {
    withEnv({ AFORA_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: undefined }, () => {
    ...
    withEnv({ AFORA_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: "3" }, () => {
      expect(freshLaneTimeoutMs("macos")).toBe(3000);
    });

    withEnv(
      { AFORA_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: String(Number.MAX_SAFE_INTEGER) },
      () => {
  ...
  it("clears update stream timers when spawning the guest command fails", async () => {
    ...
    await expect(
      runStreamingToJobLog.call(smoke, "afora-definitely-missing-command", [], 60 * 60 * 1000, {
        append: () => undefined,
        logPath: "",
  ...
  it("cleans timed-out Windows background work", async () => {
    ...
    expect(commands).toContain("function Write-AforaUtf8File");
    expect(commands).toContain("[System.Text.UTF8Encoding]::new($false)");
    expect(payloads).toContain("Write-AforaUtf8File $exitPath '0'");
    expect(payloads).toContain("Write-AforaUtf8File $donePath 'done'");
    expect(payloads).toContain("Write-AforaUtf8File $pidPath ([string]$PID)");
    expect(commands).toContain('cmd.exe /d /s /c start "" /b powershell.exe');
    expect(commands).toContain("icacls.exe $runDir /inheritance:r");
    expect(commands).toContain("Stop-AforaBackgroundProcessTree ([int]$backgroundPid)");
  ...
  it("drains a completed Windows job when setup exhausts the active budget", async () => {
    ...
    const fakeRun: typeof hostCommandRun = (_command, args, options) => {
      ...
      if (args.includes("cmd.exe")) {
        ...
        if (command.includes("__AFORA_BACKGROUND_DONE__")) {
          logProbes++;
  ...
  it("does not treat Windows background log text as completion control", async () => {
    ...
    expect(decodedCommands.join("\n")).toContain(
      "Stop-AforaBackgroundProcessTree ([int]$backgroundPid)",
    );
  });
  ...
  it("drains completed Windows background logs before cleanup", async () => {
    ...
    expect(decodedCommands.join("\n")).not.toContain("Stop-AforaBackgroundProcessTree");
    expect(decodedCommands.join("\n")).toContain(
      "Remove-Item -Path $scriptPath, $logPath, $donePath, $exitPath, $pidPath",
    );
  });
...
`,
  ...
  it("scrubs future plugin entries before invoking old same-guest updaters", () => {
    ...
    expect(macosScript).toContain('AFORA_BIN="$(resolve_required_command afora)"');
    expect(macosScript).toContain("/usr/local/bin:/usr/local/sbin");
    expect(macosScript).not.toContain("/opt/homebrew/bin/afora");
  });
  ...
  it("preserves bundled plugin inventory during updates while isolating POSIX gateway stops", () => {
    ...
    expect(windowsScript).toContain(
      "Invoke-WithScopedEnv @{ AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'",
    );
    expect(macosScript).toContain(
      'AFORA_DISABLE_BUNDLED_PLUGINS=1 "$AFORA_BIN" gateway stop',
    );
    expect(linuxScript).toContain(
      "AFORA_DISABLE_BUNDLED_PLUGINS=1 AFORA_ALLOW_ROOT=1 afora gateway stop",
    );
  });
  ...
  it("limits the Windows update environment to the update invocation", () => {
    ...
    const updateIndex = script.indexOf("Invoke-Afora update --tag");
    const scopedIndex = script.indexOf(
      "Invoke-WithScopedEnv @{ AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS",
    );
    const versionIndex = script.indexOf("Invoke-Afora --version", scopedIndex);
    const startIndex = script.indexOf("\nStart-AforaGateway\n", updateIndex);
    const agentIndex = script.indexOf("Invoke-Afora agent --local");
    ...
    expect(script).not.toContain("AFORA_DISABLE_BUNDLED_PLUGINS");
  });
  ...
  it("generates a .NET-safe Windows stale import regex in the update-failure guard", () => {
    ...
    expect(staleImportPattern).toBe(
      String.raw`node_modules\\afora\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`,
    );
    expect(staleImportPattern).not.toContain("node_modules\\afora\\dist\\");
    expect(staleImportPattern.match(/\\\\/g)).toHaveLength(4);
    const representativeUpdateFailure = String.raw`Error [ERR_MODULE_NOT_FOUND]: Cannot find module 'C:\Users\runner\AppData\Roaming\npm\node_modules\afora\dist\main-a1_B2.js' imported from C:\Users\runner\AppData\Roaming\npm\node_modules\afora\dist\cli.js`;
    const generatedRegex = new RegExp(staleImportPattern);
    expect(generatedRegex.test(representativeUpdateFailure)).toBe(true);
    expect(generatedRegex.test(String.raw`node_modules\afora\dist\main.js`)).toBe(false);
  });
});
