// Resolve Afora Package Candidate tests cover resolve afora package candidate script behavior.
...
import {
  ...
  cleanPackedAforaTarballsForTest,
  downloadUrl,
  ...
  validateAforaPackageSpec,
} from "../../scripts/resolve-afora-package-candidate.mts";
...
describe("resolve-afora-package-candidate", () => {
  it("allows Unreleased notes when packaging an exact ref candidate", () => {
    const script = readFileSync("scripts/resolve-afora-package-candidate.mts", "utf8");
    const refPackageBuild = script.slice(
      script.indexOf('if (options.source === "ref")'),
      script.indexOf('} else if (options.source === "npm")'),
    );

    expect(refPackageBuild).toContain('"scripts/package-afora-for-docker.mjs"');
    expect(refPackageBuild).toContain('"--allow-unreleased-changelog"');
  });

  it("accepts only Afora release package specs for npm candidates", () => {
    for (const spec of [
      "afora@beta",
      "afora@alpha",
      "afora@extended-stable",
      "afora@latest",
      "afora@2026.4.27",
      "afora@2026.4.27-1",
      "afora@2026.4.27-beta.2",
      "afora@2026.4.27-alpha.2",
    ]) {
      expect(validateAforaPackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateAforaPackageSpec("@evil/afora@1.0.0")).toThrow(
      "package_spec must be afora@alpha",
    );
    expect(() => validateAforaPackageSpec("afora@canary")).toThrow(
      "package_spec must be afora@alpha",
    );
    expect(() => validateAforaPackageSpec("afora@2026.04.27")).toThrow(
      "package_spec must be afora@alpha",
    );
    expect(() => validateAforaPackageSpec("afora@npm:other-package")).toThrow(
      "package_spec must be afora@alpha",
    );
    expect(() => validateAforaPackageSpec("afora@file:../other-package.tgz")).toThrow(
      "package_spec must be afora@alpha",
    );
  });
  ...
  it("rejects duplicate package candidate CLI options", () => {
    ...
    const duplicateCases = [
      ...
      [
        "--package-spec",
        [...requiredArgs, "--package-spec", "afora@beta", "--package-spec", "afora@latest"],
      ],
      [
        "--package-url",
        [...requiredArgs, "--package-url", "", "--package-url", "https://example.com/afora.tgz"],
      ],
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "afora@2026.5.26-beta.1",
      "C:\\afora\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack afora@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\afora\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("keeps npm pack filenames inside the package candidate output directory", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "afora-2026.6.17.tgz"), "package");

    await expect(
      moveNewestPackedTarballForTest(
        dir,
        JSON.stringify([{ filename: "afora-2026.6.17.tgz" }]),
        "afora-current.tgz",
      ),
    ).resolves.toBe(path.join(dir, "afora-current.tgz"));
    await expect(readFile(path.join(dir, "afora-current.tgz"), "utf8")).resolves.toBe("package");
  });
  ...
  it("reads npm 12 name-keyed package candidate filenames", async () => {
    ...
    await expect(
      moveNewestPackedTarballForTest(
        dir,
        JSON.stringify({ afora: { filename: "afora-2026.6.17.tgz" } }),
        "afora-current.tgz",
      ),
    ).resolves.toBe(path.join(dir, "afora-current.tgz"));
  });
  ...
  it("rejects path-like npm pack filenames instead of renaming outside the output directory", async () => {
    ...
    const unsafeFilenames = [
      "../afora-2026.6.17.tgz",
      "nested/afora-2026.6.17.tgz",
      "nested\\afora-2026.6.17.tgz",
      "/tmp/afora-2026.6.17.tgz",
      "C:\\temp\\afora-2026.6.17.tgz",
      "afora-2026.6.17.tar.gz",
    ];

    for (const filename of unsafeFilenames) {
      await expect(
        moveNewestPackedTarballForTest(dir, JSON.stringify([{ filename }]), "afora-current.tgz"),
  ...
  it("rejects unsafe text npm pack filenames instead of using loose stdout fallback", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "afora-2026.6.17.tgz"), "safe fallback");

    for (const filename of ["../afora-2026.6.17.tgz", "C:afora-2026.6.17.tgz"]) {
      await expect(
        moveNewestPackedTarballForTest(
          dir,
  ...
  it("cleans stale package tarballs before npm fallback scanning", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-npm-pack-stale-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "afora-9999.1.1.tgz"), "stale");
    await writeFile(path.join(dir, "afora-C:evil.tgz"), "unsafe");

    await cleanPackedAforaTarballsForTest(dir);
    await writeFile(path.join(dir, "afora-2026.6.17.tgz"), "current");

    await expect(
      moveNewestPackedTarballForTest(dir, "npm notice\n", "afora-current.tgz"),
    ).resolves.toBe(path.join(dir, "afora-current.tgz"));
    await expect(missing(path.join(dir, "afora-9999.1.1.tgz"))).resolves.toBe(true);
    await expect(readFile(path.join(dir, "afora-C:evil.tgz"), "utf8")).resolves.toBe("unsafe");
    await expect(readFile(path.join(dir, "afora-current.tgz"), "utf8")).resolves.toBe("current");
  });
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("clamps oversized package runner kill grace before scheduling", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-runner-grace-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
  ...
  it("rejects timed-out package runner commands when descendants exit cleanly", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-runner-timeout-clean-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    ...
    const childScript = [
      "const fs = require('node:fs');",
      "process.on('SIGTERM', () => {",
      "  fs.writeFileSync(process.env.AFORA_TEST_CHILD_CLEANUP, 'clean');",
      "  setTimeout(() => process.exit(0), 25);",
      "});",
      "fs.writeFileSync(process.env.AFORA_TEST_CHILD_READY, 'ready');",
      "setInterval(() => {}, 1000);",
    ].join("");
    const parentScript = [
      "const { spawn } = require('node:child_process');",
      ...
      "  env: {",
      "    ...process.env,",
      "    AFORA_TEST_CHILD_CLEANUP: process.env.AFORA_TEST_CHILD_CLEANUP,",
      "    AFORA_TEST_CHILD_READY: process.env.AFORA_TEST_CHILD_READY,",
      "  },",
      "});",
      "fs.writeFileSync(process.env.AFORA_TEST_CHILD_PID, String(child.pid));",
    ...
    const timeoutAssertion = expect(
      runCommandForTest(process.execPath, ["-e", parentScript], {
        env: {
          ...process.env,
          AFORA_TEST_CHILD_CLEANUP: cleanupPath,
          AFORA_TEST_CHILD_PID: childPidPath,
          AFORA_TEST_CHILD_READY: readyPath,
        },
        killAfterMs: 250,
        timeoutMs: 250,
      }),
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-runner-signal-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-afora-package-candidate.mts"),
    ).href;
    let childPid: number | undefined;
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/afora-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/afora-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-trusted-package-source-"));
    tempDirs.push(dir);
      ...
      JSON.stringify({
        ...
        sources: {
          "enterprise-artifactory": {
            ...
            pathPrefixes: ["/artifactory/afora/"],
            ports: [443, 8443],
  ...
  it("rejects unsafe package_url downloads before fetching private targets", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-download-"));
    tempDirs.push(dir);
    const target = path.join(dir, "afora.tgz");

    await expect(
      downloadUrl("http://packages.example/afora.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must use https");
    await expect(
      downloadUrl("https://user@packages.example/afora.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must not include credentials");
    await expect(
      downloadUrl("https://localhost/afora.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "127.0.0.1", family: 4 }]),
      }),
    ).rejects.toThrow(/private\/internal\/special-use/iu);
    await expect(
      downloadUrl("https://packages.example/afora.tgz", target, {
        fetchImpl: unexpectedFetch,
  ...
  it("allows private package_url downloads only through an explicit trusted source policy", async () => {
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/other/afora.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "203.0.113.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("matches trusted package_url path prefixes on path segment boundaries", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/afora/pkg.tgz", target, {
      fetchImpl: async (url: URL) => {
        requestedUrls.push(url.toString());
    ...
    expect(requestedUrls).toEqual(["https://packages.internal:8443/artifactory/afora/pkg.tgz"]);
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/afora-malicious/pkg.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "203.0.113.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("does not forward trusted package auth headers to redirect hosts", async () => {
    ...
    tempDirs.push(dir);
    const target = path.join(dir, "afora.tgz");
    const previousToken = process.env.AFORA_TRUSTED_PACKAGE_TOKEN;
    process.env.AFORA_TRUSTED_PACKAGE_TOKEN = "token-123";
    const trustedSource = {
      allowPrivateNetwork: true,
      auth: { type: "bearer" },
      hosts: ["packages.internal"],
    ...
    try {
      ...
      if (previousToken === undefined) {
        ...
        process.env.AFORA_TRUSTED_PACKAGE_TOKEN = previousToken;
      }
    }
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/afora.tgz"]);
  });
  ...
  it("bounds package_url downloads and writes completed files atomically", async () => {
    ...
    await downloadUrl("https://packages.example/afora.tgz", target, {
      fetchImpl: async () =>
  ...
  it("times out stalled package_url response bodies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-download-timeout-"));
    tempDirs.push(dir);
    const target = path.join(dir, "afora.tgz");
    let bodyCancelled = false;
    const startedAt = Date.now();
    ...
    await expect(
      ...
      "package_url download timed out after 25ms: https://packages.example/afora.tgz",
    );
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("normalizes artifact package source SHAs before workflow output", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-candidate-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("normalizes whitespace-only artifact package source SHAs to absent", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-candidate-empty-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("rejects malformed artifact package source SHAs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-candidate-bad-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("accepts uppercase package artifact SHA-256 metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-sha-"));
    tempDirs.push(dir);
    const file = path.join(dir, "afora.tgz");
    await writeFile(file, "afora package bytes");
  ...
  it("rejects source artifact scans that exceed the filesystem entry limit", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-artifact-scan-"));
    tempDirs.push(dir);
    const maxEntries = 3;
  ...
  it("rejects source artifact directories with multiple tarballs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-artifact-duplicates-"));
    tempDirs.push(dir);

    await writeFile(path.join(dir, "afora-a.tgz"), "a");
    ...
    expect(message).toContain("afora-a.tgz");
    expect(message).not.toContain(path.join(dir, "nested.tar.gz"));
    expect(message).not.toContain(path.join(dir, "afora-a.tgz"));
  });

  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "afora-package-build-info-"));
    tempDirs.push(dir);
    const root = path.join(dir, "package");
    await mkdir(path.join(root, "dist"), { recursive: true });
    await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "afora" }));
    await writeFile(
      path.join(root, "dist", "build-info.json"),
      JSON.stringify({ commit: "66CE632B9B7C5C7FDD3E66C739687D51638AD6E2" }),
    );
    const tarball = path.join(dir, "afora.tgz");
