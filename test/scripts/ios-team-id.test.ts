...
describe("scripts/ios-team-id.sh", () => {
  beforeAll(async () => {
    fixtureRoot = makeTempDir(tempDirs, "afora-ios-team-id-");
...
printf 'BBBBB22222\\t0\\tBeta Team\\r\\n'`,
  ...
  it("prefers the canonical Afora iOS team when it is present", async () => {
    const homeDir = makeTempDir(tempDirs, "afora-ios-team-id-canonical-");
    const binDir = path.join(homeDir, "bin");
    ...
    await writeExecutable(
      ...
      `#!/usr/bin/env bash
printf 'AAAAA11111\\t0\\tAlpha Team\\r\\n'
printf '${CANONICAL_TEAM_ID}\\t0\\tAfora\\r\\n'`,
    );
  ...
  it("loads teams from Xcode account identifier team metadata", async () => {
    const homeDir = makeTempDir(tempDirs, "afora-ios-team-id-by-identifier-");
    const binDir = path.join(homeDir, "bin");
    await mkdir(path.join(homeDir, "Library", "Preferences"), { recursive: true });
    await mkdir(binDir, { recursive: true });
    await writeFile(
...
if [[ "$1" == "-extract" && "$2" == "IDEProvisioningTeamByIdentifier" ]]; then
  cat <<'JSON'
{"account-id":[{"teamID":"FWJYW4S8P8","teamName":"Afora Foundation","isFreeProvisioningTeam":false,"teamType":"Company"}]}
JSON
  exit 0
fi
echo '{}'`,
  ...
  it("fails canonical-only resolution when only fallback teams are available", () => {
    ...
    expect(result.stderr).toContain(
      `Canonical Afora iOS Team ID '${CANONICAL_TEAM_ID}' is not available`,
    );
  });
  ...
  it("rejects explicit non-canonical teams in canonical-only mode", () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.stderr).toContain("is not the canonical Afora iOS team");
  });
