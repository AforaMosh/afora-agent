...
import { buildMarkdown, parseArgs } from "../../scripts/afora-performance-source-summary.mts";

const tmpRoots: string[] = [];

function mkTmpRoot() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-source-summary-"));
  tmpRoots.push(root);
  return root;
}
...
function runCli(...args: string[]) {
  return spawnSync(
    process.execPath,
    ["--import", "tsx", "scripts/afora-performance-source-summary.mts", ...args],
    {
      cwd: path.resolve("."),
      encoding: "utf8",
    },
