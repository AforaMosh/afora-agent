...
function runStopExistingLocalApp(params: { fakeLsof?: string; fakePgrep: string }) {
  const root = mkdtempSync(join(tmpdir(), "afora-build-run-mac-test-"));
  tempRoots.push(root);
  ...
  writeFileSync(
    ...
    [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
      'BIN_ABS="/worktree/apps/macos/.build-local/debug/Afora"',
      'BIN=".build-local/debug/Afora"',
      'APP_CWD="/worktree/apps/macos"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$AFORA_TEST_KILL_CALLS"',
      '  touch "$AFORA_TEST_KILLED_MARKER"',
      "  return 0",
      "}",
      stopFunction,
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      AFORA_TEST_KILLED_MARKER: join(root, "killed"),
      AFORA_TEST_KILL_CALLS: killCallsPath,
      AFORA_TEST_PGREP_CALLS: pgrepCallsPath,
      AFORA_TEST_PGREP_COUNT: join(root, "pgrep-count.txt"),
...
describe("scripts/build-and-run-mac.sh", () => {
  ...
  it("stops only the local debug app binary before relaunching", () => {
    ...
    const { killCalls, pgrepCalls, result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        `printf '%s\\n' "$*" >> "$AFORA_TEST_PGREP_CALLS"`,
        'count="$(cat "$AFORA_TEST_PGREP_COUNT" 2>/dev/null || echo 0)"',
        'next="$((count + 1))"',
        'printf "%s\\n" "$next" > "$AFORA_TEST_PGREP_COUNT"',
        'if [[ "$2" == "/worktree/apps/macos/.build-local/debug/Afora" ]]; then exit 1; fi',
        'if [[ "$2" == ".build-local/debug/Afora" && "$count" == "1" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
    ...
    expect(pgrepCalls).toContain("-f /worktree/apps/macos/.build-local/debug/Afora");
    expect(pgrepCalls).toContain("-f .build-local/debug/Afora");
  ...
  it("fails when the scoped local app process survives cleanup", () => {
    const { result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        'if [[ "$2" == ".build-local/debug/Afora" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
