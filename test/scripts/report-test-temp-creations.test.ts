...
describe("report-test-temp-creations", () => {
  ...
  it("reports repository-observed mkdtemp call forms", () => {
    const sources = [
      ...
      ["const root = await fsPromises.", "mkdtemp", '("/tmp/afora-case-");'].join(""),
  ...
  it("honors explicit allow comments with reasons", () => {
    ...
    const inlineAllowedSource = `const inlineAllowed = ${tmpDirCall}; // afora-temp-dir: allow verifies tmp API behavior`;
    const blockedSource = `const blocked = ${mkdtempCall};`;
    const stringMarkerSource = `const stringMarker = ${mkdtempCall}; const note = "afora-temp-dir: allow quoted text";`;
  ...
  it("formats GitHub warning annotations for CI report mode", () => {
    expect(
      formatGithubWarning({
        ...
        line: 12,
        reason: "new mkdtemp temp directory creation",
        // afora-temp-dir: allow test fixture for GitHub warning formatting
  ...
  it("reads staged source for manual helper scans", () => {
    const root = tempDirs.make("afora-temp-report-staged-source-");
  ...
  it("exits non-zero for staged findings when requested", () => {
    const root = tempDirs.make("afora-temp-report-");
    const env = createNestedGitEnv();
    execFileSync("git", ["init", "-q", "--initial-branch=main"], { cwd: root, env });
  ...
  it("falls back to a two-dot diff when refs have no merge base", () => {
    const root = tempDirs.make("afora-temp-report-no-merge-base-");
    const env = createNestedGitEnv();
