...
const AFORA_E2E_INSTANCE_HELPER_PATH = "scripts/lib/afora-e2e-instance.sh";
...
describe("docker build helper", () => {
  it("allows deployments to build an immutable sandbox image tag", () => {
    ...
    expect(script).toContain(
      'IMAGE_NAME="${AFORA_SANDBOX_IMAGE:-afora-sandbox:bookworm-slim}"',
    );
  });

  it("treats Docker registry auth 5xx failures as transient build failures", () => {
    const workDir = tempDirs.make("afora-docker-build-transient-");
...
`;
  ...
  it("detects Docker builder memory exhaustion failures", () => {
    const workDir = tempDirs.make("afora-docker-build-memory-");
    const logPath = join(workDir, "docker-build.log");
    writeFileSync(
      logPath,
      [
...
`;
  ...
  it("detects compiler processes killed by the OOM killer", () => {
    const workDir = tempDirs.make("afora-docker-build-killed-compiler-");
    const logPath = join(workDir, "docker-build.log");
...
`;
  ...
  it("retries Corepack connect timeouts without misreading Dockerfile comments as OOM", () => {
    const workDir = tempDirs.make("afora-docker-build-connect-timeout-");
    const logPath = join(workDir, "docker-build.log");
    writeFileSync(
      logPath,
      [
...
`;
  ...
  it("routes standalone Docker smoke runs through the timeout-aware helper", () => {
    ...
    expect(cleanupSmoke).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${AFORA_CLEANUP_SMOKE_DOCKER_TIMEOUT:-600s}}"',
    );
    expect(cleanupSmoke).toContain(
    ...
    expect(installE2eSmoke).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${AFORA_INSTALL_E2E_DOCKER_TIMEOUT:-2700s}}"',
    );
  ...
  it("runs the sandbox browser sidecar proof from the package-installed image", () => {
    ...
    expect(scenario).toContain('from "afora-agent/plugin-sdk/agent-harness-runtime"');
  ...
  it("gives cleanup-smoke builds enough Node heap while preserving explicit callers", () => {
    ...
    expect(cleanupRun.indexOf("ensure_cleanup_smoke_node_options")).toBeLessThan(
      cleanupRun.indexOf("pnpm build >/tmp/afora-cleanup-build.log"),
    );
  });

  it("rejects invalid cleanup-smoke log byte limits", () => {
    const workDir = tempDirs.make("afora-cleanup-smoke-log-invalid-");
    const logPath = join(workDir, "cleanup.log");
    writeFileSync(logPath, "cleanup output\n");
    const script = `
set -euo pipefail
LOG_PATH=${shellQuote(logPath)}
export AFORA_CLEANUP_SMOKE_LOG_PRINT_BYTES=64kb
...
`;
    ...
    expect(result.stderr).toContain("invalid AFORA_CLEANUP_SMOKE_LOG_PRINT_BYTES: 64kb");
    expect(result.stdout).toBe("");
  });

  it("normalizes zero-padded cleanup-smoke log byte limits", () => {
    const workDir = tempDirs.make("afora-cleanup-smoke-log-tail-");
    const logPath = join(workDir, "cleanup.log");
    writeFileSync(logPath, "old-cleanup-output-recent\n");
    const script = `
set -euo pipefail
LOG_PATH=${shellQuote(logPath)}
export AFORA_CLEANUP_SMOKE_LOG_PRINT_BYTES=0008
...
`;
  ...
  it("prints in-container Docker client logs through bounded helpers", () => {
    for (const scriptPath of [CODEX_MEDIA_PATH_SCENARIO_PATH, OPENAI_CHAT_TOOLS_SCENARIO_PATH]) {
      ...
      expect(script, scriptPath).toContain("source scripts/lib/afora-e2e-instance.sh");
      expect(script, scriptPath).toContain('afora_e2e_print_log "$CLIENT_LOG"');
      expect(script, scriptPath).not.toContain('cat "$CLIENT_LOG"');
    }
  });
  ...
  it("runs cleanup smoke on the native ARM platform instead of pulling an amd64 tag", () => {
    ...
    expect(
      runCleanupDefaultPlatform({ AFORA_CLEANUP_SMOKE_PLATFORM: "linux/s390x" }, "x86_64"),
    ).toBe("linux/s390x");
  });
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(liveBuild).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${AFORA_LIVE_DOCKER_PULL_TIMEOUT:-600s}}"',
    );
    expect(liveBuild).toContain(
      'LIVE_IMAGE_PULL_ATTEMPTS="${AFORA_LIVE_DOCKER_PULL_ATTEMPTS:-3}"',
    );
    ...
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (AFORA_SKIP_DOCKER_BUILD=1)"',
    );
  });
  ...
  it("resolves source and compiled candidate test-state entrypoints", () => {
    ...
    expect(sourceResult.stdout.trim()).toBe(
      join(process.cwd(), "scripts/lib/afora-test-state.mts"),
    );

    const compiledRoot = tempDirs.make("afora-compiled-test-state-");
    const missingResult = resolveEntrypoint(compiledRoot);
    expect(missingResult.status).toBe(1);
    expect(missingResult.stderr).toContain("Afora test-state entrypoint not found");

    const compiledDir = join(compiledRoot, "scripts/lib");
    mkdirSync(compiledDir, { recursive: true });
    const compiledEntrypoint = join(compiledDir, "afora-test-state.mjs");
  ...
  it("runs current TypeScript and frozen JavaScript Docker harness entrypoints", () => {
    const fixtureRoot = tempDirs.make("afora-docker-script-entrypoint-");
    ...
    const runFixture = (value: string) =>
      spawnSync(
        ...
        [
          "-c",
          `source "${AFORA_E2E_INSTANCE_HELPER_PATH}"; afora_e2e_run_script_entrypoint "$1" "$2"`,
          "afora-docker-script-entrypoint",
          scriptStem,
          value,
        ],
        {
  ...
  it("rejects malformed Docker E2E resource limits before a suite starts", () => {
    ...
    const runProbe = (value: string) => {
      const script = [
        ...
        "docker_e2e_read_nonnegative_decimal_env AFORA_SAMPLE_RESOURCE_LIMIT 2048",
      ].join("\n");
      return spawnSync("bash", ["-c", script], {
        ...
        env: {
          ...process.env,
          AFORA_SAMPLE_RESOURCE_LIMIT: value,
        },
      });
    };
    ...
    expect(invalid.stderr).toContain("invalid AFORA_SAMPLE_RESOURCE_LIMIT: 12mb");
    expect(overlarge.status).toBe(2);
    expect(overlarge.stderr).toContain("invalid AFORA_SAMPLE_RESOURCE_LIMIT: 9999999999");
    expect(overprecise.status).toBe(2);
    expect(overprecise.stderr).toContain("invalid AFORA_SAMPLE_RESOURCE_LIMIT: 12.1234567");
  ...
  it("keeps Testbox image-build fallback before isolating live MCP code-mode runtime flags", () => {
    ...
    const unsetIndex = script.indexOf("unset AFORA_TESTBOX");
  ...
  it("wraps centralized Docker builds with the timeout helper", () => {
    const workDir = tempDirs.make("afora-docker-build-timeout-");
...
export AFORA_DOCKER_BUILD_TIMEOUT=17s
...
`;
  ...
  it("prints heartbeat progress for long successful centralized Docker builds", () => {
    const workDir = tempDirs.make("afora-docker-build-heartbeat-");
    writeExecutables(join(workDir, "bin"), {
      timeout: `#!/bin/bash
set -euo pipefail
if [[ "$1" = "--kill-after=1s" ]]; then
...
`,
    ...
    const script = repoShell(workDir)`
export PATH="$TMPDIR/bin:$PATH"
export AFORA_DOCKER_BUILD_HEARTBEAT_SECONDS=1
...
`;
  ...
  it("stops the tracked build command without retrying when interrupted", async () => {
    const workDir = tempDirs.make("afora-docker-build-signal-");
    writeExecutables(join(workDir, "bin"), {
      docker: `#!/bin/bash
set -euo pipefail
count=0
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export AFORA_DOCKER_BUILD_RETRIES=3
source "$ROOT_DIR/scripts/lib/docker-build.sh"
docker_build_run e2e-build -t demo-image .
`,
    });
  ...
  it("does not delay fast successful centralized Docker builds until the next heartbeat", () => {
    const workDir = tempDirs.make("afora-docker-build-fast-heartbeat-");
    writeExecutables(join(workDir, "bin"), {
      timeout: `#!/bin/bash
set -euo pipefail
if [[ "$1" = "--kill-after=1s" ]]; then
...
`,
    ...
    const script = repoShell(workDir)`
export PATH="$TMPDIR/bin:$PATH"
export AFORA_DOCKER_BUILD_HEARTBEAT_SECONDS=30
...
`;
  ...
  it("normalizes zero-padded centralized Docker build heartbeat intervals", () => {
    const script = repoRootShell`
export ROOT_DIR
export AFORA_DOCKER_BUILD_HEARTBEAT_SECONDS=08
...
`;
  ...
  it("normalizes zero-padded centralized Docker build retry counts", () => {
    const script = repoRootShell`
export ROOT_DIR
export AFORA_DOCKER_BUILD_RETRIES=08
...
`;
  ...
  it.each([
    [
      "retry count",
      "AFORA_DOCKER_BUILD_RETRIES",
      "2x",
      "invalid AFORA_DOCKER_BUILD_RETRIES: 2x",
    ],
    [
      "heartbeat interval",
      "AFORA_DOCKER_BUILD_HEARTBEAT_SECONDS",
      "soon",
      "invalid AFORA_DOCKER_BUILD_HEARTBEAT_SECONDS: soon",
    ],
  ])(
    "rejects invalid centralized Docker build %s before invoking docker",
    (_label, envName, value, expectedError) => {
      const workDir = tempDirs.make("afora-docker-build-config-");
...
`;
  ...
  it("fails centralized Docker builds fast when timeout is unavailable", () => {
    const workDir = tempDirs.make("afora-docker-build-timeout-required-");
    mkdirSync(join(workDir, "bin"));
    const script = repoShell(workDir)`
export PATH="$TMPDIR/bin"
export AFORA_DOCKER_BUILD_TIMEOUT=19s
...
`;
  ...
  it("keeps setup-style Docker builds compatible when timeout is unavailable", () => {
    const workDir = tempDirs.make("afora-docker-build-timeout-optional-");
    writeExecutables(join(workDir, "bin"), {
      env: `#!/bin/sh
while [ "$#" -gt 0 ]; do
  case "$1" in
...
`,
    ...
    const script = repoShell(workDir)`
export PATH="$TMPDIR/bin"
export AFORA_DOCKER_BUILD_TIMEOUT=23s
...
grep -q '^image inspect afora-reuse-image$' "$TMPDIR/docker-seen"
grep -q '^pull afora-reuse-image$' "$TMPDIR/docker-seen"
`,
    },
    {
      title: "explains how to opt out when Docker rejects default resource limits",
      tempPrefix: "afora-docker-resource-diagnostic-",
      scriptSource: (workDir: string) => repoShell(workDir)`
export AFORA_DOCKER_E2E_AVAILABLE_CPUS=8
unset AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset AFORA_DOCKER_E2E_MEMORY AFORA_DOCKER_E2E_CPUS AFORA_DOCKER_E2E_PIDS_LIMIT
...
[[ "$stderr" = *"AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS=1"* ]]
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-resource-unrelated-",
      scriptSource: (workDir: string) => repoShell(workDir)`
export AFORA_DOCKER_E2E_AVAILABLE_CPUS=8
unset AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset AFORA_DOCKER_E2E_MEMORY AFORA_DOCKER_E2E_CPUS AFORA_DOCKER_E2E_PIDS_LIMIT
...
[[ "$stderr" != *"AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS"* ]]
[[ "$(grep -c '^run ' "$TMPDIR/docker-seen")" = "1" ]]
`,
    },
    {
      title: "rejects invalid Docker run pids limits before invoking docker",
      tempPrefix: "afora-docker-resource-pids-",
      scriptSource: (workDir: string) => repoShell(workDir)`
...
set +e
AFORA_DOCKER_E2E_PIDS_LIMIT=many docker_e2e_docker_cmd run demo 2>"$TMPDIR/stderr"
status="$?"
set -e

[[ "$status" = "2" ]]
[[ "$(<"$TMPDIR/stderr")" = *"invalid AFORA_DOCKER_E2E_PIDS_LIMIT: many"* ]]
[[ ! -e "$TMPDIR/docker-seen" ]]
`,
    },
    {
      title: "rejects invalid package-backed Docker run pids limits before invoking docker",
      tempPrefix: "afora-docker-package-pids-",
      scriptSource: (workDir: string) => repoShell(workDir)`
...
set +e
AFORA_DOCKER_E2E_PIDS_LIMIT=many docker_e2e_docker_run_cmd run demo 2>"$TMPDIR/stderr"
status="$?"
set -e
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-package-diagnostic-",
      scriptSource: (workDir: string) => repoShell(workDir)`
export AFORA_DOCKER_E2E_AVAILABLE_CPUS=8
unset AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset AFORA_DOCKER_E2E_MEMORY AFORA_DOCKER_E2E_CPUS AFORA_DOCKER_E2E_PIDS_LIMIT
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-build-cleanup-",
      scriptSource: (workDir: string) => repoShell(workDir)`

node() {
  local script="$1"
  shift
  if [[ "$script" != "$ROOT_DIR/scripts/package-afora-for-docker.mjs" ]]; then
    command node "$script" "$@"
    return
  fi
...
docker_build_run() {
  ...
  for arg in "$@"; do
    case "$arg" in
      afora_package=*)
        build_context="\${arg#afora_package=}"
        ;;
    esac
  done

  test -n "$build_context"
  test -f "$build_context/afora-current.tgz"
...
leftovers="$(find "$TMPDIR" -maxdepth 1 \\( \\
  -name 'afora-docker-e2e-pack.*' \\
  -o -name 'afora-docker-e2e-package-context.*' \\
\\) -print)"
if [[ -n "$leftovers" ]]; then
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-build-external-package-",
      scriptSource: (workDir: string) => repoShell(workDir)`

external_dir="$TMPDIR/external-package"
mkdir -p "$external_dir"
printf fixture >"$external_dir/afora-current.tgz"
AFORA_CURRENT_PACKAGE_TGZ="$external_dir/afora-current.tgz"
export AFORA_CURRENT_PACKAGE_TGZ
...
test -f "$TMPDIR/build-context-seen"
test -f "$AFORA_CURRENT_PACKAGE_TGZ"
leftovers="$(find "$TMPDIR" -maxdepth 1 -name 'afora-docker-e2e-package-context.*' -print)"
if [[ -n "$leftovers" ]]; then
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-package-mount-cleanup-",
      scriptSource: (workDir: string) => repoShell(workDir)`
export DOCKER_COMMAND_TIMEOUT=3s
...
test ! -e "$pack_dir"
test -z "$(find "$TMPDIR" -maxdepth 1 -name 'afora-docker-e2e-container.*' -print)"

external_dir="$TMPDIR/external-package"
mkdir -p "$external_dir"
printf fixture >"$external_dir/afora-current.tgz"
docker_e2e_package_mount_args "$external_dir/afora-current.tgz"
...
test -f "$external_dir/afora-current.tgz"
`,
    },
    {
      title: "propagates shared E2E command timeouts into package-backed containers",
      tempPrefix: "afora-docker-package-timeout-env-",
      scriptSource: (workDir: string) => repoShell(workDir)`
source "$ROOT_DIR/scripts/lib/docker-e2e-package.sh"

package="$TMPDIR/afora-current.tgz"
printf fixture >"$package"
export AFORA_E2E_NPM_INSTALL_TIMEOUT=42s
export AFORA_E2E_COMMAND_TIMEOUT=23s
docker_e2e_package_mount_args "$package"
printf "%s\\n" "\${DOCKER_E2E_PACKAGE_ARGS[@]}" >"$TMPDIR/package-args"

grep -qx -- "-e" "$TMPDIR/package-args"
grep -qx -- "AFORA_CURRENT_PACKAGE_TGZ=/tmp/afora-current.tgz" "$TMPDIR/package-args"
grep -qx -- "AFORA_E2E_NPM_INSTALL_TIMEOUT=42s" "$TMPDIR/package-args"
grep -qx -- "AFORA_E2E_COMMAND_TIMEOUT=23s" "$TMPDIR/package-args"
`,
    },
    {
      title:
        "keeps both harness run wrappers available when the package helper is sourced directly",
      tempPrefix: "afora-docker-package-helper-guard-",
      scriptSource: (workDir: string) => repoShell(workDir)`
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-harness-stdin-",
      scriptSource: (workDir: string) => repoShell(workDir)`
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-e2e-log-print-tail-",
      scriptSource: (workDir: string) => repoShell(workDir)`
export AFORA_DOCKER_E2E_LOG_PRINT_BYTES=64
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-e2e-log-heartbeat-",
      scriptSource: (workDir: string) => repoShell(workDir)`
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-e2e-log-term-cleanup-",
      scriptSource: (workDir: string) => repoShell(workDir)`
export AFORA_DOCKER_E2E_HEARTBEAT_TERM_GRACE_SECONDS=1
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-e2e-harness-term-cleanup-",
      scriptSource: (workDir: string) => repoShell(workDir)`
...
`,
    },
    {
      ...
      tempPrefix: "afora-docker-e2e-stats-zero-heartbeat-",
      scriptSource: (workDir: string) => repoShell(workDir)`
...
`,
  ...
  it("derives the browser CDP image from the shared functional image", () => {
    const workDir = tempDirs.make("afora-browser-cdp-shared-image-");
    writeExecutables(join(workDir, "bin"), {
...
export PATH="$TMPDIR/bin:$PATH"
export AFORA_SKIP_DOCKER_BUILD=1
export AFORA_DOCKER_E2E_IMAGE=shared-functional
export AFORA_DOCKER_ALL_LANE_NAME=browser-cdp-snapshot

bash "$ROOT_DIR/scripts/e2e/browser-cdp-snapshot-docker.sh"

grep -q '^image inspect shared-functional$' "$TMPDIR/docker-seen"
grep -Fq 'build -t afora-browser-cdp-snapshot-e2e:browser-cdp-snapshot' "$TMPDIR/docker-seen"
grep -Fq ' afora-browser-cdp-snapshot-e2e:browser-cdp-snapshot ' "$TMPDIR/docker-seen"
...
`;
  ...
  it("fails fast on invalid browser CDP snapshot byte limits", () => {
    const result = spawnSync("bash", [BROWSER_CDP_SNAPSHOT_DOCKER_E2E_PATH], {
      ...
      env: {
        ...process.env,
        AFORA_BROWSER_CDP_SNAPSHOT_MAX_BYTES: "64kb",
        AFORA_SKIP_DOCKER_BUILD: "1",
      },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid AFORA_BROWSER_CDP_SNAPSHOT_MAX_BYTES: 64kb");
  });
  ...
  it("forwards browser CDP snapshot byte limits into the Docker runner", () => {
    ...
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env AFORA_BROWSER_CDP_SNAPSHOT_MAX_BYTES 524288",
    );
    expect(runner).toContain('-e "AFORA_BROWSER_CDP_SNAPSHOT_MAX_BYTES=$SNAPSHOT_MAX_BYTES"');
  });
  ...
  it("opens the browser CDP fixture before snapshotting", () => {
    const runner = readFileSync(BROWSER_CDP_SNAPSHOT_DOCKER_E2E_PATH, "utf8");
    const quarantineIndex = runner.indexOf("mkdir -p /tmp/afora-browser-cdp");
  ...
  it("fails Docker commands fast when timeout is unavailable", () => {
    const workDir = tempDirs.make("afora-docker-timeout-required-");
    mkdirSync(join(workDir, "bin"));
    const script = repoShell(workDir)`
export PATH="$TMPDIR/bin"
...
`;
  ...
  it("uses a Node watchdog for Docker commands when timeout is unavailable", () => {
    const workDir = tempDirs.make("afora-docker-node-timeout-");
    writeExecutables(join(workDir, "bin"), {
...
`;
  ...
  it("adds default Docker run resource limits without overriding explicit limits", () => {
    const workDir = tempDirs.make("afora-docker-resource-limits-");
    writeExecutables(join(workDir, "bin"), {
      timeout: `#!/bin/bash
set -euo pipefail
if [[ "$1" = "--kill-after=1s" ]]; then
...
unset AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset AFORA_DOCKER_E2E_MEMORY AFORA_DOCKER_E2E_CPUS AFORA_DOCKER_E2E_PIDS_LIMIT
export AFORA_DOCKER_E2E_AVAILABLE_CPUS=32
...
AFORA_DOCKER_E2E_MEMORY=12g AFORA_DOCKER_E2E_CPUS=4 AFORA_DOCKER_E2E_PIDS_LIMIT=512 docker_e2e_docker_cmd run demo
AFORA_DOCKER_E2E_AVAILABLE_CPUS=8 AFORA_DOCKER_E2E_MEMORY=12g AFORA_DOCKER_E2E_CPUS=16 AFORA_DOCKER_E2E_PIDS_LIMIT=512 docker_e2e_docker_cmd run demo
docker_e2e_docker_cmd run --memory 2g --cpus 3 --pids-limit 99 demo
AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS=1 docker_e2e_docker_cmd run demo
...
`;
  ...
  it("runs Docker when resource diagnostic capture is unavailable", () => {
    const workDir = tempDirs.make("afora-docker-resource-no-temp-");
...
`;
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Docker watchdog children that ignore parent SIG${shellSignal}`, () => {
      const workDir = tempDirs.make("afora-docker-node-signal-");
      writeExecutables(join(workDir, "bin"), {
        node: `#!/bin/bash\nexec ${shellQuote(process.execPath)} "$@"\n`,
        docker: `#!/bin/bash
printf "%s\\n" "$$" >"$TMPDIR/docker-pid"
...
export AFORA_DOCKER_TIMEOUT_KILL_GRACE_MS=100
...
`;
  ...
  it("uses plain timeout when kill-after is unsupported", () => {
    const workDir = tempDirs.make("afora-docker-plain-timeout-");
    writeExecutables(join(workDir, "bin"), {
      timeout: `#!/bin/bash
set -euo pipefail
if [[ "$1" = "--kill-after=1s" ]]; then
...
`;
  ...
  it("uses gtimeout when timeout is unavailable", () => {
    const workDir = tempDirs.make("afora-docker-gtimeout-");
    writeExecutables(join(workDir, "bin"), {
      gtimeout: `#!/bin/bash
set -euo pipefail
if [[ "$1" = "--kill-after=1s" ]]; then
...
`,
    ...
    const script = repoShell(workDir)`
export PATH="$TMPDIR/bin"
export AFORA_DOCKER_E2E_RUN_TIMEOUT=13s
export AFORA_DOCKER_E2E_AVAILABLE_CPUS=8
unset AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset AFORA_DOCKER_E2E_MEMORY AFORA_DOCKER_E2E_CPUS AFORA_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("keeps package-backed Docker runs bounded when the package helper is sourced directly", () => {
    const workDir = tempDirs.make("afora-docker-package-timeout-required-");
    mkdirSync(join(workDir, "bin"));
    const script = repoShell(workDir)`
export PATH="$TMPDIR/bin"
export AFORA_DOCKER_E2E_RUN_TIMEOUT=11s
...
`;
  ...
  it("uses gtimeout for package-backed Docker runs sourced through the package helper", () => {
    const workDir = tempDirs.make("afora-docker-package-gtimeout-");
    writeExecutables(join(workDir, "bin"), {
      gtimeout: `#!/bin/bash
set -euo pipefail
if [[ "$1" = "--kill-after=1s" ]]; then
...
`,
    ...
    const script = repoShell(workDir)`
export PATH="$TMPDIR/bin"
export AFORA_DOCKER_E2E_RUN_TIMEOUT=15s
export AFORA_DOCKER_E2E_AVAILABLE_CPUS=8
unset AFORA_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset AFORA_DOCKER_E2E_MEMORY AFORA_DOCKER_E2E_CPUS AFORA_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("passes plugin lifecycle sampler timeout overrides into Docker", () => {
    ...
    expectTextToIncludeAll(runner, [
      ...
      "append_positive_int_env AFORA_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS",
      "append_positive_int_env AFORA_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS",
      "append_positive_int_env AFORA_PLUGIN_LIFECYCLE_METRIC_POLL_MS",
      "append_positive_int_env AFORA_PLUGIN_LIFECYCLE_MAX_RSS_KB",
      "append_positive_int_env AFORA_PLUGIN_LIFECYCLE_MAX_WALL_MS",
      "append_positive_number_env AFORA_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO",
      'docker_e2e_run_with_harness \\\n  "${DOCKER_ENV_ARGS[@]}"',
    ]);
  });

  it.each([
    ["phase timeout", "AFORA_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS", "150ms"],
    ["CPU ratio", "AFORA_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO", "0"],
  ])(
    ...
    (_label, envName, value) => {
      const result = spawnSync("bash", [PLUGIN_LIFECYCLE_MATRIX_DOCKER_E2E_PATH], {
        ...
        env: {
          ...process.env,
          AFORA_CURRENT_PACKAGE_TGZ: "/tmp/afora-missing-package.tgz",
          [envName]: value,
        },
      });
      ...
      expect(result.stderr).not.toContain("Afora package tarball does not exist");
    },
  );
  ...
  it("wraps direct Docker E2E npm installs with the shared timeout helper", () => {
    ...
    expect(multiNode).toContain(
      'afora_e2e_install_package "$ARTIFACTS/install-a.log" "Afora package under node-A prefix" "$NPM_PREFIX_A"',
    );
    expectTextToIncludeAll(updateChannel, [
      'afora_e2e_maybe_timeout "${AFORA_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
      'afora_e2e_maybe_timeout "${AFORA_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path"',
      "afora_e2e_print_log /tmp/afora-git-install.log",
      'afora_e2e_print_log "$package_install_log"',
    ]);

    expect(updateChannel).not.toContain("cat /tmp/afora-git-install.log");
    expect(updateChannel).not.toContain('cat "$package_install_log"');
    expect(doctorSwitch).toContain(
      'afora_e2e_maybe_timeout "${AFORA_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(doctorSwitch).toContain(
      'afora_e2e_maybe_timeout "${AFORA_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz"',
    );
    for (const script of [releaseUpgrade, upgradeSurvivor, pluginCorrupt]) {
      expect(script).toContain(
        'afora_e2e_maybe_timeout "${AFORA_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g',
      );
    }
  });
  ...
  it("keeps upgrade survivor mutable state off the host-mounted artifact tree", () => {
    ...
    for (const script of [runner, publishedRunner]) {
      expectTextToIncludeAll(script, [
        "afora-upgrade-survivor-runtime",
        "AFORA_UPGRADE_SURVIVOR_TMPDIR",
        "AFORA_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR",
      ...
      expect(script).not.toContain('export TMPDIR="$AFORA_UPGRADE_SURVIVOR_ARTIFACT_ROOT/tmp"');
      expect(script).not.toContain('export npm_config_cache="$ARTIFACT_ROOT/npm-cache"');
      expect(script).not.toContain(
        'export npm_config_cache="$AFORA_UPGRADE_SURVIVOR_ARTIFACT_ROOT/npm-cache"',
      );
    }
  });
  ...
  it("lets upgrade survivor fixture registries resolve transitive public packages", () => {
    ...
    for (const script of [runner, publishedRunner]) {
      expect(script).toContain("AFORA_NPM_REGISTRY_UPSTREAM=https://registry.npmjs.org");
    ...
    expect(runner).toContain('AFORA_NPM_REGISTRY_DIST_TAGS="beta=$package_version"');
    expect(publishedRunner).toContain('AFORA_NPM_REGISTRY_DIST_TAGS="beta=$candidate_version"');
  });
  ...
  it("starts the upgrade survivor plugin registry before updates with scenario-owned config", () => {
    ...
    expect(publishedRunner).toContain('prepublish_package="@afora/whatsapp"');
    expect(publishedRunner).toContain('if [ "$SCENARIO" = "configured-plugin-installs" ]; then');
    expect(publishedRunner).toContain('prepublish_package="@afora/matrix"');
    expect(publishedRunner).toContain(
      'assert-prepublish-requests "$AFORA_CLAWHUB_URL" "$prepublish_package" "$candidate_version"',
    );
    expect(publishedRunner).toContain(
      'local tarball="$fixture_root/afora-brave-plugin-${candidate_version}.tgz"',
    );
    expect(publishedRunner).toContain('FIXTURE_PACKAGE_VERSION="$candidate_version"');
    expect(publishedRunner).toContain("version,");
    expect(publishedRunner).toContain(
      'registry_args+=("@afora/brave-plugin" "$candidate_version" "$tarball")',
    );
    ...
    expect(runner.indexOf('afora "${update_args[@]}"')).toBeLessThan(
      runner.indexOf(
    ...
    ).toBeLessThan(runner.indexOf("afora doctor --fix --non-interactive"));
    expect(runner).toContain(
    ...
    expectTextToIncludeAll(publishedRunner, [
      ...
      "cmp -s",
      "'^(GATEWAY_AUTH_TOKEN_REF|AFORA_CLAWHUB_URL)='",
      "AFORA_CLAWHUB_URL=%s",
    ]);
    ...
    expect(
      runner.match(
        /-v "\$HARNESS_ROOT_DIR\/scripts\/e2e\/lib\/clawhub-fixture-server\.cjs:\/tmp\/afora-clawhub-fixture-server\.cjs:ro"/gu,
      ),
    ).toHaveLength(2);
    expect(
      runner.match(
        /-e AFORA_UPGRADE_SURVIVOR_CLAWHUB_FIXTURE_SERVER=\/tmp\/afora-clawhub-fixture-server\.cjs/gu,
      ),
    ).toHaveLength(2);
  });
  ...
  it("wraps package-backed scenario Afora CLI calls with the shared timeout helper", () => {
    const paths = [
    ...
    for (const path of paths) {
      ...
      expect(script, path).toContain("afora_e2e_enable_afora_cli_timeout");
    }
    expect(readFileSync(RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH, "utf8")).toContain(
      'afora_e2e_run_command node "$baseline_entry" onboard',
    );
  });
  ...
  it("preserves actionable, secret-safe typed onboarding failure diagnostics", () => {
    ...
    expect(script).not.toContain('"$HOME/.afora/agents/main/agent/auth-profiles.json"');
  });
  ...
  it("keeps append-only mock E2E state under per-run scratch roots", () => {
    const scripts = [
      {
        ...
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/afora-release-typed-onboarding.XXXXXX")"',
        ...
        removed: [
          "/tmp/afora-release-typed-onboarding-openai.jsonl",
          "/tmp/afora-release-typed-onboarding-install.log",
          "/tmp/afora-release-typed-onboarding.log",
          "/tmp/afora-release-typed-onboarding-openai.log",
          "/tmp/afora-release-typed-onboarding-agent.log",
          'mktemp -d "/tmp/afora-release-typed-onboarding.XXXXXX"',
        ],
      },
      {
        path: RELEASE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/afora-release-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        ...
        removed: [
          "/tmp/afora-release-user-journey-openai.jsonl",
          "/tmp/afora-release-user-journey-clickclack.json",
          "/tmp/afora-release-user-journey-install.log",
          "/tmp/afora-release-user-journey-onboard.log",
          "/tmp/afora-release-user-journey-agent.log",
          "/tmp/afora-release-user-journey-plugin-a-install-path.txt",
          "/tmp/afora-release-user-journey-plugin-a-source-path.txt",
          'mktemp -d "/tmp/afora-release-journey-plugin-a.XXXXXX"',
          'mktemp -d "/tmp/afora-release-journey-plugin-b.XXXXXX"',
        ],
      },
      {
        path: RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/afora-release-upgrade-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        extraState: 'CLICKCLACK_STATE="$scenario_tmp/clickclack.json"',
        expectedPaths: [
        ...
        removed: [
          "/tmp/afora-release-upgrade-user-journey-openai.jsonl",
          "/tmp/afora-release-upgrade-user-journey-clickclack.json",
          "/tmp/afora-release-upgrade-baseline-install.log",
          "/tmp/afora-release-upgrade-candidate-install.log",
          "/tmp/afora-release-upgrade-onboard.log",
          "/tmp/afora-release-upgrade-agent.log",
          'mktemp -d "/tmp/afora-release-upgrade-plugin.XXXXXX"',
        ],
      },
      {
        ...
        removed: ["/tmp/afora-mock-openai-requests.jsonl"],
      },
    ];
    ...
    for (const {
      ...
      expect(script, path).not.toMatch(/\/tmp\/afora-release-[\w-]+\.(?:log|json|err|txt)/u);
    }
  });
  ...
  it("keeps multi-node update Docker artifacts isolated by default", () => {
    const multiNode = readFileSync(MULTI_NODE_UPDATE_DOCKER_E2E_PATH, "utf8");
    expect(multiNode).toContain(
      'RUN_ID="${AFORA_MULTI_NODE_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"',
    );
    expect(multiNode).toContain(
      'ARTIFACT_DIR="${AFORA_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update/$RUN_ID}"',
    );
    expect(multiNode).toContain('-v "$ARTIFACT_DIR:/tmp/artifacts"');
    expect(multiNode).not.toContain(
      'ARTIFACT_DIR="${AFORA_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update}"',
    );
  });

  it("reuses the shared bare image for multi-node update targeted runs", () => {
    const workDir = tempDirs.make("afora-multi-node-shared-image-");
    writeFileSync(join(workDir, "afora-current.tgz"), "fake package");
    writeExecutables(join(workDir, "bin"), {
      docker: `#!/usr/bin/env bash
printf "%s\\n" "$*" >>"$TMPDIR/docker-seen"
case "$1 $2" in
...
export PATH="$TMPDIR/bin:$PATH"
export AFORA_SKIP_DOCKER_BUILD=1
export AFORA_DOCKER_E2E_IMAGE=shared-bare
export AFORA_CURRENT_PACKAGE_TGZ="$TMPDIR/afora-current.tgz"
export AFORA_MULTI_NODE_ARTIFACT_DIR="$TMPDIR/artifacts"
...
if grep -Fq 'afora-multi-node-update-e2e' "$TMPDIR/docker-seen"; then
...
`;
  ...
  it("bounds upgrade survivor foreground Afora CLI calls", () => {
    const runner = readFileSync(UPGRADE_SURVIVOR_DOCKER_E2E_PATH, "utf8");
    const publishedRunner = readFileSync(UPGRADE_SURVIVOR_RUN_SCRIPT, "utf8");
    const updateRestartAuth = readFileSync(UPGRADE_SURVIVOR_UPDATE_RESTART_AUTH_PATH, "utf8");
    ...
    expectTextToIncludeAll(runner, [
      ...
      'command_timeout="${AFORA_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"',
      'afora_e2e_maybe_timeout "$command_timeout" env -u AFORA_GATEWAY_TOKEN',
      'afora_e2e_maybe_timeout "$command_timeout" afora doctor --fix --non-interactive',
      'afora_e2e_maybe_timeout "$command_timeout" afora config validate',
      'afora_e2e_maybe_timeout "$command_timeout" afora gateway status',
      ...
      "AFORA_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING",
      "AFORA_UPGRADE_SURVIVOR_READYZ_ALLOW_DEGRADED",
    ]);
    ...
    expect(publishedRunner).toContain(
      'budget="$(afora_e2e_read_positive_int_env AFORA_UPGRADE_SURVIVOR_START_BUDGET_SECONDS 90)"',
    );
    expect(publishedRunner).toContain(
      'budget="$(afora_e2e_read_positive_int_env AFORA_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS 30)"',
    );
    expect(publishedRunner).toContain(
      'afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u AFORA_GATEWAY_TOKEN',
    );
    expect(publishedRunner).toContain(
      'afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora --version',
    );
    expect(publishedRunner).toContain(
      'afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora config validate >"$BASELINE_CONFIG_VALIDATE_LOG"',
    );
    expect(publishedRunner).toContain(
      'afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" afora',
    );
    expect(publishedRunner).toContain(
      'afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" afora',
    );
    expect(publishedRunner).toContain(
      'afora_e2e_maybe_timeout "$COMMAND_TIMEOUT" afora doctor --fix --non-interactive',
    );
    ...
    expect(publishedRunner).toContain('afora gateway --port "$port" --bind loopback');
    expect(publishedRunner).toContain("start_gateway legacy-ready-log-ok");
    expect(publishedRunner).toContain(
      'afora_e2e_wait_gateway_ready "$gateway_pid" "$GATEWAY_LOG" 360 "$port" "${1:-strict}"',
    );
    ...
    expect(updateRestartAuth).toContain('afora gateway --port "$port" --bind loopback');
    expect(updateRestartAuth).toContain(
      'afora_e2e_wait_gateway_ready "$gateway_pid" "$log_file" 360 "$port"',
    );
  });
  ...
  it("models systemd restart supervision in update-restart auth fixtures", () => {
    ...
    for (const script of [publishedRunner, updateRestartAuth]) {
      expectTextToIncludeAll(script, [
        ...
        'AFORA_SYSTEMCTL_SHIM_EXEC_START="$exec_start"',
        'nohup node "$supervisor_script"',
        'if (key.startsWith("AFORA_UPDATE_")) {',
        "delete childEnv.AFORA_COMPATIBILITY_HOST_VERSION;",
    ...
    for (const script of [runner, publishedRunner]) {
      expect(script).toContain("systemctl --user stop afora-gateway.service");
    }
  });

  it("stops supervised gateway restarts after the systemd burst limit", async () => {
    const workDir = tempDirs.make("afora-update-restart-supervisor-");
    const scripts = [
    ...
    for (const [index, script] of scripts.entries()) {
      ...
      const supervisor = spawn(process.execPath, [supervisorPath], {
        env: {
          ...
          AFORA_SYSTEMCTL_SHIM_DAEMON_LOG: logPath,
          AFORA_SYSTEMCTL_SHIM_EXEC_START: command,
        },
        stdio: "ignore",
      });
  ...
  it("allows a supervised gateway to drain within the systemd stop timeout", async () => {
    const workDir = tempDirs.make("afora-update-restart-graceful-stop-");
    const scripts = [
      readFileSync(UPGRADE_SURVIVOR_RUN_SCRIPT, "utf8"),
      readFileSync(UPGRADE_SURVIVOR_UPDATE_RESTART_AUTH_PATH, "utf8"),
    ];
  ...
  it("preserves the ClawHub fixture URL across a supervised gateway restart", async () => {
    const workDir = tempDirs.make("afora-update-restart-clawhub-env-");
    const gatewayPath = join(workDir, "gateway.mjs");
    writeFileSync(
      gatewayPath,
      `import fs from "node:fs";
fs.appendFileSync(process.env.URLS_FILE, process.env.AFORA_CLAWHUB_URL + "\\n");
...
`,
    ...
    for (const [index, script] of scripts.entries()) {
      ...
      const supervisor = spawn(process.execPath, [supervisorPath], {
        env: {
          ...process.env,
          AFORA_CLAWHUB_URL: "http://127.0.0.1:43123",
          AFORA_SYSTEMCTL_SHIM_DAEMON_LOG: logPath,
          AFORA_SYSTEMCTL_SHIM_EXEC_START: `${shellQuote(process.execPath)} ${shellQuote(gatewayPath)}`,
          URLS_FILE: urlsPath,
        },
        stdio: "ignore",
      });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const workDir = tempDirs.make("afora-update-restart-process-group-");
...
`,
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const workDir = tempDirs.make("afora-update-restart-process-group-restart-");
      const descendantPath = writeTermIgnoringDescendant(workDir);
...
`,
  ...
  it.each([
    ["start budget", "AFORA_UPGRADE_SURVIVOR_START_BUDGET_SECONDS", "90s"],
    ["status budget", "AFORA_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS", "30s"],
    ["probe timeout", "AFORA_UPGRADE_SURVIVOR_PROBE_TIMEOUT_MS", "soon"],
    ["probe attempt timeout", "AFORA_UPGRADE_SURVIVOR_PROBE_ATTEMPT_TIMEOUT_MS", "0"],
    ["probe body cap", "AFORA_UPGRADE_SURVIVOR_PROBE_MAX_BODY_BYTES", "64bytes"],
  ...
  it("bounds upgrade survivor failure log diagnostics", () => {
    ...
    expectTextToIncludeInOrder(runner, [
      ...
      "afora config validate --json >/tmp/afora-upgrade-survivor-post-update-validate.json",
    ...
    expectTextToIncludeInOrder(publishedRunner, [
      ...
      'afora "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR" || update_status=$?',
      'if [ "$update_status" -ne 0 ]; then',
      'echo "afora update failed" >&2',
      'afora config validate --json >"$POST_UPDATE_VALIDATE_JSON"',
      'echo "post-update config validation probe status=$validate_status" >&2',
      'afora_e2e_print_log "$POST_UPDATE_VALIDATE_ERR" >&2 || true',
      'afora_e2e_print_log "$POST_UPDATE_VALIDATE_JSON" >&2 || true',
      'afora_e2e_print_log "$UPDATE_ERR" >&2 || true',
      'afora_e2e_print_log "$UPDATE_JSON" >&2 || true',
      'return "$update_status"',
    ]);

    expectTextToIncludeAll(runner, [
      "afora_e2e_print_log /tmp/afora-upgrade-survivor-update.err",
      "afora_e2e_print_log /tmp/afora-upgrade-survivor-update.json",
      "afora_e2e_print_log /tmp/afora-upgrade-survivor-post-update-validate.err",
      "afora_e2e_print_log /tmp/afora-upgrade-survivor-post-update-validate.json",
      "afora_e2e_print_log /tmp/afora-upgrade-survivor-doctor.log",
      "afora_e2e_print_log /tmp/afora-upgrade-survivor-status.err",
      "afora_e2e_print_log /tmp/afora-upgrade-survivor-status.json",
      'afora_e2e_print_log "$GATEWAY_LOG"',
      'afora_e2e_print_log "$SYSTEMCTL_SHIM_DAEMON_LOG"',
      'afora_e2e_print_log "$log_file"',
    ]);

    expect(runner).not.toContain("cat /tmp/afora-upgrade-survivor-update.err");
    expect(runner).not.toContain("cat /tmp/afora-upgrade-survivor-update.json");
    expect(runner).not.toContain("cat /tmp/afora-upgrade-survivor-post-update-validate.err");
    expect(runner).not.toContain("cat /tmp/afora-upgrade-survivor-post-update-validate.json");
    expect(runner).not.toContain("cat /tmp/afora-upgrade-survivor-doctor.log");
    expect(runner).not.toContain("cat /tmp/afora-upgrade-survivor-status.err");
    expect(runner).not.toContain("cat /tmp/afora-upgrade-survivor-status.json");
    ...
    expect(runner).not.toContain('afora_e2e_print_log "$SYSTEMCTL_SHIM_LOG"');

    expect(publishedRunner).toContain('afora_e2e_print_log "$BASELINE_INSTALL_LOG"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$UPDATE_ERR"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$UPDATE_JSON"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$DOCTOR_LOG"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$GATEWAY_LOG"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$STATUS_ERR"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$STATUS_JSON"');
    expect(publishedRunner).toContain('afora_e2e_print_log "$log_file"');
    ...
    expect(publishedRunner).not.toContain('afora_e2e_print_log "$SYSTEMCTL_SHIM_LOG"');
    expect(publishedRunner).not.toContain('afora_e2e_print_log "$SYSTEMCTL_SHIM_DAEMON_LOG"');
  });

  it("preserves caller-owned file descriptors around harness runs", () => {
    const workDir = tempDirs.make("afora-docker-harness-fd-");
...
`;
  ...
  it("wires the Codex npm plugin live assertion boundary into Docker", () => {
    ...
    expectTextToIncludeAll(runner, [
      "docker_e2e_print_log /tmp/afora-codex-plugin-pack.log",
      ...
      'afora_e2e_stop_process "${registry_pid:-}"',
      'if [ "$status" -ne 0 ] && [ "$debug_logs_dumped" -eq 0 ]; then',
      "assert-agent-error",
      "assert-followthrough",
      "followthrough-turn.mjs",
      "docker_e2e_read_positive_int_env AFORA_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS 420",
      'docker_e2e_read_positive_int_env AFORA_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS"',
      '-e "AFORA_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS"',
      '-e "AFORA_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"',
      '--timeout "$AGENT_TURN_TIMEOUT_SECONDS"',
    ]);
    expect(runner).not.toContain("cat /tmp/afora-codex-plugin-pack.log");
    expect(runner).not.toContain('CODEX_PLUGIN_SPEC="npm-pack:$container_path"');
    expect(runner).not.toContain("trap 'afora_e2e_stop_process \"${registry_pid:-}\"' EXIT");
  ...
  it("writes the packaged Codex follow-through result independently of stdout logs", () => {
    const workDir = tempDirs.make("afora-codex-followthrough-");
  ...
  it.each([
    [
      ...
      CODEX_NPM_PLUGIN_LIVE_DOCKER_E2E_PATH,
      "AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES",
      "64kb",
    ],
    [
      "Codex npm plugin live agent timeout",
      CODEX_NPM_PLUGIN_LIVE_DOCKER_E2E_PATH,
      "AFORA_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS",
      "420s",
    ],
    [
      "npm onboard channel-agent",
      NPM_ONBOARD_CHANNEL_AGENT_DOCKER_E2E_PATH,
      "AFORA_NPM_ONBOARD_JSON_ARTIFACT_MAX_BYTES",
      "64kb",
    ],
    [
      "plugins",
      PLUGINS_DOCKER_E2E_PATH,
      "AFORA_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS",
      "soon",
    ],
    [
      "release user journey",
      RELEASE_USER_JOURNEY_DOCKER_E2E_PATH,
      "AFORA_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES",
      "64kb",
    ],
  ])(
  ...
  it("forwards package assertion env limits into Docker runners", () => {
    const expectations = [
      [
        CODEX_NPM_PLUGIN_LIVE_DOCKER_E2E_PATH,
        [
          ["AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES", "1048576"],
          ["AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES", "65536"],
          ["AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES", "64"],
          ["AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES", "4096"],
          ["AFORA_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES", "2097152"],
          ["AFORA_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS", "420"],
        ],
      ],
      [
        NPM_ONBOARD_CHANNEL_AGENT_DOCKER_E2E_PATH,
        [
          ["AFORA_NPM_ONBOARD_JSON_ARTIFACT_MAX_BYTES", "1048576"],
          ["AFORA_NPM_ONBOARD_STATUS_TEXT_MAX_BYTES", "1048576"],
        ],
      ],
      [
        PLUGINS_DOCKER_E2E_PATH,
        [
          ["AFORA_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES", "1048576"],
          ["AFORA_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", "30000"],
        ],
      ],
      [
        RELEASE_USER_JOURNEY_DOCKER_E2E_PATH,
        [
          ["AFORA_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS", "5000"],
          ["AFORA_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES", "1048576"],
        ],
      ],
    ] as const;
  ...
  it("gives Codex on-demand package installs enough time to reach Codex assertions", () => {
    ...
    expect(runner).toContain(
      'export AFORA_E2E_NPM_INSTALL_TIMEOUT="${AFORA_E2E_NPM_INSTALL_TIMEOUT:-1200s}"',
    );
  });
  ...
  it("threads the live plugin tool output cap into the Docker harness", () => {
    ...
    expectTextToIncludeAll(runner, [
      'source "$ROOT_DIR/scripts/lib/afora-e2e-instance.sh"',
      'AGENT_TURN_TIMEOUT_SECONDS="$(afora_e2e_read_positive_int_env AFORA_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS 300)"',
      'AGENT_TURN_TIMEOUT_SECONDS="$(afora_e2e_read_positive_int_env AFORA_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS")"',
      'COMMAND_TIMEOUT="${AFORA_E2E_COMMAND_TIMEOUT:-$((10#$AGENT_TURN_TIMEOUT_SECONDS + 60))s}"',
      'AGENT_OUTPUT_MAX_BYTES="$(afora_e2e_read_positive_int_env AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES 1048576)"',
      'AGENT_OUTPUT_DUMP_BYTES="$(afora_e2e_read_nonnegative_int_env AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES 16384)"',
      'SESSION_SCAN_MAX_ENTRIES="$(afora_e2e_read_positive_int_env AFORA_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES 50000)"',
      '-e "AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES=$AGENT_OUTPUT_DUMP_BYTES"',
      '-e "AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES=$AGENT_OUTPUT_MAX_BYTES"',
      '-e "AFORA_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES=$SESSION_SCAN_MAX_ENTRIES"',
      '-e "AFORA_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"',
      "AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES",
      'tail -c "$agent_output_dump_bytes" /tmp/afora-agent.json',
    ]);
    const earlyTimeoutEnvIndex = runner.indexOf(
      "afora_e2e_read_positive_int_env AFORA_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS 300",
    );
    const profileSourceIndex = runner.indexOf('source "$PROFILE_FILE"');
    const finalTimeoutEnvIndex = runner.lastIndexOf(
      "afora_e2e_read_positive_int_env AFORA_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS",
    );
    ...
    expect(runner).not.toContain(
      'AGENT_OUTPUT_MAX_BYTES="${AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES:-1048576}"',
    );

    const dumpLogsStart = runner.indexOf("afora_e2e_dump_logs \\");
    const dumpLogsEnd = runner.indexOf("\n}", dumpLogsStart);
    expect(runner.slice(dumpLogsStart, dumpLogsEnd)).not.toContain("/tmp/afora-agent.json");
  });

  it.each([
    ["timeout", "AFORA_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS", "1e3"],
    ["output cap", "AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES", "64kb"],
    ["output dump cap", "AFORA_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES", "64kb"],
    ["session scan cap", "AFORA_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES", "0"],
  ])(
    ...
    (_label, envName, value) => {
      const result = spawnSync("bash", [LIVE_PLUGIN_TOOL_DOCKER_E2E_PATH], {
        ...
        env: {
          ...process.env,
          AFORA_LIVE_PLUGIN_TOOL_HOST_BUILD: "0",
          AFORA_SKIP_DOCKER_BUILD: "1",
          [envName]: value,
        },
      });
  ...
  it("keeps live plugin tool npm pack tarball paths inside the fixture directory", () => {
    ...
    expectTextToIncludeAll(runner, [
      ...
      "/tmp/afora-live-plugin-tool-pack.log",
      "find \"$fixture_dir\" -maxdepth 1 -type f -name '*.tgz' | sort",
      "Expected one packed fixture plugin tarball",
      "afora_e2e_dump_logs /tmp/afora-live-plugin-tool-pack.log",
  ...
  it.each([
    ["printed log bytes", "AFORA_DOCKER_E2E_LOG_PRINT_BYTES", "64kb"],
    ["heartbeat termination grace", "AFORA_DOCKER_E2E_HEARTBEAT_TERM_GRACE_SECONDS", "soon"],
  ])("rejects invalid Docker E2E %s before setup", (_label, envName, value) => {
    const workDir = tempDirs.make("afora-docker-e2e-log-invalid-");
    const script = repoShell(workDir)`
...
`;
  ...
  it("rejects invalid Docker E2E log heartbeat env before harness setup", () => {
    const workDir = tempDirs.make("afora-docker-e2e-log-heartbeat-invalid-");
    const script = repoShell(workDir)`
export AFORA_DOCKER_E2E_LOG_HEARTBEAT_SECONDS=1e3
...
`;
    ...
    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid AFORA_DOCKER_E2E_LOG_HEARTBEAT_SECONDS: 1e3");
    expect(result.stdout).toBe("");
  });

  it("preserves heredoc stdin through Docker E2E heartbeat logging", () => {
    const workDir = tempDirs.make("afora-docker-e2e-log-stdin-");
    const script = repoShell(workDir)`
...
`;
  ...
  it("preserves failing heredoc output and status through Docker E2E heartbeat logging", () => {
    const workDir = tempDirs.make("afora-docker-e2e-log-failing-stdin-");
    const script = repoShell(workDir)`
...
`;
  ...
  it("does not delay fast successful Docker E2E log captures until the next heartbeat", () => {
    const workDir = tempDirs.make("afora-docker-e2e-log-fast-heartbeat-");
    const script = repoShell(workDir)`
...
`;
  ...
  it("keeps onboarding Docker E2E resource-guarded", () => {
    ...
    expectTextToIncludeAll(runner, [
      "AFORA_ONBOARD_MAX_MEMORY_MIB",
      "AFORA_ONBOARD_MAX_CPU_PERCENT",
      'COMMAND_TIMEOUT="${AFORA_ONBOARD_COMMAND_TIMEOUT:-${AFORA_E2E_COMMAND_TIMEOUT:-300s}}"',
      'GATEWAY_WAIT_ATTEMPTS="$(afora_e2e_read_positive_int_env AFORA_ONBOARD_GATEWAY_WAIT_ATTEMPTS 20)"',
      'GATEWAY_WAIT_INTERVAL_S="$(docker_e2e_read_nonnegative_decimal_env AFORA_ONBOARD_GATEWAY_WAIT_INTERVAL_S 1)"',
      '-e "AFORA_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"',
      '-e "AFORA_ONBOARD_GATEWAY_WAIT_ATTEMPTS=$GATEWAY_WAIT_ATTEMPTS"',
      '-e "AFORA_ONBOARD_GATEWAY_WAIT_INTERVAL_S=$GATEWAY_WAIT_INTERVAL_S"',
      '--name "$CONTAINER_NAME"',
  ...
  it("cleans resource-sampled Docker E2E temp logs on every exit path", () => {
    for (const { path, label } of [
      ...
      expect(runner, path).toContain('DOCKER_RUN_TIMEOUT="${AFORA_');
  ...
  it("keeps captured Docker E2E run log replay bounded", () => {
    ...
    expect(pluginBinding).not.toContain("process.env.AFORA_DOCKER_E2E_LOG_PRINT_BYTES");
  ...
  it("keeps Open WebUI Docker E2E resource-guarded", () => {
    ...
    expectTextToIncludeAll(runner, [
      'validate_positive_int AFORA_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS "$PROVIDER_TIMEOUT_SECONDS"',
      'validate_positive_int AFORA_OPENWEBUI_FETCH_TIMEOUT_MS "$PROBE_FETCH_TIMEOUT_MS"',
      "docker_e2e_read_tcp_port_env AFORA_OPENWEBUI_GATEWAY_PORT 18789",
      "docker_e2e_read_tcp_port_env AFORA_OPENWEBUI_PORT 8080",
      "AFORA_OPENWEBUI_MAX_MEMORY_MIB",
      "AFORA_OPENWEBUI_MAX_CPU_PERCENT",
      'STATS_LOG="$(mktemp',
      'PROBE_LOG="$(mktemp',
      ...
      `afora_e2e_exec_gateway "$entry" '"$PORT"' lan`,
  ...
  it.each([
    ["gateway", "AFORA_OPENWEBUI_GATEWAY_PORT", "1e3"],
    ["webui", "AFORA_OPENWEBUI_PORT", "65536"],
  ...
  it.each([
    ["provider", "AFORA_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS", "300s"],
    ["fetch", "AFORA_OPENWEBUI_FETCH_TIMEOUT_MS", "8000ms"],
  ])(
  ...
  it("accepts decimal Open WebUI Docker numeric inputs with leading zeroes", () => {
    const result = spawnSync("bash", [OPENWEBUI_DOCKER_E2E_PATH], {
      ...
      env: {
        ...process.env,
        OPENAI_API_KEY: "",
        AFORA_OPENWEBUI_FETCH_TIMEOUT_MS: "09000",
        AFORA_OPENWEBUI_GATEWAY_PORT: "018789",
        AFORA_OPENWEBUI_PORT: "08080",
        AFORA_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: "08",
      },
    });
  ...
  it.each([
    [MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH, "AFORA_MCP_CODE_MODE_GATEWAY_PORT", "1e3"],
    [MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH, "AFORA_MCP_CODE_MODE_MOCK_PORT", "65536"],
    [MCP_CODE_MODE_GATEWAY_LIVE_DOCKER_E2E_PATH, "AFORA_MCP_CODE_MODE_LIVE_GATEWAY_PORT", "0"],
    [CODEX_MEDIA_PATH_DOCKER_E2E_PATH, "AFORA_CODEX_MEDIA_PATH_PORT", "18790tcp"],
    [OPENAI_CHAT_TOOLS_DOCKER_E2E_PATH, "AFORA_OPENAI_CHAT_TOOLS_PORT", "0"],
    [OPENAI_WEB_SEARCH_MINIMAL_E2E_PATH, "AFORA_OPENAI_WEB_SEARCH_MINIMAL_PORT", "18789tcp"],
  ...
  it.each([
    ["timeout", "AFORA_CODEX_MEDIA_PATH_TIMEOUT_SECONDS", "180s"],
    ["log tail cap", "AFORA_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES", "64kb"],
  ...
  it("forwards Codex media path client limits into Docker", () => {
    ...
    expect(runner).toContain(
      'LOG_TAIL_MAX_BYTES="$(docker_e2e_read_positive_int_env AFORA_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES 2097152)"',
    );
    expect(runner).toContain(
      '-e "AFORA_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES=$LOG_TAIL_MAX_BYTES"',
    );
  });

  it.each([
    [MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH, "AFORA_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", "1e3"],
    [
      MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH,
      "AFORA_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES",
      "64bytes",
    ],
    [MCP_CODE_MODE_GATEWAY_LIVE_DOCKER_E2E_PATH, "AFORA_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", "1e3"],
    [
  ...
  it.each([MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH, MCP_CODE_MODE_GATEWAY_LIVE_DOCKER_E2E_PATH])(
    ...
    (scriptPath) => {
      ...
      expectTextToIncludeAll(runner, [
        'CLIENT_TIMEOUT_MS="$(docker_e2e_read_positive_int_env AFORA_MCP_CODE_MODE_CLIENT_TIMEOUT_MS 300000)"',
        'CLIENT_BODY_MAX_BYTES="$(docker_e2e_read_positive_int_env AFORA_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES 1048576)"',
        '-e "AFORA_MCP_CODE_MODE_CLIENT_TIMEOUT_MS=$CLIENT_TIMEOUT_MS"',
        '-e "AFORA_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES=$CLIENT_BODY_MAX_BYTES"',
      ]);
    },
  );

  it.each([
    ["timeout", "AFORA_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS", "180s"],
    ["body cap", "AFORA_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES", "64kb"],
  ...
  it("forwards every OpenAI chat tools runtime env knob into Docker", () => {
    ...
    const consumed = new Set(
      [...`${client}\n${writer}`.matchAll(/["`](AFORA_OPENAI_CHAT_TOOLS_[A-Z0-9_]+)["`]/gu)]
        .map((match) => match[1])
        .filter((envName): envName is string => envName !== undefined),
    );
    const forwarded = new Set(
      [...runner.matchAll(/-e\s+"(AFORA_OPENAI_CHAT_TOOLS_[A-Z0-9_]+)=/gu)]
        .map((match) => match[1])
        .filter((envName): envName is string => envName !== undefined),
    );
  ...
  it("forwards every kitchen-sink RPC runtime env knob into Docker", () => {
    ...
    const consumed = new Set(
      [...walk.matchAll(/\b(?:env|process\.env)\.(AFORA_KITCHEN_SINK_[A-Z0-9_]+)/gu)]
        .map((match) => match[1])
        .filter((envName): envName is string => envName !== undefined),
    );
    const forwarded = new Set(
      [...runner.matchAll(/\b(AFORA_KITCHEN_SINK_[A-Z0-9_]+)\b/gu)]
        .map((match) => match[1])
        .filter((envName): envName is string => envName !== undefined),
    );
    const missing = [...consumed]
  ...
  it("keeps the kitchen-sink RPC Docker watchdog above the internal walk budgets", () => {
    const runner = readFileSync(KITCHEN_SINK_RPC_DOCKER_E2E_PATH, "utf8");
    expect(runner).toContain(
      'DOCKER_RUN_TIMEOUT="${AFORA_KITCHEN_SINK_RPC_DOCKER_RUN_TIMEOUT:-1500s}"',
    );
  });
  ...
  it("bounds kitchen-sink plugin CLI commands inside the Docker sweep", () => {
    ...
    expectTextToIncludeAll(runner, [
      'KITCHEN_SINK_CLI_TIMEOUT="${AFORA_KITCHEN_SINK_PLUGIN_CLI_TIMEOUT:-${KITCHEN_SINK_CLI_TIMEOUT:-180s}}"',
      "docker_e2e_read_positive_int_env AFORA_DOCKER_E2E_LOG_PRINT_BYTES 65536",
      "docker_e2e_read_positive_int_env AFORA_CLAWHUB_FIXTURE_WAIT_ATTEMPTS 600",
      '-e "AFORA_CLAWHUB_FIXTURE_WAIT_ATTEMPTS=$CLAW_HUB_FIXTURE_WAIT_ATTEMPTS"',
    ...
    expectTextToIncludeAll(sweep, [
      ...
      "run_kitchen_sink_afora_logged()",
      "run_kitchen_sink_afora_capture()",
      'afora_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$AFORA_ENTRY" "$@" >"$log_file" 2>&1',
      'local log_file="${KITCHEN_SINK_TMP_DIR}/${safe_label}.log"',
    ]);

    for (const line of sweep.split("\n")) {
      if (!line.includes('node "$AFORA_ENTRY" plugins')) {
        continue;
      }

      expect(line).toContain("afora_e2e_maybe_timeout");
    }
  });
  ...
  it("executes each CLI distribution boundary instead of promoting metadata", () => {
    ...
    expectTextToIncludeAll(packageRunner, [
      "npm install -g --prefix /tmp/afora-proof",
      "pnpm add --global --allow-build=afora",
      "bun@1.3.14",
      'test "$(command -v afora)" = "/tmp/afora-proof/bin/afora"',
      'test "$(command -v afora)" = "$PNPM_HOME/afora"',
      "AFORA_BUN_GLOBAL_SMOKE_PROOF_PATH",
    ...
    expectTextToIncludeAll(updateRunner, [
      ...
      'AFORA_NPM_REGISTRY_DIST_TAGS="latest=0.0.0,beta=$package_version"',
      "AFORA_NPM_REGISTRY_UPSTREAM=https://registry.npmjs.org",
      "assert-update beta",
    ...
    expect(updateRunner).toContain("afora update --channel beta --yes --json --no-restart");
    expect(updateRunner).not.toContain("afora update --channel beta --tag");
  });
  ...
  it("proves gateway suspension across a same-container process restart", () => {
    ...
    expectTextToIncludeAll(runner, [
      ...
      "afora_e2e_probe_http http://127.0.0.1:$PORT/readyz ok 400",
  ...
  it.each([
    ["connect", "AFORA_GATEWAY_NETWORK_CLIENT_CONNECT_TIMEOUT_MS", "100ms"],
    ["ready", "AFORA_GATEWAY_NETWORK_CONNECT_READY_TIMEOUT_MS", "1e3"],
  ])(
  ...
  it("forwards gateway network client timeout env into the Docker client", () => {
    const runner = readFileSync(GATEWAY_NETWORK_DOCKER_E2E_PATH, "utf8");
    expectTextToIncludeAll(runner, [
      "docker_e2e_read_positive_int_env AFORA_GATEWAY_NETWORK_CLIENT_CONNECT_TIMEOUT_MS 80000",
      "docker_e2e_read_positive_int_env AFORA_GATEWAY_NETWORK_CONNECT_READY_TIMEOUT_MS 80000",
      '-e "AFORA_GATEWAY_NETWORK_CLIENT_CONNECT_TIMEOUT_MS=$CLIENT_CONNECT_TIMEOUT_MS"',
      '-e "AFORA_GATEWAY_NETWORK_CONNECT_READY_TIMEOUT_MS=$CONNECT_READY_TIMEOUT_MS"',
  ...
  it("requires TCP readiness for the gateway network runner", () => {
    const runner = readFileSync(GATEWAY_NETWORK_DOCKER_E2E_PATH, "utf8");
    expect(runner).toContain("afora_e2e_probe_tcp 127.0.0.1 $PORT");
    expect(runner).not.toMatch(/afora_e2e_probe_tcp[^\n]*\|\|[^\n]*gateway-net-e2e\.log/u);
  });
  ...
  it("runs release installer E2E against the npm beta tag", () => {
    ...
    expect(scenarios).toContain(
      '"AFORA_INSTALL_TAG=beta AFORA_E2E_MODELS=openai AFORA_INSTALL_E2E_IMAGE=afora-install-e2e-openai:local AFORA_INSTALL_E2E_AGENT_TOOL_SMOKE=0 AFORA_INSTALL_E2E_OPENAI_MODEL=openai/gpt-5.4-mini AFORA_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS=120 AFORA_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS=120"',
    );
    expect(scenarios).toContain(
      '"AFORA_INSTALL_TAG=beta AFORA_E2E_MODELS=anthropic AFORA_INSTALL_E2E_IMAGE=afora-install-e2e-anthropic:local"',
    );
    expect(scenarios).toContain('"test-install-sh-e2e-docker.sh"');
    expect(scenarios).not.toContain("pnpm test:install:e2e");
    expect(scenarios).toContain(
      '"AFORA_OPENWEBUI_MODEL=openai/gpt-5.4-mini AFORA_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=300 AFORA_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui"',
    );
    expect(scenarios).not.toContain("OPENWEBUI_SMOKE_MODE=models");
    expect(openWebUiRunner).toContain(
      'SMOKE_MODE="${OPENWEBUI_SMOKE_MODE:-${AFORA_OPENWEBUI_SMOKE_MODE:-chat}}"',
    );
  ...
  it("times and parallelizes release installer E2E agent turns after gateway startup", () => {
    ...
    expectTextToIncludeAll(runner, [
      'AGENT_TURNS_PARALLEL="$(read_boolean_env AFORA_INSTALL_E2E_AGENT_TURNS_PARALLEL 1)"',
      'AGENT_TOOL_SMOKE="$(read_boolean_env AFORA_INSTALL_E2E_AGENT_TOOL_SMOKE 1)"',
      "time_phase",
      "phase_mark_start",
      "run_agent_turn_bg",
      ...
      "AFORA_INSTALL_E2E_OPENAI_MODEL",
      "AFORA_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS",
      'AGENT_TURN_TIMEOUT_SECONDS="$(read_positive_int_env AFORA_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS 300)"',
    ]);

    expect(runner).not.toContain('run_agent_turn_bg "read proof"');

    expectTextToIncludeAll(wrapper, [
      "AFORA_INSTALL_E2E_AGENT_TURNS_PARALLEL",
      "AFORA_INSTALL_E2E_AGENT_TOOL_SMOKE",
      "AFORA_INSTALL_E2E_OPENAI_MODEL",
      "AFORA_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS",
      "docker_e2e_read_positive_int_env AFORA_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS 300",
      'docker_e2e_read_positive_int_env AFORA_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS"',
      '-e AFORA_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS="$AGENT_TURN_TIMEOUT_SECONDS"',
      "AFORA_INSTALL_E2E_PROFILE_FILE",
      "AFORA_PROFILE_FILE",
      "AFORA_TESTBOX_PROFILE_FILE",
  ...
  it("keeps package acceptance plugin coverage offline-capable", () => {
    ...
    expect(scenarios).toContain("AFORA_PLUGINS_E2E_CLAWHUB=0");
  });
  ...
  it("keeps a stalled multi-node health request inside the probe deadline", () => {
    ...
    const workDir = tempDirs.make("afora-multi-node-health-timeout-");
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    expectTextToIncludeAll(doctorScenario, [
      ...
      "AFORA_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR=1",
    ...
    expect(scripts.join("\n")).toContain("AFORA_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("keeps the doctor switch systemctl shim system scope empty", () => {
    const home = tempDirs.make("afora-doctor-systemctl-shim-");
    const env = { ...process.env, HOME: home };
    const loadState = spawnSync(
      DOCTOR_SWITCH_SYSTEMCTL_SHIM_PATH,
      ["show", "--property=LoadState", "--value", "afora-gateway.service"],
      { encoding: "utf8", env },
    );
  ...
  it("routes doctor install switch commands through the E2E timeout helper", () => {
    ...
    expectTextToIncludeAll(runner, [
      'NPM_INSTALL_TIMEOUT="${AFORA_E2E_NPM_INSTALL_TIMEOUT:-600s}"',
      'COMMAND_TIMEOUT="${AFORA_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"',
      '-e "AFORA_E2E_NPM_INSTALL_TIMEOUT=$NPM_INSTALL_TIMEOUT"',
      '-e "AFORA_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"',
    ]);
    ...
    expectTextToIncludeAll(scenario, [
      ...
      "unset AFORA_HOME AFORA_STATE_DIR AFORA_CONFIG_PATH",
      'afora_test_state_create "switch-${name}" empty\n  use_default_service_identity',
      'afora_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd"',
      'afora_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd"',
      'afora_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force',
      'afora_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes',
    ]);

    expect(
      scenario.match(/unset AFORA_HOME AFORA_STATE_DIR AFORA_CONFIG_PATH/gu),
    ).toHaveLength(1);
  ...
  it("uses the account home for upgrade survivor auto-auth state", () => {
    ...
    expectTextToIncludeAll(runner, [
      ...
      'export USERPROFILE="$account_home"',
      "unset AFORA_HOME",
      'export AFORA_STATE_DIR="$account_home/.afora"',
      'export AFORA_CONFIG_PATH="$AFORA_STATE_DIR/afora.json"',
    ]);

    expect(runner.indexOf("unset AFORA_HOME")).toBeLessThan(
      runner.indexOf('export AFORA_STATE_DIR="$account_home/.afora"'),
    );
    expect(
      runner.indexOf('export AFORA_CONFIG_PATH="$AFORA_STATE_DIR/afora.json"'),
  ...
  it("bounds doctor install switch command log diagnostics", () => {
    const scenario = readFileSync(DOCTOR_SWITCH_SCENARIO_PATH, "utf8");
    expectTextToIncludeAll(scenario, [
      'afora_e2e_print_log "$npm_log"',
      'afora_e2e_print_log "$install_log"',
      'afora_e2e_print_log "$doctor_log"',
      'afora_e2e_print_log "$reinstall_log"',
      'afora_e2e_print_log "$env_repair_log"',
      'afora_e2e_print_log "$clear_log"',
    ]);
  ...
  it("prepares pnpm workspace package fixtures without package dependencies", () => {
    const root = tempDirs.make("afora-update-channel-fixture-");
    mkdirSync(join(root, "patches"));
    writeFileSync(
      join(root, "package.json"),
      `${JSON.stringify({ name: "afora", version: "2026.5.6", scripts: {} }, null, 2)}\n`,
      "utf8",
    );
    writeFileSync(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    const forwardedRuntimeEnv = [
      "AFORA_BUNDLED_PLUGIN_LIST_TIMEOUT_MS",
      "AFORA_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_GATEWAY_LOG_BYTES",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_READY_MS",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_RPC_MS",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_HTTP_MS",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_KILL_GRACE_MS",
      "AFORA_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS",
    ] as const;

    expectTextToIncludeAll(runner, [
      "AFORA_BUNDLED_PLUGIN_SWEEP_TOTAL",
      "AFORA_BUNDLED_PLUGIN_SWEEP_INDEX",
      "AFORA_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT",
      "AFORA_PLUGIN_LIFECYCLE_TRACE",
      "docker_e2e_read_tcp_port_env AFORA_BUNDLED_PLUGIN_RUNTIME_PORT_BASE 19000",
      '-e "AFORA_BUNDLED_PLUGIN_RUNTIME_PORT_BASE=$RUNTIME_PORT_BASE"',
      "scripts/e2e/lib/bundled-plugin-install-uninstall/sweep.sh",
      'tee "$RUN_LOG"',
    ]);
    ...
    for (const [envName, fallback] of [
      ["AFORA_BUNDLED_PLUGIN_LIST_TIMEOUT_MS", "30000"],
      ["AFORA_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES", "4194304"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS", "1048576"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES", "262144"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_GATEWAY_LOG_BYTES", "16777216"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_READY_MS", "900000"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_RPC_MS", "60000"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS", "210000"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS", "1000"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS", "120000"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_HTTP_MS", "5000"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS", "10000"],
      ["AFORA_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_KILL_GRACE_MS", "1000"],
    ] as const) {
    ...
    expect(probe).toContain('"afora.plugin.json"');
    expect(runtimeSmoke).toContain(
      'readPositiveIntEnv("AFORA_BUNDLED_PLUGIN_RUNTIME_READY_MS", 900000)',
    );
    expectTextToIncludeAll(sweep, [
      "read -r plugin_id plugin_dir requires_config",
      'node "$AFORA_ENTRY" plugins install "$plugin_id"',
      'node "$AFORA_ENTRY" plugins uninstall "$plugin_id" --force',
      "now_ms()",
  ...
  it.each([
    ["list timeout", "AFORA_BUNDLED_PLUGIN_LIST_TIMEOUT_MS", "100ms"],
    ["runtime port base", "AFORA_BUNDLED_PLUGIN_RUNTIME_PORT_BASE", "99999"],
    ["runtime log scan", "AFORA_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES", "64bytes"],
    ["runtime command timeout", "AFORA_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS", "soon"],
    ["runtime teardown grace", "AFORA_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS", "0"],
  ])(
  ...
  it("passes installer tag env to bash, not curl", () => {
    const runner = readFileSync(INSTALL_E2E_RUNNER_PATH, "utf8");
    expect(runner).toContain('AFORA_BETA=1 bash "$installer"');
    expect(runner).toContain('AFORA_VERSION="$INSTALL_TAG" bash "$installer"');
    expect(runner).not.toContain('AFORA_BETA=1 curl -fsSL "$INSTALL_URL" | bash');
    expect(runner).not.toContain(
      'AFORA_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash',
    );
  });
  ...
  it("bounds installer E2E session transcript tool scans", () => {
    ...
    expect(end).toBeGreaterThan(start);
    expectTextToIncludeAll(helper, [
      "AFORA_INSTALL_E2E_SESSION_SCAN_BYTES",
      "AFORA_INSTALL_E2E_SESSION_LINE_BYTES",
      "AFORA_INSTALL_E2E_SESSION_SCAN_DEPTH",
      "AFORA_INSTALL_E2E_SESSION_SCAN_NODES",
      "fs.createReadStream",
      "Buffer.concat",
  ...
  it("exports SQLite-backed installer E2E sessions before scanning tools", () => {
    ...
    expectTextToIncludeAll(helper, [
      ...
      'if [[ ! -f "$jsonl" ]]',
      'afora --profile "$profile" sessions export-trajectory',
      '--session-key "agent:main:explicit:${session_id}"',
      '--workspace "$export_workspace"',
      'jsonl="$export_workspace/.afora/trajectory-exports/scan/events.jsonl"',
  ...
  it("keeps OpenAI web search smoke on one gateway agent connection", () => {
    ...
    expectTextToIncludeAll(runner, [
      'PORT="$(docker_e2e_read_tcp_port_env AFORA_OPENAI_WEB_SEARCH_MINIMAL_PORT 18789)"',
      'MOCK_PORT="443"',
      '-e "PORT=$PORT"',
    ...
    expect(runner).not.toContain("AFORA_OPENAI_WEB_SEARCH_MINIMAL_MOCK_PORT");

    expectTextToIncludeAll(scenario, [
      'export NODE_EXTRA_CA_CERTS="$TLS_CA_CERT"',
      'MOCK_TLS_CERT="$TLS_SERVER_CERT"',
      'MOCK_TLS_KEY="$TLS_SERVER_KEY"',
      'afora_e2e_wait_mock_openai "$MOCK_PORT" 80 400 "https://api.openai.com:$MOCK_PORT"',
      "scripts/e2e/lib/openai-web-search-minimal/client.mjs",
    ]);
  ...
  it("cleans OpenAI web search smoke processes through the E2E helpers", () => {
    ...
    expectTextToIncludeAll(scenario, [
      ...
      'gateway_pid="$(afora_e2e_start_gateway "$entry" "$PORT" "$GATEWAY_LOG")"',
      'afora_e2e_wait_mock_openai "$MOCK_PORT" 80 400 "https://api.openai.com:$MOCK_PORT"',
  ...
  it("runs agents delete shared workspace smoke through one managed gateway", () => {
    ...
    expect(runner.match(/afora_e2e_start_gateway/gu)).toHaveLength(1);
    expect(runner.match(/afora_e2e_wait_gateway_ready/gu)).toHaveLength(1);
    expect(runner).not.toContain("run_afora()");
  ...
  it("keeps OpenAI web search smoke logs isolated per run", () => {
    const scenario = readFileSync(OPENAI_WEB_SEARCH_MINIMAL_SCENARIO_PATH, "utf8");
    expectTextToIncludeAll(scenario, [
      'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/afora-openai-web-search-minimal.XXXXXX")"',
      ...
      'afora_e2e_print_log "$file"',
      'rm -rf "$scenario_tmp"',
    ]);

    expect(scenario).not.toContain("sed -n '1,260p'");
    expect(scenario).not.toContain("/tmp/afora-openai-web-search-minimal-requests.jsonl");
    expect(scenario).not.toContain("/tmp/afora-openai-web-search-minimal-client-success.log");
    expect(scenario).not.toContain("/tmp/afora-openai-web-search-minimal-client-reject.log");
  });
  ...
  it("keeps the plugin binding command escape Docker smoke focused", () => {
    ...
    expectTextToIncludeAll(runner, [
      "--reporter=verbose -t",
      'DOCKER_RUN_TIMEOUT="${AFORA_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"',
    ...
    expect(dockerfile).toContain("AFORA_DISABLE_BUNDLED_PLUGIN_POSTINSTALL=1");
    expect(dockerfile).toContain(
      "pnpm install --frozen-lockfile --ignore-scripts --filter afora",
    );
  });
  ...
  it("covers plugin CLI sources in the Docker plugin sweep", () => {
    ...
    expectTextToIncludeAll(sweep, [
      'AFORA_PLUGINS_CLI_TIMEOUT="${AFORA_PLUGINS_CLI_TIMEOUT:-180s}"',
      "run_plugins_afora_capture()",
      'afora_e2e_maybe_timeout "$AFORA_PLUGINS_CLI_TIMEOUT" node "$AFORA_ENTRY" "$@" >"$output_file"',
      "plugins_lifecycle_trace_enabled()",
      "print_plugins_stderr_log()",
      'afora_e2e_maybe_timeout "$AFORA_PLUGINS_CLI_TIMEOUT" node "$AFORA_ENTRY" "$@" >"$output_file" 2>"$error_file"',
      ...
      'plugins install "npm:@afora/demo-plugin-npm@0.0.1" --force',
      "plugins update demo-plugin-npm",
      'plugins install "git:$git_update_repo_url@main" --force',
      "plugins update demo-plugin-git-update",
    ]);
    expect(runner).toContain('PLUGINS_CLI_TIMEOUT="${AFORA_PLUGINS_CLI_TIMEOUT:-180s}"');
    expect(runner).toContain('-e "AFORA_PLUGINS_CLI_TIMEOUT=$PLUGINS_CLI_TIMEOUT"');
    expect(runner).toContain("AFORA_PLUGIN_LIFECYCLE_TRACE");
    expect(sweep).not.toContain('run_logged install-npm node "$AFORA_ENTRY"');
    for (const [path, script] of [
      ...
      const unboundedPluginCliLines = script
        .split("\n")
        .filter((line) => line.includes('node "$AFORA_ENTRY" plugins'))
        .filter((line) => !line.includes("afora_e2e_maybe_timeout"));
    ...
    expectTextToIncludeAll(clawhub, [
      ...
      "run_plugins_afora_logged install-clawhub",
      'afora_e2e_maybe_timeout "$AFORA_PLUGINS_CLI_TIMEOUT"',
      "clawhub:@afora/kitchen-sink",
    ]);
  });
});
