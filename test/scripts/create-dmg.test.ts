...
function makeApp(plistEntries: string[]): string {
  const dir = mkdtempSync(path.join(tmpdir(), "afora-create-dmg-"));
  tempDirs.push(dir);
  const app = path.join(dir, "Afora.app");
...
function makeFakeDmgTools() {
  const dir = mkdtempSync(path.join(tmpdir(), "afora-create-dmg-tools-"));
  tempDirs.push(dir);
...
describe("create-dmg plist validation", () => {
  ...
  it("keeps temporary DMG artifacts scoped to one run", () => {
    ...
    expect(script).toContain('DMG_TEMP="$(mktemp -d "${TMPDIR:-/tmp}/afora-dmg.XXXXXX")"');
    ...
    expect(script).toContain(
      'DMG_OUTPUT_TEMP="$(mktemp -d "$(dirname "$OUT_PATH")/.afora-dmg.XXXXXX")"',
    );
    ...
    expect(script).not.toContain("/tmp/afora-dmg-limits.txt");
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      const app = makeApp(["<key>CFBundleName</key>", "<string>Afora</string>"]);
...
describe.runIf(process.platform === "darwin")("create-dmg ownership boundaries", () => {
  it("uses private intermediate paths without deleting caller-owned siblings", () => {
    ...
    const outputDir = mkdtempSync(path.join(tmpdir(), "afora-create-dmg-output-"));
    tempDirs.push(outputDir);
    const output = path.join(outputDir, "Afora.dmg");
    const sibling = path.join(outputDir, "Afora-rw.dmg");
    ...
    expect(log).toContain(`${outputDir}${path.sep}.afora-dmg.`);
  ...
  it("creates a caller-provided output directory before finalizing the DMG", () => {
    const app = makeValidApp();
    const root = mkdtempSync(path.join(tmpdir(), "afora-create-dmg-output-"));
    tempDirs.push(root);
  ...
  it("styles the private mount without closing unrelated Finder windows", () => {
    ...
    expect(applescript).not.toContain('tell disk "Afora"');
