...
async function writeIpaFixture(root: string): Promise<string> {
  ...
  const ipaPath = path.join(root, "Afora.ipa");
...
async function writeValidFixture(
  ...
  const appDir = path.join(payloadDir, "Afora.app");
  ...
  const infoBody = [
    plistString("CFBundleIdentifier", "ai.aforafoundation.app"),
    plistString("CFBundleDisplayName", options.displayName ?? "Afora"),
    plistString("AforaGitCommit", options.buildCommit ?? BUILD_COMMIT),
    plistString("AforaBuildTimestamp", options.buildTimestamp ?? BUILD_TIMESTAMP),
    plistString("AforaPushMode", options.pushMode ?? "appStore"),
    plistString("AforaPushRelayBaseURL", ""),
    plistString(
      "NSHealthShareUsageDescription",
      "Afora reads Health data for Health Summaries.",
    ),
    options.healthUpdateUsage === null
      ...
      : typeof options.healthUpdateUsage === "boolean"
        ...
        : plistString(
            ...
            options.healthUpdateUsage ?? "Afora reads Health data for Health Summaries.",
          ),
    options.legacyKey ? plistString("AforaPushRelayProfile", "production") : "",
  ].join("");
  ...
  writeFileSync(
    ...
    plist(
      options.localizedDisplayName === undefined
        ? plistString("NSCameraUsageDescription", "Afora verwendet die Kamera.")
  ...
  writeFileSync(
    ...
    plist(
      [
        plistString("application-identifier", "FWJYW4S8P8.ai.aforafoundation.app"),
        ...
        plistArray("com.apple.security.application-groups", [
          "group.ai.aforafoundation.app.shared",
        ]),
      ].join(""),
    ),
    "utf8",
  ...
  writeFileSync(
    ...
    plist(
      [
        plistString("Name", "Afora App Store ai.aforafoundation.app"),
...
describe("scripts/ios-validate-app-store-ipa.sh", () => {
  ...
  it("fake plutil escapes regex-metacharacter keys before matching", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "afora-ios-ipa-"));
    tempDirs.push(root);
  ...
  it("rejects an IPA with the wrong canonical display name", async () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "afora-ios-ipa-"));
    tempDirs.push(root);
    const fixture = await writeValidFixture(root, { displayName: "Afora Debug" });
  ...
  it("rejects unresolved build settings in localized plist resources", async () => {
    ...
    tempDirs.push(root);
    const fixture = await writeValidFixture(root, {
      localizedDisplayName: "$(AFORA_APP_DISPLAY_NAME)",
    });
  ...
  it("rejects malformed or mismatched embedded build provenance", async () => {
    const malformedRoot = mkdtempSync(path.join(os.tmpdir(), "afora-ios-ipa-"));
    const mismatchRoot = mkdtempSync(path.join(os.tmpdir(), "afora-ios-ipa-"));
