...
const CHECK_SCRIPT = resolve("scripts/check-afora-package-tarball.mjs");
...
function withPackedPackage(
  ...
  const root = mkdtempSync(join(tmpdir(), "afora-control-ui-package-inventory-"));
...
function installPackedPackage(root: string, tarball: string) {
  ...
  const result = spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        AFORA_CONFIG_PATH: join(state, "afora.json"),
        TMPDIR: temporary,
        TMP: temporary,
        TEMP: temporary,
      },
  ...
  return join(installRoot, "node_modules", "afora");
}
