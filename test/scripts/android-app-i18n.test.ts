...
describe("Android app i18n resources", () => {
  ...
  it("preserves the existing Swedish app name", async () => {
    ...
    expect(strings).toContain('<string name="app_name">Afora-nod</string>');
  });
  ...
  it("finds direct, typed, conditional, interpolated, Elvis, and accessibility literals", () => {
    ...
    expect(
      findUnlocalizedAndroidUiLiterals(
        source,
        "apps/android/app/src/main/java/ai/afora/app/ui/Example.kt",
      ),
    ).toEqual([
  ...
  it("inventories command, attention, and overview model display literals", () => {
    const source = `
      ...
      CommandItem(
        ...
        subtitle = nativeString("Talk with Afora"),
      )
      HomeAttentionRow(
        title = "Gateway",
  ...
  it("ignores preview fixtures", () => {
    expect(
      findUnlocalizedAndroidUiLiterals(
        'Text("Preview copy")',
        "apps/android/app/src/main/java/ai/afora/app/ui/design/ClawComponents.kt",
      ),
    ).toEqual([]);
  });
  ...
  it("scans Wear presentation sources but ignores Wear screenshot fixtures", () => {
    ...
    expect(
      findUnlocalizedAndroidUiLiterals(
        source,
        "apps/android/wear/src/main/java/ai/afora/wear/WearViewModel.kt",
      ).map((finding) => finding.source),
    ).toContain("Current session");
    expect(
      findUnlocalizedAndroidUiLiterals(
        source,
        "apps/android/wear/src/main/java/ai/afora/wear/WearScreenshotMode.kt",
      ),
    ).toEqual([]);
  });
  ...
  it("scans flavor-specific activity surfaces", () => {
    expect(
      findUnlocalizedAndroidUiLiterals(
        ...
        "apps/android/app/src/thirdParty/java/ai/afora/app/accessibility/AccessibilityDevActivity.kt",
      ).map((finding) => finding.source),
