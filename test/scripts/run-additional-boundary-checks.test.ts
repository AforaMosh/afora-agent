...
describe("run-additional-boundary-checks", () => {
  ...
  it("rejects malformed timeout and output limit integers", () => {
    expect(resolvePositiveInteger("25", 50, "AFORA_ADDITIONAL_BOUNDARY_TIMEOUT_MS")).toBe(25);
    expect(resolvePositiveInteger(undefined, 50, "AFORA_ADDITIONAL_BOUNDARY_TIMEOUT_MS")).toBe(
      50,
    );
    expect(() =>
      resolvePositiveInteger("1000ms", 50, "AFORA_ADDITIONAL_BOUNDARY_TIMEOUT_MS"),
    ).toThrow("AFORA_ADDITIONAL_BOUNDARY_TIMEOUT_MS must be a positive integer; got: 1000ms");
    expect(() =>
      resolvePositiveInteger("1e3", 50, "AFORA_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES"),
    ).toThrow("AFORA_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES must be a positive integer; got: 1e3");
  });
  ...
  it("parses CLI help and shard args before running checks", () => {
    ...
    expect(parseCliArgs([], { AFORA_ADDITIONAL_BOUNDARY_SHARD: "4/4" })).toEqual({
      help: false,
      shardSpec: "4/4",
    });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-boundary-timeout-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-boundary-signal-"));
      ...
      try {
        ...
        const parentScript = [
          ...
          `const child = spawn(process.execPath, ['-e', ${JSON.stringify(childScript)}], { stdio: 'ignore' });`,
          "fs.writeFileSync(process.env.AFORA_TEST_CHILD_PID, String(child.pid));",
          "fs.writeFileSync(process.env.AFORA_TEST_READY, 'ready');",
