...
describe("dev tooling safety helpers", () => {
  ...
  it("redacts home paths and masks opaque ids", () => {
    expect(redactHomePath("/home/alice/.afora/state.json", "/home/alice")).toBe(
      "~/.afora/state.json",
    );
...
describe("script-specific dev tooling hardening", () => {
  ...
  it("rejects unknown Discord smoke args before live Discord/Afora work", async () => {
  ...
  it("reads TUI PTY mirror updates incrementally with a bounded chunk", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-tui-watch-test-"));
  ...
  it("rejects invalid OpenAI realtime smoke timeout values", () => {
    ...
    expect(() => realtimeSmokeTesting.resolveOpenAIHttpTimeoutMs("2s")).toThrow(
      /AFORA_REALTIME_OPENAI_HTTP_TIMEOUT_MS must be an integer/u,
    );
  });
  ...
  it("reads only the bounded Anthropic prompt probe gateway log tail", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-prompt-probe-log-"));
    tempDirs.push(tempRoot);
  ...
  it("cleans Anthropic prompt probe temp dirs unless explicitly kept", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-prompt-probe-test-"));
    const keepRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-prompt-probe-test-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-direct-prompt-tree-"));
      tempDirs.push(tempRoot);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-direct-parent-signal-"));
      tempDirs.push(tempRoot);
      const descendantPidPath = path.join(tempRoot, "descendant.pid");
      let descendantPid = 0;
      const fakeClaudeBin = await writeFakePromptCli(tempRoot, descendantPidPath);
      const probe = spawn(
        ...
        ["--import", "tsx", "scripts/anthropic-prompt-probe.ts"],
        {
          ...
          env: {
            ...
            AFORA_PROMPT_TEXT: "parent signal cleanup proof",
            AFORA_PROMPT_TIMEOUT_MS: "10000",
            AFORA_PROMPT_TRANSPORT: "direct",
          },
          stdio: "ignore",
        },
      );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-prompt-gateway-tree-"));
      tempDirs.push(tempRoot);
      const descendantPidPath = path.join(tempRoot, "descendant.pid");
      let descendantPid = 0;
      const descendantScript = [
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-prompt-parent-signal-"));
      tempDirs.push(tempRoot);
      const descendantPidPath = path.join(tempRoot, "descendant.pid");
  ...
  it("rejects invalid Claude usage timeout values", () => {
    ...
    expect(() => claudeUsageTesting.resolveFetchTimeoutMs("1.5")).toThrow(
      /AFORA_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS must be an integer/u,
    );
  });
