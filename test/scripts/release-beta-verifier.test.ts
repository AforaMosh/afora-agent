...
import {
  ...
  resolveAforaNpmPostpublishVerifier,
  runNpmViewWithRetry,
...
describe("parseReleaseVerifyBetaArgs", () => {
  ...
  it("parses child run IDs and repair flags", () => {
    expect(
      ...
      pluginSelection: ["@afora/plugin-a", "@afora/plugin-b"],
      clawHubBootstrapPlugins: ["@afora/plugin-b"],
  ...
  it("only accepts the trusted tooling postpublish verifier override", () => {
    expect(resolveAforaNpmPostpublishVerifier("/tmp/release")).toBe(
      "/tmp/release/scripts/afora-npm-postpublish-verify.ts",
    );
    const trustedVerifier = resolve("scripts/afora-npm-postpublish-verify.ts");
    expect(resolveAforaNpmPostpublishVerifier("/tmp/release", trustedVerifier)).toBe(
      trustedVerifier,
    );
    expect(() =>
      resolveAforaNpmPostpublishVerifier("/tmp/release", "/tmp/untrusted-verifier.ts"),
...
describe("validateClawHubBootstrapEvidence", () => {
  ...
  const run = {
    ...
    status: "completed",
    conclusion: "success",
    html_url: "https://github.com/AforaMosh/afora-agent/actions/runs/34",
  ...
  function validate(
    ...
    return validateClawHubBootstrapEvidence({
      ...
      expectedPackages: overrides.expectedPackages ?? ["@afora/meta"],
  ...
  it("rejects legacy release-ref runs and mismatched target/package evidence", () => {
    ...
    expect(() => validate({ expectedPackages: ["@afora/other"] })).toThrow(
    ...
    expect(
      parseNpmViewFields(
        JSON.stringify({
          ...
          "dist.tarball": "https://registry.example/afora.tgz",
        }),
        "beta",
      ),
    ).toEqual({
      ...
      tarball: "https://registry.example/afora.tgz",
    });
  });
...
describe("runNpmViewWithRetry", () => {
  it("retries transient registry failures with online metadata reads", async () => {
    ...
    await expect(
      runNpmViewWithRetry(["view", "afora@2026.5.10-beta.3", "version", "--json"], {
        attempts: 3,
        delay: async (delayMs) => {
          delays.push(delayMs);
        },
