...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "afora-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "afora-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$AFORA_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$AFORA_TEST_CONTRIB"',
    ],
    {
      AFORA_PR_CHANGELOG_SH: changelogScriptPath,
      AFORA_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$AFORA_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      AFORA_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @afora.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @Afora.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "afora", text: "- Org-owned fix. Thanks @afora." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      { line: 4, handle: "afora", text: "- Mixed credit. Thanks @contributor and @Afora." },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @afora and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "afora",
        text: "- Mixed credit (#123). Thanks @afora and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("afora")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("afora-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("afora-clawsweeper[bot]")).toBe(true);
      ...
      try {
        run(
          ...
          [
            ...
            `
set -euo pipefail
source "$AFORA_PR_COMMON_SH"
source "$AFORA_PR_CHANGELOG_SH"
source "$AFORA_PR_GATES_SH"
...
`,
          ...
          {
            AFORA_PR_COMMON_SH: commonScriptPath,
            AFORA_PR_CHANGELOG_SH: changelogScriptPath,
            AFORA_PR_GATES_SH: gatesScriptPath,
            AFORA_TEST_CALLS: callsPath,
            AFORA_TESTBOX: "0",
          },
        );
      } catch (error) {
...
`,
        ...
        {
          AFORA_ALLOW_ROOT_CHANGELOG_PR: "1",
          AFORA_PR_COMMON_SH: commonScriptPath,
          AFORA_PR_CHANGELOG_SH: changelogScriptPath,
          AFORA_PR_GATES_SH: gatesScriptPath,
          AFORA_TEST_CALLS: callsPath,
