...
function writeFixture(params?: {
  ...
  const root = makeTempDir(tempDirs, "afora-release-version-");
  fs.mkdirSync(path.join(root, "apps", "macos", "Sources", "Afora", "Resources"), {
    recursive: true,
  });
  ...
  fs.writeFileSync(
    path.join(root, "apps", "macos", "Sources", "Afora", "Resources", "Info.plist"),
    [
      "<plist>",
      "<dict>",
...
describe("release version planning", () => {
  ...
  it("keeps an existing Android build increment on the same release train", () => {
    ...
    ).toContain("AFORA_ANDROID_VERSION_CODE=2026070102");
    expect(
      fs.readFileSync(
        path.join(
          root,
