...
const PACKAGE_NAME = "@afora/meta-provider";
...
const TARBALL_NAME = "afora-meta-provider-2026.7.1-beta.3.tgz";
const MANUAL_OVERRIDE_REASON =
  "Afora Release Publish run 12345 approved token release for v2026.7.1-beta.3";
...
const PUBLISHER_POLICY = {
  ...
  schema: "afora.plugin-npm-publisher-policy/v1",
...
function tempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "afora-plugin-publication-artifact-"));
  tempDirs.push(dir);
  return dir;
}
...
function createFixture(
  ...
  const tarball = createTarball(
    options.tarEntries ?? [
      ...
      { content: '{"id":"meta"}\n', path: "package/afora.plugin.json" },
...
describe("plugin publication artifact", () => {
    ...
    expect(() =>
      verifyFixture(fixture, {
        manualOverrideReason:
          "Afora Release Publish run 12345 approved token release for v2026.7.1-beta.4",
      }),
  ...
  it("rejects every mutable artifact identity dimension", () => {
    ...
    const mismatches: Array<[string, Record<string, unknown>]> = [
      ...
      ["repository", { repository: "afora-agent/not-afora" }],
      ...
      ["package name", { packageName: "@afora/not-meta" }],
  ...
  it("binds authoritative workflow status, conclusion, and head repository", () => {
    const mutations = [
      ...
      (run: Record<string, unknown>) => {
        run.head_repository = { full_name: "afora-agent/not-afora" };
      },
    ];
  ...
  it("requires the exact Meta package name, dir, and both publication flags", () => {
    ...
    const cases: Array<{
      ...
      {
        manifestOverrides: {
          afora: { release: { publishToClawHub: false, publishToNpm: true } },
        },
      },
      {
        manifestOverrides: {
          afora: { release: { publishToClawHub: true, publishToNpm: false } },
        },
      },
      {
        manifestOverrides: { name: "@afora/not-meta" },
        params: { packageName: "@afora/not-meta" },
      },
      {
