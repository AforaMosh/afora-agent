...
const WORKFLOW_PATH = ".github/workflows/afora-release-telegram-qa.yml";
...
function runIdentityVerification(params: {
  ...
  const trustedWorkflowRef = `${repository}/.github/workflows/afora-release-telegram-qa.yml@refs/heads/main`;
  const invocation = params.invocation ?? "dispatch";
  const workflowRef =
    invocation === "dispatch"
      ? trustedWorkflowRef
      : `${repository}/.github/workflows/afora-release-checks.yml@refs/heads/release-ci/test`;
  const workflowRefName =
    invocation === "dispatch" ? "refs/heads/main" : "refs/heads/release-ci/test";
  const workdir = tempDirs.make("afora-telegram-identity-");
...
function runAdvisoryStatus(overrides: Record<string, string> = {}) {
  ...
  const workdir = tempDirs.make("afora-telegram-advisory-status-");
  const githubOutput = join(workdir, "github-output");
...
function runCandidateProvenance(
  ...
  const workdir = tempDirs.make("afora-telegram-provenance-");
  const fakeBin = join(workdir, "bin");
  mkdirSync(fakeBin);
          ...
          associatedPullRequests: {
            nodes: [
                ...
                ? [
                    {
                      ...
                      headRepository: { nameWithOwner: "AforaMosh/afora-agent" },
                    },
                  ]
                : []),
              ...(params.mergedPullRequests ?? []).map((pullRequest) => ({
                ...
                baseRepository: {
                  nameWithOwner: pullRequest.baseRepository ?? "AforaMosh/afora-agent",
                },
...
describe("release Telegram QA workflow", () => {
    ...
    for (const provenanceBlock of PROVENANCE_BLOCKS) {
      const cases = [
        ...
        },
        {
          ...
          params: {
            candidateVersion: "2026.7.1",
            mergedPullRequests: [{ baseRepository: "fork/afora" }],
            signature: "web-flow" as const,
            targetContextRef: "release/2026.7.1",
          },
        },
  ...
  it.runIf(process.platform === "linux")("retains only bounded, allowlisted diagnostics", () => {
    ...
    const workdir = tempDirs.make("afora-telegram-diagnostics-");
  ...
  it.runIf(process.platform === "linux")("keeps the newest eight gateway logs", () => {
    ...
    const workdir = tempDirs.make("afora-telegram-log-selector-");
    ...
    const paths = Array.from({ length: 12 }, (_, index) => {
      const path = join(runtimeRoot, "tmp", `afora-${index}.log`);
  ...
  it("keeps generated SUT programs syntactically valid", () => {
    ...
    const workdir = tempDirs.make("afora-telegram-preload-");
    const preloadPath = join(workdir, "preload.mjs");
    writeFileSync(preloadPath, preload);
    const env = { ...process.env };
    delete env.AFORA_QA_SUT_PREENTRY_STOP;
    expect(
