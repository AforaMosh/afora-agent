...
function runAssertInstalled({
  ...
  const pluginId = "afora-kitchen-sink-fixture";
  const home = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-home-"));
  const installPath = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-install-"));
  ...
  const installsPath = path.join(home, ".afora", "plugins", "installs.json");
  ...
  try {
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          installPath,
          resolvedSpec: "@afora/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          source: "npm",
          spec: "@afora/kitchen-sink@latest",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        KITCHEN_SINK_SPEC: "npm:@afora/kitchen-sink@latest",
...
function runAssertClawhubInstalled({
  ...
  const installPath = installPathRelative
    ...
    : path.join(home, ".afora", "extensions", pluginId);
  const scratchRoot = tmpdir();
  const pluginsJsonPath = path.join(scratchRoot, `kitchen-sink-${label}-plugins.json`);
  const inspectJsonPath = path.join(scratchRoot, `kitchen-sink-${label}-inspect.json`);
  const inspectAllJsonPath = path.join(scratchRoot, `kitchen-sink-${label}-inspect-all.json`);
  ...
  const installsPath = path.join(home, ".afora", "plugins", "installs.json");
  try {
    mkdirSync(path.join(home, ".afora", "extensions"), { recursive: true });
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          ...
          installPath,
          resolvedSpec: "clawhub:@afora/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          resolvedAt: 1,
          source: "clawhub",
          spec: "clawhub:@afora/kitchen-sink@latest",
          version: "1.0.0",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        KITCHEN_SINK_SPEC: "clawhub:@afora/kitchen-sink@latest",
...
describe("kitchen-sink plugin assertions", () => {
  it("bounds expected-failure output before matching failure diagnostics", () => {
    const scratchRoot = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-failure-cap-"));
      ...
      const result = spawnSync(
        ...
        [ASSERTIONS_SCRIPT, "expect-failure", outputPath],
        {
          ...
          env: {
            ...
            KITCHEN_SINK_SOURCE: "npm",
            KITCHEN_SINK_SPEC: "npm:@afora/kitchen-sink@0.0.0",
          },
        },
      );
  ...
  it("persists the scenario personality in plugin config", () => {
    const home = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-config-"));
    try {
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "configure-runtime"], {
        ...
        env: {
          ...process.env,
          HOME: home,
          KITCHEN_SINK_ID: "afora-kitchen-sink-fixture",
      ...
      const config = JSON.parse(
        readFileSync(path.join(home, ".afora", "afora.json"), "utf8"),
      );
      expect(config.plugins.entries["afora-kitchen-sink-fixture"]).toMatchObject({
  ...
  it("fails kitchen-sink inspect-all diagnostics for the installed plugin", () => {
    const inspectPayload = fullSurfaceInspectPayload("afora-kitchen-sink-fixture");
    const result = runAssertInstalled({
      allInspectPayload: [
        {
          ...inspectPayload,
  ...
  it("accepts ClawHub kitchen-sink fixtures with a context engine", () => {
    const result = runAssertClawhubInstalled({
      contextEngineIds: ["afora-kitchen-sink-fixture"],
    });
  ...
  it("rejects ClawHub kitchen-sink install paths that resolve outside managed extensions", () => {
    const result = runAssertClawhubInstalled({
      contextEngineIds: ["afora-kitchen-sink-fixture"],
      installPathRelative: [".afora", "extensions", "..", "escaped-kitchen-sink"].join(path.sep),
    });
  ...
  it("scans only the configured kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-scan-"));
  ...
  it("bounds irrelevant Afora home traversal during log scans", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-scan-"));
    const home = path.join(parent, "home");
    const scratchRoot = path.join(parent, "scratch");
    try {
      mkdirSync(path.join(home, ".afora"), { recursive: true });
      mkdirSync(scratchRoot, { recursive: true });
      writeFileSync(path.join(scratchRoot, "scenario.log"), "0 errors\n");
      for (let index = 0; index < 20; index += 1) {
        const dir = path.join(home, ".afora", `cache-${index}`);
  ...
  it("allocates an isolated scratch root by default", () => {
    ...
    expect(sweep).toContain('mktemp -d "/tmp/afora-kitchen-sink.XXXXXX"');
    ...
    expect(sweep).not.toContain('mktemp -d "/tmp/afora-kitchen-sink-clawhub.XXXXXX"');
  });

  it("cleans the default kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-cleanup-"));
...
`,
      ...
      expect(scratchRoot).toContain("/tmp/afora-kitchen-sink.");
  ...
  it("preserves successful kitchen-sink CLI command logs for the final scan", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-log-"));
    const scratchRoot = path.join(parent, "scratch");
...
export AFORA_ENTRY="$ENTRY"
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_afora_logged "install/log" plugins install demo
...
`,
  ...
  it("bounds printed kitchen-sink CLI command logs without truncating saved logs", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-log-print-"));
    const scratchRoot = path.join(parent, "scratch");
    const entry = path.join(parent, "entry.mjs");
    try {
      mkdirSync(scratchRoot, { recursive: true });
...
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_afora_logged "install/noisy" plugins install demo
...
`,
  ...
  it("rejects invalid kitchen-sink log byte limits before CLI setup", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-log-invalid-"));
    const scratchRoot = path.join(parent, "scratch");
    const entry = path.join(parent, "entry.mjs");
    try {
      mkdirSync(scratchRoot, { recursive: true });
...
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export AFORA_ENTRY="$ENTRY"
export AFORA_DOCKER_E2E_LOG_PRINT_BYTES=64kb
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_afora_logged "install/log" plugins install demo
`,
        {
  ...
  it("includes expected-failure transcripts in the final kitchen-sink log scan", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-failure-log-"));
    const home = path.join(parent, "home");
    const scratchRoot = path.join(parent, "scratch");
    try {
      mkdirSync(home, { recursive: true });
...
export KITCHEN_SINK_SPEC=npm:@afora/kitchen-sink@0.0.0
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_expect_failure "install/failure" bash -c 'printf "%s\\n" "npm ERR! No matching version @afora/kitchen-sink@0.0.0"; exit 1'
...
`,
  ...
  it("cleans a ClawHub fixture server that times out before readiness", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-clawhub-"));
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export AFORA_CLAWHUB_FIXTURE_WAIT_ATTEMPTS=1
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
...
`,
  ...
  it("rejects invalid ClawHub fixture wait attempts before starting the server", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-clawhub-attempts-"));
    const fakeBin = path.join(parent, "bin");
    const scratchRoot = path.join(parent, "scratch");
    const fixtureDir = path.join(scratchRoot, "clawhub-fixture");
    const nodeShim = path.join(fakeBin, "node");
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export AFORA_CLAWHUB_FIXTURE_WAIT_ATTEMPTS=2x
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
start_kitchen_sink_clawhub_fixture_server "$FIXTURE_DIR"
status="$?"
...
`,
      ...
      expect(result.status).toBe(2);
      expect(result.stderr).toContain("invalid AFORA_CLAWHUB_FIXTURE_WAIT_ATTEMPTS: 2x");
  ...
  it("bounds ClawHub fixture server logs on startup timeout", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "afora-kitchen-sink-clawhub-log-"));
    const fakeBin = path.join(parent, "bin");
    const scratchRoot = path.join(parent, "scratch");
    const fixtureDir = path.join(scratchRoot, "clawhub-fixture");
    const nodeShim = path.join(fakeBin, "node");
