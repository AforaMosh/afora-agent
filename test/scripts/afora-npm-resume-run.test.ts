...
import {
  resolveAforaNpmResumeRun,
  runAforaNpmResumeGh,
  validateAforaNpmResumeRun,
} from "../../scripts/afora-npm-resume-run.mts";
import type { AforaNpmResumeValidationInput } from "../../scripts/afora-npm-resume-run.mts";

const SHA = "a".repeat(40);
const TAG_OBJECT_SHA = "b".repeat(40);
const BRANCH = `release-publish/${SHA.slice(0, 12)}-123`;
const URL = "https://github.com/AforaMosh/afora-agent/actions/runs/456";

function fixture(
  overrides: Partial<AforaNpmResumeValidationInput> = {},
): AforaNpmResumeValidationInput {
  return {
    ...
    run: {
      ...
      head_sha: SHA,
      html_url: URL,
      path: `.github/workflows/afora-npm-release.yml@refs/tags/${BRANCH}`,
      workflow_id: 101,
    },
    tag: {
...
describe("afora npm resume run identity", () => {
  it("bounds each GitHub lookup", () => {
    const execFileSyncImpl = vi.fn(() => "result");

    expect(
      runAforaNpmResumeGh(["api", "repos/AforaMosh/afora-agent/actions/runs/456"], {
        execFileSyncImpl,
      }),
    ).toBe("result");
    expect(execFileSyncImpl).toHaveBeenCalledWith(
      "gh",
      ["api", "repos/AforaMosh/afora-agent/actions/runs/456"],
      {
        encoding: "utf8",
        killSignal: "SIGKILL",
  ...
  it("accepts a successful run bound to a signed main-reachable tooling tag", () => {
    expect(validateAforaNpmResumeRun(fixture())).toEqual({
  ...
  it.each([
    ...
    expect(() => validateAforaNpmResumeRun(fixture(overrides))).toThrow(message);
  });

  it("loads the exact run, workflow, signed tag, ancestry, and approval job", () => {
    const responses = new Map<string, unknown>([
      [`api repos/AforaMosh/afora-agent/actions/runs/456 --method GET`, fixture().run],
      [
        `api repos/AforaMosh/afora-agent/actions/workflows/afora-npm-release.yml --method GET`,
        { id: 101 },
      ],
      [`api repos/AforaMosh/afora-agent/git/ref/tags/${BRANCH} --method GET`, fixture().tagRef],
      [`api repos/AforaMosh/afora-agent/git/tags/${TAG_OBJECT_SHA} --method GET`, fixture().tag],
      [`api repos/AforaMosh/afora-agent/compare/${SHA}...main --method GET`, { status: "identical" }],
      [`run view 456 --repo AforaMosh/afora-agent --json jobs --jq .jobs`, fixture().jobs],
    ]);
    ...
    expect(
      resolveAforaNpmResumeRun({ repo: "AforaMosh/afora-agent", runGh, runId: "456" }),
