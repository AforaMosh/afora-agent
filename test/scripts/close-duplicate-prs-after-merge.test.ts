...
function pr(params: {
  ...
  return {
    ...
    url: `https://github.com/AforaMosh/afora-agent/pull/${params.number}`,
  };
}
    ...
    expect(plan).toStrictEqual([
      {
        action: "close",
        candidate,
        comment: `Thanks for the fix. This is now covered by the landed #70532 / commit https://github.com/AforaMosh/afora-agent/commit/6415e35.
...
Closing #70592 as a duplicate.`,
  ...
  it("dry-runs through gh reads without mutating", () => {
    ...
    const responses = new Map<string, string>([
      [
        "pr view 70532 --repo AforaMosh/afora-agent --json number,title,body,state,mergedAt,mergeCommit,closingIssuesReferences,files,url",
        JSON.stringify(
          pr({
            body: "Fixes #70491",
            mergeCommit: "6415e35",
      ...
      [
        "pr view 70592 --repo AforaMosh/afora-agent --json number,title,body,state,mergedAt,mergeCommit,closingIssuesReferences,files,url",
        JSON.stringify(pr({ body: "Closes #70491", number: 70592 })),
      ],
      [
        "pr diff 70532 --repo AforaMosh/afora-agent --color=never",
        "diff --git a/ui/src/ui/chat/grouped-render.ts b/ui/src/ui/chat/grouped-render.ts\n@@ -402,8 +402,11 @@",
      ],
      [
        "pr diff 70592 --repo AforaMosh/afora-agent --color=never",
        "diff --git a/ui/src/ui/chat/grouped-render.ts b/ui/src/ui/chat/grouped-render.ts\n@@ -286,8 +286,11 @@",
      ],
    ]);
  ...
  it("applies labels, comment, and close commands for close actions", () => {
    ...
    expect(calls).toEqual([
      ...
      ["pr", "comment", "70592", "--repo", "AforaMosh/afora-agent", "--body", "closing"],
      ["pr", "close", "70592", "--repo", "AforaMosh/afora-agent"],
    ]);
  });
});
