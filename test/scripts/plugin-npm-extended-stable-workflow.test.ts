...
const metaManifestPath = "extensions/meta/afora.plugin.json";
...
describe("plugin npm extended-stable workflow", () => {
  ...
  it("uses one override for check, plan, preview, pack, and publish", () => {
    ...
    for (const name of [
      ...
      ).toMatchObject({ AFORA_PLUGIN_NPM_PUBLISH_TAG: expectedOverride });
    }
  });
  ...
  it("prepares and independently reads back immutable package evidence", () => {
    ...
    expect(prepare.run).toContain('kind: "afora-plugin-npm-preflight"');
  ...
  it("attests the canonical Meta provider package and install route", () => {
    ...
    expect(packageJson.name).toBe("@afora/meta-provider");
    expect(packageJson.afora?.install?.npmSpec).toBe("@afora/meta-provider");
  ...
  it("publishes extended-stable with OIDC only and verifies every package tag", () => {
    ...
    expect(publish.env).toMatchObject({
      AFORA_NPM_PUBLISH_AUTH_MODE: "trusted-publisher",
    });
    ...
    expect(bootstrap.run).not.toContain("@afora/meta-provider");
