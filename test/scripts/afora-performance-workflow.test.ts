// Afora Performance Workflow tests cover afora performance workflow script behavior.
...
const WORKFLOW = ".github/workflows/afora-performance.yml";
...
describe("Afora performance workflow", () => {
  it("uses an optional dispatch identifier to name parent-owned runs", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");

    expect(workflow).toContain(
      "run-name: ${{ inputs.dispatch_id != '' && format('Afora Performance {0}', inputs.dispatch_id) || 'Afora Performance' }}",
    );
  ...
  it("pins the Kova evaluator with release validation contracts", () => {
    ...
    const resolveTarget = findStep("Resolve Afora target ref", "resolve_target");
    ...
    expect(readWorkflow().jobs?.kova?.env?.KOVA_AFORA_CONFIG_CONTRACT).toBe(
  ...
  it("keeps live credentials away from custom Kova refs", () => {
    ...
    const root = mkdtempSync(join(realpathSync(tmpdir()), "afora-kova-live-ref-"));
  ...
  it("resolves each target once before benchmark and publication fan out", () => {
    ...
    const resolveTarget = findStep("Resolve Afora target ref", "resolve_target");
    const checkout = findStep("Checkout Afora");
    const record = findStep("Record tested revision");
    const sourceCheckout = findStep("Checkout Afora source target", "source_performance");
    ...
    expect(
      Object.values(workflow.jobs ?? {})
        ...
        .filter((step) => step.name === "Resolve Afora target ref"),
    ).toHaveLength(1);
  });
  ...
  it("sparse-fetches only the public source baseline without publisher credentials", () => {
    ...
    expect(run).toContain('remote add origin "https://github.com/afora/clawgrit-reports.git"');
  ...
  it("builds only the QA and startup artifacts required by source probes", () => {
    const run = findStep("Run Afora source performance probes", "source_performance").run ?? "";
    const build =
      "AFORA_BUILD_PRIVATE_QA=1 node --import tsx scripts/build-all.mts sourcePerformance";
  ...
  it("runs trusted CLI performance cases against the frozen candidate entrypoint", () => {
    ...
    expect(run).toContain('--entry "$GITHUB_WORKSPACE/afora.mjs"');
  ...
  it("isolates required publication in a fresh artifact-consuming job", () => {
    ...
    expect(kovaSteps.some((step) => step.name === "Run Afora source performance probes")).toBe(
      false,
    );
    expect(
      workflow.jobs?.source_performance?.steps?.some(
        (step) => step.name === "Run Afora source performance probes",
      ),
    ).toBe(true);
  ...
  it("keeps app credentials out of artifact processing and scopes them to Git push", () => {
    ...
    expect(prepare.run).toContain(
      'source_path="${input_root}/afora-performance-source-${GITHUB_RUN_ID}-${SOURCE_PRODUCER_ATTEMPT}/${LANE_ID}"',
    );
    ...
    expect(prepare.run).toContain(
      'remote add origin "https://github.com/afora/clawgrit-reports.git"',
    );
  ...
  it("publishes bounded bundle metadata while retaining full diagnostics as an artifact", () => {
    ...
    expect(upload.with?.path).not.toContain(".artifacts/afora-performance/source");
    expect(sourceUpload.with).toMatchObject({
      name: "afora-performance-source-${{ github.run_id }}-${{ github.run_attempt }}",
      path: ".artifacts/afora-performance/source",
  ...
  it("reuses the producing artifact when only publisher jobs rerun", () => {
    const artifact = findStep("Resolve Kova artifact", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "afora-artifact-resolver-"));
    ...
    writeFileSync(
      ...
      `#!/bin/sh
printf '%s\\n' \
  '101	afora-performance-mock-provider-9001-1' \
  '202	afora-performance-source-9001-2' \
  '303	afora-performance-mock-provider-9001-3'
`,
    );
  ...
  it("advertises a clawgrit URL only after a direct or remotely verified push", () => {
    const publish = findStep("Publish to clawgrit reports", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "afora-publish-shell-"));
    const bin = join(root, "bin");
    const reportsRoot = join(root, "reports");
    const reportUrl =
      "https://github.com/afora/clawgrit-reports/tree/main/afora-performance/main/123-1/mock-provider";
    mkdirSync(bin);
...
case "$*" in
  ...
  *"remote get-url origin"*) echo https://github.com/afora/clawgrit-reports.git ;;
...
`,
    ...
    const execute = ({
      ...
      const result = spawnSync("bash", ["-c", publish.run ?? ""], {
        ...
        env: {
          ...
          DEST_REL: "afora-performance/main/123-1/mock-provider",
  ...
  it("preserves both reports when concurrent writers update one latest pointer", () => {
    const root = mkdtempSync(join(realpathSync(tmpdir()), "afora-report-race-"));
    ...
    const latest = "afora-performance/main/latest-mock-provider.json";
    const reportA = "afora-performance/main/100-1/mock-provider";
    const reportB = "afora-performance/main/200-1/mock-provider";
    ...
    const commitReport = (repo: string, reportPath: string, marker: string) => {
      ...
      runGit(repo, ["add", "--", "afora-performance"]);
  ...
  it("preserves required PARTIAL failures and clears only advisory PARTIAL failures", () => {
    ...
    const root = tempDirs.make("afora-kova-partial-gate-");
  ...
  it("lets OCM discover its native workspace dependency adapter", () => {
    ...
    expect(workflowText).not.toContain("AFORA_OCM_NPM_WRAPPER");
    expect(workflowText).not.toContain("AFORA_OCM_WORKSPACE_DEPENDENCY_DIRS");
