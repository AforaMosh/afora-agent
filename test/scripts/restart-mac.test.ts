...
function runGatewayPortCheck(fakeLsof: string) {
  const root = mkdtempSync(join(tmpdir(), "afora-restart-mac-test-"));
  tempRoots.push(root);
...
function runCleanupFunction(fakePs: string) {
  ...
  writeFileSync(
    ...
    [
      ...
      'DEBUG_PROCESS_PATTERN="/worktree/apps/macos/.build/debug/Afora"',
      'LOCAL_PROCESS_PATTERN="/worktree/apps/macos/.build-local/debug/Afora"',
      'RELEASE_PROCESS_PATTERN="/worktree/apps/macos/.build/release/Afora"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$AFORA_TEST_KILL_CALLS"',
      "  return 0",
      "}",
      "kill_all_afora",
    ].join("\n"),
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      AFORA_TEST_KILL_CALLS: killCallsPath,
      PATH: `${binDir}:${process.env.PATH ?? ""}`,
    },
  });
...
function runManagedSupervisorClassifier(
  ...
  const root = mkdtempSync(join(tmpdir(), "afora-restart-mac-supervisor-test-"));
  tempRoots.push(root);
  ...
  const classifierFunctions = script.slice(
    script.indexOf("print_managed_afora_supervisor_label()"),
    script.indexOf("kill_managed_afora()"),
  );
  ...
  writeFileSync(
    ...
    [
      ...
      classifierFunctions,
      "loaded_launch_jobs() {",
      '  [[ "${AFORA_TEST_FAIL_ENUMERATION:-0}" != "1" ]] || return 1',
      "  cut -d'|' -f1,2 \"$AFORA_TEST_LOADED_JOBS\"",
      "}",
      "launch_job_snapshot() {",
      '  grep "^$1|$2|" "$AFORA_TEST_LOADED_JOBS" |',
      "    awk -F'|' '{ print \"program = \" $3; print \"properties = \" $4 }'",
      "}",
      'TARGET_EXECUTABLE="/worktree/dist/Afora.app/Contents/MacOS/Afora"',
      'INSTALLED_EXECUTABLE="/Applications/Afora.app/Contents/MacOS/Afora"',
  ...
  return spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      AFORA_TEST_FAIL_ENUMERATION: options.failEnumeration ? "1" : "0",
      AFORA_TEST_LOADED_JOBS: recordsPath,
    },
  });
}
...
function runSigningEnvironmentBlock(signIdentity: string) {
  const root = mkdtempSync(join(tmpdir(), "afora-restart-mac-signing-test-"));
  tempRoots.push(root);
  const script = readFileSync(restartScriptPath, "utf8");
...
function runProfileGuard(profile: string) {
  const root = mkdtempSync(join(tmpdir(), "afora-restart-mac-profile-test-"));
  tempRoots.push(root);
  const script = readFileSync(restartScriptPath, "utf8");
  const start = script.indexOf('if [[ -n "${AFORA_PROFILE:-}" ]]');
  ...
  return spawnSync("/bin/bash", [harnessPath], {
    encoding: "utf8",
    env: { ...process.env, AFORA_PROFILE: profile },
  });
}
...
function runForeignProcessClassifier(fakePs: string) {
  ...
  writeFileSync(
    ...
    [
      ...
      functions,
      'APP_EXECUTABLE_RELATIVE_PATH="Contents/MacOS/Afora"',
      'TARGET_EXECUTABLE="/Users/steipete/afora/dist/Afora.app/Contents/MacOS/Afora"',
      'INSTALLED_EXECUTABLE="/Applications/Afora.app/Contents/MacOS/Afora"',
...
describe("scripts/restart-mac.sh", () => {
  ...
  it("keeps the default restart log scoped to the current worktree lock", () => {
    ...
    expect(script).not.toContain('LOG_PATH="${AFORA_RESTART_LOG:-/tmp/afora-restart.log}"');
  });

  it("rejects named app profiles before global process or launchd cleanup", () => {
    const script = readFileSync(restartScriptPath, "utf8");

    expect(script).toContain('if [[ -n "${AFORA_PROFILE:-}"');
  ...
  it("does not remove a live restart lock it did not acquire", () => {
    const root = mkdtempSync(join(tmpdir(), "afora-restart-mac-test-"));
    tempRoots.push(root);
    const lockDir = join(root, "afora-restart-lock");
    mkdirSync(lockDir);
  ...
  it("prefers the freshly packaged app unless an explicit app bundle is set", () => {
    ...
    expect(script).toContain('fail "AFORA_APP_BUNDLE does not exist: ${APP_BUNDLE}"');
    expect(chooseBlock).toContain("canonicalize_app_bundle");
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Afora.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("/Applications/Afora.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Afora.app")).toBeLessThan(
      chooseBlock.indexOf("/Applications/Afora.app"),
    );
  });

  it("keeps restart cleanup scoped to known Afora app and build paths", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    ...
    expect(cleanupBlock).toContain('"/Applications/Afora.app/${APP_EXECUTABLE_RELATIVE_PATH}"');
  ...
  it("target-only mode refuses foreign app processes without broad cleanup", () => {
    ...
    expect(initialTargetBlock).toContain("foreign_afora_process_pids");
    expect(initialTargetBlock).not.toContain("kill_managed_afora");
    expect(initialTargetBlock).not.toContain("stop_launch_agent");
    expect(initialTargetBlock).not.toContain("kill_all_afora");
    expect(switchTargetBlock).toContain("foreign_afora_process_pids");
    expect(switchTargetBlock).toContain("kill_managed_afora");
  ...
  it("finds persistent launchd supervisors across explicit domains", () => {
    const result = runManagedSupervisorClassifier([
      {
        domain: "gui/501",
        label: "ai.afora.mac.custom",
        program: "/Applications/Afora.app/Contents/MacOS/Afora",
        properties: "keepalive | runatload",
      },
      {
        domain: "user/501",
        label: "ai.afora.mac.target",
        program: "/worktree/dist/Afora.app/Contents/MacOS/Afora",
        properties: "keepalive",
      },
      {
        domain: "gui/501",
        label: "application.ai.afora.mac.123",
        program: "/Applications/Afora.app/Contents/MacOS/Afora",
      },
      {
        domain: "system",
  ...
  it("checks managed launchd supervisors before starting the Swift package build", () => {
    ...
    const supervisorIndex = script.indexOf(
      'managed_supervisors="$(managed_afora_supervisor_labels',
    );
  ...
  it("keeps the managed app alive until the signed replacement is ready", () => {
    ...
    expect(script).toContain('AFORA_PACKAGE_APP_ROOT="${STAGED_APP_BUNDLE}"');
  ...
  it("escalates only exact managed app processes when graceful shutdown stalls", () => {
    ...
    const broadKillBlock = script.slice(
      script.indexOf("kill_all_afora()"),
      script.indexOf("known_afora_executables()"),
    );

    expect(managedKillBlock).toContain('kill -KILL "${pid}"');
    expect(managedKillBlock).toContain("managed_afora_process_pids");
  ...
  it("treats the canonical installed app as managed but temp bundles as foreign", () => {
    const result = runForeignProcessClassifier(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  101 /Applications/Afora.app/Contents/MacOS/Afora --attach-only'",
        "printf '%s\\n' '  102 /Users/steipete/afora/dist/Afora.app/Contents/MacOS/Afora --attach-only'",
        "printf '%s\\n' '  103 /tmp/agent/Afora.app/Contents/MacOS/Afora --attach-only'",
        "printf '%s\\n' '  104 /bin/sh test.sh /Applications/Afora.app/Contents/MacOS/Afora'",
      ].join("\n"),
    );
  ...
  it("normalizes custom app bundle paths before process matching", () => {
    const root = mkdtempSync(join(tmpdir(), "afora-restart-mac-test-"));
    tempRoots.push(root);
    const appBundle = join(root, "dist", "Afora.app");
    mkdirSync(appBundle, { recursive: true });

    const { result } = runCanonicalizeAppBundle(`${appBundle}/../Afora.app/`);
  ...
  it("fails restart cleanup when scoped processes survive every kill attempt", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  321 /worktree/dist/Afora.app/Contents/MacOS/Afora --attach-only'",
      ].join("\n"),
    );
  ...
  it("waits beyond the app signal failsafe for scoped processes to exit", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        'kill_count="$(wc -l < "$AFORA_TEST_KILL_CALLS" 2>/dev/null || echo 0)"',
  ...
  it("keeps the restart grace period longer than the app signal failsafe", () => {
    ...
    const watcher = readFileSync(
      "apps/macos/Sources/Afora/TerminationSignalWatcher.swift",
      "utf8",
    );
  ...
  it("does not kill unrelated Afora app bundles", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  654 /tmp/Other/Afora.app/Contents/MacOS/Afora'",
      ].join("\n"),
    );
