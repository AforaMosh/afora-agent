...
} from "../../.agents/skills/afora-live-updater/scripts/update-main.mjs";
import {
  BUILD_STAMP_FILE,
...
const script = path.join(repoRoot, ".agents/skills/afora-live-updater/scripts/update-main.mjs");
...
function initializeFixture(root: string) {
  ...
  const canonicalOrigin = "https://github.com/AforaMosh/afora-agent.git";
...
function makeFixture(options?: { includeSeed?: boolean }) {
  ...
  const root = realpathSync(tempDirs.make("afora-live-updater-"));
  const origin = path.join(root, "origin.git");
  const seed = path.join(root, "seed");
  const mirror = path.join(root, "mirror");
...
function createManagedLaunchAgentFixture(root: string, mirror: string) {
  ...
  const plistPath = path.join(root, "ai.afora.gateway.plist");
...
describe("afora live updater", () => {
  let cleanupProbeRoot = "";

  beforeAll(() => {
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), "afora-live-updater-template-")));
  ...
  test("parses the loaded launchd ProgramArguments block", () => {
    expect(
      parseLaunchctlArguments(`gui/501/ai.afora.gateway = {
\tprogram = /bin/sh
\targuments = {
\t\t/bin/sh
\t\t/Users/test/.afora/service-env/ai.afora.gateway-env-wrapper.sh
\t\t/Users/test/.afora/service-env/ai.afora.gateway.env
\t\t/opt/homebrew/bin/node
\t\t/Users/test/afora/dist/index.js
\t\tgateway
\t\t--port
\t\t18789
\t}
\tpid = 123
  ...
  test("fails closed on same-label system LaunchDaemon ownership", () => {
    ...
    expect(() =>
      assertNoSystemLaunchDaemonOwnership("ai.afora.gateway", {
        readdirSync: () => ["com.example.other.plist", "afora-system.plist"],
        spawnSync: (command: string, args: string[]) => {
          ...
          return {
            ...
            stdout: JSON.stringify({
              Label: args.at(-1)?.endsWith("afora-system.plist")
                ? "ai.afora.gateway"
                : "com.example.other",
            }),
            stderr: "",
    ...
    ).toThrow("afora-system.plist already owns the managed Gateway label");
      ...
      assertNoSystemLaunchDaemonOwnership("ai.afora.gateway", {
        ...
        spawnSync: (command: string, args: string[]) => {
          ...
          return calls.length === 1
            ? missing
            : { status: 0, stdout: "system/ai.afora.gateway", stderr: "" };
        },
      }),
    ).toThrow("system/ai.afora.gateway already owns the managed Gateway label");
  ...
  test("ignores restart-window logs emitted by a foreign Afora checkout", () => {
    const root = tempDirs.make("afora-log-attribution-");
    const sourceRoot = path.join(root, "managed/afora/dist");
    const foreignRoot = path.join(root, "worktree/afora");
    mkdirSync(path.join(foreignRoot, ".git"), { recursive: true });
    writeFileSync(path.join(foreignRoot, "package.json"), '{"name":"afora"}\n');
    const output = [
      {
        "0": '{"subsystem":"gateway"}',
        "1": "managed warning",
  ...
  test("keeps managed restart logs when the deployment root is symlinked", () => {
    const root = tempDirs.make("afora-log-symlink-attribution-");
    ...
    writeFileSync(path.join(releaseRoot, "package.json"), '{"name":"afora"}\n');
  ...
  test("scopes embedded RPC records without dropping unattributed errors", () => {
    const root = tempDirs.make("afora-rpc-log-attribution-");
    const sourceRoot = path.join(root, "managed/afora/dist");
    const foreignRoot = path.join(root, "worktree/afora");
    mkdirSync(path.join(foreignRoot, ".git"), { recursive: true });
    writeFileSync(path.join(foreignRoot, "package.json"), '{"name":"afora"}\n');
    ...
    expect(
      resolveManagedPluginSourceRoots({
        plugins: [
          ...
          { id: "workspace", rootDir: "/srv/workspace/.afora/extensions/workspace" },
          { id: "global", rootDir: "/Users/test/.afora/extensions/global" },
        ],
      }),
    ).toEqual([
      "/opt/configured-plugin",
      "/srv/workspace/.afora/extensions/workspace",
      "/Users/test/.afora/extensions/global",
    ]);
  ...
  test("scopes restart logs to the effective managed runtime", () => {
    expect(
      resolveManagedGatewaySourceRoot("/srv/afora", {
  ...
  test("retries bounded Gateway readiness after restart", async () => {
    ...
    expect(calls).toEqual([
      "pnpm afora gateway status --deep --require-rpc --json",
      "pnpm afora gateway status --deep --require-rpc --json",
      "pnpm afora gateway status --deep --require-rpc --json",
      "pnpm afora gateway status --deep --require-rpc --json",
      "pnpm afora gateway status --deep --require-rpc --json",
      "pnpm afora gateway status --deep --require-rpc --json",
      "pnpm afora gateway status --deep --require-rpc --json",
      "pnpm afora health --verbose --json",
    ]);
  });
...
appendFileSync(${JSON.stringify(callsPath)}, JSON.stringify({
  args,
  port: process.env.AFORA_GATEWAY_PORT,
}) + "\\n");
...
`,
  ...
  test("bounds built Gateway CLI probes and cleans their config overlay", () => {
    ...
    expect(
      readdirSync(root).filter((name) => name.startsWith(".afora-live-updater-config-")),
    ).toEqual([]);
  });

  test("parses ready and busy atomic Gateway suspension responses", () => {
    const deployment = {
      configPath: "/snapshot/afora.json",
      ...
      prepareGatewaySuspension(
        ...
        (
          ...
          expect(params).toEqual({
            requestId: expect.stringMatching(/^afora-live-updater-/u),
  ...
  test("retries exact legacy suspension params with preserve semantics", () => {
    const root = realpathSync(tempDirs.make("afora-legacy-gateway-suspension-"));
  ...
  test("does not downgrade unrelated Gateway suspension failures", () => {
    const root = realpathSync(tempDirs.make("afora-gateway-suspension-failure-"));
    const stub = createGatewaySuspensionCliStub(root, {
      type: "gateway_request_error",
      code: "UNAVAILABLE",
  ...
  test("pins managed Gateway calls with a backward-compatible local overlay", () => {
    const root = realpathSync(tempDirs.make("afora-gateway-call-"));
    const checkout = path.join(root, "checkout");
    const entrypoint = path.join(checkout, "dist/index.js");
    ...
    writeFileSync(
      capture,
      'import fs from "node:fs"; console.log(JSON.stringify({ argv: process.argv.slice(2), config: JSON.parse(fs.readFileSync(process.env.AFORA_CONFIG_PATH, "utf8")), hasToken: Boolean(process.env.AFORA_GATEWAY_TOKEN), url: process.env.AFORA_GATEWAY_URL ?? null }));\n',
    );
    ...
    const result = JSON.parse(
      runBuiltGatewayCall(
        ...
        { requestId: "request-1" },
        {
          ...
          port: 19001,
          serviceEnvironment: { AFORA_GATEWAY_TOKEN: ["fixture", "value"].join("-") },
          wrapperPath: null,
        },
      ),
    );
  ...
  test("accepts supported Afora GitHub origins", () => {
    expect(originMatches("https://github.com/AforaMosh/afora-agent.git")).toBe(true);
    expect(originMatches("git@github.com:AforaMosh/afora-agent.git")).toBe(true);
    expect(originMatches("https://github.com/example/afora.git")).toBe(false);
  });
  ...
  test("accepts only immutable canonical runtime snapshots owned by the checkout", () => {
    ...
    const runtimeRoot = path.join(home, ".afora/runtime");
    const snapshot = path.join(runtimeRoot, `gateway-${head.slice(0, 7)}`);
    mkdirSync(runtimeRoot, { recursive: true });
    git(runtimeRoot, "clone", origin, snapshot);
    git(snapshot, "remote", "set-url", "origin", "https://github.com/AforaMosh/afora-agent.git");
  ...
  test("accepts owned entrypoints only in supported LaunchAgent command layouts", () => {
    ...
    const entrypoint = "/Users/test/.afora/runtime/gateway-1234567/dist/index.js";
    const wrapper = `${home}/.afora/service-env/ai.afora.gateway-env-wrapper.sh`;
    const envFile = `${home}/.afora/service-env/ai.afora.gateway.env`;
    ...
    const customWrapper = `${customState}/service-env/ai.afora.gateway-env-wrapper.sh`;
    const customEnvFile = `${customState}/service-env/ai.afora.gateway.env`;
    expect(
      resolveManagedGatewayEntrypoint(
  ...
  test("retargets an accepted snapshot to the exact source build", () => {
    const checkout = "/Users/test/afora";
    const snapshot = "/Users/test/.afora/runtime/gateway-1234567/dist/index.js";
    const source = path.join(checkout, "dist/index.js");
    const replacements: string[] = [];
    const deployment = {
      configPath: "/Users/test/config/afora.json",
      entrypoint: snapshot,
      entrypointIndex: 1,
      label: "ai.afora.gateway",
      plistPath: "/Users/test/Library/LaunchAgents/ai.afora.gateway.plist",
      port: 18789,
    };
  ...
  test("replaces a wrapped LaunchAgent entrypoint without inserting another argument", () => {
    const snapshot = "/Users/test/.afora/runtime/gateway-1234567/dist/index.js";
    const source = "/Users/test/afora/dist/index.js";
    const original = [
      "/bin/sh",
      "/Users/test/.afora/service-env/ai.afora.gateway-env-wrapper.sh",
      "/Users/test/.afora/service-env/ai.afora.gateway.env",
  ...
  test("classifies exact-head build, install, macOS rebuild, and native UI proof", () => {
    ...
    expect(
      classifyActions(["package.json", "apps/macos/Sources/Afora/AppDelegate.swift"], {
        buildProvenanceKnown: true,
        buildRequired: true,
        nodeModulesPresent: true,
      }),
    ...
    expect(
      classifyActions(["apps/shared/AforaKit/Sources/AforaProtocol/GatewayModels.swift"], {
        buildProvenanceKnown: true,
        buildRequired: true,
        nodeModulesPresent: true,
      }),
  ...
  test("accepts only the delayed exact target bundle process", () => {
    const executable = "/fixture/live-checkout/dist/Afora.app/Contents/MacOS/Afora";
    const foreign = "41 /tmp/agent/Afora.app/Contents/MacOS/Afora";
  ...
  test("fast-forwards, builds exact SHA, restarts Gateway, then proves exact Mac target", async () => {
    ...
    mkdirSync(path.join(seed, "apps/macos/Sources/Afora"), { recursive: true });
    writeFileSync(path.join(seed, "apps/macos/Sources/Afora/App.swift"), "// changed\n");
    git(seed, "add", ".");
    ...
    const output = await maintainFixture(
      ...
      },
      {
        ...
        verifyMacTarget: () => ({
          executable: path.join(mirror, "dist/Afora.app/Contents/MacOS/Afora"),
          pid: 123,
        }),
      },
    );
    ...
    expect(output.buildChangedPaths).toEqual(["apps/macos/Sources/Afora/App.swift"]);
    ...
    expect(output.macTarget?.executable).toBe(
      path.join(mirror, "dist/Afora.app/Contents/MacOS/Afora"),
    );
  });
  ...
  test("preserves the signed Mac bundle while a Gateway build replaces dist", async () => {
    const { root, mirror } = makeFixture();
    mkdirSync(path.join(mirror, "node_modules"));
    const appBundle = path.join(mirror, "dist/Afora.app");
    ...
    expect(
      readdirSync(path.join(mirror, ".git")).filter((entry) =>
        entry.startsWith(".afora-live-mac-"),
      ),
    ).toEqual([]);
  });
  ...
  test("restores the Mac bundle when the Gateway build fails", async () => {
    const { root, mirror } = makeFixture();
    mkdirSync(path.join(mirror, "node_modules"));
    const appMarker = path.join(mirror, "dist/Afora.app/Contents/signature-marker");
    mkdirSync(path.dirname(appMarker), { recursive: true });
    writeFileSync(appMarker, "signed\n");
  ...
  test("accepts a delayed external restore of the exact preserved Mac bundle", async () => {
    ...
    writeFileSync(appMarker, "signed\n");
    const commands = fakeCommands(mirror);
    const delayedBundle = path.join(root, "delayed-afora.app");
    let restored = false;
  ...
  test("repoints an ancestor snapshot across the next source update", async () => {
    ...
    expect(commands.calls).toEqual([
      ...
      `/bin/launchctl bootout gui/${uid}/ai.afora.gateway`,
      "prove gateway stopped",
      "pnpm build",
      "install replacement plist",
      `/bin/launchctl setenv AFORA_GATEWAY_STARTUP_TRACE 1`,
      `/bin/launchctl enable gui/${uid}/ai.afora.gateway`,
      `/bin/launchctl bootstrap gui/${uid} ${plistPath}`,
      `/bin/launchctl unsetenv AFORA_GATEWAY_STARTUP_TRACE`,
    ]);
  });
  ...
  test("restores an absent managed service past bootout exit 3 and an unlabeled vendor plist", async () => {
    ...
    expect(deployedEntrypoint).toBe(snapshot);
    expect(calls).toContain(`/bin/launchctl bootout gui/${uid}/ai.afora.gateway`);
  ...
  test("bootstraps an owned LaunchAgent left unloaded by a failed restart", async () => {
    ...
    expect(commands.calls).toEqual([
      ...
      `/bin/launchctl enable gui/${uid}/ai.afora.gateway`,
      `/bin/launchctl bootstrap gui/${uid} ${plistPath}`,
      `/bin/launchctl setenv AFORA_GATEWAY_STARTUP_TRACE already-enabled`,
    ...
    await expect(
      maintainMain(
        { checkout: mirror, remote: "origin", lockPath: path.join(root, "maintenance.lock") },
        {
          fetchMain: fetchFixtureMain,
          runCommand(command: string, args: string[]) {
            if (command === "pnpm" && args.slice(0, 3).join(" ") === "afora gateway status") {
              statusCalls += 1;
              throw new Error("RPC unavailable");
            }
          },
