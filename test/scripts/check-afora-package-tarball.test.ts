// Check Afora Package Tarball tests cover check afora package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-afora-package-tarball.mts";
const PUBLIC_CHECK_SCRIPT = "scripts/check-afora-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "afora-package-tarball-test-"));
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: "afora", version, ...options.packageJson }),
    );
    writeFileSync(
    ...
    const controlUiFiles =
      options.includeControlUi === false
        ? {}
        : {
            "dist/control-ui/index.html": "<!doctype html><afora-app></afora-app>",
      ...
      options.includeInstallGuard === false
        ...
        : {
            [PACKAGE_INSTALL_GUARD_RELATIVE_PATH]:
              "Afora package preinstall has not completed.\n",
          };
    ...
    const tarball = join(root, "afora.tgz");
...
describe("check-afora-package-tarball", () => {
  it("prints help before touching tarball state", () => {
    ...
    expect(result.stdout).toContain(
      "Usage: node scripts/check-afora-package-tarball.mjs [--require-bundled-workspace-deps] <afora.tgz>",
    );
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown Afora package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("Afora package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "afora.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected Afora package tarball check argument: extra");
    expect(extra.stderr).not.toContain("Afora package tarball does not exist");
  });
  ...
  it("accepts tarballs whose entry list exceeds Node's default spawn buffer", () => {
    ...
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status, result.stderr).toBe(0);
        expect(result.stdout).toContain("Afora package tarball integrity passed.");
      },
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), "afora-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.AFORA_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            AFORA_TEST_EXTRACT_DIR_FILE: extractDirFile,
  ...
  it("rejects workspace protocol dependencies in package manifests", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "package.json dependencies.@afora/ai must not use workspace protocol workspace:*",
        );
      },
      "2026.6.11",
      { packageJson: { dependencies: { "@afora/ai": "workspace:*" } } },
    );
  });
  ...
  it("accepts separately published private workspace dependencies by default", () => {
    withTarball(
      ...
      "2026.6.11",
      { packageJson: { dependencies: { "@afora/ai": "2026.6.11" } } },
    );
  });
  ...
  it("rejects private workspace dependencies that are not bundled when strict packaging requires it", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "package.json dependencies.@afora/ai must be listed in bundleDependencies because it is private to the Afora workspace",
        );
        expect(result.stderr).toContain(
          "package.json dependencies.@afora/ai must be bundled in node_modules/@afora/ai",
        );
      },
      "2026.6.11",
      { packageJson: { dependencies: { "@afora/ai": "2026.6.11" } } },
  ...
  it("rejects private workspace dependencies when only metadata is bundled", () => {
    withTarball(
      ...
      {
        "dist/index.js": "export {};\n",
        "node_modules/@afora/ai/package.json": AI_RUNTIME_PACKAGE_JSON,
      },
      (tarball) => {
        const result = spawnSync(
          "node",
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "bundled @afora/ai is missing required runtime entry dist/index.mjs",
        );
        expect(result.stderr).toContain(
          "bundled @afora/ai is missing required runtime entry dist/providers.mjs",
        );
        expect(result.stderr).toContain(
          "bundled @afora/ai is missing required runtime entry dist/internal/runtime.mjs",
        );
      },
      "2026.6.11",
      {
        packageJson: {
          dependencies: { "@afora/ai": "2026.6.11" },
  ...
  it("accepts private workspace dependencies when their runtime is bundled", () => {
    withTarball(
      ["dist/index.js"],
      {
        "dist/index.js": "export {};\n",
        "node_modules/@afora/ai/package.json": AI_RUNTIME_PACKAGE_JSON,
        "node_modules/@afora/ai/dist/index.mjs": "export {};\n",
        "node_modules/@afora/ai/dist/providers.mjs": "export {};\n",
        "node_modules/@afora/ai/dist/transports.mjs": "export {};\n",
        "node_modules/@afora/ai/dist/internal/openai-responses-payload-policy.mjs":
          "export {};\n",
        "node_modules/@afora/ai/dist/internal/runtime.mjs": "export {};\n",
      },
      (tarball) => {
        const result = spawnSync(
          "node",
  ...
  it("accepts frozen AI runtimes that predate an optional exported subpath", () => {
    withTarball(
      ...
      {
        "dist/index.js": "export {};\n",
        "node_modules/@afora/ai/package.json": LEGACY_AI_RUNTIME_PACKAGE_JSON,
        "node_modules/@afora/ai/dist/index.mjs": "export {};\n",
        "node_modules/@afora/ai/dist/providers.mjs": "export {};\n",
        "node_modules/@afora/ai/dist/internal/runtime.mjs": "export {};\n",
      },
      ...
      {
        packageJson: {
          dependencies: { "@afora/ai": "2026.7.2-beta.4" },
          bundleDependencies: ["@afora/ai"],
        },
      },
    );
  ...
  it("rejects bundled AI entries that its manifest does not export", () => {
    withTarball(
      ...
      {
        "dist/index.js": "export {};\n",
        "node_modules/@afora/ai/package.json": JSON.stringify({
          name: "@afora/ai",
          version: "2026.6.11",
          exports: {
            ".": "./dist/index.mjs",
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "bundled @afora/ai runtime specifier @afora/ai/providers is not resolvable",
        );
      },
      "2026.6.11",
      {
  ...
  it("rejects missing relative imports from bundled AI runtime entries", () => {
    withTarball(
      ["dist/index.js"],
      {
        ...
        "node_modules/@afora/ai/dist/internal/openai-responses-payload-policy.mjs":
          "export {};\n",
        "node_modules/@afora/ai/dist/internal/runtime.mjs": 'export * from "./missing.mjs";\n',
      },
      (tarball) => {
        const result = spawnSync(
          "node",
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "bundled @afora/ai dist/internal/runtime.mjs imports missing dist/internal/missing.mjs",
        );
      },
      "2026.6.11",
      {
