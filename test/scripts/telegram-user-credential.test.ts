...
describe("telegram user credential path handling", () => {
  ...
  it("writes private JSON files", async () => {
    const dir = tempDirs.make("afora-telegram-credential-");
...
describe("telegram user credential IO", () => {
  ...
  it("rejects loose and unsafe credential timeout env values", async () => {
    const previous = process.env.AFORA_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS;
    try {
      for (const value of ["1e3", String(Number.MAX_SAFE_INTEGER + 1)]) {
        process.env.AFORA_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS = value;
        await expect(
          import(
            `${new URL("../../scripts/e2e/telegram-user-credential.ts", import.meta.url).href}?case=loose-timeout-${value}-${Date.now()}`
          ),
        ).rejects.toThrow(
          `AFORA_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS must be a positive integer. Got: ${JSON.stringify(value)}.`,
        );
      }
    } finally {
      if (previous === undefined) {
        delete process.env.AFORA_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS;
      } else {
        process.env.AFORA_TELEGRAM_USER_CREDENTIAL_COMMAND_TIMEOUT_MS = previous;
      }
    }
  });
  ...
  it("rejects loose numeric credential limits instead of parsing prefixes", async () => {
    ...
    ).toThrow('AFORA_QA_CREDENTIAL_LEASE_TTL_MS must be a positive integer. Got: "1e3".');
    expect(() =>
      credentialModule.optionalPositiveInteger(
        "9007199254740992",
        30_000,
      ...
      'AFORA_QA_CREDENTIAL_PAYLOAD_MAX_BYTES must be a positive integer. Got: "9007199254740992".',
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const dir = tempDirs.make("afora-telegram-credential-timeout-");
...
`,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const dir = tempDirs.make("afora-telegram-credential-tree-timeout-clean-");
  ...
  it.runIf(process.platform !== "win32")("kills timed-out child process groups", async () => {
    const dir = tempDirs.make("afora-telegram-credential-tree-timeout-");
    const childPidPath = path.join(dir, "child.pid");
    let childPid: number | undefined;
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const dir = tempDirs.make("afora-telegram-credential-signal-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const dir = tempDirs.make("afora-telegram-credential-grandchild-signal-");
      const runnerPath = path.join(dir, "runner.mjs");
      const readyPath = path.join(dir, "ready.txt");
      ...
      const runner = spawn(process.execPath, ["--import", "tsx", runnerPath], {
        env: {
          ...process.env,
          AFORA_QA_CREDENTIAL_KILL_GRACE_MS: "100",
        },
        stdio: ["ignore", "pipe", "pipe"],
      });
