...
function makeTempRepo() {
  const dir = mkdtempSync(path.join(tmpdir(), "afora-root-deps-audit-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("collectRootDependencyOwnershipCheckErrors", () => {
  ...
  it("allows runtime deps for bundled plugins that are still packaged in core", () => {
    ...
    writeRepoFile(repoRoot, "extensions/internal/afora.plugin.json", JSON.stringify({}));
    writeRepoFile(
      repoRoot,
  ...
  it("keeps excluded bundled plugin deps localizable", () => {
    ...
    writeRepoFile(repoRoot, "extensions/externalized/afora.plugin.json", JSON.stringify({}));
    writeRepoFile(
      repoRoot,
