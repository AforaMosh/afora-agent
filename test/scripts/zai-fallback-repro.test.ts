...
describe("zai fallback repro command resolution", () => {
  it("wraps Windows pnpm.cmd without Node shell argv", () => {
    expect(
      resolveZaiFallbackPnpmCommand(["afora", "agent", "--message", "hello world"], {
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\pnpm.cmd" afora agent --message "hello world""`,
      ],
  ...
  it("scans session transcripts with a byte cap", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-zai-session-test-"));
    try {
  ...
  it("cleans temporary repro state after fallback proof", async () => {
    ...
    const exitCode = await runZaiFallbackRepro({
      env: {
        ...
        AFORA_ZAI_FALLBACK_SESSION_ID: "session-test",
      ...
      log: () => {},
      mkdtemp: (async () => {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-zai-fallback-test-"));
        tempRoots.push(root);
        return root;
