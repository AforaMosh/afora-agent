...
describe("Mantis Web UI chat proof workflow", () => {
  ...
  it("only treats explicit candidate assignments as PR head overrides", () => {
    ...
    expect(
      "@afora-mantis web ui chat proof: verify candidate=e63393c publishes evidence".match(
        pattern,
      )?.[1],
    ).toBe("e63393c");
    expect(
      "@afora-mantis web ui chat proof: verify head: e63393c publishes evidence".match(
        pattern,
      )?.[1],
    ).toBe("e63393c");
    expect(
