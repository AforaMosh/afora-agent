// Afora NPM Publish tests cover publish wrapper argument safety.
...
const scriptPath = "scripts/afora-npm-publish.sh";
...
function makeReleaseCheckout(root: string, version: string): string {
  ...
  copyFileSync(
    "scripts/afora-npm-extended-stable-release.mjs",
    path.join(checkout, "scripts/afora-npm-extended-stable-release.mjs"),
  );
...
function makePackageTarball(root: string, packageJson?: string): string {
  ...
  const tarball = path.join(root, "afora.tgz");
...
describe("afora npm publish wrapper", () => {
  ...
  it("rejects extra publish arguments before npm publish", () => {
    const tempRoot = tempDirs.make("afora-npm-publish-");
    const tarball = path.join(tempRoot, "afora.tgz");
  ...
  it.each([
    ...
    ["missing version", JSON.stringify({ name: "afora" }), "has no valid version"],
  ...
  it("rejects publishing the current pre-.33 final version to extended-stable", () => {
    ...
    const result = runPublishWrapper(
      ["--publish"],
      {
        AFORA_NPM_PUBLISH_TAG: "extended-stable",
      },
      checkout,
    );
