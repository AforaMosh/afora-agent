// Afora launcher E2E tests validate launcher process behavior.
...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "afora-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "afora.mjs"),
    path.join(fixtureRoot, "afora.mjs"),
  );
  await fs.copyFile(
...
async function addGitMarker(fixtureRoot: string): Promise<void> {
  await fs.writeFile(path.join(fixtureRoot, ".git"), "gitdir: .git/worktrees/afora\n", "utf8");
}
...
async function addCompileCacheProbe(fixtureRoot: string): Promise<void> {
  await fs.writeFile(
    ...
    [
      ...
      '  `${module.getCompileCacheDir?.() ? "cache:enabled" : "cache:disabled"};respawn:${process.env.AFORA_COMPILE_CACHE_DISABLED_RESPAWNED ?? "0"}`',
      ");",
    ].join("\n"),
    "utf8",
  );
...
function launcherEnv(extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  delete env.AFORA_BUNDLED_PLUGINS_DIR;
  delete env.AFORA_CONFIG_PATH;
  delete env.AFORA_DISABLE_BUNDLED_PLUGINS;
  delete env.AFORA_HOME;
  delete env.AFORA_STATE_DIR;
  delete env.AFORA_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
describe("afora launcher", () => {
  ...
  it("keeps the bootstrap Node range aligned with the package engine", async () => {
    ...
    for (const version of NODE_RELEASE_VERSION_CASES) {
      ...
      if (isSupportedAforaNodeVersion(version)) {
        ...
        expect(result.stderr, version).toContain(
          `afora: Node.js >=22.22.3 <23, >=24.15.0 <25, or >=25.9.0 is required (current: v${version}).`,
        );
      }
    }
  });
  ...
  it("prints recovery guidance before legacy-incompatible modules can load", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(legacyRuntimePath).href, path.join(fixtureRoot, "afora.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    expect(result.stderr).toContain(
      "afora: Node.js >=22.22.3 <23, >=24.15.0 <25, or >=25.9.0 is required (current: v20.0.0).",
    );
  ...
  it("rejects Bun without node:sqlite even when its Node compatibility version is new enough", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(mockRuntime).href, path.join(fixtureRoot, "afora.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      'import "missing-afora-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "afora.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    expect(result.stderr).toContain("missing-afora-launcher-dep");
  ...
  it("executes an entry.mjs-only compiled entry through the root launcher", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", import.meta.resolve("tsx"), path.join(fixtureRoot, "afora.mjs"), "--profile"],
      {
        cwd: process.cwd(),
        env: launcherEnv({ AFORA_NO_RESPAWN: "1" }),
        encoding: "utf8",
      },
    );
  ...
  it.runIf(process.env.AFORA_TEST_BUN_LAUNCHER === "1" && hasBunRuntime())(
    ...
    async () => {
      ...
      const result = spawnSync(bunBin, [path.join(fixtureRoot, "afora.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
      });
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "afora.mjs"), params.command, "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(["doctor", "gateway", "models", "plugins", "sessions", "tasks"])(
    ...
    async (command) => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "afora.mjs"), command, "--help"],
        {
          cwd: fixtureRoot,
          env: launcherEnv(),
          encoding: "utf8",
  ...
  it("uses precomputed subcommand help with leading root options", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "afora.mjs"), "--profile", "work", "--no-color", "models", "-h"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("defers precomputed subcommand help to the runtime entry when container env is set", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "afora.mjs"), "models", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ AFORA_CONTAINER: "demo" }),
        encoding: "utf8",
      },
    );
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "afora.mjs"), ...params.args],
      {
        cwd: fixtureRoot,
  ...
  it("defers root help to the runtime entry when plugin config can change help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const configPath = path.join(fixtureRoot, "afora.json");
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "cli-startup-metadata.json"),
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "afora.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ AFORA_CONFIG_PATH: configPath }),
      encoding: "utf8",
    });
  ...
  it("defers nodes help to the runtime entry when plugin config can change help", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "afora.mjs"), "nodes", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ AFORA_CONFIG_PATH: configPath }),
        encoding: "utf8",
  ...
  it("checks the AFORA_HOME default config path before using precomputed root help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const aforaHome = path.join(fixtureRoot, "home");
    const configDir = path.join(aforaHome, ".afora");
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "afora.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ AFORA_HOME: aforaHome }),
      encoding: "utf8",
    });
  ...
  it("checks legacy config candidates before using precomputed root help", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "afora.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ HOME: home, AFORA_HOME: undefined }),
      encoding: "utf8",
    });
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:AforaMosh/afora-agent#<ref>");
  });
  ...
  it("respawns source-checkout launchers without inherited NODE_COMPILE_CACHE", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "afora.mjs")], {
      cwd: fixtureRoot,
      env: launcherEnv({
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'process.title = "afora-launcher-sigterm-test-child";',
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "afora.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
  ...
  it.runIf(process.platform !== "win32").each([
    ...
    await fs.writeFile(
      ...
      [
        'import { writeFileSync } from "node:fs";',
        `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
        'process.title = "afora-launcher-default-signal-test-child";',
        "setInterval(() => {}, 1000);",
        "",
      ].join("\n"),
      "utf8",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          'import { writeFileSync } from "node:fs";',
          'process.title = "afora-launcher-sigterm-ignore-test-child";',
          'process.on("SIGTERM", () => {});',
          `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
          "setInterval(() => {}, 1000);",
          "",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await addGitMarker(fixtureRoot);
      await addCompileCacheProbe(fixtureRoot);
      const linkParent = makeTempDir(fixtureRoots, "afora-launcher-link-");
      const linkedRoot = path.join(linkParent, "afora-linked");
      await fs.symlink(fixtureRoot, linkedRoot, "dir");

      const result = spawnSync(process.execPath, [path.join(linkedRoot, "afora.mjs")], {
        cwd: linkParent,
        env: launcherEnv({
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.writeFile(
        path.join(fixtureRoot, "dist", "entry.js"),
        'process.stdout.write(process.env.AFORA_PACKAGED_COMPILE_CACHE_RESPAWNED ?? "0");\n',
        "utf8",
      );

      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "afora.mjs"), "hooks", "relay"],
        {
          cwd: fixtureRoot,
          env: launcherEnv({
            NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
  ...
  it("scopes packaged launcher compile cache inside configured cache roots", async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join(".node-compile-cache", "afora", "2026.4.29"));
  });

  it("falls back to the default packaged launcher compile cache when NODE_COMPILE_CACHE is empty", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const runCwd = makeTempDir(fixtureRoots, "afora-launcher-cwd-");
    const tmpRoot = makeTempDir(fixtureRoots, "afora-launcher-tmp-");
    await fs.writeFile(path.join(fixtureRoot, "package.json"), '{"version":"2026.4.29"}\n');
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      [
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join("node-compile-cache", "afora", "2026.4.29"));
    expect(result.stdout).not.toContain(path.join(runCwd, "afora"));
  });
