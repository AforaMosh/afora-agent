...
function createConfig(providerBaseUrl: string, maxBytes: number): AforaConfig {
  return {
    models: {
      providers: {
        openai: {
...
async function createAudioContext(
  ...
  tempRoot ??= await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "afora-audio-qa-")));
