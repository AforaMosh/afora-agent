...
import { createAforaCodingTools } from "../../../../src/agents/agent-tools.js";
...
function toolNames(
  config: AforaConfig,
  options: Pick<AforaCodingToolsOptions, "sandbox"> = {},
) {
  return new Set(
    createAforaCodingTools({
      ...
      agentId: "policy",
      workspaceDir: path.join(os.tmpdir(), "afora-tool-policy-workspace"),
      agentDir: path.join(os.tmpdir(), "afora-tool-policy-agent"),
...
test("Afora applies every configured tool policy as a restrictive intersection", () => {
  const profileConfig: AforaConfig = {
    tools: { profile: "coding" },
  };
  expectIncluded(toolNames(profileConfig), ["read", "write", "edit", "exec"], ["message"]);

  const globalConfig: AforaConfig = {
    tools: {
      profile: "coding",
  ...
  const providerConfig: AforaConfig = {
    tools: {
      ...globalConfig.tools,
      byProvider: {
        openai: {
  ...
  const sandboxDir = path.join(os.tmpdir(), "afora-tool-policy-sandbox");
  const sandbox = createAgentToolsSandboxContext({
    workspaceDir: sandboxDir,
    agentWorkspaceDir: path.join(os.tmpdir(), "afora-tool-policy-workspace"),
    workspaceAccess: "rw",
