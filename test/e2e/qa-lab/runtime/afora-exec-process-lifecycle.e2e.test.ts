...
test("Afora executes and controls the complete real process lifecycle", async () => {
  ...
  try {
    ...
    const foregroundCommand =
      process.platform === "win32"
        ? `Write-Output -NoNewline ${shellQuote(shellMarker)}; Write-Output -NoNewline "|$env:AFORA_SHELL"`
        : `printf '%s' ${shellQuote(shellMarker)} && printf '|%s' "$AFORA_SHELL"`;
    ...
    const pty = await foregroundExecTool.execute("foreground-pty", {
      command: nodeEvalCommand(
        `process.stdout.write(${JSON.stringify(ptyMarker + ":")} + String(Boolean(process.stdout.isTTY)) + ":" + (process.env.AFORA_SHELL || ""));`,
      ),
      pty: true,
    });
