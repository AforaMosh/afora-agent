// Afora bundle MCP tools Docker harness.
...
const probeTool = server.tool("docker_probe", "Docker Afora MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("afora-agent-bundle-mcp-");
