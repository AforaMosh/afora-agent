...
async function captureMonitorLogs(
  ...
  // afora-temp-dir: standalone producer removes this log root in its finally block
  const logDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-health-monitor-proof-"));
