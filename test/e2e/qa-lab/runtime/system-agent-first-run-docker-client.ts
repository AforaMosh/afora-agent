// Afora first-run Docker harness.
...
const DISCORD_CREDENTIAL_FIXTURE = ["afora", "discord", "fixture"].join("-");
...
function resolveDefaultModel(config: AforaConfig): string | undefined {
...
async function runPackagedCli(args: string[]): Promise<PackagedCommandResult> {
  ...
  if (timedOut) {
    throw new Error(
      `Packaged CLI timed out after ${PACKAGED_CLI_TIMEOUT_MS}ms: afora ${args.join(" ")}\n${stdout}\n${stderr}`,
    );
  }
...
async function main() {
  ...
  const tempState = await createE2eStateDir("afora-system-agent-first-run-");
  ...
  setEnvValue("AFORA_STATE_DIR", stateDir);
  setEnvValue("AFORA_CONFIG_PATH", configPath);
  ...
  clearConfigCache();
  assert(
    await shouldStartOnboardingForFreshInstall(["node", "afora"]),
    "fresh bare Afora invocation did not route to onboarding",
  );

  const blocked = await runPackagedCli(["setup", "--message", "overview"]);
  assert(blocked.code === 1, "Afora did not fail closed without inference");
  assert(
    `${blocked.stdout}\n${blocked.stderr}`.includes("afora onboard"),
    "blocked Afora did not direct the user to inference onboarding",
  );
  ...
  const inferenceConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as AforaConfig;
  assert(
  ...
  assert(
    ...
    "inference activation configured the rest before Afora started",
  );
  ...
  assert(
    ...
    "modern compatibility entrypoint did not expose Afora after activation",
  );
  ...
  for (const command of spec.commands) {
    ...
    assert(
      ...
      `Afora first-run command ${command.id} did not apply: ${output}`,
    );
    if (command.id === "setup") {
      assert(result.code === 0, `Afora setup exited with ${result.code}: ${output}`);
      assert(
        output.includes("[afora] done: afora.setup"),
        `Afora setup did not report completion: ${output}`,
      );
      assert(
        output.includes(
          "Gateway: Afora gateway lifecycle is managed by an external supervisor " +
            "(AFORA_SUPERVISOR_MODE=external). Use that supervisor to start the gateway.",
        ),
        `Afora setup did not report the externally supervised gateway: ${output}`,
      );
      assert(
        !output.includes("Systemd user services are not available"),
        `Afora setup probed systemd before honoring external supervision: ${output}`,
      );
      assert(
        !output.includes("Gateway service install failed"),
        `Afora setup attempted and failed gateway service installation: ${output}`,
      );
      assert(
        !output.includes("service management skipped: non-default state dir or config path"),
        `Afora setup used the non-default-path service-management skip: ${output}`,
      );
    }
    if (command.planner) {
      assert(
        output.includes(`[afora] planner: ${spec.model}`) &&
          output.includes(FAKE_PLANNER_REPLY) &&
          output.includes(`[afora] interpreted: ${plannerCommand}`),
        `Afora first-run command ${command.id} did not use the verified planner: ${output}`,
      );
    }
    ...
    assert(
      ...
      `Afora command ${command.id} ran ${probeDelta} inference probes; expected at least ${minimumProbes} for preflight${command.approve ? " plus its persistent boundary" : ""}`,
    );
  }
  ...
  assert(reef, "Afora did not create reef agent");
  assert(reef.workspace === spec.dockerAgentWorkspace, "Afora did not write reef workspace");
  assert(
    reef.model === undefined,
    "Afora wrote a per-agent model instead of inheriting the verified default",
  );
  assert(config.channels?.discord?.enabled === true, "Afora did not enable Discord");
  ...
  assert(
    ...
    "Afora did not write Discord token SecretRef",
  );
  assert(
    !JSON.stringify(config.channels.discord).includes(DISCORD_CREDENTIAL_FIXTURE),
    "Afora persisted the raw Discord token",
  );
  ...
  assert(
    ...
    "Afora persisted an unrelated ambient credential",
  );
  ...
  console.log("Afora first-run Docker E2E passed");
}
