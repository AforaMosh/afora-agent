// QA OTEL Smoke runtime supports Afora repository automation.
...
const DEFAULT_DOCKER_COLLECTOR_IMAGE =
  process.env.AFORA_QA_OTEL_COLLECTOR_IMAGE || "otel/opentelemetry-collector:0.104.0";
...
const REQUIRED_METRIC_NAMES = ["afora.harness.duration_ms"] as const;
...
const MAX_STDOUT_DIAGNOSTIC_LINE_BYTES = readPositiveIntegerEnv(
  "AFORA_QA_OTEL_MAX_STDOUT_DIAGNOSTIC_LINE_BYTES",
  512 * 1024,
);
...
function isStdoutDiagnosticLogRecord(value: unknown): value is StdoutDiagnosticLogRecord {
  return (
    ...
    value !== null &&
    !Array.isArray(value) &&
    objectValue(value, "signal") === "afora.diagnostic.log"
  );
}
...
async function startDockerOtelCollector(
  ...
  const tempDir = await makeTempDir(path.join(osTmpdir(), "afora-otel-collector-"));
  const configPath = path.join(tempDir, "collector.yaml");
  const containerName = `afora-otel-smoke-${makeUuid()}`;
...
function isLatestGenAiModelCallSpan(span: CapturedSpan): boolean {
  ...
  return (
    ...
    typeof span.attributes["afora.provider"] === "string" &&
    typeof span.attributes["afora.model"] === "string"
  );
}
...
async function runDirectTelemetryProducer(params: {
  ...
  process.env.OTEL_SERVICE_NAME = "afora-qa-lab-otel-smoke";
...
function assertSmoke(params: {
  ...
  if (spanNames.has("afora.model.call")) {
    failures.push("legacy afora.model.call span exported with GenAI semconv opt-in");
  }
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("afora.content."));
  ...
  const modelErrorSpans = modelSpans.filter((span) => {
    ...
    return (
      ...
      Object.hasOwn(span.attributes, "afora.errorCategory") ||
  ...
  const failedRunSpans = params.spans.filter(
    (span) =>
      (span.name === "afora.run" || span.name === "afora.harness.run") &&
      span.attributes["afora.error"] === `${DIRECT_ERROR_MESSAGE} OPENAI_API_KEY=***`,
  );
  if (failedRunSpans.length !== 2) {
    const observed = params.spans
      .filter((span) => span.name === "afora.run" || span.name === "afora.harness.run")
      .map((span) => ({ name: span.name, error: span.attributes["afora.error"] }));
    failures.push(
