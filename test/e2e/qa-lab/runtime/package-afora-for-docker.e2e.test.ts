// Package Afora For Docker tests cover QA Lab package artifact evidence.
...
import {
  ...
  packAforaPackageForDocker,
  parseArgs,
  prepareBundledAiRuntimePackage,
  runCaptureForTest,
  runCommandForTest,
  writePackageInventoryForDocker,
} from "../../../../scripts/package-afora-for-docker.mts";
...
describe("package-afora-for-docker", () => {
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = tempDirs.make("afora-package-pnpm-runner-");
      fs.writeFileSync(
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = tempDirs.make("afora-package-pnpm-timeout-");
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "fs.writeFileSync(process.env.AFORA_TEST_CHILD_PID, String(process.pid));",
          "setInterval(() => {}, 1000);",
        ].join("\n"),
      );
      fs.writeFileSync(
      ...
      env.PATHEXT = ".CMD";
      env.npm_execpath = "";
      env.AFORA_TEST_CHILD_PID = childPidPath;
  ...
  it("loads from a trusted harness checkout without installed dependencies", async () => {
    const tempRoot = tempDirs.make("afora-package-harness-");
    const copiedFiles = [
      "scripts/package-afora-for-docker.mts",
        ...
        (resolve, reject) => {
          const child = spawn(
            ...
            [
              "--import",
              tsxImport,
              path.join(tempRoot, "scripts/package-afora-for-docker.mts"),
              "--invalid",
            ],
  ...
  it("writes inventory for a frozen source checkout without the trusted helper", async () => {
    const sourceDir = tempDirs.make("afora-package-frozen-source-");
    ...
    fs.writeFileSync(path.join(sourceDir, "package.json"), '{"name":"afora"}\n');
    fs.writeFileSync(
  ...
  it("rejects package artifact output names that escape the output directory", () => {
    ...
    expect(parseArgs(["--output-name", "afora-current.tar.gz"]).outputName).toBe(
      "afora-current.tar.gz",
    );
  });
  ...
  it("uses the source package build entrypoint with declaration generation", async () => {
    ...
    const previousTimeout = process.env.AFORA_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    const previousSkipDts = process.env.AFORA_RUN_NODE_SKIP_DTS_BUILD;
    const previousPackageExtensions = process.env.AFORA_EXTENSIONS;
    const previousDockerBuildExtensions = process.env.AFORA_DOCKER_BUILD_EXTENSIONS;
    const previousInternalPluginIds = process.env[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV];
    const previousPrivateQa = process.env.AFORA_BUILD_PRIVATE_QA;
    process.env.AFORA_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    process.env.AFORA_RUN_NODE_SKIP_DTS_BUILD = "1";
    process.env.AFORA_EXTENSIONS = "clickclack";
    process.env.AFORA_DOCKER_BUILD_EXTENSIONS = "slack";
    process.env[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV] = "msteams";
    process.env.AFORA_BUILD_PRIVATE_QA = "1";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.AFORA_BUILD_ALL_NO_PNPM,
            packageExtensions: options.env?.AFORA_EXTENSIONS,
            dockerBuildExtensions: options.env?.AFORA_DOCKER_BUILD_EXTENSIONS,
            internalDockerBuildPluginIds: options.env?.[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV],
            privateQa: options.env?.AFORA_BUILD_PRIVATE_QA,
            skipDts: options.env?.AFORA_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.AFORA_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.AFORA_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
      if (previousSkipDts === undefined) {
        delete process.env.AFORA_RUN_NODE_SKIP_DTS_BUILD;
      } else {
        process.env.AFORA_RUN_NODE_SKIP_DTS_BUILD = previousSkipDts;
      }
      for (const [envName, previousValue] of [
        ["AFORA_EXTENSIONS", previousPackageExtensions],
        ["AFORA_DOCKER_BUILD_EXTENSIONS", previousDockerBuildExtensions],
        [DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV, previousInternalPluginIds],
        ["AFORA_BUILD_PRIVATE_QA", previousPrivateQa],
      ] as const) {
  ...
  it("rejects loose package artifact timeout env values", async () => {
    ...
    try {
      for (const value of ["1e3", "123.9", "9007199254740993", "0"]) {
        process.env.AFORA_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = value;
        ...
        await expect(
          ...
          "AFORA_DOCKER_PACKAGE_BUILD_TIMEOUT_MS must be a positive timeout in milliseconds",
        );
      }
    } finally {
      if (previousTimeout === undefined) {
  ...
  it("bundles and restores the separately packed AI runtime", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-ai-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-ai-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify(
      {
        dependencies: { "@afora/ai": "workspace:*", "dep-a": "workspace:1.2.3" },
        devDependencies: { "@afora/session-url-contract": "workspace:*" },
        files: ["dist"],
        name: "afora",
        version: "2026.6.17",
      },
    ...
    const installedAiPath = path.join(sourceDir, "node_modules", "@afora", "ai");
    const aiPackageJsonPath = path.join(sourceDir, "packages", "ai", "package.json");
    const originalAiPackageJson =
      '{"name":"@afora/ai","version":"2026.6.17","devDependencies":{"@afora/normalization-core":"workspace:*"}}\n';
    ...
    try {
      const cleanup = await prepareBundledAiRuntimePackage(
        ...
        async (command: string, args: string[], cwd: string) => {
          ...
          fs.writeFileSync(path.join(outputDir, "afora-ai-2026.6.17.tgz"), "ai package");
          return "";
        },
        {
          extractAiRuntime: async (_tarballPath: string, destination: string) => {
            fs.writeFileSync(
              ...
              `${JSON.stringify({
                dependencies: {
                  "@afora/private-runtime": "0.0.0-private",
                  "dep-a": "1.2.3",
                },
                name: "@afora/ai",
                version: "2026.6.17",
      ...
      expect(packageJson.dependencies["@afora/ai"]).toBe("2026.6.17");
      expect(packageJson.dependencies["@afora/private-runtime"]).toBeUndefined();
      expect(packageJson.dependencies["dep-a"]).toBe("1.2.3");
      expect(packageJson.devDependencies?.["@afora/session-url-contract"]).toBe("workspace:*");
      expect(packageJson.bundleDependencies).toContain("@afora/ai");
      ...
      expect(fs.existsSync(path.join(outputDir, "afora-ai-2026.6.17.tgz"))).toBe(false);
    } finally {
  ...
  it("keeps real AI runtime pack failures visible for installer diagnostics", async () => {
    const sourceDir = tempDirs.make("afora-docker-ai-failure-source-");
    const outputDir = tempDirs.make("afora-docker-ai-failure-output-");
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify({
      dependencies: { "@afora/ai": "workspace:*" },
      name: "afora",
    })}\n`;
    const aiPackageJsonPath = path.join(sourceDir, "packages", "ai", "package.json");
    const originalAiPackageJson =
      '{"name":"@afora/ai","devDependencies":{"@afora/normalization-core":"workspace:*"}}\n';
    fs.mkdirSync(path.join(sourceDir, "packages", "ai"), { recursive: true });
    fs.writeFileSync(aiPackageJsonPath, originalAiPackageJson);
    fs.writeFileSync(packageJsonPath, originalPackageJson);
  ...
  it("reuses the source manifest lifecycle for ignore-scripts package artifacts", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-manifest-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-manifest-output-"));
    const scriptsDir = path.join(sourceDir, "scripts");
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify(
      {
        devDependencies: {
          "@afora/session-url-contract": "workspace:*",
          vitest: "4.1.10",
        },
        name: "afora",
        version: "2026.8.1",
    ...
    try {
      const tarball = await packAforaPackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          expect(fs.readFileSync(aiPackageJsonPath, "utf8")).toBe(originalAiPackageJson);
          const packedPath = path.join(outputDir, "afora-2026.8.1.tgz");
          fs.writeFileSync(packedPath, "package");
          return `${path.basename(packedPath)}\n`;
        },
      });

      expect(tarball).toBe(path.join(outputDir, "afora-2026.8.1.tgz"));
      expect(fs.readFileSync(packageJsonPath, "utf8")).toBe(originalPackageJson);
      expect(fs.readFileSync(aiPackageJsonPath, "utf8")).toBe(originalAiPackageJson);
      expect(
        fs.existsSync(
  ...
  it("leaves pre-AI-workspace package sources unchanged", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-legacy-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-legacy-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify({
  ...
  it("rejects incomplete AI workspace package sources", async () => {
    const cases = [
      {
        dependencies: { "@afora/ai": "workspace:*" },
        expected: "@afora/ai dependency requires the packages/ai workspace",
        withWorkspace: false,
      },
      {
        dependencies: {},
        expected: "root package.json must declare @afora/ai as a dependency",
        withWorkspace: true,
      },
    ];

    for (const testCase of cases) {
      const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-invalid-source-"));
      const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-invalid-output-"));
      fs.writeFileSync(
        path.join(sourceDir, "package.json"),
        `${JSON.stringify({ dependencies: testCase.dependencies, name: "afora" })}\n`,
      );
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packAforaPackageForDocker("/repo", "/out", {
      ...
      runCaptureImpl: async (command: string, args: string[], cwd: string) => {
        ...
        return "afora-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "afora-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare-docs:/repo",
      "prepare:/repo",
  ...
  it("does not touch other source artifacts when the docs-map lock fails", async () => {
    ...
    await expect(
      packAforaPackageForDocker("/repo", "/out", {
  ...
  it("keeps the docs-map lock when changelog restoration fails", async () => {
    const outputDir = tempDirs.make("afora-package-restore-order-");
    const calls: string[] = [];

    await expect(
      packAforaPackageForDocker("/repo", outputDir, {
        prepareBundledAiRuntime: skipBundledAiRuntime,
        prepareChangelog: async () => {},
        prepareDocsMap: async () => {},
        restoreChangelog: async () => {
  ...
  it("packages Unreleased notes for explicitly non-publish stable artifacts", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-unreleased-package-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-unreleased-output-"));
    ...
    try {
      const tarball = await packAforaPackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "afora-2026.5.29.tgz");
          fs.writeFileSync(packedPath, "package");
          return "afora-2026.5.29.tgz\n";
        },
      });

      expect(tarball).toBe(path.join(outputDir, "afora-2026.5.29.tgz"));
  ...
  it("keeps a frozen pre-map source package byte-owned by that ref", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-frozen-package-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-frozen-output-"));
    const docsDir = path.join(sourceDir, "docs");
    fs.mkdirSync(docsDir);
    fs.writeFileSync(
      path.join(sourceDir, "package.json"),
      '{"name":"afora","version":"2026.6.33"}\n',
    );
    fs.writeFileSync(
      path.join(docsDir, "index.md"),
      '---\nsummary: "Frozen Afora docs"\n---\n\n# Afora\n',
    );
    ...
    try {
      const tarball = await packAforaPackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "afora-2026.6.33.tgz");
          fs.writeFileSync(packedPath, "frozen package");
          return `${path.basename(packedPath)}\n`;
        },
      });

      expect(tarball).toBe(path.join(outputDir, "afora-2026.6.33.tgz"));
      expect(fs.existsSync(path.join(docsDir, "docs_map.md"))).toBe(false);
    } finally {
      fs.rmSync(sourceDir, { recursive: true, force: true });
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("uses pnpm pack when requested", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-pnpm-pack-"));
    const calls: string[] = [];
    const packedPath = path.join(outputDir, "afora-2026.5.28.tgz");

    try {
      const tarball = await packAforaPackageForDocker("/repo", outputDir, {
        ...skipDocsMapLifecycle,
        pnpmPack: true,
        prepareBundledAiRuntime: skipBundledAiRuntime,
        prepareChangelog: async () => {},
  ...
  it("normalizes npm 12 pack metadata for renamed package artifacts", async () => {
    const sourceDir = tempDirs.make("afora-docker-pack-source-");
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-pack-json-"));
    const packJsonPath = path.join(outputDir, "pack.json");
    const npmPackOutput = JSON.stringify({
      afora: {
        entryCount: 15_000,
        filename: "afora-2026.5.28.tgz",
    ...
    try {
      const tarball = await packAforaPackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async (_command, _args, cwd, options) => {
          fs.writeFileSync(path.join(outputDir, "afora-2026.5.28.tgz"), "package");
      ...
      expect(tarball).toBe(path.join(outputDir, "afora-current.tgz"));
  ...
  it("cleans receipts without obscuring runner and parse failures", async () => {
    ...
    for (const failure of ["runner", "parse"] as const) {
      for (const cleanupFails of [false, true]) {
        const outputDir = tempDirs.make(`afora-docker-pack-${failure}-`);
        ...
        try {
          const packPromise = packAforaPackageForDocker("/repo", outputDir, {
            ...skipDocsMapLifecycle,
  ...
  it("rejects path-like npm pack stdout before resolving Docker package tarballs", async () => {
    for (const filename of [
      "../afora-2026.6.17.tgz",
      "/tmp/afora-2026.6.17.tgz",
      String.raw`C:\temp\afora-2026.6.17.tgz`,
      "afora-nested/evil.tgz",
      String.raw`afora-nested\evil.tgz`,
      "afora-C:evil.tgz",
    ]) {
      await expect(
        packAforaPackageForDocker("/repo", "/out", {
      ...
      ).rejects.toThrow("npm pack reported unsafe Afora tarball filename");
    }
  });

  it("ignores unsafe output directory tarball names when npm stdout is not usable", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-pack-"));
    try {
      if (process.platform === "win32") {
        const nestedDir = path.join(outputDir, "afora-nested");
        fs.mkdirSync(nestedDir);
        fs.writeFileSync(path.join(nestedDir, "evil.tgz"), "");
      } else {
        fs.writeFileSync(path.join(outputDir, "afora-C:evil.tgz"), "");
        fs.writeFileSync(path.join(outputDir, String.raw`afora-nested\evil.tgz`), "");
      }
      await expect(
        packAforaPackageForDocker("/repo", outputDir, {
          ...skipDocsMapLifecycle,
      ...
      ).rejects.toThrow("missing packed Afora tarball");
      ...
      await expect(
        packAforaPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "afora-2026.6.17.tgz"), "");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "afora-2026.6.17.tgz"));
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
    }
  });
  ...
  it("ignores stale package tarballs before fallback scanning npm output", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-docker-pack-stale-"));
    try {
      fs.writeFileSync(path.join(outputDir, "afora-9999.1.1.tgz"), "stale");
      ...
      await expect(
        packAforaPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "afora-2026.6.17.tgz"), "current");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "afora-2026.6.17.tgz"));

      expect(fs.existsSync(path.join(outputDir, "afora-9999.1.1.tgz"))).toBe(false);
      expect(fs.readFileSync(path.join(outputDir, "afora-2026.6.17.tgz"), "utf8")).toBe(
        "current",
      );
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-package-timeout-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
  ...
  it("clamps oversized kill grace before scheduling", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-package-grace-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("writes exact stdout bytes to a file and rejects capture conflicts", async () => {
    const tempDir = tempDirs.make("afora-package-stdout-file-");
  ...
  it("restores source artifacts before exiting after receipt-read termination", async () => {
    ...
    const tempDir = tempDirs.make("afora-package-receipt-signal-");
    const markerPath = path.join(tempDir, "restored");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-afora-for-docker.mts")).href;
    const runnerScript = `
import fs from "node:fs";
...
const { packAforaPackageForDocker } = await import(${JSON.stringify(scriptUrl)});
try {
  await packAforaPackageForDocker("/repo", ${JSON.stringify(tempDir)}, { packJsonPath: "result.json", prepareBundledAiRuntime: async () => async () => {}, prepareChangelog: async () => {}, prepareDocsMap: async () => {}, prepareManifest: async () => {}, restoreChangelog: async () => {}, restoreDocsMap: async () => { fs.writeFileSync(${JSON.stringify(markerPath)}, "done"); }, restoreManifest: async () => {}, runCaptureImpl: async (_command, _args, _cwd, options) => { fs.writeFileSync(options.stdoutFilePath, '[{"filename":"afora-2026.5.28.tgz"}]'); fs.writeFileSync(${JSON.stringify(path.join(tempDir, "afora-2026.5.28.tgz"))}, "package"); return ""; } });
...
`;
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-afora-for-docker.mts")).href;
    let childPid = 0;
    let runnerPid;
