...
async function runVoiceCallProof(options: ProducerOptions): Promise<string> {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-voice-call-gateway-"));
  ...
  try {
    gateway = await startQaGatewayChild({
      ...
      runtimeEnvPatch: {
        AFORA_QA_VOICE_BRIDGE_CALLS_PATH: fixture.bridgeCallsPath,
        AFORA_QA_VOICE_TOOL_RESULTS_PATH: fixture.toolResultsPath,
      },
      mutateConfig: (config) =>
    ...
    if (!bridgeCalls.some((call) => call.tools?.includes("afora_agent_consult"))) {
      throw new Error(
