...
type GatewayToken = NonNullable<NonNullable<AforaConfig["gateway"]>["auth"]>["token"];
...
function localGatewayConfig(token?: GatewayToken): AforaConfig {
  return {
    gateway: {
      mode: "local",
      port: instance?.port,
...
describe.skipIf(process.platform === "win32")("doctor auth and SecretRef product proof", () => {
  it(
    ...
    async () => {
      ...
      instance.env.AFORA_GATEWAY_TOKEN = "qa-ambient-token-must-not-win";
      const unresolvedRef = {
        source: "env" as const,
        provider: "default",
      ...
      delete instance.env.AFORA_GATEWAY_TOKEN;
