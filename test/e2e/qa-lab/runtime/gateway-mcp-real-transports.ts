...
async function createFixturePlugin() {
  // afora-temp-dir: allow standalone producer cleans this root in each scenario finally block
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-mcp-fixture-"));
...
function withFixturePlugin(config: AforaConfig, pluginDir: string): AforaConfig {
    ...
    return {
      args: [
        ...
        [
          ...
          `.then((module) => module.serveAforaChannelMcp({`,
          `gatewayUrl: process.env.AFORA_QA_GATEWAY_URL,`,
          `gatewayToken: process.env.AFORA_QA_GATEWAY_TOKEN,`,
      ...
      cwd: params.repoRoot,
      envPatch: {
        AFORA_QA_GATEWAY_TOKEN: params.gatewayToken,
        AFORA_QA_GATEWAY_URL: params.gatewayUrl,
      },
    };
  }

  throw new Error(
    "Afora channel MCP entry not found: expected dist/index.(m)js or src/mcp/channel-server.ts",
  );
}
...
async function connectChannelMcpClient(params: {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      AFORA_STATE_DIR: tempState.stateDir,
    },
    stderr: "pipe",
  });
...
async function writePluginToolsConfig(root: string, pluginDir: string) {
  ...
  const config = withFixturePlugin({} as AforaConfig, pluginDir);
...
async function runMcpPluginToolsProof(options: ProducerOptions): Promise<string> {
  const fixture = await createFixturePlugin();
  // afora-temp-dir: allow standalone producer cleans and verifies this root in its finally block
  const runtimeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-plugin-tools-mcp-"));
