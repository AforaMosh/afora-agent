...
const QA_DOCKER_E2E_LANES = {
  ...
  "update-migration": {
    env: (env) => ({
      AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "afora@2026.4.23",
      AFORA_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      AFORA_UPGRADE_SURVIVOR_SCENARIO:
        env.AFORA_UPGRADE_SURVIVOR_SCENARIO ?? "plugin-deps-cleanup",
    }),
  ...
  "update-restart-auth": {
    env: (env) => ({
      AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "afora@latest",
      AFORA_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT:
        env.AFORA_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT ?? "1500s",
      AFORA_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      AFORA_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE: "auto-auth",
    }),
    script: "scripts/e2e/upgrade-survivor-docker.sh",
  },
  "upgrade-survivor": {
