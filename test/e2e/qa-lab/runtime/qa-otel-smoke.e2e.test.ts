...
describe("qa-otel-smoke receiver bounds", () => {
  ...
  beforeAll(() => {
    configuredBodyLimitLoad = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          AFORA_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES: "1024",
          AFORA_QA_OTEL_MAX_COMPRESSED_BODY_BYTES: "2048",
          AFORA_QA_OTEL_MAX_DECODED_BODY_BYTES: "4096",
        },
      },
    );
  });
  ...
  function makePassingSmokeAssertionInput(): Parameters<typeof testing.assertSmoke>[0] {
    return {
      ...
      metrics: [{ name: "afora.harness.duration_ms" }],
      requests: [
        {
          path: "/v1/traces",
          signal: "traces",
      ...
      spans: [
        {
          ...
          parent: false,
          attributes: {
            "afora.error": "QA OTEL provider stream failed OPENAI_API_KEY=***",
          },
        },
        {
          name: "afora.harness.run",
          parent: true,
          attributes: {
            "afora.error": "QA OTEL provider stream failed OPENAI_API_KEY=***",
          },
        },
        { name: "afora.context.assembled", parent: true, attributes: {} },
        { name: "afora.message.delivery", parent: true, attributes: {} },
        {
  ...
  it("ignores inherited OTEL exporter endpoints during direct producer execution", () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "afora-qa-otel-env-isolation-"));
    try {
    ...
    input.stdoutLogRecords = [
      {
        ...
        "service.name": "afora-qa-lab-otel-smoke",
        severityText: "INFO",
        severityNumber: 9,
        body: "log",
        attributes: {
          "afora.log.level": "INFO",
        },
      },
    ];
  ...
  it("moves Docker collector telemetry off the default host port", async () => {
    ...
    const collector = await testing.startDockerOtelCollector(4317, {
      mkdtemp: async () => "/tmp/afora-otel-collector-test",
      platform: "linux",
    ...
    expect(stopDockerContainer).toHaveBeenCalledWith(
      "afora-otel-smoke-00000000-0000-4000-8000-000000000000",
    );
    expect(removePath).toHaveBeenCalledWith("/tmp/afora-otel-collector-test", {
      force: true,
      recursive: true,
    });
  });
  ...
  it("cleans Docker collector containers and temp config after readiness failures", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "afora-qa-otel-collector-"));
  ...
  it("reports bounded Docker collector output when readiness exits", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "afora-qa-otel-collector-output-"));
    const collectorDir = path.join(tempRoot, "collector");
    const child = new EventEmitter() as EventEmitter & {
      stderr: EventEmitter;
      stdout: EventEmitter;
