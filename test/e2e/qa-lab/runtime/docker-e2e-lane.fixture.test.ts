...
describe("QA Docker E2E lane fixture", () => {
  ...
  it("resolves lane-specific environment overlays at run time", () => {
    ...
    const updateMigration = resolveQaDockerE2eLane("update-migration", {
      AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC: "afora@custom",
      AFORA_UPGRADE_SURVIVOR_SCENARIO: "custom-scenario",
    });

    expect(updateMigration.script).toBe("scripts/e2e/upgrade-survivor-docker.sh");
    expect(updateMigration.env.AFORA_UPGRADE_SURVIVOR_PUBLISHED_BASELINE).toBe("1");
    expect(updateMigration.env.AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC).toBe("afora@custom");
    expect(updateMigration.env.AFORA_UPGRADE_SURVIVOR_SCENARIO).toBe("custom-scenario");

    const updateRestartAuth = resolveQaDockerE2eLane("update-restart-auth", {});
    expect(updateRestartAuth.env.AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC).toBe("afora@latest");
    expect(updateRestartAuth.env.AFORA_UPGRADE_SURVIVOR_PUBLISHED_BASELINE).toBe("1");
    expect(updateRestartAuth.env.AFORA_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE).toBe("auto-auth");
    expect(updateRestartAuth.env.AFORA_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT).toBe("1500s");

    expect(
      resolveQaDockerE2eLane("update-restart-auth", {
        AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC: "afora@custom",
      }).env.AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC,
    ).toBe("afora@custom");
  });
