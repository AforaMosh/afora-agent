...
import {
  ...
  buildAforaReleaseClawHubRuntimeState,
  parseAforaReleaseClawHubPlanArgs,
} from "../scripts/lib/afora-release-clawhub-plan.ts";
import {
...
function createClawPackBytes(
  ...
  const packageJson = JSON.stringify({
    name: packageName,
    version,
    afora: { release: { publishToClawHub: true } },
  });
  ...
  return gzipSync(
    Buffer.concat([
      ...packageJsonEntries,
      entry("package/afora.plugin.json", JSON.stringify({ id: "demo-plugin" })),
      Buffer.alloc(1024),
    ]),
  );
}
...
describe("Afora dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "cohere",
      packageName: "@afora/cohere-provider",
      install: {
    ...
    },
    {
      ...
      install: {
        clawhubSpec: "clawhub:@afora/diagnostics-prometheus",
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: "@afora/diagnostics-prometheus",
      },
    },
    {
      extensionId: "gmi",
      packageName: "@afora/gmi-provider",
      install: {
    ...
    {
      extensionId: "novita",
      packageName: "@afora/novita-provider",
      install: {
        clawhubSpec: "clawhub:@afora/novita-provider",
        defaultChoice: "npm",
        minHostVersion: ">=2026.7.2",
        npmSpec: "@afora/novita-provider",
      },
    },
  ] as const;
  ...
  it("keeps dual-published plugins selectable through both ClawHub and npm release paths", () => {
    ...
    for (const plugin of dualPublishedPlugins) {
      ...
      expect(packageJson.afora?.install).toEqual(plugin.install);
      expect(packageJson.afora?.release).toEqual({
        publishToClawHub: true,
        publishToNpm: true,
      });
    }
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@afora/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
      ...
      JSON.stringify(
        {
          ...
          afora: {
            ...
            install: {
              npmSpec: "@afora/demo-plugin",
            },
            build: {
              aforaVersion: "2026.4.1",
            },
            release: {
              publishToClawHub: true,
            },
...
describe("collectPluginClawHubReleasePlan", () => {
  ...
  it("rejects stale required dependencies before querying ClawHub", async () => {
    ...
    await expect(
      ...
      '@afora/demo-plugin@2026.4.1: demo-runtime must match npm latest for release; found "1.2.3", latest is "1.2.4".',
    );
  });
  ...
  it("accepts required dependencies matching npm latest", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@afora/demo-plugin@2026.4.1": 404,
      },
    });
    ...
    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@afora/demo-plugin"]);
  });
  ...
  it("fails closed when npm latest cannot be resolved", async () => {
    ...
    await expect(
      ...
      "@afora/demo-plugin@2026.4.1: could not resolve npm latest for demo-runtime: registry unavailable",
    );
  });
  ...
  it("keeps existing trusted packages with missing versions as normal candidates", async () => {
    ...
    expect(requests).toEqual([
      "/api/v1/packages/%40afora%2Fdemo-plugin",
      "/api/v1/packages/%40afora%2Fdemo-plugin/trusted-publisher",
      "/api/v1/packages/%40afora%2Fdemo-plugin/versions/2026.4.1",
    ]);
  });
  ...
  it("cancels unused ClawHub package and version response bodies", async () => {
    ...
    const fetchImpl: typeof fetch = async (input) => {
      ...
      if (url.pathname === "/api/v1/packages/%40afora%2Fdemo-plugin") {
        return new Response(
      ...
      if (url.pathname === "/api/v1/packages/%40afora%2Fdemo-plugin/trusted-publisher") {
        return new Response(
          JSON.stringify({
            trustedPublisher: {
              repository: "AforaMosh/afora-agent",
      ...
      if (url.pathname === "/api/v1/packages/%40afora%2Fdemo-plugin/versions/2026.4.1") {
        return new Response(
          new ReadableStream<Uint8Array>({
            cancel() {
  ...
  it("retries a rate-limited trusted publisher lookup", async () => {
    ...
    const fetchImpl: typeof fetch = async (input) => {
      ...
      if (pathname === "/api/v1/packages/%40afora%2Fdemo-plugin") {
        return new Response("{}", { status: 200 });
      }
      if (pathname === "/api/v1/packages/%40afora%2Fdemo-plugin/trusted-publisher") {
      ...
      if (pathname === "/api/v1/packages/%40afora%2Fdemo-plugin/versions/2026.4.1") {
  ...
  it("preserves ClawHub response details after package retries are exhausted", async () => {
    ...
    await expect(
      ...
      "Failed to query ClawHub package @afora/demo-plugin: 503 Rate limit temporarily unavailable [retry-after=1; x-request-id=request-123]",
    );
  ...
  it.each([
    ...
    async ({ responseBody, expectedDetail }) => {
      ...
      await expect(
        ...
        `Failed to query ClawHub package @afora/demo-plugin: 503 ${expectedDetail}`,
      );
    },
  );
  ...
  it("routes missing package rows to bootstrap candidates instead of normal candidates", async () => {
    ...
    expect(plan.bootstrapCandidates[0]).toMatchObject({
      ...
      artifactName: "clawhub-package-afora-demo-plugin-2026.4.1",
      packageName: "@afora/demo-plugin",
      version: "2026.4.1",
    });
    expect(plan.missingTrustedPublisher).toStrictEqual([]);
  ...
  it("skips versions that already exist on ClawHub", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@afora/demo-plugin@2026.4.1": 200,
      },
    });
  ...
  it("plans selected packages without validating unrelated publishable packages", async () => {
    ...
    expect(plan.candidates.map((plugin) => plugin.artifactName)).toEqual([
      "clawhub-package-afora-demo-plugin-2026.4.1",
    ]);
  });
});

describe("buildAforaReleaseClawHubPlan", () => {
  it("emits a dispatch plan that keeps ClawHub children on the release tag", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@afora/demo-plugin@2026.4.1": 404,
        "@afora/demo-three@2026.4.1": 404,
      },
    });

    const plan = await buildAforaReleaseClawHubPlan(
      {
    ...
    expect(plan.normal).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@afora/demo-plugin"],
      inputs: {
        publish_scope: "selected",
        plugins: "@afora/demo-plugin",
    ...
    expect(plan.bootstrap).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@afora/demo-two", "@afora/demo-three"],
      inputs: {
        bootstrap_workflow_sha: "d".repeat(40),
        ref: "a".repeat(40),
        release_tag: "v2026.4.1-beta.1",
        plugins: "@afora/demo-two,@afora/demo-three",
    ...
    expect(plan.summary).toEqual({
      ...
      normalPlugins: "@afora/demo-plugin",
      bootstrapPlugins: "@afora/demo-two,@afora/demo-three",
      missingTrustedPlugins: "@afora/demo-three",
    });
  ...
  it("routes already-published packages missing trusted publisher config to bootstrap repair", async () => {
    ...
    expect(plan.summary).toMatchObject({
      ...
      bootstrapCount: 1,
      missingTrustedPublisherCount: 1,
      bootstrapPlugins: "@afora/demo-plugin",
      missingTrustedPlugins: "@afora/demo-plugin",
    });
  });
  ...
  it("requires an exact lowercase release SHA for bootstrap targeting", () => {
    ...
    expect(() => parseAforaReleaseClawHubPlanArgs(baseArgs)).toThrow(
      "--release-sha is required.",
    );
    expect(() =>
      parseAforaReleaseClawHubPlanArgs([...baseArgs, "--release-sha", "ABCDEF"]),
  ...
  it("requires an exact parent release run attempt for bootstrap approval binding", () => {
    ...
    expect(() => parseAforaReleaseClawHubPlanArgs(args)).toThrow(
      "--release-publish-run-attempt is required.",
    );
    expect(() =>
      parseAforaReleaseClawHubPlanArgs([...args, "--release-publish-run-attempt", "0"]),
...
describe("runPluginClawHubReleaseCheck", () => {
  it("rejects stale required dependencies", async () => {
    ...
    await expect(
      runPluginClawHubReleaseCheck(["--plugins", "@afora/demo-plugin"], {
        rootDir: repoDir,
        resolveLatestVersion: () => "1.2.4",
      }),
    ).rejects.toThrow(
...
describe("buildAforaReleaseClawHubRuntimeState", () => {
  it("includes the normal ClawHub run in verifier args when the release waits for it", () => {
    const state = buildAforaReleaseClawHubRuntimeState({
      repository: "AforaMosh/afora-agent",
      waitForClawHub: true,
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: https://github.com/AforaMosh/afora-agent/actions/runs/111",
    );
  ...
  it("includes a completed bootstrap run even when there is no normal ClawHub run", () => {
    ...
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: https://github.com/AforaMosh/afora-agent/actions/runs/222",
    );
  });
  ...
  it("skips ClawHub verification for non-awaited incomplete runs while keeping proof links", () => {
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: dispatched separately, not awaited by this proof: https://github.com/AforaMosh/afora-agent/actions/runs/111",
    );
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: dispatched separately, not awaited by this proof: https://github.com/AforaMosh/afora-agent/actions/runs/222",
    );
  });
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  pack_path="$pack_destination/afora-demo-plugin-2026.4.1.tgz"
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@afora/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
...
`,
    ...
    execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          AFORA_CLAWHUB_MANUAL_OVERRIDE_REASON:
...
`,
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          AFORA_CLAWHUB_PACK_OUTPUT_DIR: outputDir,
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
      },
    );
    ...
    expect(existsSync(join(outputDir, "afora-demo-plugin-2026.4.1.tgz"))).toBe(true);
    const invocations = readFileSync(markerPath, "utf8");
  ...
  it("rejects duplicate normalized paths before invoking the ClawHub CLI", () => {
    ...
    const tgzBytes = createClawPackBytes("@afora/demo-plugin", "2026.4.1", {
      ...
      execFileSync(
        ...
        [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--validate-packed", tgzPath],
        {
          ...
          env: {
            ...
            EXPECTED_CLAWHUB_PACKAGE_NAME: "@afora/demo-plugin",
...
if [[ " $* " == *" --dry-run "* ]]; then
  printf '{"name":"@afora/demo-plugin","version":"2026.4.1"}\\n'
  exit 0
fi
attempts=0
    ...
    execFileSync(
      ...
      [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--publish-packed", tgzPath],
      {
        ...
        env: {
          ...
          EXPECTED_CLAWHUB_PACKAGE_NAME: "@afora/demo-plugin",
          EXPECTED_CLAWHUB_PACKAGE_VERSION: "2026.4.1",
          AFORA_CLAWHUB_PUBLISH_ATTEMPTS: "2",
          AFORA_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS: "1",
          PACKAGE_DIR: "extensions/demo-plugin",
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
      },
    ...
    execFileSync(
      ...
      [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--publish-packed", tgzPath],
      {
        ...
        env: {
          ...
          EXPECTED_CLAWHUB_PACKAGE_VERSION: "2026.4.1",
          AFORA_CLAWHUB_PUBLISH_ATTEMPTS: "2",
          AFORA_CLAWHUB_PUBLISH_ATTEMPT_TIMEOUT_SECONDS: "1",
          AFORA_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS: "1",
          PACKAGE_DIR: "extensions/demo-plugin",
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
...
function createTempPluginRepo(
  ...
  const repoDir = makeTempRepoRoot(tempDirs, "afora-clawhub-release-");
  const extensionId = options.extensionId ?? "demo-plugin";
  const extensionIds = [extensionId, ...(options.extraExtensionIds ?? [])];

  writeFileSync(
    join(repoDir, "package.json"),
    JSON.stringify({ name: "afora-test-root", type: "module" }, null, 2),
  );
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    if (options.includeClawHubContract === false) {
      ...
      const manifest = JSON.parse(readFileSync(manifestPath, "utf8")) as {
        afora?: { build?: unknown; compat?: unknown };
      };
      if (manifest.afora) {
        delete manifest.afora.compat;
        delete manifest.afora.build;
      }
