...
const ensureSupportedRuntimeVersion = () => {
  if (process.versions.bun) {
    ...
    // rejecting Bun outright so capable Bun builds can run Afora.
    let hasNodeSqlite;
    try {
    ...
    process.stderr.write(
      "afora: this Bun runtime is unsupported because it does not provide node:sqlite.\n" +
        `Use Node.js ${SUPPORTED_NODE_RANGE}; Bun remains supported for installs and package scripts.\n`,
    );
    process.exit(1);
  }
  if (isSupportedAforaNodeVersion(process.versions.node)) {
    return;
  }

  process.stderr.write(
    `afora: Node.js ${SUPPORTED_NODE_RANGE} is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[afora] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.AFORA_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    AFORA_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["afora: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:AforaMosh/afora-agent#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g afora@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.AFORA_CONTAINER)) {
    return true;
  }
...
const isHelpFastPathDisabled = () =>
  process.env.AFORA_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.AFORA_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.AFORA_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.AFORA_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "afora.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".afora", "afora.json"),
    path.join(homeDir, ".afora", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "afora.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.AFORA_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.AFORA_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `Afora ${version} (${commit})\n` : `Afora ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.AFORA_BUNDLED_VERSION) ?? "0.0.0";
}
