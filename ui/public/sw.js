// Afora Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "afora-control-";
const EMBEDDED_CACHE_VERSION = "__AFORA_CONTROL_UI_BUILD_ID__";
const URL_CACHE_VERSION = new URL(self.location.href).searchParams
  .get("v")
  ?.replace(/[^a-zA-Z0-9._-]/g, "-");
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__AFORA_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "Afora", body: event.data.text() };
  }

  const title = data.title || "Afora";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "afora-notification",
    data: {
