...
export function buildMemoryEngineOptions(
    ...
    .map((plugin) => ({
      ...
      label:
        plugin.id === DEFAULT_MEMORY_ENGINE_ID
          ? t("memoryPage.engine.aforaMemory")
          : plugin.name,
      available: true,
    }))
    .toSorted((left, right) => {
  ...
  if (selected && !options.some((option) => option.id === selected)) {
    const unavailable = {
      id: selected,
      label:
        selected === DEFAULT_MEMORY_ENGINE_ID ? t("memoryPage.engine.aforaMemory") : selected,
      available: false,
    };
...
const MEMORY_DOCS_URL = "https://docs.afora.ai/concepts/memory";
