...
function visibleTab(element: HTMLElement): "overview" | "memories" | "dreams" | "settings" | null {
  ...
  if (panel.querySelector("afora-memory-dreaming")) {
    return "dreams";
  }
  if (panel.querySelector("afora-memory-memories")) {
    return "memories";
  }
...
describe("MemorySettingsPage tab routing", () => {
  ...
  it("loads Overview status once per activation, header agent change, and reconnect", async () => {
    ...
    try {
      ...
      expect(element.querySelectorAll("afora-agent-select")).toHaveLength(1);
      expect(element.textContent).not.toContain("Agent view");
      const select = element.querySelector(
        ".hub-page-header__actions afora-agent-select",
      ) as HTMLElement & {
...
describe("MemorySettingsPage dreaming support", () => {
  it("links the dreaming intro to its guide", async () => {
    ...
    try {
      await waitForFast(() =>
        expect(
          element.querySelector(
            '.settings-page__intro a[href="https://docs.afora.ai/concepts/dreaming"]',
          ),
        ).not.toBeNull(),
      );
      ...
      expect(link?.href).toBe("https://docs.afora.ai/concepts/dreaming");
    } finally {
      element.remove();
    }
  });
