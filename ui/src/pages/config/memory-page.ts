...
const DREAMING_DOCS_URL = "https://docs.afora.ai/concepts/dreaming";
...
class MemorySettingsPage extends AforaLightDomElement {
  ...
  override render() {
    ...
    return renderMemory({
      ...
      memories: html`
        ...
        ></afora-memory-memories>
      `,
      dreams: html` <afora-memory-dreaming .agentId=${agentId}></afora-memory-dreaming> `,
      editor:
...
if (!customElements.get("afora-memory-settings")) {
  customElements.define("afora-memory-settings", MemorySettingsPage);
}

export function renderMemoryPage(props: MemoryPageProps) {
  return html`
    <afora-memory-settings
