  ...
  unavailableCandidates: [
    {
      ...
      reason: "This local runtime must be configured outside Afora.",
    },
  ],
  manualProviders: [
    {
...
describe("renderModelSetup", () => {
  ...
  it("renders candidate, unavailable, sign-in, and manual sections", () => {
    ...
    expect(text(container)).toContain("This local runtime must be configured outside Afora");
  ...
  it("shows verified connections in an actionable success dialog", () => {
    ...
    const dialog = container.querySelector('afora-modal-dialog[label="Connection verified"]');
    expect(dialog).not.toBeNull();
    expect(text(dialog!)).toContain(
      "Afora received a real reply from openai/gpt-5.6-sol. You can start chatting now.",
    );
  ...
  it("renders admin and older-gateway gates without actions", () => {
    ...
    expect(text(old)).toContain("The Gateway is running an older Afora version");
