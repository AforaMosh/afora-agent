...
describe("dreaming controller", () => {
  ...
  it("reads dreaming enabled state from the selected memory slot plugin", () => {
    expect(
      ...
      pluginId: "memos-local-afora-plugin",
      enabled: true,
      overridden: true,
      engineOff: false,
    });
  ...
  it("repairs dreaming artifacts and reloads only dreaming status", async () => {
    ...
    request.mockImplementation(async (method: string) => {
      if (method === "doctor.memory.repairDreamingArtifacts") {
        return {
          ...
          changed: true,
          archiveDir: "/tmp/afora/.afora-repair/dreaming/2026-04-11T22-10-00-000Z",
    ...
    expect(state.dreamDiaryActionMessage).toEqual({
      kind: "success",
      text: "Dream cache repair complete: archived session corpus, archived ingestion state. Archive: /tmp/afora/.afora-repair/dreaming/2026-04-11T22-10-00-000Z",
    });
    expect(state.dreamDiaryActionArchivePath).toBe(
      "/tmp/afora/.afora-repair/dreaming/2026-04-11T22-10-00-000Z",
    );
    expect(state.dreamDiaryActionLoading).toBe(false);
  });
  ...
  it("reports when the dreaming repair archive path cannot be copied", async () => {
    const { state } = createState();
    state.dreamDiaryActionArchivePath = "/tmp/afora/archive";
