  ...
  it.each([
    {
      ...
      input: {
        gatewayUrl: "wss://gateway.example/afora",
    ...
    expect(result.command).toMatch(/^afora resume --handoff [A-Za-z0-9_-]+$/u);
    const encoded = result.command.slice("afora resume --handoff ".length);
