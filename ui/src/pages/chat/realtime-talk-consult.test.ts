...
describe("RealtimeTalkSession consult handoff", () => {
  ...
  it("submits the Gateway-recorded source reply after an empty final", async () => {
    let listener: ((event: { event: string; payload?: unknown }) => void) | undefined;
    const request = vi.fn(async (method: string) => {
      if (method === "talk.client.toolCall") {
        setImmediate(() => {
      ...
      if (method === "agent.wait") {
        return { runId: "run-1", status: "ok", resultText: "The recorded source reply wins." };
      }
      throw new Error(`unexpected request: ${method}`);
    });
    const addEventListener = vi.fn((callback: typeof listener) => {
    ...
    expect(submit).toHaveBeenCalledWith("call-1", {
      result: "The recorded source reply wins.",
    });
  });
