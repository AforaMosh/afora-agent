...
function handleLargeTextPaste(e: ClipboardEvent, props: ChatAttachmentControlsProps): boolean {
  ...
  const reservation = reserveAttachmentFiles([file], props);
  if (!reservation) {
    return true;
  }
  try {
...
function dataImageClipboardFile(
  ...
  const base64 = canonicalizeBase64(base64Source);
  if (!base64) {
    return null;
  }
  try {
...
async function appendAttachmentFiles(files: readonly File[], props: ChatAttachmentControlsProps) {
  ...
  const reservation = reserveAttachmentFiles(files, props);
  if (!reservation) {
    return;
  }
...
export function handleChatAttachmentPaste(e: ClipboardEvent, props: ChatAttachmentControlsProps) {
  ...
  if (imageFiles.length === 0) {
    ...
    e.preventDefault();
    const reservation = reserveAttachmentFiles([pasted.file], props);
    if (!reservation) {
      return;
    }
    try {
