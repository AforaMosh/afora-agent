...
function isManagedCanvasDocumentPreview(preview: ToolPreview): boolean {
  ...
  try {
    ...
    const prefix = "/__afora__/canvas/documents/";
...
const WIDGET_SIZE_MESSAGE_TYPE = "afora:widget-size";
const WIDGET_PROMPT_OFFER_MESSAGE_TYPE = "afora:widget-prompt-offer";
const WIDGET_PROMPT_MESSAGE_TYPE = "afora:widget-prompt";
const WIDGET_PROMPT_HOST_READY_MESSAGE_TYPE = "afora:widget-prompt-host-ready";
...
function renderMcpAppView(params: {
  ...
  void ensureCustomElementDefined("mcp-app-view", loadMcpAppView).catch((error: unknown) => {
    console.error("[afora] failed to load MCP App view", error);
  });
