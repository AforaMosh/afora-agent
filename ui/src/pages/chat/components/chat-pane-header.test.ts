...
afterEach(() => {
  ...
  Reflect.deleteProperty(window, "__AFORA_NATIVE_WEB_CHROME__");
});
...
function mount(patch: Partial<ChatPaneHeaderProps> = {}) {
  ...
  const props: ChatPaneHeaderProps = {
    ...
    workspaceRoot: "/repo/afora",
    workspaceLabel: "afora",
    workspaceIcon: null,
    parentSession: null,
...
describe("chat pane header", () => {
  it("hides the gateway picker without capability and with one gateway", () => {
    Object.assign(window, { __AFORA_NATIVE_WEB_CHROME__: true });
  ...
  it("re-renders gateway rows from a changed snapshot property", () => {
    ...
    current = next;
    window.dispatchEvent(new CustomEvent("afora:native-gateways-changed", { detail: next }));
  ...
  it("renders an editable title and workspace chip", () => {
    ...
    expect(chip?.textContent?.trim()).toContain("afora");
    title?.click();
  ...
  it("renders the permanent owner chip only when attribution chrome is enabled", () => {
    ...
    expect(shown.container.querySelector("afora-session-owner-chip")).not.toBeNull();

    const dormant = mount({
      showOwnerChip: false,
      session: row({ createdActor: { type: "human", id: "profile-ada", label: "Ada" } }),
    });
    expect(dormant.container.querySelector("afora-session-owner-chip")).toBeNull();
  });
  ...
  it("renders the bounded static participant facepile beside the owner", async () => {
    ...
    >("afora-viewer-facepile.chat-pane__participants");
    await facepile?.updateComplete;

    expect(mounted.container.querySelector("afora-session-owner-chip")).not.toBeNull();
    expect(
  ...
  it("renders the durable session actor avatar with the header attribution semantics", async () => {
    ...
    await vi.waitFor(() => {
      expect(mounted.container.querySelector("afora-session-owner-chip img")).not.toBeNull();
    });
  ...
  it("hides one branch and lists multiple branches with the active tip marked", () => {
    ...
    // wrapper (like afora-tooltip) has a zero rect and pins the menu to the
...
describe("chat pane workspace chip icon", () => {
  async function mountChip(workspaceIcon: ChatPaneHeaderProps["workspaceIcon"]) {
    ...
    const element = container.querySelector("afora-workspace-icon") as
      | (HTMLElement & { updateComplete?: Promise<unknown> })
      | null;
  ...
  it("keeps the folder glyph while credentials are not ready", async () => {
    ...
    const { container, element } = await mountChip({
      routeUrl: "/__afora__/workspace-icon/agent%3Amain%3Aone",
      authTokens: [],
      authReady: false,
    });
  ...
  it("keeps the folder glyph when the icon route fails", async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      "/__afora__/workspace-icon/agent%3Amain%3Aone",
  ...
  it("does not refetch a missing project icon when the header rerenders", async () => {
    ...
    const element = mounted.container.querySelector("afora-workspace-icon") as
      | (HTMLElement & { updateComplete?: Promise<unknown> })
      | null;
...
describe("chat pane workspace resolution", () => {
    ...
    expect(
      resolveChatPaneWorkspace({
        session: row({
          ...
          worktree: { id: "wt-1", branch: "title-bar", repoRoot: "/src/afora" },
        }),
      }),
    ).toEqual({ root: "/tmp/worktrees/title-bar", label: "afora" });
  });
  ...
  it("does not substitute the agent workspace for a missing worktree checkout", () => {
    expect(
      resolveChatPaneWorkspace({
        session: row({
          worktree: { id: "wt-missing", branch: "feature", repoRoot: "/src/afora" },
        }),
        agentWorkspace: "/src/default-agent-workspace",
        worktreePath: null,
      }),
    ).toEqual({ root: null, label: "afora" });
  });
  ...
  it("matches the gateway root order: spawned workspace before spawned cwd", () => {
    expect(
      resolveChatPaneWorkspace({
        session: row({
          spawnedWorkspaceDir: "/src/afora",
          spawnedCwd: "/src/afora/packages/nested",
        }),
      }),
    ).toEqual({ root: "/src/afora", label: "afora" });
    // execCwd is exec-node routing state; it never overrides local facts.
    expect(
      resolveChatPaneWorkspace({
        session: row({ execCwd: "/remote/stale", spawnedCwd: "/src/afora" }),
      }),
    ).toEqual({ root: "/src/afora", label: "afora" });
  });
  ...
  it("prefers exec cwd and falls back to the agent workspace", () => {
    ...
    expect(resolveChatPaneWorkspace({ session: row(), agentWorkspace: "/src/afora" })).toEqual({
      root: "/src/afora",
      label: "afora",
    });
  });
