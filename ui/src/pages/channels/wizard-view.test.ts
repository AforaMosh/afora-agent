...
describe("renderChannelWizard", () => {
  ...
  it("copies setup text through the plain-HTTP clipboard fallback", async () => {
    ...
    expect(copiedText).toBe("afora channels add");
    ...
    async (copied) => {
      ...
      const props = {
        wizard: {
          phase: "step",
          channel: null,
          step: { id: "copy-command", type: "note", message: "afora channels add" },
          stepIndex: 1,
          busy: false,
          validationError: null,
        },
      ...
      expect(writeText).toHaveBeenCalledWith("afora channels add");
