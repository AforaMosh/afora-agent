// Pairing overlay runtime: the wizard lifecycle, its pending-device counter,
// and the Gateway target it belongs to. Loaded only once an operator opens the
// dialog, so the wizard and its endpoint probe stay out of the startup graph.
import {
  createPairingWizard,
...
export function createPairingOverlayRuntime(params: {
