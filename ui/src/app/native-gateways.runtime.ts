...
type NativeGatewaysWindow = Window & {
  __AFORA_NATIVE_GATEWAYS__?: unknown;
  webkit?: {
    messageHandlers?: { aforaGateways?: { postMessage(message: NativeGatewaysMessage): void } };
  };
};

const NATIVE_GATEWAYS_CHANGED_EVENT = "afora:native-gateways-changed";
...
function createNativeGatewaysCapability(): NativeGatewaysCapability | null {
  ...
  const handler = nativeWindow.webkit?.messageHandlers?.aforaGateways;
  ...
  const postWithId = (type: "select" | "open-window" | "set-primary", id: string) =>
    post({ type, id });
  let snapshot = snapshotFrom(nativeWindow["__AFORA_NATIVE_GATEWAYS__"]);
