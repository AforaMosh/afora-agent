...
const EVENT = "afora:native-gateways-changed";
const snapshot = {
  gateways: [
    {
      id: "primary",
...
afterEach(() => {
  Reflect.deleteProperty(window, "__AFORA_NATIVE_GATEWAYS__");
  vi.unstubAllGlobals();
...
function installBridge() {
  ...
  vi.stubGlobal("webkit", { messageHandlers: { aforaGateways: { postMessage } } });
  return postMessage;
}
...
describe("native gateways", () => {
  ...
  it("initializes from the native global and posts actions", async () => {
    ...
    Object.assign(window, { __AFORA_NATIVE_GATEWAYS__: snapshot });
    const { nativeGatewaysCapability } = await import("./native-gateways.runtime.ts");
  ...
  it("reads the latest global when attached lazily, then publishes native updates", async () => {
    ...
    Object.assign(window, { __AFORA_NATIVE_GATEWAYS__: attachedSnapshot });
    const { nativeGatewaysCapability } = await import("./native-gateways.runtime.ts");
    const capability = nativeGatewaysCapability();
