...
describe("createApplicationGateway connection phase", () => {
  ...
  it("publishes the hello canvas URL synchronously and clears it on disconnect", () => {
    ...
    current().opts.onHello?.({
      ...HELLO,
      pluginSurfaceUrls: {
        canvas: "https://canvas.test/__afora__/cap/hello",
      },
    });

    expect(gateway.snapshot.canvasPluginSurfaceUrl).toBe(
      "https://canvas.test/__afora__/cap/hello",
    );
  ...
  it("does not let a superseded canvas refresh publish into the current snapshot", async () => {
    ...
    current().opts.onHello?.({
      ...HELLO,
      pluginSurfaceUrls: { canvas: "https://canvas.test/__afora__/cap/current" },
    });
    resolveRefresh({
      surface: "canvas",
      pluginSurfaceUrls: { canvas: "https://canvas.test/__afora__/cap/stale-refresh" },
    ...
    expect(gateway.snapshot.canvasPluginSurfaceUrl).toBe(
      "https://canvas.test/__afora__/cap/current",
    );
    gateway.stop();
  });
  ...
  it("does not copy selected-remote settings into an ephemeral document Gateway", () => {
    ...
    const selectionKey = `afora.control.currentGateway.v1:${pageGateway}`;
