...
function setControlUiBasePath(value: string | undefined) {
  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ? ({} as TestWindow)
        : ({ __AFORA_CONTROL_UI_BASE_PATH__: value } as unknown as TestWindow),
    );
    return;
  }
  if (value == null) {
    delete (window as TestWindow)["__AFORA_CONTROL_UI_BASE_PATH__"];
    return;
  }
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", () => {
    ...
    setControlUiBasePath(" /afora/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/afora"));
  });
  ...
  it("binds standalone documents to the page Gateway without persisting a selection", () => {
    setTestLocation({
      protocol: "https:",
      host: "gateway.example:8443",
      pathname: "/afora/approve/exec%3A1",
    });
    setControlUiBasePath("/afora");
    ...
    persistSessionToken(expectedGatewayUrl("/afora"), sessionCredential);
    ...
    expect(resolvePageGatewaySettings(remote)).toMatchObject({
      gatewayUrl: expectedGatewayUrl("/afora"),
  ...
  it("infers base path from nested pathname when configured base path is not set", () => {
    setTestLocation({
      ...
      pathname: "/apps/afora/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/afora"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", () => {
    ...
    sessionStorage.setItem("afora.control.token.v1", "legacy-session-token");
    const gatewayUrl = "wss://gateway.example:8443/afora";
    const scopedKey = `afora.control.settings.v1:${gatewayUrl}`;
    ...
    localStorage.setItem(
      "afora.control.currentGateway.v1:wss://gateway.example:8443",
      gatewayUrl,
    );
    ...
    expect(rewritten.sessionsByGateway).toEqual({
      "wss://gateway.example:8443/afora": {
        sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", () => {
    ...
    const scopedKey = `afora.control.settings.v1:${gwUrl}`;
  ...
  it("normalizes persisted text scale to the nearest supported stop", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    localStorage.setItem(
      `afora.control.settings.v1:${gwUrl}`,
      JSON.stringify({
        gatewayUrl: gwUrl,
        textScale: 123,
      }),
  ...
  it("omits the inherited text scale and removes an authored override on reset", () => {
    ...
    const scopedKey = `afora.control.settings.v1:${defaults.gatewayUrl}`;
  ...
  it("caps persisted session scopes to the most recent gateways", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `afora.control.settings.v1:wss://gateway.example:8443`;
  ...
  it("does not let a saved sibling base path override the current page gateway", () => {
    ...
    expect(localStorage.getItem("afora.control.settings.v1")).toBeNull();
  });
  ...
  it("loads local user identity separately from gateway settings", () => {
    ...
    expect(JSON.parse(localStorage.getItem("afora.control.user.v1") ?? "{}")).toEqual({
      name: "Buns",
      avatar: "🦞",
    });
  });
