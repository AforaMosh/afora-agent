...
const EXEC_APPROVAL_TAG = "afora-exec-approval";
...
export function ensureOptionalElementForHost(
  ...
  const load = ensureCustomElementDefined(element.tagName, element.loadModule)
    ...
    .catch((error: unknown) => {
      console.error(`[afora] failed to load ${element.label}`, error);
      throw error;
    })
    .finally(() => {
