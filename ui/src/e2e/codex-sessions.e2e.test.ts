...
const catalogGroupingStorageKey = "afora:sidebar:sessions:catalog-grouping";
...
suite.define(() => {
  ...
  it("groups sessions by host and hides empty offline nodes", async () => {
              ...
              hosts: [
                {
                  ...
                  sessions: [
                    ...
                    },
                    {
                      ...
                      cwd: "/Users/dev/afora/.claude/worktrees/fix-1",
                      status: "idle",
                      archived: false,
                      canContinue: true,
                      canArchive: true,
    ...
    try {
      ...
      const aforaProject = section.locator(
        '[data-session-catalog-project="/Users/dev/afora"]',
      );
      const aforaProjectItem = aforaProject.locator("..");
      const aforaProjectList = aforaProjectItem.locator(":scope > [role=list]");
      expect(await aforaProjectItem.getAttribute("role")).toBe("listitem");
      expect(await aforaProjectList.getAttribute("aria-label")).toBe("Local Codex: afora");
      expect(
        await aforaProject.locator(".sidebar-session-catalog-project__label").textContent(),
      ).toBe("afora");
      expect(
        await aforaProject.locator(".sidebar-session-catalog-project__count").textContent(),
      ).toBe("2");
      const projectRows = section.locator(".sidebar-recent-session--catalog-project-child");
      await expect.poll(() => projectRows.count()).toBe(3);
      expect(
        await aforaProjectList
          .locator(":scope > *")
          .evaluateAll((items) => items.map((item) => item.getAttribute("role"))),
      ).toEqual(["listitem", "listitem"]);
      ...
      const projectLabelTone = await aforaProject
      ...
      await aforaProject.click();
      await expect.poll(() => aforaProject.getAttribute("aria-expanded")).toBe("false");
      ...
      expect(await aforaProject.count()).toBe(1);
      expect(
        await aforaProject.locator(".sidebar-session-catalog-project__count").textContent(),
      ).toBe("2");
      expect(
      ...
      ).toContain("catalog-project:codex:gateway:local:/Users/dev/afora");

      await aforaProject.click();
      await expect.poll(() => aforaProject.getAttribute("aria-expanded")).toBe("true");
      ...
      ).not.toContain("catalog-project:codex:gateway:local:/Users/dev/afora");
  ...
  it("adopts from the native chat composer, navigates, and auto-sends", async () => {
    ...
    const catalogPane = page
      .locator("afora-chat-pane.chat-pane-cache__pane--visible")
