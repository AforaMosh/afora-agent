...
suite.define(() => {
  ...
  it("shows permanent owner chips and filters existing custom groups", async () => {
    ...
    await expect.poll(() => currentPage.locator("afora-session-owner-chip").count()).toBe(3);
  ...
  it("renders zero ownership chrome for a single creator", async () => {
    ...
    expect(await currentPage.locator("afora-session-owner-chip").count()).toBe(0);
  });
  ...
  it("scrolls high-volume sharing through one compact menu", async () => {
    ...
    await expectBrowser(
      dropdown.locator(".chat-pane__sharing-member afora-session-owner-chip"),
    ).toHaveCount(30);
