...
const CAPTURE_UI_PROOF = process.env.AFORA_CAPTURE_UI_PROOF === "1";
...
async function waitForApprovalPage(page: Page): Promise<void> {
  await page.locator("afora-approval-page").waitFor();
...
async function expectStandaloneApprovalPage(page: Page): Promise<void> {
  expect(await page.locator("afora-approval-page").count()).toBe(1);
  expect(await page.locator(".approval-page").count()).toBe(1);
  expect(await page.locator("afora-app-shell, .shell").count()).toBe(0);
  expect(await page.locator("afora-app-topbar, afora-app-sidebar").count()).toBe(0);
  expect(await page.locator("afora-exec-approval").count()).toBe(0);
}
...
suite.define(() => {
  ...
  it("keeps one canonical winner across conflicting surfaces and terminal reload", async () => {
    ...
    try {
      ...
      expect(await mobile.page.title()).toBe("Command approval — Afora");
      ...
      expect(await mobile.page.title()).toBe("Approved here — Afora");
      ...
      expect(await mobile.page.title()).toBe("Approved — Afora");
      expect(new URL(mobile.page.url()).pathname).toBe(approvalPath(""));
  ...
  it("preserves a mounted deep link across the authentication gate", async () => {
    ...
    await surface.page.locator("afora-login-gate").waitFor();
  ...
  it("uses the page Gateway without replacing a saved remote selection", async () => {
    ...
    const selectionKey = `afora.control.currentGateway.v1:${pageGatewayScope}`;
    const pageGateway = pageGatewayScope;
    const pageSettingsKey = `afora.control.settings.v1:${pageGateway}`;
    ...
    const remoteSettingsKey = `afora.control.settings.v1:${remoteGateway}`;
