...
suite.define(() => {
  ...
  it("keeps a rejected sidebar mutation visible until the user dismisses it", async () => {
    ...
    try {
      ...
      const dialog = page.locator('afora-modal-dialog[label="Rename session"]');
  ...
  it("shows a rejected Sessions-page custom group instead of leaking a page error", async () => {
    ...
    try {
      ...
      const error = page.locator('afora-modal-dialog [role="alert"]');
      await error.waitFor({ state: "visible" });
  ...
  it("renames, deletes, and toggles sidebar session groups", async () => {
    ...
    try {
      ...
      await expect
        .poll(() => page.locator("afora-modal-dialog input").inputValue())
        .toBe("Research");
      ...
      const deleteConfirm = page.locator("afora-modal-dialog");
      await expect
