...
function codingAgentSkill(missingAnyBins: string[]) {
  return {
    ...
    bundled: true,
    filePath: "/tmp/afora-e2e/skills/coding-agent/SKILL.md",
    baseDir: "/tmp/afora-e2e/skills/coding-agent",
  ...
  it("explains alternative missing binaries and installs one through the Gateway", async () => {
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        const dialog = page.locator("afora-modal-dialog", { hasText: "Coding Agent" });
