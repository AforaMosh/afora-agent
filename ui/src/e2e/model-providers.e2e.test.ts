...
const NOW = Date.now();
const recordVisuals = process.env.AFORA_UI_E2E_RECORD === "1";
...
describeControlUiE2e("Control UI Models mocked Gateway E2E", () => {
  ...
  it("defers live provider discovery until refresh while preserving model setup", async () => {
    ...
    const gateway = await installMockGateway(page, {
      featureMethods: ["chat.metadata", "chat.startup", "models.probe", "afora.setup.detect"],
      methodResponses: {
        "config.get": {
          config,
          sourceConfig: config,
    ...
    for (const colorScheme of ["light", "dark"] as const) {
      ...
      try {
        ...
        await expect
          ...
          .toBe("__afora_automatic_utility__");
  ...
  it("reloads the selected agent and clears a failed model draft after reconnect", async () => {
    ...
    try {
      await page.goto(`${server.baseUrl}settings/model-providers`);
      const agentPicker = page.locator(".agent-scope-control afora-agent-select");
