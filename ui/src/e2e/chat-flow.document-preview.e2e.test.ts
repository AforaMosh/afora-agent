...
suite.define(() => {
  it("previews a text document from chat history with a download action", async () => {
    ...
    await page.route("**/__afora__/pasted-text.txt", async (route) => {
    ...
    try {
      ...
      expect(await download.getAttribute("href")).toBe("/__afora__/pasted-text.txt");
