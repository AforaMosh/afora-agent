...
suite.define(() => {
  ...
  it("reorders sidebar sections from keyboard and touch controls", async () => {
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        await sidebar
          ...
          .getByRole("menuitem", { name: "Customize sidebar" })
          .click();
        const coding = sidebar.locator('[data-sidebar-customizer-id="work"]');
        const writesBefore = (await gateway.getRequests("sessions.groups.put")).length;

        await coding.getByRole("button", { name: "Move Coding up" }).focus();
        await page.keyboard.press("Enter");
        await expect
          .poll(async () => (await gateway.getRequests("sessions.groups.put")).length)
          .toBe(writesBefore + 1);

        await coding.getByRole("button", { name: "Move Coding down" }).tap();
        await expect
          .poll(async () => (await gateway.getRequests("sessions.groups.put")).length)
          .toBe(writesBefore + 2);
      },
