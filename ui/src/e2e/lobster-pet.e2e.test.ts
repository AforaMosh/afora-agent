...
async function settlePet() {
  await page.evaluate(
    () => (document.querySelector("afora-lobster-pet") as BrowserLobsterPet).updateComplete,
  );
}
...
suite.define(() => {
  ...
  it("keeps a vigil-only failure present through droop and sweep before leaving", async () => {
    ...
    await page.evaluate(async () => {
      const pet = document.querySelector("afora-lobster-pet") as BrowserLobsterPet;
      pet.mode = "idle";
      await pet.updateComplete;
    });
  ...
  it("shows a clickable dismissal menu above the clipped footer ledge", async () => {
    await mountPet({ mode: "offline", outcome: "ok", seed: 42 });
    await page.evaluate(() => {
      const pet = document.querySelector<HTMLElement>("afora-lobster-pet");
      if (pet) {
