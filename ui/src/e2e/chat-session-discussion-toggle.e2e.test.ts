...
describeControlUiE2e("session discussion toggle", () => {
  ...
  it("keeps an existing discussion closed until the header action opens it", async () => {
    ...
    await expect
      .poll(() =>
        page.locator("afora-session-discussion").evaluate((node) => node.childElementCount),
      )
      .toBeGreaterThan(0);
  ...
  it("keeps a cross-origin discussion in sync when the real host color scheme changes", async () => {
    ...
    await page.route("https://discussion.example/embed/channel/**", (route) =>
      route.fulfill({
        ...
        body: `<!doctype html><html><head><meta charset="utf-8">
          ...
          <p>The sidebar follows its Afora host.</p></article>
          <script>
            ...
            window.addEventListener("message", (event) => {
              ...
              if (event.data?.type !== "afora:widget-theme") return;
    ...
    const gateway = await installMockGateway(page, {
      ...
      methodResponses: {
        ...
        "session.discussion.open": {
          embedUrl: "https://discussion.example/embed/channel/T1/C1?aforaHostTheme=1",
