...
const suite = createControlUiE2eSuite({
  ...
  unavailableMessage: (executablePath) =>
    `Playwright Chromium is not available at ${executablePath}. Run \`pnpm --dir ui exec playwright install --with-deps chromium\`, or set AFORA_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
});
    ...
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        await expect
          ...
          .toEqual(["Afora Memory", "Memory LanceDB", "Off"]);
        ...
        await engineGroup.getByRole("radio", { name: "Afora Memory", exact: true }).click();
        ...
        if (captureUiProofEnabled) {
          ...
          await page
            ...
            .screenshot({
              animations: "disabled",
              path: path.join(uiProofArtifactDir, "01-afora-memory-selected.png"),
            });
        }
      },
    );
  ...
  it("keeps a committed add-on enabled and makes a failed config refresh visible", async () => {
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        if (captureUiProofEnabled) {
          await mkdir(uiProofArtifactDir, { recursive: true });
          await page.locator("afora-memory-settings").screenshot({
            animations: "disabled",
    ...
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        await expect
          ...
          .toEqual(["Afora Memory (Unavailable)", "Memory LanceDB", "Off"]);
        await expect
          .poll(() =>
            engineGroup
              .getByRole("radio", { name: "Afora Memory (Unavailable)", exact: true })
