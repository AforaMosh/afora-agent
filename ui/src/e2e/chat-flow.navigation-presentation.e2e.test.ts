...
suite.define(() => {
  it("coalesces persisted same-session split panes during cold startup", async () => {
    ...
    try {
      ...
      const panes = page.locator("afora-chat-pane.chat-split-view__pane");
  ...
  it("renders always-on pane headers without desktop topbar chrome", async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        document.documentElement.classList.add("afora-native-macos");
      ...
      await expect
        ...
        .toBe("90px");
      await page.evaluate(() => {
        document.documentElement.classList.remove("afora-native-macos");
      ...
      await expect
        ...
        .toBe(true);
      await page.locator("afora-chat-pane").evaluate((pane) => {
        (
      ...
      await expect
        .poll(() =>
          targetHeader.evaluate((header) => {
            const owner = header.closest("afora-chat-pane");
            return (
  ...
  it("flips a sidebar short route before any list refresh and refreshes only for an outbox", async () => {
    ...
    try {
      ...
      await page.locator('afora-chat-pane[aria-hidden="false"]').evaluate((pane, targetKey) => {
        const state = (
          pane as HTMLElement & {
            state: {
        ...
        const key = `afora.control.chatComposer.v2:${encodeURIComponent(gatewayOwner)}`;
