...
describeControlUiE2e("Control UI chat message actions", () => {
  ...
  it("offers Reply inline and mirrors every assistant action in the context menu", async () => {
      ...
      historyMessages: [
        {
          ...
          timestamp: Date.now(),
          __afora: { id: "assistant-action-proof", seq: 1 },
        },
        {
          role: "user",
          content: [{ type: "text", text: "Keep the next assistant message separate." }],
          timestamp: Date.now() + 1,
          __afora: { id: "user-action-separator", seq: 2 },
        },
        {
          role: "assistant",
          content: [
          ...
          __afora: { id: "assistant-thinking-proof", seq: 3 },
        },
        {
          role: "user",
          content: [{ type: "text", text: "Keep the truncated message separate." }],
          timestamp: Date.now() + 3,
          __afora: { id: "user-truncated-separator", seq: 4 },
        },
        {
          role: "assistant",
          content: [{ type: "text", text: truncatedPreview }],
          timestamp: Date.now() + 4,
          __afora: { id: "assistant-full-message", seq: 5 },
        },
      ],
      methodResponses: {
        "chat.message.get": {
    ...
    try {
      ...
      await multilineTooltipButton.evaluate((element) =>
        element.closest("afora-tooltip")?.remove(),
      );
