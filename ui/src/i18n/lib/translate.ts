...
class I18nManager {
  ...
  private readStoredLocale(): string | null {
    ...
    try {
      return storage.getItem("afora.i18n.locale");
    } catch {
      return null;
    }
  }
  ...
  private persistLocale(locale: Locale) {
    ...
    try {
      storage.setItem("afora.i18n.locale", locale);
    } catch {
  ...
  private clearPersistedLocale() {
    ...
    try {
      storage.removeItem("afora.i18n.locale");
    } catch {
      // Ignore storage write failures in private/blocked contexts.
    }
  }
...
if (typeof process !== "undefined" && (process.env?.VITEST || process.env?.NODE_ENV === "test")) {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("afora.i18nManagerTestApi")] = {
