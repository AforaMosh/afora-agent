  ...
  describe("normalizeMessage", () => {
      ...
      expect(result.content).toEqual([
        { type: "text", text: "Here." },
        {
          ...
          preview: {
            ...
            render: "url",
            viewId: "cv_status",
            url: "/__afora__/canvas/documents/cv_status/index.html",
            title: "Status",
            preferredHeight: 320,
          },
          rawText: null,
        ...
        content: [
          {
            ...
            preview: {
              ...
              surface: "assistant_message",
              render: "url",
              url: "/__afora__/canvas/documents/cv_widget/index.html",
              sandbox: "scripts",
    ...
    it("extracts MEDIA attachments and reads persisted delivery facts", () => {
      const result = normalizeMessage({
        ...
        aforaDelivery: { audioAsVoice: true, replyToId: "thread-123" },
      });
    ...
    it("classifies signed same-origin MEDIA image and audio routes", () => {
      ...
      const audioUrl = "/__afora__/media/voice%2Eogg?mediaTicket=signed";
      const result = normalizeMessage({
        role: "assistant",
        content: `MEDIA:${imageUrl}\nMEDIA:${audioUrl}`,
      });
    ...
    it("keeps valid local MEDIA paths as assistant attachments", () => {
      const result = normalizeMessage({
        role: "assistant",
        content: "Hello\nMEDIA:/tmp/afora/test-image.png\nWorld",
      });
    ...
    it("keeps spaced local filenames together instead of leaking suffix text", () => {
      const result = normalizeMessage({
        role: "assistant",
        content: "MEDIA:/tmp/afora/shinkansen kato - Google Shopping.pdf",
      });
      ...
      expect(result.content).toEqual([
        {
          type: "attachment",
          attachment: {
            url: "/tmp/afora/shinkansen kato - Google Shopping.pdf",
            kind: "document",
    ...
    it("formats durable sender metadata for transcript attribution", () => {
      ...
      expect(
        normalizeMessage({
          ...
          __afora: { senderId: "profile_123", senderName: "Alice Example" },
        }).senderLabel,
      ).toBe("Alice Example");
    });
  });
...
describe("sender label opaque-id stripping", () => {
  ...
  it("prefers durable metadata identity over the legacy label identity", () => {
    const normalized = normalizeMessage({
      ...
      content: "hi",
      senderLabel: "steipete (c3e32452-0467-47e5-aafa-233cd5dae29f)",
      __afora: { senderId: "meta-profile", senderName: "Meta Name" },
    });
