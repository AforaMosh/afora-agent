...
describe("resolveGatewayTokenForUrlEdit", () => {
  ...
  it("loads a scoped token when the normalized gateway endpoint changes", () => {
    ...
    sessionStorage.setItem(
      "afora.control.token.v1:wss://other-gateway.example/afora",
      "other-token",
    );
    ...
    expect(
      resolveGatewayTokenForUrlEdit(
        "wss://gateway.example/afora",
        "wss://other-gateway.example/afora/",
        "abc123",
      ),
    ).toBe("other-token");
  });
  ...
  it("does not restore legacy durable tokens when the gateway endpoint changes", () => {
    ...
    localStorage.setItem(
      "afora.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://other-gateway.example/afora",
        token: "gateway-token",
      }),
    );
