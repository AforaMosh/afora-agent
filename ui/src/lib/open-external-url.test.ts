...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://afora.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://afora.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://afora.ai/abc-123", baseHref)).toBe(
      "blob:https://afora.ai/abc-123",
    );
  });
