...
class AforaMascot extends LitElement {
  static override styles = css`
    :host {
      display: inline-block;
      width: var(--afora-mascot-size, 120px);
      height: var(--afora-mascot-size, 120px);
      overflow: visible;
  ...
  protected override updated(changed: PropertyValues<this>): void {
    if (changed.has("size")) {
      this.style.setProperty("--afora-mascot-size", `${this.resolvedSize}px`);
    }
    if (changed.has("mood")) {
      this.animator.setMood(this.resolvedMood, currentSeconds());
    }
...
if (!customElements.get("afora-mascot")) {
  customElements.define("afora-mascot", AforaMascot);
}
