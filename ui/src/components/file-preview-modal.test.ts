...
const FILE_PREVIEW_MODAL_ELEMENT_NAME = `test-afora-file-preview-modal-${crypto.randomUUID()}`;

customElements.define(FILE_PREVIEW_MODAL_ELEMENT_NAME, class extends AforaFilePreviewModal {});
...
describe("afora-file-preview-modal", () => {
  beforeEach(() => {
  ...
  it("handles file navigation from the modal dialog event path", async () => {
    ...
    const dialog = modal.shadowRoot?.querySelector<HTMLElement>("afora-modal-dialog");
    const arrowDown = new KeyboardEvent("keydown", {
      key: "ArrowDown",
      bubbles: true,
      cancelable: true,
  ...
  it("rerenders default copy when the locale changes", async () => {
    ...
    expect(
      modal.shadowRoot?.querySelector<HTMLElement & { label: string }>("afora-modal-dialog")
        ?.label,
