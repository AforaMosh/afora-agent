...
describe("AforaTerminalPanel", () => {
  ...
  it("restores persisted open state when a mounted tag upgrades lazily", async () => {
    ...
    const panel = element as unknown as AforaTerminalPanel;
    await panel.updateComplete;
  ...
  it.each([
    ...
    async ({ dock, label }) => {
      ...
      expect(JSON.parse(localStorage.getItem("afora.terminal.panel.v1") ?? "{}")).toMatchObject({
        open: true,
        dock: "main",
      });

      panel.remove();
      const restored = document.createElement(TERMINAL_PANEL_ELEMENT_NAME) as AforaTerminalPanel;
  ...
  it("opens new sessions for the selected agent", async () => {
    ...
    const styleResults = Array.isArray(AforaTerminalPanel.styles)
      ? AforaTerminalPanel.styles
      : [AforaTerminalPanel.styles];
  ...
  it("discards buffered startup input when open fails", async () => {
    ...
    await waitForFast(() => {
      const panel = document.querySelector(TERMINAL_PANEL_ELEMENT_NAME) as AforaTerminalPanel;
  ...
  it("reattaches persisted sessions before opening a catalog tab", async () => {
    sessionStorage.setItem("afora.terminal.sessions.v1", JSON.stringify(["persisted-1"]));
  ...
  it("restores a vanished persisted session as exited without replaying stale output", async () => {
    sessionStorage.setItem("afora.terminal.sessions.v1", JSON.stringify(["gone-1"]));
    const controller = createTerminalController();
    createGhosttyTerminalMock.mockResolvedValue(controller);
    const requests: Array<{ method: string; params: unknown }> = [];
    const client: TerminalGatewayClient = {
  ...
  it("does not mark a live persisted session exited after a transient attach failure", async () => {
    sessionStorage.setItem("afora.terminal.sessions.v1", JSON.stringify(["live-1"]));
  ...
  it("marks the old session exited when a replacement client no longer lists it", async () => {
    ...
    await waitForFast(() => {
      expect(sessionStorage.getItem("afora.terminal.sessions.v1")).toContain("old-session");
    });
  ...
  it("retranslates cached exit state when the locale changes", async () => {
    ...
    panel.toggle();
    await waitForFast(() => {
      expect(sessionStorage.getItem("afora.terminal.sessions.v1")).toContain("session-1");
    });
