...
const TERMINAL_PANEL_ELEMENT_NAME = `test-afora-terminal-panel-upload-${crypto.randomUUID()}`;

class TestTerminalPanel extends AforaTerminalPanel {
...
describe("AforaTerminalPanel upload lifecycle", () => {
  beforeEach(async () => {
  ...
  it("uploads dropped files and pastes shell-safe paths without executing", async () => {
    ...
    const client: TerminalGatewayClient = {
      ...
      request: async <T>(method: string, params?: unknown) => {
        ...
        if (method === "terminal.upload") {
          return { path: "/tmp/afora upload/scan final.pdf", size: 3 } as T;
        }
        return {} as T;
      },
      addEventListener: () => () => {},
    ...
    expect(controller.terminal.paste).toHaveBeenCalledWith("'/tmp/afora upload/scan final.pdf'");
  ...
  it("shows file progress and retries only the failed remainder", async () => {
    ...
    const client: TerminalGatewayClient = {
      ...
      request: async <T>(method: string, params?: unknown, options?: { signal?: AbortSignal }) => {
        ...
        if (method === "terminal.upload") {
          ...
          return { path: "/tmp/afora upload/notes.txt", size: 4 } as T;
        }
        return {} as T;
      },
      addEventListener: () => () => {},
    ...
    await waitForFast(() => {
      expect(controller.terminal.paste).toHaveBeenCalledWith(
        "'/tmp/afora upload/scan final.pdf' '/tmp/afora upload/notes.txt'",
      );
  ...
  it("cancels an active batch without pasting staged paths", async () => {
    ...
    pendingUpload.resolve({ path: "/tmp/afora upload/archive.zip", size: 3 });
