...
function createHarness(
  params: {
    ...
    rotatedKeys?: readonly string[];
    goneKeys?: readonly string[];
  ...
  const request = vi.fn(async (_method: string, rawParams?: unknown, _options?: unknown) => {
    ...
    const result = {
      outcomes: patchParams.targets.map((target) => {
        // A rotated row is refused once, then accepted when the caller re-aims at
        // the identity this reports; a gone row reports no surviving identity.
        const rotated =
          params.rotatedKeys?.includes(target.key) &&
          target.expectedSessionId !== `${target.key}-rotated`;
        if (rotated || params.goneKeys?.includes(target.key)) {
          const error = {
            code: ErrorCodes.INVALID_REQUEST,
            message: `Session ${target.key} changed before patch. Retry.`,
            details: {
              code: GatewayErrorDetailCodes.SESSION_CHANGED,
              ...(rotated ? { currentSessionId: `${target.key}-rotated` } : {}),
            },
          };
...
describe("patchSessionRows", () => {
  it("dispatches 101 rows as ordered protocol-sized chunks and refreshes once", async () => {
    ...
    expect(archived).toEqual({ rows, gone: [] });
  ...
  it("keeps ordered partial outcomes and prunes only successful archived rows", async () => {
    ...
    ).resolves.toEqual({ rows: [rows[1]], gone: [] });
  ...
  it("uses the supplied fallback when the method is unavailable", async () => {
    ...
    const fallbackResult = { rows: [sessionRow(1)], gone: [] };
  ...
  it("does not fallback for an unrelated INVALID_REQUEST", async () => {
    ...
    const fallback = vi.fn(async () => ({ rows: [sessionRow(1)], gone: [] }));
  ...
  it("does not fallback after an earlier chunk succeeds", async () => {
    ...
    ).resolves.toEqual({ rows: rows.slice(0, 100), gone: [] });
  ...
  it("re-aims at rotated rows without a word and accounts only the vanished", async () => {
    const rows = [
    ...
    const harness = createHarness({
      rotatedKeys: [rows[0]!.key],
      goneKeys: [rows[2]!.key],
    });

    const patched = await patchSessionRows(
      harness.host,
      rows,
      { category: "Projects" },
      harness.scope,
    ...
    // The rotated row landed on its surviving session, so it counts as applied.
    expect(patched?.rows.map((row) => row.key)).toEqual([rows[1]!.key, rows[0]!.key]);
    expect(patched?.gone).toEqual([rows[2]]);
    ...
    // Exactly one extra request re-aims the rotated row; never a loop.
    expect(harness.request).toHaveBeenCalledTimes(2);
    expect(harness.request.mock.calls[1]?.[1]).toEqual({
      targets: [
        { key: rows[0]!.key, agentId: "main", expectedSessionId: `${rows[0]!.key}-rotated` },
      ],
      patch: { category: "Projects" },
    });
    // A routine rotation is invisible, and a vanished row is the caller's to word.
    expect(harness.publishSessionMutationError).not.toHaveBeenCalled();
  });

  it("leaves a rotated row alone when the patch is not row presentation", async () => {
    const rows = [{ ...sessionRow(0), sessionId: "session-0" }];
    const harness = createHarness({ rotatedKeys: [rows[0]!.key] });

    const patched = await patchSessionRows(harness.host, rows, { archived: true }, harness.scope);

    // Archiving guards its own generation; re-aiming it would defeat that.
    expect(patched).toEqual({ rows: [], gone: rows });
    expect(harness.request).toHaveBeenCalledOnce();
  });
});
