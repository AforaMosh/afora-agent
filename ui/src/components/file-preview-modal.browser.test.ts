...
import type { AforaModalDialog } from "./modal-dialog.ts";
...
async function resolveRenderedDialog(modal: AforaModalDialog) {
...
async function mountPreview(width: number, activePath = initialFilePath) {
  ...
  const preview = document.createElement("afora-file-preview-modal") as AforaFilePreviewModal;
  ...
  const ownerDialog =
    preview.shadowRoot?.querySelector<AforaModalDialog>("afora-modal-dialog");
...
async function mountModal(
  ...
  modal.style.setProperty("--afora-modal-width", options.modalWidth);
