...
describe("option card", () => {
  ...
  it("accents and focuses the recommended choice, then emits its value", async () => {
    ...
    render(
      html`<afora-option-card
        .props=${{
          header: "Access",
          question: "How should Afora help?",
          options: [
    ...
    const card = container.querySelector("afora-option-card")!;
  ...
  it("always renders a skip affordance and emits dismissal", async () => {
    ...
    await container.querySelector("afora-option-card")!.updateComplete;
