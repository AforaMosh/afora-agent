...
title: "Cerebras"
read_when:
  - You want to use Cerebras with Afora
...
| Plugin          | official external package (`@afora/cerebras-provider`) |
...
```bash Onboarding
afora onboard --auth-choice cerebras-api-key
```
...
```
  ...
  <Step title="Verify models are available">
    ```bash
    afora models list --provider cerebras
    ```

    Lists all three static models. If `CEREBRAS_API_KEY` is unresolved, `afora models status --json` reports the missing credential under `auth.unusableProfiles`.
...
<Note>
If the Gateway runs as a daemon (launchd, systemd, Docker), make sure `CEREBRAS_API_KEY` is available to that process — for example in `~/.afora/.env` or through `env.shellEnv`. A key exported only in an interactive shell will not help a managed service unless the env is imported separately.
</Note>
