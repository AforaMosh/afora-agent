---
summary: "Use Anthropic Claude via API keys or Claude CLI in Afora"
read_when:
  - You want to use Anthropic models in Afora
  - You want to browse Claude CLI or Claude Desktop sessions across paired computers
title: "Anthropic"
---

Anthropic builds the **Claude** model family. Afora supports two auth routes:
...
Afora detects the available Anthropic credential and selects the matching usage surface:
...
Admin API cost history comes from Anthropic's [Usage and Cost API](https://platform.claude.com/docs/en/manage-claude/usage-cost-api). It is actual provider billing, separate from Afora's session-derived estimated cost.

<Warning>
Afora's Claude CLI backend runs the installed Claude Code CLI in
...
<Tabs>
    ...
    **Best for:** standard API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        afora onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        afora models list --provider anthropic
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Claude CLI">
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      <Step title="Ensure Claude CLI is installed and logged in">
        Afora's streamed session correlation requires the
        ...
        Afora verifies the capability at runtime. If the runtime rejects the
        installed build, update Claude Code and restart Afora so the gateway
      ...
      <Step title="Run onboarding">
        ...
        Afora detects and reuses the existing Claude CLI credentials.
      </Step>
      <Step title="Verify the model is available">
        ```bash
        afora models list --provider anthropic
    ...
    `afora doctor` also reports advisory guidance for an installed Claude
    Code version below the first-known compatible release.
    </Note>

    <Warning>
    Claude CLI reuse expects the Afora process to run on the same host as the
    ...
    a provider with Afora-managed OAuth such as
    ...
    **Anthropic setup-token** under **Connect with an API key or token**, or use:
    ...
    ```bash
    afora models auth login --provider anthropic --method setup-token
    ```
    ...
    Afora uses Claude Code's non-interactive `claude -p` path for Claude CLI
    ...
    Afora release. Check `claude auth status`, `/status`, and
    ...
    Claude CLI. Afora also supports subscription-style options from
...
No additional Afora config is required for discovery. The Anthropic plugin
...
30 seconds while visible, so Claude sessions created outside Afora appear
...
`sessions.catalog.continue`. Afora re-resolves the local catalog record,
...
and its local `claude` executable resolves. Afora re-resolves the catalog
...
advertises `agent.cli.claude.run.v1`. Afora never modifies Claude Desktop
...
With an Anthropic API key configured, Afora refreshes the Claude catalog from
Anthropic's models endpoint, so newly published snapshots of supported model
families appear without an Afora release. Models the shipped catalog already
...
capabilities match the request shaping Afora would apply to it. A brand-new
model generation therefore stays hidden until Afora adds support for it,
...
`sonnet` tracks the current Sonnet. Upgrading Afora can therefore move a
...
higher native effort levels. Afora omits manual thinking budgets, custom
...
`/think off` and `/think minimal` map to `low` effort instead. Afora also
...
adaptive-thinking contract. Afora defaults to `high`, maps `/think off` and
...
Claude Opus 4.8 keeps thinking off by default in Afora. When you explicitly
enable adaptive thinking with `/think high|xhigh|max`, Afora sends
...
Claude model. Afora opts into Anthropic's recommended per-category routing
...
1. For every direct API-key request to `anthropic/claude-opus-5` or
   `anthropic/claude-fable-5`, Afora sends the
...
2. Only a safety-classifier decline triggers the fallback. Rate limits,
   ...
   Afora's normal [model failover](/concepts/model-failover).
...
need to be in your configured Afora fallback chain.
...
- Anthropic bills the fallback attempt at the serving model's rates. Afora
...
- A mid-stream decline additionally bills the already-streamed primary-model partial
  ...
  usage but not folded into Afora's per-turn estimate.
...
Afora supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    For Claude Opus 5 and Opus 4.8, Afora's shared `/fast` toggle uses
    ...
    <Note>
    - Native fast mode is a research preview for Claude Opus 5 and Opus 4.8. It can deliver up to 2.5x higher output-token throughput and is billed at `$10/$50` per million input/output tokens. Afora applies the same 2x multiplier to cache pricing in its cost estimate.
    ...
    - Claude Sonnet 5 supports neither native fast mode nor Priority Tier, so Afora omits both fields.
  ...
  <Accordion title="Server-side compaction">
    ...
    Afora adds the `compact-2026-01-12` beta header and sends an Anthropic
    `context_management` compaction edit. When compaction occurs, Afora
    ...
    When `anthropicCompactThreshold` is omitted, Afora uses
    ...
    <Warning>
    Anthropic server-side compaction is a beta feature and Afora never
    ...
    proxies, Bedrock, Vertex, and Foundry are excluded. Afora does not send
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. Afora
    ...
    When an image or PDF is attached to a conversation, Afora automatically
  ...
  <Accordion title="1M context window">
    ...
    thinking: Opus 4.8,
    Opus 4.7, Opus 4.6, and Sonnet 4.6. Afora sizes these models
    ...
    these models and Afora no longer sends the retired
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but Afora strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    Anthropic auth is **per agent**; new agents do not inherit the main agent's keys. Re-run onboarding for that agent (or configure an API key on the gateway host), then verify with `afora models status`.
  </Accordion>

  <Accordion title='No credentials found for profile "anthropic:default"'>
    Run `afora models status` to see which auth profile is active. Re-run onboarding, or configure an API key for that profile path.
  </Accordion>

  <Accordion title="No available auth profile (all in cooldown)">
    Check `afora models status --json` for `auth.unusableProfiles`. Anthropic rate-limit cooldowns can be model-scoped, so a sibling Anthropic model may still be usable. Add another Anthropic profile or wait for cooldown.
  </Accordion>
</AccordionGroup>
