---
summary: "Use Z.AI (GLM models) with Afora"
read_when:
  - You want Z.AI / GLM models in Afora
...
Afora uses the `zai` provider with a Z.AI API key.

| Property | Value                                        |
| -------- | -------------------------------------------- |
| Provider | `zai`                                        |
| Package  | `@afora/zai-provider`                     |
| Auth     | `ZAI_API_KEY` (legacy alias: `Z_AI_API_KEY`) |
| API      | Z.AI Chat Completions (Bearer auth)          |

## GLM models

GLM is a model family, not a separate provider. In Afora, GLM models use
...
```bash
afora plugins install @afora/zai-provider
```

<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Afora probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        afora onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        afora models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        afora onboard --auth-choice zai-coding-cn

        # General API
        afora onboard --auth-choice zai-global

        # General API CN (China region)
        afora onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
`https://api.z.ai/api/anthropic`. Afora's Z.AI choices use the documented
...
- [General-purpose agent tools](https://docs.z.ai/devpack/tool/others),
  including Afora, are served on a best-effort basis. During high inference
...
```bash
afora models list --all --provider zai
afora config get models.providers.zai.baseUrl
```
...
<Tip>
`zai-api-key` lets Afora detect the matching Z.AI endpoint from the key and
...
| `zai/glm-5-turbo`  | Afora-optimized text model; 200K context       |
...
GLM-5.3. Run
`afora models list --all --provider zai` to see the catalog known to your
installed version.
</Note>
...
<Tabs>
  <Tab title="GLM-5.3">
    Levels: `low`, `high`, and `max` (default `max`). Afora maps these to
  ...
  <Tab title="GLM-5.2">
    Full range: `off`, `low`, `high`, `max` (default `off`). Afora maps
...
<AccordionGroup>
  ...
  <Accordion title="Preserved thinking">
    ...
    When enabled and thinking is on, Afora sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; Afora copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Afora config schema, including provider and model settings.
  </Card>
</CardGroup>
