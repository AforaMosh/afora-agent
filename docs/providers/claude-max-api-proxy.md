...
**claude-max-api-proxy** is a community npm package (not an Afora plugin) that
...
plan articles, and [Anthropic provider](/providers/anthropic) for Afora's
...
<Steps>
  ...
  <Step title="Configure Afora">
    Point Afora at the proxy as a custom OpenAI-compatible endpoint:
...
<Note>
The model ids below are the proxy's own catalog, not Afora's Anthropic
...
<AccordionGroup>
  <Accordion title="Proxy-style OpenAI-compatible notes">
    This uses Afora's generic custom `/v1` OpenAI-compatible route, the same
    ...
    - No Responses `store`, prompt-cache hints, or OpenAI reasoning-compat
      payload shaping.
    - Afora's OpenAI/Codex attribution headers (`originator`, `version`,
...
<CardGroup cols={2}>
  <Card title="Anthropic provider" href="/providers/anthropic" icon="bolt">
    Native Afora integration with Claude CLI or API keys.
  </Card>
