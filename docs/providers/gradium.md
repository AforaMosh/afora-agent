---
summary: "Use Gradium text-to-speech in Afora"
read_when:
...
[Gradium](https://gradium.ai) is a text-to-speech provider for Afora. It renders standard audio replies (WAV), voice-note-compatible Opus output, and 8 kHz u-law audio for telephony surfaces.
...
Output format is chosen automatically by target surface (see [Output](#output)) and is not configurable in `afora.json`.
...
Among configured TTS providers, Gradium's auto-select order is `30`. See [Text-to-Speech](/tools/tts) for how Afora picks the active provider when `tts.provider` is not pinned.
