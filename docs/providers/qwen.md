...
```bash
afora plugins install @afora/qwen-provider
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        afora onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        afora onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        afora models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including `qwen3.7-max` and `qwen3.6-flash`, which are not available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        afora onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        afora onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
    ...
    **Best for:** credit-based team subscription access to Qwen and supported third-party models through Alibaba Cloud Model Studio.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        afora onboard --auth-choice qwen-token-plan
        ```

        For the **China** endpoint in Beijing:

        ```bash
        afora onboard --auth-choice qwen-token-plan-cn
        ```
      </Step>
      <Step title="Verify the provider">
        ```bash
        afora models list --provider qwen-token-plan
        afora agent --model qwen-token-plan/qwen3.7-plus --message "Reply with: token plan ready"
        ```
      </Step>
    </Steps>

    <Note>
    Alibaba's Afora guide uses `bailian-token-plan` for a manual custom
    ...
    <Warning>
    Use Token Plan only for interactive Afora sessions. Do not select it for
...
Afora ships this Qwen static catalog. The catalog is endpoint-aware: Coding
...
family, the provider maps Afora thinking levels to DashScope's top-level
...
`MiniMax-M2.5` are thinking-only, so Afora keeps thinking enabled even when
...
<AccordionGroup>
  <Accordion title="Qwen 3.6 and 3.7 availability">
    ...
    Afora omits `qwen3.7-max` and `qwen3.6-flash` from Coding Plan catalogs.
  ...
  <Accordion title="Video generation region routing">
    Afora maps the configured Qwen region to the matching DashScope AIGC host
