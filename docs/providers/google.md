...
title: "Google (Gemini)"
read_when:
  - You want to use Google Gemini models with Afora
    ...
    **Recommended for:** standard Gemini API access.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        afora onboard --auth-choice gemini-api-key
        ```
      ...
      <Step title="Verify the model is available">
        ```bash
        afora models list --provider google
        ```
      </Step>
    </Steps>
    ...
    With a configured API key, Afora refreshes Google AI Studio's text-model
    catalog from the Gemini `models.list` API. Newly released Gemini 3 Pro, Flash,
    and Flash-Lite variants therefore appear in
    `afora models list --provider google` without waiting for an Afora
    release. If discovery is unavailable, Afora keeps the bundled fallback
    catalog.
    ...
    **Advanced use only:** run a canonical `google/*` model through an installed
    ...
    <Steps>
      <Step title="Configure Google AI Studio">
        Complete the API-key setup in the first tab. Afora must have a usable
      ...
      <Step title="Install Gemini CLI">
        ...
        Afora supports both Homebrew installs and global npm installs, including
    ...
    but Afora cannot create or repair them. If one breaks, replace it with a
...
<Note>
`google/gemini-3-pro-preview` was retired on 2026-03-09; use `google/gemini-3.1-pro-preview` instead. Re-running Gemini API key setup (`afora onboard --auth-choice gemini-api-key` or `afora models auth login --provider google`) rewrites a stale configured default to the current model.
</Note>
...
<Tip>
Gemini 3 models use `thinkingLevel` rather than `thinkingBudget`. Afora maps
...
a fixed Afora level. Gemini 3 and Gemini 3.1 omit a fixed `thinkingLevel` so
Google can choose the level; Gemini 2.5 sends Google's dynamic sentinel
`thinkingBudget: -1`.

Gemma 4 models (for example `gemma-4-26b-a4b-it`) support thinking mode. Afora
...
`thinkingBudget: 0`; Afora strips that value for Gemini 2.5 Pro requests
...
Afora adapts telephony/Meet bridge audio to Gemini's PCM Live API stream and
keeps tool calls on the shared realtime voice contract. Leave `temperature`
unset unless you need sampling changes; Afora omits non-positive values
...
sequential function calling. Afora omits the older `NON_BLOCKING`, function
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    For direct Gemini API runs (`api: "google-generative-ai"`), Afora
    ...
    - Gemini cache-hit usage is normalized into Afora `cacheRead` from
  ...
  <Accordion title="Gemini CLI usage notes">
    ...
    - `stats.cached` is normalized into Afora `cacheRead`.
    - If `stats.input` is missing, Afora derives input tokens from
  ...
  <Accordion title="Environment and daemon setup">
    ...
    is available to that process (for example, in `~/.afora/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
