...
title: "Cohere"
read_when:
  - You want to use Cohere with Afora
  - You need the Cohere API key env var or CLI auth choice
---

[Cohere](https://cohere.com) provides OpenAI-compatible inference through its Compatibility API. Afora provides Cohere as an official external plugin.

| Property        | Value                                    |
| --------------- | ---------------------------------------- |
| Provider id     | `cohere`                                 |
| Plugin          | `@afora/cohere-provider`              |
...
Reasoning-capable Cohere models support two Compatibility API reasoning modes. Afora maps **off** to `none` and every enabled thinking level to `high`. Command A Vision does not support tool use, so Afora keeps agent tools disabled for that model.
...
```bash
afora models list --provider cohere
```
