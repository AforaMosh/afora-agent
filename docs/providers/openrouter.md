---
summary: "Use OpenRouter's unified API to access many models in Afora"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Afora
...
OpenAI-compatible, so Afora talks to it over the same
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        afora onboard --auth-choice openrouter-oauth
        ```

        Afora opens OpenRouter's browser sign-in flow (PKCE), exchanges the
        code for an OpenRouter API key, and stores it in the default
        OpenRouter auth profile. On remote/headless hosts, Afora prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        afora models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        afora onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
Afora sends canonical OpenRouter image requests to the dedicated image API
...
optional `media_type`; when `media_type` is absent, Afora sniffs the image
...
Afora submits text-to-video and image-to-video jobs, polls the returned
...
and also exposes `google/lyria-3-clip-preview`. Afora sends `modalities:
...
Afora sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter's STT contract), not as multipart OpenAI form
uploads.

## Fusion router

OpenRouter Fusion sends one Afora model ref to several OpenRouter models in
...
`openrouter/fusion`, so the Afora model ref carries both the Afora
provider prefix and the upstream OpenRouter namespace:

```bash
afora models set openrouter/openrouter/fusion
```
...
in normal agent/chat turns to try to force Fusion: Afora turns can include
...
Afora adds a sanitized system-prompt note listing the configured analysis
...
login flow that issues an OpenRouter API key, so Afora stores the result in
...
```bash
afora models auth login --provider openrouter --method oauth
afora models auth login --provider openrouter --method api-key
```

On verified OpenRouter requests (`https://openrouter.ai/api/v1`), Afora adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Afora
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Afora sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Afora maps the selected thinking level
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    V4's required follow-up shape. Afora sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Afora keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    Afora forwards that object to OpenRouter as the request `provider`
