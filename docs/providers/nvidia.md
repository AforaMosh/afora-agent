---
summary: "Use NVIDIA's OpenAI-compatible API in Afora"
read_when:
  - You want to use open models in Afora for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). Afora
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    afora onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    afora models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
afora onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
for an Afora release. A fresh NVIDIA catalog overrides bundled retirement
...
Afora falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in Afora. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    Afora prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="OpenAI-compatible endpoint">
    Afora talks to NVIDIA with the `openai-completions` adapter against the
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. Afora's bundled Ultra row
