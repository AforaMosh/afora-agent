...
and `max` (the default). Afora exposes those exact levels and maps `/think
    ...
    **Best for:** Kimi K3 and K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      <Step title="Install the plugin">
        ```bash
        afora plugins install @afora/moonshot-provider
      ...
      <Step title="Run onboarding">
        ```bash
        afora onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        afora onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      <Step title="Confirm the Kimi K3 default">
        ...
        ```bash
        afora models set moonshot/kimi-k3
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        afora models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        AFORA_CONFIG_PATH=/tmp/afora-kimi/afora.json \
        AFORA_STATE_DIR=/tmp/afora-kimi \
        afora agent --local \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        afora onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        afora models list --provider kimi
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    Moonshot API Kimi K3 always reasons at maximum effort. Afora exposes only
    ...
    so Afora preserves the requested `tool_choice`. For multi-turn tool use,
    Afora preserves the assistant reasoning content required by Moonshot's
    replay contract.

    Kimi K2.7 Code always uses native thinking. Moonshot requires clients to
    omit the `thinking` field for this model, so Afora exposes only `on` and
    ignores stale `off` settings. K2.7 also fixes `temperature`, `top_p`, `n`,
    `presence_penalty`, and `frequency_penalty`; Afora omits configured
    ...
    Afora maps runtime `/think` levels for those models:
    ...
    default strategy. Afora only forwards `thinking.keep` for
    `moonshot/kimi-k2.6` and strips it from other models. Kimi K2.7 Code
    preserves full reasoning history by default while Afora omits the entire
    `thinking` field.
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves native tool_call ids shaped like `functions.<name>:<index>`. Afora preserves the first occurrence of each native Kimi id and rewrites later duplicates to deterministic OpenAI-style `call_*` ids. Matching tool results are remapped with the same id so replay remains unique without stripping Kimi's first native id. This behavior is wired into the bundled Moonshot provider and is not a user-configurable setting.
  </Accordion>
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    Afora keys this off the endpoint host, not the provider id, so a custom
