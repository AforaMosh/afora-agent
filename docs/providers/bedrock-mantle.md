---
summary: "Use Amazon Bedrock Mantle OpenAI-compatible and Claude Messages models with Afora"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with Afora
...
Afora includes a bundled **Amazon Bedrock Mantle** provider that connects to
    ...
    **Best for:** using AWS SDK-compatible credentials (shared config, SSO, web identity, instance or task roles).
    ...
    <Steps>
      ...
      <Step title="Verify models are discovered">
        ...
        Afora generates a Mantle bearer token from the credential chain automatically.
      </Step>
    </Steps>

    <Tip>
    When `AWS_BEARER_TOKEN_BEDROCK` is not set, Afora mints the bearer token for you from the AWS default credential chain, including shared credentials/config profiles, SSO, web identity, and instance or task roles.
    </Tip>
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, Afora uses it directly. Otherwise,
Afora attempts to generate a Mantle bearer token from the AWS default
...
```bash
afora config set plugins.entries.amazon-bedrock-mantle.config.discovery.enabled false
```
...
<AccordionGroup>
  <Accordion title="Reasoning support">
    ...
    `gpt-oss-safeguard-120b`. Afora sets `reasoning: true` automatically for
  ...
  <Accordion title="Endpoint unavailability">
    ...
    provider is skipped. Afora does not error; other configured providers
    continue to work normally.
  </Accordion>

  <Accordion title="Claude via the Anthropic Messages route">
    When automatic discovery owns the model list, Afora appends five Claude
    ...
    `/think xhigh|max` uses the model's native effort levels. Afora omits
    ...
    route cannot disable thinking. Afora also omits custom temperature for
    Sonnet 5 requests.
    ...
    model-provided reasoning, and Afora omits its `temperature` parameter
