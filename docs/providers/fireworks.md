...
title: "Fireworks"
read_when:
  - You want to use Fireworks with Afora
...
| Package         | `@afora/fireworks-provider`                      |
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    afora plugins install @afora/fireworks-provider
    ```
  </Step>
...
```bash Onboarding
afora onboard --auth-choice fireworks-api-key
```
...
```
  ...
  <Step title="Verify the model is available">
    ```bash
    afora models list --provider fireworks
    ```

    The list should include `GLM 5.2 Fast`, `Kimi K2.6`, and `Kimi K2.6 Fast`. If `FIREWORKS_API_KEY` is unresolved, `afora models status --json` reports the missing credential under `auth.unusableProfiles`.
...
<AccordionGroup>
  <Accordion title="How model id prefixing works">
    Every Fireworks model ref in Afora starts with `fireworks/` followed by the exact id or router path from the Fireworks platform. For example:

    - Router model: `fireworks/accounts/fireworks/routers/kimi-k2p6-turbo`
    - Direct model: `fireworks/accounts/fireworks/models/<model-name>`

    Afora strips the `fireworks/` prefix when constructing the API request and sends the remaining path to the Fireworks endpoint as the OpenAI-compatible `model` field.

  </Accordion>

  <Accordion title="Why thinking is forced off for Kimi">
    Fireworks serves Kimi without a separate reasoning channel, so chain-of-thought can surface in the visible `content` stream. On every Fireworks Kimi request Afora sends `thinking: { type: "disabled" }` and strips `reasoning`, `reasoning_effort`, and `reasoningEffort` from the payload (`extensions/fireworks/stream.ts`). The provider policy (`extensions/fireworks/thinking-policy.ts`) advertises only the `off` thinking level for Kimi model ids, so manual `/think` switches and provider-policy surfaces stay aligned with the runtime contract.
  ...
  <Accordion title="Environment availability for the daemon">
    ...
    <Warning>
      A key exported only in an interactive shell will not help a launchd or systemd daemon unless that environment is imported there too. Set the key in `~/.afora/.env` or via `env.shellEnv` to make it readable from the gateway process.
    </Warning>

    Afora loads `~/.afora/.env` when it loads config, so keys stored there reach managed gateway services on every platform. Restart the gateway (or re-run `afora doctor --fix`) after rotating the key.
