---
summary: "Use ElevenLabs speech, Scribe STT, and realtime transcription with Afora"
read_when:
  - You want ElevenLabs text-to-speech in Afora
...
Afora uses ElevenLabs for text-to-speech, batch speech-to-text with Scribe
...
| Capability               | Afora surface                                                     | Default                  |
| ------------------------ | -------------------------------------------------------------------- | ------------------------ |
...
Set `modelId` to `eleven_v3` to use ElevenLabs v3 TTS. Afora keeps
...
returned audio stream instead of waiting for Afora to download the whole
audio file first. `latencyTier` maps to ElevenLabs' `optimize_streaming_latency`
query parameter for models that accept it; Afora omits that parameter for
...
Afora sends multipart audio to ElevenLabs `/v1/speech-to-text` with
