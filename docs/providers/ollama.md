---
summary: "Run Afora with Ollama (cloud and local models)"
read_when:
  - You want to run Afora with cloud or local models via Ollama
  - You need Ollama setup and configuration guidance
  - You want Ollama vision models for image understanding
title: "Ollama"
---

Afora talks to Ollama's native API (`/api/chat`), not the OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Local and LAN hosts">
    Loopback, private-network, `.local`, and bare-hostname Ollama URLs do not need a real bearer token. Afora uses the `ollama-local` marker for these.
  </Accordion>
  ...
  <Accordion title="Auth profiles">
    `auth-profiles.json` stores the credential for a provider id; put endpoint settings (`baseUrl`, `api`, models, headers, timeouts) in `models.providers.<id>`. Older flat files such as `{ "ollama-windows": { "apiKey": "ollama-local" } }` are not a runtime format; `afora doctor --fix` rewrites them into a canonical `ollama-windows:default` API-key profile with a backup. A `baseUrl` value in that legacy file is noise and should move to provider config.
  </Accordion>
  ...
  <Tab title="Onboarding (recommended)">
    <Steps>
      <Step title="Run onboarding">
        ...
        On a fresh guided setup, Afora first checks the default or configured
      ...
      <Step title="Verify">
        ```bash
        afora models list --provider ollama
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Manual setup">
    <Steps>
      ...
      <Step title="Set a credential">
        ...
        Or in config: `afora config set models.providers.ollama.apiKey "OLLAMA_API_KEY"`.
      </Step>
      <Step title="Select the model">
        ```bash
        afora models list
        afora models set ollama/gemma4
        ```
...
Afora prompts for the base URL, discovers local models, and checks
...
For cloud-only access without a local daemon, use `afora onboard --auth-choice ollama-cloud` and see [Ollama Cloud](/providers/ollama-cloud) — that path does not need `ollama signin` or a running server:

```bash
afora onboard --auth-choice ollama-cloud
afora models set ollama-cloud/kimi-k2.5:cloud
```

The cloud model list shown during `afora onboard` is populated live from
...
models at setup time, Afora falls back to its hardcoded suggested list so
...
defined, Afora discovers models from `http://127.0.0.1:11434`:
...
| Token limits         | `maxTokens` defaults to Afora's Ollama max-token cap                                                                                                                                                                                                                                       |
...
hand-written `models.json` entry; Afora resolves it live. For signed-in
...
`afora infer audio transcribe` for audio):
...
provider and `/api/tags` is unreachable, Afora records that run as
...
```bash
AFORA_LIVE_TEST=1 AFORA_LIVE_OLLAMA=1 AFORA_LIVE_OLLAMA_WEB_SEARCH=0 \
  pnpm test:live -- extensions/ollama/ollama.live.test.ts
```

For Ollama Cloud, point the same live test at the hosted endpoint (skips
embeddings by default; force with `AFORA_LIVE_OLLAMA_EMBEDDINGS=1` since a
cloud key may not authorize `/api/embed`):

```bash
export OLLAMA_API_KEY='<your-ollama-cloud-api-key>'
AFORA_LIVE_TEST=1 AFORA_LIVE_OLLAMA=1 \
AFORA_LIVE_OLLAMA_BASE_URL=https://ollama.com \
AFORA_LIVE_OLLAMA_MODEL=glm-5.1:cloud \
AFORA_LIVE_OLLAMA_WEB_SEARCH=1 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```
...
<Steps>
  ...
  <Step title="Connect the node host">
    ...
    ```bash
    afora devices list
    afora devices approve <deviceRequestId>
    afora nodes pending
    afora nodes approve <nodeRequestId>
    ...
    `ollama.models` and `ollama.chat`, check `afora nodes pending` again.
...
```bash
afora config set plugins.entries.ollama.config.nodeInference.enabled false
```

Restart the node (`afora node restart`, or stop/rerun `afora node run`
...
surface may need `afora nodes pending` approval again after reconnect.
...
media-understanding provider, so Afora can route explicit image-description
...
afora infer image describe --file ./photo.jpg --model ollama/qwen2.5vl:7b --json
```
...
that already support native vision. If the call fails, Afora can continue
through `agents.defaults.imageModel.fallbacks`; file/URL preparation errors
fail before fallback is attempted. Use `infer image describe` for Afora's
...
```bash
AFORA_LIVE_TEST=1 AFORA_LIVE_OLLAMA_IMAGE=1 \
...
Afora rejects image-description requests for models not marked
...
<Tabs>
  <Tab title="Basic (implicit discovery)">
    ...
    <Tip>
    If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry; Afora fills it in for availability checks.
    </Tip>
...
<AccordionGroup>
  ...
  <Accordion title="LAN Ollama host with manual models">
    ...
    `contextTokens` caps Afora's active-input budget; `params.num_ctx` sets
  ...
  <Accordion title="Multiple Ollama hosts">
    ...
    Afora strips the active provider prefix (falling back to a bare
...
prefix, such as `ollama-spark/qwen3:32b`, Afora strips that prefix before
...
# Afora catalog and selected model
afora models list --provider ollama
afora models status
...
works but Afora does not, check whether the Gateway runs on a different
machine, container, or service account.

## Ollama Web Search

Afora bundles **Ollama Web Search** as a `web_search` provider.
...
Choose it during `afora onboard` or `afora configure --section web`, or set:
...
For a self-hosted host, Afora first tries the local `/api/experimental/web_search`
...
<AccordionGroup>
  <Accordion title="Legacy OpenAI-compatible mode">
    ...
    **Tool calling is not reliable in this mode.** Use it only when a proxy needs OpenAI format and you do not depend on native tool calling.
    ...
    Afora injects `options.num_ctx` by default in this mode so Ollama does
  ...
  <Accordion title="Context windows">
    For auto-discovered models, Afora uses the context window `/api/show`
    reports, including larger `PARAMETER num_ctx` values from custom
    Modelfiles; otherwise it falls back to Afora's default Ollama context
    window.
    ...
    configuration, run `afora doctor --fix`, then set `params.num_ctx`
    ...
    top-level request fields instead of nested `options`. Afora only
  ...
  <Accordion title="Thinking control">
    Afora forwards thinking as Ollama expects it: top-level `think`, not
    ...
    ```bash
    afora agent --model ollama/gemma4 --thinking off
    afora agent --model ollama/gemma4 --thinking low
    ```
    ...
    thinking for a specific model. Afora preserves that explicit config
  ...
  <Accordion title="Memory embeddings">
    ...
    loopback origin derived from the configured `baseUrl` use Afora's
  ...
  <Accordion title="Streaming configuration">
    ...
    and `afora agent --thinking off` send top-level `think: false` unless
...
<AccordionGroup>
  <Accordion title="WSL2 crash loop (repeated reboots)">
    ...
    Afora logs a startup warning when it detects WSL2, `ollama.service`
  ...
  <Accordion title="Remote host works with curl but not Afora">
  ...
  <Accordion title="Kimi or GLM returns garbled symbols">
    ...
    ```bash
    afora infer model run --model ollama/kimi-k2.5:cloud --prompt "Reply with exactly: ok" --json
    afora models set ollama/gemma4
    ```
  ...
  <Accordion title="Large-context model is too slow or runs out of memory">
    ...
    `params.num_ctx` is set. Cap both Afora's budget and Ollama's request
    ...
    Lower the model entry's `contextTokens` if Afora sends too much prompt. Lower
