...
read_when:
  - You want to use Vercel AI Gateway with Afora
...
| Package       | `@afora/vercel-ai-gateway-provider` |
...
<Tip>
Afora auto-discovers the Gateway `/v1/models` catalog, so both the
`/models vercel-ai-gateway` chat command and
`afora models list --provider vercel-ai-gateway` include current model
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    afora plugins install @afora/vercel-ai-gateway-provider
    ```
  </Step>
  <Step title="Set the API key">
    ```bash
    afora onboard --auth-choice ai-gateway-api-key
    ```
  </Step>
  ...
  <Step title="Verify the model is available">
    ```bash
    afora models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Afora normalizes Claude shorthand model refs at runtime:
...
<Tip>
Use either form in your configuration; Afora resolves the canonical
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Afora Gateway runs as a daemon (launchd/systemd), make sure
    ...
    the key in `~/.afora/.env` or via `env.shellEnv` to ensure the gateway
    process can read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow the upstream model prefix when Afora recognizes
