---
summary: "Use xAI Grok models in Afora"
read_when:
  - You want to use Grok models in Afora
  - You are configuring xAI auth or model ids
title: "xAI"
---

Afora ships a bundled `xai` provider plugin for Grok models. The
...
show Grok Build on the consent screen because Afora uses xAI's shared
OAuth client.
...
<Steps>
  <Step title="New install">
    ...
    ```bash
    afora onboard --install-daemon --auth-choice xai-oauth
    ```
  ...
  <Step title="Existing install">
    ...
    ```bash
    afora models auth login --provider xai --method oauth
    ```
    ...
    default, so future xAI default changes do not require an Afora update.
    It preserves an existing primary; opt in explicitly when needed:

    ```bash
    afora models set xai/auto
    ```
  ...
  <Step title="API-key path">
    ...
    ```bash
    afora models auth login --provider xai --method api-key
...
<Note>
Afora uses the xAI Responses API as the bundled xAI transport. The same
credential from `afora models auth login --provider xai --method oauth` or
...
<Tip>
Use `xai-oauth` when signing in from SSH, Docker, or a VPS. Afora prints a
...
Afora's flat catalog cost fields record the short-context rates. The current
[Grok Build](https://docs.x.ai/build/overview) coding agent uses Grok 4.6. The
historical Afora `grok-build-latest` compatibility alias remains pinned to
Grok 4.5.

## Feature coverage

The bundled plugin maps supported xAI APIs onto Afora's shared provider and
tool contracts. Capabilities that do not fit the shared contract are listed
below or under known limits.

| xAI capability             | Afora surface                        | Status                                               |
| -------------------------- | --------------------------------------- | ---------------------------------------------------- |
...
| Files / batches            | Generic model API compatibility only    | Not a first-class Afora tool                      |

<Note>
Afora uses xAI's REST image/video/TTS/STT APIs for media generation and
...
The dated 0309 ids are the selectable catalog entries. Afora sends all other
...
xAI retired the following exact ids. Afora keeps them as hidden compatibility
...
`afora doctor --fix` updates persisted xAI server-tool defaults and the
...
<Warning>
  ...
  `enabled` setting omitted, Afora exposes it only for an active xAI model.
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    afora models auth login --provider xai --method oauth
    afora config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    Afora encodes those as data URLs for xAI.
    </Warning>

    Video 1.5 also recognizes xAI's `grok-imagine-video-1.5-preview` and
    `grok-imagine-video-1.5-2026-05-30` identifiers. Afora forwards the
  ...
  <Accordion title="Image generation">
    ...
    Afora asks xAI for `b64_json` image responses so generated media can be
    ...
    Afora forwards only the shared cross-provider image controls today;
  ...
  <Accordion title="Text-to-speech">
    ...
    - Voices: authenticated live catalog from xAI; list it with
      `afora infer tts voices --provider xai`
    ...
    <Note>
    Afora uses xAI's batch `/v1/tts` endpoint for buffered synthesis,
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through Afora's
    ...
    transcription request. Prompt hints are accepted by the shared Afora
  ...
  <Accordion title="Realtime voice (Talk)">
    ...
    - Barge-in: xAI server VAD interrupts the response; Afora clears queued playback
    ...
    <Note>
    `sessionResumption` defaults to `false`. When set to `true`, Afora asks
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an Afora tool for
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an Afora tool for
  ...
  <Accordion title="Context compaction">
    ...
    compaction. The session keeps its Afora transcript unchanged and stores
    ...
    Endpoint failures fall back to Afora's client-side summarization.
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config
      ...
      even though Afora does not require the Grok Build app.
    - Afora does not currently expose the xAI multi-agent model family. xAI
      serves these models through the Responses API, but they do not accept
      the client-side or custom tools used by Afora's shared agent loop.
      See the
  ...
  <Accordion title="Advanced notes">
    - Afora applies xAI-specific tool-schema and tool-call compatibility
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as Afora
      tools. Afora attaches only the specific xAI built-in each tool needs
...
AFORA_LIVE_TEST=1 AFORA_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/xai.live.test.ts
...
AFORA_LIVE_TEST=1 AFORA_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/x-search.live.test.ts
AFORA_LIVE_GATEWAY_MODELS="xai/grok-4.6,xai/grok-4.5,xai/grok-build-0.1,xai/grok-4.3,xai/grok-4.20-0309-reasoning,xai/grok-4.20-0309-non-reasoning" AFORA_LIVE_GATEWAY_MAX_MODELS=0 AFORA_LIVE_GATEWAY_SMOKE=0 pnpm test:live -- src/gateway/gateway-models.profiles.live.test.ts
AFORA_LIVE_TEST=1 AFORA_LIVE_TEST_QUIET=1 AFORA_LIVE_IMAGE_GENERATION_PROVIDERS=xai pnpm test:live -- test/image-generation.runtime.live.test.ts
```
...
The shared image live file verifies the same xAI provider through Afora's
