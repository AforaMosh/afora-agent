...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in Afora
---

ClawRouter gives Afora one policy-scoped key for multiple upstream model
...
the key's budget and aggregate usage on Afora usage surfaces.
...
Afora host. The plugin ships bundled with Afora (`enabledByDefault: true`);
you only need an issued ClawRouter credential.

| Property      | Value                                    |
| ------------- | ---------------------------------------- |
| Provider      | `clawrouter`                             |
| Plugin        | bundled (included in Afora)           |
| Auth          | `CLAWROUTER_API_KEY`                     |
| Default URL   | `https://clawrouter.afora.ai`         |
...
<Steps>
  ...
  <Step title="Configure Afora">
    ...
    afora onboard --auth-choice clawrouter-api-key
    afora plugins enable clawrouter
    ```
    ...
    ClawRouter origin; the default is `https://clawrouter.afora.ai`.

  </Step>
  <Step title="List granted models">
    ```bash
    afora models list --all --provider clawrouter
    ```
  ...
  <Step title="Select a model">
    ```bash
    afora models set clawrouter/<provider>/<model>
    ```

    You can also select a returned model for one run with
    `afora agent --model clawrouter/<provider>/<model> --message "..."`.
...
SecretRef in `afora.json`. The canonical managed fields are:
...
```bash
afora config patch --file ./clawrouter.patch.json5 --dry-run --json
afora config patch --file ./clawrouter.patch.json5
```
...
such as `AFORA_EXTENSIONS=clickclack`, `slack`, or `msteams`; see
...
# Afora gateway startup readiness only; no model call is made.
curl -fsS http://127.0.0.1:18789/readyz

# Credential-scoped catalog discovery.
afora models list --all --provider clawrouter --json

# Minimal real inference probe through the configured ClawRouter provider.
afora models status --probe --probe-provider clawrouter --probe-max-tokens 8 --json

# Workload canary using an exact granted model ref.
afora agent --agent main \
...
`X-Request-ID`, so an Afora model-call event can be joined to ClawRouter's
...
supported request routes. Afora does not ship a second, fixed list of
ClawRouter models. A catalog model is advertised as an Afora model when:
...
Adding a model to a supported ClawRouter provider needs no Afora release:
...
ClawRouter owns upstream credentials; its catalog tells Afora which
transport to use, so you never install every upstream company's auth plugin.

| Catalog capability / route                               | Afora transport     |
| -------------------------------------------------------- | ---------------------- |
...
unsupported request format is intentionally not advertised as an Afora
...
ClawRouter's `/v1/usage` response feeds the normal Afora provider-usage
...
The same provider snapshot is available to `/status` in chat and Afora's
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter credential; Afora does not fall back to upstream provider keys.                                          |
...
<CardGroup cols={2}>
  ...
  <Card title="Usage tracking" href="/concepts/usage-tracking" icon="chart-line">
    Afora usage and status surfaces.
  </Card>
</CardGroup>
