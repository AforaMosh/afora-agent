---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in Afora"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in Afora
title: "DeepInfra"
---
...
```bash
afora onboard --deepinfra-api-key <key>
```
...
live from `https://api.deepinfra.com/v1/openai/models?sort_by=afora&filter=with_meta`
...
| Surface                  | Default model                                                                  | Afora config/tool                                  |
| ------------------------ | ------------------------------------------------------------------------------ | ----------------------------------------------------- |
...
native model types. Afora has no provider contract for those categories
yet, so this plugin does not register them.

## Available models

Afora discovers DeepInfra models dynamically once a key is configured. Use
`/models deepinfra` or `afora models list --provider deepinfra` to see the
current list.
