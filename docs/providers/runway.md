---
summary: "Runway video generation setup in Afora"
title: "Runway"
read_when:
  - You want to use Runway video generation in Afora
  - You need the Runway API key/env setup
  - You want to make Runway the default video provider
---

Afora ships a bundled `runway` provider for hosted video generation, enabled by default, registered against the `videoGenerationProviders` contract.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    afora onboard --auth-choice runway-api-key
    ```
  </Step>
  <Step title="Set Runway as the default video provider">
    ```bash
    afora config set agents.defaults.mediaModels.video.primary "runway/gen4.5"
    ```
  </Step>
...
<Note>
  Picking a Runway model id from the wrong column produces an explicit error before the API request leaves Afora. The provider validates `model` against the mode's allowlist (`TEXT_ONLY_MODELS`, `IMAGE_MODELS`, `VIDEO_MODELS`) in `extensions/runway/video-generation-provider.ts`.
</Note>
...
<AccordionGroup>
  <Accordion title="Environment variable aliases">
    Afora recognizes both `RUNWAYML_API_SECRET` (canonical) and `RUNWAY_API_KEY`.
    Either variable authenticates the Runway provider.
  </Accordion>

  <Accordion title="Task polling">
    Runway uses a task-based API. After submitting a generation request, Afora
