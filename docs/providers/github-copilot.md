---
summary: "Sign in to GitHub Copilot from Afora using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `afora models auth login-github-copilot` flow
  - You are choosing between the built-in Copilot provider, Copilot SDK harness, and Copilot Proxy
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. Afora can use Copilot as a model
provider or agent runtime in three different ways.

## Three ways to use Copilot in Afora

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain and store a GitHub token. When
    Afora runs, it validates Copilot access and resolves the account-specific
    ...
    <Steps>
      ...
      <Step title="Set a default model">
        ```bash
        afora models set github-copilot/claude-sonnet-5
        ```
  ...
  <Tab title="Copilot SDK harness plugin (copilot)">
    Install the external `@afora/copilot` plugin when you want GitHub's
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Afora talks to
    the proxy's `/v1` endpoint (default `http://localhost:3000/v1`) and uses the
    model list you configure.

    The `copilot-proxy` plugin ships with Afora and is enabled by default.
    Configure the base URL and model ids with:

    ```bash
    afora models auth login --provider copilot-proxy --set-default
    ```
...
endpoints rather than public `github.com`. Afora exposes this as a
first-class auth choice so you do not have to hand-edit URLs.

<Steps>
  <Step title="Pick the Enterprise auth choice">
    In onboarding or `afora models auth`, choose
    **GitHub Copilot (Enterprise / data residency)**. You will be prompted for
    ...
    Afora derives those endpoints from the tenant root automatically.

    ```bash
    afora models auth login --provider github-copilot --method device-enterprise
    ```
...
tenant, or one tenant to another), Afora will not reuse the existing token —
...
`afora models auth login-github-copilot` shortcut, account validation,
...
| ---------------------------------------------------------------------- | --------------- | ---------------------------------------------------- |
| `afora models auth login-github-copilot`                            | `--yes`         | Overwrite an existing auth profile without prompting |
| `afora models auth login --provider github-copilot --method device` | `--set-default` | Also apply the provider's recommended default model  |

```bash
# Skip the re-login confirmation
afora models auth login-github-copilot --yes

# Login and set the default model in one step
afora models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

The device-login flow requires an interactive TTY. For headless setup, import
an existing GitHub OAuth access token with `afora onboard --non-interactive`:
...
must choose a default, it also checks the live Copilot model catalog. Afora
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    Afora refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    Newly published Copilot models become visible without an Afora upgrade,
  ...
  <Accordion title="Transport selection">
    ...
    models keep the OpenAI Responses transport. Afora selects the correct
    transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    Afora sends Copilot-compatible request headers with a Copilot CLI request
  ...
  <Accordion title="Environment variable resolution order">
    Afora resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Afora uses the highest-priority one.
    The device-login flow (`afora models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    `github-copilot:github`). At runtime, Afora validates Copilot access,
...
have logged in, Afora can use it for embeddings without a separate API key.

### Config

Set `memory.search.provider` explicitly to use GitHub Copilot embeddings. If a
GitHub token is available, Afora discovers available embedding models from
...
1. Afora resolves your GitHub token (from env vars or auth profile).
...
available, Afora skips Copilot and tries the next provider.
