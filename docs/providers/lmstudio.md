---
summary: "Run Afora with LM Studio"
read_when:
  - You want to run Afora with open source models via LM Studio
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    On a fresh guided setup, Afora first queries `/api/v1/models` on the
...
```bash
afora models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys use an `author/model-name` format (e.g. `qwen/qwen3.5-9b`); Afora model refs
...
```bash
afora onboard --non-interactive --accept-risk --skip-health --auth-choice lmstudio
```
...
servers; omit it for unauthenticated servers and Afora stores a local non-secret marker instead.
...
LM Studio doesn't always emit an OpenAI-shaped `usage` object on streamed responses. Afora
...
When LM Studio's `/api/v1/models` discovery reports model-specific reasoning options, Afora
...
"on"]`) while rejecting those literal values on `/v1/chat/completions`; Afora normalizes that
...
LM Studio supports just-in-time (JIT) model loading, loading models on first request. Afora
