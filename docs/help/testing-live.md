...
- Bring your own gateway: use the in-process gateway (Layer 2 below) or start a
  dev instance with an isolated state dir (`AFORA_STATE_DIR=<scratch>`) and a
  free port. Do not bind the default gateway port (18789) while a real gateway
  is running on it.
- Do not `afora gateway stop`/`restart` (or `launchctl`/`systemctl`/tmux
...
```bash
pnpm afora infer tts convert --local --json \
  --text "Afora live smoke." \
  --output /tmp/afora-live-smoke.mp3
```

Safe voice-call readiness smoke:

```bash
pnpm afora voicecall setup --json
pnpm afora voicecall smoke --to "+15555550123"
```
...
- Optional target overrides:
  - `AFORA_ANDROID_NODE_ID` or `AFORA_ANDROID_NODE_NAME`.
  - `AFORA_ANDROID_GATEWAY_URL` / `AFORA_ANDROID_GATEWAY_TOKEN` / `AFORA_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `AFORA_LIVE_TEST=1` if invoking Vitest directly)
  - Set `AFORA_LIVE_MODELS=modern`, `small`, or `all` (alias for `modern`) to actually run this suite; otherwise it skips, so `pnpm test:live` on its own stays focused on gateway smoke.
- How to select models:
  - `AFORA_LIVE_MODELS=modern` runs the curated high-signal priority list (see [Live: model matrix](#live-model-matrix-what-we-cover))
  - `AFORA_LIVE_MODELS=small` runs the curated small-model priority list
  - `AFORA_LIVE_MODELS=all` is an alias for `modern`
  - or `AFORA_LIVE_MODELS="openai/gpt-5.6-luna,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Local Ollama small-model runs default to `http://127.0.0.1:11434`; set `AFORA_LIVE_OLLAMA_BASE_URL` only for LAN, custom, or Ollama Cloud endpoints.
  - Modern/all and small sweeps default to their curated-list length as a cap; set `AFORA_LIVE_MAX_MODELS=0` for an exhaustive selected-profile sweep or a positive number for a smaller cap.
  - Exhaustive sweeps use `AFORA_LIVE_TEST_TIMEOUT_MS` for the whole direct-model test timeout. Default: 60 minutes.
  - Direct-model probes run with 20-way parallelism by default; set `AFORA_LIVE_MODEL_CONCURRENCY` to override.
- How to select providers:
  - `AFORA_LIVE_PROVIDERS="google,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `AFORA_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates "provider API is broken / key is invalid" from "gateway agent pipeline is broken"
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@afora" actually does)
...
- How to select models:
  ...
  - `AFORA_LIVE_GATEWAY_MODELS=small` runs the curated small-model list through the full gateway+agent pipeline
  - `AFORA_LIVE_GATEWAY_MODELS=all` is an alias for `modern`
  - Or set `AFORA_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all and small gateway sweeps default to their curated-list length as a cap; set `AFORA_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive selected sweep or a positive number for a smaller cap.
- How to select providers (avoid "OpenRouter everything"):
  - `AFORA_LIVE_GATEWAY_PROVIDERS="google,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `AFORA_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-sonnet-4-6"`
  - `AFORA_LIVE_CLI_BACKEND_COMMAND="/full/path/to/claude"`
  - `AFORA_LIVE_CLI_BACKEND_ARGS='["-p","--output-format","json"]'`
  - `AFORA_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt). Off by default in Docker recipes.
  - `AFORA_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `AFORA_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `AFORA_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `AFORA_LIVE_CLI_BACKEND_CACHE_PROBE=1` to run a fresh Claude CLI turn plus two native resumes and require at least 90% prompt-cache reuse on the second resume. This probe disables the image, MCP, and model-switch probes.
  - `AFORA_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` to opt into the Claude Sonnet -> Opus same-session continuity probe when the selected model supports a switch target. Off by default, including in Docker recipes.
  - `AFORA_LIVE_CLI_BACKEND_MCP_PROBE=1` to opt into the MCP/tool loopback probe. Off by default in Docker recipes.
...
settings Afora gives Gemini, then runs `gemini --debug mcp list` to prove a
...
- It resolves CLI smoke metadata from the owning plugin, then installs the matching Linux CLI package (`@anthropic-ai/claude-code` or `@google/gemini-cli`) into a cached writable prefix at `AFORA_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/afora/docker-cli-tools`).
...
- Enable:
  - `AFORA_LIVE_TEST=1 AFORA_LIVE_APNS_REACHABILITY=1 pnpm test:live src/infra/push-apns-http2.live.test.ts`
- Optional timeout:
  - `AFORA_LIVE_APNS_TIMEOUT_MS=30000`
...
- Defaults:
  ...
  - ACP backend: `acpx`
- Overrides:
  - `AFORA_LIVE_ACP_BIND_AGENT=claude`
  - `AFORA_LIVE_ACP_BIND_AGENT=codex`
  - `AFORA_LIVE_ACP_BIND_AGENT=droid`
  - `AFORA_LIVE_ACP_BIND_AGENT=gemini`
  - `AFORA_LIVE_ACP_BIND_AGENT=opencode`
  - `AFORA_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `AFORA_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
  - `AFORA_LIVE_ACP_BIND_CODEX_MODEL=gpt-5.6-luna`
  - `AFORA_LIVE_ACP_BIND_OPENCODE_MODEL=opencode/kimi-k2.6`
  - `AFORA_LIVE_ACP_BIND_IMAGE_PROBE=1` (or `on`/`true`/`yes`) to force the image probe on; any other value forces it off. Runs by default for every agent except `opencode`.
  - `AFORA_LIVE_ACP_BIND_REQUIRE_CRON=1`
  - `AFORA_LIVE_ACP_BIND_PARENT_MODEL=openai/gpt-5.6-luna`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `AFORA_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
  - Bound-session cron MCP creation is best-effort by default because external ACP harnesses can cancel MCP calls after the bind/image proof has passed; set `AFORA_LIVE_ACP_BIND_REQUIRE_CRON=1` to make that post-bind cron probe strict.
...
- Use `AFORA_LIVE_ACP_BIND_AGENTS=claude`, `AFORA_LIVE_ACP_BIND_AGENTS=codex`, `AFORA_LIVE_ACP_BIND_AGENTS=droid`, `AFORA_LIVE_ACP_BIND_AGENTS=gemini`, or `AFORA_LIVE_ACP_BIND_AGENTS=opencode` to narrow the matrix.
...
- The OpenCode Docker variant is a strict single-agent regression lane. It writes a temporary `OPENCODE_CONFIG_CONTENT` default model from `AFORA_LIVE_ACP_BIND_OPENCODE_MODEL` (default `opencode/kimi-k2.6`).
- Direct `acpx` CLI calls are only a manual/workaround path for comparing behavior outside the Gateway. The Docker ACP bind smoke exercises Afora's embedded `acpx` runtime backend.
...
- Goal: validate the plugin-owned Codex harness through the normal gateway
  ...
  - send a second turn to the same Afora session and verify the app-server
    thread can resume
...
- Enable: `AFORA_LIVE_CODEX_HARNESS=1`
- Harness baseline model: `openai/gpt-5.6-luna`
- Fresh OpenAI API-key selection default: `openai/gpt-5.6-sol`
- Default thinking: `low`
- Model override: `AFORA_LIVE_CODEX_HARNESS_MODEL=openai/<model>`
- Thinking override: `AFORA_LIVE_CODEX_HARNESS_THINKING=<level>`
- Non-default model effort assertion:
  `AFORA_LIVE_CODEX_HARNESS_EXPECTED_EFFORT=<level>`
- Matrix override: `AFORA_LIVE_CODEX_HARNESS_TARGETS=<model>=<thinking>,...`
- Auth mode: `AFORA_LIVE_CODEX_HARNESS_AUTH=codex-auth` (default) uses the
  copied Codex login; `api-key` uses `OPENAI_API_KEY` through Codex app-server.
- Optional image probe: `AFORA_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `AFORA_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- Optional Guardian probe: `AFORA_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`
- Optional resume stress: `AFORA_LIVE_CODEX_HARNESS_RESUME_STRESS=1` adds
  ...
  `AFORA_LIVE_CODEX_HARNESS_RESUME_STRESS_HISTORY_TURNS` (1-20) and
  `AFORA_LIVE_CODEX_HARNESS_RESUME_STRESS_RESTARTS` (1-10).
- Optional fan-out stress: set `AFORA_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1`
  and `AFORA_LIVE_CODEX_HARNESS_SUBAGENT_COUNT` (1-12). The harness starts
  every child concurrently, waits for every terminal run, and verifies each
  unique child reply and native thread identity.
- Optional compaction stress: `AFORA_LIVE_CODEX_HARNESS_COMPACTION_STRESS=1`
  ...
  `AFORA_LIVE_CODEX_HARNESS_COMPACTION_STRESS_TURNS` (1-8) and
  `AFORA_LIVE_CODEX_HARNESS_LARGE_OUTPUT_BYTES` (100000-800000).
- Full direct-API context: `AFORA_LIVE_CODEX_HARNESS_FULL_CONTEXT=1` applies
  ...
  requires
  `AFORA_LIVE_CODEX_HARNESS_AUTH=api-key` plus an absolute
  `AFORA_LIVE_CODEX_HARNESS_MODEL_CATALOG` path. The catalog must expose the
...
- Optional loop-relay opt-out probe:
  `AFORA_LIVE_CODEX_HARNESS_DISABLE_LOOP_RELAY=1`
...
- The smoke forces provider/model `agentRuntime.id: "codex"` so a broken Codex
  harness cannot pass by silently falling back to Afora.
- Auth: Codex app-server auth from the local Codex subscription login, or
  `OPENAI_API_KEY` when `AFORA_LIVE_CODEX_HARNESS_AUTH=api-key`. Docker can
  copy `~/.codex/auth.json` and `~/.codex/config.toml` for subscription runs.

Local recipe:

```bash
AFORA_LIVE_CODEX_HARNESS=1 \
  AFORA_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  AFORA_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  AFORA_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1 \
...
```bash
AFORA_LIVE_CODEX_HARNESS_RESUME_STRESS=1 \
pnpm test:docker:live-codex-harness
```
...
```bash
AFORA_LIVE_CODEX_HARNESS_AUTH=api-key \
  AFORA_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 \
  AFORA_LIVE_CODEX_HARNESS_SUBAGENT_COUNT=8 \
  AFORA_LIVE_CODEX_HARNESS_RESUME_STRESS=1 \
  AFORA_LIVE_CODEX_HARNESS_COMPACTION_STRESS=1 \
  pnpm test:docker:live-codex-harness
```
...
AFORA_LIVE_CODEX_HARNESS=1 \
  AFORA_LIVE_CODEX_HARNESS_AUTH=api-key \
  AFORA_LIVE_CODEX_HARNESS_FULL_CONTEXT=1 \
  AFORA_LIVE_CODEX_HARNESS_MODEL_CATALOG=/absolute/path/to/models-api-1m.json \
  AFORA_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.6-sol \
  AFORA_LIVE_CODEX_HARNESS_THINKING=low \
  AFORA_LIVE_CODEX_HARNESS_COMPACTION_STRESS_TURNS=8 \
  AFORA_LIVE_CODEX_HARNESS_LARGE_OUTPUT_BYTES=800000 \
  AFORA_LIVE_CODEX_HARNESS_DEBUG=1 \
...
```bash
AFORA_LIVE_CODEX_HARNESS_AUTH=api-key \
  AFORA_LIVE_CODEX_HARNESS_TARGETS='openai/gpt-5.6-sol=ultra,openai/gpt-5.6-terra=ultra,openai/gpt-5.6-luna=max' \
  pnpm test:docker:live-codex-harness
```

## Live: OpenAI long context

- Goal: validate exact-model embedded Afora execution through a
...
- Enable: `AFORA_LIVE_OPENAI_LONG_CONTEXT=1`
- Profiles: `AFORA_LIVE_OPENAI_LONG_CONTEXT_PROFILE=full` selects exact
...
- Metrics: `AFORA_LIVE_OPENAI_LONG_CONTEXT_METRICS=1` emits phase timing and
  token observations. These measurements are informational, not pass/fail
  latency targets.
- Long output: `AFORA_LIVE_OPENAI_LONG_CONTEXT_OUTPUT=1` requires a
  deterministic response between 4000 and 8000 output tokens.
- Optional raw read-tool stress:
  `AFORA_LIVE_OPENAI_LONG_CONTEXT_TOOL_OUTPUT=1`. It is not part of the
...
```bash
AFORA_LIVE_OPENAI_LONG_CONTEXT=1 \
  AFORA_LIVE_OPENAI_LONG_CONTEXT_PROFILE=full \
  AFORA_LIVE_OPENAI_LONG_CONTEXT_METRICS=1 \
  AFORA_LIVE_OPENAI_LONG_CONTEXT_OUTPUT=1 \
...
```bash
AFORA_LIVE_OPENAI_LONG_CONTEXT=1 \
  AFORA_LIVE_OPENAI_LONG_CONTEXT_PROFILE=reduced \
  AFORA_LIVE_OPENAI_LONG_CONTEXT_METRICS=1 \
  AFORA_LIVE_OPENAI_LONG_CONTEXT_OUTPUT=1 \
  node --import tsx scripts/test-live.mts --quiet src/gateway/gateway-openai-long-context.live.test.ts
```
...
- Runtime and model identity are exact: embedded Afora or native Codex as
...
- Embedded Afora receives and persists a first-class encrypted Responses
...
```bash
AFORA_LIVE_GATEWAY_OPENAI_API_DEFAULT=1 \
  AFORA_LIVE_GATEWAY_PROVIDERS=openai \
  AFORA_LIVE_GATEWAY_THINKING=off \
  pnpm test:live -- src/gateway/gateway-models.profiles.live.test.ts
```

This proof leaves `AFORA_LIVE_GATEWAY_MODELS` unset, resolves the model through
...
GPT-5.6 embedded Afora matrix:

```bash
AFORA_LIVE_GATEWAY_THINKING=ultra \
  AFORA_LIVE_GATEWAY_PROVIDERS=openai \
  AFORA_LIVE_GATEWAY_MODELS='openai/gpt-5.6-sol,openai/gpt-5.6-terra,openai/gpt-5.6-luna' \
  pnpm test:live -- src/gateway/gateway-models.profiles.live.test.ts
```
...
- Docker enables the image, MCP/tool, and Guardian probes by default. Set
  `AFORA_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `AFORA_LIVE_CODEX_HARNESS_MCP_PROBE=0` or
  `AFORA_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0` when you need a narrower debug
  run.
- Docker uses the same explicit Codex runtime config, so legacy aliases or Afora
...
- Single model, direct (no gateway):
  - `AFORA_LIVE_MODELS="openai/gpt-5.6-luna" pnpm test:live src/agents/models.profiles.live.test.ts`

- Small-model direct profile:
  - `AFORA_LIVE_MODELS=small pnpm test:live src/agents/models.profiles.live.test.ts`

- Small-model gateway profile:
  - `AFORA_LIVE_GATEWAY_MODELS=small pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Ollama Cloud API smoke:
  - `AFORA_LIVE_TEST=1 AFORA_LIVE_OLLAMA=1 AFORA_LIVE_OLLAMA_BASE_URL=https://ollama.com AFORA_LIVE_OLLAMA_MODEL=glm-5.1:cloud AFORA_LIVE_OLLAMA_WEB_SEARCH=0 pnpm test:live -- extensions/ollama/ollama.live.test.ts`

- Single model, gateway smoke:
  - `AFORA_LIVE_GATEWAY_MODELS="openai/gpt-5.6-luna" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `AFORA_LIVE_GATEWAY_MODELS="openai/gpt-5.6-luna,anthropic/claude-opus-4-6,google/gemini-3.5-flash,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M3" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Z.AI Coding Plan GLM-5.3 direct smoke:
  - `ZAI_CODING_LIVE_TEST=1 pnpm test:live src/agents/zai.live.test.ts`

- Google focus:
  - Gemini (API key): `AFORA_LIVE_GATEWAY_MODELS="google/gemini-3.5-flash" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google adaptive thinking smoke (`qa manual` from the private QA CLI - requires `AFORA_ENABLE_PRIVATE_QA_CLI=1` and a source checkout; see [QA overview](/concepts/qa-e2e-automation)):
  - Gemini 3 dynamic default: `AFORA_ENABLE_PRIVATE_QA_CLI=1 pnpm afora qa manual --provider-mode live-frontier --model google/gemini-3.1-pro-preview --alt-model google/gemini-3.1-pro-preview --message '/think adaptive Reply exactly: GEMINI_ADAPTIVE_OK' --timeout-ms 180000`
  - Gemini 2.5 dynamic budget: `AFORA_ENABLE_PRIVATE_QA_CLI=1 pnpm afora qa manual --provider-mode live-frontier --model google/gemini-2.5-flash --alt-model google/gemini-2.5-flash --message '/think adaptive Reply exactly: GEMINI25_ADAPTIVE_OK' --timeout-ms 180000`
...
- Gemini API vs Gemini CLI:
  - API: Afora calls Google's hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by "Gemini".
  - CLI: Afora shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).

## Live: model matrix (what we cover)

Live is opt-in, so there is no fixed "CI model list." `AFORA_LIVE_MODELS=modern` / `AFORA_LIVE_GATEWAY_MODELS=modern` (and their `all` alias) run the curated priority list from `HIGH_SIGNAL_LIVE_MODEL_PRIORITY` in `src/agents/live-model-filter.ts`, in this priority order:
...
The curated **small-model** list (`AFORA_LIVE_MODELS=small` / `AFORA_LIVE_GATEWAY_MODELS=small`), from `SMALL_LIVE_MODEL_PRIORITY`:
...
- Include at least one image-capable model (Claude/Gemini/OpenAI-family vision variants, etc.) in `AFORA_LIVE_GATEWAY_MODELS` to exercise the image probe.

Run gateway smoke with tools + image across a hand-picked cross-provider set:

```bash
AFORA_LIVE_GATEWAY_MODELS="openai/gpt-5.6-luna,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3.5-flash,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M3" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts
```
...
- OpenRouter: `openrouter/...` (hundreds of models; use `afora models scan` to find tool+image capable candidates)
...
Docker runners below with an explicit `AFORA_PROFILE_FILE`.
...
- Enable: `AFORA_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `xai`
- Optional narrowing:
  - `AFORA_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google,openrouter,xai"`
  - `AFORA_LIVE_IMAGE_GENERATION_PROVIDERS="deepinfra"`
  - `AFORA_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-2,google/gemini-3.1-flash-image,openrouter/google/gemini-3.1-flash-image-preview,xai/grok-imagine-image"`
  - `AFORA_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit,openrouter:generate,xai:default-generate,xai:default-edit"`
- Optional auth behavior:
  - `AFORA_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

For the shipped CLI path, add an `infer` smoke after the provider/runtime live
test passes:

```bash
AFORA_LIVE_TEST=1 AFORA_LIVE_INFER_CLI_TEST=1 pnpm test:live -- test/image-generation.infer-cli.live.test.ts
afora infer image providers --json
afora infer image generate \
  --model google/gemini-3.1-flash-image \
  --prompt "Minimal flat test image: one blue square on a white background, no text." \
  --output ./afora-infer-image-smoke.png \
  --json
```
...
- Enable: `AFORA_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `AFORA_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `AFORA_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.6"`
- Optional auth behavior:
  - `AFORA_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `AFORA_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
...
- Scope:
  ...
  - Defaults to the release-safe smoke path: one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `AFORA_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `AFORA_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` (or clear the skip list) to run it explicitly
  - Uses already-exported provider env vars before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in `auth-profiles.json` do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `AFORA_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `AFORA_LIVE_TEST=1 AFORA_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
    - That file runs `veo3` text-to-video plus a `kling` image-to-video lane that uses a remote image URL fixture by default (`AFORA_LIVE_VYDRA_KLING_IMAGE_URL` to override).
  - Provider-specific xAI coverage:
    - `AFORA_LIVE_TEST=1 AFORA_LIVE_XAI_VIDEO=1 pnpm test:live -- extensions/xai/xai.live.test.ts -t "classic Grok Imagine"`
    ...
    - `AFORA_LIVE_TEST=1 AFORA_LIVE_XAI_VIDEO=1 pnpm test:live -- extensions/xai/xai.live.test.ts -t "Grok Imagine Video 1.5"`
...
- Optional narrowing:
  - `AFORA_LIVE_VIDEO_GENERATION_PROVIDERS="deepinfra,google,openai,runway"`
  - `AFORA_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `AFORA_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `AFORA_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `AFORA_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
