...
Afora has three Vitest suites (unit/integration, e2e, live) plus Docker
...
- Linux VM-backed QA lane: `pnpm afora qa suite --runner multipass --scenario channel-chat-baseline`
...
it("uses a temp workspace", () => {
  const workspace = tempDirs.make("afora-example-");
  // use workspace
});
```
...
```ts
// afora-temp-dir: allow verifies raw fs cleanup behavior
...
- Runtime performance reports: dispatch `Afora Performance` with
  ...
  `afora/clawgrit-reports` from a separate artifact-consuming publisher job;
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `AFORA_LIVE_MODEL_FILE_PROBE=0` or
    `AFORA_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `Afora Scheduled Live And E2E Checks` and manual
    `Afora Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes Docker live model matrix jobs
    sharded by provider.
  - For focused CI reruns, dispatch `Afora Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/afora-live-and-e2e-checks-reusable.yml` and its
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server
    ...
    sub-agent probe with `AFORA_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when
    isolating other failures. For a focused sub-agent check, disable the
    other probes:
    `AFORA_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 AFORA_LIVE_CODEX_HARNESS_MCP_PROBE=0 AFORA_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 AFORA_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `AFORA_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
- Codex on-demand install smoke: `pnpm test:docker:codex-on-demand`
  - Installs the packaged Afora tarball in Docker, runs OpenAI API-key
...
- Codex npm-plugin live package smoke: `pnpm test:docker:live-codex-npm-plugin`
  - Installs the candidate Afora package and exact Codex plugin into Docker,
...
- Afora rescue command smoke: `pnpm test:live:system-agent-rescue-channel`
  - Opt-in belt-and-suspenders check for the message-channel rescue command
    surface. Exercises `/afora status`, queues a persistent model
    change, replies `/afora yes`, and verifies the audit/config write
    path.
- Afora first-run Docker smoke: `pnpm test:docker:system-agent-first-run`
  - Starts from an empty Afora state dir and first proves the packaged
    `afora setup` CLI fails closed without inference. It then
    ...
    Afora agent/tool/approval proof. The same lane is exposed in QA Lab by
    `pnpm afora qa suite --scenario system-agent-ring-zero-setup`.
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `afora models list --provider moonshot --json`, then run an isolated
  `afora agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
`ghcr.io/AforaMosh/afora-agent-live-media-runner:ubuntu-24.04`, which already has
`ffmpeg` and `ffprobe`. Docker live model/backend shards use the shared
`ghcr.io/AforaMosh/afora-agent-live-test:<sha>` image built once per selected
commit, then pull it with `AFORA_SKIP_DOCKER_BUILD=1` instead of rebuilding
inside every shard.
...
- `pnpm afora qa suite`
  ...
  - When dispatched by `pnpm afora qa run --qa-profile <profile>`, embeds
  ...
  - Supports provider modes `live-frontier`, `mock-openai`, and `aimock`.
    ...
    `mock-openai` lane.
- `pnpm afora qa coverage --match <query>`
...
- `pnpm test:plugins:kitchen-sink-live`
  - Runs the live OpenAI Kitchen Sink plugin gauntlet through QA Lab.
    ...
    profile when the `afora-testbox-env` helper is present.
...
- `pnpm afora qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  ...
  - Use `AFORA_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. Verifies hidden Afora runtime context persists as a
    ...
    `afora doctor --fix` rewrites it to the active branch with a backup.
- `pnpm test:docker:npm-telegram-live`
  - Installs an Afora package candidate in Docker, runs installed-package
  ...
  - Defaults to `AFORA_NPM_TELEGRAM_PACKAGE_SPEC=afora@beta`; set
    `AFORA_NPM_TELEGRAM_PACKAGE_TGZ=/path/to/afora-current.tgz` or
    `AFORA_CURRENT_PACKAGE_TGZ` to test a resolved local tarball instead
    of installing from the registry.
  - Emits repeated RTT timing in `qa-evidence.json` by default with
    `AFORA_NPM_TELEGRAM_RTT_SAMPLES=20`. Override
    `AFORA_NPM_TELEGRAM_RTT_SAMPLES`,
    `AFORA_NPM_TELEGRAM_RTT_TIMEOUT_MS`, or
    `AFORA_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune the run.
    `AFORA_NPM_TELEGRAM_RTT_CHECKS` selects the Telegram QA scenario to
  ...
  - Uses the same Telegram env credentials or Convex credential source as
    `pnpm afora qa telegram`. For CI/release automation, set
    `AFORA_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `AFORA_QA_CONVEX_SITE_URL` and a role secret. If
    `AFORA_QA_CONVEX_SITE_URL` and a Convex role secret are present in
  ...
  - The wrapper validates Telegram or Convex credential env on the host
    ...
    `AFORA_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT=1` only when
    deliberately debugging pre-credential setup.
  - `AFORA_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the
    shared `AFORA_QA_CREDENTIAL_ROLE` for this lane only. When Convex
...
- GitHub Actions also exposes `Package Acceptance` for side-run product proof
  ...
  normalized `afora-current.tgz` as `package-under-test`, then runs the
...
gh workflow run package-acceptance.yml --ref main \
  -f source=url \
  -f package_url=https://registry.npmjs.org/afora/-/afora-VERSION.tgz \
...
`source=trusted-url` reads `.github/package-trusted-sources.json` from the trusted workflow ref and does not accept URL credentials or a workflow-input private-network bypass. If the named policy declares bearer auth, configure the fixed `AFORA_TRUSTED_PACKAGE_TOKEN` secret.
...
- `pnpm test:docker:plugins`
  - Packs and installs the current Afora build in Docker, starts the
  ...
  - Also installs a known older npm baseline, enables Telegram before
    running `afora update --tag <candidate>`, and verifies the
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests.
    ...
    then runs the installed `afora update` command in the same guest and
  ...
  - The OpenAI lane uses `openai/gpt-5.6-luna` for the live agent-turn proof by
    ...
    `AFORA_PARALLELS_OPENAI_MODEL` to validate another OpenAI model.
  ...
  - The script writes nested lane logs under
    `/tmp/afora-parallels-npm-update.*`. Inspect `windows-update.log`,
...
- `pnpm afora qa telegram`
  - Runs the Telegram live QA lane against a real private group using the
    driver and SUT bot tokens from env.
  - Requires `AFORA_QA_TELEGRAM_GROUP_ID`,
    `AFORA_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
    `AFORA_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric
    Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials.
    Use env mode by default, or set `AFORA_QA_CREDENTIAL_SOURCE=convex`
...
```text
@afora-mantis telegram
@afora-mantis telegram scenario=telegram-status-command
@afora-mantis telegram scenarios=telegram-status-command,channel-canary
```
...
```text
@afora-mantis telegram desktop proof
```
...
- `pnpm afora qa mantis telegram-desktop-builder`
  - Leases or reuses a Crabbox Linux desktop, installs native Telegram
    Desktop, configures Afora with a leased Telegram SUT bot token,
  ...
  - Defaults to `--credential-source convex` so workflows only need the
    ...
    `AFORA_QA_TELEGRAM_*` variables as `pnpm afora qa telegram`.
  - Telegram Desktop still needs a user login/profile. The bot token
    configures Afora only. Use `--telegram-profile-archive-env <name>`
...
When `--credential-source convex` (or `AFORA_QA_CREDENTIAL_SOURCE=convex`)
...
- `AFORA_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `AFORA_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `AFORA_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `AFORA_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `AFORA_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `AFORA_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `AFORA_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `AFORA_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `AFORA_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `AFORA_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `AFORA_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`AFORA_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`AFORA_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm afora qa credentials doctor
pnpm afora qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm afora qa credentials list --kind telegram
pnpm afora qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`AFORA_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`).
...
plugin manifest, mount as `afora qa <runner>`, and author scenarios under
`qa/scenarios/`.
...
- Compare native opus performance in the `libopus-wasm` benchmark repo, not
  in default Afora install/test loops. Do not set `@discordjs/opus` to
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `AFORA_TEST_CHANGED_BROAD=1 pnpm test:changed`.
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-afora-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `AFORA_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
    - `scripts/run-vitest.mjs` terminates explicit non-watch Vitest runs
      after 5 minutes with no stdout or stderr output. Set
      `AFORA_VITEST_NO_OUTPUT_TIMEOUT_MS=0` to disable the watchdog for
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `AFORA_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `AFORA_VITEST_FS_MODULE_CACHE` enabled on
      supported hosts; set `AFORA_VITEST_FS_MODULE_CACHE_PATH=/abs/path`
...
- Useful overrides:
  - `AFORA_E2E_WORKERS=<n>` to opt into parallel workers (capped at 16).
  - `AFORA_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Afora's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `AFORA_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `AFORA_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
  - `AFORA_E2E_OPENSHELL_CONFIG_HOME=/path/to/config` to expose the registered gateway config to the isolated test
  - `AFORA_E2E_OPENSHELL_HOST_IP=172.18.0.1` to override the Docker gateway IP used by the host policy fixture
...
- Default: **enabled** by `pnpm test:live` (sets `AFORA_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.afora`.
- Set `AFORA_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` defaults to a quieter mode: it keeps `[live] ...` progress output and mutes gateway bootstrap logs/Bonjour chatter. Set `AFORA_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `AFORA_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `AFORA_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `AFORA_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners keep their own practical caps where needed:
  ...
  `test:docker:live-gateway` defaults to `AFORA_LIVE_GATEWAY_SMOKE=1`,
  `AFORA_LIVE_GATEWAY_MAX_MODELS=8`,
  `AFORA_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `AFORA_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Set `AFORA_LIVE_MAX_MODELS`
  or the gateway env vars when you explicitly want a smaller cap or larger scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs Afora once as an npm tarball through `scripts/package-afora-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mts`; planner logic lives in `scripts/lib/docker-e2e-plan.mts`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `AFORA_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `AFORA_DOCKER_ALL_LIVE_LIMIT=9`, `AFORA_DOCKER_ALL_NPM_LIMIT=5`, and `AFORA_DOCKER_ALL_SERVICE_LIMIT=7`; tune `AFORA_DOCKER_ALL_WEIGHT_LIMIT` or `AFORA_DOCKER_ALL_DOCKER_LIMIT` (and other `AFORA_DOCKER_ALL_<RESOURCE>_LIMIT` overrides) only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale Afora E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `AFORA_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Docker/Bash E2E lanes that install the packed Afora tarball through `scripts/lib/afora-e2e-instance.sh` cap `npm install` at `AFORA_E2E_NPM_INSTALL_TIMEOUT` (default `600s`; set `0` to disable the wrapper for debugging).
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed Afora tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `AFORA_CURRENT_PACKAGE_TGZ=/path/to/afora-*.tgz`, skip the host rebuild with `AFORA_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `AFORA_NPM_ONBOARD_CHANNEL=discord` or `AFORA_NPM_ONBOARD_CHANNEL=slack`.

- Release user journey smoke: `pnpm test:docker:release-user-journey` installs the packed Afora tarball globally in a clean Docker home, runs onboarding, configures a mocked OpenAI provider, runs an agent turn, installs/uninstalls external plugins, configures ClickClack against a local fixture, verifies outbound/inbound messaging, restarts Gateway, and runs doctor.
- Release typed onboarding smoke: `pnpm test:docker:release-typed-onboarding` installs the packed tarball, drives `afora onboard` through a real TTY, configures OpenAI as an env-ref provider, verifies no raw key persistence, and runs a mocked agent turn.
...
- Release upgrade user journey smoke: `pnpm test:docker:release-upgrade-user-journey` installs the newest published baseline older than the candidate tarball by default, configures provider/plugin/ClickClack state on the published package, upgrades to the candidate tarball, then reruns the core agent/plugin/channel journey. If no older published baseline exists, it reuses the candidate version. Override the baseline with `AFORA_RELEASE_UPGRADE_BASELINE_SPEC=afora@<version>`.
...
- Skill install smoke: `pnpm test:docker:skill-install` installs the packed Afora tarball globally in Docker, disables uploaded archive installs in config, resolves the current live ClawHub skill slug from search, installs it with `afora skills install`, and verifies the installed skill plus `.clawhub` origin/lock metadata.
- Update channel switch smoke: `pnpm test:docker:update-channel-switch` installs the packed Afora tarball globally in Docker, switches from package `stable` to git `dev`, verifies the persisted channel and plugin post-update work, then switches back to package `stable` and checks update status.
- Upgrade survivor smoke: `pnpm test:docker:upgrade-survivor` installs the packed Afora tarball over a dirty old-user fixture with agents, channel config, plugin allowlists, stale plugin dependency state, and existing workspace/session files. It runs package update plus non-interactive doctor without live provider or channel keys, then starts a loopback Gateway and checks config/state preservation plus startup/status budgets.
- Published upgrade survivor smoke: `pnpm test:docker:published-upgrade-survivor` installs `afora@latest` by default, seeds realistic existing-user files, configures that baseline with a baked command recipe, validates the resulting config, updates that published install to the candidate tarball, runs non-interactive doctor, writes `.artifacts/upgrade-survivor/summary.json`, then starts a loopback Gateway and checks configured intents, state preservation, startup, `/healthz`, `/readyz`, and RPC status budgets. Override one baseline with `AFORA_UPGRADE_SURVIVOR_BASELINE_SPEC`, ask the aggregate scheduler to expand exact local baselines with `AFORA_UPGRADE_SURVIVOR_BASELINE_SPECS` such as `afora@2026.5.2 afora@2026.4.23 afora@2026.4.15`, and expand issue-shaped fixtures with `AFORA_UPGRADE_SURVIVOR_SCENARIOS` such as `reported-issues`; the reported-issues set includes `configured-plugin-installs` for automatic external Afora plugin install repair. Package Acceptance exposes those as `published_upgrade_survivor_baseline`, `published_upgrade_survivor_baselines`, and `published_upgrade_survivor_scenarios`, resolves meta baseline tokens such as `last-stable-4` or `all-since-2026.4.23`, and Full Release Validation expands the release-soak package gate to `last-stable-4 2026.4.23 2026.5.2 2026.4.15` plus `reported-issues`.
...
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `afora infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `AFORA_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/afora-*.tgz`, skip the host build with `AFORA_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `AFORA_BUN_GLOBAL_SMOKE_DIST_IMAGE=afora-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Override with `AFORA_INSTALL_SMOKE_UPDATE_BASELINE=2026.4.22` locally, or with the Install Smoke workflow's `update_baseline_version` input on GitHub. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `AFORA_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `AFORA_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `AFORA_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=afora-dockerfile-smoke:local AFORA_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- Afora bundle MCP tools (real stdio MCP server + embedded Afora profile allow/deny smoke): `pnpm test:docker:agent-bundle-mcp-tools` (script: `scripts/e2e/agent-bundle-mcp-tools-docker.sh`)
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, malformed npm package metadata, git moving refs, ClawHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `AFORA_PLUGINS_E2E_CLAWHUB=0` to skip the ClawHub block, or override the default kitchen-sink package/runtime pair with `AFORA_PLUGINS_E2E_CLAWHUB_SPEC` and `AFORA_PLUGINS_E2E_CLAWHUB_ID`. Without `AFORA_CLAWHUB_URL`/`CLAWHUB_URL`, the test uses a hermetic local ClawHub fixture server.
- Plugin update unchanged smoke: `pnpm test:docker:plugin-update` (script: `scripts/e2e/plugin-update-unchanged-docker.sh`)
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed Afora tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
...
```bash
AFORA_DOCKER_E2E_IMAGE=afora-docker-e2e-functional:local pnpm test:docker:e2e-build
AFORA_DOCKER_E2E_IMAGE=afora-docker-e2e-functional:local AFORA_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `AFORA_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `AFORA_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
outputs such as `.pnpm-store`, `.worktrees`, `__afora_vitest__`, and
...
`AFORA_SKIP_CHANNELS=1` so gateway live probes do not start real
...
`AFORA_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.

`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Afora gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `afora/default`, then sends a
...
container, starts a second container that spawns `afora mcp serve`, then
...
embedded Afora bundle MCP runtime, executes the tool, then verifies
...
- `AFORA_CONFIG_DIR=...` (default: `~/.afora`) mounted to `/home/node/.afora`
- `AFORA_WORKSPACE_DIR=...` (default: `~/.afora/workspace`) mounted to `/home/node/.afora/workspace`
- `AFORA_PROFILE_FILE=...` mounted and sourced before running tests
- `AFORA_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `AFORA_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `AFORA_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/afora/docker-cli-tools`, unless the run already uses a CI/managed bind dir) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `AFORA_LIVE_PROVIDERS` / `AFORA_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `AFORA_DOCKER_AUTH_DIRS=all`, `AFORA_DOCKER_AUTH_DIRS=none`, or a comma list like `AFORA_DOCKER_AUTH_DIRS=.claude,.codex`
- `AFORA_LIVE_GATEWAY_MODELS=...` / `AFORA_LIVE_MODELS=...` to narrow the run
- `AFORA_LIVE_GATEWAY_PROVIDERS=...` / `AFORA_LIVE_PROVIDERS=...` to filter providers in-container
- `AFORA_SKIP_DOCKER_BUILD=1` to reuse an existing `afora:local-live` image for reruns that do not need a rebuild
- `AFORA_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `AFORA_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `AFORA_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
