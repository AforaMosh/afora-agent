...
- Prefer CLI surfaces when they exist (example: `afora models status --check`).
...
- `scripts/claude-auth-status.sh [full|json|simple]` - checks Claude Code + Afora auth status.
- `scripts/auth-monitor.sh` - polls status and sends a notification (via Afora send, and/or ntfy.sh) when the token nears expiry. Env: `WARN_HOURS` (default `2`), `NOTIFY_PHONE`, `NOTIFY_NTFY`. Run on a schedule via the bundled `scripts/systemd/afora-auth-monitor.{service,timer}` (every 30 minutes).
...
- `AFORA_GH_READ_APP_ID`
- `AFORA_GH_READ_PRIVATE_KEY_FILE`

Optional env:

- `AFORA_GH_READ_INSTALLATION_ID` when you want to skip repo-based installation lookup
- `AFORA_GH_READ_PERMISSIONS` as a comma-separated override for the read permission subset to request
...
- `scripts/gh-read run list -R AforaMosh/afora-agent`
- `scripts/gh-read api repos/AforaMosh/afora-agent/pulls/123`
