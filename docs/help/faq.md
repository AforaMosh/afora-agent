---
summary: "Frequently asked questions about Afora setup, configuration, and usage"
read_when:
...
<Steps>
  ...
  <Step title="Gateway snapshot (WS-only)">
    ```bash
    afora health --json
    afora health --verbose   # shows the target URL + config path on errors
    ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for Afora?">
  ...
  <Accordion title="Can Afora help with lead gen, outreach, ads, and blogs for a SaaS?">
    ...
    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and platform policies, and review anything before it sends. Let Afora draft; you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Afora is a **personal assistant** and coordination layer, not an IDE replacement. Use Claude Code or Codex for the fastest direct coding loop inside a repo. Use Afora for durable memory, cross-device access, and tool orchestration.
    ...
    Showcase: [https://afora.ai/showcase](https://afora.ai/showcase).
...
<AccordionGroup>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Afora falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is exactly `NO_REPLY` / `no_reply` or `ANNOUNCE_SKIP`, Afora intentionally suppresses the announce instead of posting stale earlier progress.

    Debug: `afora tasks show <lookup>` where `<lookup>` is a task id, run id, or session key.
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `AFORA_SKIP_CRON` is not set.
    - Confirm the Gateway is running 24/7 (no sleep/restarts).
    - Verify job timezone (`--tz` vs host timezone).

    Debug:
    ```bash
    afora cron run <jobId>
    afora cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `afora skills` commands or drop skills into your workspace; the macOS Skills UI is not available on Linux. Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    afora skills search "calendar"
    afora skills search --limit 20
    afora skills install @owner/<skill-slug>
    afora skills install @owner/<skill-slug> --version <version>
    afora skills install @owner/<skill-slug> --force
    afora skills install @owner/<skill-slug> --global
    afora skills update --all
    afora skills update --all --global
    afora skills list --eligible
    afora skills check
    ```

    Native `afora skills install` writes into the active workspace `skills/` directory by default. Add `--global` to install into the shared managed skills directory for all local agents. Install the separate `clawhub` CLI only to publish or sync your own skills. Use `agents.defaults.skills` or `agents.entries.*.skills` to narrow which agents see shared skills.

  </Accordion>

  <Accordion title="Can Afora run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.afora.os` plus required binaries, and only load when eligible on the **Gateway host**. On Linux, `darwin`-only skills (`apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH)**. Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Afora treats macOS-only skills as eligible when required binaries exist on the node; the agent runs them via the `nodes` tool. With "Always Ask," approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced)**. Keep the Gateway on Linux, but make the required CLI binaries resolve to SSH wrappers that run on a Mac, then override the skill to allow Linux so it stays eligible.
    ...
    3. Override the skill metadata (workspace or `~/.afora/skills`) to allow Linux:
       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "afora": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="How do I use my existing signed-in Chrome with Afora?">
    ...
    Current limits on `existing-session` / `user` profiles versus the managed `afora` profile:
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `AFORA_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `AFORA_IMAGE_APT_PACKAGES`.
    - Bake Playwright Chromium and its system dependencies into the image with `AFORA_INSTALL_BROWSER=1`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Afora validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor, so symlink-parent escapes fail closed even when the final path segment does not exist yet.
  ...
  <Accordion title="How does memory work?">
    Afora memory is Markdown files in the agent workspace: daily notes in `memory/YYYY-MM-DD.md`, curated long-term notes in `MEMORY.md` (main/private sessions only).

    Afora also runs a silent **pre-compaction memory flush** before compaction summarizes the conversation, reminding the model to write durable notes first. It only runs when the workspace is writable (read-only sandboxes skip it); disable with `agents.defaults.compaction.memoryFlush.enabled: false`. See [Memory](/concepts/memory).
...
<AccordionGroup>
  <Accordion title="Is all data used with Afora saved locally?">
    No: **Afora's own state is local**, but **external services still see what you send them**.

    - **Local by default**: sessions, memory files, config, and workspace live on the Gateway host (`~/.afora` plus your workspace directory).
  ...
  <Accordion title="Where does Afora store its data?">
    Everything lives under `$AFORA_STATE_DIR` (default: `~/.afora`):

    | Path                                                               | Purpose                                                            |
    | ------------------------------------------------------------------ | ------------------------------------------------------------------ |
    | `$AFORA_STATE_DIR/afora.json`                                 | Main config (JSON5)                                                 |
    | `$AFORA_STATE_DIR/credentials/oauth.json`                        | Legacy OAuth import (copied into auth profiles on first use)        |
    | `$AFORA_STATE_DIR/agents/<agentId>/agent/auth-profiles.json`     | Auth profiles (OAuth, API keys, optional `keyRef`/`tokenRef`)        |
    | `$AFORA_STATE_DIR/secrets.json`                                  | Optional file-backed secret payload for `file` SecretRef providers   |
    | `$AFORA_STATE_DIR/agents/<agentId>/agent/auth.json`              | Legacy compatibility file (static `api_key` entries scrubbed)        |
    | `$AFORA_STATE_DIR/credentials/`                                  | Provider state (for example `whatsapp/<accountId>/creds.json`)      |
    | `$AFORA_STATE_DIR/agents/`                                       | Per-agent state (agentDir + legacy/archive session artifacts)        |
    | `$AFORA_STATE_DIR/agents/<agentId>/agent/afora-agent.sqlite`  | Per-agent SQLite state, including session rows and transcripts      |
    | `$AFORA_STATE_DIR/agents/<agentId>/sessions/`                    | Legacy session migration sources and archive/support artifacts      |

    Legacy single-agent path `~/.afora/agent/*` is migrated by `afora doctor`.

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate, configured via `agents.defaults.workspace` (default: `~/.afora/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These live in the **agent workspace**, not `~/.afora`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`, `MEMORY.md`, `memory/YYYY-MM-DD.md`. Lowercase root `memory.md` is legacy repair input only; `afora doctor --fix` can merge it into `MEMORY.md` when both exist.
    - **State dir (`~/.afora`)**: config, channel/provider state, auth profiles, sessions, logs, shared skills (`~/.afora/skills`).

    Default workspace is `~/.afora/workspace`, configurable:
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.afora` (credentials, sessions, tokens, encrypted secrets payloads). For a full restore, back up the workspace and state directory separately.

    Docs: [Agent workspace](/concepts/agent-workspace).

  </Accordion>

  <Accordion title="How do I completely uninstall Afora?">
    See [Uninstall](/install/uninstall).
  </Accordion>

  <Accordion title="Can agents work outside the workspace?">
    Yes. The workspace is the **default cwd** and memory anchor, not a hard sandbox. Relative paths resolve inside the workspace; absolute paths can access other host locations unless sandboxing is enabled. For isolation, use [`agents.defaults.sandbox`](/gateway/sandboxing) or per-agent sandbox settings. To make a repo the default working directory, point that agent's `workspace` at the repo root - the Afora repo itself is just source code, so keep the workspace separate unless you intentionally want the agent to work inside it.
...
<AccordionGroup>
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `AFORA_GATEWAY_PASSWORD`).
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Afora enforces gateway auth by default, including loopback. If no explicit auth path is configured, startup resolves to token mode and generates a runtime-only token for that startup, so local WS clients must authenticate. This blocks other local processes from calling the Gateway.

    On a fresh loopback start, the Gateway prepares the canonical same-user CLI device credential before `/readyz`, so normal `afora` CLI calls can authenticate without persisting the generated token. Other clients still need an explicit shared secret or an approved device pairing.

    Configure `gateway.auth.token`, `gateway.auth.password`, `AFORA_GATEWAY_TOKEN`, or `AFORA_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. You can also choose password mode, or `trusted-proxy` for identity-aware reverse proxies. For open loopback, set `gateway.auth.mode: "none"` explicitly. `afora doctor --generate-gateway-token` generates a token any time.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    Grok can also reuse xAI OAuth from model auth (`afora onboard --auth-choice xai-oauth`).

    **Recommended**: `afora configure --section web` and pick a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Afora auto-detects the first ready fetch fallback provider from available credentials; the official Firecrawl plugin provides that fallback.
    - Daemons read env vars from `~/.afora/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Afora protects most accidental clobbers:

    - Afora-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Afora-owned writes are rejected and saved as `afora.json.rejected.*`.
    - A direct edit that breaks startup or hot reload makes the Gateway fail closed or skip the reload; it does not rewrite `afora.json`.
    - `afora doctor --fix` owns repair, can restore last-known-good, and saves the rejected file as `afora.json.clobbered.*`.

    Recover:

    - Check `afora logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `afora.json.clobbered.*` or `afora.json.rejected.*` beside the active config.
    - Run `afora config validate` and `afora doctor --fix`.
    - Copy only the intended keys back with `afora config set` or `config.patch`.
    - No last-known-good or rejected payload: restore from backup, or re-run `afora doctor` and reconfigure channels/models.
  ...
  <Accordion title="Can the Afora browser run headless?">
    Yes:
...
<AccordionGroup>
  ...
  <Accordion title="Can two Afora instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic)**: run a script that calls the other Gateway with `afora agent --message ... --deliver`, targeting a chat where the other bot listens. If one bot is on a remote VPS, point your CLI at that remote Gateway via SSH/Tailscale (see [Remote access](/gateway/remote)):

    ```bash
    afora agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
...
<AccordionGroup>
  <Accordion title="How does Afora load environment variables?">
    Afora reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory.
    - a global fallback `.env` from `~/.afora/.env` (`$AFORA_STATE_DIR/.env`).

    Normally, neither `.env` file overrides existing env vars. For an Afora-installed systemd service, the global `.env` may replace only service values that Afora recorded as managed; operator-owned service values still take precedence. Provider credential and endpoint-routing keys are an exception for workspace `.env`: keys such as `GEMINI_API_KEY`, `XAI_API_KEY`, `MISTRAL_API_KEY`, or any key ending in `_ENDPOINT` (and other bundled-provider auth or endpoint env vars) are ignored from workspace `.env` and should live in the process environment, `~/.afora/.env`, or config `env.vars`.
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.afora/.env` so they load even when the service does not inherit your shell env.
    2. Enable shell import (opt-in convenience):
       ...
       ```
       This runs your login shell and imports only missing expected keys (never overrides). Env var equivalents: `AFORA_LOAD_SHELL_ENV=1`, `AFORA_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `afora models status` reports whether **shell env import** is enabled. "Shell env: off" does **not** mean your env vars are missing - it just means Afora will not load your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it will not inherit your shell environment. Fix by putting the token in `~/.afora/.env`, enabling `env.shellEnv.enabled: true`, or adding it to config `env` (applies only if missing), then restarting the gateway and rechecking:

    ```bash
    afora models status
    ```

    Copilot tokens resolve in this order: `AFORA_GITHUB_TOKEN`, then `COPILOT_GITHUB_TOKEN`, then `GH_TOKEN`, then `GITHUB_TOKEN`.
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Afora instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Afora but keep it installed?">
    ```bash
    afora reset
    ```
    ...
    ```bash
    afora reset --scope full --yes --non-interactive
    ```
    ...
    To reset and immediately re-run onboarding, pass `afora onboard --reset`; reset is a command flag, not a **Setup mode** menu choice. See [Onboarding (CLI)](/start/wizard). If you used profiles (`--profile` / `AFORA_PROFILE`), reset each state dir (default `~/.afora-<profile>`). Dev-only reset: `afora gateway --dev --reset` wipes dev config, credentials, sessions, and workspace.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Afora runs on **your own account** - if you are in the group, Afora can see it. By default, group replies are blocked until you allow senders (`groupPolicy: "allowlist"`).
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    afora directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Afora not reply in a group?">
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth**: active sessions and transcripts live in the per-agent SQLite database; legacy/archive artifacts can still accumulate under `~/.afora/agents/<agentId>/sessions/`.
    - **Token cost**: more agents means more concurrent model usage.
    - **Ops overhead**: per-agent auth profiles, workspaces, and channel routing.

    Keep one **active** workspace per agent (`agents.defaults.workspace`), prune old sessions with `afora sessions cleanup` if disk grows (do not edit active SQLite state by hand), and use `afora doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```text
    --port > AFORA_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does afora gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    "Running" is the **supervisor's** view (launchd/systemd/schtasks); the connectivity probe is the CLI actually connecting to the gateway WebSocket. Trust these lines from `afora gateway status`: `Probe target:` (the URL the probe used), `Listening:` (what is actually bound on the port), `Last gateway error:` (common root cause when the process is alive but the port is not listening).
  </Accordion>

  <Accordion title='Why does afora gateway status show "Config (cli)" and "Config (service)" different?'>
    You are editing one config file while the service runs another (often a `--profile` / `AFORA_STATE_DIR` mismatch).
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Afora enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` ("another gateway instance is already listening").

    Fix: stop the other instance, free the port, or run with `afora gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Afora in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `afora gateway` only starts when `gateway.mode` is `local` (or you pass an override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `afora dashboard` (prints + copies the dashboard URL, tries to open; shows an SSH hint if headless).
    - No token yet: `afora doctor --generate-gateway-token`.
    - Remote: tunnel first with `ssh -N -L 18789:127.0.0.1:18789 user@host`, then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `AFORA_GATEWAY_TOKEN` or `gateway.auth.password` / `AFORA_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - Mismatch persists after the one retry: rotate/re-approve the paired device token:
      ```bash
      afora devices list
      afora devices rotate --device <id> --role operator
      ```
    ...
    - Still stuck: `afora status --all` plus [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    Usually no - one Gateway can run multiple messaging channels and agents. Use multiple Gateways only for redundancy (for example a rescue bot) or hard isolation, and isolate each with its own `AFORA_CONFIG_PATH`, `AFORA_STATE_DIR`, `agents.defaults.workspace`, and unique `gateway.port`.

    Recommended: `afora --profile <name> ...` per instance (auto-creates `~/.afora-<name>`), a unique `gateway.port` per profile config (or `--port` for manual runs), and a per-profile service with `afora --profile <name> gateway install`.

    Profiles also suffix service names: launchd `ai.afora.<profile>`, systemd `afora-gateway-<profile>.service`, Windows `Afora Gateway (<profile>)`. The unqualified `afora-gateway` systemd unit only exists for the default profile; the legacy pre-rename systemd unit name `clawdbot-gateway` is migrated automatically.
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```bash
    afora tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS launchd stdout: `~/Library/Logs/afora/gateway.log` (profiles use `gateway-<profile>.log`; stderr is suppressed).
    - Linux: `journalctl --user -u afora-gateway[-<profile>].service -n 200 --no-pager`.
    - Windows: `schtasks /Query /TN "Afora Gateway (<profile>)" /V /FO LIST`.
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `afora gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Afora?">
    Three Windows install modes:

    **1) Windows Hub local setup**: the native app manages a local app-owned WSL Gateway. Open **Afora Companion** from the Start menu or tray, then use **Gateway Setup** or the Connections tab.
    ...
    **2) Manual WSL2 Gateway**: the Gateway runs inside Linux.
    ...
    afora gateway restart
    ```
    If you never installed the service, start it in the foreground: `afora gateway run`.
    ...
    **3) Native Windows CLI/Gateway**: runs directly in Windows.
    ...
    afora gateway restart
    ```
    If you run it manually (no service): `afora gateway run`.
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    Usually means the UI lost the WebSocket connection. Check: is the Gateway running (`afora gateway status`)? Is it healthy (`afora status`)? Does the UI have the right token (`afora dashboard`)? If remote, is the tunnel/Tailscale link up?
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ```bash
    afora channels status
    afora channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Afora already trims to the Telegram limit and retries with fewer commands, but some menu entries may still be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="How do I completely stop then start the Gateway?">
    ...
    In the foreground, stop with Ctrl-C, then `afora gateway run`.

    Docs: [Gateway service runbook](/gateway).

  </Accordion>

  <Accordion title="ELI5: afora gateway restart vs afora gateway">
    `afora gateway restart` restarts the **background service** (launchd/systemd). `afora gateway` runs the gateway **in the foreground** for this terminal session. Use the gateway subcommands if you installed the service; use the bare foreground run for a one-off.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must use structured media fields such as `media`, `mediaUrl`, `path`, or `filePath`. See [Afora assistant setup](/start/afora) and [Agent send](/tools/agent-send).

    ```bash
    afora message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Afora to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults reduce risk:

    - Default behavior on DM-capable channels is **pairing**: unknown senders receive a pairing code and their message is not processed. Approve with `afora pairing approve --channel <channel> [--account <id>] <code>`. Pending requests are capped at **3 per channel**; check `afora pairing list --channel <channel> [--account <id>]` if a code did not arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `afora doctor` to surface risky DM policies.
  ...
  <Accordion title="Is Afora less safe because it uses TypeScript/Node instead of Rust/WASM?">
  ...
  <Accordion title="I saw reports about exposed Afora instances. What should I check?">
    ...
    Public binds without auth, open DMs/groups with tools, and exposed browser control are the findings to fix first. Details: [afora security audit](/gateway/security#afora-security-audit).

  </Accordion>

  <Accordion title="Are ClawHub skills and third-party plugins safe to install?">
    Treat third-party skills and plugins as code you are choosing to trust. ClawHub skill pages expose scan state before install, but scans are not a complete security boundary. Afora does not run built-in local dangerous-code blocking during plugin/skill install or update; use operator-owned `security.installPolicy` for local allow/warn/block decisions.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code; their message is **not processed**. Afora only replies to chats it receives or to explicit sends you trigger.
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Afora blocks **cross-provider** messaging by default. If a tool call is bound to Telegram, it will not send to Discord unless you explicitly allow it - and this takes effect immediately, no gateway restart needed:
...
Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or use the [GitHub issue chooser](https://github.com/AforaMosh/afora-agent/issues/new/choose).
