...
read_when:
  - You need a beginner-friendly overview of Afora logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

Afora has two main log surfaces:
...
`afora-dev-YYYY-MM-DD.log`. Case, underscores, and literal dashes use a
...
The date uses the gateway host's local timezone. When `/tmp/afora` is unsafe
or unavailable (and always on Windows), Afora uses a user-scoped
`afora-<uid>` directory under the OS temp dir instead. Dated log files are
pruned after 24 hours.

Each file rotates when the next write would exceed `logging.maxFileBytes`
(default: 100 MB). Afora keeps up to five numbered archives beside the
active file, such as `afora-YYYY-MM-DD.1.log` or
...
```bash
afora logs --follow
afora --dev logs --follow
afora --profile work logs --follow
```
...
or times out before `logs.tail` answers, `afora logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `afora logs --follow` is stricter: on Linux it uses the active
...
```bash
afora channels logs --channel whatsapp
```
...
Afora preserves the original structured log arguments alongside these fields
...
`afora gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
afora gateway
afora gateway --verbose --ws-log compact
afora gateway --verbose --ws-log full
```
...
You can override both via the **`AFORA_LOG_LEVEL`** environment variable (e.g. `AFORA_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `afora.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `afora --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
AFORA_DEBUG_MODEL_TRANSPORT=1 afora gateway
AFORA_DEBUG_MODEL_PAYLOAD=tools AFORA_DEBUG_SSE=events afora gateway
```

Available flags:

- `AFORA_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `AFORA_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `AFORA_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `AFORA_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `AFORA_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `AFORA_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `AFORA_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal Afora logging, so `afora logs --follow`
...
`AFORA_DEBUG_MODEL_TRANSPORT`, so basic model transport hygiene is visible
without debug flags.

### Trace correlation

File logs are JSONL. When a log call carries a valid diagnostic trace context,
Afora writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
config value; `afora doctor --fix` maps a stored `consoleStyle: "compact"`
to `"pretty"`.

### Redaction

Afora can redact sensitive tokens before they hit console output, file logs,
...
Afora also redacts safety-boundary payloads shown to UI clients, support
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `AFORA_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `afora doctor` first.
