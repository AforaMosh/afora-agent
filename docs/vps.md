---
summary: "Run Afora on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Afora
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Afora Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Afora on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Afora bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/afora-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `AFORA_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
- Service env for a stable startup path: `AFORA_NO_RESPAWN=1` and
  `NODE_COMPILE_CACHE=/var/tmp/afora-compile-cache`
...
The standard `afora onboard --install-daemon` path installs a systemd user
unit; edit it with:
...
`sudo systemctl edit afora-gateway.service`.
