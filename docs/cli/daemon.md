---
summary: "CLI reference for `afora daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `afora daemon ...` in scripts
...
Legacy alias for Gateway service management. `afora daemon ...` maps to the same service-control commands as `afora gateway ...`. Prefer [`afora gateway`](/cli/gateway) for current docs and examples.

## Usage

```bash
afora daemon status
afora daemon install
afora daemon start
afora daemon stop
afora daemon restart
afora daemon uninstall
```
