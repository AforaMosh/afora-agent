---
summary: "CLI reference for `afora security` (audit and fix common security footguns)"
read_when:
...
afora security audit --deep --password <password>
afora security audit --deep --token <token>
afora security audit --auth password --password <password>
afora security audit --fix
...
Run `afora doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new token.
...
- Warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `afora.browserConfigEpoch`) and recommends `afora sandbox recreate --browser --all`.
...
```bash
afora security audit --json | jq '.summary'
afora security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

With `--fix --json`, output includes both fix actions and the final report:

```bash
afora security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files (`credentials/*.json`, `auth-profiles.json`, `afora-agent.sqlite`, and legacy session artifacts)
- also tightens config include files referenced from `afora.json`
