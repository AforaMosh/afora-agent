---
summary: "CLI reference for `afora nodes` (status, pairing, invoke, camera/canvas/screen/location/notify)"
read_when:
...
afora nodes status --last-connected 24h
afora nodes list
...
afora nodes rename --node <id|name|ip> --name <displayName>
```

These commands drive the gateway-owned `node.pair.*` store, separate from device pairing (`afora devices approve`) that gates the node's WS `connect` handshake. See [Nodes](/nodes) for how the two relate.
...
```bash
afora nodes invoke --node <id> --command system.which --params '{"bins":["uname"]}'
```
...
```bash
afora nodes notify --node <id> --title "Build" --body "Done" --priority timeSensitive
afora nodes push --node <id> --title "Afora" --environment sandbox
afora nodes location get --node <id> --accuracy precise
afora nodes screen record --node <id> --duration 10s --fps 10 --out ./clip.mp4
```

- `notify` sends a local notification on a node that declares `system.notify`, including macOS, iOS, Android, and direct watchOS nodes. Direct watchOS delivery requires Afora to be active. Requires `--title` or `--body`. Options: `--sound <name>`, `--priority <passive|active|timeSensitive>`, `--delivery <system|overlay|auto>` (default `system`), `--invoke-timeout <ms>` (default `15000`).
- `push` sends an APNs test push to an iOS node. Options: `--title <text>` (default `Afora`), `--body <text>`, `--environment <sandbox|production>` to override the detected APNs environment. Accepted delivery exits `0`; a typed APNs rejection preserves the complete text or JSON diagnostic and exits non-zero.
...
Camera and Canvas commands have their own docs: [Camera nodes](/nodes/camera), [Canvas](/platforms/mac/canvas). Canvas is implemented by the bundled experimental Canvas plugin; core keeps `afora nodes canvas` as a compatibility mount point.
