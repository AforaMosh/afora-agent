---
summary: "CLI reference for `afora attach` (launch Claude Code with a scoped Gateway MCP grant)"
read_when:
  - You want Claude Code to use Afora Gateway MCP tools
  - You need a temporary session-bound MCP grant for an external harness
title: "Attach CLI"
---

`afora attach` launches Claude Code with a strict temporary MCP config bound to one Gateway session.
...
```sh
afora attach
afora attach https://gateway.example/dashboard/main/movies-a1166b81
afora attach movies-a1166b81
afora attach --session agent:main:telegram:123 --ttl 600000
afora attach --print-config
```
...
The bearer token is passed through environment variables, not argv. Afora launches Claude Code with `--strict-mcp-config --mcp-config <path>` so ambient Claude MCP servers do not join the attached session. Normal launches (without `--print-config`) revoke the grant when the Claude Code process exits.
