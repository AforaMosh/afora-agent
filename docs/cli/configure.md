---
summary: "CLI reference for `afora configure` (interactive configuration prompts)"
read_when:
...
Use `afora onboard` or `afora setup` for the full guided first-run journey, `afora setup --baseline` for the baseline config/workspace only, and `afora channels add` when you only need channel account setup.

<Tip>
`afora config` with no subcommand opens the same wizard. Use `afora config get|set|unset` for non-interactive edits.
</Tip>
...
afora configure --section model --section channels
afora configure --section gateway --section daemon
```
...
<Note>
`afora configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `afora config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `afora models auth login --provider <id> --set-default` or `afora models set <model>` to intentionally change the default model.
</Note>
...
`afora configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:
