...
explicit noninteractive approval, and every approved warning is re-evaluated
before continuing. Automatic and managed skill installs cannot use that flag
themselves. Use an equivalent direct CLI command when one exists; otherwise,
change `security.installPolicy` to return `allow` for the reviewed request, then
retry the managed flow. The flag is consumed by the first warning in one
command; a later warning fails closed and requires interactive review. Neither
`--force` nor the acknowledgement overrides `block` or a policy failure.
