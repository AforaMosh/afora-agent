---
summary: "Afora CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `afora` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`afora` is the main CLI entry point. Each core command has a dedicated
...
- `afora setup` and `afora onboard` verify inference first, then start Afora for Gateway, workspace, channels, skills, and health setup.
- `afora setup --baseline` creates the baseline config and workspace without walking the guided onboarding flow.
- `afora configure` changes targeted parts of an existing setup: model auth, gateway, channels, plugins, or skills.
- `afora channels add` configures channel accounts after the baseline exists; a channel selection alone uses guided setup, while account, credential, or channel-config flags use the direct path for scripts.

## Command pages

| Area                         | Commands                                                                                                                                                                                                                              |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Setup and onboarding         | [`afora`](/cli/afora) · [`setup`](/cli/setup) · [`onboard`](/cli/onboard) · [`configure`](/cli/configure) · [`config`](/cli/config) · [`completion`](/cli/completion) · [`doctor`](/cli/doctor) · [`dashboard`](/cli/dashboard) |
...
| Flag                    | Purpose                                                                                                 |
| ----------------------- | ------------------------------------------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.afora-dev`, default gateway port 19001, and shift derived ports              |
| `--profile <name>`      | Isolate state under `~/.afora-<name>` (`AFORA_STATE_DIR`/`AFORA_CONFIG_PATH`)                  |
| `--container <name>`    | Run the CLI inside a running Podman/Docker container named `<name>` (default: env `AFORA_CONTAINER`) |
...
| `--update`              | Shorthand for [`afora update`](/cli/update); works for both source checkouts and package installs    |
...
Afora uses a lobster palette for CLI output:
...
```
afora [--dev] [--profile <name>] <command>
  afora
  setup
  onboard
  configure
...
[`afora workboard`](/cli/workboard) or `afora voicecall`.
...
`afora status --usage` and the Control UI surface provider usage/quota when
