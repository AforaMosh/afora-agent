---
summary: "CLI reference for `afora proxy`, including operator-managed proxy validation and the local debug proxy capture inspector"
read_when:
  - You need to validate operator-managed proxy routing before deployment
  - You need to capture Afora transport traffic locally for debugging
...
```bash
afora proxy validate [--json] [--proxy-url <url>] [--proxy-ca-file <path>] [--allowed-url <url>] [--denied-url <url>] [--apns-reachable] [--apns-authority <url>] [--timeout-ms <ms>]
afora proxy start [--host <host>] [--port <port>]
afora proxy run [--host <host>] [--port <port>] -- <cmd...>
afora proxy coverage [--json]
afora proxy sessions [--limit <count>] [--json]
afora proxy query --preset <name> [--session <id>] [--json]
afora proxy blob --id <blobId>
afora proxy purge
```
...
Checks the effective operator-managed proxy URL from `--proxy-url`, config (`proxy.proxyUrl`), or `AFORA_PROXY_URL`, in that precedence order. Reports a config problem if no proxy is enabled and configured; pass `--proxy-url` for a one-off preflight without touching config.

Managed proxy URLs use `http://` for a plain forward-proxy listener, or `https://` when Afora must open TLS to the proxy endpoint itself before sending proxy requests. Use `--proxy-ca-file` to trust a private CA for that TLS connection.
...
The debug proxy's direct upstream forwarding opens upstream sockets for diagnostics. When Afora managed proxy mode is active, direct forwarding for proxy requests and CONNECT tunnels is disabled by default; set `AFORA_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY=1` only for approved local diagnostics.
