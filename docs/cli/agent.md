---
summary: "CLI reference for Gateway-backed `afora agent` turns and isolated `agent exec` runs"
read_when:
...
`afora agent exec` runs one embedded agent turn without connecting to a Gateway. It is the recommended headless entry point for CI and coding automation because it owns setup, cleanup, output projection, and process status.

```bash
afora agent exec "Run the focused tests and fix failures"
afora agent exec --message-file task.md --cwd ./repo
cat task.md | afora agent exec --message-file - --json
```

By default, the command creates and later removes a temporary state directory, and it runs against your ordinary Afora config, so configured providers, credentials, and `agentRuntime` harness selection apply exactly as they do elsewhere. `--cwd` defaults to the process working directory and is passed as both the agent workspace and tool working directory.
...
Use `--state-dir <dir>` to retain sessions and other run state. The directory must already exist and is never created or deleted by the command. A retained state directory requires exclusive ownership: exec refuses to start while a Gateway or another embedded writer owns it, then holds the state lock for the complete run. Omit `--state-dir` for isolated temporary state, or stop the Gateway first with `afora gateway stop`.
...
```bash
afora agent exec "Inspect this repository" \
...
- `codeModeEngaged`: `true` only when [code mode](/tools/code-mode) actually owned the model tool surface for the run. `tools.codeMode.enabled=true` alone does not guarantee engagement, and harnesses that own their native tool surface always read `false` because Afora code mode never owns their tools.
...
The agent run-stat fields appear on `meta.agentMeta` in the `afora agent --json` response; the outer tool summary remains at `meta.toolSummary`.
...
```bash
afora agent --to +15555550123 --message "status update" --deliver
afora agent --agent ops --message "Summarize logs"
afora agent --agent ops --message-file ./task.md
afora agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
afora agent --session-key agent:ops:incident-42 --message "Summarize status"
afora agent --agent ops --session-key incident-42 --message "Summarize status"
afora agent --session-id 1234 --message "Summarize inbox" --thinking medium
afora agent --to +15555550123 --message "Trace logs" --verbose on --json
afora agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
afora agent --agent ops --message "Run locally" --local
```
...
- `--message` does not run the channel slash-command dispatcher. Recognized `$skill-name` references and leading `/skill-name [input]` are the scoped exception: Afora expands them into model instructions to read the skill before acting. Other slash-prefixed messages keep normal agent-turn behavior; `/compact` is rejected with a pointer to `afora sessions compact <key>`.
...
- `--local` requires exclusive ownership of the configured state directory. It refuses to start while a Gateway or another `agent --local` run owns that directory, then holds the same state lock for the full embedded turn. Run without `--local` to use the active Gateway, or stop it first with `afora gateway stop`.
