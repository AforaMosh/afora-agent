---
summary: "CLI reference for `afora health` (gateway health snapshot via RPC)"
read_when:
...
afora health
afora health --json
afora health --timeout 2500
afora health --verbose
afora health --debug
```
...
- When outbound or session deliveries, or inbound channel events, are dead-lettered, text output reports their counts and oldest failure age. Inbound counts are grouped by channel account; inspect or recover individual events with [`afora channels dead-letters`](/cli/channels#inbound-dead-letters).
...
- [CLI reference](/cli)
- [`afora status`](/cli/status) — local diagnosis and channel probes without a full health snapshot
