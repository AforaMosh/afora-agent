---
summary: "CLI reference for `afora logs` (tail gateway logs via RPC)"
read_when:
...
afora logs --follow --interval 2000
afora logs --limit 500 --max-bytes 500000
afora logs --json
afora logs --plain
afora logs --no-color
afora logs --utc
afora logs --follow --local-time
afora logs --url ws://127.0.0.1:18789 --token "$AFORA_GATEWAY_TOKEN"
```

The selected root profile matches the Gateway's rolling file: the default
profile uses `afora-YYYY-MM-DD.log`, while named profiles use
`afora-<profile>-YYYY-MM-DD.log` (for example,
`afora-dev-YYYY-MM-DD.log`).

## Fallback and recovery behavior

- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `afora logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets never use this fallback.
