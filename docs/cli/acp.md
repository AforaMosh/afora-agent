...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Afora Gateway.

`afora acp` speaks ACP over stdio for IDEs and forwards prompts to the Gateway over WebSocket, keeping ACP sessions mapped to Gateway session keys. It is a Gateway-backed ACP bridge, not a full ACP-native editor runtime: it focuses on session routing, prompt delivery, and streaming updates.

If you want an external MCP client to talk directly to Afora channel conversations instead of hosting an ACP harness session, use [`afora mcp serve`](/cli/mcp) instead.

## What this is not

`afora acp` means Afora acts as an ACP server: an IDE or ACP client connects to Afora, and Afora forwards that work into a Gateway session.

This is different from [ACP Agents](/tools/acp-agents), where Afora runs an external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Afora: use `afora acp`
- Afora should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Afora parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                     |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Afora Gateway or agent instead.                                                                                                                          |
...
# Remote Gateway
afora acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
afora acp --url wss://gateway-host:18789 --token-file ~/.afora/gateway.token

# Attach to an existing session key
afora acp --session agent:main:main

# Attach by label (must already exist)
afora acp --session-label "support inbox"

# Reset the session key before the first prompt
afora acp --session agent:main:main --reset-session
```
...
afora acp client --server-args --url wss://gateway-host:18789 --token-file ~/.afora/gateway.token

# Override the server command (default: afora)
afora acp client --server "node" --server-args afora.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Afora through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive `afora acp` over stdio with an ACP JSON-RPC client. Cover `initialize`, `session/new`, `session/list` with an absolute `cwd`, `session/resume`, `session/close`, duplicate close, and missing resume.
...
Avoid using `afora gateway call sessions.list` as the only ACP proof. That CLI path may request a fresh-token operator scope upgrade; ACP bridge correctness is proven by ACP stdio frames plus the Gateway `sessions.list` log.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want it to drive an Afora Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `afora acp` over stdio.

Example config (persisted):

```bash
afora config set gateway.remote.url wss://gateway-host:18789
afora config set gateway.remote.token <token>
```
...
```bash
afora acp --session agent:main:main
afora acp --session agent:design:main
afora acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Afora plugin tools or selected built-in tools such as `cron`, enable the gateway-side ACPX MCP bridges instead of trying to pass per-session `mcpServers`. See [ACP Agents](/tools/acp-agents-setup#plugin-tools-mcp-bridge) and [Afora tools MCP bridge](/tools/acp-agents-setup#afora-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your Afora bot over ACP, use `acpx` with its built-in `afora` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx afora` at `afora acp`.
3. Target the Afora session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Afora ACP session
acpx afora exec "Summarize the active Afora session state."

# Persistent named session for follow-up turns
acpx afora sessions ensure --name codex-bridge
acpx afora -s codex-bridge --cwd /path/to/repo \
  "Ask my Afora work agent for recent context relevant to this repo."
```

If you want `acpx afora` to target a specific Gateway and session key every time, override the `afora` agent command in `~/.acpx/config.json`:

```json validate=false
{
  "agents": {
    "afora": {
      "command": "env AFORA_HIDE_BANNER=1 AFORA_SUPPRESS_NOTES=1 afora acp --url ws://127.0.0.1:18789 --token-file ~/.afora/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Afora checkout, use the direct CLI entrypoint instead of the dev runner so the ACP stream stays clean:

```bash
env AFORA_HIDE_BANNER=1 AFORA_SUPPRESS_NOTES=1 node afora.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client pull contextual information from an Afora agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Afora ACP" to start a thread.
...
- `--token` and `--password` can be visible in local process listings on some systems. Prefer `--token-file`/`--password-file` or environment variables (`AFORA_GATEWAY_TOKEN`, `AFORA_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: `gateway.auth.*` then env (`AFORA_GATEWAY_*`), falling back to `gateway.remote.*` only when `gateway.auth.*` is unset (a configured-but-unresolved local SecretRef fails closed instead of silently falling back)
...
- `--server <command>`: ACP server command (default: `afora`).
- `--server-args <args...>`: extra arguments passed to the ACP server.
- `--server-verbose`: enable verbose logging on the ACP server.
- `--verbose, -v`: verbose client logging.
- `afora acp client` sets `AFORA_SHELL=acp-client` on the spawned bridge process, which can be used for context-specific shell/profile rules.
