---
summary: "CLI reference for `afora wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, ChatGPT import, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `afora wiki`
title: "Wiki"
---
...
```bash
afora plugins enable memory-wiki
...
afora wiki okf import ./knowledge-catalog/okf/bundles/ga4
afora wiki compile
afora wiki lint
afora wiki search "alpha"
afora wiki search "who should I ask about Teams?" --mode route-question
...
afora wiki chatgpt import --export ./chatgpt-export --dry-run
afora wiki chatgpt rollback <run-id>
...
afora wiki ingest ./campaign-notes.md --agent marketing
```
...
```bash
afora wiki okf import ./bundles/ga4
afora wiki okf import ./bundles/ga4 --json
afora wiki search "BigQuery Table" --mode source-evidence --json
afora wiki get <path-from-json-result>
```

### `wiki compile`

Rebuild indexes, related blocks, dashboards, and the compiled query/prompt snapshot. The snapshot is persisted in Afora's shared SQLite plugin state and kept in memory for synchronous prompt projection; it does not create cache files in the vault.
...
Use `wiki search` for wiki-specific ranking and provenance. For one broad shared recall pass, prefer `afora memory search` when the active memory plugin exposes shared search.
...
```bash
afora wiki search "bgroux" --mode find-person
afora wiki search "who knows Teams rollout?" --mode route-question
afora wiki search "maintainer-whois" --mode source-evidence
afora wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
afora wiki get entity.alpha
afora wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
```bash
afora wiki chatgpt import --export ./chatgpt-export
afora wiki chatgpt import --export ./conversations.json --dry-run
```
...
Roll back a previously applied ChatGPT import run, removing pages it created and restoring pages it overwrote. Pages changed after import are moved under the run's `.afora-wiki/import-runs/<run-id>/recovered/` directory instead of being deleted. Recovery paths remain in the command result on retries and later `alreadyRolledBack` responses. Interrupted runs remain `rolling_back` while target recovery or derived-artifact compilation is incomplete. A persisted process-restart fence separates those phases: after it, retries rebuild indexes and compiled caches without rewriting source pages or moving later pathname writes. A later normal compile may refresh machine-managed Related blocks. This covers in-process failure and process restart after ordinary filesystem calls return, not kernel or host power-loss ordering.
...
`afora wiki` behavior is shaped by:
