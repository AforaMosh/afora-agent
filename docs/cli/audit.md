...
`afora agent exec` deletes its temporary state directory by default, so its
...
```bash
afora audit
afora audit --agent main --status failed
afora audit --session "agent:main:main" --after 2026-07-01T00:00:00Z
afora audit --run 8c69f72e-8b11-4c54-98d5-1a3dd67450c3
afora audit --run 8c69f72e-8b11-4c54-98d5-1a3dd67450c3 --explain
afora audit --execution 5da4c4c3-e1c9-4c95-a17d-6e5c10fd45cf --explain
afora audit --execution 5da4c4c3-e1c9-4c95-a17d-6e5c10fd45cf --explain --json
afora audit --run 8c69f72e-8b11-4c54-98d5-1a3dd67450c3 --explain --json
afora audit --kind tool_action --limit 50 --json
afora audit --kind message --direction outbound --channel telegram --json
```
...
agent, run, and action. Missing message provenance renders as `-`; Afora
...
```bash
afora audit --execution <execution-id> --explain
```

Afora never silently selects the first or latest execution. The exact text
...
```bash
afora gateway call audit.activity.list --params '{"channel":"telegram","limit":50}'
```
...
```bash
afora gateway call audit.run.inspect \
