---
summary: "CLI reference for `afora models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
| `--agent <id>`            | Configured agent id; overrides `AFORA_AGENT_DIR`.                                                                                     |
...
Direct `models status --probe` runs create temporary internal sessions in the selected agent's canonical database, so the command requires exclusive ownership of the configured state directory. Stop a running Gateway with `afora gateway stop` before probing; the command removes its internal sessions and releases the state lock when it finishes or is interrupted.
...
- `no_model`: provider auth exists, but Afora could not resolve a probeable model candidate for that provider.

For OpenAI ChatGPT/Codex OAuth troubleshooting, `afora models status`, `afora models auth list --provider openai`, and `afora config get agents.defaults.model --json` are the quickest way to confirm whether an agent has a usable `openai` OAuth profile for `openai/*` through the native Codex runtime. See [OpenAI provider setup](/providers/openai#check-and-recover-codex-oauth-routing).

### List

`afora models list` is read-only: it reads config, auth profiles, existing catalog state, and provider-owned catalog rows, but never rewrites `models.json`.

`afora models refresh [--json]` forces an immediate hosted catalog check.
...
[`afora/catalog`](https://github.com/afora/catalog), where each content
...
- If you omit the provider, Afora resolves the input as an alias first, then as a unique configured-provider match for that exact model id, and only then falls back to the configured default provider with a deprecation warning. If that provider no longer exposes the configured default model, Afora falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
By default Afora tries to probe tool and image support with live model calls. If no OpenRouter key is configured, the command falls back to metadata-only output and explains that `:free` models still require `OPENROUTER_API_KEY` for probes and inference.
...
```bash
afora models aliases list [--json] [--plain]
afora models aliases add <alias> <model-or-alias>
afora models aliases remove <alias>
```
...
```bash
afora models fallbacks list [--json] [--plain]
afora models fallbacks add <model-or-alias>
afora models fallbacks remove <model-or-alias>
afora models fallbacks clear
```

Manages `agents.defaults.model.fallbacks`. `afora models image-fallbacks list|add|remove|clear` manages the parallel `agents.defaults.imageModel.fallbacks` list with the same subcommand shape.

## Auth profiles

```bash
afora models auth add
afora models auth list [--provider <id>] [--json]
afora models auth login --provider <id> [--agent <agentId>]
afora models auth login --provider openai --profile-id openai:work
afora models auth login-github-copilot
afora models auth logout <profileId> [--yes]
afora models auth paste-api-key --provider <id>
afora models auth setup-token --provider <id>
afora models auth paste-token --provider <id>
afora models auth order get --provider <id>
afora models auth order set --provider <id> <profileIds...>
afora models auth order clear --provider <id>
```
...
`models auth login` runs a provider plugin's auth flow (OAuth/API key). Use `afora plugins list` to see which providers are installed. `login` accepts `--profile-id <id>` for providers that support named profiles during login (use this to keep multiple logins for the same provider separate), `--method <id>` to pick a specific auth method, `--device-code` as a shortcut for `--method device-code`, `--set-default` to apply the provider's recommended default model, and `--force` to remove existing profiles for that provider first (use when a cached OAuth profile is stuck or you want to switch accounts).
...
Use either `afora models auth --agent <id> <subcommand>` or `afora models auth <subcommand> --agent <id>` to target a specific configured agent store. Both forms are supported by `add`, `list`, `login`, `logout`, `paste-api-key`, `setup-token`, `paste-token`, `login-github-copilot`, and `order get`/`set`/`clear`.

For OpenAI models, `--provider openai` defaults to ChatGPT/Codex account login. Use `--method api-key` only when you want to add an OpenAI API-key profile, usually as a backup for Codex subscription limits. Run `afora doctor --fix` to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
afora models auth login --provider openai --set-default
afora models auth login --provider openai --method api-key
afora models auth paste-api-key --provider openai
afora models auth list --provider openai
afora models auth logout openai:manual --yes
```

Notes:

- `paste-api-key` accepts API keys generated elsewhere, prompts for the key value, and writes it to the default profile id `<provider>:manual` unless you pass `--profile-id`. In automation, pipe the key on stdin, for example `printf "%s\n" "$OPENAI_API_KEY" | afora models auth paste-api-key --provider openai`.
...
- Anthropic: `setup-token`/`paste-token` are supported Afora auth paths for `anthropic`, but Afora prefers reusing the Claude CLI (`claude -p`) on the host when it is available.
