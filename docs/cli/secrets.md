---
summary: "CLI reference for `afora secrets` (store, reload, audit, configure, apply)"
read_when:
...
`afora secrets store` writes directly to the local shared state database. The store is Gateway-wide and team-scoped; this release accepts only `--scope team`. `--scope me` is rejected because identity scope is not supported yet.

```bash
afora secrets store list
afora secrets store set <NAME>
afora secrets store get <NAME>
afora secrets store rm <NAME>...
afora secrets store import [--from <file>]
```
...
```bash
afora secrets store set LOG_LEVEL --kind env --value debug
```
...
op read 'op://Engineering/OpenAI/apiKey' | \
  afora secrets store set OPENAI_API_KEY --kind secret

afora secrets store set TLS_PRIVATE_KEY \
  --kind secret \
  --value-file ./client-key.pem
```

`set` is idempotent and updates an existing name. Add `--dry-run` to validate and preview the operation without writing. A successful write reminds you to run `afora secrets reload` before a config-referenced value can take effect.
...
afora secrets store set SERVICE_TOKEN \
  --allow-host api.example.com \
  --allow-host uploads.example.com
afora secrets store set SERVICE_TOKEN --clear-allowed-hosts
```
...
```bash
afora secrets store list --json
afora secrets store list --plain
afora secrets store get LOG_LEVEL
```
...
```bash
afora secrets store import --from .env
afora secrets store import --from .env --dry-run
afora secrets store import --from .env --yes
op read 'op://Engineering/service-account/dotenv' | afora secrets store import --yes
```
...
```bash
afora secrets reload
afora secrets reload --json
afora secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scans Afora state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `afora.json` refs)
- store residue (a team store value duplicated by plaintext in `afora.json`)
...
```bash
afora secrets configure
afora secrets configure --plan-out /tmp/afora-secrets-plan.json
afora secrets configure --apply --yes
afora secrets configure --providers-only
afora secrets configure --skip-provider-setup
afora secrets configure --agent ops
afora secrets configure --json
```
...
- Targets secret-bearing fields in `afora.json` plus `auth-profiles.json` for the selected agent scope; canonical supported surface: [SecretRef Credential Surface](/reference/secretref-credential-surface).
...
afora secrets apply --from /tmp/afora-secrets-plan.json --json
```
...
- `afora.json` (SecretRef targets + provider upserts/deletes)
