---
summary: "CLI reference for `afora channels` (accounts, status, dead letters, capabilities, resolve, logs, login/logout)"
read_when:
...
```bash
afora channels list
afora channels list --all
afora channels status
afora channels capabilities
afora channels capabilities --channel discord --target channel:123
afora channels resolve --channel slack "#general" "@jane"
afora channels logs --channel all
afora channels dead-letters list --channel telegram --account default
```

`channels list` shows chat channels only: configured accounts by default, with `installed`, `configured`, and `enabled` status tags per account (`--json` for machine output). Pass `--all` to also surface bundled channels that have no configured account yet and installable catalog channels that are not yet on disk. Provider auth and model usage live elsewhere: `afora models auth list` for provider auth profiles, `afora status` or `afora models list` for usage/quota.
...
```bash
afora channels dead-letters list --channel telegram --account default
afora channels dead-letters list --channel telegram --account default --json
```
...
```bash
afora channels dead-letters resubmit <event-id> --channel telegram --account default
```
...
`afora health` reports dead-letter counts and oldest failure age per channel account. `afora doctor` names affected accounts and points back to the inspection command.

Do not use `afora sessions`, Gateway `sessions.list`, or the agent
...
```bash
afora channels add --channel telegram --token <bot-token>
afora channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
afora channels remove --channel telegram --delete
```
...
<Tip>
`afora channels add telegram --help` or `afora channels add --channel telegram --help` shows only Telegram's setup flags. `afora channels add --help` shows only the shared command envelope.
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, Afora uses the channel's default install source without opening the interactive plugin install prompt.
...
When you run `afora channels add` with no direct account, credential, or channel-config flags, the interactive wizard can prompt. A positional channel id and `--channel <id>` both open that channel's guided setup immediately. Back returns to the full channel picker:

```bash
afora channels add telegram
afora channels add --channel telegram
```

Guided setup requires an interactive terminal. In a non-TTY shell, Afora exits immediately instead of waiting for input; use `afora channels add --channel <id> --use-env` or pass the selected plugin's credential flags.
...
You can also manage the same routing rules later with `afora agents bindings`, `afora agents bind`, and `afora agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, Afora promotes those top-level values into the channel's account map before writing the new account. Promotion reuses an existing named account when the channel has exactly one, or when `defaultAccount` points at one; otherwise the values land in `channels.<channel>.accounts.default`.
...
If your config was already in a mixed state (named accounts present and top-level single-account values still set), run `afora doctor --fix` to move account-scoped values into the promoted account chosen for that channel.

## Login and logout (interactive)

```bash
afora channels login --channel whatsapp
afora channels logout --channel whatsapp
```
...
- Run `afora status --deep` for a broad probe.
- Use `afora doctor` for guided fixes.
- `afora channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
afora channels resolve --channel slack "#general" "@jane"
afora channels resolve --channel discord "My Server/#support" "@someone"
afora channels resolve --channel matrix "Project Room"
afora channels --agent ops resolve --channel slack "#general"
afora channels resolve --agent ops --channel slack "#general"
```
