---
summary: "CLI reference and security model for the inference-backed Afora setup and repair helper"
read_when:
  - You finished inference setup and want Afora to configure the rest
  - You need to inspect or repair Afora with the local setup agent
  - You are designing or enabling message-channel rescue mode
title: "Afora setup agent"
---

# `afora setup`

Afora ships with a built-in system agent — it speaks as "Afora" — for
...
Running `afora` with no subcommand routes based on config state:
...
- Config exists but fails validation: starts classic onboarding, which reports the issues and directs you to `afora doctor`.
- Config exists and is valid: opens the normal agent TUI. A reachable
  ...
  without onboarding or Afora. Use `/afora` inside the TUI, or run
  `afora setup` directly, to reach Afora later.

Running `afora setup` first live-tests the configured default model. A passing turn starts Afora. An interactive failure opens guided inference setup and hands off to Afora after a candidate passes. One-shot, JSON, and other noninteractive requests fail with instructions to run `afora onboard` when inference is unavailable. `afora --help` and `afora --version` keep their normal fast paths.

Noninteractive bare `afora` (no TTY) exits with a short message instead of printing root help: it points to non-interactive onboarding on a fresh or invalid install, or to `afora agent --local ...` when config is valid.

`afora onboard --modern` remains a compatibility alias for Afora, but uses the same inference gate: working inference opens the chat, interactive failures start guided inference setup, and noninteractive failures exit with onboarding guidance. `afora onboard --classic` opens the full step-by-step wizard.

## What Afora shows

Interactive Afora opens the same TUI shell as `afora tui`, with an Afora chat backend. The startup greeting covers:

- config validity and the default agent
- the verified model Afora is using
...
Afora uses the same reference discovery as regular agents: in a Git checkout it points at local `docs/` and the source tree; in an npm install it uses bundled docs and links to [https://github.com/AforaMosh/afora-agent](https://github.com/AforaMosh/afora-agent), with guidance to check source when docs are not enough.
...
afora setup
afora setup --json
afora setup --message "models"
afora setup --message "validate config"
afora setup --message "setup workspace ~/Projects/work" --yes
afora setup --message "set default model openai/gpt-5.6" --yes
afora onboard --modern
```

Inside the Afora TUI:
...
config set-ref gateway.auth.token env AFORA_GATEWAY_TOKEN
gateway status
configure gateway
open gateway wizard
restart gateway
...
plugin install clawhub:afora-codex-app-server
talk to work agent
...
Afora uses typed operations instead of editing config ad hoc.
...
Configured agents can ask Afora to create another agent through their
`afora` tool. The request enters the same typed create-agent operation and
operator approval flow used by Ask Afora; the approval summary names the
requesting agent. Afora remains the executor, and approved creation records
that requesting agent as the new agent's creator.

Doctor repairs are unavailable inside Afora because they can rewrite the provider, authentication, or default-agent inference route powering the session. Exit Afora and run `afora doctor --fix` in a terminal. Read-only `doctor` remains available inside Afora.

New agents inherit the live-verified default inference route. The agent ids `afora` and `crestodian` are reserved for the system agent and cannot be created as normal agents. The retired id remains blocked so an old config cannot claim it.
...
configure or repair provider/auth access, exit Afora and run
`afora onboard`.
...
refused for the same reason; exit Afora and run
`afora plugins uninstall <id>` from a terminal.
...
The Ask Afora page can show recent applied system-agent operations, Doctor
migrations, Settings and CLI config writes, and manual edits to
`afora.json`. The config journal detects external edits while the Gateway
is watching, during an Afora-owned write, or at the next startup after an
offline edit.

History is stored in the `diagnostic_events` table of the shared
`~/.afora/state/afora.sqlite` database, under the `system-agent-audit`
...
until they reach a secret. The local Afora TUI does not accept sensitive wizard answers
...
`afora channels add --channel <channel>` or
`afora configure --section web` or `afora configure --section gateway`
later.
...
when it finishes, run `afora gateway restart` to apply the saved settings.

Afora never changes provider/auth access from inside its own session: the
...
starting a wizard or writing config. Exit Afora and run `afora
onboard`; onboarding stages the credentials and saves only a route that
completes a real live turn. Start Afora again after onboarding succeeds.
...
If inference is missing or its live check fails, leave Afora and run `afora onboard`. Guided onboarding tries the configured model first, then authenticated subscription CLIs, API keys, and remaining supported CLIs; it asks each candidate for a real reply and persists only a passing route. Afora starts immediately after that boundary and can then configure the workspace, Gateway, channels, agents, plugins, and other optional features.
...
the `afora.setup.detect` and `afora.setup.activate` Gateway methods:
...
credential, and provider/runtime state needed for that route after the test passes. Workspace and Gateway defaults remain for Afora. A failing candidate
...
working and Afora has started; existing plugin policy and explicit
supervision opt-outs remain untouched during inference setup.

## AI conversation

Interactive Afora's free-form conversation runs through the same agent loop as regular Afora agents, restricted to one ring-zero Afora authority tool, `afora`, that wraps the typed operations. Read actions run freely, mutations require your conversational approval for that exact operation (see Operations and approval), and every applied write is audited and re-validated. The agent session persists, so Afora has real multi-turn memory. If the verified inference route later stops working, return to `afora onboard` and repair it before continuing.
...
restriction directly: the run carries an Afora tool allow-list with only
the `afora` tool. For Codex, Afora also disables environments, native
...
harnesses do not consume Afora's allow-list,
so Afora admits only backends whose own tool-selection contract can prove
the same restriction:

- Selectable backends, including Claude Code, launch with an empty native-tool
  selection and one MCP tool, `afora`. Claude's generated MCP config is
  applied with `--strict-mcp-config`, so no other MCP servers are loaded.
- Backends that declare no native tools receive the same dedicated Afora
  MCP server.
- Always-on or unknown native-tool backends fail closed before inference; they
  cannot host an Afora session.

Only Afora sessions get the afora MCP server; normal agent runs
...
a single Afora authority tool plus the inert native planning utility. In all
three cases, setup writes remain confined to Afora's audited approval
contract.
...
Use a natural-language selector to leave Afora and open the normal TUI:
...
`afora tui`, `afora chat`, and `afora terminal` open the normal agent TUI directly; they do not start Afora. After switching into the normal TUI, `/afora` returns to Afora, optionally with a follow-up request:

```text
/afora
/afora restart gateway
```

## Message rescue mode

Message rescue mode is the message-channel entrypoint for Afora: use it when your normal agent is dead but a trusted channel (for example WhatsApp) still receives commands.

This is a deterministic emergency command handler, not the conversational
Afora agent. It does not bootstrap a fresh setup or relax the inference
gate for Afora chat.

Supported command: `/afora <request>`. Rescue accepts the exact typed command grammar only — natural language is rejected with a hint, never guessed into an operation, and no model is ever consulted.

```text
You, in a trusted owner DM: /afora status
Afora: Afora rescue mode. Gateway reachable: no. Config valid: no.
You: /afora restart gateway
Afora: Plan: restart the Gateway. Reply /afora yes to apply.
You: /afora yes
Afora: Applied. Audit entry written.
```
...
/afora create agent work workspace ~/Projects/work
```
...
- Disabled when sandboxing is active for the agent/session; Afora refuses remote rescue and points to local CLI repair.
...
- Plugin search and list are read-only. Plugin install is always local-only (blocked in rescue, even when otherwise enabled) because it downloads executable code. Plugin uninstall is refused in both local Afora and rescue; run `afora plugins uninstall <id>` from a terminal.
- Remote rescue cannot open the local TUI or switch into an interactive agent session; use local `afora` for agent handoff.
...
expire after 15 minutes. `afora doctor --fix` removes the retired
...
An opt-in live channel command-surface smoke checks `/afora status` plus a persistent approval roundtrip through the rescue handler:
...
That packaged-CLI lane starts with an empty state dir and proves Afora
...
Afora agent/tool/approval conversation. The QA Lab scenario below redirects
