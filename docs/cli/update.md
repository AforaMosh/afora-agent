---
summary: "CLI reference for `afora update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `afora update` output or options
...
Update Afora and switch between stable/extended-stable/beta/dev channels.
...
afora update --tag beta
afora update --tag main
afora update --dry-run
afora update --no-restart
afora update --yes
afora update --acknowledge-clawhub-risk
afora update --json
afora --update
```

`afora --update` rewrites to `afora update` (useful for shells and
launcher scripts).
...
| `--tag <dist-tag\|version\|spec>`                | Override the package target for this update only. It cannot be combined with an effective `extended-stable` channel, whose verified exact target is mandatory. For other package installs, `main` maps to `github:AforaMosh/afora-agent#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install. |
...
| `--acknowledge-clawhub-risk`                     | Allow post-update plugin sync to continue past community ClawHub trust warnings without an interactive prompt. Without it, risky community releases are skipped and left unchanged when Afora cannot prompt. Official ClawHub packages and bundled plugin sources bypass this prompt.                                                     |

There is no `--verbose` flag. Use `--dry-run` to preview planned actions,
`--json` for machine-readable results, and `afora update status --json`
...
<Note>
In Nix mode (`AFORA_NIX_MODE=1`), mutating `afora update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-afora, use the agent-first [Quick Start](https://github.com/afora/nix-afora#quick-start). `afora update status` and `afora update --dry-run` remain read-only.
</Note>
...
afora update status --timeout 10
```
...
`afora update` installed the new core package but post-core plugin sync,
managed npm plugin metadata, registry refresh, or doctor repair did not
converge.

```bash
afora update repair
afora update repair --channel beta
afora update repair --acknowledge-clawhub-risk
afora update repair --json
```
...
| `--acknowledge-clawhub-risk`                     | Same behavior as on `afora update`.                                                                                                                                                                                                                              |
| `--no-restart`                                   | Accepted for parity; repair never restarts the Gateway.                                                                                                                                                                                                             |

`update repair` runs `afora doctor --fix`, reloads the repaired config and
...
- `dev` -> ensures a git checkout (default `~/afora`, or
  `$AFORA_HOME/afora` when `AFORA_HOME` is set; override with
  `AFORA_GIT_DIR`), updates it, and installs the global CLI from that
  checkout.
...
detached helper and exits, and that helper runs `afora update --yes --json`
...
healthy. If the plist is installed but launchd is not supervising it, Afora
...
`Gateway: restart failed: ...` with a manual `afora gateway restart` hint.
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `afora update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"`: Afora could not find a supervising
  service boundary and durable service identity for a safe handoff (for
  example, systemd handoff requires the `AFORA_SYSTEMD_UNIT` unit identity,
...
`afora status` and `afora status --all` show an `Update restart` row
...
<Steps>
  ...
  <Step title="Run doctor">
    `afora doctor` runs as the final safe-update check.
  </Step>
...
beta release, Afora falls back to the recorded default/latest spec and
reports a warning. For npm plugins, Afora also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `afora update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (for example an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `afora update repair` and `afora plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `afora update repair`. When a failed update leaves a managed plugin unusable, Afora disables its runtime entry and resets active slots without changing the operator-authored `plugins.allow` or `plugins.deny` policy.

After the per-plugin sync step, `afora update` runs a mandatory **post-core convergence** pass before the gateway restarts: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable and its declared `afora.extensions` entries are loadable. When a package does not declare Afora extensions, the check instead verifies any explicitly declared npm `main`. Failures from this pass, and an invalid config snapshot, return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `afora update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `afora update repair` and `afora plugins inspect <id> --runtime --json`. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here (mirroring the `skipDisabledPlugins` policy used by the missing-payload check), so a stale disabled plugin record cannot block an otherwise valid update.
...
version. For default/`latest` intent, Afora does not query plugin
...
For package-manager installs, `afora update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: Afora installs the new package into a temporary npm prefix,
...
the updater approves only the candidate Afora lifecycle; transitive
dependency scripts remain blocked. Afora then swaps the clean package tree
...
plugin records aligned with the installed Afora build, while leaving full
plugin-command completion rebuilds to explicit
`afora completion --write-state` runs.

## Related

- `afora doctor` (offers to run update first on git checkouts)
