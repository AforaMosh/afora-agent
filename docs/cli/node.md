---
summary: "CLI reference for `afora node` (headless node host)"
read_when:
...
node connection and adds native Mac capabilities. Use `afora node run` on a
...
`afora node run` can publish plugin or MCP-backed tools after it connects.
...
`nodeHost.mcp.servers` in `afora.json` on the node machine, then restart the
...
For one-paste onboarding, use [`afora connect`](/cli/connect). It accepts a
...
- `--context-path <path>`: Gateway WebSocket context path (e.g. `/afora-gw`). Appended to the WebSocket URL.
...
`afora node run` and `afora node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `AFORA_GATEWAY_TOKEN` / `AFORA_GATEWAY_PASSWORD` are checked first.
...
`CF_ACCESS_CLIENT_SECRET` together before `afora connect`, `afora node
run`, or `afora node install`. The node stores env SecretRefs under its
...
trusted private-DNS names, set `AFORA_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `afora.json` config
key.
`afora node install` persists it into the supervised node service when it is
...
```bash
afora node install --host <gateway-host> --port 18789
```
...
> `afora node install` prints this warning when it detects lingering is
> disabled.
...
Use `afora node run` for a foreground node host (no service).

Service commands accept `--json` for machine-readable output.
`node start` and `node restart` print install hints and exit nonzero when no
managed node service is installed; run `afora node install` first. Stopping
...
runs `afora node identity --json` on the node host over SSH and approves on
...
```bash
afora node identity --json
```

It prints the device ID and public key from the `primary` row in
`state/afora.sqlite` and never creates the database or a new identity.
...
Run `afora devices list` again before approval.
...
Afora state directory (`~/.afora` by default, or `$AFORA_STATE_DIR`
when set):

| State                                                    | Purpose                                                                                                                          |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `state/afora.sqlite` (`node_host_config`)             | Client instance ID, display name, and Gateway connection metadata. The client sends this ID as `instanceId`.                     |
| `state/afora.sqlite` (`device_identities`, `primary`) | Signed Ed25519 keypair and derived device ID. For signed connections, this device ID is the routed node ID and pairing identity. |
| `state/afora.sqlite` (`device_auth_tokens`)           | Paired device tokens, keyed by cryptographic device ID and role.                                                                 |
...
`node.json` with `afora doctor --fix` likewise does not reset pairing. To
revoke and re-pair a node:

1. On the Gateway, run `afora nodes remove --node <id|name|ip>`.
2. On the node, restart the installed service with `afora node restart`, or
   stop and rerun the foreground `afora node run` command. This starts the
   device-pairing flow. If `afora devices list` does not show a request
...
3. On the Gateway, run `afora devices list`, then
...
5. On the Gateway, run `afora nodes pending`, then
...
Older Afora releases stored node-host state in `node.json`, the signed
identity in `identity/device.json`, and paired auth in
`identity/device-auth.json`. Stop the node host and run
`afora doctor --fix` once; Doctor claims each retired source, validates it,
...
or an interrupted Doctor claim remains. Keep `state/afora.sqlite` private;
...
- `$AFORA_STATE_DIR/state/afora.sqlite#exec_approvals_config`, or
  `~/.afora/state/afora.sqlite#exec_approvals_config` when the variable is unset
- [Exec approvals](/tools/exec-approvals)
- `afora approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Afora prepares a canonical `systemRunPlan`
