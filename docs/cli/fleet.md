---
summary: "CLI reference for provisioning and managing isolated per-tenant Afora cells"
read_when:
...
`afora fleet` manages complete Afora instances called **cells**. Each cell has its own Gateway, state, credentials, channel accounts, container, and loopback-only host port. Use one cell for each tenant trust boundary; do not use one shared Gateway as a hostile multi-tenant boundary.
...
Fleet supports Docker and Podman. The default image is `ghcr.io/AforaMosh/afora-agent:latest`.
...
The ID becomes part of the container name: `afora-cell-<tenant>`.
...
Create and upgrade accept digest-pinned image references such as `--image ghcr.io/AforaMosh/afora-agent@sha256:<digest>`. Fleet passes the image reference through verbatim to Docker or Podman, which lets an operator keep a cell on immutable image bytes instead of a moving tag.
...
```bash
afora fleet logs acme
afora fleet logs acme --follow
afora fleet logs acme --tail 200
afora fleet logs acme --since 10m
```
...
```bash
afora fleet start acme
afora fleet stop acme
afora fleet restart acme
```
...
```bash
afora fleet upgrade acme --image ghcr.io/AforaMosh/afora-agent:<version>
```
...
The Gateway token is intentionally not stored in the fleet registry. Before removing the old container, Fleet reads its environment and carries `AFORA_GATEWAY_TOKEN` into the replacement. Do not manually remove the old container before an upgrade if the token exists nowhere else you control.
...
```bash
afora fleet stop acme
afora fleet backup acme --out ./acme.tgz
```

Restore that archive into the registered cell:

```bash
afora fleet restore acme --from ./acme.tgz
```
...
Cell state and auth-profile encryption keys use separate per-tenant host paths under the active Afora state directory:
