---
summary: "CLI reference for `afora setup` (system-agent chat with onboarding fallback)"
read_when:
  - You want to chat with Afora for setup or repair
...
`afora setup` is the system-agent entry point. On a configured system, bare
`afora setup` opens an interactive Afora chat. On a fresh system, it
...
1. Any onboarding option (`--wizard`, `--baseline`, workspace, reset,
   ...
   options) runs onboarding exactly as `afora onboard` does.
2. `-m`/`--message` or `--yes` runs the system agent.
3. With no routing option, a configured interactive system opens Afora. A
...
In guided mode, `--workspace <dir>` is the workspace proposed to Afora;
...
candidates; when several local models are available, Afora prefers the
...
`setup` accepts the same onboarding flags as `afora onboard`, including
...
terminal hatch as `afora onboard --tui`. See [Onboard](/cli/onboard) and
[CLI automation](/start/wizard-cli-automation) for the full flag reference and
non-interactive examples. `afora onboard --modern` remains a compatibility
entry for the same inference-gated Afora assistant.

<Note>
`afora setup` is for mutable config installs. In Nix mode (`AFORA_NIX_MODE=1`) Afora refuses setup writes because the config file is managed by Nix. Use the first-party [nix-afora Quick Start](https://github.com/afora/nix-afora#quick-start) or the equivalent source config for another Nix package.
</Note>
...
| Flag                       | Description                                                                                          |
| -------------------------- | ---------------------------------------------------------------------------------------------------- |
| `-m, --message <text>`     | Run one Afora request.                                                                            |
...
| `--json`                   | Configured system: Afora overview. Onboarding route: onboarding summary.                          |
...
`afora setup --baseline` preserves the older baseline-only behavior: it
...
it and asks you to run `afora doctor` before retrying.

## Examples

```bash
afora setup
afora setup -m "status"
afora setup -m "restart gateway" --yes
afora setup --json
afora setup --wizard
afora setup --baseline
afora setup --workspace ~/.afora/workspace
afora setup --import-from hermes --import-source ~/.hermes
afora setup --non-interactive --accept-risk --mode remote --remote-url wss://gateway-host:18789 --remote-token <token>
```

## Notes

- Inside the interactive Afora chat, `configure skills`, `configure web search`, and `configure gateway` run hosted setup flows. `open search wizard` and `open gateway wizard` hand credential entry to masked terminal wizards. Gateway setup is local-only and config-only; restart afterward with `restart gateway` in chat or `afora gateway restart` in the terminal. See [`afora setup` operations](/cli/afora#operations-and-approval).
...
- After baseline setup, run `afora onboard` for the full guided journey, `afora configure` for targeted changes, or `afora channels add` to add channel accounts.
