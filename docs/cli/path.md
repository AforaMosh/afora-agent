---
summary: "CLI reference for `afora path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
Afora state is spread across human-edited markdown, commented JSONC
...
`afora path` gives those callers a stable address instead of a one-off
...
Skip `afora path` for ordinary whole-file edits, rich config migrations, or
...
```bash
afora path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
afora path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
afora path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
afora path resolve 'oc://AGENTS.md/runtime-safety/afora-gateway'
```
...
```bash
afora path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
afora path resolve 'oc://gateway.jsonc/version'

# Wildcard search
afora path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
afora path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
afora path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
afora path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
afora path emit ./AGENTS.md
```
...
afora path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
afora path set 'oc://afora.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
afora path set 'oc://afora.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
afora path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
afora path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
afora path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
afora path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
afora path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
afora path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
afora path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
afora path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
afora path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
afora path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
afora path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ afora path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ afora path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ afora path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ afora path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ afora path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ afora path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ afora path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: afora.invoke
```

```bash
$ afora path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ afora path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
afora path resolve 'oc://AGENTS.md/tools/gh/risk' --human
afora path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
afora path find 'oc://AGENTS.md/tools/**/risk'
afora path find 'oc://session.jsonl/[event=action]/userId'
afora path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
afora path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
afora path set 'oc://gateway.jsonc/version' '2.0'
afora path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ afora path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
afora path emit ./AGENTS.md
afora path emit ./gateway.jsonc --json
```
...
`afora path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__AFORA_REDACTED__` (verbatim or as a substring) is refused at write
  time.
