...
read_when:
  - Moving Afora runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.afora/state/afora.sqlite`
...
- Configuration stays file-backed: `afora.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside Afora's database.
...
- Raw stream logging uses `AFORA_RAW_STREAM=1` and writes
  `logs/raw-stream.jsonl` by default; `AFORA_RAW_STREAM_PATH` overrides the
  artifact path. The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` contract is not part of Afora runtime.
...
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/afora-agent.sqlite`.
- Config remains file-backed. `afora.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/afora.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/afora-state-schema.sql`,
      `src/state/afora-agent-schema.sql`,
...
- `afora doctor --fix` owns legacy file-to-database migration. Runtime
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the Afora `sessionId` as the canonical
...
former stores now write to `state/afora.sqlite` or the owning
`afora-agent.sqlite` database.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `afora-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/afora-agent-db.ts` opens
  `agents/<agentId>/agent/afora-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one Afora session to one or more external
  ...
  Afora session can represent multiple external DM peers without demoting
...
- TUI last-session restore pointers now live in typed shared
  ...
  excludes heartbeat sessions. `afora doctor --fix` strictly validates the
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `afora.json` remains the config store;
...
- Workspace bootstrap completion, attestation recency, and generated bootstrap
  ...
  `afora-workspace-state.json`, `.afora/workspace-state.json`, state-dir
  `workspace-attestations/*.attested`, or sibling `<workspace>.attested`
  sidecars. `afora doctor --fix` validates and claims legacy sources,
...
- GitHub Copilot compaction no longer writes `afora-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and Afora keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`AforaKit`) uses the same
  `state/afora.sqlite#table/device_identities` shape and row keys for device
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `afora update --channel` now fails on invalid legacy config and asks the
...
- APNs runtime reads and writes only `apns_registrations`. Explicit
  `afora doctor --fix` strictly imports the retired
...
- Node-host config now uses a typed singleton row in the shared SQLite database.
  ...
  remains; explicit `afora doctor --fix` strictly imports and removes it
...
- Plugin state now uses the shared `state/afora.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/afora.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `afora sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline output are
  opt-in JSONL artifacts. `AFORA_CACHE_TRACE_FILE`,
  `AFORA_ANTHROPIC_PAYLOAD_LOG_FILE`, `AFORA_RAW_STREAM_PATH`, and
  `AFORA_DIAGNOSTICS_TIMELINE_PATH` remain supported path controls. Gateway
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.afora-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.afora-wiki/state.json` or the unused
  `.afora-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `afora.json`. The config
...
- System-agent rescue pending approvals now use core SQLite plugin state instead
  of `crestodian/rescue-pending/*.json` or `afora/rescue-pending/*.json`.
...
- Codex app-server conversation bindings now key SQLite plugin state by
  Afora session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `afora-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the Afora temp root, not durable Afora state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$AFORA_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$AFORA_STATE_DIR/completions/*` cache
...
- Subagent inline attachments materialize under the child workspace's
  `.afora/attachments/<attachmentId>/` directory. The subagent registry keeps
  the attachment and root directories so lifecycle cleanup can remove them.
- CLI image hydration no longer maintains stable `afora-cli-images` cache
...
- The macOS companion has an opt-in, rotating `diagnostics.jsonl` writer under
  `~/Library/Logs/Afora`, alongside unified logging.
...
- `afora secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/afora.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- MCP OAuth sessions now use versioned `mcp_oauth_stores` rows in shared
  `state/afora.sqlite`. SDK-owned token, client-registration, and discovery
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.afora-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.afora-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store compressed SQLite plugin-blob rows
  instead of writing `.afora-wiki/cache/agent-digest.json` and
  `.afora-wiki/cache/claims.jsonl`. The cache is rebuildable, so doctor
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  Afora temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `afora backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `afora backup restore` validates the archive before extraction, reuses the
...
- `afora reset --scope config+creds+sessions` removes per-agent
  `afora-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureAforaModelCatalog`; there is no `models.json` compatibility API in
...
- The optional `memory-lancedb` plugin no longer creates
  `~/.afora/memory/lancedb` as an implicit Afora-managed store. It is an
...
`afora doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `afora migrate` must not import legacy Afora state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `afora doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/afora.sqlite`:
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `afora backup create` does this archive verification by default;
...
5. Verify every SQLite snapshot's file shape, then open canonical Afora
   ...
   `afora backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `afora sessions` listing command now read per-agent rows directly
...
6. Delete file-lock-shaped session mutation.
   ...
   - QMD writers no longer take Afora state leases. Runtime no longer
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `afora backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  Afora test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `afora-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- `afora/rescue-pending/*.json`
- `plugins/phone-control/armed.json`
- Memory Wiki `.afora-wiki/log.jsonl`
- Memory Wiki `.afora-wiki/state.json`
- Memory Wiki `.afora-wiki/locks/`
- Memory Wiki `.afora-wiki/source-sync.json`
- Memory Wiki `.afora-wiki/import-runs/*.json`
- Memory Wiki `.afora-wiki/cache/agent-digest.json`
- Memory Wiki `.afora-wiki/cache/claims.jsonl`
- ClawHub `.clawhub/lock.json`
- ClawHub `.clawhub/origin.json`
- Browser profile decoration `.afora-profile-decorated`
