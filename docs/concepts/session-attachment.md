...
Afora keeps shared session state on the Gateway. The Control UI, mobile
clients, ACP, `afora tui <target>`, and `afora attach <target>` project
...
Use `afora tui` when you want to continue the conversation in a terminal.
Use `afora attach` when you want a coding harness beside the session with a
temporary, session-scoped MCP grant.

Embedded local mode is separate: `afora tui --local`, `afora chat`, and
`afora terminal` use the local agent runtime and cannot accept a session
...
terminal…**. The dialog copies a credential-free `afora resume` command with
...
`cmd.exe`. Run it in an Afora CLI profile that is already configured for that
...
```bash
afora resume
afora resume agent:main:deploy-monitor
```
...
```bash
afora tui https://claw.example.com/dashboard/main/deploy-monitor-6db92d48
afora tui deploy-monitor-6db92d48
```

You can also paste a complete session URL directly at the CLI root:

```bash
afora https://claw.example.com/dashboard/main/deploy-monitor-6db92d48
```
...
Pass the same URL or reference to `afora attach`:

```bash
afora attach https://claw.example.com/dashboard/main/deploy-monitor-6db92d48
afora attach deploy-monitor-6db92d48
```
...
origin. Afora never reuses configured credentials or a stored device token
from another origin for that target. The credential-free command copied by
**Continue in terminal…** has a narrower rule: `afora resume` may reuse the
...
`AFORA_GATEWAY_TOKEN` and `AFORA_GATEWAY_PASSWORD` fallback while keeping
...
2. Open **Settings > Devices** in that Gateway's Control UI and approve the
   ...
   request with `afora devices approve --latest`, verify it, and run the
   printed `afora devices approve <requestId>` command.
3. Retry the original command. Afora stores the issued operator device token
...
| Session missing                    | The selected Gateway cannot find that key or short ID.                                   | For the configured Gateway, run `afora sessions list`. For a URL target, choose the session in that Gateway's Control UI.                                    |
...
| `pairing-required`                 | The device is new or an existing device needs a role, scope, or metadata approval.       | Approve the pending request in **Settings > Devices**, or preview it with `afora devices approve --latest` and run the printed exact-ID command, then retry. |
| `device-identity-required`         | The Gateway requires a signed device identity for this connection.                       | Use a current Afora client, let it create its device identity, and complete pairing.                                                                         |
| `scope-mismatch`                   | The stored device token is valid but lacks the requested operator scope.                 | Review `afora devices list`, approve the pending scope upgrade, and reconnect.                                                                               |
| `auth-rejected`                    | An explicit shared credential is wrong, or a paired-device token was revoked or rotated. | Verify explicit Gateway auth. For a stale device token, rotate it with `afora devices rotate --device <deviceId> --role operator` or pair again.             |
