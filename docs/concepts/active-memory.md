...
enabled, Afora indexes that agent's session transcripts and runs an Active
...
case-insensitive. If `allowedChatIds` is non-empty and Afora cannot
...
With those on, Afora appends diagnostic lines after the normal reply (as a
...
external context-engine plugin (`afora plugins install
...
store. By default, Afora removes the temporary sub-agent session rows after
...
2. For Remember across conversations, confirm the agent's effective
   ...
   `afora doctor` to verify the current memory provider supports protected
...
6. Verify memory search itself works with `afora status --deep`.
...
provider must support Afora's protected same-agent/private-session recall
path.

<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memory.search.provider` is unset, Afora uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `afora status --deep` to inspect the memory-search backend and
      index health.
