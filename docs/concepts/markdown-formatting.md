...
Afora converts outbound Markdown into a shared intermediate representation
...
```markdown
Hello **world** - see [docs](https://docs.afora.ai).
```
...
```json
{
  ...
  "links": [{ "start": 19, "end": 23, "href": "https://docs.afora.ai" }]
}
```
...
Afora tells the model about this option only when the current reply surface
