...
- `/usage cost` in chats: local cost summary aggregated from Afora session logs.
- CLI: `afora status --usage` prints a full per-provider usage/quota breakdown.
- CLI: `afora models status` lists OAuth/token auth profiles and shows a usage-window summary next to each provider that has one.
- Control UI: **Usage** shows provider plan and billing cards above Afora's session-derived token and estimated-cost analysis. Anthropic and OpenAI Admin API credentials add provider-reported today, 7-day, and 30-day spend, daily trends, token totals, top models, and cost categories.
...
`afora channels list` no longer prints provider usage; it points users to `afora status` or `afora models list` instead.
...
- OpenAI ChatGPT/Codex OAuth continues to show plan, quota windows, and credit balance. Set `OPENAI_ADMIN_KEY` to show organization cost and completions-usage history instead; optionally set `OPENAI_PROJECT_ID` to scope it to one project. Afora never sends inference credentials from `OPENAI_API_KEY`, provider config, or auth profiles to organization APIs because those keys may belong to custom endpoints.

Admin credentials take precedence because they provide actual organization billing. Afora does not combine these provider-reported totals with its local session estimates; the two sections intentionally answer different questions.
...
{
  "messages": {
    "usageTemplate": "~/.afora/usage-footer.json"
  }
}
```
...
Usage is hidden when no usable provider usage auth can be resolved. Afora
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Afora treats
  ...
  quota, so Afora inverts them before display; count-based fields win when
  present.
  - Window labels come from provider hours/minutes fields when present, then
    fall back to the `start_time` / `end_time` span.
  - If the coding-plan endpoint returns `model_remains`, Afora prefers the
...
- **OpenAI (Codex/ChatGPT plan)**: OAuth tokens in auth profiles (`ChatGPT-Account-Id`
  ...
  credits; Afora does not label them as dollars. `OPENAI_ADMIN_KEY` adds
