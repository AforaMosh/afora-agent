...
When a normal prompt arrives while a session run is already streaming and the queue mode is `steer` (the default, no config needed), Afora tries to send that prompt into the active runtime. Afora and the native Codex app-server harness implement the delivery details differently.
...
Steering does not interrupt a tool call that is already running. The Afora runtime checks at tool-launch boundaries as well as model boundaries:

1. The assistant asks for tool calls.
2. In sequential mode, Afora checks immediately before each call starts, including after asynchronous resolution, validation, and pre-execution hooks.
3. A running call finishes. If a steer is waiting afterward, the unstarted sequential tail is skipped.
4. In parallel mode, Afora prepares calls first, then checks once immediately before launching the prepared calls. Calls that have crossed that checkpoint continue together.
...
6. Afora appends the exact drained steering message before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of Afora runtime's internal steering queue. Afora batches queued prompts for the configured quiet window, then sends a single `turn/steer` request with all collected user input in arrival order. Codex's upstream turn scheduler owns its tool scheduling and consumes accepted steering at the next model boundary; Afora does not add per-tool preemption to that runtime.

Codex review and manual compaction turns reject same-turn steering. When a runtime cannot accept steering in `steer` mode, Afora waits for the active run to finish before starting the prompt.

## Tool launch boundaries

Afora distinguishes started work from requested work:

- A sequential call that is already running completes. Later calls have not started, so Afora returns synthetic skipped results for them and lets the model reconsider with the steer visible.
...
- Afora preserves the runtime's configured steering drain mode and FIFO order. One-at-a-time consumers keep later messages for later boundaries; `all` consumers inject the queued FIFO batch together. Codex receives messages collected during its quiet window as one batched `turn/steer`.
- With `/queue collect`, Afora does not steer. It waits until the active run ends, then creates a followup turn with compatible queued messages after the debounce window.
- With `/queue interrupt`, Afora aborts the active run and starts the newest message instead of steering.
