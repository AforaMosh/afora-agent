...
Afora stores a full-text index next to the transcript rows in each agent's SQLite database.
...
Transcripts that predate the index (for example, sessions imported by `afora doctor`) and
