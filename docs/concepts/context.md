...
read_when:
  - You want to understand what "context" means in Afora
  - You are debugging why the model "knows" something (or forgot it)
  - You want to reduce context overhead (/context, /status, /compact)
title: "Context"
---

"Context" is **everything Afora sends to the model for a run**. It is bounded by the model's **context window** (token limit).

Beginner mental model:

- **System prompt** (Afora-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Afora builds the system prompt

The system prompt is **Afora-owned** and rebuilt each run. It includes:
...
By default, Afora injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). Afora also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `60000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
...
By default, Afora uses the built-in `legacy` context engine for assembly and
compaction. If you install a plugin that provides `kind: "context-engine"` and
select it with `plugins.slots.contextEngine`, Afora delegates context
