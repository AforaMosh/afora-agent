...
<AccordionGroup>
  <Accordion title="Model refs and CLI helpers">
    ...
    - CLI helpers: `afora onboard`, `afora models list`, `afora models set <provider/model>`.
  ...
  <Accordion title="Adding provider auth does not change your primary model">
    `afora configure` preserves an existing `agents.defaults.model.primary` when you add or reauth a provider. `afora models auth login` does the same unless you pass `--set-default`. Provider plugins may still return a recommended default model in their auth config patch, but Afora treats that as "make this model available" when a primary model already exists, not "replace the current primary model."

    To intentionally switch the default model, use `afora models set <provider/model>` or `afora models auth login --provider <id> --set-default`.
  ...
  <Accordion title="OpenAI provider/runtime split">
    ...
    - Authored Completions adapters, custom endpoints, and routes with authored request behavior stay on Afora. Plaintext official HTTP endpoints are rejected.
    - legacy Codex model refs are legacy config that doctor rewrites to `openai/<model>`.
    - Provider/model `agentRuntime.id: "afora"` explicitly keeps an otherwise eligible route on Afora. `agentRuntime.id: "codex"` requires Codex and fails closed when the effective route is not Codex-compatible.
  ...
  <Accordion title="CLI runtimes">
    ...
    Legacy `claude-cli/*` and `google-gemini-cli/*` refs migrate back to canonical provider refs with the runtime recorded separately. Legacy `codex-cli/*` refs migrate to `openai/*` and use the Codex app-server route; Afora no longer keeps a bundled Codex CLI backend.
...
<AccordionGroup>
  <Accordion title="Key sources and priority">
    ...
    - `AFORA_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- Optional rotation: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, plus `AFORA_LIVE_OPENAI_KEY` (single override)
...
- Verify account/model availability with `afora models list --provider openai` if a specific install or API key behaves differently.
- CLI: `afora onboard --auth-choice openai-api-key`
...
- Hidden Afora attribution headers (`originator`, `version`, `User-Agent`) apply only on native OpenAI traffic to `api.openai.com`, not generic OpenAI-compatible proxies
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `AFORA_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/claude-opus-5`
- CLI: `afora onboard --auth-choice apiKey`
- Direct public Anthropic requests support the shared `/fast` toggle and `params.fastMode`, including API-key and OAuth-authenticated traffic sent to `api.anthropic.com`; Afora maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
<Note>
Claude CLI reuse (`claude -p`) is a sanctioned Afora integration path. Anthropic setup-token auth remains supported, but Afora prefers Claude CLI reuse when available.
</Note>
...
- CLI: `afora onboard --auth-choice openai` or `afora models auth login --provider openai`
- Afora's embedded ChatGPT Responses transport defaults to `auto` (WebSocket-first, SSE fallback).
- `agents.defaults.models["openai/<model>"].params.transport` and `params.serviceTier` are authored embedded-provider request settings. They keep implicit runtime selection on Afora; native Codex owns its app-server transport and service tier.
- Valid model-scoped `params.fastMode` / `params.fast_mode` values and valid cutoff keys are portable typed agent-runtime controls. They do not count as authored provider request params and do not select a runtime. Pin `agentRuntime.id: "afora"` or `agentRuntime.id: "codex"` when a recipe depends on one runtime.
- Hidden Afora attribution headers (`originator`, `version`, `User-Agent`) are only attached on native Codex traffic to `chatgpt.com/backend-api`, not generic OpenAI-compatible proxies
...
- Use provider/model `agentRuntime.id: "afora"` to keep an otherwise eligible route on the built-in runtime. With runtime unset or `auto`, only an exact official HTTPS Responses/ChatGPT-compatible route with no authored provider request override may select Codex implicitly.
- Legacy Codex GPT refs are legacy state, not a live provider route. Use canonical `openai/*` refs for new agent config, and run `afora doctor --fix` to migrate `codex/*` and `openai-codex/*` refs while preserving their native Codex semantics with model-scoped `agentRuntime.id: "codex"`. Existing explicit canonical `openai/gpt-5.5` selections are not upgraded.
...
- CLI: `afora onboard --auth-choice opencode-zen` or `afora onboard --auth-choice opencode-go`
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `AFORA_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3.5-flash`
- Compatibility: legacy Afora config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- Alias: `google/gemini-3.1-pro` is accepted and normalized to Google's live Gemini API id, `google/gemini-3.1-pro-preview`
- CLI: `afora onboard --auth-choice gemini-api-key`
...
- Direct Gemini runs also accept `agents.defaults.models["google/<model>"].params.cachedContent` (or legacy `cached_content`) to forward a provider-native `cachedContents/...` handle; Gemini cache hits surface as Afora `cacheRead`
...
Afora does not create Gemini CLI OAuth or Antigravity OAuth profiles. Connect
...
Gemini CLI uses `stream-json` by default. Afora reads assistant stream
...
- CLI: `afora onboard --auth-choice zai-api-key`
...
- CLI: `afora onboard --auth-choice ai-gateway-api-key`
...
Install `@afora/moonshot-provider` before onboarding. Add an explicit `models.providers.moonshot` entry only when you need to override the base URL or model metadata:

- Provider: `moonshot`
- Auth: `MOONSHOT_API_KEY`
- Example model: `moonshot/kimi-k3`
- CLI: `afora onboard --auth-choice moonshot-api-key` or `afora onboard --auth-choice moonshot-api-key-cn`
...
- CLI: `afora onboard --auth-choice volcengine-api-key`
...
In onboarding/configure model pickers, the Volcengine auth choice prefers both `volcengine/*` and `volcengine-plan/*` rows. If those models are not loaded yet, Afora falls back to the unfiltered catalog instead of showing an empty provider-scoped picker.
...
- Plugin: `@afora/byteplus-provider`
- Provider: `byteplus` (coding: `byteplus-plan`)
- Auth: `BYTEPLUS_API_KEY`
- Example model: `byteplus-plan/ark-code-latest`
- CLI: `afora onboard --auth-choice byteplus-api-key`
...
In onboarding/configure model pickers, the BytePlus auth choice prefers both `byteplus/*` and `byteplus-plan/*` rows. If those models are not loaded yet, Afora falls back to the unfiltered catalog instead of showing an empty provider-scoped picker.
...
- CLI: `afora onboard --auth-choice synthetic-api-key`
...
<Note>
On MiniMax's Anthropic-compatible streaming path, Afora disables thinking by default for the M2.x family unless you explicitly set it; MiniMax-M3 (and M3.x) stays on the provider's omitted/adaptive thinking path by default. `/fast on` rewrites `MiniMax-M2.7` to `MiniMax-M2.7-highspeed`.
</Note>
...
Afora uses LM Studio's native `/api/v1/models` and `/api/v1/models/load` for discovery + auto-load, with `/v1/chat/completions` for inference by default. If you want LM Studio JIT loading, TTL, and auto-evict to own model lifecycle, set `models.providers.lmstudio.params.preload: false`. See [/providers/lmstudio](/providers/lmstudio) for setup and troubleshooting.
...
Ollama is detected locally at `http://127.0.0.1:11434` when you opt in with `OLLAMA_API_KEY`, and the bundled provider plugin adds Ollama directly to `afora onboard` and the model picker. See [/providers/ollama](/providers/ollama) for onboarding, cloud/local mode, and custom configuration.
...
<AccordionGroup>
  <Accordion title="Default optional fields">
    For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional. When omitted, Afora defaults to:
  ...
  <Accordion title="Proxy-route shaping rules">
    - For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), Afora forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
    - Proxy-style OpenAI-compatible routes also skip native OpenAI-only request shaping: no `service_tier`, no Responses `store`, no Completions `store`, no prompt-cache hints, no OpenAI reasoning-compat payload shaping, and no hidden Afora attribution headers.
...
```bash
afora onboard --auth-choice opencode-zen
afora models set opencode/claude-opus-4-6
afora models list
```
