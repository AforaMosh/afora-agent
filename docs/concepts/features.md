---
summary: "Afora capabilities across channels, routing, media, and UX."
read_when:
  - You want a full list of what Afora supports
title: "Features"
---
...
- Telegram and WebChat ship with the core install; every other channel is an
  official plugin installed with `afora plugins install @afora/<id>` (or on demand
  during `afora onboard` / `afora channels add`)
...
- External plugin channels maintained outside the Afora repo: WeChat, Yuanbao, and Zalo ClawBot
...
<CardGroup cols={2}>
  ...
  <Card title="Plugins" href="/tools/plugin" icon="plug">
    Official and external plugins that extend Afora.
  </Card>
</CardGroup>
