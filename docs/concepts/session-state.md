...
Afora appends a typed event to the shared state database (`session_state_events`) when a watched session materially changes. Events carry metadata and a one-line summary — never message content.
...
| `adopted`              | A catalog session is adopted into Afora               | No (log only)     |
...
- Upstream self-echo detection compares normalized user text. An external prompt matching one of the session's 10 most recent Afora-side user messages is treated as self-echo.
