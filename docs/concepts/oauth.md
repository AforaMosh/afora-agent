---
summary: "OAuth in Afora: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand Afora OAuth end-to-end
...
Afora supports OAuth ("subscription auth") for providers that offer it,
notably **OpenAI Codex (ChatGPT OAuth)** and **Anthropic Claude CLI reuse**.
For Anthropic, the practical split is:

- **Anthropic API key**: normal Anthropic API billing.
- **Anthropic Claude CLI / subscription auth inside Afora**: Anthropic staff
  told us this usage is allowed again, so Afora treats Claude CLI reuse and
...
Afora stores both OpenAI API-key auth and ChatGPT/Codex OAuth under the
...
`afora doctor --fix`; use `openai:*` profile ids and `auth.order.openai` for
new config.
...
```bash
afora models auth login --provider <id>
```
...
issued for the same user/app. Practical symptom: log in via Afora _and_
via Claude Code / Codex CLI, and one of them randomly gets logged out later.

To reduce that, Afora treats the auth profile store as a **token sink**:

- the runtime reads credentials from one place per agent
- multiple profiles can coexist and route deterministically
- external CLI reuse is provider-specific: once Afora owns a local OAuth
  profile for a provider, the local refresh token is canonical. If that local
  refresh token is rejected, Afora reports the profile for
  ...
  `openai:default`-style profile before Afora owns OAuth for that
  provider; after that, Afora-owned refreshes stay canonical
...
`afora doctor --fix` once after upgrading. Doctor imports verified values,
...
The database and migration sources respect `$AFORA_STATE_DIR`. Full reference: [/gateway/configuration-reference#auth-storage](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, Afora uses read-through
...
Afora supports Anthropic Claude CLI reuse and `claude -p` as a sanctioned
...
available as a supported token-auth path, but Afora prefers Claude CLI
reuse when it is available.

<Warning>
Anthropic's public Claude Code docs say direct Claude Code use stays within
Claude subscription limits, and Anthropic staff told us Afora-style Claude
CLI usage is allowed again. Afora therefore treats Claude CLI reuse and
`claude -p` usage as sanctioned for this integration unless Anthropic
...
If you want other subscription-style options in Afora, see [OpenAI
...
Afora's interactive login flows are implemented in `afora/plugin-sdk/llm.ts` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. create the token by running `claude setup-token` on any machine with Claude Code, then start Anthropic setup-token or paste-token from Afora
2. Afora stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including Afora workflows.
...
`afora models auth list --provider openai` after repair before copying
...
3. try to capture the callback on `http://localhost:1455/auth/callback` (the
   ...
   override with `AFORA_OAUTH_CALLBACK_HOST`)
...
Wizard path is `afora onboard` → auth choice `openai`.
...
- externally managed CLI credentials (Claude CLI, narrow Codex CLI bootstrap;
  ...
  spending a copied refresh token. If a managed refresh fails, Afora
...
```bash
afora models auth list --provider <id>
```
