...
| `Mantis Slack Desktop Smoke`      | manual dispatch                                                                            | Leases a Crabbox Linux desktop (defaults to `aws`, choice of `hetzner`), runs the selected Slack QA scenario against the candidate, records the desktop, generates a motion preview, uploads artifacts, and posts PR evidence when a PR number is given. Set `gateway_setup=true` only for a persistent manual gateway session. |
