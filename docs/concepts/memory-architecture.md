---
summary: "End-to-end architecture of Afora memory: tiers, provenance, dreaming, recall lanes, the user model, and standing intents"
title: "Memory architecture"
sidebarTitle: "Memory architecture"
read_when:
  - You want the complete picture of how Afora memory works end to end
...
Afora memory is a set of plain files and one SQLite index, organized into
...
2. **Writing is the hard part.** Retrieval over notes files is competitive
   ...
   arXiv:2410.10813). Afora therefore moves curation off the busy reply
   path and into a dedicated background pass.
3. **The write path is the security boundary.** Content-level scanning of
   memory cannot catch poisoned facts reliably, so Afora enforces
...
```markdown
- Use the release helper for package validation. <!-- project: github.com/AforaMosh/afora-agent -->
```
...
event benchmarks). Afora therefore compiles intentions out of the model:
...
measure poorly. Afora defends structurally:
...
more conservatively they write; Afora keeps those properties even with
