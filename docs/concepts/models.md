...
- **Use the configured default:** `/model default` (with or without `-s`) clears the current session model and auth-profile selection so it inherits the current effective configured default. It does not restore an older configured default that a previous owner/admin `/model <model>` replaced.
