...
- Eligible automatic attempt-1 runs transport preflight's repacked `.git` directory to shared Linux Node jobs as a one-day raw tar artifact. Preflight materializes the exact diff base, creates its canonical remote ref, and repacks with lazy fetching disabled; consumers verify the raw artifact digest, extract only `.git`, and reconstruct the working tree with an offline forced checkout. They then verify exact `HEAD`, the required setup action, and an offline binary diff against `CHECKOUT_BASE_SHA` when present. The runtime `runner.environment` check is authoritative: only self-hosted attempt-1 jobs download, so GitHub-hosted jobs inside a hybrid run, hosted reruns, manual dispatches, unknown-author pull requests, non-canonical repositories, and the `github` backend retain the existing fetch path without artifact cost. A returning-contributor fork may use the artifact only when its job already routes to Blacksmith; the artifact is scoped to that workflow run and is not a cross-run cache. Repack, tar, upload, download, extraction, checkout, or verification failure is an optimization miss and falls back to the canonical five-attempt network checkout.
