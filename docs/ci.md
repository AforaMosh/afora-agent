...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| `preflight`                        | Detect changed scopes and build the CI manifest; for eligible automatic runs with Blacksmith Linux consumers, publish repacked git metadata before fanout; on canonical Node-relevant `main` and same-repo PRs, also publish or restore the exact dependency cache                                       | Always on non-draft pushes and PRs                     |
...
1. `preflight` decides which lanes exist at all. The `docs-scope` and `changed-scope` logic are steps inside this job, not standalone jobs. Canonical `main` starts immediately in one of two parity slots; each slot admits one complete run and coalesces later pushes into its newest pending tip. For eligible automatic attempt-1 runs, preflight repacks and uploads real git metadata only when the manifest selects a downstream Blacksmith Linux consumer. The pack and raw-file upload are separate GitHub steps, so local repack/tar time, upload time, and uploaded bytes remain directly measurable. On Node-relevant canonical `main` pushes and same-repository pull requests, preflight is also the sole exact dependency-cache writer; downstream jobs wait for it, then restore the immutable archive or fall back to the ordinary pnpm-store cache on a miss.
...
- Eligible automatic attempt-1 runs transport preflight's repacked `.git` directory to shared Linux Node jobs as a one-day raw tar artifact. Preflight materializes the exact diff base, creates its canonical remote ref, and repacks with lazy fetching disabled. Consumers use the pinned official `download-artifact` v8 action, allowing Blacksmith to accelerate the artifact transfer, then independently verify the raw digest, extract only `.git`, and reconstruct the working tree with an offline forced checkout. Reported checkout cost is the action download plus shell materialization. Consumers then verify exact `HEAD`, the required setup action, and an offline binary diff against `CHECKOUT_BASE_SHA` when present. The runtime `runner.environment` check is authoritative: only self-hosted attempt-1 jobs download, so GitHub-hosted jobs inside a hybrid run, hosted reruns, manual dispatches, unknown-author pull requests, non-canonical repositories, and the `github` backend retain the existing fetch path without artifact cost. A returning-contributor fork may use the artifact only when its job already routes to Blacksmith; the artifact is scoped to that workflow run and is not a cross-run cache. Repack, tar, upload, download, extraction, checkout, or verification failure is an optimization miss and falls back to the canonical five-attempt network checkout.
