...
- Flags are case-insensitive strings, resolved from `diagnostics.flags` in
  config plus the `AFORA_DIAGNOSTICS` env override, deduped and lowercased.
...
```bash
AFORA_DIAGNOSTICS=telegram.http,brave.http
```
...
`AFORA_DIAGNOSTICS=0` disables flags from both env and config for that
...
```bash
AFORA_DIAGNOSTICS=profiler afora gateway run
```

Enable only reply-dispatch profiler spans:

```bash
AFORA_DIAGNOSTICS=reply.profiler afora gateway run
```

Enable only Codex app-server startup/tool/thread profiler spans:

```bash
AFORA_DIAGNOSTICS=codex.profiler afora gateway run
```
...
`AFORA_DIAGNOSTICS=0` to override every diagnostics flag for that run.
...
AFORA_DIAGNOSTICS_TIMELINE_PATH=/tmp/afora-timeline.jsonl \
afora gateway run
```
...
The output path always comes from `AFORA_DIAGNOSTICS_TIMELINE_PATH`, even
...
are missing because Afora has not read config yet; subsequent startup spans
are captured normally.

`AFORA_DIAGNOSTICS=1`, `=all`, and `=*` also enable the timeline, since they
...
`timeline`: set `AFORA_DIAGNOSTICS_EVENT_LOOP=1` (or `on`/`true`/`yes`) on
top of enabling the timeline.

Timeline records use the `afora.diagnostics.v1` envelope and can include
...
afora --profile work logs --plain
```

Filter for Telegram HTTP diagnostics:

```bash
afora logs --plain --limit 5000 | rg "telegram http error"
```

Filter for Brave Search HTTP diagnostics:

```bash
afora logs --plain --limit 5000 | rg "brave http"
```

Or tail while reproducing:

```bash
afora logs --follow --plain | rg "telegram http error"
```

For remote gateways, use `afora logs --follow` instead (see
[/cli/logs](/cli/logs)).
