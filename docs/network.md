...
This hub links the core docs for how Afora connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`afora gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__afora__/canvas/`, `/__afora__/a2ui/`), protected by Gateway auth when bound beyond loopback.
