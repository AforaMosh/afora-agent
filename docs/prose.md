...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In Afora it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to Afora primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In Afora it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    afora plugins enable open-prose
    ```
...
`afora plugins install ./extensions/open-prose`
...
## Afora runtime mapping

OpenProse programs map to Afora primitives:

| OpenProse concept         | Afora tool                                   |
| ------------------------- | ----------------------------------------------- |
...
executed. Use Afora tool allowlists and approval gates to control side
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    Afora's native multi-agent coordination layer.
  </Card>
