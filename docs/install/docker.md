---
summary: "Optional Docker-based setup and onboarding for Afora"
read_when:
...
<Steps>
  <Step title="Build the image">
    ...
    This builds the gateway image locally as `afora:local`. To use a pre-built image instead:

    ```bash
    export AFORA_IMAGE="ghcr.io/AforaMosh/afora-agent:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published first to the [GitHub Container Registry](https://github.com/AforaMosh/afora-agent/pkgs/container/afora). GHCR is the primary registry for release automation, pinned deployments, and provenance checks. The same release publishes a Docker Hub mirror at `AforaMosh/afora-agent`:

    ```bash
    export AFORA_IMAGE="AforaMosh/afora-agent:latest"
    ./scripts/docker/setup.sh
    ```

    Use `ghcr.io/AforaMosh/afora-agent` or `AforaMosh/afora-agent` and avoid unofficial mirrors, which don't share Afora's release timing or retention policy. Version-specific tags include releases such as `2026.2.26` and prereleases such as `2026.2.26-beta.1`. Stable releases move `latest` and `main`; trailing-month Gateway releases move only `extended-stable`. Variants include `slim`, `main-slim`, `extended-stable-slim`, `latest-browser`, `main-browser`, and `extended-stable-browser`. The default images bundle the `codex` and `diagnostics-otel` plugins. A `-browser` variant also ships with Chromium baked in, useful for the [sandboxed browser](/gateway/sandboxing#sandboxed-browser) tool without a first-run Playwright install.
  ...
  <Step title="Airgapped rerun">
    ...
    ```bash
    docker load -i afora-image.tar
    export AFORA_IMAGE="ghcr.io/AforaMosh/afora-agent:latest"
    ./scripts/docker/setup.sh --offline
    ```

    `--offline` verifies `AFORA_IMAGE` already exists locally, disables implicit Compose pulls/builds, then runs the normal flow: `.env` sync, permission fixes, onboarding, gateway config sync, Compose startup.

    If `AFORA_SANDBOX=1`, offline setup also checks the configured default and per-agent sandbox images on the daemon behind `AFORA_DOCKER_SOCKET`, including the browser-contract label on Docker-backed browser images. If a required image is missing or stale, setup exits without changing sandbox config rather than reporting a broken success.
  ...
  <Step title="Complete onboarding">
    ...
    Pre-start onboarding and config writes run through `afora-gateway` directly (with `--no-deps --entrypoint node`), since `afora-cli` shares the gateway's network namespace and only works once the gateway container exists.
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm afora-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ```bash
    # WhatsApp (QR)
    docker compose run --rm afora-cli channels login

    # Telegram
    docker compose run --rm afora-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm afora-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker compose run -T --rm --no-deps --entrypoint node afora-gateway \
...
The channel command fails before changing config if a plugin-declared environment variable is missing. Keep `TELEGRAM_BOT_TOKEN` in `.env` after bootstrap: `--use-env` leaves credential lookup to the environment without copying the token into `afora.json`, and the running Gateway needs the same variable. When channel config changes after startup, the Gateway's config watcher hot-reloads the affected channel automatically.

See [`afora channels`](/cli/channels) for credential-flag alternatives and other channel plugins.
...
docker build \
  ...
  --build-arg "AFORA_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}" \
  -t afora:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node afora-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `AFORA_EXTRA_MOUNTS` or `AFORA_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`; include it after any `docker-compose.override.yml` you maintain yourself, e.g. `-f docker-compose.yml -f docker-compose.override.yml -f docker-compose.extra.yml`.
</Note>

### Upgrading container images

When you replace the Afora image but keep the same mounted state/config, the
...
same image once with `afora doctor --fix` as the container command, using the
same state/config mounts the gateway uses:

```bash
docker run --rm -v <afora-state>:/home/node/.afora <image> afora doctor --fix
podman run --rm -v <afora-state>:/home/node/.afora <image> afora doctor --fix
```
...
```bash
docker compose run --rm afora-cli doctor --json
```
...
| Variable                                        | Purpose                                                                                                           |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `AFORA_IMAGE`                                | Use a remote image instead of building locally                                                                    |
| `AFORA_IMAGE_APT_PACKAGES`                   | Install extra apt packages during build (space-separated). Legacy alias: `AFORA_DOCKER_APT_PACKAGES`           |
| `AFORA_IMAGE_PIP_PACKAGES`                   | Install extra Python packages during build (space-separated)                                                      |
| `AFORA_EXTENSIONS`                           | Compile/package supported selected plugins and install their runtime dependencies (comma- or space-separated ids) |
| `AFORA_DOCKER_BUILD_NODE_OPTIONS`            | Override the local source-build Node options (default `--max-old-space-size=8192`)                                |
| `AFORA_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB` | Override the local source-build tsdown heap in MB                                                                 |
| `AFORA_DOCKER_BUILD_SKIP_DTS`                | Skip declaration output during runtime-only local image builds (default `1`)                                      |
| `AFORA_INSTALL_BROWSER`                      | Bake Chromium + Xvfb into the image at build time                                                                 |
| `AFORA_EXTRA_MOUNTS`                         | Extra host bind mounts (comma-separated `source:target[:opts]`)                                                   |
| `AFORA_HOME_VOLUME`                          | Persist `/home/node` in a named Docker volume                                                                     |
| `AFORA_TZ`                                   | Set the gateway and CLI container timezone to an IANA name (default `UTC`)                                        |
| `AFORA_SANDBOX`                              | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                                                            |
| `AFORA_SKIP_ONBOARDING`                      | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)                                                   |
| `AFORA_DOCKER_SOCKET`                        | Override the Docker socket path                                                                                   |
| `AFORA_DISABLE_BONJOUR`                      | Force Bonjour/mDNS advertising on (`0`) or off (`1`); see [Bonjour / mDNS](/install/docker#bonjour-%2F-mdns)      |
| `AFORA_DISABLE_BUNDLED_SOURCE_OVERLAYS`      | Disable bundled plugin source bind-mount overlays                                                                 |
...
| `AFORA_OTEL_PRELOADED`                       | Skip starting a second OpenTelemetry SDK when one is preloaded                                                    |

The official image ships no Homebrew. During onboarding, Afora hides brew-only skill dependency installers in a Linux container without `brew`; provide those dependencies through a custom image or install manually. Use `AFORA_IMAGE_APT_PACKAGES` for Debian-packaged dependencies and `AFORA_IMAGE_PIP_PACKAGES` for Python dependencies (runs `python3 -m pip install --break-system-packages` at build time, so pin versions and use only indexes you trust).
...
```bash
AFORA_DOCKER_BUILD_NODE_OPTIONS=--max-old-space-size=4096 AFORA_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=4096
```

### Source-built images with selected plugins

`AFORA_EXTENSIONS` selects plugin manifest ids from the source checkout;
...
`afora.build.bundledDist: false`; that marker still preserves the plugin's
...
already part of the root Afora runtime, so the ClickClack image selects only
...
build_gateway_image() {
  ...
  docker buildx build \
    ...
    --build-arg "AFORA_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}" \
    --build-arg "AFORA_EXTENSIONS=${selected_plugin}" \
    --build-arg AFORA_INSTALL_BROWSER= \
    --provenance=mode=max \
    --sbom=true \
    --tag "${REGISTRY}/afora-${gateway}:${SOURCE_SHA}" \
    --push \
    .
}
...
docker buildx imagetools inspect \
  "${REGISTRY}/afora-clickclack:${SOURCE_SHA}"
# Deploy: registry.example.com/fakeco/afora-clickclack@sha256:<manifest-digest>
```
...
separate x86_64 appliance archive containing an Afora npm tarball and pins
the Node, archive, and manifest digests. Build that appliance independently
from the same landed Afora source.

To test bundled plugin source against a packaged image, mount one plugin source directory over its packaged source path, e.g. `AFORA_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`. That overrides the matching compiled `/app/dist/extensions/synology-chat` bundle for the same plugin id.
...
```bash
export AFORA_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="afora-gateway"
./scripts/docker/setup.sh
```

Official prebuilt images already bundle `diagnostics-otel`; install `clawhub:@afora/diagnostics-otel` yourself only if you removed it. To enable export, allow and enable the `diagnostics-otel` plugin in config, then set `diagnostics.otel.enabled=true` (see the full example in [OpenTelemetry export](/gateway/opentelemetry)). Collector auth headers go through `diagnostics.otel.headers`, not Docker environment variables.

Prometheus metrics reuse the already-published Gateway port. Install `clawhub:@afora/diagnostics-prometheus`, enable the `diagnostics-prometheus` plugin, then scrape:
...
```bash
docker compose exec afora-gateway sh -lc 'node dist/index.js gateway health --token "$AFORA_GATEWAY_TOKEN"'
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `AFORA_GATEWAY_BIND=lan` so `http://127.0.0.1:18789` on the host works with Docker port publishing.
...
```bash
export AFORA_IMAGE="ghcr.io/AforaMosh/afora-agent:latest"
export AFORA_HOME_VOLUME="afora_home"
./scripts/docker/setup.sh
```
...
. ./.env
set +a
export AFORA_HOME_VOLUME="${AFORA_HOME_VOLUME:-afora_home}"
./scripts/docker/setup.sh
```

If `.env` contains values your shell can't source, re-export what you rely on manually first (`AFORA_IMAGE`, ports, bind mode, custom paths, `AFORA_EXTRA_MOUNTS`, sandbox, skip-onboarding). The generated overlay mounts the home volume for both `afora-gateway` and `afora-cli`; run the remaining commands with that overlay (and `docker-compose.override.yml` first, if you use one):

```bash
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint sh afora-cli -lc \
  'curl -fsSL https://claude.ai/install.sh | bash'
```

The native installer writes `claude` to `/home/node/.local/bin/claude`. The
Afora image includes `/home/node/.local/bin` on `PATH`, so the bundled
...
```bash
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint /home/node/.local/bin/claude afora-cli auth login
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint /home/node/.local/bin/claude afora-cli auth status --text
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  afora-cli models auth login \
  --provider anthropic --method cli --set-default
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  afora-cli models list --provider anthropic
```
...
`AFORA_HOME_VOLUME` persists the native install under `/home/node/.local/bin` and `/home/node/.local/share/claude`, plus Claude Code settings/auth under `/home/node/.claude` and `/home/node/.claude.json`. Persisting only `/home/node/.afora` is not enough; if you use `AFORA_EXTRA_MOUNTS` instead of a home volume, mount all of those Claude paths into both services.
...
Docker bridge networking usually doesn't forward Bonjour/mDNS multicast (`224.0.0.251:5353`) reliably. When `AFORA_DISABLE_BONJOUR` is unset, the bundled Bonjour plugin auto-disables LAN advertising once it detects it's running in a container, so it won't crash-loop retrying multicast the bridge drops. Set `AFORA_DISABLE_BONJOUR=1` to force it off regardless of detection, or `0` to force it on (only on host networking, macvlan, or another network where mDNS multicast is known to work).
...
Docker Compose bind-mounts `AFORA_CONFIG_DIR` to `/home/node/.afora`, `AFORA_WORKSPACE_DIR` to `/home/node/.afora/workspace`, and `AFORA_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/afora`, so those paths survive container replacement. When a variable is unset, `docker-compose.yml` falls back under `${HOME}`, or `/tmp` if `HOME` itself is missing, so `docker compose up` never emits an empty-source volume spec on bare environments.

That mounted config directory holds:

- `afora.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `AFORA_GATEWAY_TOKEN`

The auth-profile secret directory stores the local encryption key for OAuth-backed auth profile token material. Keep it with your Docker host state, but separate from `AFORA_CONFIG_DIR`.

Installed downloadable plugins store package state under the mounted Afora home, so install records and package roots survive container replacement; gateway startup does not regenerate bundled-plugin dependency trees.

For full VM persistence details, see [Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** `media/`, per-agent SQLite databases, legacy session JSONL transcripts, the shared SQLite state database, installed plugin package roots, and rolling file logs under `/tmp/afora/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/AforaMosh/afora-agent/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export AFORA_SANDBOX=1
    export AFORA_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm afora-cli gateway probe
    docker compose run -T --rm afora-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `afora-cli` uses `network_mode: "service:afora-gateway"` so CLI commands can reach the gateway over `127.0.0.1`. Treat this as a shared trust boundary. The compose config drops `NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on both `afora-gateway` and `afora-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in afora-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network `afora-cli` sidecar after `NET_RAW` is dropped, showing up as `EAI_AGAIN` during npm-backed commands like `afora plugins install`. Keep the default hardened compose file for normal operation. The override below restores default capabilities for the `afora-cli` container only — use it for the one-off command that needs registry access, not as your default invocation:
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm afora-cli plugins install <package>
    ```

    If you already created a long-running `afora-cli` container, recreate it with the same override — `docker compose exec`/`docker exec` can't change Linux capabilities on an already-created container.

  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on `/home/node/.afora`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/afora-config /path/to/afora-workspace
    ```

    The same mismatch can show up as `blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root)` followed by `plugin present but blocked` — the process uid and the mounted plugin directory owner disagree. Prefer running as the default uid 1000 and fixing the bind mount ownership. Only chown `/path/to/afora-config/npm` to `root:root` if you intentionally run Afora as root long term.
  ...
  <Accordion title="Faster rebuilds">
    ...
    the `/usr/local/bin/afora` symlink. Dependabot refreshes the reviewed base
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export AFORA_HOME_VOLUME="afora_home"`
    2. **Bake system deps**: `export AFORA_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export AFORA_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export AFORA_INSTALL_BROWSER=1`, or use the official `-browser` image tag
    5. **Persist browser downloads and caches**: use `AFORA_HOME_VOLUME` or `AFORA_EXTRA_MOUNTS`. Afora auto-detects the image's Playwright-managed Chromium on Linux.
...
Scanner totals can include Debian findings that the distribution marks `wont-fix`. To rebuild locally against current base and package metadata, run `docker build --pull -t afora:local .`.

### Weekly image refreshes

The `latest*`, `main*`, and `extended-stable*` moving tags are rebuilt weekly from the same tagged release source so they pick up current OS security updates between Afora releases. Stable and extended-stable refreshes remain separate, and beta images are not rebuilt on this schedule.
...
<AccordionGroup>
  ...
  <Accordion title="Custom tools not found in sandbox">
    Afora runs commands with `sh -lc` (login shell), which sources `/etc/profile` and may reset PATH. Set `docker.env.PATH` to prepend your custom tool paths, or add a script under `/etc/profile.d/` in your Dockerfile.
  </Accordion>
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm afora-cli dashboard --no-open
    docker compose run --rm afora-cli devices list
    docker compose run --rm afora-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm afora-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm afora-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Afora up to date
