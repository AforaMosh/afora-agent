---
summary: "ClawDock shell helpers for Docker-based Afora installs"
read_when:
  - You run Afora with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based Afora installs.
...
The helpers auto-detect your Afora checkout on first use (checking common paths like `~/afora`, `~/projects/afora`) and cache the result in `~/.clawdock/config`. Set `CLAWDOCK_DIR` yourself if your checkout lives elsewhere.
...
| `clawdock-cli <command>`  | Run Afora CLI commands in Docker           |
...
- The project `.env` next to `docker-compose.yml`: Docker-specific values like image name, ports, and `AFORA_GATEWAY_TOKEN`. `clawdock-token` reads the token from here.
- `~/.afora/.env` (mounted into the container): env-backed secrets Afora itself manages, alongside `afora.json` and `agents/<agentId>/agent/auth-profiles.json`.
...
Use `clawdock-show-config` to inspect `afora.json` and both `.env` files quickly; it redacts `.env` values in its printed output.

## Related

<CardGroup cols={2}>
  <Card title="Docker" href="/install/docker" icon="docker">
    Canonical Docker install for Afora.
  </Card>
  ...
  <Card title="Updating" href="/install/updating" icon="arrow-up-right-from-square">
    Updating the Afora package and managed services.
  </Card>
</CardGroup>
