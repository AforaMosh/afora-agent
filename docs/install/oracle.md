---
summary: "Host Afora on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Afora on Oracle Cloud
  - Looking for free VPS hosting for Afora
  - Want 24/7 Afora on a small server
title: "Oracle Cloud"
---

Run a persistent Afora Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname afora
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=afora
    ```

    From now on, connect via Tailscale: `ssh ubuntu@afora`.
  ...
  <Step title="Verify">
    ...
    ```
    https://afora.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.afora` to restrict credential file permissions.
- `afora security audit` for an Afora-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Afora features work fine; a small number of native binaries need ARM builds:
...
- `~/.afora/` -- `afora.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@afora
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=afora --reset` to re-authenticate.
