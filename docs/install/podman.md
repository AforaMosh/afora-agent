---
summary: "Run Afora in a rootless Podman container"
read_when:
  - You want a containerized gateway with Podman instead of Docker
title: "Podman"
---

Run the Afora Gateway in a rootless Podman container, managed by your current non-root user.

The model:

- Podman runs the gateway container.
- Your host `afora` CLI is the control plane.
- Persistent state lives on the host under `~/.afora` by default.
- Day-to-day management uses `afora --container <name> ...` instead of `sudo -u afora`, `podman exec`, or a separate service user.

## Prerequisites

- **Podman** in rootless mode
- **Afora CLI** installed on the host
...
<Steps>
  <Step title="One-time setup">
    ...
    | Var | Effect |
    | --- | --- |
    | `AFORA_IMAGE` / `AFORA_PODMAN_IMAGE` | Use an existing/pulled image instead of building `afora:local` |
    | `AFORA_IMAGE_APT_PACKAGES` | Install extra apt packages during image build (also accepts legacy `AFORA_DOCKER_APT_PACKAGES`) |
    | `AFORA_IMAGE_PIP_PACKAGES` | Install extra Python packages during image build; pin versions and use only package indexes you trust |
    | `AFORA_EXTENSIONS` | Compile/package supported selected plugins and install their runtime dependencies |
    | `AFORA_INSTALL_BROWSER` | Pre-install Chromium and Xvfb for browser automation (set to `1`) |
  ...
  <Step title="Start the Gateway container">
    ...
    Starts the container as your current uid/gid with `--userns=keep-id` and bind-mounts your Afora state into the container.
  ...
  <Step title="Run onboarding inside the container">
    ...
    Then open `http://127.0.0.1:18789/` and use the token from `~/.afora/.env`.

    Model auth: use Afora-managed auth during setup (Anthropic API keys, or OpenAI Codex browser OAuth/device-code auth for Codex-backed OpenAI). The Podman launcher does not mount host CLI credential homes such as `~/.claude` or `~/.codex` into the setup or gateway container. Existing host CLI logins are same-host convenience paths only -- for container installs, keep provider auth in the mounted `~/.afora` state that setup manages.

  </Step>

  <Step title="Manage the running container from the host CLI">
    ```bash
    export AFORA_CONTAINER=afora
    ```

    Then normal `afora` commands run inside that container automatically:

    ```bash
    afora dashboard --no-open
    afora gateway status --deep   # includes extra service scan
    afora doctor
...
The manual launcher reads only a small allowlist of Podman-related keys from `~/.afora/.env` and passes explicit runtime env vars to the container; it does not hand the full env file to Podman.
...
- Prefer host-managed `tailscale serve` over `afora gateway --tailscale serve`.
...
| Logs   | `journalctl --user -u afora.service -f` |

After editing the Quadlet file:

```bash
systemctl --user daemon-reload
systemctl --user restart afora.service
```
...
The generated Quadlet service keeps a fixed, hardened default shape: `127.0.0.1` published ports (`18789` gateway, `18790` bridge), `--bind lan` inside the container, `keep-id` user namespace, `AFORA_NO_RESPAWN=1`, `Restart=on-failure`, and `TimeoutStartSec=300`. It reads `~/.afora/.env` as a runtime `EnvironmentFile` for values such as `AFORA_GATEWAY_TOKEN`, but does not consume the manual launcher's Podman-specific override allowlist. For custom publish ports, publish host, or other container-run flags, use the manual launcher instead, or edit `~/.config/containers/systemd/afora.container` directly and then reload and restart the service.

## Config, env, and storage

- **Config dir:** `~/.afora`
- **Workspace dir:** `~/.afora/workspace`
- **Token file:** `~/.afora/.env`
- **Launch helper:** `./scripts/run-afora-podman.sh`

The launch script and Quadlet bind-mount host state into the container: `AFORA_CONFIG_DIR` -> `/home/node/.afora`, `AFORA_WORKSPACE_DIR` -> `/home/node/.afora/workspace`. By default those are host directories, not anonymous container state, so `afora.json`, per-agent `auth-profiles.json`, channel/provider state, sessions, and workspace survive container replacement. Setup also seeds `gateway.controlUi.allowedOrigins` for `127.0.0.1` and `localhost` on the published gateway port so the local dashboard works with the container's non-loopback bind.
...
On first startup for a new Afora version, the gateway runs safe state and
plugin repairs before reporting ready.

If the gateway exits instead of becoming ready, run the same image once with
`afora doctor --fix` against the same mounted state/config, then restart the
gateway normally:

```bash
AFORA_CONFIG_DIR="${AFORA_CONFIG_DIR:-$HOME/.afora}"
AFORA_WORKSPACE_DIR="${AFORA_WORKSPACE_DIR:-$AFORA_CONFIG_DIR/workspace}"
AFORA_PODMAN_IMAGE="${AFORA_PODMAN_IMAGE:-${AFORA_IMAGE:-afora:local}}"

podman run --rm -it \
  --userns=keep-id \
  --user "$(id -u):$(id -g)" \
  -e HOME=/home/node \
  -e NPM_CONFIG_CACHE=/home/node/.afora/.npm \
  -v "$AFORA_CONFIG_DIR:/home/node/.afora:rw" \
  -v "$AFORA_WORKSPACE_DIR:/home/node/.afora/workspace:rw" \
  "$AFORA_PODMAN_IMAGE" \
  afora doctor --fix
```
...
- **Container logs:** `podman logs -f afora`
- **Stop container:** `podman stop afora`
- **Remove container:** `podman rm -f afora`
- **Open dashboard URL from host CLI:** `afora dashboard --no-open`
- **Health/status via host CLI:** `afora gateway status --deep` (RPC probe + extra service scan)
...
- **Quadlet service does not start:** Run `systemctl --user daemon-reload`, then `systemctl --user start afora.service`. On headless systems you may also need `sudo loginctl enable-linger "$(whoami)"`.
