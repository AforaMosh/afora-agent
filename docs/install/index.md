---
summary: "Install Afora - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Afora, and launches onboarding.
...
Use this when you want Afora and Node kept under a local prefix such as
`~/.afora`, without depending on a system-wide Node install:

```bash
curl -fsSL https://afora.ai/install-cli.sh | bash
```
...
`afora update --channel dev` and `afora update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    `--allow-scripts=afora`. Do not use npm 11.13–11.15 for this install;
    upgrade to npm 11.16+ first.

    <Note>
    npm 12 blocks unapproved package lifecycle scripts by default. The
    `--allow-scripts=afora` option explicitly allows Afora's `preinstall`
    ...
    relying on their unflagged behavior. The `npm approve-scripts afora`
    command suggested by npm 11.16 does not work for a global install — it fails
    with `ENOMATCH  No installed packages match: afora`.
    </Note>

    <Note>
    The hosted installer clears npm freshness filters such as `min-release-age`
    for the Afora package install. If you install manually with npm, your own
  ...
  <Tab title="pnpm">
    ...
    <Note>
    pnpm requires explicit approval for packages with build scripts. `approve-builds -g` is not supported for global installs, so pass `--allow-build=afora` on the `pnpm add -g` command instead.
    </Note>
  ...
  <Tab title="bun">
    ...
    <Note>
    `--trust` allows Afora's package lifecycle scripts for this install. Bun
    can install the global package, but the resulting `afora` executable
    requires a supported Node runtime because Afora state uses `node:sqlite`.
    </Note>
...
Or skip the link and use `pnpm afora ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
afora --version      # confirm the CLI is available
afora doctor         # check for config issues
afora gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `afora onboard --install-daemon` or `afora gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Afora on a cloud server or VPS. See [Linux server](/vps) for the full
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Afora completely.
  </Card>
</CardGroup>

## Troubleshooting: `afora` not found
