...
summary: "How Afora structures the built-in agent runtime: code layout, boundaries, resource manifests, and runtime selection."
---

Afora owns the built-in agent runtime. Runtime code lives under `src/agents/`, model/provider transport lives under `src/llm/`, and plugin-facing contracts are exposed through `afora/plugin-sdk/*` barrels.
...
| `packages/agent-core/`              | Reusable agent core (`@afora/agent-core`): agent loop, harness types, messages, compaction helpers, prompt templates, skills, and session storage contracts.                                                           |
| `src/agents/runtime/`               | Afora facade that wires `@afora/agent-core` to the plugin SDK LLM runtime and re-exports it plus local proxy utilities.                                                                                             |
| `src/agents/agent-tools*.ts`        | Afora-owned tool definitions, parameter schemas, tool policy, before/after tool-call adapters, and host/sandbox edit tools.                                                                                            |
...
Core calls the built-in runtime through Afora modules and SDK barrels; no external agent framework packages remain. Plugins use documented `afora/plugin-sdk/*` entrypoints and do not import `src/**` internals.
...
Resource packages declare Afora resources in `package.json` metadata. Entries are file paths or globs relative to the package root:
...
- The built-in runtime id is `afora`. The legacy alias `pi` normalizes to `afora`; `codex-app-server` normalizes to `codex`.
...
- `auto` selects a registered plugin harness that supports the effective provider route, otherwise the built-in Afora runtime. A provider or model prefix alone never selects a harness.
- OpenAI may select `codex` implicitly only for an exact official HTTPS Platform Responses or ChatGPT Responses route with no authored request override. Completions adapters, custom endpoints, and routes with authored request behavior stay on `afora`; plaintext official HTTP endpoints are rejected. See [OpenAI implicit agent runtime](/providers/openai#implicit-agent-runtime).
...
- [Afora agent runtime workflow](/afora-agent-runtime)
