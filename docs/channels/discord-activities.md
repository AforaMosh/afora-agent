---
summary: "Launch self-contained Afora HTML widgets inside Discord Activities"
read_when:
...
The feature is off by default. Afora registers the Activity HTTP routes, the `show_widget` agent tool, and the launch-button handler only when `channels.discord.activities` is present and a client secret resolves. The deprecated `discord_widget` alias remains available for one release.

## Prerequisites

- an existing [Afora Discord bot](/channels/discord)
- a public HTTPS hostname that reaches the Afora gateway
...
tunnel: afora-discord
credentials-file: /home/you/.cloudflared/TUNNEL-ID.json
ingress:
  - hostname: afora.example.com
...
cloudflared tunnel create afora-discord
cloudflared tunnel route dns afora-discord afora.example.com
cloudflared tunnel run afora-discord
```
...
<Steps>
  <Step title="Expose the gateway over HTTPS">
    Start your tunnel or reverse proxy and verify that `https://afora.example.com/discord/activity/` reaches the gateway after Activities configuration is added. Replace the example hostname with your own.
  </Step>
  ...
  <Step title="Enable Activities in Discord">
    ...
    - prefix: `ROOT` (`/`)
    - target: `afora.example.com/discord/activity`
...
- Everyone who Discord permits into that channel can open its widgets. To narrow the audience, use Discord channel permissions. Afora command and DM allowlists do not grant or remove access to already-posted channel content.
...
- restart the gateway after changing Discord or Afora configuration
...
Launch the button from the channel where the agent posted it. Afora tracks launches server-side when clicked, so a fresh launch record can resolve the exact widget even when Discord omits or mangles the button's custom ID. When neither the custom ID nor a launch record resolves, Afora opens the most recently posted live widget in that channel. Older widgets remain addressable through buttons that preserve their custom ID.

### “You cannot launch Activities in this channel”

Discord does not launch Activities from forum-post threads. Afora can post the widget message and button there, but launch the Activity from a regular text channel instead. This restriction comes from Discord, not Afora.
