...
Signal is a downloadable channel plugin (`@afora/signal`). The gateway talks to `signal-cli` over HTTP: either the native daemon (JSON-RPC + SSE) or the [bbernhard/signal-cli-rest-api](https://github.com/bbernhard/signal-cli-rest-api) container (REST + WebSocket). Afora does not embed libsignal.
...
```bash
afora plugins install @afora/signal
```

Bare plugin specs try ClawHub first, then npm fallback. Force a source with `afora plugins install clawhub:@afora/signal` or `npm:@afora/signal`. `plugins install` registers and enables the plugin; no separate `enable` step is needed. See [Plugins](/tools/plugin) for general install rules.
...
<Steps>
  ...
  <Step title="Run the guided setup">
    ...
    For non-interactive setup, `afora channels add --channel signal` also accepts `--signal-number <e164>` for the bot phone number, plus `--http-host <host>` and `--http-port <port>` for the Signal daemon endpoint (default `127.0.0.1:8080`).

  </Step>
  <Step title="Link or register the account">
    - **QR link (fastest):** `signal-cli link -n "Afora"`, then scan with Signal. See [Path A](#setup-path-a-link-existing-signal-account-qr).
  ...
  <Step title="Verify and pair">
    ```bash
    afora gateway call channels.status --params '{"probe":true}'
    ```
    Send a first DM and approve pairing: `afora pairing approve signal <CODE>`.
  </Step>
</Steps>
...
1. Install `signal-cli` (JVM or native build), or let `afora channels add` install it for you.
2. Link a bot account: `signal-cli link -n "Afora"`, then scan the QR in Signal.
...
4. Configure Afora, restart the gateway, verify the channel:

```bash
# If you run the gateway as a user systemd service:
systemctl --user restart afora-gateway.service
...
5. Pair your DM sender:
   ...
   - Approve on the server: `afora pairing approve signal <PAIRING_CODE>`.
...
To manage `signal-cli` yourself (slow JVM cold starts, container init, shared CPUs), run the daemon separately and point Afora at it:

For non-interactive setup, select the endpoint kind explicitly when needed:

```bash
afora channels add --channel signal --signal-number +15551234567 \
...
This skips auto-spawn and Afora's startup wait. For a managed daemon with a slow start, set `channels.signal.transport.startupTimeoutMs`.
...
- Register or link your Signal account inside the container before connecting Afora.
...
`transport.kind` controls which protocol and process lifecycle Afora uses:
...
Setup and `afora doctor --fix` may probe an existing endpoint once to identify its concrete kind. Runtime operations do not auto-detect or switch protocols.

Container mode supports the same Signal operations as native mode where the container exposes matching APIs: sends, receives, attachments, typing indicators, read/viewed receipts, reactions, groups, and styled text. Afora translates native Signal RPC calls into the container's REST payloads, including `group.{base64(internal_id)}` group IDs and `text_mode: "styled"` for formatted text.
...
- Approve via `afora pairing list signal` and `afora pairing approve signal <CODE>`.
...
- Replies to inbound messages include native Signal quote metadata when the backend accepts the inbound timestamp and author; if quote metadata is missing or rejected, Afora sends the reply as a normal message.
...
- **Typing indicators**: Afora sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, Afora forwards read receipts for allowed DMs.
...
Configure aliases for stable names on recurring Signal targets. Aliases are Afora-side config only; they do not create or edit Signal contacts.
...
```bash
afora message send --channel signal --target signal:ops --message "Deployment is complete"
```
...
`afora directory peers list --channel signal` and `afora directory groups list --channel signal` list configured aliases. The Signal directory is config-backed; it does not live-query Signal contacts or mutate the Signal account.
...
- Config validation errors after edits: run `afora doctor --fix`.
...
afora pairing list signal
pgrep -af signal-cli
afora logs --plain --limit 500 | grep -i "signal" | tail -20
```
...
- `channels.signal.aliases`: Afora-side aliases for DM or group delivery targets.
