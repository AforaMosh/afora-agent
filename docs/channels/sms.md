...
read_when:
  - You want to connect Afora to SMS or MMS through Twilio
  - You need SMS/MMS webhook or allowlist setup
title: "SMS"
---

Afora receives and sends SMS/MMS through a Twilio phone number or Messaging Service. The Gateway registers a webhook route (default `/webhooks/sms`), validates Twilio request signatures by default, sends replies through Twilio's Messages API, and records outbound delivery callbacks.
...
- The official SMS plugin installed with `afora plugins install @afora/sms`.
...
- A public HTTPS URL that reaches your Afora Gateway.
...
SMS and MMS sent by an application from a US local 10DLC number to US recipients require US A2P 10DLC registration. Toll-free numbers and short codes use separate verification processes. This is separate from Afora channel setup: webhook signature validation, pairing, and outbound credentials can all be correct while carriers still block or filter delivery.
...
- The Campaign describes the real Afora message use case and includes matching sample messages.
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    afora plugins install @afora/sms
    ```
  </Step>
...
```bash
afora config patch --file ./sms.patch.json5 --dry-run
afora config patch --file ./sms.patch.json5
```
...
```
  ...
  <Step title="Expose the exact SMS webhook path">
    Your public URL must route the SMS path to the Gateway process (default port `18789`). The same path serves inbound Twilio webhooks and short-lived, tokenized attachments when Afora sends MMS. If you use Tailscale Funnel for local testing, expose `/webhooks/sms` explicitly:
...
```bash
afora pairing list sms
afora pairing approve sms <CODE>
```
...
`authToken` can be a SecretRef (`source: "env" | "file" | "exec" | "store"`). Use this when the Gateway should resolve the Twilio Auth Token from the Afora secrets runtime instead of storing plaintext config:
...
- `pairing` (default): unknown senders get a pairing code; approve with `afora pairing approve sms <CODE>`.
...
```bash
afora message send --channel sms --target sms:+15551234567 --message "hello"
```
...
```bash
afora message send --target twilio-sms:+15551234567 --message "hello"
```
...
SMS output is plain text. Afora strips markdown, flattens fenced code blocks, rewrites links as `label (url)`, and splits long replies into chunks of at most `textChunkLimit` characters (default 1500) before sending them through Twilio.
...
Afora loads the attachment through the shared outbound-media policy, stores it temporarily in plugin-scoped SQLite state, and gives Twilio a tokenized HTTPS URL on the configured `publicWebhookUrl` path. Media-only sends are supported.

The generated media URL is a bearer capability that expires after 10 minutes. Treat its full query string as a secret: configure reverse-proxy and access logs to omit the query string or redact every query value. Afora Gateway route diagnostics record only the pathname, but cannot control upstream proxy logs.

Outbound Afora deliveries attach one media item. Afora caps JPEG, JPG, PNG, and GIF attachments at 5,000,000 bytes; other supported media types are capped at 500,000 bytes. `application/vcard` attachments must be media-only; Twilio does not accept them with a caption. Destination carriers may enforce smaller limits or reject unsupported formats. Twilio must be able to fetch the generated URL without HTTP authentication, so `publicWebhookUrl` cannot contain embedded userinfo; query-based reverse-proxy tokens are preserved.

For incoming MMS, Afora processes at most 10 attachments and downloads at most 5 MiB total. Any additional or unavailable attachments produce a visible unavailable-media notice instead of discarding the signed message or silently delivering an empty turn. Downloads happen only after sender authorization, with Twilio authentication and an `api.twilio.com` host restriction.

### Delivery status

After each successful outbound send, Afora stores the initial Twilio API status when the response includes one. When `publicWebhookUrl` is valid, every outbound message also gives Twilio a derived `StatusCallback` URL that preserves its base URL and connection overrides while adding the required delivery-callback retry settings. Invalid or oversized derived URLs are omitted.
...
```bash
afora channels capabilities --channel sms
afora channels status --channel sms --probe --json
```

3. Send an SMS to the Twilio number from your phone.
4. Run `afora pairing list sms`.
5. Approve the pairing code with `afora pairing approve sms <CODE>`.
6. Send another SMS and confirm the agent replies.

For outbound-only testing, use:

```bash
afora message send --channel sms --target sms:+15557654321 --message "Afora SMS test"
```
...
```bash
imsg send --to "+15551234567" --service sms --text "Afora SMS E2E $(date -u +%Y%m%dT%H%M%SZ)" --json
afora pairing list sms
afora pairing approve sms <CODE>
...
By default, Afora validates `X-Twilio-Signature` using `publicWebhookUrl` and `authToken`. Keep the endpoint portion of `publicWebhookUrl` byte-for-byte aligned with the URL configured in Twilio, including scheme, host, path, and query string. Afora excludes Twilio [connection-override](https://www.twilio.com/docs/usage/webhooks/webhooks-connection-overrides) fragments (`#...`) from signature computation, as Twilio requires.
...
- Client addresses are resolved through the shared Gateway trusted-proxy rules. If `gateway.trustedProxies` contains the reverse proxy that forwards Twilio callbacks, Afora keys the address-based limits from the forwarded client address; otherwise it falls back to the direct socket address.
...
### Twilio returns 403 or Afora rejects the webhook
...
`afora channels status --channel sms --probe` surfaces both mismatched Twilio webhook settings and recent `11200` errors.
...
Start with Afora's stored delivery observation:
