---
summary: "Mattermost bot setup and Afora config"
read_when:
...
<Tabs>
  ...
  <Tab title="Local checkout">
    ```bash
    afora plugins install ./path/to/local/mattermost-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Install `@afora/mattermost` with the command above, then restart the Gateway if it is already running.
  </Step>
  ...
  <Step title="Configure Afora and start the gateway">
    Minimal config:
    ...
    ```bash
    afora channels add --channel mattermost --bot-token <token> --http-url https://chat.example.com
    ```
...
Native slash commands are opt-in. When enabled, Afora registers `oc_*` slash commands on every team the bot is a member of and receives callback POSTs on the gateway HTTP server.
...
<AccordionGroup>
  <Accordion title="Behavior notes">
    ...
    - If `callbackUrl` is omitted, Afora derives `http://<gateway.customBindHost or localhost>:<gateway.port, default 18789><callbackPath>`. Wildcard bind hosts (`0.0.0.0`, `::`) fall back to `localhost`.
    ...
    - Command callbacks are validated with the per-command tokens returned by Mattermost when Afora registers `oc_*` commands.
    - Afora refreshes current Mattermost command registration before accepting each callback, so stale tokens from deleted or regenerated slash commands stop being accepted without a gateway restart.
  ...
  <Accordion title="Reachability requirement">
    ...
    - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Afora.
    - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Afora.
    - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Afora, not `404`.
...
- Approve via:
  - `afora pairing list mattermost`
  - `afora pairing approve mattermost <CODE>`
...
Use these target formats with `afora message send` or cron/webhooks:
...
Afora resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Afora sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Afora sends to a Mattermost DM target and needs to resolve the direct channel first, it retries transient direct-channel creation failures by default.
...
Preview streaming is **on by default** in `partial` mode. Configure via `channels.mattermost.streaming.mode` (legacy scalar/boolean `streaming` values are migrated by `afora doctor --fix`):
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    - If the stream cannot be finalized in place (for example the post was deleted mid-stream), Afora falls back to sending a fresh final post so the reply is never lost.
...
```bash
afora message read --channel mattermost --target channel:<channelId> --limit 5 --json
```
...
Buttons come from the semantic `presentation` payload (in normal agent replies and in `message action=send`). Afora renders value buttons as Mattermost interactive buttons, keeps URL buttons visible in the message text, and downgrades select menus to readable text.
...
<AccordionGroup>
  ...
  <Accordion title="Config and reachability">
    ...
    - If `interactions.callbackBaseUrl` is omitted, Afora derives the callback URL from `gateway.customBindHost` + `gateway.port` (default 18789), then falls back to `http://localhost:<port>`. The callback path is `/mattermost/interactions/<accountId>`.
    - Reachability rule: the button callback URL must be reachable from the Mattermost server. `localhost` only works when Mattermost and Afora run on the same host/network namespace.
...
External scripts and webhooks can post buttons directly via the Mattermost REST API instead of going through the agent's `message` tool. Prefer Afora's `message` tool. For direct integrations, import `buildButtonAttachments` from `@afora/mattermost/api.js`; if posting raw JSON, follow these rules:
...
<Steps>
  <Step title="Derive the secret from the bot token">
    `HMAC-SHA256(key="afora-mattermost-interactions", data=botToken)`, hex-encoded.
  </Step>
...
secret = hmac.new(
    b"afora-mattermost-interactions",
...
The Mattermost plugin includes a directory adapter that resolves channel and user names via the Mattermost API. This enables `#channel-name` and `@username` targets in `afora message send` and cron/webhook deliveries.
...
<AccordionGroup>
  ...
  <Accordion title="Native slash commands fail">
    - `Unauthorized: invalid command token.`: Afora did not accept the callback token. Typical causes:
    ...
    - If `callbackUrl` is omitted and logs warn that the callback resolved to a loopback URL like `http://localhost:18789/...`, that URL is probably only reachable when Mattermost runs on the same host/network namespace as Afora. Set an explicit externally reachable `commands.callbackUrl` instead.
