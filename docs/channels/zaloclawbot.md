---
summary: "Zalo ClawBot channel setup through the external afora-zaloclawbot plugin"
read_when:
  - You want a personal Zalo assistant bot with QR-code login
  - You are installing or troubleshooting the afora-zaloclawbot channel plugin
title: "Zalo ClawBot"
---

Afora connects to Zalo ClawBot through the catalog-listed external `@zalo-platforms/afora-zaloclawbot` plugin. Login uses a Zalo Mini App QR code; the plugin id in config is `afora-zaloclawbot`.

## Compatibility

| Plugin Version | Afora Version | npm dist-tag | Status        |
| -------------- | ---------------- | ------------ | ------------- |
...
- Node.js >= 22
- [Afora](https://docs.afora.ai/install) installed (`afora` CLI available)
...
```bash
afora plugins install "@zalo-platforms/afora-zaloclawbot@0.1.4"
```

Use the exact pinned version so Afora verifies the package against the catalog integrity hash during install.

### 2. Enable the plugin in config

```bash
afora config set plugins.entries.afora-zaloclawbot.enabled true
```

### 3. Generate a QR code and log in

```bash
afora channels login --channel afora-zaloclawbot
```
...
The plugin manages bot credentials under the Afora state directory. Treat that directory as sensitive and cover it under the same access-control and backup policy as the rest of Afora state.

This plugin's runtime lives entirely in the external `@zalo-platforms/afora-zaloclawbot` package; behavior details below beyond install/config are as reported by the plugin's maintainers and are not verified against Afora core source.
...
- **Gateway fails to load:** confirm your Afora host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger this ID requires.
