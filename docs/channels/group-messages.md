...
Goal: let Afora sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
  ...
  agents: {
    entries: {
      main: {
        default: true,
        groupChat: {
          mentionPatterns: ["@?afora", "\\+?15555550123"],
        },
      },
    },
  },
...
1. Add your WhatsApp account (the one running Afora) to the group.
2. Say `@afora ...` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@afora` ping in the group and confirm a reply that references the sender name.
