...
read_when:
  - Setting up Matrix in Afora
  - Configuring Matrix E2EE and verification
title: "Matrix"
---

Matrix is a downloadable channel plugin (`@afora/matrix`) built on the official `matrix-js-sdk`. It supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.
...
Bare plugin specs try ClawHub first, then npm fallback. Force a source with `afora plugins install clawhub:@afora/matrix` or `npm:@afora/matrix`. From a local checkout: `afora plugins install ./path/to/local/matrix-plugin`.
...
```bash
afora channels add
afora configure --section channels
```

The wizard asks for homeserver URL, auth method (token or password), user ID (password auth only), optional device name, whether to enable E2EE, and room access/auto-join. If matching `MATRIX_*` env vars already exist and the account has no saved auth, the wizard offers an env-var shortcut. Resolve room names before saving an allowlist with `afora channels resolve --channel matrix "Project Room"`. Enabling E2EE in the wizard runs the same bootstrap as [`afora matrix encryption setup`](#encryption-and-verification).
...
Matrix caches account credentials in the shared `state/afora.sqlite` plugin state. When cached credentials exist, Afora treats Matrix as configured even without an `accessToken` in the config file - this covers setup, `afora doctor`, and channel-status probes. Upgrades import the retired `~/.afora/credentials/matrix/credentials*.json` files through `afora doctor --fix`, verify the SQLite rows, then archive the files.
...
Matrix reply streaming is opt-in. `streaming.mode` controls how Afora delivers the in-flight assistant reply; `streaming.block.enabled` controls whether each completed block is kept as its own Matrix message.
...
- If a preview grows past Matrix's per-event size limit, Afora stops preview streaming and falls back to final-only delivery.
- Media replies always send attachments normally. If a visible preview cannot be reused safely, Afora keeps it until the complete replacement is confirmed and then redacts it. If replacement delivery fails, is partial, or produces no visible event, the preview remains visible.
...
- Legacy scalar/boolean `streaming` values and the flat `blockStreaming` / `chunkMode` keys are rewritten to this nested shape by `afora doctor --fix`.
...
- In encrypted rooms, Afora decrypts the attachment through the existing Matrix media path before transcription.
...
Matrix native approval prompts are normal `m.room.message` events with Afora-specific content under the `com.afora.approval` key. Stock clients still render the text body; Afora-aware clients can read the structured approval id, kind, state, decisions, and exec/plugin details.

When a prompt is too long for one Matrix event, Afora chunks the visible text and attaches `com.afora.approval` to the first chunk only. Allow/deny reactions bind to that first event, so long prompts keep the same approval target as single-event prompts.
...
By default, Matrix messages from other configured Afora Matrix accounts are ignored. Use `allowBots` to intentionally allow inter-agent traffic:
...
- Afora still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix has no native bot flag; Afora treats "bot-authored" as "sent by another configured Matrix account on this Afora gateway".
...
All `afora matrix` commands accept `--verbose` (full diagnostics), `--json` (machine-readable output), and `--account <id>` (multi-account setups). Output is concise by default.

### Enable encryption

```bash
afora matrix encryption setup
printf '%s\n' "$MATRIX_RECOVERY_KEY" | afora matrix encryption setup --recovery-key-stdin
```
...
```bash
afora matrix verify status
afora matrix verify status --include-recovery-key --json
```
...
```bash
printf '%s\n' "$MATRIX_RECOVERY_KEY" | afora matrix verify device --recovery-key-stdin
```
...
```bash
afora matrix verify self
```

`verify self` waits for `Cross-signing verified: yes` before exiting successfully. Use `--timeout-ms <ms>` to tune the wait.

The literal-key form `afora matrix verify device "<recovery-key>"` also works, but the key ends up in shell history.

### Bootstrap or repair cross-signing

```bash
afora matrix verify bootstrap
```
...
If the homeserver requires UIA to upload cross-signing keys, Afora tries no-auth first, then `m.login.dummy`, then `m.login.password` (requires `channels.matrix.password`).
...
```bash
afora matrix verify backup status
printf '%s\n' "$MATRIX_RECOVERY_KEY" | afora matrix verify backup restore --recovery-key-stdin
```
...
```bash
afora matrix verify backup reset --yes
```
...
```bash
afora matrix verify list
```

Lists pending verification requests for the selected account.

```bash
afora matrix verify request --own-user
afora matrix verify request --user-id @ops:example.org --device-id ABCDEF
```
...
| Command                                    | Purpose                                                             |
| ------------------------------------------ | ------------------------------------------------------------------- |
| `afora matrix verify accept <id>`       | Accept an inbound request                                           |
| `afora matrix verify start <id>`        | Start the SAS flow                                                  |
| `afora matrix verify sas <id>`          | Print the SAS emoji or decimals                                     |
| `afora matrix verify confirm-sas <id>`  | Confirm that the SAS matches what the other client shows            |
| `afora matrix verify mismatch-sas <id>` | Reject the SAS when the emoji or decimals do not match              |
| `afora matrix verify cancel <id>`       | Cancel; takes optional `--reason <text>` and `--code <matrix-code>` |
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Startup also runs a conservative crypto bootstrap pass reusing the current secret storage and cross-signing identity. If bootstrap state is broken, Afora attempts a guarded repair even without `channels.matrix.password`; if the homeserver requires password UIA, startup logs a warning and stays non-fatal. Already-owner-signed devices are preserved.
  ...
  <Accordion title="Verification notices">
    ...
    Incoming requests from another Matrix client are tracked and auto-accepted. For self-verification, Afora starts the SAS flow automatically and confirms its own side once emoji verification is available - you still need to compare and confirm "They match" in your Matrix client.
  ...
  <Accordion title="Deleted or invalid Matrix device">
    If `verify status` says the current device is no longer listed on the homeserver, create a new Afora Matrix device. For password login:
...
afora matrix account add \
  ...
  --password '<password>' \
  --device-name Afora-Gateway
```

    For token auth, create a fresh access token in your Matrix client or admin UI, then update Afora:
...
```
  ...
  <Accordion title="Device hygiene">
    Old Afora-managed devices can accumulate. List and prune:

```bash
afora matrix devices list
afora matrix devices prune-stale
```
  ...
  <Accordion title="Crypto store">
    ...
    Encrypted runtime state lives under `~/.afora/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/` and includes the sync store, crypto store, recovery key, IDB snapshot, thread bindings, and startup verification state. When the token changes but the account identity stays the same, Afora reuses the best existing root so prior state remains visible.

    A single older token-hash root can be a normal token-rotation continuity path. If Afora logs `matrix: multiple populated token-hash storage roots detected`, inspect the account directory and archive stale sibling roots only after confirming the selected active root is healthy. Prefer moving stale roots into an `_archive/` directory over deleting them immediately.
...
```bash
afora matrix profile set --name "Afora Assistant"
afora matrix profile set --avatar-url https://cdn.example.org/avatar.png
```
...
`dm.sessionScope` decides how Matrix DM rooms map to Afora sessions:
...
- DM user-target reuse only kicks in when current session metadata proves the same DM peer on the same Matrix account; otherwise Afora falls back to normal user-scoped routing.
...
When Afora detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` pointing to the `/focus` escape hatch and suggesting a `dm.sessionScope` change. The notice only appears when thread bindings are enabled.
...
`--bind here` does not create a child Matrix thread. `threadBindings.spawnSessions` gates `/acp spawn --thread auto|here`, where Afora needs to create or bind a child thread.
...
- Deprecated `threadBindings.spawnSubagentSessions` / `threadBindings.spawnAcpSessions` keys are migrated to `spawnSessions` by `afora doctor --fix`.
...
- Room history is pending-only: Afora buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
afora pairing list matrix
afora pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging before approval, Afora reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state drifts, Afora can end up with stale `m.direct` mappings pointing at old solo rooms instead of the live DM. Inspect the current mapping for a peer:

```bash
afora matrix direct inspect --user-id @alice:example.org
```

Repair it:

```bash
afora matrix direct repair --user-id @alice:example.org
```
...
- If you have multiple accounts and one is literally named `default`, Afora uses it implicitly even when `defaultAccount` is unset.
...
- When Afora promotes a single-account config to multi-account during repair or setup, it preserves the existing named account if one exists or `defaultAccount` already points at one. Only Matrix auth/bootstrap keys move into the promoted account; shared delivery-policy keys stay at the top level.

See [Configuration reference](/gateway/config-channels#multi-account-all-channels) for the shared multi-account pattern.

## Private/LAN homeservers

By default, Afora blocks private/internal Matrix homeservers for SSRF protection unless you opt in per account.
...
Named accounts can override the top-level default with `channels.matrix.accounts.<id>.proxy`. Afora uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere Afora asks for a room or user target:
...
Matrix room IDs are case-sensitive. Use the exact room ID casing from Matrix when configuring explicit delivery targets, cron jobs, bindings, or allowlists. Afora keeps internal session keys canonical for storage, so those lowercase keys are not a reliable source for Matrix delivery IDs.
...
- `streaming`: nested object `{ mode, chunkMode, block: { enabled, coalesce }, preview: { toolProgress }, progress: { label, labels, maxLines, maxLineChars, toolProgress } }`. `mode` is `"off"` (default), `"partial"`, `"quiet"`, or `"progress"`. Legacy scalar/boolean spellings migrate via `afora doctor --fix`.
