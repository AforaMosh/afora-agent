...
read_when:
  - You want to connect Afora to QQ
  - You need QQ Bot credential setup
  - You want QQ Bot group or private chat support
title: QQ bot
---

QQ Bot connects to Afora via the official QQ Bot API (WebSocket gateway).
...
```bash
afora plugins install @tencent-connect/afora-qqbot
```
...
```bash
afora channels add --channel qqbot --token "AppID:AppSecret"
```
...
For QQ gateway turn events, Afora persists the raw event before advancing the saved gateway resume sequence. Pending or retryable turns survive a Gateway restart, remain serialized per conversation, and use the provider event ID to suppress duplicate queue entries while the active or retained completion record exists.

If durable admission fails, Afora terminates the current gateway socket without advancing the sequence. The reconnect/resume path can then request the uncommitted event again. Delivery is still at least once across the queue-to-agent boundary, so a crash during handoff can replay a turn.
...
binding. Afora persists the returned credentials under the account's config
scope.
...
- `afora channels add --channel qqbot --token-file ...` sets the AppSecret
...
- Known limitation: the external `@tencent-connect/afora-qqbot` package does
...
- Legacy `streaming: true|false` scalars and the `streaming.c2cStreamApi` key
  migrate to this shape via `afora doctor --fix`.
...
Run multiple QQ bots under a single Afora instance:
...
```bash
afora channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
Old QQBot `toolPolicy` entries are retired. Run `afora doctor --fix` to migrate them to `tools`.
...
| `/bot-version`       | —         | private only | Show the Afora framework version and plugin version                         |
...
- Inbound, outbound, and gateway-bridge media share one payload root under
  `~/.afora/media/qqbot` (honoring `AFORA_HOME` when set), so uploads,
...
- If a hot upgrade interrupts the Gateway before it finishes writing
  `afora.json`, the plugin restores the last-known `appId` / `clientSecret`
