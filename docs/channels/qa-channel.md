---
summary: "Synthetic Slack-class channel plugin for deterministic Afora QA scenarios"
title: "QA channel"
read_when:
...
- `botUserId` - synthetic bot user id used in target grammar (default: `afora`).
- `botDisplayName` - display name for outbound messages (default: `Afora QA`).
...
```bash
pnpm afora qa suite --scenario channel-participant-identity-inspection
```
