...
Tlon is a decentralized messenger built on Urbit. Afora connects to your Urbit ship and
...
Tlon ships bundled in current Afora releases; packaged builds do not need a separate install.

On an older build or custom install that excludes it, install from npm:

```bash
afora plugins install @afora/tlon
```

Use the bare package name to track the current release tag. Pin a version (`@afora/tlon@x.y.z`)
only for reproducible installs.

From a local checkout:

```bash
afora plugins install ./path/to/local/tlon-plugin
```
...
```bash
afora channels add --channel tlon --ship ~sampel-palnet --url https://your-ship-host --code lidlut-tabwed-pillex-ridrup
```
...
Afora persists accepted Tlon DM and group-chat events before agent dispatch. Pending or retryable turns survive a Gateway restart, and work remains serialized per group channel or direct peer. Stable Urbit message IDs also suppress a redelivered event while its queue record or retained completion record exists.
...
Afora blocks private/internal hostnames and IP ranges for SSRF protection by default. If your
...
<Note>
`channels.tlon.allowPrivateNetwork` (flat key) is retired. `afora doctor --fix` moves it to
...
prompt to yes and writes `true` explicitly. With it on, Afora scries joined groups on startup,
...
Use with `afora message send` or cron delivery:
