...
read_when:
  - You want people to reach an Afora agent from Buzz
  - You are setting up a Buzz bot identity and room access
  - You are troubleshooting a Buzz connection
title: "Buzz"
---

Buzz is an official channel plugin that connects Afora agents to team rooms
...
- Preserves Markdown in replies and sends text through Afora's built-in
  `message` tool
...
- Resolves current Buzz profile names, avatars, room names, and room membership
  through Afora's directory commands
...
- The **private key** lets Afora authenticate and sign messages. It stays with
  the Gateway.
- The **public key** identifies the bot. Buzz owners use it for relay approval,
  room admins use it to grant the **Bot** role, and Afora can use public keys
  in sender allowlists.

The relay URL points to one Buzz workspace. Each room has a UUID, and Afora
...
<Warning>
Never give Afora a human Buzz owner's private key. Afora creates or uses a
...
```bash
afora plugins install @afora/buzz
```
...
2. Afora reuses the configured bot identity or generates one automatically.
3. If the bot does not have room access yet, give the displayed public key to a
   Buzz room owner or admin.
4. Afora waits for Buzz to confirm the **Bot** role and continues
...
5. If Buzz returns one room, Afora selects it. If Buzz returns several,
   select the rooms to use and the default outbound room.
6. Afora saves the configuration and silently verifies the authenticated
...
Afora identity. Use the Buzz CLI as the existing human room owner or admin:
...
Run that command as the existing human owner or admin. Never give Afora that
human private key.

After the Gateway connects, Afora preserves an existing non-empty Buzz
...
configured Buzz rooms, and finally `Afora`. This replaces the shortened
public key in Buzz after its profile cache refreshes.

Afora also registers the same public identity in Buzz's agent directory. It
...
instead of treating it as a normal member. Afora still receives messages
...
`channels.buzz.authTag` is configured, Afora includes that attestation in the
published profile so Buzz can show the verified human owner.

While the Gateway is connected, Afora publishes and refreshes the bot's
...
Afora cannot grant room or relay access. It displays only the bot public key
...
as a destination for Afora's built-in `message` tool and normal reply
delivery.
...
Afora command or textual mention.
...
replies use Afora's shared typing interval, which defaults to six seconds.
Afora stops refreshing when the turn completes, is cancelled, fails, or the
...
Write a unique current room member's profile name as `@Display Name`. Afora
...
Afora keeps a bounded snapshot of the configured rooms, their current
...
afora directory peers list --channel buzz --query "alice"
afora directory groups list --channel buzz --query "engineering"
afora directory groups members \
  --channel buzz \
...
profile subscription does not reach EOSE within 10 seconds, Afora treats the
...
If a configured room is archived or restored while Afora is connected, the
...
Each configured room uses one room-scoped relay subscription. Afora reserves
...
Unique current room names can resolve as outbound targets through Afora's
...
Standard Afora bindings can send each Buzz room to a different agent,
...
```json5
{
  agents: {
    entries: {
      support: { default: true, workspace: "~/.afora/workspace-support" },
      engineering: { workspace: "~/.afora/workspace-engineering" },
    },
  },
  bindings: [
    {
...
Without a room-specific binding, normal Afora routing selects the default
...
rooms. Afora loads Buzz's relay-signed room roster before accepting messages,
...
key and stores it in `channels.buzz.privateKey`, following Afora's current
...
```bash
afora channels status --channel buzz --probe
```
...
- Creating or administering rooms from Afora
...
| Setup says the Gateway is not running        | Start it with `afora gateway`, then run `afora channels status --probe`.                                       |
