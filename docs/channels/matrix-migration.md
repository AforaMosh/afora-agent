---
summary: "How Afora upgrades the previous Matrix plugin in place, including encrypted-state recovery limits and manual recovery steps."
read_when:
...
- the plugin stays `@afora/matrix`
- the channel stays `matrix`
- your config stays under `channels.matrix`
- cached credentials move into the shared `state/afora.sqlite` plugin state
- runtime state stays under `~/.afora/matrix/`

You do not need to rename config keys or reinstall the plugin under a new name.
The root `afora` package no longer bundles Matrix runtime code or Matrix SDK
dependencies. If `afora channels status` shows Matrix is configured but the
plugin is not installed, run `afora doctor --fix` or
`afora plugins install @afora/matrix`; do not install Matrix SDK packages
into the root Afora package.

## What the migration does automatically

Matrix migration runs when you run [`afora doctor --fix`](/gateway/doctor). File-based sidecars next to the dedicated Matrix store retain their client-start fallback, but credential-file import is Doctor-only; runtime reads only canonical SQLite credential state.

Doctor migration covers:

- importing and verifying retired `~/.afora/credentials/matrix/credentials*.json` files before archiving them
...
## Upgrading from Afora releases older than 2026.4

Releases through the 2026.6 train also migrated the original flat single-store
Matrix layout (`~/.afora/matrix/bot-storage.json` plus
`~/.afora/matrix/crypto/`) and prepared encrypted-state recovery from the
...
upgrade to a 2026.6 release, run `afora doctor --fix`, and start the gateway
...
1. Update Afora and the Matrix plugin normally.
2. Run:
...
6. If Afora tells you a recovery key is needed, run the command for the matching account:

   ```bash
   printf '%s\n' "$MATRIX_RECOVERY_KEY" | afora matrix verify backup restore --recovery-key-stdin
   printf '%s\n' "$MATRIX_RECOVERY_KEY_ASSISTANT" | afora matrix verify backup restore --recovery-key-stdin --account assistant
   ```
...
7. If this device is still unverified, run the command for the matching account:
   ...
   printf '%s\n' "$MATRIX_RECOVERY_KEY_ASSISTANT" | afora matrix verify device --recovery-key-stdin --account assistant
   ```
...
- Meaning: the Matrix client-side fallback found file-based sidecar state, but the import into SQLite failed. Afora rolls back completed moves and aborts that fallback instead of silently starting with a fresh store.
...
- What to do: reinstall with `afora plugins install @afora/matrix` when you want to return to the default Matrix plugin.

`Matrix is installed from a custom path that no longer exists: ...`

- Meaning: your plugin install record points at a local path that is gone.
- What to do: reinstall with `afora plugins install @afora/matrix`, or if you are running from a repo checkout, `afora plugins install ./path/to/local/matrix-plugin`. `afora doctor --fix` can also remove the stale Matrix plugin references for you.

### Manual recovery messages

`afora matrix verify status` and `afora matrix verify backup status` print a `Backup issue:` line plus `Next steps:` guidance when the room-key backup is not healthy on this device:

| Backup issue                                                          | Meaning                                            | Fix                                                                                                                                       |
| --------------------------------------------------------------------- | -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `no room-key backup exists on the homeserver`                         | nothing to restore from                            | `afora matrix verify bootstrap` to create a room key backup                                                                            |
| `backup decryption key is not loaded on this device`                  | key exists but is not active here                  | `afora matrix verify backup restore`; if it still cannot load the key, pipe the recovery key via `--recovery-key-stdin`                |
...
| `backup exists but is not active on this device`                      | server backup present, local session inactive      | verify the device first, then recheck with `afora matrix verify backup status`                                                         |
| `backup trust state could not be fully determined`                    | diagnostics were inconclusive                      | `afora matrix verify status --verbose`                                                                                                 |
...
- What to do: run `afora matrix verify self`, accept the request in another Matrix client, compare the SAS, and type `yes` only when it matches. Use `printf '%s\n' "$MATRIX_RECOVERY_KEY" | afora matrix verify bootstrap --recovery-key-stdin --force-reset-cross-signing` only when you intentionally want to replace the current cross-signing identity.

If you accept losing unrecoverable old encrypted history, you can instead reset the
current backup baseline with `afora matrix verify backup reset --yes`. When the
...
```bash
afora matrix verify status --verbose
afora matrix verify backup status --verbose
printf '%s\n' "$MATRIX_RECOVERY_KEY" | afora matrix verify backup restore --recovery-key-stdin --verbose
```
