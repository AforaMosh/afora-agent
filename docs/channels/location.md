...
Afora normalizes shared locations from chat channels into:
...
When the channel does not set an explicit source, Afora infers it: live shares become `live`, locations with a name or address become `place`, everything else is `pin`.
