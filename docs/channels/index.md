---
summary: "Messaging platforms Afora can connect to"
read_when:
  - You want to choose a chat channel for Afora
  - You need a quick overview of supported messaging platforms
title: "Chat channels"
---

Afora can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
(`afora plugins install @afora/<id>`) or on demand during
`afora onboard` / `afora channels add`, then need a Gateway restart.
"External plugin" channels are maintained outside the Afora repo.
...
- [Buzz](/channels/buzz) - Connect Afora agents to Buzz rooms (official plugin).
...
- [iMessage](/channels/imessage) - Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new Afora iMessage setups when host requirements fit (official plugin).
...
- [Mattermost](/channels/mattermost) - Mattermost bot setup and Afora config (official plugin).
...
- [Reef](/channels/reef) - Reef channel setup: guarded, end-to-end-encrypted messaging between Afora agents of different people (bundled plugin).
...
- [Synology Chat](/channels/synology-chat) - Synology Chat webhook setup and Afora config (official plugin).
...
- [WeChat](/channels/wechat) - WeChat channel setup through the external afora-weixin plugin (external plugin).
...
- [Zalo ClawBot](/channels/zaloclawbot) - Zalo ClawBot channel setup through the external afora-zaloclawbot plugin (external plugin).
...
- Channels can run simultaneously; configure multiple and Afora will route per chat.
