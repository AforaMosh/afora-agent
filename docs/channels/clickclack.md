...
read_when:
  - Connecting Afora to a ClickClack workspace
  - Testing ClickClack bot identities
title: "ClickClack"
---

ClickClack connects Afora to a self-hosted ClickClack workspace through first-class ClickClack bot tokens.

Use this when you want an Afora agent to appear as a ClickClack bot user. ClickClack supports independent service bots and user-owned bots; user-owned bots keep an `owner_user_id` and receive only the token scopes you grant.

## Quick setup

In ClickClack, open **Workspace settings → Integrations → Afora**, create a
bot using **Setup code (recommended)**, and copy the generated command:

```bash
afora channels add clickclack --code 'https://clickclack.example.com/#XXXX-XXXX-XXXX'
```
...
```bash
afora channels add clickclack --code 'https://api.example.com/services/clickclack/api/bot-setup-codes/claim#XXXX-XXXX-XXXX'
```

The setup code is single-use and expires after 10 minutes. Afora claims it,
...
For versioned exact endpoints, Afora validates and saves the canonical API
base returned by ClickClack, including any path prefix. The setup code itself is
not stored in Afora config.
...
If Afora is already running, ClickClack connects automatically and no second
...
```bash
afora channels add clickclack --code XXXX-XXXX-XXXX --base-url https://clickclack.example.com
```
...
Choose **Manual token** in ClickClack when configuring a non-Afora client or
when you explicitly need to manage the token yourself:

```bash
afora channels add clickclack --base-url https://clickclack.example.com --token ccb_... --workspace default
```
...
afora channels add clickclack --base-url https://clickclack.example.com --workspace default --use-env
afora gateway
```
...
Use `apiBaseUrl` when ClickClack and the Afora gateway run on the same host
...
The public hostname can remain fully auth-gated for browser users. Afora
...
ClickClack in channel setup or running `afora plugins enable clickclack`
...
`afora plugins install @afora/clickclack` follows the normal
...
Enable discussions on one ClickClack account to give each Afora session a
...
or deleting an Afora session never archives or replaces the ClickClack
...
concrete Afora session id, so resetting a reusable session key rotates the
...
updates and return them in channel responses. Afora verifies that contract
...
was created.
That reference contains a durable per-Afora-installation namespace plus a
...
workspace, Afora records the old channel as revoked and releases the binding
...
At gateway startup, each configured account publishes Afora's native
...
the deterministic Afora run id `clickclack:<message-id>`. Each model call is
...
Agent replies containing media use required durable delivery. Afora assigns
...
Afora does not reread the original local path or remote media URL.
...
Afora leaves the delivery unresolved rather than risking a duplicate; update
...
Accepted bot messages also pass through Afora's shared bot-pair loop guard.
...
Afora inbound turns, even when their author bot is explicitly allowed.
...
remote media URLs follow Afora's normal media-access policy, with a 64 MiB
...
```bash
afora message send --channel clickclack --target channel:general --message "hello"
afora message send --channel clickclack --target dm:usr_123 --message "hello"
afora message send --channel clickclack --target thread:msg_123 --message "following up"
```
...
Afora only needs current `bot:write` for normal agent chat and command-menu sync. Add `agent_activity:write` when enabling [native progress and agent activity rows](#native-progress-and-agent-activity-rows).
