...
Microsoft Teams ships as a bundled plugin in current Afora releases; no separate install is required in the normal packaged build.

On an older build or a custom install that excludes bundled Teams, install the npm package directly:

```bash
afora plugins install @afora/msteams
```
...
```bash
afora plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-afora-bot --allow-anonymous
devtunnel port create my-afora-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-afora-bot
...
**4. Configure Afora** using the credentials from the output:
...
- Talk to Afora via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists; they can change. Afora disables direct name matching by default; opt in with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, Afora resolves team/channel and user allowlist names to IDs (when Graph permissions allow) and logs the mapping. Unresolved names are kept as typed but ignored for routing unless `channels.msteams.dangerouslyAllowNameMatching: true` is set.
...
For production, Afora supports **federated authentication** as an alternative to client secrets, via `channels.msteams.authType: "federated"`. Two methods:
...
3. At runtime, Afora uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint.
...
**Default:** when `authType` is unset, Afora uses client-secret authentication (`appPassword`). Existing configs keep working unchanged.
...
These auth-related config keys can be set via environment variables instead of `afora.json` (other config keys, such as `groupPolicy` or `historyLimit`, are config-only):
...
Afora exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve verified roster details for a configured conversation.
...
```json5
{
  ...
  description: { short: "Afora in Teams", full: "Afora in Teams" },
...
Teams delivers messages via HTTP webhook. Afora applies fixed HTTP server
...
By default, Afora only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Per-user sharing is more secure since only chat participants can access the file. Afora requires a successful member lookup for group chats; timeouts, transport failures, empty results, and Graph API denials fail the send instead of widening access to the organization.
...
Uploaded files are stored in a `/AforaShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Afora sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `afora message poll --channel msteams --target conversation:<id> --poll-question "..." --poll-option "..." --poll-option "..."`.
- Votes are recorded by the gateway in Afora plugin-state SQLite under `state/afora.sqlite`.
- Existing `msteams-polls.json` files are imported by `afora doctor --fix`, not by the running plugin.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries, and there is no poll-results CLI yet.

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. Afora renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer, so Afora downgrades them to readable text before delivery.
...
```bash
afora message send --channel msteams \
...
afora message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
afora message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
afora message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
afora message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- Proactive messages are only possible **after** a user has interacted, because Afora stores conversation references at that point.
...
- **Ignore** the `groupId` query parameter for Afora routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
