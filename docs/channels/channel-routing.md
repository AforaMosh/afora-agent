...
Afora routes replies **back to the channel where a message came from**. The
...
Afora infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Afora still records inbound session metadata, but it
...
when a guarded path must not create a new Afora session. In that mode,
Afora may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Afora would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
  ...
  agents: {
    entries: {
      support: {
        default: true,
        name: "Support",
        workspace: "~/.afora/workspace-support",
      },
    },
  },
  bindings: [
...
store under `~/.afora/agents/<agentId>/sessions/`. Gateway startup and
`afora doctor --fix` import hot legacy rows/history into SQLite
automatically. Use `afora doctor --session-sqlite inspect
