---
summary: "Run Afora embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / afora / copilot) and want it to run through the Copilot CLI
---

The external `@afora/copilot` plugin runs embedded subscription Copilot
agent turns through the GitHub Copilot CLI (`@github/copilot-sdk`) instead of
Afora's built-in harness. The Copilot CLI session owns the low-level
agent loop: native tool execution, native compaction (`infiniteSessions`), and
CLI-managed thread state under `copilotHome`. Afora still owns chat
...
[Side questions (`/btw`)](/plugins/copilot#side-questions-%2Fbtw)), and `afora doctor`.
...
- Afora with the `@afora/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest id the
  ...
  `@afora/copilot` will not match and leaves the plugin blocked, even with
...
- A writable `copilotHome` directory. Defaults to `<agentDir>/copilot` when
  Afora provides an agent directory, otherwise
  `~/.afora/agents/<agentId>/copilot`.

`afora doctor` runs the plugin's [doctor contract](#doctor) for
session-state ownership and future config migrations. It does not probe the
Copilot CLI environment.

## Install

The Copilot runtime ships as an external plugin so the core `afora`
...
```bash
afora plugins install @afora/copilot
```
...
[Quickstart](#quickstart). Without that opt-in, Afora uses its built-in
GitHub Copilot provider and never installs this plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@afora/copilot`
   package.
2. The fallback dir `~/.afora/npm-runtime/copilot/` (legacy on-demand
   install target).
...
through Afora's guarded fetch path so DNS pinning and SSRF policy stay
owned by Afora. Use the native Afora runtime for local Ollama, LM
Studio, or LAN model servers.

## BYOK

Copilot BYOK uses the SDK's session-level custom provider contract. Afora
...
4. **Env-var fallback**, checked in this order (first non-empty value wins,
   ...
   1. `AFORA_GITHUB_TOKEN` — harness-specific override; lets you pin a
      token for the Afora harness without disturbing system-wide `gh` /
      Copilot CLI config.
...
Afora's `models.json` / `auth-profiles.json`), or
`~/.afora/agents/<agentId>/copilot` when no agent directory is supplied.
Override with `copilotHome: <path>` on the attempt input for a custom
location (for example, a shared mount for migration).

Live harness tests use `AFORA_COPILOT_AGENT_LIVE_TOKEN` for a direct
...
| `reasoningEffort`        | `"low" \| "medium" \| "high" \| "xhigh"`. Maps from Afora's `ThinkLevel` / `ReasoningLevel` resolution in `auto-reply/thinking.ts`.                                                                                                                                                          |
| `infiniteSessionConfig`  | Optional override for the SDK `infiniteSessions` block driven by `harness.compact`. Safe to leave as-is.                                                                                                                                                                                        |
| `hooksConfig`            | Optional native Copilot SDK `SessionHooks` config for tool/MCP, user-prompt, session, and error callbacks. Separate from Afora's portable lifecycle hooks.                                                                                                                                   |
...
Afora plugin hooks need no Copilot-specific attempt configuration. The
...
`before_compaction` and `after_compaction`. Bridged Afora tools run
`before_tool_call` and report `after_tool_call`; `hooksConfig` remains for
native SDK-only callbacks with no portable equivalent.

Nothing else in Afora needs to know about these fields. Other plugins,
...
The Afora-side transcript mirror (below) keeps receiving post-compaction
...
so Afora's `/btw` dispatcher (`src/agents/btw.ts`) falls through to the
...
- `ask_user` uses the provider-neutral gateway question runtime. The Control
  UI shows the same question card as other Afora questions, supported
...
Permission enforcement for bridged Afora tools happens **inside the tool
...
`createAforaCodingTools` to every coding tool: loop detection, trusted
...
- `skipPermission: true` — tells the SDK not to fire
  ...
  wrapped `execute()` already performs the richer Afora policy check; an
  SDK-level prompt would either short-circuit Afora's enforcement
  (allow-all) or block every tool call (reject-all) — neither matches PI
  parity.

The in-tree Codex harness uses the same split: bridged Afora tools are
...
`createAforaCodingTools`: identity (`senderIsOwner`, `memberRoleIds`,
...
`afora/plugin-sdk/agent-harness-tool-runtime` for PI parity. When
tool-search is enabled, the SDK sees compact control tools plus a hidden
catalog executor instead of every Afora tool schema. When code mode is
...
when the SDK request allows them. Aborting the Afora attempt cancels the
