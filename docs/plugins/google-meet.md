...
read_when:
  - You want an Afora agent to join a Google Meet call
  - You want an Afora agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

The `google-meet` plugin joins explicit Meet URLs on behalf of an Afora agent. It is deliberately narrow:
...
- `mode: "agent"` (default) transcribes participant speech with a realtime provider, routes it to the configured Afora agent, and speaks the answer with regular Afora TTS. `mode: "bidi"` lets a realtime voice model answer directly. `mode: "transcribe"` joins observe-only with no talk-back.
...
```bash
afora plugins install npm:@afora/google-meet
...
Afora provisions an `Afora Meeting Audio` null sink and matching source in that desktop user's audio session. Run the Gateway or paired node as the same user that runs Chrome.
...
Run `afora plugins disable google-meet` if you do not want the plugin active.

Check setup, then join:

```bash
afora googlemeet setup
afora googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
afora googlemeet setup --transport chrome-node --mode transcribe
```
...
```bash
afora googlemeet setup --transport twilio
```
...
```bash
afora googlemeet create --transport chrome-node --mode agent
afora googlemeet create --no-join
```
...
- **`browser`**: used without OAuth credentials. Afora opens `https://meet.google.com/new` on the pinned Chrome node and waits for Google to redirect to a real meeting-code URL; the Afora Chrome profile on that node must already be signed in to Google. Join and create both reuse an existing Meet tab (or an in-progress `.../new` / Google account prompt tab) before opening a new one; tab matching ignores harmless query strings like `authuser`.
...
```bash
afora googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
This only applies to API-created rooms, so OAuth must be configured. If you authenticated before this option existed, rerun `afora googlemeet auth login --json` after adding the `meetings.space.settings` scope to your OAuth consent screen.
...
Set `"mode": "transcribe"` to skip the duplex realtime bridge (no virtual-audio requirement, no talk-back). Transcribe-mode Chrome joins also skip Afora's microphone/camera permission grant and the Meet **Use microphone** path; if Meet shows the audio-choice interstitial, automation tries **Continue without microphone** first. Managed Chrome transports install a best-effort Meet caption observer in every mode so durable notes are available without changing the live agent-consult path. `googlemeet status --json` and `googlemeet doctor` report `captioning`, `captionsEnabledAttempted`, `transcriptLines`, `lastCaptionAt`, `lastCaptionSpeaker`, `lastCaptionText`, and a `recentTranscript` tail.

For the bounded session transcript, read the exact tracked Meet tab:

```bash
afora googlemeet transcript <session-id>
afora googlemeet transcript <session-id> --since <next-index> --json
```

The observer keeps at most 2,000 completed caption lines in the Meet page. Visible progressive text stays in the status health tail until the caption row completes, so saving `nextIndex` cannot skip a later text expansion; leaving finalizes visible rows before the snapshot. `droppedLines` reports lines lost from the head when the cap is exceeded. The bounded `googlemeet transcript` tail still keeps only the four most recently ended sessions and resets with the Gateway. Separately, Afora appends completed caption rows to the shared state database throughout the meeting and writes a derived summary on leave. Use [`afora transcripts`](/cli/transcripts) to inspect or export those durable notes.
...
```bash
afora googlemeet test-listen <meet-url> --transport chrome-node
```
...
Local Chrome joins through the signed-in Afora browser profile and routes its microphone and speaker through the native backend selected by `chrome.audioBackend`. The default shared loopback device is enough for a first smoke test but can echo; use separate virtual devices or a Loopback-style graph for clean duplex audio.
...
| Runs where           | What                                                                                            |
| -------------------- | ----------------------------------------------------------------------------------------------- |
| Gateway host         | Afora Gateway, agent workspace, model/API keys, realtime provider, Google Meet plugin config |
| Parallels macOS VM   | Afora CLI/node host, Chrome, SoX, BlackHole 2ch, a Chrome profile signed in to Google        |
...
```bash
afora plugins install npm:@afora/google-meet
afora node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
AFORA_ALLOW_INSECURE_PRIVATE_WS=1 \
  afora node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```

Use the same flag when installing as a LaunchAgent (it is process environment, stored in the LaunchAgent environment when present on the install command, not an `afora.json` setting):

```bash
AFORA_ALLOW_INSECURE_PRIVATE_WS=1 \
  afora node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
afora node restart
```
...
```bash
afora googlemeet test-speech https://meet.google.com/abc-defg-hij
```
...
If `chromeNode.node` is omitted, Afora auto-selects only when exactly one connected node advertises both `googlemeet.chrome` and browser control; pin `chromeNode.node` (node id, display name, or remote IP) when several capable nodes are connected.
...
| `No connected Google Meet-capable node`                  | Install `npm:@afora/google-meet` in the VM, run `afora plugins enable browser`, start `afora node run`, and approve pairing. If Google Meet was explicitly disabled, enable it too. Confirm `gateway.nodes.commands.allow` includes `googlemeet.chrome` and `browser.proxy`. |
...
| Chrome opens but cannot join                             | Sign in to the browser profile in the VM, or keep `chrome.guestName` set. Guest auto-join uses Afora browser automation through the node browser proxy; point the node's `browser.defaultProfile` (or a named existing-session profile) at the profile you want.                   |
| Duplicate Meet tabs                                      | Leave `chrome.reuseExistingTab: true`. Afora activates an existing tab for the same URL, and creation reuses an in-progress `.../new` or Google account prompt tab, before opening another.                                                                                        |
| No audio                                                 | Route Meet mic/speaker through the virtual audio path used by Afora; use separate virtual devices or Loopback-style routing for clean duplex audio.                                                                                                                                |

## Install notes

The Chrome talk-back default uses host audio tools that Afora does not bundle or redistribute:
...
- `pactl`, `pacat`, and `parec`: Linux PulseAudio utilities used against PipeWire-Pulse to provision and stream through `Afora Meeting Audio`.

SoX is licensed `LGPL-2.0-only AND GPL-2.0-only`; BlackHole is GPL-3.0. If you build an installer or appliance that bundles BlackHole with Afora, review BlackHole's upstream licensing or get a separate license from Existential Audio.
...
Opens the Meet URL through Afora browser control and joins as the signed-in Afora browser profile. Before launch, the plugin checks or provisions the host's native virtual-audio backend and then runs any configured audio bridge health/startup command. For local Chrome, pick the profile with `browser.defaultProfile`; `chrome.browserProfile` is passed to `chrome-node` hosts instead.

```bash
afora googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
afora googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Chrome mic/speaker audio routes through the local Afora audio bridge. If the native backend is unavailable, the join fails with a setup error instead of joining without an audio path.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Afora agent. Be brief.",
...
Provide Twilio credentials through environment to keep secrets out of `afora.json`:
...
afora plugins list | grep -E 'google-meet|voice-call'
afora googlemeet setup
```
...
`meetings.space.created` is required by `spaces.create`. `meetings.space.readonly` resolves Meet URLs/codes to spaces. `meetings.space.settings` lets Afora pass `SpaceConfig` settings such as `accessType` during API room creation. `meetings.conference.media.readonly` is for Meet Media API preflight and media work; Google may require Developer Preview enrollment for actual Media API use. `calendar.events.readonly` is only needed for `--today`/`--event` calendar lookup. `drive.meet.readonly` is only needed for `--include-doc-bodies` export. If you only need browser-based Chrome joins, skip OAuth entirely.
...
```bash
afora googlemeet auth login --json
```
...
```bash
AFORA_GOOGLE_MEET_CLIENT_ID="your-client-id" \
AFORA_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
afora googlemeet auth login --json --manual
```
...
```bash
afora googlemeet doctor --oauth --create-space --json
```

Prove read access to an existing space:

```bash
afora googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
afora googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```

A `403` from these checks usually means the Meet REST API is disabled, the refresh token is missing the required scope, or the Google account cannot access that space. A refresh-token error means rerun `afora googlemeet auth login --json` and store the new `oauth` block.

No OAuth is needed for the browser fallback; Google auth there comes from the signed-in Chrome profile on the selected node, not Afora config.

These environment variables are accepted as fallbacks:

- `AFORA_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `AFORA_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `AFORA_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `AFORA_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `AFORA_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `AFORA_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `AFORA_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`

### Resolve, preflight, and read artifacts

```bash
afora googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
afora googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

After Meet has created conference records:

```bash
afora googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
afora googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
afora googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
afora googlemeet latest --today
afora googlemeet calendar-events --today --json
afora googlemeet artifacts --event "Weekly sync"
afora googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
afora googlemeet latest --meeting https://meet.google.com/abc-defg-hij
afora googlemeet artifacts --conference-record conferenceRecords/abc123 --json
afora googlemeet attendance --conference-record conferenceRecords/abc123 --json
```

Close the room for an API-created space:

```bash
afora googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```

Calls `spaces.endActiveConference` and requires OAuth with the `meetings.space.created` scope for a space the authorized account can manage. Accepts a Meet URL, meeting code, or `spaces/{id}` and resolves it to the API space resource first. This is separate from `googlemeet leave`: `leave` stops Afora's local/session participation; `end-active-conference` asks Google Meet to end the active conference for the space.

Write a readable report:

```bash
afora googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
afora googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
afora googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
AFORA_LIVE_TEST=1 \
AFORA_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
pnpm test:live -- extensions/google-meet/google-meet.live.test.ts
```

```bash
afora googlemeet setup --transport chrome-node --mode transcribe
afora googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

| Variable                                                                                                                  | Purpose                                                                |
| ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `AFORA_LIVE_TEST=1`                                                                                                    | Enables guarded live tests                                             |
| `AFORA_GOOGLE_MEET_LIVE_MEETING`                                                                                       | Retained Meet URL, code, or `spaces/{id}`                              |
| `AFORA_GOOGLE_MEET_CLIENT_ID` / `GOOGLE_MEET_CLIENT_ID`                                                                | OAuth client id                                                        |
| `AFORA_GOOGLE_MEET_REFRESH_TOKEN` / `GOOGLE_MEET_REFRESH_TOKEN`                                                        | Refresh token                                                          |
| `AFORA_GOOGLE_MEET_CLIENT_SECRET`, `AFORA_GOOGLE_MEET_ACCESS_TOKEN`, `AFORA_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` | Optional; same fallback names without the `AFORA_` prefix also work |
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Afora browser profile, then retry meeting creation.",
  "manualAction": {
    "reason": "google-login-required",
    "message": "Sign in to Google in the Afora browser profile, then retry meeting creation."
  },
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
Creating joins by default, but Chrome/Chrome-node still needs a signed-in Google profile to join through the browser; if signed out, Afora returns `manualAction` or a browser fallback error and asks the operator to finish Google login before retrying.
...
The common Chrome agent path only needs the plugin enabled, BlackHole, SoX, a realtime provider key, and a configured Afora TTS provider:
...
| `chrome.guestName`                | `"Afora Agent"`                       | Shown on the signed-out Meet guest screen                                                                                                                                                                         |
...
| `chrome.audioInputCommand`        | generated native command                 | SoX/CoreAudio on macOS; `parec` from the Afora PipeWire-Pulse source on Linux                                                                                                                                  |
| `chrome.audioOutputCommand`       | generated native command                 | SoX/CoreAudio on macOS; `pacat` into the Afora PipeWire-Pulse sink on Linux                                                                                                                                    |
...
| `realtime.instructions`           | brief spoken-reply instructions          | Tells the model to speak briefly and use `afora_agent_consult` for deeper answers                                                                                                                              |
...
| `realtime.agentId`                | `"main"`                                 | Afora agent id used for `afora_agent_consult`                                                                                                                                                               |
...
An `afora doctor --fix` migration exists for the legacy `realtime.provider: "google"` shape: it moves that intent to `realtime.voiceProvider: "google"` plus `realtime.transcriptionProvider: "openai"` when those fields are not already set.
...
For Chrome transports, `leave` keeps a reused user-owned tab open after clicking Meet's Leave call button. Tabs opened by Afora are closed after departure.

Use `transport: "chrome"` when Chrome runs on the Gateway host, `transport: "chrome-node"` when it runs on a paired node. In both cases the model providers and `afora_agent_consult` run on the Gateway host, so model credentials stay there. Agent-mode logs include the resolved transcription provider/model at bridge startup and the TTS provider/model/voice/output format/sample rate after each synthesized reply. Raw `mode: "realtime"` is still accepted as a legacy compatibility alias for `mode: "agent"`, but it is no longer advertised in the tool's `mode` enum.
...
| `speechReady` / `speechBlockedReason` / `speechBlockedMessage` | Whether managed Chrome speech is allowed now; `speechReady: false` means Afora did not send the intro/test phrase   |
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Afora agent | Normal Afora TTS runtime            | You want "my agent is in the meeting" behavior        |
...
```bash
afora googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `afora logs --follow` shows DTMF TwiML served before realtime TwiML, then a realtime bridge with the initial greeting queued.
...
```bash
afora plugins list | grep google-meet
afora googlemeet setup
```
...
- **API creation**: `oauth.clientId` and `oauth.refreshToken` (or matching `AFORA_GOOGLE_MEET_*` env vars) are present, and the refresh token was minted after create support was added; older tokens may lack `meetings.space.created`, so rerun `afora googlemeet auth login --json`.
- **Browser fallback**: `defaultTransport: "chrome-node"` and `chromeNode.node` point at a connected node with `browser.proxy` and `googlemeet.chrome`; the Afora Chrome profile on that node is signed in and can open `https://meet.google.com/new`.
...
- **Audio-choice interstitial**: if Meet shows "Do you want people to hear you in the meeting?", leave the tab open. Afora should click **Use microphone** or (create-only) **Continue without microphone** and keep waiting for the generated URL; if it cannot, the error should mention `meet-audio-choice-required`, not `google-login-required`.

### Agent joins but does not talk

```bash
afora googlemeet setup
afora googlemeet doctor
```

Use `mode: "agent"` for the STT -> Afora agent -> TTS path, `mode: "bidi"` for the direct realtime voice fallback. `mode: "transcribe"` intentionally starts no talk-back bridge. For observe-only debugging, run `afora googlemeet status --json <session-id>` after participants speak and check `captioning`, `transcriptLines`, `lastCaptionText`. If `inCall` is true but `transcriptLines` stays `0`, Meet captions may be disabled, no one has spoken since the observer was installed, the Meet UI changed, or live captions are unavailable for the meeting language/account.

`googlemeet test-speech` always checks the realtime path and reports whether bridge output bytes were observed for that invocation. If `speechOutputVerified` is false and `speechOutputTimedOut` is true, the realtime provider may have accepted the utterance but Afora did not see new output bytes reach the Chrome audio bridge.
...
```bash
afora googlemeet recover-tab
afora googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
- `afora googlemeet doctor <session-id>`: confirm the delegated Twilio call ID, whether DTMF was queued, and whether the intro greeting was requested.
- `afora voicecall status --call-id <id>`: confirm the call is still active.
- `afora voicecall tail`: confirm Twilio webhooks are arriving at the Gateway.
- `afora logs --follow`: look for the Twilio Meet sequence: Google Meet delegates the join, Voice Call stores and serves pre-connect DTMF TwiML, Voice Call serves realtime TwiML for the Twilio call, then Google Meet requests intro speech with `voicecall.speak`.
- Re-run `afora googlemeet setup --transport twilio`; a green setup check is required but does not prove the meeting PIN sequence is correct.
...
- If the participant joins but you do not hear the greeting, check `afora logs --follow` for the post-DTMF `voicecall.speak` request and either media-stream TTS playback or the Twilio `<Say>` fallback. If the transcript still shows "enter the meeting PIN", the phone leg has not joined the Meet room yet, so participants will not hear speech.
