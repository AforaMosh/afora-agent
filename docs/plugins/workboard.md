...
Afora Gateway. It is not a replacement for GitHub Issues, Linear, Jira, or
...
1. Open **Plugins** in the Control UI, or use `/settings/plugins` relative to
   the configured Control UI base path. For example, a base path of `/afora`
   uses `/afora/settings/plugins`.
2. Find **Workboard** and choose **Enable**. Because Workboard is included with
   Afora, it does not need an **Install** action.
...
```bash
afora plugins disable workboard
afora gateway restart
```
...
that Gateway's Afora state (see [Storage](#storage)).
...
Afora subagent sessions still own execution. One dispatch pass:
...
Gateway (`AFORA_GATEWAY_URL` or `gateway.mode: remote`) apply, the CLI runs
...
`defaultAssignee`, and `orchestratorProfile`. Afora records this intent and
...
afora workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
afora workboard show <card-id> [--json]
afora workboard move <card-id> --status <status> [--json]
afora workboard dispatch [--board <id>] [--json]
```
...
under the Afora state directory: boards, cards, labels, lifecycle events,
...
`afora doctor --fix` to migrate the shipped legacy plugin-state namespaces
