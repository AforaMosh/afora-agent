---
summary: "Step-by-step guide to building a model provider plugin for Afora"
title: "Building provider plugins"
sidebarTitle: "Provider plugins"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Afora
  - You need to understand provider auth, catalogs, and runtime hooks
---

Build a provider plugin to add a model provider (LLM) to Afora: a model
...
<Tip>
  Provider plugins add models to Afora's normal inference loop. If the
...
<Steps>
  <Step title="Package and manifest">
    ...
    `setup.providers[].envVars` lets Afora detect credentials without
    ...
    optional and lets Afora auto-load your provider plugin from shorthand
    model ids like `acme-large` before runtime hooks exist. `afora.compat`
    and `afora.build` in `package.json` are required for ClawHub
    publishing (`afora.compat.pluginApi` and `afora.build.aforaVersion`
    are the two required fields; `minGatewayVersion` falls back to
    `afora.install.minHostVersion` when omitted).
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "afora-agent/plugin-sdk/provider-auth";
    ...
    calls and response mapping in the plugin; Afora owns the shared row
    shape, source labels, and help rendering.

    That is a working provider. Users can now run
    `afora onboard --acme-ai-api-key <key>` and select
    ...
    | Network limits | Fetches use Afora's SSRF guard, one 5-second timeout budget across pagination, a 4 MiB response limit per page, and a 50-page limit. Cross-origin pagination links are rejected; credentials are removed after a cross-origin redirect. |
    ...
    If the upstream provider uses different control tokens than Afora, add a
    ...
    Afora parses its own control markers or channel delivery.
    ...
    `buildProvider` is the live catalog path used when Afora can resolve real
    ...
    Afora's `models list --all` display currently executes static catalogs
    ...
    `afora/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `afora/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    import { buildProviderStreamFamilyHooks } from "afora-agent/plugin-sdk/provider-stream";
    ...
    <Accordion title="SDK seams powering the family builders">
      ...
      - `afora/plugin-sdk/provider-model-shared` - `ProviderReplayFamily`, `buildProviderReplayFamilyHooks(...)`, and the raw replay builders (`buildOpenAICompatibleReplayPolicy`, `buildAnthropicReplayPolicyForModel`, `buildGoogleGeminiReplayPolicy`, `buildHybridAnthropicOrOpenAIReplayPolicy`). Also exports Gemini replay helpers (`sanitizeGoogleGeminiReplayHistory`, `resolveTaggedReasoningOutputMode`) and endpoint/model helpers (`resolveProviderEndpoint`, `normalizeProviderId`, `normalizeGooglePreviewModelId`).
      - `afora/plugin-sdk/provider-stream` - `ProviderStreamFamily`, `buildProviderStreamFamilyHooks(...)`, `composeProviderStreamWrappers(...)`, plus the shared OpenAI/Codex wrappers (`createOpenAIAttributionHeadersWrapper`, `createOpenAIFastModeWrapper`, `createOpenAIServiceTierWrapper`, `createOpenAIResponsesContextManagementWrapper`, `createCodexNativeWebSearchWrapper`), DeepSeek V4 OpenAI-compatible wrapper (`createDeepSeekV4OpenAICompatibleThinkingWrapper`), Anthropic Messages thinking prefill cleanup (`createAnthropicThinkingPrefillPayloadWrapper`), plain-text tool-call compat (`createPlainTextToolCallCompatWrapper`), and shared proxy/provider wrappers (`createOpenRouterWrapper`, `createToolStreamWrapper`, `createMinimaxFastModeWrapper`).
      - `afora/plugin-sdk/provider-stream-shared` - lightweight payload and event wrappers for hot provider paths, including `createOpenAICompatibleCompletionsThinkingOffWrapper`, `createPayloadPatchStreamWrapper`, `createPlainTextToolCallCompatWrapper`, `normalizeOpenAICompatibleReasoningPayload(...)`, and `setQwenChatTemplateThinking(...)`.
      - `afora/plugin-sdk/provider-tools` - `ProviderToolCompatFamily`, `buildProviderToolCompatFamilyHooks("deepseek" | "gemini" | "openai")`, and underlying provider schema helpers.
      ...
      reasoning output so Afora consumes native thought parts without adding
    ...
    <Tabs>
      ...
      <Tab title="Usage and billing">
        ...
        auth but has no usable usage token, and Afora must skip generic
        API-key/OAuth fallback. Return `null` or `undefined` when the provider did
        not handle the request and Afora should continue with generic fallback.

        Declare the provider id in `contracts.usageProviders`. When that manifest
        contract and **both** hooks are present, Afora automatically includes
        ...
        is not auto-discovered, because Afora cannot resolve its usage credential.
      </Tab>
    </Tabs>

    <Accordion title="Common provider hooks">
      Afora calls hooks in roughly this order for model/provider plugins.
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    web fetch, and web search alongside text inference. Afora classifies this as a
    **hybrid-capability** plugin - the recommended pattern for company plugins
    ...
    <Tabs>
      ...
      <Tab title="Realtime transcription">
        ...
        `afora/plugin-sdk/provider-http`. The helper normalizes upload
      ...
      <Tab title="Realtime voice">
        ...
        honor `options.suppressResponse`. Afora then avoids suppression for
        ...
        the flag use Afora's local input-audio fallback detection.
      ...
      <Tab title="Media understanding">
        ...
        Afora still keeps the normal auth gate for providers that do not
...
├── package.json              # afora.providers metadata
├── afora.plugin.json      # Manifest with provider auth metadata
