...
The Policy plugin contributes doctor health checks for policy-managed Afora
...
posture, ingress/channel access posture, data-handling posture, and Afora config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
Afora settings and workspace declarations as evidence, and reports drift
through `afora policy check` and `afora doctor --lint`. A clean policy
check emits policy, evidence, findings, and attestation hashes that operators
can record for audit.

`afora policy check`, `watch`, and workspace-relative `compare` accept
`--agent <id>`. Explicit multi-agent fleets must select the workspace owner;
the plugin does not infer one from roster order.

`afora policy compare --baseline <file>` compares one policy file to another
