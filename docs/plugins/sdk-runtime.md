...
The broad compatibility barrel still exposes deprecated `writeConfigFile(config, options)` for published plugins. It accepts only a config derived from the current runtime snapshot, projects changed values onto the current authored source, and preserves untouched environment references and `$include` nodes. Its legacy explicit-set, unset, and allowed-roster-removal options are translated to source operations during the compatibility window; `allowIncludeAncestorExplicitSetPaths` is accepted as a no-op because canonical descendant sets preserve ancestor includes by construction. The adapter fails closed when a candidate cannot be projected unambiguously, including top-level type changes and position-ambiguous reorders or removals of duplicate resolved array values with distinct authored references. New code must use `mutateConfigFile(...)` or `replaceConfigFile(...)`; explicit full replacements that are not runtime-snapshot-derived must use `replaceConfigFile(...)`. The adapter is scheduled for the next approved Plugin SDK break window, no earlier than 2026-10-01.
