...
The broad compatibility barrel still exposes deprecated `writeConfigFile(config, options)` for published plugins. It snapshots the current config, projects the runtime-shaped candidate onto the authored source, creates an RFC 7396 merge intent, and sends that intent through the canonical writer. Untouched environment references and `$include` nodes survive, while writes that overlap read-only include ownership reject. Legacy explicit-set and unset options become direct source operations; `allowedAgentRosterRemovals` and `allowIncludeAncestorExplicitSetPaths` remain accepted for API compatibility but grant no extra write authority. The adapter performs no roster, model, include, map-rename, or array-identity inference. Candidate removal of a value that exists only after runtime defaulting has no authored source operation and is ignored. A changed container rejects when runtime resolution makes replacement lossy; otherwise arrays follow RFC 7396 replacement semantics. New code must use `mutateConfigFile(...)` or `replaceConfigFile(...)`. The adapter is scheduled for the next approved Plugin SDK break window, no earlier than 2026-10-01.
