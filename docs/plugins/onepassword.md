...
read_when:
  ...
  - You want Afora config credentials to resolve from 1Password
  - You need per-secret approval policy and audit history
  - You are configuring a 1Password service account for Afora
title: "1Password"
---
...
- Service-account authentication only. The token stays in a local credentials
  file and is never accepted in `afora.json`.
- Curated agent registry only. Agents can list configured slugs, but the plugin
  ...
  explicitly stored on registered Afora credential targets.
- Per-slug `auto`, `approve`, or `deny` policy.
- Approval grants expire. A cached value never bypasses current policy.
- Every access attempt is recorded in Afora's shared SQLite state. Audit
...
- After the current tool execution, Afora-owned transcript persistence
...
Create the token directory and file under the Afora state directory:
...
When `AFORA_STATE_DIR` is set, replace `~/.afora` with that directory.
...
Afora's shared exec-id grammar in a plugin-local opaque form and decodes them
...
```bash
afora onepassword secretref status --json
```

## Configure registered secrets

Add plugin config to `afora.json`:
...
approval. Afora offers allow always only when the caller has a concrete agent
...
```bash
afora onepassword audit
afora onepassword audit --limit 100
```
