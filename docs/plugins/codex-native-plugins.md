---
summary: "Configure native Codex plugins for Codex-mode Afora agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Afora agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are discovering or installing a Codex marketplace plugin
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Afora agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Afora turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. Afora does not translate
Codex plugins into synthetic `codex_plugin_*` Afora dynamic tools.
...
- Migration supports only `openai-curated` plugins that it observed as
  ...
  Afora treats both names as the one curated catalog, so configured
...
`codexPlugins` has no effect on Afora-provider runs, ACP conversation
...
```bash
afora migrate codex --dry-run --verify-plugin-apps
```
...
owner does not need to add that marketplace to Afora configuration before
...
allowlisted, those requirements still apply; Afora does not bypass them.
...
Afora install command afterward to authorize the already-installed plugin.
Afora keeps apps hidden when the response omits the exact marketplace,
...
When an authenticated owner creates an automation from a Codex turn, Afora
...
Jobs created before app authority capture may keep their ordinary Afora
...
the `operator.admin` scope. Afora's reserved `/codex` command is dispatched
...
again. `enable` and `disable` change Afora's persisted policy; qualified
...
| Enabled    | Codex reports the plugin enabled, and Afora config allows it for Codex harness turns.                                           |
...
- During planning, Afora reads source Codex `plugin/read` details and
...
For explicitly approved plugins from any discovered marketplace, Afora uses
...
- Afora does not add new Git or local marketplace sources in this flow.
  Additional sources must already be configured in Codex or be discoverable
  from the bound repository.

## App inventory and ownership

Afora first reads and caches one `plugin/installed` snapshot scoped to the
...
Afora reads installed app runtime state through `app/installed` and fetches
...
configured curated plugins are installed, Afora combines their cache
invalidations into a single app-inventory refresh. Ordinary cached reads do
not force a connector refresh for every new thread. Afora caches the
...
A plugin app is exposed only when Afora can map it back to the configured
...
and callable for that thread. Afora does not install, authenticate, or enable
...
names, Afora uses its `plugin/installed` snapshot and reads only that exact
...
Afora injects a restrictive `config.apps` patch for the Codex thread:
...
snapshot while `_default` is disabled. Afora provisionally admits only
...
thread is never bound or used. Afora deletes a failed persistent provisional
...
get `open_world_enabled: true`. Afora does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: Afora returns a deterministic decline.
- `true`: Afora auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: Afora exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into Afora plugin
  approvals before returning the Codex approval response.
- `"ask"`: Afora uses the same Codex write/destructive gating as
  ...
  admitted app using `"ask"`, Afora selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  Afora; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                    |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in Afora.                                                      |
...
| `plugin_detail_unavailable`                       | Afora could not read the exact configured plugin's ownership details.                                                             | Inspect the target app-server's `plugin/installed` and `plugin/read` responses.                                        |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | Afora schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known.  |
...
account-wide default can become callable after Afora starts and verifies
...
`/reset`. Afora does not authenticate plugin apps on the owner's behalf.

For `plugin_detail_unavailable`, verify that the exact installed marketplace
and plugin identity select a matching `plugin/read` result. Afora keeps
...
Codex thread bindings keep the app config they started with until Afora
