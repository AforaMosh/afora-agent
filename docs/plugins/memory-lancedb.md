...
```bash
afora plugins install @afora/memory-lancedb
```

The plugin is published to npm; it is not bundled into the Afora runtime
...
`afora doctor` reports when Remember across conversations is unavailable
...
Afora has a built-in dimension for `text-embedding-3-small` (1536) and
...
`memory_recall` tool, the `memory_forget` query path, and `afora ltm search`.
...
```bash
afora ltm list [--agent <id>] [--limit <n>] [--order-by-created-at]
afora ltm search <query> [--agent <id>] [--limit <n>]
afora ltm stats [--agent <id>]
```

`ltm query` runs a non-vector query directly against the LanceDB table:

```bash
afora ltm query --agent research --cols id,text,createdAt --limit 20
afora ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
LanceDB data defaults to `~/.afora/memory/lancedb`. Override with `dbPath`:
...
On upgrade, `afora doctor --fix` assigns those legacy rows once to the
...
plugin package (not the Afora core dist). Gateway startup does not repair
...
```bash
afora ltm stats
afora ltm search "recent preference"
```
