...
| `loopDetectionPreToolUseRelay`                | `true`                                                 | Run OpenClaw loop detection in the Codex `PreToolUse` relay. Set `false` to skip loop detection; the policy-free standby subprocess remains installed so policy adopted by a loaded thread can still be enforced.                                                                                                                                                                                                                  |
