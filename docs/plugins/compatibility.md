...
read_when:
  - You maintain an Afora plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Afora keeps older plugin contracts wired through named compatibility
...
Afora should not remove a documented plugin contract in the same release
...
machine-readable output in CI annotations. Afora core should expose
contracts and fixtures the inspector can consume, but should not publish the
inspector binary from the main `afora` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against Afora plugin
packages. Run it from a clean Afora checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @afora/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @afora/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @afora/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
