---
summary: "Run Afora embedded agent turns through the official Codex app-server harness"
title: "Codex harness"
read_when:
  - You want to use the official Codex app-server harness
  - You need Codex harness config examples
  - You want Codex-only deployments to fail instead of falling back to Afora
---

The official `codex` plugin runs embedded OpenAI agent turns through Codex
app-server instead of the built-in Afora harness. Codex owns the
low-level agent session: native thread resume, native tool continuation,
native compaction, and app-server execution. Afora still owns chat
channels, session files, model selection, Afora dynamic tools, approvals,
...
`remoteWorkspaceRoot` to validate remote workspace attachment paths. Afora
...
If Codex owns auth before Platform versus ChatGPT routing is known, Afora
...
When no Afora sandbox is active, Afora starts Codex app-server threads
with Codex native code mode enabled (code-mode-only stays off by default), so
native workspace/code capabilities remain available alongside Afora
dynamic tools routed through the app-server `item/tool/call` bridge. An
active Afora sandbox or restricted tool policy disables native code mode
entirely unless you opt into the experimental sandbox exec-server path.

With the default `tools.exec.host: "auto"` and no active Afora sandbox,
...
name or id and keeps Afora's node approval policy in force. If a finite
runtime allowlist disables native Code Mode and leaves the turn without an
execution environment, Afora keeps its policy-filtered `exec` and `process`
...
default, Afora hides the Codex-native shell and exposes `node_exec` and
...
[Afora Code Mode](/tools/code-mode), an opt-in QuickJS-WASI runtime
for generic Afora runs with a different `exec` input shape. For the
...
- The official `@afora/codex` plugin installed. Include `codex` in
...
- Codex auth through `afora models auth login --provider openai`, an
...
```bash
afora plugins install @afora/codex
afora models auth login --provider openai
```
...
The default `appServer.homeScope: "agent"` isolates each Afora agent from
...
that home's native Codex auth, config, plugins, and thread store. Afora does
not inject an Afora auth profile into this app-server, even when the agent's
...
- A Platform (API-key) route refuses a native home signed in with a ChatGPT
  ...
  and let Afora inject the key it already holds.

A stored OpenAI profile is fine alongside `homeScope: "user"`; Afora keeps it
for agent-scoped connections and simply does not hand it to the native home. Use
`afora models auth list --provider openai` to inspect stored profiles and
`afora models auth logout <profileId> --yes` to remove one you no longer want.
...
Afora; the fork attaches to the current Afora session and stays
...
`sessionCatalog.enabled: false` to remove it from the Afora sidebar without
...
native configuration; the outer Afora model and fallback chain never replace
...
| Need                                                | Set                                                                                                       | Where                              |
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| Enable the harness                                  | `plugins.entries.codex.enabled: true`                                                                     | Afora config                    |
...
| Keep an allowlisted plugin install                  | Include `codex` in `plugins.allow`                                                                        | Afora config                    |
...
| Sign in with ChatGPT/Codex OAuth                    | `afora models auth login --provider openai`                                                            | CLI auth profile                   |
| Add API-key backup for Codex runs                   | `openai:*` API-key profile listed after subscription auth in `auth.order.openai`                          | CLI auth profile + Afora config |
| Fail closed when Codex is unavailable               | Provider or model `agentRuntime.id: "codex"`                                                              | Afora model/provider config     |
| Use direct OpenAI API traffic                       | Provider or model `agentRuntime.id: "afora"` with normal OpenAI auth                                   | Afora model/provider config     |
...
Afora ignores those local summarizer overrides at runtime, and
`afora doctor --fix` removes them when the agent uses Codex.
...
window. When the model entry has no authored cap, Afora sends no override.
...
`agents.defaults.compaction.provider`. `afora doctor --fix` migrates the
...
Afora asks Codex to interrupt the native turn and keeps the per-thread fence
...
For the Afora Codex app-server harness, keep the default agent-scoped Codex
home and let Afora inject an `openai` API-key profile. Create the profile by
...
configuration unit. Apply them together. Afora can keep embedded and native
...
produced `5442` output tokens, and Afora sent the Codex app-server tier
...
output pricing. Afora currently sends the wire value
...
After installing or updating Afora, explicitly verify the managed package
...
exact Codex version pinned by Afora. It does not execute custom, remote, or
macOS desktop-owned app-servers.

`/status` reports the resolved Afora Fast policy (`on`, `off`, or `auto`)
...
- Do not use legacy Codex GPT refs in config; run `afora doctor --fix` to
...
- `agentRuntime.id: "afora"` opts a provider or model into the embedded
  Afora runtime when that is intentional.
...
| Direct OpenAI API-key traffic through Afora  | Provider or model `agentRuntime.id: "afora"` and normal OpenAI auth                                               | `/status` shows Afora runtime        | Use only when Afora is intentional                   |
| Legacy config                                   | legacy Codex GPT refs                                                                                                | `afora doctor --fix` rewrites it     | Do not write new config this way                        |
...
With Codex forced, Afora fails early if the effective route is not declared
Codex-compatible, the plugin is disabled, the app-server is too old, or the
app-server cannot start.

## App-server policy

By default, the plugin starts Afora's managed Codex binary locally with
...
implicit YOLO posture, Afora selects allowed guardian permissions
instead. When an Afora sandbox is active for the session, Afora
...
Shell access instead goes through Afora sandbox-backed dynamic tools such
as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

Use normalized Afora exec mode for Codex native auto-review before
...
Afora does not preserve legacy unsafe Codex `approvalPolicy: "never"` or
...
works, but `tools.exec.mode: "auto"` is the normalized Afora surface.
...
- `/codex goal` reads or updates the attached thread's native Codex goal. Codex automatic goal continuation stays disabled; Afora does not own autonomous follow-on turns yet.
- `/codex resume <thread-id>` attaches the current Afora session to an
...
`/fast` controls the shared Afora policy. A directive-only `/fast off`
persists `off` in the Afora session and sends `null` on affected Codex
harness turns to clear the Afora-owned service-tier override. `/fast default`
...
is not a synonym for `/fast off`, and it does not change the shared Afora
...
1. Ordered OpenAI auth profiles for the agent, preferably under
   `auth.order.openai`. Run `afora doctor --fix` to migrate older legacy
...
When Afora sees a ChatGPT subscription-style Codex auth profile, it
...
If a subscription profile hits a Codex usage limit, Afora records the
...
When native Codex plugins are configured, Afora reads and caches one
...
supplies authenticated app metadata in batches of at most 100 app IDs. Afora
...
Afora provisionally admits only explicitly allowed, ownership-proven apps,
...
The check runs before Afora starts a turn or commits a thread binding. A
failed persistent provisional thread is deleted; an ephemeral thread is
unsubscribed. If cleanup cannot be confirmed, Afora retires the app-server
connection instead of reusing an unsafe thread.

Account-wide app access never overrides an explicitly disabled configured
workspace plugin. When `app/read` omits that plugin's ownership, Afora uses
...
Afora does not install unknown apps or let the model authorize new plugin
...
For local stdio app-server launches, Afora sets `CODEX_HOME` to a
...
`~/.codex` by default. Afora preserves the normal process `HOME`;
...
`appServer.homeScope: "user"`, Afora instead uses the native user Codex
home and its existing account without injecting an Afora auth profile.
...
process. Afora removes `CODEX_HOME` and `HOME` from this list during
...
Codex dynamic tools default to `searchable` loading. Afora normally does
...
so Afora does not project a second goal store into Codex turns. Most
remaining Afora integration tools, such as messaging, media, cron,
browser, nodes, gateway, `progress_card`, and `heartbeat_respond` are available through
Codex tool search under the `afora` namespace, keeping the initial model
...
tool, use the `afora_direct` namespace instead. Codex treats that namespace
...
Afora's managed `web_search` dynamic tool are mutually exclusive so
managed search cannot bypass native domain restrictions. Afora uses the
managed tool when hosted search is unavailable, explicitly disabled, or
replaced by a selected managed provider. Afora keeps Codex's standalone
...
while explicit Afora or ACP delegation remains directly callable through
`sessions_spawn`. In Codex Code Mode, generic Afora
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Afora builds its inherited environment. Afora keeps the selected `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                                                                                                                              |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. Ordinary Afora dynamic tools remain available through nested `tools.*` calls; `afora_direct` tools stay directly model-visible.                                                                                                                                                                                                                                                |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an Afora sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the Afora sandbox egress setting.                                                                                                                        |
...
contract. When enabled, Afora also sets `features.network_proxy.enabled`
and `default_permissions` in the Codex thread config so the generated
permission profile can start Codex managed networking. By default, Afora
generates a collision-resistant `afora-network-<fingerprint>` profile
...
`view_image` and Afora suppresses its duplicate loader. The native Codex
schema accepts one local filesystem `path`. For text-only models, or when the
native tool surface is disabled, Afora supplies `view_image` with its
...
second Afora watchdog by default. A positive per-call `timeoutMs`
...
reduced by earlier preparation work. On timeout, Afora aborts the tool
...
releases the Afora session lane so follow-up chat messages are not
...
Tool handoffs use a longer post-tool idle budget: after Afora returns an
...
quiet without `turn/completed`, Afora best-effort interrupts the native
turn and releases the session lane. If another turn watch wins that release
race, Afora still accepts the completed final assistant item once no
...
`AFORA_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config
...
capabilities in the same Codex thread as the Afora harness turn. Afora
does not translate Codex plugins into synthetic `codex_plugin_*` Afora
dynamic tools.
...
Thread app config is computed when Afora establishes a Codex harness
...
Short version: Afora does not vendor the desktop-control app or execute
...
- Afora dynamic tools are supported. Codex asks Afora to execute
  those tools, so Afora remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  Afora can observe or block selected native events through the
  supported relay, but it does not rewrite native tool arguments.
- Codex owns native compaction. Afora keeps a transcript mirror for
  channel history, search, `/new`, `/reset`, and future model or harness
  switching, but does not replace Codex compaction with an Afora or
  context-engine summarizer.
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching Afora provider/model settings.
- `tool_result_persist` applies to Afora-owned transcript tool results,
...
**Afora uses the built-in harness instead of Codex:** confirm the effective
...
(
  ...
  if ls /tmp/AforaMosh/afora-agent-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/AforaMosh/afora-agent-*.log 2>/dev/null || true
  else
    journalctl --user -u afora-gateway --since today --no-pager 2>/dev/null \
...
**Legacy Codex model refs config remains:** run `afora doctor --fix`.
...
are rejected because Afora validates generated schemas and runtime contracts
...
first. Afora runs the local Codex app-server as a separate Rust child.
...
for up to 30 minutes. Afora independently keeps up to 64 idle conversation
...
id that Afora no longer has registered. This is a native Codex hook
...
Afora Gateway so old threads are dropped and native hook registrations
are recreated.
...
used for Afora loop detection and its no-policy marker. Required
