...
read_when:
  ...
  - You need to map a third-party CLI into Afora's text fallback runner
---

CLI backend plugins let Afora call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Afora at the plugin runtime module              |
| Manifest ownership   | `afora.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `afora.runtimeExtensions` pointing at the
  ...
  <Step title="Declare backend ownership">
    ```json afora.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Afora",
    ...
    `cliBackends` is the runtime ownership list; it lets Afora auto-load the
  ...
  <Step title="Register the backend">
    ...
    adapter is authoritative plugin code; Afora config selects the backend
...
`CliBackendConfig` describes how Afora should launch and parse the CLI. The
...
| `modelAliases`                                            | Map Afora model ids to CLI-native ids                                          |
| `sessionArgs`                                             | How to pass a session id using `{sessionId}`                                      |
| `sessionMode`                                             | `always`, `existing`, or `none`                                                   |
| `sessionIdFields`                                         | JSON fields Afora reads from CLI output                                        |
...
| `defaultAuthProfileId`             | Prefer a specific Afora auth profile                                     |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into Afora's loopback MCP tool bridge                                |
| `ownsNativeCompaction`             | Backend owns its own automatic compaction - Afora defers                 |
...
advertise in its initialization record before Afora trusts streamed output.
...
command. Afora hashes the bounded complete installed package tree, including
...
`sideQuestionToolMode: "disabled"`; otherwise Afora fails closed when BTW
...
`ctx.toolAvailability.afora` is the exact list of Afora tool names. The
host independently limits the generated MCP configuration and grant to that
Afora list; plugins must not translate it in core or add transport prefixes.
...
- `toolAvailabilityEnforcement: "prepare-execution"` requires
  ...
  Afora cleans up the staged resources before launch.

Runtime caps such as cron `toolsAllow` are normalized and group-expanded by
Afora before this contract is built. Native tools are disabled, and a
...
`resolveExecutionArgs`. Afora recognizes that shipped beta path from the
plugin package's required `afora.build.aforaVersion` metadata and
preserves it through the `2026.8.x` line. New and updated plugins should use canonical
`ctx.toolAvailability.afora` names and declare
...
Tool events describe work the backend already performed. Afora renders and
...
### `ownsNativeCompaction`: opting out of Afora compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so Afora's safeguard summarizer never runs
...
`manualCompaction`, so an explicit Afora `/compact` resumes the
...
over-budget session can stay over budget or go stale (Afora no longer
rescues it):
...
CLI backends do not receive Afora tools by default. If the CLI can consume
...
"always-on"` so Afora can fail closed when a caller requires no native
...
Put credentials in Afora auth profiles or plugin-owned config. Ensure the
...
```bash
afora plugins inspect acme-cli --runtime --json
afora agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `afora.extensions` and built runtime entries for published packages</Check>
<Check>`afora.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
