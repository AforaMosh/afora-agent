---
summary: "Manage Afora plugins from the Control UI or CLI"
read_when:
...
rules, and edge cases, see [`afora plugins`](/cli/plugins).
...
configured Control UI base path. For example, a base path of `/afora` uses
`/afora/settings/plugins`. The page has two tabs:
...
- **Discover** is the store: featured plugins included with Afora, official
...
or **Disable**. Workboard, for example, is included with Afora and disabled
...
OAuth-backed MCP connectors still need a one-time `afora mcp login <name>`
...
`plugins list` is a cold inventory check: what Afora can discover from
...
an install hint (`afora plugins install clawhub:<package>`) per result.

## Enable and disable plugins

```bash
afora plugins enable <plugin-id>
afora plugins disable <plugin-id>
```
...
afora plugins install clawhub:<package>@1.2.3
afora plugins install clawhub:<package>@beta

# Install from npm.
afora plugins install npm:<package>
afora plugins install npm:@scope/afora-plugin@1.2.3
afora plugins install npm:@afora/codex

# Install from a local npm-pack artifact.
afora plugins install npm-pack:<path.tgz>

# Install from git or a local development checkout.
afora plugins install git:github.com/acme/afora-plugin@v1.0.0
...
name matches a bundled or official plugin id, in which case Afora uses
that local/official copy instead. Use `clawhub:`, `npm:`, `git:`, or
`npm-pack:` for deterministic source selection. Afora's bundled and official
...
ClawHub, or hook-pack install, use `afora plugins update` instead. With
...
Afora records the install but leaves the plugin disabled. Configure
`plugins.entries.<id>.config`, then run `afora plugins enable <id>`. If an
...
```bash
afora plugins update <plugin-id>
afora plugins update <npm-package-or-spec>
afora plugins update --all
afora plugins update <plugin-id> --dry-run
```
...
If Afora cannot prove exactly one package owner and a complete child list,
update and uninstall fail closed without changing package files, config, or the
installed index. Run `afora plugins registry --refresh`, inspect
`afora plugins doctor`, and use `afora doctor --fix` for repairable legacy
index state. If the ambiguity remains, reinstall the package before retrying.

`afora plugins update --all` is the bulk maintenance path. It still
respects ordinary tracked install specs, but trusted official Afora
...
```bash
afora plugins update @scope/afora-plugin@beta
afora plugins update @scope/afora-plugin
```
...
See [`afora plugins`](/cli/plugins#update) for the exact fallback and
pinning rules.

## Uninstall plugins

```bash
afora plugins uninstall <plugin-id> --dry-run
afora plugins uninstall <plugin-id>
afora plugins uninstall <plugin-id> --keep-files
```
...
may break those Claws; use `afora claws status` to review ownership first.
Removing a Claw releases its plugin reference but retains the process-wide
plugin by default.

In Nix mode (`AFORA_NIX_MODE=1`), plugin install, update, uninstall,
...
| ----------- | --------------------------------------------------------------------------- | -------------------------------------------------------------- |
| ClawHub     | You want Afora-native discovery, scan summaries, versions, and hints     | `afora plugins install clawhub:<package>`                   |
| git         | You want a branch, tag, or commit from a repository                         | `afora plugins install git:github.com/<owner>/<repo>@<ref>` |
...
| npm pack    | You are proving a local package artifact through npm install semantics      | `afora plugins install npm-pack:<path.tgz>`                 |
| npmjs.com   | You already ship JavaScript packages or need npm dist-tags/private registry | `afora plugins install npm:@acme/afora-plugin`           |
...
ClawHub is the primary public discovery surface for Afora plugins. Publish
...
Native npm plugins must ship a plugin manifest (`afora.plugin.json`) plus
`package.json` metadata before publishing:

```json package.json
{
  "name": "@acme/afora-plugin",
  "version": "1.0.0",
  "type": "module",
  "afora": {
...
afora plugins install npm:@acme/afora-plugin
afora plugins install npm:@acme/afora-plugin@beta
afora plugins install npm:@acme/afora-plugin@1.0.0
```
...
- [Building plugins](/plugins/building-plugins) shows the full plugin
  package shape (including `afora.plugin.json`) and first publish
  workflow.
