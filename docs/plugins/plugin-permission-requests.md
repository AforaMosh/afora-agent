...
| Codex native permission requests | Codex asks before native shell, file, MCP, or app-server actions.        | Codex app-server or native hook approval handling, routed through plugin approvals when Afora owns the prompt. |
| MCP approval elicitations        | A Codex MCP server requests approval for a tool call.                    | MCP approval responses bridged through Afora plugin approvals.                                                 |
...
runs after the model selects a tool and before Afora executes it:
...
Afora creates a pending approval with a `plugin:` ID, delivers it to the
...
Afora treats `allow-once` and `allow-always` as approval decisions for the
...
If the hook also returns `params`, Afora snapshots the base parameters and
...
- Codex app-server approval requests route through Afora after Codex review.
