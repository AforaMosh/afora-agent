...
read_when:
  ...
  - You are changing plugin behavior across Afora and Codex harness turns
---
...
continuation, and native compaction. Afora owns channel routing, session
files, visible message delivery, Afora dynamic tools, approvals, media
...
policy. Afora caches a runtime-and-workspace-scoped `plugin/installed`
...
This check finishes before Afora injects history, starts a turn, or commits a
thread binding. Failed persistent provisional threads are deleted; ephemeral
threads are unsubscribed. Afora retires the app-server connection when safe
...
workspace plugin. Afora uses its installed snapshot and reads only that
...
Afora compatibility route keeps the normal Afora system prompt even when
it uses Codex-flavored OpenAI auth or transport.

Afora starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and Afora agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. Lightweight
Afora runs (for example cron) still suppress project-doc loading.

Afora developer instructions cover Afora runtime concerns: source-channel
delivery, Afora dynamic tools, ACP delegation, adapter context, and the
...
When `afora_direct.sessions_yield` is available, those instructions also
...
Most Afora dynamic tools use the searchable `afora` namespace. Tools
marked `catalogMode: "direct-only"` use `afora_direct`, which Codex keeps
directly model-visible as `DirectModelOnly` instead of exposing it to nested
Code Mode execution.

## Thread bindings and model changes

When an Afora session is attached to an existing Codex thread, the next
...
Supervised bindings are the exception. The Afora model picker stays locked,
...
model-difference warning; the outer Afora model and fallback chain never
...
not resume a thread, subscribe Afora to its live events, or answer its
approvals.
...
On the same supervision connection, Afora starts the canonical
...
resumed. The canonical thread has the full Afora harness tool surface;
...
Afora allows archive for a local `idle` or `notLoaded` row only after explicit
...
prove that another process is not using the thread. Afora blocks a known
...
contract: the agent replies normally and Afora posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable Afora tool
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Afora plugin hooks                 | Afora                 | Product/plugin compatibility across Afora and Codex harnesses.   |
| Codex app-server extension middleware | Afora bundled plugins | Per-turn adapter behavior around Afora dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Afora does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, Afora injects
...
so Codex's app-server reviewer and Afora's approval bridge handle real
...
controls; they are not exposed as Afora plugin hooks in the v1 contract.

For Afora dynamic tools, Afora executes the tool after Codex asks for
...
canonical tool record; Afora can mirror selected events but cannot rewrite
...
matching app-server approval. If an Afora `before_tool_call` hook returns
`requireApproval` while the native payload sets `afora_approval_mode:
...
request for the same tool use, Afora opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through Afora approvals when
...
notifications and Afora adapter state, not native Codex hook commands.
...
debugging. They do not invoke Afora plugin hooks.
...
Afora sandbox, sandboxed processes stream ordered stdout, stderr, or PTY
output notifications. Afora retains only a bounded recent-output buffer for
...
| Afora channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                    |
| Afora dynamic tools                        | Supported                                                                        | Codex asks Afora to execute these tools, so Afora stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | Afora projects Afora-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. Afora disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Afora-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through Afora after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | Afora records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Afora does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Afora owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Afora-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Afora can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Afora does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Afora can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, Afora only returns explicit allow or deny
...
request, Afora remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through Afora's plugin approval
...
resolve the same gateway record before Afora returns the app-server answer.
...
default `messages.queue.mode: "steer"`, Afora batches steer-mode chat
...
that case, Afora waits for the active run to finish before starting the
...
harness, Afora also calls Codex app-server `feedback/upload` for relevant
...
Afora session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, Afora does not print those Codex ids
...
belongs to Codex app-server. Afora does not run preflight compaction for
Codex turns, replace Codex compaction with context-engine compaction, or fall
back to Afora or public OpenAI summarization when native compaction cannot
be started. Afora keeps a transcript mirror for channel history, search,
...
Afora keeps the request and shared-client lease open until Codex emits the
...
timeout, Afora requests a native turn interrupt. The lease and per-thread
...
period, Afora retires the connection before releasing the fence. Remote
...
compaction is Codex's job; Afora only starts native compaction for manually
requested triggers.

When a context engine requests Codex thread-bootstrap projection, Afora
...
Codex reasoning or plan records when the app-server emits them. Afora
...
not rewrite Codex-native tool result records. It only applies when Afora
writes an Afora-owned session transcript tool result.

## Media and delivery

Afora continues to own media delivery and media provider selection. Image,
...
through the normal Afora delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, Afora forwards that exact file through the normal reply-media
