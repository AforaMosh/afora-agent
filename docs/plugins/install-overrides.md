...
[`afora plugins install`](/cli/plugins).
...
```bash
export AFORA_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export AFORA_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/afora-codex-2026.5.8.tgz",
  "afora-web-search": "npm:@afora/web-search@2026.5.8"
}'
```
...
When a setup-time flow installs a plugin whose id appears in the map, Afora
...
- Overrides do not inherit official trusted-source status. Even when the
  catalog entry normally represents an Afora-owned package, an override is
...
- Workspace `.env` files cannot enable install overrides; both env vars are on
  ...
  remote test command that launches Afora.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal Afora state:

```bash
npm pack extensions/codex --pack-destination /tmp

AFORA_STATE_DIR="$(mktemp -d)" \
AFORA_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
AFORA_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/afora-codex-2026.5.8.tgz"}' \
pnpm afora onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$AFORA_STATE_DIR/npm/projects" -path '*/node_modules/@afora/codex/package.json' -print
grep -R '"@afora/codex"' "$AFORA_STATE_DIR/npm/projects"/*/package-lock.json
```
