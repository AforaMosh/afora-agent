...
It ships with Afora but is disabled by default; when disabled, the route is not registered. When enabled, it adds `POST /api/v1/admin/rpc` on the same listener as the Gateway (`http://<gateway-host>:<port>/api/v1/admin/rpc`).
...
For Afora clients and interactive tools that can keep a Gateway WebSocket connection open, use WebSocket RPC instead.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    afora plugins enable admin-http-rpc
...
```bash
afora plugins disable admin-http-rpc
afora gateway restart
```
...
- Shared-secret bearer auth (`token`/`password` modes) proves possession of the gateway operator secret; narrower `x-afora-scopes` headers are ignored on that path and normal full operator defaults are restored.
- Trusted identity-bearing HTTP auth (`trusted-proxy` mode) honors `x-afora-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for Afora clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
