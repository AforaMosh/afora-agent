...
- Re-open or repair a browser anytime: `afora dashboard`. It copies/opens a single-use pairing link
...
- If clipboard and browser delivery both fail, `afora dashboard` either gives a safe manual-token
  hint or tells you to run `afora dashboard --json` and open its short-lived `browserUrl`; it never
...
- **Shared-secret token source**: `gateway.auth.token` (or `AFORA_GATEWAY_TOKEN`). Manual token entry
  is kept in sessionStorage for the current tab and selected gateway URL, not localStorage.
- **Host-authorized browser handoff**: `afora dashboard` issues a short-lived, single-use bootstrap
...
- **Missing-config runtime token**: if startup says it generated a runtime token, that token is ephemeral and cannot be recovered. Loopback still requires auth. Run `afora doctor --generate-gateway-token`, restart the Gateway, then run `afora gateway auth-token --show` in an interactive terminal and paste the output into Control UI settings.
...
- **Shared-secret password**: use the configured `gateway.auth.password` (or `AFORA_GATEWAY_PASSWORD`). The dashboard does not persist passwords across reloads.
...
- Confirm the gateway is reachable: local `afora status`; remote, SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@gateway-host` then open `http://127.0.0.1:18789/`.
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: run `afora gateway auth-token --show` in an interactive terminal on the Gateway host
  - Password: resolve the configured `gateway.auth.password` or `AFORA_GATEWAY_PASSWORD`
  - SecretRef-managed token: run `afora gateway auth-token --show`; if resolution fails, repair the external secret provider and rerun it
  - Runtime token generated because no shared secret was configured: run `afora doctor --generate-gateway-token`, restart the Gateway, then use the configured token
