...
Staged images, files, pasted images, large pasted text, browser annotations, and mixed attachment packages stay with their composer and session across route changes, split-pane remounts, hard reloads, and application restarts. Restart recovery is browser-local and isolated to the same Gateway login and session; switching the Gateway or authenticated profile hides drafts owned by the other connection. The browser keeps the 20 most recently edited drafts for each Gateway login for up to seven days, with at most 25 MiB of staged attachment data in one draft. Sending the draft, removing an attachment, deleting its session, or clearing browser site data retires the corresponding stored data. If the browser refuses storage, the current tab keeps the live composer and shows a storage warning, but that draft cannot be recovered after restart.
