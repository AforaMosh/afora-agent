...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - Chat uploads accept images, videos, and other files. Before reading selected files into full data URLs, the Control UI checks their count and decoded sizes. The shared Control UI/Gateway budget is 4 attachments, 8 MiB per non-image attachment, 6 MiB per image, 12 MiB decoded aggregate, and 18 MiB for the encoded `chat.send` request frame; the frame stays below the Gateway's 25 MiB WebSocket cap. Native video accepts `video/mp4`, `video/mpeg`, `video/mov`, `video/quicktime`, `video/avi`, `video/x-msvideo`, `video/x-flv`, `video/mpg`, `video/webm`, `video/wmv`, `video/x-ms-wmv`, and `video/3gpp`; unsupported formats follow the description/omission behavior below. A rejected batch stays out of the composer and the UI explains whether its item size, count, aggregate size, or encoded frame exceeded the limit.
    - Native video is user-message-only. Ordinary Google AI Studio Gemini chat and ordinary Moonshot K3 Chat Completions can receive it; unsupported reply routes use an available video description or one bounded visible omission. Tool-result video is omitted as bounded text and never forwarded as raw bytes.
    - Sent user videos are stored as managed `media://inbound/...` references plus immutable metadata; raw video bytes and data URLs are not stored in the SQLite transcript. After a restart, replay resolves the managed file only while it still exists. `attachments.ttlHours` can enable the general staged-media cleanup for 1–168 hours; leaving it unset disables that sweep. Missing or expired historical video replays as a bounded omission.
