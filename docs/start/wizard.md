---
summary: "CLI onboarding: verify inference, then hand remaining setup to Afora"
read_when:
...
the machine, verifies it with a real completion, and starts Afora to
configure the workspace, Gateway, and optional features. `afora setup` runs the same flow ([Setup](/cli/setup) covers
...
requires a real completion, and only then starts [Afora](/cli/afora)
to configure the rest of Afora. Choosing **Skip for now** exits onboarding
without starting Afora.
...
with `afora onboard --classic`; the guided inference picker does not delegate
into it. After inference passes, Afora can use `open channel wizard for
...
`afora gateway restart`. Remote Gateway mode remains an onboarding or
`afora configure` choice rather than a hosted chat wizard.
...
To change the model provider or its authentication, exit Afora and run
`afora onboard`; Afora does not open guided or classic provider flows.

<Info>
Fastest first chat: finish guided setup, run `afora dashboard`, and chat in
...
`AFORA_LOCALE`, `LC_ALL`, `LC_MESSAGES`, and `LANG`, in that order, then
...
key-free. Configure this later with `afora configure --section web`, or say
`configure web search` in the Afora chat to run the same provider setup
conversationally. Docs: [Web tools](/tools/web).
</Tip>

## Guided default

Plain `afora onboard` follows this path:
...
4. If detection is exhausted, choose OpenAI, Anthropic, xAI (Grok), Google, or
   ...
   Choose **Skip for now** to exit without starting Afora.
...
6. Start Afora with the verified model so it can configure the workspace,
...
asks how to continue. Run `afora channels add` or `afora configure` for
later non-inference additions; use `afora onboard` for provider or auth route
changes.

## Classic wizard setup modes

Run `afora onboard --classic` to open the full wizard. Its **Setup mode**
...
1. **Workspace** - directory for agent files (default `~/.afora/workspace`). Seeds bootstrap files.
2. **Model/Auth** - pick a provider auth flow (API key, OAuth, or
   ...
   classic wizard. Ignoring it does not unlock Afora; conversational setup
...
[Install](/install/migrating-hermes). `afora onboard --modern` is a
compatibility alias for [Afora](/cli/afora). It uses the same
inference gate as `afora setup`: verified inference starts the
assistant, while an interactive failure returns to guided inference setup.

## Add another agent

Use `afora agents add <name>` to create a separate agent with its own
...
not the full `afora onboard` wizard.
...
- Default workspace: `~/.afora/workspace-<agentId>` (or under
...
For the full flag reference, see [`afora onboard`](/cli/onboard).

## Related docs

- CLI command reference: [`afora onboard`](/cli/onboard)
