...
Afora exposes four user-facing update channels:
...
gh workflow run afora-npm-release.yml \
...
```bash
node --import tsx scripts/afora-npm-postpublish-verify.ts YYYY.M.P
npm view afora@YYYY.M.P version --userconfig "$(mktemp)"
npm view afora@extended-stable version --userconfig "$(mktemp)"
```
...
`npm dist-tag add afora@YYYY.M.P extended-stable` repair command printed in
...
`Afora Release Publish`. Its Docker-only extended-stable path rechecks the
...
```bash
gh workflow run afora-release-publish.yml \
  --ref main \
  -f tag=vYYYY.M.P \
  -f preflight_run_id=<npm-preflight-run-id> \
  -f full_release_validation_run_id=<full-validation-run-id> \
...
11. For stable, continue only after the vetted beta or release candidate has the required validation evidence. Stable npm publish also goes through `Afora Release Publish`, reusing the successful preflight artifact via `preflight_run_id`. Stable macOS release readiness also requires the packaged `.zip`, `.dmg`, `.dSYM.zip`, and updated `appcast.xml` on `main`; the macOS publish workflow publishes the signed appcast to public `main` automatically after release assets verify, or opens/updates an appcast PR if branch protection blocks the direct push. Stable Windows Hub readiness requires the signed `AforaCompanion-Setup-x64.exe`, `AforaCompanion-Setup-arm64.exe`, and `AforaCompanion-SHA256SUMS.txt` assets on the Afora GitHub release. Pass the exact signed `AforaMosh/afora-agent-windows-node` release tag as `windows_node_tag` and its candidate-approved installer digest map as `windows_node_installer_digests`; `Afora Release Publish` keeps the release draft, dispatches `Windows Node Release`, and verifies all three assets before publication.
...
5. Run `pnpm release:generated:check`, `pnpm deps:npm-lock:check`, and `AFORA_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main` contains the shipped version and changelog before calling the stable release done.
...
- Plugin version sync updates the publishable `@afora/ai` runtime package, official plugin package versions, and existing `afora.compat.pluginApi` floors to the Afora release version by default. Treat that field as the plugin SDK/runtime API floor, not just a copy of the package version: for plugin-only releases that intentionally remain compatible with older Afora hosts, keep the floor at the oldest supported host API and document that choice in the plugin release proof.
- Run the manual `Full Release Validation` workflow before release approval to kick off all pre-release test boxes from one entrypoint. It accepts a branch, tag, or full commit SHA, dispatches manual `CI`, and dispatches `Afora Release Checks` for install smoke, package acceptance, cross-OS package checks, QA Lab parity, Matrix, and Telegram lanes. Stable and full runs always include exhaustive live/E2E and Docker release-path soak; `run_release_soak=true` is retained for an explicit beta soak. Package Acceptance provides the canonical package Telegram E2E during candidate validation, avoiding a second concurrent live poller.
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof for a package candidate while release work continues. Use `source=npm` for `afora@beta`, `afora@latest`, or an exact release version; `source=ref` to pack a trusted `package_ref` branch/tag/SHA with the current `workflow_ref` harness; `source=url` for a public HTTPS tarball with a required SHA-256 and strict public URL policy; `source=trusted-url` for a named trusted-source policy using required `trusted_source_id` and SHA-256; or `source=artifact` for a tarball uploaded by another GitHub Actions run.
  ...
  ```bash
  gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=afora@beta -f suite_profile=product -f published_upgrade_survivor_baseline=afora@2026.4.26 -f telegram_mode=mock-openai
  ```
...
- `Afora NPM Release` preflight generates dependency release evidence before it packs the npm tarball. The npm advisory vulnerability gate is release-blocking. The transitive manifest risk, dependency ownership/install surface, and dependency change reports are release evidence only. The dependency change report compares the release candidate with the previous reachable release tag. The preflight uploads dependency evidence as `afora-release-dependency-evidence-<tag>` and also embeds it under `dependency-evidence/` inside the prepared npm preflight artifact. The real publish path reuses that preflight artifact, then attaches the same evidence to the GitHub release as `afora-<version>-dependency-evidence.zip`.
- Run `Afora Release Publish` for the mutating publish sequence after the tag exists. Dispatch regular beta and stable publishes from trusted `main`; the release tag still selects the exact target commit and may point into `release/YYYY.M.PATCH`. Tideclaw alpha publishes remain on their matching alpha branch. Pass the successful Afora npm `preflight_run_id`, successful `full_release_validation_run_id`, and exact `full_release_validation_run_attempt`, and keep the default plugin publish scope `all-publishable` unless you are deliberately running a focused repair. The workflow serializes plugin npm publish, plugin ClawHub publish, and Afora npm publish so the core package is not published before its externalized plugins; Windows and Android promotion runs concurrently with the core npm publish against the draft release page. Publish reruns are resumable: an already-published core npm version skips the core dispatch after the workflow proves the registry tarball matches the tag's preflight artifact, and Windows/Android promotion is skipped when the release already carries the verified asset contract, so a retry only redoes the failed stages. Focused plugin-only repairs require `plugin_publish_scope=selected` and a nonempty plugin list. Plugin-only `all-publishable` runs require complete immutable preflight and Full Release Validation evidence; partial evidence is rejected.
- Stable `Afora Release Publish` requires an exact `windows_node_tag` after the matching non-prerelease `AforaMosh/afora-agent-windows-node` release exists, plus the candidate-approved `windows_node_installer_digests` map. Before dispatching any publish child, it verifies that source release is published, non-prerelease, contains the required x64/ARM64 installers, and still matches that approved map. It then dispatches `Windows Node Release` while the Afora release is still a draft, carrying the pinned installer digest map unchanged. The child workflow downloads the signed Windows Hub installers from that exact tag, matches them against the pinned digests, verifies their Authenticode signatures use the expected Afora Foundation signer on a Windows runner, writes a SHA-256 manifest, and uploads the installers plus manifest onto the canonical Afora GitHub release, then re-downloads the promoted assets and verifies manifest membership and hashes. The parent verifies the current x64, ARM64, and checksum asset contract before publication. Direct recovery rejects unexpected `AforaCompanion-*` asset names before replacing the expected contract assets with the pinned source bytes.

  Manually dispatch `Windows Node Release` only for recovery, and always pass an exact tag, never `latest`, plus the explicit `expected_installer_digests` JSON map from the approved source release. Website download links should target exact Afora release asset URLs for the current stable release, or `releases/latest/download/...` only after verifying GitHub's latest redirect points at that same release; do not link only to the companion repo release page.

- Release checks now run in a separate manual workflow: `Afora Release Checks`. It always runs the QA Lab mock parity lane. The Matrix catalog and Telegram QA-live lanes run for stable/full validation, soak-enabled validation, or an explicit `qa`/`qa-live` rerun group. Bounded beta-publish `all` without soak defers those live lanes to postpublish-confidence. The live lanes use the `qa-live-shared` environment; Telegram also uses Convex CI credential leases.
- Cross-OS install and upgrade runtime validation is part of public `Afora Release Checks` and `Full Release Validation`, which call the reusable workflow `.github/workflows/afora-cross-os-release-checks-reusable.yml` directly. This split is intentional: keep the real npm release path short, deterministic, and artifact-focused, while slower live checks stay in their own lane so they do not stall or block publish.
...
- That workflow runs `AFORA_LIVE_TEST=1 AFORA_LIVE_CACHE_TEST=1 pnpm test:live:cache` using both `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` workflow secrets.
- npm release preflight no longer waits on the separate release checks lane.
- Before tagging a release candidate locally, run `RELEASE_TAG=vYYYY.M.PATCH-beta.N pnpm release:fast-pretag-check`. The helper runs the fast release guardrails, plugin npm/ClawHub release checks, build, UI build, and `release:afora:npm:check` in the order that catches common approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.PATCH node --import tsx scripts/afora-npm-release-check.ts` (or the matching prerelease/correction tag) before approval.
- After npm publish, run `node --import tsx scripts/afora-npm-postpublish-verify.ts YYYY.M.PATCH` (or the matching beta/correction version) to verify the published registry install path in a fresh temp prefix.
- After a beta publish, run `AFORA_NPM_TELEGRAM_PACKAGE_SPEC=afora@YYYY.M.PATCH-beta.N AFORA_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex AFORA_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live` to verify installed-package onboarding, Telegram setup, and real Telegram E2E against the published npm package using the shared leased Telegram credential pool. Local maintainer one-offs may omit the Convex vars and pass the three `AFORA_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation uses preflight-then-promote:
  ...
  - Token-based npm dist-tag mutation lives in `afora/releases/.github/workflows/afora-npm-dist-tags.yml` because `npm dist-tag add` still needs `NPM_TOKEN` while the source repo keeps OIDC-only publish.
...
pnpm ci:full-release \
  --sha <release-sha> \
  --target-ref release/YYYY.M.PATCH \
  -f release_package_spec=afora@YYYY.M.PATCH-beta.N \
  -f evidence_package_spec=afora@YYYY.M.PATCH-beta.N \
...
The Docker box lives in `Afora Release Checks` through `afora-live-and-e2e-checks-reusable.yml`, plus the release-mode `install-smoke` workflow. It validates the release candidate through packaged Docker environments instead of only source-level tests.
...
The QA Lab box is also part of `Afora Release Checks`. It is the agentic behavior and channel-level release gate, separate from Vitest and Docker package mechanics.
...
The Package box is the installable-product gate. It is backed by `Package Acceptance` and the resolver `scripts/resolve-afora-package-candidate.mts`. The resolver normalizes a candidate into the `package-under-test` tarball consumed by Docker E2E, validates the package inventory, records the package version and SHA-256, and keeps the workflow harness ref separate from the package source ref.

Supported candidate sources:

- `source=npm`: `afora@beta`, `afora@latest`, or an exact Afora release version
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=afora@2026.4.26
```
...
`Afora Release Publish` is the normal mutating entrypoint. The monthly
...
6. Dispatch `Afora NPM Release` with the release tag, npm dist-tag, and saved `preflight_run_id` after verifying the saved `full_release_validation_run_id` and exact run attempt.
...
gh workflow run afora-release-publish.yml \
  ...
  -f preflight_run_id=<successful-afora-npm-preflight-run-id> \
...
gh workflow run afora-release-publish.yml \
  ...
  -f windows_node_installer_digests='{"AforaCompanion-Setup-x64.exe":"sha256:<approved-x64-sha256>","AforaCompanion-Setup-arm64.exe":"sha256:<approved-arm64-sha256>"}' \
  -f preflight_run_id=<successful-afora-npm-preflight-run-id> \
  -f full_release_validation_run_id=<successful-full-release-validation-run-id> \
  -f full_release_validation_run_attempt=<successful-full-release-validation-run-attempt> \
  -f plugin_sdk_api_acknowledgement=<reviewed-8-character-digest> \
...
Use the lower-level `Plugin NPM Release` and `Plugin ClawHub Release` workflows only for focused repair or republish work. `Afora Release Publish` rejects `plugin_publish_scope=selected` when `publish_afora_npm=true` so the core package cannot ship without every publishable official plugin, including `@afora/diffs-language-pack`. For a selected plugin repair, set `publish_afora_npm=false` with `plugin_publish_scope=selected` and `plugins=@afora/name`, or dispatch the child workflow directly.
...
release tag plus the parent `Afora Release Publish` run id, attempt, and
...
`Afora NPM Release` accepts these operator-controlled inputs:
...
- `release_publish_run_id`: approved `Afora Release Publish` run id; required when this workflow is dispatched by that parent (bot-actor real-publish calls)
...
`Afora Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Afora NPM Release` preflight run id; required when `publish_afora_npm=true` or `plugin_publish_scope=all-publishable`
- `full_release_validation_run_id`: successful `Full Release Validation` run id; required when `publish_afora_npm=true` or `plugin_publish_scope=all-publishable`
...
- `windows_node_tag`: exact non-prerelease `AforaMosh/afora-agent-windows-node` release tag; required for stable Afora publish
- `windows_node_installer_digests`: candidate-approved compact JSON map of the current Windows installer names to their pinned `sha256:` digests; required for stable Afora publish
- `npm_telegram_run_id`: optional successful `NPM Telegram Beta E2E` run id to include in final release evidence
- `npm_dist_tag`: npm target tag for the Afora package, one of `alpha`, `beta`, `latest`, or `extended-stable`
- `publish_docker_only`: beta or extended-stable recovery/closeout path. It requires `publish_afora_npm=false`, complete preflight and Full Release Validation evidence, then verifies the exact npm package, selected dist-tag, and tarball digest before invoking Docker publication.
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only for focused plugin-only repair work with `publish_afora_npm=false`
- `plugins`: comma-separated `@afora/*` package names when `plugin_publish_scope=selected`
- `publish_afora_npm`: defaults to `true`; set `false` only when using the workflow as a plugin-only repair orchestrator
...
`Afora Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks require the resolved commit to be reachable from an Afora branch or release tag.
...
- For `Afora NPM Release`, full commit SHA input is allowed only when `preflight_only=true`
- `Afora Release Checks` and `Full Release Validation` are always validation-only
...
1. Run `Afora NPM Release` with `preflight_only=true`. Before a tag exists, you may use the current full workflow-branch commit SHA for a validation-only dry run of the preflight workflow.
...
4. Select the exact non-prerelease `AforaMosh/afora-agent-windows-node` release tag whose signed x64 and ARM64 installers should ship. Save it as `windows_node_tag`, and save their validated digest map as `windows_node_installer_digests`. The release-candidate helper records both and includes them in its generated publish command.
5. Save the successful `preflight_run_id`, `full_release_validation_run_id`, and exact `full_release_validation_run_attempt`.
6. Run `Afora Release Publish` from trusted `main` with the same `tag`, the same `npm_dist_tag`, the selected `windows_node_tag`, its saved `windows_node_installer_digests`, the saved `preflight_run_id`, `full_release_validation_run_id`, and `full_release_validation_run_attempt`. It publishes externalized plugins to npm and ClawHub before promoting the Afora npm package.
7. If the release landed on `beta`, use the `afora/releases/.github/workflows/afora-npm-dist-tags.yml` workflow to promote that stable version from `beta` to `latest`.
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/AforaMosh/afora-agent/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/AforaMosh/afora-agent/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/afora-npm-release.yml`](https://github.com/AforaMosh/afora-agent/blob/main/.github/workflows/afora-npm-release.yml)
- [`.github/workflows/afora-release-checks.yml`](https://github.com/AforaMosh/afora-agent/blob/main/.github/workflows/afora-release-checks.yml)
- [`.github/workflows/afora-cross-os-release-checks-reusable.yml`](https://github.com/AforaMosh/afora-agent/blob/main/.github/workflows/afora-cross-os-release-checks-reusable.yml)
- [`.github/workflows/docker-release.yml`](https://github.com/AforaMosh/afora-agent/blob/main/.github/workflows/docker-release.yml)
- [`scripts/resolve-afora-package-candidate.mts`](https://github.com/AforaMosh/afora-agent/blob/main/scripts/resolve-afora-package-candidate.mts)
- [`scripts/afora-npm-release-check.ts`](https://github.com/AforaMosh/afora-agent/blob/main/scripts/afora-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/AforaMosh/afora-agent/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/AforaMosh/afora-agent/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in [`afora/maintainers/release/README.md`](https://github.com/afora/maintainers/blob/main/release/README.md) for the actual runbook.
