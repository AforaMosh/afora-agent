...
Afora only seeds this file into a brand-new workspace, alongside `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, and `USER.md`. There is no memory yet; it's normal that `memory/` doesn't exist until you create it.
...
```bash
afora agents set-identity --workspace "<this workspace>" --name "<name>" --theme "<vibe>" --emoji "<emoji>"
```
...
```bash
afora onboard recommendations acknowledge --retry "<failed-id>" ["<failed-id>"...]
```
...
JSON output has `afora.resolution.source` set to `installed`. A registry
...
> Ask me anything; for system things I'll ask Afora.

Once the file is removed, Afora treats the birth sequence as complete and
