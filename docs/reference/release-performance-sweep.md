...
read_when:
  ...
  - You need the numbers behind the Afora performance and dependency blog post
...
This page captures the evidence behind the May 2026 Afora performance,
...
- **Release performance sweep:** GitHub Releases from `v2026.5.28` back through
  stable `v2026.4.23`, using the `Afora Performance` workflow,
...
- **npm package size sweep:** `npm pack afora@<version> --dry-run --json`
...
<CardGroup cols={2}>
  ...
  <Card title="Latest stable install" icon="hard-drive">
    **361.7MiB fresh install**
    ...
    Cuts the nested Afora dependency tree sharply from the `2026.5.22`
...
<CardGroup cols={2}>
  ...
  <Card title="Nested tree" icon="unplug">
    Nested `afora/node_modules` fell from **656.1MiB** to **259.7MiB** in
...
| Nested `afora/node_modules` from `2026.5.22` |   911.8MB |     259.7MiB | 71.5% lower |
| Nested `afora/node_modules` from `2026.5.27` |  656.1MiB |     259.7MiB | 60.4% lower |
...
| Point              | Installed deps | Fresh install | Afora package | Nested `afora/node_modules` | Root shrinkwrap | Canvas install behavior                   |
| ------------------ | -------------: | ------------: | ---------------: | -----------------------------: | --------------- | ----------------------------------------- |
...
`2026.5.20` shipped with no root shrinkwrap and no large nested Afora
dependency tree. `2026.5.22` introduced root shrinkwrap and installed 911.8MB
under nested `afora/node_modules`. `2026.5.28` keeps shrinkwrap and still
installs 259.7MiB under nested `afora/node_modules`, but no longer installs
...
that made npm materialize a large nested Afora dependency tree and all 12
