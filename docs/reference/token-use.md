---
summary: "How Afora builds prompt context and reports token usage + costs"
read_when:
...
Afora tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Afora assembles its own system prompt on every run. It includes:
...
- Workspace + bootstrap files (`AGENTS.md`, `SOUL.md`,
  ...
  - Lowercase root `memory.md` is never injected. It is legacy repair input
    for `afora doctor --fix`, which migrates it into `MEMORY.md`.
...
Afora derives the live tool-result cap from the effective model context
...
publish a `1050000` token total window, but Afora defaults their active
...
For images, Afora downscales transcript/tool image payloads before
...
- `/usage off|tokens|full` -> appends a per-response usage footer to every
  ...
  - `/usage full` shows compact model/context/cost details; estimated cost
    appears only when Afora has usage metadata and local pricing for the
    active model. Custom `messages.usageTemplate` layouts can include
    token/cache fields.
- `/usage cost` -> local cost summary from Afora session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` and `/usage` are supported.
- **CLI:** `afora status --usage` and `afora channels list` show
...
Afora falls back to matching OAuth/API-key credentials from auth
...
Afora keeps provider usage accounting separate from the current context
...
Afora does not fetch OpenRouter or LiteLLM directly. Set
...
Provider prompt caching only applies within the cache TTL window. Afora
...
Afora sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus
...
Older configs can keep `context1m: true`, but Afora no longer sends
...
(`sk-ant-oat-*`), Afora preserves the OAuth-required Anthropic beta
