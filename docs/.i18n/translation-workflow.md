...
1. `AforaMosh/afora-agent` syncs English docs into `afora/docs`.
...
The default cooldown is controlled by the publish repo variable `AFORA_DOCS_TRANSLATION_COOLDOWN_SECONDS`, which defaults to `3600`. Repository dispatch callers may override it with `client_payload.cooldown_seconds`, and manual runs may set `cooldown_seconds`.

If `.afora-sync/source.json` changed during the wait, it waits again from the newer state. If `main` keeps moving, the wait is capped by `AFORA_DOCS_TRANSLATION_MAX_WAIT_SECONDS`, which defaults to the cooldown value. The newest observed state is translated after the cap.
...
`metadata.json` includes the locale, locale slug, source SHA, pending count, changed count, and any failure reason. The finalizer rejects artifacts whose `source_sha` does not match the current `.afora-sync/source.json`.
