...
- Docs are hosted on Mintlify (`https://docs.afora.ai`).
...
- JSON5/JSON config fences that look like whole `afora.json` documents are schema-validated in CI with `pnpm docs:check-config-examples`; deliberately partial or legacy snippets opt out with `validate=false` in the fence info string.
...
- `scripts/docs-sync-publish.mjs` excludes and prunes `docs/internal/**` from the public `afora/docs` publish repo if a page is force-added later.
...
`.github/workflows/maturity-scorecard.yml` renders artifact previews and can open generated-doc PRs; `.github/workflows/afora-release-checks.yml` dispatches it for release QA.
...
- Foreign-language docs are not maintained in this repo. The generated publish output lives in the separate `afora/docs` repo (often cloned locally as `../afora-docs`).
...
- Pipeline: update English docs here, update `docs/.i18n/glossary.<locale>.json` as needed, then let the publish-repo sync and `scripts/docs-i18n` run in `afora/docs`.
