...
read_when:
  ...
  - Debugging why afora is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Afora-managed)"
---

Afora can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls. It runs through a small local control service inside the Gateway (loopback only) and is isolated from your personal browser.

- Think of it as a **separate, agent-only browser**. The `afora` profile never touches your personal browser profile.
...
- A separate browser profile named **afora** (orange accent by default).
...
If `afora browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `afora browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `afora browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `afora`, `user`, `chrome`

- `afora`: managed, isolated browser (no extension required).
- `user`: built-in Chrome DevTools MCP attach profile for your **real
  ...
  prompt the first time Afora attaches, so someone must be at the computer.
- `chrome`: built-in [Chrome extension](/tools/chrome-extension) profile for
  ...
  desk because it drives tabs through the Afora browser extension instead of
  the remote-debugging port, so there is no "Allow remote debugging?" prompt.

For agent browser tool calls:

- Default: use the isolated `afora` browser.
...
Set `browser.defaultProfile: "afora"` if you want managed mode by default.
...
Session tab cleanup applies only to tabs created by the Afora browser tool
with `action: "open"`. Afora does not adopt tabs that were already open,
...
safely after restart. This durable path can cover Afora-managed profiles,
regular remote CDP profiles, and existing-session profiles with an explicit
`cdpUrl`, provided Afora can resolve both the native target and a stable
browser identity. Before closing a durable record, Afora verifies that the
...
but Afora does not automatically attach them to channel replies. To share a
...
understanding model is configured, Afora keeps the normal image result so the
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). `AFORA_GATEWAY_PORT` takes priority over `gateway.port`; either shifts the derived ports in the same family.
- Local `afora` profiles auto-assign `cdpPort`/`cdpUrl` from a range starting 9 ports above the control port (default `18800`-`18899`); set those only for
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, Afora pauses new launch
...
- Guarded remote CDP connections now fail closed when the selected driver cannot
  ...
  `afora` driver for Browserless, Browserbase, Notte, or other guarded
  ...
  default strict Browser policy because Chrome MCP cannot carry Afora's
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the Afora-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
- Afora-managed local CDP readiness probes and DevTools WebSocket connections bypass the managed network proxy for the exact launched loopback endpoint, so `afora browser start` still works when an operator proxy blocks loopback egress.
...
- `POST /start?headless=true` and `afora browser start --headless` request a
  ...
  remote CDP profiles reject the override because Afora does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  ...
  `afora browser --json status`; trailing `afora browser status --json`
...
- `AFORA_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `AFORA_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- For a running local managed profile, status and doctor query Chrome's
  ...
  `afora browser --json status`. A passive status call does not launch Chrome.
...
- Default profile is `afora` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
- `driver: "extension"` drives your signed-in Chrome through the [Afora Chrome extension](/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
Afora uses it automatically. Set `browser.executablePath` to override
...
Per-profile `executablePath` only affects local managed profiles that Afora
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Afora will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local Afora-managed browser profile.
- `headless` only affects local managed profiles that Afora launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `afora browser stop` stops the browser process that
  Afora launched
- attach-only and remote CDP profiles: `afora browser stop` closes the active
  ...
  though no browser process was launched by Afora
...
Afora preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Afora can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Afora treats it as a least-privilege boundary limiting which profile names the proxy will target.
...
CDP connection URLs over HTTPS and WebSocket. Afora can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Afora
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and Afora runs on the host, treat
...
Afora process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-Afora WebSocket base, such
...
an address Afora cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, Afora treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by Afora.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Afora accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  Afora calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. Afora connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). Afora tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise Afora
  ...
  accepts it, Afora falls back to that root as well. This lets a bare `ws://`
...
`afora browser doctor` uses the same discovery-first, WebSocket-fallback
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-afora-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Afora
...
- Configure `gateway.auth.token`, `gateway.auth.password`, `AFORA_GATEWAY_TOKEN`, or
  `AFORA_GATEWAY_PASSWORD` explicitly if you want a stable secret you control
...
Afora supports multiple named profiles (routing configs). Profiles can be:

- **afora-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `afora` profile is auto-created if missing.
...
Afora can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Afora attaches.
...
```bash
afora browser --browser-profile user start
afora browser --browser-profile user status
afora browser --browser-profile user tabs
afora browser --browser-profile user snapshot --format ai
```
...
- `afora doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `afora` profile because it can
  act inside your signed-in browser session.
- Afora does not launch the browser for this driver; it only attaches.
- Afora uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, Afora skips
...
Compared to the managed `afora` profile, existing-session drivers are more constrained:
...
- **Dialog visibility** - Managed browser action responses include `blockedByDialog` and `browserState.dialogs.pending` when an action opens a modal dialog; snapshots also include pending dialog state. Respond with `browser dialog --accept/--dismiss --dialog-id <id>` while a dialog is pending. Dialogs handled outside Afora appear under `browserState.dialogs.recent`.
...
When launching locally, Afora picks the first available:
...
control API** plus a matching `afora browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means Afora cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "afora" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by afora` when a
...
- For the local loopback `afora` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Afora's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (afora, chrome, or remote CDP).
