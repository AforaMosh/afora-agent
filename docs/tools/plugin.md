---
summary: "Install, configure, and manage Afora plugins"
read_when:
...
Plugins extend Afora with channels, model providers, agent harnesses, tools,
...
- an Afora checkout or installation with the `afora` CLI available
...
<Steps>
  <Step title="Find the plugin">
    ...
    they match an official plugin id. Raw `@afora/*` specs that match a
  ...
  <Step title="Install the plugin">
    ```bash
    # From ClawHub.
    afora plugins install clawhub:<package>

    # From npm.
    afora plugins install npm:<package>

    # From git.
    afora plugins install git:github.com/<owner>/<repo>@<ref>

    # From a local development checkout.
    afora plugins install ./my-plugin
    afora plugins install --link ./my-plugin
    ```

    Treat plugin installs like running code. Prefer pinned versions for
    reproducible production installs. ClawHub packages and Afora's
  ...
  <Step title="Configure and enable it">
    ...
    before the plugin can load. `afora plugins install` adds the installed
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want Afora-native discovery, scans, version metadata, and install hints | `afora plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `afora plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `afora plugins install git:github.com/<owner>/<repo>@<ref>` |
| local path  | You are developing or testing a plugin on the same machine                     | `afora plugins install --link ./my-plugin`                  |
| marketplace | You are installing a Claude-compatible marketplace plugin                      | `afora plugins install <plugin> --marketplace <source>`     |
...
bare spec installs through npm during the launch cutover. Raw `@afora/*`
specs that match bundled plugins also resolve to the bundled copy before npm
fallback. Use `npm:@afora/<plugin>@<version>` to deliberately install the
external npm package instead of the bundled copy. Use `clawhub:`, `npm:`,
`git:`, or `npm-pack:` for deterministic source selection. See
[`afora plugins`](/cli/plugins#install) for the full command contract.

For npm installs, unpinned specs and `@latest` choose the newest stable
package that advertises compatibility with this Afora build. If npm's
current latest release declares a newer `afora.compat.pluginApi` or
`afora.install.minHostVersion` than this build supports, Afora scans
...
warning. Plugin
`before_install` hooks run later, and only in Afora processes where plugin
...
snippet. Run [`afora plugins list --enabled --verbose`](/cli/plugins#list)
or [`afora plugins inspect <id>`](/cli/plugins#inspect) on the listed
plugin id before copying trusted plugins into `afora.json`. The same
...
`plugins.allow` or reinstall from a trusted source so Afora records install
provenance.

Run `afora doctor` or `afora doctor --fix` when config validation
reports stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin
paths.

## Understand plugin formats

Afora recognizes two plugin formats:

| Format                 | How it loads                                                                                | Use when                                                               |
| ---------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native Afora plugin | `afora.plugin.json` plus a runtime module loaded in process                              | You are installing or building Afora-specific runtime capabilities  |
| Compatible bundle      | Agent Plugins, Codex, Claude, or Cursor plugin layout mapped into Afora plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `afora plugins list`, `afora plugins inspect`,
`afora plugins enable`, and `afora plugins disable`. See
...
Plugin-managed internal hooks show up in `afora hooks list` with
`plugin:<id>`. You cannot enable or disable them through `afora hooks`;
...
`afora.plugin.json`, then enable the plugin and include it in
...
`afora plugins list` and plain `afora plugins inspect` read cold config,
...
sure any manual restart targets the actual `afora gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `afora plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `afora plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `afora doctor --fix` if it points to stale plugin state                                             | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `afora plugins registry --refresh`                    |
| `AFORA_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `afora plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |

When an enabled managed plugin fails payload verification during Gateway
startup, Afora quarantines that exact installed plugin root for the boot and
continues serving other plugins. `afora status --all`, `afora health`,
and `afora doctor` report it as `configured-unavailable`. Fix or reinstall
...
`afora doctor --fix` to remove stale plugin and channel entries. Unknown
...
plugin id. If both plugins are explicitly enabled, Afora keeps that request
...
Afora needs at runtime. Update or reinstall after the publisher ships
...
and validation follows with `plugin present but blocked`, Afora found
plugin files owned by a different Unix user than the process loading them.
Keep the plugin config in place; fix the filesystem ownership or run Afora
as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Afora config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Afora as root, repair the managed plugin root to
...
After fixing ownership, rerun `afora doctor --fix` or
`afora plugins registry --refresh` so the persisted plugin registry
...
```bash
afora config set logging.level trace
afora logs --follow
```
...
Afora caches successful plugin tool factory results for repeated
...
- [`afora plugins`](/cli/plugins) - full CLI reference
