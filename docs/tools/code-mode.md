...
if (!listed.complete) {
  return { requestedAddress, candidates: listed.conversations, incomplete: true };
}
...
address. A complete owner snapshot with one exact normalized address supplies
provenance evidence for one dependent mutation under the same tool policy as a
direct call. It does not authorize the mutation or prove the semantics of
natural-language intent. Duplicate exact addresses, incomplete snapshots, and
conflicting authority remain ambiguous, so return evidence without sending.
