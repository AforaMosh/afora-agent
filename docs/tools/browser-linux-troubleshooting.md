---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for Afora browser control on Linux"
...
```json
{ "error": "Error: Failed to start Chrome CDP on port 18800 for profile \"afora\"." }
```
...
Snap's AppArmor confinement interferes with how Afora spawns and monitors
the browser process.

Other common Linux launch failures:

- `The profile appears to be in use by another Chromium process`: stale
  `Singleton*` lock files in the managed profile directory. Afora removes
...
- `Missing X server or $DISPLAY`: a visible browser was explicitly requested
  ...
  If you set `AFORA_BROWSER_HEADLESS=0`, `browser.headless: false`, or
  `browser.profiles.<name>.headless: false`, remove that headed override, set
  `AFORA_BROWSER_HEADLESS=1`, start `Xvfb`, run
  `afora browser start --headless` for a one-shot managed launch, or run
  Afora in a real desktop session.
...
If you must keep snap Chromium, configure Afora to attach to a
...
chromium-browser --headless --no-sandbox --disable-gpu \
  ...
  --user-data-dir=$HOME/.afora/browser/afora/user-data \
  about:blank &
```
...
```ini
# ~/.config/systemd/user/afora-browser.service
[Unit]
Description=Afora Browser (Chrome CDP)
After=network.target

[Service]
ExecStart=/snap/bin/chromium --headless --no-sandbox --disable-gpu --remote-debugging-port=18800 --user-data-dir=%h/.afora/browser/afora/user-data about:blank
Restart=on-failure
RestartSec=5
...
```bash
systemctl --user enable --now afora-browser.service
```
...
1. Use the managed browser instead:
   `afora browser --browser-profile afora start` (or set
...
- Local `afora`-driver profiles auto-assign `cdpPort`/`cdpUrl`; only set
