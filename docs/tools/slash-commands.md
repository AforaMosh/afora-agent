...
the Afora command handler, and `/status` plus `/unfocus` stay local whenever
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `afora.json`). Owner-only.
</ParamField>

<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes Afora-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Discovery and status">
    ...
    | `/afora <request>` | Run the Afora setup and repair helper from an owner DM |
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | Read or write `afora.json`. Owner-only |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write Afora-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart Afora |
...
/config set channels.whatsapp.responsePrefix="[afora]"
...
`/mcp` stores config in Afora config, not embedded-agent project settings.
...
/plugins install npm:@afora/<official-package>
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` supports a single `/afora`-style command.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `Afora Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
