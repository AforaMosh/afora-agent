...
read_when:
  ...
  - Enabling the plugin-tools or Afora-tools MCP bridge
...
| `afora`   | Afora ACP bridge (native `afora acp`)                                                            |
...
When Afora uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Afora `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by Afora before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Afora/acpx cannot force a model selection.
...
Packaged installs use the official `@afora/acpx` runtime plugin for ACP.
...
```bash
afora plugins install ./path/to/local/acpx-plugin
```
...
signal. Set `AFORA_ACPX_RUNTIME_STARTUP_PROBE=0` or
`AFORA_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before Afora passes it through the current acpx command-string registry.
...
manually, and there is no separate postinstall step for Afora itself. If an
adapter download or spawn fails, `/acp doctor` reports the failure.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Afora plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Afora plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
afora config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `afora-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Afora
  plugins.
...
### Afora tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Afora tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
afora config set plugins.entries.acpx.config.aforaToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `afora-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Afora tools. The initial server exposes `cron`.
...
```bash
afora config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use Afora agent/run timeouts, including `/acp timeout`.
...
```bash
afora config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from Afora exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between Afora `tools.exec.mode`, Codex Guardian
...
```bash
afora config set plugins.entries.acpx.config.permissionMode approve-all
afora config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Afora defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `PermissionPromptUnavailableError: Permission prompt unavailable in non-interactive mode`.
