...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Afora
...
Today this applies to Afora embedded runs and Afora dynamic tools in the Codex
app-server harness. Tokenjuice hooks Afora's tool-result middleware and
...
```bash
afora plugins install clawhub:@afora/tokenjuice
```
...
```bash
afora config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
afora plugins disable tokenjuice
```
