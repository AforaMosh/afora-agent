---
summary: "Tool Search: compact large Afora tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want Afora agents to use a large tool catalog without adding every tool schema to the prompt
  - You want Afora tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for Afora runs
---

Tool Search is an experimental Afora agent runtime feature. It gives agents one
...
This page documents Afora Tool Search. It is not the Codex-native tool
...
For the generic Afora runtime that exposes a QuickJS-WASI `exec`/`wait`
surface instead of Tool Search controls, see [Code Mode](/tools/code-mode).

When enabled for Afora runs, the model automatically receives a bounded
...
JavaScript body in an isolated Node subprocess with an `afora.tools` bridge:

```js
const hits = await afora.tools.search("create a GitHub issue");
const tool = await afora.tools.describe(hits[0].id);
return await afora.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include catalog-eligible Afora tools, plugin tools, MCP
...
Afora. Direct-only tools remain model-visible and are not added to the
catalog.

Codex harness runs do not receive these experimental Afora Tool Search
controls. Afora passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the Afora embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible Afora and plugin tools.
...
7. Expose the Afora code bridge, the structured fallback tools, or the
   compact directory surface alongside those stable, directly callable tools.

At execution time every real tool call returns to Afora. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`afora.tools.call(...)` crosses the bridge back into the Gateway, where the
...
All modes use the same policy-filtered catalog and normal Afora execution
...
for the current run while Afora tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small Afora tool
...
catalog includes catalog-eligible Afora, MCP, and client tools after normal
...
and built from the already policy-filtered catalog. Afora reuses the
...
authorized catalog changes, Afora builds a new directory for the new
snapshot.

## API

`afora.tools.search(query, options?)`
...
Afora core or plugin tool may also include a compact `output` hint, such as
...
```js
const hits = await afora.tools.search("calendar event", { limit: 5 });
```

`afora.tools.describe(id)`
...
```js
const calendarCreate = await afora.tools.describe("mcp:calendar:create_event");
```

`afora.tools.call(id, args)`

Calls a selected tool through Afora and returns the raw `{ tool, result }`
envelope. JSON-returning tools normally place their value in
`result.details`. Afora validates a trusted core or plugin tool's declared
...
If a trusted tool also declares `outputSchema`, Afora compiles that schema
...
```js
await afora.tools.call(calendarCreate.id, {
  summary: "Planning",
...
hidden directory tool name directly, Afora resolves it from the authorized
catalog before normal execution.
Directory-mode client tool names must not collide with Afora, plugin, or MCP
...
network grants, and no child-process or worker grants. Afora enforces a
...
- `afora.tools.search`
- `afora.tools.describe`
- `afora.tools.call`

Normal Afora behavior still applies to final calls:
...
Enable Tool Search for Afora runs with the default code bridge:

```bash
afora config set tools.toolSearch true
```
...
Use the structured fallback tools instead for Afora runs:
...
Use the compact directory surface instead for Afora runs:
...
- `sources`: catalog entry counts split into `afora`, `mcp`, and `client`
...
operation. Afora does not record serialized tool or prompt byte counts. The
...
- whether the result came from Afora, MCP, or a client tool

## E2E validation

The QA Lab gateway scenario proves all three paths with the Afora runtime:

```bash
pnpm afora qa suite --provider-mode mock-openai --scenario tool-search-gateway-e2e
```
