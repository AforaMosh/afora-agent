---
summary: "Export redacted trajectory bundles for debugging an Afora agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Afora session
...
Trajectory capture is Afora's per-session flight recorder. It records a
...
Afora writes the bundle under the workspace:

```text
.afora/trajectory-exports/afora-trajectory-<session>-<timestamp>/
```
...
The name resolves inside `.afora/trajectory-exports/`. Absolute paths and
`~` paths are rejected.
...
bundle; do not use allow-all. In group chats, Afora sends the approval
...
`.afora/trajectory-exports`), `--store <path>` (session store override),
...
Trajectory capture is on by default for Afora agent runs.
...
| `metadata.json`       | Afora version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
```bash
export AFORA_TRAJECTORY=0
```

This disables runtime trajectory capture before starting Afora.
...
Afora flushes runtime trajectory rows during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`AFORA_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Afora:

```bash
export AFORA_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Afora logs an `afora-trajectory-flush` timeout and
...
`AFORA_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are for support and debugging, not public posting. Afora
...
- confirm Afora was started without `AFORA_TRAJECTORY=0`
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.afora/trajectory-exports/`
