...
The tool registers only when Afora can resolve a PDF-capable model for the agent. Resolution order:
...
Every fallback candidate is auth-checked before use, so a configured `provider/model` only counts if Afora can authenticate that provider for the agent. If no usable model resolves, the `pdf` tool is not exposed.
...
- If image rendering fails, Afora drops the images and continues with the extracted text.
- If the target model is text-only and extraction produced images, Afora drops the images and sends text only.
