...
frontmatter and a markdown body. Afora loads bundled skills plus any local
...
Afora loads from these sources, **highest precedence first**. When the same
...
Skill roots support grouped layouts. Afora discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an Afora
  skill root. Use `afora migrate plan codex` to inventory those skills, then
  `afora migrate codex` to copy them into your Afora workspace.
</Note>

## Node-hosted skills

A connected headless node can publish skills installed in its active Afora
skills directory (`~/.afora/skills` by default; profile environment overrides
...
When `AFORA_STATE_DIR` points somewhere other than the default
`~/.afora`, session skill indexes exclude home-scoped personal or
...
`afora.plugin.json` (paths relative to the plugin root). Plugin skills load
...
`metadata.afora.requires` in its frontmatter, same as any other skill.
...
Afora resolves explicit references from authorized senders on every channel
...
distinct skills; Afora returns a visible error instead of ignoring extra or
...
```bash
afora skills workshop list
afora skills workshop inspect <proposal-id>
afora skills workshop evaluate <proposal-id>
afora skills workshop apply <proposal-id>
```
...
| Install a local skill directory    | `afora skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `afora skills install @owner/<slug> --global`       |
| Update all workspace skills        | `afora skills update --all`                         |
| Update a shared managed skill      | `afora skills update @owner/<slug> --global`        |
...
<AccordionGroup>
  <Accordion title="Install details">
    `afora skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.afora/skills` directory, visible to all local agents unless agent
    ...
    `afora skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `afora skills verify @owner/<slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `afora.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.afora/skills` and personal `~/.agents/skills` may contain
...
<Note>
  Afora follows the [AgentSkills](https://agentskills.io) spec. Frontmatter
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Afora keeps the skill's instructions out of the agent's normal
...
Afora filters skills at load time using `metadata.afora` (JSON5 object
embedded in the frontmatter, see the parsing note above). A skill with no
`metadata.afora` block is always eligible unless explicitly disabled.
...
<ParamField path="requires.config" type="string[]">
  Each `afora.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.afora` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Afora lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `afora.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** Afora does not auto-install Homebrew or translate brew
    ...
    - **Go:** Afora requires Go 1.21 or newer for automatic skill installs.
      If `go` is missing and Homebrew is available, Afora installs Go via
      ...
      dependency always targets a dedicated Afora-managed bin directory
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.afora/tools/<skillKey>`).
  </Accordion>
...
<Note>
  ...
  `metadata.afora.skillKey`, use that key under `skills.entries` instead.
  Quote hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, Afora:

<Steps>
  <Step title="Reads skill metadata">
    Afora resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, Afora also materializes the same
...
Afora snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, Afora refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, Afora watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, Afora can treat macOS-only skills as eligible when
    ...
    answering bin probes, Afora clears its cached bin matches.
...
When skills are eligible, Afora injects a compact XML block into the system
...
(`skills.limits.maxSkillsPromptChars`), Afora first preserves as many skill
...
note pointing at `afora skills check` whenever compact formatting or list
