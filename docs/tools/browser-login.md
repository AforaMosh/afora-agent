...
When a site requires login, sign in manually in the host browser's `afora`
...
Afora controls a dedicated Chrome profile named `afora` (orange-tinted
UI), separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent uses its isolated `afora` browser.
...
Two ways to access the `afora` profile:
...
```bash
afora browser start
afora browser open https://x.com
```

For a non-default profile, put `--browser-profile <name>` before the
subcommand (default is `afora`):

```bash
afora browser --browser-profile <name> open https://x.com
```
...
```bash
afora browser --browser-profile afora open https://x.com
```
