...
- **Gateway host** -> `afora` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, Afora also tries to bind one concrete local file operand. If that file changes after approval but before execution, the run is denied instead of executing drifted content.
- File binding is best-effort, not a complete model of every interpreter/runtime loader path. If exactly one concrete local file cannot be identified, Afora refuses to mint an approval-backed run rather than pretend full coverage.
...
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `afora approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                           |
| `afora exec-policy show`                                      | Local-machine merged view.                                                                 |
| `afora exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals document in one step. |
...
`AFORA_STATE_DIR` is set, the database follows that state directory;
otherwise it uses the default Afora state directory:

```text
$AFORA_STATE_DIR/state/afora.sqlite#exec_approvals_config
# otherwise
...
`$AFORA_STATE_DIR/exec-approvals.sock`, or
`~/.afora/exec-approvals.sock` when the variable is unset.

State directories are independent trust scopes. When `AFORA_STATE_DIR`
points somewhere else, Afora never imports or archives approvals from the
...
`afora doctor --fix` once to import the active state directory's retired
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.afora/exec-approvals.sock",
...
| `auto`      | Use allowlist policy, run deterministic matches directly, and send approval misses through Afora's native auto reviewer before falling back to a human approval route. |
...
the command has an enforceable plan; otherwise Afora asks a human.
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  Presentation only: when enabled, Afora may attach parser-derived
...
requested exec policy in Afora config (`tools.exec.*`) **and**
...
`--permission-mode bypassPermissions` when Afora's effective exec
policy is YOLO. For Afora-managed Claude live sessions, Afora's
...
If you want a more conservative setup, tighten Afora exec policy back to
...
`afora approvals set --gateway` or
`afora approvals set --node <id|name|ip>`.
...
`afora exec-policy preset cautious`.

To set individual fields instead of a full preset, use `afora exec-policy set --host <auto|sandbox|gateway|node> --security <deny|allowlist|full> --ask <off|on-miss|always> --ask-fallback <deny|allowlist|full>` with any subset of those flags.
...
- `afora exec-policy` does not synchronize node approvals.
- `afora exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `afora approvals --node ...`.
...
specific argument shape. Afora uses ECMAScript (JavaScript) regular
...
instead of hand-editing the encoded value. If Afora cannot parse argv
...
companion app or `afora approvals set --node <id|name|ip>` with the native
policy shape to edit them; see [Approvals CLI](/cli/approvals).

CLI: `afora approvals` supports gateway or node editing - see
...
session after the node reports completion. Afora can also emit an
...
- For main-agent async approvals with an originating session, Afora
  ...
  repair.
- If there is no session or the session cannot be resumed, Afora can
