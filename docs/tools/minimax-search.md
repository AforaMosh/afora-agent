...
Afora supports MiniMax as a `web_search` provider through the MiniMax
...
Afora also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, Afora resolves
...
| `count`   | integer | 1-10, default 5 | Number of results to return. Afora trims the returned list to this size. |
