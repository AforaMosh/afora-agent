...
read_when:
  - Looking for an overview of Afora's media capabilities
...
Afora generates images, videos, and music, understands inbound media
...
Afora until the shared realtime-voice contract can represent it.
</Note>
...
For async tools, Afora submits the request to the provider, returns a task
...
Afora wakes the agent with the generated media paths so it can tell the
...
Afora sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    other native model types; Afora has no provider contract for those
  ...
  <Accordion title="xAI">
    ...
    not registered in Afora until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
