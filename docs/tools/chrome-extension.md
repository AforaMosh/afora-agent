...
read_when:
  ...
  - You are installing, pairing, disabling, or troubleshooting the Afora Chrome extension
...
The Afora Chrome extension lets the browser tool automate eligible tabs in
...
- Afora installed on the same machine as Chrome, or an Afora browser node
  on that machine
...
only when the registered host is a Windows executable; Afora does not install
...
exact official Chrome Web Store identity and for Afora's deterministic
development IDs. After pre-registration succeeds, add
[Afora from the Chrome Web Store](https://chromewebstore.google.com/detail/afora/kcdjddhmeafeomebliikmbpblkmkfoig).
...
stable Afora-owned directory. Use that unpacked copy only as a development
fallback:
...
Foundation Store ID. That identity never makes a recorded path Afora-owned.
...
host name are not overwritten unless they are verifiably Afora-owned.

Use a different bounded wait when needed:

```bash
afora browser extension install --wait-ms 60000
```
...
```bash
afora config set browser.defaultProfile chrome
```
...
Afora and local clients such as mcporter then use that profile relay port.
Keep `afora gateway run` or the managed Gateway service running. A separate
...
- **Selected tabs** uses the **Afora** tab group as the access-control
...
in flight. Agent-created tabs stay in the Afora group in either mode.
...
- **Use local Afora** clears the opt-out and retries the native host.
...
and disable automatic setup**, then **Use local Afora** to create the new
...
**Use local Afora** to reconnect. Until that explicit disconnect succeeds,
...
Remove only Afora-owned native-host manifests and launchers:
...
`afora browser extension path` is read-only. It prints the stable installed
...
For a laptop that has Chrome but does not run Afora or a browser node, pair
...
The native host is `ai.afora.browser_bootstrap`. Each
...
Afora-owned mode-`0700` directory. Manifests are mode `0600`; the launcher is
...
```bash
afora browser extension status --json
afora browser doctor --browser-profile chrome
afora doctor
```
...
- **Waiting for local Afora:** run `extension status`; install or repair the
  owned native host.
...
- **Relay unavailable:** confirm `afora gateway run` or the managed Gateway
...
`afora` and Chrome MCP `user` profiles.
