...
Afora supports Brave Search API as a `web_search` provider.
...
or gateway. Afora appends `/res/v1/web/search` or `/res/v1/llm/context` to
...
- Afora uses the Brave **Search** plan. If you have a legacy subscription (e.g. the original Free plan with 2,000 queries/month), it remains valid but does not include newer features like LLM Context or higher rate limits.
