---
summary: "Typed workflow runtime for Afora with resumable approval gates."
title: Lobster
read_when:
...
see [Task Flow](/automation/taskflow) (`afora tasks flow`); for the task
...
→ afora calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ afora calls gmail.send
...
Afora runs Lobster workflows **in-process** using the bundled
...
[Lobster repo](https://github.com/afora/lobster) and put `lobster` on
`PATH`.
...
gog.gmail.search --query 'newer_than:1d' \
  | afora.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
### Important limitation: embedded Lobster vs `afora.invoke`

The bundled Lobster plugin runs workflows **in-process** inside the gateway.
In that embedded mode, `afora.invoke` does **not** automatically inherit a
gateway URL/auth context for nested Afora CLI tool calls.

That means this pattern is **not currently reliable in the embedded runner**:

```lobster
afora.invoke --tool llm-task --action json --args-json '{ ... }'
```

Use the example below only when running the **standalone Lobster CLI** in an
environment where `afora.invoke` is already configured with the correct
...
- non-`afora.invoke` steps inside the Lobster pipeline until a supported
...
a bare envelope: Afora creates or resumes a durable flow record, applies the
...
- **No secrets** - Lobster doesn't manage OAuth; it calls Afora tools that
  do.
