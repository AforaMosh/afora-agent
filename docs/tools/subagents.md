...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, Afora best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - Afora hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, Afora first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, Afora falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
    ...
    - Blocked canonical results are retained for 7 days. Operators can retry or intentionally dismiss them from the Tasks page or with `afora tasks retry` / `afora tasks dismiss`; retry can duplicate a visible result after an ambiguous provider acknowledgement.
    - Delivery keeps the resolved requester route: thread-bound or conversation-bound completion routes win when available. If the completion origin only provides a channel, Afora fills the missing target/account from the requester session's resolved route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery still works.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - Afora hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.entries.*` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal Afora config agents from `agents_list`.
...
- **Run timeout:** pass `runTimeoutSeconds` to set a timeout for a specific native, ACP, or visible sub-agent run. When omitted, Afora uses `agents.defaults.subagents.runTimeoutSeconds` if configured; otherwise it falls back to `0` (no timeout). An explicit `0` disables the timeout for that run.
- **Process lifetime:** a detached Afora sub-agent has its own run lifecycle. A background task created inside an external CLI backend is different: it shares the parent CLI subprocess and stops if that parent reaches `agents.defaults.timeoutSeconds`.
...
When active children exist, Afora injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    Afora creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish Afora, Codex
...
and `agents_list` omits it. Run `afora doctor --fix` to clean stale
...
- If a nested requester subagent session is gone, Afora falls back to that session's requester when available.
...
target agent first. After that, Afora applies the sub-agent restriction
layer.
...
Afora warns when the delivery backlog reaches 25 and blocks new subagent
spawns at 50 until operators retry or dismiss enough retained deliveries. It
does not prune results to make room.

## Liveness and recovery

Afora does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, Afora persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`afora tasks maintenance --apply` to reconcile the task record, or
`afora doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
