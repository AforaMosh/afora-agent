...
```text
Use x_search to find posts mentioning Afora this week, then use code_execution to count them by day.
```
...
```json
{
  ...
  "message": "code_execution needs xAI credentials. Run `afora onboard --auth-choice xai-oauth` to sign in with Grok, run `afora onboard --auth-choice xai-api-key`, set `XAI_API_KEY` in the Gateway environment, or configure `plugins.entries.xai.config.webSearch.apiKey`.",
  "docs": "https://docs.afora.ai/tools/code-execution"
}
```
