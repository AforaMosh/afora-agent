...
Afora can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `afora configure --section web` enables the installed Firecrawl plugin automatically.
...
- The explicitly selected Firecrawl `web_fetch` fallback works without an API key. When configured, Afora sends `plugins.entries.firecrawl.config.webFetch.apiKey` or `FIRECRAWL_API_KEY` for higher limits.
- Choosing Firecrawl during onboarding or `afora configure --section web` enables the plugin and selects Firecrawl for `web_fetch` unless another fetch provider is already configured.
...
- Legacy `tools.web.fetch.firecrawl.*` and `tools.web.search.firecrawl.*` config is auto-migrated by `afora doctor --fix`.
...
Set `plugins.entries.firecrawl.config.webSearch.baseUrl`, `plugins.entries.firecrawl.config.webFetch.baseUrl`, or `FIRECRAWL_BASE_URL` when you run Firecrawl yourself. Afora accepts `http://` only for loopback, private-network, `.local`, `.internal`, or `.localhost` targets. Public custom hosts are rejected so Firecrawl API keys are not sent to arbitrary endpoints by accident.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Afora auto-detects the first ready web-fetch provider from available credentials. The official Firecrawl plugin provides that fallback.
