...
read_when:
  ...
  - Comparing Afora exec approvals with ACPX harness permissions
...
```bash
afora config set tools.exec.mode auto
afora approvals get
...
Set ACPX permissions separately from Afora exec approvals:
...
| Use Codex/Afora auto-review before humans  | `tools.exec.mode: "auto"`                                   |
...
Host exec uses the stricter result of Afora config and the host-local approvals file. ACPX harness permissions do not loosen host exec approvals, and host exec approvals do not loosen ACPX harness prompts.
