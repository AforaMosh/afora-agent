...
Afora runs one Gateway (the master) on a host and connects every client to it. The Gateway owns sessions, auth profiles, channels, and state; everything else is a client.
...
| Home desktop                      | Desktop; laptop connects remotely via the macOS app's remote mode (Settings → Connection → Afora runs) | Keeping the agent on hardware that stays powered on. Runbook: [macOS remote access](/platforms/mac/remote).                                       |
...
With the tunnel up, `afora health` and `afora status --deep` reach the remote Gateway via `ws://127.0.0.1:18789`. `afora gateway status`, `afora gateway health`, `afora gateway probe`, and `afora gateway call` can also target a forwarded URL via `--url`.
...
<Note>
Replace `18789` with your configured `gateway.port` (or `--port` / `AFORA_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env `AFORA_GATEWAY_URL` may use env credentials only (`AFORA_GATEWAY_TOKEN` / `AFORA_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `gateway.auth.token` -> `AFORA_GATEWAY_TOKEN` -> `gateway.remote.token` (remote fallback only when the local token is unset)
  - password: `gateway.auth.password` -> `AFORA_GATEWAY_PASSWORD` -> `gateway.remote.password` (remote fallback only when the local password is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `AFORA_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `AFORA_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
...
- Gateway env overrides use `AFORA_GATEWAY_*` only.
...
```bash
afora config set gateway.remote.token "<your-token>"
```

Use `gateway.remote.password` instead if the remote Gateway uses password auth. `AFORA_GATEWAY_TOKEN` is still valid as a shell-level override, but the durable remote-client setup is `gateway.remote.token` / `gateway.remote.password`.

#### Step 4: create the LaunchAgent

Save as `~/Library/LaunchAgents/ai.afora.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.afora.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.afora.ssh-tunnel.plist
```

The tunnel starts automatically at login, restarts on crash, and keeps the forwarded port live.

<Note>
If you have a leftover `com.afora.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
# Restart the tunnel
launchctl kickstart -k gui/$UID/ai.afora.ssh-tunnel

# Stop the tunnel
launchctl bootout gui/$UID/ai.afora.ssh-tunnel
```
