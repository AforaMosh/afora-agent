...
- `afora status` - local summary: gateway reachability/mode, update hint, linked channel auth age, sessions + recent activity.
- `afora status --all` - full local diagnosis (read-only, color, safe to paste for debugging).
- `afora status --deep` - asks the running gateway for a live probe (`health` with `probe:true`), including per-account channel probes when supported.
- `afora status --usage` - show model provider usage/quota snapshots.
- `afora health` - asks the running gateway for its health snapshot (WS-only; no direct channel sockets from the CLI).
- `afora health --verbose` (alias `--debug`) - forces a live health probe and prints gateway connection details.
- `afora health --json` - machine-readable health snapshot output.
- Send `/status` as a standalone chat command in any channel to get a status reply without invoking the agent.
- Logs: run `afora logs --follow` (or `afora --profile <profile> logs --follow`) and filter for `web-heartbeat`, `web-reconnect`, `web-auto-reply`, `web-inbound`.

For Discord and other chat providers, session rows are not socket liveness.
`afora sessions`, Gateway `sessions.list`, and the agent `sessions_list` tool
...
- Creds on disk: `ls -l ~/.afora/credentials/whatsapp/<accountId>/creds.json` (mtime should be recent).
- Session store: `ls -l ~/.afora/agents/<agentId>/agent/afora-agent.sqlite`. Count and recent recipients are surfaced via `status`.
- Relink flow: `afora channels logout && afora channels login --verbose` when status codes 409-515 or `loggedOut` appear in logs. The QR login flow auto-restarts once for status 515 after pairing.
...
- The same heartbeat drives the bounded stability recorder: `afora gateway stability` (or the `diagnostics.stability` Gateway RPC). Fatal Gateway exits, shutdown timeouts, and restart startup failures persist the latest snapshot under `~/.afora/logs/stability/`. Inspect the newest bundle with `afora gateway stability --bundle latest`.
- For bug reports, run `afora gateway diagnostics export` and attach the generated zip: a Markdown summary, the newest stability bundle, sanitized log metadata, sanitized Gateway status/health snapshots, and config shape. Chat text, webhook bodies, tool outputs, credentials, cookies, account/message identifiers, and secret values are omitted or redacted. See [Diagnostics Export](/gateway/diagnostics).
...
- When a channel cannot open its durable ingress queue, its start fails and the gateway records the account as unable to receive. `afora channels status` reports `Channel cannot admit inbound events; its durable ingress queue is unavailable. Outbound may still work.`
...
- `logged out` or status 409-515 -> relink with `afora channels logout` then `afora channels login`.
- Gateway unreachable -> start it: `afora gateway --port 18789` (use `--force` if the port is busy).
...
`afora health` asks the running gateway for its health snapshot (no direct channel
