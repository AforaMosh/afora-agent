...
Rolling-history tool-loop checks are **disabled by default**. The exact-retry guard after a confirmed no-op `write`, `edit`, or `apply_patch` is always active. The post-compaction guard remains active unless `enabled` is explicitly `false`. Set `enabled: true` to activate the rolling detectors too. Settings can be defined globally in `tools.loopDetection` and overridden per-agent at `agents.entries.*.tools.loopDetection`.
