...
Rolling-history tool-loop checks are **disabled by default**. Exact retries after a confirmed no-op `write`, `edit`, or `apply_patch`, plus the post-compaction guard, remain active unless `enabled` is explicitly `false`. Set `enabled: true` to activate the rolling detectors too. Settings can be defined globally in `tools.loopDetection` and overridden per-agent at `agents.entries.*.tools.loopDetection`.
