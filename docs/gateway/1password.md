---
summary: "Use the 1Password plugin, bundled skill, or official MCP with Afora"
read_when:
  - You want API keys out of afora.json and inside 1Password
...
Afora pairs with **1Password** in four independent ways:

- **Config secrets:** any [SecretRef](/gateway/secrets) field in `afora.json` can resolve through the `op` CLI at runtime, so API keys never live in the config file.
...
- **Browser sign-in:** the `claude-cli` backend can use Claude Code's Chrome integration with [1Password for Claude](https://support.1password.com/1password-claude/), letting the agent sign in to websites without the password ever reaching the model or Afora.
...
```bash
afora plugins enable onepassword
mkdir -p ~/.afora/credentials/onepassword
chmod 700 ~/.afora/credentials/onepassword
printf '%s' "$OP_SERVICE_ACCOUNT_TOKEN" > \
  ~/.afora/credentials/onepassword/service-account-token
chmod 600 ~/.afora/credentials/onepassword/service-account-token
unset OP_SERVICE_ACCOUNT_TOKEN
```

When `AFORA_STATE_DIR` is set, use that directory instead of
`~/.afora`. Then generate and apply a SecretRef plan:

```bash
afora onepassword secretref setup \
  --openai-id op://Automation/OpenAI/credential \
  --anthropic-id op://Automation/Anthropic/credential \
  --plan-out ./afora-1password-secrets-plan.json

afora onepassword secretref status
afora secrets apply --from ./afora-1password-secrets-plan.json --dry-run --allow-exec
afora secrets apply --from ./afora-1password-secrets-plan.json --allow-exec
...
registered Afora credential targets, bounds parallel `op read` calls, and
...
Afora bundles a `1password` skill that teaches agents to select an available
...
into a one-off command. When a credential is already wired to an Afora
SecretRef target, the owning Afora workflow should resolve it; the agent does
...
and the Afora plugin does not call it. If an MCP-managed Environment launches
Afora with variables already mounted, use Afora's `env` SecretRefs for
...
[1Password for Claude](https://support.1password.com/1password-claude/) lets Claude request a login while the 1Password browser extension fills the credential directly into the page over an encrypted channel. The secret never enters the model context, the transcript, or Afora. When Afora runs the [`claude-cli` backend](/gateway/cli-backends#claude-cli-specifics) with Claude Code's Chrome integration enabled, agent tasks can use that flow for websites that need a real signed-in session.
...
- A person at the gateway host: every credential use shows a 1Password prompt confirmed there (for example with Touch ID). Under a restrictive exec policy the browser tool calls themselves are also relayed to your channel as Afora approvals first.

Before wiring this into Afora, verify the pieces in an interactive session on the gateway host: run `claude --chrome`, confirm the extension connects, and check that the `claude-in-chrome` tools include the credential tools. If they do not appear there, they will not appear through Afora either.
...
- Never place secret values in `afora.json`, logs, or chat. Scope the service
  account to only the vaults and items Afora needs.
...
- Authentication fails: check the plugin token file, its contents, and the
  service account's vault permissions with `afora onepassword status`.
