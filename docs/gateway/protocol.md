...
<AccordionGroup>
  ...
  <Accordion title="Talk and TTS">
    ...
    - `talk.client.create` creates or resumes a client-owned realtime provider session using `webrtc` or `provider-websocket` while the gateway owns credentials, instructions, tool policy, and the returned `voiceSessionId`. Clients that advertise `browser-allocation-v1` receive a prepared `allocationId`: start the candidate transport, then commit that exact allocation before replacing active media. If the result also includes `terminal`, the provider ended during setup; do not start the returned transport, and acknowledge that exact allocation with `talk.client.close`.
    - `talk.client.commit` atomically promotes one prepared browser allocation; repeated commits for the same active allocation are idempotent. Its result is `committed` or `terminal`; on `terminal`, stop only the matching candidate transport, drain accepted transcripts, and close that exact allocation. `talk.client.abort` retires a definitively failed pre-commit candidate and returns `aborted` or `terminal`. Retry an ambiguous commit once with the same allocation id, but never abort when the commit result is unknown because the server may already own the replacement.
    ...
    - `talk.client.close` closes the exact committed or terminal allocation after pending transcript writes. Closing is idempotent and may deliver a mutation-only call digest to the session's last non-WebChat channel.
    ...
    - `talk.client.allocation.terminal` tells the owning capability-negotiated client that a prepared or committed provider allocation ended. Clients stop only the local transport whose `allocationId` matches the event, emit the corresponding terminal Talk event, drain accepted transcripts, then acknowledge that exact allocation with `talk.client.close`; a stale event must not stop a newer replacement, and a bounded server grace closes abandoned allocations.
    - Clients without `browser-allocation-v1` keep the shipped behavior: `talk.client.create` auto-commits the provider session and omits `allocationId`.
    - `talk.event` remains the canonical Talk event channel for realtime, transcription, STT/TTS, managed-room, telephony, and meeting adapters.
