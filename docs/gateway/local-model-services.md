---
summary: "Start local model servers on demand before Afora model and embedding requests"
read_when:
  - You want Afora to start a local model server only when its model or embedding provider is selected
...
`models.providers.<id>.localService` starts a provider-owned local model server on demand. When a model or embedding request selects that provider, Afora probes the health endpoint, starts the process if it is down, waits for readiness, then sends the request. Use it to avoid keeping expensive local servers running all day.

## How it works

1. A model or embedding request resolves to a configured provider.
2. If that provider has `localService`, Afora probes `healthUrl`.
3. On a successful probe, Afora uses the already-running server.
4. On a failed probe, Afora spawns `command` with `args`.
5. Afora polls the health endpoint until `readyTimeoutMs` expires.
6. The request goes through the normal model or embedding transport.
7. If Afora started the process and `idleStopMs` is set, it stops the process after the last in-flight request has been idle that long.

Afora does not install launchd, systemd, Docker, or any daemon for this. The server is a plain child process of whichever Afora process first needed it.
...
If another Afora process already has a healthy server at the same `healthUrl`, this process reuses it without adopting it (each process only manages the child it personally started). Startup and exit logs include bounded, redacted child-output tails plus timing and exit details; configured environment values are never emitted.
...
each Gateway host so Afora selects and verifies the matching platform build.
...
| `env`            | no       | Environment variables merged over the Afora process environment.                                                                  |
...
| `idleStopMs`     | no       | Idle shutdown delay for an Afora-started process. `0` or omitted keeps it alive until Afora exits.                             |
...
Replace `command` with the result of `which inferrs` on the machine running Afora. Full inferrs setup: [Inferrs](/providers/inferrs).
...
<CardGroup cols={2}>
  ...
  <Card title="Inferrs" href="/providers/inferrs" icon="cpu">
    Run Afora through the inferrs OpenAI-compatible local server.
  </Card>
</CardGroup>
