...
read_when:
  - Generating or reviewing `afora secrets apply` plans
  - Debugging `Invalid plan target path` errors
  - Understanding target type and path validation behavior
title: "Secrets apply plan contract"
---

This page defines the strict contract enforced by `afora secrets apply`. If a target does not match these rules, apply fails before mutating any file.

## Plan file requirements

`afora secrets apply --from <plan.json>` accepts regular files up to 16 MiB (16,777,216 bytes). The limit applies to the complete serialized file, including whitespace. Directories, FIFOs, device files, and files larger than the limit are rejected before JSON parsing or target validation.

`afora secrets configure --plan-out <plan.json>` enforces the same limit on the UTF-8 serialized output before creating the file. Hand-written plans and external plan generators must also keep the serialized file within this boundary.

## Plan file shape

`afora secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
`afora secrets configure` generates plans in this shape. You can also hand-write or edit one.
...
- `providerUpserts` -- an object keyed by provider alias. Each value is a provider definition (the same shape accepted under `secrets.providers.<alias>` in `afora.json`, e.g. an `exec` or `file` provider).
- `providerDeletes` -- an array of provider aliases to remove.

`providerUpserts` runs before `targets`, so a `target.ref.provider` may reference a provider alias that the same plan introduces in `providerUpserts`. Without this ordering, plans that reference an alias not yet configured in `afora.json` fail with `provider "<alias>" is not configured`.
...
If apply fails with an invalid target path message, regenerate the plan with `afora secrets configure` or fix the target path to a supported shape above.
