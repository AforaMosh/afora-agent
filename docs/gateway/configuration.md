...
<Tip>
**New to configuration?** Start with `afora onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    afora onboard       # full onboarding flow
    afora configure     # config wizard
    ```
  </Tab>
...
<Warning>
Afora only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`afora config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`afora doctor`, `afora logs`, `afora health`, `afora status`)
- Run `afora doctor` to see exact issues
- Run `afora doctor --fix` (`--repair` is the same flag; `--yes` skips prompts) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically - only `afora doctor --fix`
does. If `afora.json` fails validation (including plugin-local validation), Gateway
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push for public App Store builds uses the hosted Afora relay: `https://ios-push-relay.afora.ai`.
    ...
    ```bash
    afora config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `AFORA_APNS_RELAY_BASE_URL` and `AFORA_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    ...
    - `AFORA_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `AFORA_GATEWAY_TOKEN` or `gateway.auth.password` / `AFORA_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-afora-token`); query-string tokens are rejected.
  ...
  <Accordion title="Configure multi-agent routing">
    ...
    ```json5
    {
      agents: {
        entries: {
          home: { default: true, workspace: "~/.afora/workspace-home" },
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **Afora-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      Afora updates that included file and leaves `afora.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for Afora-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `afora.json`. To share a tree across machines or users, set
      `AFORA_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
invalid external edits without rewriting `afora.json`. Afora-owned config
...
config`, inspect the config, run `afora config validate`, then run `afora
...
The earlier `hot` and `restart` modes are retired; [`afora doctor --fix`](/cli/doctor) maps both to `hybrid`. Reload debounce is no longer configurable and runs behind a built-in default.
...
When you edit a source file that is referenced through `$include`, Afora plans
...
```bash
afora gateway call config.get --params '{}'  # capture payload.hash
afora gateway call config.patch --params '{
...
Afora reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.afora/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Afora runs your login shell and imports only the missing keys:
...
Env var equivalent: `AFORA_LOAD_SHELL_ENV=1`. Default `timeoutMs`: `15000`.
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${AFORA_GATEWAY_TOKEN}" } },
