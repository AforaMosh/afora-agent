---
summary: "SSH tunnel setup for Afora.app connecting to a remote gateway"
...
# Running Afora.app with a Remote Gateway

Afora.app reaches a remote Gateway over an SSH tunnel: an SSH `LocalForward` maps a local port to the Gateway WebSocket port on the remote host.
...
3. Set `gateway.remote.token` (or `gateway.remote.password`) via `afora config set gateway.remote.token "<your-token>"`.
4. Start the tunnel: `ssh -N remote-gateway &`.
5. Quit and reopen Afora.app.
...
Afora.app connects to `ws://127.0.0.1:18789` on the client. The tunnel forwards that connection to port 18789 on the remote host running the Gateway.
