---
summary: "Run multiple Afora Gateways on one host (isolation, ports, and profiles)"
read_when:
...
afora --profile rescue onboard
afora --profile rescue gateway install --port 19789
```
...
During `afora --profile rescue onboard`:
...
- Profile/config file
- State directory
- Workspace (default: `~/.afora/workspace-rescue`)
...
# extra gateway
afora --profile ops setup
afora --profile ops gateway --port 19789
```

Named profiles on both sides also work:

```bash
afora --profile main setup
afora --profile main gateway --port 18789
...
afora --profile ops gateway install --port 19789
```
...
| Setting                      | Purpose                              |
| ---------------------------- | ------------------------------------ |
| `AFORA_CONFIG_PATH`       | Per-instance config file             |
| `AFORA_STATE_DIR`         | Per-instance sessions, creds, caches |
...
`AFORA_ALLOW_MULTI_GATEWAY=1` skips the per-config singleton.

## Port mapping (derived)

Base port = `gateway.port` (or `AFORA_GATEWAY_PORT` / `--port`).
...
```bash
AFORA_CONFIG_PATH=~/.afora/main.json \
AFORA_STATE_DIR=~/.afora \
afora gateway --port 18789

AFORA_CONFIG_PATH=~/.afora/rescue.json \
AFORA_STATE_DIR=~/.afora-rescue \
afora gateway --port 19789
```
...
afora --profile rescue gateway status --deep
afora --profile rescue gateway probe
afora status
afora --profile rescue status
afora --profile rescue browser status
```

- `gateway status --deep` catches stale launchd/systemd/schtasks services from older installs.
- `gateway probe` warning text such as `multiple reachable gateway identities detected` is expected only when you intentionally run more than one isolated gateway, or when Afora cannot prove reachable probe targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
