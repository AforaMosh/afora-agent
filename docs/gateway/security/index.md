...
<Warning>
  **Personal assistant trust model.** This guidance assumes one trusted
  ...
  Afora is not a hostile multi-tenant security boundary for multiple
...
- If someone can modify Gateway host state/config (`~/.afora`, including `afora.json`), treat them as a trusted operator.
...
## `afora security audit`

Run this after any config change or before exposing network surfaces:

```bash
afora security audit
afora security audit --deep    # attempts a live Gateway probe
afora security audit --fix     # apply safe remediations
afora security audit --json
```
...
Most failures here are not exotic exploits - they are "someone messaged the bot and the bot did what they asked." Afora's stance, in order:
...
```bash
afora pairing list <channel>
afora pairing approve <channel> <code>
```
...
By default, Afora routes all DMs into the main session for cross-device continuity. If multiple people can DM the bot (open DMs or a multi-person allowlist), isolate DM sessions:
...
- "Paste the full contents of ~/.afora or your logs."
...
- `gateway` reads config with `config.schema.lookup` / `config.get`. It cannot write config, update Afora, or restart the Gateway.
- `cron` creates scheduled jobs that keep running after the original chat/task ends.

The `gateway` tool stays owner-only because config reads can expose secrets and host topology. Agents request persistent config or lifecycle changes through the `afora` delegation tool; Afora maps them to typed operations and requires human approval before applying them. See [Afora setup agent](/cli/afora#operations-and-approval).
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Afora cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
...
Afora can refresh the skills list mid-session: the skills watcher updates the snapshot on the next agent turn when `SKILL.md` changes, and connecting a macOS node can make macOS-only skills eligible (based on bin probing). Treat skill folders as trusted code and restrict who can modify them.
...
- Installing/updating plugins runs executable code:
  ...
  - ClawHub packages and Afora's bundled/official catalog are trusted sources. A new arbitrary npm, `npm-pack:`, git, local path/archive, or marketplace source warns before install; noninteractive installs require `--force` after you review and trust that source. `--force` confirms provenance and permits overwrite; it does not bypass `security.installPolicy` or remaining install safety checks. Updates reuse the already selected source.
  - Afora does not run built-in local dangerous-code blocking during install/update. Use `security.installPolicy` for operator-owned local allow/warn/block decisions and `afora security audit --deep` for diagnostic scanning.
  - npm and git plugin installs run package-manager dependency convergence only during the explicit install/update flow. Local paths and archives are treated as self-contained packages; Afora copies/references them without running `npm install`.
...
- `"none"` (default): tools see a sandbox workspace under `~/.afora/sandboxes`; agent workspace is off-limits.
...
- Keep filesystem roots narrow - avoid broad roots like your home directory for agent/sandbox workspaces, which can expose sensitive local files (for example state/config under `~/.afora`) to filesystem tools.
...
- Prefer a dedicated profile for the agent (the default `afora` profile); avoid your personal daily-driver profile.
...
- Chrome extension pairing stores its access mode in extension-owned Chrome
  ...
  Afora tab group as its ACL. Existing pairings migrate to **Selected tabs**,
...
- Native-host manifests, launchers, and status output contain no pairing key.
  Afora refuses symlinks, unsafe ownership/modes, wildcard origins, and
...
The Gateway multiplexes WebSocket + HTTP on one port (default `18789`; config/flags/env: `gateway.port`, `--port`, `AFORA_GATEWAY_PORT`). That HTTP surface includes the Control UI (SPA assets, default base path `/`) and the canvas host (`/__afora__/canvas` and `/__afora__/a2ui` - arbitrary HTML/JS; treat as untrusted content when loaded in a normal browser; do not expose it to untrusted networks/users or share an origin with privileged web surfaces).
...
When the bundled `bonjour` plugin is enabled, the Gateway broadcasts presence via mDNS (`_afora-gw._tcp`, port 5353) for local device discovery. Full mode includes TXT records that expose operational details: `cliPath` (filesystem path revealing username and install location), `sshPort` (advertises SSH availability), `displayName`/`lanHost` (hostname info). Broadcasting infrastructure details makes LAN reconnaissance easier.
...
- Or set `AFORA_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
`afora doctor --generate-gateway-token` can generate one for you.
...
- `"password"`: prefer setting via `AFORA_GATEWAY_PASSWORD`.
...
Rotation checklist (token/password): generate/set a new secret (`gateway.auth.token` or `AFORA_GATEWAY_PASSWORD`); restart the Gateway (or the macOS app if it supervises the Gateway); update remote clients (`gateway.remote.token`/`.password`); verify the old credentials no longer work.

### Tailscale Serve identity headers

When `gateway.auth.allowTailscale` is `true` (default for Serve), Afora accepts the Tailscale Serve identity header `tailscale-user-login` for Control UI/WebSocket authentication. It verifies identity by resolving the `x-forwarded-for` address through the local Tailscale daemon (`tailscale whois`) and matching it to the header. This only triggers on Afora's dedicated managed-Tailscale listener and requires `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`; headers on the ordinary Gateway listener do not establish Serve provenance. For this async check, failed attempts for the same `{scope, ip}` are serialized before the limiter records the failure, so concurrent bad retries from one Serve client can lock out the second attempt immediately.
...
rejected on Gateway-authenticated routes because Afora cannot attribute its
...
gateway:
  ...
  auth:
    mode: password
    password: ${AFORA_GATEWAY_PASSWORD}
```
...
- Afora's gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS there.
- If the gateway itself terminates HTTPS, `gateway.http.securityHeaders.strictTransportSecurity` emits the HSTS header from Afora responses.
...
- `gateway.controlUi.dangerouslyDisableDeviceAuth`: retired break-glass input, now fully inert. Control UI browsers pair through the normal device flow; `afora doctor --fix` removes the legacy key.
...
- Published package dependency lock: source checkouts use `pnpm-lock.yaml`; the published `afora` npm package and Afora-owned npm plugin packages include `npm-shrinkwrap.json` so installs use the reviewed transitive dependency graph from the release instead of resolving a fresh graph at install time. This is a supply-chain hardening and release reproducibility boundary, not a sandbox - see [npm shrinkwrap](/gateway/security/dependency-locking).
- Secure file operations: Afora uses `@afora/fs-safe` for root-bounded file access, atomic writes, archive extraction, temp workspaces, and secret-file helpers. Afora ships no native fs-safe binding: `AFORA_FS_SAFE_NATIVE_MODE` defaults to `off`, and `require` fails closed. Details: [Secure file operations](/gateway/security/secure-file-operations).
...
Assume anything under `~/.afora/` (or `$AFORA_STATE_DIR/`) may contain secrets or private data:

| Path                                        | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `afora.json`                                | Config may include tokens (gateway, remote gateway), provider settings, and allowlists.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `credentials/**`                            | Channel credentials (for example WhatsApp creds), pairing allowlists, legacy OAuth imports.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `state/afora.sqlite`                        | Shared runtime state, including native MCP OAuth access/refresh tokens, dynamic client registration secrets, and discovery state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `agents/<agentId>/agent/afora-agent.sqlite` | Per-agent runtime state, including model auth profiles.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `$CODEX_HOME/**` or `~/.codex/**`           | Native Codex runtime state. The ordinary harness accesses it only with explicit `plugins.entries.codex.config.appServer.homeScope: "user"`. The separate supervision connection accesses it when its resolved home scope is `"user"`, which is the default for stdio or Unix when unset. Contains the native Codex account, config, plugins, and thread store. Supervision lists source metadata and keeps a continued Chat's canonical native branch and later turns on that connection; branching copies bounded persisted user and assistant history into an authenticated, model-locked Afora Chat. Enable only for an owner-controlled Gateway. See [Codex harness](/plugins/codex-harness#share-threads-with-codex-desktop-and-cli) and [Codex supervision](/plugins/codex-supervision). |
| `secrets.json` (optional)                   | File-backed secret payload used by `file` SecretRef providers (`secrets.providers`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `agents/<agentId>/agent/auth.json`          | Legacy compatibility file; static `api_key` entries are scrubbed when discovered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `agents/<agentId>/agent/afora-agent.sqlite` | Per-agent runtime state, including session rows and transcripts that can contain private messages and tool output.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- WhatsApp: `~/.afora/credentials/whatsapp/<accountId>/creds.json`
- Telegram bot token: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- Discord bot token: config/env or SecretRef (env/file/exec/store providers)
- Slack tokens: config/env (`channels.slack.*`)
- Pairing allowlists: `~/.afora/credentials/<channel>-allowFrom.json` (default account) / `<channel>-<accountId>-allowFrom.json` (non-default accounts)
- Model auth profiles: `~/.afora/agents/<agentId>/agent/afora-agent.sqlite` (`auth_profile_store`)
- MCP OAuth sessions: `~/.afora/state/afora.sqlite` (`mcp_oauth_stores`)
- Legacy OAuth import: `~/.afora/credentials/oauth.json`
...
Afora loads workspace-local `.env` files for agents and tools, but never lets them silently override gateway runtime controls:

- Provider credential environment variables are blocked from untrusted workspace `.env` files - for example `GEMINI_API_KEY`, `GOOGLE_API_KEY`, `XAI_API_KEY`, `MISTRAL_API_KEY`, `GROQ_API_KEY`, `DEEPSEEK_API_KEY`, `PERPLEXITY_API_KEY`, `BRAVE_API_KEY`, `TAVILY_API_KEY`, `EXA_API_KEY`, `FIRECRAWL_API_KEY`, and provider auth keys declared by installed trusted plugins. Put provider credentials in the Gateway process environment, `~/.afora/.env` (`$AFORA_STATE_DIR/.env`), the config `env` block, or an optional login-shell import instead.
- Any key starting with `AFORA_` is blocked from untrusted workspace `.env` files, reserving the whole runtime namespace so a future `AFORA_*` control is fail-closed by default rather than silently inheritable from checked-in or attacker-supplied `.env` content.
...
Afora stores session transcripts on disk under `~/.afora/agents/<agentId>/sessions/*.jsonl` for session continuity and optional memory indexing - any process/user with filesystem access can read them. Treat disk access as the trust boundary and lock down `~/.afora` permissions; run agents under separate OS users or hosts for stronger isolation.
...
- When sharing diagnostics, prefer `afora status --all` (pasteable, secrets redacted) over raw logs.
...
1. Stop it: stop the macOS app (if it supervises the Gateway) or terminate your `afora gateway` process.
...
1. Rotate Gateway auth (`gateway.auth.token` / `AFORA_GATEWAY_PASSWORD`) and restart.
...
- Timestamp, gateway host OS + Afora version.
...
Found a vulnerability in Afora? Report responsibly:

1. Email: [security@afora.ai](mailto:security@afora.ai)
