...
afora gateway --port 18789 --verbose
...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and a `Capability` line that matches what you expect. Use `afora gateway status --require-rpc` for read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `AFORA_CONFIG_PATH` when set). Default mode is `gateway.reload.mode="hybrid"`. After the first successful load, the running process serves the active in-memory config snapshot; a successful reload swaps that snapshot atomically.
</Note>
...
- Auth is required by default. Shared-secret setups use `gateway.auth.token` / `gateway.auth.password` (or `AFORA_GATEWAY_TOKEN` / `AFORA_GATEWAY_PASSWORD`), and non-loopback reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Afora's highest-leverage compatibility surface:
...
`/v1/models` is agent-first: it returns `afora`, `afora/default`, and `afora/<agentId>` for every configured agent. `afora/default` is the stable alias that always maps to the configured default agent. Send `x-afora-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | -------------------------------------------------------------------- |
| Gateway port | `--port` → `AFORA_GATEWAY_PORT` → `gateway.port` → `18789`        |
| Bind mode    | CLI/override → `gateway.bind` → `loopback` (or `auto` in containers) |

Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `afora doctor --fix` or `afora gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
The earlier `hot` and `restart` modes are retired; [`afora doctor --fix`](/cli/doctor) maps both to `hybrid`.

## Operator command set

```bash
afora gateway status
afora gateway status --deep   # adds a system-level service scan
afora gateway status --json
afora gateway install
afora gateway restart
afora gateway stop
afora secrets reload
afora logs --follow
afora doctor
```
...
- `gateway probe` can warn about `multiple reachable gateway identities` when distinct gateways answer, or when Afora cannot prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
- If that is intentional, isolate ports, config/state, and workspace roots per gateway.

Checklist per instance:

- Unique `gateway.port`
- Unique `AFORA_CONFIG_PATH`
- Unique `AFORA_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
AFORA_CONFIG_PATH=~/.afora/a.json AFORA_STATE_DIR=~/.afora-a afora gateway --port 19001
AFORA_CONFIG_PATH=~/.afora/b.json AFORA_STATE_DIR=~/.afora-b afora gateway --port 19002
```
...
Use `afora gateway restart` for restarts. Do not chain `afora gateway stop` and `afora gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default. This removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `afora gateway stop --disable`.

LaunchAgent labels are `ai.afora.gateway` (default) or `ai.afora.<profile>` (named profile). `afora doctor` audits and repairs service config drift.

### Existing system LaunchDaemons

Afora installs and manages a per-user LaunchAgent. It does not install or manage system LaunchDaemons. If a custom LaunchDaemon already uses the same gateway label, Afora refuses to write, start, restart, or repair a user LaunchAgent because two `KeepAlive` managers can repeatedly restart the same gateway.

The ownership check reads `launchctl print system/<label>` and also checks installed plists under `/Library/LaunchDaemons`. It fails closed when system ownership cannot be verified, and `--force` does not bypass it. `afora gateway status` reports a loaded same-label system job; add `--deep` to scan installed system service files.

Choose one lifecycle owner before retrying:

- To keep the custom system LaunchDaemon, remove any competing user LaunchAgent and set `AFORA_SERVICE_REPAIR_POLICY=external` when running Doctor so it remains diagnostic-only for service lifecycle.
- To return to the supported user LaunchAgent, unload the system job with `sudo launchctl bootout system/<label>`, remove or relocate its actual plist, sign in to the macOS desktop as the target user, then run `afora gateway install`.

For the default profile, `<label>` is `ai.afora.gateway`. Named profiles use `ai.afora.<profile>`.
...
```bash
afora gateway install
systemctl --user enable --now afora-gateway[-<profile>].service
afora gateway status
```
...
[Service]
ExecStart=/usr/local/bin/afora gateway --port 18789
Restart=always
RestartSec=5
...
Native Windows managed startup uses a Scheduled Task named `Afora Gateway`
(or `Afora Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Afora falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now afora-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/afora-gateway[-<profile>].service` and adjust
`ExecStart=` if your `afora` binary lives elsewhere.

Do not also let `afora doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level Afora gateway service; use `AFORA_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
...
```bash
afora --dev setup
afora --dev gateway --allow-unconfigured
afora --dev status
```
