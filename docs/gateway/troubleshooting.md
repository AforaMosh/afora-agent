...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `afora onboard --mode local` / `afora setup` to restamp the expected local-mode config. If you are running Afora via Podman, the default config path is `~/.afora/afora.json`.
