---
summary: "Supervise the Afora Gateway as a child process from Electron or another host app"
read_when:
  - Embedding Afora in a desktop or server application
...
title: "Embedding Afora"
---

An embedding host should supervise the installed `afora` executable, use the
...
and upgrades explicit without depending on Afora's private state layout.
...
const packageEntry = fileURLToPath(import.meta.resolve("afora"));
const aforaEntry = resolve(dirname(packageEntry), "..", "afora.mjs");
const gateway = spawn(hostNodeExecutable, [aforaEntry, "gateway", "--allow-unconfigured"], {
  env: {
    ...process.env,
    AFORA_DISABLE_BONJOUR: "1",
    AFORA_EXEC_SHELL_SNAPSHOT: "0",
    AFORA_NO_RESPAWN: "1",
...
Resolve Afora through the installed package as shown; do not assume that a
project-local `afora` binary is on the host process's `PATH`. The example
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AFORA_DISABLE_BONJOUR=1`     | Disables Gateway-owned LAN multicast advertising when the host owns discovery.                                                                                                             |
| `AFORA_NO_RESPAWN=1`          | In an unmanaged embedding child, prevents Afora from handing an update restart to a detached child. Routine restarts remain in process, so the host keeps ownership of the tracked PID. |
| `AFORA_EXEC_SHELL_SNAPSHOT=0` | Disables login-shell snapshot capture for host exec commands.                                                                                                                              |
| `AFORA_SKIP_CHANNELS=1`       | Skips channel startup and reload. Set it only when the embedding app wants a control-plane or WebChat-only Gateway.                                                                        |
...
`AFORA_EXEC_SHELL_SNAPSHOT=0` in the Gateway child's environment, not only in
...
1. Run `afora doctor --fix --yes --non-interactive` against the same config and
...
`@afora/gateway-protocol/startup-unavailable` or the reference client's built-in
...
Keep the Gateway as the only owner of Afora state. Common embedding operations
...
under `~/.afora` to implement app features. Those layouts are private runtime
implementation details and can move or change without protocol compatibility.

## Install; do not flatten

The root `afora` package is not a single-file vendoring target. Bundled runtime
files under `dist/extensions` retain bare self-imports such as
`afora/plugin-sdk/*`, while the npm package intentionally excludes
per-extension `node_modules` trees.

Install Afora through npm, pnpm, or another normal Node package installation so
Node can resolve the package exports and root dependency tree. Spawn the installed
`afora` executable. Do not copy only `dist`, flatten the package into an app
bundle, or vendor selected extension files.

## Related

- [Building a Gateway client](https://docs.afora.ai/gateway/clients)
- [Gateway protocol](https://docs.afora.ai/gateway/protocol)
- [Gateway CLI](https://docs.afora.ai/cli/gateway)
- [Gateway integrations for external apps](https://docs.afora.ai/gateway/external-apps)
