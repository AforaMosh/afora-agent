...
- **Settings -> Afora** opens a dedicated Gateway settings assistant when the operator connection has `operator.admin` and the Gateway supports `afora.chat`. Its setup conversation stays separate from ordinary Chat, redacts secret replies locally, and moves to Chat only after you tap **Open Chat**.
...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `afora.internal.`), **or**
...
```bash
afora gateway --port 18789 --tailscale serve
```
...
Gateway has not been configured yet, run `afora onboard` first so setup-code
...
4. The official app connects automatically. If **Pending approval** shows a
   ...
   **Full** or **Limited** access. Plaintext LAN `ws://` setup is automatically
   ...
   Tailscale Serve, scan a new full-access code from Control UI or `afora qr`,
...
If the app retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Run `afora devices list` again before approval.
...
```bash
afora nodes status
afora gateway call node.list --params "{}"
```
...
Apple's Watch app, install Afora from **Watch app -> My Watch -> Available
Apps**, then open Afora once on both devices.
...
Afora is active, even when the paired iPhone is unavailable.
...
- Afora is active on the watch. Apple does not allow ordinary watchOS apps to
...
3. Open Afora on the watch before the short-lived setup code expires.
4. Verify the separate Apple Watch row with `afora nodes status`.
...
- `AFORA_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway (`gateway.push.apns.relay.baseUrl` is the config-first path).
- The App Store release build's push mode hardcodes the hosted relay host and never reads a relay-URL override — the `AFORA_PUSH_RELAY_BASE_URL` build-time env var only affects local/sandbox iOS build modes.

## Authentication and trust flow

The relay exists to enforce two constraints direct APNs-on-gateway cannot provide for official iOS builds:

- Only genuine Afora iOS builds distributed through Apple can use the hosted relay.
...
Why this design was created: to keep production APNs credentials out of user gateways, avoid storing raw official-build APNs tokens on the gateway, allow hosted relay usage only for official Afora iOS builds, and prevent one gateway from sending wake pushes to iOS devices owned by a different gateway.
...
```bash
export AFORA_APNS_TEAM_ID="TEAMID"
export AFORA_APNS_KEY_ID="KEYID"
export AFORA_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
Recommended gateway-host storage, consistent with other provider credentials under `~/.afora/credentials/`:

```bash
mkdir -p ~/.afora/credentials/apns
chmod 700 ~/.afora/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.afora/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.afora/credentials/apns/AuthKey_KEYID.p8
export AFORA_APNS_PRIVATE_KEY_PATH="$HOME/.afora/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_afora-gw._tcp` on `local.` and, when configured, the same wide-area DNS-SD discovery domain. Same-LAN gateways appear automatically from `local.`; cross-network discovery can use the configured wide-area domain without changing the beacon type.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `afora.internal.`) and Tailscale split DNS. See [Bonjour](/gateway/bonjour) for the CoreDNS example.
...
```bash
afora nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__afora__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__afora__/canvas/` and `/__afora__/a2ui/`, from the Gateway HTTP server (same port as `gateway.port`, default `18789`).
...
The iOS app is a mobile node surface, not a Codex Computer Use backend. Codex Computer Use and `cua-driver mcp` control a local macOS desktop through MCP tools; the iOS app exposes iPhone capabilities through Afora node commands such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Afora by invoking node commands, but those calls go through the gateway node protocol and follow iOS foreground/background limits. Use [Codex Computer Use](/plugins/codex-computer-use) for local desktop control and this page for iOS node capabilities.

### Canvas eval / snapshot

```bash
afora nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__afora; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
afora nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `afora devices list` and approve manually.
- Watch shows no iPhone state: confirm the iPhone reports `watchPaired: true`
  ...
  from **My Watch -> Available Apps**. After either change, open Afora on the
