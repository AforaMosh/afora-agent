...
Afora core is written in TypeScript. **Node is the required runtime** because
...
- Service status: `afora gateway status`

## Gateway service install (CLI)

Use one of these (all supported):

- Wizard (recommended): `afora onboard --install-daemon`
- Direct: `afora gateway install`
- Configure flow: `afora configure` → select **Gateway service**
- Repair/migrate: `afora doctor` (offers to install or fix the service)

The service target depends on OS:

- macOS: LaunchAgent (`ai.afora.gateway`, or `ai.afora.<profile>` for a named profile)
- Linux/WSL2: systemd user service (`afora-gateway[-<profile>].service`)
- Native Windows: Scheduled Task (`Afora Gateway` or `Afora Gateway (<profile>)`), with a per-user Startup-folder login item fallback if task creation is denied
