---
summary: "macOS app flow for controlling a remote Afora gateway"
read_when:
  - Setting up or debugging remote mac control
title: "Remote control"
---

This flow lets the macOS app act as a full remote control for an Afora gateway running on another host (desktop/server). The app connects directly to trusted LAN/Tailnet gateway URLs, or manages an SSH tunnel when the remote gateway is loopback-only. Health checks, Voice Wake forwarding, and Web Chat reuse the same remote configuration from _Settings -> General_.

## Modes

- **Local (this Mac)**: everything runs on the laptop; no SSH involved.
- **Remote over SSH (default)**: Afora commands run on the remote host. The app opens an SSH connection with `-o BatchMode`, your chosen identity/key, and a local port-forward.
...
SSH host-key verification is strict by default because gateway credentials travel through this tunnel. To opt into a managed SSH alias's own trust behavior, set `--ssh-host-key-policy openssh` via `afora-mac configure-remote`, or set `gateway.remote.sshHostKeyPolicy` to `"openssh"` directly. Review the alias and any matching `Host *` or system configuration before opting in. Changing the SSH target (in the app or via `configure-remote`) resets the policy back to `strict` unless you explicitly opt in again for the new target.
...
Browser automation in remote mode is owned by the CLI node host, not the native macOS app node. The app starts the installed node host service when possible; to enable browser control from that Mac, install/start it with `afora node install ...` and `afora node start` (or run `afora node run ...` in the foreground), then target that browser-capable node.

## Prereqs on the remote host

1. Install Node + pnpm and build/install the Afora CLI (`pnpm install && pnpm build && pnpm link --global`).
2. Ensure `afora` is on PATH for non-interactive shells (symlink into `/usr/local/bin` or `/opt/homebrew/bin` if needed).
...
`afora-mac connect`, `wizard`, and `configure-remote` resolve the active config in this order: `AFORA_CONFIG_PATH`, then `$AFORA_STATE_DIR/afora.json`, then `~/.AforaMosh/afora-agent.json`. Both configuration forms write that active file, mark onboarding complete, and let the app own the selected transport on next start. `--local-port`/`--remote-port` default to `18789`. Other flags: `--password`, `--identity <path>`, `--ssh-host-key-policy <strict|openssh>`, `--project-root <path>`, `--cli-path <path>`, `--json`. Run `afora-mac configure-remote --help` for the full reference.

To configure from the UI instead:

1. Open _Settings -> General_.
2. Under **Afora runs**, pick **Remote** and set:
   ...
   - **CLI path** (advanced): optional path to a runnable `afora` entrypoint/binary (auto-filled when advertised).
3. Hit **Test remote**. Success means the remote `afora status --json` ran correctly. Failures usually mean PATH/CLI issues; exit 127 means the CLI was not found remotely.
...
- Run `afora channels login --channel whatsapp --verbose` **on the remote host**. Scan the QR with WhatsApp on your phone.
...
| Symptom                                          | Cause / fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `exit 127` / not found                           | `afora` is not on PATH for non-login shells. Add it to `/etc/paths`, your shell rc, or symlink into `/usr/local/bin`/`/opt/homebrew/bin`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Health probe failed                              | Check SSH reachability, PATH, and that Baileys (WhatsApp) is logged in (`afora status --json`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
Pick sounds per notification from scripts with `afora nodes notify`, for example:

```bash
afora nodes notify --node <id> --title "Ping" --body "Remote gateway ready" --sound Glass
```
