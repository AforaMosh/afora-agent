...
Voice subsystem is `ai.afora`; each component logs under its own category:
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "ai.afora" AND category CONTAINS "voicewake"' --level info --style compact
  ```
