...
read_when:
  - Hosting PeekabooBridge in Afora.app
...
Afora can host **PeekabooBridge** as a local, permission-aware UI automation broker (`PeekabooBridgeHostCoordinator`, backed by the `steipete/Peekaboo` Swift package). This lets the `peekaboo` CLI drive UI automation while reusing the macOS app's TCC permissions.

## What this is (and is not)

- **Host**: Afora.app can act as a PeekabooBridge host.
- **Client**: the `peekaboo` CLI (there is no separate `afora ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app; Afora is a thin broker host.

## Relationship to other desktop-control paths

Afora has four desktop-control paths that intentionally stay separate:

- **PeekabooBridge host**: Afora.app hosts the local PeekabooBridge socket. The `peekaboo` CLI is the client and uses Afora.app's macOS permissions for screenshots, clicks, menus, dialogs, Dock actions, and window management.
...
- **Codex Computer Use**: the bundled `codex` plugin checks and can install Codex's `computer-use` MCP plugin (`extensions/codex/src/app-server/computer-use.ts`), then lets Codex own native desktop-control tool calls during Codex-mode turns. Afora does not proxy those actions through PeekabooBridge.
- **Direct `cua-driver` MCP**: Afora can register TryCua's upstream `cua-driver mcp` server as a normal MCP server, giving agents the CUA driver's own schemas and pid/window/element-index workflow without routing through the Codex marketplace or the PeekabooBridge socket.

Use Peekaboo for the broad macOS automation surface via Afora.app's permission-aware bridge host. Use agent-driven computer use when the gateway agent should see and control the desktop through a uniform `computer.act` node command that any vision model can drive. Use Codex Computer Use when a Codex-mode agent should rely on Codex's native plugin. Use direct `cua-driver mcp` to expose the CUA driver to any Afora-managed runtime as a normal MCP server.
...
When enabled (and Computer Control is on), Afora starts a local UNIX socket server at `~/Library/Application Support/Afora/<socket-name>`. If disabled, the host stops and `peekaboo` falls back to other available hosts. The coordinator also maintains legacy socket symlinks (`clawdbot`, `clawdis`, `moltbot` under Application Support) pointing at the current socket for older `peekaboo` installs.

For a one-off unattended run, `--attach-only --background-only` suppresses automatic windows and GUI-owned Keychain
loading. The persistent elevation host is a managed-deployment path for Afora Foundation release operators. Its
`package` command requires the Foundation signing identity and notarization credentials; Afora does not currently
...
export PREFIX="Afora-<full-afora-sha>-Peekaboo-<full-peekaboo-sha>-stable"
...
./"$PREFIX-installer.sh" migration-plan \
  --migrate-launch-agent "$HOME/Library/LaunchAgents/ai.afora.node.plist"
...
not need an Afora source checkout. The authorized operator handoff must independently provide the installer and
...
Gateway. It recognizes the canonical CLI-managed `ai.afora.node` job and app-backed background LaunchAgents. If the
...
still incomplete, but it commits only after the exact paired node identity reconnects as `afora-macos/node` with the
...
uses the separate `ai.afora.mac.elevation-host` job and refuses to race or rewrite ordinary **Launch at login**
(`ai.afora.mac`).
...
The elevation archive is Foundation-signed, notarized, stapled, named by the full Afora and Peekaboo source
commits, and contains exactly `Afora.app`. Its receipt binds the archive and portable-installer names and digests,
Afora and Peekaboo source revisions, signer, per-architecture CDHashes, architectures, entitlement digests, and Apple notarization
...
- The bridge validates **caller code signatures**. The production Afora host accepts only the exact Peekaboo CLI
...
- If no hosts are found, open one of the host apps (Peekaboo.app or Afora.app) and confirm permissions are granted.
