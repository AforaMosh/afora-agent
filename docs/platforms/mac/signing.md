...
[`scripts/package-mac-app.sh`](https://github.com/AforaMosh/afora-agent/blob/main/scripts/package-mac-app.sh) builds and packages the app to a fixed path (`dist/Afora.app`), then calls [`scripts/codesign-mac-app.sh`](https://github.com/AforaMosh/afora-agent/blob/main/scripts/codesign-mac-app.sh) to sign it. TCC permissions are tied to the bundle ID and code signature; keeping both stable (and the app at a fixed path) across rebuilds keeps macOS from forgetting TCC grants (notifications, accessibility, screen recording, mic, speech).

- Debug bundle identifier defaults to `ai.afora.mac.debug` (override with `BUNDLE_ID=...`).
...
- Stamps Info.plist with `AforaBuildTimestamp` (ISO8601 UTC) and `AforaGitCommit` (short hash, `unknown` if unavailable) so the About tab can show build, git, and debug/release channel.
...
The About tab reads `AforaBuildTimestamp` and `AforaGitCommit` from Info.plist to show version, build date, git commit, and whether the build is DEBUG (via `#if DEBUG`). Re-run the packager after code changes to refresh these values.
