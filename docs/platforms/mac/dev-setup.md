---
summary: "Setup guide for developers working on the Afora macOS app"
read_when:
...
Build and run the Afora macOS application from source.
...
Outputs `dist/Afora.app`. Without an Apple Developer ID certificate, the
script falls back to ad-hoc signing.

Set `AFORA_SKIP_MLX_TTS=1` to package a dev/proof build without the local
MLX voice helper. This skips the `afora-mlx-tts` binary and its large
...
[apps/macos/README.md](https://github.com/AforaMosh/afora-agent/blob/main/apps/macos/README.md).
...
`pnpm add -g --allow-build=afora afora@<version>` and
`bun add -g --trust afora@<version>` also work. Bun's `--trust` allows the
Afora lifecycle scripts for that install. Node remains the recommended
...
only the `afora-mlx-tts` helper may fail while the main app builds fine. The
...
```bash
AFORA_SKIP_MLX_TTS=1 ./scripts/package-mac-app.sh
```
...
1. Reset TCC permissions for the debug bundle id:
   ...
   ```bash
   tccutil reset All ai.afora.mac.debug
   ```

2. If that fails, temporarily change `BUNDLE_ID` in
   [`scripts/package-mac-app.sh`](https://github.com/AforaMosh/afora-agent/blob/main/scripts/package-mac-app.sh)
