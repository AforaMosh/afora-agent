---
summary: "Afora logging: rolling diagnostics file log + unified log privacy flags"
read_when:
...
- Location: `~/Library/Logs/Afora/diagnostics.jsonl`.
...
## Enable for Afora (`ai.afora`)

Write the plist to a temp file first, then install it atomically as root:

```bash
cat <<'EOF' >/tmp/ai.afora.plist
...
EOF
sudo install -m 644 -o root -g wheel /tmp/ai.afora.plist /Library/Preferences/Logging/Subsystems/ai.afora.plist
```
...
- Remove the override: `sudo rm /Library/Preferences/Logging/Subsystems/ai.afora.plist`.
