---
summary: "Android app (node): connection runbook + Connect/Chat/Afora/Voice/Canvas command surface"
read_when:
...
<Note>
The official Android app is available on [Google Play](https://play.google.com/store/apps/details?id=ai.afora.app&hl=en_IN) and as a signed standalone APK on supported [GitHub Releases](https://github.com/AforaMosh/afora-agent/releases). It is a companion node and requires a running Afora Gateway. Source: [apps/android](https://github.com/AforaMosh/afora-agent/tree/main/apps/android) ([build instructions](https://github.com/AforaMosh/afora-agent/blob/main/apps/android/README.md)).
</Note>
...
- Install: [Google Play](https://play.google.com/store/apps/details?id=ai.afora.app&hl=en_IN) or `Afora-Android.apk` from a supported [GitHub Release](https://github.com/AforaMosh/afora-agent/releases), [Getting Started](/start/getting-started) for the Gateway, then [Pairing](/channels/pairing).
...
Regular final and correction GitHub Releases include a universal `Afora-Android.apk` and `Afora-Android-SHA256SUMS.txt`. The APK is built from the release tag, signed with the Afora Android release key, and carries GitHub Actions provenance.

Choose a [release](https://github.com/AforaMosh/afora-agent/releases) that lists both assets, then download and verify that exact tag before sideloading:

```bash
release_tag=vYYYY.M.PATCH
gh release download "$release_tag" \
  --repo AforaMosh/afora-agent \
  --pattern Afora-Android.apk \
  --pattern Afora-Android-SHA256SUMS.txt
sha256sum --check Afora-Android-SHA256SUMS.txt
gh attestation verify Afora-Android.apk \
  --repo AforaMosh/afora-agent \
  --signer-workflow AforaMosh/afora-agent/.github/workflows/android-release.yml \
...
workflow, separate from the Afora node connection. It is useful when the Android device and the
...
- The `afora` CLI available on the gateway machine (or via SSH), to approve pairing requests.
...
1. Set up a DNS-SD zone (example `afora.internal.`) on the gateway host and publish `_afora-gw._tcp` records.
...
with `afora qr`, then scan or paste it on that page and reconnect. Operators
...
1. Create `~/.afora/workspace/canvas/index.html` on the gateway host.
2. Navigate the node to it (LAN):

```bash
afora nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__afora__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__afora__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes. The Gateway also serves `/__afora__/a2ui/`, but the Android app treats remote A2UI pages as render-only. Action-capable A2UI commands use the bundled app-owned A2UI page.
...
Android supports launching Afora from the system assistant trigger (Google Assistant). Holding the home button (or another `ACTION_ASSIST` trigger) opens the app; saying "Hey Google, ask Afora `<prompt>`" matches the app's declared App Actions query pattern and hands the prompt into the chat composer without auto-sending it.
...
<Note>
App Actions availability depends on the device, Google Play Services version, and whether the user has set Afora as the default assistant app.
</Note>

## Notification forwarding

Android can forward device notifications to the gateway as `node.event` items. This is configured **on the device**, in the app's Settings sheet — not in gateway/`afora.json` config.

| Setting                     | Description                                                                                                                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Forward Notification Events | Master toggle. Off by default; requires Notification Listener Access to be granted first.                                                                                                              |
| Package Filter              | **Allowlist** (only listed package IDs forwarded) or **Blocklist** (default: all packages except listed IDs). Afora's own package is always excluded in Blocklist mode to prevent forwarding loops. |
...
WhatsApp, WhatsApp Business, Telegram, Telegram X, Discord, and Signal notifications are always excluded. Their messages are already owned by native Afora channel sessions; forwarding the Android notification as a separate node event could route a reply through the wrong conversation.
