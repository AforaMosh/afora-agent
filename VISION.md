...
Afora is the AI that actually does things.
...
Afora started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> Afora.
...
Afora runtime code reads the current configuration schema only.
...
`afora doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in Afora is a deliberate tradeoff: strong defaults without killing capability.
...
Afora sends no usage analytics, tracking identifiers, or attribution tags unless the operator turned that on themselves.
A change that needs such signals waits until an explicit user-facing opt-in exists for them.

## Plugins & Memory

Afora has an extensive plugin API.
...
- Code plugins run Afora plugin code and are appropriate for deeper runtime extension.
...
Afora docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
Afora supports MCP as both a server and a runtime integration surface.
...
Afora is currently terminal-first by design.
...
Afora is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep Afora hackable by default.
