...
- `AFORA_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `AFORA_QA_ALLOW_INSECURE_HTTP=1`.
...
Admin endpoints require `AFORA_QA_CONVEX_SECRET_MAINTAINER`.
