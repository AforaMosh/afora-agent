...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    if (command.startsWith(AforaCanvasCommand.NamespacePrefix)) {
  ...
  private suspend fun dispatchInvoke(
    ...
    // Command strings come from AforaProtocolConstants; the registry above owns advertised availability.
    return when (command) {
      // Canvas commands
      ...
      AforaCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      AforaCanvasA2UICommand.Push.rawValue, AforaCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      AforaCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      AforaCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      AforaCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      AforaLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      AforaDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      AforaDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      AforaDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      AforaDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)
      AforaDeviceCommand.Apps.rawValue -> deviceHandler.handleDeviceApps(paramsJson)

      // Notifications command
      AforaNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      AforaNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      AforaSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      AforaTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      AforaTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      AforaTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      AforaTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      ai.afora.app.protocol.AforaPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      AforaContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      AforaContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      AforaCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      AforaCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      AforaMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      AforaMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      AforaSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      AforaSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      AforaCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)

      // Mobile accessibility commands
      AforaMobileUiCommand.Observe.rawValue -> mobileUiHandler.handleObserve(paramsJson)
      AforaMobileUiCommand.Act.rawValue -> mobileUiHandler.handleAct(paramsJson)
