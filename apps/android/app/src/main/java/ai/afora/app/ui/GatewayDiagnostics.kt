...
import ai.afora.app.gatewayConnectionStatusForDisplay
...
internal fun aforaAndroidVersionLabel(): String {
...
internal fun gatewayNodeApprovalCommand(
  ...
  when (state) {
    ...
    -> normalizeGatewayApprovalRequestId(requestId)?.let { "afora nodes approve $it" } ?: "afora nodes status"
    GatewayNodeApprovalState.Unapproved -> "afora nodes status"
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return nativeString(
    "Help diagnose this Afora Android gateway connection failure.\n\n" +
      "Please:\n" +
      ...
      "- tell me whether `afora devices list` should show a pending pairing request\n" +
      "- if more signal is needed, ask for `afora qr --json`, `afora devices list`, and `afora nodes status`\n" +
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Afora gateway diagnostics", report))
