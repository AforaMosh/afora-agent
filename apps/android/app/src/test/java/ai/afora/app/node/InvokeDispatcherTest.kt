...
import ai.afora.app.gateway.testDeviceIdentityStore
...
class InvokeDispatcherTest {
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(AforaSmsCommand.Search.rawValue, "not-json")
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(AforaSmsCommand.Send.rawValue, """{"to":"+15551234567","message":"hi"}""")
  ...
  fun handleInvoke_blocksCameraCommandsWhenCameraDisabled() =
    runTest {
      val result = newDispatcher(cameraEnabled = false).handleInvoke(AforaCameraCommand.List.rawValue, null)
  ...
  fun handleInvoke_blocksLocationCommandWhenLocationDisabled() =
    runTest {
      val result = newDispatcher(locationEnabled = false).handleInvoke(AforaLocationCommand.Get.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(installedAppsSharingEnabled = false)
          .handleInvoke(AforaDeviceCommand.Apps.rawValue, """{"limit":1}""")
    ...
    runTest {
      val result =
        newDispatcher(motionActivityAvailable = false)
          .handleInvoke(AforaMotionCommand.Activity.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(motionPedometerAvailable = false)
          .handleInvoke(AforaMotionCommand.Pedometer.rawValue, null)
  ...
  fun handleInvoke_blocksCallLogWhenUnavailable() =
    runTest {
      val result =
        newDispatcher(callLogAvailable = false).handleInvoke(AforaCallLogCommand.Search.rawValue, null)
  ...
  fun handleInvoke_blocksPhotosWhenUnavailable() =
    runTest {
      val result = newDispatcher(photosAvailable = false).handleInvoke(AforaPhotosCommand.Latest.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(mobileUiAvailable = false)
          .handleInvoke(AforaMobileUiCommand.Observe.rawValue, null)
  ...
  fun handleInvoke_routesTalkPttCommands() =
    runTest {
      ...
      val start = dispatcher.handleInvoke(AforaTalkCommand.PttStart.rawValue, null)
      val stop = dispatcher.handleInvoke(AforaTalkCommand.PttStop.rawValue, null)
      val cancel = dispatcher.handleInvoke(AforaTalkCommand.PttCancel.rawValue, null)
      val once = dispatcher.handleInvoke(AforaTalkCommand.PttOnce.rawValue, null)
  ...
  fun handleInvoke_presentAndHideDriveTheShellOwnedCanvasState() =
    runTest {
      ...
      val hide = dispatcher.handleInvoke(AforaCanvasCommand.Hide.rawValue, null)
    ...
    runTest {
      ...
      val result =
        newDispatcher(isForeground = false, canvas = canvas)
          .handleInvoke(AforaCanvasCommand.Present.rawValue, """{"url":"https://example.com"}""")
