...
import ai.afora.app.MainActivity
...
class SystemHandlerTest {
  ...
  fun handleSystemNotify_rejectsUnauthorized() {
    ...
    val result = handler.handleSystemNotify("""{"title":"Afora","body":"hi"}""")
  ...
  fun handleSystemNotify_rejectsInvalidRequestObject() {
    ...
    val result = handler.handleSystemNotify("""{"title":"Afora"}""")
  ...
  fun handleSystemNotify_postsNotification() {
    ...
    val result = handler.handleSystemNotify("""{"title":"Afora","body":"done","priority":"active"}""")
  ...
  fun handleSystemNotify_trimsAndPassesOptionalFields() {
    ...
    val result =
      handler.handleSystemNotify(
        """{"title":" Afora ","body":" done ","priority":" passive ","sound":" silent "}""",
      )

    assertTrue(result.ok)
    assertEquals("Afora", poster.lastRequest?.title)
  ...
  fun buildSystemNotificationSetsImmutableAppLaunchIntent() {
    ...
    val notification =
      buildSystemNotification(
        appContext = context,
        channelId = "test",
        request = SystemNotifyRequest("Afora", "done", sound = null, priority = null),
      )
  ...
  fun handleSystemNotify_returnsUnauthorizedWhenPostFailsPermission() {
    ...
    val result = handler.handleSystemNotify("""{"title":"Afora","body":"done"}""")
