...
class DesktopScreenTest {
  ...
  fun desktopUrlUsesDocumentModeWithoutSource() {
    val url = desktopUrl(baseUrl = "https://gateway.example.com:8443/afora/")

    assertEquals("https://gateway.example.com:8443/afora/?view=desktop", url)
  ...
  fun desktopUrlEncodesProvidedSession() {
    val url =
      desktopUrl(
        baseUrl = "https://gateway.example.com:8443/afora/",
        session = "agent:main:mobile session",
      )

    assertEquals(
      "https://gateway.example.com:8443/afora/?view=desktop&session=agent%3Amain%3Amobile%20session",
      url,
    )
  }
