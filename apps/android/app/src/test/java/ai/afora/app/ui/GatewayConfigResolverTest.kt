...
class GatewayConfigResolverTest {
  ...
  fun parseGatewayEndpointPreservesDecodedContextPath() {
    val parsed = parseGatewayEndpoint("wss://gateway.example/afora%20gateway")

    assertEquals("/afora%20gateway", parsed?.contextPath)
    assertEquals("https://gateway.example/afora%20gateway", parsed?.displayUrl)
  }

  @Test
  fun parseGatewayEndpointPreservesEscapedPathDelimiter() {
    val parsed = parseGatewayEndpoint("wss://gateway.example/afora%2Fgateway")

    assertEquals("/afora%2Fgateway", parsed?.contextPath)
    assertEquals("https://gateway.example/afora%2Fgateway", parsed?.displayUrl)
  }

  @Test
  fun parseGatewayEndpointPreservesRepeatedLeadingPathSlashes() {
    val parsed = parseGatewayEndpoint("wss://gateway.example//afora")

    assertEquals("//afora", parsed?.contextPath)
    assertEquals("https://gateway.example//afora", parsed?.displayUrl)
  }
  ...
  fun resolveGatewayConnectConfigPreservesSetupContextPath() {
    val resolved =
      resolveConnectConfigFixture(
        useSetupCode = true,
        setupCode = setupCode("wss://gateway.example/afora-gw"),
      )

    assertEquals("gateway.example", resolved?.host)
    assertEquals(443, resolved?.port)
    assertEquals(true, resolved?.tls)
    assertEquals("/afora-gw", resolved?.contextPath)
  }
  ...
  fun composeGatewayManualUrlRejectsPastedAuthorityUserInfoQueriesAndFragments() {
    val hosts =
      listOf(
        ...
        "wss://user@gateway.example/afora-gw",
        "wss://gateway.example/afora-gw?mode=manual",
        "wss://gateway.example/afora-gw#fragment",
      )
