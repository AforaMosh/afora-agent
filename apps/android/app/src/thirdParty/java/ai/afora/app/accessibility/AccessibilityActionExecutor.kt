package ai.afora.app.accessibility
...
class AccessibilityActionExecutor internal constructor(
  private val connectionProvider: () -> AccessibilityServiceConnection<AforaAccessibilityService> = {
    AforaAccessibilityService.connection.value
  },
  private val captureSnapshot: (AforaAccessibilityService) -> AccessibilitySnapshotCapture =
    AccessibilitySnapshotter()::capture,
  private val foregroundPackageProvider: (AforaAccessibilityService) -> String? =
    AforaAccessibilityService::foregroundPackageName,
  private val uiEpochProvider: () -> Long = { AforaAccessibilityService.uiEpoch },
) : AutoCloseable {
  ...
  private fun coordinateGesturePreflight(service: AforaAccessibilityService): ActionResult? {
  ...
  private fun nodeActionPackagePreflight(service: AforaAccessibilityService): ActionResult? {
    val expectedPackage = generation.packageName
    val currentPackage = foregroundPackageProvider(service)
    if (expectedPackage == null || currentPackage == null || expectedPackage != currentPackage) {
      return ActionResult(
  ...
  private suspend fun dispatchGesture(
    service: AforaAccessibilityService,
