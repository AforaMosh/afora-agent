...
Afora Android is the officially released Google Play app. It connects to an Afora Gateway as a companion node for chat, voice, approvals, screen, and device-aware automation.
...
tasks still require explicit `aforaBuildCommit` and
`aforaBuildTimestamp` properties so signed artifacts remain reproducible.
...
`Afora_Screenshots_API36` (Pixel 2) and
`Afora_Wear_Screenshots_API34` (Wear OS Large Round) AVDs. It creates a
...
- Play build: `afora-<version>-play-release.aab`
- Wear build: `afora-<version>-wear-release.aab`
- Third-party build: `afora-<version>-third-party-release.apk`

`pnpm android:bundle:release` is an alias for the same Fastlane archive lane.

Regular final and correction Afora releases publish the signed third-party APK as `Afora-Android.apk` with a checksum manifest and GitHub Actions provenance. `.github/workflows/android-release.yml` is the only automated GitHub Release upload path; `Afora Release Publish` dispatches it while the canonical release is still a draft and blocks publication until the uploaded asset contract verifies.

The protected `android-release` environment supplies `MATCH_PASSWORD`; the repository's read-only GitHub App token checks out encrypted material from `afora/apps-signing`. The workflow builds the exact release tag, refuses to replace different existing bytes, and re-downloads the APK for checksum, certificate, and provenance verification.
...
./gradlew -PaforaBuildCommit="$commit" -PaforaBuildTimestamp="$built_at" :app:bundlePlayRelease
./gradlew -PaforaBuildCommit="$commit" -PaforaBuildTimestamp="$built_at" :wear:bundleRelease
./gradlew -PaforaBuildCommit="$commit" -PaforaBuildTimestamp="$built_at" :app:bundleThirdPartyRelease
```
...
```bash
pnpm afora gateway --port 18789 --verbose
```
...
- Canvas web content already supports live reload when loaded from Gateway `__afora__/canvas/` (see `docs/platforms/android.md`).
...
Current Afora Android implication:
...
2) Android app is connected to that gateway and `afora nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device for remote Canvas checks (do not run gateway with `AFORA_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__afora__/`).
...
```bash
afora devices list
afora devices approve --latest   # preview only; copy the requestId from output
afora devices approve <requestId>
```
...
- `AFORA_ANDROID_GATEWAY_URL=ws://...` (default: from your local Afora config)
- `AFORA_ANDROID_GATEWAY_TOKEN=...`
- `AFORA_ANDROID_GATEWAY_PASSWORD=...`
...
- `pairing required` before tests start:
  - list pending requests (`afora devices list`), then approve with the exact ID (`afora devices approve <requestId>`) and rerun.
