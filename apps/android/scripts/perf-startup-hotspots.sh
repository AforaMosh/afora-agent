...
PACKAGE="ai.afora.app.debug"
ACTIVITY="ai.afora.app.MainActivity"
DURATION_SECONDS="10"
OUTPUT_PERF_DATA=""
...
Default package/activity target Afora Android startup.
EOF
}
...
if [[ -z "$OUTPUT_PERF_DATA" ]]; then
  OUTPUT_PERF_DATA="/tmp/afora-startup-$(date +%Y%m%d-%H%M%S).perf.data"
fi
...
tmp_dir="$(mktemp -d -t afora-android-hotspots.XXXXXX)"
