...
class AforaTileService :
  Material3TileService(
    allowDynamicTheme = false,
    defaultColorScheme = aforaTileColorScheme,
  ) {
  override suspend fun MaterialScope.tileResponse(requestParams: RequestBuilders.TileRequest): TileBuilders.Tile {
    val talkAction = wearLaunchAction(this@AforaTileService, WearLaunchTarget.Voice)
    val openAction = wearLaunchAction(this@AforaTileService, WearLaunchTarget.Chat)
    val talkClickable = clickable(action = talkAction, id = "talk_afora")
    val openClickable = clickable(action = openAction, id = "open_afora")
    val layout =
      primaryLayout(
          ...
          avatarButton(
            ...
            avatarContent = {
              avatarImage(
                ...
                protoLayoutResourceId = "afora_core_mascot",
              )
            },
            labelContent = {
              text(
...
private val aforaTileColorScheme =
  ColorScheme(
