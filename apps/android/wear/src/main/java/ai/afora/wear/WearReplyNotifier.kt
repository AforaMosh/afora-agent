...
internal class WearReplyNotifier(
  ...
  fun show(inbound: WearInboundEvent) {
    ...
    val agent = Person.Builder().setName("Afora").build()
    val notification =
      NotificationCompat
  ...
  private companion object {
    const val CHANNEL_ID = "afora_wear_replies"
  }
}
...
internal const val EXTRA_SESSION_KEY = "afora_wear_session_key"
internal const val EXTRA_NOTIFICATION_TAG = "afora_wear_notification_tag"
internal const val EXTRA_PHONE_NODE_ID = "afora_wear_phone_node_id"
...
internal fun replyNotificationTag(
  ...
  return "ai.afora.wear.NOTIFICATION.${sha256("$sessionKey\u0000$messageIdentity")}"
}

internal fun replyPendingIntentAction(
  sessionKey: String,
  notificationTag: String,
): String = "ai.afora.wear.REPLY.${sha256("$sessionKey\u0000$notificationTag")}"
...
private const val LOG_TAG = "AforaWear"
