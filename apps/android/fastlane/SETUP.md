...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the Afora app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=ai.afora.app
```
...
AFORA_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
AFORA_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
AFORA_ANDROID_STORE_PASSWORD=<store-password>
AFORA_ANDROID_KEY_ALIAS=<upload-key-alias>
AFORA_ANDROID_KEY_PASSWORD=<key-password>
```
...
- Stable GitHub Release APK publication is separate from Google Play: `Afora Release Publish` dispatches `.github/workflows/android-release.yml`, whose protected `android-release` environment provides `MATCH_PASSWORD`; the repository GitHub App reads the encrypted signing repo.
