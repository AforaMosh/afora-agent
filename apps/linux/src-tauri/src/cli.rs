...
impl fmt::Display for CliError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Missing => write!(formatter, "Afora CLI not found"),
...
impl AforaCli {
    pub fn discover() -> Result<Self, CliError> {
        let home = afora_home()?;
        if let Some(override_path) = env::var_os("AFORA_DESKTOP_CLI") {
            let cli = Self::new(PathBuf::from(override_path), home);
            cli.verify()?;
            return Ok(cli);
        }

        let managed = home.join("bin/afora");
        if managed.is_file() {
        ...
        let cli = Self::new(PathBuf::from("afora"), home);
        match cli.verify() {
            Ok(()) => Ok(cli),
    ...
    fn new(executable: PathBuf, afora_home: PathBuf) -> Self {
        Self {
            executable,
            afora_home,
        }
        ...
        S: AsRef<std::ffi::OsStr>,
    {
        self.command(args)?
            .output()
            .map_err(|error| CliError::Spawn(format!("Failed to run Afora CLI: {error}")))
    }
        ...
        S: AsRef<std::ffi::OsStr>,
    {
        ...
        let value = serde_json::from_slice(&output.stdout).map_err(|error| {
            CliError::InvalidJson(format!("Afora CLI returned invalid JSON: {error}"))
        })?;
        Ok((value, output))
    }
    ...
    fn command_path(&self) -> Result<OsString, CliError> {
        let mut paths = vec![
            self.afora_home.join("bin"),
            self.afora_home.join("tools/node/bin"),
        ];
...
pub fn afora_home() -> Result<PathBuf, CliError> {
    ...
    Ok(PathBuf::from(home).join(".afora"))
}
