...
impl ServerCertVerifier for GatewayTlsPinVerifier {
    fn verify_server_cert(
        ...
        // app. A present pin replaces CA/hostname trust, matching AforaKit; the signature
...
mod tests {
    ...
    fn connect_frame_matches_gateway_schema() {
        let directory = std::env::temp_dir().join(format!(
            "afora-linux-connect-frame-test-{}",
            Uuid::new_v4()
        ));
    ...
    fn hello_tick_policy_sets_two_interval_watchdog() {
        let hello = validate_hello(json!({
            ...
            "pluginSurfaceUrls": {
                "canvas": "https://gateway.example/__afora__/cap/fixture-capability"
            }
        }))
        .expect("valid hello");
        ...
        assert_eq!(
            ...
            Some("https://gateway.example/__afora__/cap/fixture-capability")
        );
        assert_eq!(
    ...
    fn plugin_surface_refresh_response_decodes_canvas_url() {
        let response: PluginSurfaceRefreshResponse = serde_json::from_value(json!({
            "pluginSurfaceUrls": {
                "canvas": "https://gateway.example/__afora__/cap/refreshed-capability"
            }
        }))
        ...
        assert_eq!(
            response
                ...
                .and_then(|urls| urls.get("canvas").cloned())
                .as_deref(),
            Some("https://gateway.example/__afora__/cap/refreshed-capability")
        );
    }
    ...
    fn gateway_state_event_carries_canvas_surface_in_camel_case() {
        let event = serde_json::to_value(GatewayStateEvent::new(
            ...
            None,
            Some("https://gateway.example/__afora__/cap/fixture-capability".to_string()),
        ))
        .expect("serialize gateway state");

        assert_eq!(
            event["canvasSurfaceUrl"],
            "https://gateway.example/__afora__/cap/fixture-capability"
        );
