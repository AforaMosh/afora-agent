...
function friendlyError(error) {
  ...
  return error?.message || "Afora could not complete the operation.";
}
...
function renderGateways(gateways) {
  ...
  if (!gateways.length) {
    ...
    empty.textContent = "Looking for nearby Afora gateways…";
  ...
  try {
    ...
    if (snapshot.phase === "missingCli") {
      ...
      render({
        ...
        description: "Afora is installing its managed CLI and Node runtime.",
        eyebrow: "FIRST-RUN SETUP",
        title: "Preparing Afora",
      });
      await install();
    }
  } catch (error) {
...
async function runGatewayAction(action) {
  render({
    ...
    description: "Afora is waiting for the local gateway to become healthy.",
    eyebrow: "GATEWAY",
    title: "One moment",
  });
  try {
...
if (mode === "reconnecting") {
  render({
    ...
    description: "The gateway connection dropped. Afora will restore the dashboard automatically.",
