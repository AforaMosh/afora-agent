    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Text(self.notice.body)
                .font(AforaChatTypography.footnote)
...
struct ChatAgentAvatar: View {
    ...
    var body: some View {
        Text(self.displayText)
            .font(AforaChatTypography.avatar(size: self.fontSize))
            ...
            .background(
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                (self.tint ?? AforaChatTheme.accent).opacity(0.95),
            ...
            .shadow(color: (self.tint ?? AforaChatTheme.accent).opacity(0.18), radius: 8, y: 4)
...
struct ChatMessageBubble: View {
    ...
    let displayOptions: AforaChatDisplayOptions
    ...
    let inlineWidgetResourceResolver: @MainActor @Sendable (
        String,
        AforaChatWidgetResource?) async -> AforaChatWidgetResource?
    ...
    let loadMediaArtifact: @MainActor @Sendable (
        String,
        AforaChatMediaKind,
        AforaChatPlaybackMode?) async throws -> AforaChatLoadedMedia?
...
private struct ChatMessageBody: View {
    @Environment(\.aforaAssistantBubblesInCleanChrome) private var assistantBubblesInClean
    let message: AforaChatMessage
    let isUser: Bool
    let style: AforaChatView.Style
    let markdownVariant: ChatMarkdownVariant
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AforaChatTheme.userText : AforaChatTheme.assistantText
    ...
    private func usageLine(_ presentation: ChatMessageUsagePresentation) -> some View {
        Text(presentation.text)
            .font(AforaChatTypography.caption2)
            .monospacedDigit()
    ...
    private var toolActivityItems: [ChatToolActivityItem] {
        ...
        // (AforaChatView.mergeToolResults); this branch is the orphan
    ...
    private var inlineAttachments: [AforaChatMessageContent] {
        self.message.content.filter(\.isInlineAttachment)
    }

    private var visibleInlineAttachments: [AforaChatMessageContent] {
        var imageCount = 0
    ...
    private var inlineWidgets: [AforaChatCanvasPreview] {
    ...
    private var toolCalls: [AforaChatMessageContent] {
    ...
    private var inlineToolResults: [AforaChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func usageTint(_ pressure: ChatMessageUsagePresentation.Pressure) -> Color {
        switch pressure {
        case .normal:
            AforaChatTheme.muted
        case .warning:
            AforaChatTheme.warning
        case .danger:
            AforaChatTheme.danger
        }
    }

    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AforaChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AforaChatTheme.onboardingAssistantBubble
        }
        return AforaChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AforaChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AforaChatMessageContent
    let isUser: Bool
    ...
    private var fallbackRow: some View {
        HStack(spacing: 8) {
            ...
            Text(self.isAudio ? "Voice note" : self.attachmentLabel)
                .font(AforaChatTypography.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AforaChatTheme.userText : AforaChatTheme.assistantText)
            if self.isAudio, let durationSeconds = self.att.durationSeconds {
                Text(aforaVoiceNoteDurationLabel(durationSeconds))
                    .font(AforaChatTypography.footnote)
                    .foregroundStyle(
                        self.isUser
                            ? AforaChatTheme.userText.opacity(0.72)
                            : AforaChatTheme.assistantText.opacity(0.72))
            }
            Spacer()
        }
        .padding(10)
...
struct ChatOutboxStatusLabel: View {
    let state: AforaChatOutboxMessageState
    ...
    var body: some View {
        ...
        .foregroundStyle(self.state.isFailed ? AnyShapeStyle(AforaChatTheme.danger) : AnyShapeStyle(.secondary))
    ...
    private var title: LocalizedStringResource {
        ...
        case .confirming:
            "Confirming…"
        case let .failed(reason) where reason == AforaChatSQLiteTranscriptCache.outboxUnconfirmedError:
            "Delivery unknown"
        case .failed:
            "Not sent"
        }
...
private struct AforaAssistantBubblesInCleanChromeKey: EnvironmentKey {
...
extension EnvironmentValues {
    ...
    public var aforaAssistantBubblesInCleanChrome: Bool {
        get { self[AforaAssistantBubblesInCleanChromeKey.self] }
        set { self[AforaAssistantBubblesInCleanChromeKey.self] = newValue }
    }
}
...
private struct AssistantBubbleContainerStyle: ViewModifier {
    ...
    @Environment(\.aforaAssistantBubblesInCleanChrome) private var bubblesInClean
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AforaChatPendingToolCall]
