...
extension AforaChatViewModel {
    static func decodeMessages(_ raw: [AnyCodable]) -> [AforaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AforaChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    static func stripInboundMetadata(from message: AforaChatMessage) -> AforaChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> AforaChatMessageContent in
    ...
    static func messageContentFingerprint(for message: AforaChatMessage) -> String {
    ...
    static func finalMessageContentFingerprint(for message: AforaChatMessage) -> String {
        message.content.map { item in
            let type = (item.type ?? "text").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            let text = (item.text ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
            let artifactId = (item.artifactId ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func messageIdentityKey(for message: AforaChatMessage) -> String? {
    ...
    static func userRefreshIdentityKey(for message: AforaChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func finalMessageReconciliationKey(for message: AforaChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func adoptingCanonicalMessage(
        _ incoming: AforaChatMessage,
        over existing: AforaChatMessage) -> AforaChatMessage
    {
        AforaChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func preservingLocalAudioDurations(
        in incoming: [AforaChatMessageContent],
        from existing: [AforaChatMessageContent]) -> [AforaChatMessageContent]
    {
    ...
    static func messageRange(
        ...
        in messages: [AforaChatMessage]) -> Range<[AforaChatMessage].Index>
    {
    ...
    func adoptingProvisionalFinalMessageIDs(
        in incoming: [AforaChatMessage]) -> [AforaChatMessage]
    {
    ...
    func clearProvisionalFinalMarkersAdoptedByHistory(_ incoming: [AforaChatMessage]) {
    ...
    func adoptCorrelatedUserMessage(incoming: AforaChatMessage) -> Bool {
    ...
    func adoptProvisionalFinalMessage(incoming: AforaChatMessage) -> Bool {
    ...
    static func reconcileMessageIDs(
        previous: [AforaChatMessage],
        incoming: [AforaChatMessage]) -> [AforaChatMessage]
    {
        guard !previous.isEmpty, !incoming.isEmpty else { return incoming }

        var previousMessagesByKey: [String: [AforaChatMessage]] = [:]
    ...
    static func reconcileRunRefreshMessages(
        previous: [AforaChatMessage],
        incoming: [AforaChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [AforaChatMessage]
    {
    ...
    static func dedupeMessages(_ messages: [AforaChatMessage]) -> [AforaChatMessage] {
    ...
    static func dedupeKey(for message: AforaChatMessage) -> String? {
...
extension AforaChatViewModel {
    ...
    func applyHistoryPayload(
        _ payload: AforaChatHistoryPayload,
    ...
    private func provisionalFinalMessagesMissing(
        from incoming: [AforaChatMessage]) -> [AforaChatMessage]
    {
