...
public struct ChatModelProviderSection: Identifiable, Sendable, Equatable {
    ...
    public let models: [AforaChatModelChoice]
    public let isDefaultProvider: Bool
}

public struct ChatModelPickerSections: Sendable, Equatable {
    public let pinned: [AforaChatModelChoice]
    public let recent: [AforaChatModelChoice]
    public let providers: [ChatModelProviderSection]
}

@MainActor
public final class ChatModelPickerStore {
    private static let favoritesKey = "afora.chat.modelFavorites"
    private static let recentsKey = "afora.chat.modelRecents"
    ...
    public func recordRecent(_ selectionID: String) {
        guard !selectionID.isEmpty, selectionID != AforaChatViewModel.defaultModelSelectionID else { return }
    ...
    private static func effectiveProvider(_ choice: AforaChatModelChoice) -> String {
        let metadataProvider = self.normalizedProvider(choice.provider)
        if metadataProvider != "other" {
            return metadataProvider
        }
