...
private let outboxLogger = Logger(subsystem: "ai.afora", category: "AforaChatOutbox")

/// Display state for a transcript row backed by a durable outbox command.
public enum AforaChatOutboxMessageState: Equatable, Sendable {
    case queued
    case sending
    case confirming
...
extension AforaChatViewModel {
    ...
    public func outboxState(for messageID: UUID) -> AforaChatOutboxMessageState? {
        self.outboxStatesByMessageID[messageID]
    }

    static func activeBranchLeafEntryID(in branches: [AforaChatSessionBranch]) -> String? {
    ...
    func outboxBranchScope(for session: SessionSnapshot) -> AforaChatOutboxScope? {
        let agentID = self.outboxAgentID(for: session)
        guard !self.outboxRequiresAgentID(for: session) || agentID != nil else { return nil }
        return AforaChatOutboxScope(sessionKey: session.key, agentID: agentID)
    }

    private static func outboxBranchScope(for command: AforaChatOutboxCommand) -> AforaChatOutboxScope {
        AforaChatOutboxScope(sessionKey: command.sessionKey, agentID: command.agentID)
    }
    ...
    func captureOutboxBranchState(for session: SessionSnapshot) async -> AforaChatOutboxBranchState? {
    ...
    func observeOutboxTranscriptTip(_ message: AforaChatMessage, session: SessionSnapshot) {
    ...
    func reconcileOutboxBranchScope(
        _ session: SessionSnapshot,
        branches: [AforaChatSessionBranch],
    ...
    func enqueueOutboxCommand(
        ...
        draftAttachments: [AforaPendingAttachment] = [],
        session: SessionSnapshot) async -> Bool
    {
        ...
        let thinking = self.effectiveThinkingLevelForSend(
            ...
            agentID: agentID,
            sessionRoutingContract: routingContract)
        let command = AforaChatOutboxCommand(
            id: UUID().uuidString,
            sessionKey: session.key,
            deliverySessionKey: deliverySessionKey,
            routingContract: routingContract,
        ...
        deliveryIsAmbiguous: Bool) async -> Bool
    {
        ...
        let command = AforaChatOutboxCommand(
            ...
            lastError: deliveryIsAmbiguous
                ? AforaChatSQLiteTranscriptCache.outboxUnconfirmedError
                : nil)
        await self.waitForBootstrapOutboxBranchCapture(for: session)
    ...
    func confirmOutboxCommands(in messages: [AforaChatMessage]) {
    ...
    func confirmOutboxCommandsNow(in messages: [AforaChatMessage]) async {
        self.observeCanonicalOutboxMessageKeys(in: messages)
        guard let outbox else { return }
    ...
    private func persistCanonicalOutboxEvidence(
        _ message: AforaChatMessage,
        for command: AforaChatOutboxCommand) async
    {
        guard let transcriptCache = transcriptCache as? any AforaChatCanonicalTranscriptMerging else { return }
    ...
    private func observeCanonicalOutboxMessageKeys(in messages: [AforaChatMessage]) {
    ...
    private func presentOutboxCommands(_ commands: [AforaChatOutboxCommand]) {
    ...
    private static func outboxUserMessage(for command: AforaChatOutboxCommand) -> AforaChatMessage {
        var content = [
    ...
    private func mapOutboxCommand(_ command: AforaChatOutboxCommand, to messageID: UUID) {
    ...
    private func deliverOutboxCommand(
        _ command: AforaChatOutboxCommand,
        outbox: any AforaChatCommandOutbox,
        routeLease: AforaChatTransportRouteLease) async -> OutboxFlushDisposition
    {
        do {
            ...
            if error.detailsReason == AforaChatSessionRoutingContract.changedErrorReason {
    ...
    private func finishAcceptedOutboxCommand(
        _ command: AforaChatOutboxCommand,
        outbox: any AforaChatCommandOutbox) async -> OutboxFlushDisposition
    {
    ...
    private static func needsOutboxDeliveryReconciliation(_ command: AforaChatOutboxCommand) -> Bool {
        command.status == .awaitingConfirmation ||
            (command.status == .failed &&
                command.lastError == AforaChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    private static func deliveryTarget(for command: AforaChatOutboxCommand) -> OutboxDeliveryTarget {
    ...
    private func parkOutboxCommandWithUnconfirmedDelivery(
        _ command: AforaChatOutboxCommand,
        outbox: any AforaChatCommandOutbox) async -> AforaChatOutboxUpdateResult
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            attemptVersion: command.attemptVersion,
            retryCount: command.retryCount,
            lastError: AforaChatSQLiteTranscriptCache.outboxUnconfirmedError)
        switch update {
        case .updated:
            self.setOutboxState(
                .failed(reason: AforaChatSQLiteTranscriptCache.outboxUnconfirmedError),
    ...
    private func parkOutboxCommandForChangedTarget(
        _ command: AforaChatOutboxCommand,
        outbox: any AforaChatCommandOutbox) async -> Bool
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            attemptVersion: command.attemptVersion,
            retryCount: command.retryCount,
            lastError: AforaChatSQLiteTranscriptCache.outboxChangedTargetError)
        guard update != .unavailable else {
            self.applyTransportHealth(false)
            return false
        }
    ...
    private func recordOutboxRejection(
        of command: AforaChatOutboxCommand,
        outbox: any AforaChatCommandOutbox,
    ...
    private func outboxRoutingContract(for session: SessionSnapshot) -> String? {
        if !self.transport.outboxRequiresSessionRoutingContract {
            return AforaChatOutboxCommand.legacyUnboundRoutingContract
        }
        ...
        agentID: String?) -> String?
    {
        ...
        if AforaChatSessionKey.agentID(from: raw) != nil { return raw }
    ...
    private func setOutboxState(_ state: AforaChatOutboxMessageState, forCommandID commandID: String) {
    ...
    func handleOutboxChange(_ change: AforaChatOutboxChange) {
    ...
    private static func outboxDisplayState(for command: AforaChatOutboxCommand)
        -> AforaChatOutboxMessageState
    {
        ...
        case .awaitingConfirmation:
            .confirming
        case .failed:
            .failed(reason: AforaChatSQLiteTranscriptCache.outboxDisplayError(command.lastError))
        }
    }
