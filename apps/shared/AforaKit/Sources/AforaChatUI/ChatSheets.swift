...
public struct ChatSessionsSheet: View {
    ...
    @State private var scopedSessions: [AforaChatSessionEntry] = []
    @State private var isLoadingScoped = false
    @State private var renameTarget: AforaChatSessionEntry?
    ...
    @State private var inspectedSession: AforaChatSessionEntry?
    @State private var isPresentingGroups = false

    public init(viewModel: AforaChatViewModel) {
    ...
    private var displayedSessions: [AforaChatSessionEntry] {
    ...
    public var body: some View {
        NavigationStack {
            ...
            .onAppear {
                self.viewModel.refreshSessions(limit: AforaChatViewModel.sessionListFetchLimit)
            }
    ...
    private func interactiveSessionRow(_ session: AforaChatSessionEntry) -> some View {
        ...
        return Button {
            ...
            .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                    ...
                    mainSessionKey: self.viewModel.resolvedMainSessionKey)
                {
                    ...
                    .tint(session.isArchived ? AforaChatTheme.accent : AforaChatTheme.danger)
                }
            }
            .contextMenu {
                Button {
    ...
    private func sessionRowContent(_ session: AforaChatSessionEntry) -> some View {
        HStack(spacing: 8) {
            VStack(alignment: .leading, spacing: 4) {
                Text(session.displayName ?? session.key)
                    .font(AforaChatTypography.mono(size: 17, relativeTo: .body))
                    .lineLimit(1)
