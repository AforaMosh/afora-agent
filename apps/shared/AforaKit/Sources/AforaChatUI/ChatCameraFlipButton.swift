...
struct ChatCameraFlipButton: View {
    let control: AforaChatTalkControl
    let size: CGFloat

    static func isAvailable(for control: AforaChatTalkControl) -> Bool {
    ...
    var body: some View {
        Button {
            ...
            Image(systemName: "arrow.triangle.2.circlepath.camera")
                .font(AforaChatTypography.body(size: 14, weight: .semibold, relativeTo: .subheadline))
                .foregroundStyle(.primary)
                .frame(width: self.size, height: self.size)
                .background {
                    Circle()
                        .fill(AforaChatTheme.accent.opacity(0.12))
                }
...
struct ChatTalkButton: View {
    ...
    private func compactButton(controlHeight: CGFloat, iconControlSize: CGFloat) -> some View {
        Button {
            ...
            ChatTalkButtonGlyph(control: self.control)
                ...
                .background {
                    Circle()
                        .fill(self.control.isEnabled ? self.fill : AnyShapeStyle(AforaChatTheme.accent))
    ...
    private var stroke: Color {
        ...
        return AforaChatTheme.composerBorder
    }
...
enum ChatDictationActions {
    static func start(
        _ control: AforaChatDictationControl,
    ...
    static func cancel(
        task: Binding<Task<Void, Never>?>,
        control: AforaChatDictationControl?)
    {
        guard task.wrappedValue != nil else { return }
