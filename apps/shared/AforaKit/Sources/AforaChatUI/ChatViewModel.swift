...
let chatUILogger = Logger(subsystem: "ai.afora", category: "AforaChatUI")

@MainActor
@Observable
public final class AforaChatViewModel {
    ...
    public internal(set) var messages: [AforaChatMessage] = []
    ...
    public internal(set) var replyTarget: AforaChatReplyTarget?
    @ObservationIgnored
    ...
    /// Setter is module-internal for the thinking-level extension only.
    public internal(set) var thinkingLevelOptions: [AforaChatThinkingLevelOption]
    /// Setter is module-internal for the thinking-level extension only.
    ...
    public private(set) var modelChoices: [AforaChatModelChoice] = []
    ...
    public internal(set) var slashCommands: [AforaChatCommandChoice] = []
    ...
    public var attachments: [AforaPendingAttachment] = []
    ...
    public internal(set) var sessionBranches: [AforaChatSessionBranch] = []
    ...
    public internal(set) var questionCards: [AforaQuestionCardModel] = []
    ...
    public private(set) var pendingToolCalls: [AforaChatPendingToolCall] = []
    var subagentActivities: [ChatSubagentActivity] = []
    var hiddenWorkingSubagentCount = 0
    public internal(set) var planSteps: [AforaChatPlanStep] = []
    ...
    public internal(set) var sessions: [AforaChatSessionEntry] = [] {
        didSet {
            syncContextUsageFraction()
            syncActiveSessionRunIDsFromCurrentSession()
        }
    ...
    public internal(set) var swarmSessions: [AforaChatSessionEntry] = []
    var activeSwarmGroups: [AforaChatSwarmGroup] = []
    var swarmActivityState = AforaChatSwarmActivityState()
    @ObservationIgnored
    ...
    let transport: any AforaChatTransport
    let haptics: AforaChatHaptics
    let transcriptCache: (any AforaChatTranscriptCache)?
    let outbox: (any AforaChatCommandOutbox)?
    @ObservationIgnored
    ...
    public internal(set) var outboxStatesByMessageID: [UUID: AforaChatOutboxMessageState] = [:]
    @ObservationIgnored
    ...
    @ObservationIgnored
    var reconciledOutboxBranchScopes: Set<AforaChatOutboxScope> = []
    @ObservationIgnored
    var reconcilingOutboxBranchScopes: Set<AforaChatOutboxScope> = []
    @ObservationIgnored
    var outboxBranchReconcileRetryAttempts: [AforaChatOutboxScope: Int] = [:]
    @ObservationIgnored
    var outboxBranchReconcileRetryTasks: [AforaChatOutboxScope: Task<Void, Never>] = [:]
    @ObservationIgnored
    ...
    var bootstrapOutboxBranchStateCapture: (
        ...
        task: Task<AforaChatOutboxBranchState?, Never>)?
    @ObservationIgnored
    ...
    var sessionDefaults: AforaChatSessionsDefaults? {
    ...
    let onToolActivity: AforaChatToolActivityHandler?
    ...
    var lastSuccessfulSettingsPatchResultsByTarget: [ModelPatchTarget: AforaChatModelPatchResult] = [:]
    ...
    var acceptedSettingsPatchResultsByTarget: [ModelPatchTarget: AforaChatModelPatchResult] = [:]
    ...
    struct FastModeState {
        let override: AforaChatFastMode?
        let effective: AforaChatFastMode?
    }
    ...
    private struct SettingsPatchTail {
        ...
        let routeLeaseTask: Task<AforaChatSessionSettingsRouteLease?, Never>
    ...
    struct HistoryRequest {
        ...
        var visibleMessagesByID: [UUID: AforaChatMessage]
    ...
    var pendingToolCallsById: [String: AforaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        ...
        haptics: AforaChatHaptics = AforaChatHaptics(),
        ...
        onToolActivity: AforaChatToolActivityHandler? = nil,
    ...
    private var usesMutableAgentRouting: Bool {
        AforaChatSessionKey.agentID(from: self.sessionKey) == nil
    }
    ...
    func usesMutableContractRouting(sessionKey: String, contract: String?) -> Bool {
        if AforaChatSessionKey.agentID(from: sessionKey) == nil {
            return true
        }
        ...
        let contractMainKey = AforaChatSessionRoutingContract.parse(contract)?.mainKey ?? ""
    ...
    static let baseThinkingLevelOptions: [AforaChatThinkingLevelOption] = [
        AforaChatThinkingLevelOption(id: "off", label: "off"),
        AforaChatThinkingLevelOption(id: "minimal", label: "minimal"),
...
extension AforaChatViewModel {
    ...
    func appendMessage(_ message: AforaChatMessage) {
    ...
    func currentSessionSnapshot() -> SessionSnapshot {
        SessionSnapshot(
            ...
            deliveryAgentID: AforaChatSessionKey.agentID(from: self.sessionKey) ?? self.activeAgentId,
    ...
    func fetchSessions(limit: Int?, sessionSnapshot: SessionSnapshot? = nil) async {
        ...
        while true {
            ...
            let res: AforaChatSessionsListResponse
            do {
            ...
            let organized = AforaChatSessionListOrganizer.organize(res.sessions)
    ...
    private func performSelectModel(
        ...
        routeLease: AforaChatSessionSettingsRouteLease?) async
    {
        do {
            guard let routeLease else { throw AforaChatTransportSendError.notDispatched }
            let patchResult = try await routeLease.patchSessionSettings(
                ...
                patch: AforaChatSessionSettingsPatch(model: .some(request.modelRef)))
    ...
    func enqueueSessionSettingsPatch(
        ...
        operation: @escaping @MainActor (AforaChatSessionSettingsRouteLease?) async -> Void)
    {
    ...
    func generatedNewSessionKey(agentID explicitAgentID: String? = nil) -> String {
        ...
        guard let agentID = explicitAgentID ??
            AforaChatSessionKey.agentID(from: sessionKey) ??
            activeAgentId ??
            AforaChatSessionKey.agentID(from: resolvedMainSessionKey) ??
            sessions.lazy.compactMap({ AforaChatSessionKey.agentID(from: $0.key) }).first
        else {
            return baseKey
        }
    ...
    private func applySuccessfulModelSelection(
        ...
        syncSelection: Bool,
        patchResult: AforaChatModelPatchResult? = nil)
    {
        self.recordSuccessfulModelPatch(
    ...
    private func recordSuccessfulModelPatch(
        selectionID: String,
        patchResult: AforaChatModelPatchResult?,
        target: ModelPatchTarget)
    {
        ...
        if let patchResult {
            ...
            self.acceptedSettingsPatchResultsByTarget[target] = AforaChatModelPatchResult(
