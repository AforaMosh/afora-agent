...
struct ChatPlanPill: View {
    ...
    private var currentStep: AforaChatPlanStep? {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if self.isExpanded {
                Divider()
                    .overlay(AforaChatTheme.divider)
                    .padding(.horizontal, 12)
    ...
    private func stepRow(_ step: AforaChatPlanStep) -> some View {
    ...
    private static func marker(for status: AforaChatPlanStep.Status) -> String {
        switch status {
        case .completed: "✓"
        case .inProgress: "▸"
        case .pending: "▢"
    ...
    private static func markerColor(for status: AforaChatPlanStep.Status) -> Color {
        switch status {
        case .completed, .inProgress: AforaChatTheme.accent
        case .pending: AforaChatTheme.muted
        }
    }

    private static func stepAccessibilityLabel(_ step: AforaChatPlanStep) -> String {
        "\(self.accessibilityLabel(for: step.status)), \(step.step)"
    }

    private static func accessibilityLabel(for status: AforaChatPlanStep.Status) -> String {
        switch status {
