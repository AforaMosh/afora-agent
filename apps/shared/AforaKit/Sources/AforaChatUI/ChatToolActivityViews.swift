...
enum ChatToolActivity {
    static func items(
        calls: [AforaChatMessageContent],
        results: [AforaChatMessageContent]) -> [ChatToolActivityItem]
    {
...
struct ChatToolActivityRow: View {
                    ...
                    if self.resolvedDiff != nil {
                        ...
                        if !self.formattedResult.isEmpty {
                            Text(self.formattedResult)
                                .font(AforaChatTypography.mono(size: 12, relativeTo: .footnote))
    ...
    private var collapsedRow: some View {
        HStack(alignment: .firstTextBaseline, spacing: 7) {
            ...
            Image(systemName: Self.symbol(forToolName: self.item.name))
                ...
                .foregroundStyle(self.item.isError ? AforaChatTheme.danger : Color.secondary)

            Text(self.display.title)
                .font(AforaChatTypography.footnoteSemiBold)
                .foregroundStyle(
                    self.item.isError ? AforaChatTheme.danger : AforaChatTheme.assistantText)
                .lineLimit(1)
    ...
    private func diffRow(_ line: ChatToolDiffLine) -> some View {
        if line.kind == .file {
            Text(verbatim: String(line.text.unicodeScalars.prefix(2000)))
                .font(AforaChatTypography.mono(size: 11, relativeTo: .caption))
                .fontWeight(.semibold)
                .foregroundStyle(.secondary)
            ...
            HStack(spacing: 6) {
                if let lineNo = line.lineNo {
                    Text(verbatim: "\(lineNo)")
                        .font(AforaChatTypography.mono(size: 10, relativeTo: .caption2))
                        .foregroundStyle(.secondary.opacity(0.6))
    ...
    private func diffBackground(_ kind: ChatToolDiffLineKind) -> Color {
        switch kind {
        case .add:
            AforaChatTheme.success.opacity(0.14)
        case .del:
            AforaChatTheme.danger.opacity(0.12)
        case .ctx, .file, .skip:
            .clear
        }
    }
    ...
    var body: some View {
        Group {
            Text(verbatim: "+\(self.stat.added)")
                .foregroundStyle(AforaChatTheme.success.opacity(0.9))
            Text(verbatim: "−\(self.stat.removed)")
                .foregroundStyle(AforaChatTheme.danger.opacity(0.9))
        }
        .font(AforaChatTypography.mono(size: 12, relativeTo: .footnote))
        .lineLimit(1)
    }
