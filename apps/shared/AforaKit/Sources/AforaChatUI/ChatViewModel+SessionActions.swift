...
extension AforaChatViewModel {
    ...
    func performStartNewSession(
        ...
        routeLease: AforaChatNewSessionRouteLease? = nil) async -> Bool
    {
        ...
        let currentAgentID = (
            AforaChatSessionKey.agentID(from: self.sessionKey) ??
                self.activeAgentId ??
                AforaChatSessionKey.agentID(from: self.resolvedMainSessionKey))?
            .lowercased()
    ...
    static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "AforaChatTransport"
    ...
    func startNewSession(
        ...
        using routeLease: AforaChatNewSessionRouteLease) async -> Bool
    {
        await self.performStartNewSession(
            agentID: agentID,
            worktree: worktree,
    ...
    func newSessionRouteLease() async throws -> AforaChatNewSessionRouteLease {
    ...
    public func fetchSessionGroups() async throws -> [AforaChatSessionGroup] {
        let routeLease = try await self.sessionGroupsRouteLease()
        return try await self.fetchSessionGroups(using: routeLease)
    }

    func sessionGroupsRouteLease() async throws -> AforaChatSessionGroupsRouteLease {
    ...
    func fetchSessionGroups(
        using routeLease: AforaChatSessionGroupsRouteLease) async throws -> [AforaChatSessionGroup]
    {
    ...
    func performSessionBatch(
        sessions selectedSessions: [AforaChatSessionEntry],
        action: ChatSessionBatchAction) async -> ChatSessionBatchResult
    {
        ...
        case .pin, .unpin:
            ...
            self.sessions = AforaChatSessionListOrganizer.organize(self.sessions)
        case .archive, .delete:
    ...
    public func fetchSessionList(search: String?, archived: Bool) async -> [AforaChatSessionEntry] {
        ...
        do {
            let res = try await self.transport.listSessions(
                ...
                search: query,
                archived: archived)
            return AforaChatSessionListOrganizer.organize(res.sessions)
        } catch {
            ...
            return AforaChatSessionListOrganizer.filter(self.sessions, search: query)
        }
    }
    ...
    public func rewindToMessage(_ message: AforaChatMessage) async {
    ...
    func refreshSessionBranches(
        ...
        preBootstrapBranchState: AforaChatOutboxBranchState?) async -> Bool
    {
        let refreshGeneration = self.beginSessionBranchesRefresh()
        return await self.performSessionBranchesRefresh(
            for: session,
    ...
    public func forkAtMessage(_ message: AforaChatMessage) async {
        guard let entryID = Self.sessionMutationEntryID(for: message) else { return }
        guard self.canPerformMessageSessionAction else { return }
        guard self.canCreateSessionForImmediateSwitch() else { return }
        let initiatingSession = self.currentSessionSnapshot()
    ...
    private static func sessionMutationEntryID(for message: AforaChatMessage) -> String? {
    ...
    public func setSessionArchived(_ session: AforaChatSessionEntry, archived: Bool) {
        let key = session.key
        guard archived else {
    ...
    @discardableResult
    public func restoreSession(_ session: AforaChatSessionEntry) async -> Bool {
        guard let expectedSessionID = session.sessionId?
            .trimmingCharacters(in: .whitespacesAndNewlines),
            !expectedSessionID.isEmpty
        else {
    ...
    func markCurrentSessionReadAfterActivation(
        ...
        fallbackEntry: AforaChatSessionEntry?) async
    {
