...
extension AforaChatViewModel {
    ...
    public func removeAttachment(_ id: AforaPendingAttachment.ID) {
        attachments.removeAll { $0.id == id }
        applyDeferredExternalStateIfReady()
    }

    func restoreEditorAttachments(_ editorAttachments: [AforaChatEditorAttachment]?) {
    ...
    public func addVoiceNoteAttachment(fileURL: URL, durationSeconds: Double) async {
        ...
        let normalizedDuration = durationSeconds.isFinite
            ? min(max(0, durationSeconds), AforaVoiceNoteRecorder.maximumDurationSeconds)
            : 0
        attachments.append(
        ...
        expectedSession: SessionSnapshot?) async
    {
        ...
        guard self.ownsAttachmentSession(expectedSession) else { return }
        self.attachments.append(AforaPendingAttachment(
            url: nil,
            data: data,
            fileName: fileName,
    ...
    static func previewImage(data: Data) -> AforaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
        ...
        fileExtension: String) async -> Data?
    {
        await Task.detached(priority: .userInitiated) {
            let fileURL = FileManager.default.temporaryDirectory
                .appendingPathComponent("afora-upload-preview-\(UUID().uuidString)")
