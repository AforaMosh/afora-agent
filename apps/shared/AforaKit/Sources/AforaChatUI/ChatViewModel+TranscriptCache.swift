...
extension AforaChatViewModel {
    ...
    func replaceMessages(_ messages: [AforaChatMessage]) {
    ...
    nonisolated static func durableSessionCacheProjection(
        _ session: AforaChatSessionEntry) -> AforaChatSessionEntry
    {
        ...
        canonicalMessageIdempotencyKeys: Set<String>)
    {
        ...
        let branchStateTask: Task<AforaChatOutboxBranchState?, Never>? = if let outbox, let scope {
    ...
    func persistSessionsToCache(_ sessions: [AforaChatSessionEntry]) {
    ...
    func paintFromCacheIfNeeded(session: SessionSnapshot) {
        ...
        if sessions.isEmpty, !hasAppliedLiveSessions {
            Task { [weak self] in
                ...
                let organized = AforaChatSessionListOrganizer.organize(durableSessions)
    ...
    static func transcriptCacheAgentID(sessionKey: String, agentID: String?) -> String? {
        guard AforaChatSessionKey.agentID(from: sessionKey) == nil else { return nil }
