...
private let cacheLogger = Logger(subsystem: "ai.afora", category: "AforaChatTranscriptCache")

final class OutboxChangeHub: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<AforaChatOutboxChange>.Continuation] = [:]

    func stream() -> AsyncStream<AforaChatOutboxChange> {
        let id = UUID()
        let pair = AsyncStream<AforaChatOutboxChange>.makeStream()
        self.lock.lock()
    ...
    func yield(_ change: AforaChatOutboxChange) {
        self.lock.lock()
...
public actor AforaChatSQLiteTranscriptCache: AforaChatTranscriptCache,
    AforaChatCanonicalTranscriptMerging,
    AforaChatCommandOutbox
{
    ...
    private let databases: AforaClientDatabases
    ...
    init(databases: AforaClientDatabases, gatewayID: String) {
    ...
    public func loadSessions() async -> [AforaChatSessionEntry] {
            ...
            return try await self.databases.cacheQueue.write { db in
                ...
                do {
                    return try rows.map { row in
                        ...
                        return try JSONDecoder().decode(AforaChatSessionEntry.self, from: Data(payload.utf8))
                    }
                } catch {
                    cacheLogger.error(
    ...
    public func loadTranscript(sessionKey: String) async -> [AforaChatMessage] {
        await self.loadTranscript(sessionKey: sessionKey, agentID: nil)
    }

    public func loadTranscript(sessionKey: String, agentID: String?) async -> [AforaChatMessage] {
        guard !self.isRetired else { return [] }
        ...
        do {
            return try await self.databases.cacheQueue.write { db in
                ...
                do {
                    return try rows.map { row in
                        let payload: String = row["payload_json"]
                        return try JSONDecoder().decode(AforaChatMessage.self, from: Data(payload.utf8))
                    }
                } catch {
                    cacheLogger.error(
    ...
    public func storeSessions(_ sessions: [AforaChatSessionEntry]) async {
        guard !self.isRetired else { return }
        ...
        do {
            try await self.databases.cacheQueue.write { db in
                let rows = try Row.fetchAll(
                    ...
                    """,
                    arguments: [gatewayID, sessionKey, normalizedAgentID])
                var cached = rows.compactMap { row -> AforaChatMessage? in
                    let payload: String = row["payload_json"]
                    return try? JSONDecoder().decode(AforaChatMessage.self, from: Data(payload.utf8))
                }
    ...
    public func loadSessionRoutingIdentity() async -> AforaChatSessionRoutingIdentity? {
        guard !self.isRetired else { return nil }
        return self.databases.loadSessionRoutingIdentity(gatewayID: self.gatewayID)
    }

    public func storeSessionRoutingIdentity(_ identity: AforaChatSessionRoutingIdentity) async {
        guard !self.isRetired else { return }
        let gatewayID = self.gatewayID
        do {
...
extension AforaChatSQLiteTranscriptCache {
    ...
    private nonisolated static func replaceTranscript(
        ...
        sessionKey: String,
        agentID: String,
        messages: [AforaChatMessage]) throws
    {
...
extension AforaChatSQLiteTranscriptCache {
    ...
    public nonisolated func changes() -> AsyncStream<AforaChatOutboxChange> {
        self.storeChangeHub.stream()
    }

    public func enqueueCommand(_ command: AforaChatOutboxCommand) async -> Bool {
        ...
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                let scope = AforaChatOutboxScope(
    ...
    public func loadCommands() async -> [AforaChatOutboxCommand] {
        await self.loadCommandsIfAvailable() ?? []
    }

    public func loadCommandsIfAvailable() async -> [AforaChatOutboxCommand]? {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await self.databases.stateQueue.write { db in
    ...
    public func claimNextCommand() async -> AforaChatOutboxCommand? {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> (
                AforaChatOutboxCommand?,
                [AforaChatOutboxScope]) in
                try Self.applyOutboxStaleness(db, gatewayID: gatewayID)
    ...
    public func markCommandAwaitingConfirmation(
        id: String,
        attemptVersion: Int) async -> AforaChatOutboxUpdateResult
    {
        await transitionClaimedCommand(
            id: id,
            attemptVersion: attemptVersion,
    ...
    public func markCommandRetriedIfPresent(
        id: String,
        expectation: AforaChatOutboxRetryExpectation,
        agentID: String?,
        deliverySessionKey: String,
        routingContract: String,
        replacementID: String?) async -> AforaChatOutboxUpdateResult
    {
        ...
        let allowsUntargetedAgent = normalizedRoutingContract == AforaChatOutboxCommand
        ...
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                let retryScope = AforaChatOutboxScope(
    ...
    public func cancelCommand(id: String) async -> AforaChatOutboxUpdateResult {
        guard !self.isRetired else { return .unavailable }
    ...
    public func branchState(
        for scope: AforaChatOutboxScope) async -> AforaChatOutboxBranchState?
    {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                state = try AforaChatOutboxBranchState(
                    epoch: state.epoch,
    ...
    public func beginBranchSwitch(_ scope: AforaChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
    ...
    public func cancelBranchSwitch(_ scope: AforaChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
    ...
    public func demoteBranchSwitchToReconcile(_ scope: AforaChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
            let changed = try await databases.stateQueue.write { db -> Bool in
        ...
        lastError: String) async -> [AforaChatOutboxCommand]?
    {
        ...
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> ([AforaChatOutboxCommand], Bool) in
                try Self.ensureBranchScope(db, gatewayID: gatewayID, scope: scope)
...
extension AforaChatSQLiteTranscriptCache {
    private func transitionClaimedCommand(
        id: String,
        attemptVersion: Int,
        status: AforaChatOutboxCommand.Status,
        retryCount: Int,
        lastError: String?) async -> AforaChatOutboxUpdateResult
    {
    ...
    private nonisolated static func readCommands(
        _ db: Database,
        gatewayID: String) throws -> [AforaChatOutboxCommand]
    {
        let rows = try Row.fetchAll(
            db,
            sql: """
        ...
        gatewayID: String) throws -> AforaChatOutboxCommand
    {
        ...
        guard let status = AforaChatOutboxCommand.Status(rawValue: statusRaw) else {
            throw DatabaseError(message: "unknown outbox status")
        }
        let lastError: String = row["last_error"]
        return AforaChatOutboxCommand(
            id: id,
    ...
    private nonisolated static func ensureBranchScope(
        _ db: Database,
        gatewayID: String,
        scope: AforaChatOutboxScope) throws
    {
        try db.execute(
            sql: """
    ...
    private nonisolated static func readBranchState(
        _ db: Database,
        gatewayID: String,
        scope: AforaChatOutboxScope) throws -> AforaChatOutboxBranchState
    {
        guard let row = try Row.fetchOne(
            db,
            sql: """
        ...
        return AforaChatOutboxBranchState(
    ...
    private nonisolated static func expireBranchSwitchLeases(
        _ db: Database,
        gatewayID: String,
        scope: AforaChatOutboxScope? = nil) throws -> [AforaChatOutboxScope]
    {
        ...
        return rows.map { row in
            AforaChatOutboxScope(sessionKey: row["session_key"], agentID: row["agent_id"])
        }
    }
...
extension AforaChatSQLiteTranscriptCache {
    ...
    static func cacheableMessages(_ messages: [AforaChatMessage]) -> [AforaChatMessage] {
    ...
    private static func cacheablePatchArguments(_ item: AforaChatMessageContent) -> AnyCodable? {
    ...
    static func boundedSessions(_ sessions: [AforaChatSessionEntry]) -> [AforaChatSessionEntry] {
    ...
    private static func attachmentByteCount(_ attachments: [AforaChatOutboxAttachment]) -> Int? {
        var total = 0
