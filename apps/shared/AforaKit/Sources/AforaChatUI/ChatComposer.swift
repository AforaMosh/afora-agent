...
private struct AforaSendableCameraImage: @unchecked Sendable {
    let value: UIImage
}

private struct AforaVideoTransfer: Sendable, Transferable {
    let url: URL
    ...
    static var transferRepresentation: some TransferRepresentation {
        FileRepresentation(contentType: .movie) { video in
            ...
            let destination = FileManager.default.temporaryDirectory
                .appendingPathComponent("afora-picker-video-\(UUID().uuidString)")
                .appendingPathExtension(fileExtension.isEmpty ? "mov" : fileExtension)
            try FileManager.default.copyItem(at: received.file, to: destination)
            return AforaVideoTransfer(url: destination)
        }
    }
}
#endif
...
@MainActor
private struct AforaChatAttachmentCaptureOwner {
    let viewModel: AforaChatViewModel
    let session: AforaChatViewModel.SessionSnapshot
}
...
struct AforaChatComposerPresentationOwner: Equatable {
    let viewModelID: ObjectIdentifier
    let session: AforaChatViewModel.SessionSnapshot

    @MainActor
    init(viewModel: AforaChatViewModel) {
...
@MainActor
struct AforaChatComposer: View {
    @Bindable var viewModel: AforaChatViewModel
    let style: AforaChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: AforaChatView.ComposerChrome
    let isComposerEnabled: Bool
    let isAttachmentInputEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: AforaChatTalkControl?
    let dictationControl: AforaChatDictationControl?
    let voiceNoteControl: AforaChatVoiceNoteControl?
    ...
    @State private var photoPickerOwner: AforaChatAttachmentCaptureOwner?
    @State private var fileImporterOwner: AforaChatAttachmentCaptureOwner?
    #if canImport(UIKit)
    @State private var cameraEncodingTask: Task<Void, Never>?
    @State private var cameraEncodingGeneration = UUID()
    @State private var cameraCaptureOwner: AforaChatAttachmentCaptureOwner?
    #endif
    ...
    private var platformComposer: some View {
        self.styledComposer
            ...
            .fileImporter(
                ...
                allowedContentTypes: AforaChatPickerAttachmentMetadata.allowedFileContentTypes,
            ...
            .fullScreenCover(
                ...
                content: {
                    ...
                    AforaChatCameraPicker { image in
        ...
        VStack(alignment: .leading, spacing: 4) {
            if self.composerChrome == .clean {
                ...
                if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                    AforaVoiceNoteRecordingRow(recorder: voiceNoteControl.recorder)
    ...
    private var presentationOwner: AforaChatComposerPresentationOwner {
        AforaChatComposerPresentationOwner(viewModel: self.viewModel)
    }

    #if !os(macOS)
    private func attachmentCaptureOwner() -> AforaChatAttachmentCaptureOwner {
        AforaChatAttachmentCaptureOwner(
        ...
        if self.composerChrome == .full {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AforaChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AforaChatTheme.composerBorder, lineWidth: 1))
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ...
                    if self.viewModel.showsModelPicker {
                        self.modelPicker
                        if self.viewModel.modelSelectionID != AforaChatViewModel.defaultModelSelectionID {
                            self.modelPinButton
                        }
                    }
                }
    ...
    private func contextUsageIndicator(_ fraction: Double) -> some View {
        ...
        let color = fraction >= 0.8 ? AforaChatTheme.warning : AforaChatTheme.muted
        return ZStack {
            Circle()
                .stroke(AforaChatTheme.muted.opacity(0.2), lineWidth: 2)
            Circle()
            ...
            set: { next in self.viewModel.switchSession(to: next) }))
        {
            ForEach(self.viewModel.sessionChoices, id: \.key) { session in
                Text(session.displayName ?? session.key)
                    .font(AforaChatTypography.mono(size: 12, relativeTo: .caption))
                    .tag(session.key)
            }
        } label: {
            Text("Thread")
    ...
    private func branchTitle(_ branch: AforaChatSessionBranch) -> String {
    ...
    private func branchMetadata(_ branch: AforaChatSessionBranch) -> String {
    ...
    private var fullEditor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(AforaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(AforaChatTheme.composerBorder)))
    ...
    private func talkHelpText(_ talkControl: AforaChatTalkControl) -> String {
...
extension AforaChatComposer {
                      ...
                      self.viewModel.slashCommands.isEmpty
            {
                VStack(alignment: .leading, spacing: 10) {
                    Text("Commands unavailable")
                        .font(AforaChatTypography.footnoteSemiBold)
                    Text(error)
                        .font(AforaChatTypography.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(3)
    ...
    private func selectSlashCommand(_ command: AforaChatCommandChoice) {
...
extension AforaChatComposer {
            ...
            Button {
                ...
                if self.viewModel.isAborting {
                    ...
                    Image(systemName: "stop.fill")
                        .font(AforaChatTypography.display(size: 13, weight: .semibold, relativeTo: .caption))
                }
            }
            .buttonStyle(.plain)
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? AforaChatTheme.userBubble
    }
    ...
    private func pickFilesMac() {
        ...
        panel.allowedContentTypes = AforaChatPickerAttachmentMetadata.allowedFileContentTypes
        panel.begin { resp in
    ...
    private func handleFileImport(
        ...
        owner: AforaChatAttachmentCaptureOwner?)
    {
        guard self.isAttachmentInputEnabled else { return }
        guard let owner,
    ...
    #if canImport(UIKit)
    private func addCameraImage(_ image: UIImage, owner: AforaChatAttachmentCaptureOwner) {
        guard self.isAttachmentInputEnabled else { return }
        guard self.viewModel === owner.viewModel,
              owner.viewModel.isCurrentSession(owner.session)
        else { return }
        self.cancelActiveCameraEncoding()
        let sendableImage = AforaSendableCameraImage(value: image)
    ...
    private func loadPhotosPickerItems(
        _ items: [PhotosPickerItem],
        owner: AforaChatAttachmentCaptureOwner) async
    {
        guard self.isAttachmentInputEnabled else {
            self.pickerItems = []
            return
        ...
        for item in items {
            do {
                ...
                if type.conforms(to: .movie) {
                    guard let transfer = try await item.loadTransferable(type: AforaVideoTransfer.self)
                    else { continue }
                    defer { try? FileManager.default.removeItem(at: transfer.url) }
                    let metadata = AforaChatPickerAttachmentMetadata.resolve(
                        contentType: type,
                    ...
                    let metadata = AforaChatPickerAttachmentMetadata.resolve(contentType: type)
