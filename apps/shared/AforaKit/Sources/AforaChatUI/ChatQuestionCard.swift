...
public enum AforaQuestionCardStatus: Sendable, Equatable {
    case pending
    case submitting
    case answered
...
@MainActor
@Observable
public final class AforaQuestionCardModel: Identifiable {
    ...
    public func status(at date: Date = Date()) -> AforaQuestionCardStatus {
    ...
    public func apply(resolved: AforaQuestionResolvedEvent) {
...
struct AforaQuestionCard: View {
    @Bindable private var model: AforaQuestionCardModel
    private let onSubmit: @MainActor @Sendable (AforaQuestionCardModel) async -> Void
    private let onSkip: (@MainActor @Sendable (AforaQuestionCardModel) async -> Void)?
    #if os(macOS)
    @FocusState private var focusedQuestionID: String?
    #endif

    init(
        model: AforaQuestionCardModel,
        onSubmit: @escaping @MainActor @Sendable (AforaQuestionCardModel) async -> Void,
        onSkip: @escaping @MainActor @Sendable (AforaQuestionCardModel) async -> Void)
    {
        self.model = model
    ...
    private var pendingCard: some View {
        TimelineView(.periodic(from: .now, by: 1)) { context in
            ...
            .padding(16)
            .background(AforaChatTheme.subtleCard, in: RoundedRectangle(cornerRadius: 16))
    ...
    private var terminalSummary: some View {
            ...
            ForEach(self.model.record.questions, id: \.questionid) { question in
                HStack(alignment: .firstTextBaseline, spacing: 5) {
                    Text(verbatim: "\(question.header):")
                        .font(AforaChatTypography.body(size: 14, weight: .semibold, relativeTo: .callout))
                    Text(self.model.terminalSummaryText(for: question))
                        .font(AforaChatTypography.body(size: 14, weight: .regular, relativeTo: .callout))
        ...
        .padding(.vertical, 9)
        .background(AforaChatTheme.subtleCard, in: RoundedRectangle(cornerRadius: 12))
    ...
    private func questionSection(_ question: Question, now: Date) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(question.header.uppercased())
                ...
                .foregroundStyle(AforaChatTheme.accent)
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: selected
                    ...
                    .foregroundStyle(selected ? AforaChatTheme.accent : .secondary)
                VStack(alignment: .leading, spacing: 2) {
                    Text(option.label).font(AforaChatTypography.body)
    ...
    private func footer(now: Date) -> some View {
        ...
        if status == .pending || status == .submitting {
            ...
            if let errorText = self.model.errorText {
                Text(errorText)
                    .font(AforaChatTypography.caption)
                    .foregroundStyle(AforaChatTheme.danger)
            }
        }
    }
...
@MainActor
struct AforaQuestionCards: View {
    let viewModel: AforaChatViewModel

    var body: some View {
        ForEach(self.viewModel.visibleQuestionCards) { card in
            AforaQuestionCard(model: card) { [weak viewModel = self.viewModel] model in
...
extension AforaChatViewModel {
    public var visibleQuestionCards: [AforaQuestionCardModel] {
    ...
    private func fetchMissingQuestionLookups(
        _ models: [AforaQuestionCardModel]) async
        -> [(AforaQuestionCardModel, QuestionLookupResult)]
    {
        ...
        var lookups: [(AforaQuestionCardModel, QuestionLookupResult)] = []
        for model in models {
            do {
    ...
    private func applyQuestionRefresh(
        ...
        lookups: [(AforaQuestionCardModel, QuestionLookupResult)]) -> QuestionRefreshApplyResult
    {
        var changed = false
        for record in records {
            if let model = self.questionCards.first(where: { $0.id == record.id }) {
                ...
                self.questionCards.append(AforaQuestionCardModel(record: record))
                changed = true
            }
        }
        var complete = true
    ...
    func resolveQuestionEvent(_ event: AforaQuestionResolvedEvent) {
    ...
    func submitQuestion(_ model: AforaQuestionCardModel) async {
    ...
    func skipQuestion(_ model: AforaQuestionCardModel) async {
    ...
    func expireQuestionIfNeeded(
        _ model: AforaQuestionCardModel,
        at date: Date = Date())
    {
