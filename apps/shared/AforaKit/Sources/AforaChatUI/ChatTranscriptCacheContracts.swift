...
public protocol AforaChatTranscriptCache: Sendable {
    func loadSessions() async -> [AforaChatSessionEntry]
    func loadTranscript(sessionKey: String) async -> [AforaChatMessage]
    func loadTranscript(sessionKey: String, agentID: String?) async -> [AforaChatMessage]
    func storeSessions(_ sessions: [AforaChatSessionEntry]) async
...
extension AforaChatTranscriptCache {
...
protocol AforaChatCanonicalTranscriptMerging: AforaChatTranscriptCache {
...
public struct AforaChatOutboxScope: Hashable, Sendable {
...
public struct AforaChatOutboxBranchState: Equatable, Sendable {
...
public struct AforaChatOutboxRetryExpectation: Equatable, Sendable {
...
public struct AforaChatOutboxAttachment: Codable, Hashable, Sendable {
...
public struct AforaChatOutboxCommand: Hashable, Sendable, Identifiable {
    ...
    public let attachments: [AforaChatOutboxAttachment]
...
public enum AforaChatOutboxUpdateResult: Equatable, Sendable {
    case updated
    case confirmed
    case missing
    case superseded
...
public enum AforaChatOutboxChange: Equatable, Sendable {
    case canceled(gatewayID: String, id: String)
    case confirmed(gatewayID: String, id: String)
    case invalidated(gatewayID: String, scope: AforaChatOutboxScope)
...
public protocol AforaChatCommandOutbox: Sendable {
    ...
    func demoteBranchSwitchToReconcile(_ scope: AforaChatOutboxScope) async -> Bool
    ...
    func changes() -> AsyncStream<AforaChatOutboxChange>
}

extension AforaChatCommandOutbox {
    public func markCommandQueued(
        id _: String,
        attemptVersion _: Int,
        retryCount _: Int,
        lastError _: String?) async -> AforaChatOutboxUpdateResult
    {
        .unavailable
    }
    ...
    public func markCommandAwaitingConfirmation(
        id _: String,
        attemptVersion _: Int) async -> AforaChatOutboxUpdateResult
    {
        .unavailable
    }
    ...
    public func branchState(for _: AforaChatOutboxScope) async -> AforaChatOutboxBranchState? {
        nil
    }

    public func beginBranchSwitch(_: AforaChatOutboxScope) async -> Bool {
        false
    }

    public func cancelBranchSwitch(_: AforaChatOutboxScope) async -> Bool {
        false
    }

    public func demoteBranchSwitchToReconcile(_: AforaChatOutboxScope) async -> Bool {
        false
    }

    public func reconcileBranchScope(
        _: AforaChatOutboxScope,
        previousState _: AforaChatOutboxBranchState,
        activeLeafEntryID _: String?,
        branchLeafEntryIDs _: Set<String>,
        activeTranscriptEntryIDs _: Set<String>,
        lastError _: String) async -> [AforaChatOutboxCommand]?
    {
        nil
    }
    ...
    public func updateLastActiveLeafEntryID(
        _: String,
        expectedEpoch _: Int,
        for _: AforaChatOutboxScope) async -> Bool
    {
        false
    }
    ...
    public func markCommandRetriedIfPresent(
        id _: String,
        expectation _: AforaChatOutboxRetryExpectation,
        agentID _: String?,
        deliverySessionKey _: String,
        routingContract _: String,
        replacementID _: String? = nil) async -> AforaChatOutboxUpdateResult
    {
        .unavailable
    }
}

public struct AforaChatSessionRoutingIdentity: Equatable, Sendable {
    ...
    public init?(contract: String?) {
        guard let components = AforaChatSessionRoutingContract.parse(contract) else { return nil }
    ...
    public init?(scope: String?, mainSessionKey: String?, defaultAgentID: String?) {
        guard let contract = AforaChatSessionRoutingContract.make(
            scope: scope,
