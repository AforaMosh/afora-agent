...
public enum ChatSessionSidebarModel {
    ...
    static func tree(from sessions: [AforaChatSessionEntry]) -> [Node] {
        ...
        var entriesByKey: [String: AforaChatSessionEntry] = [:]
        ...
        var childrenByParent: [String: [AforaChatSessionEntry]] = [:]
        for session in sessions {
        ...
        func build(_ session: AforaChatSessionEntry, ancestors: Set<String>) -> Node {
    ...
    private static func node(session: AforaChatSessionEntry, children: [Node]) -> Node {
    ...
    public static func displayName(for session: AforaChatSessionEntry) -> String {
    ...
    public static func workSubtitle(for session: AforaChatSessionEntry) -> String? {
    ...
    public static func applying(
        ...
        to sessions: [AforaChatSessionEntry],
        activeAgentId: String? = nil) -> [AforaChatSessionEntry]
    {
        ...
        let candidate = AforaChatSessionObserverDigest(digest)
    ...
    public static func reconcilingGlobalObserverDigestOwner(
        in sessions: [AforaChatSessionEntry],
        activeAgentId: String?) -> [AforaChatSessionEntry]
    {
    ...
    private static func reconcilingGlobalObserverDigestOwner(
        ...
        adoptOwnerless: Bool) -> [AforaChatSessionEntry]
    {
        ...
        updated[index].observerDigest = if digestAgentId == nil, adoptOwnerless {
            AforaChatSessionObserverDigest(
    ...
    private static func observerProjection(
        for change: AforaChatSessionsChangedEvent,
        activeAgentId: String?) -> (digest: AforaChatSessionObserverDigest?, present: Bool)?
    {
    ...
    public static func applying(
        sessionChange change: AforaChatSessionsChangedEvent,
        to sessions: [AforaChatSessionEntry],
        activeAgentId: String? = nil) -> [AforaChatSessionEntry]?
    {
    ...
    private static func visibleObserverDigest(
        for session: AforaChatSessionEntry) -> AforaChatSessionObserverDigest?
    {
    ...
    private static func activeAgentStatus(
        _ status: AforaChatSessionAgentStatus?,
        now: Double) -> AforaChatSessionAgentStatus?
    {
        guard let status,
    ...
    private static func unreadFailureReason(for session: AforaChatSessionEntry) -> String? {
        let status = session.status?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func isRunning(_ session: AforaChatSessionEntry) -> Bool {
        session.hasActiveRun == true ||
            session.status?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "running"
    }

    private static func isNewer(
        _ candidate: AforaChatSessionObserverDigest,
        than previous: AforaChatSessionObserverDigest) -> Bool
    {
    ...
    private static func visibleSessions(
        ...
        query: String) -> [AforaChatSessionEntry]
    {
        ...
        return AforaChatSessionListOrganizer.filter(
            AforaChatSessionListOrganizer.organize(entries),
            search: query)
    }

    private static func placeholder(key: String) -> AforaChatSessionEntry {
