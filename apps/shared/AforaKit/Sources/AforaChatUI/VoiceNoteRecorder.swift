...
public struct AforaVoiceNoteRecording: Equatable, Sendable {
...
@MainActor
@Observable
public final class AforaVoiceNoteRecorder {
    public enum State: Equatable {
        case idle
        case requestingPermission
        case recording(startedAt: Date, fileURL: URL)
        case finished(recording: AforaVoiceNoteRecording)
        case staging(recording: AforaVoiceNoteRecording)
    ...
    public convenience init() {
        self.init(capture: AforaVoiceNoteAudioCapture())
    }

    /// Creates a recorder with an injectable audio capture seam.
    public init(
        capture: any VoiceNoteAudioCapture,
        durationLimit: TimeInterval = AforaVoiceNoteRecorder.maximumDurationSeconds,
    ...
    public var completedRecording: AforaVoiceNoteRecording? {
    ...
    func claimCompletedRecording() -> AforaVoiceNoteRecording? {
        guard case let .finished(recording) = self.state else { return nil }
    ...
    func completeStaging(_ recording: AforaVoiceNoteRecording) {
    ...
    @discardableResult
    public func finish() -> AforaVoiceNoteRecording? {
        guard case let .recording(_, fileURL) = self.state else { return nil }

        self.timerTask?.cancel()
        self.timerTask = nil
        let duration = max(0, self.capture.stop())
        let recording = AforaVoiceNoteRecording(fileURL: fileURL, durationSeconds: duration)
...
@MainActor
public final class AforaVoiceNoteAudioCapture: NSObject, VoiceNoteAudioCapture, AVAudioRecorderDelegate {
    ...
    public func start(url: URL) throws {
        do {
            ...
            guard recorder.record() else {
                throw NSError(
                    domain: "AforaVoiceNoteAudioCapture",
                    code: 1,
...
func aforaVoiceNoteDurationLabel(_ durationSeconds: TimeInterval) -> String {
