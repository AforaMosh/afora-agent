...
struct CleanChatComposerSurface: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay(
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .strokeBorder(AforaChatTheme.composerBorder, lineWidth: 1))
        #else
...
struct AforaChatAttachmentsStrip: View {
    let attachments: [AforaPendingAttachment]
    let onRemove: @MainActor (AforaPendingAttachment.ID) -> Void

    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(self.attachments, id: \AforaPendingAttachment.id) { attachment in
                    HStack(spacing: 6) {
                        if let image = attachment.preview {
                            AforaPlatformImageFactory.image(image)
                                .resizable()
                                .scaledToFill()
                            ...
                            if let duration = attachment.durationSeconds {
                                Text(aforaVoiceNoteDurationLabel(duration))
                                    .font(AforaChatTypography.caption)
                                    .foregroundStyle(.secondary)
                            }
                        } else {
                    ...
                    .padding(.vertical, 5)
                    .background(AforaChatTheme.accent.opacity(0.08))
                    .clipShape(Capsule())
                }
            }
        }
...
#if !os(macOS)
struct AforaChatAttachmentMenu: View {
...
#if canImport(UIKit)
struct AforaChatCameraPicker: UIViewControllerRepresentable {
    ...
    final class Coordinator: NSObject, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
        let parent: AforaChatCameraPicker

        init(parent: AforaChatCameraPicker) {
            self.parent = parent
        }
...
struct AforaChatMicButton: View {
    ...
    private var label: some View {
        ...
        return Image(systemName: showsStop ? "stop.fill" : "mic")
            .font(AforaChatTypography.display(size: 17, weight: .medium, relativeTo: .body))
            .foregroundStyle(showsStop ? AforaChatTheme.accent : .secondary)
            .frame(width: 44, height: 44)
            .contentShape(Rectangle())
    }
    ...
    private func voiceNoteAction(_ voiceNoteControl: AforaChatVoiceNoteControl) -> some View {
        Button {
    ...
    private func isVoiceNoteRecordingEnabled(_ voiceNoteControl: AforaChatVoiceNoteControl) -> Bool {
...
private struct UnifiedChatMicMetadata: ViewModifier {
    let control: AforaChatDictationControl
    let isPending: Bool
