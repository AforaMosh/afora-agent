...
enum ChatTranscriptRow: Hashable, Identifiable {
    ...
    init?(_ message: AforaChatMessage) {
    ...
    static func build(from messages: [AforaChatMessage]) -> [Self] {
        messages.compactMap(Self.init)
    }

    private static func savedTokens(for marker: AforaChatHistoryMarker) -> Double? {
