...
struct ChatMediaAudioAttachment: View {
    ...
    let playback: AforaChatPlaybackMode?
    let resolverReady: Bool
    let playbackAllowed: @MainActor @Sendable () -> Bool
    let load: @MainActor @Sendable (String) async throws -> AforaChatLoadedMedia?
    ...
    private func playbackPositionLabel(_ player: ChatMediaAudioPlayer) -> String {
        let elapsed = aforaVoiceNoteDurationLabel(player.currentTime)
        let total = aforaVoiceNoteDurationLabel(player.duration)
