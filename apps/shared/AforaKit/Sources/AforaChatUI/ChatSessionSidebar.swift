...
struct ChatSessionSidebar: View {
    ...
    @State private var sessionPendingDeletion: AforaChatSessionEntry?
    @State private var sessionPendingRename: AforaChatSessionEntry?
    ...
    @AppStorage("afora.chat.collapsedSessionGroups") private var collapsedSessionGroups = ""
        ...
        return HStack(spacing: 6) {
            VStack(alignment: .leading, spacing: 2) {
                Text(ChatSessionSidebarModel.displayName(for: session))
                    .font(AforaChatTypography.body(size: 13, weight: .medium, relativeTo: .body))
                    .lineLimit(1)
    ...
    private func badges(for node: ChatSessionSidebarModel.Node) -> some View {
        ...
        if node.badges.failedCount > 0 {
            Image(systemName: "exclamationmark.triangle.fill")
                .foregroundStyle(AforaChatTheme.warning)
    ...
    @ViewBuilder
    private func contextMenu(for session: AforaChatSessionEntry) -> some View {
        Button {
    ...
    private func rowSubtitle(for session: AforaChatSessionEntry) -> String? {
        var parts: [String] = []
