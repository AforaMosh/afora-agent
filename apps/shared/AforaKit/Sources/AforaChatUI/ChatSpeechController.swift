...
public struct AforaChatSpeechClip: Equatable, Sendable {
...
public typealias AforaChatSpeechSynthesis =
    @Sendable (_ text: String) async throws -> AforaChatSpeechClip
...
@MainActor
@Observable
public final class AforaChatSpeechController: ChatMediaPlaybackOwner {
    ...
    private let synthesize: AforaChatSpeechSynthesis
    ...
    public convenience init(synthesize: @escaping AforaChatSpeechSynthesis) {
        self.init(
    ...
    init(
        synthesize: @escaping AforaChatSpeechSynthesis,
    ...
    private func run(messageID: UUID, text: String, generation: UInt64) async {
        let clip: AforaChatSpeechClip? = await {
            do {
