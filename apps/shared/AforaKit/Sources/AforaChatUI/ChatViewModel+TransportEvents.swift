...
private let transportEventsLogger = Logger(subsystem: "ai.afora", category: "AforaChatUI")

@MainActor
private final class PendingRunOwnerReference {
    weak var value: AforaChatViewModel?

    init(_ value: AforaChatViewModel) {
        self.value = value
    }
}
...
extension AforaChatViewModel {
    ...
    func handleTransportEvent(_ evt: AforaChatTransportEvent) {
        switch evt {
        case let .health(ok):
    ...
    private func applySessionChangeProjection(
        _ change: AforaChatSessionsChangedEvent,
    ...
    private func handleSessionsChangedEvent(_ change: AforaChatSessionsChangedEvent) {
        ...
        runID: String?) -> LifecycleSessionMergeResult
    {
        ...
        updated[index] = Self.mergedLifecycleSession(
            ...
            phase: phase,
            runID: runID)
        self.sessions = AforaChatSessionListOrganizer.organize(updated)
        self.persistSessionsToCache(self.sessions)
        return .merged
    }

    private func lifecycleSnapshotMatchesEvent(
        _ snapshot: AforaChatSessionEntry,
        change: AforaChatSessionsChangedEvent) -> Bool
    {
    ...
    private func lifecycleSessionIndex(
        _ snapshot: AforaChatSessionEntry,
        change: AforaChatSessionsChangedEvent) -> Int?
    {
    ...
    private static func lifecycleSnapshotRejection(
        snapshot: AforaChatSessionEntry,
        existing: AforaChatSessionEntry,
        phase: String,
        runID: String?) -> LifecycleSessionMergeResult?
    {
        if phase == "start" {
    ...
    private static func mergedLifecycleSession(
        ...
        snapshot: AforaChatSessionEntry,
        phase: String,
        runID: String?) -> AforaChatSessionEntry
    {
        let isTerminal = phase == "end" || phase == "error"
    ...
    private func handleSessionMessageEvent(_ payload: AforaSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: AforaChatEventPayload) {
        ...
        if chat.state == "delta", let runID = explicitRunID {
            ...
            self.adoptRun(
                runId: runID,
                bufferedText: AforaChatEventText.assistantText(from: chat) ?? "")
            return
        }
        ...
        let hapticEvent: AforaChatHaptics.Event? = switch chat.state {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: AforaChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = AforaChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: AforaChatMessage.self)
        }
    ...
    static func isAssistantMessage(_ message: AforaChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: AforaChatMessage) -> AforaChatMessage {
    ...
    private func handleAgentEvent(_ evt: AforaAgentEventPayload) {
        if evt.stream == "usage" {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AforaChatPendingToolCall(
    ...
    private func handleAgentUsageEvent(_ evt: AforaAgentEventPayload) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: AforaAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    func finishPendingRunAfterTerminalOkSendAck(_ response: AforaChatSendResponse) {
    ...
    func finishPendingRunIfTerminalSendAck(_ response: AforaChatSendResponse) -> Bool {
    ...
    private func refreshIfPending(
        ...
        terminalState: AforaChatRunTerminalState? = nil,
    ...
    private func finishPendingRun(runId: String, terminalState: AforaChatRunTerminalState) {
        let hapticEvent: AforaChatHaptics.Event
    ...
    static func hasUnansweredLatestUser(in messages: [AforaChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    static func latestUserTurn(in messages: [AforaChatMessage]) -> LatestUserTurn? {
    ...
    static func userTurn(
        at userIndex: [AforaChatMessage].Index,
        in messages: [AforaChatMessage]) -> LatestUserTurn?
    {
        ...
        -> Bool
    {
        ...
        var occurrence = 0
        var latestMatchingUserIndex: [AforaChatMessage].Index?
    ...
    private static func hasAssistantMessage(
        after userIndex: [AforaChatMessage].Index,
        in messages: [AforaChatMessage]) -> Bool
    {
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [AforaChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
    ...
    private static func assistantHapticEvent(
        for message: AforaChatMessage) -> AforaChatHaptics.Event?
    {
    ...
    func assistantHapticEventAfterLatestUser() -> AforaChatHaptics.Event? {
    ...
    private func assistantHapticEvent(after timestamp: Double) -> AforaChatHaptics.Event? {
    ...
    func refreshHistoriesAfterOutboxFlush(
        ...
        routeLease: AforaChatTransportRouteLease) async
    {
    ...
    private nonisolated static func runPendingRunOwner(
        ...
        armID: UInt64,
        transport: any AforaChatTransport) async
    {
    ...
    private nonisolated static func observePendingRunCompletion(
        ...
        transport: any AforaChatTransport) async
    {
        var terminalState: AforaChatRunTerminalState?
            ...
            armID: armID)
        {
            ...
            let effectiveObservation = terminalState.map(AforaChatRunObservation.terminal) ?? observation
    ...
    private static func processPendingRunObservation(
        ...
        userMessageTimestamp: Double?,
        armID: UInt64,
        observation: AforaChatRunObservation,
    ...
    func clearPendingRun(
        _ runId: String,
        hapticEvent: AforaChatHaptics.Event? = nil)
    {
    ...
    private func removePendingRun(
        _ runId: String,
        hapticEvent: AforaChatHaptics.Event?)
    {
