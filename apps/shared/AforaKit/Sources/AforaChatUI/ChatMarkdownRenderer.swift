...
/// full Afora chat transcript.
@MainActor
public struct AforaChatMarkdownView: View {
...
struct ChatMarkdownRenderer: View {
    ...
    enum Typography: Equatable {
        ...
        var proseFont: Font {
            self == .response
                ? AforaChatTypography.body
                : AforaChatTypography.callout.italic()
        }
        ...
        func headingFont(level: Int) -> Font {
            ...
            case .hierarchy: AforaChatTypography.heading(level: level)
    ...
    struct InlineMathTypography: Equatable {
        static let body = Self(size: AforaChatTypography.bodySize, relativeTo: .body)
    ...
    private var linkColor: Color {
        self.context == .user ? self.textColor : AforaChatTheme.accent
    }
}
...
private struct ChatMarkdownDisclosureView: View {
    ...
    var body: some View {
        ...
        .tint(self.context == .user ? self.textColor : AforaChatTheme.accent)
    }
}
    ...
    var body: some View {
        ForEach(self.images, id: \.id) { item in
            if let img = item.image {
                AforaPlatformImageFactory.image(img)
                    .resizable()
                    .scaledToFit()
                    .frame(maxHeight: 260)
