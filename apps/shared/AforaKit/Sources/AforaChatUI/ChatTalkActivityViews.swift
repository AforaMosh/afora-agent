                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            if !model.partialTranscript.isEmpty {
                                Text(model.partialTranscript)
                                    .font(AforaChatTypography.caption.italic())
    ...
    private func header(model: ChatTalkTranscriptStripModel, showsDisclosure: Bool) -> some View {
        HStack(spacing: 7) {
            ...
            if let collapsedText = model.collapsedText, !self.isExpanded {
                Text(collapsedText)
                    .font(AforaChatTypography.caption)
                    .foregroundStyle(model.partialTranscript.isEmpty ? .secondary : AforaChatTheme.accent)
...
extension View {
    func chatTalkInputDeviceMenu(_ control: AforaChatTalkControl) -> some View {
