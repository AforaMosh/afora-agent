...
struct SlashFilterCache {
    let query: String
    let filter: AforaChatCommandFilter
    let result: [AforaChatCommandChoice]
}

private let chatSendingLogger = Logger(subsystem: "ai.afora", category: "AforaChatUI")
...
extension AforaChatViewModel {
    ...
    public func slashCommandMatches(
        query: String,
        filter: AforaChatCommandFilter) -> [AforaChatCommandChoice]
    {
    ...
    public func applySlashCommandSelection(_ command: AforaChatCommandChoice) {
    ...
    private static func isKnownSlashCommandText(
        _ text: String,
        commands: [AforaChatCommandChoice]) -> Bool
    {
    ...
    private static func commands(
        _ commands: [AforaChatCommandChoice],
        ...
        filter: AforaChatCommandFilter) -> [AforaChatCommandChoice]
    {
        ...
        let effectiveFilter: AforaChatCommandFilter =
            self.queryTargetsSkills(trimmed) && filter == .all ? .skills : filter
        return commands.enumerated()
            .compactMap { index, command -> (Int, Int, AforaChatCommandChoice)? in
    ...
    private static func command(
        _ command: AforaChatCommandChoice,
        isIncludedIn filter: AforaChatCommandFilter) -> Bool
    {
        switch filter {
        case .all:
            true
    ...
    private struct SendDraft {
        ...
        let replyTarget: AforaChatReplyTarget?
    ...
    private struct LiveSendAttempt {
        ...
        let encodedAttachments: [AforaChatAttachmentPayload]
        ...
        mustPreserveOutboxOrder: Bool) async -> AttachmentPersistenceDecision
    {
        ...
        guard !mustPreserveOutboxOrder else {
            ...
            errorText = reason ?? AforaChatTransportUpgradeMessage.routingContract
            return .stop
        }
    ...
    private static func userContent(
        messageText: String,
        attachments: [AforaPendingAttachment],
        encodedAttachments: [AforaChatAttachmentPayload]) -> [AforaChatMessageContent]
    {
        var content: [AforaChatMessageContent] = [
    ...
    private func handleLiveSendResponse(
        _ response: AforaChatSendResponse,
    ...
    private func handleLiveSendFailure(_ error: Error, attempt: LiveSendAttempt) async {
        ...
        if attempt.encodedAttachments.isEmpty, !(error is GatewayResponseError) {
            ...
            let deliveryIsAmbiguous = !(error is AforaChatTransportSendError)
