...
extension AforaChatViewModel {
    ...
    func performSelectThinkingLevel(_ level: String) {
        ...
        if acceptedThinkingLevelsByTarget[target] == nil {
            ...
            acceptedSettingsPatchResultsByTarget[target] = AforaChatModelPatchResult(
    ...
    private func mergedThinkingPatchSuccess(
        _ patchResult: AforaChatModelPatchResult?,
        acceptedLevel: String,
        target: ModelPatchTarget) -> AforaChatModelPatchResult
    {
    ...
    func updateCurrentSessionThinkingLevels(
        _ thinkingLevels: [AforaChatThinkingLevelOption],
        sessionKey: String,
    ...
    func inactiveSettingsStateKey(for target: ModelPatchTarget) -> String? {
        if target.agentID != nil || target.sessionRoutingContract != nil {
            guard AforaChatSessionKey.agentID(from: target.canonicalSessionKey) != nil else { return nil }
        }
    ...
    private func thinkingLevelWithoutGatewayMetadata(
        _ level: String,
        target: ModelPatchTarget,
        session: AforaChatSessionEntry?) -> String?
    {
    ...
    private func thinkingPickerIsAvailable(
        ...
        modelChoice: AforaChatModelChoice?) -> Bool
    {
        let resolved = self.resolvedThinkingLevelOptions(for: session)
    ...
    private struct ThinkingLevelOptionsResolution {
        let options: [AforaChatThinkingLevelOption]
        let isGatewayMetadata: Bool
    }

    private func resolvedThinkingLevelOptions(
        for currentSession: AforaChatSessionEntry?) -> ThinkingLevelOptionsResolution
    {
    ...
    private func selectedModelChoice(
        for currentSession: AforaChatSessionEntry?) -> AforaChatModelChoice?
    {
    ...
    private func modelChoice(modelID: String?, provider: String?) -> AforaChatModelChoice? {
    ...
    private static func sessionModelMatchesDefaults(
        ...
        defaults: AforaChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [AforaChatThinkingLevelOption]?) -> [AforaChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return AforaChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [AforaChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return AforaChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    static func withCurrentThinkingOption(
        _ options: [AforaChatThinkingLevelOption],
        current: String) -> [AforaChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [AforaChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [AforaChatThinkingLevelOption]) -> [AforaChatThinkingLevelOption]
    {
        var result: [AforaChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    static func normalizedThinkingLevel(
        _ level: String?,
        options: [AforaChatThinkingLevelOption],
