...
extension AforaChatViewModel {
    nonisolated static func chatContextUsageFraction(for session: AforaChatSessionEntry?) -> Double? {
    ...
    func currentSessionEntry() -> AforaChatSessionEntry? {
    ...
    func sessionMutationIdentity(for key: String, listedKey: String? = nil) -> String {
        ...
        return self.modelPatchTarget(
            ...
            agentID: AforaChatSessionKey.agentID(from: key) ?? self.activeAgentId,
            sessionRoutingContract: nil).canonicalSessionKey
    }

    func applyingLocalUnreadOverrides(
        to sessions: [AforaChatSessionEntry]) -> [AforaChatSessionEntry]
    {
        ...
        sessionRoutingContract: String?) -> ModelPatchTarget
    {
        ...
        if AforaChatSessionKey.agentID(from: candidate) != nil || normalizedCandidate == "unknown" {
            targetKey = candidate
            targetAgentID = nil
    ...
    func sessionEntryForThinking(
        ...
        agentID: String?) -> AforaChatSessionEntry?
    {
    ...
    func successfulModelPatchResult(
        ...
        session: AforaChatSessionEntry?) -> AforaChatModelPatchResult?
    {
        guard let session,
    ...
    func placeholderSession(key: String) -> AforaChatSessionEntry {
    ...
    public var sessionChoices: [AforaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = AforaChatSessionListOrganizer.organize(sessions)
        let mainSessionKey = resolvedMainSessionKey

        var result: [AforaChatSessionEntry] = []
