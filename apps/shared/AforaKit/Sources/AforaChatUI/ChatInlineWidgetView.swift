...
enum AforaChatWidgetSurfaceRole: Sendable, Hashable {
    case node
    case operatorSurface
    case legacy
}

public struct AforaChatWidgetResource: Sendable, Equatable {
    public let url: URL
    public let tlsFingerprintSHA256: String?
    let surfaceRole: AforaChatWidgetSurfaceRole
    let attemptedSurfaceRoles: Set<AforaChatWidgetSurfaceRole>
    ...
    init(
        ...
        surfaceRole: AforaChatWidgetSurfaceRole,
        attemptedSurfaceRoles: Set<AforaChatWidgetSurfaceRole>)
    {
        self.url = url
        self.tlsFingerprintSHA256 = tlsFingerprintSHA256
...
public enum AforaChatWidgetURLResolver {
    private static let documentsPath = "/__afora__/canvas/documents"
    ...
    public static func resolveResource(
        target: String,
        replacing failedResource: AforaChatWidgetResource?,
    ...
    private static func capabilitySurface(_ rawSurfaceURL: String?) -> URLComponents? {
        ...
        guard segments.count >= 3,
              segments[segments.count - 3] == "__afora__",
    ...
    private static func resolve(
        ...
        role: AforaChatWidgetSurfaceRole,
        target: String,
        attemptedRoles: Set<AforaChatWidgetSurfaceRole>) -> AforaChatWidgetResource?
    {
        guard let url = resolve(surfaceURL: surface.url, target: target) else { return nil }
        let resource = AforaChatWidgetResource(
            url: url,
    ...
    private static func resolvePreferred(
        ...
        target: String,
        excluding failedResource: AforaChatWidgetResource?,
        blockedRoles: Set<AforaChatWidgetSurfaceRole>,
        attemptedRoles: Set<AforaChatWidgetSurfaceRole>) -> AforaChatWidgetResource?
    {
        [
            (role: AforaChatWidgetSurfaceRole.node, surface: surfaces.node),
            (role: AforaChatWidgetSurfaceRole.operatorSurface, surface: surfaces.operatorSurface),
        ]
            .lazy
    ...
    private static func isReplacement(
        _ candidate: AforaChatWidgetResource,
        for failedResource: AforaChatWidgetResource?) -> Bool
    {
...
struct ChatInlineWidgetView: View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            if let title = self.preview.title?.trimmingCharacters(in: .whitespacesAndNewlines), !title.isEmpty {
                Text(title)
                    ...
                    .fontWeight(.semibold)
                    .foregroundStyle(AforaChatTheme.muted)
            }
    ...
    #if canImport(WebKit) && (os(iOS) || os(macOS))
    private func renderedWidget(resource: AforaChatWidgetResource) -> some View {
        ChatInlineWidgetWebView(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(AforaChatTheme.muted.opacity(0.24), lineWidth: 1)
            }
    ...
    private func handleLoadFailure(resource: AforaChatWidgetResource) {
...
extension AforaChatWidgetResource {
...
private final class ChatInlineWidgetNavigationDelegate: NSObject, WKNavigationDelegate {
    var resource: AforaChatWidgetResource {
        didSet {
    ...
    init(
        resource: AforaChatWidgetResource,
