...
public struct AforaChatDisplayOptions: OptionSet, Sendable {
...
@MainActor
public struct AforaChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private let displayOptions: AforaChatDisplayOptions
    ...
    private let talkControl: AforaChatTalkControl?
    private let dictationControl: AforaChatDictationControl?
    private let voiceNoteControl: AforaChatVoiceNoteControl?
    ...
    public init(
        ...
        displayOptions: AforaChatDisplayOptions? = nil,
        ...
        talkControl: AforaChatTalkControl? = nil,
        dictationControl: AforaChatDictationControl? = nil,
        voiceNoteControl: AforaChatVoiceNoteControl? = nil,
        speech: AforaChatSpeechController? = nil,
    ...
    private var swarmProgress: some View {
        ...
        if !groups.isEmpty {
            AforaChatSwarmProgressView(groups: groups)
        }
    }
    ...
    private var messageListRows: some View {
        ...
        AforaQuestionCards(viewModel: self.viewModel)
    ...
    private func isListenable(_ msg: AforaChatMessage) -> Bool {
    ...
    private func speechChipIsPreparing(
        _ speech: AforaChatSpeechController,
        messageID: UUID) -> Bool?
    {
        switch speech.phase {
    ...
    private var jumpToLatestButton: some View {
        Button {
            ...
            Image(systemName: "arrow.down")
                ...
                .background(
                    Circle()
                        .fill(AforaChatTheme.subtleCard)
        ...
        .buttonStyle(.plain)
        .foregroundStyle(AforaChatTheme.assistantText)
...
extension AforaChatView {
    private func mergeToolResults(in messages: [AforaChatMessage]) -> [AforaChatMessage] {
        var result: [AforaChatMessage] = []
    ...
    private func isToolResultMessage(_ message: AforaChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: AforaChatMessage) -> Bool {
    ...
    private func primaryText(in message: AforaChatMessage) -> String {
        ChatMessageVisibleText.displayText(
            in: message,
            includeThinking: self.displayOptions.contains(.reasoning))
    }

    private func hasInlineAttachments(in message: AforaChatMessage) -> Bool {
        message.content.contains(where: \.isInlineAttachment)
    }

    private func toolCalls(in message: AforaChatMessage) -> [AforaChatMessageContent] {
    ...
    private func inlineToolResults(in message: AforaChatMessage) -> [AforaChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: AforaChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: AforaChatMessage) -> String {
        self.primaryText(in: message)
    }

    @ViewBuilder
    private func copyMessageButton(for message: AforaChatMessage) -> some View {
    ...
    @ViewBuilder
    private func openFullMessageButton(for message: AforaChatMessage) -> some View {
    ...
    @ViewBuilder
    private func rewindMessageButton(for message: AforaChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if role == "user",
    ...
    @ViewBuilder
    private func forkMessageButton(for message: AforaChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if role == "user",
           message.transcriptMessageID?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
        {
    ...
    @ViewBuilder
    private func replyMessageButton(for message: AforaChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
...
private struct ChatAssistantIntroCard: View {
    let text: String
    let prompts: [AforaChatView.StarterPrompt]
    let onPrompt: (AforaChatView.StarterPrompt) -> Void
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(self.text)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .fill(AforaChatTheme.assistantBubble))
            ...
            ForEach(self.prompts) { prompt in
                Button {
                    ...
                    HStack(spacing: 8) {
                        Text(prompt.title)
                            .font(AforaChatTypography.body(size: 15, weight: .semibold, relativeTo: .callout))
                            .multilineTextAlignment(.leading)
                        Spacer(minLength: 8)
    ...
    var body: some View {
        ...
        ContentUnavailableView {
            Label(self.title, systemImage: self.systemImage)
                .font(AforaChatTypography.headline)
        } description: {
            Text(self.message)
                .font(AforaChatTypography.body)
        } actions: {
            if let actionTitle, let action {
                Button(action: action) {
                    Text(actionTitle)
                        .font(AforaChatTypography.body(size: 15, weight: .semibold, relativeTo: .subheadline))
                }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: self.systemImage)
                .font(AforaChatTypography.display(size: 15, weight: .semibold, relativeTo: .subheadline))
