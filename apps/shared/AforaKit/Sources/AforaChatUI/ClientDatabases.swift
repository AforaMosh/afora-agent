...
private let databaseLogger = Logger(subsystem: "ai.afora", category: "AforaClientDatabases")
...
public final class AforaClientDatabases: @unchecked Sendable {
    ...
    public func store(gatewayID: String) -> AforaChatSQLiteTranscriptCache {
        AforaChatSQLiteTranscriptCache(databases: self, gatewayID: gatewayID)
    }
    ...
    public func loadSessionRoutingIdentity(
        gatewayID: String) -> AforaChatSessionRoutingIdentity?
    {
        do {
            return try self.stateQueue.read { db in
                ...
                return AforaChatSessionRoutingIdentity(
                    scope: row["scope"],
...
extension AforaClientDatabases {
    ...
    private static func openStateDatabase(at url: URL) throws -> DatabaseQueue {
        let queue = try DatabaseQueue(
            path: url.path,
            configuration: self.configuration(label: "Afora.client-state"))
    ...
    private static func openRepairableCacheDatabase(at url: URL) throws -> DatabaseQueue {
        do {
            let queue = try DatabaseQueue(
                path: url.path,
                configuration: self.configuration(label: "Afora.gateway-cache"))
...
extension AforaClientDatabases {
    ...
    private struct LegacyCommand {
        ...
        var agentID: String
        var text: String
        var attachments: [AforaChatOutboxAttachment]
        var thinking: String
    ...
    private static func readLegacySnapshot(at url: URL) throws -> LegacySnapshot? {
        var configuration = Configuration()
        configuration.label = "Afora.legacy-chat-import"
            ...
            if try db.tableExists("outbox_commands") {
                ...
                for row in rows {
                    ...
                    let attachments = try JSONDecoder().decode(
                        [AforaChatOutboxAttachment].self,
                        from: Data(attachmentsJSON.utf8))
                    let originalStatus: String = row["status"]
                    guard AforaChatOutboxCommand.Status(rawValue: originalStatus) != nil else {
                    ...
                    let status = lacksVerifiedTarget ? AforaChatOutboxCommand.Status.failed.rawValue : originalStatus
                    let lastError: String = if lacksVerifiedTarget {
                        if originalStatus == AforaChatOutboxCommand.Status.sending.rawValue ||
                            originalStatus == AforaChatOutboxCommand.Status.awaitingConfirmation.rawValue ||
                            originalError == AforaChatSQLiteTranscriptCache.outboxUnconfirmedError
                        {
                            AforaChatSQLiteTranscriptCache.outboxUnconfirmedError
                        } else {
                            AforaChatSQLiteTranscriptCache.outboxUnknownTargetError
                        }
                    } else {
                        originalError
                    }
