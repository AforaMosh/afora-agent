...
extension AforaChatViewModel {
    ...
    public func isDefaultModel(_ model: AforaChatModelChoice) -> Bool {
        ...
        self.enqueueSessionSettingsPatch(requestID: requestID, target: target) { [weak self] routeLease in
            ...
            do {
                ...
                let result = try await routeLease.patchSessionSettings(
                    ...
                    patch: AforaChatSessionSettingsPatch(verboseLevel: .some(next)))
        ...
        self.enqueueSessionSettingsPatch(requestID: requestID, target: target) { [weak self] routeLease in
            ...
            do {
                ...
                let result = try await routeLease.patchSessionSettings(
                    sessionKey: target.canonicalSessionKey,
                    agentID: target.agentID,
                    patch: AforaChatSessionSettingsPatch(fastMode: .some(next)))
    ...
    func applyModelControlPatchResult(
        _ result: AforaChatModelPatchResult,
        sessionKey: String,
    ...
    private func recordModelControlPatchSuccess(
        ...
        fastModeOverride: AforaChatFastMode?? = nil,
        ...
        verboseLevelOverride: String?? = nil)
    {
        ...
        let recordedFastMode: AforaChatFastMode? = if let fastModeOverride {
            fastModeOverride
        } else {
        ...
        self.lastSuccessfulSettingsPatchResultsByTarget[target] = AforaChatModelPatchResult(
    ...
    private func updateCurrentSessionFastMode(
        _ mode: AforaChatFastMode?,
        effective: AforaChatFastMode?,
        sessionKey: String,
        exactMatchOnly: Bool = false)
    {
        let index = exactMatchOnly
