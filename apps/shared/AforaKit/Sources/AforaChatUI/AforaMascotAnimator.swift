...
public enum AforaMascotMood: String, CaseIterable, Equatable, Sendable {
...
enum AforaMascotEffect: Equatable {
    case none
    case sparkles
    case hearts
    case zzz
...
final class AforaMascotAnimator {
    ...
    private(set) var mood: AforaMascotMood = .idle
    ...
    private var activeGesture: AforaMascotGesture?
    private var activeGestureStart: TimeInterval = 0
    private var pendingGesture: AforaMascotGesture?
    ...
    private var lastClickReaction: AforaMascotGesture?
    ...
    private var requestedAccessory: AforaMascotAccessory = .none
    ...
    func setMood(_ mood: AforaMascotMood, at time: TimeInterval) {
    ...
    func setAccessory(_ accessory: AforaMascotAccessory, at time: TimeInterval) {
    ...
    func handleTap(at time: TimeInterval) {
        ...
        var reactions: [AforaMascotGesture] = [.hop, .wave, .wink]
    ...
    func pose(at time: TimeInterval) -> AforaMascotPose {
        ...
        let effectiveMood: AforaMascotMood = dozing ? .sleepy : self.mood
        ...
        if self.requestedAccessory != .none {
            ...
            pose.accessoryAmount = AforaMascotGesture.easeOut(progress)
        }
    ...
    private func advanceSchedules(at time: TimeInterval) {
        ...
        self.blinkStarts.removeAll { time - $0 > AforaMascotGesture.blinkDuration }
    ...
    private func randomQuirk() -> AforaMascotGesture {
    ...
    private func startGesture(_ gesture: AforaMascotGesture, at time: TimeInterval) {
        self.activeGesture = gesture
        self.activeGestureStart = time
    }

    private static func entranceGesture(for mood: AforaMascotMood) -> AforaMascotGesture? {
        switch mood {
        case .happy: .hop
    ...
    private func basePose(for mood: AforaMascotMood, at time: TimeInterval) -> AforaMascotPose {
        ...
        case .working:
            ...
            if phase < 0.05 {
                ...
                pose.rightClawDegrees = -6 - 28 * AforaMascotGesture.easeInOut((phase - 0.05) / 0.55)
            } else if phase < 0.72 {
                let strike = ((phase - 0.60) / 0.12).clamped(to: 0...1)
                pose.rightClawDegrees = -34 + 46 * strike * strike
            } else {
                pose.rightClawDegrees = 12 - 18 * AforaMascotGesture.easeInOut((phase - 0.72) / 0.28)
            }
            pose.leftClawDegrees = 4 + 2 * sin(2 * .pi * phase)
            let impact = AforaMascotGesture.bell(((phase - 0.72) / 0.14).clamped(to: 0...1))
    ...
    private func applyGaze(
        _ pose: inout AforaMascotPose,
        mood: AforaMascotMood,
    ...
    private func applyBlinks(_ pose: inout AforaMascotPose, at time: TimeInterval) {
        guard pose.happyEyes < 0.6 else { return }
        for start in self.blinkStarts {
            let progress = (time - start) / AforaMascotGesture.blinkDuration
            guard progress >= 0, progress <= 1 else { continue }
            let closure = AforaMascotGesture.bell(CGFloat(progress))
    ...
    private func applyDizzy(_ pose: inout AforaMascotPose, at time: TimeInterval) {
...
enum AforaMascotGesture: Equatable {
    case wave
    case hop
    case wink
    case celebrate
    ...
    func apply(to pose: inout AforaMascotPose, progress p: CGFloat) {
        switch self {
        case .wave:
        ...
        case .sneeze:
            if p < 0.42 {
                let inhale = AforaMascotGesture.easeInOut(p / 0.42)
                ...
                let recover = 1 - AforaMascotGesture.easeInOut((p - 0.58) / 0.42)
        ...
        case .sigh:
            let rise = AforaMascotGesture.easeInOut((p / 0.3).clamped(to: 0...1))
            let fall = AforaMascotGesture.easeInOut(((p - 0.3) / 0.45).clamped(to: 0...1))
