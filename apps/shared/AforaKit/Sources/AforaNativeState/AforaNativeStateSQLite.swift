...
public struct AforaNativeStateError: Error, LocalizedError, Sendable {
...
public enum AforaNativeStateCanonicalTable: Sendable {
...
public enum AforaNativeStateSQLiteStep: Equatable, Sendable {
    case row
    case done
}

public enum AforaNativeStateSQLiteValueType: Equatable, Sendable {
    case integer
    case float
    case text
    case blob
...
public final class AforaNativeStateSQLite: @unchecked Sendable {
    // Keep aligned with AFORA_STATE_SCHEMA_VERSION. Native clients never upgrade this database.
    ...
    private static let canonicalTables = [
        AforaNativeStateSQLite.deviceAuthTokens,
        AforaNativeStateSQLite.deviceIdentities,
        AforaNativeStateSQLite.execApprovalsConfig,
        AforaNativeStateSQLite.macosPortGuardianRecords,
    ]
        ...
        createIfMissing: Bool = true) throws
    {
        ...
        guard result == SQLITE_OK, let database else {
            ...
            throw AforaNativeStateError("Could not open native state database: \(detail)")
        }
    ...
    public func ensureCanonicalTable(
        _ table: AforaNativeStateCanonicalTable,
        ...
        try self.withConnectionLock {
            ...
            if userVersion == 0 {
                ...
                guard try self.schemaObjectExists(type: "table", name: descriptor.name) else {
                    throw AforaNativeStateError(
                        "Versioned Afora state database is missing \(descriptor.name)")
                }
            }
    ...
    public func validateCanonicalTable(_ table: AforaNativeStateCanonicalTable) throws {
        try self.withConnectionLock {
            try self.validateTableShape(Self.descriptor(table))
        }
    }

    public func prepare(_ sql: String) throws -> AforaNativeStateSQLiteStatement {
        try self.withConnectionLock {
            ...
            return AforaNativeStateSQLiteStatement(connection: self, statement: statement)
        }
    }
    ...
    public func execute(_ sql: String) throws {
        try self.withConnectionLock {
            ...
            guard result == SQLITE_OK else {
                ...
                throw AforaNativeStateError("SQLite operation failed: \(detail)")
            }
        }
    }
    ...
    public func scalarInt64(_ sql: String) throws -> Int64 {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .row,
                ...
                throw AforaNativeStateError("SQLite integer query did not return one integer row")
            }
            let value = statement.int64(at: 0)
            guard try statement.step() == .done else {
                throw AforaNativeStateError("SQLite integer query returned multiple rows")
            }
            return value
        }
    }
    ...
    public func scalarText(_ sql: String) throws -> String? {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .done else {
                throw AforaNativeStateError("SQLite text query returned multiple rows")
            }
            return value
        }
    }
    ...
    public func schemaObjectExists(type: String, name: String) throws -> Bool {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .done else {
                throw AforaNativeStateError("SQLite schema query returned multiple rows")
            }
            return true
        }
    }

    fileprivate func databaseError(operation: String) -> AforaNativeStateError {
        AforaNativeStateError(
    ...
    private static func descriptor(_ table: AforaNativeStateCanonicalTable) -> CanonicalTable {
        switch table {
    ...
    private func validateVersionZeroOwnership() throws {
        ...
        guard objects.isSubset(of: allowedObjects) else {
            throw AforaNativeStateError(
                "Schema version zero database contains objects not owned by a native Afora store")
        }
    ...
    private func validateSharedDatabaseMetadata(userVersion: Int64) throws {
        guard try self.schemaObjectExists(type: "table", name: "schema_meta") else {
            throw AforaNativeStateError("Versioned Afora state database is missing schema_meta")
        }
        let statement = try self.prepare(
        ...
        guard try statement.step() == .row,
            ...
            throw AforaNativeStateError(
                "Afora state database schema metadata does not match its global schema version")
        }
    }

    private func validateTableShape(_ table: CanonicalTable) throws {
        guard try self.tableColumns(table.name) == table.columns else {
            throw AforaNativeStateError("\(table.name) has an incompatible schema")
        }
        ...
        guard normalizedTableSQL.hasSuffix(") STRICT") else {
            throw AforaNativeStateError("\(table.name) must be a STRICT table")
        }
        guard try self.validRequiredIndex(table) else {
            throw AforaNativeStateError("\(table.indexName ?? table.name) has an incompatible index schema")
        }
    }
...
public final class AforaNativeStateSQLiteStatement {
    private let connection: AforaNativeStateSQLite
    private let statement: OpaquePointer

    fileprivate init(connection: AforaNativeStateSQLite, statement: OpaquePointer) {
    ...
    public func step() throws -> AforaNativeStateSQLiteStep {
    ...
    public func valueType(at column: Int32) -> AforaNativeStateSQLiteValueType {
    ...
    public func requiredText(at column: Int32, field: String) throws -> String {
        try self.connection.withConnectionLock {
            guard self.valueType(at: column) == .text,
                ...
                throw AforaNativeStateError("SQLite \(field) must be text")
            }
