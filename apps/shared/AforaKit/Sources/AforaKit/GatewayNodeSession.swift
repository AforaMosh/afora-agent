...
public actor GatewayNodeSession {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "node.gateway")
    ...
    // periphery:ignore - Shipped public refresh API; keep until a breaking AforaKit window.
    @discardableResult
...
extension GatewayNodeSession {
        ...
        -> BridgeInvokeResponse
    {
        ...
        let requiresRouteScopedCancellation = request.command == "computer.act" ||
            request.command == AforaCameraCommand.ptzControl.rawValue ||
            AforaTalkCommand(rawValue: request.command) != nil
    ...
    private func decodeInvokeRequest(from payload: AforaProtocol.AnyCodable) throws -> NodeInvokeRequestPayload {
        try self.decodeEventPayload(from: payload)
    }

    private func decodeEventPayload<T: Decodable>(from payload: AforaProtocol.AnyCodable) throws -> T {
        do {
