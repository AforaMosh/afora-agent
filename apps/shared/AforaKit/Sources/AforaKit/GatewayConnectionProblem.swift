            ...
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        ...
                        fallback: "https://docs.afora.ai/gateway/tailscale"),
        ...
        case .authRateLimited:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.afora.ai/gateway/troubleshooting"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        ...
        case .authTokenMissing:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.afora.ai/gateway/authentication"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        ...
        case .authTokenNotConfigured:
            self.problem(
                .init(
                    ...
                    actionCommand: authError.actionCommand
                        ?? "afora config set gateway.auth.token <new-token>",
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.afora.ai/gateway/authentication"),
                    requestId: authError.requestId,
        ...
        case .authPasswordNotConfigured:
            self.problem(
                .init(
                    ...
                    actionLabel: authError.actionLabel ?? "Fix on gateway",
                    actionCommand: authError.actionCommand
                        ?? "afora config set gateway.auth.password <new-password>",
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.afora.ai/gateway/authentication"),
                    requestId: authError.requestId,
        ...
        case .authBootstrapTokenInvalid:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.afora.ai/platforms/ios"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        case .authDeviceTokenMismatch:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.afora.ai/gateway/pairing"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
...
extension GatewayConnectionProblemMapper {
    ...
    private static func map(_ tlsError: GatewayTLSValidationError) -> GatewayConnectionProblem {
        ...
        case .pinMismatch:
            ...
            return GatewayConnectionProblem(
                ...
                actionCommand: nil,
                docsURL: URL(string: "https://docs.afora.ai/gateway/troubleshooting"),
                retryable: false,
                pauseReconnect: true,
        ...
        case .certificateUnavailable:
            return GatewayConnectionProblem(
                ...
                message: "Afora could not read the gateway certificate for \(failure.host).",
                actionLabel: "Retry",
                messagePresentation: .localizedFormat(
                    "Afora could not read the gateway certificate for %@.",
                    [failure.host]),
                actionCommand: nil,
                docsURL: URL(string: "https://docs.afora.ai/gateway/troubleshooting"),
                retryable: true,
        ...
        case .pinStorageUnavailable:
            return GatewayConnectionProblem(
                ...
                owner: .unknown,
                title: "Gateway certificate unavailable",
                message: "Afora could not securely save the TLS certificate pin for \(failure.host).",
                actionLabel: "Retry",
                titlePresentation: .localized("Gateway certificate unavailable"),
                messagePresentation: .localizedFormat(
                    "Afora could not securely save the TLS certificate pin for %@.",
                    [failure.host]),
        ...
        technicalDetails: String) -> GatewayConnectionProblem
    {
        ...
        case .reachabilityFailed:
            (
                ...
                "Afora could not reach the gateway over the current network.",
                "Check network")
        case .websocketCancelled:
    ...
    private static func protocolMismatchProblem(for authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
           ...
           clientMax < expected
        {
            ...
            message = authError.userMessageOverride
                ?? "This app is older than the gateway. Update Afora on this device, then retry."
            owner = .iphone
                  ...
                  clientMin > expected
        {
            ...
            message = authError.userMessageOverride
                ?? "The gateway is older than this app. Update Afora on the gateway host, then retry."
            owner = .gateway
            actionLabel = authError.actionLabel ?? "Copy update command"
            actionCommand = authError.actionCommand ?? "afora update"
        } else {
            title = authError.titleOverride ?? "Afora update required"
            message = authError.userMessageOverride
                ?? "The app and gateway use incompatible protocol versions. Update Afora on both, then retry."
            owner = .both
            actionLabel = authError.actionLabel ?? "Update Afora"
            actionCommand = authError.actionCommand
        }
        return self.problem(
            .init(
    ...
    private static func approvalCommand(requestId: String?) -> String {
        self.nonEmpty(requestId).map { "afora devices approve \($0)" }
            ?? "afora devices list"
    }
