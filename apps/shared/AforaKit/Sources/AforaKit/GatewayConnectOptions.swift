...
public enum AforaGatewayClientCapability {
...
extension GatewayConnectOptions {
    ...
    func applyOptionalConnectParams(to params: inout [String: AforaProtocol.AnyCodable]) {
        if !self.commands.isEmpty {
            params["commands"] = AforaProtocol.AnyCodable(self.commands)
        }
        ...
        if let pathEnv = self.pathEnv?.trimmingCharacters(in: .whitespacesAndNewlines),
           !pathEnv.isEmpty
        {
            params["pathEnv"] = AforaProtocol.AnyCodable(pathEnv)
        }
        if !self.permissions.isEmpty {
            params["permissions"] = AforaProtocol.AnyCodable(self.permissions)
        }
    }
}
