...
enum DeviceIdentityPaths {
    private static let stateDirEnv = ["AFORA_STATE_DIR"]
    ...
    private static let appGroupStateDirAvailable =
        DeviceIdentityPaths.hasAppGroupEntitlement(AforaAppGroup.identifier)
        ...
        temporaryDirectory: URL) -> URL
    {
        ...
        return temporaryDirectory.appendingPathComponent("afora", isDirectory: true)
    }

    private static func stateDirOverrideURL() -> URL? {
        // Test-scoped stores must win over the process environment. Parallel Swift tests
        // otherwise race whenever another suite temporarily swaps AFORA_STATE_DIR.
    ...
    static func legacyStateDirURL() -> URL? {
        if let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            return appSupport.appendingPathComponent("Afora", isDirectory: true)
        }
        return nil
    }
    ...
    static func appGroupStateDirURL() -> URL? {
        guard
            let containerURL = FileManager.default
                .containerURL(forSecurityApplicationGroupIdentifier: AforaAppGroup.identifier)
        else {
            return nil
        }
        return containerURL.appendingPathComponent("Afora", isDirectory: true)
    }
...
public enum DeviceIdentityStore {
    ...
    static func storageError(_ message: String) -> NSError {
        NSError(
            domain: "ai.afora.device-identity-store",
            code: 1,
    ...
    public static func loadOrCreate(profile: GatewayDeviceIdentityProfile) -> DeviceIdentity {
        do {
            ...
            preconditionFailure("Could not persist the Afora device identity: \(error.localizedDescription)")
        }
    }
