...
public enum ShareGatewayRelaySettings {
    ...
    private static let relayCredentialService = "ai.aforafoundation.app.share-gateway-relay"
        ...
        if config.token != nil || config.password != nil {
            return self.resolveLegacyConfig(
                ...
                discard: {
                    ...
                    self.saveLastEvent("Share unavailable after upgrade: open Afora to reconnect securely.")
                })
        }
    ...
    public static func saveConfig(_ config: ShareGatewayRelayConfig) -> Bool {
        ...
        guard saved else {
            self.defaults.removeObject(forKey: self.relayConfigKey)
            self.saveLastEvent("Share unavailable: reconnect Afora to save gateway access securely.")
            return false
        }
        return true
    }
