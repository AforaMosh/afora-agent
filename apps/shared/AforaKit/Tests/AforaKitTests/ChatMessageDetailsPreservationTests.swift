...
/// Tool-result diff metadata rides on `AforaChatMessage.details`; every
...
struct ChatMessageDetailsPreservationTests {
    private func toolResultMessage(id: UUID = UUID()) -> AforaChatMessage {
        AforaChatMessage(
            id: id,
            role: "toolResult",
            content: [
    ...
    private func systemNoticeMessage(id: UUID = UUID()) -> AforaChatMessage {
        AforaChatMessage(
            id: id,
            role: "user",
            content: [
    ...
    @MainActor @Test func `decode pipeline keeps message details`() throws {
        ...
        let decoded = AforaChatViewModel.decodeMessages(anyMessages)
    ...
    @MainActor @Test func `canonical adoption keeps incoming details`() {
        ...
        let adoptedNotice = AforaChatViewModel.adoptingCanonicalMessage(
            incomingNotice,
