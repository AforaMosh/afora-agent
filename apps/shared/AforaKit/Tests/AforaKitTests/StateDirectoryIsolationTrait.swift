...
/// Serializes state-directory tests and pins `AFORA_STATE_DIR` to a fresh
...
/// (app group container or ~/Library/Application Support/Afora), so real
...
struct StateDirectoryIsolationTrait: TestTrait, TestScoping {
    ...
    private static let restoreStateDirPath: String = {
        if let raw = getenv("AFORA_STATE_DIR") {
        ...
        let quarantine = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-test-state-quarantine-\(UUID().uuidString)", isDirectory: true)
        ...
        performing function: @Sendable () async throws -> Void) async throws
    {
        ...
        let stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-test-state-\(UUID().uuidString)", isDirectory: true)
        var thrown: (any Error)?
        do {
            try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
            setenv("AFORA_STATE_DIR", stateDir.path, 1)
            try await function()
        } catch {
            thrown = error
        }
        ...
        setenv("AFORA_STATE_DIR", restorePath, 1)
