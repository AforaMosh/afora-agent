...
struct DeviceAuthPayloadTests {
    ...
    func `builds Swift connect compatibility payload with v2 canonical fields`() {
        ...
        let payload = GatewayDeviceAuthPayload.buildConnectCompatibilityPayload(
            fields: .init(
                deviceId: "dev-1",
                client: .init(id: "afora-macos", mode: "ui"),
                role: "operator",
        ...
        #expect(
            payload
                == "v2|dev-1|afora-macos|ui|operator|operator.admin,operator.read|1800000000000|tok-123|nonce-abc")
    }
    ...
    func `builds canonical v3 payload vector`() {
        ...
        #expect(
            payload
                ==
                "v3|dev-1|afora-macos|ui|operator|operator.admin,operator.read|1800000000000|tok-123|nonce-abc|ios|iphone")
    }
    ...
    func `signed device dictionary preserves 64-bit timestamp`() throws {
        ...
        let databaseURL = tempDir.appendingPathComponent("afora.sqlite", isDirectory: false)
