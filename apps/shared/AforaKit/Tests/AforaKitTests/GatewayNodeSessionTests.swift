...
private func nodeConnectOptions(
    caps: [String] = [],
    commands: [String] = [],
    clientId: String = "afora-ios-test",
...
struct GatewayNodeSessionTests {
    @Test func `operator canvas refresh uses the operator surface method`() async throws {
        ...
        let options = operatorConnectOptions(
            caps: [AforaGatewayClientCapability.inlineWidgets],
            clientId: "afora-ios")
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__afora__/cap/operator-token",
                ],
            ])
    ...
    @Test func `canvas surface refresh is shared across callers with different timeouts`() async throws {
        ...
        let gateway = GatewayNodeSession()
        let options = nodeConnectOptions(caps: ["canvas"], clientId: "afora-macos", clientDisplayName: "macOS Test")
        ...
        task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__afora__/cap/new-token",
                ],
            ])
    ...
    @Test func `last timed out surface waiter releases stalled rotation for retry`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__afora__/cap/retry-token",
                ],
            ])
    ...
    @Test func `lagging canvas refresh reuses the rotated capability`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__afora__/cap/old-token",
                ],
            ])
    ...
    func `watch approval warning text is optional and round trips`() throws {
        let legacy = try JSONDecoder().decode(
            AforaWatchExecApprovalItem.self,
    ...
    func `watch approval recovery schema carries exact resolution attempt identifiers`() throws {
        ...
        let prompt = AforaWatchExecApprovalPromptMessage(
        ...
        let request = AforaWatchExecApprovalSnapshotRequestMessage(
            requestId: "request",
    ...
    func `route switch cancels an in flight push to talk start`() async throws {
        ...
        let options = nodeConnectOptions(caps: ["talk"], commands: ["talk.ptt.start"], clientId: "afora-ios")
            ...
            await gateway.invokeIfCurrentRoute(
                ...
                onInvoke: { request in
                    ...
                    return BridgeInvokeResponse(
                        id: request.id,
                        ok: false,
                        error: AforaNodeError(code: .unavailable, message: "UNAVAILABLE: route changed"))
                })
        }
    ...
    func `route switch cancels queued PTZ control and waits for invoke cleanup`() async throws {
        ...
        let options = nodeConnectOptions(
            caps: ["camera"],
            commands: [AforaCameraCommand.ptzControl.rawValue],
            clientId: "afora-macos")
        ...
        let invoking = Task {
            await gateway.invokeIfCurrentRoute(
                BridgeInvokeRequest(
                    id: "queued-ptz",
                    command: AforaCameraCommand.ptzControl.rawValue,
                    paramsJSON: nil),
                expectedRoute: route,
                onInvoke: { request in
                ...
                onInvoke: { request in
                    ...
                    if Task.isCancelled {
                        ...
                        return BridgeInvokeResponse(
                            id: request.id,
                            ok: false,
                            error: AforaNodeError(code: .unavailable, message: "UNAVAILABLE: canceled"))
                    }
    ...
    func `replacement invoke fails promptly while disconnect lifecycle is blocked`() async throws {
        ...
        #expect(error["code"] as? String == AforaNodeErrorCode.unavailable.rawValue)
    ...
    func `route bound node event request distinguishes handled and legacy acknowledgements`() async throws {
        let session = FakeGatewayWebSocketSession()
        let gateway = GatewayNodeSession()
        let options = nodeConnectOptions(clientId: "afora-macos-test", clientDisplayName: "macOS Test")
    ...
    func `computer invoke receipts deduplicate in flight and after reconnect`() async throws {
        ...
        #expect(mismatchError["code"] as? String == AforaNodeErrorCode.invalidRequest.rawValue)
        #expect(await probe.count() == 1)
    ...
    func `connect failure preserves protocol mismatch details`() async throws {
        ...
        do {
            ...
            #expect(problem?
                .message == "This app is older than the gateway. Update Afora on this device, then retry.")
    ...
    func `failed device token write is not reported as an issued role`() async throws {
        let stateDir = try #require(ProcessInfo.processInfo.environment["AFORA_STATE_DIR"])
    ...
    func `normalize canvas host url preserves explicit secure canvas port`() {
        let normalized = GatewayPluginSurfaceURL.canonicalize(
            raw: "https://canvas.example.com:9443/__afora__/cap/token",
            against: URL(string: "wss://gateway.example.com"))

        #expect(normalized == "https://canvas.example.com:9443/__afora__/cap/token")
    }

    @Test
    func `normalize canvas host url backfills gateway host for loopback canvas`() {
        let normalized = GatewayPluginSurfaceURL.canonicalize(
            raw: "http://127.0.0.1:18789/__afora__/cap/token",
            against: URL(string: "wss://gateway.example.com:7443"))

        #expect(normalized == "https://gateway.example.com:7443/__afora__/cap/token")
    }
    ...
    func `gateway TLS pin applies only to the same canvas endpoint`() {
        ...
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://gateway.example.com:7443/__afora__/cap/token",
            gatewayURL: gateway) == fingerprint)
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://canvas.example.com:7443/__afora__/cap/token",
            gatewayURL: gateway) == nil)
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://gateway.example.com:9443/__afora__/cap/token",
            gatewayURL: gateway) == nil)
    }
    ...
    func `main session key follows the current node snapshot route`() async throws {
        ...
        let options = nodeConnectOptions(clientId: "afora-macos", clientDisplayName: "macOS Test")
