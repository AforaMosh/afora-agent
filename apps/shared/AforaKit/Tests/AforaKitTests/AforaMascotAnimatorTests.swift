...
struct AforaMascotAnimatorTests {
    private func makeAnimator(seed: UInt64 = 7, interactive: Bool = false) -> AforaMascotAnimator {
        AforaMascotAnimator(seed: seed, hourOfDay: 12, allowsAutoSleep: interactive)
    }

    @Test func `poses stay inside drawable bounds for every mood`() {
        for mood in AforaMascotMood.allCases {
    ...
    @Test func `sleepy mood wears the nightcap`() {
        ...
        let staticPose = AforaMascotPose.staticPose(for: .sleepy)
    ...
    @Test func `static poses carry the mood signature`() {
        let sad = AforaMascotPose.staticPose(for: .sad)
        #expect(sad.antennaDroop > 0)
        #expect(sad.mouthCurve < 0)
        let celebrating = AforaMascotPose.staticPose(for: .celebrating)
        #expect(celebrating.leftClawDegrees > 0)
        #expect(celebrating.mouthCurve > 0)
        let idle = AforaMascotPose.staticPose(for: .idle)
        #expect(idle == AforaMascotPose())
        let working = AforaMascotPose.staticPose(for: .working)
