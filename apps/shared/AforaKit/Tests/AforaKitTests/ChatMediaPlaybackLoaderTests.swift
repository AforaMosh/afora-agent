...
private final class PlaybackLoadSequence {
    private var responses: [AforaChatLoadedMedia?]
    private(set) var requestCount = 0
    private(set) var preparingCount = 0
    private(set) var sleeps: [Duration] = []

    init(_ responses: [AforaChatLoadedMedia?]) {
        self.responses = responses
    }

    func request() -> AforaChatLoadedMedia? {
        self.requestCount += 1
...
struct ChatMediaPlaybackLoaderTests {
    @Test func `retries preparing responses until rendition bytes are ready`() async throws {
        let expected = AforaChatMediaData(data: Data([1, 2, 3]), mimeType: "audio/mp4")
