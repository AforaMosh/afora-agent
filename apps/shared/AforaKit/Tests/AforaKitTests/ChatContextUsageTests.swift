...
private final class ContextUsageTestTransport: @unchecked Sendable, AforaChatTransport {
...
struct ChatContextUsageTests {
    private func message(
        ...
        usage: AforaChatUsage? = nil) -> AforaChatMessage
    {
        AforaChatMessage(
            id: UUID(),
            role: role,
    ...
    private func usage(
        ...
        cacheWrite: Int? = nil,
        total: Int? = nil,
        costTotal: Double? = nil) throws -> AforaChatUsage
    {
        ...
        return try JSONDecoder().decode(AforaChatUsage.self, from: data)
    }
    ...
    @Test @MainActor func `view model resolves context totals through a selected global alias`() {
        let vm = AforaChatViewModel(
            ...
            activeAgentId: "ops")
        vm.sessions = [AforaChatSessionEntry(
