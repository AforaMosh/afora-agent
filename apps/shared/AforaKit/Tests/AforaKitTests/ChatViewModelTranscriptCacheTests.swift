...
private func visibleTexts(_ vm: AforaChatViewModel) async -> [String] {
...
private actor TestTranscriptCache: AforaChatTranscriptCache {
    private var transcripts: [String: [AforaChatMessage]]
    private var sessions: [AforaChatSessionEntry]
    private let loadSessionsHook: (@Sendable () async -> Void)?
    private(set) var storedTranscripts: [[AforaChatMessage]] = []

    init(
        transcripts: [String: [AforaChatMessage]] = [:],
...
private final class GatedHistoryChatTransport: @unchecked Sendable, AforaChatTransport {
    private let historyResult: @Sendable (String, Int) async throws -> AforaChatHistoryPayload
    ...
    init(historyResult: @escaping @Sendable (String, Int) async throws -> AforaChatHistoryPayload) {
    ...
    {
        AforaChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
private func makeViewModel(
    ...
    load: Bool = true) -> AforaChatViewModel
{
...
struct ChatViewModelTranscriptCacheTests {
    ...
    @Test func `prior agent cache cannot repaint global digest after owner switch`() async throws {
        ...
        global.observerDigest = AforaChatSessionObserverDigest(
            agentId: "main",
            runId: "run-main",
            revision: 3,
            updatedAt: 1000,
    ...
    @Test func `session cache strips active markers and preserves terminal recap`() {
        ...
        let projected = AforaChatViewModel.durableSessionCacheProjection(active)
        ...
        let terminalProjection = AforaChatViewModel.durableSessionCacheProjection(terminal)
