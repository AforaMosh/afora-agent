...
final class VoiceNoteRecorderTests: XCTestCase {
    ...
    func testSuccessiveRecordingsUseUniqueTemporaryFiles() async throws {
        let capture = FakeVoiceNoteAudioCapture()
        let recorder = AforaVoiceNoteRecorder(
            capture: capture,
    ...
    func testDurationLabelBoundsMalformedHistoryValues() {
        XCTAssertEqual(aforaVoiceNoteDurationLabel(.infinity), "0:00")
        XCTAssertEqual(aforaVoiceNoteDurationLabel(.nan), "0:00")
        XCTAssertEqual(aforaVoiceNoteDurationLabel(1e100), "3:00")
        XCTAssertEqual(aforaVoiceNoteDurationLabel(-1), "0:00")
    }

    func testUnifiedMicKeepsVoiceNoteAvailableWithoutDictation() {
        XCTAssertFalse(AforaChatMicButton.dictationActionEnabled(
        ...
        XCTAssertTrue(AforaChatMicButton.voiceNoteRecordingEnabled(
            isComposerEnabled: true,
    ...
    func testUnifiedMicPreventsCompetingVoiceCapture() {
        XCTAssertFalse(AforaChatMicButton.voiceNoteRecordingEnabled(
            isComposerEnabled: true,
            isAttachmentInputEnabled: true,
        ...
        XCTAssertTrue(AforaChatMicButton.dictationActionEnabled(
    ...
    func testUnifiedMicCancelsPendingDictationStart() {
        XCTAssertEqual(AforaChatMicButton.dictationPrimaryAction(
            isPending: true,
    ...
    func testCompactTalkControlYieldsToLocalVoiceCapture() {
        XCTAssertTrue(AforaChatComposer.showsCompactTalkControl(
            hasDraftToSend: false,
            hasBlockingRunActivity: false,
            isLocalVoiceCaptureActive: false))
        XCTAssertFalse(AforaChatComposer.showsCompactTalkControl(
            hasDraftToSend: false,
            hasBlockingRunActivity: false,
    ...
    func testRecordButtonRequiresAttachmentInput() {
        let recorder = AforaVoiceNoteRecorder(capture: FakeVoiceNoteAudioCapture())
        let control = AforaChatVoiceNoteControl(recorder: recorder, isTalkActive: false)
        let button = AforaVoiceNoteButton(
            control: control,
            compact: false,
            isComposerEnabled: true,
