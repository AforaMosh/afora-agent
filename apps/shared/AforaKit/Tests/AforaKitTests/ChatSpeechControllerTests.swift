...
private final class GatedClipPlayer: ChatSpeechClipPlaying {
    var playedClips: [AforaChatSpeechClip] = []
    var stopCount = 0
    private var pending: CheckedContinuation<Bool, Never>?

    func play(clip: AforaChatSpeechClip) async -> Bool {
...
private struct SpeechHarness {
    let controller: AforaChatSpeechController
    let clipPlayer: GatedClipPlayer
    let localSpeaker: RecordingLocalSpeaker

    init(synthesize: @escaping AforaChatSpeechSynthesis) {
        let clipPlayer = GatedClipPlayer()
        let localSpeaker = RecordingLocalSpeaker()
        self.clipPlayer = clipPlayer
        self.localSpeaker = localSpeaker
        self.controller = AforaChatSpeechController(
...
private func waitForPhase(
    _ controller: AforaChatSpeechController,
    _ expected: AforaChatSpeechController.Phase) async -> Bool
{
    for _ in 0..<200 {
...
struct ChatSpeechControllerTests {
    ...
    @Test func `falls back to local speech when clip is unplayable`() async {
        let clip = AforaChatSpeechClip(data: Data([1]), mimeType: nil)
        let harness = SpeechHarness { _ in clip }
        let messageID = UUID()
    ...
    @Test func `toggle while active stops without fallback`() async {
        let clip = AforaChatSpeechClip(data: Data([5]), mimeType: nil)
        let harness = SpeechHarness { _ in clip }
        let messageID = UUID()
    ...
    @Test func `starting another message supersedes the first`() async {
        let clip = AforaChatSpeechClip(data: Data([7]), mimeType: nil)
        let harness = SpeechHarness { _ in clip }
        let first = UUID()
        let second = UUID()
    ...
    @Test func `blank text stays idle`() async {
        let harness = SpeechHarness { _ in
            AforaChatSpeechClip(data: Data([1]), mimeType: nil)
        }
    ...
    @Test func `identifies container hints and headerless formats`() {
        let mp3 = AforaChatSpeechClip(
            data: Data([1]),
            outputFormat: "mp3",
            mimeType: "audio/mpeg",
            fileExtension: ".mp3")
        let pcm = AforaChatSpeechClip(
            data: Data([1]),
            outputFormat: "pcm",
            mimeType: "audio/pcm",
            fileExtension: ".pcm")
        let azureRaw = AforaChatSpeechClip(
            data: Data([1]),
            outputFormat: "raw-8khz-8bit-mono-mulaw",
            fileExtension: ".pcm")
        let sampledPCM = AforaChatSpeechClip(
            data: Data([1]),
            outputFormat: "pcm_24000",
            fileExtension: ".pcm")
        let rawULaw = AforaChatSpeechClip(
            data: Data([1]),
