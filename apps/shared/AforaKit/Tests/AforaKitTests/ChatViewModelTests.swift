    ...
    idempotencyKey: String? = nil) -> AnyCodable
{
    ...
    if let idempotencyKey {
        message["__afora"] = ["idempotencyKey": idempotencyKey]
    }
...
private func historyPayload(
    ...
    inFlightRun: AforaChatInFlightRun? = nil) -> AforaChatHistoryPayload
{
    AforaChatHistoryPayload(
        sessionKey: sessionKey,
        sessionId: sessionId,
        messages: messages,
        thinkingLevel: "off",
        sessionInfo: supportsActiveRunState
            ? AforaChatSessionInfo(
...
private func usageEvent(runId: String, outputTokens: Int, seq: Int) -> AforaAgentEventPayload {
    AforaAgentEventPayload(
        runId: runId,
        seq: seq,
        stream: "usage",
        ts: seq,
...
private func lifecycleSessionEntry(
    ...
    outputTokens: Int? = nil) -> AforaChatSessionEntry
{
...
private func sessionEntry(
    ...
    thinkingLevels: [AforaChatThinkingLevelOption]? = nil,
    thinkingOptions: [String]? = nil,
    thinkingDefault: String? = nil,
    verboseLevel: String? = nil,
    fastMode: AforaChatFastMode? = nil,
    effectiveFastMode: AforaChatFastMode? = nil,
    totalTokens: Int? = nil,
    totalTokensFresh: Bool? = nil,
    contextTokens: Int? = nil) -> AforaChatSessionEntry
{
    AforaChatSessionEntry(
        key: key,
        kind: nil,
...
private func sessionsResponse(
    ...
    ts: Double? = nil,
    defaults: AforaChatSessionsDefaults? = nil) -> AforaChatSessionsListResponse
{
...
private func thinkingOption(_ id: String, label: String? = nil) -> AforaChatThinkingLevelOption {
    AforaChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func modelChoice(
    ...
    reasoning: Bool? = nil) -> AforaChatModelChoice
{
...
private func openAIModelPatchResult(
    _ model: String,
    thinking: String?,
    levels: [AforaChatThinkingLevelOption]? = nil) -> AforaChatModelPatchResult
{
    AforaChatModelPatchResult(
...
private func historyPayloadWithoutRunState(
    ...
    sessionId: String? = "sess-main",
    messages: [AnyCodable] = [],
    thinkingLevel: String = "off") -> AforaChatHistoryPayload
{
    AforaChatHistoryPayload(
        sessionKey: sessionKey,
        sessionId: sessionId,
...
private func commandChoice(
    ...
    aliases: [String],
    description: String = "",
    source: AforaChatCommandChoice.Source = .command,
    acceptsArgs: Bool = false) -> AforaChatCommandChoice
{
    AforaChatCommandChoice(
...
private func makeViewModel(
    ...
    historyResponses: [AforaChatHistoryPayload],
    sessionRoutingContract: String? = nil,
    sessionsResponses: [AforaChatSessionsListResponse] = [],
    modelResponses: [[AforaChatModelChoice]] = [],
    modelPatchResults: [AforaChatModelPatchResult?] = [],
    thinkingPatchResults: [AforaChatModelPatchResult?] = [],
    commandResponses: [[AforaChatCommandChoice]] = [],
    requestHistoryHook: (@Sendable (String) async throws -> Void)? = nil,
    historyResponseHook: (@Sendable (String, Int, [String]) async throws -> AforaChatHistoryPayload?)? = nil,
    ...
    sessionSettingsPatchHook: (
        @Sendable (AforaChatSessionSettingsPatch) async throws -> AforaChatModelPatchResult?)? = nil,
    ...
    listSessionsHook: (
        @Sendable (TestSessionListQuery) async throws -> AforaChatSessionsListResponse?)? = nil,
    sendMessageHook: (@Sendable (String) async throws -> AforaChatSendResponse)? = nil,
    sendMessageStatus: String = "ok",
    waitForRunCompletionHook: (@Sendable (String, Int) async -> AforaChatRunObservation)? = nil,
    acquireSessionSettingsRouteLeaseHook: (@Sendable () async -> Void)? = nil,
    swarmEnabledHook: (@Sendable (String) async throws -> Bool)? = nil,
    listChildSessionsHook: (@Sendable (String) async throws -> [AforaChatSessionEntry])? = nil,
    ...
    -> (TestChatTransport, AforaChatViewModel)
{
...
private func sendUserMessage(_ vm: AforaChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, AforaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AforaChatHistoryPayload]
    private let sessionsResponses: [AforaChatSessionsListResponse]
    private let modelResponses: [[AforaChatModelChoice]]
    private let modelPatchResults: [AforaChatModelPatchResult?]
    private let thinkingPatchResults: [AforaChatModelPatchResult?]
    private let commandResponses: [[AforaChatCommandChoice]]
    private let requestHistoryHook: (@Sendable (String) async throws -> Void)?
    private let historyResponseHook:
        (@Sendable (String, Int, [String]) async throws -> AforaChatHistoryPayload?)?
    ...
    private let sessionSettingsPatchHook:
        (@Sendable (AforaChatSessionSettingsPatch) async throws -> AforaChatModelPatchResult?)?
    ...
    private let listSessionsHook:
        (@Sendable (TestSessionListQuery) async throws -> AforaChatSessionsListResponse?)?
    private let sendMessageHook: (@Sendable (String) async throws -> AforaChatSendResponse)?
    private let sendMessageStatus: String
    private let waitForRunCompletionHook:
        (@Sendable (String, Int) async -> AforaChatRunObservation)?
    ...
    private let listChildSessionsHook: (@Sendable (String) async throws -> [AforaChatSessionEntry])?
    ...
    private let stream: AsyncStream<AforaChatTransportEvent>
    private let continuation: AsyncStream<AforaChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<AforaChatTransportEvent>.Continuation!
        ...
        worktree _: Bool?) async throws -> AforaChatCreateSessionResponse
    {
        ...
        return AforaChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AforaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AforaChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [AforaChatAttachmentPayload]) async throws -> AforaChatSendResponse
    {
        ...
        return AforaChatSendResponse(runId: idempotencyKey, status: self.sendMessageStatus)
    }
        ...
        archived: Bool) async throws -> AforaChatSessionsListResponse
    {
        ...
        return self.sessionsResponses.last ?? AforaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
        ...
        patch: AforaChatSessionSettingsPatch) async throws -> AforaChatModelPatchResult?
    {
        ...
        var result: AforaChatModelPatchResult?
        ...
        if let thinkingLevelUpdate = patch.thinkingLevel {
            ...
            let thinkingResult = try await patchSessionThinking(
                sessionKey: sessionKey,
                thinkingLevel: thinkingLevel)
            result = AforaChatModelPatchResult(
    ...
    private func patchSessionThinking(
        sessionKey: String,
        thinkingLevel: String) async throws -> AforaChatModelPatchResult?
    {
        ...
        return AforaChatModelPatchResult(
            key: sessionKey,
            modelProvider: nil,
            model: nil,
            thinkingLevel: thinkingLevel)
    ...
    func emit(_ evt: AforaChatTransportEvent) {
...
struct ChatViewModelTests {
    ...
    @Test @MainActor func `subagent task events filter by session and retain terminal activity`() {
        let viewModel = AforaChatViewModel(
            sessionKey: "main",
    ...
    @Test @MainActor func `tool input delta updates the matching pending edit diff`() {
        ...
        viewModel.handleTransportEvent(.agent(AforaAgentEventPayload(
            runId: "run-1",
            seq: 1,
            stream: "tool",
            ts: 1000,
    ...
    @Test @MainActor func `transient Swarm capability failure preserves state and retries until explicit false`() async throws {
        ...
        let viewModel = AforaChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test @MainActor func `global Swarm activity follows the selected agent owner`() async {
        ...
        let foreign = AforaChatSessionsChangedEvent(
            sessionKey: "global",
            agentId: "main",
            reason: "swarm-note",
        ...
        let selected = AforaChatSessionsChangedEvent(
            sessionKey: "global",
            agentId: "work",
            reason: "swarm-note",
            swarmGroupId: "swarm:global:turn-1",
    ...
    @Test func `context usage fraction validates freshness and token bounds`() {
        func fraction(total: Int?, fresh: Bool? = true, context: Int?) -> Double? {
            AforaChatViewModel.chatContextUsageFraction(
                for: sessionEntry(
                    key: "main",
                    updatedAt: 1,
                    totalTokens: total,
    ...
    @Test @MainActor func `advertised terminal chat retires the run without clearing local work`() {
        ...
        viewModel.handleTransportEvent(.chat(AforaChatEventPayload(
            runId: "remote-run",
            sessionKey: "main",
            state: "final",
            message: nil,
    ...
    @Test @MainActor func `terminal chat without run ID requires one current local owner`() {
        let viewModel = AforaChatViewModel(
            ...
            transport: TestChatTransport(historyResponses: []))
        viewModel.pendingRuns = ["run-a", "run-b"]
        let terminal = AforaChatEventPayload(
            runId: nil,
            sessionKey: nil,
            state: "final",
            message: nil,
    ...
    @Test @MainActor func `event listener does not retain discarded view model`() async throws {
        ...
        var viewModel: AforaChatViewModel? = AforaChatViewModel(
            sessionKey: "main",
            transport: transport)
    ...
    @Test func `decodes in-flight run from chat history`() throws {
        ...
        let payload = try JSONDecoder().decode(AforaChatHistoryPayload.self, from: data)

        #expect(payload.inFlightRun?.runId == "run-active")
        #expect(payload.inFlightRun?.text == "partial")
        #expect(payload.inFlightRun?.plan?.steps == [
            AforaChatPlanStep(step: "Reconnect", status: .inProgress),
        ])
    ...
    @Test func `decodes agent scope from chat event`() throws {
        ...
        let payload = try JSONDecoder().decode(AforaChatEventPayload.self, from: data)
    ...
    @Test func `bootstrap adopts active history run and consumes live events`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "partial reply"))
    ...
    @Test func `adopts Codex history run without buffered text`() async throws {
        let history = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-codex", text: ""))
    ...
    @Test func `bootstrap adopts in-flight plan snapshot`() async throws {
        let history = historyPayload(
            inFlightRun: AforaChatInFlightRun(
                runId: "run-plan",
                text: "working",
                plan: AforaChatPlanSnapshot(
                    steps: [
                        AforaChatPlanStep(step: "Inspect", status: .completed),
                        AforaChatPlanStep(step: "Reconnect", status: .inProgress),
                    ],
    ...
    @Test func `history plan reconciliation contract`() async {
        let retainedSteps = [AforaChatPlanStep(step: "Retained", status: .inProgress)]
        let liveSteps = [AforaChatPlanStep(step: "New live plan", status: .inProgress)]
        let cases: [(
            name: String,
            payload: AforaChatHistoryPayload,
            expectedRunId: String?,
            expectedSteps: [AforaChatPlanStep],
            ...
            (
                ...
                historyPayload(
                    inFlightRun: AforaChatInFlightRun(
                        ...
                        text: "working",
                        plan: AforaChatPlanSnapshot(
                            steps: [AforaChatPlanStep(step: "Replacement", status: .completed)]))),
                "run-retained",
                [AforaChatPlanStep(step: "Replacement", status: .completed)],
                false),
            (
                "legacy-preserve",
                historyPayload(
                    inFlightRun: AforaChatInFlightRun(runId: "run-retained", text: "working")),
                "run-retained",
                retainedSteps,
                false),
            (
                ...
                historyPayload(
                    inFlightRun: AforaChatInFlightRun(
                        ...
                        text: "next",
                        plan: AforaChatPlanSnapshot(
                            steps: [AforaChatPlanStep(step: "Next run", status: .inProgress)]))),
                "run-next",
                [AforaChatPlanStep(step: "Next run", status: .inProgress)],
                false),
            (
                "active-preserve",
            ...
            (
                ...
                historyPayload(
                    inFlightRun: AforaChatInFlightRun(
                        runId: "run-retained",
                        text: "working",
                        plan: AforaChatPlanSnapshot(steps: []))),
                nil,
                [],
                false),
        ]
    ...
    @Test func `foreground history refreshes adopted run snapshot`() async throws {
        let firstHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "first partial"))
        let resumedHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "resumed partial"))
        let historyCalls = AsyncCounter()
        ...
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                return historyPayload(
                    ...
                    inFlightRun: AforaChatInFlightRun(runId: runId, text: "working"))
            },
    ...
    @Test func `foreground discovers run started without local ownership`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-external", text: "external partial"))
    ...
    @Test func `foreground keeps active run with intermediate assistant history`() async throws {
        ...
        let activeHistory = historyPayload(
            ...
            inFlightRun: AforaChatInFlightRun(runId: "run-tool", text: "still working"))
        let (_, vm) = await makeViewModel(historyResponses: [idleHistory, activeHistory])
    ...
    @Test func `foreground synthesizes activity when no run snapshot or local run exists`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                AforaSessionMessageEventPayload(
                    sessionKey: "main",
    ...
    @Test func `foreground clears completed run without assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: AforaChatInFlightRun(
                ...
                text: "",
                plan: AforaChatPlanSnapshot(
                    steps: [AforaChatPlanStep(step: "Finish", status: .inProgress)])))
        let completedHistory = historyPayload(
            messages: [chatTextMessage(role: "user", text: "quiet task", timestamp: 1)],
            hasActiveRun: false)
    ...
    @Test func `foreground missing snapshot does not clear an in-flight send`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "accepted"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), historyPayload(), activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            sendMessageHook: { _ in
                await sendGate.wait()
                return AforaChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send history keeps active run with intermediate assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "working"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), activeHistory],
            sendMessageHook: { _ in
                AforaChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send stale inactive history preserves newer live plan`() async throws {
        ...
        #expect(await MainActor.run { vm.planSteps == [
            AforaChatPlanStep(step: "Finish", status: .inProgress),
        ] })
    }
    ...
    @Test func `live chat delta owns run while bootstrap history is pending`() async throws {
        ...
        let staleHistory = historyPayload(
            ...
            inFlightRun: AforaChatInFlightRun(runId: "run-stale", text: "stale partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `global chat delta adopts only selected agent run`() async throws {
        let bareGlobalMatches = await MainActor.run {
            (
                AforaChatViewModel.matchesCurrentSessionKey(
                    incoming: "global",
                    agentId: "work",
                    current: "global",
                    mainSessionKey: "main",
    ...
    @Test @MainActor func `global session changes reconcile nested digest ownership`() async {
        ...
        selected.observerDigest = AforaChatSessionObserverDigest(
            agentId: "work",
            runId: "run-work",
            revision: 2,
            updatedAt: 200,
        ...
        vm.handleTransportEvent(.sessionsChanged(.init(
            ...
            agentId: "work",
            updatedAt: 900,
            observerDigest: AforaChatSessionObserverDigest(
                agentId: "main",
                runId: "run-work",
                revision: 9,
                updatedAt: 900,
    ...
    @Test func `intermediate session message preserves pending recovery snapshot`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "active partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `manual refresh re-adopts active run after clearing local ownership`() async throws {
        ...
        let refreshedHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "refreshed partial"))
    ...
    @Test func `older history cannot replace newer run snapshot`() async throws {
        ...
        let initialHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-initial", text: "initial"))
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    return historyPayload(
                        inFlightRun: AforaChatInFlightRun(runId: "run-older", text: "older"))
                }
                if index == 2 {
                    return historyPayload(
                        inFlightRun: AforaChatInFlightRun(runId: "run-newer", text: "newer"))
                }
                return nil
            })
    ...
    @Test func `delayed history cannot overwrite newer live run text`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "initial"))
        let (transport, vm) = await makeViewModel(
            historyResponses: [activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            historyResponseHook: { _, index, _ in
                ...
                return historyPayload(
                    inFlightRun: AforaChatInFlightRun(runId: "run-active", text: "stale"))
            })
    ...
    @Test func `external terminal event protects current run from delayed snapshot`() async throws {
        ...
        let currentHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-current", text: "current"))
        let (transport, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    _ = await staleCompletions.increment()
                    return historyPayload(
                        inFlightRun: AforaChatInFlightRun(runId: "run-finished", text: "stale"))
                }
                if index == 2 {
                    throw NSError(domain: "test", code: 1)
                }
    ...
    @Test func `sequence gap re-adopts active run from history`() async throws {
        ...
        let recoveredHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(runId: "run-recovered", text: "recovered partial"))
    ...
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> AforaChatMessage {
        ...
        #expect(
            AforaChatMessage.errorDisplayText(
    ...
    @Test func `composer presentation owner changes when the model is replaced for the same session`() async {
        ...
        let firstOwner = await MainActor.run {
            AforaChatComposerPresentationOwner(viewModel: first)
        }
        let replacementOwner = await MainActor.run {
            AforaChatComposerPresentationOwner(viewModel: replacement)
        }
    ...
    @Test func `composer presentation owner changes with same-session agent routing`() async {
        ...
        let alphaOwner = await MainActor.run {
            AforaChatComposerPresentationOwner(viewModel: vm)
        }
        let betaOwner = await MainActor.run {
            vm.syncActiveAgentId("beta")
            return AforaChatComposerPresentationOwner(viewModel: vm)
        }
    ...
    @Test func `balances tool activity when a terminal event clears pending calls`() async throws {
        ...
        transport.emit(.chat(AforaChatEventPayload(
            runId: runId,
            sessionKey: "main",
            state: "final",
            message: nil,
    ...
    @Test func `duplicate final events append one provisional reply`() async throws {
        ...
        await sendUserMessage(vm, text: "hello")
        let runId = try await waitForLastSentRunId(transport)
        let final = AforaChatEventPayload(
            runId: runId,
            sessionKey: "main",
            state: "final",
            message: chatTextMessage(
    ...
    @Test func `later identical session reply does not adopt prior turn provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                AforaChatSendResponse(runId: runId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `plan event parses typed and legacy steps`() async throws {
        ...
        #expect(await MainActor.run { vm.planSteps } == [
            AforaChatPlanStep(step: "Inspect state", status: .inProgress),
            AforaChatPlanStep(step: "Write fix", status: .pending),
            AforaChatPlanStep(step: "Verify", status: .completed),
        ])
    ...
    @Test func `plan snapshots replace and empty snapshot clears`() async throws {
        ...
        try await waitUntil("replacement plan snapshot applies") {
            await MainActor.run {
                vm.planSteps == [AforaChatPlanStep(step: "Replacement", status: .pending)] &&
    ...
    @Test func `terminal event for another run preserves active plan`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: AforaChatInFlightRun(
                ...
                text: "",
                plan: AforaChatPlanSnapshot(
                    steps: [AforaChatPlanStep(step: "Keep working", status: .inProgress)])))
        let (transport, vm) = await makeViewModel(
            historyResponses: [initialHistory, activeHistory, activeHistory],
            sendMessageHook: { _ in
                AforaChatSendResponse(runId: activeRunId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        ...
        #expect(await MainActor.run { vm.pendingRunCount } == 1)
        #expect(await MainActor.run { vm.planSteps } == [
            AforaChatPlanStep(step: "Keep working", status: .inProgress),
        ])
    }
    ...
    @Test func `rekeys optimistic user message when gateway reuses active run`() async throws {
        ...
        let (_, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                ...
                return AforaChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `canonical projected reply after steering adopts reused provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                AforaChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(AforaChatTransportEvent.seqGap)
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = sessionsResponse(
            ...
            ts: now,
            defaults: AforaChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil))
        let models = [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(AforaChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == AforaChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = sessionsResponse(
            ...
            ts: now,
            defaults: AforaChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil))
        let models = [
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                if (1...3).contains(index) {
                    ...
                    return historyPayload(
                        ...
                        sessionId: sessionId,
                        messages: [chatTextMessage(role: "user", text: "hello", timestamp: now)],
                        inFlightRun: AforaChatInFlightRun(runId: runId, text: ""))
                }
    ...
    @Test @MainActor func `Ultra is canonical while ultrathink remains a high alias`() {
        #expect(AforaChatViewModel.normalizedThinkingLevel("ultra") == "ultra")
        #expect(AforaChatViewModel.normalizedThinkingLevel("ULTRA") == "ultra")
        #expect(AforaChatViewModel.normalizedThinkingLevel("ultrathink") == "high")
        #expect(
            AforaChatViewModel.normalizedThinkingLevel(
                "ultra",
    ...
    @Test func `decodes authoritative model patch thinking state`() throws {
        ...
        let result = try JSONDecoder().decode(AforaChatModelPatchResult.self, from: data)
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            AforaChatSessionsListResponse.self,
            from: Data(json.utf8))
        ...
        try await waitUntil("models loaded with default selection") {
            await MainActor.run {
                vm.modelChoices.count == 2 &&
                    vm.modelSelectionID == AforaChatViewModel.defaultModelSelectionID
            }
        }
    ...
    @Test func `default settings patch returns accepted thinking state`() async throws {
        ...
        let result = try await transport.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            patch: AforaChatSessionSettingsPatch(thinkingLevel: .some("high")))
    ...
    @Test func `default thinking selection clears override and adopts resolved level`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectThinkingLevel(AforaChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")

        #expect(await MainActor.run { vm.thinkingSelectionID } ==
            AforaChatViewModel.inheritedThinkingSelectionID)
        #expect(await MainActor.run { vm.thinkingLevel } == "medium")
        #expect(await MainActor.run { vm.sessions.first?.thinkingLevel } == nil)
    ...
    @Test func `inherited verbosity does not masquerade as persisted override`() async throws {
        ...
        #expect(await MainActor.run { vm.verboseLevel } == AforaChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity rejection restores inherited overrides`() async throws {
        ...
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.fastModeSelectionID } == AforaChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity default selections clear overrides`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectFastMode(AforaChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.fastMode } == nil)
        ...
        await MainActor.run {
            vm.selectVerboseLevel(AforaChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.verboseLevel } == nil)
        #expect(await MainActor.run { vm.verboseLevel } == AforaChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        let history = historyPayloadWithoutRunState(
            messages: [
                AnyCodable([
                    ...
                    Conversation info: \u{27E6}afora:ctx\u{27E7}
                    ```json
                    { \"sender\": \"afora-ios\" }
                    ```
        ...
        let vm = await MainActor.run { AforaChatViewModel(sessionKey: "main", transport: transport) }
...
struct ChatViewModelSessionManagementTests {
    @Test @MainActor func `session list organizer orders pinned first with key tiebreak`() {
        let organized = AforaChatSessionListOrganizer.organize([
    ...
    @Test @MainActor func `session list organizer filters across display fields`() {
        ...
        let matched = AforaChatSessionListOrganizer.filter(sessions, search: "TRIP")
        #expect(matched.map(\.key) == ["agent:main:topic-a", "agent:main:trip-notes"])
        #expect(AforaChatSessionListOrganizer.filter(sessions, search: "  ") == sessions)
    }
