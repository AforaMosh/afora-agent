...
struct ChatGatewayRequestTests {
    @Test func `session observation requests encode global subscription and actual visibility`() {
        let subscribe = AforaChatGatewayRequests.subscribeSessions()
        let visible = AforaChatGatewayRequests.setSessionObserverVisibility(true)
        let hidden = AforaChatGatewayRequests.setSessionObserverVisibility(false)
        let longerSubscription = AforaChatGatewayRequests.subscribeSessions(timeoutMs: 12000)
        let longerVisibility = AforaChatGatewayRequests.setSessionObserverVisibility(
            true,
            timeoutMs: 12000)
    ...
    @Test func `session targets share normalization while preserving platform routing policy`() {
        #expect(AforaChatSessionTarget.resolve(
            " Matrix:Channel:Room ",
    ...
    @Test func `settings patch request encodes fast values and explicit resets`() {
        let reset = AforaChatGatewayRequests.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            thinkingLevel: .some(nil),
            fastMode: .some(nil),
            verboseLevel: .some(nil))
        let automatic = AforaChatGatewayRequests.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
    ...
    @Test func `fork and create requests preserve routing identity`() {
        let fork = AforaChatGatewayRequests.forkSession(
        ...
        let activeFork = AforaChatGatewayRequests.forkSession(
            parentSessionKey: "agent:reviewer:telegram:group:1",
            agentID: "reviewer",
            fromLastCompleted: true)
        #expect(activeFork.params["forkFrom"]?.value as? String == "last-completed")

        let create = AforaChatGatewayRequests.createSession(
    ...
    @Test func `message rewind and fork requests preserve routing identity`() {
        let rewind = AforaChatGatewayRequests.rewindSession(
            sessionKey: "agent:reviewer:telegram:group:1",
            agentID: " reviewer ",
            entryId: " message-42 ")
        let fork = AforaChatGatewayRequests.forkAtMessage(
            sessionKey: "global",
            agentID: nil,
            entryId: "message-43")
    ...
    @Test func `branch list and switch requests preserve routing identity`() {
        let list = AforaChatGatewayRequests.listSessionBranches(
            sessionKey: "agent:reviewer:telegram:group:1",
            agentID: " reviewer ")
        let switchBranch = AforaChatGatewayRequests.switchSessionBranch(
            sessionKey: "global",
            agentID: nil,
            leafEntryId: " leaf-42 ")
    ...
    @Test func `session group requests encode exact gateway contracts`() {
        let list = AforaChatGatewayRequests.sessionGroupsList()
        let put = AforaChatGatewayRequests.sessionGroupsPut(names: ["Work", "Personal"])
        let rename = AforaChatGatewayRequests.sessionGroupsRename(name: "Work", to: "Projects")
        let delete = AforaChatGatewayRequests.sessionGroupsDelete(name: "Personal")
    ...
    @Test func `rename clear archive and fork use session mutation contracts`() {
        let rename = AforaChatGatewayRequests.patchSession(
            ...
            archived: nil,
            unread: nil)
        let archive = AforaChatGatewayRequests.patchSession(
            sessionKey: "agent:main:child",
            agentID: nil,
            ...
            archived: true,
            unread: nil)
        let restore = AforaChatGatewayRequests.patchSession(
            sessionKey: "agent:main:child",
            agentID: nil,
            expectedSessionID: "session-child",
            label: nil,
    ...
    @Test func `commands request selects session agent before fallback`() {
        let scoped = AforaChatGatewayRequests.commandsList(
            sessionKey: "agent:reviewer:main",
            fallbackAgentID: "fallback")
        ...
        let global = AforaChatGatewayRequests.commandsList(
            sessionKey: "global",
            fallbackAgentID: "reviewer")
        #expect(global.params["agentId"]?.value as? String == "reviewer")
    }
    ...
    @Test func `send request omits inherited thinking override`() {
        let inherited = AforaChatGatewayRequests.sendMessage(
            sessionKey: "global",
            agentID: nil,
    ...
    @Test func `question resolve request preserves answer arrays`() throws {
        let request = AforaChatGatewayRequests.resolveQuestion(
            id: "ask_123",
    ...
    @Test func `question get request carries id`() {
        let request = AforaChatGatewayRequests.questionGet(id: "ask_123")
    ...
    @Test func `question cancel request uses resolve cancel contract`() {
        let request = AforaChatGatewayRequests.cancelQuestion(id: "ask_123")
    ...
    @Test func `long running requests share exact gateway timeout margins`() {
        #expect(AforaChatGatewayRequests.agentWait(runID: "run-1", timeoutMs: 1).timeoutMs == 5001)
        #expect(AforaChatGatewayRequests.agentWait(runID: "run-1", timeoutMs: 30000).timeoutMs == 35000)
        #expect(AforaChatGatewayRequests.compactSession(
            sessionKey: "main",
...
struct ChatGatewayPayloadCodecTests {
    @Test func `session key extracts canonical agent identity`() {
        #expect(AforaChatSessionKey.agentID(from: " agent:Reviewer:main ") == "Reviewer")
        #expect(AforaChatSessionKey.agentID(from: "agent::main") == nil)
        #expect(AforaChatSessionKey.agentID(from: "global") == nil)
    }

    @Test func `agent wait distinguishes terminal and retryable timeouts`() throws {
        #expect(try AforaChatGatewayPayloadCodec.decodeAgentWaitObservation(
    ...
    @Test func `routing identity decodes agent and canonical contract`() throws {
        let identity = try AforaChatGatewayPayloadCodec.decodeSessionRoutingIdentity(
    ...
    @Test func `model choices preserve metadata and replace blank names`() throws {
        let choices = try AforaChatGatewayPayloadCodec.decodeModelChoices(Data(
            #"{"models":[{"id":"gpt-5","name":"  ","provider":"openai","contextWindow":200000,"reasoning":true}]}"#
                .utf8))

        #expect(choices == [AforaChatModelChoice(
            modelID: "gpt-5",
            name: "gpt-5",
            provider: "openai",
    ...
    @Test func `command choice normalizes source aliases and identity`() {
        let choice = AforaChatGatewayPayloadCodec.commandChoice(CommandEntry(
            name: "review",
            textaliases: [" /review ", ""],
    ...
    @Test func `event frames map to shared chat transport events`() {
        ...
        guard case let .sessionsChanged(change) = AforaChatGatewayPayloadCodec.event(from: sessionsChanged)
        else {
        ...
        guard case let .sessionsChanged(note) = AforaChatGatewayPayloadCodec.event(from: swarmNote)
        else {
        ...
        guard case let .sessionsChanged(lifecycle) = AforaChatGatewayPayloadCodec.event(
        ...
        guard case let .chat(payload) = AforaChatGatewayPayloadCodec.event(from: chat) else {
        ...
        guard case let .sessionObserver(digest) = AforaChatGatewayPayloadCodec.event(from: observer)
        else {
        ...
        guard case let .task(.upserted(summary)) = AforaChatGatewayPayloadCodec.event(from: task)
        else {
        ...
        #expect(AforaChatGatewayPayloadCodec.event(from: EventFrame(
            type: "event",
            event: "unknown")) == nil)
    }

    @Test func `session change decoding distinguishes absent fields from explicit clears`() {
        func decode(_ fields: [String: AnyCodable]) throws -> AforaChatSessionsChangedEvent {
            let frame = EventFrame(
                type: "event",
                event: "sessions.changed",
                payload: AnyCodable(fields))
            guard case let .sessionsChanged(change) = AforaChatGatewayPayloadCodec.event(from: frame)
            else {
    ...
    @Test func `session change decodes nested fallback with outer null precedence and no reason`() throws {
        ...
        let event = try JSONDecoder().decode(AforaChatSessionsChangedEvent.self, from: data)
    ...
    @Test func `session change remains codable without exposing presence flags`() throws {
        let event = AforaChatSessionsChangedEvent(
        ...
        #expect(try JSONDecoder().decode(AforaChatSessionsChangedEvent.self, from: data) == event)
    }
}
