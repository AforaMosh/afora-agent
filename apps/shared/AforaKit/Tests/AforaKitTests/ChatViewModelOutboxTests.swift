...
private func outboxTestCommand(id: String, text: String, createdAt: Double) -> AforaChatOutboxCommand {
    AforaChatOutboxCommand(
        id: id,
        sessionKey: "main",
...
private func userTexts(_ vm: AforaChatViewModel) async -> [String] {
    await MainActor.run {
        vm.messages
            .filter { $0.role == "user" }
...
private final class OutboxTestTransport: @unchecked Sendable, AforaChatTransport {
    let state: OutboxTransportState
    private let sessions: [AforaChatSessionEntry]
    ...
    init(
        ...
        sessions: [AforaChatSessionEntry] = [],
    ...
    func emit(_ event: AforaChatTransportEvent) {
    ...
    func listCommands(sessionKey _: String) async throws -> [AforaChatCommandChoice] {
    ...
    private func requestHistory(
        sessionKey: String,
        agentID: String?,
        expectedRoute: Int?) async throws -> AforaChatHistoryPayload
    {
        ...
        let durableUserRows = keys.indices.compactMap { index -> AnyCodable? in
            ...
            return AnyCodable([
                ...
                "__afora": ["idempotencyKey": "\(key):user"],
            ] as [String: Any])
        }
        return AforaChatHistoryPayload(
            sessionKey: sessionKey,
    ...
    private func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        expectedRoute: Int?) async throws -> AforaChatSendResponse
    {
        if let expectedRoute, await state.routeGeneration != expectedRoute {
        ...
        if await self.state.sendRoutingChanged {
            throw GatewayResponseError(
                ...
                details: [
                    "reason": AnyCodable(AforaChatSessionRoutingContract.changedErrorReason),
                ])
        }
        if await self.state.sendRejects {
            // Gateway responded but refused to start the run.
            return AforaChatSendResponse(runId: idempotencyKey, status: "error")
        }
        ...
        return AforaChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
private func outboxSessionEntry(
    key: String,
    thinkingLevels: [String]) -> AforaChatSessionEntry
{
    AforaChatSessionEntry(
        ...
        model: nil,
        contextTokens: nil,
        thinkingLevels: thinkingLevels.map { AforaChatThinkingLevelOption(id: $0, label: $0) })
}

private func makeOutboxViewModel(
    transport: OutboxTestTransport,
    outbox: any AforaChatCommandOutbox,
    transcriptCache: (any AforaChatTranscriptCache)? = nil,
    retryDelaysMs: [UInt64] = [1, 1],
    sessionKey: String = "main",
    activeAgentID: String? = "main",
    sessionRoutingContract: String? = "per-sender|main|main") async -> AforaChatViewModel
{
    await MainActor.run {
...
private func sendWhileOffline(_ vm: AforaChatViewModel, text: String) async throws {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private actor ScriptedOutbox: AforaChatCommandOutbox {
    enum Forwarding { case full, minimal, holdingCancellation }

    private nonisolated let base: AforaChatSQLiteTranscriptCache
    ...
    init(base: AforaChatSQLiteTranscriptCache, forwarding: Forwarding = .full) {
        self.base = base
        self.forwarding = forwarding
    }

    nonisolated func changes() -> AsyncStream<AforaChatOutboxChange> {
        self.base.changes()
    }
    ...
    func enqueueCommand(_ command: AforaChatOutboxCommand) async -> Bool {
    ...
    func loadCommands() async -> [AforaChatOutboxCommand] {
    ...
    func loadCommandsIfAvailable() async -> [AforaChatOutboxCommand]? {
        await self.delayLoad()
    ...
    func claimNextCommand() async -> AforaChatOutboxCommand? {
    ...
    func markCommandQueued(
        ...
        attemptVersion: Int,
        retryCount: Int,
        lastError: String?) async -> AforaChatOutboxUpdateResult
    {
    ...
    func markCommandAwaitingConfirmation(id: String, attemptVersion: Int) async -> AforaChatOutboxUpdateResult {
    ...
    func cancelCommand(id: String) async -> AforaChatOutboxUpdateResult {
    ...
    func confirmCommand(id: String, attemptVersion: Int) async -> AforaChatOutboxUpdateResult {
        await self.base.confirmCommand(id: id, attemptVersion: attemptVersion)
    }

    func branchState(for scope: AforaChatOutboxScope) async -> AforaChatOutboxBranchState? {
        guard self.forwarding == .full else { return nil }
        return await self.base.branchState(for: scope)
    }

    func beginBranchSwitch(_ scope: AforaChatOutboxScope) async -> Bool {
        guard self.forwarding == .full else { return false }
        return await self.base.beginBranchSwitch(scope)
    }

    func cancelBranchSwitch(_ scope: AforaChatOutboxScope) async -> Bool {
        guard self.forwarding == .full else { return false }
        return await self.base.cancelBranchSwitch(scope)
    }

    func reconcileBranchScope(
        _ scope: AforaChatOutboxScope,
        previousState: AforaChatOutboxBranchState,
        activeLeafEntryID: String?,
        branchLeafEntryIDs: Set<String>,
        activeTranscriptEntryIDs: Set<String>,
        lastError: String) async -> [AforaChatOutboxCommand]?
    {
        guard self.forwarding == .full else { return nil }
    ...
    func updateLastActiveLeafEntryID(
        ...
        expectedEpoch: Int,
        for scope: AforaChatOutboxScope) async -> Bool
    {
        guard self.forwarding == .full else { return false }
...
struct ChatViewModelOutboxTests {
    ...
    @Test func `unsupported gateway keeps queued work and surfaces upgrade action`() async throws {
        let (store, _, databaseDirectory) = try makeOutboxStore()
        defer { try? FileManager.default.removeItem(at: databaseDirectory) }
        let message = AforaChatTransportUpgradeMessage.routingContract
    ...
    @Test func `inert outbox does not capability gate healthy live chat`() async throws {
        ...
        let transport = OutboxTestTransport(
            healthy: true,
            routeUnavailableReason: AforaChatTransportUpgradeMessage.routingContract)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        ...
        parked.lastError = AforaChatSQLiteTranscriptCache.outboxChangedTargetError
    ...
    @Test func `reconnect waits for canonical session metadata before flushing Ultra`() async throws {
        ...
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: AforaChatOutboxScope(sessionKey: "main", agentID: "main")))
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: AforaChatOutboxScope(sessionKey: "main", agentID: nil)))
        #expect(await store.enqueueCommand(AforaChatOutboxCommand(
            id: "alpha-ultra",
            sessionKey: "main",
    ...
    @Test func `changed default agent parks command visibly for retry`() async throws {
        ...
        #expect(await store.loadCommands().first?.lastError ==
            AforaChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `atomic gateway routing rejection parks without retrying`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == AforaChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `failed alias row remains reachable after owner change`() async throws {
        ...
        #expect(await store.enqueueCommand(AforaChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 1,
            lastError: AforaChatSQLiteTranscriptCache.outboxExpiredError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `ownerless global retry stays failed without a selected agent`() async throws {
        ...
        #expect(await store.enqueueCommand(AforaChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 0,
            lastError: AforaChatSQLiteTranscriptCache.outboxUnknownTargetError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `delete race preserves a turn already proven by canonical history`() async throws {
        ...
        let canonicalMessage = AforaChatMessage(
            role: "user",
    ...
    @Test func `gateway rejections burn attempts then fail terminally and support tap retry`() async throws {
        ...
        #expect(failed.retryCount == AforaChatViewModel.maxOutboxSendAttempts)
    ...
    @Test func `unavailable terminal write drops health instead of advancing FIFO`() async throws {
        ...
        #expect(await store.enqueueCommand(AforaChatOutboxCommand(
            ...
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: AforaChatViewModel.maxOutboxSendAttempts - 1,
            lastError: "rejected")))
        let outbox = ScriptedOutbox(base: store)
    ...
    @Test func `ambiguous live transport failure requires explicit retry`() async throws {
        ...
        #expect(preserved.lastError == AforaChatSQLiteTranscriptCache.outboxUnconfirmedError)
    ...
    @Test func `tap retry refreshes createdAt so an expired command can resend`() async throws {
        ...
        let staleCreatedAt = Date().timeIntervalSince1970 -
            AforaChatSQLiteTranscriptCache.outboxCommandMaxAge - 60
        ...
        #expect(await store.loadCommands().map(\.lastError) == [
            AforaChatSQLiteTranscriptCache.outboxExpiredError,
        ])
    ...
    @Test func `background canonical alias event confirms by idempotency key`() async throws {
        ...
        transport.emit(.sessionMessage(AforaSessionMessageEventPayload(
    ...
    @Test func `full queue refuses enqueue and keeps the draft`() async throws {
        let (store, _, databaseDirectory) = try makeOutboxStore()
        defer { try? FileManager.default.removeItem(at: databaseDirectory) }
        for index in 0..<AforaChatSQLiteTranscriptCache.maxQueuedCommands {
        ...
        #expect(await userTexts(vm).isEmpty)
        #expect(await store.loadCommands().count == AforaChatSQLiteTranscriptCache.maxQueuedCommands)
    }
    ...
    @Test func `queued send transport failure fails closed until explicit retry`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == AforaChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(command.retryCount == 0)
        #expect(await transport.state.sentIdempotencyKeys.isEmpty)
    ...
    @Test func `deleting a queued message removes bubble and durable row`() async throws {
        ...
        let staleMessage = AforaChatMessage(
            role: "user",
            content: [AforaChatMessageContent(
                type: "text",
...
extension ChatViewModelOutboxTests {
    ...
    @Test func `send right after a session switch still queues behind that session's backlog`() async throws {
        ...
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: AforaChatOutboxScope(sessionKey: "second", agentID: nil)))
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: AforaChatOutboxScope(sessionKey: "second", agentID: "main")))
