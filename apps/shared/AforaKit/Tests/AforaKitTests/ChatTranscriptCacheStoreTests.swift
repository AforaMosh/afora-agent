...
func cacheSessionEntry(key: String, updatedAt: Double) -> AforaChatSessionEntry {
...
private func messageTexts(_ messages: [AforaChatMessage]) -> [String] {
...
private func outboxCommand(
    ...
    text: String,
    attachments: [AforaChatOutboxAttachment] = [],
    thinking: String = "off",
    createdAt: Double = Date().timeIntervalSince1970,
    status: AforaChatOutboxCommand.Status = .queued) -> AforaChatOutboxCommand
{
...
class ClientDatabaseTestSuite: TemporaryDatabaseTestSuite {
    let databases: AforaClientDatabases
    let store: AforaChatSQLiteTranscriptCache

    required init() throws {
        let directory = try makeDatabaseDirectory()
        let databases: AforaClientDatabases
        do {
            databases = try AforaClientDatabases(directoryURL: directory)
        } catch {
...
private func retryExpectation(_ command: AforaChatOutboxCommand) -> AforaChatOutboxRetryExpectation {
    AforaChatOutboxRetryExpectation(
...
private func forgottenGatewayProbe(
    in databases: AforaClientDatabases,
    identity: String) async throws -> (hash: String?, gatewayID: String?, cleanupPhase: Int?)
{
    try await databases.stateQueue.read { db in
        let row = try Row.fetchOne(
            ...
            arguments: [AforaClientDatabases.gatewayIdentityHash(identity)])
...
extension AforaChatSQLiteTranscriptCache {
    fileprivate func reconcileForTest(
        ...
        branchLeafEntryIDs: Set<String> = ["leaf-b"]) async -> [AforaChatOutboxCommand]?
    {
...
final class ChatTranscriptCacheStoreTests: ClientDatabaseTestSuite, @unchecked Sendable {
    ...
    @Test func `full removal deletes both databases legacy files and sidecars`() async throws {
        ...
        try AforaClientDatabases.removeDatabaseFiles(in: directory)

        for filename in [
            AforaClientDatabases.gatewayCacheFilename,
            AforaClientDatabases.clientStateFilename,
    ...
    @Test func `cache format mismatch rebuilds without touching client state`() async throws {
        let stateIdentity = try #require(AforaChatSessionRoutingIdentity(
            scope: "per-sender",
            mainSessionKey: "main",
            defaultAgentID: "main"))
        ...
        let reopened = try AforaClientDatabases(directoryURL: directory)
    ...
    @Test func `corrupt cache rebuilds while corrupt client state is preserved`() async throws {
        ...
        let repaired = try AforaClientDatabases(directoryURL: cacheDirectory)
        ...
        #expect(throws: (any Error).self) {
            _ = try AforaClientDatabases(directoryURL: stateDirectory)
        }
    ...
    @Test func `cache bounds sessions messages and transcript partitions`() async {
        let sessions = (0..<(AforaChatSQLiteTranscriptCache.maxCachedSessions + 10)).map {
            cacheSessionEntry(key: "s\($0)", updatedAt: Double($0))
        }
        await store.storeSessions(sessions)
        #expect(await store.loadSessions().count == AforaChatSQLiteTranscriptCache.maxCachedSessions)
        #expect(await store.loadSessions().contains(where: { $0.key == "s0" }) == false)

        let messages = (0..<(AforaChatSQLiteTranscriptCache.maxCachedMessagesPerSession + 20)).map {
        ...
        #expect(await store.loadTranscript(sessionKey: "bounded").count ==
            AforaChatSQLiteTranscriptCache.maxCachedMessagesPerSession)
        #expect(await messageTexts(store.loadTranscript(sessionKey: "bounded")).first == "m20")

        for index in 0...AforaChatSQLiteTranscriptCache.maxCachedTranscripts {
        ...
        #expect(await store.loadTranscript(
            sessionKey: "partition-\(AforaChatSQLiteTranscriptCache.maxCachedTranscripts)").isEmpty == false)
    }
    ...
    @Test func `cache projection strips payloads and keeps bounded diffs`() throws {
        ...
        let cached = try #require(AforaChatSQLiteTranscriptCache.cacheableMessages([message]).first)
    ...
    @Test func `staged gateway removal reconciles against the pairing registry`() async throws {
        ...
        let registered = try AforaClientDatabases(
        ...
        let forgotten = try AforaClientDatabases(
            directoryURL: directory,
        ...
        #expect(probe.hash == AforaClientDatabases.gatewayIdentityHash("gw-a") && probe.gatewayID == nil)
    }
    ...
    @Test func `staged gateway removal uses exact registry identifier bytes`() async throws {
        ...
        let recovered = try AforaClientDatabases(
            directoryURL: directory,
        ...
        #expect(probe.hash == AforaClientDatabases.gatewayIdentityHash(composedGatewayID) && probe.gatewayID == nil)
    }
    ...
    @Test func `hash only scrub marker remains recoverable`() async throws {
        ...
        let recovered = try AforaClientDatabases(directoryURL: directory)
        let probe = try await forgottenGatewayProbe(in: recovered, identity: "gw-a")
        #expect(probe.cleanupPhase == 0)
    }
    ...
    @Test func `gateway removal scrubs its payloads from shared database files`() async throws {
        ...
        let attachment = AforaChatOutboxAttachment(
            type: "file",
...
final class ClientDatabaseLegacyImportTests: TemporaryDatabaseTestSuite, @unchecked Sendable {
    ...
    @Test func `legacy v2 outbox imports parked into client state`() async throws {
        ...
        #expect(commands.map(\.lastError) == [AforaChatSQLiteTranscriptCache.outboxUnknownTargetError])
    ...
    @Test func `forget removes an unreadable per gateway legacy database and sidecars`() throws {
        ...
        let legacyURL = AforaClientDatabases.legacyPerGatewayDatabaseURL(
            gatewayID: gatewayID,
        ...
        let databases = try AforaClientDatabases(
        ...
        } == AforaClientDatabases.gatewayIdentityHash(gatewayID))
        try databases.close()
        for suffix in ["", "-wal", "-shm"] {
            let url = URL(fileURLWithPath: databaseDirectory
                .appendingPathComponent(AforaClientDatabases.clientStateFilename).path + suffix)
    ...
    @Test func `legacy import accepts only gateways in the pairing registry`() async throws {
        let keptURL = AforaClientDatabases.legacyPerGatewayDatabaseURL(
            gatewayID: "gw-kept",
            directoryURL: directory)
        let orphanedURL = AforaClientDatabases.legacyPerGatewayDatabaseURL(
    ...
    @Test func `unknown or corrupt legacy files remain untouched`() async throws {
        ...
        _ = try AforaClientDatabases(directoryURL: directory)
        ...
        let databases = try AforaClientDatabases(directoryURL: corruptDirectory)
...
final class ChatCommandOutboxStoreTests: ClientDatabaseTestSuite, @unchecked Sendable {
    @Test func `nil agent rows use the canonical empty scope owner`() async throws {
        let scope = AforaChatOutboxScope(sessionKey: "main", agentID: nil)
    ...
    @Test func `parked accepted rows mint retry identity while queued rows keep it`() async throws {
        let sendingScope = AforaChatOutboxScope(sessionKey: "sending", agentID: "main")
        ...
        let queuedScope = AforaChatOutboxScope(sessionKey: "queued", agentID: "main")
        ...
        let requeuedScope = AforaChatOutboxScope(sessionKey: "requeued", agentID: "main")
    ...
    @Test func `parked accepted attachment follows the fresh retry identity`() async throws {
        let scope = AforaChatOutboxScope(sessionKey: "main", agentID: "main")
        let attachment = AforaChatOutboxAttachment(
            type: "audio",
            mimeType: "audio/m4a",
            fileName: "note.m4a",
    ...
    @Test func `empty root reconcile permits first row and parks a wiped scope`() async throws {
        let root = AforaChatOutboxScope(sessionKey: "root", agentID: "main")
        ...
        let wiped = AforaChatOutboxScope(sessionKey: "wiped", agentID: "main")
    ...
    @Test func `interrupted sends fail closed once`() async throws {
        ...
        #expect(recovered.lastError == AforaChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(await store.recoverInterruptedSends())
    }
    ...
    @Test func `stale queued and acknowledged commands require user action`() async throws {
        let old = Date().timeIntervalSince1970 - AforaChatSQLiteTranscriptCache.outboxCommandMaxAge - 1
        ...
        #expect(commandsByID["old-queued"]?.lastError == AforaChatSQLiteTranscriptCache.outboxExpiredError)
        #expect(commandsByID["old-ack"]?.status == .failed)
        #expect(commandsByID["old-ack"]?.lastError == AforaChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }
    ...
    @Test func `queue and attachment budgets are gateway scoped`() async {
        ...
        let oversized = AforaChatOutboxAttachment(
            type: "file",
            mimeType: "application/octet-stream",
            fileName: "large.bin",
            data: Data(count: AforaChatSQLiteTranscriptCache.maxAttachmentBytesPerCommand + 1))
        #expect(await storeB.enqueueCommand(outboxCommand(
            id: "too-large",
            text: "large",
            attachments: [oversized])) == false)
        #expect(AforaChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
            commandBytes: 1,
            queuedBytes: AforaChatSQLiteTranscriptCache.maxQueuedAttachmentBytes - 1))
        #expect(!AforaChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
            commandBytes: 2,
            queuedBytes: AforaChatSQLiteTranscriptCache.maxQueuedAttachmentBytes - 1))
    }
