...
struct ChatSessionSidebarModelTests {
    private func entry(
        ...
        endedAt: Double? = nil,
        hasActiveSubagentRun: Bool? = nil) -> AforaChatSessionEntry
    {
    ...
    @Test func `gateway groups preserve canonical session order`() {
        let sections = ChatSessionSidebarModel.sections(
            ...
            currentSessionKey: "recent",
            groups: [AforaChatSessionGroup(name: "Projects", position: 0)],
            query: "")
    ...
    @Test func `session list hierarchy fields decode with gateway spellings`() throws {
        let data = try #require("""
        {
          ...
          "runtimeMs": 2000,
          "agentRuntime": {"id": "codex", "fallback": "afora", "source": "session"},
          "worktree": {"id": "wt-1", "branch": "feature/chat", "repoRoot": "/repo"}
        }
        """.data(using: .utf8))

        let entry = try JSONDecoder().decode(AforaChatSessionEntry.self, from: data)
    ...
    @Test func `inspector derives gateway metadata without composer overrides`() {
        let session = AforaChatSessionEntry(
            ...
            category: "Projects",
            hasActiveRun: true,
            worktree: AforaChatSessionWorktree(
                id: "wt-1",
                branch: "feature/review",
                repoRoot: "/repo"),
            runtimeMs: 1500,
            agentRuntime: AforaChatAgentRuntime(id: "codex", fallback: nil, source: "session"))
    ...
    @Test func `unknown session change requests an authoritative refetch`() {
        let change = AforaChatSessionsChangedEvent(
    ...
    @Test func `partial session change preserves observer and status metadata`() throws {
        let agentStatus = AforaChatSessionAgentStatus(
            note: "Waiting on review",
            expiresAt: 5000,
            attention: "hand")
        let observerDigest = AforaChatSessionObserverDigest(
            runId: "run-1",
            revision: 3,
            updatedAt: 300,
            headline: "Reviewing",
    ...
    @Test func `subtitle precedence keeps attention and status above observer digest`() {
        let digest = AforaChatSessionObserverDigest(
            ...
            headline: "Observer",
            health: "on-track")
        let agent = AforaChatSessionAgentStatus(
            note: "Agent note",
            expiresAt: 10000,
            attention: nil)
