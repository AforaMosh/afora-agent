...
/// Replays scripted gateway traffic against `AforaChatViewModel` with deterministic
...
private final class ScriptedChatTransport: @unchecked Sendable, AforaChatTransport {
    private actor State {
        var history: AforaChatHistoryPayload
        var historyRequestCount = 0
        var sentRunIds: [String] = []

        init(history: AforaChatHistoryPayload) {
            self.history = history
        }

        func setHistory(_ payload: AforaChatHistoryPayload) {
            self.history = payload
        }

        func recordHistoryRequest() -> AforaChatHistoryPayload {
    ...
    func setHistory(_ payload: AforaChatHistoryPayload) async {
        ...
        attachments _: [AforaChatAttachmentPayload]) async throws -> AforaChatSendResponse
    {
        ...
        return AforaChatSendResponse(runId: idempotencyKey, status: "pending")
    }
    ...
    {
        AforaChatSessionsListResponse(ts: nil, path: nil, count: 0, defaults: nil, sessions: [])
    }
...
private func replayHistory(
    ...
    messages: [AnyCodable] = []) -> AforaChatHistoryPayload
{
...
/// `__afora.idempotencyKey` metadata used for turn correlation.
...
private func replaySessionMessageEvent(
    ...
    messageId: String) -> AforaChatTransportEvent
{
    .sessionMessage(
...
private func replayFinalEvent(
    runId: String,
    text: String,
    timestamp: Double) -> AforaChatTransportEvent
{
    .chat(
...
private func replayAssistantDeltaEvent(
    ...
    seq: Int) -> AforaChatTransportEvent
{
    .agent(
...
private struct StreamReplayHarness {
    ...
    static func bootstrapped(
        initialHistory: AforaChatHistoryPayload = replayHistory()) async throws -> StreamReplayHarness
    {
    ...
    func converge(
        _ label: String,
        _ condition: @escaping @MainActor @Sendable (AforaChatViewModel) -> Bool) async throws
    {
        let vm = self.vm
...
extension AforaChatViewModel {
    fileprivate var replayAssistantRows: [AforaChatMessage] {
        self.messages.filter { $0.role == "assistant" }
    }

    fileprivate func replayAssistantRows(text: String) -> [AforaChatMessage] {
        self.replayAssistantRows.filter { message in
            message.content.compactMap(\.text).joined() == text
        }
    }

    fileprivate var replayUserRows: [AforaChatMessage] {
        ...
        let frame = EventFrame(
            ...
            payload: AnyCodable([
                ...
                "message": [
                    ...
                    "content": [],
                    "timestamp": 1,
                    "__afora": ["kind": "reset", "id": "live-reset"],
                ],
            ]))
        let event = try #require(AforaChatGatewayPayloadCodec.event(from: frame))
