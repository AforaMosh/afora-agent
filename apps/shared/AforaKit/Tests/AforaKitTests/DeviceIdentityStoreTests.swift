...
private final class DeviceIdentityMigrationFixture {
    ...
    init(destinationName: String = "destination", databasePath: String = "afora.sqlite") {
...
struct DeviceIdentityStoreTests {
    @Test func `process state root configures once before identity use`() {
        ...
        let work = URL(fileURLWithPath: "/Users/test/.afora-work", isDirectory: true)
        let other = URL(fileURLWithPath: "/Users/test/.afora-other", isDirectory: true)
    ...
    func `task scoped state directories isolate concurrent identity stores`() async throws {
        ...
        for stateDirectory in stateDirectories {
            #expect(FileManager.default.fileExists(
                atPath: stateDirectory.appendingPathComponent("state/afora.sqlite").path))
        }
    }
    ...
    func `device auth store reports failed durable writes`() throws {
        ...
        setenv("AFORA_STATE_DIR", blocker.path, 1)
    ...
    func `device auth owners preserve exact unicode bytes`() throws {
        ...
        let stateDirPath = try #require(getenv("AFORA_STATE_DIR").map { String(cString: $0) })
        let stateDirURL = URL(fileURLWithPath: stateDirPath, isDirectory: true)
        #expect(try Self.scalarInt(
            stateDirURL.appendingPathComponent("state/afora.sqlite"),
    ...
    func `secondary profiles use separate identity and auth rows`() throws {
        ...
        // getenv, not ProcessInfo: the trait pins AFORA_STATE_DIR via setenv and
        ...
        #expect(try Self.scalarInt(
            stateDir.appendingPathComponent("state/afora.sqlite"),
    ...
    func `gateway identity failure names corrupt row and state directory without rotating it`() async throws {
        let fixture = DeviceIdentityMigrationFixture(databasePath: "state/afora.sqlite")
        try Self.seedCanonicalSchema(fixture.databaseURL)
        try Self.execute(fixture.databaseURL, """
        INSERT INTO device_identities (
          identity_key, device_id, public_key_pem, private_key_pem, created_at_ms, updated_at_ms
    ...
    func `migration imports auth rows without removing its source`() throws {
        let fixture = DeviceIdentityMigrationFixture(
            ...
            databasePath: "state/afora.sqlite")
