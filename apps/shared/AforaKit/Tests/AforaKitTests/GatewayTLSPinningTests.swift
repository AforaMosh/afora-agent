...
private final class GatewayTLSFakeKeychain: @unchecked Sendable {
    ...
    func seed(account: String, data: Data) {
        ...
        self.items[account] = [
            ...
            kSecAttrService as String: "ai.afora.tls-pinning",
...
struct GatewayTLSPinningTests {
    @Test func `keychain namespace configures once and fails closed after use`() {
        ...
        let workService = state.service(base: "ai.afora.tls-pinning")
        ...
        #expect(workService == "ai.afora.tls-pinning.profile.work")
        #expect(configuredWorkAfterUse)
        #expect(!configuredDefaultAfterUse)

        var usedDefault = GatewayTLSKeychainNamespaceState()
        let defaultService = usedDefault.service(base: "ai.afora.tls-pinning")
        ...
        #expect(defaultService == "ai.afora.tls-pinning")
