...
struct WatchCommandsTests {
    @Test func `app snapshot dual writes semantic and legacy status fields`() throws {
        let message = AforaWatchAppSnapshotMessage(
            ...
            chatStatus: AforaWatchAppStatus(code: .chatNoMessages),
    ...
    @Test func `semantic chat status always writes the shipped text field`() throws {
        let message = AforaWatchAppSnapshotMessage(
            ...
            talkSpeaking: false,
            pendingApprovalCount: 0,
            chatStatus: AforaWatchAppStatus(code: .chatUnavailable))
    ...
    @Test func `unknown semantic statuses fall back to legacy text`() throws {
        ...
        let message = try JSONDecoder().decode(
            AforaWatchAppSnapshotMessage.self,
            from: Data(json.utf8))

        #expect(message.gatewayStatus == AforaWatchAppStatus(
            code: .legacy,
            verbatim: "Future gateway state"))
        #expect(message.talkStatus == AforaWatchAppStatus(
            code: .legacy,
            verbatim: "Future Talk state"))
        #expect(message.chatStatus == AforaWatchAppStatus(
            code: .legacy,
