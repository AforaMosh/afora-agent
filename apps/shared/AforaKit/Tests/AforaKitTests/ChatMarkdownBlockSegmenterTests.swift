...
struct ChatMarkdownBlockSegmenterTests {
    ...
    @Test @MainActor func `native list items reject compressed multiline heights`() {
        let samples = [
            "The issue is not posted by an official Afora organization account or a separate assistant account, so the complete author identity guidance must remain visible to the reader.",
