...
@Suite struct DeepLinksSecurityTests {
    ...
    @Test func gatewayDeepLinkRejectsInsecureNonLoopbackWs() {
        let url = URL(
            string: "afora://gateway?host=attacker.example&port=18789&tls=0&token=abc")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func gatewayDeepLinkRejectsInsecurePrefixBypassHost() {
        let url = URL(
            string: "afora://gateway?host=127.attacker.example&port=18789&tls=0&token=abc")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func gatewayDeepLinkAllowsLoopbackWs() {
        let url = URL(
            string: "afora://gateway?host=127.0.0.1&port=18789&tls=0&token=abc")!
        #expect(
    ...
    @Test func setupCodePreservesPrimaryGatewayContextPath() {
        let payload = #"{"url":"wss://gateway.example/afora-gw","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))

        #expect(link?.contextPath == "/afora-gw")
        #expect(link?.websocketURL?.absoluteString == "wss://gateway.example:443/afora-gw")
    }

    @Test func setupCodeDecodesGatewayContextPathExactlyOnce() {
        let payload = #"{"url":"wss://gateway.example/afora%20gateway","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))

        #expect(link?.contextPath == "/afora%20gateway")
        #expect(link?.websocketURL?.absoluteString == "wss://gateway.example:443/afora%20gateway")
    }

    @Test func setupCodePreservesEscapedGatewayPathDelimiter() {
        let payload = #"{"url":"wss://gateway.example/afora%2Fgateway","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))

        #expect(link?.contextPath == "/afora%2Fgateway")
        #expect(link?.websocketURL?.absoluteString == "wss://gateway.example:443/afora%2Fgateway")
    }

    @Test func setupCodePreservesNonUTF8GatewayPathOctet() {
        let payload = #"{"url":"wss://gateway.example/afora%FFgateway","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))

        #expect(link?.contextPath == "/afora%FFgateway")
        #expect(link?.websocketURL?.absoluteString == "wss://gateway.example:443/afora%FFgateway")
    }
    ...
    @Test func setupCodeAllowsMDNSWs() {
        let payload = #"{"url":"ws://afora.local:18789","bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "afora.local",
                port: 18789,
    ...
    @Test func setupCodeRejectsGatewayURLMetadata() {
        let urls = [
            "wss://user@gateway.example/afora-gw",
            "wss://gateway.example/afora-gw?mode=setup",
            "wss://gateway.example/afora-gw#fragment",
        ]
    ...
    @Test func setupCodeAllowsPrivateLanHostPayload() {
        let payload = #"{"host":"afora.local","port":18789,"tls":false,"bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "afora.local",
                port: 18789,
