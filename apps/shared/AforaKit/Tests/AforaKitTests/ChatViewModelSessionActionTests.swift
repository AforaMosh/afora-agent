...
private func sessionActionOutboxCommand(
    id: String,
    text: String) -> AforaChatOutboxCommand
{
...
private final class LegacyForkTransport: @unchecked Sendable, AforaChatTransport {
    let state = LegacyForkTransportState()

    func requestHistory(sessionKey _: String) async throws -> AforaChatHistoryPayload {
...
private final class SessionActionTransport: @unchecked Sendable, AforaChatTransport {
    ...
    private let rewindEditorAttachments: [AforaChatEditorAttachment]?
    private let forkAtMessageSessionKey: String
    private let forkAtMessageEditorText: String?
    private let forkAtMessageEditorAttachments: [AforaChatEditorAttachment]?
    private let branches: [AforaChatSessionBranch]
    private let branchListResponses: [[AforaChatSessionBranch]]
    ...
    init(
        ...
        rewindEditorAttachments: [AforaChatEditorAttachment]? = nil,
        forkAtMessageSessionKey: String = "forked-at-message",
        forkAtMessageEditorText: String? = "forked draft",
        forkAtMessageEditorAttachments: [AforaChatEditorAttachment]? = nil,
        branches: [AforaChatSessionBranch] = [],
        branchListResponses: [[AforaChatSessionBranch]] = [],
        ...
        entryId: String) async throws -> AforaChatRewindResponse
    {
        ...
        return AforaChatRewindResponse(
        ...
        entryId: String) async throws -> AforaChatForkAtMessageResponse
    {
        ...
        return AforaChatForkAtMessageResponse(
        ...
        agentID _: String?) async throws -> AforaChatSessionBranchesResponse
    {
        ...
        let branches = self.branchListResponses.indices.contains(callIndex)
            ...
            : self.branches
        return AforaChatSessionBranchesResponse(branches: branches)
    }
    ...
    func acquireSessionGroupsRouteLease() async -> AforaChatSessionGroupsRouteLease? {
        ...
        return AforaChatSessionGroupsRouteLease(
            listGroups: {
                AforaChatSessionGroupsResponse(groups: [
                    AforaChatSessionGroup(name: "Existing", position: 0),
                ])
            },
            putGroups: { names in
                await state.recordGroupPut(names)
                return AforaChatSessionGroupsMutationResponse(
                    ok: true,
                    groups: names.enumerated().map {
                        AforaChatSessionGroup(name: $0.element, position: $0.offset)
                    },
                    updatedSessions: nil)
            },
            renameGroup: { _, _ in
                AforaChatSessionGroupsMutationResponse(ok: true, groups: [], updatedSessions: nil)
            },
            deleteGroup: { _ in
                AforaChatSessionGroupsMutationResponse(ok: true, groups: [], updatedSessions: nil)
            })
    ...
    func acquireNewSessionRouteLease() async -> AforaChatNewSessionRouteLease? {
        ...
        return AforaChatNewSessionRouteLease(
            listAgents: {
                AforaChatAgentsListResponse(
                    defaultId: "worker",
                    agents: [AforaChatAgentChoice(id: "worker", workspaceGit: true)])
            },
            createSession: { key, _, agentID, parentKey, _, _ in
                await state.recordCreate(agentID: agentID, parentKey: parentKey)
                return AforaChatCreateSessionResponse(ok: true, key: key, sessionId: nil)
            })
    }
...
struct ChatViewModelSessionActionTests {
    ...
    @Test func `remote group mutations bump the catalog revision`() async {
        ...
        let viewModel = await MainActor.run {
            AforaChatViewModel(sessionKey: "main", transport: transport)
        }
    ...
    @Test func `batch delete rejects current session while attachment owner is pinned`() async {
        let transport = SessionActionTransport()
        let viewModel = AforaChatViewModel(sessionKey: "worker", transport: transport)
        viewModel.attachments = [AforaPendingAttachment(
            url: nil,
            data: Data([1]),
            fileName: "draft.png",
            mimeType: "image/png",
    ...
    @Test func `rewind seeds editor and refreshes history`() async {
        ...
        let transport = SessionActionTransport(
            ...
            rewindEditorAttachments: [
                AforaChatEditorAttachment(
                    mimeType: "image/png",
                    data: imageData.base64EncodedString()),
                AforaChatEditorAttachment(mimeType: "image/png", data: "%%%"),
            ])
        let viewModel = AforaChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test func `fork at message waits for current session outbox confirmation`() async throws {
        let directory = try makeSessionActionOutboxDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try AforaClientDatabases(directoryURL: directory).store(gatewayID: "gw-test")
        #expect(await store.enqueueCommand(sessionActionOutboxCommand(
            id: "fork-pending",
    ...
    @Test func `rewind bumps branch epoch and parks a racing enqueue`() async throws {
        ...
        #expect(AforaChatSQLiteTranscriptCache.outboxDisplayError(racedCommand.lastError) ==
    ...
    @Test func `branch message count uses localized singular and plural forms`() {
        #expect(AforaChatComposer.branchMessageCount(1) == "1 message")
        #expect(AforaChatComposer.branchMessageCount(2) == "2 messages")
    }
    ...
    @Test func `fork at message switches and seeds editor`() async {
        ...
        let transport = SessionActionTransport(
            ...
            forkAtMessageEditorAttachments: [AforaChatEditorAttachment(
                mimeType: "image/webp",
                data: imageData.base64EncodedString())])
        let viewModel = AforaChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test func `boundary-aware fork remains compatible with legacy transports`() async throws {
        ...
        let transport: any AforaChatTransport = legacy
    ...
    private func waitForBranchSwitchActivityToClear(
        _ viewModel: AforaChatViewModel,
        timeout: Duration = .seconds(15)) async -> Bool
    {
        let clock = ContinuousClock()
        let deadline = clock.now + timeout
    ...
    private func userMessage(entryID: String) -> AforaChatMessage {
        AforaChatMessage(
            role: "user",
            content: [],
            timestamp: nil,
    ...
    private func confirmingMessage(commandID: String) -> AforaChatMessage {
        AforaChatMessage(
            role: "user",
            content: [],
            timestamp: nil,
    ...
    private func branches(activeLeafEntryID: String = "leaf-active") -> [AforaChatSessionBranch] {
        [
            AforaChatSessionBranch(
    ...
    private func entry(
        ...
        hasActiveRun: Bool? = nil) -> AforaChatSessionEntry
    {
