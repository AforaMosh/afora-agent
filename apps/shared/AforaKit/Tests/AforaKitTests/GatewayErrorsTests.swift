...
struct GatewayErrorsTests {
    ...
    @Test func `connect auth error preserves structured metadata`() {
        let error = GatewayConnectAuthError(
            ...
            actionCommand: "afora devices approve req-123",
            docsURLString: "https://docs.afora.ai/gateway/pairing",
        ...
        #expect(error.actionCommand == "afora devices approve req-123")
        #expect(error.docsURLString == "https://docs.afora.ai/gateway/pairing")
    ...
    @Test func `protocol mismatch maps older app to update problem`() {
        ...
        #expect(problem?.message == "This app is older than the gateway. Update Afora on this device, then retry.")
    ...
    @Test func `protocol mismatch maps older gateway to update problem`() {
        ...
        #expect(problem?
            .message == "The gateway is older than this app. Update Afora on the gateway host, then retry.")
        #expect(problem?.actionLabel == "Copy update command")
        #expect(problem?.actionCommand == "afora update")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `protocol mismatch without versions still gives actionable fallback`() {
        ...
        #expect(problem?
            .message == "The app and gateway use incompatible protocol versions. Update Afora on both, then retry.")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `pairing problem uses structured request metadata`() {
        ...
        #expect(problem?.pauseReconnect == true)
        #expect(problem?.actionCommand == "afora devices approve req-123")
    }
    ...
    private static let troubleshootingDocs = "https://docs.afora.ai/gateway/troubleshooting"
        ...
        technicalDetails: String) -> GatewayConnectionProblem
    {
        ...
        case .reachabilityFailed:
            facts = (
                "Gateway is not reachable", "Afora could not reach the gateway over the current network.",
                "Check network")
