...
private actor AttachmentSendCapture {
    private(set) var attachments: [AforaChatAttachmentPayload] = []
    private(set) var sends = 0

    func store(_ attachments: [AforaChatAttachmentPayload]) {
    ...
    func first() -> AforaChatAttachmentPayload? {
...
private struct AttachmentProcessingTransport: AforaChatTransport {
        ...
        attachments: [AforaChatAttachmentPayload]) async throws -> AforaChatSendResponse
    {
        ...
        return AforaChatSendResponse(runId: idempotencyKey, status: self.responseStatus)
    }
...
private func makeAttachmentOutbox() throws -> AforaChatSQLiteTranscriptCache {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("attachment-outbox-\(UUID().uuidString)", isDirectory: true)
    return try AforaClientDatabases(directoryURL: directory).store(gatewayID: "attachment-tests")
}

@MainActor
private func makeDurableAttachmentViewModel(
    transport: AttachmentProcessingTransport,
    outbox: AforaChatSQLiteTranscriptCache) -> AforaChatViewModel
{
    AforaChatViewModel(
        sessionKey: "main",
        transport: transport,
...
final class ChatViewModelAttachmentTests: XCTestCase {
    func testImageAttachmentsAreProcessedBeforeStaging() async throws {
        ...
        let viewModel = await MainActor.run {
            AforaChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testVideoFileUsesServerDefaultTwentyMiBCap() async throws {
        ...
        try Data(count: AforaChatViewModel.maxVideoAttachmentBytes + 1).write(to: fileURL)
        defer { try? FileManager.default.removeItem(at: fileURL) }
        let viewModel = await MainActor.run {
            AforaChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testUnsupportedAudioFileIsRejectedBeforeReadingItsPayload() async throws {
        ...
        try handle.truncate(atOffset: UInt64(AforaChatViewModel.maxVideoAttachmentBytes * 10))
        try handle.close()
        defer { try? FileManager.default.removeItem(at: fileURL) }
        let viewModel = await MainActor.run {
            AforaChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
    ...
    func testPartialIdentitySyncPreservesTheOtherDeferredComponent() async {
        let state = await MainActor.run {
            ...
            let contractViewModel = AforaChatViewModel(
                sessionKey: "main",
                transport: AttachmentProcessingTransport(),
                activeAgentId: "main",
                sessionRoutingContract: oldContract)
            let contractAttachment = AforaPendingAttachment(
                url: nil,
            ...
            let agentViewModel = AforaChatViewModel(
                sessionKey: "main",
                transport: AttachmentProcessingTransport(),
                activeAgentId: "main",
                sessionRoutingContract: oldContract)
            let agentAttachment = AforaPendingAttachment(
                url: nil,
    ...
    func testIndeterminateOutboxRouteRetainsAttachmentAndRetriesOnceAvailable() async throws {
        ...
        let attachmentID = await MainActor.run {
            let attachment = AforaPendingAttachment(
                url: nil,
                data: attachmentData,
                fileName: "retry.jpg",
                mimeType: "image/jpeg",
    ...
    func testVoiceNoteSendKeepsCapturedDurationWhenDraftChangesDuringHealthCheck() async throws {
        ...
        let (viewModel, draftAttachmentID) = await MainActor.run {
            ...
            let draftAttachment = AforaPendingAttachment(
                url: nil,
    ...
    func testAmbiguousVoiceNoteSurvivesViewModelRecreation() async throws {
        let outbox = try makeAttachmentOutbox()
        var firstViewModel: AforaChatViewModel? = await MainActor.run {
            let viewModel = makeDurableAttachmentViewModel(
    ...
    func testCanonicalVoiceNoteConfirmationPreservesDurationAndDeletesDurableBytes() async throws {
        ...
        let canonical = try JSONDecoder().decode(
            ...
            from: Data(
                """
                {"role":"user","content":"See attached.","__afora":{"idempotencyKey":"\(command
    ...
    func testCanonicalVoiceNotePreservesOptimisticDuration() throws {
        let localAudio = AforaChatMessageContent(
            type: "file",
            text: nil,
            mimeType: "audio/mp4",
        ...
        let incoming = try JSONDecoder().decode(
            AforaChatMessage.self,
            from: Data(
                #"{"role":"user","content":"See attached.","__afora":{"idempotencyKey":"run:user"},"MediaPaths":["media/inbound/media-1.m4a"],"MediaTypes":["audio/mp4"]}"#
                    .utf8))
