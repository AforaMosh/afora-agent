...
struct ChatSwarmProgressTests {
    @Test func `session rows decode Swarm identity and activity fields`() throws {
        ...
        """.utf8)
        let row = try JSONDecoder().decode(AforaChatSessionEntry.self, from: data)
    ...
    @Test func `activity notes decorate children in observation order`() {
        ...
        var activity = AforaChatSwarmActivityState()

        let observedPhase = activity.observe(AforaChatSessionsChangedEvent(
            sessionKey: "agent:main:parent",
            reason: "swarm-note",
            swarmGroupId: groupID,
            kind: "phase",
            text: "Research"))
        let observedChild = activity.observe(AforaChatSessionsChangedEvent(
            sessionKey: "agent:main:child",
            reason: "create",
            swarmGroupId: groupID))
        let observedLog = activity.observe(AforaChatSessionsChangedEvent(
            sessionKey: "agent:main:parent",
            reason: "swarm-note",
            swarmGroupId: groupID,
            kind: "log",
    ...
    @Test func `projection renders active group states and hides terminal groups`() {
        ...
        let groups = buildAforaChatSwarmGroups(sessions: sessions) { $0 == "agent:main:parent" }
    ...
    @Test func `child pager repeats from zero when rows move across offsets`() async throws {
        ...
        var call = 0
        let rows = try await AforaChatChildSessionPager.collect { offset in
            let page = pages[call]
            call += 1
    ...
    @Test func `metadata capability defaults missing Swarm support to disabled`() throws {
        let capabilities = try JSONDecoder().decode(
            AforaChatMetadataCapabilities.self,
            from: Data("{}".utf8))
    ...
    @Test func `Swarm events ignore other parents`() {
        ...
        let ownChild = AforaChatSessionsChangedEvent(
            sessionKey: "agent:main:child",
            parentSessionKey: current,
            reason: "create",
            swarmGroupId: "custom-group")
        let otherPhase = AforaChatSessionsChangedEvent(
    ...
    @Test func `projection uses derived title before the raw session key`() {
        ...
        let group = buildAforaChatSwarmGroups(sessions: [row]) { $0 == "agent:main:parent" }.single
    ...
    @Test func `projection caps historical dots while retaining active workers`() {
        ...
        let group = buildAforaChatSwarmGroups(
    ...
    private func session(
        ...
        status: String?,
        groupID: String,
        subagentRunState: String? = nil) -> AforaChatSessionEntry
    {
