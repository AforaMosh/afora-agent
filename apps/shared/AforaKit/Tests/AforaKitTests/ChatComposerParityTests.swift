...
private final class ComposerParityTransport: @unchecked Sendable, AforaChatTransport {
        ...
        attachments _: [AforaChatAttachmentPayload]) async throws -> AforaChatSendResponse
    {
        ...
        return AforaChatSendResponse(runId: idempotencyKey, status: "started")
    }
...
struct ChatComposerStateTests {
    @Test func `file picker allows images and movie containers only`() {
        #expect(AforaChatPickerAttachmentMetadata.allowedFileContentTypes == [
            .image,
            .movie,
            .mpeg4Movie,
            .quickTimeMovie,
        ])
        #expect(!AforaChatPickerAttachmentMetadata.allowedFileContentTypes.contains(.audiovisualContent))
        #expect(!AforaChatPickerAttachmentMetadata.allowedFileContentTypes.contains(.audio))
    }

    @Test func `tagless movie metadata stages as video`() async throws {
        let metadata = AforaChatPickerAttachmentMetadata.resolve(contentType: .movie)
        ...
        let vm = AforaChatViewModel(sessionKey: "main", transport: ComposerParityTransport())
    ...
    @Test func `accepted normal send quotes and clears reply and stored draft`() async throws {
        ...
        let vm = AforaChatViewModel(sessionKey: "main", transport: transport)
        vm.healthOK = true
        vm.input = "continue"
    ...
    @Test func `accepted attachment only send quotes and clears reply`() async throws {
        ...
        let vm = AforaChatViewModel(sessionKey: "main", transport: transport)
        vm.healthOK = true
        vm.attachments = [AforaPendingAttachment(
            url: nil,
            data: Data([1, 2, 3]),
            fileName: "image.png",
            mimeType: "image/png",
