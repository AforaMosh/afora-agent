...
struct ComputerInputGeometryTests {
    private func display(
        ...
        originY: Double = 0,
        width: Double,
        height: Double) -> AforaComputerDisplayGeometry
    {
        AforaComputerDisplayGeometry(
            originX: originX,
            originY: originY,
            widthPoints: width,
            heightPoints: height)
    ...
    private func capturedWidth(refWidth: Int?, sourceWidth: Double, sourceHeight: Double = 0) -> Double {
        AforaComputerInputGeometry.capturedWidth(
            refWidth: refWidth,
    ...
    @Test func `display frame identity changes with physical display geometry`() {
        ...
        let original = AforaComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let repeated = AforaComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let reindexed = AforaComputerInputGeometry.displayFrameId(
            displayID: 43,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let resized = AforaComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 1920,
            sourceHeight: 1200,
            referenceWidth: 1280,
            display: display)
        let moved = AforaComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
        ...
        let rescaled = AforaComputerInputGeometry.displayFrameId(
            displayID: 42,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 640,
    ...
    @Test func `maps one to one when capture matches display`() {
        ...
        let mapped = AforaComputerInputGeometry.mapReferencePointToGlobal(
            x: 100,
            y: 200,
    ...
    @Test func `accepts finite positive mapping geometry with negative display origins`() {
        ...
        #expect(AforaComputerInputGeometry.isValidMappingGeometry(
            sourceWidth: 2560,
            sourceHeight: 1440,
            display: display))
    }
    ...
    @Test func `clamps far-edge and out-of-bounds points strictly inside the display`() {
        ...
        let farEdge = AforaComputerInputGeometry.clampToDisplay(
            x: 100 + 1280,
            y: 200 + 800,
            display: display)
        ...
        let negative = AforaComputerInputGeometry.clampToDisplay(x: 99, y: 199, display: display)
        #expect(negative.x == 100)
        #expect(negative.y == 200)
        // A point already inside is unchanged.
        let inside = AforaComputerInputGeometry.clampToDisplay(x: 640, y: 400, display: display)
        #expect(inside.x == 640)
        #expect(inside.y == 400)
    }
    ...
    @Test func `decodes partial act params`() throws {
        ...
        let params = try JSONDecoder().decode(AforaComputerActParams.self, from: data)
    ...
    @Test func `decodes scroll and hold actions`() throws {
        ...
        let scrollParams = try JSONDecoder().decode(AforaComputerActParams.self, from: scroll)
        ...
        let holdParams = try JSONDecoder().decode(AforaComputerActParams.self, from: hold)
    ...
    @Test func `v1 result encoding omits every additive v2 field`() throws {
        let data = try JSONEncoder().encode(AforaComputerActResult(
            ok: true,
            cursorX: 12,
            cursorY: 34))
    ...
    @Test func `v2 result encoding matches the shared result envelope`() throws {
        let result = AforaComputerActResult(
            ok: false,
            ...
            observation: AforaComputerObservation(
                ...
                height: 20,
                observationId: "observation-1",
                elements: [AforaComputerObservationElement(
                    elementRef: "element-1",
                    role: "button",
                    label: "Save",
                    bounds: AforaComputerBounds(x: 1, y: 2, width: 3, height: 4))]),
