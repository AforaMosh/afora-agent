...
struct ChatInlineWidgetTests {
    ...
    @Test func `decodes projected canvas widget block`() throws {
          ...
          "content": [
            {"type": "text", "text": "Done"},
            {
              ...
              "preview": {
                ...
                "title": "Status",
                "preferredHeight": 240,
                "url": "/__afora__/canvas/documents/widget-1/index.html",
                "sandbox": "scripts"
              }
            }
          ]
        ...
        let message = try JSONDecoder().decode(AforaChatMessage.self, from: data)
        let preview = try #require(message.content.last?.preview)
        #expect(preview.inlineWidgetPath == "/__afora__/canvas/documents/widget-1/index.html")
        #expect(preview.inlineWidgetHeight == 240)

        let unsafe = AforaChatCanvasPreview(
            kind: "canvas",
            surface: "assistant_message",
            render: "url",
            title: nil,
    ...
    @Test func `resolves only capability scoped widget documents`() {
        let surface = "https://gateway.example/__afora__/cap/token"
        let target = "/__afora__/canvas/documents/widget-1/index.html"

        #expect(AforaChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: target)?.absoluteString ==
            "https://gateway.example/__afora__/cap/token/__afora__/canvas/documents/widget-1/index.html")
        #expect(AforaChatWidgetURLResolver.resolve(surfaceURL: "https://gateway.example", target: target) == nil)
        #expect(AforaChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "https://attacker.example/widget.html") == nil)
        #expect(AforaChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__afora__/a2ui/index.html") == nil)
        #expect(AforaChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__afora__/canvas/documents/%252e%252e/index.html") == nil)
        #expect(AforaChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__afora__/canvas/documents/%2525252525252525252525252525252525/index.html") == nil)
    }
    ...
    @Test func `resource equality ignores internal recovery metadata`() throws {
        ...
        let publicResource = AforaChatWidgetResource(url: url)
        let trackedResource = AforaChatWidgetResource(
            url: url,
    ...
    @Test func `uses replacement route after capability refresh loses its lease`() async throws {
        let target = "/__afora__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://gateway.example/__afora__/cap/old"
        let newSurface = "https://gateway.example/__afora__/cap/new"
        let failedURL = try #require(AforaChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = AforaChatWidgetResource(url: failedURL)
        ...
        let resolved = await AforaChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(resolved?.url == AforaChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))
    ...
    @Test func `accepts a same URL widget route when it acquires a TLS pin`() async throws {
        ...
        let initialResource = await AforaChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
    ...
    @Test func `refreshes node once before trying the operator fallback`() async {
        ...
        let oldSurface = "https://gateway.example/__afora__/cap/old"
        let newSurface = "https://gateway.example/__afora__/cap/new"
        let fallbackSurface = "https://operator.example/__afora__/cap/fallback"
        let probe = ChatWidgetRouteReconnectProbe(
            route: GatewayCanvasHostRoute(url: oldSurface, tlsFingerprintSHA256: nil),
            replacement: GatewayCanvasHostRoute(url: newSurface, tlsFingerprintSHA256: nil),
            operatorRoute: GatewayCanvasHostRoute(url: fallbackSurface, tlsFingerprintSHA256: nil))
        let initialNode = await AforaChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
            refreshNodeSurfaceRoute: { _ in nil },
        ...
        let refreshedNode = await AforaChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(refreshedNode?.url == AforaChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))

        let fallback = await AforaChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(fallback?.url == AforaChatWidgetURLResolver.resolve(
    ...
    @Test func `refreshes the operator capability when the node route is unavailable`() async throws {
        let target = "/__afora__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://operator.example/__afora__/cap/old"
        let newSurface = "https://operator.example/__afora__/cap/new"
        let failedURL = try #require(AforaChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = AforaChatWidgetResource(url: failedURL)
    ...
    @Test func `rejects a TLS pin on a cleartext widget route`() async {
        ...
        let route = GatewayCanvasHostRoute(
            url: "http://gateway.example/__afora__/cap/token",
