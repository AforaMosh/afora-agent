...
struct ComputerCommandsWireDecodeTests {
    private struct DecodeCase {
        let json: String
        let action: AforaComputerAction
        var app: String?
        var windowRef: String?
        var elementRef: String?
        var observationId: String?
        var deliveryMode: AforaComputerDeliveryMode?
        var query: String?
        var depth: Int?
        var maxElements: Int?
        var value: String?
    ...
    private func decode(_ testCase: DecodeCase) throws -> AforaComputerActParams {
        try JSONDecoder().decode(AforaComputerActParams.self, from: Data(testCase.json.utf8))
    }

    private func expectFields(_ params: AforaComputerActParams, match testCase: DecodeCase) {
    ...
    @Test func `rejects unknown and native-unimplemented action names`() throws {
        for action in ["totally_unknown", "browser_click", "start_recording"] {
            ...
            #expect(throws: DecodingError.self) {
                _ = try JSONDecoder().decode(AforaComputerActParams.self, from: data)
            }
        }
    }
    ...
    @Test func `action raw values match the frozen computer use v2 contract`() {
        ...
        #expect(AforaComputerAction.allCases.map(\.rawValue) == frozenActionNames)
    }
}
