...
private func pickerModel(_ selectionID: String) -> AforaChatModelChoice {
...
@Suite struct ChatModelPickerStoreTests {
    ...
    @Test func `recents dedupe move to front cap and skip invalid ids`() throws {
        try withPickerStore { store, _ in
            store.recordRecent("")
            store.recordRecent(AforaChatViewModel.defaultModelSelectionID)
    ...
    @Test func `missing default provider does not mark other as default`() {
        let unqualified = AforaChatModelChoice(
            modelID: "local-model",
            name: "Local",
            provider: "",
            contextWindow: nil)
    ...
    @Test func `qualified global default matches provider model pair`() {
        ...
        let qualifiedModelID = AforaChatModelChoice(
            modelID: "openai/gpt",
            name: "GPT",
            provider: "openai",
            contextWindow: nil)
