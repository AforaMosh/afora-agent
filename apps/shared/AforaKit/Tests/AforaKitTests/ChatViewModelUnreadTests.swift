...
private actor UnreadTestTransportState {
    ...
    var sessionOverride: [AforaChatSessionEntry]?
...
private final class UnreadTestTransport: @unchecked Sendable, AforaChatTransport {
    ...
    func setSessions(_ sessions: [AforaChatSessionEntry]) async {
...
extension UnreadTestTransportState {
    ...
    fileprivate func setSessions(_ sessions: [AforaChatSessionEntry]) {
...
struct ChatViewModelUnreadTests {
    ...
    @Test func `failed route lease preserves mutation queue ordering`() async throws {
        ...
        let firstLease = AforaChatSessionMutationRouteLease { _, _, _, _, _, _, _ in
            await recorder.append("first-start")
            try await Task.sleep(for: .milliseconds(100))
            await recorder.append("first-end")
        }
        let thirdLease = AforaChatSessionMutationRouteLease { _, _, _, _, _, _, _ in
            await recorder.append("third")
        }

        let first = queue.reserve(
            routeLease: Task<AforaChatSessionMutationRouteLease?, Never> { firstLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)
        let second = queue.reserve(
            routeLease: Task<AforaChatSessionMutationRouteLease?, Never> { nil },
            queueKey: "a",
            routeKey: "a",
            unread: true)
        let third = queue.reserve(
            routeLease: Task<AforaChatSessionMutationRouteLease?, Never> { thirdLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)

        try await first.value
        await #expect(throws: AforaChatTransportSendError.self) {
    ...
    private func viewModel(
        ...
        transport: UnreadTestTransport) -> AforaChatViewModel
    {
    ...
    private func entry(
        ...
        pinned: Bool? = nil) -> AforaChatSessionEntry
    {
