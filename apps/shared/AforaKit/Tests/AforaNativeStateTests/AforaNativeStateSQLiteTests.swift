import Foundation
import Testing
@testable import AforaNativeState

struct AforaNativeStateSQLiteTests {
    @Test
    ...
    func `version zero rejects incomplete known sibling schema`() throws {
        try self.withDatabase { database in
            ...
            #expect(throws: AforaNativeStateError.self) {
                try database.ensureCanonicalTable(.deviceIdentities)
            }
        }
    }
    ...
    func `concurrent version zero stores compose different tables`() async throws {
        ...
        let databaseURL = directory.appendingPathComponent("afora.sqlite", isDirectory: false)

        try await withThrowingTaskGroup(of: Void.self) { group in
            for table in [
                AforaNativeStateCanonicalTable.deviceIdentities,
                .macosPortGuardianRecords,
            ] {
                group.addTask {
                    let database = try AforaNativeStateSQLite(databaseURL: databaseURL)
                    try database.withImmediateTransaction {
    ...
    func `vanished transient sidecar does not fail committed write cleanup`() throws {
        let databaseURL = URL(fileURLWithPath: "/tmp/afora-native-state.sqlite")
        var attemptedPaths: [String] = []

        try AforaNativeStateSQLite.secureDatabaseFiles(
            databaseURL,
    ...
    private func withDatabase(
        body: (AforaNativeStateSQLite) throws -> Void) throws
    {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let database = try AforaNativeStateSQLite(
            databaseURL: directory.appendingPathComponent("afora.sqlite", isDirectory: false))
        try body(database)
    }
}
