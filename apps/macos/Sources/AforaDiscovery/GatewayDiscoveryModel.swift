...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in AforaBonjour.gatewayServiceDomains {
            let browser = GatewayDiscoveryBrowserSupport.makeBrowser(
                serviceType: AforaBonjour.gatewayServiceType,
                domain: domain,
                queueLabelPrefix: "ai.afora.macos.gateway-discovery",
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = AforaBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = AforaBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = AforaBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Afora)", with: "")
