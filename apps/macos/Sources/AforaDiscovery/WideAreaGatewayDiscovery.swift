...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) async -> [WideAreaGatewayBeacon]
    {
        ...
        for raw in ptrLines {
            ...
            let suffix = "._afora-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = AforaBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_afora-gw._tcp.\(domainTrimmed)"
