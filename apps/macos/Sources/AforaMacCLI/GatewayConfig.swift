...
/// An explicit config path wins; otherwise the selected state directory owns afora.json.
func resolveAforaConfigURL() -> URL {
    if let configPath = aforaEnvironmentPath("AFORA_CONFIG_PATH") {
        return URL(fileURLWithPath: NSString(string: configPath).expandingTildeInPath)
    }
    let stateDir = aforaEnvironmentPath("AFORA_STATE_DIR").map {
        URL(fileURLWithPath: NSString(string: $0).expandingTildeInPath, isDirectory: true)
    } ?? FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".afora", isDirectory: true)
    return stateDir.appendingPathComponent("afora.json")
}

private func aforaEnvironmentPath(_ key: String) -> String? {
...
func loadGatewayConfig() -> GatewayConfig {
    guard let data = try? Data(contentsOf: resolveAforaConfigURL()) else { return GatewayConfig() }
