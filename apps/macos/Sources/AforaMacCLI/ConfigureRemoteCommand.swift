...
private let appDefaultsSuites = ["ai.afora.mac", "ai.afora.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              afora-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
              ...
              afora-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote Afora checkout for CLI commands.
              --cli-path <path>   Remote afora executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
    ...
    defaultsSuites: [String]) throws -> ConfigureRemoteOutput
{
    ...
    let configURL = resolveAforaConfigURL()
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String, suites: [String]) {
    for suite in suites {
        ...
        defaults.set("remote", forKey: "afora.connectionMode")
        setDefaultString(defaults, key: "afora.remoteTarget", value: target)
        defaults.set(true, forKey: "afora.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "afora.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "afora.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "afora.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "afora.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("Afora macOS Remote Config")
