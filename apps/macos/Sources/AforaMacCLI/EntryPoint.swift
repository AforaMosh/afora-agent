...
struct AforaMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("afora-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      afora-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      afora-mac discover [--timeout <ms>] [--json] [--include-local]
      afora-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      afora-mac connect
      afora-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      afora-mac connect --url ws://127.0.0.1:18789 --json
      afora-mac discover --timeout 3000 --json
      afora-mac wizard --mode local
    """)
}
