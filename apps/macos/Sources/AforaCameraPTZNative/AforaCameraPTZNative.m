...
enum {
    AforaUVCVideoClass = 0x0e,
    AforaUVCVideoControlSubclass = 0x01,
    AforaUVCClassInterfaceDescriptor = 0x24,
    AforaUVCVideoControlHeader = 0x01,
    AforaUVCInputTerminal = 0x02,
    AforaUVCInputTerminalCamera = 0x0201,
    AforaUVCSetCurrent = 0x01,
};

struct AforaUVCController {
...
static void AforaUVCSetError(char **error_out, const char *format, ...) {
    if (error_out == NULL) {
        return;
    }
    va_list args;
...
static uint32_t AforaUVCNumberProperty(io_service_t service, CFStringRef key) {
...
static IOUSBDeviceInterface **AforaUVCCreateDeviceInterface(
    ...
    for (size_t class_index = 0; class_index < sizeof(class_names) / sizeof(class_names[0]); class_index++) {
        ...
        while ((service = IOIteratorNext(iterator)) != IO_OBJECT_NULL) {
            uint32_t found_vendor = AforaUVCNumberProperty(service, CFSTR(kUSBVendorID));
            uint32_t found_product = AforaUVCNumberProperty(service, CFSTR(kUSBProductID));
            uint32_t found_location = AforaUVCNumberProperty(service, CFSTR(kUSBDevicePropertyLocationID));
...
static int AforaUVCParseCameraTerminalDescriptor(
    ...
    if (descriptor == NULL || terminal_id_out == NULL || controls_out == NULL || descriptor_length < 17 ||
        ...
        descriptor[1] != AforaUVCClassInterfaceDescriptor || descriptor[2] != AforaUVCInputTerminal) {
        return 0;
    }

    uint16_t terminal_type = (uint16_t)descriptor[4] | ((uint16_t)descriptor[5] << 8);
    size_t control_size = descriptor[14];
    if (terminal_type != AforaUVCInputTerminalCamera || control_size < 2 ||
...
int afora_uvc_parse_camera_terminal(
    ...
    if (descriptors == NULL || terminal_id_out == NULL || controls_out == NULL || descriptors_length < 7 ||
        descriptors[0] < 7 || descriptors[1] != AforaUVCClassInterfaceDescriptor ||
        descriptors[2] != AforaUVCVideoControlHeader) {
        return 0;
    }
    ...
    for (size_t offset = 0; offset < scan_length;) {
        ...
        if (AforaUVCParseCameraTerminalDescriptor(
                descriptor,
                length,
                terminal_id_out,
                controls_out
...
static void AforaUVCReadTerminal(
    IOUSBInterfaceInterface220 **interface,
    uint8_t *terminal_id_out,
    uint32_t *controls_out
) {
    ...
    *controls_out = 0;
    ...
    while ((current = (*interface)->FindNextAssociatedDescriptor(
                interface,
                current,
                AforaUVCClassInterfaceDescriptor
            )) != NULL) {
        if (AforaUVCParseCameraTerminalDescriptor(
...
int afora_uvc_open(
    ...
    if (controller_out == NULL || controls_out == NULL) {
        AforaUVCSetError(error_out, "invalid controller output pointers");
        return 0;
    }
    *controller_out = NULL;
    *controls_out = 0;
    ...
    IOUSBDeviceInterface **device = AforaUVCCreateDeviceInterface(
        location_id,
        vendor_id,
        product_id,
        &result
    ...
    if (result != kIOReturnSuccess) {
        (*device)->Release(device);
        AforaUVCSetError(error_out, "find VideoControl interface: %s (0x%08x)", mach_error_string(result), result);
        return 0;
    }
    ...
    if (service == IO_OBJECT_NULL) {
        (*device)->Release(device);
        AforaUVCSetError(error_out, "USB device has no UVC VideoControl interface");
        return 0;
    }
    ...
    if (result != kIOReturnSuccess || plugin == NULL) {
        ...
        (*device)->Release(device);
        AforaUVCSetError(error_out, "create VideoControl interface: %s (0x%08x)", mach_error_string(result), result);
        return 0;
    }
    ...
    if (query_result != S_OK || interface == NULL) {
        ...
        (*device)->Release(device);
        AforaUVCSetError(error_out, "query VideoControl interface: 0x%08x", (unsigned int)query_result);
        return 0;
    }
    ...
    if (result != kIOReturnSuccess) {
        (*interface)->Release(interface);
        (*device)->Release(device);
        AforaUVCSetError(error_out, "read VideoControl interface number: %s (0x%08x)", mach_error_string(result), result);
        return 0;
    }
    ...
    if (result == kIOReturnSuccess) {
        ...
        (*interface)->Release(interface);
        (*device)->Release(device);
        AforaUVCSetError(error_out, "open VideoControl interface: %s (0x%08x)", mach_error_string(result), result);
        return 0;
    }
    ...
    AforaUVCController *controller = calloc(1, sizeof(AforaUVCController));
    if (controller == NULL) {
        if (interface_open) {
        ...
        (*interface)->Release(interface);
        (*device)->Release(device);
        AforaUVCSetError(error_out, "allocate UVC controller");
        return 0;
    }
    ...
    AforaUVCReadTerminal(interface, &controller->terminal_id, controls_out);
...
int afora_uvc_control(
    ...
    if (controller == NULL || controller->interface == NULL || data == NULL || length == 0) {
        AforaUVCSetError(error_out, "invalid control request");
        return 0;
    }

    IOUSBDevRequest control_request = {
        .bmRequestType = request == AforaUVCSetCurrent ? 0x21 : 0xa1,
        .bRequest = request,
