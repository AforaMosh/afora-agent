...
final class QuickChatWindowPicker {
    ...
    {
        let logging = LoggingService(subsystem: "ai.afora.quickchat-picker")
    ...
    private func requestScreenRecordingPermission(mode: QuickChatCapturePickerMode) async {
        ...
        alert.messageText = mode == .window
            ? String(localized: "Allow Afora to capture windows")
            : String(localized: "Allow Afora to capture a screen area")
