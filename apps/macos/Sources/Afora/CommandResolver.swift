...
enum CommandResolver {
    private static let projectRootDefaultsKey = "afora.gatewayProjectRootPath"
    private static let helperName = "afora"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let aforaEntry = root.appendingPathComponent("afora.mjs").path
        if FileManager().isReadableFile(atPath: aforaEntry) { return aforaEntry }
        let binEntry = root.appendingPathComponent("bin/afora.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/afora")
    ...
    static func preferredPaths() -> [String] {
        ...
        let validatedExecutable = self.validatedAforaExecutable(
        ...
        profile: AppProfile = .current) -> [String]
    {
        ...
        let managedPaths = self.aforaManagedPaths(home: home, profile: profile)
        // Other profiles' managed trees must not leak in via stale validation or the
        // inherited shell PATH (the CLI installer adds ~/.afora/bin to shell profiles).
    ...
    static func validatedAforaExecutable(
    ...
    /// Exactly the AppProfile.stateDirectoryURL namespace; ~/.afora2 is not managed.
    private static func isManagedDirectoryName(_ name: String) -> Bool {
        name == ".afora" || name.hasPrefix(".afora-")
    }

    private static func aforaManagedPaths(home: URL, profile: AppProfile) -> [String] {
    ...
    static func aforaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAforaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
        ...
        let sourceEntrypoint = root.appendingPathComponent("afora.mjs").path
        ...
        profile: AppProfile = .current) async -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let aforaPath = projectAforaExecutable(projectRoot: root) {
            return profile.localCLICommand(prefix: [aforaPath], arguments: [subcommand] + extraArgs)
        }
        if let aforaPath = aforaExecutable(searchPaths: searchPaths) {
            return profile.localCLICommand(prefix: [aforaPath], arguments: [subcommand] + extraArgs)
        }
        ...
        case .success:
            let missingEntry = """
            afora CLI not found. Install the CLI, or run pnpm build in an Afora source checkout.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real afora CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/afora"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/afora.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/afora.mjs"
            node "$PRJ/afora.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/afora.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/afora.js"
            node "$PRJ/bin/afora.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent afora"
          pnpm --silent afora \(quotedArgs);
        else
          echo "afora CLI missing on remote host"; exit 127;
        fi
        """
    ...
    {
        let root = configRoot ?? AforaConfigFile.loadDict()
