...
enum AforaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = AforaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AforaFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ...
        let label = AforaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AforaOSLogHandler: AppLogLevelBackedHandler {
...
struct AforaFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]

    func log(event: LogEvent) {
        guard AppLogSettings.fileLoggingEnabled() else { return }
        let (subsystem, category) = AforaLogging.parseLabel(self.label)
