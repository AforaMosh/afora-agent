...
enum DashboardGatewayMenuModel {
        ...
        entries: [DashboardGatewayEntry]) -> String
    {
        guard entries.count >= 2,
            ...
            return switch mode {
            case .unconfigured:
                String(localized: "Afora Not Configured")
            case .remote:
                self.remoteConnectionLabel(state: controlState)
            case .local:
                String(localized: "Afora Active")
            }
        }
        return switch mode {
        case .unconfigured:
            String(localized: "Afora Not Configured — \(primaryName)")
        case .remote:
            self.remoteConnectionLabel(state: controlState, primaryName: primaryName)
        case .local:
            String(localized: "Afora Active — \(primaryName)")
        }
    }
        ...
        primaryName: String? = nil) -> String
    {
        ...
        case (.connected, nil):
            String(localized: "Remote Afora Active")
        case let (.connected, .some(name)):
            String(localized: "Remote Afora Active — \(name)")
        case (.connecting, nil):
            String(localized: "Remote Afora Connecting")
        case let (.connecting, .some(name)):
            String(localized: "Remote Afora Connecting — \(name)")
        case (.disconnected, nil), (.degraded, nil):
            String(localized: "Remote Afora Needs Attention")
        case let (.disconnected, .some(name)), let (.degraded, .some(name)):
            String(localized: "Remote Afora Needs Attention — \(name)")
        }
    }
}
