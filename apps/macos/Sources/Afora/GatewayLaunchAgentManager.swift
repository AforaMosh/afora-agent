...
enum GatewayLaunchAgentManager {
    ...
    private static let logger = Logger(subsystem: "ai.afora", category: "gateway.launchd")
    ...
    private static var disableLaunchAgentMarkerURL: URL {
        ...
        #endif
        return self.disableLaunchAgentMarkerURL(in: AforaPaths.stateDirURL)
    }
    ...
    private static func profile(forLaunchAgentLabel label: String) -> AppProfile? {
        ...
        let profile = AppProfile(environment: ["AFORA_PROFILE": name])
    ...
    private static func isCanonicalGatewayClaim(_ snapshot: LaunchAgentPlistSnapshot) -> Bool {
        snapshot.environment["AFORA_SERVICE_MARKER"] == "afora" &&
            snapshot.environment["AFORA_SERVICE_KIND"] == "gateway" &&
            snapshot.programArguments.contains("gateway")
    }

    private static var generatedEnvironmentDirectoryURL: URL {
        AforaPaths.stateDirURL.appendingPathComponent("service-env", isDirectory: true)
    }
...
extension GatewayLaunchAgentManager {
    ...
    private static func gatewayPort(from service: [String: Any]) -> Int? {
        ...
        return self.validGatewayPort(environment?["AFORA_GATEWAY_PORT"] as? String)
    }
