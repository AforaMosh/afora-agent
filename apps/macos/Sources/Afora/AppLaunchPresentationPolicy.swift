...
enum ElevationExclusiveRename {
    ...
    static func runIfRequested(arguments: [String] = CommandLine.arguments) -> Int32? {
        ...
        guard arguments.count == 4 else {
            fputs("Afora elevation rename requires absolute source and destination paths\n", stderr)
            return 2
        }
        let source = arguments[2]
        let destination = arguments[3]
        guard source.hasPrefix("/"), destination.hasPrefix("/") else {
            fputs("Afora elevation rename paths must be absolute\n", stderr)
            return 2
        }
        ...
        guard result == 0 else {
            let code = errno
            fputs("Afora elevation rename failed: \(String(cString: strerror(code)))\n", stderr)
            return 1
        }
        return 0
    }
...
enum ElevationFilesystemSync {
    ...
    static func runIfRequested(arguments: [String] = CommandLine.arguments) -> Int32? {
        ...
        guard arguments.count == 3 else {
            fputs("Afora elevation sync requires one absolute path\n", stderr)
            return 2
        }
        let path = arguments[2]
        guard path.hasPrefix("/") else {
            fputs("Afora elevation sync path must be absolute\n", stderr)
            return 2
        }
    ...
    private static func syncPath(_ path: String, expectsDirectory: Bool) -> Bool {
        ...
        guard descriptor >= 0 else {
            fputs("Afora elevation sync could not open path\n", stderr)
            return false
        }
        ...
        guard fstat(descriptor, &metadata) == 0,
            ...
            fputs("Afora elevation sync path has an unsupported type\n", stderr)
            return false
        }
        if expectsDirectory {
            guard fsync(descriptor) == 0 else {
                fputs("Afora elevation directory sync failed\n", stderr)
                return false
            }
            return true
        }
        guard fcntl(descriptor, F_FULLFSYNC) == 0 || fsync(descriptor) == 0 else {
            fputs("Afora elevation file sync failed\n", stderr)
            return false
        }
        let parent = URL(fileURLWithPath: path).deletingLastPathComponent().path
        let parentDescriptor = open(parent, O_RDONLY | O_NOFOLLOW | O_DIRECTORY)
        guard parentDescriptor >= 0 else {
            fputs("Afora elevation sync could not open parent directory\n", stderr)
            return false
        }
        defer { close(parentDescriptor) }
        guard fsync(parentDescriptor) == 0 else {
            fputs("Afora elevation parent directory sync failed\n", stderr)
            return false
        }
        return true
    }
    ...
    private static func syncTree(_ path: String) -> Bool {
        ...
        guard let enumerator = FileManager.default.enumerator(
            ...
            fputs("Afora elevation sync could not enumerate tree\n", stderr)
            return false
        }
        var directories = [root]
        for case let url as URL in enumerator {
            guard let values = try? url.resourceValues(forKeys: Set(keys)) else {
                fputs("Afora elevation sync could not inspect tree entry\n", stderr)
                return false
            }
            ...
            if values.isDirectory == true {
                ...
                fputs("Afora elevation sync tree contains an unsupported entry\n", stderr)
                return false
            }
        }
        guard !enumerationFailed else {
            fputs("Afora elevation sync could not enumerate complete tree\n", stderr)
            return false
        }
