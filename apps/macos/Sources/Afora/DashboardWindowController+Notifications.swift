...
extension DashboardWindowController {
    static let notificationsMessageHandlerName = "aforaNotifications"
    ...
    private func publishNotificationsStatus() async {
        ...
        _ = try? await self.webView.evaluateJavaScript(
            """
            window.__AFORA_NATIVE_NOTIFICATIONS__ = {permission:"\(permission)"};
            window.dispatchEvent(new CustomEvent('afora:native-notifications-status', \
            {detail:window.__AFORA_NATIVE_NOTIFICATIONS__}));
            """)
    }
}
