...
extension OnboardingAISetupModel {
    ...
    enum ProviderWizardKind: Equatable {
        ...
        var startMethod: String {
            switch self {
            case .auth: "afora.setup.auth.start"
            case .prepare: "afora.setup.prepare.start"
            }
        }
    }
    ...
    /// Afora rather than the existing-Gateway onboarding bypass.
    ...
    static func friendlyTransportError(_ raw: String) -> String {
        if raw.localizedCaseInsensitiveContains("unknown method") {
            return "The Gateway is running an older Afora version that doesn’t support " +
                "app-guided setup. Update Afora on the gateway, then try again."
        }
        return raw.isEmpty
            ...
            return code == "UNKNOWN_METHOD" ||
                (code == "INVALID_REQUEST" &&
                    (message.contains("unknown method") ||
                        message.contains("invalid afora.setup.activate params")))
        }
        return error is GatewayConnectAuthError ||
            error is GatewayTLSValidationError ||
            error is AforaChatTransportSendError
    }
    ...
    static func activationAdmissionIsBusy(_ error: Error) -> Bool {
        ...
        return response.method == "afora.setup.activate" &&
...
enum OnboardingAISetupError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .providerCatalogUnavailable:
            "The Gateway is running an older Afora version that doesn’t provide the " +
                "supported provider list. Update Afora on the gateway, then try again."
        }
    }
}
