...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AforaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AforaKit.AnyCodable]()) {
                $0[$1.key] = AforaKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking Afora Local Network access.
                    Allow Afora in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AforaProtocol.AnyCodable?) -> [String: AforaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AforaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AforaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AforaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlChannelStateDidChange = Notification.Name("afora.control-channel.state-did-change")
    static let controlHeartbeat = Notification.Name("afora.control.heartbeat")
}
