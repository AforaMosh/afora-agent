...
enum RemoteGatewayAuthIssue: Equatable {
    ...
    var body: String {
        ...
        case .tokenRequired:
            "Paste the token configured on the gateway host. "
                + "On the gateway host, run `afora gateway auth-token --show` "
        ...
        case .gatewayTokenNotConfigured:
            "This gateway is set to token auth, but no `gateway.auth.token` is configured on the gateway host. "
                ...
                + "set `AFORA_GATEWAY_TOKEN` before starting the gateway."
        case .setupCodeExpired:
            "Scan or paste a fresh setup code from an already-paired Afora client, then try again."
        case .passwordRequired:
            "This onboarding flow does not support password auth yet. "
                + "Reconfigure the gateway to use token auth, then retry."
        case .pairingRequired:
            "Approve this device from an already-paired Afora client. "
                + "In your Afora chat, run `/pair approve`, then click **Check connection** again."
        }
    }
    ...
    var footnote: String? {
        ...
        case .tokenRequired, .gatewayTokenNotConfigured:
            "No token yet? Generate one on the gateway host with "
                + "`afora doctor --generate-gateway-token`, then set it as `gateway.auth.token`."
        case .setupCodeExpired:
            nil
        case .pairingRequired:
            "If you do not have another paired Afora client yet, "
                + "approve the pending request on the gateway host with `afora devices approve`."
    ...
    var statusMessage: String {
        switch self {
        case .tokenRequired:
            "This gateway requires an auth token. Run afora gateway auth-token --show on the gateway host."
        case .tokenMismatch:
            "Gateway token mismatch. Run afora gateway auth-token --show on the gateway host."
        case .gatewayTokenNotConfigured:
        ...
        case .pairingRequired:
            "Pairing required. In an already-paired Afora client, "
