        ...
        cuaDaemonReady: Bool?) -> [ComputerControlReadinessRow]
    {
        ...
        var rows = [
            ComputerControlReadinessRow(
                ...
                nextStep: providerAvailable
                    ? nil
                    : String(localized: "Use a packaged Afora build that includes the CUA driver.")),
            self.permissionRow(
                id: .accessibility,
                ...
                nextStep: String(
                    localized: """
                    Grant Afora in System Settings → Privacy & Security → Accessibility, then reopen it.
                    """)),
            self.permissionRow(
                id: .screenRecording,
                ...
                nextStep: String(
                    localized: """
                    Grant Afora in System Settings → Privacy & Security → Screen Recording, then reopen it.
                    """)),
        ]
        ...
        if provider == .cua {
            ...
            case .unavailable:
                String(localized: "Use a packaged Afora build that includes the CUA driver.")
            case .notReady:
                String(localized: "Reopen Afora to retry the embedded CUA daemon.")
            case .unknown:
                String(localized: "Keep Afora open while it checks the embedded CUA daemon.")
...
struct ComputerControlReadinessView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .aforaPermissionsChanged)) { _ in
            Task { await self.refreshPermissions() }
        }
        .onReceive(NotificationCenter.default.publisher(for: .aforaCuaDriverAvailabilityChanged)) { _ in
            self.refreshCuaDaemonReadiness()
        }
    }
