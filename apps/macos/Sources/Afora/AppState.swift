...
final class AppState {
    private static let logger = Logger(subsystem: "ai.afora", category: "app-state")
    ...
    init(
        ...
        gatewayConfigSaver: @escaping ([String: Any]) -> Bool = { AforaConfigFile.saveDict($0) })
    {
        ...
        let configRoot = AforaConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            AforaConfigFile.canonicalHostForComparison(host) ==
                AforaConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
...
extension AppState {
    ...
    private func startConfigWatcher() {
        let configUrl = AforaConfigFile.url()
    ...
    private func applyConfigFromDisk() {
        ...
        NotificationCenter.default.post(name: .aforaConfigDidChange, object: nil)
    }
...
extension AppState {
    ...
    private func syncGatewayConfigNow(remoteTLSFingerprintUpdate: RemoteTLSFingerprintUpdate) -> Bool {
        ...
        let currentRoot = AforaConfigFile.loadDict()
