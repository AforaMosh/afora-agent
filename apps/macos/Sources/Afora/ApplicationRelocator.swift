...
enum ApplicationRelocator {
    ...
    private static let logger = Logger(subsystem: "ai.afora", category: "app-relocation")
    ...
    private nonisolated static let replacementSourceBundleEnvironmentKey = "AFORA_REPLACEMENT_SOURCE_BUNDLE"
    private nonisolated static let replacementParentPIDEnvironmentKey = "AFORA_REPLACEMENT_PARENT_PID"
    private nonisolated static let replacementCodeHashEnvironmentKey = "AFORA_REPLACEMENT_CODE_HASH"
    private nonisolated static let replacementReadyFDEnvironmentKey = "AFORA_REPLACEMENT_READY_FD"
    private nonisolated static let replacementBootoutTargetEnvironmentKey = "AFORA_REPLACEMENT_BOOTOUT_TARGET"
    private nonisolated static let replacementSupervisorLabelEnvironmentKey =
        "AFORA_REPLACEMENT_SUPERVISOR_LABEL"
    private nonisolated static let replacementSupervisorPlistEnvironmentKey =
        "AFORA_REPLACEMENT_SUPERVISOR_PLIST"
        ...
        processInfo: ProcessInfo = .processInfo) -> LaunchDisposition
    {
        ...
        case .continueLaunch:
            #if DEBUG
            let monitorDebugReplacement = processInfo.environment["AFORA_MONITOR_APP_REPLACEMENT"] == "1"
            #else
        ...
        case let .offerInstall(destination, replacing):
            ...
            do {
                ...
                showFailure(
                    "Afora couldn’t be installed in Applications. Move it there manually, then open that copy.")
                return .continueLaunch(startUpdater: false)
            }
        case .cannotInstall:
            let message =
                "Afora is running from a temporary location. " +
...
extension ApplicationRelocator {
    ...
    private static func confirmInstall(replacing: Bool) -> Bool {
        ...
        alert.messageText = replacing
            ? "Replace the older Afora in Applications?"
            : "Install Afora in Applications?"
        alert.informativeText = replacing
            ? "This copy is newer than the installed app. Afora will replace it and reopen from Applications."
            : "Afora will copy itself to Applications and reopen there so updates and launch at login stay reliable."
    ...
    private static func relaunchAndTerminate(at destination: URL) -> LaunchDisposition {
        ...
        do {
            ...
            self.showFailure(
                "Afora is installed in Applications, but couldn’t reopen automatically. Open it there manually.")
            return .continueLaunch(startUpdater: false)
        }
    }
        ...
        reason: String) async
    {
        ...
        case .stopMonitoring:
            ...
            self.logger.critical(
                ...
                Quit and reopen Afora to load the installed replacement.
                """)
        case .terminateForSupervisor:
    ...
    private static func showFailure(_ message: String) {
        let alert = NSAlert()
        alert.messageText = "Move Afora to Applications"
