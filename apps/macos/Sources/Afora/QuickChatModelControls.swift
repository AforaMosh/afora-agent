...
struct QuickChatModelControlSnapshot: Sendable {
    let models: [AforaChatModelChoice]
    let currentModelSelectionID: String?
    let currentThinkingLevel: String?
    let thinkingOptions: [AforaChatThinkingLevelOption]
...
enum QuickChatModelControlLogic {
    static let baseThinkingOptions = ["off", "minimal", "low", "medium", "high"].map {
        AforaChatThinkingLevelOption(id: $0, label: $0)
    }

    static func snapshot(
        target: QuickChatRoutingTarget,
        models: [AforaChatModelChoice],
        sessions: AforaChatSessionsListResponse,
        ...
        currentSessionSelectionID: String? = nil) -> QuickChatModelPatchDecision
    {
        ...
        if selectionID == AforaChatViewModel.defaultModelSelectionID {
        ...
        automaticLabel: String) -> String
    {
        guard let selectionID,
              selectionID != AforaChatViewModel.defaultModelSelectionID
    ...
    static func validatedThinkingSelection(
        _ selection: String?,
        options: [AforaChatThinkingLevelOption]) -> String?
    {
    ...
    private static func sessionEntry(
        target: QuickChatRoutingTarget,
        sessions: [AforaChatSessionEntry]) -> AforaChatSessionEntry?
    {
    ...
    private static func agent(target: QuickChatRoutingTarget, agents: AgentsListResult?) -> AgentSummary? {
        ...
        let targetAgentID = self.normalized(target.agentID) ??
            AforaChatSessionKey.agentID(from: target.sessionKey) ??
    ...
    private static func thinkingOptions(
        entry: AforaChatSessionEntry?,
        agent: AgentSummary?,
        defaults: AforaChatSessionsDefaults?) -> [AforaChatThinkingLevelOption]
    {
        let agentOptions = agent?.thinkinglevels?.compactMap { option -> AforaChatThinkingLevelOption? in
            ...
            return AforaChatThinkingLevelOption(id: id, label: label)
        }
        let options = entry?.thinkingLevels ??
            entry?.thinkingOptions?.map { AforaChatThinkingLevelOption(id: $0, label: $0) } ??
            agentOptions ??
            defaults?.thinkingLevels ??
            defaults?.thinkingOptions?.map { AforaChatThinkingLevelOption(id: $0, label: $0) } ??
