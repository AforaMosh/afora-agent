...
private let gatewayConnectionLogger = Logger(subsystem: "ai.afora", category: "gateway.connection")
...
actor GatewayConnection {
    ...
    nonisolated static let operatorClientCaps =
        [AforaGatewayClientCapability.agentKind, AforaGatewayClientCapability.inlineWidgets]
...
extension GatewayConnection {
    ...
    func sessionRoutingIdentity(
        ifCurrentRoute route: Route) async throws -> AforaChatSessionRoutingIdentity
    {
        let data = try await request(
            AforaChatGatewayRequests.agentsList(),
            ifCurrentRoute: route)
        return try AforaChatGatewayPayloadCodec.decodeSessionRoutingIdentity(data)
    }
...
extension GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: AforaProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(AforaGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> AforaSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return AforaSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        ...
        maxChars: Int? = nil,
        timeoutMs: Int? = nil,
        ifCurrentRoute route: Route? = nil) async throws -> AforaChatHistoryPayload
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        ...
        if let route {
            let data = try await self.request(
                request,
                ifCurrentRoute: route)
            return try self.decoder.decode(AforaChatHistoryPayload.self, from: data)
        }
        ...
        distinguishPreDispatchRouteChange: Bool = false) async throws -> AforaChatSendResponse
    {
        ...
        if let route {
            let data = try await self.request(
                ...
                ifCurrentRoute: route,
                distinguishPreDispatchRouteChange: distinguishPreDispatchRouteChange)
            return try self.decoder.decode(AforaChatSendResponse.self, from: data)
        }
        let data = try await self.request(request)
        return try self.decoder.decode(AforaChatSendResponse.self, from: data)
    }
