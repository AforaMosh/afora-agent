...
final class CookieSyncManager: NSObject {
    ...
    @ObservationIgnored private let logger = Logger(subsystem: "ai.afora", category: "cookie-sync")
    @ObservationIgnored private let queue = DispatchQueue(label: "ai.afora.cookie-sync")
    ...
    private func reconcile(generation: UInt64) async {
        ...
        guard let executable = await self.resolveLocalAforaExecutable(),
              !Task.isCancelled,
              generation == self.reconcileGeneration
        else {
            if !Task.isCancelled, generation == self.reconcileGeneration {
                self.state = .error("Afora CLI not found locally; cookie sync needs the CLI on this Mac")
            }
            return
        }
    ...
    private func resolveLocalAforaExecutable() async -> String? {
        #if DEBUG
        if let executable = CommandResolver.projectAforaExecutable() {
            return executable
        }
        #endif
    ...
    private func launch(executable: String, intent: SyncIntent) {
        ...
        environment["AFORA_GATEWAY_URL"] = intent.endpoint.url.absoluteString
        environment.removeValue(forKey: "AFORA_GATEWAY_TOKEN")
        environment.removeValue(forKey: "AFORA_GATEWAY_PASSWORD")
        if let token = intent.endpoint.token {
            environment["AFORA_GATEWAY_TOKEN"] = token
        } else if let password = intent.endpoint.password {
            environment["AFORA_GATEWAY_PASSWORD"] = password
        }
