...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "portguard")
        ...
        currentAppPID: Int32? = nil) -> TunnelRecordAction
    {
        ...
        // Any other live parent may be a concurrent Afora instance (prod + dev).
        return .keep
    }
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "afora", "tsx", "pnpm", "bun"]
        ...
        managedGatewayPID: Int32? = nil) -> Bool
    {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("afora-gateway")
                || cmd.contains("afora-gateway")
            {
                return true
            }
            if self.isNodeAforaGatewayCommand(full) { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("afora"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
    ...
    private static func isNodeAforaGatewayCommand(_ fullCommand: String) -> Bool {
        ...
        return self.isAforaDistEntrypointToken(tokens[1])
            && tokens[2].lowercased() == "gateway"
    }

    private static func isAforaDistEntrypointToken(_ token: String) -> Bool {
    ...
    private nonisolated static func openRecordStore() throws -> PortGuardianRecordStore {
        guard !self.hasLegacyAforaAppProcess() else {
            throw PortGuardianStoreError(
                "Quit older Afora app copies before opening the SQLite PortGuardian ledger")
        }
    ...
    private nonisolated static func requirePostSpawnCompatibility() throws {
        guard !self.hasLegacyAforaAppProcess(),
              !FileManager.default.fileExists(atPath: PortGuardianRecordStore.liveLegacyRecordURL.path)
        else {
            throw PortGuardianStoreError(
                "Older Afora storage appeared after tunnel preflight; SSH launch cancelled")
        }
    }
    ...
    private nonisolated static func hasLegacyAforaAppProcess() -> Bool {
    ...
    private nonisolated static func runningPortGuardianStorageVersion(pid: pid_t) -> Int? {
        ...
        guard SecCodeCopyStaticCode(code, SecCSFlags(), &staticCode) == errSecSuccess,
              ...
              let version = securedInfo["AforaPortGuardianStorageVersion"] as? NSNumber
        else { return nil }
        ...
        storageVersion: Int?) -> Bool
    {
        guard let bundleIdentifier,
              bundleIdentifier == "ai.afora.mac" || bundleIdentifier.hasPrefix("ai.afora.mac.")
        else { return false }
