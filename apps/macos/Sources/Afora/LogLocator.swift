...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["AFORA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/afora")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("afora-stdout.log")
    }

    private static var gatewayLog: URL {
        let suffix = AppProfile.current.name.map { "-\($0)" } ?? ""
        return logDir.appendingPathComponent("afora-gateway\(suffix).log")
    }
    ...
    /// Returns the newest log file under /tmp/afora/ (rolling or stdout), or nil if none exist.
