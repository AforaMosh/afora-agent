...
actor TalkMLXSpeechSynthesizer {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "talk.mlx")
    ...
    fileprivate static func helperInvocation() -> HelperInvocation {
        ...
        if let override = ProcessInfo.processInfo.environment["AFORA_MLX_TTS_BIN"], !override.isEmpty {
        ...
        if let executableDir = Bundle.main.executableURL?.deletingLastPathComponent() {
            let bundled = executableDir.appendingPathComponent("afora-mlx-tts")
        ...
        return HelperInvocation(
            ...
            argumentPrefix: ["afora-mlx-tts"],
            displayName: "afora-mlx-tts")
    }
