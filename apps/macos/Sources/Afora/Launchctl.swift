...
enum LaunchAgentPlist {
        ...
        generatedEnvironmentWrapperURL: URL? = nil) -> LaunchAgentPlistSnapshot?
    {
        ...
        let token = env["AFORA_GATEWAY_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
        let password = env["AFORA_GATEWAY_PASSWORD"]?.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
        ...
        wrapperURL: URL?) -> [String: String]
    {
        ...
        // Read only the canonical file when the LaunchAgent uses Afora's generated wrapper.
