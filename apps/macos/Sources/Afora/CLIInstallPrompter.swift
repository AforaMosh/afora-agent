...
final class CLIInstallPrompter {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "cli.prompt")
    ...
    func installTargetForCurrentBuild(confirmStable: Bool = false) -> CLIInstaller.InstallTarget? {
            ...
            isDebug: CLIInstallBuild.isDebug)
        {
            ...
            let alert = NSAlert()
            alert.messageText = "Install Afora CLI?"
    ...
    private func chooseChannel(suggested: CLIInstaller.Channel) -> CLIInstaller.Channel? {
        ...
        let alert = NSAlert()
        alert.messageText = "Choose Afora CLI channel"
        alert.informativeText =
            "This is an unreleased Afora build. " +
        ...
        restartManagedGateway: Bool = false) async -> Bool
    {
        ...
        if installed {
            ...
            if usesLocalGateway {
                await status.set("Starting Afora Gateway…")
                if !showCompletionAlert {
                    self.logger.info("managed CLI repair: Starting Afora Gateway…")
                }
            ...
            case .ready:
                "Afora Gateway is ready."
            case .deferred:
                "Afora is installed. The Gateway will start when This Mac is active and resumed."
            case .failed:
                "Afora was installed, but the Gateway did not start. Open Settings to retry."
            case nil:
                "Afora CLI is ready for the Mac node."
            }
            await status.set(message)
            if !showCompletionAlert {
                self.logger.info("managed CLI repair: \(message, privacy: .public)")
    ...
    private func openSettings(tab: SettingsTab) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .aforaSelectSettingsTab, object: tab)
        }
    }
