...
enum ExecApprovalsStore {
    ...
    private static let logger = Logger(subsystem: "ai.afora", category: "exec-approvals")
    ...
    static func socketPath(stateDirectoryURL: URL, profileActive: Bool) -> String {
        ...
        let digest = SHA256.hash(data: Data(canonical.utf8))
            ...
            .joined()
        return "/tmp/afora-\(geteuid())/exec-approvals-\(digest).sock"
    }
    ...
    private static func homeURL() -> URL {
        guard let configured = AforaEnv.path("AFORA_HOME") else {
    ...
    private static func stateDirURL() -> URL {
        ...
        guard let configured = AforaEnv.path("AFORA_STATE_DIR") else {
...
extension ExecApprovalsStore {
    ...
    static func expandPath(_ raw: String) -> String {
        ...
        let configuredHome = AforaEnv.path("AFORA_HOME")
