...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.afora", category: "gateway.env")
    ...
    private static func selectedGatewayPort() -> Int {
        self.resolvedGatewayPort(
            ...
            configPort: AforaConfigFile.gatewayPort(),
    ...
    {
        if let raw = environment["AFORA_GATEWAY_PORT"] {
    ...
    private static func resolveEnvironment(searchPaths: [String]) async -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.aforaExecutable(searchPaths: searchPaths)
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "afora CLI not found in PATH; install the CLI.")
            }
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("afora ") {
            normalized = String(normalized.dropFirst("afora ".count))
        }
