...
/// pinned at `--afora-native-titlebar-height`, resurrecting the traffic-light
...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    private static let linkMessageHandlerName = "aforaLink"
    private static let windowDragMessageHandlerName = "aforaWindowDrag"
    private static let updateMessageHandlerName = "aforaUpdate"
    ...
    func setUpdateBridgeEnabled(_ enabled: Bool) {
        ...
        self.webView.evaluateJavaScript(
            "window.dispatchEvent(new CustomEvent('afora:native-update-availability-changed'))")
    }
    ...
    fileprivate func receiveUpdateMessage(_ message: WKScriptMessage) {
        ...
        guard DashboardManager.updateBridgeEnabled(mode: AppStateStore.shared.connectionMode) else {
            ...
            self.webView.evaluateJavaScript(
                "window.dispatchEvent(new CustomEvent('afora:native-update-declined'))")
            return
        }
    ...
    private func publishNativeHistoryState() {
        ...
        self.webView.evaluateJavaScript(
            """
            window.__AFORA_NATIVE_HISTORY__ = {canGoBack:\(canGoBack),canGoForward:\(canGoForward)};
            window.dispatchEvent(new CustomEvent('afora:native-history-state', \
            {detail:window.__AFORA_NATIVE_HISTORY__}));
            """)
    }
        ...
        reusing existingWindow: NSWindow?) -> NSWindow
    {
        ...
        // (--afora-native-titlebar-height); without it they hug the top edge.
    ...
    static func installNativeChromeScript(into userContentController: WKUserContentController) {
        ...
        let capabilityScript = "window.__AFORA_NATIVE_WEB_CHROME__ = true;"
        ...
        // `html.afora-native-macos` styles fold the titlebar clearance into
        ...
        @media (min-width: 700px) {
          ...
          html.afora-native-macos .sidebar-shell,
          html.afora-native-macos .settings-sidebar__header {
            padding-top: max(14px, var(--afora-native-titlebar-height)) !important;
          }
        }
        """
        let script = """
        (() => {
          try {
            if (document.getElementById("afora-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "afora-native-macos-chrome";
        ...
        auth: DashboardWindowAuth)
    {
        ...
        (() => {
          try {
            ...
            Object.defineProperty(window, "__AFORA_NATIVE_CONTROL_AUTH__", {
              value: \(json),
              configurable: true,
            });
          } catch {}
...
extension DashboardWindowController {
    ...
    private static func makeJavaScriptConfirmAlert(message: String, host: String?) -> NSAlert {
        let alert = NSAlert()
        alert.messageText = "Afora Dashboard"
    ...
    private func evaluateNativeNavigation(_ navigation: DashboardNativeNavigation) {
        ...
        let script =
            """
            (() => !window.dispatchEvent(new CustomEvent('afora:native-navigate', {
              cancelable: true,
