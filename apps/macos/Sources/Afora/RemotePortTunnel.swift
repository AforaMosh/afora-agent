...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.afora", category: "remote.tunnel")
        ...
        root: [String: Any]) -> Int?
    {
        ...
        guard let sshKey = AforaConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = AforaConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.afora.remote.tunnel.port", qos: .utility)
            do {
