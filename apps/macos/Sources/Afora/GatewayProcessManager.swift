...
final class GatewayProcessManager {
    ...
    #endif
    private let logger = Logger(subsystem: "ai.afora", category: "gateway.process")
    ...
    func startIfNeeded() {
        ...
        guard AforaConfigFile.migrateRetiredAppMetadataForGatewayStart() else {
            let message =
                "Could not repair retired macOS config metadata. Run `afora doctor --fix`, then retry."
    ...
    private func failProfilePortOwnership(port: Int, startGeneration: UInt64) async {
        ...
        let message = "Gateway port \(port) is already owned by another process or Afora profile. " +
