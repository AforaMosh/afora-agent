...
final class WebChatManager {
    ...
    private static let lastGatewayProfileIDKey = "afora.webchat.lastGatewayProfileID"
    ...
    func showSwarmFixture() {
        ...
        let controller = WebChatSwiftUIWindowController(
            ...
            transport: transport,
            windowTitle: "Afora Swarm Fixture",
            windowAutosaveName: "AforaSwarmFixture")
        controller.onClosed = { [weak self, weak controller] in
            guard let self, let controller, self.windowController === controller else { return }
            self.windowController = nil
            self.windowRoute = nil
    ...
    func show(profile: MacGatewayProfile) async throws {
        ...
        let controller = WebChatSwiftUIWindowController(
            ...
            windowTitle: "\(profile.name) — Afora",
            windowAutosaveName: "AforaChatWindow-\(profile.id)")
        controller.onVisibilityChanged = { [weak self, weak controller] visible in
            guard let self, let controller else { return }
        ...
        remainingHiddenRetries: Int = 1)
    {
        ...
        let task = Task { @MainActor [weak self] in
            ...
            do {
                ...
                let request = AforaChatGatewayRequests.setSessionObserverVisibility(visible)
                _ = try await connection.request(
