...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.afora", category: "gateway-endpoint")
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        ...
        let raw = env["AFORA_GATEWAY_PASSWORD"] ?? ""
    ...
    {
        let serviceEnv = launchdSnapshot?.environment ?? [:]
        let raw = env["AFORA_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = AppDefaults.standard.bool(forKey: "afora.onboardingSeen")
...
extension GatewayEndpointStore {
    ...
    {
        if let envBind = env["AFORA_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["AFORA_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
