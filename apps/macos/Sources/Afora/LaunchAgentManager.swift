...
enum LaunchAgentManager {
    private static let logger = Logger(subsystem: "ai.afora", category: "app.login-agent")
    private static var plistURL: URL {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Library/LaunchAgents/ai.afora.mac.plist")
    }
        ...
        preferredPaths: [String] = CommandResolver.preferredPaths()) -> String
    {
        ...
        <dict>
          <key>Label</key>
          <string>ai.afora.mac</string>
          <key>ProgramArguments</key>
          <array>
            <string>\(bundlePath)/Contents/MacOS/Afora</string>
          </array>
    ...
    private static func profileEnvironmentPlistEntries() -> String {
        ["AFORA_CONFIG_PATH", "AFORA_STATE_DIR"].compactMap { key in
            guard let value = AforaEnv.path(key) else { return nil }
            return """
