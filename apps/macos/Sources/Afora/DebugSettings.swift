...
struct DebugSettings: View {
    ...
    private var launchdSection: some View {
        GroupBox("Gateway startup") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "When enabled, Afora won't install or manage \(gatewayLaunchdLabel). " +
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("Copy sample URL") {
                        ...
                        let url = "afora://agent?message=\(encoded)&key=\(key)"
                        NSPasteboard.general.clearContents()
                ...
                Text("Deep links (afora://…) are always enabled; the key controls unattended runs.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
    ...
    private var pathsSection: some View {
        GroupBox("Paths") {
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("Afora project root")
                        .font(.caption.weight(.semibold))
                    HStack(spacing: 8) {
                        TextField("Path to afora repo", text: self.$gatewayRootInput)
                        ...
                        Button("Reset") {
                            let def = FileManager().homeDirectoryForCurrentUser
                                .appendingPathComponent("Projects/afora").path
                    ...
                    GridRow {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if let sessionStoreSaveError {
                                ...
                                Text("Used by the CLI session loader; stored in ~/.AforaMosh/afora-agent.json.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
    ...
    private var quickActionsSection: some View {
        GroupBox("Quick actions") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(
                        "Note: macOS may require restarting Afora after enabling Accessibility or Screen Recording.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    if AppProfile.current.isActive {
                        ...
                        Button {
                            LaunchdManager.startAfora()
                        } label: {
                            Label("Restart Afora", systemImage: "arrow.counterclockwise")
                        }
                        .buttonStyle(.bordered)
                        .controlSize(.small)
                    }
    ...
    private func loadSessionStorePath() {
        let parsed = AforaConfigFile.loadDict()
        guard
