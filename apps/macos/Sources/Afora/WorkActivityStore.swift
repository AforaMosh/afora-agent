...
final class WorkActivityStore {
    ...
    func handleTool(
        ...
        name: String?,
        meta: String?,
        args: [String: AforaProtocol.AnyCodable]?)
    {
    ...
    private static func buildLabel(
        name: String?,
        meta: String?,
        args: [String: AforaProtocol.AnyCodable]?) -> String
    {
    ...
    private static func wrapToolArgs(_ args: [String: AforaProtocol.AnyCodable]?) -> AforaKit.AnyCodable? {
        guard let args else { return nil }
        let converted: [String: Any] = args.mapValues { self.unwrapJSONValue($0.value) }
        return AforaKit.AnyCodable(converted)
    }

    private static func unwrapJSONValue(_ value: Any) -> Any {
        if let dict = value as? [String: AforaProtocol.AnyCodable] {
            return dict.mapValues { self.unwrapJSONValue($0.value) }
        }
        if let array = value as? [AforaProtocol.AnyCodable] {
