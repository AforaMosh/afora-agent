...
extension Notification.Name {
    static let aforaCuaDriverAvailabilityChanged = Notification.Name(
        "afora.cua-driver.availability-changed")
}
...
final class CuaDriverStderrRelay: @unchecked Sendable {
    static let managedModeNotice =
        ...
        Afora command arming and pairing are the authorization boundary.
        """
...
protocol CuaDriverProcessControlling: AnyObject {
    ...
    /// Spawned daemon pid. Afora records this itself because `serve` ignores
...
/// Owns the embedded CUA daemon as a direct Afora.app child so macOS TCC
...
final class CuaDriverHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "cua-driver-host")
    ...
    private func setReadyEndpoint(_ endpoint: CuaDriverWorkerEndpoint?) {
        ...
        self.notificationCenter.post(name: .aforaCuaDriverAvailabilityChanged, object: nil)
    }
        ...
        inheritedEnvironment: [String: String] = ProcessInfo.processInfo.environment) -> CuaDriverProcessLaunch
    {
        ...
        // Afora command arming, pairing, and tool policy own authorization upstream, matching the
        ...
        return CuaDriverProcessLaunch(
            ...
            arguments: [
                ...
                // Afora records the spawned pid itself so reaping can attribute
        ...
        onTermination: @escaping @Sendable (Int32) -> Void) throws -> any CuaDriverProcessControlling
    {
        ...
        let logger = Logger(subsystem: "ai.afora", category: "cua-driver-host")
    ...
    static func createSocketDirectory(in applicationSupportURL: URL) throws -> CuaDriverSocketDirectory {
        let aforaRoot = applicationSupportURL.appendingPathComponent("Afora", isDirectory: true)
        let root = aforaRoot.appendingPathComponent("cua", isDirectory: true)
        for directory in [applicationSupportURL, aforaRoot, root] {
            var status = stat()
