...
private let deepLinkLogger = Logger(subsystem: "ai.afora", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        case let .agent(link):
            guard !AppStateStore.shared.isPaused else {
                self.presentAlert(title: "Afora is paused", message: "Unpause Afora to run agent actions.")
                return
            }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Afora agent?", message: body) else { return }
        }
