...
extension AforaComputerActParams {
...
final class ComputerWindowActionExecutor {
    ...
    private func listApps() async throws -> AforaComputerActResult {
        ...
        return AforaComputerActResult(ok: true, details: [
    ...
    private func listWindows() async throws -> AforaComputerActResult {
        ...
        return AforaComputerActResult(ok: true, details: details)
    }

    private func getAccessibilityTree(
        _ params: AforaComputerActParams) async throws -> AforaComputerActResult
    {
    ...
    private func getCursorPosition() -> AforaComputerActResult {
        ...
        _ params: AforaComputerActParams) async throws -> AforaComputerActResult
    {
        ...
        let elements = zip(bounded, issued.elementRefs).map { element, ref in
            AforaComputerObservationElement(
                elementRef: ref,
        ...
        return AforaComputerActResult(
            ok: true,
            observation: AforaComputerObservation(
                kind: "window",
    ...
    private func launchApp(_ params: AforaComputerActParams) async throws -> AforaComputerActResult {
        ...
        return AforaComputerActResult(ok: true, effect: .confirmed, details: [
            "name": AnyCodable(launched.name),
            "pid": AnyCodable(Int(launched.processIdentifier)),
        ])
    }

    private func killApp(_ params: AforaComputerActParams) async throws -> AforaComputerActResult {
        ...
        _ params: AforaComputerActParams) async throws -> AforaComputerActResult
    {
        ...
        return AforaComputerActResult(
            ...
            escalation: confirmed ? nil : AforaComputerEscalation(
                recommended: "desktop",
    ...
    private func click(_ params: AforaComputerActParams) async throws -> AforaComputerActResult {
        let target = try self.requiredWindow(params)
        ...
        if mode == .foreground {
            ...
            return AforaComputerActResult(ok: true, effect: .unverifiable, details: [
    ...
    private func type(_ params: AforaComputerActParams) async throws -> AforaComputerActResult {
    ...
    private func key(_ params: AforaComputerActParams) async throws -> AforaComputerActResult {
    ...
    private func setValue(_ params: AforaComputerActParams) async throws -> AforaComputerActResult {
    ...
    private func invokeMenu(_ params: AforaComputerActParams) async throws -> AforaComputerActResult {
        guard params.deliveryMode != .foreground else {
            throw ComputerActionService.ComputerActionError.unsupportedAction(params.action)
        }
        let target = try self.requiredWindow(params)
    ...
    private func scroll(_ params: AforaComputerActParams) async throws -> AforaComputerActResult {
        let windowRef = try Self.require(params.windowRef, field: "windowRef")
        let target = try self.resolveWindow(windowRef)
    ...
    private func requiredWindow(_ params: AforaComputerActParams) throws -> WindowTarget {
    ...
    private func focusElement(
        _ elementRef: String,
        params: AforaComputerActParams,
        target: WindowTarget,
        foreground: Bool) async throws -> AforaComputerActResult
    {
        let windowRef = try Self.require(params.windowRef, field: "windowRef")
        let observation = try self.resolveObservation(params.observationId, windowRef: windowRef)
        ...
        if foreground {
            ...
            return AforaComputerActResult(ok: true, effect: .unverifiable)
        }
        guard let identity = target.window.mutationIdentity else {
            throw ComputerActionService.ComputerActionError.staleObservation
        }
...
extension ComputerWindowActionExecutor {
    private static func result(
        ...
        background: Bool) -> AforaComputerActResult
    {
        ...
        if outcome.effect == .refused {
            return background
                ...
                : AforaComputerActResult(
                    ...
                    escalation: AforaComputerEscalation(
                        recommended: "desktop",
                        reasonCode: "foreground_ineffective"))
        }
        let effect: AforaComputerActionEffect = switch outcome.effect {
        ...
        let escalation = background && outcome.effect == .suspectedNoop
            ? AforaComputerEscalation(recommended: "foreground", reasonCode: "suspected_noop")
            : nil
        var details: [String: AnyCodable] = [
    ...
    private static func backgroundEscalation(reason: String) -> AforaComputerActResult {
        AforaComputerActResult(
            ok: false,
            effect: .suspectedNoop,
            escalation: AforaComputerEscalation(
    ...
    private static func observationLimits(
        _ params: AforaComputerActParams) throws -> (depth: Int, maxElements: Int)
    {
    ...
    private static func bounds(_ rect: CGRect) -> AforaComputerBounds {
        AforaComputerBounds(
    ...
    private static func scrollDirection(
        _ direction: AforaComputerScrollDirection) -> PeekabooFoundation.ScrollDirection
    {
        switch direction {
        case .up: .up
        case .down: .down
