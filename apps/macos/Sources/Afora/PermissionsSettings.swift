            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                SettingsCardGroup("Privacy") {
                    SettingsCardToggleRow(
                        ...
                        subtitle: """
                        Share this Mac's idle duration so Afora can identify the Mac you used most recently \
        ...
        return HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(complete ? "All access granted" : "\(granted) of \(total) permissions granted")
                    .font(.headline)
                Text("Afora only asks for macOS capabilities when a feature needs them.")
                    .font(.footnote)
...
private struct LocationAccessSettings: View {
    ...
    private var locationModeRaw: String = AforaLocationMode.off.rawValue
    @AppStorage(locationPreciseKey, store: AppDefaults.standard) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = AforaLocationMode.off.rawValue
        ...
        VStack(spacing: 0) {
                ...
                subtitle: "Allow agents to use device location when a tool asks for it.")
            {
                Picker("Location Access", selection: self.$locationModeRaw) {
                    Text("Off").tag(AforaLocationMode.off.rawValue)
                    Text("While Using").tag(AforaLocationMode.whileUsing.rawValue)
                    Text("Always").tag(AforaLocationMode.always.rawValue)
                }
                .labelsHidden()
                .pickerStyle(.menu)
    ...
    private var locationMode: AforaLocationMode {
        AforaLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: AforaLocationMode) async -> Bool {
