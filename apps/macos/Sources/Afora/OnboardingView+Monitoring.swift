...
extension OnboardingView {
    ...
    func startExistingCLIActivationIfNeeded() {
        ...
        OnboardingController.shared.busyReason = "Afora is starting the Gateway service."
        cliStatus = "Starting Afora Gateway…"
    ...
    func finishExistingCLIActivation() async {
        ...
        case .ready:
            cliInstalled = true
            cliStatus = "Afora Gateway is ready."
        case .deferred:
            cliInstalled = false
            cliStatus = "Afora is paused. Resume it, then retry setup to start the Gateway."
        case .failed:
            cliInstalled = false
            cliStatus = "Afora is installed, but the Gateway did not start. Retry setup."
        }
    }
    ...
    func startCLIInstall() {
        ...
        OnboardingController.shared.busyReason = "Afora is installing the Gateway service."
    ...
    func runCLIInstall() async {
        ...
        if !Self.shouldActivateLocalGateway(afterCLIInstallFor: self.state.connectionMode) {
            cliStatus = "Afora CLI is ready for the Mac node."
            cliInstalled = true
            return
        }
        cliStatus = "Starting Afora Gateway…"
        ...
        case .ready:
            cliStatus = "Afora Gateway is ready."
        case .deferred:
            cliStatus = "Afora is installed. The Gateway will start when This Mac is active and resumed."
        case .failed:
            cliStatus = "Afora was installed, but the Gateway did not start. Retry setup."
            return
        }
        cliInstalled = true
    }
