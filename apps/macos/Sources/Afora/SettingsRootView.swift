...
struct SettingsRootView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .aforaSelectSettingsTab)) { note in
        ...
        .onReceive(NotificationCenter.default.publisher(for: .aforaConfigDidChange)) { _ in
    ...
    private var nixManagedBanner: some View {
        ...
        let configPath = self.snapshotPaths.configPath ?? AforaPaths.configURL.path
        let stateDir = self.snapshotPaths.stateDir ?? AforaPaths.stateDirURL.path
    ...
    private func scheduleInferenceRefresh(clearPrevious: Bool, resetSystemAgent: Bool = false) {
        if resetSystemAgent {
            // Afora sessions are gateway-owned. Re-key the cached detail so a route
            // change cannot send old conversation state to a new endpoint.
            self.systemAgentChatIdentity = UUID()
        }
        if clearPrevious {
            // Preserve an active or pending Afora request while config truth is revalidated.
...
struct SettingsTabGroup: Identifiable {
    ...
    static func defaultGroups(showDebug: Bool, showSystemAgent: Bool) -> [SettingsTabGroup] {
        ...
        var groups = [
            ...
            SettingsTabGroup(title: "Afora", tabs: [.about]),
        ]
...
extension Notification.Name {
    static let aforaSelectSettingsTab = Notification.Name("aforaSelectSettingsTab")
}
