...
final class ComputerScreenActionExecutor {
        ...
        -> AforaComputerActResult
    {
        ...
        return AforaComputerActResult(ok: true, cursorX: cursor.x, cursorY: cursor.y)
    }
    ...
    private func peekabooClick(at point: CGPoint, action: AforaComputerAction) async throws {
    ...
    private struct ResolvedDisplay {
        var geometry: AforaComputerDisplayGeometry
        ...
        display: ResolvedDisplay) throws -> CGPoint?
    {
        ...
        let capturedWidth = AforaComputerInputGeometry.capturedWidth(
            refWidth: refWidth,
        ...
        let clamped = AforaComputerInputGeometry.clampToDisplay(
            x: mapped.x,
            y: mapped.y,
            display: geometry)
    ...
    static func validatedCurrentCursorPoint(
        _ point: CGPoint?,
        display: AforaComputerDisplayGeometry) throws -> CGPoint
    {
        guard let point,
    ...
    private func resolveDisplay(params: AforaComputerActParams) async throws -> ResolvedDisplay {
        ...
        let geometry = AforaComputerDisplayGeometry(
        ...
        guard AforaComputerInputGeometry.isValidMappingGeometry(
        ...
        if Self.usesScreenshotCoordinates(params) {
            let refWidth = try Self.requiredReferenceWidth(params)
            let currentFrameId = AforaComputerInputGeometry.displayFrameId(
    ...
    private static func usesScreenshotCoordinates(_ params: AforaComputerActParams) -> Bool {
        params.x != nil || params.y != nil || params.fromX != nil || params.fromY != nil
    }

    private static func requiredReferenceWidth(_ params: AforaComputerActParams) throws -> Int {
    ...
    private func rawScroll(direction: AforaComputerScrollDirection, amount: Int, flags: CGEventFlags) throws {
