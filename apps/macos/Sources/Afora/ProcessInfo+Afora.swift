...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.afora.mac`) even if the shipped
    ...
    {
        if environment["AFORA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "afora.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "afora.nixMode") { return true }
