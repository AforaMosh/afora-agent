...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        onboardingPage {
            VStack(spacing: 18) {
                ...
                Label {
                    Text(
                        "Afora can take actions using the permissions and services you enable. " +
    ...
    func connectionPage() -> some View {
        onboardingPage {
            Text("Where should your assistant live?")
                .font(.largeTitle.weight(.semibold))
            Text(
                "Most people pick this Mac — Afora installs everything and keeps it " +
            ...
            if self.selectedConnectionMode == .unconfigured {
                Text("OK — Afora won’t start anything yet. Pick Local or Remote later in Settings → General.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
        ...
        .onChange(of: self.state.connectionMode) { _, newValue in
            ...
            // retires route-owned AI/Afora state. This nested observer owns probe copy only.
        ...
        return VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Verify Afora can reach this gateway.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer(minLength: 0)
            ...
            GridRow {
                ...
                Text(
                    "The current gateway.remote.token value is not plain text. "
                        + "Afora for macOS cannot use it directly; "
        ...
        if self.state.remoteTransport == .ssh {
            ...
            GridRow {
                Text("Project root")
                    .font(.callout.weight(.semibold))
                    .frame(width: labelWidth, alignment: .leading)
                TextField("/home/you/Projects/afora", text: self.$state.remoteProjectRoot)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: fieldWidth)
            }
            GridRow {
        ...
        action: @escaping () -> Void) -> some View
    {
        Button {
            ...
            .aforaSelectableRowChrome(selected: selected)
        }
        .buttonStyle(.plain)
    }
    ...
    func cliPage() -> some View {
        ...
        let setupDetail = if remoteMode {
            "Afora is installing the matching runtime for this Mac node. " +
                "It will connect to your selected Gateway without starting another one here."
        } else {
            "Afora is setting up its background service on this Mac."
        }
        let detail = setupDetail + " Published Stable and Beta installs are usually quick. " +
            "Dev (Git main) downloads and builds Afora from source, so allow several minutes " +
        ...
        onboardingPage {
            ...
            self.onboardingCard {
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the Afora menu bar icon for the compact chat panel and status.",
