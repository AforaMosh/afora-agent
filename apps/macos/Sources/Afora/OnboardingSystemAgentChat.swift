...
/// Settings talks to Afora over the gateway `afora.chat` RPC after
...
final class SystemAgentOnboardingChatModel {
    ...
    var input = ""
    /// Set when Afora hands off to the normal agent ("talk to agent").
    ...
    /// status/repair greeting (used by Settings → Afora).
    ...
    func startIfNeeded() async {
        ...
        if Task.isCancelled, self.requestGeneration == generation {
            self.started = false
            self.errorMessage = "Afora was interrupted. Restart to try again."
        }
    }
        ...
        do {
            ...
            if error is CancellationError || Task.isCancelled {
                ...
                self.errorMessage = Task.isCancelled
                    ? "Afora was interrupted. Restart to try again."
                    : "The Gateway connection changed. Restart Afora to reconnect."
                return
            }
            ...
            HStack(spacing: 8) {
                Group {
                    if self.model.expectsSensitiveReply {
                        ...
                        TextField(
                            "Reply to Afora… (yes sets everything up)",
