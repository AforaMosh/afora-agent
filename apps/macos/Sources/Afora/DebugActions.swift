...
enum DebugActions {
    private static let verboseDefaultsKey = "afora.debug.verboseMain"
    private static let onboardingSeenKey = "afora.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AforaPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Afora", body: "Test notification", sound: nil)
    }
    ...
    static func restartApp() {
        ...
        if let profile = AppProfile.current.name {
            task.arguments = [
                "-c",
                "sleep 0.2; open -n --env AFORA_PROFILE=\"$2\" \"$1\"",
                "_",
                url.path,
                profile,
            ]
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AforaPaths.configURL
        guard
