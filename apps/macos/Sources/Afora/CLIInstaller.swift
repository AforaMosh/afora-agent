...
extension Notification.Name {
    static let aforaCLIInstalled = Notification.Name("afora.cli.installed")
}
...
enum CLIInstaller {
    ...
    enum Status: Equatable {
        ...
        var message: String {
            ...
            case let .ready(_, version):
                "Afora Gateway \(version) is ready."
            case .missing:
                "Afora Gateway is not installed yet."
            case .unusable:
                "The Afora Gateway could not be verified. Setup will repair it."
        ...
        fileManager: FileManager) -> [String]
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("afora").path
        ...
        profile: AppProfile = .current) -> String
    {
        URL(fileURLWithPath: self.installPrefix(homeDirectory: homeDirectory, profile: profile))
            .appendingPathComponent("bin/afora")
            .path
    }
        ...
        statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async -> Bool
    {
        ...
        await statusHandler("Installing Afora CLI (\(target.selector))…")
        guard let installerURL = Bundle.main.url(forResource: "install-cli", withExtension: "sh") else {
            await statusHandler("Install failed: installer resource is missing. Reinstall Afora.")
            return false
        }
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed afora \($0)." } ?? "Installed afora."
            self.rememberInstallPolicy(target)
            await statusHandler(summary)
            NotificationCenter.default.post(name: .aforaCLIInstalled, object: nil)
            return true
        }
        ...
        -> ManagedCLIUpdateOutcome
    {
        ...
        await statusHandler(repair
            ? String(localized: "Repairing the Afora Gateway update…")
            : String(localized: "Updating the Afora Gateway to \(targetVersion)…"))
        ...
        NotificationCenter.default.post(name: .aforaCLIInstalled, object: nil)
        await statusHandler(String(localized: "Afora Gateway \(installedVersion) is installed."))
        return .success(
    ...
    private static func devCheckoutLocation(prefix: String) -> String {
        URL(fileURLWithPath: prefix)
            .appendingPathComponent("dev/afora")
            .path
    }
    ...
    nonisolated static func installStatus(forEventLine line: String) -> String? {
        ...
        case ("git-clone", "start"): "Downloading Afora source…"
        case ("git-update", "start"): "Updating Afora source…"
        case ("dependencies", "start"): "Installing dependencies…"
        case ("control-ui", "start"): "Building interface…"
        case ("cli-build", "start"): "Building Afora CLI…"
        case ("afora", "retry"): "Retrying Afora CLI install…"
        case ("disk-space", "warn"): "Couldn’t verify free disk space; continuing…"
        case ("git-update", "warn"): "Using the existing modified Afora source…"
