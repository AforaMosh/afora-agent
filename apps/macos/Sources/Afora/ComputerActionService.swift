...
final class ComputerActionExecutionQueue {
    typealias Operation = @MainActor (AforaComputerActParams, UInt64) async throws
    ...
    private struct QueuedAction {
        ...
        let continuation: CheckedContinuation<AforaComputerActResult, Error>
    ...
    private var currentActionTask: Task<AforaComputerActResult, Error>?
    ...
    func perform(
        ...
        operation: @escaping Operation) async throws -> AforaComputerActResult
    {
        let actionID = UUID()
        let cancellationState = ComputerActionCancellationState()
    ...
    private func drain() async {
        while !self.pendingActions.isEmpty {
            ...
            let outcome: Result<AforaComputerActResult, Error>
            do {
...
final class ComputerActionService {
    enum ComputerActionError: LocalizedError {
        ...
        case unsupportedAction(AforaComputerAction)
        case refused(String)
    ...
    func typeTextForTesting(
        _ text: String,
        lifecycleGeneration: UInt64 = 0) async throws -> AforaComputerActResult
    {
        let params = AforaComputerActParams(action: .type, text: text)
        ...
        { [weak self] _, generation in
            ...
            return AforaComputerActResult(ok: true, cursorX: 0, cursorY: 0)
        }
    }
    #endif
}
