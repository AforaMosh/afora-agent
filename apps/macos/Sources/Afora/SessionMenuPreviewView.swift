...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.afora", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AforaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AforaSessionPreviewEntry,
    ...
    private static func cache(payload: AforaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AforaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AforaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AforaKit.AnyCodable]) -> [AforaChatMessage] {
    ...
    private static func previewText(for message: AforaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AforaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AforaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AforaChatMessage) -> String? {
