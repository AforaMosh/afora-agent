...
extension Notification.Name {
    static let aforaNodeHostWorkerFailed = Notification.Name("afora.node-host-worker.failed")
    static let aforaNodeHostWorkerRetryExhausted = Notification.Name(
        "afora.node-host-worker.retry-exhausted")
}
...
final class MacNodeHostWorker: MacNodeHostWorking, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "node-host-worker")
    private let queue = DispatchQueue(label: "ai.afora.node-host-worker")
    private let writerQueue = DispatchQueue(label: "ai.afora.node-host-worker.writer")
    ...
    private func startLocked(launch: MacNodeHostWorkerLaunch) {
        ...
        environment["AFORA_NODE_EXEC_HOST"] = "app"
        environment["AFORA_NODE_EXEC_FALLBACK"] = "0"
        self.launchedWorker = launch
    ...
    private static func decodeInvokeResponse(_ result: [String: Any], id: String) -> BridgeInvokeResponse {
        ...
        let code = AforaNodeErrorCode(rawValue: rawError?["code"] as? String ?? "UNAVAILABLE") ?? .unavailable
        let error = ok ? nil : AforaNodeError(
            code: code,
    ...
    private static func unavailableResponse(_ id: String, _ message: String) -> BridgeInvokeResponse {
        BridgeInvokeResponse(
            id: id,
            ok: false,
            error: AforaNodeError(code: .unavailable, message: message))
    }
