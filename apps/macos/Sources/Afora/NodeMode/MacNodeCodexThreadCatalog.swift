...
enum MacNodeCodexThreadCatalog {
    ...
    private static let commandEnvironmentKey = "AFORA_CODEX_APP_SERVER_BIN"
    private static let argumentsEnvironmentKey = "AFORA_CODEX_APP_SERVER_ARGS"
    ...
    static func shouldAdvertise(root: [String: Any]? = nil) -> Bool {
        let root = root ?? AforaConfigFile.loadDict()
        guard AforaConfigFile.configuredBundledPluginAllowed(
...
extension MacNodeCodexThreadCatalog {
    ...
    private static func configuredPlugin(root: [String: Any]) throws -> ConfiguredPlugin? {
        guard let entry = AforaConfigFile.pluginEntry(
            MacNodeCodexThreadCatalogContract.pluginId,
            root: root)
        else { return nil }
