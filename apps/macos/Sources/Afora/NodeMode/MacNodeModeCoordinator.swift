...
final class MacNodeModeCoordinator: NSObject {
    ...
    private let logger = Logger(subsystem: "ai.afora", category: "mac-node")
    ...
    override private convenience init() {
        ...
        let nodeHostWorker = MacNodeHostWorker(session: session) { generation in
            NotificationCenter.default.post(name: .aforaNodeHostWorkerFailed, object: NSNumber(value: generation))
        }
        self.init(
            session: session,
        ...
        nodeHostWorkerRetryPolicy: MacNodeHostWorkerRetryPolicy = MacNodeHostWorkerRetryPolicy())
    {
        ...
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.refreshNodeConfiguration),
            name: .aforaPermissionsChanged,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostWorkerFailed),
            name: .aforaNodeHostWorkerFailed,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .aforaConfigDidChange,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .aforaCLIInstalled,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .aforaCuaDriverAvailabilityChanged,
            object: nil)
    }
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = AforaConfigFile.browserControlEnabled()
        ...
        claudeSessionCatalogEnabled: Bool) async throws -> ConnectionAttempt?
    {
        ...
        if !nativeCaps.contains(AforaCapability.computer.rawValue) {
            ...
            onInvoke: { [weak self] req in
                guard let self else {
                    return BridgeInvokeResponse(
                        id: req.id,
                        ok: false,
                        error: AforaNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                }
...
extension MacNodeModeCoordinator {
        ...
        claudeSessionCatalogEnabled: Bool) -> [String]
    {
        ...
        return Self.resolvedCaps(
            ...
            computerControlEnabled: computerControlEnabled,
            computerControlProvider: computerControlProvider,
            locationMode: AforaLocationMode(rawValue: rawLocationMode) ?? .off,
    ...
    private func handleNodeHostWorkerFailure(configurationGeneration: UInt64) {
        ...
        case let .giveUp(unexpectedExitCount):
            ...
            self.notificationCenter.post(
                name: .aforaNodeHostWorkerRetryExhausted,
                object: self,
...
extension MacNodeModeCoordinator {
    ...
    nonisolated static func resolvedCaps(
        ...
        computerControlEnabled: Bool,
        computerControlProvider: ComputerControlProvider = .peekaboo,
        locationMode: AforaLocationMode,
        ...
        claudeSessionCatalogEnabled: Bool = false) -> [String]
    {
        ...
        if computerControlEnabled, computerControlProvider == .peekaboo {
            caps.append(AforaCapability.computer.rawValue)
        }
        ...
        computerControlProvider: ComputerControlProvider = .peekaboo) -> [String]
    {
        ...
        commands.append(AforaSystemCommand.notify.rawValue)

        let capsSet = Set(caps)
        if capsSet.contains(AforaCapability.camera.rawValue) {
            ...
            commands.append(AforaCameraCommand.ptzStatus.rawValue)
            commands.append(AforaCameraCommand.ptzControl.rawValue)
        }
        if capsSet.contains(AforaCapability.location.rawValue) {
        ...
        if capsSet.contains(AforaCapability.computer.rawValue) {
            commands.append(AforaComputerCommand.act.rawValue)
        }
        ...
        for provider: ComputerControlProvider) -> MacNodeHostManifest?
    {
        ...
        return MacNodeHostManifest(
            ...
            caps: manifest.caps.filter { $0 != AforaCapability.computer.rawValue },
    ...
    nonisolated static func computerUseDescriptor(
        ...
        commands: [String],
        workerManifest: MacNodeHostManifest?) -> AforaProtocol.AnyCodable?
    {
        guard commands.contains(MacNodeScreenCommand.snapshot.rawValue),
              commands.contains(AforaComputerCommand.act.rawValue)
        else { return nil }
