...
enum SessionActions {
    ...
    {
        let request = AforaChatGatewayRequests.patchSessionPreferences(
            sessionKey: key,
            agentID: nil,
    ...
    static func resetSession(key: String) async throws {
        let request = AforaChatGatewayRequests.resetSession(sessionKey: key, agentID: nil)
        _ = try await ControlChannel.shared.request(request)
    }

    static func deleteSession(key: String) async throws {
        let request = AforaChatGatewayRequests.deleteSession(sessionKey: key, agentID: nil)
        _ = try await ControlChannel.shared.request(request)
    }
    ...
    static func openSessionLogInCode(sessionId: String, storePath: String?) {
        let candidates: [URL] = {
            ...
            urls.append(AforaPaths.stateDirURL.appendingPathComponent("sessions/\(sessionId).jsonl"))
            return urls
        }()
