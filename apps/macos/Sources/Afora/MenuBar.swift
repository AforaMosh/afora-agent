...
@main
enum AforaProcessMain {
    static func main() {
        if let status = AforaProcessEntrypoint.run(arguments: CommandLine.arguments, launchApplication: {
            AforaApp.main()
        }) {
            Darwin.exit(status)
        }
...
struct AforaApp: App {
    ...
    private static let logger = Logger(subsystem: "ai.afora", category: "app")
    ...
    init() {
        ...
        if let error = AppProfile.current.validationError {
            if launchPlan.isElevationHost {
                fputs("Afora elevation host profile is invalid: \(error.localizedDescription)\n", stderr)
                Darwin.exit(2)
            }
            let alert = NSAlert()
            alert.alertStyle = .critical
            alert.messageText = "Afora profile is invalid"
            alert.informativeText = error.localizedDescription
            alert.runModal()
            Darwin.exit(2)
        }
        if AppProfile.current.isActive,
           !DeviceIdentityStore.configureStateDirectory(AforaPaths.stateDirURL)
        {
        ...
        AforaLogging.bootstrapIfNeeded()
    ...
    var body: some Scene {
        ...
        Window("Afora Settings", id: SettingsWindowOpener.windowID) {
    ...
    private func applyStatusItemAppearance(paused _: Bool, sleeping _: Bool) {
        ...
        self.statusItem?.button?.toolTip = self.state.voiceWakeMeterActive
            ? "Afora - Voice Wake live meter active"
            : "Afora"
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let webChatAutoLogger = Logger(subsystem: "ai.afora", category: "Chat")
    ...
    override init() {
        ...
        if hasReplacementMetadata, !isReplacementHandoff {
            fputs("Afora replacement handoff authentication failed.\n", stderr)
            Darwin.exit(2)
        }
        ...
        if let exitCode = Self.processExitCode(for: ownership) {
            fputs("Afora profile is already running.\n", stderr)
        ...
        if let instanceOwnershipFailure {
            if AppLaunchRuntimePlan.current.isElevationHost {
                fputs(
                    "Afora elevation host could not claim its instance lock: \(instanceOwnershipFailure)\n",
                    stderr)
                Darwin.exit(2)
            }
            let alert = NSAlert()
            alert.alertStyle = .critical
            alert.messageText = "Afora could not claim its instance lock"
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        if !AppProfile.current.isActive, !launchPlan.isElevationHost {
            ...
            case let .continueLaunch(startUpdater):
                if startUpdater, launchPlan.allowsUpdater {
                    if AforaConfigFile.gatewayUpdateChannel() == nil {
        ...
        if launchPlan.allowsAutomaticPresentation,
           ProcessInfo.processInfo.environment["AFORA_DEBUG_PAIRING_DEMO"] == "1"
        {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
    ...
    private func scheduleFirstRunOnboardingIfNeeded(gatewayConnected: Bool) async {
           ...
           gatewayConnected
        {
            ...
            // default-agent model cannot run Afora and must stay in onboarding.
            do {
...
final class SparkleUpdaterController: NSObject, UpdaterProviding {
        ...
        onStart: (() -> Void)? = nil)
    {
        self.gatewayUpdateChannelResolver = gatewayUpdateChannelResolver ?? {
            ...
            return AforaConfigFile.normalizedGatewayUpdateChannel(response.effectiveChannel)
        }
...
extension SparkleUpdaterController: SPUUpdaterDelegate {
    func allowedChannels(for _: SPUUpdater) -> Set<String> {
        allowedSparkleChannels(
            forGatewayUpdateChannel: self.gatewayUpdateChannel ?? AforaConfigFile.gatewayUpdateChannel())
    }

    func bestValidUpdate(in appcast: SUAppcast, for _: SPUUpdater) -> SUAppcastItem? {
        guard self.gatewayUpdateChannel ?? AforaConfigFile.gatewayUpdateChannel() == "extended-stable" else {
            return nil
        }
