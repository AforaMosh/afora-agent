...
/// shared with iOS through AforaKit.
enum MacChatTranscriptCache {
    ...
    @MainActor private static var databasesByDirectory: [String: AforaClientDatabases] = [:]
    @MainActor private static var storesByKey: [String: AforaChatSQLiteTranscriptCache] = [:]
    ...
    ///   session data: distinct profiles (distinct `AFORA_STATE_DIR`) never
    ...
    static func currentGatewayID() -> String? {
        ...
        return self.gatewayID(
            mode: mode,
            localStateDir: AforaConfigFile.stateDirURL(),
    ...
    static func makeContext(gatewayID: String) -> Context? {
        ...
        let root = base.appendingPathComponent("Afora", isDirectory: true)
    ...
    @MainActor
    static func store(databaseDirectoryURL: URL, gatewayID: String) -> AforaChatSQLiteTranscriptCache? {
        guard let databases = self.databases(
            directoryURL: databaseDirectoryURL,
    ...
    private static func store(
        ...
        gatewayID: String) -> AforaChatSQLiteTranscriptCache
    {
    ...
    private static func databases(
        directoryURL: URL,
        legacyDirectoryURL: URL) -> AforaClientDatabases?
    {
        ...
        guard let databases = try? AforaClientDatabases(
