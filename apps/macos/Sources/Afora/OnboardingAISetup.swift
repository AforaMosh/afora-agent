...
///
/// Drives the gateway's `afora.setup.detect` / `afora.setup.activate`
    ...
    private(set) var phase: Phase = .idle {
        didSet {
            ...
            OnboardingController.shared.busyReason = if self.phase == .testing {
                "Afora is testing your AI connection."
            } else if self.activeAuthOption != nil {
                self.isPreparingModel
                    ? "Afora is preparing a local model."
                    : "Afora is completing provider sign-in."
            } else {
                nil
            }
        }
        ...
        context: AttemptContext) async -> PendingVerificationOutcome
    {
        ...
        if let activationOwner = pendingActivationOwner {
            ...
            guard let currentFingerprint = await gateway.activationOwnershipFingerprint(
                ...
                self.phase = .ready
                self.detectError = Self.transportFailure(
                    "Secure storage is unavailable, so Afora cannot verify which Gateway completed AI setup.")
                return .notConnected
            }
