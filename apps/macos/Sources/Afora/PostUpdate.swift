...
final class PostUpdateModel {
    ...
    var title = String(localized: "Finishing your Afora update")
...
final class PostUpdateController: NSObject, NSWindowDelegate {
    ...
    static let updateGuideURL = URL(string: "https://docs.afora.ai/install/updating")!
    ...
    private func show() {
        ...
        window.title = String(localized: "Afora updated")
    ...
    private func run() {
        ...
        self.model.title = String(localized: "Finishing your Afora update")
    ...
    private func finishUpdate(receipt: PostAppUpdateReceipt) async {
        ...
        let ownsManagedRuntime = Self.ownsManagedRuntime(
            ...
            gatewayUpdateChannel: AforaConfigFile.gatewayUpdateChannel(),
        ...
        case .install:
            ...
            guard installed else {
                self.fail(
                    ...
                    details: String(localized: "The managed Afora runtime could not be reinstalled."))
                return
            }
        }
        ...
        connectionMode: AppState.ConnectionMode)
    {
        ...
        self.model.message = connectionMode == .local
            ? String(localized: "Afora \(receipt.toVersion) and its Gateway are ready.")
            : String(localized: "Afora \(receipt.toVersion) and its Mac node runtime are ready.")
        self.model.details = switch (notification, notificationRetryScheduled) {
        case (.retryLater, true):
            String(localized: "Your agent could not be notified yet. Afora will retry after the next app launch.")
        case (.retryLater, false):
            if connectionMode == .local {
                String(
                    localized: """
                    Afora could not notify your agent automatically. \
        ...
        case (.deliveryUnconfirmed, _):
            String(
                localized: """
                Afora could not confirm the agent notification. \
                It will not retry, to avoid a duplicate welcome.
                """)
        case (.skippedUnsupportedGateway, _):
            String(
                localized: "The remote Gateway is older than this Mac app, so Afora skipped the agent notification.")
        case (.skippedWhilePaused, _):
            String(localized: "The Gateway remains paused, so Afora did not wake your agent.")
        default:
            nil
        }
    }
    ...
    private func verifyRuntime(connectionMode: AppState.ConnectionMode) async -> Bool {
        ...
        guard ControlChannel.shared.state == .connected else {
            self.fail(
                ...
                details: String(
                    localized: "Afora installed the update but could not verify the Gateway connection."))
            return false
        }
        return true
    }
        ...
        receipt: PostAppUpdateReceipt) async -> PostUpdateNotificationOutcome
    {
        ...
        do {
            guard let selected = try await Self.preferredNotificationSession(loadPage: { offset in
                var params: [String: AforaKit.AnyCodable] = [
                    "limit": AforaKit.AnyCodable(100),
                    "includeGlobal": AforaKit.AnyCodable(false),
                    "includeUnknown": AforaKit.AnyCodable(false),
                    "configuredAgentsOnly": AforaKit.AnyCodable(true),
                    "requireLastInteraction": AforaKit.AnyCodable(true),
                    "sortBy": AforaKit.AnyCodable("lastInteractionAt"),
                ]
                if offset > 0 {
                    params["offset"] = AforaKit.AnyCodable(offset)
                }
        ...
        let text =
            "Afora updated to \(version). Briefly welcome the user back and say you are updated, " +
        ...
        do {
            _ = try await GatewayConnection.shared.request(
                method: "system-event",
                params: [
                    "text": AforaKit.AnyCodable(text),
                    "sessionKey": AforaKit.AnyCodable(session.key),
                    "wake": AforaKit.AnyCodable(true),
                ],
                ifCurrentServerLease: serverLease)
            return .delivered
        } catch {
    ...
    static func notificationSendFailureOutcome(_ error: Error) -> PostUpdateNotificationOutcome {
        if error is AforaChatTransportSendError || error is GatewayResponseError {
            return .retryLater
        }
        ...
        receipt: PostAppUpdateReceipt)
    {
        ...
        case .local:
            self.fail(
                ...
                details: String(
                    localized: """
                    Afora could not read the Gateway service ownership record. \
        ...
        case .remote:
            self.fail(
                ...
                details: String(
                    localized: """
                    Afora could not read the node service ownership record. \
...
private struct PostUpdateView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            GlowingAforaIcon(size: 150, mood: self.model.mood)
                .frame(height: 205)
