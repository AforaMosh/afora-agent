...
struct OnboardingAISetupView: View {
        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(self.model.waitingForPendingActivationDeadline
                    ...
                    .font(.callout.weight(.semibold))
                Text(self.model.waitingForPendingActivationDeadline
                    ? "Afora will check again before changing any inference settings."
    ...
    private func candidateRow(_ candidate: OnboardingAISetupModel.Candidate) -> some View {
        ...
        .aforaSelectableRowChrome(selected: selected && !Self.isFailed(status))
    }
        ...
        if !self.model.prepareOptions.isEmpty {
            VStack(alignment: .leading, spacing: 8) {
                ...
                ForEach(self.model.prepareOptions) { option in
                    ...
                    .aforaSelectableRowChrome(selected: false)
                }
            }
            .padding(12)
            .frame(maxWidth: .infinity, alignment: .leading)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "Use an existing subscription or provider account. " +
                        "Afora opens the provider’s own sign-in flow, then verifies it with a real reply.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private var apiKeysRow: some View {
        ...
        .buttonStyle(.plain)
        .disabled(self.model.isBusy)
        .aforaSelectableRowChrome(selected: self.model.showManualEntry)
    }
            ...
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(self.model.isPreparingModel
                        ? "Afora will detect and verify the prepared model before using it."
    ...
    private var manualProviderHelp: String {
        ...
        guard let hint, !hint.isEmpty else {
            return "Paste the key or token here, and Afora checks it with a real test question."
        }
        return "\(hint). Paste it here, and Afora checks it with a real test question."
    }
}

/// Friendly error presentation with a consistent docs escape hatch.
/// Every onboarding failure points at a docs.afora.ai page so people are
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 14) {
                    ...
                    Button("Open help…") {
                        if let url = URL(string: "https://docs.afora.ai/\(docsSlug)") {
                            NSWorkspace.shared.open(url)
                        }
                    }
                    .buttonStyle(.link)
