...
@Suite(.serialized)
struct AforaConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("afora-config-\(UUID().uuidString)")
            .appendingPathComponent("afora.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["AFORA_CONFIG_PATH": override]) {
            #expect(AforaConfigFile.url().path == override)
        }
    }
    ...
    func `browser control enabled reads config flag`() async {
        ...
        await TestIsolation.withEnvValues(["AFORA_CONFIG_PATH": override]) {
            #expect(AforaConfigFile.browserControlEnabled() == true)
            AforaConfigFile.saveDict(["browser": ["enabled": false]])
            #expect(AforaConfigFile.browserControlEnabled() == false)
            AforaConfigFile.setBrowserControlEnabled(true)
            #expect(AforaConfigFile.browserControlEnabled() == true)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["AFORA_CONFIG_PATH": override]) {
            ...
            #expect(AforaConfigFile.remoteGatewayPort() == 19999)
            #expect(AforaConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(AforaConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(AforaConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(AforaConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(AforaConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("afora-state-\(UUID().uuidString)", isDirectory: true)
            .path
        ...
        await TestIsolation.withEnvValues([
            ...
            #expect(AforaConfigFile.stateDirURL().path == dir)
            #expect(AforaConfigFile.url().path == "\(dir)/afora.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("afora-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("afora.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "AFORA_STATE_DIR": stateDir.path,
            "AFORA_CONFIG_PATH": configPath.path,
        ]) {
            AforaConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-afora-config-file")
    ...
    func `save dict removes retired config metadata`() async throws {
        ...
        try await TestIsolation.withEnvValues(["AFORA_CONFIG_PATH": configPath]) {
            #expect(AforaConfigFile.saveDict([
                "gateway": ["mode": "local"],
    ...
    func `gateway start migration repairs existing retired config metadata`() async throws {
        ...
        try await TestIsolation.withEnvValues(["AFORA_CONFIG_PATH": configPath]) {
            #expect(AforaConfigFile.migrateRetiredAppMetadataForGatewayStart())
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let root = AforaConfigFile.loadDict()
            ...
            let allowedRoot = AforaConfigFile.loadDict()
    ...
    func `load dict ignores legacy config health sidecar`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try AforaConfigFile.withTestingFileLock {
                let loaded = AforaConfigFile.loadDict()
    ...
    func `load dict skips unchanged forensic fingerprints`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try AforaConfigFile.withTestingFileLock {
                let before = AforaConfigFile.testingConfigObservationCount()
                _ = AforaConfigFile.loadDict()
                let afterFirstRead = AforaConfigFile.testingConfigObservationCount()
                _ = AforaConfigFile.loadDict()
                let afterUnchangedRead = AforaConfigFile.testingConfigObservationCount()
                ...
                _ = AforaConfigFile.loadDict()
                let afterMetadataChange = AforaConfigFile.testingConfigObservationCount()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = AforaConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
