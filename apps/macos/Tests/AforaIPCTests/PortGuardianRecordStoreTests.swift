...
struct PortGuardianRecordStoreTests {
    @Test
    func `storage marker distinguishes older Afora writers from aligned copies`() {
        ...
        #expect(PortGuardian.usesLegacyPortGuardianStorage(
            ...
            storageVersion: nil))
        #expect(PortGuardian.usesLegacyPortGuardianStorage(
            bundleIdentifier: "ai.afora.mac.debug",
            storageVersion: 1))
        #expect(!PortGuardian.usesLegacyPortGuardianStorage(
            bundleIdentifier: "ai.afora.mac",
            storageVersion: 2))
    ...
    func `storage marker in app bundle matches runtime capability`() throws {
        let macOSRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let infoURL = macOSRoot.appendingPathComponent("Sources/Afora/Resources/Info.plist")
        let data = try Data(contentsOf: infoURL)
        let values = try #require(
            PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any])
        let version = try #require(values["AforaPortGuardianStorageVersion"] as? NSNumber)
    ...
    private static func fixture() throws -> Fixture {
        ...
        return Fixture(
            root: root,
            databaseURL: root.appendingPathComponent("state/afora.sqlite"),
