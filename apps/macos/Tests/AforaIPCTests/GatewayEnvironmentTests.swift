...
struct GatewayEnvironmentTests {
    @Test func `semver parses common forms`() {
        ...
        // Product-prefixed output from `afora --version` should NOT parse as semver
        // (the prefix must be stripped by the caller, not the parser).
        #expect(Semver.parse("Afora 2026.3.23-1") == nil)
    }

    @Test func `gateway version output strips product prefix before parsing`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("  Afora 2026.3.23-1 \n")
    ...
    @Test func `gateway version output strips trailing commit hash`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("Afora 2026.4.2 (d74a122)")
        ...
        let normalized2 = GatewayEnvironment.normalizeGatewayVersionOutput("Afora 2026.4.2-1 (d74a122)")
    ...
    @Test func `failed global version probe falls back to local package`() async throws {
        let projectRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-gateway-environment-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `gateway version probe tolerates loaded host delay`() async throws {
        ...
        let gateway = root.appendingPathComponent("afora")
        try "#!/bin/sh\nsleep 2.1\necho Afora 2026.7.30\n"
    ...
    @Test func `gateway port defaults and respects override`() async {
        ...
        await TestIsolation.withIsolatedState(
            env: ["AFORA_CONFIG_PATH": configPath],
    ...
    @Test func `named profiles derive stable distinct gateway ports after explicit precedence`() {
        ...
        let personal = AppProfile(environment: ["AFORA_PROFILE": "personal"])
        ...
        #expect(GatewayEnvironment.resolvedGatewayPort(
            environment: ["AFORA_GATEWAY_PORT": "21001"],
            configPort: 22001,
            storedPort: 23001,
            profile: work) == 21001)
        #expect(GatewayEnvironment.resolvedGatewayPort(
