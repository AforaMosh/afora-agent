...
struct ExecApprovalPolicySnapshotTests {
    ...
    func `portable snapshot round trips canonical policy semantics`() {
        ...
        #expect(portable.allowlistRules == [
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/usr/bin/tool"),
            AforaSystemRunApprovalPolicySnapshot.Rule(
                pattern: "/usr/bin/tool",
                argPattern: "^ok$",
                source: .allowAlways),
        ])
    ...
    func `portable snapshot decode deduplicates and sorts rules by utf8 bytes`() throws {
        ...
        let portable = try JSONDecoder().decode(
            AforaSystemRunApprovalPolicySnapshot.self,
            from: data)

        #expect(portable.allowlistRules == [
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/"),
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/", argPattern: "A"),
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/", argPattern: "é"),
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/A"),
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/A", source: .allowAlways),
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/e\u{0301}"),
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/ä"),
            AforaSystemRunApprovalPolicySnapshot.Rule(pattern: "/é"),
        ])
