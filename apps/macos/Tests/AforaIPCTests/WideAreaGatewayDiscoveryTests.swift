import Foundation
import Testing
@testable import AforaDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() async {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._afora-gw._tcp.afora.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.afora.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Afora)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/afora/src/entry.ts\""
                }
                return ""
            })
        ...
        let beacons = await TestIsolation.withEnvValues(
            ["AFORA_WIDE_AREA_DOMAIN": "afora.internal"])
        {
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Afora)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/afora/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._afora-gw._tcp.afora.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
