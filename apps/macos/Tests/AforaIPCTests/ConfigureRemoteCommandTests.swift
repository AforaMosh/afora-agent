...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("afora-configure-remote-\(UUID().uuidString).json")
        ...
        try await TestIsolation.withIsolatedState(env: ["AFORA_CONFIG_PATH": configURL.path]) {
            let output = try configureRemote(
                .init(
                    ...
                    identity: nil,
                    projectRoot: nil,
                    cliPath: "/opt/homebrew/bin/afora"),
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "afora.connectionMode") == "remote")
                #expect(defaults.string(forKey: "afora.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "afora.onboardingSeen") == true)
                #expect(defaults.string(forKey: "afora.remoteCliPath") == "/opt/homebrew/bin/afora")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("afora-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote defaults SSH host key policy to strict`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("afora-configure-remote-strict-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("afora-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote writes state dir config when config path is unset`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("afora-configure-state-\(UUID().uuidString)", isDirectory: true)
        let configURL = stateDir.appendingPathComponent("afora.json")
        ...
        try await TestIsolation.withIsolatedState(env: [
            ...
            try #require(resolveAforaConfigURL().path == configURL.path)
            let output = try configureRemote(
                .init(
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("afora-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["AFORA_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
...
struct GatewayConfigTests {
    @Test @MainActor func `config path wins when both config and state dir are set`() async throws {
        let rootDir = FileManager().temporaryDirectory
            .appendingPathComponent("afora-cli-config-precedence-\(UUID().uuidString)", isDirectory: true)
        let explicitConfigURL = rootDir.appendingPathComponent("explicit.json")
        let stateConfigURL = rootDir.appendingPathComponent("state/afora.json")
        ...
        await TestIsolation.withEnvValues([
            "AFORA_CONFIG_PATH": explicitConfigURL.path,
            "AFORA_STATE_DIR": stateConfigURL.deletingLastPathComponent().path,
        ]) {
    ...
    @Test @MainActor func `config path trims surrounding whitespace and expands tilde`() async {
        let relativePath = ".afora-cli-config-\(UUID().uuidString)/afora.json"
        let expectedURL = FileManager().homeDirectoryForCurrentUser.appendingPathComponent(relativePath)

        await TestIsolation.withEnvValues([
            "AFORA_CONFIG_PATH": "  ~/\(relativePath)\n",
            "AFORA_STATE_DIR": "/tmp/afora-unused-state-dir",
        ]) {
            #expect(resolveAforaConfigURL().path == expectedURL.path)
        }
    }

    @Test @MainActor func `state dir trims surrounding whitespace for connect and wizard`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("afora cli state profile \(UUID().uuidString)", isDirectory: true)
        let configURL = stateDir.appendingPathComponent("afora.json")
        defer { try? FileManager().removeItem(at: stateDir) }
