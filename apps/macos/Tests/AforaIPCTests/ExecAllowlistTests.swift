...
struct ExecAllowlistTests {
    ...
    @Test func `match normalizes macOS private var alias`() {
        let entry = ExecAllowlistEntry(pattern: "/var/tmp/afora-tool")
        let resolution = ExecCommandResolution(
            rawExecutable: "/var/tmp/afora-tool",
            resolvedPath: "/var/tmp/afora-tool",
            resolvedRealPath: "/private/var/tmp/afora-tool",
    ...
    @Test func `direct PATH command binds to resolved executable`() throws {
        let root = FileManager().temporaryDirectory
            .appendingPathComponent("afora-bind-path-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `symlink allowlist match and execution stay bound to canonical target`() throws {
        let root = FileManager().temporaryDirectory
            .appendingPathComponent("afora-bind-symlink-\(UUID().uuidString)", isDirectory: true)
            .resolvingSymlinksInPath()
        defer { try? FileManager().removeItem(at: root) }
        try FileManager().createDirectory(at: root, withIntermediateDirectories: true)
    ...
    @Test func `resolve for allowlist splits shell chains`() {
        let command = ["/bin/sh", "-c", "echo allowlisted && /usr/bin/touch /tmp/afora-allowlist-test"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "echo allowlisted && /usr/bin/touch /tmp/afora-allowlist-test",
            cwd: nil,
            env: ["PATH": "/usr/bin:/bin"])
    ...
    @Test func `resolve for allowlist uses wrapper argv payload even with canonical raw command`() {
        let command = ["/bin/sh", "-c", "echo allowlisted && /usr/bin/touch /tmp/afora-allowlist-test"]
        let canonicalRaw = "/bin/sh -c \"echo allowlisted && /usr/bin/touch /tmp/afora-allowlist-test\""
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
    ...
    @Test func `resolve for allowlist fails closed on command substitution`() {
        let command = ["/bin/sh", "-c", "echo $(/usr/bin/touch /tmp/afora-allowlist-test-subst)"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "echo $(/usr/bin/touch /tmp/afora-allowlist-test-subst)",
            cwd: nil,
            env: ["PATH": "/usr/bin:/bin"])
        #expect(resolutions.isEmpty)
    }
    ...
    @Test func `resolve for allowlist fails closed on quoted command substitution`() {
        let command = ["/bin/sh", "-c", "echo \"ok $(/usr/bin/touch /tmp/afora-allowlist-test-quoted-subst)\""]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "echo \"ok $(/usr/bin/touch /tmp/afora-allowlist-test-quoted-subst)\"",
            cwd: nil,
            env: ["PATH": "/usr/bin:/bin"])
        #expect(resolutions.isEmpty)
    }
    ...
    @Test func `resolve for allowlist fails closed on line-continued command substitution`() {
        let command = ["/bin/sh", "-c", "echo $\\\n(/usr/bin/touch /tmp/afora-allowlist-test-line-cont-subst)"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "echo $\\\n(/usr/bin/touch /tmp/afora-allowlist-test-line-cont-subst)",
            cwd: nil,
    ...
    @Test func `resolve for allowlist fails closed on chained line-continued command substitution`() {
        let command = [
            "/bin/sh",
            "-c",
            "echo ok && $\\\n(/usr/bin/touch /tmp/afora-allowlist-test-chained-line-cont-subst)",
        ]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "echo ok && $\\\n(/usr/bin/touch /tmp/afora-allowlist-test-chained-line-cont-subst)",
    ...
    @Test func `resolve for allowlist unwraps env shell wrapper chains`() {
        let command = [
            ...
            "/bin/sh",
            "-c",
            "echo allowlisted && /usr/bin/touch /tmp/afora-allowlist-test",
        ]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: nil,
    ...
    @Test func `resolve for allowlist unwraps env dispatch wrappers inside shell segments`() {
        let command = ["/bin/sh", "-c", "env /usr/bin/touch /tmp/afora-allowlist-test"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "env /usr/bin/touch /tmp/afora-allowlist-test",
            cwd: nil,
            env: ["PATH": "/usr/bin:/bin"])
        #expect(resolutions.count == 1)
    ...
    @Test func `resolve for allowlist preserves env assignments inside shell segments`() {
        let command = ["/bin/sh", "-c", "env FOO=bar /usr/bin/touch /tmp/afora-allowlist-test"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "env FOO=bar /usr/bin/touch /tmp/afora-allowlist-test",
            cwd: nil,
            env: ["PATH": "/usr/bin:/bin"])
        #expect(resolutions.count == 1)
