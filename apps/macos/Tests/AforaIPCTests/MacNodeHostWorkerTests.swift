...
struct MacNodeHostWorkerTests {
    ...
    @Test(arguments: [
        AforaSystemCommand.run.rawValue,
        "mcp.tools.call.v1",
    ...
    @Test(arguments: [MacNodeScreenCommand.snapshot.rawValue, AforaComputerCommand.act.rawValue])
    ...
    @Test(arguments: [AforaCanvasCommand.present.rawValue, "canvas.plugin.render"])
    ...
    @Test func `provider selection filters command ownership and publishes each provider descriptor`() throws {
        let descriptor = AforaProtocol.AnyCodable([
            "contractVersion": AforaProtocol.AnyCodable(2),
        ])
        let manifest = MacNodeHostManifest(
            version: "test",
            caps: ["screen", "computer"],
            commands: [MacNodeScreenCommand.snapshot.rawValue, AforaComputerCommand.act.rawValue],
        ...
        #expect(!peekaboo.commands.contains(AforaComputerCommand.act.rawValue))
    ...
    @Test func `worker forces app exec host without fallback`() async throws {
        ...
        let script = """
        test "$AFORA_NODE_EXEC_HOST" = app || exit 42
        test "$AFORA_NODE_EXEC_FALLBACK" = 0 || exit 43
    ...
    @Test func `worker strips inherited CUA values and receives only the app-provided endpoint`() async throws {
        let endpoint = CuaDriverWorkerEndpoint(
            ...
            binaryPath: "/Applications/Afora.app/Contents/Resources/cua-driver")
        ...
        let script = """
        test "$AFORA_CUA_DRIVER_ENDPOINT" = "$1" || exit 41
        test "$(env | grep -Ec '^(AFORA_)?CUA_DRIVER_')" = 1 || exit 42
    ...
    @Test func `unbound worker strips every inherited CUA endpoint value`() async throws {
        ...
        let script = """
        test "$(env | grep -Ec '^(AFORA_)?CUA_DRIVER_')" = 0 || exit 41
        printf '%s\\n' '{"type":"ready","version":"test","manifest":{"caps":[],"commands":[],"pathEnv":"/usr/bin:/bin"},"inventory":{"skills":null,"pluginTools":[]}}'
        while IFS= read -r line; do :; done
        """
    ...
    @Test func `worker cancellation settles when the child suppresses its result`() async throws {
        ...
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-worker-cancel-\(UUID().uuidString)")
    ...
    @Test func `worker deinit terminates its owned process`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-worker-deinit-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `stop cancels a changed launch during worker cleanup`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-worker-restart-stop-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    @Test func `stop waits for the worker leader and reaps its descendants`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-worker-stop-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
