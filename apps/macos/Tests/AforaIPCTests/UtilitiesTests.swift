...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func `gateway entrypoint prefers dist over bin`() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/afora.js")
    ...
    @Test func `log locator picks newest log file`() async throws {
        ...
        let dir = fm.temporaryDirectory
            .appendingPathComponent("afora-log-locator-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: dir) }

        try await TestIsolation.withEnvValues(["AFORA_LOG_DIR": dir.path]) {
            try fm.createDirectory(at: dir, withIntermediateDirectories: true)
            let older = dir.appendingPathComponent("afora-old-\(UUID().uuidString).log")
            let newer = dir.appendingPathComponent("afora-new-\(UUID().uuidString).log")
