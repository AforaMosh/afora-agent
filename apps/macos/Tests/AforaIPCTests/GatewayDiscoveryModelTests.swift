...
struct GatewayDiscoveryModelTests {
    ...
    @Test func `local gateway matches display name`() {
        ...
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Peter's Mac Studio (Afora)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func `service name does not false positive on substring host token`() {
        ...
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            ...
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (Afora)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipete (Afora)",
            local: local))
    }
    ...
    @Test func `parses gateway TXT fields`() {
        ...
        #expect(parsed.cliPath == "/opt/afora")
    }
    ...
    @Test func `dedupe key prefers resolved endpoint across sources`() {
        let wideArea = GatewayDiscoveryModel.DiscoveredGateway(
            ...
            gatewayPort: 443,
            cliPath: nil,
            stableID: "wide-area|afora.internal.|gateway-host",
            debugID: "wide-area",
            isLocal: false)
