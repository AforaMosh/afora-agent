...
private func makeGatewayGenerationSnapshot(version: String) -> HelloOk {
    HelloOk(
        type: "hello-ok",
        _protocol: 3,
        server: ["version": AforaProtocol.AnyCodable(version)],
        features: [:],
        snapshot: Snapshot(
            presence: [],
            health: [String: AforaProtocol.AnyCodable](),
...
@Suite(.serialized) struct GatewayConnectionControlTests {
    @Test func `operator widget capability refresh is shared and retained`() async throws {
        let rawOldSurface = "http://127.0.0.1:18789/__afora__/cap/old-token"
        let rawNewSurface = "http://127.0.0.1:18789/__afora__/cap/new-token"
        let oldSurface = "https://gateway.example.invalid:9443/__afora__/cap/old-token"
        let newSurface = "https://gateway.example.invalid:9443/__afora__/cap/new-token"
        let recorder = WebSocketMessageRecorder()
        let session = GatewayTestWebSocketSession(taskFactory: {
    ...
    @Test func `routing identity decodes agent and contract from one response`() throws {
        ...
        let identity = try AforaChatGatewayPayloadCodec.decodeSessionRoutingIdentity(data)
