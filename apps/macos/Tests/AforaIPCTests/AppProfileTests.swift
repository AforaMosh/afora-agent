...
struct AppProfileTests {
    @Test func `default profile preserves historical identities`() {
        ...
        for raw in [nil, "", " default ", "Default"] {
            let profile = AppProfile(environment: raw.map { ["AFORA_PROFILE": $0] } ?? [:])
            #expect(profile.name == nil)
            #expect(profile.validationError == nil)
            #expect(profile.gatewayLaunchAgentLabel == "ai.afora.gateway")
            #expect(profile.defaultsSuiteName == nil)
            #expect(profile.keychainService(base: "ai.afora.test") == "ai.afora.test")
            #expect(GatewayTLSStore.resolvedKeychainService(suffix: profile.keychainServiceSuffix) ==
                "ai.afora.tls-pinning")
        ...
        #if DEBUG
        #expect(MacGatewayProfileStore.service == "ai.afora.gateway-profiles.debug")
        #expect(GatewayActivationBindingKeyStore.service == "ai.afora.onboarding-route-binding.debug")
        #else
        #expect(MacGatewayProfileStore.service == "ai.afora.gateway-profiles")
        #expect(GatewayActivationBindingKeyStore.service == "ai.afora.onboarding-route-binding")
        #endif
    }

    @Test func `named profile owns every persistent namespace`() {
        let profile = AppProfile(environment: ["AFORA_PROFILE": "work_2"])
        #expect(profile.name == "work_2")
        #expect(profile.validationError == nil)
        #expect(profile.gatewayLaunchAgentLabel == "ai.afora.work_2")
        #expect(profile.defaultsSuiteName == "ai.afora.mac.profile.work_2")
        #expect(profile.keychainService(base: "ai.afora.test") == "ai.afora.test.profile.work_2")
        #expect(profile.stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")).path ==
            "/Users/test/.afora-work_2")
        #expect(profile.stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")) !=
            AppProfile(environment: ["AFORA_PROFILE": "personal"])
            .stateDirectoryURL(homeDirectory: URL(fileURLWithPath: "/Users/test")))
        #expect(profile.cliRootArguments == ["--profile", "work_2"])
        #expect(GatewayTLSStore.resolvedKeychainService(suffix: profile.keychainServiceSuffix) ==
            "ai.afora.tls-pinning.profile.work_2")
        ...
        #expect(CommandResolver.nodeHostWorkerCommand(
            prefix: ["/opt/afora"],
            profile: profile) == [
            "/opt/afora", "--profile", "work_2", "node", "worker",
        ])
    }
    ...
    @Test func `default worker commands preserve exact argument shapes`() {
        ...
        #expect(CommandResolver.nodeHostWorkerCommand(
            prefix: ["/opt/afora"],
            profile: profile) == ["/opt/afora", "node", "worker"])
    }
    ...
    @Test func `invalid and colliding profile names fail closed`() {
        ...
        for raw in invalid {
            let profile = AppProfile(environment: ["AFORA_PROFILE": raw])
            #expect(profile.name == nil)
            #expect(profile.validationError != nil)
        }
        #expect(AppProfile(environment: ["AFORA_PROFILE": String(repeating: "a", count: 64)]).name != nil)
    }
    ...
    @Test func `colliding profile apps cannot reserve the same exact port`() throws {
        ...
        let first = ProfileGatewayPortReservation.acquire(
            profile: AppProfile(environment: ["AFORA_PROFILE": "p1402"]),
            port: 55636,
            homeDirectory: home,
            temporaryDirectory: root)
        let second = ProfileGatewayPortReservation.acquire(
            profile: AppProfile(environment: ["AFORA_PROFILE": "p2380"]),
            port: 55636,
            homeDirectory: home,
            temporaryDirectory: root)
    ...
    @Test func `persisted other profile gateway reserves its exact port`() throws {
        ...
        let serviceEnv = home.appendingPathComponent(".afora-p1402/service-env", isDirectory: true)
        try FileManager.default.createDirectory(at: serviceEnv, withIntermediateDirectories: true)
        let wrapper = serviceEnv.appendingPathComponent("ai.afora.p1402-env-wrapper.sh")
        let environment = serviceEnv.appendingPathComponent("ai.afora.p1402.env")
        try Data("#!/bin/sh\n".utf8).write(to: wrapper)
        try Data("""
        export AFORA_SERVICE_MARKER='afora'
        export AFORA_SERVICE_KIND='gateway'
        ...
        let data = try PropertyListSerialization.data(
            fromPropertyList: [
                "ProgramArguments": [
                    ...
                    "node", "afora.mjs", "gateway", "--port", "55636",
                ],
            ],
            format: .xml,
            options: 0)
    ...
    @Test func `profile launch at login reason outranks bundle location`() {
        ...
        let defaultUnavailable = LaunchAtLoginPresentation.resolve(
            ...
            bundleLocationAllowsPersistentIntegration: false,
            isEnabled: false)
        #expect(defaultUnavailable.subtitle == "Move Afora to Applications before enabling launch at login.")
        ...
        let defaultAvailable = LaunchAtLoginPresentation.resolve(
            ...
            isEnabled: false)
        #expect(defaultAvailable.subtitle == "Automatically start Afora after you sign in.")
    ...
    @MainActor @Test func `profile lock excludes only the same profile`() throws {
        ...
        let workProfile = AppProfile(environment: ["AFORA_PROFILE": "work"])
        ...
        #expect(workProfile.instanceLockURL(systemTemporaryDirectory: root) ==
            AppProfile(environment: [
                "AFORA_PROFILE": "work",
                "AFORA_STATE_DIR": "/different/state",
    ...
    @Test func `profile lock reports unsafe ownership errors`() throws {
        ...
        let unsafeURL = AppProfile(environment: ["AFORA_PROFILE": "unsafe"])
            .instanceLockURL(systemTemporaryDirectory: root)
    ...
    @Test func `long profile approvals socket uses deterministic private short path`() {
        ...
        #expect(first == second)
        #expect(first.hasPrefix("/tmp/afora-\(geteuid())/exec-approvals-"))
