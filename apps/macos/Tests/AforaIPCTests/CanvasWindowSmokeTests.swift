...
struct CanvasWindowSmokeTests {
    @Test func `panel controller shows and hides`() async throws {
        let root = FileManager().temporaryDirectory
            .appendingPathComponent("afora-canvas-test-\(UUID().uuidString)")
    ...
    @Test func `A2UI auto navigation is idempotent for current host target`() throws {
        ...
        let oldTarget = "http://127.0.0.1:18789/__afora__/a2ui/?platform=macos"
        let currentTarget = "http://127.0.0.1:18790/__afora__/a2ui/?platform=macos"
        let userTarget = "https://github.com/AforaMosh/afora-agent"
    ...
    @Test func `hosted Canvas URL resolver keeps capability scope and only trusts A2UI`() throws {
        let surface = "https://gateway.example/root/__afora__/cap/token%20value"
        let canvas = try #require(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__afora__/canvas/demo%20page.html?mode=proof#result"))
        #expect(canvas.url.absoluteString ==
            "https://gateway.example/root/__afora__/cap/token%20value/__afora__/canvas/demo%20page.html?mode=proof#result")
        #expect(canvas.allowsA2UIActions == false)

        let a2ui = try #require(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__afora__/a2ui/?platform=macos"))
        #expect(a2ui.url.absoluteString ==
            "https://gateway.example/root/__afora__/cap/token%20value/__afora__/a2ui/?platform=macos")
        ...
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__afora__/a2ui/../canvas/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__afora__/a2ui/%252e%252e/canvas/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: surface,
            target: "/__afora__/a2ui/%25252525252e%25252525252e/canvas/") == nil)
        #expect(CanvasHostedURLResolver.resolve(
            surfaceURL: "https://gateway.example/not-capability-scoped",
            target: "/__afora__/canvas/") == nil)
    }

    @Test func `A2UI action trust is exact and capability scoped`() throws {
        let expected = try #require(URL(string:
            "https://gateway.example/__afora__/cap/current-token/__afora__/a2ui/?platform=macos"))
        let sameWithFragment = try #require(URL(string: expected.absoluteString + "#card"))
        let staleCapability = try #require(URL(string:
            "https://gateway.example/__afora__/cap/stale-token/__afora__/a2ui/?platform=macos"))
        let changedQuery = try #require(URL(string:
            "https://gateway.example/__afora__/cap/current-token/__afora__/a2ui/?platform=other"))
        let canvasPage = try #require(URL(string:
            "https://gateway.example/__afora__/cap/current-token/__afora__/canvas/"))
        let traversingA2UI = try #require(URL(string:
            "https://gateway.example/__afora__/cap/current-token/__afora__/a2ui/%2e%2e/canvas/"))
        let localCanvas = try #require(URL(string: "afora-canvas://main/"))
