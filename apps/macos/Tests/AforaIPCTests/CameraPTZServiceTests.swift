...
struct CameraPTZServiceTests {
    ...
    @Test func `readable axes without SET capability are not exposed or writable`() async throws {
        ...
        await #expect(throws: CameraPTZError.axisUnsupported("pan")) {
            try await service.control(AforaCameraPTZControlParams(
                deviceId: "camera-id",
                operation: .set,
                target: AforaCameraPTZAxisValues(panDegrees: 5)))
        }
    ...
    @Test func `set clamps snaps preserves omitted axes and reports adjustments`() async throws {
        let controller = self.makeController()
        let response = try await self.makeService(controller).control(AforaCameraPTZControlParams(
            deviceId: "camera-id",
            operation: .set,
            target: AforaCameraPTZAxisValues(panDegrees: 120.2, zoomPercent: 53)))
    ...
    @Test func `move reads current then writes absolute values`() async throws {
        ...
        let response = try await self.makeService(controller).control(AforaCameraPTZControlParams(
            deviceId: "camera-id",
            operation: .move,
            delta: AforaCameraPTZAxisValues(panDegrees: 2, tiltDegrees: -1, zoomPercent: 10)))
    ...
    @Test func `combined pan tilt write preserves omitted off grid raw axis`() async throws {
        ...
        _ = try await self.makeService(controller).control(AforaCameraPTZControlParams(
            deviceId: "camera-id",
            operation: .set,
            target: AforaCameraPTZAxisValues(panDegrees: 2)))
    ...
    @Test func `unsupported requested axis fails before mutation`() async {
        ...
        await #expect(throws: CameraPTZError.axisUnsupported("zoom")) {
            try await service.control(AforaCameraPTZControlParams(
                deviceId: "camera-id",
                operation: .set,
                target: AforaCameraPTZAxisValues(panDegrees: 5, zoomPercent: 20)))
        }
        #expect(controller.panTiltWrites.isEmpty)
        #expect(controller.closeCount == 1)
    }
    ...
    @Test func `later zoom failure reports applied pan tilt and best effort state`() async {
        ...
        do {
            _ = try await self.makeService(controller).control(AforaCameraPTZControlParams(
                deviceId: "camera-id",
                operation: .set,
                target: AforaCameraPTZAxisValues(panDegrees: 5, zoomPercent: 75)))
    ...
    @Test func `cancellation after preflight prevents the first hardware write`() async {
        ...
        let control = Task { () -> Result<CameraPTZControlResponse, Error> in
            do {
                return try await .success(service.control(AforaCameraPTZControlParams(
                    deviceId: "camera-id",
                    operation: .set,
                    target: AforaCameraPTZAxisValues(panDegrees: 5, zoomPercent: 50))))
            } catch {
