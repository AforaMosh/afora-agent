...
struct ApplicationRelocatorTests {
    ...
    func `stable application locations continue normally`() {
        let paths = [
            ...
            "/Users/tester/Applications/Afora.app",
            "/Users/tester/Tools/Afora.app",
            "/Volumes/External/Apps/Afora.app",
        ]
        for path in paths {
    ...
    func `debug and test builds never relocate`() {
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Afora.app",
                debugOrTesting: true
            )
        )
    ...
    func `transient copy offers system installation when available`() {
        let destination = URL(fileURLWithPath: "/Applications/Afora.app")
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Afora.app",
    ...
    func `read only mounted copy offers installation`() {
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Volumes/AforaMosh/afora-agent.app",
                candidates: [missing(destination, writable: true)],
                readOnlyVolume: true
            )
        )
    ...
    func `translocated copy offers installation`() {
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/private/var/folders/x/AppTranslocation/y/d/Afora.app",
                candidates: [missing(destination, writable: true)]
            )
        )
    ...
    func `different same named app is never replaced`() {
        let systemDestination = URL(fileURLWithPath: "/Applications/Afora.app")
        let userDestination = home.appendingPathComponent("Applications/Afora.app")
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Desktop/Afora.app",
                candidates: [
    ...
    func `unwritable destinations require manual installation`() {
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Afora.app",
                candidates: [
                    missing(URL(fileURLWithPath: "/Applications/Afora.app"), writable: false),
                    missing(home.appendingPathComponent("Applications/Afora.app"), writable: false),
                ]
            )
        )
    ...
    func `unauthenticated replacement marker cannot bypass duplicate launch rejection`() {
        let forgedHandoff = [
            "AFORA_REPLACEMENT_SOURCE_BUNDLE": "/Applications/Afora.app",
            "AFORA_REPLACEMENT_PARENT_PID": "1",
            "AFORA_REPLACEMENT_CODE_HASH": "ZmFrZQ==",
            "AFORA_REPLACEMENT_READY_FD": "19",
        ]
        ...
        #expect(!ApplicationRelocator.acceptReplacementHandoff(environment: [
            "AFORA_REPLACEMENT_SOURCE_BUNDLE": "relative/Afora.app",
        ]))
    }
    ...
    func `replacement requirement preserves Developer ID certificate class`() {
        ...
        #expect(ApplicationRelocator.developerIDRequirementString(
            bundleIdentifier: "ai.afora.mac\" or true",
            teamIdentifier: "Y5PE65HELJ"
        ) == nil)
    }
    ...
    func `bundle file reference stays bound after canonical path replacement`() throws {
        ...
        let canonicalBundle = root.appendingPathComponent("Afora.app", isDirectory: true)
        let archivedBundle = root.appendingPathComponent("Afora.previous.app", isDirectory: true)
        let executableRelativePath = "Contents/MacOS/Afora"
    ...
    func `replacement handoff launches the bound executable without filesystem locks`() {
        let reference = ApplicationRelocator.BundleFileReference(
            deviceIdentifier: 42,
            fileIdentifier: 99,
            executableRelativePath: "Contents/MacOS/Afora"
        )

        #expect(reference.bundleURL.path == "/.vol/42/99")
        #expect(reference.executableURL.path == "/.vol/42/99/Contents/MacOS/Afora")
    }
    ...
    func `verified KeepAlive launchd supervisor retains relaunch ownership`() throws {
        ...
        let executable = root.appendingPathComponent("Afora.app/Contents/MacOS/Afora")
        let serviceName = "ai.afora.mac.test-node"
        ...
        #expect(ApplicationRelocator.relaunchStrategy(
            xpcServiceName: "application.ai.afora.mac.123",
            executableURL: executable,
            homeDirectory: home
        ) == .openAfterTermination)
        #expect(ApplicationRelocator.relaunchStrategy(
    ...
    func `replacement metadata is read from disk instead of Bundle cache`() throws {
        ...
        let bundleURL = root.appendingPathComponent("Afora.app")
        ...
        #expect(ApplicationRelocator.applicationOnDisk(at: bundleURL) == .init(
            bundleIdentifier: "ai.afora.mac",
            executableURL: contentsURL.appendingPathComponent("MacOS/Afora")
        ))

        try writeInfoPlist(
            at: contentsURL.appendingPathComponent("Info.plist"),
            bundleIdentifier: "ai.afora.mac.replaced",
            executable: "AforaNext"
        )
        #expect(ApplicationRelocator.applicationOnDisk(at: bundleURL) == .init(
            bundleIdentifier: "ai.afora.mac.replaced",
            executableURL: contentsURL.appendingPathComponent("MacOS/AforaNext")
        ))
    }
