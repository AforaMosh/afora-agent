...
struct PortGuardianIsExpectedTests {
    @Test func `local mode preserves launchd node dist gateway command`() {
        let fullCommand = """
        /opt/homebrew/bin/node /opt/homebrew/lib/node_modules/afora/dist/index.js gateway --port 18789 --bind loopback
        """
    ...
    @Test func `local mode preserves git checkout node dist gateway command`() {
        let fullCommand = """
        /usr/local/bin/node /Users/dev/Projects/afora/dist/index.js gateway --port 18789
        """
    ...
    @Test func `local mode rejects similarly named node project`() {
        #expect(!PortGuardian._testIsExpected(
            command: "node",
            fullCommand: "/usr/local/bin/node /tmp/afora-tools/dist/index.js gateway --port 18789",
            port: 18789,
            mode: .local))
    }
    ...
    @Test func `local mode preserves exact launchd pid from renamed checkout`() {
        let fullCommand = """
        /usr/local/bin/node /Users/dev/Projects/afora-codex-coexistence-live/dist/index.js gateway --port 18789
        """
    ...
    @Test func `local mode rejects stale launchd pid after listener replacement`() {
        #expect(!PortGuardian._testIsExpected(
            command: "node",
            fullCommand: "/tmp/afora-tools/dist/index.js gateway --port 18789",
            port: 18789,
            mode: .local,
            pid: 5252,
            managedGatewayPID: 4242))
    ...
    @Test func `local mode rejects gateway appearing after another node argument`() {
        #expect(!PortGuardian._testIsExpected(
            command: "node",
            fullCommand: "/usr/local/bin/node --inspect /tmp/afora/dist/index.js gateway --port 18789",
            port: 18789,
            mode: .local))
    }
    ...
    @Test func `local mode rejects node dist entrypoint without gateway subcommand`() {
        #expect(!PortGuardian._testIsExpected(
            command: "node",
            fullCommand: "/opt/homebrew/bin/node /opt/homebrew/lib/node_modules/afora/dist/index.js doctor",
            port: 18789,
            mode: .local))
    }
}
