...
struct CLIInstallerTests {
    @Test func `managed install locations follow the app profile`() {
        ...
        let cases = [
            (AppProfile(environment: [:]), "/Users/Test User/.afora"),
            (
                AppProfile(environment: [
                    "AFORA_PROFILE": "onboardtest",
                    "AFORA_STATE_DIR": "/tmp/ignored-state",
                ]),
                "/Users/Test User/.afora-onboardtest"),
        ]
        ...
        for (profile, expectedPrefix) in cases {
            ...
            #expect(CLIInstaller.managedExecutableLocation(homeDirectory: home, profile: profile) ==
                "\(expectedPrefix)/bin/afora")

            let command = CLIInstaller.installScriptCommand(
                target: .exact("2026.7.3"),
                prefix: prefix,
                scriptPath: "/Applications/Afora.app/Contents/Resources/install-cli.sh")
    ...
    @Test func `installed location finds executable`() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "afora-cli-installer-\(UUID().uuidString)")
        defer { try? fm.removeItem(at: root) }

        let binDir = root.appendingPathComponent("bin")
        try fm.createDirectory(at: binDir, withIntermediateDirectories: true)
        let cli = binDir.appendingPathComponent("afora")
    ...
    @Test func `installer command runs the signed bundled script without a shell pipeline`() {
        let command = CLIInstaller.installScriptCommand(
            ...
            prefix: "/Users/Test User/.afora",
            scriptPath: "/Applications/Afora.app/Contents/Resources/install-cli.sh",
            compatibleWith: "2026.7.4")

        #expect(command == [
            "/bin/bash",
            "/Applications/Afora.app/Contents/Resources/install-cli.sh",
            "--json",
            "--no-onboard",
            "--prefix",
            "/Users/Test User/.afora",
            "--version",
            "2026.7.3-beta.1",
        ])
    ...
    @Test func `dev installer uses a managed git main checkout`() {
        ...
        #expect(command.suffix(5) == [
            ...
            "git",
            "--git-dir",
            "/Users/Test User/.afora/dev/afora",
        ])
    }
    ...
    @Test func `installer events map to concise live status`() {
        let cases: [(String, String)] = [
            ...
            (#"{"event":"step","name":"git-clone","status":"start"}"#, "Downloading Afora source…"),
            (#"{"event":"step","name":"git-update","status":"start"}"#, "Updating Afora source…"),
            ...
            (#"{"event":"step","name":"cli-build","status":"start"}"#, "Building Afora CLI…"),
            (#"{"event":"step","name":"afora","status":"retry"}"#, "Retrying Afora CLI install…"),
            (
                #"{"event":"step","name":"disk-space","status":"warn"}"#,
                "Couldn’t verify free disk space; continuing…"),
            (
                #"{"event":"step","name":"git-update","status":"warn"}"#,
                "Using the existing modified Afora source…"),
            (
    ...
    @Test func `managed update uses the canonical updater without accepting downgrades`() {
        let command = CLIInstaller.managedUpdateCommand(
            executable: "/Users/Test User/.afora/bin/afora",
            targetVersion: "2026.7.4")

        #expect(command == [
            "/Users/Test User/.afora/bin/afora",
            "update",
            "--tag",
            "2026.7.4",
            "--json",
        ...
        #expect(CLIInstaller.managedUpdateCommand(
            executable: "/opt/afora",
            targetVersion: "2026.7.4",
            profile: profile) == [
            "/opt/afora", "--profile", "work", "update", "--tag", "2026.7.4",
            "--json", "--timeout", "900",
        ])
        #expect(CLIInstaller.managedUpdateCommand(
            executable: "/opt/afora",
            ...
            repair: true,
            profile: profile) == [
            "/opt/afora", "--profile", "work", "update", "repair", "--json",
            "--timeout", "900", "--yes",
        ])
    }
    ...
    @Test func `validated CLI cache changes only when the ready tuple changes`() throws {
        ...
        let initialLocation = "/Users/test/.local/bin/afora"
        ...
        let updatedLocation = "/opt/homebrew/bin/afora"
        CLIInstaller.rememberValidated(
    ...
    @Test func `managed setup requires a parseable compatible version`() {
        let location = "/Users/test/.afora/bin/afora"

        #expect(CLIInstaller.classifyVersion(
            location: location,
            output: "Afora 2026.7.3\n",
    ...
    @Test func `compatible external CLI satisfies setup`() async throws {
        let root = FileManager().temporaryDirectory.appendingPathComponent(
            "afora-compatible-cli-\(UUID().uuidString)")
    ...
    @Test func `matching external CLI with unsupported Node is unusable`() async throws {
        let root = FileManager().temporaryDirectory.appendingPathComponent(
            "afora-old-node-cli-\(UUID().uuidString)")
        defer { try? FileManager().removeItem(at: root) }
        try FileManager().createDirectory(at: root, withIntermediateDirectories: true)
        let executable = root.appendingPathComponent("afora")
    ...
    @Test func `managed CLI probe prefers its private runtime`() {
        let executable = "/Users/test/.afora/bin/afora"
        let environment = CLIInstaller.probeEnvironment(
            location: executable,
            processEnvironment: [:],
            preferredPaths: ["/Users/test/.nvm/versions/node/v20/bin", "/usr/bin"],
            managedExecutable: executable,
            managedRuntimeDirectory: "/Users/test/.afora/tools/node/bin")

        #expect(environment["PATH"] == [
            "/Users/test/.afora/bin",
            "/Users/test/.afora/tools/node/bin",
