...
struct GatewayLaunchAgentManagerTests {
    @Test func `attach-only marker belongs to the selected state directory`() {
        let stateDirectory = URL(fileURLWithPath: "/tmp/afora-elevation-state", isDirectory: true)

        #expect(GatewayLaunchAgentManager.disableLaunchAgentMarkerURL(in: stateDirectory).path ==
            "/tmp/afora-elevation-state/disable-launchagent")
    }
    ...
    @Test func `gateway launchd artifacts follow default and named profile labels`() {
        ...
        let workArtifacts = GatewayLaunchAgentManager.generatedEnvironmentArtifacts(
            directory: directory,
            profile: work)
        #expect(baseArtifacts.environment.path == "/state/service-env/ai.afora.gateway.env")
        #expect(baseArtifacts.wrapper.path == "/state/service-env/ai.afora.gateway-env-wrapper.sh")
        #expect(workArtifacts.environment.path == "/state/service-env/ai.afora.work.env")
        #expect(workArtifacts.wrapper.path == "/state/service-env/ai.afora.work-env-wrapper.sh")
    }

    @Test func `gateway daemon command selects named profile at the root`() async throws {
        let root = try makeTempDirForTests()
        defer { try? FileManager.default.removeItem(at: root) }
        let executable = root.appendingPathComponent("node_modules/.bin/afora")
        try makeExecutableForTests(at: executable)
        let command = await CommandResolver.aforaCommand(
            subcommand: "gateway",
            extraArgs: ["status", "--json"],
            configRoot: ["gateway": ["mode": "local"]],
            projectRoot: root,
            profile: AppProfile(environment: ["AFORA_PROFILE": "work"]))
    ...
    @Test func `malformed canonical profile claims fail closed`() throws {
        ...
        let data = try PropertyListSerialization.data(
            fromPropertyList: [
                "ProgramArguments": ["node", "afora.mjs", "gateway"],
            ...
            format: .xml,
            options: 0)
        try data.write(to: agents.appendingPathComponent("ai.afora.p1402.plist"))

        let claim = GatewayLaunchAgentManager.conflictingProfileClaimOwner(
            port: 55636,
            excludingLabel: "ai.afora.p2380",
            homeDirectory: home)
    ...
    @Test func `same prefix ssh tunnel is not a profile Gateway claim`() throws {
        ...
        let data = try PropertyListSerialization.data(
            ...
            format: .xml,
            options: 0)
        try data.write(to: agents.appendingPathComponent("ai.afora.gateway-tunnel.plist"))
        try Data("not a plist".utf8).write(to: agents.appendingPathComponent("ai.afora.unrelated.plist"))

        #expect(GatewayLaunchAgentManager.conflictingProfileClaimOwner(
            port: 55636,
            excludingLabel: "ai.afora.qa",
    ...
    @Test func `default generated Gateway claim is recognized`() throws {
        ...
        let data = try PropertyListSerialization.data(
            fromPropertyList: [
                "ProgramArguments": ["node", "afora.mjs", "gateway", "--port", "18789"],
                "EnvironmentVariables": [
                    "AFORA_SERVICE_MARKER": "afora",
                    "AFORA_SERVICE_KIND": "gateway",
                ],
            ...
            format: .xml,
            options: 0)
        try data.write(to: agents.appendingPathComponent("ai.afora.gateway.plist"))
    ...
    @Test func `reads Gateway service ownership command directly from launchd`() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-gateway-\(UUID().uuidString).plist")
        defer { try? FileManager.default.removeItem(at: url) }
        let arguments = [
            "/Users/Test/.afora/tools/node/bin/node",
            "/Users/Test/.afora/lib/node_modules/afora/dist/index.js",
            "gateway",
        ]
        let data = try PropertyListSerialization.data(
    ...
    @Test func `attach only runtime override blocks gateway launch agent writes`() async throws {
        try await TestIsolation.withIsolatedState {
            let dir = FileManager().temporaryDirectory
                .appendingPathComponent("afora-attach-only-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `unintercepted daemon commands fail closed during tests`() async {
        await TestIsolation.withIsolatedState {
            let marker = FileManager.default.temporaryDirectory
                .appendingPathComponent("afora-no-disable-marker-\(UUID().uuidString)")
            defer {
                GatewayLaunchAgentManager.setTestingDisableLaunchAgentMarkerURL(nil)
                GatewayLaunchAgentManager.setTestingInterceptDaemonCommands(false)
            }
    ...
    @Test func `launch agent plist snapshot parses args and env`() throws {
        let url = FileManager().temporaryDirectory
            .appendingPathComponent("afora-launchd-\(UUID().uuidString).plist")
        let plist: [String: Any] = [
            "ProgramArguments": ["afora", "gateway", "--port", "18789", "--bind", "loopback"],
            "EnvironmentVariables": [
                "AFORA_GATEWAY_TOKEN": " secret ",
                "AFORA_GATEWAY_PASSWORD": "pw",
            ],
        ]
    ...
    @Test func `launch agent plist snapshot merges canonical generated environment`() throws {
        let directory = FileManager().temporaryDirectory
            .appendingPathComponent("afora-launchd-env-\(UUID().uuidString)", isDirectory: true)
        let plistURL = directory.appendingPathComponent("ai.afora.gateway.plist")
        let environmentFileURL = directory.appendingPathComponent("ai.afora.gateway.env")
        let wrapperURL = directory.appendingPathComponent("ai.afora.gateway-env-wrapper.sh")
        ...
        export AFORA_GATEWAY_PASSWORD='service'\\''pass'
        export AFORA_GATEWAY_TOKEN=' service-token '
        ...
        let plist: [String: Any] = [
            ...
            "EnvironmentVariables": ["AFORA_GATEWAY_TOKEN": "stale-inline-token"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
    ...
    @Test func `launch agent plist snapshot ignores unreferenced generated environment`() throws {
        ...
        try FileManager().createDirectory(at: directory, withIntermediateDirectories: true)
        try "#!/bin/sh\n".write(to: wrapperURL, atomically: true, encoding: .utf8)
        try "export AFORA_GATEWAY_TOKEN='unreferenced-token'\n"
            .write(to: environmentFileURL, atomically: true, encoding: .utf8)
        let plist: [String: Any] = [
            "ProgramArguments": ["afora", "gateway"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
        try data.write(to: plistURL, options: [.atomic])
        defer { try? FileManager().removeItem(at: directory) }
