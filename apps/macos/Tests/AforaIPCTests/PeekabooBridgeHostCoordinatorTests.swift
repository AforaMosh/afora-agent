...
struct PeekabooBridgeHostCoordinatorTests {
    ...
    func `legacy aliases never replace regular files or unrelated symlinks`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-peekaboo-alias-tests-\(UUID().uuidString)", isDirectory: true)
        ...
        LegacyPeekabooSocketAliasManager(
            ...
            .ensureAliases(logger: .init(subsystem: "ai.afora.tests", category: "PeekabooBridge"))
    ...
    @Test(.enabled(if: ProcessInfo.processInfo.environment["AFORA_PEEKABOO_HANDSHAKE_CLI"] != nil))
    func `embedded runtime serves an exact socket handshake`() async throws {
        let cliPath = try #require(ProcessInfo.processInfo.environment["AFORA_PEEKABOO_HANDSHAKE_CLI"])
    ...
    private static func coordinator(factory: ScriptedRuntimeFactory) -> PeekabooBridgeHostCoordinator {
        PeekabooBridgeHostCoordinator(
            ...
            aliasManager: .init(targetSocketPath: "/tmp/afora-test.sock", aliasSocketPaths: []))
    }
}
...
private actor ScriptedBridgeRuntime: PeekabooBridgeRuntimeControlling {
    private let socketPath = "/tmp/afora-scripted-peekaboo.sock"
