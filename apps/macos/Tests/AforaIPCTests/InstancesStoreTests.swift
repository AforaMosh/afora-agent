...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: AforaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AforaProtocol.AnyCodable] = [
            "presence": .init([AforaProtocol.AnyCodable(entry)]),
        ]
        let payload = AforaProtocol.AnyCodable(payloadMap)
