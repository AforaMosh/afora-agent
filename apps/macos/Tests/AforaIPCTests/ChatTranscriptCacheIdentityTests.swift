...
struct ChatTranscriptCacheIdentityTests {
    private static let defaultStateDir = URL(fileURLWithPath: "/Users/tester/.afora", isDirectory: true)
    ...
    @Test @MainActor func `windows share one outbox owner per gateway`() {
        let databaseDirectoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-cache-owner-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `local mode keys on state dir so profiles never collide`() {
        ...
        let devProfile = MacChatTranscriptCache.gatewayID(
            mode: .local,
            localStateDir: URL(fileURLWithPath: "/Users/tester/.afora-dev", isDirectory: true),
            remoteTransport: .ssh,
            directURL: nil,
            sshTarget: "",
            sshRemotePort: 18789)
        #expect(defaultProfile == "local:/Users/tester/.afora")
        #expect(devProfile == "local:/Users/tester/.afora-dev")
    ...
    @Test func `local state dir aliases resolve to one identity`() throws {
        ...
        let canonical = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-cache-identity-\(UUID().uuidString)", isDirectory: true)
