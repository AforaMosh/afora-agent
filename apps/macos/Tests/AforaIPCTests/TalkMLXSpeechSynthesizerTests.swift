...
struct TalkMLXSpeechSynthesizerTests {
    ...
    func `shutdown reaps a TERM-resistant helper before returning`() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-mlx-tts-lifecycle-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let helper = directory.appendingPathComponent("afora-mlx-tts-test-helper")
        ...
        #!/bin/sh
        trap '' TERM
        printf '%s\\n' "$$" > "$AFORA_MLX_TTS_PID_FILE"
        /bin/cat "$AFORA_MLX_TTS_READY_FILE"
        exec /bin/sleep 30
        """.utf8).write(to: helper)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: helper.path)

        try await TestIsolation.withEnvValues([
            "AFORA_MLX_TTS_BIN": helper.path,
            "AFORA_MLX_TTS_PID_FILE": pidFile.path,
            "AFORA_MLX_TTS_READY_FILE": readyFrame.path,
        ]) {
