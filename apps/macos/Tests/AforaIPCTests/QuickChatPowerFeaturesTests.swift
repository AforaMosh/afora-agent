...
struct QuickChatPowerFeaturesTests {
    private static let solModelChoice = AforaChatModelChoice(
        modelID: "gpt-5.6-sol",
        name: "Sol",
        provider: "openai",
    ...
    @Test func `paste rejects a stale assistant and the Afora process`() {
        let messages = [
            Self.message(role: "user", text: "Question", idempotencyKey: "send-1"),
    ...
    @Test func `model thinking options reject an unsupported explicit override`() {
        let options = [AforaChatThinkingLevelOption(id: "off", label: "Off")]
