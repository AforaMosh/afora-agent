...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers afora binary`() async throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()

        let aforaPath = tmp.appendingPathComponent("node_modules/.bin/afora")
        try makeExecutableForTests(at: aforaPath)

        let searchPaths = [tmp.appendingPathComponent("node_modules/.bin").path]
        let cmd = await CommandResolver.aforaCommand(
            subcommand: "gateway",
            defaults: defaults,
            configRoot: [:],
            searchPaths: searchPaths,
            projectRoot: tmp)
        #expect(cmd.prefix(2).elementsEqual([aforaPath.path, "gateway"]))
    }
    ...
    @Test func `source checkout worker uses the freshness aware runner`() async throws {
        ...
        let sourceEntrypoint = tmp.appendingPathComponent("afora.mjs")
        let distEntrypoint = tmp.appendingPathComponent("dist/entry.js")
        let projectExecutable = tmp.appendingPathComponent("node_modules/.bin/afora")
    ...
    @Test func `falls back to node and script`() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/afora.js")
    ...
    @Test func `prefers afora binary over pnpm`() async throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()

        let binDir = tmp.appendingPathComponent("bin")
        let aforaPath = binDir.appendingPathComponent("afora")
        ...
        #expect(cmd.prefix(2).elementsEqual([aforaPath.path, "rpc"]))
    }

    @Test func `uses afora binary without node runtime`() async throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func `falls back to pnpm`() async throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "afora", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "afora", "health", "--json"]))
    ...
    @Test func `missing CLI explains install and source checkout paths`() async throws {
        ...
        #expect(cmd.last?.contains("afora CLI not found") == true)
        #expect(cmd.last?.contains("Install the CLI") == true)
        #expect(cmd.last?.contains("run pnpm build in an Afora source checkout") == true)
    }
    ...
    @Test func `managed install only precedes external installs after validation`() throws {
        ...
        let managedBin = home.appendingPathComponent(".afora/bin")
        try FileManager().createDirectory(at: managedBin, withIntermediateDirectories: true)
        let managedExecutable = managedBin.appendingPathComponent("afora")
    ...
    @Test func `managed paths follow the app profile`() throws {
        ...
        let hostBase = home.appendingPathComponent(".afora")
        let profileBase = home.appendingPathComponent(".afora-onboardtest")
        for base in [hostBase, profileBase] {
            try makeExecutableForTests(at: base.appendingPathComponent("bin/afora"))
            try FileManager().createDirectory(
        ...
        let cases = [
            ...
            (AppProfile(environment: ["AFORA_PROFILE": "onboardtest"]), profileBase, hostBase),
        ]
        ...
        let namedProfile = AppProfile(environment: ["AFORA_PROFILE": "onboardtest"])
        let staleCases = [
        ...
        for (profile, staleBase) in staleCases {
            ...
            let paths = CommandResolver.preferredPaths(
                ...
                projectRoot: home,
                validatedExecutable: staleBase.appendingPathComponent("bin/afora").path,
                profile: profile)
            #expect(!paths.contains(staleBase.appendingPathComponent("bin").path))
            #expect(paths.contains("/usr/bin"))
        }

        // ~/.afora2 is a lookalike, not the managed profile namespace; validation must survive.
        for external in ["custom/bin/afora", ".afora2/bin/afora"] {
    ...
    @Test func `validated CLI preference expires when the app requires a newer version`() throws {
        ...
        let executable = root.appendingPathComponent("afora")
        ...
        #expect(CommandResolver.validatedAforaExecutable(
            defaults: defaults,
            fileManager: .default,
    ...
    @Test func `builds SSH command for remote mode`() async {
        ...
        defaults.set("afora@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/afora", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "afora@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/afora'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("afora"))
    ...
    @Test func `remote SSH with explicit blank target fails closed`() async throws {
        ...
        defaults.set("stale-gateway-alias", forKey: remoteTargetKey)
        let tmp = try makeTempDirForTests()
        let localAfora = tmp.appendingPathComponent("node_modules/.bin/afora")
        try makeExecutableForTests(at: localAfora)
    ...
    @Test func `direct remote route uses local CLI despite stale SSH defaults`() async throws {
        ...
        #expect(command == [localAfora.path, "status"])
    }
    ...
    @Test func `config root local overrides remote defaults`() async throws {
        ...
        #expect(cmd.first == aforaPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
