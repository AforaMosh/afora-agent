...
struct AppStateRemoteConfigTests {
    ...
    func `remote identity edit updates canonical SSH config before routing resumes`() async {
            ...
            defaults: [connectionModeKey: AppState.ConnectionMode.remote.rawValue])
        {
            ...
            let remote = (AforaConfigFile.loadDict()["gateway"] as? [String: Any])?["remote"]
                as? [String: Any]
    ...
    func `successful gateway config sync clears dirty fields`() async {
        let configPath = TestIsolation.tempConfigPath()
        await TestIsolation.withIsolatedState(env: ["AFORA_CONFIG_PATH": configPath]) {
            #expect(AforaConfigFile.saveDict([
                "gateway": [
                    "mode": "remote",
                    "remote": [
        ...
        await TestIsolation.withIsolatedState(env: ["AFORA_CONFIG_PATH": configPath]) {
            ...
            let state = AppState(
                ...
                gatewayConfigSaver: { root in
                    rejectSaves ? false : AforaConfigFile.saveDict(root)
                })
            ...
            state._testEnableGatewayConfigSync()
            #expect(!state.syncGatewayConfigNow())
            var persistedRemote = (AforaConfigFile.loadDict()["gateway"] as? [String: Any])?["remote"]
                as? [String: Any]
            #expect(persistedRemote?["token"] as? String == "external-token")
            #expect(state.remoteToken == "app-token")
            ...
            externalRoot = AforaConfigFile.loadDict()
            ...
            persistedRemote = (AforaConfigFile.loadDict()["gateway"] as? [String: Any])?["remote"]
                as? [String: Any]
