...
struct MacNodeRuntimeTests {
    ...
    actor CanvasRefreshProbe {
        ...
        func refresh() -> String? {
            self.calls += 1
            return "http://127.0.0.1:18789/__afora__/cap/refreshed-token"
        }
    }

    actor CanvasReconnectProbe {
        private var surfaceURL = "http://127.0.0.1:18789/__afora__/cap/old-token"
        ...
        func reconnectDuringRefresh() -> String? {
            self.surfaceURL = "http://127.0.0.1:18789/__afora__/cap/new-token"
            return nil
        }
    }
    ...
    final class MainActorServicesProbe: MacNodeRuntimeMainActorServices, @unchecked Sendable {
        typealias SnapshotInspection = (Int?, Int?, Double?, AforaScreenSnapshotFormat?) -> Void
        ...
        var receivedParams: AforaComputerActParams?
        var actError: Error?
        ...
        func snapshotScreen(
            ...
            maxWidth: Int?,
            quality: Double?,
            format: AforaScreenSnapshotFormat?) async throws -> ScreenSnapshotResult
        {
            ...
            outPath _: String?) async throws -> (path: String, hasAudio: Bool)
        {
            let url = FileManager().temporaryDirectory
                .appendingPathComponent("afora-test-screen-record-\(UUID().uuidString).mp4")
        ...
        func performComputerAct(
            _ params: AforaComputerActParams,
            lifecycleGeneration: UInt64) async throws -> AforaComputerActResult
        {
            ...
            return AforaComputerActResult(ok: true, cursorX: params.x ?? 0, cursorY: params.y ?? 0)
        }
    ...
    @Test func `A2UI host capability refresh uses injected node session refresher`() async {
        ...
        let resolver = MacNodeCanvasHostedSurfaceResolver(
            currentSurfaceURL: { "http://127.0.0.1:18789/__afora__/cap/current-token" },
            refreshSurfaceURL: { _ in await probe.refresh() })

        let current = await resolver.resolveA2UIURL()
        #expect(current ==
            "http://127.0.0.1:18789/__afora__/cap/current-token/__afora__/a2ui/?platform=macos")
        #expect(await probe.calls == 0)

        let refreshed = await resolver.resolveA2UIURL(forceRefresh: true)
        #expect(refreshed ==
            "http://127.0.0.1:18789/__afora__/cap/refreshed-token/__afora__/a2ui/?platform=macos")
    ...
    @Test func `hosted Canvas commands refresh capability and preserve target components`() async throws {
        ...
        let resolved = try await resolver.resolveTarget(
            "/__afora__/canvas/demo%20page.html?mode=proof#result")
        #expect(resolved?.url.absoluteString ==
            "http://127.0.0.1:18789/__afora__/cap/refreshed-token/__afora__/canvas/demo%20page.html?mode=proof#result")
    ...
    @Test func `hosted Canvas commands use replacement route after refresh fails`() async throws {
        ...
        let resolved = try await resolver.resolveTarget("/__afora__/canvas/demo.html")

        #expect(resolved?.url.absoluteString ==
            "http://127.0.0.1:18789/__afora__/cap/new-token/__afora__/canvas/demo.html")
    }

    @Test func `handle invoke rejects empty notification`() async throws {
        let runtime = MacNodeRuntime()
        let params = AforaSystemNotifyParams(title: "", body: "")
        let response = try await self.invoke(
            runtime, "req-3", AforaSystemCommand.notify.rawValue, params: params)
        #expect(response.ok == false)
    }
    ...
    @Test func `handle invoke camera list requires enabled camera`() async {
        await TestIsolation.withUserDefaultsValues([cameraEnabledKey: false]) {
            let runtime = MacNodeRuntime()
            let response = await self.invoke(runtime, "req-4", AforaCameraCommand.list.rawValue)
            #expect(response.ok == false)
    ...
    @Test func `handle location invoke applies authorization required by mode`() async throws {
        ...
        let cases: [(mode: AforaLocationMode, status: CLAuthorizationStatus, accepted: Bool)] = [
        ...
        for testCase in cases {
            await TestIsolation.withUserDefaultsValues([locationModeKey: testCase.mode.rawValue]) {
                ...
                let response = await self.invoke(
                    runtime, "req-location", AforaLocationCommand.get.rawValue)
    ...
    @Test func `handle invoke rejects computer act when control disabled`() async throws {
        ...
        let params = AforaComputerActParams(action: .leftClick, x: 5, y: 6, refWidth: 1280)
        let response = try await self.invoke(
            runtime, "req-computer-disabled", AforaComputerCommand.act.rawValue, params: params)
    ...
    @Test func `handle invoke routes computer act to the injected services when enabled`() async throws {
        ...
        let params = AforaComputerActParams(action: .leftClick, x: 12, y: 34, refWidth: 1280)
        let response = try await self.invoke(
            runtime, "req-computer-ok", AforaComputerCommand.act.rawValue, params: params)
        ...
        let payloadJSON = try #require(response.payloadJSON)
        let result = try JSONDecoder().decode(AforaComputerActResult.self, from: Data(payloadJSON.utf8))
    ...
    @Test func `provider selection owns both snapshot and action without cross-provider fallback`() async throws {
        let commands: Set<String> = [MacNodeScreenCommand.snapshot.rawValue, AforaComputerCommand.act.rawValue]
        ...
        let action = AforaComputerActParams(action: .leftClick, x: 12, y: 34, refWidth: 1280)
    ...
    @Test func `concurrent invokes share one main actor services initialization`() async throws {
        ...
        let first = Task {
            await self.invoke(runtime, "req-computer-single-flight-1", AforaComputerCommand.act.rawValue, json)
        }
        try #require(await self.waitForCount(1, counter: factoryCalls))
        let second = Task {
            await self.invoke(runtime, "req-computer-single-flight-2", AforaComputerCommand.act.rawValue, json)
        }
    ...
    @Test func `lifecycle release invalidates first invoke awaiting service initialization`() async throws {
        ...
        let runtime = MacNodeRuntime(
            ...
            computerControlEnabled: { true })
        let params = AforaComputerActParams(action: .leftMouseDown, x: 12, y: 34, refWidth: 1280)
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let invoke = Task {
            await self.invoke(runtime, "req-computer-release-during-init", AforaComputerCommand.act.rawValue, json)
        }
        try #require(await self.waitForCount(1, counter: factoryCalls))
    ...
    @Test func `handle invoke maps accessibility denial to unavailable`() async throws {
        ...
        let params = AforaComputerActParams(action: .leftClick, x: 1, y: 1, refWidth: 1280)
        let response = try await self.invoke(
            runtime, "req-computer-ax", AforaComputerCommand.act.rawValue, params: params)

        #expect(response.ok == false)
        #expect(response.error?.code == .unavailable)
        #expect(response.error?.message == "ACCESSIBILITY_REQUIRED: grant Accessibility permission to Afora")
    }
    ...
    @Test func `handle invoke rejects malformed computer act params`() async {
        ...
        let response = await self.invoke(
            runtime, "req-computer-bad", AforaComputerCommand.act.rawValue, #"{"action":"#)
