...
struct MacNodeModeCoordinatorTests {
    ...
    @Test @MainActor func `terminal worker failure is reported instead of scheduling another restart`() async throws {
        ...
        try coordinator.prepareNodeHostWorkerRetryForTesting(
            command: ["/usr/local/bin/afora", "node", "worker"])
        await confirmation("terminal worker failure") { confirmed in
            let observer = notificationCenter.addObserver(
                forName: .aforaNodeHostWorkerRetryExhausted,
                object: coordinator,
                queue: nil)
            { notification in
    ...
    @Test @MainActor func `worker cannot restart before its crash backoff expires`() async throws {
        ...
        let command = ["/usr/local/bin/afora", "node", "worker"]
    ...
    @Test func `native manifest excludes CLI-owned node commands`() {
        ...
        #expect(!caps.contains(AforaCapability.browser.rawValue))
        #expect(!commands.contains(AforaBrowserCommand.proxy.rawValue))
        #expect(commands.contains(AforaCanvasCommand.present.rawValue))
        #expect(commands.contains(AforaSystemCommand.notify.rawValue))
        #expect(!commands.contains(AforaFileSystemCommand.listDir.rawValue))
    ...
    @Test func `Codex supervision activation respects the plugin flag and global policy`() {
        ...
        #expect(AforaConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
        ...
        #expect(AforaConfigFile.configuredBundledPluginAllowed(
            "codex",
        ...
        #expect(AforaConfigFile.pluginEntry("codex", root: ambiguousEntryAliases) == nil)
        #expect(!AforaConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
            path: ["supervision", "enabled"],
    ...
    @Test func `computer control cap gates the computer.act command`() {
        ...
        #expect(enabledCaps.contains(AforaCapability.computer.rawValue))
        #expect(enabledCommands.contains(AforaComputerCommand.act.rawValue))
        ...
        #expect(!disabledCaps.contains(AforaCapability.computer.rawValue))
        #expect(!disabledCommands.contains(AforaComputerCommand.act.rawValue))
    }
    ...
    @Test func `camera cap gates capture and PTZ commands`() {
        let enabledCaps = MacNodeModeCoordinator.resolvedCaps(
            ...
            connectionMode: .local)
        let enabledCommands = MacNodeModeCoordinator.resolvedCommands(caps: enabledCaps)
        #expect(enabledCommands.contains(AforaCameraCommand.list.rawValue))
        #expect(enabledCommands.contains(AforaCameraCommand.ptzStatus.rawValue))
        #expect(enabledCommands.contains(AforaCameraCommand.ptzControl.rawValue))
        ...
        let disabledCaps = MacNodeModeCoordinator.resolvedCaps(
            ...
            connectionMode: .local)
        let disabledCommands = MacNodeModeCoordinator.resolvedCommands(caps: disabledCaps)
        #expect(!disabledCommands.contains(AforaCameraCommand.ptzStatus.rawValue))
        #expect(!disabledCommands.contains(AforaCameraCommand.ptzControl.rawValue))
    }
