...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: AforaChatTransport {
        func requestHistory(sessionKey: String) async throws -> AforaChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(AforaChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [AforaChatAttachmentPayload]) async throws -> AforaChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(AforaChatSendResponse.self, from: Data(json.utf8))
        }
    ...
    @Test func `window controller merges titlebar and keeps toolbar controls`() throws {
        ...
        #expect(window.title == "Studio — Afora")
        window.title = "main"
        #expect(window.title == "Studio — Afora")
    ...
    @Test func `controller explicit agent wins and nil falls back to cached default`() throws {
        let cachedIdentity = try #require(AforaChatSessionRoutingIdentity(
            scope: "global",
            mainSessionKey: "main",
            defaultAgentID: "main"))
    ...
    @Test func `max and Ultra thinking preferences survive reopen`() throws {
        ...
        for level in ["max", "ultra"] {
            defaults.set(level, forKey: "afora.webchat.thinkingLevel")
    ...
    @Test func `verbosity preference survives reopen`() throws {
        ...
        defaults.set("full", forKey: "afora.webchat.verboseLevel")
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == "full")
        defaults.set("invalid", forKey: "afora.webchat.verboseLevel")
    ...
    @Test func `inherited verbosity preference clears persisted override`() throws {
        ...
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == nil)
        #expect(defaults.object(forKey: "afora.webchat.verboseLevel") == nil)
    }
}
