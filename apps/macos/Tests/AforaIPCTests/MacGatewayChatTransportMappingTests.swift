...
struct MacGatewayChatTransportMappingTests {
    ...
    @Test func `legacy trace preference migrates to independent defaults once`() throws {
        ...
        defaults.set(false, forKey: AforaChatWindowShell.assistantTraceDefaultsKey)

        #expect(WebChatTracePreferences.displayOptions(defaults: defaults).isEmpty)
        #expect(defaults.object(forKey: AforaChatWindowShell.assistantReasoningDefaultsKey) as? Bool == false)
        #expect(defaults.object(forKey: AforaChatWindowShell.assistantToolActivityDefaultsKey) as? Bool == false)

        defaults.set(true, forKey: AforaChatWindowShell.assistantReasoningDefaultsKey)
    ...
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: ["ok": AforaProtocol.AnyCodable(false)],
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AforaProtocol.AnyCodable(["ok": AforaProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `sessions changed event maps to authoritative refresh signal`() {
        let payload = AforaProtocol.AnyCodable([
            "sessionKey": AforaProtocol.AnyCodable("agent:main:main"),
            "agentId": AforaProtocol.AnyCodable("main"),
            "reason": AforaProtocol.AnyCodable("command-metadata"),
        ])
        let frame = EventFrame(
            type: "event",
    ...
    @Test func `chat event maps to chat`() {
        let payload = AforaProtocol.AnyCodable([
            "runId": AforaProtocol.AnyCodable("run-1"),
            "sessionKey": AforaProtocol.AnyCodable("main"),
            "state": AforaProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `session message event maps to session message`() {
        let payload = AforaProtocol.AnyCodable([
            "sessionKey": AforaProtocol.AnyCodable("agent:main:main"),
            "messageId": AforaProtocol.AnyCodable("msg-1"),
            "messageSeq": AforaProtocol.AnyCodable(7),
            "message": AforaProtocol.AnyCodable([
                "role": AforaProtocol.AnyCodable("user"),
                "content": AforaProtocol.AnyCodable([
                    AforaProtocol.AnyCodable([
                        "type": AforaProtocol.AnyCodable("text"),
                        "text": AforaProtocol.AnyCodable("spoken transcript"),
                    ]),
                ]),
                "timestamp": AforaProtocol.AnyCodable(1234.5),
            ]),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AforaProtocol.AnyCodable(["a": AforaProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
