...
struct GatewayProcessManagerTests {
    @Test func `colliding profile ports cannot attach another profile gateway`() {
        let first = AppProfile(environment: ["AFORA_PROFILE": "p1402"])
        let second = AppProfile(environment: ["AFORA_PROFILE": "p2380"])
        ...
        _ body: () async throws -> T) async throws -> T
    {
        ...
        return try await TestIsolation.withEnvValues(["AFORA_CONFIG_PATH": configPath], body)
    }
        ...
        _ body: () async throws -> T) async throws -> T
    {
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-launchagent-marker-\(UUID().uuidString)")
    ...
    private func gatewayDescriptor(
        pid: Int32,
        command: String = "afora-gateway",
        executablePath: String = "/tmp/afora-gateway") -> PortGuardian.Descriptor
    {
        ...
        configAudit: String = #"{"ok":true,"issues":[]}"#) -> String
    {
        ...
        {"ok":true,"service":{
          ...
          "command":{"programArguments":["afora","gateway","--port","\(port)"]},
        ...
        try await self.withLaunchAgentEnvironment {
            let staleStatuses = [
                ...
                {"ok":true,"service":{
                  ...
                  "command":{"programArguments":["afora","gateway","--port","19084"]},
                  "configAudit":{"ok":true,"issues":[]}
                }}
                """,
                """
    ...
    @Test func `readiness refreshes when the endpoint pid changes during the probe`() async throws {
            ...
            GatewayTestWebSocketTask(
                sendHook: { task, message, sendIndex in
                    ...
                    let replacement = PortGuardian.Descriptor(
                        pid: 4343,
                        command: "afora-gateway",
                        executablePath: "/tmp/afora-gateway")
        ...
        let stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-gateway-pid-refresh-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `transient unavailable health response retries until ready`() async throws {
        let stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("afora-gateway-ready-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: stateDir) }
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        ...
        do {
            ...
            do {
                ...
                #expect(details.contains("pid 4343 afora-gateway @ /tmp/afora-gateway"))
                #expect(manager._testControlChannelRefreshForces().last == true)
                await cleanup()
            } catch {
                await cleanup()
