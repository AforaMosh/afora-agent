...
struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: AforaProtocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: AforaProtocol.AnyCodable(["triggers": [
            "afora",
            "computer",
        ]]))
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: AforaProtocol.AnyCodable(["unexpected": 123]))
