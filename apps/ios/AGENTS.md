...
- iOS SwiftUI text should use branded typography helpers, not bare system fonts. Use `AforaType` in `apps/ios/Sources/**`, `AforaChatTypography` in shared chat UI, `WatchClawType` in watch UI, and `AforaActivityType` in Live Activity UI.
...
- When touching iOS text surfaces, update/keep `apps/ios/Tests/AforaTypographyTests.swift` so bare text/control regressions are caught, then run the focused typography test.
...
- Do not add Afora, Afora Foundation, or other first-party/self-owned license entries. The screen is for third-party/open-source dependency acknowledgements.
- When adding, removing, or upgrading iOS dependencies, audit whether `apps/ios/Resources/Licenses/` needs updates. Exclude dependencies owned by Afora Foundation from the published license list.
