...
Use these steps to exercise the live Afora iOS App Review Gateway.

## Demo Account / Setup

Use the Afora iOS app with the live review Gateway setup code included in
...
9. On `Connected`, tap `Open Afora`.
10. Confirm the `Control` screen shows `Gateway Online`.
11. Tap `Settings`.
12. Tap `Approvals`.
13. Tap `Open Notifications`.
14. Tap `Enable Notifications`.
15. On `Enable Afora Hosted Push Relay?`, tap `Continue`.
16. If iOS asks whether Afora may send notifications, tap `Allow`.
...
command `printf 'Afora App Review approval demo complete\n'`. Tap
...
9. While Talk is active, send Afora to the background by returning to the
   Home Screen or locking the iPhone. Do not force quit the app.
10. Continue speaking then wait for assistant audio reply.

Expected result: realtime Talk audio continues while Afora is backgrounded.
Reopen Afora, confirm Talk is still active, then tap `Stop Talk`.
...
1. Tap `Settings`.
2. Tap `Reconnect`.
3. Immediately send Afora to the background by returning to the Home Screen
   or locking the iPhone.
4. Watch the Lock Screen or Dynamic Island while the Gateway reconnects.

Expected result: while reconnecting, iOS can show an `Afora` Live Activity
with connection status such as `Connecting...` or `Reconnecting...`. On a fast
network this status may be brief because Afora ends the Live Activity after
...
4. Immediately send Afora to the background and lock the iPhone. Do not
   force quit the app.

Expected result: the iPhone Lock Screen receives a visible `Afora`
notification with this body:

```text
Afora App Review push notification demo
```

Tap the notification and unlock the iPhone if prompted. If Afora opens on
...
3. Immediately send Afora to the background and lock the iPhone. Do not
   force quit the app.
4. Wait for the `Afora` notification on the Lock Screen. It normally appears
   about 10 seconds after the message is sent.
5. Tap the notification and unlock the iPhone if prompted. If Afora opens on
   `Control`, tap `Chat`.
...
Afora does not present an aggregate permissions page during onboarding.
...
the native iOS authorization prompt, with no additional Afora dialog or
deferral action.
...
permission sheet. Afora requests read-only access to steps, sleep, resting
...
4. Tap `Share`.
5. Tap `Afora`.
6. Confirm the Afora share extension appears and shows
   `Edit text, then tap Send.` and `Send to Afora`.
7. Tap `Send to Afora`.

Expected result: the Afora share extension sends the shared Safari page to
the live review Gateway and shows `Sent to Afora.` Returning to Afora
