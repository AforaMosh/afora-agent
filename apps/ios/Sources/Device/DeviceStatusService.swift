...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    private func batteryStatus() -> AforaBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: AforaBatteryState = switch device.batteryState {
        ...
        return AforaBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> AforaThermalStatusPayload {
        let state: AforaThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return AforaThermalStatusPayload(state: state)
    }

    private func storageStatus() -> AforaStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return AforaStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
