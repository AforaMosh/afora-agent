...
enum TalkPushToTalkOnceStart {
    case busy(AforaTalkPTTStopPayload)
...
private final class TalkPushToTalkOnceOperation {
    private var result: AforaTalkPTTStopPayload?
    private var continuation: CheckedContinuation<AforaTalkPTTStopPayload, Never>?

    func wait() async -> AforaTalkPTTStopPayload {
        if let result {
            return result
        }
    ...
    func finish(_ payload: AforaTalkPTTStopPayload) {
...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__AFORA_REDACTED__"
    ...
    private let logger = Logger(subsystem: "ai.aforafoundation.app", category: "TalkMode")
    ...
    func stop() {
        ...
        if let pendingCaptureId, pttOnceOperations[pendingCaptureId] != nil {
            let payload = AforaTalkPTTStopPayload(
    ...
    func beginPushToTalk(
        ...
        onCaptureReserved: @MainActor (String) -> Void = { _ in }) async throws -> AforaTalkPTTStartPayload
    {
        ...
        if self.isPushToTalkActive, let activePushToTalk {
            ...
            return AforaTalkPTTStartPayload(captureId: activePushToTalk.captureId)
        }
        ...
        return AforaTalkPTTStartPayload(captureId: captureId)
    }

    func endPushToTalk() -> AforaTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        return self.endPushToTalk(captureId: captureId)
    }

    func endPushToTalk(expectedTranscriptionOnly: Bool) -> AforaTalkPTTStopPayload {
        guard let activePushToTalk,
              activePushToTalk.transcriptionOnly == expectedTranscriptionOnly
        else {
            return AforaTalkPTTStopPayload(
    ...
    func endPushToTalk(captureId: String) -> AforaTalkPTTStopPayload {
        guard let activePushToTalk,
              activePushToTalk.captureId == captureId
        else {
            return AforaTalkPTTStopPayload(captureId: captureId, transcript: nil, status: "idle")
        }
        ...
        onCaptureReserved: @MainActor (String) -> Void = { _ in }) async throws -> TalkPushToTalkOnceStart
    {
        if let captureId = activePTTCaptureId ?? finishingPushToTalk?.captureId {
            return .busy(AforaTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy"))
        }
    ...
    func awaitPushToTalkOnce(_ start: TalkPushToTalkOnceStart) async -> AforaTalkPTTStopPayload {
        switch start {
    ...
    func cancelPushToTalk() -> AforaTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        return self.cancelPushToTalk(captureId: captureId)
    }

    func cancelPushToTalk(expectedTranscriptionOnly: Bool) -> AforaTalkPTTStopPayload {
        guard let activePushToTalk,
              activePushToTalk.transcriptionOnly == expectedTranscriptionOnly
        else {
            return AforaTalkPTTStopPayload(
    ...
    func cancelPushToTalk(captureId: String) -> AforaTalkPTTStopPayload {
    ...
    private func finishCapturedPushToTalk(
        ...
        transcript: String?,
        status: String,
        statusText: String = String(localized: "Ready")) -> AforaTalkPTTStopPayload
    {
    ...
    private func finishPTTOnce(_ payload: AforaTalkPTTStopPayload) {
    ...
    private func completeTranscriptResponse(
        acknowledgement: AforaChatSendResponse,
        startedAt: Double,
        gateway: GatewayNodeSession,
        gatewayRoute: GatewayNodeSessionRoute,
        sessionKey: String,
    ...
    private static func chatSendHistorySince(
        response: AforaChatSendResponse,
    ...
    private func sendChat(
        ...
        sessionKey: String,
        gatewayRoute: GatewayNodeSessionRoute,
        idempotencyKey: String) async throws -> AforaChatSendResponse
    {
        ...
        return try JSONDecoder().decode(AforaChatSendResponse.self, from: res)
    }
        ...
        await withTaskGroup(of: ChatCompletionResult.self) { group in
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if let text = AforaChatEventText.assistantText(from: chatEvent) {
        ...
        since: Double?) -> String?
    {
        for msg in messages.reversed() {
            ...
            let metadata = msg["__afora"] as? [String: Any]
        ...
        transcriptProcessingGeneration generation: UInt64) async
    {
        ...
        for await evt in stream {
            ...
            guard let text = AforaChatEventText.assistantText(from: chatEvent) else { continue }
...
extension TalkModeManager {
    ...
    private static func phase(forRealtimeStatus status: String) -> TalkPhase {
        ...
        case "Listening", "Listening (Realtime)":
            .listening
        case "Thinking", "Thinking…", "Asking Afora", "Still asking Afora", "Updating Afora":
            .thinking
    ...
    private static func watchPresentation(forRealtimeStatus status: String) -> TalkWatchPresentation {
        ...
        case "Listening", "Listening (Realtime)", "Thinking", "Thinking…", "Speaking", "Speaking…",
             "Asking Afora", "Still asking Afora", "Updating Afora", "Connecting",
        ...
        case "Afora unavailable":
            .localized("Afora unavailable")
    ...
    private static func presentationText(forRealtimeStatus status: String) -> String {
        ...
        case "Thinking…":
            String(localized: "Thinking…")
        case "Asking Afora":
            String(localized: "Asking Afora")
        case "Still asking Afora":
            String(localized: "Still asking Afora")
        case "Updating Afora":
            String(localized: "Updating Afora")
        case "Speaking":
        ...
        case "Afora unavailable":
            String(localized: "Afora unavailable")
        case "Confirmation needed":
