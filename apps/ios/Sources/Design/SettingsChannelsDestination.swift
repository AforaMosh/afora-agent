...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return AforaBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? AforaBrand.ok : AforaBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return AforaBrand.ok }
        if self.lastError != nil { return AforaBrand.warn }
        return self.configured ? AforaBrand.accent : .secondary
    }
