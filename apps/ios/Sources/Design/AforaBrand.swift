...
enum AppAppearancePreference: String, CaseIterable, Identifiable {
    ...
    static var launchArgumentPreference: AppAppearancePreference? {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--afora-appearance") else {
            return nil
        }
...
extension TalkWaveformPalette {
    ...
    static let aforaBrand = TalkWaveformPalette(
        active: [
            AforaBrand.carapaceCoral,
            AforaBrand.carapaceSea,
            AforaBrand.accent,
        ],
        inactive: [
...
struct AforaActivationGlyph: View {
    let size: CGFloat
    var mood: AforaMascotMood = .idle
    /// Opt-in tap Easter eggs; leave off when the glyph sits inside a control.
    var interactive = false

    var body: some View {
        AforaMascotView(floats: false, mood: self.mood, interactive: self.interactive)
            .frame(width: self.size, height: self.size)
            .shadow(
                color: AforaBrand.activationGlow.opacity(0.18),
...
extension View {
    func aforaSheetChrome() -> some View {
        self
            .tint(AforaBrand.accent)
            .background {
                AforaBrand.sheetBackground
                    .ignoresSafeArea()
            }
    }

    func aforaCraftSurface(cornerRadius: CGFloat = 24, shadow: Bool = true) -> some View {
        self.modifier(AforaCraftSurfaceModifier(cornerRadius: cornerRadius, shadow: shadow))
    }
}

private struct AforaCraftSurfaceModifier: ViewModifier {
        ...
        content
            .background {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .fill(AforaBrand.activationSurface)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .stroke(AforaBrand.activationHairline, lineWidth: 0.5)
            }
    }
}

struct AforaPrimaryActionButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(self.isEnabled ? AforaBrand.activationPrimaryActionText : Color.secondary)
            .tint(self.isEnabled ? AforaBrand.activationPrimaryActionText : Color.secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .fill(self.isEnabled ? Self.primaryFill : AforaBrand.activationDisabledGradient)
                    .shadow(
                        color: self.isEnabled ? AforaBrand.activationPrimaryAction.opacity(0.08) : .clear,
                        radius: 1,
                        x: 0,
                        y: 1)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(
                        self.isEnabled
                            ? AforaBrand.activationPrimaryAction.opacity(0.72)
...
struct AforaSecondaryActionButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(AforaType.subheadSemiBold)
            .foregroundStyle(self.isEnabled ? AforaBrand.activationPrimaryAction : .secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(AforaBrand.activationHairline, lineWidth: 0.75)
            }
            .scaleEffect(configuration.isPressed && self.isEnabled ? 0.98 : 1)
            .animation(.smooth(duration: 0.14), value: configuration.isPressed)
    }
    ...
    private static var secondaryFill: LinearGradient {
        LinearGradient(
            colors: [
                AforaBrand.activationSecondaryActionTop,
                AforaBrand.activationSecondaryActionBottom,
            ],
            startPoint: .top,
            endPoint: .bottom)
    }
...
struct AforaCloseButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
        ...
        configuration.label
            ...
            .background {
                Capsule(style: .continuous)
                    .fill(AforaBrand.activationNeutralGradient)
                    .shadow(
            ...
            .overlay {
                Capsule(style: .continuous)
                    .stroke(AforaBrand.activationNeutralStroke, lineWidth: 0.6)
            }
