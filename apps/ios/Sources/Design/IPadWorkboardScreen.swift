            ...
            Text(IPadWorkboardDefaults.label(for: status))
                ...
                .overlay {
                    Capsule()
                        .strokeBorder(
                            self.selectedStatus == status
                                ? AforaBrand.accent.opacity(0.42)
                                : Color.primary.opacity(0.08),
                            lineWidth: 1)
                }
        }
        .buttonStyle(.plain)
        .foregroundStyle(self.selectedStatus == status ? AforaBrand.accent : .primary)
        .accessibilityLabel(
            String(
...
private struct IPadWorkboardKanbanCard: View {
    ...
    private var color: Color {
        switch self.card.status {
        case "running": AforaBrand.ok
        case "review": AforaBrand.accentForeground
        case "blocked": AforaBrand.warn
        case "done": .secondary
        default: AforaBrand.accentHotForeground
        }
    }
