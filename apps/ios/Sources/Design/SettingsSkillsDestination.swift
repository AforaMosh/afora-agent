...
struct SettingsSkillsDestination: View {
    ...
    private var summaryCard: some View {
        ProCard(radius: SettingsLayout.cardRadius) {
            HStack(spacing: 12) {
                ProIconBadge(systemName: "sparkles", color: AforaBrand.accent)
                VStack(alignment: .leading, spacing: 3) {
                    Text("Skills").font(AforaType.headline)
    ...
    private var summaryColor: Color {
        ...
        return self.setupCount > 0 ? AforaBrand.warn : AforaBrand.ok
    }

    private var sectionPicker: some View {
        Picker(selection: self.$section) {
            ForEach(SkillsSettingsSection.allCases) { section in
                Text(verbatim: section.title).font(AforaType.captionSemiBold).tag(section)
            }
        } label: {
            Text("Skills section").font(AforaType.captionSemiBold)
        }
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 10) {
                TextField(
                    ...
                    prompt: Text("Search installed skills").font(AforaType.body))
                {
                    Text("Search installed skills").font(AforaType.body)
                }
                .font(AforaType.body)
                .textFieldStyle(.roundedBorder)
                Picker(selection: self.$installedFilter) {
                    ForEach(InstalledSkillFilter.allCases) { filter in
                        Text(verbatim: filter.title).font(AforaType.captionSemiBold).tag(filter)
                    }
                } label: {
                    Text("Installed skill filter").font(AforaType.captionSemiBold)
                }
                .pickerStyle(.segmented)
            }
        }
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 10) {
                TextField(
                    ...
                    prompt: Text("Search ClawHub").font(AforaType.body))
                {
                    Text("Search ClawHub").font(AforaType.body)
                }
                .font(AforaType.body)
                .textFieldStyle(.roundedBorder)
                .submitLabel(.search)
                ...
                Button {
                    ...
                    Label("Search", systemImage: "magnifyingglass").font(AforaType.subheadSemiBold)
                }
    ...
    private static var screenshotSkills: [SkillStatus] {
        [
            SkillStatus(
                ...
                primaryEnv: nil,
                emoji: "🐙",
                homepage: "https://docs.afora.ai/tools/skills",
                always: false,
                disabled: false,
                eligible: true,
...
private struct InstalledSkillRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                HStack(spacing: 7) {
                    Text(self.skill.name).font(AforaType.subheadSemiBold)
    ...
    private var statusColor: Color {
        ...
        return SettingsSkillsDestination.isReady(self.skill) ? AforaBrand.ok : AforaBrand.warn
    }
...
private struct ClawHubSkillRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(systemName: "shippingbox", color: self.installed ? AforaBrand.ok : AforaBrand.accent)
            VStack(alignment: .leading, spacing: 4) {
                Text(self.skill.displayName).font(AforaType.subheadSemiBold)
                Text(self.skill.summary ?? self.skill.reference)
                    .font(AforaType.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                HStack(spacing: 6) {
                    Text(self.skill.reference).font(AforaType.monoSmall).foregroundStyle(.secondary)
                    if let version = self.skill.version {
                        Text(verbatim: version).font(AforaType.monoSmall).foregroundStyle(.secondary)
                    }
                }
            ...
            Button(action: self.onAction) {
                Text(self.actionTitle).font(AforaType.captionSemiBold)
            }
            .buttonStyle(.bordered)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            HStack(alignment: .top, spacing: 12) {
                ProIconBadge(
                    ...
                    color: self.notice.isError ? AforaBrand.warn : AforaBrand.ok)
                VStack(alignment: .leading, spacing: 4) {
                    Text(self.notice.title).font(AforaType.subheadSemiBold)
                    Text(self.notice.message).font(AforaType.caption).textSelection(.enabled)
        ...
        NavigationStack {
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(action: self.onCancel) { Text("Cancel").font(AforaType.subheadSemiBold) }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button(action: self.onInstall) {
                        Text("Verify and install").font(AforaType.subheadSemiBold)
                    }
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    Label {
                        Text("Gateway warning").font(AforaType.headline)
                    } icon: {
                        Image(systemName: "exclamationmark.triangle.fill")
                    }
                    .foregroundStyle(AforaBrand.warn)
                    SkillsReviewDetails(review: self.review)
                    Text(self.message).font(AforaType.body)
                    DisclosureGroup(isExpanded: self.$warningExpanded) {
                        ...
                        Text("Review warning details").font(AforaType.subheadSemiBold)
                    }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button(action: self.onInstall) {
                        Text("Acknowledge and install").font(AforaType.subheadSemiBold)
                    }
    ...
    var body: some View {
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 8) {
                Text(self.review.displayName).font(AforaType.headline)
                if let summary = self.review.summary {
                    Text(summary).font(AforaType.body).foregroundStyle(.secondary)
                }
...
private struct SkillsReviewLine: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(self.label).font(AforaType.captionSemiBold).foregroundStyle(.secondary)
            Text(self.value).font(AforaType.body)
        }
    }
}
