...
@MainActor
enum AforaAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class AforaAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.aforafoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.aforafoundation.app", category: "BackgroundWake")
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? AforaAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "afora.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "afora.watch.promptId"
    static let sessionKeyKey = "afora.watch.sessionKey"
    static let gatewayStableIDKey = "afora.watch.gatewayStableID"
    static let actionPrimaryIDKey = "afora.watch.action.primary.id"
    static let actionPrimaryLabelKey = "afora.watch.action.primary.label"
    static let actionSecondaryIDKey = "afora.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "afora.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "afora.watch.action.primary"
    static let actionSecondaryIdentifier = "afora.watch.action.secondary"
    static let actionIdentifierPrefix = "afora.watch.action."
    static let actionIDKeyPrefix = "afora.watch.action.id."
    static let actionLabelKeyPrefix = "afora.watch.action.label."
    static let categoryPrefix = "afora.watch.prompt.category."
        ...
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AforaWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AforaWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Afora" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AforaWatchAction]) -> [UNNotificationAction] {
...
struct AforaApp: App {
    ...
    @UIApplicationDelegateAdaptor(AforaAppDelegate.self) private var appDelegate
    @Environment(\.scenePhase) private var scenePhase

    init() {
        Self.installUncaughtExceptionLogger()
        GatewaySettingsStore.bootstrapPersistence()
        AforaType.installUIKitAppearance()
        let appModel = NodeAppModel(audioAdmissionInitiallyAllowed: false)
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--afora-reset-onboarding") {
        ...
        #endif
        AforaAppModelRegistry.appModel = appModel
        ...
        WindowGroup {
            RootTabs()
                ...
                .onReceive(
                    ...
                    perform: { _ in
                        AforaType.refreshUIKitAppearance(in: Self.connectedWindows())
                    })
                .onOpenURL { url in
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--afora-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--afora-screenshot-notification-guidance")
        #else
        false
        #endif
    }
    ...
    private static var liveActivityVoicePreviewEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--afora-live-activity-voice-preview")
        #else
        false
        #endif
    }
    ...
    private func applyWindowTint() {
        for window in Self.connectedWindows() {
            window.tintColor = AforaBrand.uiAccent
        }
    }
...
extension AforaApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Afora: installing uncaught exception handler")
