...
struct RootTabs: View {
    ...
    static func initialDestination(arguments: [String]) -> SidebarDestination {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--afora-initial-tab") else { return .chat }
    ...
    static func requestedInitialSettingsRoute(arguments: [String]) -> SettingsRoute? {
        guard let flagIndex = arguments.firstIndex(of: "--afora-settings-route") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
        guard arguments.indices.contains(valueIndex) else { return nil }
        return switch arguments[valueIndex].trimmingCharacters(in: .whitespacesAndNewlines).lowercased() {
        case "afora", "system-agent": .systemAgent
        default: nil
        }
    }

    static func requestedInitialSidebarDestination(arguments: [String]) -> SidebarDestination? {
        guard let flagIndex = arguments.firstIndex(of: "--afora-initial-destination") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
        guard arguments.indices.contains(valueIndex) else { return nil }
    ...
    private static var initialChatSessionKey: String? {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--afora-chat-session") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
        guard arguments.indices.contains(valueIndex) else { return nil }
    ...
    private var uiTestReadinessMarker: some View {
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--afora-ui-test-readiness") {
            Color.clear
                .frame(width: 1, height: 1)
                .allowsHitTesting(false)
                .accessibilityElement(children: .ignore)
                .accessibilityIdentifier("RootTabs.Ready")
                .accessibilityLabel(Text(verbatim: "Afora test readiness"))
                .accessibilityValue(
    ...
    private var sidebarHeaderAction: AforaSidebarHeaderAction? {
        ...
        if self.isSidebarVisible {
            return AforaSidebarHeaderAction(
    ...
    private func makeHomeCanvasPayload() -> RootTabsHomeCanvasPayload {
        ...
        case .connected:
            return RootTabsHomeCanvasPayload(
                ...
                footer: "Afora only runs phone-side capabilities while the app is connected and permitted.")
        case .connecting:
            return RootTabsHomeCanvasPayload(
        ...
        case .error, .disconnected:
            return RootTabsHomeCanvasPayload(
                ...
                eyebrow: self.gatewayStatus == .error ? "Gateway needs attention" : "Afora iOS",
                ...
                agents: Array(agents.prefix(4)),
                footer:
                "Use Settings to scan a pairing QR code or paste a setup code from your Afora gateway.")
        }
    }
