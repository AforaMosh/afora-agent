...
enum WatchScreenshotMode {
    private static let defaultsKey = "afora.watch.screenshotMode"
    static let approvals = ProcessInfo.processInfo.arguments.contains(
        "--afora-watch-approval-screenshot-mode")
        || ProcessInfo.processInfo.environment["AFORA_WATCH_APPROVAL_SCREENSHOT_MODE"] == "1"
    static let enabled = ProcessInfo.processInfo.arguments.contains("--afora-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["AFORA_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: WatchScreenshotMode.defaultsKey)
        || WatchScreenshotMode.approvals
}

@main
struct AforaWatchApp: App {
    ...
    fileprivate func configureScreenshotFixture(includeApproval: Bool = false) {
        ...
        self.consume(
            appSnapshot: WatchAppSnapshotMessage(
                ...
                gatewayStableID: "watch-screenshot-gateway",
                talkStatus: AforaWatchAppStatus(code: .talkReady),
                talkEnabled: true,
                talkListening: false,
                talkSpeaking: false,
