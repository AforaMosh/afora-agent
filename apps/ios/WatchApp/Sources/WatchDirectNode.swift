...
final class WatchDirectNode {
    ...
    private static let keychainService = "ai.afora.watch.direct-node"
    ...
    func disconnectForBackground() {
        ...
        if self.isEnabled, self.isConfigured {
            self.statusText = String(localized: "Reconnects when Afora is active")
        }
    }
        ...
        notificationsAuthorized: Bool) throws -> ConnectParams
    {
        let payload = GatewayDeviceAuthPayload.buildV3(
            fields: .init(
                deviceId: identity.deviceId,
                client: .init(id: "afora-watchos", mode: "node"),
                role: "node",
                scopes: [],
    ...
    private func handleNotification(_ request: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decode(AforaSystemNotifyParams.self, from: request.paramsJSON)
    ...
    private func deviceStatus() -> AforaDeviceStatusPayload {
        let device = WKInterfaceDevice.current()
        device.isBatteryMonitoringEnabled = true
        let batteryState: AforaBatteryState = switch device.batteryState {
        ...
        let battery = AforaBatteryStatusPayload(
            level: device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil,
            state: batteryState,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
        let thermalState: AforaThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return AforaDeviceStatusPayload(
            battery: battery,
            thermal: AforaThermalStatusPayload(state: thermalState),
