...
Afora iOS is the officially released iPhone app. It connects to an Afora Gateway as a `role: node` for chat, voice, approvals, sharing, and device-aware automation.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical Afora team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical Afora team when creating bundle IDs or enabling services
- Release-owner access to the encrypted signing repo password (`MATCH_PASSWORD`)
- App Store Connect app already created for `ai.aforafoundation.app`
...
- Local development uses the canonical `ai.aforafoundation.app*` bundle IDs when the Afora team is available, and unique `ai.aforafoundation.app.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `ai.aforafoundation.app*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `AforaPushMode=appStore`, which derives relay transport, official distribution, the canonical production relay, production APNs, production relay profile, `appleStrict` proof, and the App-Attest-capable entitlement file.
...
- App Store release builds use the canonical hosted relay at `https://ios-push-relay.afora.ai`.
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `ai.aforafoundation.app`
   - `ai.aforafoundation.app.share`
   - `ai.aforafoundation.app.activitywidget`
   - `ai.aforafoundation.app.watchkitapp`
...
8. Expected outputs after a successful run:
   - `apps/ios/build/app-store/Afora-<version>.ipa`
   - `apps/ios/build/app-store/Afora-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Afora.entitlements` derives `aps-environment` from the active build configuration/signing override.
- App Attest relay builds use `apps/ios/Sources/AforaAppAttest.entitlements`; local/direct builds do not require App Attest provisioning.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual Debug builds default to `AforaPushMode=localSandbox`, direct APNs registration, and a development `aps-environment` entitlement. Local/manual Release builds default to `AforaPushMode=localProduction` and direct production APNs registration.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `AFORA_APNS_TEAM_ID`, `AFORA_APNS_KEY_ID`, and either `AFORA_APNS_PRIVATE_KEY_P8` or `AFORA_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.afora/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `AFORA_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to sandbox APNs through `AforaPushMode=localSandbox`; Release builds default to production APNs through `AforaPushMode=localProduction`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `afora.json`. `AFORA_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Afora official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as Afora node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
