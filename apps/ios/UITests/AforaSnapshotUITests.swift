...
@MainActor
final class AforaSnapshotUITests: XCTestCase {
    ...
    func testOnboardingExplainsCapabilitiesAndTrust() {
        ...
        app.launchArguments += ["--afora-reset-onboarding"]
        app.launch()
        self.app = app
    ...
    func testSidebarMoreAgentsMenuShowsAvatarsAndKeepsFooterVisible() throws {
        ...
        let gatewayFooter = try XCTUnwrap(self.app?.buttons.matching(
            NSPredicate(format: "label CONTAINS %@", "Afora Gateway")).firstMatch)
    ...
    func testVoiceNoteDraftKeepsStopAvailableDuringActiveResponse() throws {
        ...
        self.launchApp(
            ...
            additionalArguments: ["--afora-hold-initial-chat-run"])
    ...
    func testEmptyChatStarterPromptSendsMessage() throws {
        self.launchApp(
            ...
            additionalArguments: ["--afora-empty-chat-fixture"])
        ...
        starter.tap()
        let sentText = "Summarize the current Afora status and tell me what needs attention."
    ...
    func testEmptyChatStarterPromptsLocalizeInGerman() throws {
        ...
        let starter = try XCTUnwrap(self.app?.buttons["Afora-Status prüfen"])
        XCTAssertTrue(starter.exists)
        starter.tap()
        XCTAssertTrue(
            self.app?.staticTexts[
                "Fasse den aktuellen Afora-Status zusammen und sage mir, was Aufmerksamkeit erfordert.",
    ...
    func testLiveGatewayChatRoundTripAndControlOverview() throws {
        ...
        for index in 0..<3 {
            let seedMarker = "AFORA_E2E_SEED_\(index)_\(Int(Date().timeIntervalSince1970 * 1000))"
        ...
        let replyMarker = "AFORA_E2E_OK_\(Int(Date().timeIntervalSince1970 * 1000))"
        self.sendLiveGatewayMessage(
    ...
    func testManualAuthRetryUsesEditedToken() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["AFORA_IOS_RETRY_E2E"] == "1",
            "Set AFORA_IOS_RETRY_E2E=1 with a local token-auth Gateway on port 18920")
        let token = try XCTUnwrap(ProcessInfo.processInfo.environment["AFORA_IOS_RETRY_TOKEN"])
    ...
    func testPhotosLimitedAccess() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["AFORA_IOS_PHOTOS_E2E"] == "1",
            "Set AFORA_IOS_PHOTOS_E2E=1 to exercise the system Photos prompt")
...
extension AforaSnapshotUITests {
        ...
        additionalArguments: [String] = [])
    {
        ...
        XCTAssertTrue(
            ...
            "Afora root readiness marker did not appear")
        if screenshotMode {
        ...
        additionalArguments: [String]) -> XCUIApplication
    {
        ...
        app.launchArguments += [
            ...
            "hidden",
            "--afora-ui-test-readiness",
        ]
        if screenshotMode {
            app.launchArguments.append("--afora-screenshot-mode")
        }
        app.launchArguments += additionalArguments
        if let appearance {
            app.launchArguments += ["--afora-appearance", appearance]
        }
        return app
    }
        ...
        beforeCapture: ((XCUIApplication) -> Void)? = nil)
    {
        ...
        guard let app = self.app else {
            XCTFail("Afora is not running for screenshot target \(target.name)")
            return
        }
        beforeCapture?(app)
        ...
        line: UInt = #line)
    {
        ...
        XCTAssertTrue(
            ...
            "Afora did not terminate before the next launch",
            file: file,
            line: line)
        self.app = nil
    }
    ...
    {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["AFORA_IOS_LIVE_GATEWAY"] == "1",
            "Set AFORA_IOS_LIVE_GATEWAY=1 and provide a fresh setup code")

        if let setupCode = ProcessInfo.processInfo.environment["AFORA_IOS_LIVE_SETUP_CODE"] {
