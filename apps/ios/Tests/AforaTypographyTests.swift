...
struct RootSidebarTypographyTests {
    @Test func `root sidebar uses branded typography`() throws {
        ...
        #expect(sidebar.contains("AforaType.headline"))
        #expect(sidebar.contains("AforaType.subheadSemiBold"))
        #expect(sidebar.contains("AforaType.captionMedium"))
        #expect(sidebar.contains("AforaType.caption2Bold"))
        #expect(!sidebar.contains(".font(."))
    }
}

struct AforaTypographyTests {
    @Test func `thread controls use branded typography`() throws {
        ...
        #expect(support.contains(".font(AforaType.subhead)"))
        #expect(support.contains(".font(AforaType.subheadSemiBold)"))
        #expect(commandCenter.contains("Toggle(isOn: self.$showArchived)"))
        #expect(commandCenter.contains("Text(\"Show Archived\")"))
        #expect(commandCenter.contains(".font(AforaType.captionMedium)"))
    }

    @Test func `bundled fonts load from app bundle`() {
        for name in AforaType.registeredPostScriptNames {
    ...
    @Test func `app extensions register bundled branded fonts`() throws {
        ...
        for targetName in ["AforaActivityWidget", "AforaWatchApp"] {
            let target = try Self.extract(
                project,
                from: "  \(targetName):",
                to: targetName == "AforaActivityWidget" ? "  AforaWatchApp:" : "  AforaTests:")
    ...
    @Test func `extension text surfaces use branded typography helpers`() throws {
        ...
        #expect(activitySource.contains("AforaActivityType.subheadSemiBold"))
        #expect(activitySource.contains("AforaActivityType.subheadBold"))
        #expect(activitySource.contains("AforaActivityType.caption"))
    ...
    @Test func `UIKit typography refreshes when Dynamic Type changes`() throws {
        ...
        let typographySource = try String(
            contentsOf: Self.sourceURL("Design/AforaTypography.swift"),
            encoding: .utf8)

        #expect(appSource.contains("UIContentSizeCategory.didChangeNotification"))
        #expect(appSource.contains("AforaType.refreshUIKitAppearance(in: Self.connectedWindows())"))
    ...
    @Test func `listed iOS app surfaces enforce branded control typography`() throws {
        let proComponents = try String(
            contentsOf: Self.sourceURL("Design/AforaProComponents.swift"),
            encoding: .utf8)
        ...
        let automations = try String(
            ...
            encoding: .utf8)
        let docs = try String(contentsOf: Self.sourceURL("Design/AforaDocsScreen.swift"), encoding: .utf8)
        ...
        let chatTypography = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/AforaKit/Sources/AforaChatUI/ChatTypography.swift"),
            encoding: .utf8)
        let chatMessageViews = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/AforaKit/Sources/AforaChatUI/ChatMessageViews.swift"),
            encoding: .utf8)
        let chatWorkingClawView = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/AforaKit/Sources/AforaChatUI/ChatWorkingClawView.swift"),
            encoding: .utf8)
        let chatMarkdownRenderer = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/AforaKit/Sources/AforaChatUI/ChatMarkdownRenderer.swift"),
            encoding: .utf8)

        #expect(automations.contains(".font(AforaType.body)"))
        #expect(automations.contains(".font(AforaType.headline)"))
        #expect(automations.contains(".font(AforaType.subheadSemiBold)"))
        #expect(automations.contains(".font(AforaType.caption)"))
        #expect(!automations.contains(".font(.body"))
        #expect(!automations.contains(".font(.headline"))
        #expect(!automations.contains(".font(.caption"))

        #expect(proComponents.contains(".font(AforaType.subheadSemiBold)"))
        ...
        #expect(chatTab.contains(".font(AforaType.body)"))
        ...
        #expect(quickSetup.contains(".font(AforaType.subheadSemiBold)"))
        ...
        #expect(gatewayProblem.contains(".font(AforaType.subheadSemiBold)"))
        ...
        #expect(onboardingSteps.contains(".font(AforaType.subheadSemiBold)"))
        ...
        #expect(onboardingWizard.contains(".font(AforaType.subheadSemiBold)"))
        ...
        let onboardingSecureOption = try Self.extract(
            ...
            to: ".tag(true)")
        #expect(onboardingUnencryptedOption.contains(".font(AforaType.captionSemiBold)"))
        #expect(onboardingSecureOption.contains(".font(AforaType.captionSemiBold)"))

        #expect(settingsSections.contains(".font(AforaType.body)"))
        #expect(settingsSections.contains("Text(warningText)"))
        #expect(settingsSections.contains(".font(AforaType.caption)"))
        #expect(approvalDialog.contains("Text(warningText)"))
        #expect(approvalDialog.contains(".font(AforaType.footnote)"))
        ...
        #expect(settingsSections.contains("AforaToggleIndicator(isOn: isOn.wrappedValue)"))
        #expect(settingsSections.contains("TextField(\"Default Share Instruction\""))
        #expect(settingsSections.contains(".font(AforaType.subhead)"))
        ...
        #expect(settingsSections.contains(".font(AforaType.subheadSemiBold)"))
        #expect(settingsSupport.contains("struct SettingsBuildMetadataStrip"))
        #expect(settingsSupport.contains(".font(AforaType.caption2SemiBold)"))
        #expect(settingsSupport.contains(".font(AforaType.monoSmall)"))
        ...
        let settingsSecureOption = try Self.extract(
            ...
            from: "Text(\"Secure (TLS)\")",
            to: ".tag(true)")
        #expect(settingsUnencryptedOption.contains(".font(AforaType.captionSemiBold)"))
        #expect(settingsSecureOption.contains(".font(AforaType.captionSemiBold)"))
        ...
        #expect(appleHealthAccess.contains(".font(AforaType.footnote)"))
        ...
        #expect(permissionRow.contains(".font(AforaType.footnoteSemiBold)"))
        ...
        #expect(skills.contains("Text(\"Gateway warning\").font(AforaType.headline)"))
        #expect(skills.contains("Text(\"Acknowledge and install\").font(AforaType.subheadSemiBold)"))
        #expect(skills.contains("prompt: Text(\"Search ClawHub\").font(AforaType.body)"))

        for source in [agentDestinations, dreaming, instances, channels, skills, docs] {
            #expect(source.contains(".font(AforaType.body)"))
        }

        #expect(chatMessageViews.contains("typography: segment.kind.markdownTypography"))
        #expect(chatMessageViews.contains(".font(AforaChatTypography.caption)"))
        #expect(chatMessageViews.contains("chat-user-message-disclosure-toggle"))
        #expect(chatMarkdownRenderer.contains("AforaChatTypography.callout.italic()"))
        #expect(!chatMessageViews.contains("font: .body"))
        #expect(!chatMessageViews.contains("Font.body"))
        #expect(!chatMessageViews.contains("Font.callout"))
        #expect(chatWorkingClawView.contains(".font(AforaChatTypography.caption)"))
        #expect(chatWorkingClawView.contains(".font(AforaChatTypography.captionSemiBold)"))
    ...
    private static func activityWidgetSourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/AforaLiveActivity.swift")
    }

    private static func activityWidgetTypographySourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/AforaActivityTypography.swift")
    }
    ...
    private static func swiftSourcesForTypographyAudit() throws -> [URL] {
        let roots = [
            ...
            self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/AforaKit/Sources/AforaChatUI"),
        ]
    ...
    private static func unbrandedTextCallOffenders(in source: String, relativePath: String) -> [String] {
        let fontTokens = ["AforaType", "AforaChatTypography", "typography."]
    ...
    private static func hasAllowedBrandedFontParameter(_ fontCall: String, relativePath: String) -> Bool {
        switch relativePath {
        case "apps/ios/Sources/Design/AforaProComponents.swift":
            fontCall.contains(".font(self.titleFont)") ||
                fontCall.contains(".font(self.subtitleFont)")
        case "apps/shared/AforaKit/Sources/AforaChatUI/ChatMarkdownRenderer.swift":
