...
struct AforaBrandTests {
    @Test func `brand colors meet text contrast in both appearances`() {
        let foregroundColors = [
            ("accent", AforaBrand.uiAccentForeground),
            ("accentHot", AforaBrand.uiAccentHotForeground),
            ("ok", AforaBrand.uiOK),
            ("warn", AforaBrand.uiWarn),
            ("danger", AforaBrand.uiDanger),
            ("info", AforaBrand.uiInfo),
        ]
        ...
        for style in [UIUserInterfaceStyle.light, .dark] {
            ...
            let pillBackground = Self.composite(
                AforaBrand.uiAccentForeground,
                alpha: style == .dark ? 0.12 : 0.08,
                over: .secondarySystemGroupedBackground,
                traits: traits)
            #expect(Self.contrastRatio(AforaBrand.uiAccentForeground, pillBackground, traits: traits) >= 4.5)
            #expect(Self.contrastRatio(AforaBrand.uiAccent, .white, traits: traits) >= 4.5)
        }
    }
