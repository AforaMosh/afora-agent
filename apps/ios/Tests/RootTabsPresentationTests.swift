...
struct RootTabsPresentationTests {
    ...
    @Test func `dashboard deep link requests overview navigation`() async throws {
        ...
        let url = try #require(URL(string: "afora://dashboard"))
    ...
    @Test func `app launch defaults to chat destination`() {
        #expect(RootTabs.initialDestination(arguments: ["Afora"]) == .chat)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab"]) == .chat)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab", "unknown"]) == .chat)
    }

    @Test func `app launch uses requested destination before chat fallback`() {
        #expect(RootTabs
            .initialDestination(arguments: ["Afora", "--afora-initial-destination", "overview"]) == .overview)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-destination", "chat"]) == .chat)
        #expect(RootTabs
            .initialDestination(arguments: ["Afora", "--afora-initial-destination", "agents"]) == .agents)
        #expect(RootTabs
            .initialDestination(arguments: ["Afora", "--afora-initial-destination", "gateway"]) == .gateway)
        #expect(
            RootTabs.initialDestination(arguments: [
                "Afora",
                "--afora-initial-tab",
                "unknown",
                "--afora-initial-destination",
                "activity",
            ]) == .activity)
    }

    @Test func `legacy initial tab aliases map directly to sidebar destinations`() {
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab", "control"]) == .overview)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab", "overview"]) == .overview)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab", "chat"]) == .chat)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab", "talk"]) == .chat)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab", "voice"]) == .chat)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab", "agents"]) == .agents)
        #expect(RootTabs.initialDestination(arguments: ["Afora", "--afora-initial-tab", "settings"]) == .settings)
    }
    ...
    @Test func `chat sidebar destination can use native route title instead of agent branding`() {
        ...
        #expect(ChatProTab.defaultHeaderTitle(showsAgentBadge: true, agentDisplayName: "Afora") == "Afora")
        #expect(ChatProTab.defaultHeaderTitle(showsAgentBadge: false, agentDisplayName: "Afora") == "Chat")
    }
    ...
    @Test func `session work subtitle mirrors the web repo and branch line`() {
        func entry(repoRoot: String?, branch: String?) -> AforaChatSessionEntry {
            Self.sessionEntry(
                key: "agent:main:w1",
    ...
    @Test func `sidebar subtitle keeps an unread final observer digest above work metadata`() {
        let digest = AforaChatSessionObserverDigest(
            revision: 4,
            updatedAt: 2000,
        ...
        #expect(ChatSessionSidebarModel.subtitle(
            for: unread,
            workSubtitle: "afora \u{2387} observer") == "Finished with warnings")
        #expect(ChatSessionSidebarModel.subtitle(
            for: read,
            workSubtitle: "afora \u{2387} observer") == "afora \u{2387} observer")
    }
    ...
    private static func sessionEntry(
        ...
        worktree: AforaChatSessionWorktree? = nil) -> AforaChatSessionEntry
    {
