...
@Suite struct DeepLinkParserTests {
    @Test func parseRejectsUnknownHost() {
        let url = URL(string: "afora://nope?message=hi")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseHostIsCaseInsensitive() {
        let url = URL(string: "afora://AGENT?message=Hello")!
        #expect(DeepLinkParser.parse(url) == agentAction(message: "Hello"))
    }

    @Test func parseRejectsNonAforaScheme() {
        let url = URL(string: "https://example.com/agent?message=hi")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseRejectsEmptyMessage() {
        let url = URL(string: "afora://agent?message=%20%20%0A")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseAgentLinkParsesCommonFields() {
        let url =
            URL(string: "afora://agent?message=Hello&deliver=1&sessionKey=node-test&thinking=low&timeoutSeconds=30")!
    ...
    @Test func parseAgentLinkParsesTargetRoutingFields() {
        let url =
            URL(
                string: "afora://agent?message=Hello%20World&deliver=1&to=%2B15551234567&channel=whatsapp&key=secret")!
        #expect(DeepLinkParser.parse(url) == agentAction(
            message: "Hello World",
            deliver: true,
            to: "+15551234567",
    ...
    @Test func parseRejectsNegativeTimeoutSeconds() {
        let url = URL(string: "afora://agent?message=Hello&timeoutSeconds=-1")!
        #expect(DeepLinkParser.parse(url) == agentAction(message: "Hello"))
    }

    @Test func parseGatewayLinkParsesCommonFields() {
        let url = URL(
            string: "afora://gateway?host=afora.local&port=18789&tls=1&token=abc&password=def")!
        #expect(
    ...
    @Test func parseGatewayLinkAllowsPrivateLanWs() {
        let url = URL(
            string: "afora://gateway?host=afora.local&port=18789&tls=0&token=abc")!
        #expect(
            DeepLinkParser.parse(url) == .gateway(
                .init(
                    host: "afora.local",
    ...
    @Test func parseGatewayLinkRejectsInvalidPort() {
        let url = URL(string: "afora://gateway?host=gateway.example.com&port=70000&tls=1")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseGatewayLinkRejectsMalformedPort() {
        let url = URL(string: "afora://gateway?host=gateway.example.com&port=not-a-port&tls=1")!
        #expect(DeepLinkParser.parse(url) == nil)
    }
