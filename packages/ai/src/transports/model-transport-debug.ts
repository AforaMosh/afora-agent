 ...
 * interpret Afora debug environment variables consistently.
 */
...
/** Payload debug detail levels accepted by `AFORA_DEBUG_MODEL_PAYLOAD`. */
type ModelPayloadDebugMode = "off" | "summary" | "tools" | "full-redacted";
/** SSE debug detail levels accepted by `AFORA_DEBUG_SSE`. */
...
/** Resolves model payload debug verbosity from `AFORA_DEBUG_MODEL_PAYLOAD`. */
export function resolveModelPayloadDebugMode(
  env: ModelTransportDebugEnv = process.env,
): ModelPayloadDebugMode {
  const normalized = normalizeLowercaseStringOrEmpty(env.AFORA_DEBUG_MODEL_PAYLOAD);
...
/** Resolves SSE stream debug verbosity from `AFORA_DEBUG_SSE`. */
export function resolveModelSseDebugMode(
  env: ModelTransportDebugEnv = process.env,
): ModelSseDebugMode {
  const normalized = normalizeLowercaseStringOrEmpty(env.AFORA_DEBUG_SSE);
  if (normalized === "peek") {
    return "peek";
  }
...
function isModelTransportDebugEnabled(env: ModelTransportDebugEnv = process.env): boolean {
  return (
    isTruthyEnv(env.AFORA_DEBUG_MODEL_TRANSPORT) ||
    resolveModelPayloadDebugMode(env) !== "off" ||
    resolveModelSseDebugMode(env) !== "off" ||
    isTruthyEnv(env.AFORA_DEBUG_CODE_MODE)
  );
}
