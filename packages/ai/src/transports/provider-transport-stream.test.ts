...
import type { Api, Model } from "@afora/llm-core";
...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to Afora transport aliases when needed.
    const cases = [
      {
        ...
        alias: "afora-openai-responses-transport",
      },
      {
        ...
        alias: "afora-openai-chatgpt-responses-transport",
      },
      {
        ...
        alias: "afora-openai-completions-transport",
      },
      {
        ...
        alias: "afora-azure-openai-responses-transport",
      },
      {
        ...
        alias: "afora-anthropic-messages-transport",
      },
      {
        ...
        alias: "afora-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Afora transport", () => {
    const cases = [
      buildModel("openai-responses", {
        id: "gpt-5.4",
        provider: "openai",
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createAforaTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the Afora simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("afora-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the Afora transport until Afora preserves attribution", () => {
