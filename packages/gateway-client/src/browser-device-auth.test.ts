...
const client = {
  id: "afora-browser-copilot" as const,
  version: "test",
  platform: "Chrome",
...
describe("GatewayBrowserDeviceAuthLifecycle", () => {
  it("preserves stored scopes for the same token and uses hello scopes after rotation", async () => {
    ...
    expect(sign).toHaveBeenCalledWith(
      "v3|device|afora-browser-copilot|ui|operator|operator.admin,operator.write|456|test-token-placeholder|nonce|chrome|extension",
    );
    ...
    expect(store).toHaveBeenCalledWith({
      clientId: "afora-browser-copilot",
      deviceId: "device",
      role: "operator",
      token: "test-token-placeholder",
      scopes: ["operator.admin", "operator.write"],
  ...
  it("uses only the preferred bootstrap credential and never persists it", async () => {
    ...
    expect(sign).toHaveBeenCalledWith(
      "v3|device|afora-browser-copilot|ui|operator|operator.read,operator.write|123|test-bootstrap-token|nonce|chrome|extension",
    );
