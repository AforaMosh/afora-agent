...
import { GatewayClientTransport, Afora } from "./index.js";
...
describe("Afora SDK websocket e2e", () => {
  afterEach(async () => {
    await Promise.all(
...
describe("Afora SDK real Gateway e2e", () => {
...
const liveGatewayUrl = process.env.AFORA_SDK_LIVE_GATEWAY_URL;
const liveGatewayToken = process.env.AFORA_SDK_LIVE_GATEWAY_TOKEN;
...
liveGatewayDescribe("Afora SDK live Gateway e2e", () => {
  it("connects to a configured Gateway, streams a real run, and waits for completion", async () => {
    ...
    try {
      ...
      const agent = await oc.agents.get(process.env.AFORA_SDK_LIVE_AGENT_ID ?? "main");
      const run = await agent.run({
        input: "Reply with exactly: AFORA_SDK_LIVE_OK",
      ...
      expect(events.text).toContain("AFORA_SDK_LIVE_OK");
    } finally {
      await oc.close();
    }
  }, 240_000);
