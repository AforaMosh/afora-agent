...
/** Resolve the underlying OS home before applying Afora overrides. */
...
/** Resolve raw home with AFORA_HOME tilde expansion. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$AFORA_HOME" } : { home, prefix: "~" };
}
...
/** Replace the effective home path with "~" or "$AFORA_HOME" for terminal display. */
