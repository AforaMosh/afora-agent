...
const pluginListRows = [
  {
    ...
    Format: "afora",
    Status: "enabled",
    Source: "~/Projects/afora/extensions/amazon-bedrock/index.ts",
    Version: "2026.8.1",
  },
  {
    Name: "N".repeat(40),
...
describe("renderTable", () => {
  ...
  it("does not split BEL-terminated OSC-8 links when wrapping", () => {
    const open = "\x1b]8;;https://afora.ai\x07";
  ...
  it("does not split C1 OSC-8 links when wrapping", () => {
    const open = "\x9d8;;https://afora.ai\x9c";
    const close = "\x9d8;;\x9c";
    const canonicalOpen = "\x1b]8;;https://afora.ai\x07";
  ...
  it("preserves OSC-8 parameters when reopening wrapped links", () => {
    const open = "\x1b]8;id=docs;https://afora.ai\x07";
    const close = "\x1b]8;;\x07";
    const out = renderTable({
      width: 20,
      columns: [
      ...
      rows: [{ K: "X", V: `${open}${"Afora".repeat(5)}${close} after` }],
    });

    const linkLines = out.split("\n").filter((line) => line.includes("Afora"));
  ...
  it.each([
    ["BEL ST", "\x1b]8;;https://afora.ai\x07", "\x1b]8;;\x07"],
    ["ESC-backslash ST", "\x1b]8;;https://afora.ai\x1b\\", "\x1b]8;;\x1b\\"],
    ["C1 ST", "\x9d8;;https://afora.ai\x9c", "\x9d8;;\x9c"],
  ])(
    "closes and reopens embedded OSC-8 links at wrap boundaries (%s)",
    (_label, openSeq, closeSeq) => {
      const link = `${openSeq}Afora${closeSeq}`;
      const out = renderTable({
        width: 20,
        columns: [
          { key: "K", header: "K", minWidth: 3 },
  ...
  it.each([
    ...
    (_label, openSeq, closeSeq) => {
      ...
      // The link itself stays intact on the Afora line: open + close present.
      const linkLine = lines.find((line) => line.includes("Afora"));
...
describe("wrapNoteMessage", () => {
  ...
  it("preserves long Windows paths without inserting spaces/newlines", () => {
    ...
    const input = "C:\\\\State\\\\Afora\\\\bin\\\\afora.exe";
  ...
  it("widens note output columns so clack does not re-wrap copy-sensitive lines", () => {
    const wrapped = wrapNoteMessage(
      [
        ...
        "- ~/.afora/agents/main/sessions/9c2acae5-841f-4aea-936b-fdb513b60202.jsonl.lock pid=86519 (alive) age=2m47s stale=no",
      ].join("\n"),
      { columns: 80 },
    );
    const writes: string[] = [];
    ...
    expect(rendered).toContain(
      "- ~/.afora/agents/main/sessions/9c2acae5-841f-4aea-936b-fdb513b60202.jsonl.lock",
    );
  });
