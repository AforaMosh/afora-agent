...
import { genericMockPayloadWithNativeThread, isRecord, optionalString, requireNativeInboundId, } from "./native-local-mock.js";
...
function authenticateSlackWebhook(request, rawBody, signingSecret) {
...
function normalizeSlackEventsPayload(payload) {
    if (!isRecord(payload)) {
...
function matchesSlackThread(candidateThreadId, expectedThreadId, target) {
    if (!expectedThreadId) {
        return true;
    }
    const scopedExpectedThreadId = target.channelId && SLACK_TS_RULE.pattern.test(expectedThreadId)
        ...
        : expectedThreadId;
    return (candidateThreadId === scopedExpectedThreadId ||
