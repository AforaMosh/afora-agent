...
import { FEISHU_CHAT_ID_RULE, FEISHU_MESSAGE_ID_RULE, getBuiltinTargetCodec, } from "../target-normalizers.js";
...
export function resolveFeishuAdapterConfig(config, env = process.env) {
    return {
...
export function handleFeishuWebhookPayload(payload) {
    if (isRecord(payload) &&
...
export function createFeishuWebhookAuthenticator(config, env = process.env, runtime = {}) {
    return createFeishuWebhookAuthenticatorWithReplay(config, env, runtime);
}
function createFeishuWebhookAuthenticatorWithReplay(config, env, runtime, replayState) {
...
export class FeishuProviderAdapter extends LocalMockProviderAdapter {
    constructor(id, config, _userName, runtime) {
        ...
        const authenticateWebhookRequest = createFeishuWebhookAuthenticatorWithReplay(config, env, authRuntime, replayState);
        const decodePayload = (payload) => resolved.encryptKey ? decryptFeishuWebhookPayload(payload, resolved.encryptKey) : payload;
        super({
            ...
            options: {
                ...
                settleWebhookRequest({ accepted, payload, rawBody }) {
                    ...
                    if (accepted) {
                        acceptFeishuCallback(replayState, decodedPayload, encryptedEnvelope, authRuntime.now?.() ?? Date.now());
                    }
                    else {
...
export function normalizeFeishuWebhookPayload(payload) {
    if (!isRecord(payload)) {
    ...
    return {
        author: authorFromBotFlag(event
            ? ["app", "bot"].includes(optionalString(optionalRecord(event, "sender") ?? {}, "sender_type") ?? "")
            : false),
        ...(messageId
...
function readFeishuSettledEnvelope(payload, rawBody) {
    if (isRecord(payload)) {
        return payload;
    }
    try {
...
export function decryptFeishuWebhookPayload(payload, encryptKey) {
...
function readFeishuVerificationToken(payload) {
    if (!isRecord(payload)) {
        return null;
    }
    const header = optionalRecord(payload, "header");
    return (optionalString(payload, "token") ?? (header ? optionalString(header, "token") : null) ?? null);
}
function isFeishuUrlVerification(payload) {
    return (isRecord(payload) &&
        payload.type === "url_verification" &&
        typeof payload.challenge === "string" &&
        payload.challenge.length > 0);
}
function verifyFeishuSignature(request, rawBody, encryptKey) {
...
function readFeishuCallbackTimestamp(request, payload, signedRequest) {
...
function readFeishuCallbackKeys(payload, encryptedEnvelope) {
...
function reserveFeishuCallback(replayState, callbackKeys) {
...
function acceptFeishuCallback(replayState, payload, encryptedEnvelope, now) {
    const callbackKeys = readFeishuCallbackKeys(payload, encryptedEnvelope);
    pruneFeishuReplayCache(replayState.accepted, now);
    for (const key of callbackKeys) {
        replayState.accepted.set(key, now);
    }
    const reservations = new Set(callbackKeys
        .map((key) => replayState.inFlight.get(key))
...
function rejectFeishuCallback(replayState, payload, encryptedEnvelope) {
    const callbackKeys = readFeishuCallbackKeys(payload, encryptedEnvelope);
    const reservations = new Set(callbackKeys
        .map((key) => replayState.inFlight.get(key))
        .filter((reservation) => reservation !== undefined));
...
function releaseFeishuReservation(replayState, reservation, accepted) {
...
function pruneFeishuReplayCache(recentCallbacks, now) {
    for (const [key, acceptedAt] of recentCallbacks) {
        if (now - acceptedAt > FEISHU_MAX_CALLBACK_AGE_MS ||
            recentCallbacks.size > FEISHU_REPLAY_CACHE_LIMIT) {
            recentCallbacks.delete(key);
        }
    }
}
function unauthorizedFeishuWebhook() {
