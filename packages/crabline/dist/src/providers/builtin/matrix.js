...
import { getBuiltinTargetCodec, MATRIX_EVENT_ID_RULE, MATRIX_ROOM_ID_RULE, } from "../target-normalizers.js";
...
export class MatrixProviderAdapter extends LocalMockProviderAdapter {
    constructor(id, config, userName, _runtime) {
...
function matrixThreadKey(roomId, threadId) {
    return `${roomId}:thread:${threadId}`;
}
export function matchesMatrixThread(candidateThreadId, expectedThreadId, target) {
    if (!expectedThreadId) {
        return true;
    }
    const scopedExpected = target.channelId && MATRIX_EVENT_ID_RULE.pattern.test(expectedThreadId)
...
export function normalizeMatrixWebhookPayload(payload, botUserId) {
    if (!isRecord(payload)) {
    ...
    return {
        ...
        threadId: threadRootId
            ? matrixThreadKey(requireNativeInboundId(roomId, MATRIX_ROOM_ID_RULE, "Matrix room_id"), requireNativeInboundId(threadRootId, MATRIX_EVENT_ID_RULE, "Matrix thread root event_id"))
