...
const SIGNAL_UUID_RE = /^(?:uuid:)?([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/iu;
function deterministicSignalUuid(value) {
...
function signalDirectIdentity(id) {
    const value = id.trim();
    if (/^\+?[1-9]\d{2,14}$/u.test(value)) {
        const sourceNumber = value.startsWith("+") ? value : `+${value}`;
        return { recipient: sourceNumber, sourceNumber };
    }
    const sourceUuid = SIGNAL_UUID_RE.exec(value)?.[1]?.toLowerCase() ?? deterministicSignalUuid(value);
    return { recipient: sourceUuid, sourceUuid };
}
function signalRecipientValues(value) {
    const values = Array.isArray(value) ? value : [value];
    return values.map(readString).filter((entry) => entry !== undefined);
}
function signalOutboundTargets(params, account) {
    const targets = [
...
export const SIGNAL_AFORA_CRABLINE_PROVIDER_BRIDGE = createAforaCrablineProviderBridge({
    provider: "signal",
        ...
        return {
            ...
            createInbound(input) {
                ...
                return {
                    ...
                    providerTargetKey: kind === "group" ? `group:${conversationId}` : senderIdentity.recipient,
