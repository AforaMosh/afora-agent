...
const MATRIX_SEND_PATH_RE = /^\/_matrix\/client\/(?:v3|r0)\/rooms\/[^/]+\/send\/m\.room\.message\/[^/]+$/u;
const TELEGRAM_SEND_PATH_RE = /^\/bot<redacted>\/(?:sendAnimation|sendAudio|sendDocument|sendMessage|sendPhoto|sendVideo)$/iu;
const ZALO_SEND_PATH_RE = /^\/bot<redacted>\/(?:sendMessage|sendPhoto)$/u;
export function isAcceptedAforaCrablineOutbound(params) {
    ...
    switch (params.manifest.provider) {
        ...
        case "whatsapp": {
            ...
            const directWebSocketTarget = canonicalizeWhatsAppUserCorrelationJid(readNonBlankString(key?.remoteJid) ?? "");
