...
import { AFORA_CRABLINE_MANIFEST_PATH } from "./shared.js";
...
function parseCommitStagePath(contents, token) {
    ...
    try {
        ...
        throw new Error("Afora Crabline smoke lock commit stage metadata is malformed.", {
            cause: error,
        });
    }
    ...
    return {
        assertHealthy() {
            if (failure !== undefined) {
                throw new Error("Afora Crabline smoke lock heartbeat failed.", { cause: failure });
            }
        },
        async settle() {
            await pending;
...
async function readDirectoryIdentity(directoryPath) {
    try {
        ...
        if (!stats.isDirectory() || stats.ino <= 0n) {
            throw new Error("Afora Crabline smoke lock claim is not a directory.");
        }
        return {
            device: stats.dev,
            inode: stats.ino,
...
function parseLockOwner(contents) {
    ...
    try {
        ...
        throw new Error("Afora Crabline smoke lock owner metadata is malformed.", {
            cause: error,
        });
    }
...
function activeRunError(params) {
    return new Error(`Afora Crabline smoke is already running for channel "${params.channel ?? "unknown"}" in "${params.outputDir}"; cannot start channel "${params.requestedChannel}".`, params.cause === undefined ? undefined : { cause: params.cause });
}
async function removeOwnedLock(params) {
    ...
    if (stagedFilePath) {
        const revokedStagePath = path.join(path.dirname(stagedFilePath), `.revoked-commit-file.${params.token}.${randomUUID()}.tmp`);
        try {
...
export async function acquireAforaCrablineSmokeRunLock(params, dependencies = {}) {
    const outputDir = path.resolve(params.outputDir);
    const lockDirectory = path.join(outputDir, `.${AFORA_CRABLINE_MANIFEST_PATH}.lock`);
    ...
    if (!isValidProcessId(runtime.currentPid) ||
        ...
        throw new Error("Invalid Afora Crabline smoke lock runtime.");
    }
    const owner = {
    ...
    for (;;) {
        ...
        if ((await listLockClaims(lockDirectory)).length > 0 ||
        ...
        const renew = async () => {
            const renewal = pendingRenewal
                ...
                if (!Number.isSafeInteger(renewedAtMs)) {
                    throw new Error("Afora Crabline smoke lock renewal timestamp overflowed.");
                }
                if (!(await renewOwnedLock(ownedDirectory, token, renewedAtMs))) {
                    throw new Error("Afora Crabline smoke lock ownership was lost.");
                }
        ...
        try {
            heartbeat = (dependencies.startHeartbeat ?? startHeartbeat)(renew, Math.max(1, Math.floor(runtime.leaseMs / 3)));
        }
        catch (error) {
            await removeOwnedLock({
                lockDirectory,
        ...
        return {
            async assertOwned() {
                if (released) {
                    throw new Error("Afora Crabline smoke lock has already been released.");
                }
            ...
            async commitFileAtomically({ contents, destinationPath, stageDirectory, stageFile }) {
                ...
                if (commitFenceConsumed || heartbeatStopped || ownedDirectory !== lockDirectory) {
                    throw new Error("Afora Crabline smoke lock has already committed its fence.");
                }
                heartbeat.assertHealthy();
                await renew();
                heartbeat.assertHealthy();
                ...
                try {
                    ...
                    if (claimed.kind === "missing" ||
                        ...
                        throw new Error("Afora Crabline smoke lock commit fence was lost.");
                    }
                    ...
                    await fs.rename(stageDirectory ? stagedFilePath : path.join(commitClaim, stagedFileName), destinationPath);
                    committed = true;
                }
                finally {
...
export async function releaseAforaCrablineSmokeRunLock(lock, dependencies = {}) {
