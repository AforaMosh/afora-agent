...
function inflateWithInfo(buffer) {
    return new Promise((resolve, reject) => {
        inflate(buffer, { info: true, maxOutputLength: WHATSAPP_BINARY_NODE_MAX_DECOMPRESSED_BYTES }, (error, result) => {
            if (error) {
                reject(error);
            }
            else {
...
export async function decodeBinaryNode(frame) {
...
async function decompressIfRequired(buffer) {
    if (buffer.length === 0) {
    ...
    if ((flag & 2) !== 0) {
        ...
        try {
            ...
            if (error instanceof RangeError ||
                (error instanceof Error && error.code === "ERR_BUFFER_TOO_LARGE")) {
                throw new Error(`WhatsApp binary node expands beyond ${WHATSAPP_BINARY_NODE_MAX_DECOMPRESSED_BYTES} bytes.`, { cause: error });
            }
            throw error;
        }
    }
...
function decodeDecompressedBinaryNode(buffer, indexRef, depth, budget) {
    ...
    const readInt = (length, littleEndian = false) => {
        checkEOS(length);
        let value = 0;
    ...
    const readStringFromChars = (length) => readBytes(length).toString("utf8");
    ...
    const readListSize = (tag) => {
        ...
        if (budget.listItems > WHATSAPP_BINARY_NODE_MAX_LIST_ITEMS) {
            throw new Error(`WhatsApp binary node list items exceed ${WHATSAPP_BINARY_NODE_MAX_LIST_ITEMS}.`);
        }
        return size;
    };
    ...
    const readDoubleToken = (dictionary, index) => {
...
function encodeBinaryNodeInner(node, buffer, depth) {
    if (depth > WHATSAPP_BINARY_NODE_MAX_DEPTH) {
        throw new Error(`WhatsApp binary node nesting exceeds ${WHATSAPP_BINARY_NODE_MAX_DEPTH}.`);
    }
    if (!node.tag) {
...
function pushByte(buffer, value) {
...
function pushInt(buffer, value, length) {
    for (let index = 0; index < length; index += 1) {
        const shift = length - 1 - index;
        pushByte(buffer, value >> (shift * 8));
    }
}
function writeByteLength(buffer, length) {
...
function writeListStart(buffer, size) {
