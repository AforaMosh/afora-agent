...
export class WebhookDnsLookupPool {
    ...
    #pending = [];
    #active = 0;
    constructor(maxConcurrent, lookupHostname = async (hostname) => await lookup(hostname, { all: true, verbatim: true })) {
...
function createGlobalIpv4Exceptions() {
    const blockList = new BlockList();
...
function createGlobalIpv6Exceptions() {
    const blockList = new BlockList();
...
function createAllocatedGlobalIpv6Ranges() {
    const blockList = new BlockList();
...
function embeddedNat64Ipv4(address) {
...
function isBlockedWebhookAddress(address) {
    const normalized = normalizeHostname(address);
    const family = isIP(normalized);
    if (family === 4) {
        return (!GLOBAL_IPV4_EXCEPTIONS.check(normalized, "ipv4") &&
...
async function resolveWebhookAddresses(hostname, signal) {
    const normalized = normalizeHostname(hostname);
    const family = isIP(normalized);
    if (family === 4 || family === 6) {
        return [{ address: normalized, family }];
    }
    return (await webhookDnsLookupPool.resolve(normalized, signal)).flatMap((entry) => entry.family === 4 || entry.family === 6
        ? [{ address: entry.address, family: entry.family }]
        : []);
}
export async function validateWebhookTarget(params) {
    const allowThisLoopbackHttp = params.url.protocol === "http:" &&
...
export async function postWebhookRequest(params) {
    return await new Promise((resolve, reject) => {
        ...
        deadline = setTimeout(() => {
            const error = new DOMException(`Webhook delivery timed out after ${params.timeoutMs}ms`, "TimeoutError");
