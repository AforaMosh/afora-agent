...
function postEvent(event, post, senderName, channelType) {
    return {
        event,
        data: {
...
function handleAdminInbound(params) {
    ...
    const senderName = readTrimmedString(params.body.senderName) ?? previousUser?.username ?? senderId;
...
async function handleApi(params) {
    ...
    if (postMatch?.[1] && method === "PUT") {
        ...
        broadcast(state, postEvent("post_edited", updated, state.botUsername, state.channels.get(updated.channel_id)?.type ?? "O"), false);
    ...
    if (postMatch?.[1] && method === "DELETE") {
        ...
        broadcast(state, postEvent("post_deleted", post, state.botUsername, state.channels.get(post.channel_id)?.type ?? "O"), false);
        return jsonResponse({ status: "OK" });
    }
...
async function handleRequest(request, state) {
    ...
    const body = method === "GET" || method === "DELETE" ? {} : await parseUnknownRequestBody(request);
    const event = {
        at: new Date().toISOString(),
