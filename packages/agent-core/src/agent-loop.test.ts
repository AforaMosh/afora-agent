...
import { EventStream } from "@afora/ai/event-stream";
...
describe("agentLoop tool termination", () => {
  ...
  it("does not taint the recovery turn with an unexecuted network tool source", async () => {
    ...
    const readTaint = (message: unknown) =>
      (message as Record<string, unknown>)["__afora"] as
  ...
  it.each([
    ...
    async (toolExecution, failure) => {
      ...
      expect((toolResult as unknown as { __afora?: unknown })?.["__afora"]).toBeUndefined();
      expect((assistant as unknown as { __afora?: unknown })?.["__afora"]).toBeUndefined();
    },
  );
  ...
  it.each([
    ...
    async (toolExecution, failure, tainted) => {
      ...
      expect((toolResult as unknown as { __afora?: unknown })?.["__afora"]).toEqual(
        tainted ? { resultContentSource: "network" } : undefined,
      );
    },
  );
  ...
  it("does not start prepared parallel tools after the run aborts mid-batch", async () => {
    ...
    expect(
      abortedMessages
        ...
        .every((message) => !(message as unknown as { __afora?: unknown })["__afora"]),
    ).toBe(true);
