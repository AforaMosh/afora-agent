...
type PluginBlobTable = AforaStateKyselyDatabase["plugin_blob_entries"];
type PluginBlobDatabase = Pick<AforaStateKyselyDatabase, "plugin_blob_entries">;
...
function createError(params: {
  ...
  return new PluginBlobStoreError(params.message, {
    ...
    path: resolveAforaStateSqlitePath(params.env ?? process.env),
    cause: params.cause,
  });
}
...
function openDatabase(operation: PluginBlobStoreOperation, env?: NodeJS.ProcessEnv) {
  try {
    const database = openAforaStateDatabase(env ? { env } : {});
    return database;
  } catch (error) {
    throw wrapError(
      error,
