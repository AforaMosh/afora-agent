...
describe("runtime smoke", () => {
  it("writes and reads a value", async () => {
    await withAforaTestState({ label: "e2e-smoke-rw" }, async () => {
  ...
  it("consumes a value exactly once", async () => {
    await withAforaTestState({ label: "e2e-smoke-consume" }, async () => {
...
describe("persistence", () => {
  it("survives close and reopen of the store", async () => {
    await withAforaTestState({ label: "e2e-persist" }, async () => {
...
describe("TTL", () => {
  it("hides expired values and sweep removes the row", async () => {
    await withAforaTestState({ label: "e2e-ttl" }, async () => {
      vi.useFakeTimers();
...
describe("isolation", () => {
  it("segregates plugins sharing namespace and key", async () => {
    await withAforaTestState({ label: "e2e-isolation" }, async () => {
...
describe("limits", () => {
  it("accepts a value at the 64 KB boundary", async () => {
    await withAforaTestState({ label: "e2e-limit-accept" }, async () => {
  ...
  it("rejects a value one byte over 64 KB", async () => {
    await withAforaTestState({ label: "e2e-limit-reject" }, async () => {
      const store = createPluginStateKeyedStore<string>("fixture-plugin", {
        namespace: "size",
        maxEntries: 10,
      });
  ...
  it("evicts oldest entries when namespace maxEntries is exceeded", async () => {
    await withAforaTestState({ label: "e2e-limit-eviction" }, async () => {
      vi.useFakeTimers();
...
describe("failure safety", () => {
  it("probe returns redacted diagnostics without leaking stored values", async () => {
    await withAforaTestState({ label: "e2e-fail-probe" }, async () => {
      const result = probePluginStateStore();
      expect(result.ok).toBe(true);
      expect(result.databasePath).toContain("afora.sqlite");
  ...
  it("close and reopen cycle is clean", async () => {
    await withAforaTestState({ label: "e2e-fail-reopen" }, async () => {
      const store = createPluginStateKeyedStore<{ v: number }>("fixture-plugin", {
        namespace: "reopen",
        maxEntries: 10,
      });
