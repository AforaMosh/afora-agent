...
export async function finishGatewayStartup(params: {
  ...
  const databaseIntegrityResident = residentRegistry.register({
    ...
    start: async () => {
      ...
      const { startAforaDatabaseIntegrityVerifier } =
        await import("../state/afora-database-verify.js");
      databaseVerifierHandle = startAforaDatabaseIntegrityVerifier({ env: process.env });
