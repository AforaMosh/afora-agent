...
const LEGACY_AFORA_ENV_NOTE =
  " Legacy CLAWDBOT_* and MOLTBOT_* environment variables are ignored; use AFORA_* names.";
...
export function assertGatewayAuthConfigured(
  ...
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      `gateway auth mode is token, but no token was configured (set gateway.auth.token or AFORA_GATEWAY_TOKEN).${LEGACY_AFORA_ENV_NOTE}`,
    );
  }
  if (auth.mode === "password" && !auth.password) {
    if (
      ...
      throw new Error(
        "gateway auth mode is password, but gateway.auth.password contains a provider reference object instead of a resolved string — bootstrap secrets (gateway.auth.password) must be plaintext strings or set via the AFORA_GATEWAY_PASSWORD environment variable because the secrets provider system has not initialised yet at gateway startup", // pragma: allowlist secret
      );
    }
    throw new Error(
      `gateway auth mode is password, but no password was configured.${LEGACY_AFORA_ENV_NOTE}`,
    );
  }
  if (auth.mode === "trusted-proxy") {
    ...
    if (auth.token) {
      throw new Error(
        "gateway auth mode is trusted-proxy, but a shared token is also configured; remove gateway.auth.token / AFORA_GATEWAY_TOKEN because trusted-proxy and token auth are mutually exclusive",
      );
    }
  }
}
