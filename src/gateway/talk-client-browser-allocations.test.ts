...
function createAllocation(
  ...
  return {
    ...
    params: {
      ...
      durableCreated: overrides.durableCreated ?? true,
...
describe("Talk client browser allocations", () => {
  ...
  it("cancels an allocation that reaches a connection after close wins", async () => {
  ...
  it("closes only newly-created durable state when the initial candidate aborts", async () => {
    ...
    const resumed = createAllocation({ voiceSessionId: "voice-resumed", durableCreated: false });
  ...
  it("retains a released tombstone for exact durable-close retry without recancelling", async () => {
    ...
    await expect(closeBrowserAllocation(identity(allocation))).resolves.toBe(true);
    expect(entry.cancel).toHaveBeenCalledOnce();
  ...
  it("preserves resumed durable state only until an allocation has committed", async () => {
    const activeEntry = createAllocation({ durableCreated: false });
    const candidateEntry = createAllocation({ durableCreated: false, connId: "conn-2" });
