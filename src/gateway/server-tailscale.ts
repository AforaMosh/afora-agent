    ...
    if (preservedFunnel) {
      params.logTailscale.warn(
        `external Tailscale Funnel for port ${params.port} remains active only for plugin-authenticated webhook routes; Gateway-authenticated routes reject its unattributable ingress. ` +
          "First configure a durable gateway password (gateway.auth.password or AFORA_GATEWAY_PASSWORD) and set gateway.auth.mode=password, " +
          "then run `afora config set gateway.tailscale.mode funnel` and `afora config unset gateway.tailscale.preserveFunnel`; " +
          "see https://docs.afora.ai/gateway/tailscale#public-internet-funnel--shared-password",
      );
      return null;
    }
  }
