...
function hasGatewayHistoryMessageToolMirror(message: unknown) {
  return Boolean(
    ...
    (message as { aforaMessageToolMirror?: unknown }).aforaMessageToolMirror,
  );
}
...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gw-"));
    try {
  ...
  test("sessions.send accepts dashboard messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-sessions-send-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.steer accepts dashboard follow-up messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-sessions-steer-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort stops active dashboard runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-sessions-abort-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort resolves active runs by runId without a caller session key", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-sessions-abort-runid-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gw-"));
  ...
  test("chat.history hides raw delivery-mirror rows but keeps message-tool mirrors", async () => {
    ...
    expect(historyMessages).not.toContainEqual(
      expect.objectContaining({ provider: "afora", model: "delivery-mirror" }),
    );
  });
