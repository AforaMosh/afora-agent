// Focused welcome-delivery tests for afora.chat: caretaker greeting wiring,
...
describe("afora.chat caretaker welcome", () => {
  ...
  it("does not plan a greeting when a fresh session is created with a message", async () => {
    ...
    greetingMocks.resolveSystemAgentGreeting.mockResolvedValueOnce({
      text: "Hi, I'm Afora — caretaker of this gateway, config, channels, and agents.",
      source: "template",
    });
    ...
    expect(welcome.payload).toMatchObject({
      reply: "Hi, I'm Afora — caretaker of this gateway, config, channels, and agents.",
    });
    expect(greetingMocks.acknowledgeSystemAgentGreetingDelivery).toHaveBeenCalledWith({
      auditSequence: 0,
    });
  ...
  it("does not acknowledge audit entries when greeting delivery fails", async () => {
    ...
    expect(retry.payload).toMatchObject({ reply: "I'm Afora. All systems nominal." });
    expect(greetingMocks.acknowledgeSystemAgentGreetingDelivery).toHaveBeenCalledWith({
      auditSequence: 42,
    });
