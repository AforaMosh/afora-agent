...
function getVoiceCallRealtimeConfig(config: AforaConfig): {
  provider?: string;
...
function getVoiceCallStreamingConfig(config: AforaConfig): {
  provider?: string;
...
type RealtimeProviderWithConfig<TConfig extends Record<string, unknown>> = VoiceModelProvider & {
  resolveConfig?: (ctx: { cfg: AforaConfig; rawConfig: TConfig }) => TConfig;
  isConfigured: (ctx: { cfg: AforaConfig; providerConfig: TConfig }) => boolean;
};
...
export function buildTalkRealtimeConfig(config: AforaConfig, requestedProvider?: string) {
...
export function buildTalkTranscriptionConfig(config: AforaConfig, requestedProvider?: string) {
...
const DEFAULT_REALTIME_INSTRUCTIONS = [
  "You are Afora's realtime voice interface. Keep spoken replies concise.",
  `If the user asks for code, repository state, files, current Afora context, tool-backed actions, or deeper reasoning, call ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and then summarize the result naturally.`,
  `Do not claim you cannot use tools, perform actions, or reach Afora unless ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} returns that failure.`,
  `When ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} is in progress, speak one brief acknowledgement such as "Let me check that for you", then wait for the final Afora result before answering with the actual result.`,
  `If Afora is already working through ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and the user asks in any language for progress, cancellation, a redirect/change, or a follow-up, call ${REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME} with the semantic mode.`,
  "For greetings and casual chatter while Afora is working, answer naturally and do not redirect the active work.",
].join(" ");
