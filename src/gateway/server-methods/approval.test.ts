...
type OperatorApprovalDatabase = Pick<AforaStateKyselyDatabase, "operator_approvals">;
const managersForCleanup: Array<{
  listPendingRecords(): Array<{ id: string }>;
  expire(id: string, resolvedBy?: string | null): boolean;
}> = [];

function createDatabaseOptions(): AforaStateDatabaseOptions {
  const stateDir = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "afora-approval-handler-")),
  );
  tempDirs.push(stateDir);
  return { env: { ...process.env, AFORA_STATE_DIR: stateDir } };
}

function createManagers(databaseOptions: AforaStateDatabaseOptions) {
...
function deleteDurableApproval(databaseOptions: AforaStateDatabaseOptions, id: string): void {
...
describe("unified approval handlers", () => {
  ...
  it.each([
    ...
    const databasePath = fs.mkdtempSync(path.join(os.tmpdir(), "afora-unsafe-approval-id-"));
  ...
  it.each(["approval.get", "approval.resolve"] as const)(
    ...
    async (method) => {
      const databasePath = fs.mkdtempSync(path.join(os.tmpdir(), "afora-approval-broken-db-"));
      tempDirs.push(databasePath);
      const context = createContext();
      const handlers = createApprovalHandlers({
        execApprovalManager: new ExecApprovalManager(),
  ...
  it("repairs durable pending state after a transient local storage failure", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-approval-reconcile-"));
    tempDirs.push(stateDir);
    const databasePath = path.join(stateDir, "state.sqlite");
    const backupPath = path.join(stateDir, "state.backup.sqlite");
    const databaseOptions = { path: databasePath } satisfies AforaStateDatabaseOptions;
    const managers = createManagers(databaseOptions);
