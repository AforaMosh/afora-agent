// Node invocation methods authorize commands, wake disconnected nodes, route
// plugin policies, dispatch work, and translate node results for RPC callers.
...
const TALK_PTT_COMMAND_SET = new Set(TALK_PTT_COMMANDS);
...
function emitTalkPttNodeEvent(params: {
  ...
  if (!TALK_PTT_COMMAND_SET.has(params.command)) {
    return;
  }
  const payloadObj =
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          respondUnavailableOnNodeInvokeError(respond, res);
          return;
        }
