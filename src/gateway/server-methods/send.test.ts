...
const TEST_AGENT_WORKSPACE = "/tmp/afora-test-workspace";
...
async function withTempAforaStateDir<T>(test: (stateDir: string) => Promise<T>): Promise<T> {
...
describe("gateway send mirroring", () => {
  ...
  it("materializes buffer-only gateway sends before outbound delivery", async () => {
    ...
    await withTempAforaStateDir(async () => {
      const { respond } = await runSend({
        to: "+15551234567",
  ...
  it("maps gateway asVoice sends onto outbound audioAsVoice payloads", async () => {
    ...
    expect(deliveryCall()?.payloads?.[0]?.mediaUrl).toBe("file:///tmp/afora-voice.ogg");
