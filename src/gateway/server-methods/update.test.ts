...
>(async (params) => ({
  status: "started",
  pid: 12345,
  command: "afora update --yes --timeout 1800",
  logPath: "/tmp/afora-update-run-handoff/handoff.log",
...
vi.mock("../../infra/afora-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/afora-root.js")>(
    "../../infra/afora-root.js",
  );
  return {
    ...actual,
    resolveAforaPackageRoot: async () => "/tmp/afora",
  };
});
...
vi.mock("../../infra/update-managed-service-handoff.js", () => ({
  ...
  buildManagedServiceHandoffUnavailableMessage: (command: string) =>
    [
      "Afora updates cannot safely run inside the live gateway process without a managed-service handoff.",
...
beforeEach(() => {
  ...
  readConfigFileSnapshotMock.mockResolvedValue({
    ...
    raw: "{}",
    parsed: {},
    resolved: {} as AforaConfig,
    sourceConfig: {} as AforaConfig,
    valid: true,
...
async function invokeUpdateRun(
  ...
  runtimeConfig: AforaConfig = { update: {} },
) {
...
function mockGlobalInstallSurface() {
  resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
    kind: "global",
    mode: "npm",
    root: "/tmp/afora-global",
    packageRoot: "/tmp/afora-global",
  });
}
...
describe("update.run restart scheduling", () => {
  ...
  it("hands managed package updates to the CLI path instead of running them in-process", async () => {
    ...
    const payload = await withProcessEnv({ AFORA_LAUNCHD_LABEL: "ai.afora.gateway" }, () =>
  ...
  it("keeps a startup grace before restarting after systemd handoff spawn", async () => {
    ...
    await withProcessEnv({ AFORA_SYSTEMD_UNIT: "afora-gateway.service" }, () =>
  ...
  it("starts managed package handoff when the gateway cwd is unavailable", async () => {
    ...
    try {
      await withProcessEnv({ AFORA_LAUNCHD_LABEL: "ai.afora.gateway" }, () =>
        invokeUpdateRun({}),
      );
    } finally {
      cwdSpy.mockRestore();
  ...
  it("hands supervised git/dev updates to the CLI path instead of rebuilding live dist in-process", async () => {
    detectRespawnSupervisorMock.mockReturnValueOnce("launchd");
    mockGitInstallSurface("/tmp/afora-git");
    const payload = await withProcessEnv({ AFORA_LAUNCHD_LABEL: "ai.afora.gateway" }, () =>
      captureUpdateRunPayload(),
    );
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      message:
        "Afora updates cannot safely run inside the live gateway process without a managed-service handoff.\n" +
        "Run `afora update --yes --timeout 1800` from a shell outside the gateway service, or restart/update from the host UI.",
    });
  });
  ...
  it("delegates update.run without mutating or restarting under external supervision", async () => {
    ...
    const payload = await withProcessEnv({ AFORA_SUPERVISOR_MODE: "external" }, () =>
      captureUpdateRunPayload(),
    );
...
describe("update.run post-core plugin finalize", () => {
  ...
  it("resumes official plugin convergence after a git/source core update", async () => {
    runPostCoreFinalizeAfterGatewayUpdateMock.mockResolvedValueOnce({
      status: "ok",
      entrypoint: "/tmp/afora-git/dist/index.mjs",
    });
    mockGitOkUpdate("/tmp/afora-git");
  ...
  it("carries the pre-doctor source config into the git finalizer", async () => {
    ...
    const [finalizeParams] = firstMockCall(
      runPostCoreFinalizeAfterGatewayUpdateMock,
      "post-core finalize",
    ) as [{ preUpdateConfig?: { sourceConfig?: AforaConfig; authoredConfig?: AforaConfig } }];
