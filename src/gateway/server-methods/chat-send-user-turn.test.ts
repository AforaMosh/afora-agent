...
describe("prepareChatSendUserTurn", () => {
  ...
  it.each([
    ...
    expect(
      ((persisted as unknown as Record<string, unknown>)["__afora"] as { media?: unknown })
        .media,
    ).toEqual(input.media);
  });
  ...
  it("persists and prunes the managed PDF claim as structured ownership", async () => {
    ...
    expect((first?.["__afora"] as Record<string, unknown> | undefined)?.media).toBeUndefined();
  });
  ...
  it("hydrates and prunes a staged image claim-check alias as structured ownership", async () => {
    ...
    try {
      ...
      expect(
        (
          (persisted as unknown as Record<string, unknown>)["__afora"] as {
            media?: unknown;
          }
        ).media,
      ).toEqual([
