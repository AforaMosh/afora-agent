...
function expectAgentStoreAbsent(agentId: string): void {
  const env = { AFORA_STATE_DIR: requireStateDir() };
  expect(fs.existsSync(path.join(env.AFORA_STATE_DIR, "agents", agentId))).toBe(false);
  expect(fs.existsSync(resolveAforaAgentSqlitePath({ agentId, env }))).toBe(false);
  expect(listAforaRegisteredAgentDatabases({ env }).map((entry) => entry.agentId)).not.toContain(
    agentId,
  );
}
...
test("agents.list includes system rows only when negotiated", async () => {
  fs.mkdirSync(path.join(requireStateDir(), "agents", "afora"), { recursive: true });

  expect(await listAgentIdsViaRpc()).toEqual(["main"]);
  expect(await listAgentIdsViaRpc(true)).toEqual(["main", "afora"]);
});
...
test("sessions.describe reads a pre-existing store after its agent is removed from config", async () => {
  ...
  await setAgentsConfig({ list: [{ id: "main", default: true }] });
  const registeredBefore = listAforaRegisteredAgentDatabases({
    env: { AFORA_STATE_DIR: requireStateDir() },
  });
