...
describe("plugin surface refresh", () => {
  ...
  it("refreshes generic plugin surface capability urls", async () => {
    ...
    await expectDefined(
      ...
      params: {
        surface: "canvas",
        observedUrl: "https://gateway.example/__afora__/cap/old-token",
      },
      client: client as never,
      isWebchatConnect: () => false,
      respond,
    ...
    expect(parsedCanvasUrl.pathname.startsWith("/__afora__/cap/")).toBe(true);
    const capabilityToken = parsedCanvasUrl.pathname.slice("/__afora__/cap/".length);
  ...
  it("reuses a capability rotated after the caller observed its surface", async () => {
    ...
    vi.setSystemTime(1_000);
    const respond = vi.fn();
    const currentUrl = "http://127.0.0.1:18789/__afora__/cap/current-token";
    const client = {
      connect: {
        client: { id: "node-1", mode: "node" },
      },
...
describe("node.invoke APNs wake path", () => {
  ...
  it("explains when a declared node command surface awaits approval", async () => {
    ...
    expect(call[0]).toBe(false);
    expect(call[2]?.message).toBe(
      "node command not allowed: the node's declared command surface is pending approval; run `afora nodes pending`, then `afora nodes approve <requestId>`",
    );
    expect(nodeRegistry.invoke).not.toHaveBeenCalled();
  });
