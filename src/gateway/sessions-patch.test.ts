  ...
  plugins: [
    {
      ...
      source: "test",
      manifestPath: "/plugins/byteplus/afora.plugin.json",
...
async function applySubagentModelPatch(cfg: AforaConfig) {
  return expectPatchOk(
    await runPatch({
      cfg,
...
function createAllowlistedAnthropicModelCfg(): AforaConfig {
  return {
    agents: {
      defaults: {
...
describe("gateway sessions patch", () => {
  ...
  test("persists Afora Luna Ultra through the runtime-aware provider profile", async () => {
    const entry = expectPatchOk(
      await runPatch({
        cfg: {
          agents: {
  ...
  test("honors an explicit Afora session runtime override for Luna Ultra", async () => {
    const entry = expectPatchOk(
      await runPatch({
        cfg: {
  ...
  test("treats the persisted harness id as observational when validating Luna Ultra", async () => {
    const result = await runPatch({
      ...
      store: mainStoreEntry({ agentHarnessId: "afora" }),
      patch: { key: MAIN_SESSION_KEY, thinkingLevel: "ultra" },
      loadGatewayModelCatalog: async () => [],
    });
