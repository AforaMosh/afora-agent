...
import { asNullableRecord as asLoopbackSchemaRecord } from "@afora/normalization-core/record-coerce";
...
async function callLoopbackJsonRpc(params: {
  ...
  if (params.messageProvider) {
    headers["x-afora-message-channel"] = params.messageProvider;
  }
  if (params.accountId) {
    headers["x-afora-account-id"] = params.accountId;
  }
  const timeoutMs = parsePositiveInt(
    params.env?.AFORA_MCP_LOOPBACK_PROBE_TIMEOUT_MS,
    CLI_CRON_MCP_LOOPBACK_REQUEST_TIMEOUT_MS,
    "AFORA_MCP_LOOPBACK_PROBE_TIMEOUT_MS",
  );
  const maxBodyBytes = parsePositiveInt(
    params.env?.AFORA_MCP_LOOPBACK_PROBE_MAX_BODY_BYTES,
    CLI_CRON_MCP_LOOPBACK_MAX_BODY_BYTES,
    "AFORA_MCP_LOOPBACK_PROBE_MAX_BODY_BYTES",
  );
