...
export function createGatewayConfigModuleMock(actual: GatewayConfigModule): GatewayConfigModule {
  const resolveConfigPath = () => path.join(testConfigRoot.value, "afora.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "afora-gateway-test"),
      ...fileDefaults,
    ...
    return migratePersistedImplicitMainRoster(composed).config as AforaConfig;
  };
  ...
  return {
    ...
    applyConfigOverrides: (cfg: AforaConfig) =>
