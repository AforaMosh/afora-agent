...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.AFORA_LIVE_CODEX_BIND);
const describeLive = LIVE && CODEX_BIND_LIVE ? describe : describe.skip;
const CODEX_BIND_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.AFORA_LIVE_CODEX_BIND_TIMEOUT_MS,
  900_000,
);
const CODEX_BIND_REQUEST_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.AFORA_LIVE_CODEX_BIND_REQUEST_TIMEOUT_MS,
  300_000,
);
...
async function writeGatewayConfig(params: {
  ...
  const usesApiKeyAuth =
    modelProvider === "openai" && process.env.AFORA_LIVE_CODEX_HARNESS_AUTH === "api-key";
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.AFORA_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.AFORA_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
...
describeLive("gateway live (native Codex conversation binding)", () => {
  it(
    ...
    async () => {
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-live-codex-bind-"));
      ...
      const bindModel =
        process.env.AFORA_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      if (process.env.AFORA_LIVE_CODEX_HARNESS_AUTH === "api-key") {
