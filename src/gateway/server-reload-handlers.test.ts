...
describe("gateway hot reload model state", () => {
  it("keeps a supervised on-exit child alive exactly once across same-store cron reload", async () => {
    ...
    try {
      ...
      const initialHandoff = await initialCronState.prepareExitWatcherHandoff?.();
      state = {
      ...
      const replacementHandoff = await state.cronState.prepareExitWatcherHandoff?.();
      expect(replacementHandoff?.current()).toBe(initialHandoff?.current());
