...
describe("formatGatewayAuthFailureMessage", () => {
  ...
  it("makes a missing Control UI token actionable within the WebSocket close limit", () => {
    ...
    expect(message).toBe(
      "unauthorized: gateway token missing (paste in Control UI settings or afora doctor --generate-gateway-token; restart)",
    );
  ...
  it("tells rejected node hosts how to diagnose identity-header auth", () => {
    expect(
      ...
      "gateway rejected this node: trusted-proxy identity-header authentication is required and no usable machine credential was accepted; run `afora doctor` on the Gateway",
    );
  });
