...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__afora_unknown_client_ip__";

function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.AFORA_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (isVitestRuntimeEnv(env) ? env.AFORA_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP : undefined);
  if (!configured) {
