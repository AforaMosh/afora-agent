...
import { clearRuntimeConfigSnapshot, type AforaConfig } from "../config/config.js";
...
import { loadAforaPlugins } from "../plugins/loader.js";
...
const CLI_LIVE = isTruthyEnvValue(process.env.AFORA_LIVE_CLI_BACKEND);
const CLI_CACHE_PROBE = isTruthyEnvValue(process.env.AFORA_LIVE_CLI_BACKEND_CACHE_PROBE);
const CLI_RESUME =
  CLI_CACHE_PROBE || isTruthyEnvValue(process.env.AFORA_LIVE_CLI_BACKEND_RESUME_PROBE);
const CLI_DEBUG = isTruthyEnvValue(process.env.AFORA_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.AFORA_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
const CLI_MCP_SCHEMA_PROBE = isTruthyEnvValue(
  process.env.AFORA_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE,
);
...
async function initializeCacheProbeGitWorkspace(workspaceDir: string): Promise<void> {
  ...
  await execFileAsync("git", ["-C", workspaceDir, "config", "user.name", "Afora Tests"]);
  await execFileAsync("git", [
    "-C",
    workspaceDir,
    "config",
    "user.email",
    "afora-tests@localhost",
  ]);
...
const CLI_BACKEND_REQUEST_TIMEOUT_MS = parsePositiveIntegerEnv(
  "AFORA_LIVE_CLI_BACKEND_REQUEST_TIMEOUT_MS",
  15 * 60_000,
);
...
function openAiProviderConfigForCodexCli(
  modelKey: string,
): NonNullable<NonNullable<AforaConfig["models"]>["providers"]>["openai"] {
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "AFORA_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.AFORA_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.AFORA_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      ...
      if (CLI_CACHE_PROBE && providerId !== "claude-cli") {
        throw new Error("AFORA_LIVE_CLI_BACKEND_CACHE_PROBE requires provider claude-cli");
      }
      ...
      const cliCommand = process.env.AFORA_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
      if (!cliCommand) {
        throw new Error(
          `AFORA_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
        );
      }
      ...
      const cliClearEnv =
        parseJsonStringArray(
          "AFORA_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.AFORA_LIVE_CLI_BACKEND_CLEAR_ENV,
        ) ??
      ...
      const cliImageArg =
        process.env.AFORA_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.AFORA_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        providerDefaults?.imageMode;
      if (cliImageMode && !cliImageArg) {
        throw new Error(
          "AFORA_LIVE_CLI_BACKEND_IMAGE_MODE requires AFORA_LIVE_CLI_BACKEND_IMAGE_ARG.",
        );
      }
      if (!backendResolved || !providerDefaults) {
        throw new Error(`missing CLI backend metadata for ${providerId}`);
      }
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-live-cli-"));
      ...
      const disableMcpConfig = process.env.AFORA_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
      ...
      if (CLI_CACHE_PROBE) {
        ...
        cacheProbeBackend = createFreshProcessCacheProbeBackend({
          ...
          ...(registration.builtWithAforaVersion
            ? { builtWithAforaVersion: registration.builtWithAforaVersion }
            : {}),
        });
      }
        ...
        if (modelSwitchTarget) {
          ...
          if (resumeContinuityProbe) {
            ...
            if (!continuitySessionId) {
              throw new Error("Claude CLI continuity probe could not resolve its Afora session");
            }
            continuityOwner = {
