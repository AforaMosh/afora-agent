...
const CODEX_HARNESS_LIVE = process.env.AFORA_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.AFORA_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["AFORA_TRAJECTORY", "AFORA_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-afora-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.AFORA_TRAJECTORY = "1";
      process.env.AFORA_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".afora", "trajectory-exports", "bundle");
