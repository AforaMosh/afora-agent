...
const ICON_ROUTES = [
  { label: "plugin", pathname: "/__afora__/plugin-icon/firecrawl" },
  {
    label: "catalog",
    pathname: `/__afora__/catalog-icon/${encodeURIComponent(CATALOG_ICON_URL)}`,
  },
] as const;
const INVALID_ICON_ROUTES = [
  { label: "blank plugin id", pathname: "/__afora__/plugin-icon/" },
  { label: "invalid plugin id", pathname: "/__afora__/plugin-icon/%20" },
  { label: "malformed plugin id", pathname: "/__afora__/plugin-icon/%zz" },
  { label: "nested plugin id", pathname: "/__afora__/plugin-icon/one/two" },
  { label: "blank catalog URL", pathname: "/__afora__/catalog-icon/" },
  { label: "malformed catalog URL", pathname: "/__afora__/catalog-icon/%zz" },
  { label: "nested catalog URL", pathname: "/__afora__/catalog-icon/one/two" },
] as const;
...
describe("Control UI plugin and catalog icon routes", () => {
  ...
  it("resolves by plugin identity and ignores arbitrary remote URL parameters", async () => {
    const response = await request(
      "/__afora__/plugin-icon/firecrawl?url=http%3A%2F%2F127.0.0.1%2Fsecret",
    );
  ...
  it("resolves encoded catalog URLs through the server-owned allowlist", async () => {
    ...
    const response = await request(`/__afora__/catalog-icon/${encodeURIComponent(iconUrl)}`);
  ...
  it("does not fetch catalog URLs rejected by the server-owned allowlist", async () => {
    ...
    const response = await request(
      `/__afora__/catalog-icon/${encodeURIComponent("https://untrusted.example/icon.png")}`,
    );
  ...
  it("serves SVG only as a sandboxed attachment for browser-side rasterization", async () => {
    ...
    const response = await request("/__afora__/plugin-icon/simple-icons");
  ...
  it("reuses successful icon bytes from the bounded process cache", async () => {
    configForRequest = () => ({});
    const first = await request("/__afora__/plugin-icon/firecrawl");
    const second = await request("/__afora__/plugin-icon/firecrawl");
  ...
  it("accepts one canonical scoped plugin id encoded as a single path segment", async () => {
    const response = await request(
      `/__afora__/plugin-icon/${encodeURIComponent("@expediagroup/expedia-afora")}`,
    );
  ...
  it("refreshes cached icon bytes after the cache lifetime", async () => {
    ...
    try {
      const first = await request("/__afora__/plugin-icon/firecrawl");
      const cached = await request("/__afora__/plugin-icon/firecrawl");
      now.mockReturnValue(1_000 + PLUGIN_ICON_CACHE_TTL_MS + 1);
      const refreshed = await request("/__afora__/plugin-icon/firecrawl");
  ...
  it("returns not found when metadata is absent or remote image validation fails", async () => {
    mocks.resolveIconUrl.mockResolvedValueOnce(undefined);
    const missing = await request("/__afora__/plugin-icon/missing");
    ...
    const invalid = await request("/__afora__/plugin-icon/not-an-image");
    ...
    const mislabeled = await request("/__afora__/plugin-icon/mislabeled");
    ...
    const oversized = await request("/__afora__/plugin-icon/oversized");
    expect(oversized.status).toBe(404);

    mocks.readRemoteMediaBuffer.mockRejectedValueOnce(new Error("upstream failed"));
    const failed = await request("/__afora__/plugin-icon/broken");
    ...
    try {
      ...
      for (const { pathname } of ICON_ROUTES) {
        for (const method of ["GET", "HEAD"]) {
          const response = await fetch(`http://127.0.0.1:${handledPort}/afora${pathname}`, {
            headers: { Authorization: "Bearer test-token" },
            method,
          });
          expect(response.status).toBe(200);
