  ...
  const cfg = retainLegacyDefaultAgentId(
    {
      ...
      session: { mainKey: "work", store: "/tmp/afora-fixed-sessions.json" },
    },
    "ops",
  );
...
function createSingleAgentAvatarConfig(workspace: string): AforaConfig {
  return {
...
describe("gateway session utils", () => {
  ...
  test("session list search includes direct-session origin display labels", () => {
    ...
    const listed = listSessionsFromStore({
      ...
      storePath: "",
      store,
      opts: { search: "afora-tui" },
    });

    expect(listed.sessions.map((session) => session.key)).toEqual([
      "agent:main:telegram:direct:42",
    ]);
    expect(listed.sessions[0]?.displayName).toBe("afora-tui");
  });
  ...
  test("preserves persisted Ultra while projecting picker levels without a catalog", () => {
    providerArtifactMocks.resolveBundledProviderPolicySurface.mockReturnValue({
      resolveThinkingProfile: ({ modelId, agentRuntime }) => ({
        levels: [
          ...
          (agentRuntime === "afora" || !modelId.startsWith("gpt-5.6-luna"))
            ? [{ id: "ultra" as const }]
            : []),
        ],
      }),
    ...
    const aforaOverride = row({
      sessionId: "afora",
      thinkingLevel: "ultra",
      agentRuntimeOverride: "afora",
    } as SessionEntry);
    const legacyObservedAfora = row({
      sessionId: "legacy-observed-afora",
      thinkingLevel: "ultra",
    ...
    expect(aforaOverride.thinkingLevel).toBe("ultra");
    expect(aforaOverride.agentRuntime?.id).toBe("afora");
    expect(legacyObservedAfora.thinkingLevel).toBe("ultra");
    expect(legacyObservedAfora.agentRuntime?.id).toBe("codex");
    expect(legacyObservedAfora.thinkingLevels?.map((level) => level.id)).not.toContain("ultra");
  ...
  test("session rows expose estimated context budget status", () => {
    const row = buildGatewaySessionRow({
      ...
      entry: {
        sessionId: "session-1",
        sessionFile: "/tmp/afora/agents/main/sessions/session-1.jsonl",
        updatedAt: 1,
        contextBudgetStatus: {
          schemaVersion: 1,
  ...
  test("buildGatewaySessionRow displayName falls through to origin label for direct sessions", () => {
    ...
    expect(row.displayName).toBe("afora-tui");
  });
  ...
  test("refreshes a legacy Buzz UUID title from inbound room metadata", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-buzz-session-title-"));
    ...
    try {
      ...
      const row = buildGatewaySessionRow({
        cfg: { agents: { list: [{ id: "main", default: true }] } } as AforaConfig,
        storePath,
  ...
  test.each([
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-slack-session-title-"));
    const storePath = path.join(dir, "sessions.json");
  ...
  test("single-row child candidates reuse stable entry identities across sparse stores", () => {
    ...
    const storePath = "/tmp/afora-single-row-child-cache";
  ...
  test("loadSessionEntry rejects deleted main aliases when mainKey is customized", async () => {
    ...
    try {
      await withStateDirEnv("session-utils-load-deleted-main-alias-", async ({ stateDir }) => {
        ...
        expect(() => loadSessionEntry("agent:main:work")).toThrow("afora doctor --fix");
      });
    } finally {
      resetConfigRuntimeState();
    }
  ...
  test("listAgentsForGateway keeps explicit agents.list scope over disk-only agents (scope boundary)", async () => {
    await withStateDirEnv("afora-agent-list-scope-", async ({ stateDir }) => {
  ...
  test("listAgentsForGateway preserves canonical roster kinds", async () => {
    await withStateDirEnv("afora-agent-list-kinds-", async ({ stateDir }) => {
      fs.mkdirSync(path.join(stateDir, "agents", "afora"), { recursive: true });
  ...
  test("listAgentsForGateway keeps system agents out of the legacy response", async () => {
    await withStateDirEnv("afora-agent-list-legacy-", async ({ stateDir }) => {
      fs.mkdirSync(path.join(stateDir, "agents", "afora"), { recursive: true });
  ...
  test("listAgentsForGateway reports whether each workspace is a git checkout", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-agent-workspace-git-"));
...
describe("listSessionsFromStore selected model display", () => {
  test("async list yields during bulk transcript title and last-message hydration", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sessions-list-yield-"));
    try {
  ...
  test("caps transcript title and last-message hydration for bulk list responses", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sessions-list-cap-"));
    try {
      const storePath = path.join(tmpDir, "sessions.json");
      const store: Record<string, SessionEntry> = {};
      const now = Date.now();
