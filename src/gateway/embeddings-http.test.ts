...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    expect(qualifiedJson.model).toBe("afora/default");
  ...
  it("rejects explicit unknown agent ids", async () => {
    try {
      ...
      const header = await postEmbeddings(
        { model: "afora/default", input: "hello" },
        { "x-afora-agent-id": "missing-agent" },
      );
      await expectInvalidEmbeddingRequest(header, "Unknown agent 'missing-agent'.");

      const model = await postEmbeddings({ model: "afora/missing-agent", input: "hello" });
  ...
  it("rejects invalid agent targets", async () => {
    ...
    await expectInvalidEmbeddingRequest(res, "Invalid `model`. Use `afora` or `afora/<agentId>`.");
  });
  ...
  it("aborts provider work when the HTTP client disconnects", async () => {
    ...
    const body = JSON.stringify({ model: "afora/default", input: "hello" });
  ...
  it("retains failed cleanup and blocks replacement until retirement succeeds", async () => {
    ...
    const first = await postEmbeddings({ model: "afora/default", input: "first" });
    expect(first.status).toBe(200);
    expect(createEmbeddingProviderMock).toHaveBeenCalledTimes(createsBefore + 1);

    const blocked = await postEmbeddings({ model: "afora/default", input: "blocked" });
    expect(blocked.status).toBe(500);
    expect(createEmbeddingProviderMock).toHaveBeenCalledTimes(createsBefore + 1);

    const recovered = await postEmbeddings({ model: "afora/default", input: "recovered" });
  ...
  it("does not admit a replacement while provider cleanup is pending", async () => {
    ...
    const firstPromise = postEmbeddings({ model: "afora/default", input: "first" });
    await vi.waitFor(() =>
      expect(closeEmbeddingProviderMock).toHaveBeenCalledTimes(closesBefore + 1),
    );
    const secondPromise = postEmbeddings({ model: "afora/default", input: "second" });
  ...
  it("does not create a provider for a disconnected request waiting behind cleanup", async () => {
    ...
    try {
      ...
      const body = JSON.stringify({ model: "afora/default", input: "second" });
      ...
      const next = await postEmbeddings({ model: "afora/default", input: "next" });
  ...
  it("does not bypass local cleanup with a model override", async () => {
    ...
    const firstPromise = postEmbeddings(
      { model: "afora/default", input: "first" },
    ...
    const secondPromise = postEmbeddings(
      { model: "afora/default", input: "second" },
  ...
  it("allows providers without cleanup resources to embed concurrently", async () => {
    ...
    const second = await postEmbeddings({ model: "afora/default", input: "second" });
    expect(second.status).toBe(200);
  ...
  it("drains retained provider cleanup during gateway shutdown", async () => {
    ...
    const res = await postEmbeddings({ model: "afora/default", input: "hello" });
    expect(res.status).toBe(200);
