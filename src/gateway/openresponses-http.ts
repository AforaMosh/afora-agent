...
export async function handleOpenResponsesHttpRequest(
  ...
  const finalizeFailedResponse = (response: ResponseResource, emitErrorLifecycle = false) => {
    if (closed) {
      return;
    }
    closed = true;
    ...
    if (emitErrorLifecycle) {
      // Detach the stream before publishing its global failure so lifecycle
      // subscribers cannot reenter or overwrite the protocol's terminal error.
      emitAgentEvent({
        runId: responseId,
        stream: "lifecycle",
        data: { phase: "error" },
  ...
  void (async () => {
    try {
      ...
      if (isClientToolNameConflictError(err)) {
        ...
        finalizeFailedResponse(errorResponse, true);
        return;
      }
      const errorResponse = createResponseResource({
        id: responseId,
      ...
      if (mapped) {
        ...
        rememberResponseSession();
        finalizeFailedResponse(mappedResponse, true);
        return;
      }
      rememberResponseSession();
      finalizeFailedResponse(errorResponse, true);
