...
describe("handleControlUiHttpRequest", () => {
  ...
  async function withControlUiRoot<T>(
    fn: (tmp: string) => Promise<T>,
    options: { indexHtml?: string } = {},
  ) {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-ui-"));
    try {
      await fs.writeFile(path.join(tmp, "index.html"), options.indexHtml ?? "<html></html>\n");
      return await fn(tmp);
    } finally {
  ...
  async function withAllowedAssistantMediaRoot<T>(
    prefix: string,
    fn: (tmpRoot: string) => Promise<T>,
  ) {
    const tmpRoot = await fs.mkdtemp(path.join(resolvePreferredOpenClawTmpDir(), prefix));
    try {
  ...
  async function withBasePathRootFixture<T>(
    siblingDir: string,
        ...
        if (options.issuerGeneration) {
          const issued = await ensureDeviceToken({
            ...
            issuer: {
              ...
              generation: options.issuerGeneration,
            },
          });
  ...
  it("serves assistant local media through the control ui media route", async () => {
    await withAllowedAssistantMediaRoot("ui-media-", async (tmpRoot) => {
  ...
  it.each([
    ...
    await withAllowedAssistantMediaRoot("ui-media-disposition-", async (tmpRoot) => {
  ...
  it("encodes Unicode and RFC 8187 delimiter characters in assistant media filenames", async () => {
    await withAllowedAssistantMediaRoot("ui-media-filename-", async (tmpRoot) => {
  ...
  it("caps long assistant media filenames in content disposition", async () => {
    await withAllowedAssistantMediaRoot("ui-media-filename-long-", async (tmpRoot) => {
  ...
  it("caps assistant media filenames without splitting surrogate pairs", async () => {
    await withAllowedAssistantMediaRoot("ui-media-filename-surrogate-", async (tmpRoot) => {
  ...
  it("reports assistant local media availability metadata", async () => {
    await withAllowedAssistantMediaRoot("ui-media-meta-", async (tmpRoot) => {
      const filePath = path.join(tmpRoot, "photo.png");
      await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
      const { res, handled, end } = await runAssistantMediaRequest({
  ...
  it("reports assistant media metadata when the process clock is outside the Date range", async () => {
    ...
    try {
      await withAllowedAssistantMediaRoot("ui-media-bad-clock-", async (tmpRoot) => {
        const filePath = path.join(tmpRoot, "photo.png");
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__openclaw__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}&token=test-token`,
  ...
  it("serves assistant local media with a scoped media ticket after metadata auth", async () => {
    await withAllowedAssistantMediaRoot("ui-media-ticket-", async (tmpRoot) => {
      const filePath = path.join(tmpRoot, "photo.png");
      await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
  ...
  it("does not refresh assistant media tickets without operator auth", async () => {
    await withAllowedAssistantMediaRoot("ui-media-ticket-refresh-", async (tmpRoot) => {
      const filePath = path.join(tmpRoot, "photo.png");
      await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
      const meta = await runAssistantMediaRequest({
        url: `/__openclaw__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}`,
  ...
  it("rejects assistant local media with an invalid scoped media ticket", async () => {
    await withAllowedAssistantMediaRoot("ui-media-ticket-invalid-", async (tmpRoot) => {
      const filePath = path.join(tmpRoot, "photo.png");
      await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
      const { res, handled, end } = await runAssistantMediaRequest({
  ...
  it("rejects assistant local media without a valid auth token when auth is enabled", async () => {
    await withAllowedAssistantMediaRoot("ui-media-auth-", async (tmpRoot) => {
      const filePath = path.join(tmpRoot, "photo.png");
      await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
      const { res, handled, end } = await runAssistantMediaRequest({
  ...
  it("accepts paired operator device tokens on assistant media requests", async () => {
    await withPairedOperatorDeviceToken(async (operatorToken) => {
      await withAllowedAssistantMediaRoot("ui-media-device-token-", async (tmpRoot) => {
        const filePath = path.join(tmpRoot, "photo.png");
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled } = await runAssistantMediaRequest({
          url: `/__openclaw__/assistant-media?source=${encodeURIComponent(filePath)}`,
  ...
  it("accepts shared-gateway issuer tagged device tokens on assistant media requests", async () => {
    ...
    await withPairedOperatorDeviceToken(
      async (operatorToken) => {
        await withAllowedAssistantMediaRoot("ui-media-issued-device-token-", async (tmpRoot) => {
          const filePath = path.join(tmpRoot, "photo.png");
          await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
          const { res, handled } = await runAssistantMediaRequest({
            url: `/__openclaw__/assistant-media?source=${encodeURIComponent(filePath)}`,
      ...
      { issuerGeneration, browserMetadata: true },
    );
  });

  it("accepts paired operator device tokens in assistant media query auth", async () => {
    await withPairedOperatorDeviceToken(async (operatorToken) => {
      await withAllowedAssistantMediaRoot("ui-media-device-token-query-", async (tmpRoot) => {
        const filePath = path.join(tmpRoot, "photo.png");
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled } = await runAssistantMediaRequest({
  ...
  it("rejects trusted-proxy assistant media requests from disallowed browser origins", async () => {
    await withAllowedAssistantMediaRoot("ui-media-proxy-", async (tmpRoot) => {
      const filePath = path.join(tmpRoot, "photo.png");
      await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
  ...
  it("rejects trusted-proxy assistant media file reads without operator.read scope", async () => {
    await withAllowedAssistantMediaRoot("ui-media-scope-file-", async (tmpRoot) => {
      const filePath = path.join(tmpRoot, "photo.png");
      await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
      const { res, handled, end } = await runTrustedProxyAssistantMediaRequest({
        filePath,
  ...
  it("rejects trusted-proxy assistant media metadata requests with an empty scope set", async () => {
    await withAllowedAssistantMediaRoot("ui-media-scope-meta-", async (tmpRoot) => {
      const filePath = path.join(tmpRoot, "photo.png");
      await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
      const { res, handled, end } = await runTrustedProxyAssistantMediaRequest({
        filePath,
  ...
  it("rejects absolute-path escape attempts under basePath routes", async () => {
    await withBasePathRootFixture("ui-secrets", async ({ root, sibling }) => {
  ...
  it("rejects symlink escape attempts under basePath routes", async () => {
    await withBasePathRootFixture("outside", async ({ root, sibling }) => {
