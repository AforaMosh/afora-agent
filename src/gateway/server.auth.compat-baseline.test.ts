...
describe("gateway auth compatibility baseline", () => {
  describe("token mode", () => {
    ...
    beforeAll(async () => {
      previousCredential = process.env.AFORA_GATEWAY_TOKEN;
    ...
    test("keeps local backend device-token reconnects out of pairing", async () => {
      const identityPath = path.join(
        os.tmpdir(),
        `afora-backend-device-${process.pid}-${port}.sqlite`,
      );
  ...
  describe("none mode", () => {
    ...
    test("keeps auth-none control ui first-connect token absence unchanged", async () => {
      ...
      try {
        const deviceIdentityPath = path.join(
          os.tmpdir(),
          `afora-auth-none-control-ui-first-${process.pid}-${port}.sqlite`,
        );
        const res = await connectReq(ws, {
          skipDefaultAuth: true,
          client: { ...CONTROL_UI_CLIENT },
    ...
    test("keeps auth-none control ui stale-key token handoff unchanged", async () => {
      ...
      try {
        ...
        const identityPath = path.join(
          os.tmpdir(),
          `afora-auth-none-control-ui-${process.pid}-${port}.sqlite`,
        );
        const staleIdentityPath = path.join(
          os.tmpdir(),
          `afora-auth-none-control-ui-stale-${process.pid}-${port}.sqlite`,
        );
