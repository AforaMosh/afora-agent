...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.AFORA_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.AFORA_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: AforaConfig, rawSessionKey: string | undefined): string {
...
function resolveMcpSender(params: {
  ...
  const captureKey = normalizeOptionalString(getHeader(params.req, "x-afora-cli-capture-key"));
  const clientGrant =
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("AFORA_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}
...
export function resolveMcpCliCaptureKey(
  ...
  return normalizeOptionalString(getHeader(req, "x-afora-cli-capture-key"));
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    sessionId: normalizeOptionalString(getHeader(req, "x-afora-session-id")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-afora-message-channel")) ?? undefined,
    ...
    clientCaps: normalizeMcpClientCapsHeader(getHeader(req, "x-afora-client-caps")),
    currentChannelId: normalizeOptionalString(getHeader(req, "x-afora-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-afora-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-afora-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-afora-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-afora-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-afora-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-afora-source-reply-delivery-mode"),
    ),
    taskSuggestionDeliveryMode: normalizeMcpTaskSuggestionDeliveryMode(
      getHeader(req, "x-afora-task-suggestion-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-afora-require-explicit-message-target"),
    ),
    senderIsOwner: auth.senderIsOwner,
  };
}
