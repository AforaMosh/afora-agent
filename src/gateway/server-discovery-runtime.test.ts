...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "afora.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/afora.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (Afora)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/afora"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.AFORA_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.AFORA_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.AFORA_SSH_PORT = "2222";
  ...
  it("waits for delayed discovery when the configured timeout exceeds Node's timer range", async () => {
    useDevelopmentDiscoveryEnv();
    process.env.AFORA_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = "2147483648";
  ...
  it("skips local discovery services for truthy AFORA_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.AFORA_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "afora.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("afora.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (Afora)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (afora.internal. → /tmp/afora.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
