...
function computeRequestDigest(value: unknown): string {
  return sha256Base64Url(`afora.worker-session-tool-request.v1\0${JSON.stringify(value)}`);
}

function operationKey(operationSeed: string, purpose: string): string {
  return sha256Base64Url(`afora.worker-session-tool-operation.v1\0${operationSeed}\0${purpose}`);
}
