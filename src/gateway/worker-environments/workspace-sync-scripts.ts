...
git update-ref refs/heads/afora-worker "$base"
git symbolic-ref HEAD refs/heads/afora-worker
git read-tree "$base"
...
function eligiblePaths() {
  ...
  removeSelected(".afora-base.pack");
  ...
  for (const priorManifestDigest of priorManifestDigests) {
    ...
    const priorPath = path.join(process.env.HOME, ".afora-worker", "manifests", priorManifestDigest + ".json");
    ...
    for (const entry of prior.entries) {
      ...
      if (entry.path !== ".afora-base.pack" && !isDerivedWorkspacePath(entry.path)) {
...
async function main() {
  const workerRoot = path.join(process.env.HOME, ".afora-worker");
