...
describe("worker tunnel manager", () => {
  it("syncs a dirty workspace over pinned rsync and records an immutable manifest", async () => {
    ...
    const localPath = await fs.mkdtemp(path.join(os.tmpdir(), "afora-worker-sync-test-"));
    ...
    try {
      ...
      expect(transfer?.argv.at(-1)).toBe("worker@worker.example.test:afora-rsync-destination");
  ...
  it("moves a later fresh workspace transfer to an advertised fallback", async () => {
    ...
    const localPath = tempDirs.make("afora-worker-fallback-sync-");
  ...
  it("rejects an unrelated setup path before transfer", async () => {
    const unrelated = await fs.realpath(tempDirs.make("afora-worker-unrelated-"));
    const remoteRelative = [
      ".afora-worker/workspaces",
    ...
    const localPath = tempDirs.make("afora-worker-malformed-setup-");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = tempDirs.make("afora-worker-convergent-sync-");
      ...
      await fs.writeFile(
        fakeRsync,
        '#!/bin/sh\nset -eu\nprintf \'%s\\n\' "$$" > "$AFORA_TEST_RECEIVER_MARKER"\nread -r _ < "$AFORA_TEST_RECEIVER_GATE"\nprintf \'late stale write\\n\' > "$AFORA_TEST_RECEIVER_WORKSPACE/stale-late.txt"\n',
        { mode: 0o755 },
      );
        ...
        async (argv, localArgv, options, receiverTarget) => {
          ...
          receiverChild = spawn(localArgv[0]!, localArgv.slice(1), {
            env: {
              ...
              AFORA_TEST_RECEIVER_PATH: `${bin}:${process.env.PATH ?? ""}`,
              AFORA_TEST_RECEIVER_GATE: receiverGate,
              AFORA_TEST_RECEIVER_MARKER: receiverMarker,
              AFORA_TEST_RECEIVER_WORKSPACE: remoteWorkspaceDir,
            },
      ...
      try {
        ...
        const manifestDirectory = path.join(remoteHome, ".afora-worker/manifests");
        ...
        const rawManifest = await fs.readFile(
          path.join(remoteHome, ".afora-worker/manifests", `${digest}.json`),
          "utf8",
        );
        ...
        const residue = (await fs.readdir(path.dirname(result.remoteWorkspaceDir))).filter((name) =>
          name.startsWith(".afora-accepted-"),
        );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = tempDirs.make("afora-worker-retry-owner-");
      const localPath = path.join(root, "local");
      const remoteHome = path.join(root, "remote-home");
  ...
  it("does not downgrade an operational HEAD probe failure to plain sync", async () => {
    ...
    const localPath = await fs.mkdtemp(path.join(os.tmpdir(), "afora-worker-head-probe-"));
  ...
  it("does not downgrade an operational repository-root probe failure to plain sync", async () => {
    ...
    const localPath = await fs.mkdtemp(path.join(os.tmpdir(), "afora-worker-root-probe-"));
    await fs.mkdir(path.join(localPath, ".git"));
    const fake = fakeRunner((argv, options) => {
      if (argv.includes("--show-toplevel")) {
        return {
  ...
  it("materializes a large dirty git workspace as a credential-free commit-capable clone", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-worker-git-sync-"));
    const localPath = path.join(root, "local");
    const remoteHome = path.join(root, "remote-home");
    await Promise.all([
  ...
  it("mirrors plain workspaces and rejects escaping symlinks in a git overlay", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-worker-sync-modes-"));
