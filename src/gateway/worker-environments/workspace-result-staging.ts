...
// Match managed-worktree refs/afora/snapshots: deleting the owning ref is
// sufficient; unreachable objects may remain until normal Git GC.
const WORKER_RESULT_REF_PREFIX = "refs/afora/worker-results";
const WORKER_RESULT_CANDIDATE_REF_PREFIX = "refs/afora/worker-result-candidates";
const WORKER_RESULT_CLEANUP_REF_PREFIX = "refs/afora/worker-result-cleanup";
const WORKER_RESULT_CLAIM_ID_PATTERN = /^[A-Za-z0-9-]+$/u;
const STAGED_RESULT_MESSAGE = "Afora worker workspace result";
...
async function stageWorkerWorkspaceResult(params: {
  ...
  chunks.push(
    Buffer.from(
      `commit ${stagedResultRef}\nauthor Afora <afora@localhost> 0 +0000\ncommitter Afora <afora@localhost> 0 +0000\ndata ${message.byteLength}\n`,
    ),
    message,
    Buffer.from("\ndeleteall\n"),
  );
...
async function applyStagedWorkerWorkspaceResultWithMemo(
  ...
  const stagingRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-staged-result-"));
  try {
