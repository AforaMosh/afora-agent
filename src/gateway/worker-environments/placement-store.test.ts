...
describe("worker session placement store", () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(await fs.realpath(os.tmpdir()), "afora-placement-"));
  ...
  it("fences a completed worker result until manifest acceptance clears it", () => {
    ...
    const stagedResultRef = `refs/afora/worker-results/${claim.claimId}`;
    expect(() =>
      store.recordStagedWorkspaceResult(claim, "refs/afora/worker-results/unsafe.claim"),
    ...
    store.recordWorkspaceResultConflict(laterClaim, {
      ...
      stagedResultRef: `refs/afora/worker-results/${laterClaim.claimId}`,
    });
