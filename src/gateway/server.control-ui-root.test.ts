...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-ui-hardlink-"));
  try {
    const packageRoot = path.join(tmp, "pnpm-global", "5", "node_modules", "afora");
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global Afora package control-ui", async () => {
