...
const GATEWAY_ENV_TOKEN = "AFORA_GATEWAY_TOKEN";
const GATEWAY_SERVICE_KIND = "gateway";

// Doctor/startup warning shape for shells where AFORA_GATEWAY_TOKEN would
...
export function resolveGatewayAuthTokenSourceConflict(params: {
  ...
  const envToken = normalizeOptionalString(params.env.AFORA_GATEWAY_TOKEN);
  if (!envToken) {
    return null;
  }

  if (params.env.AFORA_SERVICE_KIND?.trim() === GATEWAY_SERVICE_KIND) {
  ...
  const remediation =
    `Remove ${GATEWAY_ENV_TOKEN} from the shell, ~/.afora/.env, or launchctl env if gateway.auth.token is intended, ` +
