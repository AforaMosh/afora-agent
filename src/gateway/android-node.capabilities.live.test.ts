...
const LIVE_ANDROID_NODE = isTruthyEnvValue(process.env.AFORA_LIVE_ANDROID_NODE);
...
const COMMAND_PROFILES: Record<string, CommandProfile> = {
  ...
  "contacts.search": {
    buildParams: () => ({ query: "__afora_live_no_match__", limit: 1 }),
    timeoutMs: 20_000,
    outcome: "success",
...
async function resolveGatewayConnection() {
  ...
  const urlOverride = normalizeNullableString(process.env.AFORA_ANDROID_GATEWAY_URL);
  const details = buildGatewayConnectionDetails({
    config: cfg,
    ...(urlOverride ? { url: urlOverride } : {}),
  });
  const tokenOverride = normalizeNullableString(process.env.AFORA_ANDROID_GATEWAY_TOKEN);
  const passwordOverride = normalizeNullableString(process.env.AFORA_ANDROID_GATEWAY_PASSWORD);
...
async function resolvePolicyConfigForRun(params: {
  ...
  loadLocalConfig?: () => AforaConfig | Promise<AforaConfig>;
}): Promise<AforaConfig> {
...
describe("resolvePolicyConfigForRun", () => {
  ...
  it("still uses local config loading for local loopback runs", async () => {
    const localConfig = { gateway: { bind: "127.0.0.1" } } as unknown as AforaConfig;
...
function selectTargetNode(nodes: NodeListNode[]): NodeListNode {
  const nodeIdOverride = normalizeNullableString(process.env.AFORA_ANDROID_NODE_ID);
  if (nodeIdOverride) {
    const match = nodes.find((node) => node.nodeId === nodeIdOverride);
    if (!match) {
      throw new Error(`AFORA_ANDROID_NODE_ID not found in node.list: ${nodeIdOverride}`);
    }
    return match;
  }

  const nodeNameOverride = normalizeNullableString(
    process.env.AFORA_ANDROID_NODE_NAME,
  )?.toLowerCase();
  if (nodeNameOverride) {
    ...
    if (!match) {
      throw new Error(`AFORA_ANDROID_NODE_NAME not found in node.list: ${nodeNameOverride}`);
    }
    return match;
  }
    ...
    if (!target.connected || !target.paired) {
      ...
      throw new Error(
        [
          ...
          pendingHint,
          "precondition: open app, keep foreground, ensure pairing approved (`afora nodes pending` / `afora nodes approve <requestId>`)",
        ].join("\n"),
      );
    }
