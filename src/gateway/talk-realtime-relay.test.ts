...
describe("talk realtime gateway relay", () => {
  ...
  it("closes only realtime relays owned by the disconnected connection", async () => {
    ...
    const tempDir = tempDirs.make("afora-relay-disconnect-");
  ...
  it("appends finalized relay transcripts to the canonical agent session", async () => {
    ...
    const tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-relay-voice-")),
    );
    setTestEnvValue("AFORA_STATE_DIR", tempDir);
    let bridgeRequest: RealtimeVoiceBridgeCreateRequest | undefined;
    try {
  ...
  it("emits one terminal error and close when transcript persistence overflows", async () => {
    const envSnapshot = captureEnv(["AFORA_STATE_DIR"]);
    const tempDir = tempDirs.make("afora-relay-voice-overflow-");
    setTestEnvValue("AFORA_STATE_DIR", tempDir);
    let bridgeRequest: RealtimeVoiceBridgeCreateRequest | undefined;
  ...
  it("creates the relay voice record before binding a transcript-free consult", async () => {
    const envSnapshot = captureEnv(["AFORA_STATE_DIR"]);
    const tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-relay-voice-consult-")),
    );
    setTestEnvValue("AFORA_STATE_DIR", tempDir);
    try {
      const session = createTalkRealtimeRelaySession({
  ...
  it("pins an unscoped relay owner before the configured default changes", async () => {
    const envSnapshot = captureEnv(["AFORA_STATE_DIR"]);
    const tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-relay-owner-pin-")),
    );
    setTestEnvValue("AFORA_STATE_DIR", tempDir);
    let runtimeConfig: AforaConfig = {
  ...
  it("pins a scoped relay owner from the trimmed session key", async () => {
    const envSnapshot = captureEnv(["AFORA_STATE_DIR"]);
    const tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-relay-trimmed-owner-")),
    );
    setTestEnvValue("AFORA_STATE_DIR", tempDir);
    try {
      const session = createTalkRealtimeRelaySessionRaw({
  ...
  it("logs relay transcript append failures", async () => {
    const envSnapshot = captureEnv(["AFORA_STATE_DIR"]);
    const tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-relay-voice-failure-")),
    );
    setTestEnvValue("AFORA_STATE_DIR", tempDir);
    let bridgeRequest: RealtimeVoiceBridgeCreateRequest | undefined;
    const warn = vi.fn();
  ...
  it("bridges browser audio, transcripts, and tool results through a backend provider", async () => {
    ...
    expect(bridge.submitToolResult).toHaveBeenNthCalledWith(
      ...
      {
        status: "working",
        tool: "afora_agent_consult",
        message:
          "Tell the person briefly that you are checking, then wait for the final Afora result before answering with the actual result.",
      },
      { willContinue: true },
    );
    expect(bridge.submitToolResult).toHaveBeenNthCalledWith(
    ...
    expectRecordFields(toolResultPayloads[0]?.talkEvent, {
      type: "tool.progress",
      callId: "call-1",
      payload: { name: "afora_agent_consult", status: "working" },
    });
  ...
  it("forces an agent consult when configured and realtime transcript finalizes without a provider tool call", async () => {
    ...
    expect(bridge.sendUserMessage).toHaveBeenLastCalledWith(
      "Briefly tell the person that you are checking with Afora. Do not answer the request yet. Wait for the Afora result before giving the actual answer.",
    );
    ...
    expect(bridge.submitToolResult).toHaveBeenLastCalledWith(
      ...
      {
        status: "already_delivered",
        message: "Afora already delivered this consult result internally. Do not repeat it.",
      },
      { suppressResponse: true },
    );
    expect(bridge.sendUserMessage).toHaveBeenLastCalledWith(
      [
        "Afora finished checking. Speak this result naturally and concisely.",
