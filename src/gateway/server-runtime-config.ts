...
import { warnLegacyAforaEnvVars } from "./env-deprecation.js";
...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  if (tailscaleMode === "funnel" && authMode !== "password") {
    throw new Error(
      "tailscale funnel requires gateway auth mode=password (set gateway.auth.password or AFORA_GATEWAY_PASSWORD)",
    );
  }
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret && authMode !== "trusted-proxy") {
    throw new Error(
      `refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set AFORA_GATEWAY_TOKEN/AFORA_GATEWAY_PASSWORD; legacy CLAWDBOT_* and MOLTBOT_* environment variables are ignored)`,
    );
  }
  if (
    controlUiEnabled &&
