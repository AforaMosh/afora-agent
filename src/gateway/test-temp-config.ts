...
/** Writes a temp Afora config, installs it as runtime state, then restores globals. */
export async function withTempConfig(params: {
  cfg: unknown;
  run: () => Promise<void>;
  prefix?: string;
}): Promise<void> {
  const prevConfigPath = process.env.AFORA_CONFIG_PATH;

  const testConfig = canonicalizeTempConfigForTest(params.cfg) as AforaConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "afora-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.AFORA_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
