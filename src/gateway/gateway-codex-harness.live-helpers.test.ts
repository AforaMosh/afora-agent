...
describe("gateway codex harness live helpers", () => {
  it("builds an exact large-output command without escape-sensitive newlines", () => {
    const command = buildCodexHarnessLargeOutputCommand({
      commandMarker: "AFORA-LARGE-OUTPUT-ABC",
      outputBytes: CODEX_HARNESS_MAX_LARGE_OUTPUT_BYTES,
    });

    expect(command).toContain('"AFORA-LARGE-OUTPUT-ABC|"');
  ...
  it("matches a successful wrapped native command by its per-turn marker", () => {
    const expectedCommand = `node -e 'console.log("AFORA-LARGE-OUTPUT-ABC")'`;
    const wrappedCommand = `node -e "console.log(\\"AFORA-LARGE-OUTPUT-ABC\\")"`;
    const events = [
      {
        stream: "tool",
        data: {
  ...
  it("rejects a successful command that only echoes the expected command", () => {
    ...
    ).toThrow("missing native bash command start for marker AFORA-LARGE-OUTPUT-ABC");
  });

  it("accepts a completed native command when Codex omits or nulls its optional exit code", () => {
    const expectedCommand = "node -e AFORA-NO-EXIT-CODE";
    for (const result of [{ status: "completed" }, { status: "completed", exitCode: null }]) {
      ...
      expect(
        requireSuccessfulNativeCommandExecution(events, {
          commandMarker: "AFORA-NO-EXIT-CODE",
          expectedCommand,
        }),
  ...
  it("reports a missing native command start explicitly", () => {
    expect(() =>
      requireSuccessfulNativeCommandExecution([], {
        commandMarker: "AFORA-MISSING",
        expectedCommand: "node -e AFORA-MISSING",
      }),
    ).toThrow("missing native bash command start for marker AFORA-MISSING");
  });
        ...
        [
          {
            ...
            data: {
              phase: "start",
              name: "bash",
              args: { command: "node -e AFORA-NO-ITEM" },
            },
          },
        ],
        {
          commandMarker: "AFORA-NO-ITEM",
          expectedCommand: "node -e AFORA-NO-ITEM",
        },
      ),
    ).toThrow("native bash command start for marker AFORA-NO-ITEM has no itemId");
  });
  ...
  it("reports a missing successful native command result explicitly", () => {
    expect(() =>
      requireSuccessfulNativeCommandExecution(
        [
          {
            ...
            data: {
              ...
              name: "bash",
              itemId: "item-failed",
              args: { command: "node -e AFORA-FAILED" },
            },
          },
          {
            stream: "tool",
        ...
        {
          commandMarker: "AFORA-FAILED",
          expectedCommand: "node -e AFORA-FAILED",
        },
      ),
    ).toThrow(
      "native bash command item-failed for marker AFORA-FAILED has no successful result",
    );
  });
  ...
  it("requires a successful marker-bearing large result in durable history", () => {
    const expectedCommand = `node -e 'console.log("AFORA-PERSISTED")'`;
    const messages = [
      {
        role: "assistant",
        content: [
  ...
  it("rejects echoed or failed native commands in durable history", () => {
    ...
    expect(() =>
      ...
      "persisted native bash command for marker AFORA-PERSISTED has no successful large result",
    );
  ...
  it("accepts successful request-local evidence when compaction removed durable history", () => {
    const expectedCommand = "node -e AFORA-COMPACTED";
    const events = [
      {
        stream: "tool",
        data: {
  ...
  it("rejects large durable output from a different marker-bearing command", () => {
    const expectedCommand = "node -e AFORA-EXACT";
    const messages = [
      {
        role: "assistant",
        content: [
  ...
  it("ties a result-only durable row to the exact request-local item id", () => {
    const expectedCommand = "node -e AFORA-RESULT-ONLY";
    const events = [
      {
        stream: "tool",
        data: {
    ...
    expect(
      requireSuccessfulNativeCommandCompactionEvidence({
        commandMarker: "AFORA-RESULT-ONLY",
        events,
        expectedCommand,
  ...
  it("accepts the current codex status prose from the live harness", () => {
    const text =
      "Afora is running on `openai/gpt-5.5` with low reasoning/text settings. Context is at `22k/272k` tokens, no compactions, and the current session is `agent:dev:live-codex-harness`.";
  ...
  it("accepts current status prose that reports session context without the session id", () => {
    const text = [
      "Afora is running on `openai/gpt-5.5` with low reasoning/text settings.",
      "",
  ...
  it("accepts current status prose that reports healthy session context without the session id", () => {
    const text = [
      ...
      "",
      "Session context is healthy: `22k/272k` tokens used, `0` compactions, `53%` cache hit. Current workspace is `/tmp/afora-live-codex-harness/workspace/dev`.",
    ].join("\n");

    expect(isExpectedCodexStatusCommandText(text)).toBe(true);
  });

  it("accepts current app-server status prose without the Afora prefix", () => {
    const text = [
      "Status: running on `openai/gpt-5.5` in `/tmp/afora-live-codex-harness/workspace/dev`.",
      "",
  ...
  it("accepts workspace-only healthy status prose emitted by current codex", () => {
    const text =
      "Working normally. Current workspace: `/tmp/afora-live-codex-harness/workspace/dev`.";
  ...
  it("accepts the OpenAI Codex status card emitted by the GPT-5.5 Docker harness", () => {
    const text = [
      "Afora 2026.4.30-beta.1 is running on `openai/gpt-5.5`.",
      "",
      "Session is healthy:",
  ...
  it("accepts the compact status-card pointer emitted by current codex", () => {
    const text = "Afora status shown above.";
  ...
  it("accepts the online idle status emitted by current codex", () => {
    const text =
      "I'm online in `/tmp/afora-live-codex-harness-KiaUQ4/workspace/dev`, with workspace-write access. No active task is running right now.";
  ...
  it("accepts the normal-work status emitted by current codex", () => {
    const text =
      "Working normally. Current cwd is `/tmp/afora-live-codex-harness/workspace/dev`, sandbox is workspace-write, network is restricted, and the current date is 2026-05-09 UTC.";
  ...
  it("rejects status prose for a different codex session", () => {
    const text =
      "Afora is running on `openai/gpt-5.5` with low reasoning/text settings. Context is at `22k/272k` tokens, no compactions, and the current session is `agent:dev:other`.";
  ...
  it("accepts the GPT-5.5 Docker harness shell fallback", () => {
    const text = [
      ...
      "",
      "Current session model from Afora status is `openai/gpt-5.5`.",
    ].join("\n");
