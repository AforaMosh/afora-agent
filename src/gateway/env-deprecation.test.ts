...
let warnLegacyAforaEnvVars: typeof import("./env-deprecation.js").warnLegacyAforaEnvVars;

describe("warnLegacyAforaEnvVars", () => {
  let envSnapshot: ReturnType<typeof captureEnv>;
  let emitWarning: ReturnType<typeof vi.spyOn>;

  beforeEach(async () => {
    vi.resetModules();
    ({ warnLegacyAforaEnvVars } = await import("./env-deprecation.js"));
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(message).toContain("replacing the legacy prefix with AFORA_");
    ...
    expect(options).toEqual({
      code: "AFORA_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current AFORA names", () => {
    warnLegacyAforaEnvVars({ AFORA_GATEWAY_TOKEN: "token" });

    expect(emitWarning).not.toHaveBeenCalled();
  });

  it("warns only once after a successful emit", () => {
    warnLegacyAforaEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
    warnLegacyAforaEnvVars({ MOLTBOT_GATEWAY_TOKEN: "old-token" });
  ...
  it("retries if emitWarning throws before the warning is emitted", () => {
    ...
    expect(() => warnLegacyAforaEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" })).toThrow(
      "warning sink failed",
    );
    warnLegacyAforaEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
