...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.AFORA_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.AFORA_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.AFORA_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.AFORA_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers AFORA_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.AFORA_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.AFORA_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.AFORA_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.AFORA_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(resolvePreauthHandshakeTimeoutMs()).toBe(75);
        process.env.AFORA_HANDSHAKE_TIMEOUT_MS = "";
        expect(resolvePreauthHandshakeTimeoutMs()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.AFORA_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.AFORA_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.AFORA_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.AFORA_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
    ...
    test("hello-ok separates effective scopes from a reused device token grant", async () => {
      ...
      const deviceIdentityPath = path.join(
        os.tmpdir(),
        `afora-shared-auth-scope-reuse-${randomUUID()}.json`,
      );
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `afora-test-device-${randomUUID()}.sqlite`),
        nonce,
      });
