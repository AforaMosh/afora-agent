...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.AFORA_LIVE_ACP_BIND);
const describeLive = LIVE && ACP_BIND_LIVE ? describe : describe.skip;

const CONNECT_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.AFORA_LIVE_ACP_BIND_REQUEST_TIMEOUT_MS,
  90_000,
);
...
function shouldRequireCronMcpProbe(): boolean {
  return isTruthyEnvValue(process.env.AFORA_LIVE_ACP_BIND_REQUIRE_CRON);
}
...
function resolveLiveParentModel(): string {
  return normalizeOpenAiModelRef(
    process.env.AFORA_LIVE_ACP_BIND_PARENT_MODEL?.trim() ||
      process.env.AFORA_LIVE_ACP_BIND_CODEX_MODEL?.trim() ||
...
async function prepareCodexHomeForLiveBindTest(tempRoot: string): Promise<void> {
  ...
  const model = process.env.AFORA_LIVE_ACP_BIND_CODEX_MODEL?.trim() || DEFAULT_LIVE_CODEX_MODEL;
...
describeLive("gateway live (ACP bind)", () => {
  it(
    ...
    async () => {
      ...
      const liveAgent = normalizeAcpAgent(process.env.AFORA_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.AFORA_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "afora.json");
      ...
      try {
        ...
        if (
          shouldRunLiveImageProbe({
            agent: liveAgent,
            override: process.env.AFORA_LIVE_ACP_BIND_IMAGE_PROBE,
          })
        ) {
