  ...
  return fs
    ...
    .filter(
      (entryPath) =>
        ...
        fs.existsSync(path.join(entryPath, "afora.plugin.json")),
    );
}
...
function listGitSourceBundledPluginRoots(extensionsDir: string): string[] | null {
  const result = spawnSync(
    "git",
    ["ls-files", "--", "extensions/*/package.json", "extensions/*/afora.plugin.json"],
    {
      cwd: process.cwd(),
      encoding: "utf8",
...
afterEach(() => {
  delete (globalThis as { __aforaBundledChannelReenter?: () => void })[
    "__aforaBundledChannelReenter"
  ];
  vi.resetModules();
...
describe("bundled channel entry shape guards", () => {
  ...
  it("fills sparse bundled channel plugin metadata from package metadata", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-metadata-"));
    ...
    try {
      process.env.AFORA_BUNDLED_PLUGINS_DIR = path.join(tempRoot, "dist", "extensions");
  ...
  it("uses the active bundled plugin root override for channel entry loading", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-override-"));
    const previousBundledPluginsDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(tempRoot, "dist", "extensions", "alpha");
    fs.mkdirSync(pluginDir, { recursive: true });
    fs.writeFileSync(
  ...
  it("falls back through the cached loader for package-local dist entries needing SDK aliases", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-package-dist-"));
  ...
  it("falls back through the cached loader for direct override dist entries needing SDK aliases", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-direct-dist-"));
    const previousBundledPluginsDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.AFORA_BUNDLED_PLUGINS_DIR = pluginsRoot;
  ...
  it("treats direct bundled plugin-tree overrides as scan roots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-direct-override-"));
    const previousBundledPluginsDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
  ...
  it("partitions bundled channel lazy caches by active bundled root without re-importing", async () => {
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-root-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-root-b-"));
    const previousBundledPluginsDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
    const testGlobal = globalThis as typeof globalThis & {
    ...
    try {
      ...
      process.env.AFORA_BUNDLED_PLUGINS_DIR = path.join(rootA, "dist", "extensions");
      ...
      process.env.AFORA_BUNDLED_PLUGINS_DIR = path.join(rootB, "dist", "extensions");
  ...
  it("uses dist-runtime as the boundary root for packaged setup entries", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-runtime-root-"));
  ...
  it("swallows and caches bundled plugin and setup load failures", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-load-failure-"));
    const previousBundledPluginsDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.AFORA_BUNDLED_PLUGINS_DIR = path.join(root, "dist", "extensions");
  ...
  it("falls back to a contained source-only registry root when generated lookup misses", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-source-fallback-"));
    const outsideRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "afora-bundled-source-fallback-outside-"),
    );
    const previousBundledPluginsDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.AFORA_BUNDLED_PLUGINS_DIR = pluginsDir;
      const bundled = await importFreshModule<typeof import("./bundled.js")>(
        import.meta.url,
  ...
  it("accepts canonical built entries through the active package symlink", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-alias-boundary-"));
    const previousBundledPluginsDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.AFORA_BUNDLED_PLUGINS_DIR = path.join(currentRoot, "dist", "extensions");
      const bundled = await importFreshModule<typeof import("./bundled.js")>(
        import.meta.url,
  ...
  it("caches undefined bundled plugin loads as unavailable", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-null-load-"));
    const previousBundledPluginsDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    const offenders = collectBundledChannelEntrypointOffenders(
      bundledPluginRoots,
      (source) =>
        !source.includes('from "afora-agent/plugin-sdk/channel-entry-contract"') ||
        source.includes('from "afora-agent/plugin-sdk/core"') ||
        source.includes('from "afora-agent/plugin-sdk/channel-core"'),
    );
  ...
  it("keeps setup-entry legacy feature hints mirrored in package metadata", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      ...
      for (const feature of ["legacyStateMigrations", "legacySessionSurfaces"]) {
        ...
        const hasHint = packageJson.afora?.setupFeatures?.[feature] === true;
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "afora-agent/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("keeps bundled hot runtime barrels off the broad core SDK surface", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes("afora-agent/plugin-sdk/core"),
    );
  ...
  it("keeps bundled doctor surfaces off the broad runtime barrel", () => {
    const offenders = [
      ...
      fs
        ...
        .includes('from "afora-agent/plugin-sdk/runtime"'),
    );
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-reentrant-"));
    const modulePath = path.join(pluginDir, "index.cjs");
    fs.writeFileSync(
      modulePath,
      `
const reenter = globalThis["__aforaBundledChannelReenter"];
...
`,
    ...
    let reentered = false;
    (globalThis as { __aforaBundledChannelReenter?: () => void })[
      "__aforaBundledChannelReenter"
    ] = () => {
      if (!reentered) {
        reentered = true;
