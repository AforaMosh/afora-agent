    ...
    JSON.stringify({
      ...(richManifest
        ? {
            ...
            schemaVersion: 1,
            description:
              "Extension manifest. Declares plugin packages that Afora can discover during onboarding and install on demand via `afora plugins install`.",
          }
        : {}),
      entries: [entry],
    }),
...
function createTemporaryCatalogFile(
  ...
  const directory = fs.mkdtempSync(path.join(resolvePreferredAforaTmpDir(), prefix));
...
const [richNpmCatalogFixture, clawhubCatalogFixture, yuanbaoCatalogFixture] = [
  {
    ...
    prefix: "afora-catalog-rich-",
    channelId: "wecom",
    entry: {
      name: "@wecom/wecom-afora-plugin",
      description:
        "Afora WeCom (企业微信) channel plugin — community maintained, published on npm.",
      source: "external",
      kind: "channel",
      afora: {
        channel: {
    ...
    expected: {
      ...
      installSource: {
        defaultChoice: "npm",
        npm: {
          spec: "@wecom/wecom-afora-plugin@1.2.3",
  ...
  },
  {
    ...
    entry: {
      ...
      afora: {
        ...
        install: {
          npmSpec: "afora-plugin-yuanbao@1.0.0",
          defaultChoice: "npm",
          minHostVersion: ">=2026.4.10",
...
export function describeChannelPluginCatalogEntriesContract() {
  const cases: ChannelCatalogContractCase[] = [
      ...
      setup: () => ({
        ...
        catalogPaths: [
          createTemporaryCatalogFile(
            "afora-catalog-",
            createCatalogEntry({
              packageName: "@afora/demo-channel",
              channelId: "demo-channel",
              label: "Demo Channel",
              blurb: "Demo entry",
              order: 999,
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(
          path.join(resolvePreferredAforaTmpDir(), "afora-channel-catalog-state-"),
        );
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(
          path.join(resolvePreferredAforaTmpDir(), "afora-catalog-state-"),
        );
    ...
    createRichExternalCatalogCase(richNpmCatalogFixture),
    {
      ...
      setup: () => ({
        ...
        catalogPaths: [
          createTemporaryCatalogFile("afora-catalog-prerelease-", {
            ...createCatalogEntry({
              packageName: "@afora/prerelease-demo-channel",
              channelId: "prerelease-demo",
        ...
        expected: {
          install: { npmSpec: "@afora/prerelease-demo-channel@2026.5.3-beta.1" },
          installSource: {
            npm: {
              spec: "@afora/prerelease-demo-channel@2026.5.3-beta.1",
              packageName: "@afora/prerelease-demo-channel",
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredAforaTmpDir(), "afora-catalog-home-"),
          );
          writeCatalogFile(
            path.join(home, "catalog.json"),
            createCatalogEntry({
              packageName: "@afora/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              AFORA_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              AFORA_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@afora/default-env-demo",
