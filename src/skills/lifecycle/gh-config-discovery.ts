...
// Afora process is running with a different HOME (e.g. the per-agent
...
// See https://github.com/AforaMosh/afora-agent/issues/78063.
...
type GhConfigDirMismatch = {
  ...
  // Suggested env value the operator should set on the Afora service to
...
export function formatGhConfigDirMismatchHint(mismatch: GhConfigDirMismatch): string[] {
  ...
  lines.push(
    `  Fix: set GH_CONFIG_DIR=${mismatch.suggestedEnvValue} on the Afora service environment, then restart the gateway.`,
  );
  return lines;
}
