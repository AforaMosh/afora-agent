...
describe("workspace skill mutations", () => {
  it("removes support files when the activating SKILL.md write fails", async () => {
    const workspaceDir = await tempDirs.make("afora-workspace-skill-write-failure-");
  ...
  it("restores the complete previous update bundle", async () => {
    const workspaceDir = await tempDirs.make("afora-workspace-skill-write-update-");
  ...
  it("removes a support file when its atomic write commits and then rejects", async () => {
    const workspaceDir = await tempDirs.make("afora-workspace-skill-support-commit-failure-");
  ...
  it("restores an update when the SKILL.md write commits and then rejects", async () => {
    const workspaceDir = await tempDirs.make("afora-workspace-skill-main-commit-failure-");
  ...
  it("removes every file from a restored create mutation", async () => {
    const workspaceDir = await tempDirs.make("afora-workspace-skill-write-create-");
  ...
  it("restores an interrupted update from persisted rollback facts", async () => {
    const workspaceDir = await tempDirs.make("afora-workspace-skill-recovery-");
  ...
  it("detects external edits before restoring a completed mutation", async () => {
    const workspaceDir = await tempDirs.make("afora-workspace-skill-external-edit-");
