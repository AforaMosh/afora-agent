...
metadata: {"afora":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
const workspaceSuite = createFixtureSuite("afora-skills-install-");
...
describe("installSkill before_install hooks", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("AFORA_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Afora-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["AFORA_STATE_DIR", "AFORA_CONFIG_PATH"]);
    try {
      process.env.AFORA_STATE_DIR = "/tmp/untrusted-state";
      process.env.AFORA_CONFIG_PATH = "/tmp/untrusted-config/afora.json";
  ...
  it("surfaces plugin hook findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("afora-workspace");
