...
type SkillUploadStoreOptions = AforaStateDatabaseOptions & {
...
function requireUploadRow(uploadId: string, options: AforaStateDatabaseOptions): SkillUploadRow {
...
function createSkillUploadStore(options?: SkillUploadStoreOptions) {
  ...
  function lockRoot(): string {
    return openAforaStateDatabase(stateOptions).path;
  }
      ...
      return await withLock(`${root}:upload:${uploadId}`, async () => {
        ...
        try {
          return await withTempWorkspace(
            {
              rootDir: tempRootDir ?? resolvePreferredAforaTmpDir(),
              prefix: "afora-skill-upload-",
            },
            async (tmp) => {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("afora.skillUploadStoreTestApi")] = {
