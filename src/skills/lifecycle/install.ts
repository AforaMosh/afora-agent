...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "afora");
  }
  return path.join(homedir(), ".afora");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "afora-bundled"
        ? { kind: "bundled", authority: "afora", mutable: false, network: false }
        : skillSource === "afora-managed" || skillSource === "afora-extra"
          ? { kind: "managed", authority: "afora", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["afora-bundled", "afora-managed", "afora-extra"]);
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("afora.skillsInstallTestApi")] =
    testing;
}
