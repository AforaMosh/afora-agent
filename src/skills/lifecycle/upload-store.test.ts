...
async function makeStore(options?: {
  ...
  const root = tempDirs.make("afora-skill-upload-store-");
  const databasePath = path.join(root, "afora.sqlite");
  return {
    root,
    databasePath,
...
function stateDatabase(databasePath: string) {
  return openAforaStateDatabase({ path: databasePath }).db;
}
...
describe("skill upload store", () => {
  ...
  beforeAll(async () => {
    activeLimitRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-skill-upload-limit-"));
    const store = createSkillUploadStore({
      path: path.join(activeLimitRoot, "afora.sqlite"),
