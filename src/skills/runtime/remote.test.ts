...
function createRemoteSkillWorkspace(bin: string): { cfg: AforaConfig; workspaceDir: string } {
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-remote-skills-"));
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `metadata: { "afora": { "os": ["darwin"], "requires": { "bins": ["${bin}"] } } }`,
      "---",
      "# Remote Skill",
      "",
    ].join("\n"),
