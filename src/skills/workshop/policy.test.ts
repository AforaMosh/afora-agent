...
beforeEach(async () => {
  testState = await createAforaTestState({
    layout: "state-only",
    prefix: "afora-skill-workshop-policy-",
  });
});
...
describe("resolveSkillWorkshopToolApproval", () => {
  it("describes the target proposal and bounds the approval wait", async () => {
    const workspaceDir = await tempDirs.make("afora-skill-workshop-policy-workspace-");
  ...
  it("bounds approval metadata without splitting UTF-16 surrogates", async () => {
    const workspaceDir = await tempDirs.make("afora-skill-workshop-policy-long-name-");
    const description = "d".repeat(160);
  ...
  it("renders proposal-controlled fields without approval-line injection", async () => {
    const workspaceDir = await tempDirs.make("afora-skill-workshop-policy-controls-");
    const proposal = await proposeCreateSkill({
      workspaceDir,
      name: "Line\nBreak\u202eSpoof",
      description:
  ...
  it("falls back to the action description when the proposal cannot be resolved", async () => {
    const workspaceDir = await tempDirs.make("afora-skill-workshop-policy-missing-");
