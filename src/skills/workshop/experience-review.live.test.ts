...
const LIVE =
  isLiveTestEnabled(["AFORA_LIVE_SKILL_EXPERIENCE_REVIEW"]) &&
...
function candidate(
  ...
  const modelId = process.env.AFORA_LIVE_SKILL_EXPERIENCE_MODEL ?? "gpt-5.6-luna";
  return {
    ctx: {
      agentId: "main",
      runId,
...
describeLive("skill experience review live OpenAI eval", () => {
  beforeAll(async () => {
    ...
    // store via HOME, and a real ~/.afora with pending doctor migration
    // must never leak into (or fail) this live run.
    testState = await createAforaTestState({
      layout: "home",
      prefix: "afora-live-skill-review-state-",
    });
    workspaceDir = await tempDirs.make("afora-live-skill-review-workspace-");
