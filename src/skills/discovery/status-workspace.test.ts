...
function makeEntry(params: {
  ...
  return {
    skill: createCanonicalFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "afora-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-bundled-"));
    ...
    await withEnvAsync({ AFORA_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill.source).toBe("afora-workspace");
