  ...
  if (params.truncated) {
    const compactDetails =
      params.format.kind === "compact"
        ...
        : "";
    return `⚠️ Skills truncated: included ${params.included} of ${params.total}${compactDetails}. Run \`afora skills check\` to audit.`;
  }
  if (params.format.kind === "compact") {
    const compactDetails =
      params.format.descriptionMaxChars > 0 ? "descriptions shortened" : "descriptions omitted";
    return `⚠️ Skills catalog using compact format (${compactDetails}). Run \`afora skills check\` to audit.`;
  }
  return "";
}
