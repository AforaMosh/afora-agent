...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["AFORA_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns AFORA_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-bundled-override-"));
    process.env.AFORA_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.AFORA_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "afora" }));
