// Afora dialogue parses direct commands and optionally asks the assistant planner.
...
/**
 * Dialogue helpers for turning user text into Afora operations.
 *
...
/** Resolve user input to an Afora operation, optionally using the assistant planner. */
...
function logAssistantPlan(
  ...
  runtime.log(`[afora] planner: ${modelLabel}`);
  if (plan.reply) {
    runtime.log(plan.reply);
  }
  runtime.log(`[afora] interpreted: ${plan.command}`);
}
