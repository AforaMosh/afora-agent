// Afora assistant planning converts fuzzy user text into one safe command.
...
async function runConfiguredSystemAgentText(params: {
  ...
  try {
    ...
    const shared = {
      ...
      // Afora is the planner surface, but the configured roster owner supplies runtime policy.
...
async function createTempPlannerDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "afora-planner-"));
}
