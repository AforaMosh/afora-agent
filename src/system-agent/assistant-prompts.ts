...
export const SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT = [
  ...
  "",
  "Config knowledge — the file is ~/.afora/afora.json (JSON5). You change it ONLY through `config set` / `config set-ref` / `setup` / `set default model` / `connect <channel>` / `configure skills` / `configure search` / `configure gateway`. Memory import copies files and does not change config.",
