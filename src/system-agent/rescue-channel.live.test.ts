// Afora live rescue channel tests cover live-channel rescue message delivery.
...
const originalConfigPath = process.env.AFORA_CONFIG_PATH;

const runLive =
  isTruthyEnvValue(process.env.AFORA_LIVE_TEST) &&
  isTruthyEnvValue(process.env.AFORA_LIVE_SYSTEM_AGENT_RESCUE_CHANNEL);
const describeLive = runLive ? describe : describe.skip;

function commandContext(channel = process.env.AFORA_LIVE_SYSTEM_AGENT_CHANNEL ?? "whatsapp") {
  return {
    surface: channel,
    channel,
    channelId: channel,
    ...
    senderId: "user:owner",
    rawBodyNormalized: "/afora status",
    commandBodyNormalized: "/afora status",
    from: "user:owner",
    to: "account:default",
...
describeLive("Afora live rescue channel smoke", () => {
  afterEach(() => {
    ...
    if (originalStateDir === undefined) {
      ...
      setTestEnvValue("AFORA_STATE_DIR", originalStateDir);
    }
    if (originalConfigPath === undefined) {
      deleteTestEnvValue("AFORA_CONFIG_PATH");
    } else {
      setTestEnvValue("AFORA_CONFIG_PATH", originalConfigPath);
    }
  });

  it("handles /afora status and a persistent approval roundtrip", async () => {
    await withTestDir({ prefix: "afora-live-rescue-" }, async (tempDir) => {
      ...
      await expect(runRescue({ commandBody: "/afora status", cfg })).resolves.toContain(
        "[afora] done: status.check",
      );
      await expect(
        runRescue({ commandBody: "/afora set default model openai/gpt-5.5", cfg }),
      ).resolves.toContain("Reply /afora yes to apply");
      await expect(runRescue({ commandBody: "/afora yes", cfg })).resolves.toContain(
