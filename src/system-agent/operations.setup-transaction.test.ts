...
const mockConfig = vi.hoisted(() => {
  const state = {
    path: "/tmp/afora.json",
    exists: true,
    config: { agents: { entries: { main: { default: true } } } } as AforaConfig,
  };
  let bindPluginMetadata = (_config: AforaConfig) => {};
  const snapshot = () => {
  ...
  return {
    ...
    setConfig(config: AforaConfig) {
      state.config = structuredClone(config);
      bindPluginMetadata(state.config);
    },
    bindPluginMetadata(config: AforaConfig) {
      bindPluginMetadata(config);
    },
    setPluginMetadataBinder(binder: (config: AforaConfig) => void) {
...
describe("system-agent setup transaction", () => {
  ...
  it("resumes and completes the approved pending local onboarding owner", async () => {
    setTestEnvValue("AFORA_STATE_DIR", opTempDirs.make("afora-recovery-complete-"));
  ...
  it("does not adopt a pending receipt from the replaced config at the same path", async () => {
    setTestEnvValue("AFORA_STATE_DIR", opTempDirs.make("afora-recovery-stale-"));
    const pending = createPendingLocalOnboarding();
  ...
  it.each([
    ...
    setTestEnvValue("AFORA_STATE_DIR", opTempDirs.make("afora-recovery-failure-"));
    const pending = createPendingLocalOnboarding();
    ...
    expect(localOnboarding.states.get(pending.configPath)).toEqual(pending);
    expect(localOnboarding.complete).not.toHaveBeenCalled();
    expect(lines.join("\n")).not.toContain("[afora] done: afora.setup");
  });

  it("never completes a competing onboarding owner after setup succeeds", async () => {
    setTestEnvValue("AFORA_STATE_DIR", opTempDirs.make("afora-recovery-replaced-"));
    const pending = createPendingLocalOnboarding();
  ...
  it("rechecks setup authority immediately before completing onboarding", async () => {
    setTestEnvValue("AFORA_STATE_DIR", opTempDirs.make("afora-recovery-authority-"));
    const pending = createPendingLocalOnboarding();
    const applySetup = vi.fn(async () => createRecoverySetupResult());
  ...
  it("does not adopt or complete local onboarding from a gateway-hosted setup", async () => {
    setTestEnvValue("AFORA_STATE_DIR", opTempDirs.make("afora-recovery-gateway-"));
    const pending = createPendingLocalOnboarding();
    const applySetup = vi.fn(async () => createRecoverySetupResult());
    const { runtime } = createSystemAgentTestRuntime();
