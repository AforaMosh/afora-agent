// Afora TUI backend tests cover rescue status integration with the TUI backend.
...
const verifiedInferenceMocks = vi.hoisted(() => ({
  preparedBindings: new WeakMap<object, AforaConfig>(),
}));
...
async function createVerifiedTuiOptions(
  ...
  config: AforaConfig = verifiedConfig,
...
describe("runSystemAgentTui", () => {
  ...
  it("runs Afora inside the shared TUI shell", async () => {
    let runTuiCalls = 0;
    ...
    expect(options.session).toBe("agent:afora:main");
    expect(options.historyLimit).toBe(200);
    expect(options.config).toEqual({});
    expect(options.title).toBe("afora setup");
    if (!options.backend || typeof options.backend !== "object") {
      throw new Error("expected afora TUI backend");
    }
  }, 240_000);
    ...
    for (const { handoff, expected } of cases) {
      ...
      expect(events).toEqual([
        ...
        ...(handoff.target === "gateway"
          ? ["log:Done — gateway settings saved. Run `afora gateway restart` to apply them."]
          : []),
      ]);
    }
  });
