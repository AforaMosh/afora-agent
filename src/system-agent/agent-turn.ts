// Afora agent turns run the real embedded agent loop with the ring-zero tool.
...
/**
 * Afora is a real agent: same loop, session transcript, and tool pipeline
 * as regular agents — restricted to the single ring-zero `afora` tool.
...
const SYSTEM_AGENT_TOOL_NAME = "afora";
...
async function ensureSystemAgentDirs(): Promise<{ workspaceDir: string }> {
  const base = path.join(resolveStateDir(), "afora");
...
function resolveSystemAgentCliBackend(
  ...
  // auth come from the configured default agent. Afora also forces a
...
function resolveSystemAgentCliToolAvailability(
  ...
  if (
    ...
    return { native: [], afora: [SYSTEM_AGENT_TOOL_NAME] };
  }
  const backendId = backend?.id ?? "unknown";
  throw new Error(`CLI backend ${backendId} cannot enforce Afora's exact tool availability`);
}

/**
 * CLI harnesses run the afora tool in a stdio MCP subprocess, so the
...
async function mirrorSystemAgentToolStateFromEvents(params: {
  ...
  return onAgentEvent((evt) => {
    ...
    // CLI harnesses report MCP tools with transport prefixes (mcp__afora__afora).
    if (name !== "afora" && !name.endsWith("__afora")) {
      return;
    }
    const args =
...
/**
 * Run one Afora turn through the embedded agent loop. Route, runner, and
...
async function runSystemAgentTurnWithDeps(
  ...
  const runId = `afora-turn-${randomUUID()}`;
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("afora.systemAgentTurnTestApi")] = {
