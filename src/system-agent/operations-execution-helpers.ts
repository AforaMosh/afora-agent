...
export function formatChannelDocsUrl(docsPath: string): string {
  return `https://docs.afora.ai${docsPath.startsWith("/") ? docsPath : `/${docsPath}`}`;
}
 ...
 * `[afora] running/done` markers the e2e lanes assert on; each spec only
...
export async function applyPersistentOperation(params: {
  ...
  runtime.log(`[afora] running: ${auditOperation}`);
  const { readConfigFileSnapshot } = await loadConfigModule();
  ...
  try {
    ...
    runtime.error(
      `${outcome.summary}, but Afora could not record its audit entry: ${formatErrorMessage(error)}`,
    );
  }
  runtime.log(`[afora] done: ${auditOperation}`);
  return {
    applied: true,
...
export async function assertConfigWriteDoesNotBypassInferenceVerification(
  ...
  if (verdict === "plugin-entry") {
    ...
    throw new Error(
      `Direct config writes cannot change plugin "${pluginId}" because it may back Afora's own active inference route. Editing it is a human-only change, made with Afora stopped from a trusted shell on the machine running it.`,
    );
  }
  ...
  throw new Error(
    denialReason
      ...
      : "Direct config writes cannot change the default inference route or include alternate config. Use `set_default_model` (optionally with agentId) for an already configured route; changing provider or auth access is `afora onboard` on the machine running Afora.",
  );
}
...
async function verifyCurrentSetupInference(
  ...
  if (!before.exists || !before.valid) {
    throw new Error(
      "Afora setup requires a valid configured inference route. Run `afora onboard` on the machine running Afora, then retry.",
    );
  }
  ...
  if (!beforeRoute.route) {
    throw new Error(
      "Afora setup requires working inference first. Run `afora onboard` on the machine running Afora, then retry.",
    );
  }
  ...
  if (!verification.ok) {
    throw new Error(
      `Afora setup requires working inference first. The configured route failed a live check: ${verification.error} Run \`afora onboard\` on the machine running Afora, then retry.`,
    );
  }
...
export async function executeSetup(
  ...
  if (!defaultModel) {
    throw new Error(
      "Afora setup requires working inference first. Run `afora onboard` on the machine running Afora to configure and verify a default model, then start Afora again.",
    );
  }
  const requestedModel = operation.model?.trim();
  if (requestedModel && requestedModel !== defaultModel) {
    throw new Error(
      `Afora setup will preserve the verified default model ${defaultModel}. Staging, live-testing, and saving a different inference route is \`afora onboard\` on the machine running Afora.`,
    );
  }
  if (!opts.approved) {
    const message = [
  ...
  if (requestedModel && requestedModel !== verified.modelRef) {
    throw new Error(
      `The verified default model is now ${verified.modelRef}, not ${requestedModel}. Review the current route, or run \`afora onboard\` on the machine running Afora, before retrying setup.`,
    );
  }
  return await applyPersistentOperation({
    auditOperation: "afora.setup",
    operation,
    runtime,
    opts,
    run: async (ctx) => {
...
export async function executeSetDefaultModel(
  ...
  return await applyPersistentOperation({
    ...
    run: async (ctx) => {
      ...
      const projectRoute = (config: AforaConfig) => projectInferenceRoute(config, targetAgentId);
      ...
      const result = await mutateConfigFile({
        ...
        writeOptions: {
          ...
          preCommitRuntimePreflight: async (sourceConfig) => {
            ...
            // The live probe can outlive the original Afora authority.
