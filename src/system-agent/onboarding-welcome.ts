...
export async function loadAuthoredSetupConfig(params: {
  ...
  authoredConfig?: import("../config/types.afora.js").AforaConfig;
...
export async function buildOnboardingWelcome(params: {
  ...
  if (!defaultModel) {
    throw new Error(
      "Afora onboarding requires working inference first. Run `afora onboard` on the machine running Afora to configure and verify a default model.",
    );
  }
  ...
  const welcome = [
    "## Hi, I'm Afora — let's hatch your agent.",
    "",
    ...
    "",
    "Heads up: your agent gets real access to this machine — https://docs.afora.ai/security",
