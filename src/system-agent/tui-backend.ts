// Afora TUI backend runs setup-helper dialogue inside the shared local TUI shell.
...
class SystemAgentTuiBackend implements TuiBackend {
  readonly connection = { url: "afora local" };
  ...
  stop(): void {
    // The enclosing TUI owns terminal shutdown; Afora has no transport to close.
  }
  ...
  async listAgents(): Promise<TuiAgentsList> {
    return {
      ...
      mainKey: "main",
      scope: "per-sender",
      agents: [{ id: SYSTEM_AGENT_ID, kind: "system", name: "Afora" }],
    };
  }
  ...
  async patchSession(opts: SessionsPatchParams): Promise<SessionsPatchResult> {
    if (opts.model !== undefined) {
      throw new Error(
        "Afora cannot change the model inside its active verified session. Exit and run `afora onboard`, then start Afora again.",
      );
    }
    return {
      ok: true,
  ...
  async createSession(_opts: TuiSessionCreateOptions) {
    ...
    return {
      ok: true as const,
      key: SYSTEM_AGENT_SESSION_KEY,
      entry: { sessionId: "afora", updatedAt: Date.now() },
    };
  }
  ...
  private emitFinal(runId: string, sessionKey: string, text: string): void {
    const assistant = message(
      "assistant",
      text || "Afora listened and found nothing to change.",
    );
  ...
  private async respond(runId: string, sessionKey: string, text: string): Promise<void> {
    ...
    try {
      ...
      if ((reply.action === "open-tui" || reply.action === "open-setup") && reply.handoff) {
        // The outer loop owns interactive handoffs after the Afora TUI exits.
...
async function runSetupHandoff(
  ...
  if (
    ...
    runtime.error(
      "Setup cannot replace the inference route powering Afora. Exit and run `afora onboard`, then start Afora again.",
    );
    return;
  }
  ...
  if (handoff.target === "gateway") {
    if (opts.runGatewaySetupHandoff) {
      ...
      runtime.log("Done — gateway settings saved. Run `afora gateway restart` to apply them.");
      return;
    }
...
export async function runSystemAgentTui(
  ...
  for (;;) {
    ...
    // replay it when Afora re-enters the chat shell.
    ...
    if (handoff.kind === "model-setup") {
      runtime.error(
        "Afora cannot replace its active inference route. Run `afora onboard` outside this session, then start Afora again.",
      );
      return;
    }
