...
describe("system agent setup-flow operations", () => {
  ...
  it("prints one-shot setup pointers", async () => {
    ...
    expect(output).toContain("afora onboard`");
    expect(output).toContain("afora onboard --classic");
    expect(output).toContain("afora channels add --channel slack");
    expect(output).toContain("afora configure --section web");
    expect(output).toContain("afora configure --section gateway");
    expect(output).toContain("on the machine running Afora");
  });
  ...
  it("prints one-shot pointers for hosted skills, search, and Gateway setup", async () => {
    ...
    expect(lines.join("\n")).toContain("afora configure --section skills");
    expect(lines.join("\n")).toContain("afora configure --section web");
    expect(lines.join("\n")).toContain("afora configure --section gateway");
  });
  ...
  it("points one-shot memory import at interactive owners", async () => {
    ...
    expect(lines.join("\n")).toContain("afora onboard");
  });

  it("routes one-shot model setup through the verified Afora flow", async () => {
    const { runtime, lines } = createSystemAgentTestRuntime();

    const result = await executeSystemAgentOperation({ kind: "model-setup" }, runtime);

    expect(result.applied).toBe(false);
    expect(lines.join("\n")).toContain("Run `afora onboard` on the machine running Afora");
    expect(lines.join("\n")).toContain("Stop the Afora host");
    expect(lines.join("\n")).toContain("restart the host");
    expect(lines.join("\n")).not.toContain("afora configure --section model");
  });
  ...
  it("prints discovered channel metadata and sorted unknown-channel choices", async () => {
    ...
    expect(knownOutput).toContain("https://docs.afora.ai/channels/slack");
