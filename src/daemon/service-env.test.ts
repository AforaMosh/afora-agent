...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.AFORA_GATEWAY_PORT).toBe("18789");
    expect(env.AFORA_GATEWAY_TOKEN).toBeUndefined();
    expect(env.AFORA_SERVICE_MARKER).toBe("afora");
    expect(env.AFORA_SERVICE_KIND).toBe("gateway");
    expect(env).not.toHaveProperty("AFORA_SERVICE_VERSION");
    expect(env.AFORA_SYSTEMD_UNIT).toBe("afora-gateway.service");
    expect(env.AFORA_WINDOWS_TASK_NAME).toBe("Afora Gateway");
    expect(env.AFORA_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    if (process.platform === "darwin") {
      expect(env.AFORA_LAUNCHD_LABEL).toBe("ai.afora.gateway");
    }
  });

  it("sets the Afora-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through AFORA_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.AFORA_WRAPPER).toBe("/usr/local/bin/afora-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".afora", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", AFORA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.AFORA_SYSTEMD_UNIT).toBe("afora-gateway-work.service");
    expect(env.AFORA_WINDOWS_TASK_NAME).toBe("Afora Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.AFORA_LAUNCHD_LABEL).toBe("ai.afora.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        AFORA_PROFILE: "work",
        AFORA_SYSTEMD_UNIT: "afora-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.AFORA_SYSTEMD_UNIT).toBe("afora-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        AFORA_SYSTEMD_UNIT: "afora-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", AFORA_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    ...
    expect(env.AFORA_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the Afora-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });
  ...
  it("passes through AFORA_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", AFORA_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.AFORA_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through AFORA_GATEWAY_PASSWORD for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", AFORA_GATEWAY_PASSWORD: " node-password " },
    });
    expect(env.AFORA_GATEWAY_PASSWORD).toBe("node-password");
  });
  ...
  it("passes through AFORA_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", AFORA_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.AFORA_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits AFORA_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        AFORA_GATEWAY_TOKEN: "   ",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.AFORA_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.AFORA_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".afora"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", AFORA_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".afora-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", AFORA_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".afora"));
  });

  it("uses AFORA_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", AFORA_STATE_DIR: "/var/lib/afora" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/afora"));
  });

  it("expands ~ in AFORA_STATE_DIR", () => {
    const env = { HOME: "/Users/test", AFORA_STATE_DIR: "~/afora-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/afora-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { AFORA_STATE_DIR: "C:\\State\\afora" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\afora");
  });
});
