...
async function withPreparedGatewayTask(
  ...
  await withWindowsEnv("afora-win-stop-", async ({ env }) => {
...
describe("Scheduled Task stop/restart cleanup", () => {
  it("suspends a task whose Settings.Enabled value uses the default", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query", "/TN", "Afora Gateway", "/XML"],
        ["/Change", "/TN", "Afora Gateway", "/DISABLE"],
      ]);
    });
  });
  ...
  it("preserves an already-disabled task", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([["/Query", "/TN", "Afora Gateway", "/XML"]]);
    });
  });
  ...
  it("restores an enabled task after an ambiguous disable failure", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query", "/TN", "Afora Gateway", "/XML"],
        ["/Change", "/TN", "Afora Gateway", "/DISABLE"],
        ["/Change", "/TN", "Afora Gateway", "/ENABLE"],
      ]);
    });
  });
  ...
  it("reenables a task after the update window", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([["/Change", "/TN", "Afora Gateway", "/ENABLE"]]);
    });
  });
  ...
  it.each([
    ...
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query"],
        ["/Query", "/TN", "Afora Gateway"],
        ["/End", "/TN", "Afora Gateway"],
      ]);
      expect(spawnSync).toHaveBeenCalledOnce();
        ...
        const processes = [
          {
            ProcessId: 3131,
            CommandLine:
              '"C:\\Program Files\\nodejs\\node.exe" "C:\\other-afora.cjs" gateway --port 18789',
          },
          ...(!forced
            ? [
                {
  ...
  it("starts a registered task and ignores audit observer failures", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toContainEqual(["/Run", "/TN", "Afora Gateway"]);
  ...
  it("kills the owned gateway process and waits for port release before restart", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls).toEqual([
        ...
        ["/Query", "/TN", "Afora Gateway"],
        ["/End", "/TN", "Afora Gateway"],
        ["/Run", "/TN", "Afora Gateway"],
        ["/Query"],
        ["/Query", "/TN", "Afora Gateway", "/V", "/FO", "LIST"],
        ["/Query"],
        ["/Query", "/TN", "Afora Gateway", "/V", "/FO", "LIST"],
      ]);
    });
  ...
  it("throws when /Run fails during restart", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls.at(-1)).toEqual(["/Run", "/TN", "Afora Gateway"]);
    });
  });
});
