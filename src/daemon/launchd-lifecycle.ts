...
function formatLaunchAgentLeftUnloadedError(params: {
  ...
  return [
    ...
    `Fix: run \`afora gateway start\`, or \`launchctl bootstrap ${params.domain} ${params.plistPath}\`.`,
  ].join("\n");
}
...
export async function restartLaunchAgent({
  ...
  // `afora gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    try {
      await bootstrapLaunchAgentOrThrow({
        ...
        serviceTarget,
        plistPath,
        actionHint: "afora gateway restart",
        onMutation: reportMutation,
