...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.AFORA_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.AFORA_SERVICE_MARKER) === "afora" &&
    Boolean(normalizeOptionalString(env.AFORA_SERVICE_KIND))
  ) {
