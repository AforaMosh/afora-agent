...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    ...
    Boolean(params.serviceEnvironment.AFORA_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.AFORA_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    AFORA_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
