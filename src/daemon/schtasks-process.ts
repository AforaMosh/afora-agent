...
export function resolveScheduledTaskCommandPort(
  ...
  return (
    ...
    parseTcpPort(command?.environment?.AFORA_GATEWAY_PORT) ??
    parseTcpPort(env.AFORA_GATEWAY_PORT)
  );
}
...
async function resolveScheduledTaskProcess(
  ...
  // Match full persisted argv so a same-port Afora process cannot impersonate this task.
...
export function shouldManageGatewayListenerPort(env: GatewayServiceEnv): boolean {
  return normalizeLowercaseStringOrEmpty(env.AFORA_SERVICE_KIND) !== NODE_SERVICE_KIND;
}
