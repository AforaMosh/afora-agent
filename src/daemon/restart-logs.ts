...
function resolveGatewayLogPrefix(env: GatewayServiceEnv): string {
  return env.AFORA_LOG_PREFIX?.trim() || "gateway";
}

function resolveMacLaunchAgentLogPrefix(env: GatewayServiceEnv): string {
  return (
    env.AFORA_LOG_PREFIX?.trim() || `gateway${resolveGatewayProfileSuffix(env.AFORA_PROFILE)}`
  );
}
...
function resolveMacLaunchAgentLogPaths(env: GatewayServiceEnv): GatewayLogPaths {
  ...
  const logDir = path.posix.join(home, "Library", "Logs", "afora");
...
export function resolveGatewaySupervisorLogPaths(
  ...
  // Afora state dir so generated service users can create the directory.
...
export function appendGatewayLifecycleAuditLog(
  ...
  try {
    ...
    fs.appendFileSync(
      logPath,
      `[${new Date().toISOString()}] afora gateway lifecycle ${fields.join(" ")}\n`,
      "utf8",
    );
  } catch {
...
export function renderCmdRestartLogSetup(env: GatewayServiceEnv): {
  ...
  return {
    ...
    lines: [
      ...
      `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] afora restart log initialized`,
    ],
  };
}
