...
function createGatewayAudit({
  ...
  return auditGatewayServiceConfig({
    ...
    command: {
      ...
      environment: {
        PATH: pathLocal,
        ...(serviceToken ? { AFORA_GATEWAY_TOKEN: serviceToken } : {}),
        ...extraEnvironment,
      },
...
async function writeSystemdUnitForAudit(home: string, lines: string[]) {
  ...
  const unitPath = path.join(unitDir, "afora-gateway.service");
...
describe("auditGatewayServiceConfig", () => {
  ...
  it("accepts Linux minimal PATH with user directories", async () => {
    const env = { HOME: "/tmp/afora-testuser", PNPM_HOME: "/opt/pnpm" };
  ...
  it("accepts canonical macOS gateway service PATH without user-bin defaults", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "afora-service-audit-home-"));
    try {
  ...
  it("still requires explicit env-configured tool roots in gateway service PATH", async () => {
    const audit = await auditGatewayServiceConfig({
      env: { HOME: "/tmp/afora-testuser", PNPM_HOME: "/opt/pnpm" },
      platform: "linux",
      command: {
        programArguments: ["/usr/bin/node", "gateway"],
  ...
  it("flags stale Linux version-manager and package-manager PATH entries", async () => {
    const env = { HOME: "/tmp/afora-testuser-nonminimal" };
    const minimalPath = buildMinimalServicePath({ platform: "linux", env });
    const staleEntries = [
      `${env.HOME}/.volta/bin`,
      `${env.HOME}/.asdf/shims`,
  ...
  it("accepts an expected active Afora bin even when it looks package-managed", async () => {
  ...
  it("accepts Linux fnm aliases/default without requiring the legacy current symlink", async () => {
    const env = {
      HOME: "/tmp/afora-testuser",
      FNM_DIR: "/tmp/afora-testuser/.local/share/fnm",
    };
    ...
    const audit = await auditGatewayServiceConfig({
      ...
      command: {
        programArguments: [
          "/bin/zsh",
          "-lc",
          "exec /usr/bin/node /opt/afora/dist/index.js gateway --port 18890",
        ],
        environment: {},
      },
    });
  ...
  it.each(["process", "none"])(
    ...
    async (killMode) => {
      const home = await fs.mkdtemp(path.join(os.tmpdir(), "afora-service-audit-killmode-"));
  ...
  it("accepts systemd RestartSec values with seconds suffixes", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "afora-service-audit-restartsec-"));
    await writeSystemdUnitForAudit(home, [
      "After=network-online.target",
      "Wants=network-online.target",
      "RestartSec=5s",
  ...
  it("flags inline managed service env values from the service key list", async () => {
    const audit = await createGatewayAudit({
      extraEnvironment: {
        AFORA_SERVICE_MANAGED_ENV_KEYS: "TAVILY_API_KEY,OPENROUTER_API_KEY",
...
describe("checkTokenDrift", () => {
  ...
  it("detects drift when config has token but service has different token", () => {
    ...
    expect(result).toStrictEqual({
      ...
      detail: "Run `afora gateway install --force` to sync the token.",
      level: "recommended",
    });
  });
...
describe("legacy gateway service version metadata", () => {
  it("does not treat install-time version metadata as runtime truth", async () => {
    const legacyAudit = await createGatewayAudit({
      extraEnvironment: { AFORA_SERVICE_VERSION: "2026.4.15-beta.1" },
    });
