...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds AFORA_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `afora gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service AFORA_GATEWAY_TOKEN does not match gateway.auth.token in afora.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.AFORA_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.AFORA_GATEWAY_TOKEN);
}
...
async function auditGatewayRuntime(
  ...
  if (isBunRuntime(execPath)) {
    issues.push({
      ...
      message: "Gateway service uses Bun; Afora runtime state requires node:sqlite.",
      detail: execPath,
      level: "recommended",
    });
    return;
