...
describe("resolveLaunchAgentLabel", () => {
  it("resolves default, profile, and explicit labels", () => {
    expect(resolveLaunchAgentLabel()).toBe("ai.afora.gateway");
    expect(resolveLaunchAgentLabel({ AFORA_PROFILE: "work" })).toBe("ai.afora.work");
    expect(
  ...
  it("rejects labels that cannot be passed safely to launchd", () => {
    expect(() =>
      resolveLaunchAgentLabel({ AFORA_LAUNCHD_LABEL: "ai.afora.$(echo injected)" }),
