...
async function resolveLaunchAgentEnvironmentWrapperOverwriteWarnings(params: {
  ...
  return [
    `Existing generated LaunchAgent env wrapper at ${params.wrapperPath} contains custom behavior and will be overwritten; move custom behavior to afora gateway install --wrapper <path> or AFORA_WRAPPER.`,
  ];
}
...
export async function publishLaunchAgentPlist(params: {
  ...
  const temporaryPath = `${params.plistPath}.afora-${randomUUID()}.tmp`;
    ...
    try {
      ...
      try {
        if (previousContents === null) {
          ...
          const rollbackPath = `${params.plistPath}.afora-${randomUUID()}.rollback`;
          try {
...
export async function rewriteLaunchAgentPlistForRestart({
  ...
  const canonicalEnvironment = {
    ...
    AFORA_SERVICE_VERSION: undefined,
  };
  const prepared = await prepareLaunchAgentProgramArguments({
    env,
    label,
