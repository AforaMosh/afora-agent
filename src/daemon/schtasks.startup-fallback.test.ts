...
function resolveStartupEntryPath(env: Record<string, string>, extension = "cmd") {
  const taskName = env.AFORA_WINDOWS_TASK_NAME ?? "Afora Gateway";
  return path.join(
...
async function writeNodeScript(env: Record<string, string>, port = "18789") {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "AFORA_SERVICE_KIND=node"`,
      `set "AFORA_GATEWAY_PORT=${port}"`,
      `"C:\\bin\\afora.cmd" node run --host 127.0.0.1 --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
        ...
        stdout: JSON.stringify(
          processAlive
            ? [
                {
                  ProcessId: processId,
                  CommandLine: "C:\\bin\\afora.cmd node run --host 127.0.0.1 --port 18789",
                },
...
function expectStartupFallbackSpawn() {
  ...
  expect((options.env as Record<string, string> | undefined)?.AFORA_GATEWAY_PORT).toBe("18789");
...
function installGatewayScheduledTask(
  ...
  return installScheduledTask({
    ...
    environment: { AFORA_GATEWAY_PORT: port },
...
function installNodeScheduledTask(env: Record<string, string>, stdout = new PassThrough()) {
  return installScheduledTask({
    ...
    stdout,
    programArguments: ["node", "afora", "node", "run", "--host", "127.0.0.1", "--port", "18789"],
    environment: {
      AFORA_SERVICE_KIND: "node",
...
describe("Windows startup fallback", () => {
  it("uses the locale-independent task probe when a scheduled task is missing", async () => {
    await withWindowsEnv("afora-win-startup-", async ({ env }) => {
      schtasksResponses.push(
        { code: 0, stdout: "", stderr: "" },
  ...
  it("migrates an exact persisted wrapper that owns the replacement port", async () => {
    await withWindowsEnv("afora-win-startup-", async ({ env }) => {
      ...
      await fs.writeFile(
        ...
        [
          "@echo off",
          'set "AFORA_GATEWAY_PORT=18789"',
          '"C:\\bin\\afora-doppler.exe" gateway --port 18789',
          "",
        ].join("\r\n"),
        "utf8",
      );
            ...
            stdout: JSON.stringify(
              processAlive
                ? [
                    {
                      ProcessId: 4242,
                      CommandLine: '"C:\\bin\\afora-doppler.exe" gateway --port 18789',
                    },
                    { ProcessId: 9999, CommandLine: "powershell.exe" },
                  ]
                : [{ ProcessId: 9999, CommandLine: "powershell.exe" }],
        ...
        if (
          ...
          return makeSpawnSyncResult({
            stdout: JSON.stringify([
              {
                ProcessId: 3131,
                CommandLine: "C:\\manual\\afora.cmd gateway --port 18789",
              },
              {
                ProcessId: 4242,
                CommandLine:
            ...
            ? [
                {
                  pid: 4242,
                  command: "node.exe",
                  commandLine: 'node "C:\\afora\\dist\\index.js" gateway --port 18789',
                },
              ]
            : [],
        hints: [],
  ...
  it("accepts a clean hidden-launcher exit when its gateway listener is running", async () => {
    await withWindowsEnv("afora-win-startup-", async ({ env }) => {
      const hiddenEnv = { ...env, AFORA_WINDOWS_TASK_HIDDEN_LAUNCHER: "1" };
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 5151,
                CommandLine: "node afora node run --host 127.0.0.1 --port 18789",
              },
            ]),
            stderr: "",
            status: 0,
  ...
  it("does not report a node task as running from a gateway listener", async () => {
    await withWindowsEnv("afora-win-startup-", async ({ env }) => {
      env.AFORA_SERVICE_KIND = "node";
      env.AFORA_WINDOWS_TASK_NAME = "Afora Node";
      findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4242]);
      schtasksResponses.push(
        { code: 0, stdout: "", stderr: "" },
        { code: 0, stdout: notYetRunTaskQueryOutput(), stderr: "" },
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 4242,
                CommandLine: "C:\\manual\\afora.cmd node run --host 127.0.0.1 --port 18789",
              },
              {
                ProcessId: 5151,
                CommandLine: "C:\\bin\\afora.cmd node run --host 127.0.0.1 --port 18789",
  ...
  it("kills the Startup fallback runtime even when the CLI env omits the gateway port", async () => {
    ...
    await withWindowsEnv("afora-win-startup-", async ({ env }) => {
      ...
      delete envWithoutPort.AFORA_GATEWAY_PORT;
