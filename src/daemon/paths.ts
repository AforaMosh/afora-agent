...
export function resolveGatewayStateDir(env: Record<string, string | undefined>): string {
  const override = normalizeOptionalString(env.AFORA_STATE_DIR);
  if (override) {
    const home = override.startsWith("~") ? resolveDaemonHomeDir(env) : undefined;
    return resolveUserPathWithHome(override, home);
  }
  const home = resolveDaemonHomeDir(env);
  const suffix = resolveGatewayProfileSuffix(env.AFORA_PROFILE);
  // Profile suffixes isolate managed service files while preserving the default
  // historical ~/.afora state path.
  return path.join(home, `.afora${suffix}`);
}

export function resolveGatewayTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = normalizeOptionalString(env.AFORA_TASK_SCRIPT);
  if (override) {
    return override;
  }
  const scriptName = normalizeOptionalString(env.AFORA_TASK_SCRIPT_NAME) || "gateway.cmd";
  if (/[/\\]|\.\./.test(scriptName)) {
    throw new Error(
      `AFORA_TASK_SCRIPT_NAME must be a file name only, not a path: ${scriptName}`,
    );
  }
