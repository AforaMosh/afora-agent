...
describe("installScheduledTask", () => {
  ...
  async function withUserProfileDir(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-schtasks-install-"));
    const env = {
      USERPROFILE: tmpDir,
  ...
  function expectInitialTaskQueries(taskName = "Afora Gateway"): void {
    expect(schtasksCalls[0]).toEqual(["/Query"]);
    expect(schtasksCalls[1]).toEqual(["/Query", "/TN", taskName]);
  }

  function expectTaskRunCall(index: number, taskName = "Afora Gateway"): void {
  ...
  it("writes version-free gateway and node descriptions", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      expect(gatewayScript).toContain("rem Afora Gateway");
      expect(gatewayScript).not.toContain("AFORA_SERVICE_VERSION");
      expect(xmlPayloadCaptures.at(-1)?.xml).toContain(
        "<Description>Afora Gateway</Description>",
      );
      ...
      const node = await installScheduledTask({
        env: {
          ...
          AFORA_TASK_SCRIPT_NAME: "node.cmd",
        },
        stdout: new PassThrough(),
      ...
      expect(nodeScript).toContain("rem Afora Node Host");
      expect(nodeScript).not.toContain("AFORA_SERVICE_VERSION");
      expect(xmlPayloadCaptures.at(-1)?.xml).toContain(
        "<Description>Afora Node Host</Description>",
      );
    });
  });
  ...
  it("writes quoted set assignments and escapes metacharacters", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      expect(schtasksCalls[0]).toEqual(["/Query"]);
      expect(schtasksCalls[1]).toEqual(["/Query", "/TN", "Afora Gateway"]);
      ...
      expect(schtasksCalls[4]).toEqual(["/Run", "/TN", "Afora Gateway"]);
    });
  });
  ...
  it("uses the hidden launcher for generated Windows gateway service installs", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      const callerEnv: Record<string, string | undefined> = {
        ...
        USERDOMAIN: "WORKSTATION",
        USERNAME: "alice",
        AFORA_WINDOWS_TASK_NAME: "Afora Custom Gateway",
      };
      ...
      expect(callerEnv.AFORA_WINDOWS_TASK_HIDDEN_LAUNCHER).toBeUndefined();
      expect(gatewayEnv.AFORA_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
      expect(gatewayEnv.AFORA_WINDOWS_TASK_NAME).toBe("Afora Gateway");
      ...
      expect(schtasksCalls[2]?.slice(0, 5)).toEqual([
        ...
        "/F",
        "/TN",
        "Afora Custom Gateway",
        "/XML",
      ]);
      expect(schtasksCalls[2]?.slice(6)).toEqual(["/RU", "WORKSTATION\\alice", "/NP"]);
      const captured = xmlPayloadCaptures.find((entry) => entry.index === 2);
      expect(captured?.xml).toContain("gateway.vbs</Command>");
      expect(script).toContain('set "AFORA_WINDOWS_TASK_NAME=Afora Custom Gateway"');
      expect(launcher).toContain("WScript.Shell");
      expect(launcher).toContain(`Run """${scriptPath}""", 0, False`);
      expectTaskRunCall(3, "Afora Custom Gateway");
    });
  });
  ...
  it("omits /RU for workgroup accounts so schtasks can use the current local user", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      expectInitialTaskQueries();
      const createCall = schtasksCalls[2];
      expect(createCall?.slice(0, 5)).toEqual(["/Create", "/F", "/TN", "Afora Gateway", "/XML"]);
  ...
  it("updates existing tasks to use the hidden launcher when requested", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      const { scriptPath } = await installDefaultGatewayTask({
        ...
        USERDOMAIN: "WORKSTATION",
        USERNAME: "alice",
        AFORA_WINDOWS_TASK_HIDDEN_LAUNCHER: "true",
      });
      const launcherPath = scriptPath.replace(/\.cmd$/i, ".vbs");
  ...
  it("does not persist a frozen PATH snapshot into the generated task script", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      expect(script).toContain('set "AFORA_GATEWAY_PORT=18789"');
    });
  });
