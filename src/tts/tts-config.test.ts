...
describe("shouldAttemptTtsPayload", () => {
  ...
  beforeAll(() => {
    root = mkdtempSync(path.join(tmpdir(), "afora-tts-config-"));
  });
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["AFORA_TTS_PREFS"]);
    dir = path.join(root, `case-${caseId++}`);
    mkdirSync(dir, { recursive: true });
    prefsPath = path.join(dir, "tts.json");
    process.env.AFORA_TTS_PREFS = prefsPath;
  });
  ...
  it("skips TTS when config, prefs, and session state leave auto mode off", () => {
    expect(shouldAttemptTtsPayload({ cfg: {} as AforaConfig })).toBe(false);
  });
  ...
  it("honors session auto state before prefs and config", () => {
    ...
    const cfg = { tts: { auto: "off" } } as AforaConfig;
  ...
  it("uses local prefs before config auto mode", () => {
    ...
    writeFileSync(prefsPath, JSON.stringify({ tts: { auto: "off" } }));
    expect(shouldAttemptTtsPayload({ cfg: { tts: { enabled: true } } as AforaConfig })).toBe(
      false,
    );
  });
