// Covers the hosted Afora marketplace feed entries command.
...
async function createTimelinePath(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "afora-marketplace-entries-"));
...
describe("plugins marketplace entries", () => {
        ...
        entries: [
          {
            ...
            afora: { plugin: { id: "acme-calendar", label: "Acme Calendar" } },
          },
        ],
      }),
    );
  ...
  it("prints bundled fallback entries without failing", async () => {
      ...
      entries: [
        {
          ...
          afora: {
            ...
            install: {
              clawhubSpec: "clawhub:@afora/acpx",
              npmSpec: "@afora/acpx",
              defaultChoice: "npm",
            },
          },
    ...
    expect(output).toContain("@afora/acpx");
    expect(output).not.toContain("clawhub:@afora/acpx");
  ...
  it("emits bounded diagnostics for feed entry listing", async () => {
    ...
    vi.stubEnv("AFORA_DIAGNOSTICS", "1");
