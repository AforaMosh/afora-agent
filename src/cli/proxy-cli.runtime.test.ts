...
describe("proxy cli runtime", () => {
  const envKeys = [
    "AFORA_STATE_DIR",
    "AFORA_DEBUG_PROXY_CERT_DIR",
    "AFORA_DEBUG_PROXY_SESSION_ID",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "afora-proxy-cli-runtime-"));
    process.env.AFORA_STATE_DIR = tempDir;
    process.env.AFORA_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.AFORA_DEBUG_PROXY_ENABLED;
    delete process.env.AFORA_DEBUG_PROXY_SESSION_ID;
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, AFORA_PROXY_URL, or --proxy-url so it uses a reachable http:// or https:// proxy.\n",
    );
  });
  ...
  it("prints actionable output when proxy config is disabled and missing", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        ...
        source: "disabled",
        errors: [
          "proxy validation requires proxy.enabled=true with proxy.proxyUrl or AFORA_PROXY_URL, or --proxy-url",
        ],
      },
      checks: [],
    });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or AFORA_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
      ...
      `${JSON.stringify(
        {
          ...
          config: {
            ...
            source: "missing",
            errors: [
              "proxy validation requires proxy.proxyUrl, --proxy-url, or AFORA_PROXY_URL",
            ],
          },
          checks: [],
        },
