...
function resolveMarketplaceOptionError(opts: RunPluginInstallCommandParams["opts"]): string | null {
  if (opts.link) {
    return `--link is not supported with --marketplace. Remove --link, or install a local path with ${formatCliCommand(`afora plugins install --link <path> ${NON_CLAWHUB_INSTALL_FORCE_FLAG}`)}.`;
  }
  if (opts.pin) {
    return `--pin is not supported with --marketplace. Use ${formatCliCommand(`afora plugins install <plugin> --marketplace <name> ${NON_CLAWHUB_INSTALL_FORCE_FLAG}`)} without --pin.`;
  }
  return null;
}
...
function resolveSourceOptionError(
  ...
  if (sourcePlan.request.source === "git" && opts.link) {
    return `--link is not supported with git: installs. Use ${formatCliCommand(`afora plugins install git:<repo>@<ref> ${NON_CLAWHUB_INSTALL_FORCE_FLAG}`)} for Git installs or ${formatCliCommand(`afora plugins install --link <path> ${NON_CLAWHUB_INSTALL_FORCE_FLAG}`)} for local paths.`;
  }
  if (sourcePlan.request.source === "git" && opts.pin) {
    return `--pin is not supported with git: installs. Pin the ref in the spec instead, for example ${formatCliCommand(`afora plugins install git:<repo>@<ref> ${NON_CLAWHUB_INSTALL_FORCE_FLAG}`)}.`;
  }
  if (
    opts.pin &&
  ...
  if (opts.link && sourcePlan.request.source !== "local") {
    return `--link requires a local path. Run ${formatCliCommand(`afora plugins install --link <path> ${NON_CLAWHUB_INSTALL_FORCE_FLAG}`)}.`;
  }
  return null;
}
