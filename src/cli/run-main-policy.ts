...
export function shouldUseSetupOnboardConfigureHelpFastPath(
  ...
  if (env.AFORA_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
  ...
  if (parentPluginId) {
    if (allow.length > 0 && !allow.includes(parentPluginId)) {
      if (parentPluginId === normalizedPluginId) {
        return (
          `The \`afora ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (
      ...
      return (
        `The \`afora ${normalizedPluginId}\` command is provided by the ` +
        `"${parentPluginId}" plugin, but that bundled plugin is disabled by default. Run ` +
        `\`afora plugins enable ${parentPluginId}\` to enable that CLI surface.`
      );
    }
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`afora ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
    ...
    if (ownerEnabled) {
      ...
      if (toolOwner.availability === "manifest-only") {
        return (
          ...
          "Run `afora --help` to see available CLI subcommands."
        );
      }
      return (
