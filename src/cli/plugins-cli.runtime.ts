// Runtime implementations for `afora plugins` subcommands. Heavy plugin modules stay
...
function formatBlockedRuntimePluginGuidance(params: {
  ...
  const alternative =
    pluginId === "acpx"
      ...
      : 'change the runtime policy to "afora"';
...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "afora doctor --fix" to install ${installSpec}, or install it manually with "afora plugins install ${installSpec}".`,
    ];
  });
}
...
async function runPluginsEnableCommandUnlocked(idInput: string): Promise<void> {
  ...
  let next: AforaConfig = enableResult.config;
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(`${theme.muted("Repair:")} ${theme.command("afora plugins registry --refresh")}`);
  }
...
export async function runPluginsDoctorCommand(opts: PluginDoctorOptions = {}): Promise<void> {
    ...
    defaultRuntime.writeJson({
      ...
      sourceShadowing: shadowed.map((entry) => {
        ...
        return {
          ...
          repair: [
            `afora plugins inspect ${entry.pluginId ?? "<plugin-id>"}`,
  ...
  if (shadowed.length > 0) {
    ...
    for (const diag of shadowed) {
      ...
      lines.push("  repair:");
      lines.push("    afora plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
      lines.push("    edit or remove the config-selected plugin source");
      lines.push("    afora plugins registry --refresh");
      lines.push("    afora gateway restart --force");
    }
  }
  ...
  const docs = formatDocsLink("/plugin", "docs.afora.ai/plugin");
  lines.push("");
...
const MARKETPLACE_GATEWAY_RESTART_GUIDANCE =
  'The running Gateway could not refresh its marketplace catalog. Run "afora gateway restart" to apply the current catalog state.';

/** List entries from the configured Afora marketplace feed. */
...
/** Refresh the configured Afora marketplace feed snapshot. */
