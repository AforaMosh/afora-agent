...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-payload-smoke-"));
  });
  afterEach(async () => {
    vi.restoreAllMocks();
  ...
  async function linkAforaPeerToHost(dir: string): Promise<void> {
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(resolveTestHostRoot(), path.join(dir, "node_modules", "afora"), "junction");
  }
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@afora/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("checks a selected manifest root without an installed-index record", async () => {
    ...
    await writePackage(
      dir,
      { name: "@afora/codex", afora: { extensions: ["./index.js"] } },
      "export default {};",
    );
  ...
  it.each([
    ...
    async ({ dependencyField, expectedFailures }) => {
      ...
      const staleHostDir = path.join(dir, "node_modules", "afora");
      ...
      expect(authoritativeResult.failures[0]?.reason).toBe("missing-afora-peer-link");
    },
  );
  ...
  it("reports a failure when the main entry file is missing on disk", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, { name: "@afora/brave", main: "dist/index.js" });
    const result = await runPluginPayloadSmokeCheck({
      records: { brave: { source: "npm", installPath: dir } },
      env: {},
    });
  ...
  it("accepts a manifest with no main field (Afora plugins commonly use `exports` or `afora.extensions`)", async () => {
    const dir = path.join(tmpRoot, "matrix");
    await writePackage(dir, { name: "@afora/plugin-matrix" });
    const result = await runPluginPayloadSmokeCheck({
  ...
  it("accepts a manifest that declares an existing `afora.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
  ...
  it("reports a failure when `afora.extensions` contains invalid entries", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@afora/brave-plugin",
      afora: { extensions: ["./index.js", " "] },
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json afora.extensions[1] must be a non-empty string",
      },
    ]);
  });
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json afora.extensions is empty",
      },
    ]);
  });
  ...
  it("reports a failure when an afora peer link is missing", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
        name: "@afora/codex",
        main: "dist/index.js",
        peerDependencies: { afora: ">=2026.5.18-beta.1" },
      },
      "export default {};\n",
    );
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        detail: `Plugin declares peerDependency "afora" but peer link audit failed: missing ${path.join(
          dir,
          "node_modules",
          "afora",
        )}.`,
  ...
  it("reports a failure when an afora peer link is a stale real directory", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const stalePeerDir = path.join(dir, "node_modules", "afora");
  ...
  it("reports a failure when a direct afora dependency resolves to a stale copied host", async () => {
  ...
  it("reports a failure when an afora peer link points at the wrong package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const wrongHostRoot = path.join(tmpRoot, "old-afora");
    await fs.mkdir(wrongHostRoot, { recursive: true });
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(wrongHostRoot, path.join(dir, "node_modules", "afora"), "junction");
  ...
  it("accepts an afora peer link when it resolves to the host package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
  ...
  it("reports a failure when an `afora.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@afora/brave-plugin",
