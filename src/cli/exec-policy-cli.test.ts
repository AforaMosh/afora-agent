...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: AforaConfig) => void }) => {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: AforaConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: AforaConfig }>
    >(async () => ({
      path: "/tmp/afora.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: AforaConfig) => void }) => {
  ...
  it("renders an unstored fresh-install policy as defaults instead of missing", async () => {
    mocks.readExecApprovalsSnapshot.mockImplementationOnce(() => ({
      path: "~/.afora/state/afora.sqlite#exec_approvals_config",
      exists: false,
      raw: null,
      hash: "missing-hash",
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/afora.json");
    ...
    expect(output).not.toContain("/tmp/afora.json\nforged");
