...
>(() => ({ mode: "disabled", detail: "AFORA_NO_RESPAWN" }));
...
describe("runGatewayLoop", () => {
  ...
  it("records a typed reason for media-migration startup failures", async () => {
    await withIsolatedSignals(async () => {
      const failure = new AforaAgentDatabaseMediaMigrationRequiredError(
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      expect(gatewayLog.warn).toHaveBeenNthCalledWith(
        ...
        "An unauthorized SIGUSR1 restart signal was received and ignored. " +
          "If a pending gateway restart needs to be applied, run `afora gateway restart` " +
  ...
  it("releases the lock before exiting on spawned restart", async () => {
    vi.clearAllMocks();
    peekGatewaySigusr1RestartReason.mockReturnValue(undefined);
    const originalTraceEnv = process.env.AFORA_GATEWAY_RESTART_TRACE;
    ...
    try {
      await withIsolatedSignals(async ({ captureSignal }) => {
        ...
        expect(respawnOpts?.env?.AFORA_GATEWAY_RESTART_TRACE_STARTED_AT_MS).toMatch(/^\d/u);
        expect(respawnOpts?.env?.AFORA_GATEWAY_RESTART_TRACE_LAST_AT_MS).toMatch(/^\d/u);
      ...
      if (originalTraceEnv === undefined) {
        ...
        process.env.AFORA_GATEWAY_RESTART_TRACE = originalTraceEnv;
      }
    }
  });
  ...
  it("waits briefly before exiting on launchd supervised restart", async () => {
    ...
    try {
      ...
      vi.useRealTimers();
      delete process.env.AFORA_LAUNCHD_LABEL;
      if (originalPlatformDescriptor) {
  ...
  it("records external ownership even when native supervisor markers are inherited", async () => {
    ...
    try {
      ...
      delete process.env.AFORA_SUPERVISOR_MODE;
      delete process.env.AFORA_LAUNCHD_LABEL;
    }
  });
