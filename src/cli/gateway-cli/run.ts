...
import { findAforaAgentDatabaseMediaMigrationRequiredError } from "../../state/afora-agent-db-migration-required.js";
import { findAforaStateDatabaseSchemaMigrationRequiredError } from "../../state/afora-state-db-schema-migration-required.js";
...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.AFORA_GATEWAY_STARTUP_TRACE);
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("afora setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("afora onboard --mode local")}\` or \`${formatCliCommand("afora setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
async function readGatewayStartupConfig(params: {
  ...
  let blockedRecoveryConfig: AforaConfig | null = null;
...
function resolveGatewayStartupFailureExitCode(err: unknown): number {
  return isInvalidConfigError(err) ||
    findAforaAgentDatabaseMediaMigrationRequiredError(err) ||
    findAforaStateDatabaseSchemaMigrationRequiredError(err)
    ? EXIT_CONFIG_ERROR
    : 1;
}
...
function createConfiguredGatewayHealthProbe(cfg: AforaConfig) {
...
async function runGatewayCommandOnce(opts: GatewayRunOpts, hooks: GatewayRunRuntimeHooks = {}) {
  ...
  const isDevProfile = normalizeOptionalLowercaseString(process.env.AFORA_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.AFORA_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.AFORA_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.AFORA_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (snapshot) {
    ...
    const finalConfigEnteredServiceMode = Boolean(process.env.AFORA_SERVICE_MARKER?.trim());
  ...
  if (process.env.AFORA_SERVICE_MARKER?.trim()) {
  ...
  if (opts.force) {
    ...
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("afora gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or AFORA_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set AFORA_GATEWAY_TOKEN or AFORA_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or AFORA_GATEWAY_TOKEN/AFORA_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("afora gateway stop")}`,
      );
      try {
    ...
    if (findAforaAgentDatabaseMediaMigrationRequiredError(err)) {
      try {
        const { parkCurrentLaunchAgentForMaintenance } = await import("../../daemon/launchd.js");
        if (await parkCurrentLaunchAgentForMaintenance()) {
          gatewayLog.error(
            `gateway requires offline media migration; parked the managed LaunchAgent. Run ${formatCliCommand("afora doctor --fix")} to repair and restart it.`,
          );
        }
      } catch (parkError) {
        gatewayLog.error(
    ...
    if (findAforaStateDatabaseSchemaMigrationRequiredError(err)) {
      try {
        const { parkCurrentLaunchAgentForMaintenance } = await import("../../daemon/launchd.js");
        if (await parkCurrentLaunchAgentForMaintenance()) {
          gatewayLog.error(
            `gateway requires state database schema migration; parked the managed LaunchAgent. Run ${formatCliCommand("afora doctor --fix")} to repair and restart it.`,
          );
        }
      } catch (parkError) {
        gatewayLog.error(
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("afora gateway status --deep")} for diagnostics.`,
    );
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("afora.gatewayRunTestApi")] = testing;
}
