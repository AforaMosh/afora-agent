...
function shouldShowDebugDetails(
  ...
  return hasDebugArg(argv) || isTruthyEnvValue(env.AFORA_DEBUG);
}
...
export function formatCliFailureLines(options: FormatCliFailureOptions): string[] {
  ...
  const lines = [
    `[afora] ${options.title}`,
    `[afora] Reason: ${formatCliOperatorError(options.error, {
      argv: options.argv,
      env,
    })}`,
  ];
  ...
  if (showDebugDetails) {
    lines.push("[afora] Stack:");
    pushPrefixed(lines, formatUncaughtError(options.error));
  } else {
    lines.push("[afora] Debug: set AFORA_DEBUG=1 to include the stack trace.");
  }

  if (options.includeDoctorHint !== false) {
    lines.push(`[afora] Try: ${formatCliCommand("afora doctor", env)}`);
  }
  lines.push(`[afora] Help: ${formatCliCommand("afora --help", env)}`);
  return lines;
}
