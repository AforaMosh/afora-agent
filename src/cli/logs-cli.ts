...
import {
  ...
  toStringifiedError,
} from "@afora/normalization-core/error-coercion";
import {
...
function resolveLogsSystemdUnitName(runtime: LogsCliRuntimeModule, env: NodeJS.ProcessEnv): string {
  ...
  return `${runtime.resolveGatewaySystemdServiceName(env.AFORA_PROFILE)}.service`;
}
...
function createLogWriters(onOutputClosed?: () => void) {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `afora logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
async function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("afora doctor")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.afora.ai/cli/logs")}\n`,
    );
