...
vi.mock("../infra/path-env.js", () => ({
  ensureAforaCliOnPath: ensurePathMock,
}));
...
function runBareCli(): Promise<void> {
  return withInteractiveTty(() => runCli(["node", "afora"]));
}
...
async function expectNonInteractiveBareCliError(
  ...
  try {
    await withCliTty(false, () => runCli(["node", "afora"]));
...
describe("runCli exit behavior", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.AFORA_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
  ...
  it("does not load inactive provider cleanup modules for cold help", async () => {
    ...
    await withEnvAsync({ AFORA_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1" }, async () => {
      await runCli(["node", "afora", "nodes", "--help"]);
    });

    expect(parseAsync).toHaveBeenCalledWith(["node", "afora", "nodes", "--help"]);
  ...
  it("does not import dotenv for gateway forms without a workspace file", async () => {
    ...
    await runCli(["node", "afora", "gateway"]);
    await runCli(["node", "afora", "gateway", "run"]);
    tryRouteCliMock.mockResolvedValueOnce(false);
    buildProgramMock.mockReturnValueOnce({
      commands: [{ name: () => "gateway", aliases: () => [] }],
      parseAsync: commanderParseAsyncMock,
    });
    await runCli(["node", "afora", "--log-level", "debug", "gateway", "run"]);
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "afora", "status"]);
  ...
  it("passes config get machine ownership into route-first startup", async () => {
    ...
    await runCli(["node", "afora", "config", "get", "gateway.port"]);

    expect(tryRouteCliMock).toHaveBeenCalledWith(
      ["node", "afora", "config", "get", "gateway.port"],
      { machineOutput: true },
    );
  });
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "afora", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "afora", "agent", "--local"]);
  ...
  it("completes asynchronous teardown before returning to the outer entrypoint", async () => {
    ...
    await runCli(["node", "afora", "models", "status", "--probe"]);
  ...
  it("does not fail the command when MCP loopback cleanup fails", async () => {
    ...
    await expect(runCli(["node", "afora", "status"])).resolves.toBeUndefined();
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "afora", "config"]);
  ...
  it("suppresses startup progress for json output commands before full CLI parsing", async () => {
    ...
    await runCli(["node", "afora", "sessions", "--json", "--limit", "all"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "afora", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "afora", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "afora", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "afora", "gateway", "--force"],
    });
  ...
  it.each([
    ...
    const previousOverride = process.env.AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
    if (params.marker) {
      process.env.AFORA_SERVICE_MARKER = params.marker;
    } else {
      delete process.env.AFORA_SERVICE_MARKER;
    }
    if (params.override) {
      process.env.AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = params.override;
    } else {
      delete process.env.AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
    }
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const exitSpy = vi.spyOn(process, "exit").mockImplementation(((code?: number) => {
      throw new Error(`exit:${String(code)}`);
    ...
    try {
      await expect(runCli(["node", "afora", "gateway", ...params.flags])).rejects.toThrow(
      ...
      if (previousOverride === undefined) {
        ...
        process.env.AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = previousOverride;
      }
    }
  });
  ...
  it("blocks and revokes the destructive override when selected config declares service mode", async () => {
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          await expect(runCli(["node", "afora", "gateway"])).rejects.toThrow("exit:78");
  ...
  it("ignores service mode declared by an invalid selected config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(process.env.AFORA_SERVICE_MARKER).toBeUndefined();
        expect(process.env.AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBe("1");
      },
    );
  });

  it("guards the config selected by trusted global dotenv before the default config", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-global-selection-"));
    ...
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
          AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: undefined,
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () =>
            process.env.AFORA_CONFIG_PATH === selectedConfigPath
              ? {
                  exists: true,
                  valid: true,
                  sourceConfig: { gateway: { mode: "local" } },
          ...
          expect(process.env.AFORA_CONFIG_PATH).toBe(selectedConfigPath);
          expect(process.env.AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
  ...
  it("loads state dotenv before a custom config-root fallback", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-config-env-"));
    const stateDir = path.join(homeDir, ".afora");
    const configDir = path.join(homeDir, "profile");
    const configPath = path.join(configDir, "afora.json");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.mkdir(configDir, { recursive: true });
    await fs.writeFile(path.join(stateDir, ".env"), "AFORA_GATEWAY_TOKEN=state-token\n");
    await fs.writeFile(
      path.join(configDir, ".env"),
      [
        "AFORA_GATEWAY_PASSWORD=config-root-password",
        "AFORA_GATEWAY_TOKEN=config-root-token",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(process.env.AFORA_GATEWAY_TOKEN).toBe("state-token");
          expect(process.env.AFORA_GATEWAY_PASSWORD).toBe("config-root-password");
        },
      );
    } finally {
      await fs.rm(homeDir, { recursive: true, force: true });
  ...
  it("loads and repins a legacy state dotenv after automatic state migration", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-legacy-env-"));
    const legacyStateDir = path.join(homeDir, ".clawdbot");
    const newStateDir = path.join(homeDir, ".afora");
    await fs.mkdir(legacyStateDir, { recursive: true });
    await fs.writeFile(path.join(legacyStateDir, ".env"), "AFORA_GATEWAY_TOKEN=legacy-token\n");
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
        ...
        async () => {
          ...
          expect(process.env.AFORA_GATEWAY_TOKEN).toBe("legacy-token");
  ...
  it("blocks a future-config recovery candidate before destructive gateway reset", async () => {
    ...
    try {
      await runCli(["node", "afora", "gateway", "--dev", "--reset"]);
      const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
        | { beforeRun?: (opts: { reset?: boolean }) => Promise<void> }
        | undefined;
  ...
  it("re-guards config env path selection until the gateway config is stable", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-selection-"));
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => {
            if (process.env.AFORA_CONFIG_PATH === "/tmp/afora-chain-c.json") {
              return {
                exists: true,
                valid: true,
                sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
            ...
            if (process.env.AFORA_STATE_DIR === "/tmp/afora-chain-b") {
              return {
                exists: true,
                valid: true,
                sourceConfig: {
                  env: { vars: { AFORA_CONFIG_PATH: "/tmp/afora-chain-c.json" } },
                  gateway: { mode: "local" },
                },
              };
            }
            return {
              ...
              valid: true,
              sourceConfig: {
                env: { vars: { AFORA_STATE_DIR: "/tmp/afora-chain-b" } },
                gateway: { mode: "local" },
              },
            };
          });
          ...
          try {
            await expect(runCli(["node", "afora", "gateway"])).rejects.toThrow("exit:1");
            expect(errorSpy).toHaveBeenCalledWith(
              expect.stringContaining("run automatic gateway startup migrations"),
            );
            expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
  ...
  it("drops credentials from configs superseded during state selection", async () => {
    await withEnvAsync(
      ...
      async () => {
        readConfigFileSnapshotMock.mockImplementation(async () =>
          process.env.AFORA_STATE_DIR === "/tmp/afora-selected-state"
            ? {
                exists: true,
                valid: true,
                sourceConfig: {
                  env: { vars: { AFORA_GATEWAY_TOKEN: "selected-token" } },
                  gateway: { mode: "local" },
                },
              }
            : {
                ...
                sourceConfig: {
                  env: {
                    vars: {
                      AFORA_GATEWAY_TOKEN: "superseded-token",
                      AFORA_STATE_DIR: "/tmp/afora-selected-state",
                    },
                  },
                  gateway: { mode: "local" },
                },
        ...
        expect(process.env.AFORA_STATE_DIR).toBe("/tmp/afora-selected-state");
        expect(process.env.AFORA_GATEWAY_TOKEN).toBe("selected-token");
  ...
  it("re-guards config selection from a newly selected state dotenv", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-dotenv-"));
    const futureConfigPath = path.join(stateDir, "future.json");
    await fs.writeFile(
      path.join(stateDir, ".env"),
      [
        `AFORA_CONFIG_PATH=${futureConfigPath}`,
        "AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => {
            if (process.env.AFORA_CONFIG_PATH === futureConfigPath) {
              return {
                exists: true,
                valid: true,
                sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
            ...
            return {
              ...
              valid: true,
              sourceConfig: {
                env: { vars: { AFORA_STATE_DIR: stateDir } },
                gateway: { mode: "local" },
              },
            };
          });
  ...
  it("re-inspects recovery after recovery changes config selection", async () => {
    await withEnvAsync({ AFORA_CONFIG_PATH: undefined }, async () => {
      const selectedConfigPath = "/tmp/afora-recovered-selection.json";
      const currentSnapshot = {
        exists: true,
        valid: true,
        sourceConfig: { gateway: { mode: "local" } },
      ...
      readConfigFileSnapshotMock.mockImplementation(async (options) => {
        ...
        if (recoveryReads === 1) {
          const recoveredSnapshot = {
            ...
            valid: true,
            sourceConfig: {
              env: { vars: { AFORA_CONFIG_PATH: selectedConfigPath } },
              gateway: { mode: "local" },
            },
          };
          await options.allowSuspiciousRecovery?.(
  ...
  it("discards env from a config replaced by suspicious recovery", async () => {
    await withEnvAsync(
      { AFORA_GATEWAY_TOKEN: undefined, AFORA_PROXY_ACTIVE: undefined },
      async () => {
        const clobberedSnapshot = {
          exists: true,
          valid: true,
          sourceConfig: {
            env: { vars: { AFORA_GATEWAY_TOKEN: "discarded-token" } },
            gateway: { mode: "local" },
          },
          hash: "clobbered",
          path: "/tmp/afora.json",
        ...
        expect(process.env.AFORA_PROXY_ACTIVE).toBe("1");
        expect(ensureCliExecutionBootstrapMock).toHaveBeenCalledOnce();
      },
    );
  });
  ...
  it("does not apply environment variables from invalid config snapshots", async () => {
    await withEnvAsync({ AFORA_INCLUDE_ROOTS: undefined }, async () => {
      readConfigFileSnapshotMock.mockResolvedValue({
        exists: true,
        valid: false,
        sourceConfig: {
          env: { vars: { AFORA_INCLUDE_ROOTS: "/tmp/afora-includes" } },
          gateway: { mode: "local" },
        },
      });
      ...
      expect(process.env.AFORA_INCLUDE_ROOTS).toBeUndefined();
      expect(readConfigFileSnapshotMock.mock.calls).toEqual([
        [{ isolateEnv: true, observe: false }],
        [{ isolateEnv: true, observe: false }],
      ]);
  ...
  it("loads selected state dotenv before config env and environment normalization", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-selected-env-"));
        ...
        async () => {
          readConfigFileSnapshotMock.mockResolvedValue({
            ...
            sourceConfig: {
              env: {
                vars: {
                  AFORA_GATEWAY_TOKEN: "config-token",
          ...
          normalizeEnvMock.mockImplementation(() => {
            tokenAtNormalize = process.env.AFORA_GATEWAY_TOKEN;
          });
  ...
  it("drops credentials from a trusted dotenv superseded by state selection", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-dotenv-hop-"));
    ...
    await fs.writeFile(
      ...
      "AFORA_GATEWAY_TOKEN=selected-token\n",
    );
    try {
      await withEnvAsync(
        {
  ...
  it("drops gateway.env selectors when the default state dotenv selects a custom state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-env-hop-"));
    const defaultStateDir = path.join(homeDir, ".afora");
    const selectedStateDir = path.join(homeDir, "selected-state");
    const gatewayEnvDir = path.join(homeDir, ".config", "afora");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
    ...
    await fs.writeFile(
      ...
      [
        "AFORA_CONFIG_PATH=/tmp/wrong-afora.json",
    ...
    await fs.writeFile(
      ...
      [
        "AFORA_GATEWAY_TOKEN=selected-token",
        "AFORA_INCLUDE_ROOTS=/tmp/untrusted-include-root",
  ...
  it("preserves gateway.env selectors when the compatibility fallback selects the target", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-env-select-"));
    const selectedStateDir = path.join(homeDir, "selected-state");
    const gatewayEnvDir = path.join(homeDir, ".config", "afora");
    await fs.mkdir(selectedStateDir, { recursive: true });
    await fs.mkdir(gatewayEnvDir, { recursive: true });
    await fs.writeFile(
      path.join(gatewayEnvDir, "gateway.env"),
      [`AFORA_STATE_DIR=${selectedStateDir}`, "AFORA_GATEWAY_TOKEN=fallback-token", ""].join(
        "\n",
      ),
    );
    await fs.writeFile(
  ...
  it("drops old state dotenv credentials when config selects another state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-config-state-hop-"));
    const defaultStateDir = path.join(homeDir, ".afora");
    const selectedStateDir = path.join(homeDir, "selected-state");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
      ...
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => ({
            ...
            valid: true,
            sourceConfig:
              process.env.AFORA_STATE_DIR === selectedStateDir
                ? { gateway: { mode: "local" } }
                : {
  ...
  it("drops early target credentials when a later guard selects another state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-late-state-hop-"));
    const defaultStateDir = path.join(homeDir, ".afora");
    const selectedStateDir = path.join(homeDir, "selected-state");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
    await fs.writeFile(path.join(defaultStateDir, ".env"), "AFORA_GATEWAY_TOKEN=early-token\n");
    await fs.writeFile(
      path.join(selectedStateDir, ".env"),
      "AFORA_GATEWAY_TOKEN=selected-token\n",
    );
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          readConfigFileSnapshotMock.mockImplementation(async () => ({
            ...
            valid: true,
            sourceConfig:
              selectLateState && process.env.AFORA_STATE_DIR !== selectedStateDir
                ? {
                    env: { vars: { AFORA_STATE_DIR: selectedStateDir } },
                    gateway: { mode: "local" },
                  }
          ...
          await runCli(["node", "afora", "gateway"]);
          expect(process.env.AFORA_GATEWAY_TOKEN).toBe("early-token");
  ...
  it("does not let gateway.env authorize automatic mutations of a selected future config", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-global-env-"));
    const gatewayEnvDir = path.join(homeDir, ".config", "afora");
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () =>
            process.env.AFORA_CONFIG_PATH === futureConfigPath
              ? {
                  exists: true,
                  valid: true,
                  sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
              ...
              : {
                  ...
                  valid: true,
                  sourceConfig: {
                    env: { vars: { AFORA_CONFIG_PATH: futureConfigPath } },
                    gateway: { mode: "local" },
                  },
                },
          );
  ...
  it("does not treat gateway option values as bootstrap command paths", async () => {
    await runCli(["node", "afora", "gateway", "--raw-stream-path", "status"]);
  ...
  it("retains selected config paths and invocation reset targets", async () => {
    await withEnvAsync(
      {
        AFORA_CONFIG_PATH: "/tmp/afora-invocation/afora.json",
        AFORA_GATEWAY_TOKEN: undefined,
        AFORA_HOME: "/tmp/afora-invocation-home",
        AFORA_INCLUDE_ROOTS: undefined,
        AFORA_PROFILE: undefined,
        AFORA_STATE_DIR: "/tmp/afora-invocation-state",
        AFORA_TEST_FAST: "1",
        AFORA_WORKSPACE_DIR: "/tmp/afora-invocation-workspace",
      },
      async () => {
        readConfigFileSnapshotMock.mockResolvedValue({
          exists: true,
          ...
          sourceConfig: {
            env: {
              vars: {
                ...
                AFORA_INCLUDE_ROOTS: "/tmp/afora-reset-includes",
                AFORA_PROFILE: "config-dev",
                AFORA_STATE_DIR: "/tmp/afora-reset",
                AFORA_TEST_FAST: "0",
                AFORA_WORKSPACE_DIR: "/tmp/afora-reset-workspace",
              },
            },
            gateway: { mode: "local" },
          },
        ...
        expect(process.env.AFORA_CONFIG_PATH).toBe("/tmp/afora-invocation/afora.json");
        expect(process.env.AFORA_HOME).toBe("/tmp/afora-invocation-home");
        expect(process.env.AFORA_PROFILE).toBeUndefined();
        expect(process.env.AFORA_STATE_DIR).toBe("/tmp/afora-invocation-state");
        expect(process.env.AFORA_TEST_FAST).toBe("1");
        expect(process.env.AFORA_WORKSPACE_DIR).toBe("/tmp/afora-invocation-workspace");
        expect(process.env.AFORA_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.AFORA_INCLUDE_ROOTS).toBeUndefined();
        expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
      },
  ...
  it("does not let config env authorize or retarget an explicit reset", async () => {
    await withEnvAsync(
      { AFORA_PROFILE: undefined, AFORA_WORKSPACE_DIR: undefined },
      async () => {
        readConfigFileSnapshotMock.mockResolvedValue({
          exists: true,
          valid: true,
          sourceConfig: {
            env: {
              vars: {
                AFORA_PROFILE: "dev",
                AFORA_WORKSPACE_DIR: "/tmp/afora-config-workspace",
              },
            },
            gateway: { mode: "local" },
          },
        ...
        await runCli(["node", "afora", "gateway", "--reset"]);

        expect(process.env.AFORA_PROFILE).toBeUndefined();
        expect(process.env.AFORA_WORKSPACE_DIR).toBeUndefined();
      },
    );
  });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    ...
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "afora", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "afora", "browser", "--help"]);
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "afora", "secrets", "--help"]);
  ...
  it("defers nodes help startup metadata when plugin config can change command metadata", async () => {
    const argv = ["node", "afora", "nodes", "--help"];
    const parseAsync = vi.fn().mockResolvedValueOnce(undefined);
    const program = {
      commands: [{ name: () => "nodes", aliases: () => [] }],
      parseAsync,
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "afora", "doctor", "--help"]);
  ...
  it("propagates precomputed help metadata failures", async () => {
    ...
    await expect(runCli(["node", "afora", "secrets", "--help"])).rejects.toThrow(
  ...
  it("propagates nodes live-config probe failures", async () => {
    ...
    await expect(runCli(["node", "afora", "nodes", "--help"])).rejects.toThrow(
      "live config failed",
    );
  });
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "afora", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "afora", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["local gateway status", ["node", "afora", "status"]],
    ["models JSON alias", ["node", "afora", "models", "--json"]],
    ["models status JSON alias", ["node", "afora", "models", "--status-json"]],
    ["models plain alias", ["node", "afora", "models", "--status-plain"]],
  ...
  it.each([
    ["gateway runtime", ["node", "afora", "gateway", "run"]],
    ["bare gateway runtime", ["node", "afora", "gateway"]],
    ["node runtime", ["node", "afora", "node", "run"]],
    ["local agent runtime", ["node", "afora", "agent", "--local"]],
    ["provider inference", ["node", "afora", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "afora", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "afora", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "afora", "skills", "search", "browser"]],
    ["update command", ["node", "afora", "update", "check"]],
    ["channel probe", ["node", "afora", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "afora", "channels", "capabilities"]],
    ["directory plugin command", ["node", "afora", "directory", "peers", "list"]],
    ["message plugin command", ["node", "afora", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "afora", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "afora", "--help"]],
    ["root version", ["node", "afora", "--version"]],
    ["gateway help", ["node", "afora", "gateway", "--help"]],
    ["gateway run help", ["node", "afora", "gateway", "run", "--help"]],
    ["status", ["node", "afora", "status"]],
    ["health", ["node", "afora", "health"]],
    ["gateway status", ["node", "afora", "gateway", "status"]],
    ["gateway health", ["node", "afora", "gateway", "health"]],
    ["remote agent control-plane", ["node", "afora", "agent", "run"]],
    ["chat control-plane", ["node", "afora", "chat"]],
    ["terminal control-plane", ["node", "afora", "terminal"]],
    ["config", ["node", "afora", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "afora", "channels"]],
    ["completion", ["node", "afora", "completion", "zsh"]],
    ["debug proxy cli", ["node", "afora", "proxy", "start"]],
    ["agents list", ["node", "afora", "agents", "list"]],
    ["models list", ["node", "afora", "models", "list"]],
    ["models status without live probe", ["node", "afora", "models", "status"]],
    ["skills check", ["node", "afora", "skills", "check"]],
    ["skills info", ["node", "afora", "skills", "info", "weather"]],
    ["skills list", ["node", "afora", "skills", "list"]],
    ["tasks list", ["node", "afora", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "afora", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "afora", "tools", "effective"]],
    ["migrate", ["node", "afora", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "afora", "plugins", "marketplace", "list"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it.each([
    ["root command", ["node", "afora", "update", "--dry-run", "--json"]],
    ["root shorthand", ["node", "afora", "--update", "--dry-run", "--json"]],
  ...
  it("reads source-only proxy config for mutable updates", async () => {
    ...
    await runCli(["node", "afora", "update"]);
  ...
  it("reads source-only proxy config before doctor lint owns plugin-aware validation", async () => {
    ...
    await runCli(["node", "afora", "doctor", "--lint", "--json"]);
  ...
  it("routes managed-proxy startup logs away for JSON output", async () => {
    ...
    await runCli(["node", "afora", "plugins", "marketplace", "list", "--json"]);
  ...
  it.each([
    ["cron", ["node", "afora", "cron", "status"]],
    ["cron alias", ["node", "afora", "cron", "create", "daily", "message"]],
    ["cron removal alias", ["node", "afora", "cron", "delete", "job"]],
    ["cron scratch equals", ["node", "afora", "cron", "scratch", "job", "--set=text"]],
    ["device token", ["node", "afora", "devices", "rotate", "--device", "one"]],
    [
      "gateway handoff",
      ["node", "afora", "gateway", "--port", "18789", "restart-handoff", "capabilities"],
    ],
    ["node pairing", ["node", "afora", "nodes", "approve", "request-one"]],
    ["node invoke", ["node", "afora", "nodes", "invoke", "--node", "one"]],
    ["skill verification", ["node", "afora", "skills", "verify", "@owner/skill"]],
    [
      "agent-scoped skill verification",
      ["node", "afora", "skills", "--agent", "main", "verify", "@owner/skill"],
    ],
    ["system heartbeat", ["node", "afora", "system", "heartbeat", "last"]],
    ["system presence", ["node", "afora", "system", "presence"]],
    ["doctor lint", ["node", "afora", "doctor", "--lint"]],
    ["proxy coverage", ["node", "afora", "proxy", "coverage"]],
  ...
  it("routes managed-proxy startup logs for plugin-declared machine output", async () => {
    ...
    try {
      await runCli(["node", "afora", "path", "validate", "oc://AGENTS.md"]);
    } finally {
      if (stdoutDescriptor) {
        Object.defineProperty(process.stdout, "isTTY", stdoutDescriptor);
      } else {
  ...
  it.each([
    ["bare gateway fast path", ["node", "afora", "gateway"]],
    ["fast path", ["node", "afora", "gateway", "run"]],
    [
      "full Commander path with root options",
      ["node", "afora", "--log-level", "debug", "gateway", "run"],
    ],
  ...
  it("keeps state dotenv loading for non-gateway commands", async () => {
    const stateDir = path.join(os.tmpdir(), "afora-run-main-state");
    existsSyncOverride.value = (target) => target === path.join(stateDir, ".env");
    tryRouteCliMock.mockResolvedValueOnce(true);

    await withEnvAsync({ AFORA_STATE_DIR: stateDir }, () =>
      runCli(["node", "afora", "status"]),
    );
  ...
  it("keeps agent exec outside the CLI dotenv loader", async () => {
    ...
    await runCli(["node", "afora", "agent", "exec", "test prompt"]);
  ...
  it("selects gateway config env before starting its managed proxy", async () => {
    await withEnvAsync({ AFORA_TEST_PROXY_SELECTION: undefined }, async () => {
      readConfigFileSnapshotMock.mockResolvedValue({
        exists: true,
        valid: true,
        sourceConfig: {
          env: { vars: { AFORA_TEST_PROXY_SELECTION: "selected" } },
          gateway: { mode: "local" },
        },
      });
      loadConfigMock.mockImplementationOnce(() => ({
        proxy: { selected: process.env.AFORA_TEST_PROXY_SELECTION },
      }));
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "afora", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "afora", "foo"])).rejects.toThrow(
      'Afora does not know the command "foo".',
    );
  ...
  it("routes a bare-root Control UI URL directly to the TUI action", async () => {
    ...
    await withInteractiveTty(() => runCli(["node", "afora", target]));
  ...
  it.each(["tui", "attach", "logs"])(
    ...
    async (command) => {
      const target = "https://gateway.example/dashboard/main/movies-a1166b81";
      const argv = ["node", "afora", command, target];
      buildProgramMock.mockReturnValueOnce({
  ...
  it("leaves plugin-owned URL arguments on the plugin command path", async () => {
    const target = "https://gateway.example/dashboard/main/movies-a1166b81";
    const argv = ["node", "afora", "googlemeet", target];
  ...
  it("does not steal a URL argument from an unowned command", async () => {
    ...
    await expect(runCli(["node", "afora", "unknown-owner", target])).rejects.toThrow(
      'Afora does not know the command "unknown-owner".',
    );
  ...
  it.each([
    ...
    await withEnvAsync(
      {
        AFORA_GATEWAY_TOKEN: "ambient-token",
        AFORA_GATEWAY_PASSWORD: "ambient-password",
      },
      () => withInteractiveTty(() => runCli(["node", "afora", ...args])),
    );
  ...
  it.each([
    ...
    let error: unknown;
    try {
      await runCli(["node", "afora", ...prefix, target]);
    } catch (caught) {
      error = caught;
    }
  ...
  it("rejects a missing pre-URL direct option value before command discovery", async () => {
    ...
    await expect(runCli(["node", "afora", "--token", target])).rejects.toThrow(
  ...
  it("does not claim a bare session ref as root-command sugar", async () => {
    await expect(runCli(["node", "afora", "movies-a1166b81"])).rejects.toThrow(
      'Afora does not know the command "movies-a1166b81".',
    );
  ...
  it("does not claim host shorthand as root-command sugar", async () => {
    await expect(runCli(["node", "afora", "gateway.example/main/a1166b81"])).rejects.toThrow(
      'Afora does not know the command "gateway.example/main/a1166b81".',
    );
  ...
  it("suggests close known commands for unowned command roots before proxy startup", async () => {
    const error = await runCli(["node", "afora", "upate"]).catch((cause: unknown) => cause);
  ...
  it("sanitizes control characters in unowned command diagnostics", async () => {
    ...
    await expect(runCli(["node", "afora", primary])).rejects.toThrow(
      'Afora does not know the command "bad-red\\nforged\\tline".',
    );
  ...
  it("bounds long unowned command diagnostics without splitting Unicode", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "afora", primary]);
    } catch (caught) {
      error = caught;
    }
    ...
    expect(message).toContain(`Afora does not know the command "${displayPrimary}".`);
  ...
  it.each([
    ...
    async ({ command, config, commandAlias, toolOwner, expectedText }) => {
      ...
      const error = await runCli(["node", "afora", command]).catch((cause: unknown) => cause);
  ...
  it("reports disabled-by-default plugin commands as expected after lazy registration", async () => {
    ...
    const error = await runCli(["node", "afora", "workboard", "list"]).catch(
      (cause: unknown) => cause,
    );
  ...
  it("rejects unowned command roots even when --help is appended (regression for #81077)", async () => {
    await expect(runCli(["node", "afora", "foo", "--help"])).rejects.toThrow(
      'Afora does not know the command "foo".',
    );
  ...
  it("rejects unowned command roots even when --version is appended", async () => {
    await expect(runCli(["node", "afora", "foo", "--version"])).rejects.toThrow(
      'Afora does not know the command "foo".',
    );
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "afora", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
    expect(error).toBeInstanceOf(Error);
    expect((error as Error).message).toContain(
      'Afora does not know the command "totally-unknown".',
    );
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "afora", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "afora", "lcm_recent"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "afora", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "afora", "auth", "--help"]],
    ["tool", ["node", "afora", "tool", "image_generate"]],
    ["tools", ["node", "afora", "tools", "effective"]],
  ...
  it("routes incidental logs to stderr throughout --json startup and dispatch", async () => {
    ...
    await runCli(["node", "afora", "memory", "search", "query", "--json"]);
  ...
  it("retains stderr routing for late subsystem logs in one-shot JSON commands", async () => {
    ...
    try {
      await runCli(["node", "afora", "agent", "exec", "inspect", "--json"], {
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "afora", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "afora", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "afora", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it.each([
    ...
    },
    {
      ...
      snapshot: {
        ...
        valid: true,
        sourceConfig: {
          $schema: "https://afora.ai/config.json",
  ...
  it("resumes pending local onboarding after inference persisted its model", async () => {
    const configPath = "/tmp/afora.json";
  ...
  it("starts the gateway-backed TUI for bare root invocations when config already exists", async () => {
    ...
    await withEnvAsync({ AFORA_GATEWAY_PASSWORD: "gateway-ref-password" }, async () => {
      await runBareCli();
    });
  ...
  it("keeps an explicit gateway port ahead of active local lock metadata", async () => {
    ...
    await withEnvAsync({ AFORA_GATEWAY_PORT: "19001" }, async () => {
      await runBareCli();
    });
  ...
  it("uses gateway env credentials for bare root gateway preflight", async () => {
    ...
    await withEnvAsync({ AFORA_GATEWAY_TOKEN: "bare-root-env-auth" }, async () => {
      await runBareCli();
    });
  ...
  it("resolves only the configured auth-mode SecretRef for bare root preflight", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-bare-auth-mode-"));
  ...
  it.each([
    ...
    await withEnvAsync({ AFORA_ALLOW_INSECURE_PRIVATE_WS: undefined }, async () => {
      await runBareCli();
    });
  ...
  it("keeps configured remote password authoritative from preflight through TUI launch", async () => {
    ...
    await withEnvAsync({ AFORA_GATEWAY_PASSWORD: "obsolete-shell-pass-value" }, async () => {
      await runBareCli();
    });
    ...
    await withEnvAsync(
      {
        ...
        AFORA_GATEWAY_TOKEN: "shell-fallback-auth-value",
        AFORA_GATEWAY_PASSWORD: "env-remote-password",
      },
      async () => {
        await runBareCli();
      },
  ...
  it("probes an explicitly allowed plaintext private remote gateway", async () => {
    ...
    await withEnvAsync({ AFORA_ALLOW_INSECURE_PRIVATE_WS: "1" }, async () => {
      await runBareCli();
    });
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "afora", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "afora", "status"]],
    ]);
    expect(process.exitCode).toBe(1);
    process.exitCode = exitCode;
  });
  ...
  it("requests a flushed one-shot exit after Commander renders help", async () => {
    ...
    await runCli(["node", "afora", "security", "--help"]);
  ...
  it("requests a flushed one-shot exit when plugin group help returns normally", async () => {
    ...
    await runCli(["node", "afora", "memory", "--help"]);
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "afora", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "afora", "doctor", "--help"]],
    ]);
  });
    ...
    async (machineOutput) => {
      ...
      try {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith(
          "[afora] Afora hit an unexpected runtime error.",
        );
        expect(consoleErrorSpy).toHaveBeenCalledWith("[afora] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[afora] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
