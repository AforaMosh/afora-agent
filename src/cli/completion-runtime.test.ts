...
async function withBashCompletionHome(
  ...
  stateDirPrefix = "afora-bash-completion-state-",
): Promise<void> {
  const homeDir = tempDirs.make("afora-bash-completion-home-");
...
describe("completion-runtime", () => {
  it.each([
    ...
    const homeDir = tempDirs.make("afora-shell-profile-home-");
    const stateDir = tempDirs.make("afora-shell-profile-state-");
    const configDir = tempDirs.make(`afora-${testCase.shell} profile config-`);
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        await installCompletion(testCase.shell, true, "afora");
        ...
        await expect(isCompletionInstalled(testCase.shell, "afora")).resolves.toBe(true);

        if (testCase.shell === "zsh") {
          const shell = spawnSync("zsh", ["-ic", '[[ "$AFORA_COMPLETION_LOADED" = ready ]]'], {
            encoding: "utf8",
            env: process.env,
          });
  ...
  it("preserves zsh's set-but-empty startup directory contract", () => {
    expect(
      resolveCompletionProfilePath("zsh", {
        env: { HOME: "/tmp/afora-home", ZDOTDIR: "" },
        homeDir: () => "/tmp/afora-home",
      }),
    ...
    async () => {
      for (const testCase of [
        ...
        await withBashCompletionHome(async ({ homeDir }) => {
          const profileParent = tempDirs.make(`afora-${testCase.shell}-symlink-profiles-`);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const fixtureRoot = tempDirs.make("afora-completion-home-symlink-");
      ...
      for (const testCase of [
        ...
        const stateDir = tempDirs.make(`afora-${testCase.shell}-home-symlink-state-`);
        await withEnvAsync(
          {
  ...
  it.each(["relative-xdg", "~/relative-xdg"])(
    ...
    (configHome) => {
      const homeDir = path.join(path.sep, "tmp", "afora-home");
      expect(
  ...
  it.each(["~/literal startup", "~root/startup", "-startup", "../startup"])(
    ...
    async (profileRoot) => {
      await withEnvAsync({ HOME: "/tmp/afora-home", ZDOTDIR: profileRoot }, async () => {
  ...
  it("recognizes cached Bash completion installed into the login profile", async () => {
    await withBashCompletionHome(async ({ homeDir }) => {
      const cachePath = resolveCompletionCachePath("bash", "afora");
      await fs.mkdir(path.dirname(cachePath), { recursive: true });
      await fs.writeFile(cachePath, "complete -W 'status' afora\n", "utf-8");

      await installCompletion("bash", true, "afora");

      const profilePath = path.join(homeDir, ".bash_profile");
      await expect(fs.readFile(profilePath, "utf-8")).resolves.toContain(cachePath);
      await expect(isCompletionInstalled("bash", "afora")).resolves.toBe(true);
      await expect(usesSlowDynamicCompletion("bash", "afora")).resolves.toBe(false);
      ...
      expect(shell.stdout).toContain("complete -W 'status' afora");
    });
  });

  it("prints the same canonical reload hint used by Doctor and onboarding", async () => {
    await withBashCompletionHome(async ({ homeDir }) => {
      const cachePath = resolveCompletionCachePath("zsh", "afora");
      await fs.mkdir(path.dirname(cachePath), { recursive: true });
      ...
      try {
        await installCompletion("zsh", false, "afora");
  ...
  it("preserves an existing profile when atomic publication fails", async () => {
    await withBashCompletionHome(async ({ homeDir }) => {
      ...
      await expect(installCompletion("zsh", true, "afora")).rejects.toThrow(
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withBashCompletionHome(async ({ homeDir }) => {
        const cachePath = resolveCompletionCachePath("zsh", "afora");
        const targetDir = tempDirs.make("afora-completion-profile-target-");
        ...
        await installCompletion("zsh", true, "afora");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      await withBashCompletionHome(async ({ homeDir }) => {
        ...
        await expect(fs.readFile(targetPath, "utf8")).resolves.toContain("# Afora Completion");
      });
    },
  );
  ...
  it("detects slow dynamic Bash completion in the login profile", async () => {
    await withBashCompletionHome(async ({ homeDir }) => {
      await fs.writeFile(
        ...
        "source <(afora completion --shell bash)\n",
        "utf-8",
      );

      await expect(isCompletionInstalled("bash", "afora")).resolves.toBe(true);
      await expect(usesSlowDynamicCompletion("bash", "afora")).resolves.toBe(true);
    });
  });
  ...
  it("does not mistake an orphaned completion marker for an installed profile", async () => {
    await withBashCompletionHome(async ({ homeDir }) => {
      ...
      await expect(isCompletionInstalled("bash", "afora")).resolves.toBe(false);
    });
  });
  ...
  it.each(COMPLETION_SHELLS)(
    ...
    async (shell) => {
      await withBashCompletionHome(async ({ stateDir }) => {
        const previousStateDir = tempDirs.make("afora-completion-previous-state-");
        let previousCachePath = "";
        await withEnvAsync({ AFORA_STATE_DIR: previousStateDir }, async () => {
          previousCachePath = resolveCompletionCachePath(shell, "afora");
          await fs.mkdir(path.dirname(previousCachePath), { recursive: true });
          await fs.writeFile(previousCachePath, "# previous completion\n", "utf-8");
          await installCompletion(shell, true, "afora");
        });

        const currentCachePath = resolveCompletionCachePath(shell, "afora");
        ...
        expect(profile.match(/^# Afora Completion$/gm)).toHaveLength(1);
      });
    },
  );
  ...
  it("preserves unrelated generated-looking sources that are not owned by its profile marker", async () => {
    await withBashCompletionHome(async ({ homeDir }) => {
      const cachePath = resolveCompletionCachePath("bash", "afora");
      const profilePath = path.join(homeDir, ".bash_profile");
      const unrelatedSource = 'source "/opt/tools/completions/afora.zsh"';
      const unmarkedPriorSource = 'source "/opt/tools/completions/afora.bash"';
      await fs.mkdir(path.dirname(cachePath), { recursive: true });
  ...
  it("preserves unrelated profile lines while replacing orphaned completion blocks", async () => {
    await withBashCompletionHome(async ({ homeDir }) => {
      const cachePath = resolveCompletionCachePath("bash", "afora");
      const profilePath = path.join(homeDir, ".bash_profile");
      const refreshAlias = "alias refresh_afora='afora completion --write-state'";
      await fs.mkdir(path.dirname(cachePath), { recursive: true });
      await fs.writeFile(cachePath, "complete -W 'status' afora\n", "utf-8");
      await fs.writeFile(
        profilePath,
  ...
  it("replaces documented dynamic completion without deleting unrelated aliases", async () => {
    await withBashCompletionHome(async ({ homeDir }) => {
      ...
      expect(profile).toContain("export IMPORTANT=keep\n");
      expect(profile).toContain(`${refreshAlias}\n`);
      expect(profile).not.toContain("source <(afora completion");
      expect(profile).toContain(cachePath);
    });
  });

  it.each([
    "export IMPORTANT=keep; source <(afora completion --shell bash)",
    "source <(afora completion --shell bash); export IMPORTANT=keep",
    'source <(afora completion --shell bash) >"$HOME/completion.log"',
    'eval "$(afora completion --shell bash)" >"$HOME/completion.log"',
    'source <(afora completion --shell bash >"$HOME/completion.log")',
  ...
  it.each([
    {
      ...
      sourceLine: ". <(afora completion --shell bash)",
    },
    {
      name: "eval command substitution",
      sourceLine: 'eval "$(afora completion --shell bash)"',
    },
  ...
  it("replaces PowerShell dynamic pipelines without deleting unrelated command strings", async () => {
    await withBashCompletionHome(async () => {
      const cachePath = resolveCompletionCachePath("powershell", "afora");
      const profilePath = resolveCompletionProfilePath("powershell");
      const dynamicLine = "afora completion --shell powershell | Out-String | Invoke-Expression";
      const refreshCommand = '$refresh = "afora completion --write-state"';
      await fs.mkdir(path.dirname(cachePath), { recursive: true });
      await fs.writeFile(cachePath, "# PowerShell completion\n", "utf-8");
      await fs.mkdir(path.dirname(profilePath), { recursive: true });
      await fs.writeFile(profilePath, `${dynamicLine}\n${refreshCommand}\n`, "utf-8");

      await expect(usesSlowDynamicCompletion("powershell", "afora")).resolves.toBe(true);
      await installCompletion("powershell", true, "afora");
  ...
  it.each([
    "afora completion --shell powershell | Out-String | Invoke-Expression; $env:IMPORTANT = 'keep'",
    'afora completion --shell powershell | Tee-Object "$HOME/generated.ps1" | Out-String | Invoke-Expression',
  ...
  it.each(["bash", "zsh"] as const)(
    ...
    async (shellName) => {
      const profileDir = tempDirs.make(`afora ${shellName} Ada's !42 reload profile-`);
      const profilePath = path.join(profileDir, ".shellrc");
      await fs.writeFile(profilePath, "AFORA_COMPLETION_LOADED=ready\n", "utf-8");
      ...
      const shell = spawnSync(
        shellName,
        ["-c", `${reloadCommand}; [ "$AFORA_COMPLETION_LOADED" = ready ]`],
        {
          encoding: "utf8",
        },
      );
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    await withBashCompletionHome(async () => {
      ...
      const profilePath = resolveCompletionProfilePath("powershell");
      const profile = await fs.readFile(profilePath, "utf-8");
      expect(profile).toBe(`# Afora Completion\n. '${cachePath.replace(/'/g, "''")}'\n`);
    }, "afora-completion-state-bob's-");
  });
