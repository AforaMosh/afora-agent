...
import { bundledPluginRootAt, repoInstallSpec } from "afora-agent/plugin-sdk/test-fixtures";
import { beforeEach, describe, expect, it, vi } from "vitest";
import type { AforaConfig } from "../config/config.js";
import { hashConfigIncludeRaw } from "../config/includes.js";
import type { ConfigWriteOptions } from "../config/io.js";
import type { ConfigFileSnapshot } from "../config/types.afora.js";
import {
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as AforaConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as AforaConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const discordNpmRequest = {
    rawSpec: "@afora/discord",
    normalizedSpec: "@afora/discord",
    installKind: "plugin",
  ...
  it("returns the source config and base hash when the snapshot is valid", async () => {
    const cfg = { plugins: { entries: { discord: { enabled: true } } } } as AforaConfig;
  ...
  it("allows versioned npm:-prefixed bundled-plugin reinstall recovery", async () => {
    ...
    const request = resolvePluginInstallRequestContext({
      rawSpec: "npm:@afora/discord@2026.5.22",
    });
    if (!request.ok) {
  ...
  it.each(["file:@afora/discord", "FILE:@afora/discord"])(
  ...
  it("allows versioned official npm spec reinstall recovery", async () => {
    ...
    const request = resolvePluginInstallRequestContext({
      rawSpec: "@afora/discord@2026.5.22",
    });
    if (!request.ok) {
      throw new Error(request.error);
    }
      ...
      makeSnapshot({
        ...
        issues: [
          ...
          },
          {
            ...
            message:
              'web_search provider is not available: brave (install or enable plugin "brave", then run afora doctor --fix)',
          },
        ],
      }),
    );
  ...
  it("allows explicit repo-checkout bundled-plugin reinstall recovery", async () => {
    const snapshotCfg = { plugins: {} } as AforaConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("allows recovery through an exact single-file top-level plugins include", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-plugin-include-"));
  ...
  it("carries a hook-mutation block through an external hooks include", async () => {
    ...
    const snapshotCfg = { hooks: { internal: {} } } as AforaConfig;
    includeFileTargetsForWriteMock.mockReturnValue({
  ...
  it("blocks config mutations when plugins and hooks share one canonical include target", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-shared-include-"));
    const configPath = path.join(tempRoot, "config.json5");
    ...
    const snapshotCfg = { hooks: {}, plugins: {} } as AforaConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        path: configPath,
        valid: true,
  ...
  it("blocks both mutations when an external include aliases the other section target", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-aliased-include-"));
    const configPath = path.join(tempRoot, "config.json5");
    const sharedPath = path.join(tempRoot, "shared.json5");
    const externalHooksPath = path.join(
      path.parse(process.cwd()).root,
  ...
  it("blocks nested plugins includes before plugin installation", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-plugin-nested-include-"));
    const configPath = path.join(tempRoot, "config.json5");
    const pluginsPath = path.join(tempRoot, "plugins.json5");
    ...
    const snapshotCfg = { plugins: { entries: {} } } as AforaConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        path: configPath,
        valid: true,
  ...
  it("rejects non-Discord install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `afora doctor --fix` before installing plugins.");
  });
  ...
  it("throws when invalid snapshot parsed is empty", async () => {
    ...
    await expect(loadConfigForInstall(discordNpmRequest)).rejects.toThrow(
      "Config file could not be parsed; run `afora doctor` to repair it.",
    );
  });
