...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["afora security audit", "Run a local security audit."],
          [
            "afora security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["afora security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
          ...
          ["afora security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["afora security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.afora.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Afora security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("afora security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("afora security audit --fix")}`));
        if (!fixResult) {
