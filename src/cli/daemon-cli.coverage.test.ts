...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      AFORA_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { AFORA_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { AFORA_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-daemon-cli-"));
    ...
    setTestEnvValue("AFORA_CONFIG_PATH", path.join(tmpDir, "afora.json"));
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        AFORA_PROFILE: "dev",
        AFORA_STATE_DIR: "/tmp/afora-daemon-state",
        AFORA_CONFIG_PATH: "/tmp/afora-daemon-state/afora.json",
        AFORA_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.afora.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).AFORA_WRAPPER).toBe(
      "/usr/local/bin/afora-doppler",
    );
  });
