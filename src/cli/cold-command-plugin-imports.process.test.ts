...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-cold-commands-"));
  ...
  async ({ args, label, needsMemoryOwner }) => {
    ...
    if (needsMemoryOwner) {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          name: "@example/afora-memory-owner",
          version: "1.0.0",
          type: "commonjs",
      ...
      fs.writeFileSync(
        path.join(memoryOwnerDir, "afora.plugin.json"),
        JSON.stringify({
          id: "memory-owner",
          name: "Memory Owner",
    ...
    const result = await execFileAsync(
      ...
      ["--import", "tsx", "src/entry.ts", ...args],
      {
        ...
        env: {
          ...
          AFORA_DEV_SOURCE_ROOT: path.resolve("."),
