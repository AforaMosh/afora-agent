...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-bundled-skills-test-"));
    process.env.AFORA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
