...
describe("cli program (nodes media)", () => {
  ...
  it("runs one default snap and two explicitly requested facing snaps", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /afora-camera-snap-unknown-.*\.jpg$/,
    });
    ...
    expect(mediaPaths[0]).toContain("afora-camera-snap-");
    expect(mediaPaths[1]).toContain("afora-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /afora-camera-clip-front-.*\.mp4$/,
    });
  });
  ...
  it("runs an unknown-position camera clip for a Linux node", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /afora-camera-clip-unknown-.*\.mp4$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /afora-camera-snap-front-.*\.jpg$/,
      },
      {
