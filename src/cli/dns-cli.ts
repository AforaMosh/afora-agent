...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.afora.ai/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. afora.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by afora dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
