...
export function registerNodeCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "afora node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["afora node status", "Check node host service status."],
          ["afora node install", "Install the node host service."],
          ["afora node start", "Start the installed node host service."],
          ["afora node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.afora.ai/cli/node")}\n`,
    );
  ...
  node
    ...
    .option("--context-path <path>", "Gateway WebSocket context path (e.g. /afora-gw)")
