...
describe("private-qa-cli", () => {
  ...
  afterEach(() => {
    ...
    if (originalPrivateQaCli === undefined) {
      ...
      process.env.AFORA_ENABLE_PRIVATE_QA_CLI = originalPrivateQaCli;
    }
  });

  it("loads the private QA CLI from a source checkout path", async () => {
    process.env.AFORA_ENABLE_PRIVATE_QA_CLI = "1";
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-private-qa-source-"));
  ...
  it("loads the private QA CLI from a raw synced source checkout path", async () => {
    process.env.AFORA_ENABLE_PRIVATE_QA_CLI = "1";
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-private-qa-raw-source-"));
    tempDirs.push(repoRoot);
    const expectedPaths = new Set([
  ...
  it("rejects non-source package roots even when private QA is enabled", () => {
    process.env.AFORA_ENABLE_PRIVATE_QA_CLI = "1";
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-private-qa-"));
    tempDirs.push(root);
    fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "afora" }), "utf8");
    const importModule = vi.fn(async () => ({}));
    ...
    ).toThrow("Private QA CLI is only available from an Afora source checkout.");
  ...
  it("rejects when the private QA env flag is disabled", () => {
    ...
    expect(() => loadPrivateQaCliModule({ importModule })).toThrow(
      "Private QA CLI is only available from an Afora source checkout.",
    );
    expect(importModule).not.toHaveBeenCalled();
  });
});
