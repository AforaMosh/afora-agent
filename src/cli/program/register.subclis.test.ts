...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.AFORA_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.AFORA_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.AFORA_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.AFORA_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.AFORA_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.AFORA_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "afora", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "afora"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "afora", "nodes", "list"], "afora");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "afora", "infer"], "afora");
  ...
  it("registers the resume placeholder and dispatches through its lazy registrar", async () => {
    const program = createRegisteredProgram(["node", "afora", "resume"], "afora");
  ...
  it("registers the exec-approvals placeholder and dispatches through the approvals registrar", async () => {
    const program = createRegisteredProgram(["node", "afora", "exec-approvals"], "afora");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "afora", "acp", "--help"], "afora");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "afora", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "afora", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("afora");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "afora", "completion", "--write-state"];
    const program = new Command().name("afora");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "afora", "plugins"];
    const program = new Command().name("afora");
