...
const EXAMPLES = [
  ["afora onboard", "Run guided setup for a local Gateway, workspace, auth, and channels."],
  ["afora setup", "Create the baseline config, workspace, and session folders."],
  ["afora configure", "Change models, Gateway, channels, plugins, skills, and health checks."],
  ["afora status", "Check Gateway, channel, model, and recent-session status."],
  ["afora doctor --fix", "Repair common config, service, plugin, and channel problems."],
  ["afora channels add", "Add or update a chat channel account with guided prompts."],
  ["afora channels status", "See connected messaging accounts and login state."],
  ["afora --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["afora gateway run --force", "Start the Gateway and replace anything bound to its port."],
  ["afora models status", "Show model/provider auth health before running agents."],
  ["afora plugins list", "Inspect enabled, disabled, and installed plugins."],
  [
    'afora agent --to +15555550123 --message "Run summary" --deliver',
    "Run one agent turn through the Gateway and optionally deliver the reply.",
  ],
  [
    'afora message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(
  ...
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env AFORA_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.afora-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates AFORA_STATE_DIR/AFORA_CONFIG_PATH under ~/.afora-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  if (isRootVersionInvocation(process.argv)) {
    ...
    console.log(
      commit ? `Afora ${ctx.programVersion} (${commit})` : `Afora ${ctx.programVersion}`,
    );
    process.exit(0);
  }

  program.addHelpText("beforeAll", () => {
    if (hasEmittedCliBanner() || process.env.AFORA_SUPPRESS_HELP_BANNER === "1") {
      return "";
    }
    const rich = isRich();
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.afora.ai/cli");
