...
describe("registerOnboardCommand", () => {
  ...
  it("forwards --gateway-token-ref-env", async () => {
    await runCli(["onboard", "--gateway-token-ref-env", "AFORA_GATEWAY_TOKEN"]);
    expect(setupWizardOptions().gatewayTokenRefEnv).toBe("AFORA_GATEWAY_TOKEN");
  });
  ...
  it("routes --modern through the inference-gated Afora entrypoint", async () => {
