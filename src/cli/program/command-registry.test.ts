...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "afora", "doctor"]);
  ...
  it("narrows to the primary command when command help is requested", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "afora", "doctor", "--help"]);
  ...
  it("keeps all placeholders for root help", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "afora", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "afora", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "afora", "status"], async () => {
      await program.parseAsync(["node", "afora", "status"]);
    });
