...
export function registerAgentsCommands(program: Command): void {
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.afora.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ['afora agents set-identity --agent main --name "Afora" --emoji "🦞"', "Set name + emoji."],
  ["afora agents set-identity --agent main --avatar avatars/afora.png", "Set avatar path."],
  [
    "afora agents set-identity --workspace ~/.afora/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "afora agents set-identity --identity-file ~/.afora/workspace/IDENTITY.md --agent main",
