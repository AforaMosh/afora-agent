...
import { resolveDatabasePath } from "../../state/afora-state-db-maintenance.js";
import { claimAforaStateOwnership } from "../../state/afora-state-ownership-operations.js";
import { inspectAforaStateOwnershipAtPath } from "../../state/afora-state-ownership.js";
...
async function runDatabasePreflight(databasePath: string, options: DatabaseOutputOptions) {
  const result = await preflightAforaStateDatabasePath(databasePath);
  if (options.json) {
  ...
  try {
    ...
    const ownership =
      options.manager !== undefined
        ? claimAforaStateOwnership(options.manager, {
            path: databasePath,
            env: process.env,
          })
...
export function registerDatabaseCommand(program: Command): void {
  const database = program
    ...
    .addHelpText("after", `\nDocs: ${AFORA_DATABASE_SCHEMA_DOCS_URL}\n`);
