...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.afora.ai/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["afora backup create", "Create a timestamped backup in the current directory."],
          [
          ...
          ["afora backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          [
            "afora backup verify ~/Backups/latest.tar.gz --json",
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "afora backup restore ~/Backups/latest.tar.gz --target ./restored-afora",
            "Verify, then extract the whole archive into a fresh staging directory.",
          ],
          [
            "afora backup restore ~/Backups/latest.tar.gz --target ./restored-afora --json",
...
function registerBackupGitCommands(backup: Command): void {
  ...
  git
    .command("create")
    .description("Dump selected Afora databases and commit one Git revision")
    .requiredOption("--repository <path>", "Git backup repository directory")
    .option("--all", "Back up the shared database and every registered agent database", false)
    .option("--global", "Back up the shared Afora state database", false)
...
function registerBackupSqliteCommands(backup: Command): void {
  ...
  sqlite
    .command("create")
    .description("Create a compact, verified snapshot of an Afora SQLite database")
    .option("--global", "Snapshot the shared Afora state database", false)
    .option("--agent <id>", "Snapshot one per-agent Afora database")
