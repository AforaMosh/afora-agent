...
function resolveFetchConfig(config: AforaConfig): Record<string, unknown> | undefined {
...
function resolveSearchConfig(config: AforaConfig): Record<string, unknown> | undefined {
...
function getCapabilityWebSearchAutoDetectTargets(config: AforaConfig): CommandSecretTargetScope {
...
function getCapabilityWebFetchAutoDetectTargets(config: AforaConfig): CommandSecretTargetScope {
  return getCapabilityWebProviderAutoDetectTargets({
    config,
...
function isScopedChannelSecretTargetEntry(params: {
  ...
  return (
    ...
    params.entry.configFile === "afora.json" &&
 ...
 */
export function getAgentRuntimeOptionalCommandSecretPaths(config: AforaConfig): Set<string> {
...
type CapabilityWebCommandSecretTargetParams = {
  ...
  autoDetectTargets: (config: AforaConfig) => CommandSecretTargetScope;
  selectedProviderTargetIds: (
    config: AforaConfig,
    providerId?: string | null,
  ) => SelectedProviderTargetIds;
