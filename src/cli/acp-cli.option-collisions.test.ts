...
describe("acp cli option collisions", () => {
  ...
  it.each([
    ...
    await withTempSecretFiles("afora-acp-cli-", files, async ({ tokenFile, passwordFile }) => {
  ...
  it("trims token file path before reading", async () => {
    await withTempSecretFiles("afora-acp-cli-", { token: "tok_file\n" }, async (files) => {
  ...
  it("reports missing token-file read errors", async () => {
    await parseAcp(["--token-file", "/tmp/afora-acp-missing-token.txt"]);
