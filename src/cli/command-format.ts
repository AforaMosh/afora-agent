...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+afora\b|^afora\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE = /^(?:(?:pnpm|npm|bunx|npx)\s+afora|afora)\b.*\supdate(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const rawContainer = env.AFORA_CONTAINER_HINT?.trim();
