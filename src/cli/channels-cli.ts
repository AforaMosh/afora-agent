...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["afora channels list", "List configured channels."],
          ["afora channels list --all", "Show configured, bundled, and installable channels."],
          ["afora channels add", "Open guided channel setup."],
          ["afora channels status --probe", "Run channel status checks and probes."],
          [
            "afora channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["afora channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.afora.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["afora channels add", "Open guided setup for available chat channels."],
          [
            "afora channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["afora channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
