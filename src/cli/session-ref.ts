...
} from "@afora/session-url-contract/parse";
...
function bareSessionOptionError(flag: string): Error {
  return new Error(
    `Unsupported bare session URL option: ${sanitizeTerminalText(flag)}. Use \`afora tui <url> --help\` for the full option list.`,
  );
}
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (!optionKey) {
      if (!arg.startsWith("-")) {
        ...
        throw new Error(
          "Unexpected extra argument for bare session URL. Use `afora tui <url> --help` for the full option list.",
        );
      }
