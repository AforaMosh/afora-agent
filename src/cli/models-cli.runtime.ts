...
export function rejectAgentScopedModelWrite(
  ...
  throw new Error(
    `afora models ${commandName} does not support --agent; it only updates global model defaults. Remove --agent, or run ${formatCliCommand("afora agents list")} and set the per-agent model in agent config.`,
  );
}
