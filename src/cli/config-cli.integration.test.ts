...
async function withExecDryRunConfigHarness(
  ...
  const envSnapshot = captureEnv(["AFORA_CONFIG_PATH", "AFORA_TEST_FAST"]);
  try {
    fs.writeFileSync(
      configPath,
      `${JSON.stringify(
...
describe("config cli integration", () => {
  it("accepts plugin hook conversation-access policy via config set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-config-cli-plugin-hooks-"));
    const configPath = path.join(tempDir, "afora.json");
    const envSnapshot = captureEnv(["AFORA_CONFIG_PATH", "AFORA_TEST_FAST"]);
    try {
      fs.writeFileSync(
      ...
      await runConfigSet({
        path: "plugins.entries.afora-mem0.hooks.allowConversationAccess",
        value: "true",
        cliOptions: {},
      ...
      expect(afterWrite.plugins?.entries?.["afora-mem0"]?.hooks).toEqual({
  ...
  it("supports batch-file dry-run and then writes real config changes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-config-cli-int-"));
    const configPath = path.join(tempDir, "afora.json");
    const batchPath = path.join(tempDir, "batch.json");
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-config-cli-int-fail-"));
    const configPath = path.join(tempDir, "afora.json");
    const envSnapshot = captureEnv([
      "AFORA_CONFIG_PATH",
      "AFORA_TEST_FAST",
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    await withExecDryRunConfigHarness("afora-config-cli-int-exec-skip-", async (params) => {
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    await withExecDryRunConfigHarness("afora-config-cli-int-exec-allow-", async (params) => {
      const before = fs.readFileSync(params.configPath, "utf8");
      await runConfigSet({
        cliOptions: {
          batchFile: params.batchPath,
