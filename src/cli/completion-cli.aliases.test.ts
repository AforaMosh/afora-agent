...
describe("completion-cli command aliases", () => {
  itWithFish.each([
    ["a canonical root command", "afora --profile work inf", "infer"],
    ["an aliased root command", "afora --profile work cap", "capability"],
    ["an inline profile and alias", "afora --profile=work cap", "capability"],
    ["an alias-shaped profile value", "afora --profile capability cap", "capability"],
    ["a repeated profile and alias", "afora --profile first --profile second cap", "capability"],
  ...
  it("completes root and nested aliases in zsh lists and dispatch", () => {
    ...
    expect(script).toContain("(infer|capability) _afora_infer ;;");
    expect(script).toContain("'create[Add a job]'");
    expect(script).toContain("(add|create) _afora_cron_add ;;");
  });
  ...
  it("completes aliases and their subtrees in fish", () => {
    ...
    expect(script).toContain(
      'complete -c afora -n "__afora_command_path_matches -- --profile" -a "capability" -d \'Run inference\'',
    );
    expect(script).toContain(
      'complete -c afora -n "__afora_command_path_matches capability -- --profile" -a "embed" -d \'Embed text\'',
    );
    expect(script).toContain(
      'complete -c afora -n "__afora_command_path_matches cron -- --profile" -a "create" -d \'Add a job\'',
    );
    expect(script).toContain(
      "complete -c afora -n \"__afora_command_path_matches cron create -- --profile --at\" -l at -r -d 'Schedule time'",
    );
  });

  itWithFish.each([
    ["an aliased nested command", "afora cron create -"],
    ["a canonical nested command", "afora cron add -"],
    ["a global profile", "afora --profile work cron create -"],
    ["an inline global profile", "afora --profile=work cron create -"],
    ["repeated global profiles", "afora --profile first --profile second cron create -"],
    ["an inherited global profile", "afora cron --profile work create -"],
    ["a parent long option", "afora cron --timezone UTC create -"],
    ["a parent short option", "afora cron -z UTC create -"],
    ["an inline parent option", "afora cron --timezone=UTC create -"],
    ["a parent boolean option", "afora cron --verbose create -"],
  ...
  itWithFish.each([
    ["an aliased positional argument", "afora cron create meeting -"],
    ["a canonical positional argument", "afora cron add meeting -"],
    ["a profiled positional argument", "afora --profile work cron create meeting -"],
    ["a parent option and positional argument", "afora cron -z UTC create meeting -"],
  ...
  itWithPowerShell.each([
    ["a global option", "afora --profile work cron create --a"],
    ["an inline global option", "afora --profile=work cron create --a"],
    ["repeated global options", "afora --profile first --profile second cron create --a"],
    ["an inherited option after the parent", "afora cron --profile work create --a"],
    ["the canonical nested command", "afora --profile work cron add --a"],
