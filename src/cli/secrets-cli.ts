...
async function readPlanFile(pathname: string): Promise<SecretsApplyPlan> {
  ...
  if (!isSecretsApplyPlan(parsed)) {
    throw new Error(
      `Invalid secrets plan file: ${pathname}. Generate a fresh plan with ${formatCliCommand("afora secrets configure --plan-out <path>")}.`,
    );
  }
  return parsed;
}
...
export function registerSecretsCli(program: Command): void {
  const secrets = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/gateway/security", "docs.afora.ai/gateway/security")}\n`,
    );
      ...
      defaultRuntime.error(
        danger(
          formatGatewayCommandFailure({
            ...
            error: err,
            inspectCommand: "afora gateway status --deep",
          }),
        ),
      );
  ...
  secrets
    ...
    .action(async (opts: SecretsAuditOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets audit failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("afora doctor")} to inspect config and credential state.`,
          ),
        );
  ...
  secrets
    ...
    .action(async (opts: SecretsConfigureOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets configure failed: ${formatErrorMessage(err)}. Re-run ${formatCliCommand("afora secrets audit")} before applying changes.`,
          ),
        );
        defaultRuntime.exit(1);
      }
  ...
  secrets
    ...
    .action(async (opts: SecretsApplyOptions) => {
      try {
        ...
        defaultRuntime.error(
          danger(
            `Secrets apply failed: ${message}. Re-run ${formatCliCommand("afora secrets apply --from <path> --dry-run")} to inspect the plan without writing.`,
          ),
        );
        defaultRuntime.exit(1);
      }
