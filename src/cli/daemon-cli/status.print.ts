...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("afora doctor")}" (or "${formatCliCommand("afora doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("afora gateway install --force")}\` from the same --profile / AFORA_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this Afora command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `afora --version`, `which afora`, and `afora gateway status --deep`; if this mismatch is unexpected, update PATH so `afora` points to the version you want, or reinstall the Gateway service from that same Afora install.",
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("afora gateway restart")} and re-check with ${formatCliCommand("afora gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (status.connections?.established.length) {
    ...
    defaultRuntime.log(
      warnText(
        "If logs show protocol mismatch after rollback, stop stale Afora client processes listed here and re-run gateway status.",
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.AFORA_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("afora gateway install")}`),
    );
    spacer();
  }

  if (service.staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale Afora updater launchd job(s) detected."));
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("afora gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.AFORA_PROFILE);
      defaultRuntime.error(
  ...
  if (drift && drift.drifts.length > 0) {
    ...
    if (opts.deep) {
      ...
      if (updateCommands.length === 1) {
        defaultRuntime.log(
          `${label("Fix:")} ${updateCommands[0]} && ${formatCliCommand("afora gateway restart")}.`,
        );
      } else {
        ...
        defaultRuntime.log(`Then run ${formatCliCommand("afora gateway restart")}.`);
      }
    } else {
      defaultRuntime.log(
        infoText(
          `Run ${formatCliCommand("afora gateway status --deep")} for affected plugin ids and fix commands.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("afora status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.afora.ai/troubleshooting`);
}
