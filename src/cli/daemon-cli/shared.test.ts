...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        AFORA_CONTAINER: "afora-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("afora-demo-container");
    expect(
      resolveDaemonContainerContext({
        AFORA_CONTAINER_HINT: "afora-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("afora-demo-container");
  });

  it("prepends a single container restart hint when AFORA_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        AFORA_CONTAINER: "afora-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when AFORA_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        AFORA_CONTAINER_HINT: "afora-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when AFORA_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
          "If you're in a container, run the gateway in the foreground instead of `afora gateway`.",
        ],
        { AFORA_CONTAINER: "afora-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when AFORA_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { AFORA_CONTAINER_HINT: "afora-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
