...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("afora-daemon-install-int-");
    configPath = path.join(tempHome, "afora.json");
    process.env.HOME = tempHome;
    process.env.AFORA_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer Afora", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it.each([
    ...
    serviceMock.install.mockRejectedValueOnce(
      new Error(
        "System systemd unit afora-gateway.service already owns this gateway unit name. --force does not override system ownership.",
      ),
    );
    ...
    const joined = runtimeLogs.join("\n");
    expect(joined).toContain("System systemd unit afora-gateway.service");
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.AFORA_GATEWAY_TOKEN).toBeUndefined();
  });
});
