...
async function assertUnmanagedGatewayRestartEnabled(port: number): Promise<void> {
  ...
  const probe = await probeGateway({
    ...
    auth: {
      token: normalizeOptionalString(process.env.AFORA_GATEWAY_TOKEN),
      password: normalizeOptionalString(process.env.AFORA_GATEWAY_PASSWORD),
    },
    timeoutMs: 1_000,
  }).catch(() => null);
...
async function signalGatewayRestart(
  ...
  if (pids.length > 1) {
    throw new Error(
      `multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "afora gateway status --deep" before retrying restart`,
    );
  }
    ...
    postRestartCheck: async ({ warnings, fail, stdout, warn }) => {
      if (restartedWithoutServiceManager) {
        ...
        fail(
          ...
          [formatCliCommand("afora gateway status --deep"), formatCliCommand("afora doctor")],
        );
