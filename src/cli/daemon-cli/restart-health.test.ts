...
describe("restart health", () => {
  ...
  it("treats a gateway listener child pid as healthy ownership", async () => {
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 7001, ppid: 7000, commandLine: "afora-gateway" }],
        hints: [],
      },
    });
  ...
  it("uses the configured non-loopback host for restart-health port inspection", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 7000, commandLine: "afora-gateway" }],
      hints: [],
    });
  ...
  it("treats a busy port as healthy when runtime status lags but the probe succeeds", async () => {
    ...
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 9100, commandLine: "afora-gateway" }],
        hints: [],
      },
    });
  ...
  it("requires the expected gateway version when provided", async () => {
    ...
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 8000, commandLine: "afora-gateway" }],
        hints: [],
      },
    });
  ...
  it("uses configured local probe auth while waiting for a matching-version restart", async () => {
    ...
    const serviceEnv = {
      ...process.env,
      AFORA_STATE_DIR: "/tmp/afora-restart-service-state",
