...
function assertGatewayRepairTargetMatches(params: {
  ...
  const installedStateOverride = installedEnv.AFORA_STATE_DIR?.trim();
  const installedHome =
    installedEnv.AFORA_HOME?.trim() ||
    installedEnv.HOME?.trim() ||
    installedEnv.USERPROFILE?.trim();
  if (!installedStateOverride && !installedHome) {
    throw new Error(
      `Refusing to repair the managed Gateway service because its installed state directory cannot be determined from the service definition. Run \`afora gateway install --force\` to replace it intentionally.`,
    );
  }
  ...
  for (const [name, installed, ambient] of [
    ["AFORA_STATE_DIR", installedStateDir, ambientStateDir],
    ["AFORA_CONFIG_PATH", installedConfigPath, ambientConfigPath],
  ] as const) {
  ...
  const details = differences
    ...
    .join("\n");
  throw new Error(
    `Refusing to repair the managed Gateway service because the current invocation targets a different Gateway:\n${details}\nRun \`afora gateway ${params.action}\` with the installed state directory, config path, and port (or unset conflicting environment overrides). To retarget intentionally, run \`afora gateway install --force\`.`,
  );
}
...
export async function repairLoadedGatewayServiceForStart(
  ...
  const wrapperPath = await resolveAforaWrapperPath(installEnv[AFORA_WRAPPER_ENV_KEY]);
  ...
  return {
    ...
    message:
      params.action === "restart"
        ...
        : "Gateway service definition repaired and started. Reopen the Control UI with `afora dashboard` or copy a fresh auth URL with `afora dashboard --no-open`.",
