...
export function formatUnknownChannelMessage(params: {
  ...
  const listCommand = params.listCommand ?? "afora channels list --all";
...
export function formatUnsupportedChannelActionMessage(params: {
  ...
  const inspectCommand =
    params.inspectCommand ?? `afora channels capabilities --channel ${params.channel}`;
...
export function formatStrictJsonParseFailure(params: { value: string; cause: unknown }): string {
  ...
  return [
    ...
    `Use valid JSON, for example ${formatInlineCliCommand(
      "afora config set gateway.port 18789 --strict-json",
    )}.`,
...
export function formatGatewayCommandFailure(params: {
  ...
  const message = raw
    .replace(/\s*Run [`"]?afora doctor[`"]? for diagnostics\.?/gi, "")
    .replace(/\s+Gateway target:\s+.*$/isu, "")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/[.。]+$/u, "");
  const inspectCommand = params.inspectCommand ?? "afora gateway status --deep";
...
export function formatMissingPluginMessage(params: {
  ...
  const listCommand = params.listCommand ?? "afora plugins list";
  const searchHint = params.includeSearch
    ? `, or ${formatInlineCliCommand("afora plugins search " + params.id)} to look for installable plugins`
    : "";
