...
function createConfigureInteractiveResult(options?: {
  ...
  return {
    plan: {
      ...
      protocolVersion: 1,
      generatedAt: "2026-02-26T00:00:00.000Z",
      generatedBy: "afora secrets configure",
      targets: options?.targets ?? [],
    },
    preflight: {
      mode: "dry-run" as const,
      changed: options?.changed ?? false,
      changedFiles: options?.changed ? ["/tmp/afora.json"] : [],
      checks: {
        resolvability: true,
...
async function withPlanFile(
  ...
  const planPath = path.join(
    os.tmpdir(),
    `afora-secrets-cli-test-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
  );
...
describe("secrets CLI", () => {
  ...
  it("explains Gateway reload failures without duplicate doctor noise", async () => {
    callGatewayFromCli.mockRejectedValue(
      new Error(
        "gateway closed (1006 abnormal closure). Gateway target: ws://127.0.0.1:18789 Source: local loopback Config: /tmp/afora.json Bind: loopback Possible causes: - Gateway not yet ready. Run `afora doctor` for diagnostics.",
      ),
    );
    ...
    expect(runtimeErrors.at(-1)).toContain("afora gateway status --deep");
  ...
  it("writes generated secrets plan files at the apply limit", async () => {
    const planPath = path.join(
      os.tmpdir(),
      `afora-secrets-configure-test-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    runSecretsConfigureInteractive.mockResolvedValue(
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-secrets-cli-fifo-"));
