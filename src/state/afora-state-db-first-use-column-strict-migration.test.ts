...
import { CLAW_FIRST_USE_ADDITIVE_STATE_COLUMN_DEFINITIONS } from "./afora-state-db-additive-columns.js";
import {
...
function makePreStrictDatabaseWithoutColumns(params: {
  ...
  const options = { env: { AFORA_STATE_DIR: params.stateDir } };
...
describe("first-use additive column definitions", () => {
  ...
  it.each(
    ...
    const tableStart = AFORA_STATE_SCHEMA_SQL.indexOf(
      `CREATE TABLE IF NOT EXISTS ${tableName} (`,
    );
    expect(tableStart).toBeGreaterThanOrEqual(0);
    const tableBody = AFORA_STATE_SCHEMA_SQL.slice(
      tableStart,
      AFORA_STATE_SCHEMA_SQL.indexOf(") STRICT;", tableStart),
    );
...
describe("first-use additive column STRICT migration", () => {
  it("repairs device_bootstrap_tokens without setup_id while migrating to STRICT", () => {
    const stateDir = tempDirs.make("afora-state-first-use-column-");
  ...
  it("repairs session_groups without the folder default columns", () => {
    const stateDir = tempDirs.make("afora-state-first-use-group-");
    const options = { env: { AFORA_STATE_DIR: stateDir } };
    makePreStrictDatabaseWithoutColumns({
      stateDir,
  ...
  it("migrates on a writable cold open without a doctor repair", () => {
    const stateDir = tempDirs.make("afora-state-first-use-cold-open-");
    const options = { env: { AFORA_STATE_DIR: stateDir } };
    makePreStrictDatabaseWithoutColumns({
      stateDir,
      tableName: "device_bootstrap_tokens",
