...
function standingIntentsSchemaSql(): string {
  const start = AFORA_AGENT_SCHEMA_SQL.indexOf(STANDING_INTENTS_SCHEMA_START);
  const end = AFORA_AGENT_SCHEMA_SQL.indexOf(STANDING_INTENTS_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Afora standing-intents schema markers are missing.");
  }
  return AFORA_AGENT_SCHEMA_SQL.slice(start, end);
}
...
export function ensureAforaAgentStandingIntentsSchema(db: DatabaseSync): void {
  const ensure = () => {
