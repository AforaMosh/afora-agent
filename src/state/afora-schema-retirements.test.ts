...
import retirementManifest from "./afora-schema-retirements.json" with { type: "json" };
import { AFORA_STATE_SCHEMA_VERSION } from "./afora-state-db-contract.js";
...
it("keeps the schema retirement ledger aligned with canonical database schemas", () => {
  ...
  const databases = {
    state: {
      currentVersion: AFORA_STATE_SCHEMA_VERSION,
      schemaUrl: new URL("./afora-state-schema.sql", import.meta.url),
      database: new DatabaseSync(":memory:"),
    },
    agent: {
      currentVersion: AFORA_AGENT_SCHEMA_VERSION,
      schemaUrl: new URL("./afora-agent-schema.sql", import.meta.url),
      database: new DatabaseSync(":memory:"),
    },
  } satisfies Record<
    DatabaseOwner,
