...
function splitBoardSchema(sql: string): { board: string; withoutBoard: string } {
  ...
  if (start === -1 || end === -1) {
    throw new Error("Afora agent board schema markers are missing from the canonical schema.");
  }
  return {
...
const boardSchema = splitBoardSchema(AFORA_AGENT_SCHEMA_SQL);

const AFORA_AGENT_BOARD_SCHEMA_SQL = boardSchema.board;
export const AGENT_V14_BOARD_SCHEMA_SQL = AFORA_AGENT_BOARD_SCHEMA_SQL;
export const AFORA_AGENT_SCHEMA_WITHOUT_BOARD_SQL = boardSchema.withoutBoard;

function canonicalBoardWidgetsCreateSql(): string {
  const start = AFORA_AGENT_BOARD_SCHEMA_SQL.indexOf(BOARD_WIDGETS_SCHEMA_START);
  const end = AFORA_AGENT_BOARD_SCHEMA_SQL.indexOf(BOARD_WIDGETS_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Afora agent board widget schema markers are missing.");
  }
  return AFORA_AGENT_BOARD_SCHEMA_SQL.slice(start, end).trim();
}
...
function legacyBoardWidgetsCreateSql(): string {
  ...
  if (legacy === canonical) {
    throw new Error("Afora agent board widget legacy schema derivation failed.");
  }
  return legacy;
}
...
export function ensureAforaAgentBoardSchemaInTransaction(db: DatabaseSync): void {
  if (!db.isTransaction) {
    throw new Error("board schema ensure requires an active transaction");
  }
  db.exec(AFORA_AGENT_BOARD_SCHEMA_SQL); // sqlite-allow-raw -- Canonical DDL bootstrap for the lazy board schema.
  ...
  if (typeof row?.sql !== "string") {
    throw new Error("Afora agent board widget schema is missing after ensure.");
  }
  ...
  if (normalizedSchema !== normalizeBoardWidgetsCreateSql(legacyBoardWidgetsCreateSql())) {
    throw new Error(
      "Afora agent board widget schema has an unsupported content-kind constraint.",
    );
  }
  ...
  if (existingMigrationTable) {
    throw new Error(
      `Afora agent board migration table already exists: ${BOARD_WIDGETS_MIGRATION_TABLE}`,
    );
  }
  ...
  `);
  db.exec(AFORA_AGENT_BOARD_SCHEMA_SQL); // sqlite-allow-raw -- Restore the canonical board index after the rebuild.
}
