...
type LeaseDatabase = Pick<AforaStateKyselyDatabase, "state_leases">;
type LeaseKysely = ReturnType<typeof getNodeSqliteKysely<LeaseDatabase>>;

type AforaStateLeaseDatabase = {
  scope: "shared";
  options?: AforaStateDatabaseOptions;
};

type AforaStateLeaseOptions = {
  scope: string;
  key: string;
  database: AforaStateLeaseDatabase;
  leaseMs: number;
  waitMs: number;
  signal?: AbortSignal;
...
export type AforaStateLeaseContext = {
  signal: AbortSignal;
...
type AforaStateLeaseErrorCode =
  | "AFORA_STATE_LEASE_INVALID_INPUT"
  | "AFORA_STATE_LEASE_TIMEOUT"
  | "AFORA_STATE_LEASE_ABORTED"
  | "AFORA_STATE_LEASE_LOST"
  | "AFORA_STATE_LEASE_STORAGE_FAILED";

export class AforaStateLeaseError extends Error {
  readonly code: AforaStateLeaseErrorCode;

  constructor(message: string, options: { code: AforaStateLeaseErrorCode; cause?: unknown }) {
    super(message, { cause: options.cause });
    this.name = "AforaStateLeaseError";
...
function leaseError(
  code: AforaStateLeaseErrorCode,
  message: string,
  cause?: unknown,
): AforaStateLeaseError {
  return new AforaStateLeaseError(message, {
    code,
    ...(cause === undefined ? {} : { cause }),
  });
}

function invalidInput(message: string): AforaStateLeaseError {
  return leaseError("AFORA_STATE_LEASE_INVALID_INPUT", message);
}
...
function validateOptions(options: AforaStateLeaseOptions) {
...
function withLeaseWriteTransaction<T>(
  ...
  const stateDatabase = openAforaStateDatabase(database.options);
  const run = () =>
    runAforaStateWriteTransaction(
...
function withLeaseRead<T>(
  ...
  const sqlite = openAforaStateDatabase(database.options).db;
...
function verifyLeaseOwnership(
  params: LeaseIdentity & { database?: AforaStateLeaseDatabase; transaction?: DatabaseSync },
): void {
  try {
    ...
    if (error instanceof AforaStateLeaseError) {
      throw error;
    }
    throw leaseError(
      "AFORA_STATE_LEASE_STORAGE_FAILED",
...
export async function withAforaStateLease<T>(
  options: AforaStateLeaseOptions,
  run: (lease: AforaStateLeaseContext) => Promise<T>,
): Promise<T> {
  ...
  const abortLost = (cause?: unknown) => {
    if (!leaseLost.signal.aborted) {
      leaseLost.abort(
        cause instanceof AforaStateLeaseError
          ? cause
          : leaseError(
              "AFORA_STATE_LEASE_LOST",
  ...
  const heartbeat = setInterval(() => {
    try {
      ...
      if (error instanceof AforaStateLeaseError && error.code === "AFORA_STATE_LEASE_LOST") {
        abortLost(error);
