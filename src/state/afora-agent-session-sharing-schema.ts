import { AGENT_SCHEMA_WITHOUT_PROGRESS_CARD_SQL } from "./afora-agent-progress-card-schema.js";
...
function splitSessionSharingSchema(sql: string): { sharing: string; withoutSharing: string } {
  ...
  if (start === -1 || end === -1) {
    throw new Error("Afora agent session-sharing schema markers are missing.");
  }
  return {
...
if (sessionSuggestionsStart === -1) {
  throw new Error("Afora agent session-suggestions schema marker is missing.");
}
