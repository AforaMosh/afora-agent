...
import {
  ...
  preflightAforaStateDatabasePath,
  preflightAforaDatabaseSchemas,
} from "./afora-database-preflight.js";
import {
...
describe("Afora database schema preflight", () => {
  ...
  function createExplicitStateDatabase(schemaSql = AFORA_STATE_SCHEMA_SQL): string {
    const stateDir = tempDirs.make("afora-explicit-state-preflight-");
    const databasePath = path.join(stateDir, "candidate.sqlite");
    const { DatabaseSync } = requireNodeSqlite();
    const database = new DatabaseSync(databasePath);
    try {
      database.exec(`${schemaSql}; PRAGMA user_version = ${AFORA_STATE_SCHEMA_VERSION};`);
      database
        .prepare(
  ...
  it("reports an exact current schema for one explicit copied database", async () => {
    const stateDir = tempDirs.make("afora-runtime-state-preflight-");
    ...
    await expect(preflightAforaStateDatabasePath(databasePath)).resolves.toEqual({
      schema: "afora.state-schema-preflight.v1",
      databasePath,
      targetVersion: AFORA_STATE_SCHEMA_VERSION,
      foundVersion: AFORA_STATE_SCHEMA_VERSION,
      ownership: null,
      issues: [],
      status: "exact",
      requiresWrite: false,
  ...
  it("treats a supported persistent column definition as exact", async () => {
    const databasePath = createExplicitStateDatabase(
      AFORA_STATE_SCHEMA_SQL.replace(
  ...
  it("accepts a copied current schema with a future bare nullable column without touching it", async () => {
    ...
    const databasePath = path.join(
      tempDirs.make("afora-copied-state-preflight-"),
      "candidate.sqlite",
    );
  ...
  it("classifies the same-version run-end cleanup column as startup-repairable without touching the source", async () => {
    ...
    const snapshotPath = path.join(
      tempDirs.make("afora-consolidated-state-preflight-"),
      "candidate.sqlite",
    );
    ...
    try {
      ...
      const result = await preflightAforaStateDatabasePath(snapshotPath);
  ...
  it("accepts first-use session group columns without requiring a startup write", async () => {
    ...
    await expect(preflightAforaStateDatabasePath(databasePath)).resolves.toMatchObject({
      status: "exact",
      requiresWrite: false,
      issues: [],
    });
  ...
  it("reports an explicit unreadable path as indeterminate", async () => {
    const stateDir = tempDirs.make("afora-explicit-unreadable-preflight-");
  ...
  it("treats a current-v6 additive column as incompatible with the older v6 shape", () => {
    ...
    try {
      database.exec(AFORA_STATE_SCHEMA_SQL);
      const olderV6Schema = AFORA_STATE_SCHEMA_SQL.replace(
        "  removed_at INTEGER,\n  run_end_cleanup_json TEXT\n",
        "  removed_at INTEGER\n",
      );
  ...
  it("keeps package schema support metadata aligned", () => {
    expect(packageJson.afora.schemaVersions).toEqual({
  ...
  it("accepts a supported state schema", () => {
    const stateDir = tempDirs.make("afora-database-preflight-supported-");
    const env = { AFORA_STATE_DIR: stateDir };
    ...
    expect(() => assertAforaDatabasesReadyForRestart({ env })).not.toThrow();
  });

  it("accepts an older v6 state database without the lazy setup id during restart preflight", () => {
    const stateDir = tempDirs.make("afora-database-preflight-older-v6-setup-id-");
    const env = { AFORA_STATE_DIR: stateDir };
  ...
  it("reports a current but noncanonical state schema as indeterminate", () => {
    const stateDir = tempDirs.make("afora-database-preflight-noncanonical-state-");
    const env = { AFORA_STATE_DIR: stateDir };
    const statePath = openAforaStateDatabase({ env }).path;
    closeAforaStateDatabaseForTest();
    ...
    expect(() => assertAforaDatabasesReadyForRestart({ env })).toThrow(
  ...
  it("collects newer state and registered agent schemas with writer builds", () => {
    const stateDir = tempDirs.make("afora-database-preflight-");
    const env = { AFORA_STATE_DIR: stateDir };
    const statePath = openAforaStateDatabase({ env }).path;
    ...
    const state = new DatabaseSync(statePath);
    try {
      state.exec(`PRAGMA user_version = ${AFORA_STATE_SCHEMA_VERSION + 1};`);
      state
    ...
    try {
      agent.exec(`PRAGMA user_version = ${AFORA_AGENT_SCHEMA_VERSION + 1};`);
      agent
        .prepare("UPDATE schema_meta SET app_version = ? WHERE meta_key = 'primary'")
      ...
      incompatible: [
        {
          kind: "state",
          path: statePath,
          foundVersion: AFORA_STATE_SCHEMA_VERSION + 1,
          supportedVersion: AFORA_STATE_SCHEMA_VERSION,
        ...
        },
        {
          ...
          path: agentPath,
          agentId: "worker-1",
          foundVersion: AFORA_AGENT_SCHEMA_VERSION + 1,
          supportedVersion: AFORA_AGENT_SCHEMA_VERSION,
  ...
  it("reports a current but noncanonical registered agent schema as indeterminate", () => {
    const stateDir = tempDirs.make("afora-database-preflight-noncanonical-agent-");
    const env = { AFORA_STATE_DIR: stateDir };
    const agentPath = openAforaAgentDatabase({ agentId: "worker-1", env }).path;
    closeAforaAgentDatabasesForTest();
    closeAforaStateDatabaseForTest();
  ...
  it("reports an existing unreadable state database as indeterminate", () => {
    const stateDir = tempDirs.make("afora-database-preflight-unreadable-state-");
    const env = { AFORA_STATE_DIR: stateDir };
    const statePath = openAforaStateDatabase({ env }).path;
    closeAforaStateDatabaseForTest();
  ...
  it("reports a failed agent registry query as indeterminate", () => {
    const stateDir = tempDirs.make("afora-database-preflight-registry-");
    const env = { AFORA_STATE_DIR: stateDir };
    const statePath = openAforaStateDatabase({ env }).path;
    closeAforaStateDatabaseForTest();
    const { DatabaseSync } = requireNodeSqlite();
  ...
  it("reports an existing unreadable registered agent database as indeterminate", () => {
    const stateDir = tempDirs.make("afora-database-preflight-unreadable-agent-");
    const env = { AFORA_STATE_DIR: stateDir };
    const agentPath = openAforaAgentDatabase({ agentId: "worker-1", env }).path;
    closeAforaAgentDatabasesForTest();
    closeAforaStateDatabaseForTest();
