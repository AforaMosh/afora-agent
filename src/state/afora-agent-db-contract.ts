...
export const AFORA_AGENT_SCHEMA_VERSION = 17;

/** Open per-agent SQLite database handle plus lifecycle maintenance. */
export type AforaAgentDatabase = {
  agentId: string;
  db: DatabaseSync;
  path: string;
...
export type AforaAgentDatabaseOptions = AforaStateDatabaseOptions & {
  agentId: string;
};

/** Shared-state registry row describing an agent database seen by this process. */
export type AforaRegisteredAgentDatabase = {
  agentId: string;
  path: string;
...
export type AforaAgentDatabaseOwnerInspection =
