...
import { withAforaStateLease } from "./afora-state-lease.js";
...
describe("Afora state lease", () => {
  it("releases ownership when a CLI exits from inside the leased operation", async () => {
    await withAforaTestState({ label: "core-state-lease-process-exit" }, async (state) => {
      const leaseModuleUrl = pathToFileURL(path.resolve("src/state/afora-state-lease.ts")).href;
      const childScript = await state.writeText(
        "lease-process-exit-child.mts",
        `
          import { withAforaStateLease } from ${JSON.stringify(leaseModuleUrl)};
          const stateDir = process.argv[2];
          await withAforaStateLease({
            scope: "core:test",
            key: "process-exit",
            database: { scope: "shared", options: { env: { ...process.env, AFORA_STATE_DIR: stateDir } } },
            leaseMs: 300_000,
            waitMs: 0,
  ...
  it("keeps state database exit-cleanup diagnostics off stdout for machine-readable output", async () => {
    await withAforaTestState({ label: "core-state-lease-exit-stdout" }, async (state) => {
      const leaseModuleUrl = pathToFileURL(path.resolve("src/state/afora-state-lease.ts")).href;
      const stateDbModuleUrl = pathToFileURL(path.resolve("src/state/afora-state-db.ts")).href;
      ...
      const childResult = await new Promise<{
        ...
        const child = spawn(process.execPath, ["--import", "tsx", childScript, state.stateDir], {
          ...
          env: { ...process.env, AFORA_TEST_CONSOLE: "1" },
          stdio: ["ignore", "pipe", "pipe"],
        });
        let stdout = "";
        let stderr = "";
  ...
  it("rechecks exact ownership inside the caller's write transaction", async () => {
    await withAforaTestState({ label: "core-state-lease" }, async () => {
      await expect(
        withAforaStateLease(
          {
            scope: "core:test",
          ...
          async (lease) => {
            runAforaStateWriteTransaction(({ db }) => {
              ...
              expect(() => lease.assertOwnedInTransaction(db)).toThrowError(
                expect.objectContaining({ code: "AFORA_STATE_LEASE_LOST" }),
              );
            });
          },
        ),
      ).rejects.toMatchObject({ code: "AFORA_STATE_LEASE_LOST" });
    });
  });
});
