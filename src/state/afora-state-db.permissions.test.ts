...
vi.mock("node:fs", async (importOriginal) => {
  ...
  const chmodSync: typeof actual.chmodSync = ((target: unknown, mode: unknown) => {
    ...
    const isProbe = String(target).includes(".afora-chmod-probe-");
...
} = await import("./afora-state-db.js");
...
describe("state database permission hardening without chmod support", () => {
  ...
  it("opens the state database when chmodSync throws ENOTSUP", () => {
    stateDir = fs.mkdtempSync(join(tmpdir(), "afora-state-chmod-"));
  ...
  it("rethrows EPERM when existing permissions are too broad", () => {
    ...
    expect(() => openAforaStateDatabase({ env: { AFORA_STATE_DIR: stateDir } })).toThrow(
      /EPERM/,
    );
  });
  ...
  it("repairs the schema when chmodSync throws ENOTSUP", () => {
    ...
    expect(() =>
      repairAforaStateDatabaseSchema({ env: { AFORA_STATE_DIR: stateDir } }),
    ).not.toThrow();
  });
  ...
  it("commits write transactions when chmodSync throws ENOTSUP", () => {
    ...
    const result = runAforaStateWriteTransaction((database) => {
      expect(database.db.isOpen).toBe(true);
      return "committed";
    }, options);
