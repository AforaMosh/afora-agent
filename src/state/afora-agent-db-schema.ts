...
import { normalizeNullableString as migratedText } from "@afora/normalization-core/string-coerce";
...
import { ensureAforaAgentBoardSchemaInTransaction } from "./afora-agent-board-schema.js";
import {
...
import { registerAforaAgentDatabase } from "./afora-agent-db-registry.js";
import {
  assertExistingAgentSchemaOwner,
  assertAforaAgentSchemaContains,
  assertAforaAgentCurrentRuntimeSchema,
  assertSupportedAgentSchemaVersion,
  ensureSessionKeyContractSchemaInTransaction,
  readExistingAgentSchemaMeta,
  repairAndAssertAforaAgentV14SchemaForMigration,
...
} from "./afora-agent-db-session-migrations.js";
import { migrateSessionNodesAndWindows } from "./afora-agent-db-session-nodes-migration.js";
import {
  addSessionProvenanceColumns,
  backfillSessionEntryProvenance,
  backfillTranscriptMutationWatermarks,
} from "./afora-agent-db-session-provenance.js";
...
type AforaAgentMetadataDatabase = Pick<AforaAgentKyselyDatabase, "schema_meta">;
...
function migrateAforaAgentSchema(db: DatabaseSync): void {
...
function backfillAforaAgentSchema(db: DatabaseSync, previousVersion: number): void {
...
export function assertAgentDatabaseIntegrityBeforeMutation(
  ...
  if (userVersion === AFORA_AGENT_SCHEMA_VERSION && !hasPendingCurrentVersionMigration) {
    verifyAndRepairCanonicalSqliteIndexes(database, pathname, AFORA_AGENT_SCHEMA_SQL, {
      allowMissingColumns: true,
      validateAfterRepair: () =>
        assertAforaAgentCurrentRuntimeSchema(database, { agentId, pathname }),
    });
  } else {
...
function assertAgentSchemaVersion(
  ...
  if (userVersion !== options.version || metadata?.schemaVersion !== options.version) {
    throw new Error(
      `Afora agent database ${options.pathname} did not converge on schema version ${options.version}.`,
    );
  }
  assertAforaAgentSchemaContains(db, options.pathname, AFORA_AGENT_SCHEMA_SQL);
}
...
function ensureAgentSchema(
  ...
  agentId: string,
  pathname: string,
  targetVersion = AFORA_AGENT_SCHEMA_VERSION,
): void {
  ...
  try {
    runSqliteImmediateTransactionSync(db, () => {
      ...
      if (previousVersion > targetVersion) {
        throw new Error(
          `Afora agent database ${pathname} uses schema version ${previousVersion}; expected at most ${targetVersion} for this migration.`,
        );
      }
      ...
      if (previousVersion === targetVersion) {
        ...
        repairCanonicalSqliteIndexes(db, pathname, AFORA_AGENT_SCHEMA_SQL, {
          verifyPhysicalIntegrity: false,
        });
        assertAgentSchemaVersion(db, { agentId, pathname, version: targetVersion });
        return;
      } else if (previousVersion === 14) {
        repairAndAssertAforaAgentV14SchemaForMigration(db, { agentId, pathname });
      }
      ...
      migrateAforaAgentSchema(db);
      migrateConversationDeliveryTargetColumn(db);
      backfillAforaAgentSchema(db, previousVersion);
      ...
      const kysely = getNodeSqliteKysely<AforaAgentMetadataDatabase>(db);
...
export function ensureAforaAgentDatabaseSchema(
  db: DatabaseSync,
  options: AforaAgentDatabaseOptions & { register?: boolean },
): void {
...
export function migrateAforaAgentDatabaseToMediaPrerequisiteSchema(
  db: DatabaseSync,
  options: AforaAgentDatabaseOptions,
): void {
  const targetVersion = AFORA_AGENT_SCHEMA_VERSION - 1;
  const userVersion = readSqliteUserVersion(db);
...
export { ensureAgentSchema as ensureAforaAgentSchema };
