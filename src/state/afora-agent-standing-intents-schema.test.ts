...
import { ensureAforaAgentStandingIntentsSchema } from "./afora-agent-standing-intents-schema.js";
...
function schemaWithoutStandingIntents(): string {
  return removeSchemaSection(AFORA_AGENT_SCHEMA_SQL, STANDING_SCHEMA_START, STANDING_SCHEMA_END);
}
...
function schemaWithoutStandingIntentCreator(): string {
  if (!AFORA_AGENT_SCHEMA_SQL.includes(STANDING_CREATOR_COLUMN)) {
    throw new Error("standing-intent creator column missing in test fixture");
  }
  return AFORA_AGENT_SCHEMA_SQL.replace(STANDING_CREATOR_COLUMN, "");
}
...
describe("additive memory agent schemas", () => {
  ...
  it("accepts a current-version database before the lazy ensure and ensures idempotently", async () => {
    tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-intent-schema-")),
    );
    ...
    try {
      ...
      db.exec(`PRAGMA user_version = ${AFORA_AGENT_SCHEMA_VERSION};`);
      db.prepare(
      ...
      ensureAforaAgentStandingIntentsSchema(db);
      ensureAforaAgentStandingIntentsSchema(db);
  ...
  it("accepts current-version databases before the provenance lazy ensure", async () => {
    tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-provenance-schema-")),
    );
    const databasePath = path.join(tempDir, "afora-agent.sqlite");
    const db = openNodeSqliteDatabase(databasePath);
    try {
  ...
  it("lazily adds creator provenance to the unreleased standing-intent table", async () => {
    tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-intent-creator-schema-")),
    );
    const databasePath = path.join(tempDir, "afora-agent.sqlite");
    const db = openNodeSqliteDatabase(databasePath);
    try {
      ...
      expect(() =>
        ensureAforaAgentDatabaseSchema(db, { agentId: "main", path: databasePath }),
      ).not.toThrow();
      expect(
        db
  ...
  it("migrates unreleased inline chunk metadata into rollback-safe storage", async () => {
    tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-chunk-metadata-schema-")),
    );
    const databasePath = path.join(tempDir, "afora-agent.sqlite");
    const db = openNodeSqliteDatabase(databasePath);
    try {
