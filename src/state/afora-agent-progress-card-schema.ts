...
import { AFORA_AGENT_SCHEMA_WITHOUT_BOARD_SQL } from "./afora-agent-board-schema.js";
...
function splitProgressCardSchema(sql: string): {
  ...
  if (start === -1 || end === -1) {
    throw new Error("Afora agent progress-card schema markers are missing.");
  }
  return {
...
const progressCardSchema = splitProgressCardSchema(AFORA_AGENT_SCHEMA_WITHOUT_BOARD_SQL);
...
export function ensureAforaAgentProgressCardSchemaInTransaction(db: DatabaseSync): void {
  if (!db.isTransaction) {
