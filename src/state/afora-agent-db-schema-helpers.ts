...
import {
  ...
  ensureAforaAgentBoardSchemaInTransaction,
} from "./afora-agent-board-schema.js";
import { CONTEXT_ENGINE_TURN_OUTBOX_TABLE } from "./afora-agent-context-engine-turn-outbox-schema.js";
import { FIRST_USE_ADDITIVE_AGENT_COLUMN_DEFINITIONS } from "./afora-agent-db-additive-columns.js";
import { AFORA_AGENT_SCHEMA_VERSION } from "./afora-agent-db-contract.js";
import { AforaAgentDatabaseMediaMigrationRequiredError } from "./afora-agent-db-migration-required.js";
import { ensureSessionEntryValidityProjection } from "./afora-agent-db-session-migrations.js";
import { MESSAGE_TOOL_RUN_OUTCOMES_TABLE } from "./afora-agent-message-tool-outcome-schema.js";
import {
  ensureAforaAgentProgressCardSchemaInTransaction,
  AGENT_PROGRESS_CARD_SCHEMA_SQL,
  SESSION_PROGRESS_CARDS_TABLE,
} from "./afora-agent-progress-card-schema.js";
import { AFORA_AGENT_SCHEMA_SQL } from "./afora-agent-schema.js";
import { SESSION_PARTICIPANTS_TABLE } from "./afora-agent-session-participants-schema.js";
import {
  AGENT_V14_ADDITIVE_SCHEMA_SQL,
  AGENT_V14_CORE_SCHEMA_SQL,
  AGENT_V14_SESSION_SHARING_SCHEMA_SQL,
} from "./afora-agent-session-sharing-schema.js";
import { SESSION_TRANSCRIPT_ARCHIVES_TABLE } from "./afora-agent-session-transcript-archive-schema.js";
import {
  STANDING_INTENTS_FTS_SHADOW_TABLES,
  STANDING_INTENTS_FTS_TABLE,
  STANDING_INTENTS_TABLE,
} from "./afora-agent-standing-intents-schema.js";
...
export function assertAforaAgentSchemaContains(
...
export function assertAforaAgentCurrentRuntimeSchema(
  database: DatabaseSync,
  ...
  if (metadata.schemaVersion !== AFORA_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `Afora agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match ${AFORA_AGENT_SCHEMA_VERSION}; run afora doctor --fix before using it.`,
    );
  }
  if (hasRetiredAgentStateLeaseSchema(database)) {
    throw new Error(
      `Afora agent database ${options.pathname} retains retired state_leases storage; run afora doctor --fix before using it.`,
    );
  }
...
function repairAndAssertAgentSchemaGroup(
  ...
  assertAforaAgentSchemaContains(database, pathname, schemaSql);
}
...
export function ensureSessionKeyContractSchemaInTransaction(db: DatabaseSync): void {
  const start = AFORA_AGENT_SCHEMA_SQL.indexOf(SESSION_KEY_CONTRACT_SCHEMA_START);
  const end = AFORA_AGENT_SCHEMA_SQL.indexOf(SESSION_KEY_CONTRACT_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Afora agent session-key contract schema markers are missing.");
  }
  db.exec(AFORA_AGENT_SCHEMA_SQL.slice(start, end)); // sqlite-allow-raw -- Idempotent additive lazy ensure.
}

export function repairAndAssertAforaAgentV14SchemaForMigration(
  database: DatabaseSync,
  options: { agentId: string; pathname: string },
): void {
  ...
  if (userVersion !== 14) {
    throw new Error(
      `Afora agent database ${options.pathname} uses schema version ${userVersion}; expected 14 before migrating it.`,
    );
  }
  const agentId = normalizeAgentId(options.agentId);
  const metadata = readExistingAgentSchemaMeta(database);
  ...
  if (metadata.schemaVersion !== 14) {
    throw new Error(
      `Afora agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match 14; repair the ownership metadata before migrating it.`,
    );
  }
  ...
  if (hasAnyCanonicalTable(database, AGENT_V14_BOARD_SCHEMA_SQL)) {
    ...
    ensureAforaAgentBoardSchemaInTransaction(database);
  ...
  if (hasAnyCanonicalTable(database, AGENT_PROGRESS_CARD_SCHEMA_SQL)) {
    ...
    ensureAforaAgentProgressCardSchemaInTransaction(database);
...
export function assertCanonicalAgentMediaPersistenceVersion(
  ...
  if (userVersion < AFORA_AGENT_SCHEMA_VERSION && !isNewUnownedDatabase) {
    throw new AforaAgentDatabaseMediaMigrationRequiredError(pathname, userVersion);
  }
}
...
export function assertExistingAgentSchemaOwner(
  ...
  if (existing.role !== "agent") {
    throw new Error(
      `Afora agent database ${pathname} has schema role ${existing.role ?? "unknown"}; expected agent.`,
    );
  }
  if (!existing.agentId) {
    throw new Error(`Afora agent database ${pathname} has no agent owner.`);
  }
  if (normalizeAgentId(existing.agentId) !== agentId) {
    throw new Error(
      `Afora agent database ${pathname} belongs to agent ${existing.agentId}; requested agent ${agentId}.`,
    );
  }
}
