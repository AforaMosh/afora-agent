...
const AFORA_QUARANTINE_SCHEMA_VERSION = 2;
const AFORA_QUARANTINE_BUSY_TIMEOUT_MS = 5_000;
const AFORA_QUARANTINE_DIR_MODE = 0o700;
const AFORA_QUARANTINE_FILE_MODE = 0o600;

type AforaDatabaseKind = "agent" | "state";

type AforaDatabaseQuarantine = {
  kind: AforaDatabaseKind;
  quarantinedAt: number;
  reason: string;
};

function resolveQuarantineStorePath(env: NodeJS.ProcessEnv): string {
  return path.join(resolveAforaStateSqliteDir(env), "afora-quarantine.sqlite");
}

function ensureQuarantineStoreDirectory(storePath: string): void {
  const dir = path.dirname(storePath);
  mkdirSync(dir, { recursive: true, mode: AFORA_QUARANTINE_DIR_MODE });
  applyPrivateModeSync(dir, AFORA_QUARANTINE_DIR_MODE);
}

function configureQuarantineWriter(database: DatabaseSync, storePath: string): void {
  database.exec(`
    PRAGMA busy_timeout = ${AFORA_QUARANTINE_BUSY_TIMEOUT_MS};
    PRAGMA journal_mode = DELETE;
    PRAGMA synchronous = FULL;
  `);
  const userVersion = readQuarantineSchemaVersion(database, storePath);
  if (userVersion > AFORA_QUARANTINE_SCHEMA_VERSION) {
    throw new Error(
      `Afora quarantine store ${storePath} uses newer schema version ${userVersion}.`,
    );
  }
  if (userVersion === AFORA_QUARANTINE_SCHEMA_VERSION) {
    return;
  ...
  if (userVersion === 1) {
    database.exec(`
      ...
      PRAGMA user_version = ${AFORA_QUARANTINE_SCHEMA_VERSION};
      COMMIT;
    `);
    return;
  }
...
function readQuarantineSchemaVersion(database: DatabaseSync, storePath: string): number {
  ...
  if (typeof userVersion !== "number" || !Number.isInteger(userVersion)) {
    throw new Error(`Afora quarantine store ${storePath} has an invalid schema version.`);
  }
  return userVersion;
}
...
function withQuarantineWriter<T>(env: NodeJS.ProcessEnv, operation: (db: DatabaseSync) => T): T {
  ...
  try {
    if (!existed) {
      applyPrivateModeSync(storePath, AFORA_QUARANTINE_FILE_MODE);
    }
...
export function readAforaDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): AforaDatabaseQuarantine | undefined {
  ...
  const database = openNodeSqliteDatabase(storePath);
  try {
    database.exec(`PRAGMA busy_timeout = ${AFORA_QUARANTINE_BUSY_TIMEOUT_MS};`);
    const userVersion = readQuarantineSchemaVersion(database, storePath);
    if (userVersion === 0) {
      return undefined;
    }
    ...
    if (
      ...
      throw new Error(`Afora quarantine store ${storePath} contains an invalid row.`);
    }
...
export function recordAforaDatabaseQuarantine(options: {
...
export function clearAforaDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): boolean {
