// Afora state database tests cover state DB migrations and persistence.
...
import { listAforaRegisteredAgentDatabases } from "./afora-agent-db-registry.js";
import { FIRST_USE_STATE_TABLES } from "./afora-state-db-contract.js";
import {
...
import {
  ...
  withAforaStateStartupMigrationCheckpointDatabase,
...
function createTempStateDir(): string {
  return makeTempDir(stateDbTempDirs, "afora-state-db-");
}

function createInitialStateSchemaShape() {
  const shape = createSqliteSchemaShapeFromSql(
    new URL("./afora-state-schema.sql", import.meta.url),
  );
...
function createOlderV6StateSchemaWithoutWorkerSshFallbackPorts(): string {
  ...
  const start = AFORA_STATE_SCHEMA_SQL.indexOf(startMarker);
  const endMarker = "\n) STRICT;";
  const end = start >= 0 ? AFORA_STATE_SCHEMA_SQL.indexOf(endMarker, start) : -1;
  if (start < 0 || end < 0) {
    throw new Error("worker SSH fallback port schema block is missing");
  }
  return `${AFORA_STATE_SCHEMA_SQL.slice(0, start)}${AFORA_STATE_SCHEMA_SQL.slice(
...
function expectStateSchemaMigrationRequired(
  run: () => unknown,
  expected: {
    kind: AforaStateDatabaseSchemaMigrationRequiredError["kind"];
    pathname: string;
  },
): void {
  let caught: unknown;
  ...
  expect(caught).toBeInstanceOf(AforaStateDatabaseSchemaMigrationRequiredError);
  expect(findAforaStateDatabaseSchemaMigrationRequiredError(caught)).toMatchObject(expected);
}
...
function materializeCurrentStateDatabase(stateDir: string): string {
  ...
  const databasePath = resolveAforaStateSqlitePath({ AFORA_STATE_DIR: stateDir });
  fs.mkdirSync(path.dirname(databasePath), { recursive: true });
...
function runHotRollbackJournalRecoveryProbe(params: { moduleUrl: string; rootDir: string }): {
  ...
  const probeSource = `
    ...
    const initial = openAforaStateDatabase({ path: databasePath });
    initial.db.exec(\`
    ...
    const writerSource = \`
      ...
      const database = new DatabaseSync(process.env.AFORA_HOT_JOURNAL_DATABASE_PATH);
      database.exec(
      ...
      fs.writeFileSync(process.env.AFORA_HOT_JOURNAL_READY_PATH, "ready");
      setInterval(() => {}, 1_000);
    \`;
    const writer = spawn(
      process.execPath,
      ["--input-type=module", "-e", writerSource],
      {
        env: {
          ...process.env,
          AFORA_HOT_JOURNAL_DATABASE_PATH: databasePath,
          AFORA_HOT_JOURNAL_READY_PATH: readyPath,
        },
    ...
    try {
      ...
      let readOnly;
      try {
        const database = await openExistingAforaStateDatabaseReadOnly({ path: databasePath });
      ...
      const reopened = openAforaStateDatabase({ path: databasePath });
...
function expectNoncanonicalAuditSchemaRejected(
  ...
  expect(detectAforaStateDatabaseSchemaMigrations(options)).toEqual([
    { kind: "audit-events-v2", path: databasePath },
  ]);
  expect(() => openAforaStateDatabase(options)).toThrow(/noncanonical audit event schema/);
  expect(repairAforaStateDatabaseSchema(options)).toEqual({
    changes: [],
...
function runConcurrentSchemaProbe(params: {
  ...
  const workerSource = `
    ...
    const {
      closeAforaStateDatabaseForTest,
      openAforaStateDatabase,
    } = await import(process.env.AFORA_SCHEMA_TEST_MODULE_URL);
    const databasePath = process.env.AFORA_SCHEMA_TEST_DATABASE_PATH;
    const enteringPath = process.env.AFORA_SCHEMA_TEST_ENTERING_PATH;
    const readyPath = process.env.AFORA_SCHEMA_TEST_READY_PATH;
    const startPath = process.env.AFORA_SCHEMA_TEST_START_PATH;
    const workerIndex = process.env.AFORA_SCHEMA_TEST_WORKER_INDEX;
    ...
    try {
      const database = openAforaStateDatabase({ path: databasePath });
  ...
  const orchestratorSource = `
    ...
    const coordinatorContracts =
      mode === "fresh"
        ? await Promise.all([
            ...
            import(new URL("./afora-state-db-contract.ts", moduleUrl).href),
          ])
        : undefined;
    ...
    function openFreshInitializationCoordinator(databasePath) {
      ...
      try {
        coordinator.exec(
          \`PRAGMA busy_timeout = \${AFORA_SQLITE_BUSY_TIMEOUT_MS}; BEGIN EXCLUSIVE;\`,
        );
      } catch (error) {
        coordinator.close();
        throw error;
    ...
    for (let round = 0; round < roundCount; round += 1) {
      ...
      if (mode === "upgrade") {
        ...
        openAforaStateDatabase({ path: databasePath });
        closeAforaStateDatabaseForTest();
        ...
        legacy.exec(\`
          ...
          ALTER TABLE worker_environments DROP COLUMN bootstrap_afora_version;
      ...
      const workers = Array.from({ length: workerCount }, (_, index) => {
        return spawn(
          ...
          ["--import", "tsx", "--input-type=module", "-e", workerSource],
          {
            env: {
              ...process.env,
              AFORA_SCHEMA_TEST_DATABASE_PATH: databasePath,
              AFORA_SCHEMA_TEST_ENTERING_PATH: enteringPaths[index],
              AFORA_SCHEMA_TEST_MODULE_URL: moduleUrl,
              AFORA_SCHEMA_TEST_READY_PATH: readyPaths[index],
              AFORA_SCHEMA_TEST_START_PATH: startPath,
              AFORA_SCHEMA_TEST_WORKER_INDEX: String(index),
            },
...
beforeAll(() => {
  ...
  canonicalStateDatabaseTemplatePath = openAforaStateDatabase({
...
describe("afora state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveAforaStateSqlitePath({ AFORA_STATE_DIR: stateDir })).toBe(
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "afora-test-state", `${process.pid}-7`, "state", "afora.sqlite"),
    );
  });
  ...
  it("creates the shared state schema from the committed SQL shape", () => {
    ...
    expect(database.path).toBe(path.join(stateDir, "state", "afora.sqlite"));
    expect(
      database.db
        .prepare(
  ...
  it.each(["runtime open", "doctor repair"] as const)(
    ...
    (migrationPath) => {
      ...
      expect(detectAforaStateDatabaseSchemaMigrations(options)).toContainEqual({
      ...
      const migrated = openAforaStateDatabase(options);
      ...
      expect(detectAforaStateDatabaseSchemaMigrations(options)).not.toContainEqual({
        kind: "worker-placement-execution-mode-v8",
        path: databasePath,
      });
    },
  ...
  it("migrates v8 agent database registrations to state-relative paths", () => {
    ...
    const databasePath = materializeCurrentStateDatabase(stateDir);
    const inRootPath = path.join(stateDir, "agents", "main", "agent", "afora-agent.sqlite");
    const dualInRootPath = path.join(stateDir, "agents", "dual", "agent", "afora-agent.sqlite");
    ...
    expect(detectAforaStateDatabaseSchemaMigrations({ env })).toContainEqual({
      kind: "agent-databases-relative-paths-v9",
      path: databasePath,
    });
    expect(repairAforaStateDatabaseSchema({ env })).toEqual({
      changes: [
    ...
    const migrated = openAforaStateDatabase({ env });
    ...
    expect(
      ...
      {
        agent_id: "copied",
        path: path.join("agents", "copied", "agent", "afora-agent.sqlite"),
      },
      {
        agent_id: "dual",
        path: path.join("agents", "dual", "agent", "afora-agent.sqlite"),
      },
      { agent_id: "external", path: externalPath },
      {
        agent_id: "main",
        path: path.join("agents", "main", "agent", "afora-agent.sqlite"),
      },
        ...
        try {
          ...
          expect(
            repaired
              ...
              .get(),
          ).toEqual({ schema_version: AFORA_STATE_SCHEMA_VERSION });
        } finally {
          repaired.close();
        }
      }
  ...
  it.each([
    ...
    const migrated = openAforaStateDatabase(options);
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(AFORA_STATE_SCHEMA_VERSION);
    expect(
  ...
  it.each(["runtime open", "doctor repair"] as const)(
    ...
    (migrationPath) => {
      ...
      if (migrationPath === "doctor repair") {
        const result = repairAforaStateDatabaseSchema(options);
  ...
  it.each(["runtime open", "doctor repair"] as const)(
    ...
    (migrationPath) => {
      ...
      if (migrationPath === "doctor repair") {
        ...
        expect(() => openAforaStateDatabase(options)).toThrow(/commitments/u);
      }
  ...
  it.each(["runtime open", "doctor repair"] as const)(
    ...
    (migrationPath) => {
      ...
      if (migrationPath === "doctor repair") {
        ...
        expect(() => openAforaStateDatabase(options)).toThrow(
  ...
  it("skips exclusive repair when the automatic schema gate is already current", () => {
    ...
    expect(repairAforaStateDatabaseSchemaIfNeeded(options)).toEqual({
      changes: [],
      warnings: [],
    });
...
`);
    ...
    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(AFORA_STATE_SCHEMA_VERSION);
    expect(collectSqliteSchemaShape(database.db)).toEqual(createInitialStateSchemaShape());
  });
...
`);
  ...
  it("doctor migrates version 3 ambient watch sentinels into cursor provenance", () => {
    ...
    closeAforaStateDatabaseForTest();
    expect(repairAforaStateDatabaseSchema(options)).toEqual({ changes: [], warnings: [] });
  });
  ...
  it.each([
    ...
    ({ columnName, tableName }) => {
      ...
      expect(() =>
        assertAforaStateDatabaseForMaintenance(reopened.db, { pathname: reopened.path }),
      ).not.toThrow();
      expect(reopened.db.prepare("PRAGMA integrity_check").get()).toEqual({
        integrity_check: "ok",
      });
  ...
  it("rejects a missing current-schema table instead of recreating it empty", () => {
    ...
    expect(() => openAforaStateDatabase(options)).toThrow(/missing table auth_profile_stores/iu);
    expect(repairAforaStateDatabaseSchema(options)).toEqual({
      changes: [],
  ...
  it("classifies the released agent registry primary key as Doctor-repairable", () => {
    ...
    expectStateSchemaMigrationRequired(() => openAforaStateDatabase(options), {
  ...
  it("keeps an unrecognized agent registry schema fail-closed and nonrepairable", () => {
    ...
    expect(findAforaStateDatabaseSchemaMigrationRequiredError(caught)).toBeUndefined();
  ...
  it("migrates the released audit ledger to message-compatible attribution exactly once", () => {
    ...
    try {
      ...
      expect(readSqliteNumberPragma(db, "user_version")).toBe(AFORA_STATE_SCHEMA_VERSION);
      expect(
  ...
  it("preserves an empty audit ledger's sequence high-water mark", () => {
    ...
    expect(repairAforaStateDatabaseSchema(options).warnings).toEqual([]);
  ...
  it("lets normal open create an audit ledger for a pre-v2 database", () => {
    ...
    expect(readSqliteNumberPragma(opened.db, "user_version")).toBe(AFORA_STATE_SCHEMA_VERSION);
  });
  ...
  it("creates the bounded skill curator tables", () => {
    const stateDir = createTempStateDir();
    const database = openAforaStateDatabase({ env: { AFORA_STATE_DIR: stateDir } });
  ...
  it.runIf(process.platform === "linux")("closes the database when initialization fails", () => {
    const databasePath = path.join(createTempStateDir(), "afora.sqlite");
    fs.writeFileSync(databasePath, "not a sqlite database");

    expect(() => openAforaStateDatabase({ path: databasePath })).toThrow(
  ...
  it("accepts a missing same-version approval index read-only and repairs it on writable open", async () => {
    ...
    const beforeRepair = await openExistingAforaStateDatabaseReadOnly(options);
    expect(beforeRepair?.db.prepare("PRAGMA user_version").get()).toEqual({
      user_version: AFORA_STATE_SCHEMA_VERSION,
    });
    beforeRepair?.walMaintenance.close();

    const writable = openAforaStateDatabase(options);
    expect(
      writable.db
    ...
    const afterRepair = await openExistingAforaStateDatabaseReadOnly(options);
  ...
  it("reads committed live WAL rows without changing source database content", async () => {
    const stateDir = createTempStateDir();
    const writer = openAforaStateDatabase({ env: { AFORA_STATE_DIR: stateDir } });
    ...
    const database = await openExistingAforaStateDatabaseReadOnly({ path: writer.path });
    expect(
  ...
  it("rejects noncanonical indexes from read-only state without rewriting them", async () => {
    ...
    await expect(openExistingAforaStateDatabaseReadOnly(options)).rejects.toThrow(
  ...
  it("rejects unrelated current-schema index corruption before exposure", () => {
    ...
    expect(() =>
      withAforaStateStartupMigrationCheckpointDatabase(checkpointCallback, options),
  ...
  it("configures checkpoint lock waits before schema mutation", () => {
    ...
    withAforaStateStartupMigrationCheckpointDatabase(
      (db) => {
        expect(readSqliteNumberPragma(db, "busy_timeout")).toBe(AFORA_SQLITE_BUSY_TIMEOUT_MS);
      },
      { env: { AFORA_STATE_DIR: stateDir } },
    );
  });
  ...
  it("runs full integrity before a pending state schema migration", () => {
    ...
    const before = new DatabaseSync(databasePath);
    try {
      before.exec(`PRAGMA user_version = ${AFORA_STATE_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
  ...
  it("rejects current-schema foreign-key violations before exposure", () => {
    ...
    expect(() => openAforaStateDatabase(options)).toThrow(failure);
    expect(repairAforaStateDatabaseSchema(options)).toEqual({
      changes: [],
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const result = runHotRollbackJournalRecoveryProbe({
        moduleUrl: new URL("./afora-state-db.ts", import.meta.url).href,
  ...
  it("adds gateway boot lifecycle startup markers to existing state databases", () => {
    ...
    const reopened = openAforaStateDatabase({
      env: { AFORA_STATE_DIR: stateDir },
    });
    const columns = reopened.db
  ...
  it("adds optional Claw application provenance columns to existing state databases", () => {
    ...
    const reopened = openAforaStateDatabase({ env: { AFORA_STATE_DIR: stateDir } });
  ...
  it("adds staged worker-result refs during the v5 state migration", () => {
    ...
    expect(readSqliteNumberPragma(reopened.db, "user_version")).toBe(AFORA_STATE_SCHEMA_VERSION);
    expect(
      reopened.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
  ...
  it("migrates operator approvals to accept system-agent records", () => {
    ...
    expect(repairAforaStateDatabaseSchema(options)).toEqual({
      changes: [
        "Migrated shared state operator approvals → Afora system changes",
  ...
  it("does not recursively recommend doctor when operator approval repair refuses a shape", () => {
    ...
    expect(result.warnings[0]).not.toContain("run afora doctor --fix");
  });
  ...
  it("serializes concurrent additive schema upgrades across processes", () => {
    ...
    const moduleUrl = new URL("./afora-state-db.ts", import.meta.url).href;
  ...
  it("migrates requester and executor attribution for existing cross-agent tasks", () => {
    ...
    const currentReopened = openAforaStateDatabase({
      env: { AFORA_STATE_DIR: stateDir },
    });
    expect(
      currentReopened.db
  ...
  it("normalizes obsolete task delivery statuses in existing state databases", async () => {
    await withAforaTestState(
      { layout: "state-only", prefix: "afora-state-task-delivery-status-" },
      async ({ stateDir }) => {
        const databasePath = materializeCurrentStateDatabase(stateDir);
        const { DatabaseSync } = requireNodeSqlite();
        const database = new DatabaseSync(databasePath);
        ...
        const readStatuses = () =>
          openAforaStateDatabase({ env: { AFORA_STATE_DIR: stateDir } })
  ...
  it("configures the busy timeout before a doctor schema repair transaction", () => {
    ...
    const timeoutIndex = statements.findIndex((sql) =>
      sql.includes(`PRAGMA busy_timeout = ${AFORA_SQLITE_BUSY_TIMEOUT_MS}`),
    );
  ...
  it("latches newer global schema failures before integrity scans", () => {
    const stateDir = createTempStateDir();
    const options = { env: { AFORA_STATE_DIR: stateDir } };
    const databasePath = openAforaStateDatabase(options).path;
    closeAforaStateDatabaseForTest();
    createUnsafeIndexDrift(databasePath);
    const { DatabaseSync } = requireNodeSqlite();
    const db = new DatabaseSync(databasePath);
    ...
    expect(openAforaStateDatabase(options).db.isOpen).toBe(true);
  });

  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `afora-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openAforaStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openAforaStateDatabase({ path: firstPath });
    const second = openAforaStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openAforaStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(AFORA_STATE_SCHEMA_VERSION);
  });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-state-db-relative-"));
          ...
          try {
            ...
            const first = openAforaStateDatabase({ path: "state.sqlite" });
            first.db
            ...
            const second = openAforaStateDatabase({ path: "state.sqlite" });
            second.db
              .prepare("INSERT INTO diagnostic_events (scope, event_key, payload_json, created_at) VALUES (?, ?, ?, ?)")
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    ...
    runAforaStateWriteTransaction((database) => {
      ...
      expect(() =>
        runAforaStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runAforaStateWriteTransaction(async () => {
        return "not sync";
      }, options),
