...
function contextEngineTurnOutboxSchemaSql(): string {
  const start = AFORA_AGENT_SCHEMA_SQL.indexOf(OUTBOX_SCHEMA_START);
  const end = AFORA_AGENT_SCHEMA_SQL.indexOf(OUTBOX_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Afora context-engine turn outbox schema markers are missing.");
  }
