...
function createWalFixture(): { header: Buffer; withFrames: Buffer } {
  const fixtureDir = fs.realpathSync(tempStateDirs.make("afora-wal-header-"));
...
function prepareCase(kind: (typeof databaseKinds)[number]) {
  const stateDir = fs.realpathSync(tempStateDirs.make("afora-orphan-sidecar-"));
  const env = { AFORA_STATE_DIR: stateDir };
  const databasePath =
    kind === "state"
      ? resolveAforaStateSqlitePath(env)
