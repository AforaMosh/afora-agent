...
function sessionParticipantsSchemaSql(): string {
  const start = AFORA_AGENT_SCHEMA_SQL.indexOf(SCHEMA_START);
  const end = AFORA_AGENT_SCHEMA_SQL.indexOf(SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Afora session participant schema markers are missing.");
  }
