...
import { invalidateRegisteredAgentDatabasesMemo } from "./afora-agent-db-registry-listing.js";
...
import { resolveAforaAgentDatabaseStoredPath } from "./afora-state-db.paths.js";

export {
  listAforaRegisteredAgentDatabases,
  readAforaAgentDatabaseRegistryToken,
} from "./afora-agent-db-registry-listing.js";
...
export function createAforaAgentDatabasePathMatcher(): (left: string, right: string) => boolean {
...
export function isSameAforaAgentDatabasePath(left: string, right: string): boolean {
...
export function registerAforaAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
  schemaVersion?: number;
}): void {
  if (!isPersistentAforaAgentDatabasePath(params.path, params.env)) {
    return;
  }
  ...
  runAforaStateWriteTransaction(
    (database) => {
      ...
      const storedPath = resolveAforaAgentDatabaseStoredPath(database.path, params.path);
      const db = getNodeSqliteKysely<AforaAgentRegistryDatabase>(database.db);
      executeSqliteQuerySync(
        ...
        db
          ...
          .values({
            ...
            path: storedPath,
            schema_version: params.schemaVersion ?? AFORA_AGENT_SCHEMA_VERSION,
...
export function isPersistentAforaAgentDatabasePath(
  pathname: string,
...
export function unregisterAforaAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
}): void {
...
export function unregisterAforaAgentDatabases(params: {
  agentId: string;
  env?: NodeJS.ProcessEnv;
}): void {
  runAforaStateWriteTransaction(
