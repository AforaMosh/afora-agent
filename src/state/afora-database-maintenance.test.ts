...
describe("Afora database maintenance schema validation", () => {
  it("accepts the current global and agent schemas", () => {
    ...
    try {
      expect(() =>
        assertAforaStateDatabaseForMaintenance(globalDatabase, {
  ...
  it("accepts a global schema produced by an additive column migration", () => {
    const schemaWithoutMigratedColumn = AFORA_STATE_SCHEMA_SQL.replace(
    ...
    try {
      ...
      expect(() =>
        assertAforaStateDatabaseForMaintenance(database, {
          pathname: "global.sqlite",
        }),
      ).not.toThrow();
    } finally {
  ...
  it("keeps a newer nullable shared-state column compatible with the previous schema", () => {
    const previousSchema = AFORA_STATE_SCHEMA_SQL.replace(
      "  removed_at INTEGER,\n  run_end_cleanup_json TEXT\n",
      "  removed_at INTEGER\n",
    );
    const database = createGlobalDatabase();
    try {
      expect(previousSchema).not.toBe(AFORA_STATE_SCHEMA_SQL);
      expect(() =>
  ...
  it("keeps the cron authority companion table compatible with the previous schema", () => {
    ...
    const previousSchema = `${AFORA_STATE_SCHEMA_SQL.slice(
      0,
      start,
    )}${AFORA_STATE_SCHEMA_SQL.slice(end + endMarker.length)}`;
    const database = createGlobalDatabase();
    try {
      expect(() =>
        assertSqliteSchemaContains(database, "previous global schema", previousSchema),
  ...
  it("accepts the historical checked shared-host column but rejects other constraints", () => {
    const historicalSchema = AFORA_STATE_SCHEMA_SQL.replace(
      "  shared_host INTEGER\n) STRICT;",
      "  shared_host INTEGER CHECK (shared_host IN (0, 1))\n) STRICT;",
    );
    const database = createGlobalDatabase(historicalSchema);
    try {
      expect(historicalSchema).not.toBe(AFORA_STATE_SCHEMA_SQL);
      expect(() =>
        assertAforaStateDatabaseForMaintenance(database, {
          pathname: "global.sqlite",
        }),
  ...
  it("keeps every same-version additive column bare and canonical", () => {
    ...
    expect(AFORA_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY.allowedMissingColumns).toEqual(
      additiveColumns,
    );
    expect(
...
function createGlobalDatabase(schemaSql = AFORA_STATE_SCHEMA_SQL): DatabaseSync {
...
function createAgentDatabase(): DatabaseSync {
  const database = new DatabaseSync(":memory:");
  database.exec(AFORA_AGENT_SCHEMA_SQL);
  ...
  database
    ...
    .run(AFORA_AGENT_SCHEMA_VERSION);
  return database;
}
