// Afora agent database tests cover agent-scoped DB storage and migrations.
...
import {
  assertNoAforaAgentDatabaseLeases,
  claimAforaAgentDatabaseLease,
  releaseAforaAgentDatabaseLease,
} from "./afora-agent-db-lease.js";
import { withAforaAgentDatabaseReadOnly } from "./afora-agent-db-readonly.js";
import {
  createAforaAgentDatabasePathMatcher,
  isSameAforaAgentDatabasePath,
  registerAforaAgentDatabase,
  unregisterAforaAgentDatabase,
} from "./afora-agent-db-registry.js";
import type { DB as AforaAgentKyselyDatabase } from "./afora-agent-db.generated.js";
import {
  ...
  openAforaAgentDatabase as openAforaAgentDatabaseRuntime,
...
function createTempStateDir(): string {
  return makeTempDir(agentDbTempDirs, "afora-agent-db-");
}
...
function ensureSharedStateDatabaseTemplate(): string {
  ...
  const stateDir = makeTempDir(agentDbTempDirs, "afora-agent-db-shared-state-");
...
function openAforaAgentDatabase(
  options: Parameters<typeof openAforaAgentDatabaseRuntime>[0],
) {
  const stateDatabasePath = resolveAforaStateSqlitePath(options.env);
  ...
  return openAforaAgentDatabaseRuntime(options);
}
...
function ensureCurrentWorkerAgentDatabaseTemplate(): string {
  ...
  const stateDir = makeTempDir(agentDbTempDirs, "afora-agent-db-current-worker-");
  const env = { AFORA_STATE_DIR: stateDir };
  const template = openAforaAgentDatabase({ agentId: "worker-1", env });
  ...
  try {
    ...
    if (readSqliteNumberPragma(verified, "user_version") !== AFORA_AGENT_SCHEMA_VERSION) {
...
function ensureV13WorkerAgentDatabaseTemplate(): string {
  ...
  const databasePath = resolveAforaAgentSqlitePath({
    agentId: "worker-1",
    env: { AFORA_STATE_DIR: stateDir },
  });
  fs.mkdirSync(path.dirname(databasePath), { recursive: true });
...
function migrateAndOpenLegacyAgentDatabaseForTest(
  options: Parameters<typeof openAforaAgentDatabase>[0],
) {
  const pathname = resolveAforaAgentSqlitePath(options);
  const { DatabaseSync } = requireNodeSqlite();
  const database = new DatabaseSync(pathname);
  try {
    ensureAforaAgentDatabaseSchema(database, options);
  } finally {
    database.close();
  }
  return openAforaAgentDatabase(options);
}
...
function readRegisteredAgentDatabaseLastSeenAt(params: {
  ...
  const row = openAforaStateDatabase({ env: params.env })
...
function launchAgentSchemaOpener(params: {
  ...
  const agentModuleUrl = new URL("./afora-agent-db.ts", import.meta.url).href;
  const stateModuleUrl = new URL("./afora-state-db.ts", import.meta.url).href;
  const child = spawn(
    process.execPath,
    [
      "--import",
      ...
      `
        ...
        const db = new DatabaseSync(process.env.AFORA_AGENT_DB_RACE_PATH);
        ...
        process.stdin.once("data", () => {
          let result;
          try {
            ensureAforaAgentDatabaseSchema(observedDb, {
              agentId: process.env.AFORA_AGENT_DB_RACE_AGENT,
              env: { AFORA_STATE_DIR: process.env.AFORA_AGENT_DB_RACE_STATE_DIR },
              path: process.env.AFORA_AGENT_DB_RACE_PATH,
              register: true,
            });
            result = { agentId: process.env.AFORA_AGENT_DB_RACE_AGENT, ok: true };
          } catch (error) {
            result = {
              agentId: process.env.AFORA_AGENT_DB_RACE_AGENT,
              ok: false,
    ...
    ],
    {
      env: {
        ...process.env,
        AFORA_AGENT_DB_RACE_AGENT: params.agentId,
        AFORA_AGENT_DB_RACE_PATH: params.databasePath,
        AFORA_AGENT_DB_RACE_STATE_DIR: params.stateDir,
      },
...
describe("afora agent database", () => {
  it("uses the canonical state schema for deletion journal reads and updates", () => {
    ...
    stateDatabase.exec(
      fs.readFileSync(new URL("./afora-state-schema.sql", import.meta.url), "utf8"),
    );
    stateDatabase.close();
    ...
    const databasePaths = [path.join(entry.agentDir, "afora-agent.sqlite")];
    const cleanupPaths = [
      {
        path: entry.agentDir,
  ...
  it("fences registration and lease claims beneath another agent's pending deletion", () => {
    ...
    registerAforaAgentDatabase({ agentId: "deleted", path: registeredSidecarPath, env });
    ...
    unregisterAforaAgentDatabase({ agentId: "deleted", path: relocatedDatabasePath, env });
    unregisterAforaAgentDatabase({ agentId: "deleted", path: registeredSidecarPath, env });

    try {
      for (const fencedPath of [databasePath, relocatedDatabasePath, sidecarClaimPath]) {
        expect(() =>
          registerAforaAgentDatabase({ agentId: "survivor", path: fencedPath, env }),
        ).toThrow("deletion owns");
        expect(() =>
          claimAforaAgentDatabaseLease({ agentId: "survivor", path: fencedPath, env }),
        ).toThrow("deletion owns");
      }
    } finally {
      deletion.rollback();
    ...
    expect(() =>
      registerAforaAgentDatabase({ agentId: "survivor", path: databasePath, env }),
    ).not.toThrow();
    const leaseId = claimAforaAgentDatabaseLease({
      agentId: "survivor",
      path: databasePath,
      env,
    });
    releaseAforaAgentDatabaseLease(leaseId, { env });
    unregisterAforaAgentDatabase({ agentId: "survivor", path: databasePath, env });
  });
  ...
  it("blocks deletion on a foreign lease beneath its workspace until release", () => {
    ...
    expect(() => assertNoAforaAgentDatabaseLeases("deleted", { env })).toThrow("deletion owns");
    releaseAforaAgentDatabaseLease(leaseId, { env });
    expect(() => assertNoAforaAgentDatabaseLeases("deleted", { env })).not.toThrow();
    deletion.rollback();
  });
  ...
  it("resolves under the per-agent state directory", () => {
    ...
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "afora-agent.sqlite"));
  });
  ...
  it("returns typed not-found without creating a missing read-only database", () => {
    ...
    expect(withAforaAgentDatabaseReadOnly(() => "unused", options)).toEqual({
      found: false,
  ...
  it("refuses a newer schema from the read-only database helper", () => {
    ...
    expect(() => withAforaAgentDatabaseReadOnly(() => "unused", options)).toThrow(
      `newer schema version ${AFORA_AGENT_SCHEMA_VERSION + 1}`,
    );
  });
  ...
  it("lists a missing registry without creating the shared state database", () => {
    const stateDir = createTempStateDir();
    const env = { AFORA_STATE_DIR: stateDir };
    const stateDatabasePath = path.join(stateDir, "state", "afora.sqlite");
  ...
  it("rotates the registry token on pathname switches and committed writes", () => {
    const stateDir = createTempStateDir();
    const env = { AFORA_STATE_DIR: stateDir };
    const otherEnv = { AFORA_STATE_DIR: createTempStateDir() };
    ...
    const firstToken = readAforaAgentDatabaseRegistryToken({ env });
    expect(listAforaRegisteredAgentDatabases({ env })).toEqual([]);
    expect(readAforaAgentDatabaseRegistryToken({ env })).toBe(firstToken);
    expect(readAforaAgentDatabaseRegistryToken({ env: otherEnv })).not.toBe(firstToken);
    const returnedToken = readAforaAgentDatabaseRegistryToken({ env });
    expect(returnedToken).not.toBe(firstToken);

    registerAforaAgentDatabase({ agentId: "worker-1", path: databasePath, env });
    const registeredToken = readAforaAgentDatabaseRegistryToken({ env });
    expect(registeredToken).not.toBe(returnedToken);
    expect(listAforaRegisteredAgentDatabases({ env })).toEqual([
      expect.objectContaining({ agentId: "worker-1", path: databasePath }),
    ]);
    expect(readAforaAgentDatabaseRegistryToken({ env })).toBe(registeredToken);

    unregisterAforaAgentDatabase({ agentId: "worker-1", path: databasePath, env });
    expect(readAforaAgentDatabaseRegistryToken({ env })).not.toBe(registeredToken);
    expect(listAforaRegisteredAgentDatabases({ env })).toEqual([]);
  });
  ...
  it("stores state-owned registrations relative while preserving external paths", () => {
    ...
    const env = { AFORA_STATE_DIR: stateDir };
    const inRootPath = path.join(stateDir, "agents", "worker-1", "agent", "afora-agent.sqlite");
    const externalPath = path.join(externalStateDir, "external.sqlite");

    registerAforaAgentDatabase({ agentId: "worker-1", path: inRootPath, env });
    registerAforaAgentDatabase({ agentId: "external", path: externalPath, env });
    ...
    expect(
      ...
      {
        agent_id: "worker-1",
        path: path.join("agents", "worker-1", "agent", "afora-agent.sqlite"),
      },
    ]);
    expect(listAforaRegisteredAgentDatabases({ env })).toEqual([
    ...
    stateDatabase.db
      ...
      .run("worker-1", inRootPath, AFORA_AGENT_SCHEMA_VERSION);
    unregisterAforaAgentDatabase({ agentId: "worker-1", path: inRootPath, env });
    expect(
      stateDatabase.db
    ...
    unregisterAforaAgentDatabase({ agentId: "external", path: externalPath, env });
    expect(listAforaRegisteredAgentDatabases({ env })).toEqual([]);
  });
  ...
  it("keeps incompatible schema versions maintenance-only", () => {
    const stateDir = createTempStateDir();
    const env = { AFORA_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agents", "legacy", "agent", "afora-agent.sqlite");
    registerAforaAgentDatabase({
      agentId: "legacy",
      path: databasePath,
      env,
      schemaVersion: AFORA_AGENT_SCHEMA_VERSION - 1,
    });
  ...
  it("does not persist archived import databases in the registry", () => {
    const stateDir = createTempStateDir();
    const env = { AFORA_STATE_DIR: stateDir };
    const archivedPath = path.join(stateDir, "imports", "archive", "afora-agent.sqlite");

    registerAforaAgentDatabase({ agentId: "archive", path: archivedPath, env });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const env = { AFORA_STATE_DIR: stateDir };
      const activePath = path.join(stateDir, "active", "afora-agent.sqlite");
      const archivedPath = path.join(stateDir, "imports", "archive", "afora-agent.sqlite");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const missingPath = path.join(aliasDir, "afora-agent.sqlite");

      registerAforaAgentDatabase({ agentId: "archive", path: missingPath, env });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(() => listAforaRegisteredAgentDatabases({ env })).toThrow("is unavailable");
    },
  );
  ...
  it("creates the per-agent schema and registers it globally", () => {
    ...
    const database = openAforaAgentDatabaseRuntime({
      agentId: "worker-1",
      env: { AFORA_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./afora-agent-schema.sql", import.meta.url)),
    );
    expect(
      database.db
    ...
    expect(database.path).toBe(
      path.join(stateDir, "agents", "worker-1", "agent", "afora-agent.sqlite"),
    );

    const registered = listAforaRegisteredAgentDatabases({
      env: { AFORA_STATE_DIR: stateDir },
  ...
  it("opens a v13 database that already contains additive board storage", () => {
    ...
    expect(readSqliteNumberPragma(reopened.db, "user_version")).toBe(AFORA_AGENT_SCHEMA_VERSION);
    expect(
      reopened.db
  ...
  it("migrates v13 session entries, routes, and generations into nodes and windows", () => {
    ...
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(AFORA_AGENT_SCHEMA_VERSION);
  ...
  it("migrates version 1 memory source identities before registering version 2", () => {
    ...
    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(AFORA_AGENT_SCHEMA_VERSION);
    expect(
    ...
    expect(
      listAforaRegisteredAgentDatabases({ env }).find((entry) => entry.agentId === "worker-1"),
    ).toMatchObject({ path: databasePath, schemaVersion: AFORA_AGENT_SCHEMA_VERSION });
  ...
  it("retains a failed-open handle and lease when initialization cleanup also fails", () => {
    const stateDir = createTempStateDir();
    const env = { AFORA_STATE_DIR: stateDir };
    const database = openAforaAgentDatabase({ agentId: "worker-1", env });
    expect(closeAforaAgentDatabaseByPath(database.path)).toBe(true);
    const { DatabaseSync } = requireNodeSqlite();
    ...
    expect(() =>
      openAforaAgentDatabase({ agentId: "worker-2", env, path: database.path }),
    ).toThrow("initialization close failed");
    close.mockRestore();
    expect(() => assertNoAforaAgentDatabaseLeases("worker-2", { env })).toThrow(
    ...
    expect(disposeAforaAgentDatabaseByPath(database.path, { env })).toBe(true);
    expect(() => assertNoAforaAgentDatabaseLeases("worker-2", { env })).not.toThrow();
  });
  ...
  it("keeps multiple registered paths for the same agent", () => {
    ...
    const relocated = openAforaAgentDatabase({
      agentId: "worker-1",
      env,
      path: relocatedPath,
    });
    const defaultDatabase = openAforaAgentDatabase({
      agentId: "worker-1",
      env,
    });

    expect(
      listAforaRegisteredAgentDatabases({ env })
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      try {
        const matchesPath = createAforaAgentDatabasePathMatcher();
      ...
      expect(createAforaAgentDatabasePathMatcher()(realPath, aliasPath)).toBe(false);
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      expect(() => isSameAforaAgentDatabasePath(loopPath, loopPath)).toThrow(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      expect(isSameAforaAgentDatabasePath(realPath, aliasPath)).toBe(true);
      ...
      expect(isSameAforaAgentDatabasePath(futureRealPath, danglingAliasPath)).toBe(true);
      ...
      expect(isSameAforaAgentDatabasePath(relativeRealPath, relativeAliasPath)).toBe(true);
      ...
      expect(isSameAforaAgentDatabasePath(redirectedRealPath, redirectedAliasPath)).toBe(true);
      expect(
        isSameAforaAgentDatabasePath(
      ...
      expect(isSameAforaAgentDatabasePath(danglingTraversalAlias, livePath)).toBe(false);
      const rawMissingTraversalPath = `${stateDir}${path.sep}missing${path.sep}..${path.sep}live-worker.sqlite`;
      expect(isSameAforaAgentDatabasePath(rawMissingTraversalPath, livePath)).toBe(false);
      ...
      expect(() => isSameAforaAgentDatabasePath(loopPath, realPath)).toThrow(
        expect.objectContaining({ code: "ELOOP" }),
      );
      ...
      expect(() => isSameAforaAgentDatabasePath(expandingLoopPath, realPath)).toThrow(
        expect.objectContaining({ code: "ELOOP" }),
      );
      ...
      const database = openAforaAgentDatabase({ agentId: "worker", env, path: realPath });
      unregisterAforaAgentDatabase({ agentId: "worker", env, path: database.path });
      registerAforaAgentDatabase({ agentId: "worker", env, path: aliasPath });
    },
  );
  ...
  it("matches volume semantics for non-ASCII case aliases", () => {
    ...
    expect(isSameAforaAgentDatabasePath(upperPath, lowerPath)).toBe(aliases);
  });
  ...
  it("matches volume semantics for Unicode simple case-fold aliases", () => {
    ...
    expect(isSameAforaAgentDatabasePath(sigmaPath, finalSigmaPath)).toBe(aliases);
  });
  ...
  it("keeps case and normalization semantics distinct for missing aliases", () => {
    ...
    expect(isSameAforaAgentDatabasePath(composedUpperPath, decomposedLowerPath)).toBe(aliases);
  });
  ...
  it("requires raw normalization after an ASCII-only case difference", () => {
    ...
    expect(isSameAforaAgentDatabasePath(composedUpperPath, decomposedMixedPath)).toBe(aliases);
  });
  ...
  it.runIf(tempVolumeIsCaseInsensitive)(
    ...
    () => {
      ...
      expect(
        isSameAforaAgentDatabasePath(path.join(stateDir, upper), path.join(stateDir, lower)),
      ).toBe(true);
    },
  );
  ...
  it("uses the candidate code points for missing-path normalization semantics", () => {
    ...
    expect(isSameAforaAgentDatabasePath(leftPath, rightPath)).toBe(aliases);
  });
  ...
  it("matches volume semantics for normalization-only missing directory components", () => {
    ...
    expect(
      isSameAforaAgentDatabasePath(
        path.join(leftDir, "afora-agent.sqlite"),
        path.join(rightDir, "afora-agent.sqlite"),
      ),
    ).toBe(aliases);
  });
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(isSameAforaAgentDatabasePath(driveRelativePath, absolutePath)).toBe(true);
    },
  );
  ...
  it.runIf(process.platform === "linux")(
    ...
    () => {
      ...
      expect(
        isSameAforaAgentDatabasePath(path.join(hardLinkedNested, "d", "link"), firstHardLink),
      ).toBe(true);
    },
  );
  ...
  it("does not refresh global registry metadata on cached opens", () => {
    ...
    try {
      const first = openAforaAgentDatabase({
        agentId: "worker-1",
        env,
      });
      expect(
      ...
      const second = openAforaAgentDatabase({
        agentId: "worker-1",
        env,
      });
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    ...
    legacyDb.exec(`
      ...
      INSERT INTO agent_databases (
        ...
        'worker-1',
        '/legacy/worker-1/afora-agent.sqlite',
        1,
        10,
        20
      );
    ...
    expect(() => listAforaRegisteredAgentDatabases({ env })).toThrow(
      /run afora doctor --fix/,
    );
    ...
    ).toThrow(/run afora doctor --fix/);

    fs.rmSync(stateDatabasePath);
    const reopened = openAforaAgentDatabase({
      agentId: "worker-1",
      env,
    });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-agent-db-state-"));
          const env = { AFORA_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-agent-db-relative-"));
          ...
          try {
            ...
            console.log(JSON.stringify({
              ...
              registeredPaths: listAforaRegisteredAgentDatabases({ env })
                .filter((entry) => entry.agentId === "worker-1")
                .map((entry) => entry.path),
  ...
  it("closes only the cached database at the exact resolved path", () => {
    ...
    const first = openAforaAgentDatabase({ agentId: "worker-1", env, path: firstPath });
    const second = openAforaAgentDatabase({ agentId: "worker-2", env, path: secondPath });

    expect(closeAforaAgentDatabaseByPath(path.join(stateDir, "missing.sqlite"))).toBe(false);
    ...
    expect(second.db.isOpen).toBe(true);
    expect(closeAforaAgentDatabaseByPath(firstPath)).toBe(false);

    const reopened = openAforaAgentDatabase({ agentId: "worker-1", env, path: firstPath });
  ...
  it("retains its durable lease when closing the handle fails", () => {
    ...
    expect(() => closeAforaAgentDatabaseByPath(database.path)).toThrow("wal close failed");
    expect(database.db.isOpen).toBe(true);
    expect(() => assertNoAforaAgentDatabaseLeases("worker-1", { env })).toThrow(
      "database is still open",
    );

    close.mockRestore();
    expect(closeAforaAgentDatabaseByPath(database.path)).toBe(true);
    expect(() => assertNoAforaAgentDatabaseLeases("worker-1", { env })).not.toThrow();
  });

  it("disposes only its exact cached owner and unregisters that registry row", () => {
    const stateDir = createTempStateDir();
    const env = { AFORA_STATE_DIR: stateDir };
    const first = openAforaAgentDatabase({ agentId: "worker-1", env });
    const second = openAforaAgentDatabase({ agentId: "worker-2", env });

    expect(disposeAforaAgentDatabaseByPath(first.path, { env })).toBe(true);
    expect(first.db.isOpen).toBe(false);
    expect(second.db.isOpen).toBe(true);
    expect(listAforaRegisteredAgentDatabases({ env })).toEqual([
      expect.objectContaining({ agentId: "worker-2", path: second.path }),
    ]);
    expect(disposeAforaAgentDatabaseByPath(first.path, { env })).toBe(false);
    expect(second.db.isOpen).toBe(true);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      fs.mkdirSync(realDir, { recursive: true });
      fs.symlinkSync(realDir, aliasDir, "dir");
      const realPath = path.join(realDir, "afora-agent.sqlite");
      const aliasPath = path.join(aliasDir, "afora-agent.sqlite");
      const database = openAforaAgentDatabase({ agentId: "probe", env, path: realPath });

      expect(disposeAforaAgentDatabaseByPath(aliasPath, { env })).toBe(true);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      fs.mkdirSync(realDir, { recursive: true });
      fs.symlinkSync(realDir, aliasDir, "dir");
      const real = openAforaAgentDatabase({
        agentId: "probe",
        env,
        path: path.join(realDir, "afora-agent.sqlite"),
      });
      const alias = openAforaAgentDatabase({
        agentId: "probe",
        env,
        path: path.join(aliasDir, "afora-agent.sqlite"),
      });

      expect(disposeAforaAgentDatabaseByPath(real.path, { env })).toBe(false);
  ...
  it("reopens a recreated agent id on a fresh database after archival", () => {
    const stateDir = createTempStateDir();
    const env = { AFORA_STATE_DIR: stateDir };
    const original = openAforaAgentDatabase({ agentId: "worker-1", env });
    ...
    try {
      expect(disposeAforaAgentDatabaseByPath(original.path, { env })).toBe(true);
      deletion.commit();
      ...
      const recreated = openAforaAgentDatabase({ agentId: "worker-1", env });
      expect(recreated.db.isOpen).toBe(true);
      expect(fs.statSync(recreated.path).ino).not.toBe(originalInode);
      expect(fs.existsSync(path.join(archivedDir, "afora-agent.sqlite"))).toBe(true);
      expect(listAforaRegisteredAgentDatabases({ env })).toEqual([
  ...
  it("keeps the deleted id fenced until its completed tombstone is claimed", () => {
    ...
    const databasePath = path.join(agentDir, "afora-agent.sqlite");
    const deletion = beginAgentDeletion(
      {
        agentId: "worker-1",
        agentDir,
    ...
    expect(() =>
      registerAforaAgentDatabase({ agentId: "worker-2", path: databasePath, env }),
    ).not.toThrow();
    unregisterAforaAgentDatabase({ agentId: "worker-2", path: databasePath, env });
  ...
  it("serializes database leases with the durable deletion fence", () => {
    const stateDir = createTempStateDir();
    const env = { AFORA_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agents", "worker-1", "afora-agent.sqlite");
    const leaseId = claimAforaAgentDatabaseLease({
      agentId: "worker-1",
      path: databasePath,
      env,
    ...
    try {
      ...
      expect(() =>
        claimAforaAgentDatabaseLease({ agentId: "worker-1", path: databasePath, env }),
      ).toThrow("agent worker-1 is deleted");
      releaseAforaAgentDatabaseLease(leaseId, { env });
      expect(() => assertNoAforaAgentDatabaseLeases("worker-1", { env })).not.toThrow();
    } finally {
  ...
  it("serializes concurrent ownership claims for one unowned database", async () => {
    ...
    try {
      ...
      expect(inspectAforaAgentDatabaseOwner(databasePath)).toEqual({
        status: "owned",
        agentId: winner?.agentId,
      });
      expect(
        listAforaRegisteredAgentDatabases({ env: { AFORA_STATE_DIR: stateDir } }),
      ).toEqual([
        expect.objectContaining({
          agentId: winner?.agentId,
          path: databasePath,
  ...
  it("rechecks schema version after waiting for a concurrent writer", async () => {
    ...
    const futureVersion = AFORA_AGENT_SCHEMA_VERSION + 1;
    try {
      await opener.ready;
      opener.child.stdin.end("go\n");
      ...
      expect(fs.existsSync(path.join(stateDir, "state", "afora.sqlite"))).toBe(false);
      expect(
        listAforaRegisteredAgentDatabases({ env: { AFORA_STATE_DIR: stateDir } }),
      ).toEqual([]);
    } finally {
  ...
  it("rejects explicit paths that point at the global state database", () => {
    ...
    const stateDatabase = openAforaStateDatabase({
      env,
      path: databasePath,
    });
    closeAforaStateDatabaseForTest();
    ...
    const reopenedStateDatabase = openAforaStateDatabase({
      env,
      path: databasePath,
    });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { AFORA_STATE_DIR: parentDir };
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      runAforaAgentWriteTransaction(() => {
        runAforaAgentWriteTransaction(() => undefined, options);
  ...
  it("installs same-version session additions before maintenance index repair", () => {
    ...
    try {
      ...
      expect(() =>
        assertAforaAgentDatabaseForMaintenance(repaired, {
          agentId: "worker-1",
          pathname: databasePath,
        }),
      ).not.toThrow();
  ...
  it("rejects same-name transcript index drift when duplicate rows block repair", () => {
    ...
    try {
      ...
      expect(
        after
          .prepare(
            "SELECT name FROM sqlite_schema WHERE type = 'index' AND name LIKE 'afora_probe_%'",
          )
          .all(),
      ).toEqual([]);
    } finally {
  ...
  it("adds durable conversation delivery state when upgrading the canonical v10 schema", () => {
    ...
    expect(() =>
      assertAforaAgentDatabaseForMaintenance(database.db, {
        agentId: "worker-1",
  ...
  it.each([null, "", "   "])(
    ...
    (agentId) => {
      ...
      expect(inspectAforaAgentDatabaseOwner(databasePath)).toEqual({ status: "unreadable" });
    },
  );
  ...
  it("rejects stale schema_meta indexes before writable initialization", () => {
    ...
    try {
      expect(() =>
        ensureAforaAgentDatabaseSchema(independentlyManaged, {
          agentId: "worker-1",
          env,
        }),
  ...
  it.each([0, AFORA_AGENT_SCHEMA_VERSION - 1])(
    ...
    (version) => {
      ...
      expect(() => openAforaAgentDatabase({ agentId: "worker-1", env })).toThrow(
        "run afora doctor --fix to migrate persisted media",
      );
    },
  );
  ...
  it("does not hide a sqlitefoo-only metadata-less v0 database as internal", () => {
    ...
    expect(() =>
      withAforaAgentDatabaseReadOnly(() => "unused", {
        agentId: "worker-1",
        env,
        path: databasePath,
      }),
    ).toThrow("uses schema version 0; run afora doctor --fix");
    expect(() =>
      openAforaAgentDatabase({ agentId: "worker-1", env, path: databasePath }),
    ).toThrow("uses schema version 0; run afora doctor --fix");
  });
  ...
  it("runs full integrity before a pending agent schema migration", () => {
    ...
    try {
      before.exec(`PRAGMA user_version = ${AFORA_AGENT_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
  ...
  it("latches newer per-agent schema failures before integrity scans", () => {
    ...
    const { DatabaseSync } = requireNodeSqlite();
    const db = new DatabaseSync(databasePath);
    db.exec(`PRAGMA user_version = ${AFORA_AGENT_SCHEMA_VERSION + 1};`);
    db.close();
    ...
    expect(firstFailure).toMatchObject({
      ...
      message: expect.stringContaining("https://docs.afora.ai/reference/database-schemas"),
    });
    ...
    clearAforaAgentDatabaseOpenFailure(databasePath);
  ...
  it("closes cached handles on normal process exit so no stale WAL remains", () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          import fs from "node:fs";
          import { openAforaAgentDatabase } from ${JSON.stringify(agentModuleUrl)};

          const database = openAforaAgentDatabase({
            agentId: "worker-1",
            env: { AFORA_STATE_DIR: process.env.AFORA_AGENT_DB_EXIT_TEST_DIR },
          });
      ...
      {
        encoding: "utf8",
        env: { ...process.env, AFORA_AGENT_DB_EXIT_TEST_DIR: stateDir },
      },
    );
    const result = JSON.parse(output) as {
