...
let aforaStateCanonicalNamedIndexSet: ReadonlySet<string> | undefined;

function getAforaStateCanonicalNamedIndexSet(): ReadonlySet<string> {
  aforaStateCanonicalNamedIndexSet ??= new Set(
    getCanonicalSqliteNamedIndexContracts(AFORA_STATE_SCHEMA_SQL).map((index) => index.name),
  );
  return aforaStateCanonicalNamedIndexSet;
}

/** Project canonical SQL to the tables the shared runtime may create during this open. */
export function getAforaStateRuntimeSchema(options: {
  includeVersionLazyAdditiveTables: boolean;
}): string {
  let schema = AFORA_STATE_SCHEMA_SQL;
...
export const AFORA_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY: SqliteSchemaCompatibility = {
...
export function isAforaStateStartupRepairableSchemaIssue(issue: SqliteSchemaIssue): boolean {
  ...
  return (
    ...
    getAforaStateCanonicalNamedIndexSet().has(issue.objectName)
  );
}

/** Identify compatible schema differences repaired only by their feature owner. */
export function isAforaStateFirstUseSchemaIssue(issue: SqliteSchemaIssue): boolean {
  return (
