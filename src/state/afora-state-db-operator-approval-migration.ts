...
export function assertCanonicalOperatorApprovalKinds(db: DatabaseSync, pathname: string): void {
  if (!hasCanonicalOperatorApprovalKinds(db)) {
    throw new Error(
      `Afora state database ${pathname} has a legacy operator approval schema; run afora doctor --fix to migrate it.`,
    );
  }
}
...
function canonicalOperatorApprovalCreateSql(): string {
  ...
  const start = AFORA_STATE_SCHEMA_SQL.indexOf(marker);
  const end = AFORA_STATE_SCHEMA_SQL.indexOf(
  ...
  return AFORA_STATE_SCHEMA_SQL.slice(start, end + tableTerminator.length);
}
...
function operatorApprovalIndexSql(): string {
  const statements = AFORA_STATE_SCHEMA_SQL.split(";")
