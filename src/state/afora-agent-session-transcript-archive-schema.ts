...
function sessionTranscriptArchiveSchemaSql(): string {
  const start = AFORA_AGENT_SCHEMA_SQL.indexOf(ARCHIVE_SCHEMA_START);
  const end = AFORA_AGENT_SCHEMA_SQL.indexOf(ARCHIVE_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Afora session transcript archive schema markers are missing.");
  }
