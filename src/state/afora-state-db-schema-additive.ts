...
} from "./afora-state-db-additive-columns.js";
import {
...
} from "./afora-state-db-legacy-backfills.js";
import { ensureColumn } from "./afora-state-db-schema-helpers.js";
...
function secretStoreSchemaSql(): string {
  const start = AFORA_STATE_SCHEMA_SQL.indexOf(SECRET_STORE_SCHEMA_START);
  const endMarkerStart = AFORA_STATE_SCHEMA_SQL.indexOf(SECRET_STORE_SCHEMA_END, start);
  const hasBoundedSchema = start >= 0 && endMarkerStart >= start;
  if (!hasBoundedSchema) {
    throw new Error("Afora secret store schema marker is missing.");
  }
  return AFORA_STATE_SCHEMA_SQL.slice(start, endMarkerStart + SECRET_STORE_SCHEMA_END.length);
}
...
export function ensureMcpOAuthPendingSchema(database: DatabaseSync): void {
  const start = AFORA_STATE_SCHEMA_SQL.indexOf(MCP_OAUTH_PENDING_SCHEMA_START);
  const endMarkerStart = AFORA_STATE_SCHEMA_SQL.indexOf(MCP_OAUTH_PENDING_SCHEMA_END, start);
  if (start < 0 || endMarkerStart < start) {
    throw new Error("Afora MCP OAuth pending schema marker is missing.");
  }
  database.exec(
    AFORA_STATE_SCHEMA_SQL.slice(start, endMarkerStart + MCP_OAUTH_PENDING_SCHEMA_END.length),
...
export function ensureDevicePairingJoinCodeSchema(database: DatabaseSync): void {
  const start = AFORA_STATE_SCHEMA_SQL.indexOf(DEVICE_PAIRING_JOIN_CODE_SCHEMA_START);
  const endMarkerStart = AFORA_STATE_SCHEMA_SQL.indexOf(
    DEVICE_PAIRING_JOIN_CODE_SCHEMA_END,
    start,
  );
  if (start < 0 || endMarkerStart < start) {
    throw new Error("Afora device pairing join-code schema marker is missing.");
  }
  database.exec(
    AFORA_STATE_SCHEMA_SQL.slice(
      start,
...
export function ensureAdditiveStateColumns(db: DatabaseSync): void {
  ...
  ensureColumn(db, "worker_environments", "bootstrap_afora_version TEXT");
