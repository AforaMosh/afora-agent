...
describe("repairLegacySubagentSuspensionReasons", () => {
  it("rewrites the shipped reason on open and stays canonical after a second open", () => {
    const stateDir = tempDirs.make("afora-subagent-suspension-backfill-");
    const options = { env: { AFORA_STATE_DIR: stateDir } };
    const initial = openAforaStateDatabase(options);
    ...
    const firstOpen = openAforaStateDatabase(options);
    ...
    const secondOpen = openAforaStateDatabase(options);
