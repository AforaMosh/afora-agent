...
vi.mock("./afora-state-schema.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./afora-state-schema.js")>();
  return {
    ...actual,
    AFORA_STATE_SCHEMA_SQL: `${actual.AFORA_STATE_SCHEMA_SQL}\n${trailingSchema.sql}\n`,
  };
});

import { ensureSecretStoreSchema } from "./afora-state-db-schema-additive.js";
