...
} from "./afora-state-db-audit-migration.js";
import {
...
import { resolveDatabasePath } from "./afora-state-db-maintenance.js";
import * as operatorApprovalMigration from "./afora-state-db-operator-approval-migration.js";
import {
  tableExists,
  tableHasColumn,
  tablePrimaryKeyColumns,
} from "./afora-state-db-schema-helpers.js";
import { AforaStateDatabaseSchemaMigrationRequiredError } from "./afora-state-db-schema-migration-required.js";
import * as sessionWatchMigration from "./afora-state-db-session-watch-migration.js";
import {
  resolveAforaAgentDatabaseStoredPath,
  resolveAforaStateDirForDatabasePath,
...
function assertRecognizedRetiredCommitmentsSchema(db: DatabaseSync): void {
  ...
  assertSqliteSchemaContains(
    db,
    "retired Afora commitments schema",
    RETIRED_COMMITMENTS_SCHEMA_SQL,
    RETIRED_COMMITMENTS_SCHEMA_COMPATIBILITY,
  );
  throw new Error(
    "Retired Afora commitments schema has unsupported additional indexes; refusing destructive migration.",
  );
}
...
function assertNoRetiredCommitmentsForeignKeys(db: DatabaseSync): void {
  ...
  for (const table of tables) {
    ...
    if (
      ...
      throw new Error(
        `Retired Afora commitments schema is referenced by table ${table.name}; refusing destructive migration.`,
      );
    }
  }
}
...
function assertNoRetiredCommitmentsSchemaDependencies(db: DatabaseSync): void {
  const probeTable = "__afora_retired_commitments_probe";
  if (tableExists(db, probeTable)) {
    throw new Error(
      `Afora state database already contains ${probeTable}; refusing destructive migration.`,
    );
  }
  const before = collectRetainedSchemaSql(db);
  const savepoint = "afora_probe_commitments_dependencies";
  ...
  if (changedObject) {
    ...
    throw new Error(
      `Retired Afora commitments schema is referenced by ${type} ${name}; refusing destructive migration.`,
    );
  }
}
...
export function migrateRetiredCommitmentsSchema(
  ...
  const savepoint = "afora_retire_commitments_v7";
  db.exec(`SAVEPOINT ${savepoint};`);
  try {
...
export function migrateWorkerPlacementExecutionModeSchema(
  ...
  const placementSchema = AFORA_STATE_SCHEMA_SQL.slice(start, end + endMarker.length);
  ...
  if (
    ...
    throw new Error("Afora v7 worker placement columns are not canonical");
  }
  ...
  if (unexpectedObjects.length > 0) {
    throw new Error("Afora v7 worker placement schema has unsupported attached objects");
  }
  const migrationTable = "worker_session_placements_migration_v8";
  if (tableExists(db, migrationTable)) {
    throw new Error(`Afora worker placement migration table already exists: ${migrationTable}`);
  }
...
function isDefaultAgentDatabasePath(pathname: string, agentId: string): boolean {
  ...
  return (
    path.basename(pathname) === "afora-agent.sqlite" &&
...
export function migrateAgentDatabaseRelativePaths(
  ...
  for (const row of rows) {
    ...
    if (typeof agentId !== "string" || typeof registeredPath !== "string") {
      throw new Error("Afora v8 agent database registry paths are not canonical");
    }
    if (!path.isAbsolute(registeredPath)) {
      continue;
    }
    const storedPath = resolveAforaAgentDatabaseStoredPath(databasePath, registeredPath);
  ...
  for (const row of rows) {
    ...
    if (
      ...
      !path.isAbsolute(resolveAforaAgentDatabaseStoredPath(databasePath, registeredPath))
    ) {
      continue;
    }
    ...
    if (isDefaultAgentDatabasePath(absolutePath, agentId)) {
      ...
      const counterpartStored = resolveAforaAgentDatabaseStoredPath(
        databasePath,
        counterpartAbsolute,
      );
...
export function markCurrentStateSchemaVersion(
  ...
  if (
    ...
    if (options.createMetadataIfMissing) {
      ...
      ).run(AFORA_STATE_SCHEMA_VERSION, now, now);
      return;
    }
    db.prepare(
      "UPDATE schema_meta SET schema_version = ?, updated_at = ? WHERE meta_key = 'primary'",
    ).run(AFORA_STATE_SCHEMA_VERSION, now);
  }
}
...
export function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  if (!hasCanonicalAgentDatabasesPrimaryKey(db)) {
    if (canRepairAgentDatabasesPrimaryKey(db)) {
      throw new AforaStateDatabaseSchemaMigrationRequiredError(
        "agent-databases-composite-primary-key",
        pathname,
      );
    }
    throw new Error(
      `Afora state database ${pathname} has a noncanonical agent database registry schema that cannot be repaired automatically; restore the canonical agent_databases shape before retrying.`,
    );
  }
  if (!hasCanonicalAuditEventsSchema(db)) {
    if (canRepairLegacyAuditEventsSchema(db)) {
      throw new AforaStateDatabaseSchemaMigrationRequiredError("audit-events-v2", pathname);
    }
    throw new Error(
      `Afora state database ${pathname} has a noncanonical audit event schema that cannot be repaired automatically; restore the canonical audit_events shape before retrying.`,
    );
  }
}
export function detectAforaStateDatabaseSchemaMigrations(
  ...
  try {
    return detectAforaStateDatabaseSchemaMigrationsFromDatabase(db, pathname);
  } finally {
    db.close();
  }
}
 ...
 */
export function detectAforaStateDatabaseSchemaMigrationsFromDatabase(
  db: DatabaseSync,
  pathname: string,
): AforaStateDatabaseSchemaMigration[] {
  const migrations: AforaStateDatabaseSchemaMigration[] = [];
  ...
  if (tableExists(db, "audit_events") && userVersion < AFORA_STATE_STRICT_SCHEMA_VERSION) {
