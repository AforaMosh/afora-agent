...
import {
  resolveAforaStateSqliteDir,
  resolveAforaStateSqlitePath,
} from "./afora-state-db.paths.js";

const AFORA_STATE_DIR_MODE = 0o700;
const AFORA_STATE_FILE_MODE = 0o600;
...
export function ensureAforaStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveAforaStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveAforaStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`Afora state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: AFORA_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, AFORA_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, AFORA_STATE_FILE_MODE);
    }
  }
}
