...
} from "./afora-agent-session-transcript-archive-schema.js";
...
function schemaWithoutTranscriptArchives(): string {
  const start = AFORA_AGENT_SCHEMA_SQL.indexOf(
    `CREATE TABLE IF NOT EXISTS ${SESSION_TRANSCRIPT_ARCHIVES_TABLE} (`,
  );
  const end = AFORA_AGENT_SCHEMA_SQL.indexOf(
    "CREATE TABLE IF NOT EXISTS transcript_rewrite_watermarks (",
    start,
  );
  expect(start).toBeGreaterThanOrEqual(0);
  expect(end).toBeGreaterThan(start);
  return `${AFORA_AGENT_SCHEMA_SQL.slice(0, start)}${AFORA_AGENT_SCHEMA_SQL.slice(end)}`;
}
...
describe("session transcript archive schema", () => {
  it("keeps a current database table-free until first archive use without changing its version", () => {
    const stateDir = tempDirs.make("afora-session-archive-schema-");
    const options = { agentId: "main", env: { AFORA_STATE_DIR: stateDir } };
  ...
  it("rejects a drifted archive table instead of treating it as an optional absence", () => {
    const stateDir = tempDirs.make("afora-session-archive-drift-");
    const options = { agentId: "main", env: { AFORA_STATE_DIR: stateDir } };
    const initial = openAforaAgentDatabase(options);
    const databasePath = initial.path;
    closeAforaAgentDatabasesForTest();
    ...
    expect(() => openAforaAgentDatabase(options)).toThrow(/session_transcript_archives|schema/u);
  });
});
