...
type StateDbTestDatabase = Pick<AforaStateKyselyDatabase, "diagnostic_events">;
...
function createTempStateDir(): string {
  return fs.realpathSync(tempStateDirs.make("afora-state-db-corruption-"));
}
...
function prepareCorruptedCachedDatabase(env: NodeJS.ProcessEnv): {
  ...
  poisoned: ReturnType<typeof openAforaStateDatabase>;
} {
  const poisoned = openAforaStateDatabase({ env });
...
describe("shared state write transaction corruption recovery", () => {
  it("preserves the shared WAL when evicting a poisoned cache owner", () => {
    const env = { AFORA_STATE_DIR: createTempStateDir() };
    const cached = openAforaStateDatabase({ env });
  ...
  it("does not evict a different cached owner when an injected write handle fails", () => {
    ...
    try {
      ...
      expect(getAforaStateDatabaseIfOpen({ env })).toBe(cached);
