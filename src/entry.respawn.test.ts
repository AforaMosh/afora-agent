...
const AFORA_NODE_EXTRA_CA_CERTS_READY = "AFORA_NODE_EXTRA_CA_CERTS_READY";
const AFORA_NODE_OPTIONS_READY = "AFORA_NODE_OPTIONS_READY";
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(respawnPlan.env[AFORA_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(respawnPlan.env[AFORA_NODE_OPTIONS_READY]).toBe("1");
  ...
  it.each(["tui", "terminal", "chat"] as const)(
    ...
    (command) => {
      ...
      const respawnPlan = expectCliRespawnPlan(plan);
      expect(respawnPlan.argv).toEqual(["afora", command]);
      expect(respawnPlan.env.NODE_EXTRA_CA_CERTS).toBe("/etc/ssl/certs/ca-certificates.crt");
      expect(respawnPlan.env[AFORA_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
      expect(respawnPlan.env[AFORA_NODE_OPTIONS_READY]).toBeUndefined();
  ...
  it("keeps bare-root startup respawns attached to the terminal", () => {
    ...
    const respawnPlan = expectCliRespawnPlan(plan);
    expect(respawnPlan.argv).toEqual([EXPERIMENTAL_WARNING_FLAG, "afora"]);
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });

  it("preserves macOS system CA trust through one-shot warning respawns", () => {
    const plan = buildCliRespawnPlan({
      argv: ["node", "afora", "cron", "list", "--json"],
  ...
  it.each([
    ["interactive commands", ["node", "afora", "tui"]],
    ["the foreground Gateway", ["node", "afora", "gateway", "run"]],
  ...
  it("does not respawn interactive commands for warning suppression only", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "afora", "tui"],
        env: { [AFORA_NODE_EXTRA_CA_CERTS_READY]: "1" },
        execArgv: [],
        autoNodeExtraCaCerts: undefined,
        platform: "linux",
      }),
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "afora", "status"],
        env: {
          [AFORA_NODE_EXTRA_CA_CERTS_READY]: "1",
          [AFORA_NODE_OPTIONS_READY]: "1",
        },
        execArgv: [EXPERIMENTAL_WARNING_FLAG],
        autoNodeExtraCaCerts: "/etc/ssl/certs/ca-certificates.crt",
  ...
  it("adds a larger V8 stack size on Windows", () => {
    ...
    expect(respawnPlan.env[AFORA_NODE_EXTRA_CA_CERTS_READY]).toBeUndefined();
    expect(respawnPlan.env[AFORA_NODE_OPTIONS_READY]).toBeUndefined();
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });
  ...
  it("respawns Volta shims through node so the shim is not called directly", () => {
    const plan = buildCliRespawnPlan({
      argv: ["/home/alice/.volta/bin/volta-shim", "/usr/local/bin/afora", "status"],
    ...
    runCliRespawnPlan(
      {
        ...
        argv: ["/repo/afora/dist/entry.js", "status"],
        env: { AFORA_NODE_OPTIONS_READY: "1" },
