...
import { listAforaRegisteredAgentDatabases } from "../state/afora-agent-db-registry.js";
...
describe("setup migration stage", () => {
  it("executes provider config mutations once and projects staged paths", async () => {
    const root = tempRoots.make("afora-migration-stage-");
  ...
  it("routes staged auth writes to the staged shared registry", async () => {
    ...
    expect(
      listAforaRegisteredAgentDatabases({
        env: { ...process.env, AFORA_STATE_DIR: stagedStateDir },
      }),
    ).toEqual([
      expect.objectContaining({
        agentId: "main",
        path: path.join(stagedAgentDir, "afora-agent.sqlite"),
      }),
    ]);
    expect(
      listAforaRegisteredAgentDatabases({
        env: { ...process.env, AFORA_STATE_DIR: liveStateDir },
      }),
    ).toEqual([]);
    await expect(fs.access(path.join(liveStateDir, "state", "afora.sqlite"))).rejects.toThrow();
  });
  ...
  it("promotes the final agent registry path after verification closes the handle", async () => {
    ...
    const { disposeAforaAgentDatabaseByPath } = await import("../state/afora-agent-db.js");
    disposeAforaAgentDatabaseByPath(path.join(stage.staged.agentDir, "afora-agent.sqlite"), {
      env: { ...process.env, AFORA_STATE_DIR: stage.staged.stateDir },
    });
    ...
    expect(
      ...
      expect.objectContaining({
        agentId: "main",
        path: path.join(stage.final.agentDir, "afora-agent.sqlite"),
      }),
    ]);
