...
import type { GatewayBindMode, GatewayTailscaleMode, AforaConfig } from "../config/config.js";
...
export async function configureGatewayForSetup(
  ...
  if (authMode === "token") {
    ...
    if (tokenMode === "ref") {
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.AFORA_GATEWAY_TOKEN)) ||
        randomToken();
