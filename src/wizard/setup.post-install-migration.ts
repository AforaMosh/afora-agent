...
function logMigrationHint(runtime: RuntimeEnv, candidate: ResolvedProviderCandidate): void {
  const command = formatCliCommand(`afora migrate ${candidate.provider.id} --dry-run`);
 ...
 * prompt) is owned by `afora migrate <provider>`; this helper only owns
 * the gate prompt.
 */
    ...
    try {
      ...
      params.runtime.log(
        `${candidate.provider.label} migration failed: ${formatErrorMessage(error)}. ` +
          `Re-run with ${formatCliCommand(`afora migrate ${candidate.provider.id} --dry-run`)} to inspect.`,
      );
    } finally {
