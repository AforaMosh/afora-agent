...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers AFORA_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.AFORA_GATEWAY_TOKEN;
    process.env.AFORA_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("keeps AFORA_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.AFORA_GATEWAY_TOKEN;
    process.env.AFORA_GATEWAY_TOKEN = "advanced-env-token";
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    ...
    process.env.AFORA_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "AFORA_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
  ...
  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    ...
    try {
      const prompter = createPrompter({
        ...
        textQueue: ["18789", "AFORA_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
  ...
  it("persists an explicit classic quickstart env token ref", async () => {
    const previous = process.env.AFORA_GATEWAY_TOKEN;
    process.env.AFORA_GATEWAY_TOKEN = "token-from-env-ref";
    try {
  ...
  it("seeds an explicit env token ref into advanced gateway setup", async () => {
    ...
    try {
      const gatewayDefaults = resolveQuickstartGatewayDefaults(
        {},
        { gatewayPort: 19511, gatewayTokenRefEnv: "AFORA_GATEWAY_TOKEN" },
      );
      const result = await configureGatewayForSetup({
        flow: "advanced",
        baseConfig: {},
