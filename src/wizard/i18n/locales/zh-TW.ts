...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 Afora 設定",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `afora doctor`。",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 Afora 工作區？",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 AFORA_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密碼存入 Afora 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 Afora 設定",
    ...
    guided: {
      ...
      custodianIntro: "你好 — 我是 Afora，負責維護這套系統。我們開始設定吧。",
      failedOptionsIntro: "剛才這些沒有成功：",
      findMeLater: "之後隨時找得到我 — 在終端機執行 `afora`，或在儀表板中開啟設定。",
      ...
      appliedTitle: "推理已就緒",
      complete: "Afora 已準備就緒。",
      completeWithoutAi: "Afora 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      escapeHatches: "如需完整的分步精靈，請執行 `afora onboard --classic`。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "Afora 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
        "未進行任何變更。請執行 {fixCommand} 修復，再執行 {inspectCommand} 檢查，然後重試 onboarding。",
      invalidConfigUnknown: "- 無法解析設定。",
      manualChoice: "你想如何連接 AI？",
      nextSteps:
        "工作區：{workspace}\n新增頻道：`afora channels add`\n偏好聊天？執行 `afora setup`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`afora dashboard`\n稍後聊天：`afora`",
      nextStepsWithoutAi:
        "工作區：{workspace}\n稍後新增 AI：重新執行 `afora onboard`\n連接 AI 後新增頻道：`afora channels add`\n開啟 dashboard：`afora dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要互動式 TTY。自動化請使用 `afora onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "設定選項",
    ...
    security: {
      ...
      confirm: "我理解 Afora 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉存取控制和安全加固，請在獲得協助前不要執行 Afora。",
      learnMore: "了解更多請參閱",
      notMultitenant: "預設情況下，Afora 並非旨在安全地隔離多個使用者。",
      personalAgent: "預設情況下，Afora 是供一位操作者使用的個人 agent。",
    ...
    skills: {
      ...
      docsLine: "文件：https://docs.afora.ai/skills",
      enterEnv: "輸入 {env}",
      ...
      manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `afora doctor` 檢查技能和需求。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作區設定 → Integrations → Afora → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "Afora" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 Afora 將你加入允許清單。",
      ...
      separatePhoneLabel: "Afora 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 Afora 用來向 Chat 傳送回覆的 URL。",
      ...
      publicWebhookUrlScope: "僅公開此 webhook 路由；Afora 使用它提供短期附件下載。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "Afora"',
    ...
    twitch: {
      ...
      envPrompt: "偵測到 Twitch 環境變數 AFORA_TWITCH_ACCESS_TOKEN。使用環境 token？",
      helpCopyToken: "3. 複製 token（以 'oauth:' 開頭）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 建立 Twitch 應用",
      helpEnvVars: "支援環境變數：AFORA_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 Afora，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 afora configure --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.afora.ai/web/control-ui",
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 Afora。",
      dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 Afora。",
      ...
      daemonRuntimeNodeHint: "Afora 狀態使用 node:sqlite，因此必須使用 Node；Bun 無法執行閘道。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 Afora。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 Afora。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。Afora 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.afora.ai/security",
      ...
      webDocs: "文件：https://docs.afora.ai/tools/web",
      ...
      workspaceDocs: "文件：https://docs.afora.ai/concepts/agent-workspace",
