...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 Afora 配置",
    ...
    migration: {
      ...
      appliedTitle: "迁移已应用",
      cancelled: "迁移已取消",
      complete: "迁移完成。下一步运行 `afora doctor`。",
    ...
    memoryImport: {
      ...
      confirm: "将这些 memory 导入你的 Afora 工作区？",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 AFORA_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密码存入 Afora 配置",
      plaintextPasswordLabel: "现在输入密码",
      plaintextTokenHint: "直接把令牌存入 Afora 配置",
    ...
    guided: {
      ...
      custodianIntro: "你好 — 我是 Afora，负责维护这套系统。我们开始设置吧。",
      failedOptionsIntro: "刚才这些没有成功：",
      findMeLater: "以后随时可以找到我 — 在终端运行 `afora`，或在仪表盘中打开设置。",
      ...
      appliedTitle: "推理已就绪",
      complete: "Afora 已准备就绪。",
      completeWithoutAi: "Afora 设置已保存。连接 AI 后再打开聊天。",
      detected: "AI 检测完成。",
      ...
      escapeHatches: "如需完整的分步向导，请运行 `afora onboard --classic`。",
      ...
      intro: "连接你的 AI",
      invalidConfigDetails: "Afora 配置 {path} 无效：\n{issues}",
      invalidConfigRepair:
        "未进行任何更改。请运行 {fixCommand} 修复，再运行 {inspectCommand} 检查，然后重试 onboarding。",
      invalidConfigUnknown: "- 无法解析配置。",
      manualChoice: "你想如何连接 AI？",
      nextSteps:
        "工作区：{workspace}\n添加频道：`afora channels add`\n更喜欢聊天？运行 `afora setup`，然后说 `connect telegram`（或 `connect slack`）。\n打开 dashboard：`afora dashboard`\n稍后聊天：`afora`",
      nextStepsWithoutAi:
        "工作区：{workspace}\n稍后添加 AI：重新运行 `afora onboard`\n连接 AI 后添加频道：`afora channels add`\n打开 dashboard：`afora dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要交互式 TTY。自动化请使用 `afora onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "设置选项",
    ...
    security: {
      ...
      attribution: "Afora 是由 Afora 基金会（非营利组织）开发的开源助手，会与你一同学习成长。",
      baselineDmSessions:
      ...
      confirm: "我理解 Afora 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉访问控制和安全加固，请在获得帮助前不要运行 Afora。",
      learnMore: "了解更多请访问",
      notMultitenant: "默认情况下，Afora 并非旨在安全地隔离多个用户。",
      personalAgent: "默认情况下，Afora 是供一位操作员使用的个人 agent。",
    ...
    skills: {
      ...
      docsLine: "文档：https://docs.afora.ai/skills",
      enterEnv: "输入 {env}",
      ...
      manualPrereqsDoctorHint: "请先安装这些工具，然后运行 `afora doctor` 检查技能和要求。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作区设置 → Integrations → Afora → 创建 bot → 复制 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "Afora" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 Afora 将你加入允许列表。",
      ...
      separatePhoneLabel: "Afora 专用独立手机",
    ...
    irc: {
      ...
      nextStatusCommand: "命令：afora channels status --probe",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 Afora 用来向 Chat 发送回复的 URL。",
      ...
      publicWebhookUrlScope: "仅公开此 webhook 路由；Afora 使用它提供短期附件下载。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "Afora"',
    ...
    twitch: {
      ...
      envPrompt: "检测到 Twitch 环境变量 AFORA_TWITCH_ACCESS_TOKEN。使用环境 token？",
      helpCopyToken: "3. 复制 token（以 'oauth:' 开头）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 创建 Twitch 应用",
      helpEnvVars: "支持环境变量：AFORA_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 Afora，并启用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已配置",
      configureLaterHint: "稍后可用 afora configure --section web 配置",
      docsLine: "文档：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文档：https://docs.afora.ai/web/control-ui",
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 Afora。",
      dashboardLinkWithToken: "Dashboard 链接（含令牌）：{url}",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 Afora。",
      ...
      daemonRuntimeNodeHint: "Afora 状态使用 node:sqlite，因此必须使用 Node；Bun 无法运行网关。",
      ...
      gatewayTokenStored:
        "存储位置：$AFORA_CONFIG_PATH（默认 ~/.afora/afora.json）中的 gateway.auth.token，或 AFORA_GATEWAY_TOKEN。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 Afora。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 Afora。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。Afora 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.afora.ai/security",
      ...
      webDocs: "文档：https://docs.afora.ai/tools/web",
      ...
      whatNow: '下一步：https://afora.ai/showcase（"What People Are Building"）。',
      whatNowTitle: "下一步",
      workspaceBackupTitle: "工作区备份",
      workspaceDocs: "文档：https://docs.afora.ai/concepts/agent-workspace",
