...
import { z } from "zod";
import type { AforaChannelBridge } from "./channel-bridge.js";
import {
...
export function registerChannelMcpTools(server: McpServer, bridge: AforaChannelBridge): void {
  server.tool(
    "conversations_list",
    "List Afora channel-backed conversations available through session routes.",
    {
  ...
  server.tool(
    "conversation_get",
    "Get one Afora conversation by session key.",
  ...
  server.tool(
    "messages_read",
    "Read recent messages for one Afora conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "attachments_fetch",
    "List non-text attachments for a message in one Afora conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "events_poll",
    "Poll queued Afora conversation events since a cursor.",
    {
  ...
  server.tool(
    "events_wait",
    "Wait for the next queued Afora conversation event.",
    {
      after_cursor: z.number().int().min(0).optional(),
      session_key: z.string().optional(),
  ...
  server.tool(
    "messages_send",
    "Send a message back through the same Afora conversation route.",
    {
      session_key: z.string().min(1),
      text: z.string().min(1),
    },
  ...
  server.tool(
    ...
    "List open Afora exec or plugin approval requests visible through the Gateway.",
    {},
    async () => {
  ...
  server.tool(
    "permissions_respond",
    "Allow or deny one pending Afora exec or plugin approval request.",
    {
