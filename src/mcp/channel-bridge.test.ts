...
function makeBridge(verbose = false): BridgeInternals {
  return new AforaChannelBridge({} as never, {
    claudeChannelMode: "off",
    verbose,
  }) as unknown as BridgeInternals;
}

describe("AforaChannelBridge — Claude permission authorization", () => {
  test.each([
...
describe("AforaChannelBridge — pendingClaudePermissions / pendingApprovals memory bounds", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(0);
  });
  ...
  test("a failed notification still emits exactly one diagnostic record with verbose off", async () => {
    ...
    try {
      ...
      expect(writes[0]).toBe("afora mcp: notification channel/event failed\n");
  ...
  test("a rejected gateway event still emits exactly one diagnostic record with verbose off", async () => {
    ...
    try {
      ...
      expect(writes).toHaveLength(1);
      expect(writes[0]).toBe("afora mcp: gateway event exec.approval.requested failed\n");
  ...
  test("a rejected gateway event includes error detail with verbose on", async () => {
    ...
    try {
      ...
      expect(writes[1]).toBe(
        "afora mcp: gateway event exec.approval.requested error: Error: handler boom\n",
      );
    } finally {
      writeSpy.mockRestore();
      await bridge.close();
