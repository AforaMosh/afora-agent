...
/** Resolve the visible message id, including Afora metadata attached to raw entries. */
export function resolveMessageId(entry: Record<string, unknown>): string | undefined {
  return (
    toText(entry.id) ??
    (entry["__afora"] && typeof entry["__afora"] === "object"
      ? toText((entry["__afora"] as { id?: unknown }).id)
      : undefined)
  );
}
