...
const recordedEventsKey = Symbol.for("afora.qa.plugin-hook-contracts.events");
...
function loadFixturePlugin() {
  ...
  const plugin = writePlugin({
    id: pluginId,
    body: `
const eventsKey = Symbol.for("afora.qa.plugin-hook-contracts.events");
  ...
  it("executes message, session, and cron lifecycle hooks from the loaded plugin", async () => {
    ...
    await runner.runCronChanged(
      ...
      },
      {
        port: 18789,
        config: {},
        workspaceDir: "/tmp/afora-qa-workspace",
        getCron: () => undefined,
      },
    );
