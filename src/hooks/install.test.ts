...
const fixtureRoot = path.join(process.cwd(), ".tmp", `afora-hook-install-${randomUUID()}`);
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    afora: { hooks: [`./hooks/${params.hookName}`] },
  });
...
describe("installHooksFromPath", () => {
  it.each([
    {
      ...
      code: HOOK_INSTALL_ERROR_CODE.MISSING_AFORA_HOOKS,
    },
    {
      afora: { hooks: [] },
      error: "package.json afora.hooks is empty",
      code: HOOK_INSTALL_ERROR_CODE.EMPTY_AFORA_HOOKS,
    },
  ])("returns a stable code for $error", async ({ afora, error, code }) => {
    const pkgDir = makeTempDir();
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@afora/test-hooks", afora }),
    );
  ...
  it("blocks a staged single hook before publishing the target", async () => {
    ...
    expect(scanCall.packageDir).toContain(".afora-install-stage-");
  ...
  it.each([".codex-plugin/plugin.json", "hooks/hooks.json", "afora.plugin.json"])(
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "afora.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "afora.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
          ...
          expect(params.archiveInstallParams).toEqual(
            expect.objectContaining({
              installPolicyRequest: {
                kind: "plugin-npm",
                requestedSpecifier: "@afora/test-hooks@0.0.1",
                source: {
                  kind: "npm",
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.packageKind).toBe("hook-only");
    expect(result.npmResolution?.resolvedSpec).toBe("@afora/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@afora/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@afora/test-hooks@0.0.2-beta.1",
      name: "@afora/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@afora/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@afora/test-hooks@beta"');
    }
  });
});
