...
vi.mock("./installs.js", () => ({
  ...
  recordHookInstall: (cfg: AforaConfig, update: HookInstallRecord & { hookId: string }) => {
...
describe("updateNpmInstalledHookPacks", () => {
  ...
  it("aborts exact pinned hook pack updates on integrity drift by default", async () => {
      ...
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            ...
            resolvedSpec: "@afora/demo-hooks@1.0.0",
            version: "1.0.0",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @afora/demo-hooks@1.0.0",
          };
        }
        return {
          ok: true,
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for hook pack "demo-hooks" (@afora/demo-hooks@1.0.0): expected sha512-old, got sha512-new',
    );
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          'Failed to update hook pack "demo-hooks": aborted: npm package integrity drift detected for @afora/demo-hooks@1.0.0',
      },
    ]);
  });
  ...
  it("preserves hook pack update selector and records npm resolution metadata after update", async () => {
    installHooksFromNpmSpecMock.mockResolvedValue({
      ...
      version: "1.2.3",
      npmResolution: {
        name: "@afora/demo-hooks",
        version: "1.2.3",
        resolvedSpec: "@afora/demo-hooks@1.2.3",
        integrity: "sha512-new",
        shasum: "abc123",
