...
homepage: https://docs.afora.ai/automation/hooks#command-logger
metadata:
  {
    "afora":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.afora/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.afora/logs/commands.log ~/.afora/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/afora`:
   ```
   /home/username/.afora/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.afora/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.afora/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.afora/logs/commands.log | jq .
```
