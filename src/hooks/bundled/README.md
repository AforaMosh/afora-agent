...
This directory contains hooks that ship with Afora. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.afora/workspace`)
...
```bash
afora hooks enable bootstrap-extra-files
```
...
**Output**: `~/.afora/logs/commands.log`

**Enable**:

```bash
afora hooks enable command-logger
```
...
homepage: https://docs.afora.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.afora/hooks/` (shared across workspaces)
...
```bash
afora hooks enable session-memory
afora hooks disable command-logger
```
...
2. Restart gateway: `pkill -9 -f 'afora.*gateway' && pnpm afora gateway`
3. Enable the hook: `afora hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.afora.ai/automation/hooks
