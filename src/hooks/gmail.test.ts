...
import { type AforaConfig, DEFAULT_GATEWAY_PORT } from "../config/config.js";
import {
  buildDefaultHookUrl,
...
describe("gmail hook config", () => {
  function resolveWithGmailOverrides(
    overrides: Partial<NonNullable<AforaConfig["hooks"]>["gmail"]>,
  ) {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.account).toBe("afora@gmail.com");
