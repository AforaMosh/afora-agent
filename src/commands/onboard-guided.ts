...
export type GuidedOnboardingDeps = {
  ...
  persistRiskAcknowledgement?: (config: AforaConfig) => Promise<string | void>;
   ...
   * "chat" preserves the legacy handoff into the Afora system-agent chat —
...
async function persistRiskAcknowledgement(config: AforaConfig): Promise<string | undefined> {
...
async function runGuidedOnboardingFlow(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      t("wizard.guided.invalidConfigRepair", {
        fixCommand: formatCliCommand("afora doctor --fix"),
        inspectCommand: formatCliCommand("afora config validate"),
      }),
    );
    runtime.exit(1);
    return null;
  ...
  const assertLocalSetupOwner = (config: AforaConfig) => {
    if (
      localSetup?.status === "pending" &&
  ...
  // Inference is the only prerequisite for Afora. Use the caller's or
  // current default workspace as isolated probe context; Afora owns any
  ...
  if (alreadyConfigured) {
    ...
    if (workspaceConflict) {
      await prompter.note(
        t("wizard.guided.workspaceConflictClassic", {
          command: formatCliCommand("afora onboard --classic"),
        }),
    ...
    // would, then hand off to the hatch instead of parking in the Afora chat.
    ...
    try {
      ...
      if (!appliedSnapshot.valid) {
        throw new Error("Setup wrote an invalid Afora config.");
      }
