...
describe("detectLinuxVolatileStateDir", () => {
  const TMPFS_MOUNT_INFO = [
    ...
    "35 22 0:35 / /home/user/.afora rw - tmpfs tmpfs rw,size=1048576k",
  ].join("\n");
  const RAMFS_MOUNT_INFO = [
    "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
    "35 22 0:35 / /home/user/.afora rw - ramfs ramfs rw",
  ].join("\n");
  ...
  it.each([
    ...
    const result = detectLinuxVolatileStateDir("/home/user/.afora", {
      platform: "linux",
      mountInfo,
    ...
    expect(result).toMatchObject({
      path: "/home/user/.afora",
      mountPoint: "/home/user/.afora",
      fsType,
    });
  });
  ...
  it("uses the most specific matching mount", () => {
    const mountInfo = [
      ...
      "35 30 0:35 / /home/user/.afora rw - tmpfs tmpfs rw",
    ].join("\n");

    expect(
      detectLinuxVolatileStateDir("/home/user/.afora", {
        platform: "linux",
        mountInfo,
        resolveRealPath: (targetPath) => targetPath,
      }),
  ...
  it("detects a missing state directory through an existing symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-doctor-volatile-"));
    try {
      ...
      const result = detectLinuxVolatileStateDir(path.join(stateLink, "afora"), {
        platform: "linux",
        mountInfo: [
          "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
      ...
      expect(result).toMatchObject({
        path: path.join(resolvedVolatileMount, "afora"),
...
describe("formatLinuxVolatileStateDirWarning", () => {
  it("covers all SQLite state and sidecar files under the volatile state directory", () => {
    const warning = formatLinuxVolatileStateDirWarning("~/.afora", {
      path: "/home/user/.afora",
      mountPoint: "/home/user/.afora",
      fsType: "tmpfs",
    });
    ...
    expect(warning).toContain("AFORA_STATE_DIR");
  });
});
