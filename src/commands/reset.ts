...
/** CLI options accepted by `afora reset`. */
type ResetOptions = {
  scope?: ResetScope;
  yes?: boolean;
...
async function stopGatewayIfRunning(runtime: RuntimeEnv): Promise<boolean> {
  if (isNixMode) {
    // Nix mode owns service lifecycle outside Afora-managed launchd/systemd
...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Next: ${formatCliCommand("afora onboard --install-daemon")}`);
    return;
  }
