...
function isTempBackedAforaRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const aforaIndex = lower.lastIndexOf("afora");
  if (aforaIndex < 1) {
    return false;
  }
  return lower[aforaIndex - 1] === "tmp" || lower[aforaIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("afora@"),
    ) || isTempBackedAforaRoot(beforeSkillRoot)
  );
}
...
function resolveExpectedBundledSkillPath(params: {
  ...
  // Snapshot paths use shell `~` semantics. AFORA_HOME may point at an isolated
...
export function sessionSnapshotIssueToHealthFinding(
  ...
  return {
    ...
    fixHint:
      "To clean up the advisory artifact, run `afora doctor --fix` to rewrite stale cached session metadata paths, or start a fresh session after confirming history can be retired.",
  };
}
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("afora.doctorSessionSnapshotsTestApi")
  ] = {
