...
function serviceWithEnv(environment?: Record<string, string>) {
  return {
    readCommand: vi.fn(async () =>
      environment ? { programArguments: ["afora", "gateway"], environment } : null,
    ),
  };
}
...
describe("web_fetch proxy doctor diagnostic", () => {
  it("reports direct routing for an installed Gateway proxy without exposing its value", async () => {
    ...
    expect(diagnostic).toContain("Direct TLS connectivity to docs.afora.ai:443 failed");
    expect(diagnostic).toContain("afora config set tools.web.fetch.useTrustedEnvProxy true");
  ...
  it("reports a reachable direct path from the doctor process", async () => {
    ...
    expect(diagnostic).toContain("Direct TLS connectivity to docs.afora.ai:443 succeeded");
  });
