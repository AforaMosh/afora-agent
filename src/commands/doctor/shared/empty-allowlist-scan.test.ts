...
describe("doctor empty allowlist policy scan", () => {
  it("scans top-level and account-scoped channel warnings", () => {
    ...
    expect(warnings).toEqual([
      '- channels.signal.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to channels.signal.allowFrom, or run "afora doctor --fix" to auto-migrate from pairing store when entries exist.',
      '- channels.signal.accounts.work.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to channels.signal.accounts.work.allowFrom, or run "afora doctor --fix" to auto-migrate from pairing store when entries exist.',
    ]);
  });
  ...
  it("skips disabled channel and account entries", () => {
    ...
    const warnings = scanEmptyAllowlistPolicyWarnings(
      ...
      { doctorFixCommand: "afora doctor --fix", extraWarningsForAccount },
    );
