...
describe("doctor config flow steps", () => {
  beforeEach(() => {
    ...
    migrateLegacyConfigMock.mockImplementation((config: AforaConfig) => ({
      config,
      changes: [],
    }));
  ...
  it("collects legacy compatibility issue lines and preview fix hints", () => {
    ...
    expect(result.state.fixHints).toStrictEqual([
      'Run "afora doctor --fix" to migrate legacy config keys.',
    ]);
  ...
  it("removes unknown keys and adds preview hint", () => {
    ...
    const result = applyUnknownConfigKeyStep({
      state: {
        cfg: {},
        candidate: { bogus: true } as unknown as AforaConfig,
        pendingChanges: false,
        fixHints: [],
      },
      shouldRepair: false,
    ...
    expect(result.state.fixHints).toStrictEqual([
      'Run "afora doctor --fix" to remove these keys.',
    ]);
  });
