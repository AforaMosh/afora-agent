...
const GATEWAY_PORT_OOB_RULE: LegacyConfigRule = {
  path: ["gateway", "port"],
  message:
    'gateway.port is outside the valid TCP range (1–65535) and will be removed to avoid startup failure. Run "afora doctor --fix".',
...
const GATEWAY_BIND_RULE: LegacyConfigRule = {
  path: ["gateway", "bind"],
  message:
    'gateway.bind host aliases (for example 0.0.0.0/localhost) are legacy; use bind modes (lan/loopback/custom/tailnet/auto) instead. Run "afora doctor --fix".',
...
const GATEWAY_WEBCHAT_RULE: LegacyConfigRule = {
  ...
  message: 'gateway.webchat is retired. Run "afora doctor --fix".',
};

const GATEWAY_TAILSCALE_RESET_ON_EXIT_RULE: LegacyConfigRule = {
  path: ["gateway", "tailscale", "resetOnExit"],
  message:
    'gateway.tailscale.resetOnExit is retired because managed routes now follow the Gateway lifecycle automatically. Run "afora doctor --fix".',
...
const GATEWAY_TAILSCALE_SERVICE_NAME_RULE: LegacyConfigRule = {
  ...
  message:
    'gateway.tailscale.serviceName is retired because named Services require persistent background routes that cannot follow the Gateway lifecycle. Run "afora doctor --fix".',
};

const CONTROL_UI_DEVICE_AUTH_MIGRATION_RULE: LegacyConfigRule = {
  path: ["gateway", "controlUi", "dangerouslyDisableDeviceAuth"],
  message:
    'gateway.controlUi.dangerouslyDisableDeviceAuth is retired and ignored. Control UI browsers pair through the normal device flow; run "afora doctor --fix" to remove the legacy key.',
  match: (value) => typeof value === "boolean",
};
