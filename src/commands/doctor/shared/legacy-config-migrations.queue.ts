...
const QUEUE_MODE_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'messages.queue.mode uses a retired queue mode; use steer, followup, collect, or interrupt. Run "afora doctor --fix".',
    match: isRetiredQueueMode,
  },
  {
    path: ["messages", "queue", "byChannel"],
    message:
      'messages.queue.byChannel contains a retired queue mode; use steer, followup, collect, or interrupt. Run "afora doctor --fix".',
