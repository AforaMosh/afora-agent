...
import type { AforaConfig, PluginsConfig } from "../../../config/types.js";
...
function currentAforaReleaseBase(): string {
...
const mocks = vi.hoisted(() => ({
  ...
  getOfficialExternalPluginCatalogManifest: vi.fn(
    (entry: { afora?: unknown }) => entry.afora,
  ),
  ...
  resolveDefaultPluginExtensionsDir: vi.fn(() => "/tmp/afora-plugins"),
  resolveDefaultPluginNpmDir: vi.fn(() => "/tmp/afora-npm"),
  resolvePluginNpmProjectsDir: vi.fn((npmDir = "/tmp/afora-npm") =>
  ...
  resolvePluginInstallDir: vi.fn(
    (pluginId: string, extensionsDir = "/tmp/afora-plugins") => `${extensionsDir}/${pluginId}`,
  ),
...
function writeLegacyNpmDeclarationStub(params: {
  ...
  fs.writeFileSync(
    path.join(params.pluginDir, "afora.extension.json"),
    JSON.stringify({
...
function useManifestCatalogResolvers(): void {
  mocks.resolveOfficialExternalPluginId.mockImplementation(
    (entry: { id?: string; afora?: { plugin?: { id?: string } } }) =>
      entry.afora?.plugin?.id ?? entry.id,
  );
  mocks.resolveOfficialExternalPluginInstall.mockImplementation(
    (entry: { install?: unknown; afora?: { install?: unknown } }) =>
      entry.afora?.install ?? entry.install ?? null,
  );
  mocks.resolveOfficialExternalPluginLabel.mockImplementation(
    (entry: { label?: string; afora?: { plugin?: { label?: string } } }) =>
      entry.afora?.plugin?.label ?? entry.label ?? "plugin",
  );
}
...
vi.mock("../../../plugins/doctor-contract-registry.js", async (importOriginal) => ({
  ...
  applyPluginDoctorCompatibilityMigrations: (cfg: AforaConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("repairMissingConfiguredPluginInstalls", () => {
  ...
  beforeEach(() => {
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([]);
    mocks.resolveDefaultPluginExtensionsDir.mockReturnValue("/tmp/afora-plugins");
    mocks.resolveDefaultPluginNpmDir.mockReturnValue("/tmp/afora-npm");
    ...
    mocks.resolveOfficialExternalProviderContractPluginIds.mockImplementation(
      ({ contract, providerIds }: { contract: string; providerIds: ReadonlySet<string> }) => {
        ...
        return entries.flatMap((entry) => {
          ...
          const pluginId = candidate.afora?.plugin?.id ?? candidate.id;
          const ownedProviderIds = candidate.afora?.contracts?.[contract];
          if (
            !pluginId ||
    ...
    mocks.resolveOfficialExternalProviderPluginIds.mockImplementation(
      ({ providerIds }: { providerIds: ReadonlySet<string> }) => {
        ...
        return entries.flatMap((entry) => {
          ...
          const pluginId = candidate.afora?.plugin?.id ?? candidate.id;
          const ownsConfiguredProvider = candidate.afora?.providers?.some((provider) =>
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      targetDir: "/tmp/afora-plugins/matrix",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@afora/plugin-matrix",
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      version: "1.2.3",
      npmResolution: {
        name: "@afora/plugin-matrix",
        version: "1.2.3",
        resolvedSpec: "@afora/plugin-matrix@1.2.3",
  ...
  it("maps a missing configured plugin install to a structured finding and dry-run effect", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          npmSpec: "@afora/plugin-matrix@1.2.3",
    ...
    expect(issue).toEqual({
      ...
      pluginId: "matrix",
      installSpec: "@afora/plugin-matrix@1.2.3",
    });
    expect(
      ...
      severity: "warning",
      target: "matrix",
      fixHint: "Run `afora doctor --fix` to install @afora/plugin-matrix@1.2.3.",
    });
    expect(
  ...
  it("installs a missing configured Afora channel plugin from npm by default", async () => {
    const cfg = {
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: "@afora/plugin-matrix@1.2.3",
      extensionsDir: "/tmp/afora-plugins",
    ...
    expectRecordFields((records as Record<string, unknown>).matrix, {
      source: "npm",
      spec: "@afora/plugin-matrix@1.2.3",
      installPath: "/tmp/afora-plugins/matrix",
      version: "1.2.3",
    });
    ...
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from @afora/plugin-matrix@1.2.3.',
    ]);
  ...
  it("uses an explicit ClawHub install spec before npm", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@afora/plugin-matrix@stable",
          npmSpec: "@afora/plugin-matrix@1.2.3",
          expectedIntegrity: "sha512-test",
        },
      },
    ...
    const clawHubCall = expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: "clawhub:@afora/plugin-matrix@stable",
      expectedPluginId: "matrix",
      config: cfg,
    });
    ...
    expect(mocks.installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from clawhub:@afora/plugin-matrix@stable.',
    ]);
  ...
  it("adds actionable acknowledgement guidance for risky ClawHub candidate failures", async () => {
    mocks.installPluginFromClawHub.mockResolvedValueOnce({
      ...
      error:
        'ClawHub release "@afora/plugin-matrix@stable" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue.',
    });
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "matrix",
    ...
    expect(result.warnings[0]).toContain(
      "afora plugins install clawhub:@afora/plugin-matrix@stable --acknowledge-clawhub-risk",
    );
  });
  ...
  it("adds repair warnings for blocked ClawHub update outcomes", async () => {
    const records = {
      demo: {
        source: "clawhub",
        spec: "clawhub:@afora/plugin-demo@stable",
      ...
      outcomes: [
        {
          ...
          message:
            'Skipped demo ClawHub update: ClawHub release "@afora/plugin-demo@1.2.4" cannot be installed because ClawHub flagged it as blocked or malicious. Review the security details above or choose a different version. Existing installed plugin left unchanged.',
        },
      ],
    });
  ...
  it("sanitizes and shell-quotes ClawHub acknowledgement guidance specs before rendering commands", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@afora/plugin-matrix\n\u001b[31m@stable;$(touch /tmp/pwned)",
        },
      },
    ]);
    ...
    expect(warning).toContain(
      "afora plugins install 'clawhub:@afora/plugin-matrix\\n@stable;$(touch /tmp/pwned)' --acknowledge-clawhub-risk",
    );
  ...
  it("falls back to npm when an Afora channel plugin artifact is unavailable on ClawHub", async () => {
    mocks.installPluginFromClawHub.mockResolvedValueOnce({
      ok: false,
    ...
    expect(result.changes).toEqual([
      'ClawHub clawhub:@afora/plugin-matrix@stable unavailable for "matrix"; falling back to npm @afora/plugin-matrix@1.2.3.',
      'Installed missing configured plugin "matrix" from @afora/plugin-matrix@1.2.3.',
    ]);
    expect(result.warnings).toStrictEqual([]);
  });
  ...
  it("does not fall back from ClawHub to non-Afora npm packages", async () => {
    ...
    expect(result.warnings).toEqual([
      'Failed to install missing configured plugin "matrix" from clawhub:@afora/plugin-matrix@stable: ClawHub artifact download is not available yet.',
    ]);
  });

  it("honors npm-first catalog metadata for missing Afora channel plugins", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce(
      successfulInstall({
        pluginId: "twitch",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@afora/twitch"),
      expectedPluginId: "twitch",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "twitch" from ${expectedNpmInstallSpec("@afora/twitch")}.`,
    ]);
  });
  ...
  it("repairs official plugins at the exact extended-stable core version", async () => {
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: `@afora/diagnostics-otel@${VERSION}`,
    ...
    expectRecordFields(persistedRecords["diagnostics-otel"], {
      ...
      resolvedSpec: `@afora/diagnostics-otel@${VERSION}`,
    });
  });
  ...
  it("does not download configured channel plugins that are still bundled", async () => {
    ...
    mockCurrentBundledPlugin("matrix", "@afora/matrix");
  ...
  it("uses current bundled discovery to remove records before stale snapshots can reinstall official plugins", async () => {
    const records = {
      "google-meet": {
        source: "npm",
        spec: "@afora/google-meet",
        resolvedName: "@afora/google-meet",
      ...
      plugins: [
        {
          id: "google-meet",
          origin: "npm",
          packageName: "@afora/google-meet",
        },
      ],
      diagnostics: [],
    });
    mockCurrentBundledPlugin("google-meet", "@afora/google-meet");
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        id: "google-meet",
        label: "Google Meet",
        install: { npmSpec: "@afora/google-meet" },
        afora: {
          id: "google-meet",
          install: { npmSpec: "@afora/google-meet" },
        },
  ...
  it("installs an official external plugin when only a stale bundled descriptor remains", async () => {
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        id: "discord",
        label: "Discord",
        install: { npmSpec: "@afora/discord", defaultChoice: "npm" },
      },
    ]);
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce(
      successfulInstall({
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@afora/discord"),
    ...
    expect(result.changes).toEqual([
      `Installed missing configured plugin "discord" from ${expectedNpmInstallSpec("@afora/discord")}.`,
    ]);
  });
  ...
  it.each([
    [
      ...
      {
        source: "npm",
        spec: "@afora/matrix-fork",
        resolvedName: "@afora/matrix-fork",
        resolvedSpec: "@afora/matrix-fork@1.2.3",
    ...
    [
      ...
      {
        source: "clawhub",
        spec: "clawhub:@afora/matrix-fork@stable",
        clawhubPackage: "@afora/matrix-fork",
        installPath: "/missing/matrix-fork",
      },
    ],
  ])(
  ...
  it("defers missing external payload repair during the package update doctor pass", async () => {
    ...
    expect(result).toEqual({
      changes: [
        'Skipped package-manager repair for configured plugin "discord" during package update; rerun "afora doctor --fix" after the update completes.',
      ],
      warnings: [],
  ...
  it("updates an existing npm target when stale baseline records miss an installed package", async () => {
    const npmRoot = tempDirs.make("afora-plugin-stub-repair-");
    const packageDir = path.join(npmRoot, "node_modules", "@afora", "discord");
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      npmResolution: {
        name: "@afora/discord",
        version: "1.2.3",
        resolvedSpec: "@afora/discord@1.2.3",
  ...
  it("prefers an existing npm payload over ClawHub during post-core repair", async () => {
    const npmRoot = tempDirs.make("afora-plugin-stub-repair-");
    const packageDir = path.join(npmRoot, "node_modules", "@afora", "matrix");
    fs.mkdirSync(packageDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "@afora/matrix", version: "1.2.3" }),
    );
    mocks.resolveDefaultPluginNpmDir.mockReturnValue(npmRoot);
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@afora/matrix",
          npmSpec: "@afora/matrix",
        },
      },
    ]);
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Plugin "@afora/matrix" requires plugin API >=2026.5.18.',
    });
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      ...
      npmResolution: {
        name: "@afora/matrix",
        version: "1.2.3",
        resolvedSpec: "@afora/matrix@1.2.3",
  ...
  it("passes the post-core compatibility host version to ClawHub repair", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "WhatsApp" },
        install: {
          clawhubSpec: "clawhub:@afora/whatsapp",
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "whatsapp",
      targetDir: "/tmp/afora-plugins/whatsapp",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
    ...
    const result = await repairMissingConfiguredPluginInstalls({
      ...
      env: {
        AFORA_COMPATIBILITY_HOST_VERSION: "2026.5.19",
        AFORA_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    });

    expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: expectedClawHubInstallSpec("clawhub:@afora/whatsapp"),
      env: {
        AFORA_COMPATIBILITY_HOST_VERSION: "2026.5.19",
        AFORA_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    ...
    expectRecordFields(result.records.whatsapp, {
      ...
      installPath: "/tmp/afora-plugins/whatsapp",
      clawhubPackage: "@afora/whatsapp",
    });
  });

  it("repairs missing external payload during post-core convergence even with AFORA_UPDATE_IN_PROGRESS=1", async () => {
    const records = {
      discord: {
        source: "npm",
        spec: "@afora/discord",
  ...
  it("prefers npm over ClawHub during a legacy package update doctor pass", async () => {
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@afora/whatsapp"),
      expectedPluginId: "whatsapp",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "whatsapp" from ${expectedNpmInstallSpec("@afora/whatsapp")}.`,
    ]);
    expectRecordFields(result.records.whatsapp, {
      source: "npm",
      installPath: "/tmp/afora-plugins/whatsapp",
  ...
  it("installs a missing third-party downloadable plugin from npm only", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce(
      successfulInstall({
        pluginId: "wecom",
        npmSpec: "@wecom/wecom-afora-plugin",
        version: "2026.4.23",
        resolution: {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "WeCom" },
        install: {
          npmSpec: "@wecom/wecom-afora-plugin@2026.4.23",
        },
      },
    ]);
    ...
    expectRecordFields(installArg, {
      spec: "@wecom/wecom-afora-plugin@2026.4.23",
      expectedPluginId: "wecom",
    });
    expect(installArg).not.toHaveProperty("trustedSourceLinkedOfficialInstall", true);
    expect(result.changes).toEqual([
      'Installed missing configured plugin "wecom" from @wecom/wecom-afora-plugin@2026.4.23.',
    ]);
  });
  ...
  it("upgrades v2026.7.1-beta.3 Codex Supervisor config and installs Codex", async () => {
    ...
    const cfg = migration.next as AforaConfig;
    ...
    expectRecordFields((records as Record<string, unknown>).codex, {
      source: "npm",
      spec: "@afora/codex",
      installPath: "/tmp/afora-plugins/codex",
      version: "2026.7.2",
      resolvedName: "@afora/codex",
      resolvedSpec: "@afora/codex@2026.7.2",
      integrity: "sha512-codex-supervisor-upgrade",
    });
    expect(mockCallArg(mocks.writePersistedInstalledPluginIndexInstallRecords, 0, 1)).toEqual({
      config: cfg,
  ...
  it("refreshes a stale managed Codex runtime plugin selected by the OpenAI Codex route", async () => {
    const installDir = tempDirs.make("afora-plugin-stub-repair-");
    fs.writeFileSync(
      path.join(installDir, "package.json"),
      JSON.stringify({ name: "@afora/codex", version: "2026.5.6" }),
    );
    const records = {
      codex: {
        source: "npm",
        spec: "@afora/codex",
        resolvedName: "@afora/codex",
        resolvedSpec: "@afora/codex@2026.5.6",
  ...
  it("does not refresh a converged beta Codex runtime plugin on the second doctor pass", async () => {
    ...
    expect(firstPass.changes).toEqual([
      'Refreshed stale configured plugin "codex" from @afora/codex@beta.',
    ]);
    expectRecordFields(firstPass.records.codex, {
      ...
      resolvedSpec: `@afora/codex@${codexBetaVersion}`,
    });
  ...
  it("does not downgrade a newer managed Codex runtime plugin", async () => {
    ...
    fs.writeFileSync(
      path.join(installDir, "package.json"),
      JSON.stringify({ name: "@afora/codex", version: "9999.1.1" }),
    );
    const records = {
      codex: {
        source: "npm",
        spec: "@afora/codex",
        resolvedName: "@afora/codex",
        resolvedSpec: "@afora/codex@9999.1.1",
  ...
  it.each([
    ...
    ["environment runtime override", {}, { AFORA_AGENT_RUNTIME: "codex" }],
  ...
  it.each<{ name: string; plugins: PluginsConfig; installs: boolean }>([
    {
      ...
      plugins: { entries: { "afora-lark": { enabled: true } } },
      installs: false,
    },
    {
      ...
      plugins: {
        ...
        entries: { "afora-lark": { enabled: true } },
      },
      installs: true,
    },
    {
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@afora/feishu"),
      expectedPluginId: "feishu",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "feishu" from ${expectedNpmInstallSpec("@afora/feishu")}.`,
    ]);
  });
        ...
        plugins: {
          installs: {
            demo: {
              source: "npm",
              spec: "@afora/plugin-demo@1.0.0",
              installPath: "/tmp/afora-plugins/demo",
            },
          },
        },
      },
  ...
  it("keeps non-ClawHub updater warnings as persisted-record repair warnings", async () => {
    ...
    const repairWarning =
      'Could not repair afora peer link for "demo" at /tmp/afora-plugins/demo: permission denied';
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
  ...
  it.each([
    {
      ...
      spec: "clawhub:@afora/plugin-demo@1.0.0",
      release: "@afora/plugin-demo@1.0.0",
      expectedSpec: "clawhub:@afora/plugin-demo@1.0.0",
    },
    {
      name: "prefixes legacy persisted ClawHub package records in acknowledgement guidance",
      release: "@afora/plugin-demo@latest",
      expectedSpec: "clawhub:@afora/plugin-demo",
    },
    ...
    expect(result.warnings[0]).toContain(
      `afora plugins install ${expectedSpec} --acknowledge-clawhub-risk`,
    );
  });
  ...
  it("repairs a broken managed package entry from its attributed registry diagnostic", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@afora/plugin-demo@1.0.0",
        resolvedName: "@afora/plugin-demo",
        resolvedSpec: "@afora/plugin-demo@1.0.0",
  ...
  it("reinstalls a known configured plugin from the catalog when its recorded install path is missing", async () => {
    const records = installedRecords("discord", {
      spec: "@afora/discord",
      installPath: "/tmp/afora-missing-discord-install-record",
    });
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      plugins: [
      ...
      diagnostics: [
        {
          ...
          pluginId: "discord",
          message:
            "channel plugin manifest declares discord without channelConfigs metadata; add afora.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads",
        },
      ],
    });
    mocks.updateNpmInstalledPlugins.mockResolvedValue(
  ...
  it.each([
    {
      ...
      pluginId: "brave",
      npmSpec: "@afora/brave-plugin",
      priorSpec: "clawhub:@afora/brave-plugin@2026.5.1-beta.1",
      targetDir: "/tmp/afora-plugins/brave",
      cfg: { tools: { web: { search: { provider: "brave" } } } } satisfies AforaConfig,
    ...
    },
    {
      ...
      pluginId: "slack",
      npmSpec: "@afora/slack",
      priorSpec: "clawhub:@afora/slack@2026.5.12-beta.1",
      targetDir: "/tmp/afora-npm/node_modules/@afora/slack",
    ...
    const extensionsDir = path.join(tempDirs.make("afora-plugin-stub-repair-"), "extensions");
  ...
  it("does not delete an arbitrary recorded path when replacing a broken official plugin", async () => {
    ...
    mockBrokenBraveInstall(installDir, {
      source: "clawhub",
      spec: "clawhub:@afora/brave-plugin@2026.5.1-beta.1",
      clawhubPackage: "@afora/brave-plugin",
      clawhubChannel: "official",
      clawhubUrl: "https://clawhub.ai",
    });
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce(
  ...
  it("keeps a broken official install record when replacement install fails", async () => {
    ...
    expect(result).toEqual({
      changes: [],
      warnings: [
        `Failed to install missing configured plugin "brave" from ${expectedNpmInstallSpec("@afora/brave-plugin")}: network unavailable`,
      ],
  ...
  it("installs configured external speech and web-fetch plugins from selected providers", async () => {
    const packages = [
      ["firecrawl", "@afora/firecrawl-plugin"],
      ["gradium", "@afora/gradium-speech"],
      ["inworld", "@afora/inworld-speech"],
    ] as const;
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue(
  ...
  it.each([
    ...
    },
    {
      ...
      pluginId: "llama-cpp",
      npmSpec: "@afora/llama-cpp-provider",
      version: "2026.6.2",
      entry: {
        id: "llama-cpp",
    ...
    },
    {
      ...
      entry: {
        id: "acpx",
        label: "ACPX Runtime",
        install: { npmSpec: "@afora/acpx", defaultChoice: "npm" as const },
      },
  ...
  it("installs env-only web provider plugins before auto-detection", async () => {
    const packages = [
      ["exa", "@afora/exa-plugin", "EXA_API_KEY"],
      ["firecrawl", "@afora/firecrawl-plugin", "FIRECRAWL_API_KEY"],
    ] as const;
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue(
  ...
  it("installs env-only provider plugins before model discovery", async () => {
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@afora/groq-provider"),
      expectedPluginId: "groq",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "groq" from ${expectedNpmInstallSpec("@afora/groq-provider")}.`,
    ]);
  });
  ...
  it("installs configured external web search plugins from beta on the beta channel", async () => {
    ...
    expect(result.changes).toEqual([
      'Installed missing configured plugin "brave" from @afora/brave-plugin@beta.',
    ]);
  });

  it("repairs a configured plugin from a legacy npm declaration stub", async () => {
    const root = tempDirs.make("afora-plugin-stub-repair-");
    ...
    expectRecordFields((records as Record<string, unknown>)["guardrail-bridge"], {
      source: "npm",
      spec: "@guardrail-bridge/guardrail-bridge@1.0.0",
      installPath: "/tmp/afora-plugins/guardrail-bridge",
      version: "1.0.0",
  ...
  it("installs Firecrawl for env-only web fetch when search is disabled", async () => {
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      officialPluginEntry({
        id: "firecrawl",
        npmSpec: "@afora/firecrawl-plugin",
        label: "Firecrawl",
        manifest: {},
      }),
    ]);
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@afora/firecrawl-plugin"),
      expectedPluginId: "firecrawl",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "firecrawl" from ${expectedNpmInstallSpec("@afora/firecrawl-plugin")}.`,
    ]);
  });
