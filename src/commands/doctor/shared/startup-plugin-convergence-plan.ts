...
function hasPotentialPluginConfig(config: AforaConfig, env: NodeJS.ProcessEnv): boolean {
...
function collectConfiguredMemoryEmbeddingProviderIds(config: AforaConfig): ReadonlySet<string> {
...
function hasConfiguredCapabilityPlugin(config: AforaConfig, env: NodeJS.ProcessEnv): boolean {
