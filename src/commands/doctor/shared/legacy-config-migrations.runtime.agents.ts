...
const MEMORY_SEARCH_RULE: LegacyConfigRule = {
  path: ["memorySearch"],
  message:
    'top-level memorySearch was moved; use memory.search instead. Run "afora doctor --fix".',
};

const AGENT_MEMORY_SEARCH_OWNER_RULES: LegacyConfigRule[] = [
  {
    path: ["agents", "defaults", "memorySearch"],
    message: 'agents.defaults.memorySearch moved to memory.search. Run "afora doctor --fix".',
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].memorySearch moved to agents.list[].memory.search. Run "afora doctor --fix".',
...
const LEGACY_MEMORY_SEARCH_AUTO_PROVIDER_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'memorySearch.provider = "auto" is legacy; use "openai" explicitly. Run "afora doctor --fix".',
    match: isLegacyMemorySearchAutoProvider,
  },
  {
    path: ["memory", "search", "provider"],
    message:
      'memory.search.provider = "auto" is legacy; use "openai" explicitly. Run "afora doctor --fix".',
    match: isLegacyMemorySearchAutoProvider,
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].memorySearch.provider = "auto" is legacy; use "openai" explicitly. Run "afora doctor --fix".',
    match: (value) =>
...
const LEGACY_MEMORY_SEARCH_STORE_PATH_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'memorySearch.store.path is legacy; memory indexes now live in each agent database. Run "afora doctor --fix".',
  },
  {
    path: ["memory", "search", "store", "path"],
    message:
      'memory.search.store.path is legacy; memory indexes now live in each agent database. Run "afora doctor --fix".',
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].memorySearch.store.path is legacy; memory indexes now live in each agent database. Run "afora doctor --fix".',
    match: (value) =>
...
const LEGACY_MEMORY_SEARCH_FLAT_KEY_RULES: LegacyConfigRule[] = [
  {
    path: ["memory", "search"],
    message:
      'memory.search uses legacy flat chunkSize, chunkOverlap, or maxResults fields. Run "afora doctor --fix".',
    match: hasLegacyMemorySearchFlatKeys,
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].memorySearch uses legacy flat chunkSize, chunkOverlap, or maxResults fields. Run "afora doctor --fix".',
    match: (value) =>
      someAgentList(value, (agent) =>
...
const LEGACY_SANDBOX_SCOPE_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope instead. Run "afora doctor --fix".',
    match: (value) => hasLegacySandboxPerSession(value),
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].sandbox.perSession is legacy; use agents.list[].sandbox.scope instead. Run "afora doctor --fix".',
...
const UNSUPPORTED_SANDBOX_BROWSER_NETWORK_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.sandbox.browser.network = "none" cannot expose the browser control port. Run "afora doctor --fix" to disable the sidecar and restore the dedicated browser network.',
    match: isUnsupportedSandboxBrowserNetwork,
  },
  {
    path: ["agents", "entries"],
    message:
      'agents.entries.*.sandbox.browser.network = "none" cannot expose the browser control port. Run "afora doctor --fix" to disable the affected sidecar and restore the dedicated browser network.',
    match: hasAgentEntriesUnsupportedSandboxBrowserNetwork,
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].sandbox.browser.network = "none" cannot expose the browser control port. Run "afora doctor --fix" to disable the affected sidecar and restore the dedicated browser network.',
    match: (value) =>
      someAgentList(value, (agent) =>
...
const LEGACY_AGENT_RUNTIME_POLICY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.agentRuntime is ignored; set models.providers.<provider>.agentRuntime or a model-scoped agentRuntime instead. Run "afora doctor --fix".',
  },
  {
    path: ["agents", "defaults", "embeddedHarness"],
    message:
      'agents.defaults.embeddedHarness is legacy and ignored; set provider/model runtime policy instead. Run "afora doctor --fix".',
  ...
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].agentRuntime is ignored; set provider/model runtime policy instead. Run "afora doctor --fix".',
    match: (value) => someAgentList(value, (agent) => getRecord(agent.agentRuntime) !== null),
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].embeddedHarness is legacy and ignored; set provider/model runtime policy instead. Run "afora doctor --fix".',
...
const DEPRECATED_EMBEDDED_AGENT_KEY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.embeddedPi is legacy; use agents.defaults.embeddedAgent instead. Run "afora doctor --fix".',
    match: (value) => getRecord(value) !== null,
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].embeddedPi is legacy; use agents.list[].embeddedAgent instead. Run "afora doctor --fix".',
...
const LEGACY_AGENT_LLM_TIMEOUT_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.llm is legacy; use models.providers.<id>.timeoutSeconds for slow model/provider timeouts. Run "afora doctor --fix".',
    match: (value) => getRecord(value) !== null,
  },
];
...
const IGNORED_AGENT_MODEL_TIMEOUT_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.model.timeoutMs is ignored; agent model config only selects primary/fallback models. Run "afora doctor --fix" to remove it.',
    match: (value) => hasOwnTimeoutMs(value),
  },
  {
    path: ["agents", "defaults", "subagents", "model"],
    message:
      'agents.defaults.subagents.model.timeoutMs is ignored; subagent model config only selects primary/fallback models. Run "afora doctor --fix" to remove it.',
    match: (value) => hasOwnTimeoutMs(value),
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].model.timeoutMs and agents.list[].subagents.model.timeoutMs are ignored; agent model config only selects primary/fallback models. Run "afora doctor --fix" to remove them.',
    match: (value) =>
      someAgentList(
        value,
        (agent) =>
...
const PROFILE_CONFIGURED_TOOL_SECTION_RULES: LegacyConfigRule[] = [
  {
    path: ["tools"],
    message:
      'tools.profile filters explicit configured-section tool grants; run "afora doctor --fix" to rewrite the explicit grants into a valid allowlist.',
    match: (value) => toolProfileConfiguredSectionsNeedExplicitRepair(value),
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].tools.profile filters explicit configured-section tool grants; run "afora doctor --fix" to rewrite the explicit grants into a valid allowlist.',
    match: (value, root) => {
...
const SILENT_REPLY_LEGACY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.silentReplyRewrite was removed; exact NO_REPLY is no longer rewritten to visible fallback text. Run "afora doctor --fix" to remove it.',
  },
  {
    path: ["agents", "defaults", "silentReply"],
    message:
      'agents.defaults.silentReply.direct was removed; direct chats never receive NO_REPLY prompt guidance. Run "afora doctor --fix" to remove it.',
    match: (value) => Object.hasOwn(getRecord(value) ?? {}, "direct"),
  },
  {
    path: ["surfaces"],
    message:
      'surfaces.*.silentReplyRewrite was removed; exact NO_REPLY is no longer rewritten to visible fallback text. Run "afora doctor --fix" to remove it.',
    match: (value) => hasSurfaceSilentReplyRewrite(value),
  },
  {
    path: ["surfaces"],
    message:
      'surfaces.*.silentReply.direct was removed; direct chats never receive NO_REPLY prompt guidance. Run "afora doctor --fix" to remove it.',
...
const SYSTEM_PROMPT_OVERRIDE_LEGACY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.systemPromptOverride was removed; Afora owns the generated system prompt. Run "afora doctor --fix" to remove it.',
  },
  {
    path: ["agents", "list"],
    message:
      'agents.list[].systemPromptOverride was removed; Afora owns the generated system prompt. Run "afora doctor --fix" to remove it.',
...
function resolveLegacyAgentRuntimeIntent(raw: unknown): LegacyAgentRuntimeIntent | undefined {
  ...
  if (!runtime || runtime === "auto" || runtime === "afora") {
    return undefined;
  }
    ...
    legacyRules: [
      {
        ...
        message:
          'session.typingMode moved to agents.defaults.typingMode. Run "afora doctor --fix".',
      },
    ],
    apply: (raw, changes) => {
