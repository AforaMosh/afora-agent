...
describe("stale subagent allowlist doctor repair", () => {
  ...
  it("keeps wildcard, configured Afora agents, and configured ACP targets", () => {
    const cfg = {
      acp: {
        defaultAgent: "claude",
        allowedAgents: ["codex"],
  ...
  it("formats preview warnings with the doctor fix command", () => {
    ...
    expect(warnings).toStrictEqual([
      ...
      '- Run "afora doctor --fix" to remove stale subagent target ids, or add a configured agent or ACP target for each intended target.',
    ]);
  });
});
