...
const CRON_RUN_LOG_RULE: LegacyConfigRule = {
  path: ["cron", "runLog"],
  message:
    'cron.runLog is retired; run history now has fixed per-job retention. Run "afora doctor --fix".',
};

const CRON_WEBHOOK_RULE: LegacyConfigRule = {
  path: ["cron", "webhook"],
  message:
    'cron.webhook was retired after per-job delivery migration. Run "afora doctor --fix".',
};
