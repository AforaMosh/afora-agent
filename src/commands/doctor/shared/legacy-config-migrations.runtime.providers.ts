...
const X_SEARCH_RULE: LegacyConfigRule = {
  ...
  message:
    'tools.web.x_search.apiKey moved to the xAI plugin; use plugins.entries.xai.config.webSearch.apiKey instead. Run "afora doctor --fix".',
};

const X_SEARCH_MODEL_RULE: LegacyConfigRule = {
  path: ["tools", "web", "x_search", "model"],
  message:
    'tools.web.x_search.model uses a retired xAI model; run "afora doctor --fix" to repair it.',
    ...
    legacyRules: [
      {
        path: ["plugins"],
        message:
          'plugins.entries.codex-supervisor and related plugin policy references are retired; use plugins.entries.codex.config.supervision. Run "afora doctor --fix".',
        requireSourceLiteral: true,
        match: (_value, root) =>
    ...
    legacyRules: [
      {
        path: ["plugins"],
        message:
          'plugins.openai-codex references are retired; use the openai plugin id. Run "afora doctor --fix".',
        requireSourceLiteral: true,
        match: (_value, root) =>
