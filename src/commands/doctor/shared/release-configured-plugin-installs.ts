...
function isPluginsGloballyDisabled(cfg: AforaConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: AforaConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: AforaConfig): string[] {
...
function isPluginEntryDisabled(cfg: AforaConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: AforaConfig, channelId: string): boolean {
  const channels = asNullableRecord(cfg.channels);
  const entry = asNullableRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: AforaConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: AforaConfig): string[] {
  const entries = asNullableRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: AforaConfig): string[] {
...
function collectConfiguredChannelIds(cfg: AforaConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: AforaConfig): string[] {
...
function collectEnvWebSearchPluginIds(cfg: AforaConfig, env: NodeJS.ProcessEnv): string[] {
  if (cfg.tools?.web?.search?.enabled === false) {
    return [];
  }
...
function collectWebFetchPluginIds(cfg: AforaConfig): string[] {
...
function collectEnvWebFetchPluginIds(cfg: AforaConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectSpeechPluginIds(cfg: AforaConfig): string[] {
  return resolveOfficialExternalProviderContractPluginIds({
...
function collectAcpRuntimePluginIds(cfg: AforaConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: AforaConfig): string[] {
...
function addEligiblePluginId(cfg: AforaConfig, pluginIds: Set<string>, pluginId: string): void {
...
function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareAforaVersions(
  ...
  const touchedComparedToRelease = compareAforaVersions(params.touchedVersion, releaseVersion);
