...
const APPROVALS_DISABLED_SENTINEL = "afora:approval-disabled";
...
const QQBOT_EXTERNALIZATION_RULES: LegacyConfigRule[] = [
  {
    path: [],
    message:
      'Environment-only QQBot credentials need a safe Tencent QQBot 2.0 config shell. Run "afora doctor --fix".',
    match: (_value, root) => shouldCreateEnvironmentOnlyQQBotConfig(root),
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot defaultAccount/accounts.default must migrate to Tencent QQBot 2.0 account selection. Run "afora doctor --fix".',
    match: (value) => {
      const qqbot = getRecord(value);
      return Boolean(
        qqbot &&
  ...
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot clientSecretFile must migrate to a file-backed SecretRef for Tencent QQBot 2.0. Run "afora doctor --fix".',
    match: (value) => hasQQBotEntryMatching(value, (entry) => hasOwnKey(entry, "clientSecretFile")),
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot wildcard/empty allowFrom must be separated from Tencent QQBot 2.0 native approval access. Run "afora doctor --fix".',
    match: (value) =>
  ...
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot chat allowFrom must be reconciled with the previous commands.allowFrom approval operators for Tencent QQBot 2.0. Run "afora doctor --fix".',
    match: (value, root) => {
  ...
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot groups.*.commandLevel must migrate before Tencent QQBot 2.0 can safely handle group commands. Run "afora doctor --fix".',
    match: (value) => hasQQBotEntryMatching(value, hasLegacyGroupCommandLevel),
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot streaming.nativeTransport/c2cStreamApi must migrate to Tencent QQBot 2.0 streaming.mode. Run "afora doctor --fix".',
    match: (value) => hasQQBotEntryMatching(value, hasLegacyStreamingTransport),
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot allowFrom IDs must migrate to Tencent QQBot 2.0 canonical uppercase OpenIDs. Run "afora doctor --fix".',
    match: (value) =>
  ...
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot execApprovals must migrate to Tencent QQBot 2.0 allowFrom semantics. Run "afora doctor --fix".',
    match: (value) => hasQQBotEntryMatching(value, (entry) => hasOwnKey(entry, "execApprovals")),
  },
  {
    path: ["channels", "qqbot"],
    message:
      'QQBot group tools policies must migrate to Tencent QQBot 2.0 toolPolicy. Run "afora doctor --fix".',
    match: (value) =>
      hasQQBotEntryMatching(value, (entry) => {
        const groups = getRecord(entry.groups);
        return Boolean(
