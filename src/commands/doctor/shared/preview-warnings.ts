...
import { isRecord as hasRecord } from "@afora/normalization-core/record-coerce";
...
function listAgentRecords(cfg: AforaConfig): Record<string, unknown>[] {
  return listAgentEntries(cfg).filter(hasRecord);
}

function hasChannels(cfg: AforaConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: AforaConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: AforaConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: AforaConfig): boolean {
...
function hasConfiguredSafeBins(cfg: AforaConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: AforaConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: AforaConfig): {
...
function collectVisibleReplyToolPolicyWarnings(cfg: AforaConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Afora falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Afora falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
function collectChannelBoundMessageToolPolicyWarnings(cfg: AforaConfig): string[] {
...
function collectProfileConfiguredToolSectionWarnings(cfg: AforaConfig): string[] {
  const warnings: string[] = [];
...
export async function resolveDoctorChannelPreviewConfig(params: {
  ...
  allowExec?: boolean;
}): Promise<{ cfg: AforaConfig; diagnostics: string[] }> {
