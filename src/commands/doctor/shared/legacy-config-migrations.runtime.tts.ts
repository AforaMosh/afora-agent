...
const LEGACY_TTS_PROVIDER_RULES: LegacyConfigRule[] = [
  {
    path: ["tts"],
    message:
      'tts legacy provider aliases/keys are legacy; use provider: "microsoft" and tts.providers.<provider>. Run "afora doctor --fix".',
    match: (value) => hasLegacyTtsProviderKeys(value),
  },
  {
    path: ["plugins", "entries"],
    message:
      'plugins.entries.voice-call.config.tts legacy provider aliases/keys are legacy; use provider: "microsoft" and plugins.entries.voice-call.config.tts.providers.<provider>. Run "afora doctor --fix".',
...
const LEGACY_TTS_ENABLED_RULES: LegacyConfigRule[] = [
  {
    path: ["tts"],
    message: 'tts.enabled is legacy; use tts.auto. Run "afora doctor --fix".',
    match: (value) => hasLegacyTtsEnabled(value),
  },
  {
    path: ["agents"],
    message:
      'agents.list[].tts.enabled is legacy; use agents.list[].tts.auto. Run "afora doctor --fix".',
    match: (value) => hasLegacyTtsInAgentLocations(value, hasLegacyTtsEnabled),
  },
  {
    path: ["channels"],
    message:
      'supported channel TTS enabled fields are legacy; use the same TTS block auto field. Run "afora doctor --fix".',
    match: (value) => hasLegacyTtsInChannelLocations(value, hasLegacyTtsEnabled),
  },
  {
    path: ["plugins", "entries"],
    message:
      'plugins.entries.voice-call.config.tts.enabled is legacy; use plugins.entries.voice-call.config.tts.auto. Run "afora doctor --fix".',
...
const LEGACY_TTS_SPEAKER_SELECTION_RULES: LegacyConfigRule[] = [
  {
    path: ["tts"],
    message:
      'tts speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run "afora doctor --fix".',
    match: (value) => hasLegacyTtsSpeakerSelection(value),
  },
  {
    path: ["agents"],
    message:
      'agents.list[].tts speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run "afora doctor --fix".',
    match: (value) => hasLegacyTtsInAgentLocations(value, hasLegacyTtsSpeakerSelection),
  },
  {
    path: ["channels"],
    message:
      'supported channel TTS speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run "afora doctor --fix".',
    match: (value) => hasLegacyTtsInChannelLocations(value, hasLegacyTtsSpeakerSelection),
  },
  {
    path: ["plugins", "entries"],
    message:
      'plugins.entries.voice-call.config.tts speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run "afora doctor --fix".',
...
export const LEGACY_CONFIG_MIGRATIONS_RUNTIME_TTS: LegacyConfigMigrationSpec[] = [
  defineLegacyConfigMigration({
    ...
    legacyRules: [
      {
        path: ["messages", "tts"],
        message: 'messages.tts moved to top-level tts. Run "afora doctor --fix".',
      },
    ],
    apply: (raw, changes) => {
