...
import { asOptionalRecord as asMutableRecord } from "@afora/normalization-core/record-coerce";
import {
  normalizeFastMode,
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `afora doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.repairBlocked
    ...
    : "- Run `afora doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an Afora runtime policy.";
  return [
...
function collectCodexAppServerCommandWarnings(cfg: AforaConfig): string[] {
...
function modelUsesCodexForEveryAgent(cfg: AforaConfig, modelRef: string): boolean {
...
function formatCodexModelParamWarning(hits: readonly CodexModelParamHit[]): string {
  const fixHint = hits.some((hit) => hit.removable)
    ? '- Run `afora doctor --fix` to remove only redundant priority service-tier params; remove any remaining params or set the affected route\'s agentRuntime.id to "afora".'
    : '- Remove these params or set the affected route\'s agentRuntime.id to "afora"; Doctor cannot migrate them without changing behavior.';
  return [
...
function repairRedundantCodexServiceTiers(cfg: AforaConfig, env?: NodeJS.ProcessEnv) {
...
function collectCodexComputerUseWarnings(cfg: AforaConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
  const codex = asMutableRecord(entries?.codex);
  const config = asMutableRecord(codex?.config);
...
export function maybeRepairCodexRoutes(params: {
  ...
  blockedProviderPlan?: BlockedLegacyOpenAICodexProviderPlan;
}): { cfg: AforaConfig; warnings: string[]; changes: string[] } {
  const env = params.env ?? process.env;
  const blockedProviderPlan =
    params.blockedProviderPlan ?? collectBlockedLegacyOpenAICodexProviderPlan(params.cfg);
  const blockedModelIdentities = new Set(blockedProviderPlan.blockedModelIdentities);
