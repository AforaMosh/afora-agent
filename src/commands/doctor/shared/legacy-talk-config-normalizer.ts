...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<AforaConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as AforaConfig["talk"])?.realtime;
}

/** Normalize legacy Talk provider/realtime fields into current talk.providers and talk.realtime. */
export function normalizeLegacyTalkConfig(cfg: AforaConfig, changes: string[]): AforaConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as AforaConfig["talk"]) ?? {};
