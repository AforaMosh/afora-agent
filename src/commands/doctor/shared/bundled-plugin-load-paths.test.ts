...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "afora-agent/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/afora";
  ...
  it("removes stale bundled paths from old versioned Afora package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "afora");
  ...
  it("removes stale legacy bundled paths from old versioned Afora package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "afora");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; Afora loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "afora doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "afora", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
