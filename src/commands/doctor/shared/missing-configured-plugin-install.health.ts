...
const CONFIGURED_PLUGIN_INSTALL_ISSUE_DETAILS = {
  ...
  "missing-installed-payload": {
    ...
    fixHint: "Run `afora doctor --fix` to reinstall the configured plugin package.",
  ...
  "repairable-installed-plugin": {
    ...
    fixHint: "Run `afora doctor --fix` to repair the configured plugin package.",
    action: "would-repair-configured-plugin-install",
    dryRunSafe: false,
  },
  "stale-version-bound-runtime": {
    message: (pluginId: string) =>
      `Configured runtime plugin ${pluginId} is older than this Afora version.`,
    fixHint: "Run `afora doctor --fix` to refresh the configured runtime plugin.",
  ...
  "stale-channel-config-descriptor": {
    ...
    fixHint: "Run `afora doctor --fix` to repair the configured plugin install metadata.",
    action: "would-repair-configured-plugin-install",
    dryRunSafe: false,
  },
  "deferred-package-manager-repair": {
    ...
    fixHint: "Rerun `afora doctor --fix` after the package update completes.",
  ...
  return {
    ...
    fixHint:
      issue.kind === "missing-install-record"
        ? `Run \`afora doctor --fix\` to install ${issue.installSpec}.`
        : detail.fixHint,
  };
}
