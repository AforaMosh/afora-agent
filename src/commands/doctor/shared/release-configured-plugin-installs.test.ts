...
describe("configured plugin install release step", () => {
  ...
  it("does not stamp config during update-time deferred install repair", async () => {
    mocks.repairMissingPluginInstallsForIds.mockResolvedValue({
      changes: [
        'Skipped package-manager repair for configured plugin "codex" during package update; rerun "afora doctor --fix" after the update completes.',
      ],
      warnings: [],
