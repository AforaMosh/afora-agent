...
const MCP_SERVER_TYPE_RULE: LegacyConfigRule = {
  path: ["mcp", "servers"],
  message:
    'mcp.servers entries use Afora transport names; CLI-native type aliases are legacy here. Run "afora doctor --fix".',
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_DISABLED_RULES: LegacyConfigRule[] = [
  ...
  message:
    ...
    'Run "afora doctor --fix" to migrate it.',
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_TIMEOUT_ALIASES_RULES: LegacyConfigRule[] = [
  ...
  path,
  message: `${path.join(".")} timeout aliases were retired; use connectionTimeoutMs and requestTimeoutMs. Run "afora doctor --fix".`,
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_ALIASES_RULES: LegacyConfigRule[] = [
  ...
  path,
  message: `${path.join(".")} legacy aliases were retired; use camelCase spellings and cwd. Run "afora doctor --fix".`,
  match: (value) =>
    isRecord(value) &&
  ...
  defineLegacyConfigMigration({
    ...
    apply: (raw, changes) => {
      ...
      for (const [serverName, rawServer] of Object.entries(servers)) {
        ...
        const alias = resolveAforaMcpTransportAlias(rawServer.type);
