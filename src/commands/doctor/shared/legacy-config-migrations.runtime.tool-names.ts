  ...
  TOOL_NAME_MIGRATIONS.map((migration) =>
    defineLegacyConfigMigration({
      ...
      legacyRules: TOOL_POLICY_ROOTS.map((root) => ({
        path: [root],
        message: `Tool policies still rely on legacy ${migration.migration.legacyName} coverage; run "afora doctor --fix" to preserve equivalent ${migration.migration.canonicalName} access.`,
