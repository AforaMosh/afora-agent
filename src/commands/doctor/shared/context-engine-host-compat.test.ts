...
vi.mock("../../../agents/harness/policy.js", () => ({
  resolveAgentHarnessPolicy: vi.fn(
    (params: { config: AforaConfig; modelId: string; provider: string }) => ({
      runtime:
...
function configWithEngine(engineId: string, cfg: AforaConfig = {}): AforaConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
...
describe("doctor context-engine host compatibility", () => {
  ...
  it("evaluates native Codex and Afora agent-run hosts", async () => {
    ...
    expect(warnings.join("\n")).toContain("Afora embedded runner");
    ...
    const warnings = await collectContextEngineHostCompatibilityWarnings({
      cfg: configWithEngine(engineId, {
        agents: {
          ...
          entries: {
            main: { agentDir: "/tmp/afora-doctor-host-compat" },
            helper: {},
            third: {},
          },
        },
