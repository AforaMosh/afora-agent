...
async function writeLegacySidecarThatNeedsKeychain(): Promise<{
  ...
  ref: { source: "afora-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
}> {
  const state = await createAforaTestState({
    layout: "state-only",
    prefix: "afora-legacy-oauth-keychain-warn-",
    env: {
      AFORA_AGENT_DIR: undefined,
      AFORA_AUTH_PROFILE_SECRET_KEY: undefined,
    },
  });
  states.push(state);
...
describe("loadLegacyOAuthSidecarMaterial keychain-only headless warning", () => {
  ...
  function envWithoutVitestSignals(state: AforaTestState): NodeJS.ProcessEnv {
  ...
  it("emits one doctor-pointer warning only on Darwin", async () => {
    ...
    expect(String(firstMessage)).toContain("afora doctor --fix");
