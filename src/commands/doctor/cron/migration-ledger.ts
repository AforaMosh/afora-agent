...
type CronMigrationDatabase = Pick<AforaStateDatabase, "migration_runs" | "migration_sources">;
...
export function hasLegacyCronMigrationReceipt(source: LegacyCronMigrationSource): boolean {
  return hasLegacyCronMigrationReceiptInDatabase(openAforaStateDatabase().db, source);
}
...
export function hasLegacyCronMigrationReceiptReadOnly(source: LegacyCronMigrationSource): boolean {
  const statePath = resolveAforaStateSqlitePath(process.env);
