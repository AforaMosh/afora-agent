...
function throwPluginRegistryPersistenceFailed(reason: string): never {
  throw new Error(
    `Afora refreshed the plugin registry but could not verify the persisted replacement (${reason}); refusing to write the migration checkpoint. Run "afora doctor --fix" and retry.`,
  );
}
