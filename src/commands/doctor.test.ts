...
const mocks = vi.hoisted(() => ({
  ...
  resolveInstalledPluginIndexStorePath: vi.fn(() => "/tmp/afora-installed-plugins.json"),
}));
...
describe("doctorCommand", () => {
  ...
  it("binds explicit destructive session stores to the maintenance lock", async () => {
    ...
    const stateDir = path.resolve(process.env.AFORA_STATE_DIR ?? ".afora");
  ...
  it("creates a GitHub issue for approved session sqlite recovery reports", async () => {
    const supportIssue = {
      ...
      url: "https://github.com/AforaMosh/afora-agent/issues/new?title=run-1",
    };
    const report = {
      mode: "recover",
      supportIssue,
    ...
    mocks.createSessionSqliteGithubIssue.mockReturnValueOnce({
      ok: true,
      url: "https://github.com/AforaMosh/afora-agent/issues/123",
    });
    const runtime = {
      log: vi.fn(),
      error: vi.fn(),
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "session-sqlite recover: created GitHub issue https://github.com/AforaMosh/afora-agent/issues/123",
    );
    expect(runtime.exit).toHaveBeenCalledWith(0);
  });
