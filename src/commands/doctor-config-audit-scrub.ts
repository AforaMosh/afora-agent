...
export function configAuditScrubToHealthFinding(
  ...
  return {
    ...
    fixHint:
      "Run `afora doctor --fix` to rewrite argv/execArgv fields through the current redactor.",
  };
}
...
export async function maybeScrubConfigAuditLog(params: {
  ...
  try {
    if (params.shouldRepair) {
      ...
      if (result.aborted) {
        note(
          "Config audit scrub was aborted because new entries were appended to config-audit.jsonl during the rewrite. No records were modified. Stop the gateway (or wait until it is idle) and rerun `afora doctor --fix`.",
          NOTE_TITLE,
        );
        return;
      }
    ...
    if (preview.rewritten > 0) {
      const fixCommand = params.doctorFixCommand ?? "afora doctor --fix";
      note(
