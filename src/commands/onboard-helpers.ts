...
export function summarizeExistingConfig(config: AforaConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: AforaConfig): string | null {
...
/** Prints the onboarding banner: pixel mascot beside the AFORA wordmark. */
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    "Docs:",
    "https://docs.afora.ai/gateway/remote",
    "https://docs.afora.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
  ...
  if (
    ...
    throw new Error(
      "Full reset workspace overlaps the active onboarding lock directory. " +
        "Choose a workspace outside the Afora state migration directory or use a narrower reset scope.",
    );
  }
}
...
export async function probeGatewayConfiguredModel(
  ...
  try {
    const config = configCandidate as AforaConfig;
