 ...
 */
function stripInheritedAgentLocations(base: AforaConfig): AforaConfig {
 ...
 */
function buildExecConfigDefaults(): AforaConfig {
  return {
...
export function buildExecRunConfig(params: {
  ...
  const withDefaults = mergeDeep(buildExecConfigDefaults(), base) as AforaConfig;
  return mergeDeep(
    withDefaults,
...
function setAgentExecEnvironment(params: { stateDir: string; cwd: string }): () => void {
  ...
  const previousWorkspaceDir = process.env.AFORA_WORKSPACE_DIR;
  process.env.AFORA_STATE_DIR = params.stateDir;
  delete process.env.AFORA_CONFIG_PATH;
  process.env.AFORA_WORKSPACE_DIR = params.cwd;
  return () => {
    ...
    if (previousWorkspaceDir === undefined) {
      delete process.env.AFORA_WORKSPACE_DIR;
    } else {
      process.env.AFORA_WORKSPACE_DIR = previousWorkspaceDir;
    }
  };
}

function formatActiveGatewayExecRefusal(identity: GatewayLockIdentity): string {
  return `A Gateway is running for this state directory (pid ${identity.pid}, port ${identity.port}). Omit --state-dir to use isolated temporary state, or stop the Gateway first (${formatCliCommand("afora gateway stop")}).`;
}
...
export async function agentExecCommand(
  ...
  try {
    ...
    const stateDir = opts.stateDir
      ...
      : await fs.mkdtemp(path.join(os.tmpdir(), "afora-agent-exec-"));
    ...
    // AFORA_STATE_DIR moves so discovery and the installed-index DB agree.
    ...
    // AFORA_CONFIG_PATH would only feed this same snapshot, while writing
