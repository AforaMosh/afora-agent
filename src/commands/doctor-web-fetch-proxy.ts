...
const DIRECT_PROBE_HOST = "docs.afora.ai";
...
async function collectWebFetchProxyDiagnostic(params: {
  ...
  return [
    ...
    `  ${formatCliCommand("afora config set tools.web.fetch.useTrustedEnvProxy true")}`,
    "- Keep the opt-in disabled for untrusted proxies; enabling it lets the proxy resolve DNS after Afora's hostname checks.",
  ].join("\n");
}
