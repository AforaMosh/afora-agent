...
describe("resolveGatewayAuthTokenForService", () => {
  ...
  it("skips exec SecretRefs by default for service token checks", async () => {
    const tmp = await fs.mkdtemp(join(tmpdir(), "afora-service-token-exec-ref-"));
  ...
  it("falls back to AFORA_GATEWAY_TOKEN when SecretRef is unresolved", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
      ...
      {
        AFORA_GATEWAY_TOKEN: "env-fallback-token",
      } as NodeJS.ProcessEnv,
    );
  ...
  it("falls back to AFORA_GATEWAY_TOKEN when SecretRef resolves to empty", async () => {
    const resolved = await resolveGatewayAuthTokenForService(
      {
        gateway: {
          auth: {
...
describe("shouldRequireGatewayTokenForInstall", () => {
  ...
  it("requires token in inferred mode when password env exists only in shell", async () => {
    await withEnvAsync(
      { [envVar("AFORA", "GATEWAY", "PASSWORD")]: "password-from-env" },
      async () => {
    ...
    const required = shouldRequireGatewayTokenForInstall(
      {
        ...
        env: {
          vars: {
            AFORA_GATEWAY_PASSWORD: "configured-password", // pragma: allowlist secret
          },
        },
      } as AforaConfig,
      {} as NodeJS.ProcessEnv,
  ...
  it("does not require token in inferred mode when password env exists in state-dir .env", async () => {
    await withTempHome(async (_home) => {
      await writeStateDirDotEnv("AFORA_GATEWAY_PASSWORD=dotenv-password\n", {
        env: process.env,
      });
