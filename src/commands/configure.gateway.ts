...
import { validateDottedDecimalIPv4Input } from "@afora/net-policy/ipv4";
import {
...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "AFORA_GATEWAY_TOKEN",
          placeholder: "AFORA_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = normalizeOptionalString(value) ?? "";
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like AFORA_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Afora will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Afora trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.afora.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
