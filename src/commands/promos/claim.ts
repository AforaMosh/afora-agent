...
async function fetchLivePromotion(slug: string): Promise<ClawHubPromotion> {
  try {
    ...
    if (error instanceof ClawHubRequestError && error.status === 404) {
      throw new Error(
        `Promotion "${slug}" was not found or is not live. See ${formatCliCommand("afora promos list")}.`,
        { cause: error },
      );
    }
    throw error;
...
function requireUnchangedClaimContract(
  ...
  throw new Error(
    `Promotion "${initial.slug}" changed while the claim was in progress; no promotional models were added. Any provider credentials you just configured were kept. Run ${formatCliCommand("afora promos list")} and retry.`,
  );
}
...
function resolveManifestPluginPackageNames(pluginId: string, cfg: AforaConfig): string[] {
...
function resolveAuthChoice(
  ...
  if (!entry) {
    throw new Error(
      `Promotion "${promotion.slug}" requires auth choice "${authChoiceId}", which this Afora version does not know. Update Afora and retry.`,
    );
  }
...
function requirePromotionPlugins(
  ...
  throw new Error(
    `Promotion "${promotion.slug}" requires plugin package "${unsupported[0]}", but ${authChoiceLabel} does not provide it in this Afora version. Update Afora and retry.`,
  );
}
...
async function ensureProviderAuth(params: {
  ...
  if (!catalogEntry) {
    throw new Error(
      `No credentials configured for provider "${provider}". Add one with ${formatCliCommand("afora models auth add")} and retry.`,
    );
  }
  ...
  const applied = await applyAuthChoiceLoadedPluginProvider({
    ...
    config: structuredClone(snapshot.sourceConfig ?? snapshot.config) as AforaConfig,
...
export async function promosClaimCommand(
  ...
  if (makeDefault && suggested) {
    ...
    runtime.log(
      `  Revert anytime with ${formatCliCommand("afora models set <previous-model>")}.`,
    );
  } else if (suggested) {
    runtime.log(
      `  Try it: ${formatCliCommand(`afora models set ${suggested.modelRef}`)} (promotion ends ${new Date(promotion.endsAt).toLocaleDateString()}).`,
    );
  }
}
