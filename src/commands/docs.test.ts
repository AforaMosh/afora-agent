...
describe("docsSearchCommand", () => {
  ...
  it("calls the Cloudflare docs search API", async () => {
    ...
    expect(url.href).toBe("https://docs.afora.ai/api/search?q=plugin+allowlist");
  ...
  it("reports docs search responses with invalid UTF-8 bytes as malformed", async () => {
    const body = new Uint8Array([
      ...
      0xff,
      ...new TextEncoder().encode('list","link":"https://docs.afora.ai/plugins/allowlist"}]}'),
    ]);
    fetchMock.mockResolvedValueOnce(
          ...
          results: [
            {
              ...
              link: "https://docs.afora.ai/plugins/allowlist",
