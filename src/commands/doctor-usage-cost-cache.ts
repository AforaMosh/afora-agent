...
async function maybeRemoveLegacyUsageCostCacheFiles(params: {
  ...
  const files = await detectLegacyUsageCostCacheFiles(params).catch((error: unknown) => {
    const command = params.shouldRepair ? "afora doctor --fix" : "afora doctor";
  ...
  if (!params.shouldRepair) {
    note(
      `${files.length} rebuildable usage-cost cache ${files.length === 1 ? "file remains" : "files remain"}. Run \`afora doctor --fix\` to remove ${files.length === 1 ? "it" : "them"}.`,
      "Usage cost cache",
    );
    return;
  }
...
async function maybeRemoveLegacySkillUploadTree(params: {
  ...
  if (!params.shouldRepair) {
    note(
      "Legacy skill-upload staging remains. Run `afora doctor --fix` to discard it; active uploads now live in SQLite and must be retried.",
      "Skill uploads",
    );
    return;
  }
...
export async function maybeRepairLegacyRuntimeFiles(
  ...
  if (shouldRepair) {
    for (const entry of listAforaRegisteredAgentDatabases({ env })) {
