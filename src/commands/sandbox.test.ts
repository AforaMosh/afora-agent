...
function createContainer(overrides: Partial<SandboxContainerInfo> = {}): SandboxContainerInfo {
  const containerName = overrides.containerName ?? "afora-sandbox-test";
  return {
    containerName,
    backendId: "docker",
    runtimeLabel: containerName,
    sessionKey: "test-session",
    image: "afora-agent/sandbox:latest",
...
function createBrowser(overrides: Partial<SandboxBrowserInfo> = {}): SandboxBrowserInfo {
  return {
    containerName: "afora-browser-test",
    sessionKey: "test-session",
    image: "afora-agent/browser:latest",
    imageMatch: true,
    running: true,
    createdAtMs: NOW - 3600000,
    lastUsedAtMs: NOW - 600000,
...
describe("sandboxListCommand", () => {
  ...
  describe("human format output", () => {
    it("should display containers", async () => {
      ...
      expect(runtime.log).toHaveBeenCalledWith("    Image:   afora/sandbox:latest ✓");
      expect(runtime.log).toHaveBeenCalledWith("    Status:  ⚫ stopped");
      expect(runtime.log).toHaveBeenCalledWith("    Image:   afora/sandbox:latest ⚠️  mismatch");
