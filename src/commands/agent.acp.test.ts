...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "afora-agent-acp-" });
}

function createAcpEnabledConfig(home: string, storePath: string): AforaConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<AforaConfig["acp"]>>,
) {
  const cfg = createAcpEnabledConfig(home, storePath);
  cfg.acp = {
    ...cfg.acp,
...
describe("agentCommand ACP runtime routing", () => {
  ...
  it("blocks ACP turns when disabled by policy", async () => {
    ...
    ] satisfies Array<Partial<NonNullable<AforaConfig["acp"]>>>) {
