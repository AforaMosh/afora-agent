...
import type { AforaConfig, GatewayAuthConfig } from "../config/config.js";
...
function hasConfiguredProviderModels(cfg: AforaConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
...
function hasStaticManifestCatalogRows(cfg: AforaConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
  return (
...
function listConfiguredModelProviders(cfg: AforaConfig): string[] {
  return Object.entries(cfg.models?.providers ?? {})
    .filter(([, provider]) => (provider.models?.length ?? 0) > 0)
    .map(([provider]) => provider);
}

function resolveSingleConfiguredProvider(cfg: AforaConfig): string | undefined {
...
export function buildGatewayAuthConfig(params: {
  ...
  if (params.mode === "trusted-proxy") {
    if (!params.trustedProxy) {
      throw new Error(
        `trustedProxy config is required when mode is trusted-proxy. Run ${formatCliCommand("afora configure --section gateway")} to configure Gateway auth interactively.`,
      );
    }
