...
export async function gatewayAuthTokenCommand(
  ...
  if (!(options.interactive ?? isTerminalInteractive())) {
    throw new Error(
      "Refusing to print the Gateway token outside an interactive terminal. Run `afora gateway auth-token --show` directly in a terminal on the Gateway host.",
    );
  }

  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    throw new Error("Gateway config is invalid. Run `afora doctor --fix`, then try again.");
  }
  ...
  if (resolvedAuth.mode !== "token" || !resolvedAuth.token) {
    throw new Error(
      "No configured Gateway token is available. Run `afora doctor --generate-gateway-token`, restart the Gateway, then try again.",
    );
  }
