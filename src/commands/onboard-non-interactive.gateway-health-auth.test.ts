...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-gateway-health-auth-"));
  try {
...
describe("resolveGatewayHealthProbeToken", () => {
  ...
  const originalGatewayPassword = process.env.AFORA_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (originalGatewayPassword === undefined) {
      ...
      process.env.AFORA_GATEWAY_PASSWORD = originalGatewayPassword;
    }
  });
  ...
  it("resolves file SecretRefs for the local onboarding health probe without persisting plaintext", async () => {
    await withTempDir(async (dir) => {
      ...
      process.env.AFORA_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("does not fall back to stale AFORA_GATEWAY_TOKEN when a SecretRef is unresolved", async () => {
    await withTempDir(async (dir) => {
      process.env.AFORA_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("resolves password auth for the local onboarding health probe", async () => {
    process.env.AFORA_GATEWAY_TOKEN = "stale-env-token";
    process.env.AFORA_GATEWAY_PASSWORD = "resolved-password"; // pragma: allowlist secret
  ...
  it("resolves environment-only password auth for the local onboarding health probe", async () => {
    process.env.AFORA_GATEWAY_PASSWORD = "environment-password"; // pragma: allowlist secret
