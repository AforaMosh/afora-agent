...
function makeDoctorStateDir(): string {
  return tempDirs.make("afora-doctor-");
}
...
async function runTelegramAllowFromMigration(params: { root: string; cfg: AforaConfig }) {
  const oauthDir = ensureCredentialsDir(params.root);
  writeLegacyTelegramAllowFromStore(oauthDir);
  const env = { AFORA_STATE_DIR: params.root } as NodeJS.ProcessEnv;
...
async function runLegacyStateMigrationsForRoot(root: string) {
  const detected = await detectLegacyStateMigrations({
    cfg: {},
    env: { AFORA_STATE_DIR: root } as NodeJS.ProcessEnv,
  });
...
async function withStateDir<T>(root: string, run: () => Promise<T>): Promise<T> {
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.AFORA_STATE_DIR = previous;
    }
  }
}
...
function getStateDirMigrationPaths(root: string) {
  return {
    targetDir: path.join(root, ".afora"),
...
function getProfileWorkspaceMigrationPaths(root: string, profile = "work") {
  return {
    legacyDir: path.join(root, ".afora", `workspace-${profile}`),
    targetDir: path.join(root, `.afora-${profile}`, "workspace"),
    stateDir: path.join(root, `.afora-${profile}`),
  };
}
...
describe("doctor legacy state migrations", () => {
      ...
      expect(() =>
        db.exec(`
          INSERT INTO agent_databases (
            ...
            'worker-1',
            '/relocated/worker-1/afora-agent.sqlite',
            1,
            20,
            30
          )
  ...
  it("does not repair newer shared state schemas", async () => {
    ...
    seededDb.exec(`PRAGMA user_version = ${AFORA_STATE_SCHEMA_VERSION + 1};`);
    seededDb.close();
    ...
    expect(result.warnings[0]).toContain(
      `uses newer schema version ${AFORA_STATE_SCHEMA_VERSION + 1}`,
    );
  ...
  it("migrates legacy ACP metadata from sessions.json into shared SQLite", async () => {
    ...
    const sqlite = requireNodeSqlite();
    const db = new sqlite.DatabaseSync(path.join(root, "state", "afora.sqlite"));
    try {
      const row = db
        .prepare(
  ...
  it("imports the shipped debug proxy capture sidecar into shared state", async () => {
    ...
    const state = openAforaStateDatabase({
      env: { AFORA_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
    expect(
  ...
  it("leaves debug proxy sources in place when a session id conflicts", async () => {
    ...
    state.db
      ...
      .run("legacy-session", 999, "different", "afora", "afora");
  ...
  it("auto-migrates the plugin-state sidecar when custom agent dirs skip session migration", async () => {
    ...
    const result = await autoMigrateLegacyState({
      ...
      env: {
        ...
        AFORA_AGENT_DIR: path.join(root, "custom-agent"),
      } as NodeJS.ProcessEnv,
      log: { info: vi.fn(), warn: vi.fn() },
    });
  ...
  it("never imports default exec approvals into a custom state dir", async () => {
    ...
    const sourcePath = path.join(root, ".afora", "exec-approvals.json");
    ...
    const detected = await detectLegacyStateMigrations({
      cfg: {},
      env: { AFORA_STATE_DIR: stateDir } as NodeJS.ProcessEnv,
      homedir: () => root,
    });
  ...
  it("normalizes obsolete task delivery status before archiving the legacy sidecar", async () => {
    ...
    const shared = openAforaStateDatabase({
      env: { AFORA_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
    expect(
      shared.db
  ...
  it("keeps task sidecars when shared state already has conflicting task rows", async () => {
    ...
    await withStateDir(root, async () => {
      const sqlite = requireNodeSqlite();
      const sharedPath = path.join(root, "state", "afora.sqlite");
  ...
  it("honors session.mainKey when seeding the direct-chat bucket", async () => {
    const root = makeDoctorStateDir();
    const cfg: AforaConfig = { session: { mainKey: "work" } };
    writeLegacySessionsFixture({
      root,
      sessions: {
        "+1555": { sessionId: "a", updatedAt: 10 },
