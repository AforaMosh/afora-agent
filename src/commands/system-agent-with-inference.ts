// Afora command gate: prove inference before starting conversational setup.
...
/**
 * Start Afora only after the configured default model completes a real
...
export async function runSystemAgentWithInference(
  ...
  if (opts.yes && !opts.message?.trim()) {
    failOneShotExecution(
      opts,
      runtime,
      new Error("Afora --yes requires --message so approval is limited to one request."),
    );
    return;
  }
  ...
  if (oneShot) {
    const guidance = "Run `afora onboard` to connect and live-test AI first.";
    if (opts.json) {
      writeRuntimeJson(runtime, {
        ok: false,
        status: inference.status,
        error: `Afora requires working inference: ${inference.error}`,
        guidance,
      });
    } else {
      runtime.error(
        [`Afora requires working inference: ${inference.error}`, guidance].join("\n"),
      );
    }
    if (!requestExitAfterOneShotOutput(runtime, 1)) {
      runtime.exit(1);
  ...
  runtime.log("Afora requires working inference. Starting guided AI setup…");
