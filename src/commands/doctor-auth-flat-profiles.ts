...
import { readNonBlankString as readNonEmptyString } from "@afora/normalization-core/string-coerce";
...
function coerceLegacyConfigAuthProfileStore(cfg: AforaConfig): AuthProfileStore | null {
...
export async function maybeMigrateAuthProfileJsonStoresToSqlite(params: {
  ...
  note(
    [
      ...
      `- ${formatCliCommand("afora doctor --fix")} imports legacy auth profile JSON into SQLite, verifies it, records a receipt, and archives the original bytes.`,
    ].join("\n"),
  ...
  if (hasLegacyOAuth && sharedMainCredentialSourceRemains) {
    result.warnings.push(
      `Deferred shared legacy OAuth migration until higher-priority shared-main credential sources are resolved by ${formatCliCommand("afora doctor --fix")}.`,
    );
...
function ensureConfigAuthProfiles(config: AforaConfig): Record<string, AuthProfileConfig> {
