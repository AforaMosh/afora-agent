...
type StatusScanSharedMocks = {
  ...
  resolveGatewayPort: Mock<(cfg?: AforaConfig) => number>;
...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/afora-${configPathLabel}-missing-${process.pid}.json`),
    resolveGatewayPort: vi.fn((cfg?: AforaConfig) => cfg?.gateway?.port ?? 18789),
...
export function createStatusScanConfig<T extends object = AforaConfig>(
  overrides: T = {} as T,
): AforaConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): AforaConfig {
