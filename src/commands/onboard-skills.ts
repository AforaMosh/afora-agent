...
function isTrustedAutoInstallableSkill(skill: { bundled: boolean; source: string }): boolean {
  ...
  return skill.bundled && skill.source === "afora-bundled";
}
...
export async function setupSkills(
  ...
  let next: AforaConfig = cfg;
  if (candidateInstallable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        "No missing skill dependencies to install.",
        `To inspect available skills, run: ${formatCliCommand("afora skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("afora skills check")}`,
      ].join("\n"),
  ...
  if (candidateInstallable.length > 0) {
    ...
    for (const target of selectedReadySkills) {
      ...
      // visible through `afora skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("afora doctor")}\` to review skills + requirements.`,
      );
