...
const INSECURE_WS_URL_MESSAGE =
  ...
  "Break-glass: AFORA_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks.";
...
describe("promptRemoteGatewayConfig", () => {
  const envSnapshot = captureEnv(["AFORA_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("falls back to manual URL entry when discovery trust is declined", async () => {
    ...
    const next = await promptRemoteGatewayConfig({} as AforaConfig, prompter);
  ...
  it("allows ws:// hostname remote URLs when AFORA_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    process.env.AFORA_ALLOW_INSECURE_PRIVATE_WS = "1";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.validate?.("ws://afora-gateway.ai:18789")).toBeUndefined();
        expect(params.validate?.("ws://1.1.1.1:18789")).toBe(INSECURE_WS_URL_MESSAGE);
        return "ws://afora-gateway.ai:18789";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://afora-gateway.ai:18789");
  });

  it("supports storing remote auth as an external env secret ref", async () => {
    process.env.AFORA_GATEWAY_TOKEN = "remote-token-value";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        return "wss://remote.example.com:18789";
      }
      if (params.message === "Environment variable name") {
        return "AFORA_GATEWAY_TOKEN";
      }
      return "";
    }) as WizardPrompter["text"];
