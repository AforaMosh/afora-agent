...
describe("setupSkills", () => {
  ...
  it("bounds skill hints and install failures through the onboarding flow", async () => {
    ...
    await setupSkills({} as AforaConfig, "/tmp/ws", runtime, prompter);
  ...
  it("uses the requested node manager for selected node-backed installs", async () => {
    ...
    const { prompter } = createPrompter({ multiselect: ["node-helper"] });
    const next = await setupSkills({} as AforaConfig, "/tmp/ws", runtime, prompter, {
      nodeManager: "pnpm",
    });
  ...
  it("displays a clear empty state note when all skill dependencies are ready", async () => {
    ...
    expect(emptyStateNote?.message).toContain("afora skills list --verbose");
    expect(emptyStateNote?.message).toContain("afora skills check");
  });
