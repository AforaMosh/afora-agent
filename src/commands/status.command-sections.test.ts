...
describe("status.command-sections", () => {
  ...
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:afora security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:afora security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:afora status --all",
      "  Need to debug live? cmd:afora logs --follow",
      "  Fix reachability first: cmd:afora gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:afora devices approve req-123)",
      "muted(Fallback: cmd:afora devices approve --latest)",
      "muted(Inspect: cmd:afora devices list)",
    ]);
  });
