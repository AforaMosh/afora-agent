...
export function compactDoctorSqliteFile(
  ...
  try {
    database.exec(
      `PRAGMA busy_timeout = ${options.busyTimeoutMs ?? AFORA_SQLITE_BUSY_TIMEOUT_MS};`,
    );
...
function checkpointTruncate(database: DatabaseSync, sqlitePath: string): void {
  ...
  if (busy !== 0) {
    throw new Error(`SQLite checkpoint remained busy for ${sqlitePath}. Stop Afora and retry.`);
  }
}
