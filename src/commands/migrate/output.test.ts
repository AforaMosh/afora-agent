...
describe("formatMigrationPreview", () => {
  ...
  it("renders migration warnings with a warning glyph", () => {
    ...
    expect(output).toContain(
      "⚠️  Some Codex plugins could not be migrated. Run `afora migrate codex` after onboarding.",
    );
  });
...
describe("formatMigrationResult", () => {
  ...
  it("renders warning-backed next steps with a warning glyph", () => {
    ...
    const output = formatMigrationResult({
      ...
      warnings: [warning],
      nextSteps: [warning, "Run afora doctor after applying the migration."],
    })
      .map(stripAnsi)
      .join("\n");

    expect(output).toContain(`⚠️  ${warning}`);
    expect(output).toContain("• Run afora doctor after applying the migration.");
  });
