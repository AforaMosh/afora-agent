...
const REASON_CODE_MESSAGES: Record<string, string> = {
  ...
  already_active: "Plugin is already active in Afora",
  installed: "Plugin is already installed in Afora",
...
function formatItemMessage(item: MigrationItem, mode: FormatMode): string | undefined {
  if (mode === "preview") {
    ...
    if (item.kind === "skill" && item.action === "copy") {
      return "Copy Codex skill into Afora";
    }
    if (item.kind === "plugin" && item.action === "install") {
      return "Install Codex plugin into Afora";
    }
...
export function assertConflictFreePlan(plan: MigrationPlan, providerId: string): void {
  if (plan.summary.conflicts > 0) {
    throw new Error(
      `Migration has ${formatCount(plan.summary.conflicts, "conflict")}. Re-run with --overwrite after reviewing afora migrate plan ${providerId}.`,
    );
  }
}
...
export function writeApplyResult(
  ...
  if (result.backupPath) {
    ...
    runtime.log("Backup: skipped (no existing Afora state found)");
  }
  if (result.reportDir) {
