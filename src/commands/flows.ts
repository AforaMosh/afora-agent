...
import { truncateUtf16Safe, truncateWithMarker } from "@afora/normalization-core/utf16-slice";
...
function formatFlowLookupMiss(lookup: string): string {
  return `TaskFlow not found: ${sanitizeTerminalText(lookup)}. Run ${formatCliCommand("afora tasks flow list")} to see recent flow ids.`;
}
...
export async function flowsListCommand(
  ...
  if (flows.length === 0) {
    runtime.log(
      `No TaskFlows found. Run ${formatCliCommand("afora tasks list")} to inspect standalone background tasks.`,
    );
    return;
  }
  const rich = isRich();
