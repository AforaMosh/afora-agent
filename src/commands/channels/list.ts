// Implements `afora channels list` across runtime accounts, local config, and catalog-only entries.
...
export async function channelsListCommand(
  ...
  // there are no configured channels even though afora.json has one.
    ...
    lines.push(
      theme.muted(
        showAll
          ...
          : "- no configured chat channels (run `afora channels list --all` to see installable channels)",
      ),
    );
  } else {
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `afora status` or `afora models list`.",
    ),
  );
