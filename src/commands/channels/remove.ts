...
export async function channelsRemoveCommand(
  ...
  let cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as AforaConfig;
  ...
  if (useWizard && prompter) {
    ...
    if (!rawChannel) {
      runtime.error(
        `Missing channel. Use ${formatCliCommand("afora channels remove --channel <name>")} or run ${formatCliCommand("afora channels status")} to inspect configured channels.`,
      );
      runtime.exit(1);
      return;
    }
  ...
  if (!plugin) {
    if (resolvedPluginState?.catalogEntry) {
      runtime.error(
        `Channel plugin "${resolvedPluginState.catalogEntry.id}" is not installed. Run ${formatCliCommand(`afora channels add --channel ${resolvedPluginState.catalogEntry.id}`)} first.`,
      );
      runtime.exit(1);
      return;
    }
  ...
  if (!removal.ok) {
    runtime.error(
      removal.error.action === "delete"
        ? `${formatUnsupportedChannelActionMessage({ channel, action: "delete" })} Use ${formatCliCommand("afora channels remove --channel " + channel)} to disable it without deleting config.`
        : `${formatUnsupportedChannelActionMessage({ channel, action: "disable" })} Use ${formatCliCommand("afora channels remove --channel " + channel + " --delete")} only if you want to remove config.`,
    );
    runtime.exit(1);
    return;
  }
