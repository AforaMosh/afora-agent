...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<AforaTestState> {
  const state = await createAforaTestState({
    layout: "state-only",
    prefix: "afora-doctor-oauth-sidecar-",
    env: {
      AFORA_AGENT_DIR: undefined,
      AFORA_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ...
  const cipher = createCipheriv(
    "aes-256-gcm",
    hash("sha256", `afora:auth-profile-oauth:${params.seed}`, "buffer"),
    iv,
  );
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  it("migrates encrypted legacy oauthRef sidecars back to inline OAuth credentials", async () => {
    ...
    const ref = {
      source: "afora-credentials" as const,
  ...
  it("scans AFORA_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
