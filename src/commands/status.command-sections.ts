...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("afora security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("afora security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.afora.ai/faq",
    "Troubleshooting: https://docs.afora.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("afora status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("afora logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("afora node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("afora status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("afora gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`afora devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("afora devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("afora devices list")}`),
  ];
}
