...
type BrowserHatchHandoffDeps = {
  ...
  resolveTarget?: (config: AforaConfig, env: NodeJS.ProcessEnv) => Promise<BrowserHatchTarget>;
  probePresence?: (
...
async function probeDashboardPresence(
  ...
  try {
    // Read presence over the same trusted local CLI path every `afora`
    ...
    const presence = await callGateway<SystemPresence[]>({
      ...
      timeoutMs,
      // Connect as a CLI-mode loopback client (what every `afora` command
...
export async function runBrowserHatchHandoff(
  ...
  if (opened) {
    ...
    const pairingHint = directRemoteDisplay
      ? "\n\nIf device approval is required, run `afora devices list`, then `afora devices approve <requestId>`."
      : "";
    await params.prompter.note(
