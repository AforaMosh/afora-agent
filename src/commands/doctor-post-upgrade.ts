...
export async function runPostUpgradeProbes(params: {
  ...
  if (!installs) {
    findings.push({
      ...
      message:
        "Installed plugin index is missing, unreadable, or malformed. Run `afora plugins registry --refresh` to rebuild it before post-upgrade validation.",
    });
    ...
    if (pkgRelPath) {
      ...
      try {
        ...
        findings.push({
          ...
          message: `Plugin ${record.pluginId}: could not read package.json (${pkgRelPath}): ${reason}. Reinstall the plugin or run \`afora plugins registry --refresh\`.`,
      ...
      if (resolvedEntries.status === "invalid") {
        findings.push({
          level: "error",
          code: "plugin.entry_unresolved",
          message: `Plugin ${record.pluginId}: ${resolvedEntries.error}. Reinstall the plugin or run \`afora plugins registry --refresh\`.`,
          plugin: record.pluginId,
          entry: pkgRelPath,
        });
    ...
    if (record.manifestPath && record.manifestHash) {
      ...
      if (currentHash && currentHash !== record.manifestHash) {
        findings.push({
          ...
          message: `Plugin ${record.pluginId} manifest hash drifted from installs.json snapshot. Run \`afora plugins registry --refresh\` to re-sync.`,
          plugin: record.pluginId,
        });
      }
    }
