...
export function formatUpdateRestartStatusValue(
  ...
  if (payload.status === "error") {
    return warn(
      `failed · ${reason ?? "restart failed"} · run afora gateway status --deep${age}`,
    );
  }
  ...
  if (payload.status === "skipped") {
    if (reason === CONTROL_PLANE_UPDATE_HANDOFF_STARTED_REASON) {
      ...
      return warn(`handoff running · gateway restart pending · run afora update status${age}`);
    }
    if (reason === CONTROL_PLANE_UPDATE_RESTART_HEALTH_PENDING_REASON) {
      ...
      return warn(`restart pending health verification · run afora gateway status --deep${age}`);
    }
...
export function formatUpdateRestartActionLines(
  ...
  if (payload.status === "error") {
    return [
      "Update restart failed; run afora gateway status --deep.",
  ...
  if (
    ...
    return [
      ...
      "If it stays pending, run afora gateway status --deep.",
    ];
  }
  return [];
}
