...
const resolveAgentDir = vi.hoisted(() =>
  vi.fn<(_cfg: AforaConfig, agentId: string) => string>(() => "/tmp/agent-default"),
);
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn<(_cfg: AforaConfig, agentId: string) => string>(() => "/tmp/agent-default/workspace"),
);
...
describe("noteMemorySearchHealth", () => {
  ...
  async function runAuthLintHealth(provider: "openai" | "bedrock", config: AforaConfig = cfg) {
    await runConfiguredMemorySearch(
      provider,
      config,
    ...
    await runMemorySearchHealth(
      "local",
      {
        gatewayMemoryProbe: {
          ...
          ready: false,
          error:
            "memory embedding readiness not checked; run `afora memory status --deep` to probe",
          skipped: true,
        },
      },
  ...
  it("warns when local provider skipped readiness but configured local model is missing", async () => {
    await runMemorySearchHealth(
      ...
      { local: { modelPath: "/definitely/missing/afora-memory-model.gguf" } },
    );
  ...
  it.each([
    [
      "does not warn when an enabled alternate memory plugin owns the memory slot",
      {
        ...
        entries: { "memory-lancedb": { enabled: true, config: { dbPath: ".afora/memory" } } },
      },
      true,
    ],
    [
    ...
    resolveActiveMemoryBackendConfig.mockReturnValue(null);
    const config = { session: { dmScope: "per-peer" }, plugins } as unknown as AforaConfig;
  ...
  it("warns when an opted-in agent has memory search disabled", async () => {
    ...
    resolveMemorySearchConfig.mockImplementation((_cfg: AforaConfig, agentId: string) =>
      agentId === "personal"
        ? undefined
  ...
  it("warns for key-optional provider (lmstudio) when gateway probe timed out", async () => {
    ...
    // Regression guard: https://github.com/AforaMosh/afora-agent/issues/74608
  ...
  it("uses model configure hint when gateway probe is unavailable and API key is missing", async () => {
    ...
    expectFirstNoteContains(
      ...
      "afora configure --section model",
    );
    expectFirstNoteExcludes("afora auth add --provider");
  });
...
describe("memory recall doctor integration", () => {
  ...
  it("runs dreaming artifact repair during doctor --fix", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValueOnce({
      changed: true,
      archiveDir: "/tmp/agent-default/workspace/.afora-repair/dreaming/2026-04-11T21-35-00-000Z",
      archivedDreamsDiary: false,
