...
import { truncateWithMarker } from "@afora/normalization-core/utf16-slice";
...
export async function tasksListCommand(
  ...
  if (tasks.length === 0) {
    runtime.log(
      `No background tasks found. Run ${formatCliCommand("afora tasks audit")} to check for stale task state.`,
    );
    return;
  }
  const rich = isRich();
...
export async function tasksMaintenanceCommand(
  ...
  if (!opts.apply) {
    runtime.log("Dry run only. Re-run with `afora tasks maintenance --apply` to write changes.");
  }
}
