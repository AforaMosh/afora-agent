...
function usesClaudeCliModelSelection(cfg: AforaConfig): boolean {
...
function resolveClaudeCliAgentIds(cfg: AforaConfig): string[] {
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile && credential?.type !== "api_key_helper") {
    lines.push(`- Afora auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- Afora auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
