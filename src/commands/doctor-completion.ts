...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(afora completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `afora doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`afora completion --write-state\` or \`afora doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
