...
export async function sandboxRecreateCommand(
  ...
  if (filtered.containers.length + filtered.browsers.length === 0) {
    runtime.log(
      `No sandbox runtimes found matching the criteria. Run ${formatCliCommand("afora sandbox list")} to inspect active runtimes.`,
    );
    return;
  }
...
function validateRecreateOptions(opts: SandboxRecreateOptions, runtime: RuntimeEnv): boolean {
  if (!opts.all && !opts.session && !opts.agent) {
    runtime.error(
      `Choose the sandbox scope: --all, --session <key>, or --agent <id>. Run ${formatCliCommand("afora sandbox list")} to inspect active runtimes first.`,
    );
    runtime.exit(1);
    return false;
  }
...
async function removeContainer(
  ...
  try {
    ...
    runtime.error(
      `Failed to remove ${containerName}: ${formatErrorMessage(err)}. Run ${formatCliCommand("afora sandbox list")} to inspect what remains.`,
    );
