...
import { AforaSchema } from "../config/zod-schema.js";
import {
  formatConfigKeyPath,
...
function collectImplicitFallbackClobberWarnings(cfg: AforaConfig): string[] {
  noteMock.mockClear();
...
describe("doctor config analysis helpers", () => {
  ...
  it("strips unknown root model metadata while preserving supported agent metadata", () => {
    ...
    expect(AforaSchema.safeParse({ defaultModel: "minimax/MiniMax-M2.7" }).success).toBe(false);
  ...
  describe("stripUnknownConfigKeys during update", () => {
    const originalEnv = process.env.AFORA_UPDATE_IN_PROGRESS;
    ...
    afterEach(() => {
      if (originalEnv !== undefined) {
        process.env.AFORA_UPDATE_IN_PROGRESS = originalEnv;
      } else {
        delete process.env.AFORA_UPDATE_IN_PROGRESS;
      }
    });

    it("returns input unchanged when AFORA_UPDATE_IN_PROGRESS=1", () => {
    ...
    it("returns input unchanged when AFORA_UPDATE_IN_PROGRESS=true", () => {
      process.env.AFORA_UPDATE_IN_PROGRESS = "true";
      const input = { hooks: {}, unexpected: true } as never;
      const result = stripUnknownConfigKeys(input);
      expect(result.config).toBe(input);
      expect(result.removed).toEqual([]);
...
describe("collectImplicitFallbackClobberWarnings", () => {
  function buildConfig(overrides: { defaults?: unknown; list?: unknown[] }): AforaConfig {
    return {
      agents: {
...
describe("noteSandboxOriginProxyWarning", () => {
  function warningsFor(cfg: AforaConfig): string[] {
    noteMock.mockClear();
  ...
  it("stays silent for non-proxy auth modes", () => {
    expect(warningsFor({ gateway: { auth: { mode: "token" } } } as AforaConfig)).toHaveLength(0);
    expect(warningsFor({} as AforaConfig)).toHaveLength(0);
  });
});
