...
const SAMPLE_SECRET_REF = {
  source: "env" as const,
  provider: "default",
  id: "AFORA_GATEWAY_TOKEN_REF",
};

function createTokenConfig(token: unknown): AforaConfig {
  return {
...
describe("applyNonInteractiveGatewayConfig auth resolution", () => {
  ...
  it("prefers existing plaintext token over ambient AFORA_GATEWAY_TOKEN on re-onboard", () => {
    // A stale shell/launchd AFORA_GATEWAY_TOKEN must not rotate a
    // persisted token — that would break already-paired clients.
    const nextConfig = createTokenConfig("existing-user-token");

    const result = applyGatewayConfig({
      nextConfig,
      env: { AFORA_GATEWAY_TOKEN: "stale-env-token" },
    });
  ...
  it("uses AFORA_GATEWAY_TOKEN to fill an empty config on first-run", () => {
    const result = applyGatewayConfig({ env: { AFORA_GATEWAY_TOKEN: "env-token" } });
  ...
  it("preserves an existing SecretRef even when ambient AFORA_GATEWAY_TOKEN is set", () => {
  ...
  it("overrides an existing SecretRef when --gateway-token-ref-env is provided", () => {
    const newRefId = "AFORA_GATEWAY_TOKEN_NEW_REF";
    const nextConfig = createTokenConfig(SAMPLE_SECRET_REF);
  ...
  it("fails when --gateway-token-ref-env points to a missing env var", () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      'Environment variable "MISSING_GATEWAY_TOKEN_ENV" is missing or empty. Export it first, then rerun afora onboard --non-interactive.',
    );
  ...
  it("preserves environment-backed password auth without persisting the password", () => {
    const result = applyGatewayConfig({
      ...
      env: { AFORA_GATEWAY_PASSWORD: "environment-password" },
    });
