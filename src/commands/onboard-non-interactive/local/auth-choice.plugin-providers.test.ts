...
const ensureCodexRuntimePluginForModelSelection = vi.hoisted(() =>
  vi.fn(
    async ({ cfg }: { cfg: AforaConfig }): Promise<RuntimePluginInstallResult> => ({
      cfg,
      required: false,
      installed: false,
    }),
...
async function applyProviderModelChoice(params: {
  ...
  const runNonInteractive = vi.fn(async ({ config }: { config: AforaConfig }) => ({
    ...config,
    agents: {
      ...config.agents,
      defaults: {
...
describe("applyNonInteractivePluginProviderChoice", () => {
  ...
  it("loads plugin providers for provider-plugin auth choices", async () => {
    ...
    const result = await applyNonInteractivePluginProviderChoice({
      nextConfig: { agents: { defaults: {} } } as AforaConfig,
      authChoice: "provider-plugin:vllm:custom",
      opts: {} as never,
      runtime: runtime as never,
      baseConfig: { agents: { defaults: {} } } as AforaConfig,
      target,
      resolveApiKey: vi.fn(),
      toApiKeyCredential: vi.fn(),
    });
