...
export async function setupCommand(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(
      `Config invalid at ${await formatConfigFilePath(configPath)}. Run \`${formatCliCommand("afora doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  const cfg = shouldPersistRoster
    ? (migratePersistedImplicitMainRoster(snapshot.sourceConfig).config as AforaConfig)
  ...
  let next: AforaConfig = snapshot.exists ? resolvedConfig : cfg;
  ...
  runtime.log(`Next guided path: ${formatCliCommand("afora onboard")}.`);
  runtime.log(
    `Next targeted changes: ${formatCliCommand("afora configure")} for models, channels, Gateway, plugins, skills, and health checks.`,
  );
  runtime.log(`Add a chat channel later: ${formatCliCommand("afora channels add")}.`);
}
