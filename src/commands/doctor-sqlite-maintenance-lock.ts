...
export class DoctorSqliteMaintenanceLockUnavailableError extends Error {
  constructor(
    ...
    super(
      `Cannot run ${operation} while the Gateway or another SQLite maintenance command owns this Afora state directory. Stop the Gateway and retry.`,
    );
...
function assertMaintenancePathsOwnedByStateDir(
  ...
  for (const protectedPath of protectedPaths) {
    ...
    try {
      ...
      resolveRootPathSync({
        absolutePath,
        boundaryLabel: "Afora state directory",
        rootCanonicalPath: stateCanonicalDir,
        rootPath: stateDir,
      });
    } catch (error) {
      throw new Error(
        `Cannot run ${operation} for a path outside the active Afora state directory: ${protectedPath}. Set AFORA_STATE_DIR to the owning state directory and retry.`,
        { cause: error },
      );
    }
  }
...
export async function withDoctorSqliteMaintenanceLock<T>(
  ...
  if (!lock) {
    throw new Error(`Cannot run ${params.operation} without exclusive Afora state ownership.`);
  }
