...
/** Prompt for configured channel sections to remove from afora.json. */
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in afora.json.",
          `Tip: \`${formatCliCommand("afora channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    if (Object.keys(nextChannels).length) {
      next.channels = nextChannels as AforaConfig["channels"];
    } else {
