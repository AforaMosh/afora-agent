...
function createTelegramCfg(botToken: string, enabled?: boolean): AforaConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function createMSTeamsCatalogEntry(): ChannelPluginCatalogEntry {
  return {
    id: "external-chat",
    pluginId: "@afora/external-chat-plugin",
    meta: {
      id: "external-chat",
      label: "External Chat",
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: AforaConfig }) =>
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: AforaConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: AforaConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: AforaConfig }) => ({
        channel: "telegram",
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as AforaConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("../commands/channel-setup/plugin-install.js", async () => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: AforaConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("shows explicit dmScope config command in channel primer", async () => {
    ...
    await runSetupChannels({} as AforaConfig, prompter);
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@afora/external-chat-plugin": { enabled: true },
          },
        },
      } as AforaConfig,
      prompter,
        ...
        if (channel === "external-chat") {
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                listAccountIds: (cfg: AforaConfig) =>
                  Object.keys(
                    (
                      cfg.channels?.["external-chat"] as
                        | { accounts?: Record<string, unknown> }
                ...
                resolveAccount: (cfg: AforaConfig, accountId: string) =>
                  (
                    cfg.channels?.["external-chat"] as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: AforaConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: AforaConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
