  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("afora doctor --fix")} to retire stale generations after the gateway restarts.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
...
export function staleManagedNpmInstallGenerationToHealthFinding(
  ...
  return {
    ...
    fixHint:
      "Run `afora doctor --fix` to retire the stale generation for pruning after the gateway restarts.",
  };
}
