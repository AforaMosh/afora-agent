...
async function formatDeprecatedProviderChoiceError(
  ...
  if (deprecatedChoice) {
    return `Auth choice ${JSON.stringify(authChoice)} is no longer supported. Use ${JSON.stringify(deprecatedChoice.choiceId)} instead, or run ${formatCliCommand("afora onboard")} to choose interactively.`;
  }
  ...
  return `Auth choice ${JSON.stringify(authChoice)} is no longer supported. Use ${JSON.stringify(externalDeprecatedChoice.choiceId)} instead, or run ${formatCliCommand("afora onboard")} to choose interactively.`;
}
...
export async function prepareAuthChoice(
  ...
  if (normalizedParams.authChoice === "token" || normalizedParams.authChoice === "setup-token") {
    throw new Error(
      [
        ...
        `Run ${formatCliCommand("afora models auth login --provider <provider>")} for provider auth, or rerun ${formatCliCommand("afora onboard")} to choose interactively.`,
      ].join("\n"),
    );
  }
  ...
  if (normalizedParams.authChoice === "oauth") {
    throw new Error(
      `Auth choice "oauth" is no longer supported directly. Use a provider-specific auth entry, or run ${formatCliCommand("afora models auth login --provider <provider>")}.`,
    );
  }
