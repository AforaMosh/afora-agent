...
function makeLocalConfig(): AforaConfig {
  return {
...
describe("runRemoteGatewayInferenceOnboarding", () => {
  it.each([
    ...
    "pins $label across detect, activate, verify, Afora, and in-process TUI",
    async ({ auth, secret }) => {
      ...
      const callGatewayMock = vi.fn(async (options: CallGatewayCliOptions): Promise<unknown> => {
        ...
        if (options.method === "afora.setup.detect") {
          expect(options.timeoutMs).toBe(40_000);
          return detectResult();
        }
        if (options.method === "afora.setup.activate") {
  ...
  ])("fails closed on $label before Afora", async ({ verification, error }) => {
    const localConfig = makeLocalConfig();
    const localConfigBefore = structuredClone(localConfig);
  ...
  it("does not advance or fall back locally after an ambiguous activation request failure", async () => {
    ...
    expect(methods).toEqual(["afora.setup.detect", "afora.setup.activate"]);
    expect(runTui).not.toHaveBeenCalled();
  });

  it("treats a cancelled remote Afora conversation as a pause without opening the agent", async () => {
    const methods: string[] = [];
    const callGatewayMock = vi.fn(async (options: CallGatewayCliOptions): Promise<unknown> => {
      methods.push(options.method);
      if (options.method === "afora.setup.detect") {
    ...
    expect(prompter.outro).toHaveBeenCalledWith("Afora setup paused.");
    expect(runTui).not.toHaveBeenCalled();
  });
});
