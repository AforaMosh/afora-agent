...
function resolveConfiguredCommandOwners(cfg: AforaConfig): string[] {
...
export function hasConfiguredCommandOwners(cfg: AforaConfig): boolean {
...
export function noteCommandOwnerHealth(cfg: AforaConfig): void {
  ...
  note(
    [
      ...
      `Fix: set commands.ownerAllowFrom to your channel user id, for example ${formatCliCommand("afora config set commands.ownerAllowFrom '[\"telegram:123456789\"]'")}`,
