...
const BACKUP_RESTORE_WARNINGS = [
  ...
  "Plugin node_modules are not archived; after activation, run `afora plugins update <id>` or reinstall with `afora plugins install <spec> --force`.",
  "Generated plugin-skills links are not archived; after activation, run `afora skills list` or start an agent session to rebuild them.",
] as const;
...
async function assertTargetOutsideLiveState(targetPath: string): Promise<void> {
  ...
  if (isPathWithin(canonicalTarget, canonicalStateDir)) {
    throw new Error(
      `Backup restore target must be outside the live Afora state directory: ${targetPath}`,
    );
  }
}
...
function formatRestoreResult(result: BackupRestoreResult): string {
  return [
    ...
    "",
    "Activation is explicit: stop the Gateway, move the restored asset tree into place or point AFORA_STATE_DIR at the restored state asset, then run `afora doctor`.",
  ].join("\n");
}
