 ...
 * Mirrors the real `afora config <sub>` surface so the message only ever
...
const CONFIGURE_NON_TTY_HINT = [
  ...
  `  ${formatCliCommand("afora config set <path> <value>")}  write a config entry`,
  `  ${formatCliCommand("afora config get <path>")}          read a config entry`,
  `  ${formatCliCommand("afora config patch")}              apply a JSON patch`,
  `  ${formatCliCommand("afora config validate")}           validate configuration`,
].join("\n");
...
export async function configureCommandFromSectionsArg(
  ...
  if (invalid.length > 0) {
    runtime.error(
      `Invalid --section: ${invalid.map((section) => section || '""').join(", ")}. Expected one of: ${CONFIGURE_WIZARD_SECTIONS.join(", ")}. Run ${formatCliCommand("afora configure")} without --section to use the full wizard.`,
    );
    runtime.exit(1);
    return;
  }

  // Fail closed once at the shared entry: both `afora configure` and the
  // no-subcommand `afora config` route here, so a single guard keeps them
