...
export async function migrateListCommand(runtime: RuntimeEnv, opts: { json?: boolean } = {}) {
  ...
  if (providers.length === 0) {
    runtime.log(
      `No migration providers found. Run ${formatCliCommand("afora plugins list")} to verify provider plugins are installed and enabled.`,
    );
    return;
  }
  runtime.log(
...
export async function migratePlanCommand(
  ...
  if (!providerId) {
    throw new Error(
      `Migration provider is required. Run ${formatCliCommand("afora migrate list")} to choose one.`,
    );
  }
...
export async function migrateApplyCommand(
  ...
  if (!opts.yes && !process.stdin.isTTY) {
    throw new Error(
      `afora migrate apply requires --yes in non-interactive mode. Preview first with ${formatCliCommand(`afora migrate plan '${providerId.replaceAll("'", "'\\''")}'`)}.`,
    );
  }
