...
function getFallbacks(cfg: AforaConfig, key: DefaultsFallbackKey): string[] {
...
export async function removeFallbackCommand(
  ...
  const updated = await updateConfig((cfg) => {
    ...
    if (filtered.length === existing.length) {
      throw new Error(
        `${params.notFoundLabel} not found: ${targetKey}. Run ${formatCliCommand(`afora ${listCommandForFallbackKey(params.key)}`)} to see configured fallbacks.`,
      );
    }
