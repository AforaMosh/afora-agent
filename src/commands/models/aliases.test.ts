...
function snapshot(sourceConfig: AforaConfig) {
  return {
    valid: true,
    hash: "snapshot-hash",
    sourceConfig,
...
describe("modelsAliasesRemoveCommand", () => {
  ...
  it("removes a user-added alias from the source config", async () => {
    ...
    const written = replaceParams?.nextConfig as AforaConfig;
