...
vi.mock("../../plugins/provider-auth-choice-helpers.js", async (importOriginal) => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: AforaConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: AforaConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let lastUpdatedConfig: AforaConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/afora/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/afora/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/afora/workspace");
    ...
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: AforaConfig) => AforaConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/afora/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: AforaConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/afora/agents/coder" : "/tmp/afora/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: AforaConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/afora/workspaces/coder" : "/tmp/afora/workspace",
    );
  ...
  it("runs plugin-owned openai login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/afora/agents/main", {
      externalCli: {
        mode: "scoped",
    ...
    expect(upsertCall.agentDir).toBe("/tmp/afora/agents/main");
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Default model available: openai/gpt-5.5 (current default unchanged; run afora models set openai/gpt-5.5 to apply)",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Configure the `web_search` tool with `afora configure --section web`. Docs: https://docs.afora.ai/tools/web",
    );
  ...
  it("uses the requested agent store for provider auth login", async () => {
    ...
    expect(authRunCall.agentDir).toBe("/tmp/afora/agents/coder");
    expect(authRunCall.workspaceDir).toBe("/tmp/afora/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/afora/agents/coder");
  });
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(providerResolutionCall.workspaceDir).toBe("/tmp/afora/workspace");
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/afora/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/afora/workspace");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai. Verify plugins via `afora plugins list --json`.',
    );
  });
  ...
  it("rejects OpenAI API keys pasted as OpenAI Codex token material", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like an OpenAI API key. Use afora models auth paste-api-key --provider openai for API-key auth.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects token material pasted into the OpenAI Codex API-key command", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like token or OAuth material, not an OpenAI API key. Use afora models auth paste-token --provider openai for token auth material.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "afora agents list" to see configured agents.',
    );
  ...
  it("uses the requested agent store for setup-token provider auth", async () => {
    ...
    expect(tokenAuthCall.agentDir).toBe("/tmp/afora/agents/coder");
    expect(tokenAuthCall.workspaceDir).toBe("/tmp/afora/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/afora/agents/coder");
  });
