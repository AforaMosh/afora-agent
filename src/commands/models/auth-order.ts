...
function describeOrder(store: AuthProfileStore, provider: string, cfg: AforaConfig): string[] {
...
function describeOrderFallback(cfg: AforaConfig, provider: string): string {
  const authProvider = resolveProviderIdForAuth(provider, { config: cfg });
...
async function resolveAuthOrderContext(
  ...
  if (!rawProvider) {
    throw new Error(
      `Missing --provider. Run ${formatCliCommand("afora models auth list")} to see saved provider profiles.`,
    );
  }
...
export async function modelsAuthOrderClearCommand(
  ...
  if (!updated) {
    throw new Error(
      `Failed to update auth state; the auth state lock may be busy. Wait a moment and rerun ${formatCliCommand("afora models auth order clear --provider " + provider)}.`,
    );
  }
...
export async function modelsAuthOrderSetCommand(
  ...
  if (requested.length === 0) {
    throw new Error(
      `Missing profile ids. Run ${formatCliCommand("afora models auth list --provider " + provider)} to choose one or more profile ids.`,
    );
  }
  ...
  for (const profileId of requested) {
    ...
    if (!cred) {
      throw new Error(
        `Auth profile "${profileId}" not found in ${shortenHomePath(agentDir)}. Run ${formatCliCommand("afora models auth list --provider " + provider)} to see saved profiles.`,
      );
    }
  ...
  if (!updated) {
    throw new Error(
      `Failed to update auth state; the auth state lock may be busy. Wait a moment and rerun ${formatCliCommand("afora models auth order set --provider " + provider + " <profileIds...>")}.`,
    );
  }
