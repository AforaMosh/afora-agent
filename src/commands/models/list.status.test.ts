...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/afora-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/afora-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/afora-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/afora-dev/afora.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/afora-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/afora-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
    ...
    try {
      ...
      expect(parseFirstJsonLog(jsonRuntime).auth.unusableProfiles).toEqual([
        expect.objectContaining({
          ...
          recoveryHint:
            "Re-authenticate with `afora models auth login --provider anthropic --profile-id 'anthropic:default'`.",
        }),
      ]);
      ...
      expect(output).toContain("afora models auth login --provider anthropic");
    } finally {
  ...
  it("does not restore over plugin metadata published while status is running", async () => {
    ...
    const workspaceDir = "/tmp/afora-agent/workspace";
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/afora-dev/afora.json");
    expect(payload.auth.storePath).toBe("/tmp/afora-agent/auth-profiles.json");
  ...
  it("honors AFORA_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync({ AFORA_AGENT_DIR: "/tmp/afora-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/afora-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/afora-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/afora-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when AFORA_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync(
      {
        AFORA_AGENT_DIR: undefined,
        PI_CODING_AGENT_DIR: "/tmp/afora-legacy-agent",
      },
      async () => {
        await modelsStatusCommand({ json: true }, localRuntime as never);
      },
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/afora-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/afora-legacy-agent");
  });
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    await withAgentScopeOverrides(
      ...
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/afora-agent-custom");
    ...
    expect(payload.auth.runtimeAuthRoutes).toEqual([
      {
        ...
        effective: {
          kind: "profiles",
          detail: "/tmp/afora-agent/auth-profiles.json",
        },
      },
    ]);
    expect(localRuntime.exit).toHaveBeenCalledWith(1);
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/afora-agent/workspace"
          ? {
