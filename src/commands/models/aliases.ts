  ...
  const updated = await updateConfig((cfg) => {
    ...
    if (!found) {
      ...
      if (
        ...
        throw new Error(
          `Cannot remove "${alias}": it is a built-in alias for "${builtinTarget}" provided automatically by Afora and is not stored in your config file. To shadow it with a different target, run ${formatCliCommand(`afora models aliases add ${alias} <model>`)}.`,
        );
      }
      throw new Error(
        `Alias not found: ${alias}. Run ${formatCliCommand("afora models aliases list")} to see configured aliases.`,
      );
    }
    return {
      ...cfg,
