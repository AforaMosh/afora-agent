...
export async function printAvailablePromotionsSection(params: {
  ...
  if (unclaimed.length > 0) {
    ...
    for (const entry of unclaimed.slice(0, PROMOTIONS_SECTION_MAX_ENTRIES)) {
      ...
      runtime.log(`    Claim: ${formatCliCommand(`afora promos claim ${safe(entry.slug)}`)}`);
    }
    if (unclaimed.length > PROMOTIONS_SECTION_MAX_ENTRIES) {
      const more = unclaimed.length - PROMOTIONS_SECTION_MAX_ENTRIES;
      runtime.log(`  …and ${more} more: ${formatCliCommand("afora promos list")}`);
    }
  }
  ...
  if (unseen.length > 0) {
    runtime.log("");
    runtime.log(
      `🎁 New promotional model offers available — ${formatCliCommand("afora promos list")} for details.`,
    );
