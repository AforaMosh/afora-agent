...
async function stopAndUninstallService(runtime: RuntimeEnv): Promise<boolean> {
  if (isNixMode) {
    ...
    runtime.error(
      `Nix mode detected; service uninstall is disabled. Manage the service through your Nix profile instead, then run ${formatCliCommand("afora status")} to verify.`,
    );
    return false;
  }
  ...
  try {
    ...
    runtime.error(
      `Gateway service check failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("afora gateway status --deep")} for service diagnostics.`,
    );
    return false;
  }
  if (!loaded) {
  ...
  if (loaded) {
    try {
      ...
      stopped = false;
      runtime.error(
        `Gateway stop failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("afora gateway status --deep")} before retrying uninstall.`,
      );
    }
  }
  try {
    ...
    runtime.error(
      `Gateway uninstall failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("afora gateway status --deep")} for the service state.`,
    );
    return false;
  }
  return stopped;
...
async function removeMacApp(runtime: RuntimeEnv, dryRun?: boolean) {
  ...
  await removePath("/Applications/Afora.app", runtime, {
    dryRun,
    label: "/Applications/Afora.app",
  });
}

function logBackupRecommendation(runtime: RuntimeEnv) {
  runtime.log(`Recommended first: ${formatCliCommand("afora backup create")}`);
}
...
export async function uninstallCommand(runtime: RuntimeEnv, opts: UninstallOptions) {
  ...
  if (!interactive && !opts.yes) {
    runtime.error(
      `Non-interactive uninstall requires --yes. Preview first with ${formatCliCommand("afora uninstall --dry-run --all")}.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled<UninstallScope>({
      ...
      options: [
        ...
        { value: "state", label: "State + config", hint: "~/.afora" },
        { value: "workspace", label: "Workspace", hint: "agent files" },
        {
          value: "app",
          label: "macOS app",
          hint: "/Applications/Afora.app",
        },
      ],
