...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.afora.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.afora.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("skips every service-manager seam for a non-default install identity", async () => {
    await withEnvAsync(
      {
        AFORA_STATE_DIR: "/tmp/afora-copied-state",
        AFORA_CONFIG_PATH: "/tmp/afora-copied-state/afora.json",
      },
      async () => {
  ...
  it("still inspects the managed service for the default install identity", async () => {
    await withEnvAsync(
      { AFORA_STATE_DIR: undefined, AFORA_CONFIG_PATH: undefined },
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        AFORA_STATE_DIR: "/tmp/afora-service",
        AFORA_CONFIG_PATH: "/tmp/afora-service/afora.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { AFORA_STATE_DIR?: string; AFORA_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.AFORA_STATE_DIR).toBe("/tmp/afora-service");
    expect(handoffEnv?.AFORA_CONFIG_PATH).toBe("/tmp/afora-service/afora.json");
    expect(note).toHaveBeenCalledWith(
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "afora-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "afora-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "afora-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway service install when a system Afora gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        ...
        detail: "unit: /etc/systemd/system/afora-gateway.service",
        scope: "system",
        marker: "afora",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- afora-gateway.service (unit: /etc/systemd/system/afora-gateway.service)",
  ...
  it("reports concurrent system ownership without offering user service repair", async () => {
    ...
    service.readRuntime.mockResolvedValue({
      status: "unknown",
      detail: "System LaunchDaemon system/ai.afora.gateway owns this gateway label.",
      systemLaunchDaemon: {
        status: "loaded",
    ...
    formatGatewayRuntimeSummary.mockReturnValue(
      "unknown (System LaunchDaemon system/ai.afora.gateway owns this gateway label.)",
    );
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      "Runtime: unknown (System LaunchDaemon system/ai.afora.gateway owns this gateway label.)",
      "Gateway",
    );
    expect(note).not.toHaveBeenCalledWith("Gateway service not installed.", "Gateway");
  });
  ...
  it("surfaces typed system ownership from bootstrap repair and stops recovery", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      "System LaunchDaemon system/ai.afora.gateway owns this gateway label.",
      "Gateway",
    );
    expect(service.install).not.toHaveBeenCalled();
    expect(service.restart).not.toHaveBeenCalled();
  ...
  it("reports restart ownership failures instead of aborting doctor", async () => {
    ...
    service.restart.mockRejectedValue(
      new Error("System LaunchDaemon system/ai.afora.gateway owns this gateway label."),
    );

    await expect(runAutoRepair()).resolves.toBeDefined();

    expect(note).toHaveBeenCalledWith(
      "Gateway service restart failed: System LaunchDaemon system/ai.afora.gateway owns this gateway label.",
      "Gateway",
    );
  });
