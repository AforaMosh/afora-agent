...
describe("noteAuthProfileHealth", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-doctor-auth-"));
  ...
  function expectedAuthStorePath(agentDir: string): string {
    return path.join(agentDir, "afora-agent.sqlite");
  }
  ...
  it.each([
    [
      "auth_permanent",
      "Re-authenticate with `afora models auth login --provider openai --profile-id 'openai:disabled'`.",
    ],
  ...
  it("maps malformed API-key auth profiles to structured findings", async () => {
    ...
    expect(findings).toEqual([
      expect.objectContaining({
        ...
        fixHint: "Paste the API key value, not an Afora onboarding command.",
      }),
    ]);
  });
  ...
  it.each([
    [
      ...
      "- openai-codex:default: re-auth required [refresh_token_reused] — Run `afora models auth login --provider openai`.",
    ],
    [
      "openai-codex:default",
      "OAuth token refresh failed for openai-codex: temporary upstream issue. Please try again or re-authenticate.",
      "- openai-codex:default: OAuth refresh failed — Try again; if this persists, run `afora models auth login --provider openai`.",
    ],
    [
      "OpenAI Work Profile",
      "OAuth token refresh failed for openai: invalid_grant. Please try again or re-authenticate.",
      "- OpenAI Work Profile: re-auth required [invalid_grant] — Run `afora models auth login --provider openai --profile-id 'OpenAI Work Profile'`.",
    ],
    [
      "openai-codex:default",
      "OAuth token refresh failed for openai-codex`\nrm -rf /: invalid_grant. Please try again or re-authenticate.",
      "- openai-codex:default: re-auth required [invalid_grant] — Run `afora models auth login --provider openai`.",
    ],
  ])(
