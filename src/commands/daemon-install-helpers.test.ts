...
vi.mock("../daemon/program-args.js", () => ({
  ...
  resolveAforaWrapperPath: mocks.resolveAforaWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { AFORA_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveAforaWrapperPath.mockImplementation(async (value: string | undefined) =>
...
async function buildPluginConfigExecSecretRefPlan(home: string) {
  mockNodeGatewayPlanFixture({ serviceEnvironment: { AFORA_PORT: "3000" } });
...
describe("buildGatewayInstallPlan", () => {
  ...
  // Prevent tests from reading the developer's real ~/.afora/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ AFORA_PORT: "3000" });
  ...
  it("adds the active afora command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const aforaBinPath = path.join(isolatedHome, ".npm-global", "bin", "afora");
    process.argv = ["node", aforaBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(aforaBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".afora"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes AFORA_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/afora-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        AFORA_PORT: "3000",
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.AFORA_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.AFORA_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".afora", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        AFORA_PORT: "3000",
    ...
    expect(plan.environment.AFORA_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring AFORA_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.AFORA_PORT).toBe("3000");
    expect(plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("renders config env SecretRefs as file-backed managed values on Linux", async () => {
    ...
    expect(plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
  ...
  it("includes passEnv values for configured exec SecretRef providers", async () => {
    ...
    expect(plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/afora-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/afora-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        AFORA_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "AFORA_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.AFORA_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS).toBe("AFORA_GATEWAY_TOKEN");
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".afora"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        AFORA_PORT: "3000",
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".afora"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("retains config SecretRef env values for macOS LaunchAgent env files", async () => {
    ...
    expect(plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS).toBe("TELEGRAM_DEFAULT_BOTTOKEN");
  });
  ...
  it("retains existing generated env-file SecretRef values for macOS LaunchAgent regeneration", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        AFORA_SERVICE_MANAGED_ENV_KEYS:
  ...
  it("retains active gateway auth and channel SecretRefs through real LaunchAgent regeneration", async () => {
    ...
    const existing = await readGeneratedLaunchAgentFixture({
      root: tmpDir,
      environment: {
        AFORA_GATEWAY_AUTH_TOKEN: "gateway-existing-env-file-token",
        OLD_GATEWAY_AUTH_TOKEN: "stale-gateway-token",
        RETIRED_BOTTOKEN: "retired-channel-token",
        TELEGRAM_DEFAULT_BOTTOKEN: "telegram-existing-env-file-token",
        AFORA_SERVICE_MANAGED_ENV_KEYS:
          "OLD_GATEWAY_AUTH_TOKEN,AFORA_GATEWAY_AUTH_TOKEN,RETIRED_BOTTOKEN,TELEGRAM_DEFAULT_BOTTOKEN",
      },
    });
    ...
    expect(plan.environment.AFORA_GATEWAY_AUTH_TOKEN).toBe("gateway-existing-env-file-token");
    expect(plan.environment.TELEGRAM_DEFAULT_BOTTOKEN).toBe("telegram-existing-env-file-token");
    expect(plan.environmentValueSources?.AFORA_GATEWAY_AUTH_TOKEN).toBe("file");
    expect(plan.environmentValueSources?.TELEGRAM_DEFAULT_BOTTOKEN).toBe("file");
    expect(plan.environment.OLD_GATEWAY_AUTH_TOKEN).toBeUndefined();
    expect(plan.environment.RETIRED_BOTTOKEN).toBeUndefined();
    expect(plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS).toBe(
      "AFORA_GATEWAY_AUTH_TOKEN,TELEGRAM_DEFAULT_BOTTOKEN",
    );
    ...
    await stageLaunchAgent({
      env: { HOME: home, AFORA_STATE_DIR: stateDir, AFORA_LAUNCHD_LABEL: label },
    ...
    expect(envFile).toContain(
      "export AFORA_GATEWAY_AUTH_TOKEN='gateway-existing-env-file-token'",
    );
    expect(envFile).toContain(
    ...
    expect(plist).not.toContain("AFORA_GATEWAY_AUTH_TOKEN");
    ...
    expect(rewritten?.environment?.AFORA_GATEWAY_AUTH_TOKEN).toBe(
    ...
    expect(rewritten?.environmentValueSources?.AFORA_GATEWAY_AUTH_TOKEN).toBe("file");
  });
  ...
  it.each([
    {
      ...
      surface: "token",
      mode: "token",
      configuredKey: "AFORA_GATEWAY_AUTH_TOKEN",
      existingKey: "AFORA_GATEWAY_AUTH_TOKEN",
      existingSource: "file",
    ...
    },
    {
      ...
      surface: "password",
      mode: "password",
      configuredKey: "AFORA_GATEWAY_AUTH_PASSWORD",
      existingKey: "AFORA_GATEWAY_AUTH_PASSWORD",
      existingSource: "file",
      expectedValue: "existing-secret",
    },
    {
    ...
    const existingEnvironment = existingKey
      ? {
          ...
          AFORA_SERVICE_MANAGED_ENV_KEYS: existingKey,
        }
      : undefined;
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment,
      existingEnvironmentValueSources,
      config: { gateway: { auth } } as unknown as AforaConfig,
    });
  ...
  it("lets the state-dir dotenv source replace an older file-backed gateway credential", async () => {
    await writeStateDirDotEnv("AFORA_GATEWAY_AUTH_TOKEN=authoritative-state-token\n", {
      stateDir: path.join(tmpDir, ".afora"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      platform: "darwin",
      existingEnvironment: {
        AFORA_GATEWAY_AUTH_TOKEN: "stale-file-token",
        AFORA_SERVICE_MANAGED_ENV_KEYS: "AFORA_GATEWAY_AUTH_TOKEN",
      },
      existingEnvironmentValueSources: {
        AFORA_GATEWAY_AUTH_TOKEN: "file",
      },
      config: {
        gateway: {
          auth: {
    ...
    expect(plan.environment.AFORA_GATEWAY_AUTH_TOKEN).toBe("authoritative-state-token");
    expect(plan.environmentValueSources?.AFORA_GATEWAY_AUTH_TOKEN).toBe("inline");
  });
  ...
  it("does not retain config env values for macOS LaunchAgent env files", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        AFORA_SERVICE_MANAGED_ENV_KEYS: "BRAVE_API_KEY",
      },
      existingEnvironmentValueSources: {
        BRAVE_API_KEY: "file",
        AFORA_SERVICE_MANAGED_ENV_KEYS: "inline",
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.AFORA_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        AFORA_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.AFORA_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps differently-cased source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.AFORA_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.AFORA_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:afora|afora)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale AFORA_ALLOW_ROOT", async () => {
    const result = await buildEnvironment({ AFORA_ALLOW_ROOT: "1" });
    expect(result.AFORA_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves AFORA_CLI_CONTAINER_BYPASS and AFORA_CONTAINER_HINT", async () => {
    const result = await buildEnvironment({
      AFORA_CLI_CONTAINER_BYPASS: "1",
      AFORA_CONTAINER_HINT: "ci",
    });
    expect(result.AFORA_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.AFORA_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary AFORA_FOO", async () => {
    const result = await buildEnvironment({ AFORA_FOO: "bar" });
    expect(result.AFORA_FOO).toBeUndefined();
  });

  it("drops legacy install-time version metadata from canonical rewrites", async () => {
    const result = await buildEnvironment({ AFORA_SERVICE_VERSION: "2026.4.24" });
    expect(result.AFORA_SERVICE_VERSION).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated AFORA_* keys", async () => {
    const result = await buildEnvironment({
      AFORA_CLI_CONTAINER_BYPASS: "1",
      AFORA_CONTAINER_HINT: "ci",
      AFORA_BAZ: "qux",
    ...
    expect(result.AFORA_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.AFORA_CONTAINER_HINT).toBe("ci");
    expect(result.AFORA_BAZ).toBeUndefined();
  });
});
