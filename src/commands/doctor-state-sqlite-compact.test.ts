...
function createStateEnv(): NodeJS.ProcessEnv {
  const stateDir = tempDirs.make("afora-state-compact-");
...
function seedStateDatabase(params: {
  ...
  const sqlitePath = resolveAforaStateSqlitePath(params.env);
  fs.mkdirSync(path.dirname(sqlitePath), { recursive: true });
  const sqlite = requireNodeSqlite();
  const database = new sqlite.DatabaseSync(sqlitePath);
  const schemaVersion = params.schemaVersion ?? AFORA_STATE_SCHEMA_VERSION;
  try {
    database.exec(`
...
describe("runDoctorStateSqliteCompact", () => {
  ...
  it("clears authoritative quarantine after compaction", async () => {
    ...
    expect(readAforaDatabaseQuarantine(sqlitePath, { env })).toBeUndefined();
  ...
  it.each([
    ["legacy", AFORA_STATE_SCHEMA_VERSION - 1, /doctor --fix before compacting/],
    ["future", AFORA_STATE_SCHEMA_VERSION + 1, /uses newer schema version/],
  ] as const)(
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const env = createStateEnv();
      const canonicalPath = resolveAforaStateSqlitePath(env);
      ...
      runDoctorStateSqliteCompact(
        { env },
        {
          withMaintenanceLock: async () => {
            throw new Error("Gateway owns this Afora state directory");
          },
        },
      ),
  ...
  it("treats a busy truncating checkpoint as failure", async () => {
    ...
    try {
      ...
      expect(readAforaDatabaseQuarantine(sqlitePath, { env })?.reason).toBe("busy checkpoint");
    } finally {
