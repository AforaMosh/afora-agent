...
function readStoredToken(env: NodeJS.ProcessEnv): string | null | undefined {
  ...
  return executeSqliteQueryTakeFirstSync(
    database.db,
    getNodeSqliteKysely<Pick<AforaStateKyselyDatabase, "node_host_config">>(database.db)
...
async function runConcurrentImplicitConfigures(
  ...
  const workerSource = `
    ...
    const { configureNodeHost } = await import(process.env.AFORA_NODE_HOST_CONFIG_MODULE);
    fs.writeFileSync(process.env.AFORA_NODE_HOST_READY_PATH, "ready");
    const deadline = Date.now() + 15_000;
    while (!fs.existsSync(process.env.AFORA_NODE_HOST_START_PATH)) {
    ...
    const config = await configureNodeHost({
      candidateNodeId: process.env.AFORA_NODE_HOST_CANDIDATE,
      fallbackDisplayName: "node",
      gateway: {},
      env: { ...process.env, AFORA_STATE_DIR: process.env.AFORA_NODE_HOST_STATE_DIR },
      nowMs: Number(process.env.AFORA_NODE_HOST_NOW_MS),
    });
  ...
  const workers = ["candidate-a", "candidate-b"].map((candidate, index) => {
    ...
    const child = spawn(
      ...
      ["--import", "tsx", "--input-type=module", "-e", workerSource],
      {
        env: {
          ...process.env,
          AFORA_NODE_HOST_CANDIDATE: candidate,
          AFORA_NODE_HOST_CONFIG_MODULE: moduleUrl,
          AFORA_NODE_HOST_NOW_MS: String(index + 1),
          AFORA_NODE_HOST_READY_PATH: readyPath,
          AFORA_NODE_HOST_START_PATH: startPath,
          AFORA_NODE_HOST_STATE_DIR: stateDir,
        },
...
describe("node-host SQLite config", () => {
  ...
  function makeTestEnv(): { env: NodeJS.ProcessEnv; stateDir: string } {
    const stateDir = tempDirs.make("afora-node-host-config-");
  ...
  it("rejects corrupt canonical rows instead of rotating identity", async () => {
    ...
    runAforaStateWriteTransaction(
      ({ db }) => {
        executeSqliteQuerySync(
          db,
          getNodeSqliteKysely<Pick<AforaStateKyselyDatabase, "node_host_config">>(db)
