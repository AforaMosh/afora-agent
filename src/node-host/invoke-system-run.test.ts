...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  let previousAforaHome: string | undefined;
  const sharedRuntimeBins = new Set<string>();

  beforeAll(() => {
    closeAforaStateDatabaseForTest();
    sharedFixtureRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "afora-node-host-fixtures-")),
    );
    sharedAforaHome = path.join(sharedFixtureRoot, "afora-home");
    sharedRuntimeBinDir = path.join(sharedFixtureRoot, "bin");
    fs.mkdirSync(sharedAforaHome, { recursive: true });
  ...
  beforeEach(() => {
    ...
    process.env.AFORA_HOME = sharedAforaHome;
    execApprovalsStoreTesting.reset();
    // Cases isolate the canonical policy row, not shared-state schema bootstrap.
    deleteExecApprovalsConfigRow(openAforaStateDatabase().db);
    clearRuntimeConfigSnapshot();
  });
  ...
  afterEach(() => {
    execApprovalsStoreTesting.reset();
    clearRuntimeConfigSnapshot();
    if (previousAforaHome === undefined) {
      delete process.env.AFORA_HOME;
    } else {
      process.env.AFORA_HOME = previousAforaHome;
    }
  });
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = sharedAforaHome;
  ...
  it("uses auto reviewer for system.run approval misses when exec mode is auto", async () => {
    const tmp = createFixtureDir("afora-system-run-auto-review-");
  ...
  it("does not auto-review direct system.run approval misses without an approval plan", async () => {
    const tmp = createFixtureDir("afora-system-run-auto-review-no-plan-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
  ...
  it.each([
    ...
    const tmp = createFixtureDir("afora-system-run-auto-review-failure-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
  ...
  it.runIf(process.platform !== "win32").each(["bash", "sh", "/bin/sh"])(
    ...
    async (shell) => {
      const tmp = createFixtureDir("afora-system-run-auto-review-login-");
      setRuntimeConfigSnapshot({ tools: { exec: { mode: "auto" } } });
      try {
        const autoReviewer = vi.fn<ExecAutoReviewer>(() => ({
          decision: "allow-once",
  ...
  it("does not auto-review direct system.run security audit suppression edits", async () => {
    const tmp = createFixtureDir("afora-system-run-auto-review-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "afora" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
          mode: "auto",
  ...
  it("defers to human approval when system.run auto reviewer asks", async () => {
    const tmp = createFixtureDir("afora-system-run-auto-review-ask-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      for (const testCase of approvedEnvShellWrapperCases) {
        const tmp = createFixtureDir("afora-approved-wrapper-");
  ...
  it("denies shell payload carriers in allowlist mode without explicit approval", async () => {
    ...
    > = [
      ...
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "afora status; id"]
            : ["/bin/sh", "-lc", "afora status; id"],
        approvalRequired: true,
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "afora status; cat /etc/passwd"]
            : ["/bin/sh", "-lc", "afora status; cat /etc/passwd"],
        approvalRequired: true,
      },
      {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const { runCommand, sendInvokeResult } = await runSystemInvoke({
        ...
        security: "allowlist",
        ask: "off",
        command: ["/bin/sh", "-lc", "head -c${IFS}16${IFS}${AFORA_CONFIG_PATH}"],
        rawCommand: "head -c${IFS}16${IFS}${AFORA_CONFIG_PATH}",
      });
  ...
  it("denies abbreviated PowerShell encoded payloads even when the wrapper is allowlisted", async () => {
    const binDir = createFixtureDir("afora-pwsh-allowlist-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "afora-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "afora-allowlist-path-pin-",
      ...
      for (const testCase of [
        {
          label: "cwd symlink",
          setup: () => {
            const tmp = createFixtureDir("afora-approval-cwd-link-");
        ...
        },
        {
          ...
          setup: () => {
            const tmp = createFixtureDir("afora-approval-cwd-parent-link-");
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = createFixtureDir("afora-approval-cwd-real-");
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = createFixtureDir("afora-approval-cwd-drift-");
    const fallback = createFixtureDir("afora-approval-cwd-drift-alt-");
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("validates approved script operand bindings at dispatch", async () => {
    for (const mutate of [true, false]) {
      const tmp = createFixtureDir(
        mutate ? "afora-approval-script-drift-" : "afora-approval-script-stable-",
      );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = createFixtureDir("afora-approval-cwd-post-commit-drift-");
  ...
  it("revalidates approved script operands after authorization commit", async () => {
    const tmp = createFixtureDir("afora-approval-script-post-commit-drift-");
    const fixture = createMutableScriptOperandFixture(tmp);
    fs.writeFileSync(fixture.scriptPath, fixture.initialBody);
    if (process.platform !== "win32") {
      fs.chmodSync(fixture.scriptPath, 0o755);
  ...
  it("validates approved runtime script operand bindings at dispatch", async () => {
    await withFakeRuntimeOnPath({
      runtime: "tsx",
      run: async () => {
        const tmp = createFixtureDir("afora-approval-tsx-script-drift-");
        ...
        const missingBindingTmp = createFixtureDir("afora-approval-tsx-missing-binding-");
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `afora-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("fails closed when allow-always approval persistence fails", async () => {
    await withTempApprovalsHome({
      approvals: createAllowlistOnMissApprovals(),
      run: async () => {
        const tempDir = createFixtureDir("afora-allow-always-write-failure-");
  ...
  it("does not restore a revoked allowlist rule during explicit allow-always persistence", async () => {
    const tempDir = createFixtureDir("afora-allow-always-revoked-rule-");
    const executablePath = createTempExecutable({ dir: tempDir, name: "approved-tool" });
  ...
  it("fails closed when allowlist usage persistence fails", async () => {
    const tempDir = createFixtureDir("afora-allowlist-usage-write-failure-");
  ...
  it("does not let forwarded auto-review authorize security audit suppression edits", async () => {
    const tmp = createFixtureDir("afora-forwarded-auto-review-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "afora" });
    const prepared = buildSystemRunApprovalPlan({
      command: [executablePath, "config", "set", "security.audit.suppressions", "[]"],
      cwd: tmp,
  ...
  it("preserves exact-plan forwarded auto-review for strict inline eval", async () => {
    const plan = createStrictInlineEvalApprovalPlan("afora-forwarded-inline-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("afora-fallback-durable-");
      const prepared = buildSystemRunApprovalPlan({
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("afora-fallback-durable-revoked-");
      const prepared = buildSystemRunApprovalPlan({
        command: ["/bin/sh", "-c", "/bin/ls"],
        cwd: tempDir,
        sessionKey: "agent:main:main",
  ...
  it("does not let timeout fallback satisfy strict inline review", async () => {
    const plan = createStrictInlineEvalApprovalPlan("afora-fallback-inline-");
    setRuntimeConfigSnapshot({ tools: { exec: { strictInlineEval: true } } });
    try {
      await withTempApprovalsHome({
        approvals: {
  ...
  it("does not let timeout fallback authorize security audit suppression edits", async () => {
    const tmp = createFixtureDir("afora-timeout-fallback-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "afora" });
    const prepared = buildSystemRunApprovalPlan({
      command: [executablePath, "config", "set", "security.audit.suppressions", "[]"],
      cwd: tmp,
  ...
  it("keeps audit suppression edits approval-gated under allowlist fallback from full/off", async () => {
    const tmp = createFixtureDir("afora-timeout-fallback-full-off-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "afora" });
    const prepared = buildSystemRunApprovalPlan({
      command: [executablePath, "config", "set", "security.audit.suppressions", "[]"],
      cwd: tmp,
  ...
  it("rejects unbindable strict inline-eval carriers before delayed approval", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("afora-inline-eval-bin-");
  ...
  it("persists benign awk allow-always approvals in strict inline-eval mode without reopening inline carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("afora-inline-eval-awk-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "gawk",
          });
  ...
  it("does not persist allow-always approvals for strict inline-eval make carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("afora-inline-eval-make-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "make",
          });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("afora-shell-wrapper-inner-");
  ...
  it("keeps cmd.exe transport wrappers approval-gated on Windows", async () => {
    ...
    try {
      for (const testCase of [
        ...
        const tempDir = createFixtureDir("afora-cmd-wrapper-allow-");
  ...
  it("fails closed when cmd.exe wrapper trust is downgraded before execution", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    try {
      const tempDir = createFixtureDir("afora-cmd-wrapper-downgraded-");
  ...
  it("reuses exact-command durable trust for shell-wrapper reruns", async () => {
    ...
    const tempDir = createFixtureDir("afora-shell-wrapper-allow-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "/bin/ls"],
      cwd: tempDir,
    });
  ...
  it("does not bind safe builtin policy to a redundant exact-command grant", async () => {
    ...
    const tempDir = createFixtureDir("afora-shell-wrapper-redundant-grant-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "cd ."],
      cwd: tempDir,
    });
  ...
  it("fails closed when an exact-command grant is revoked before execution", async () => {
    ...
    const tempDir = createFixtureDir("afora-shell-wrapper-revoked-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "/bin/ls"],
      cwd: tempDir,
    });
