...
describe("node host invoke", () => {
  ...
  it("lists node-host directories for the folder browser", async () => {
    const root = fs.realpathSync(tempDirs.make("afora-node-fs-listdir-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = fs.realpathSync(tempDirs.make("afora-node-skill-cwd-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-prepare-env-"));
  ...
  it("forwards suppressNotifyOnExit on completed system.run events", async () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-node-event-suppress-"));
    const stateDir = path.join(tempHome, ".afora");
    try {
      await withEnvAsync({ AFORA_HOME: tempHome, AFORA_STATE_DIR: stateDir }, async () => {
        saveExecApprovals({
          version: 1,
