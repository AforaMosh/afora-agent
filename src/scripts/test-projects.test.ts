...
describe("test-projects args", () => {
  ...
  it.each([
    {
      ...
      target: "src/infra/afora-root.test.ts",
  ...
  it("routes infra targets to the infra config", () => {
    expect(buildVitestRunPlans(["src/infra/afora-root.test.ts"])).toEqual([
      {
        config: "test/vitest/vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/afora-root.test.ts"],
        watchMode: false,
      },
    ]);
  ...
  it("caps project-level parallelism when the Vitest worker budget is conservative", () => {
    ...
    expect(
      resolveParallelFullSuiteConcurrency(58, {
        AFORA_TEST_WORKERS: "1",
      }),
    ).toBe(1);
  });

  it("keeps conservative local full-suite runs on leaf project configs", () => {
    const originalVitestMaxWorkers = process.env.AFORA_VITEST_MAX_WORKERS;
    const originalTestWorkers = process.env.AFORA_TEST_WORKERS;
    const originalProjectParallel = process.env.AFORA_TEST_PROJECTS_PARALLEL;
    const originalLeafShards = process.env.AFORA_TEST_PROJECTS_LEAF_SHARDS;
    ...
    try {
      ...
      if (originalVitestMaxWorkers === undefined) {
        ...
        process.env.AFORA_VITEST_MAX_WORKERS = originalVitestMaxWorkers;
      }
      if (originalTestWorkers === undefined) {
        delete process.env.AFORA_TEST_WORKERS;
      } else {
        process.env.AFORA_TEST_WORKERS = originalTestWorkers;
      }
      if (originalProjectParallel === undefined) {
        delete process.env.AFORA_TEST_PROJECTS_PARALLEL;
      } else {
        process.env.AFORA_TEST_PROJECTS_PARALLEL = originalProjectParallel;
      }
      if (originalLeafShards === undefined) {
        delete process.env.AFORA_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.AFORA_TEST_PROJECTS_LEAF_SHARDS = originalLeafShards;
      }
  ...
  it("uses the global host worker budget for full-suite project parallelism", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        58,
        {
          AFORA_TEST_PROJECTS_LEAF_SHARDS: "1",
        },
        {
          cpuCount: 8,
          loadAverage1m: 0,
  ...
  it("gives parallel Vitest shards separate filesystem module caches", () => {
    ...
    expect(firstEnv?.AFORA_VITEST_FS_MODULE_CACHE_PATH?.replaceAll("\\", "/")).toBe(
      "/repo/node_modules/.experimental-vitest-cache/0-test-vitest-vitest.gateway.config.ts",
    );
    expect(specs[1]?.env.AFORA_VITEST_FS_MODULE_CACHE_PATH?.replaceAll("\\", "/")).toBe(
  ...
  it("routes the Docker package contract without private-QA E2E setup", () => {
    const target = "test/e2e/qa-lab/runtime/package-afora-for-docker.e2e.test.ts";
  ...
  it("routes bundled plugin manifest changes through the docs config audit", () => {
    expect(resolveChangedTestTargetPlan(["extensions/voice-call/afora.plugin.json"])).toEqual({
      mode: "targets",
      targets: [
        "extensions/voice-call/afora.plugin.json",
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("afora-vitest-include-");
    expect(spec?.env.AFORA_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });

  it("rejects explicit test file targets that do not exist", () => {
    expect(findUnmatchedExplicitTestTargets(["src/not-a-real-afora-test.test.ts"])).toEqual([
      {
        target: "src/not-a-real-afora-test.test.ts",
  ...
  it("skips channel contract configs with no matching external include patterns", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-contract-include-"));
    try {
