...
async function makePlan(params?: {
  ...
  const root = tempDirs.make("afora-claw-workspace-");
...
function stateEnv(root: string) {
  return { AFORA_STATE_DIR: join(root, "state") };
}
...
function readWorkspaceFileRows(agentId: string, root: string) {
  const rows = openAforaStateDatabase({ env: stateEnv(root) })
    .db.prepare(
  ...
  return rows.map((row) => ({
    schemaVersion: "afora.clawWorkspaceFileRecord.v1" as const,
...
function readInstallStatus(agentId: string, root: string): string | undefined {
  const row = openAforaStateDatabase({ env: stateEnv(root) })
...
describe("createClawWorkspaceFiles", () => {
  it("materializes the CLAW.md body as managed SOUL.md content", async () => {
    const root = tempDirs.make("afora-claw-body-workspace-");
    const workspace = join(root, "workspace-agent");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = tempDirs.make("afora-claw-linked-package-");
  ...
  it("creates canonical bootstrap and supporting files and records their hashes", async () => {
    ...
    expect(records).toEqual([
      expect.objectContaining({
        schemaVersion: "afora.clawWorkspaceFileRecord.v1",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const outside = tempDirs.make("afora-claw-outside-");
  ...
  it("does not adopt an independently created file after a failed write record", async () => {
    ...
    openAforaStateDatabase({ env: stateEnv(root) })
      .db.prepare(
      ...
      .run(
        "afora.clawWorkspaceFileRecord.v1",
        plan.agent.finalId,
