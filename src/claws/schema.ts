...
const aforaExtensionSchema = z
  .object({
    id: agentId,
    kind: z.literal("plugin"),
    format: z.enum(["afora", "claude", "codex", "cursor"]),
...
const aforaProfileSchema = z
  .object({
    ...
    agent: z
          ...
          .object({
            profile: nonEmptyString
              .refine(
                ...
                "Tool profile must name a registered Afora built-in profile.",
              )
              .optional(),
            allow: z
              ...
              .superRefine((search, ctx) => {
                if (
                  ...
                  ctx.addIssue({
                    ...
                    message:
                      "The sessions source requires rememberAcrossConversations: true in the Afora profile.",
                  });
                }
              })
              .optional(),
      ...
      .strict(),
    extensions: z.array(aforaExtensionSchema).optional().default([]),
  })
  .strict()
...
export function parseClawAforaProfile(value: unknown):
  | {
      ok: true;
  ...
  | { ok: false; diagnostics: ClawDiagnostic[] } {
  const parsed = aforaProfileSchema.safeParse(value);
  if (!parsed.success) {
    return { ok: false, diagnostics: diagnosticsFromZodError(parsed.error) };
  }
  return {
    ok: true,
    profile: parsed.data as ClawAforaProfile,
    diagnostics: [],
  };
}
