...
async function writeRichProject(root: string): Promise<void> {
  ...
  await writeFile(join(root, "profiles", "afora.yml"), "schemaVersion: 1\nagent: {}\n");
...
describe("Claw projects", () => {
  it("matches the cross-platform golden artifact digest", async () => {
    const output = join(tempDirs.make("afora-claw-golden-"), "golden.tgz");
  ...
  it("matches the golden artifact digest under a restrictive umask", () => {
    const output = join(tempDirs.make("afora-claw-umask-"), "golden.tgz");
  ...
  it("creates a minimal project that validates through the canonical reader", async () => {
    const root = join(tempDirs.make("afora-claw-create-"), "research-assistant");
    ...
    if (validated.ok) {
      ...
      expect(validated.claw.clawMarkdownBody?.toString()).toContain("purpose-built Afora agent");
    }
  });

  it("keeps one concurrent creator's completed project", async () => {
    const root = join(tempDirs.make("afora-claw-create-race-"), "shared");
    await mkdir(root);
  ...
  it("refuses occupied targets and package lifecycle scripts", async () => {
    const occupied = tempDirs.make("afora-claw-occupied-");
    ...
    const project = tempDirs.make("afora-claw-scripts-");
  ...
  it("rejects package.json as a managed workspace source", async () => {
    const project = tempDirs.make("afora-claw-package-source-");
    const output = join(tempDirs.make("afora-claw-package-source-output-"), "claw.tgz");
    await writeRichProject(project);
  ...
  it("builds byte-identical artifacts containing only declared project inputs", async () => {
    const project = tempDirs.make("afora-claw-build-");
    const output = tempDirs.make("afora-claw-output-");
    await writeRichProject(project);
    ...
    expect(entries).toEqual([
      ...
      "package/profiles/afora.yml",
      "package/workspace/reference.md",
    ]);
    expect(entries).not.toContain("package/not-packed.txt");
  });

  it("preserves the canonical metadata-selected Afora profile path", async () => {
    const project = tempDirs.make("afora-claw-custom-profile-");
    const output = join(tempDirs.make("afora-claw-custom-profile-output-"), "claw.tgz");
    await writeRichProject(project);
    await rename(
      join(project, "profiles", "afora.yml"),
    ...
    await writeFile(
      ...
      manifest.replace(
        "agent:\n  id: demo-claw",
        "agent:\n  id: demo-claw\nmetadata:\n  afora.config: profiles/custom.yaml",
      ),
    );
    ...
    if (validation.ok) {
      expect(validation.claw.snapshot.aforaProfile?.sourcePath).toBe("profiles/custom.yaml");
    ...
    expect(entries).not.toContain("package/profiles/afora.yml");
  });

  it("packages a leading-at workspace source as an ordinary file", async () => {
    const project = tempDirs.make("afora-claw-leading-at-");
    const output = join(tempDirs.make("afora-claw-leading-at-output-"), "claw.tgz");
    await writeRichProject(project);
    const manifest = await readFile(join(project, "CLAW.md"), "utf8");
    await writeFile(
      join(project, "CLAW.md"),
  ...
  it("packages a valid source whose filename begins with two dots", async () => {
    const project = tempDirs.make("afora-claw-leading-dots-");
    const output = join(tempDirs.make("afora-claw-leading-dots-output-"), "claw.tgz");
    await writeRichProject(project);
    const manifest = await readFile(join(project, "CLAW.md"), "utf8");
    await writeFile(
      join(project, "CLAW.md"),
  ...
  it("normalizes accepted backslash source separators in the built package", async () => {
    const project = tempDirs.make("afora-claw-backslash-source-");
    const output = join(tempDirs.make("afora-claw-backslash-source-output-"), "claw.tgz");
    await writeRichProject(project);
    const manifest = await readFile(join(project, "CLAW.md"), "utf8");
    await writeFile(
      join(project, "CLAW.md"),
  ...
  it("preserves long workspace source paths deterministically", async () => {
    const project = tempDirs.make("afora-claw-long-path-");
    const output = tempDirs.make("afora-claw-long-path-output-");
    await writeRichProject(project);
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      const project = tempDirs.make("afora-claw-selected-case-");
      await writeRichProject(project);
  ...
  it("dereferences only a confined CLAW.md symlink into the artifact", async () => {
    const project = tempDirs.make("afora-claw-manifest-link-");
    const output = join(tempDirs.make("afora-claw-manifest-link-output-"), "linked.tgz");
    const unpacked = tempDirs.make("afora-claw-manifest-link-unpacked-");
    await writeRichProject(project);
  ...
  it.each([".git/CLAW.md", "node_modules/example/CLAW.md"])(
    ...
    async (targetPath) => {
      const project = tempDirs.make("afora-claw-manifest-excluded-link-");
      await writeRichProject(project);
  ...
  it("rejects a CLAW.md symlink that escapes the project", async () => {
    const project = tempDirs.make("afora-claw-manifest-escape-");
    const outside = tempDirs.make("afora-claw-manifest-outside-");
    await writeRichProject(project);
  ...
  it.each([
    ...
    const project = tempDirs.make("afora-claw-excluded-source-");
    const output = join(tempDirs.make("afora-claw-excluded-source-output-"), "claw.tgz");
    await writeRichProject(project);
  ...
  it("rejects a custom profile selected from an excluded tree", async () => {
    const project = tempDirs.make("afora-claw-excluded-profile-");
    await writeRichProject(project);
    await rename(
      join(project, "profiles", "afora.yml"),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const project = tempDirs.make("afora-claw-manifest-case-collision-");
      await writeRichProject(project);
      const manifest = await readFile(join(project, "CLAW.md"), "utf8");
      await writeFile(
        join(project, "CLAW.md"),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const project = tempDirs.make("afora-claw-profile-case-collision-");
      await writeRichProject(project);
      await rename(
        join(project, "profiles", "afora.yml"),
        join(project, "profiles", "custom.yaml"),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const project = tempDirs.make("afora-claw-unicode-collision-");
      await writeRichProject(project);
  ...
  it("preserves an existing build destination", async () => {
    const project = tempDirs.make("afora-claw-build-existing-");
    const output = join(tempDirs.make("afora-claw-output-existing-"), "existing.tgz");
    await writeRichProject(project);
  ...
  it("rejects ambiguous nested project discovery", async () => {
    const outer = tempDirs.make("afora-claw-nested-");
  ...
  it("changes the artifact digest when a declared input changes", async () => {
    const project = tempDirs.make("afora-claw-build-change-");
    const output = tempDirs.make("afora-claw-output-change-");
    await writeRichProject(project);
