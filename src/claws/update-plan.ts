...
export async function buildClawUpdatePlan(params: {
  ...
  stateOptions?: AforaStateDatabaseOptions & { packageDeps?: PackageRemovalDeps };
  ...
  const database =
    ...
    (await openExistingAforaStateDatabaseReadOnly(params.stateOptions));
  if (!database) {
  ...
  const readOnlyStateOptions: AforaStateDatabaseOptions & {
    ...
    for (const [key, target] of targetPackages) {
      ...
      if (failedPackageMutationPreflight) {
        ...
        const extensionIndex =
          params.targetAforaProfile?.extensions?.findIndex(
            (extension) => clawPackageKey(extension) === key,
          ) ?? -1;
        const path =
          packageIndex >= 0
            ? `$.packages[${packageIndex}]`
            : `$.profiles.afora.extensions[${extensionIndex}]`;
