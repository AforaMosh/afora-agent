...
> {
  ...
  const conventionalExists = await packageFiles.exists(OPENCLAW_CLAW_PROFILE_PATH);
  const legacyPointer = params.metadata?.[LEGACY_PROFILE_POINTER_KEY];
  const diagnostics: ClawDiagnostic[] = [];
  let declaredPath = OPENCLAW_CLAW_PROFILE_PATH;
  ...
  if (legacyPointer !== undefined) {
    ...
    if (conventionalExists && legacyPointer !== OPENCLAW_CLAW_PROFILE_PATH) {
      return {
        ok: false,
        diagnostics: [
          diagnostic(
            "conflicting_openclaw_profile_pointer",
            `metadata.${LEGACY_PROFILE_POINTER_KEY} references ${legacyPointer} while ${OPENCLAW_CLAW_PROFILE_PATH} also exists; keep only ${OPENCLAW_CLAW_PROFILE_PATH}.`,
            LEGACY_PROFILE_POINTER_PATH,
          ),
        ],
      };
    ...
    diagnostics.push(
      warning(
        ...
        `metadata.${LEGACY_PROFILE_POINTER_KEY} is deprecated; move the profile to ${OPENCLAW_CLAW_PROFILE_PATH} and remove the metadata entry.`,
        LEGACY_PROFILE_POINTER_PATH,
      ),
    );
