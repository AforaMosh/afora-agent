...
import { parseClawAforaProfile } from "./schema.js";

const tempDirs = useAutoCleanupTempDirTracker(afterEach);

describe("Afora profile schema", () => {
  it("accepts typed settings", () => {
    const result = parseClawAforaProfile({
      schemaVersion: 1,
      agent: {
        tools: {
          profile: "coding",
  ...
  it("rejects invalid profile policy", () => {
    for (const agent of [
      ...
      expect(parseClawAforaProfile({ schemaVersion: 1, agent }).ok).toBe(false);
    }
  });
});

describe("Afora profile reader", () => {
  it("loads and integrity-binds the conventional profile", async () => {
    const root = tempDirs.make("afora-claw-profile-");
    ...
    const profilePath = join(root, "profiles", "afora.yml");
    await writeFile(
      profilePath,
      [
        "schemaVersion: 1",
    ...
    if (!first.ok) {
      throw new Error("expected Afora profile to parse");
    }
    ...
    if (!second.ok) {
      throw new Error("expected changed Afora profile to parse");
    }
  ...
  it.each([
    ...
    async ({ toolProfile, strictOk }) => {
      const root = tempDirs.make("afora-claw-legacy-profile-");
      await mkdir(join(root, "profiles"));
      await writeFile(
        join(root, "afora.claw.json"),
      ...
      const manifestPath = join(root, "afora.claw.json");
  ...
  it("requires package authors to bound a legacy full profile before update", async () => {
    const root = tempDirs.make("afora-claw-legacy-full-profile-");
    await mkdir(join(root, "profiles"));
    await writeFile(
      join(root, "afora.claw.json"),
      JSON.stringify({ schemaVersion: 1, agent: { id: "triage" } }),
    ...
    const result = await readClawManifestFile(join(root, "afora.claw.json"), {
      allowLegacyDynamicToolProfile: true,
    });
  ...
  it("rejects a hardlinked profile", async () => {
    const root = tempDirs.make("afora-claw-profile-hardlink-");
    await mkdir(join(root, "profiles"));
    await writeFile(
      join(root, "afora.claw.json"),
      JSON.stringify({
    ...
    await link(source, join(root, "profiles", "afora.yml"));

    const result = await readClawManifestFile(join(root, "afora.claw.json"));

    expect(result).toMatchObject({
      ok: false,
      diagnostics: [expect.objectContaining({ code: "afora_profile_unsafe" })],
    });
  });
  it("rejects a symlinked profile at the read boundary", async () => {
    const root = tempDirs.make("afora-claw-profile-symlink-");
    await mkdir(join(root, "profiles"));
    ...
    await symlink("../source.yml", join(root, "profiles", "afora.yml"));
  ...
  it("fails closed for an escaping metadata profile pointer", async () => {
    const root = tempDirs.make("afora-claw-profile-pointer-");
    const path = join(root, "afora.claw.json");
    await writeFile(
      path,
      JSON.stringify({
        schemaVersion: 1,
        agent: { id: "triage" },
        metadata: { "afora.config": "../afora.yml" },
      }),
      "utf8",
    );
    await writeFile(join(root, "afora.yml"), "schemaVersion: 1\nagent: {}\n", "utf8");
    ...
    expect(result).toMatchObject({
      ...
      diagnostics: [
        expect.objectContaining({
          code: "invalid_afora_profile_path",
          path: "$.metadata.afora.config",
        }),
      ],
    });
  });
  ...
  it("still reads the deprecated metadata profile pointer with a warning", async () => {
    const root = tempDirs.make("afora-claw-profile-legacy-pointer-");
    await mkdir(join(root, "profiles"));
    const path = join(root, "afora.claw.json");
    await writeFile(
      path,
      JSON.stringify({
        schemaVersion: 1,
        agent: { id: "triage" },
        metadata: { "afora.config": "profiles/triage.afora.yml" },
      }),
      "utf8",
    );
    await writeFile(
      join(root, "profiles", "triage.afora.yml"),
      "schemaVersion: 1\nagent:\n  tools:\n    profile: coding\n    allow: [read]\n",
      "utf8",
    );
    ...
    expect(result.diagnostics).toContainEqual(
      expect.objectContaining({
        level: "warning",
        code: "deprecated_afora_profile_pointer",
        path: "$.metadata.afora.config",
      }),
    );
  ...
  it("accepts a deprecated pointer that already targets the conventional profile", async () => {
    const root = tempDirs.make("afora-claw-profile-legacy-conventional-");
    await mkdir(join(root, "profiles"));
    const path = join(root, "afora.claw.json");
    await writeFile(
      path,
      JSON.stringify({
        schemaVersion: 1,
        agent: { id: "triage" },
        metadata: { "afora.config": "profiles/afora.yml" },
      }),
      "utf8",
    );
    await writeFile(
    ...
    expect(result).toMatchObject({ ok: true, aforaProfile: { schemaVersion: 1 } });
  });

  it("fails closed when a deprecated pointer diverges from the conventional profile", async () => {
    const root = tempDirs.make("afora-claw-profile-conflict-");
    await mkdir(join(root, "profiles"));
    const path = join(root, "afora.claw.json");
    await writeFile(
      path,
      JSON.stringify({
        schemaVersion: 1,
        agent: { id: "triage" },
        metadata: { "afora.config": "profiles/other.afora.yml" },
      }),
      "utf8",
    );
    await writeFile(join(root, "profiles", "afora.yml"), "schemaVersion: 1\n", "utf8");
    await writeFile(join(root, "profiles", "other.afora.yml"), "schemaVersion: 1\n", "utf8");
    ...
    expect(result).toMatchObject({
      ...
      diagnostics: [
        expect.objectContaining({
          code: "conflicting_afora_profile_pointer",
          path: "$.metadata.afora.config",
        }),
      ],
    });
  ...
  it("keeps the shipped pointer-based fixtures resolvable", async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      aforaProfile: { schemaVersion: 1, agent: { tools: { deny: ["exec", "browser"] } } },
    });
    if (!result.ok) {
  ...
  it("does not inspect profiles owned by other harnesses", async () => {
    const root = tempDirs.make("afora-claw-foreign-profile-");
    await mkdir(join(root, "profiles"));
    const path = join(root, "afora.claw.json");
    ...
    expect(result.aforaProfile).toBeUndefined();
  });
});
