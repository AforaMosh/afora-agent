// Safe loader for the conventional package-local Afora profile.
import { asOptionalRecord as record } from "@afora/normalization-core/record-coerce";
...
import type { ClawDiagnostic, ClawAforaProfile } from "./types.js";

const MAX_PROFILE_BYTES = 256 * 1024;
const CLAW_PROFILE_PATH = "profiles/afora.yml";
const LEGACY_PROFILE_POINTER_KEY = "afora.config";
const LEGACY_PROFILE_POINTER_PATH = "$.metadata.afora.config";
const CONVENTIONAL_PROFILE_PATH = "$.profiles.afora";
...
function parseProfileYaml(
  ...
  if (document.errors.length > 0) {
    return {
      ...
      diagnostics: document.errors.map((error) =>
        diagnostic("invalid_afora_profile", `Could not parse ${path}: ${error.message}`),
      ),
    };
  }
  ...
  if (unsupportedFeature) {
    return {
      ...
      diagnostics: [
        diagnostic(
          "unsupported_afora_profile_yaml_feature",
          `${path} uses ${unsupportedFeature}; Afora profile YAML must map directly to JSON data.`,
        ),
      ],
    };
  }
...
function migrateLegacyDynamicToolProfile(value: unknown): {
  value: unknown;
  legacyProfile?: ClawAforaProfile;
} {
  ...
  const validationProbe = parseClawAforaProfile({
    ...profile,
    agent: {
      ...agent,
      tools: {
  ...
  const legacyProfile: ClawAforaProfile = {
...
/**
 * Resolves the Afora profile for a package.
 *
 * `profiles/afora.yml` is the conventional location. The retired
 * `metadata.afora.config` pointer is still read for compatibility with
 ...
 */
export async function readClawAforaProfile(params: {
  packageRoot: string;
...
> {
  ...
  if (legacyPointer !== undefined) {
    ...
    if (conventionalExists && legacyPointer !== CLAW_PROFILE_PATH) {
      return {
        ...
        diagnostics: [
          diagnostic(
            "conflicting_afora_profile_pointer",
    ...
    diagnostics.push(
      warning(
        "deprecated_afora_profile_pointer",
  ...
  try {
    ...
    return {
      ...
      diagnostics: [
        diagnostic(
          unsafe
            ? "afora_profile_unsafe"
            : tooLarge
              ? "afora_profile_too_large"
              : "afora_profile_read_failed",
          unsafe
            ? "The Afora profile must be a regular, non-symlinked, non-hardlinked file."
            : tooLarge
              ? `The Afora profile exceeds ${MAX_PROFILE_BYTES} bytes.`
  ...
  const migration = params.allowLegacyDynamicToolProfile
    ...
    : { value: yaml.value };
  const parsed = parseClawAforaProfile(migration.value);
  if (!parsed.ok) {
    return {
      ok: false,
