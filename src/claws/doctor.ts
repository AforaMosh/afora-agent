...
type ClawDoctorOptions = AforaStateDatabaseOptions & {
  cfg?: AforaConfig;
  ...
  if (record.install.status !== "complete") {
    findings.push(
      finding({
        ...
        fixHint: "Inspect `afora claws status` before retrying or removing this Claw.",
      }),
    );
  }
  ...
  for (const pkg of record.packages) {
    ...
    findings.push(
      finding({
        ...
        fixHint:
          "Inspect package state with `afora claws status` before updating or removing the Claw.",
      }),
    );
  }
...
function orphanedAgentIds(options: AforaStateDatabaseOptions): string[] {
...
export async function collectClawStateHealthFindings(
  ...
  try {
    database = await openExistingAforaStateDatabaseReadOnly(options);
    if (!database) {
      return [];
    }
