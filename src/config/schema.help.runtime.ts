...
export const RUNTIME_FIELD_HELP: Record<string, string> = {
  ...
  "browser.allowSystemProfileImport":
    "Allows macOS hosts to import cookies from a local Chrome-family system profile into a managed Afora browser profile. Disable this to prevent browser profile cookie import and its macOS Keychain consent prompt.",
  "browser.cdpUrl":
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "afora" (or legacy "clawd") for CDP-based profiles, "existing-session" for Chrome DevTools MCP attachment, or "extension" for the authenticated Chrome extension relay.',
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as afora.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "tools.toolSearch":
    "Compact large Afora, MCP, and client tool catalogs. Set to true for the default code bridge or use the object form to choose structured controls or a compact visible tool directory.",
  "tools.toolSearch.enabled":
    "Enables Tool Search. When on, Afora hides large tool catalogs behind `tool_search_code` or structured search/describe/call tools during embedded runtime runs.",
  ...
  "tools.codeMode":
    "Generic Afora code mode. When enabled, agent runs expose only `exec` and `wait` to the model and hide normal tools behind a QuickJS-WASI catalog bridge.",
  ...
  "agents.defaults.sandbox.browser.network":
    'Docker network for sandbox browser containers (default: afora-sandbox-browser). Use the dedicated default or a custom bridge network; "none" is unsupported because browser control requires published CDP ports.',
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /afora).",
  ...
  "gateway.controlUi.allowExternalEmbedUrls":
    "DANGEROUS toggle that allows hosted embeds to load absolute external http(s) URLs. Keep this off unless your Control UI intentionally embeds trusted third-party pages; hosted /__afora__/canvas and /__afora__/a2ui documents do not need it.",
  ...
  "gateway.push.apns.relay":
    "External relay settings for relay-backed APNs sends. The gateway uses the hosted Afora relay by default, or this custom relay for push.test, wake nudges, and reconnect wakes after a paired official iOS build publishes a relay-backed registration.",
  ...
  "gateway.nodes.pairing.sshVerify":
    "SSH-verified auto-approval for first-time node-role device pairing (default: enabled). The gateway SSHes back to the pairing host (BatchMode, strict host keys) and approves only when the remote `afora node identity` output matches the pending device key. Set false to disable SSH verification (independent of autoApproveCidrs, which stays active); for manual-only pairing also unset autoApproveCidrs. Pass an object to override user/identity/timeoutMs/cidrs.",
  ...NODE_CAPABILITY_FIELD_HELP,
  "gateway.nodes.commands.allow":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `afora security audit`.",
  ...
  "nodeHost.workerRuns":
    "Opt in to full Afora worker session hosting from Gateway-managed bundles. Disabled by default.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, Afora enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  "nodeHost.mcp":
  ...
  "nodeHost.skills":
    "Use this section to publish skills installed in ~/.afora/skills from the headless node host. Restart the node host after changing skill files.",
  ...
  "diagnostics.otel.protocol":
    'OTel transport protocol for telemetry export. Only "http/protobuf" is accepted; run "afora doctor --fix" to repair a persisted legacy "grpc" value or get source-specific manual-edit guidance.',
  ...
  "skills.load.extraDirs":
    "Additional shared skill roots to scan at lowest precedence. Use this for sibling repos or shared skill packs that should be available without copying them into the Afora workspace.",
