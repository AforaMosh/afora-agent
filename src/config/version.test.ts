...
import { compareAforaVersions, shouldWarnOnTouchedVersion } from "./version.js";

describe("compareAforaVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareAforaVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareAforaVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareAforaVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("preserves numeric correction and build-metadata edge cases", () => {
    expect(compareAforaVersions("2026.3.23", "2026.3.23-0")).toBe(-1);
    expect(compareAforaVersions("2026.3.23", "2026.3.23-1.2")).toBe(1);
    expect(compareAforaVersions("2026.3.23+first", "2026.3.23+second")).toBe(0);
    expect(compareAforaVersions("2026.3.23-1+first", "2026.3.23-1+second")).toBe(0);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareAforaVersions("2026.6.5", "2026.6.6-beta.1")).toBe(-1);
    expect(compareAforaVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareAforaVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareAforaVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});
