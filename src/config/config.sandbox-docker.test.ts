...
describe("sandbox docker config", () => {
  ...
  it("accepts Windows drive-letter binds in sandbox.docker config", () => {
    const res = validateConfigObject({
      agents: {
        defaults: {
          sandbox: {
            docker: {
              binds: ["D:/data/afora/src:/src:ro", "D:\\data\\afora\\output:/output:rw"],
            },
          },
        },
      },
    ...
    if (res.ok) {
      expect(res.config.agents?.defaults?.sandbox?.docker?.binds).toEqual([
        "D:/data/afora/src:/src:ro",
        "D:\\data\\afora\\output:/output:rw",
      ]);
    }
  });
...
describe("sandbox browser binds config", () => {
          ...
          sandbox: {
            browser: {
              binds: [
                ...
                "D:/data/afora/chrome:/data/chrome-windows:rw",
              ],
            },
          },
        },
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("afora-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "afora-sandbox-browser-global" },
      agentBrowser: { network: "afora-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("afora-sandbox-browser-agent");
  });
