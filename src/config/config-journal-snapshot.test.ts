...
describe("config journal snapshots", () => {
  const suiteRootTracker = createSuiteTempRootTracker({
    prefix: "afora-config-journal-snapshot-",
  });
  ...
  it("hands the snapshot slot to another config path via the unfiltered CAS token", async () => {
    ...
    const env = { AFORA_STATE_DIR: path.join(home, ".afora") } as NodeJS.ProcessEnv;
    const context = { env, homedir: () => home };
    const pathA = path.join(home, ".afora", "config-a.json");
    const pathB = path.join(home, ".afora", "config-b.json");
    upsertConfigSnapshotAuditRecord({
      ...context,
      configPath: pathA,
      rawHash: "path-a-hash",
  ...
  it("falls back to a redaction marker when the fingerprint key cannot be stored", async () => {
    ...
    expect(
      fingerprintConfigSnapshotAuthoredConfig(
        ...
        { env: { AFORA_STATE_DIR: statePath } as NodeJS.ProcessEnv, homedir: () => home },
      ),
