...
const ETC_AFORA_DIR = path.join(ROOT_DIR, "etc", "afora");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "afora.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAforaPath(...parts: string[]) {
  return path.join(ETC_AFORA_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcAforaPath("agents.json");
...
describe("collectIncludePathsRecursive", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTestDir({ prefix: "afora-include-scan-" }, async (tempRoot) => {
        ...
        const includePaths = await collectIncludePathsRecursive({
          configPath: path.join(configDir, "afora.json"),
          parsed: {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTestDir({ prefix: "afora-include-root-swap-" }, async (tempRoot) => {
        ...
        expect(() =>
          resolveConfigIncludes(
            { $include: "./outer.json5" },
            path.join(configLink, "afora.json"),
            resolver,
          ),
  ...
  it("honors explicitly allowed include roots", async () => {
    await withTestDir({ prefix: "afora-include-scan-roots-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("continues past rejected nested includes to later safe siblings", async () => {
    await withTestDir({ prefix: "afora-include-scan-nested-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("revisits an include reached later at a shallower depth", async () => {
    await withTestDir({ prefix: "afora-include-scan-depth-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
...
describe("resolveConfigIncludeWritePath", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTestDir({ prefix: "afora-include-write-path-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTestDir({ prefix: "afora-includes-symlink-" }, async (tempRoot) => {
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTestDir({ prefix: "afora-includes-hardlink-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        const outsideDir = path.join(tempRoot, "outside");
        await fs.mkdir(configDir, { recursive: true });
        await fs.mkdir(outsideDir, { recursive: true });
    ...
    it("rejects include files larger than the guarded read limit", async () => {
      await withTestDir({ prefix: "afora-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        await fs.writeFile(
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "afora.json")),
...
describe("AFORA_INCLUDE_ROOTS allowlist", () => {
  ...
  it("still rejects include paths that fall outside every allowed root", () => {
    const obj = { $include: etcAforaPath("agents.json") };
    expect(() =>
  ...
  it("resolves a symlinked include whose realpath lands inside an allowed root", async () => {
    await withTestDir({ prefix: "afora-includes-allowed-symlink-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const sharedDir = path.join(tempRoot, "shared");
      await fs.mkdir(configDir, { recursive: true });
      await fs.mkdir(sharedDir, { recursive: true });
  ...
  it("rejects a symlinked include that escapes both the config directory and every allowed root", async () => {
    await withTestDir({ prefix: "afora-includes-allowed-escape-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const allowedDir = path.join(tempRoot, "allowed");
