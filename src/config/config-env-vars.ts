...
function isBlockedConfigEnvVar(key: string): boolean {
  return (
    ...
    key.toUpperCase() === "AFORA_INCLUDE_ROOTS" ||
...
function collectConfigEnvVarsByTarget(cfg?: AforaConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: AforaConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: AforaConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.AFORA_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
