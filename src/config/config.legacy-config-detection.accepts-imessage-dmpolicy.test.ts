...
function expectAforaSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = AforaSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectAforaSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = AforaSchema.safeParse(config);
