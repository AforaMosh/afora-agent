...
const mockReadSourceConfigSnapshot = vi.hoisted(() => async () => {
  ...
  const configPath = pathValue.join(process.env.AFORA_STATE_DIR ?? "", "afora.json");
  try {
...
const mockReplaceConfigFile = vi.hoisted(() => async ({ nextConfig }: { nextConfig: unknown }) => {
  ...
  const configPath = pathLocal.join(process.env.AFORA_STATE_DIR ?? "", "afora.json");
