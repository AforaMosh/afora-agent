...
export const MODEL_FIELD_HELP: Record<string, string> = {
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether Afora injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "models.providers.*.agentRuntime":
    "Optional low-level agent runtime policy for this provider. Use provider/model runtime policy instead of agent-wide runtime pins; omitted/default lets Afora choose the runtime for the selected provider.",
  "models.providers.*.agentRuntime.id":
    'Provider agent runtime id: "afora", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli". OpenAI on the official endpoint defaults to the Codex harness when omitted.',
  "models.providers.*.localService":
    "Optional on-demand local model server process for this provider. Afora probes healthUrl, starts the command when needed, waits for readiness, and then sends the model request.",
  ...
  "models.providers.*.localService.healthUrl":
    "Readiness URL probed before model requests. If omitted, Afora uses the provider baseUrl with /models appended.",
  ...
  "models.providers.*.localService.idleStopMs":
    "Milliseconds to keep an Afora-started local model server alive after the last request finishes. Set 0 to keep it alive until Afora exits.",
  ...
  proxy:
    "Operator-managed forward proxy routing for Afora runtime HTTP, HTTPS, WebSocket, and supported raw-egress paths. Use this when central egress control is part of the deployment boundary.",
  "proxy.enabled":
  ...
  "models.providers.*.models[].agentRuntime.id":
    'Model agent runtime id: "afora", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli".',
  ...
  "models.providers.*.models[].mediaInput.image.preferredSidePx":
    "Preferred image resize side for balanced compression. Leave unset to use Afora's conservative default.",
  ...
  "agents.defaults.experimental.localModelLean":
    "Experimental local-model prompt trim. When enabled, Afora drops heavyweight default tools like browser, cron, and message for weaker or smaller local-model backends.",
  ...
  "agents.defaults.models.*.agentRuntime.id":
    'Default-agent model runtime id: "afora", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli".',
