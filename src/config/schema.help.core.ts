...
export const CORE_FIELD_HELP: Record<string, string> = {
  ...
  "meta.lastTouchedVersion": "Afora version that most recently wrote this config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for Afora. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  secrets:
  ...
  "wizard.lastRunVersion": "Afora version used by the last wizard run.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping Afora runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  "update.channel":
  ...
  "gateway.tailscale.preserveFunnel":
    "Deprecated migration guard for mode='serve'. If an external Funnel still targets the ordinary Gateway listener, Afora leaves exposure unchanged and warns that only plugin-authenticated webhooks remain usable until password auth is configured and mode is migrated to 'funnel'.",
  "gateway.remote":
  ...
  "talk.realtime.instructions":
    "Additional system instructions appended to Afora's built-in realtime Talk prompt. Use this for voice style, tone, and other provider-facing realtime behavior while keeping agent-consult guidance intact.",
  ...
  "talk.realtime.consultRouting":
    "Gateway relay fallback for final user transcripts when the realtime provider skips afora_agent_consult. provider-direct preserves provider replies; force-agent-consult routes through Afora.",
  ...
  "agents.ownership":
    'Durable multi-agent ownership generation marker. "explicit" means ambient channels, heartbeat, system-agent consults, Talk, cron, and bare CLI operations must resolve a surface-specific owner or fail closed. Afora stamps this automatically when creating or migrating a fleet; omit it for a sole agent.',
  "agents.defaults":
  ...
  "agents.entries.*.runtime":
    "Optional runtime descriptor for this agent. Use embedded for default Afora execution or acp for external ACP harness defaults.",
  "agents.entries.*.runtime.type":
    'Runtime type for this agent: "embedded" (default Afora runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.entries.*.runtime.acp.agent":
    "Optional ACP harness agent id to use for this Afora agent (for example codex, claude, cursor, gemini, afora).",
  ...
  "agents.defaults.systemAgent":
    "Target settings for ambient Afora system-agent and Custodian inference.",
