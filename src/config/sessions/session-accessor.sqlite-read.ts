...
export function loadSqliteTranscriptEventsSync(
  ...
  return runSqliteDeferredTransactionSync(
    ...
    () => {
      ...
      const rows = readSqliteTranscriptEventRows(database, resolved.sessionId).filter(
...
export function loadSqliteTranscriptHeaderSync(scope: SessionTranscriptReadScope): unknown {
  ...
  const row = filterAuthorizedTranscriptRows(
    database,
    resolved.sessionId,
    readSqliteTranscriptEventRows(database, resolved.sessionId),
  )[0];
...
export function readSqliteTranscriptSnapshot(
  ...
  const rows = filterAuthorizedTranscriptRows(
    database,
    sessionId,
    readSqliteTranscriptEventRows(database, sessionId),
  );
  return {
