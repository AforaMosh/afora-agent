...
export function assertCanonicalAgentRosterRetainsEntries(params: {
...
function omitImplicitDefaultAgentEntries(params: {
  ...
  if (getPathValue(params.rootAuthoredConfig, entriesPath) !== undefined) {
    return params.persistedCandidate;
  }
  if (
    params.explicitSetPaths?.some(
      (path) => pathStartsWith(path, entriesPath) || pathStartsWith(entriesPath, path),
    )
  ) {
    return params.persistedCandidate;
  }
  if (
    !isDeepStrictEqual(
      getPathValue(params.runtimeConfig, entriesPath),
      getPathValue(params.nextConfig, entriesPath),
    )
  ) {
    return params.persistedCandidate;
  }
  return deletePathValue(params.persistedCandidate, entriesPath);
}
...
export function resolvePersistCandidateForWrite(params: {
  ...
  const withSchema = preserveRootSchemaUri({
    rootAuthoredConfig,
    nextConfig: params.nextConfig,
    persistedCandidate: withoutImplicitAgentEntries,
  });
