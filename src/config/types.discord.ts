...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the Afora agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus Afora, or the agent id plus Afora. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the Afora agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which Afora conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing Afora Discord conversation. */
