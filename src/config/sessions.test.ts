...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ AFORA_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-afora",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-afora#general");
  });
  ...
  it("derives session transcripts dir from AFORA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDirForAgent(
      "main",
      { AFORA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when AFORA_STATE_DIR differs from stored paths", () => {
