...
export async function withConfigMutationExclusive<T>(
  fn: (config: AforaConfig) => Promise<T>,
): Promise<T> {
...
async function tryWriteSingleTopLevelIncludeMutation(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
}): Promise<{ persistedHash: string | null; persistedConfig: AforaConfig } | null> {
...
function resolveConfigWriteResult(
  ...
  fallbackConfig: AforaConfig,
): { persistedHash: string | null; persistedConfig: AforaConfig } {
  if (result) {
    return {
...
export async function mutateConfigFile<T = void>(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
  mutate: (draft: AforaConfig, context: ConfigMutationContext) => Promise<T | void> | T | void;
}): Promise<ConfigMutationResult<T>> {
