...
import { buildWebSearchProviderConfig, withTempHome, writeAforaConfig } from "./test-helpers.js";
...
describe("boolean config validation", () => {
  it.each(nonBooleanConfigCases)("rejects non-boolean values for $name", ({ config }) => {
    const result = AforaSchema.safeParse(config);
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://afora.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = AforaSchema.safeParse({ $schema: 123 });
    expect(result.success).toBe(false);
  });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://afora.ai/config.json");
    }
  });
});
...
describe("models.catalogRefresh", () => {
  ...
  it("rejects invalid refresh values", () => {
    expect(
      AforaSchema.safeParse({ models: { catalogRefresh: { enabled: "false" } } }).success,
    ).toBe(false);
    expect(
      AforaSchema.safeParse({ models: { catalogRefresh: { url: "file:///tmp/catalog.json" } } })
        .success,
    ).toBe(false);
    expect(
      AforaSchema.safeParse({ models: { catalogRefresh: { url: "not a url" } } }).success,
    ).toBe(false);
    expect(
      AforaSchema.safeParse({
...
describe("diagnostics.otel.captureContent", () => {
  it("accepts supported OTEL log exporters and rejects unknown values", () => {
    ...
    const invalid = AforaSchema.safeParse({
      diagnostics: {
        otel: {
          logs: true,
          logsExporter: "stderr",
...
describe("mcp.apps.enabled", () => {
  it.each([true, false])("accepts %s", (enabled) => {
    expect(AforaSchema.safeParse({ mcp: { apps: { enabled } } }).success).toBe(true);
  });

  it("rejects non-boolean values", () => {
    expect(AforaSchema.safeParse({ mcp: { apps: { enabled: "yes" } } }).success).toBe(false);
  });
  ...
  it("accepts only a bare HTTP(S) sandbox origin", () => {
    ...
    expect(AforaSchema.safeParse({ mcp: { apps: { sandboxPort: 65536 } } }).success).toBe(false);
    for (const sandboxOrigin of [
      ...
      expect(AforaSchema.safeParse({ mcp: { apps: { sandboxOrigin } } }).success).toBe(false);
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@afora"]);
    }
  });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken SecretRef values", () => {
    const res = AforaSchema.safeParse({
      cron: {
        webhookToken: {
          source: "env",
          provider: "default",
...
describe("config strict validation", () => {
  ...
  it("rejects legacy sandbox perSession without read-time auto-migration", async () => {
    await withTempHome(async (home) => {
      ...
      expect(issuePaths(snap.issues)).toContain("agents.entries.afora.sandbox");
      ...
      expect(snap.sourceConfig.agents?.entries?.afora?.sandbox).toEqual({
        perSession: false,
      });
    });
  });
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      await writeAforaConfig(home, {
        gateway: { bind: "${AFORA_BIND}" },
      });

      const prev = process.env.AFORA_BIND;
      process.env.AFORA_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.AFORA_BIND;
        } else {
          process.env.AFORA_BIND = prev;
        }
      }
    });
  });
