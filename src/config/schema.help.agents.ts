...
export const AGENT_FIELD_HELP: Record<string, string> = {
  ...
  "agents.entries.*.models.*.agentRuntime.id":
    'Per-agent model runtime id: "afora", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli".',
  ...
  "agents.defaults.compaction.enabled":
    "Enable embedded proactive auto-compaction (default: true). Set false to stop threshold-driven embedded compaction while preserving Afora overflow recovery, preflight compaction, and manual /compact.",
  ...
  "agents.defaults.compaction.thinkingLevel":
    'Optional thinking level used only for embedded Afora compaction summaries: "off", "minimal", "low", "medium", "high", "xhigh", "adaptive", "max", or "ultra". It overrides the session level and is clamped to the actual compaction model/runtime; leave unset to inherit the session level. Native Codex app-server compaction ignores this setting because its compact request has no per-operation thinking override, and Afora logs a warning.',
  ...
  "agents.defaults.compaction.midTurnPrecheck":
    "Optional embedded Afora tool-loop precheck that detects context pressure after a tool result is appended and before the next model call. When enabled, Afora reuses existing precheck recovery to truncate tool results or compact before retrying.",
  "agents.defaults.compaction.midTurnPrecheck.enabled":
    "Enable structured mid-turn context pressure checks for embedded Afora tool loops. Default: false. Keep disabled unless long tool-heavy sessions hit context overflow before normal turn-end compaction can run.",
  ...
  "agents.defaults.embeddedAgent":
    "Embedded Afora runner hardening controls for how workspace-local agent settings are trusted and applied in Afora sessions.",
  "agents.defaults.embeddedAgent.projectSettingsPolicy":
    'How embedded Afora handles workspace-local `.afora/settings.json`: "sanitize" (default) strips shellPath/shellCommandPrefix, "ignore" disables project settings entirely, and "trusted" applies project settings as-is.',
  "agents.defaults.embeddedAgent.executionContract":
    'Embedded Afora execution contract: "default" keeps the standard runner behavior, while "strict-agentic" enables structured plan tracking and non-visible turn recovery for supported OpenAI/OpenAI Codex GPT-5-family runs.',
  "agents.entries.*.embeddedAgent":
    "Optional per-agent embedded Afora overrides. Use this to opt specific agents into stricter GPT-5 execution behavior without changing the global default.",
  "agents.entries.*.embeddedAgent.executionContract":
    'Optional per-agent embedded Afora execution contract override. Set "strict-agentic" to enable structured plan tracking and non-visible turn recovery for that agent on supported OpenAI/OpenAI Codex GPT-5-family runs, or "default" to inherit the standard runner behavior.',
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage Afora MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by Afora. Embedded Afora and other runtime adapters can consume these servers without storing them inside runtime-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. Afora stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  "mcp.servers.*.codex":
    "Afora projection metadata for Codex app-server threads only. It does not affect ACP sessions or generic Codex harness config. Omit this block to keep the server available to every Codex app-server agent with Codex's default MCP approval behavior.",
  ...
  "mcp.servers.*.oauth.authProfileId":
    "Refresh-capable auth profile id used to inject the current bearer token into this remote MCP server. When set, Afora resolves and refreshes the profile at runtime and does not project refresh material downstream.",
  "mcp.servers.*.codex.agents":
    "Optional non-empty Afora agent ids that should receive this MCP server in Codex app-server thread config. Empty, blank, or invalid lists fail closed; when omitted, the server is projected for all Codex app-server agents.",
  ...
  "mcp.servers.*.codex.default_tools_approval_mode":
    "Codex-native spelling for the same per-server MCP tool approval mode. Prefer defaultToolsApprovalMode in Afora config.",
};
