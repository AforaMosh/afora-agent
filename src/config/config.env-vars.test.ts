...
describe("config env vars", () => {
  it("applies env vars from env block when missing", async () => {
    await withEnvOverride({ OPENROUTER_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { OPENROUTER_API_KEY: "config-key" } } } as AforaConfig);
  ...
  it("applies config env above normalized lower-precedence aliases", () => {
    ...
    applyConfigEnvVars({ env: { vars: { Z_AI_API_KEY: "config-key" } } } as AforaConfig, env, {
  ...
  it("preserves a higher-precedence normalized alias", () => {
    ...
    applyConfigEnvVars({ env: { vars: { ZAI_API_KEY: "config-key" } } } as AforaConfig, env, {
      lowerPrecedenceEnv: { ZAI_API_KEY: "shell-key" },
    });
  ...
  it.runIf(process.platform !== "win32")("keeps unrelated POSIX env casing distinct", () => {
    ...
    applyConfigEnvVars({ env: { vars: { foo: "config-key" } } } as AforaConfig, env);
  ...
  it("applies env vars from env.vars when missing", async () => {
    await withEnvOverride({ GROQ_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { GROQ_API_KEY: "gsk-config" } } } as AforaConfig);
  ...
  it("does not infer an equal-valued ambient env entry as config-owned", async () => {
    const key = "AFORA_TEST_EQUAL_AMBIENT_ENV";
  ...
  it("unwinds overlapping same-value publications after both roll back", async () => {
    const key = "AFORA_TEST_OVERLAPPING_ENV";
  ...
  it.each(["older-first", "newer-first"] as const)(
    ...
    async (rollbackOrder) => {
      const key = "AFORA_TEST_OVERLAPPING_DIFFERENT_ENV";
      await withEnvOverride({ [key]: "old" }, async () => {
        try {
          const previousConfig = { env: { vars: { [key]: "old" } } };
  ...
  it("lets a newer committed publication supersede an older late rollback", async () => {
    const key = "AFORA_TEST_COMMITTED_OVERLAPPING_ENV";
    await withEnvOverride({ [key]: "old" }, async () => {
      try {
        const previousConfig = { env: { vars: { [key]: "old" } } };
        const olderConfig = { env: { vars: { [key]: "older" } } };
  ...
  it("lets a newer publication remove a key added by an overlapping predecessor", async () => {
    const key = "AFORA_TEST_OVERLAPPING_REMOVED_ENV";
  ...
  it("rejects process-stable Gateway selector changes during reload", () => {
    expect(() =>
      assertGatewayConfigEnvSelectionUnchanged(
        {},
        { env: { vars: { AFORA_CONFIG_PATH: "/tmp/other.json" } } },
      ),
    ).toThrow("process-stable Gateway selector AFORA_CONFIG_PATH");
  });
  ...
  it("preserves Windows case-insensitive env precedence in merged runtime env", () => {
    ...
    try {
      const merged = createConfigRuntimeEnv(
        { env: { vars: { AFORA_LOAD_SHELL_ENV: "1" } } } as AforaConfig,
        { Afora_Load_Shell_Env: "0" },
      );

      expect(merged.AFORA_LOAD_SHELL_ENV).toBe("0");
      expect(Object.keys(merged)).toEqual(["Afora_Load_Shell_Env"]);
    } finally {
  ...
  it("does not adopt a concurrent Windows case-only rename as config-owned", () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const originalKey = "Afora_Test_Windows_Owned_Case";
  ...
  it("blocks dangerous startup env vars from config env", async () => {
    await withEnvOverride(
      ...
      async () => {
        const config = {
          env: {
            vars: {
              ...
              AFORA_INCLUDE_ROOTS: "/tmp/evil-include-root",
              afora_allow_older_binary_destructive_actions: "1",
              OPENROUTER_API_KEY: "config-key",
            },
          },
        };
        const entries = collectConfigRuntimeEnvVars(config as AforaConfig);
        ...
        expect(entries.AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
        expect(entries.AFORA_INCLUDE_ROOTS).toBeUndefined();
        expect(entries.afora_allow_older_binary_destructive_actions).toBeUndefined();
        expect(entries.OPENROUTER_API_KEY).toBe("config-key");

        applyConfigEnvVars(config as AforaConfig);
  ...
  it("loads ${VAR} substitutions from ~/.afora/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.AFORA_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected AFORA_STATE_DIR to be set by withTempHome");
        }
        ...
        const first = resolveConfigEnvVars(config, process.env) as AforaConfig;
        expect(
          (
        ...
        loadDotEnv({ quiet: true });
        const second = resolveConfigEnvVars(config, process.env) as AforaConfig;
        expect(
          (
  ...
  it("drops dangerous and empty values from the state-dir .env file", async () => {
    await withTempHome(async (_home) => {
      ...
      expect(vars.AFORA_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
      expect(vars.EMPTY).toBeUndefined();
      expect(vars.VALID).toBe("ok");
    });
  });

  it("respects AFORA_STATE_DIR when reading state-dir .env vars", async () => {
    await withTempHome(async (_home) => {
      const customStateDir = path.join(process.env.AFORA_STATE_DIR ?? "", "custom-state");
      await writeStateDirDotEnv("CUSTOM_KEY=from-override\n", {
        stateDir: customStateDir,
      });
      expect(
        collectDurableServiceEnvVars({
          env: { AFORA_STATE_DIR: customStateDir },
        }).CUSTOM_KEY,
      ).toBe("from-override");
    });
  });
