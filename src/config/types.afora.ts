// Defines the top-level Afora configuration type.
...
/** Top-level Afora config as read from user/project config files. */
export type AforaConfig = {
  ...
  $schema?: string;
  meta?: {
    /** Last Afora version that wrote this config. */
  ...
  ui?: {
    /** Accent color for Afora UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type AforaConfigInput = Omit<AforaConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const aforaConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = AforaConfig & {
  readonly [aforaConfigStateBrand]?: TState;
};
