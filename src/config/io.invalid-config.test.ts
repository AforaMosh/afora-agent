...
describe("config io invalid config formatting", () => {
  ...
  it("creates INVALID_CONFIG errors with inline details", () => {
    const err = createInvalidConfigError("/tmp/afora.json", "- gateway.port: bad") as Error & {
      code?: string;
      details?: string;
    };

    expect(err.message).toBe("Invalid config at /tmp/afora.json:\n- gateway.port: bad");
    ...
    expect(
      isDoctorRecoverableInvalidConfigError(
        createInvalidConfigError("/tmp/afora.json", "manual repair", {
          recovery: "manual",
        }),
      ),
    ).toBe(false);
