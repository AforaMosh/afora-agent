...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "afora-version-" });
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "afora-agent", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "afora-agent", version: "2.3.4" });
  ...
  it("ignores non-afora package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers AFORA_VERSION over package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        AFORA_VERSION: "9.9.9",
  ...
  it("prefers runtime VERSION over stale AFORA_VERSION for compatibility checks", () => {
    const previousCompatibility = process.env.AFORA_COMPATIBILITY_HOST_VERSION;
    const previous = process.env.AFORA_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      delete process.env.AFORA_COMPATIBILITY_HOST_VERSION;
      process.env.AFORA_VERSION = "2026.3.25";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("AFORA_COMPATIBILITY_HOST_VERSION", previousCompatibility);
      restoreEnvValue("AFORA_VERSION", previous);
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        AFORA_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        AFORA_VERSION: "2026.3.99",
        npm_package_version: "2026.3.97",
      }),
    ).toBe("2026.4.8");
