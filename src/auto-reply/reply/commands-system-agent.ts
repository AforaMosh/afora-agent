// Implements maintenance commands for Afora-backed session cleanup.
...
export const handleSystemAgentCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (!params.command.isAuthorizedSender) {
    logVerbose(
      `Ignoring /afora from unauthorized sender: ${params.command.senderId || "<unknown>"}`,
    );
  ...
  return {
    ...
    reply: {
      text:
        ...
        })) ?? "Afora did not find a rescue request.",
    },
  };
};
