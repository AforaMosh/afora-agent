...
const mocks = vi.hoisted(() => ({
  ...
  triggerAforaRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerAforaRestart: mocks.triggerAforaRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerAforaRestart.mockReset();
    mocks.triggerAforaRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerAforaRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerAforaRestart).not.toHaveBeenCalled();
  ...
  it("clears the success sentinel when fallback restart fails", async () => {
    mocks.triggerAforaRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
