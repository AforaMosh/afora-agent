...
describe("executeAgentTurn: authentication failures", () => {
  ...
  it("surfaces gateway reauth guidance from typed OAuth refresh failures", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Model login expired on the gateway for openai. Send `/login codex` from a private chat or Web UI session to pair a new Codex login, or re-auth with `afora models auth login --provider openai --profile-id 'openai:user@example.com'` in a terminal, then try again.",
      );
    }
  });
  ...
  it("omits OAuth profile ids from group reauth guidance", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toContain(
        "afora models auth login --provider openai` in a terminal",
      );
  ...
  it("keeps non-OpenAI OAuth refresh failures on provider-specific terminal guidance", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Model login expired on the gateway for anthropic. Re-auth with `afora models auth login --provider anthropic` in a terminal, then try again.",
      );
  ...
  it("surfaces claude-cli re-auth hint over generic provider auth copy for 401 OAuth expiry", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Model login expired on the gateway for claude-cli. Re-auth with `claude auth login && afora models auth login --provider anthropic --method cli` in a terminal, then try again.",
      );
    }
  });
  ...
  it("surfaces typed missing API-key auth guidance without parsing the message", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        '⚠️ Missing API key for provider "openai". Run `afora doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `afora models auth login --provider openai` or run `afora configure`.',
      );
    }
  });
  ...
  it("formats auth-profile failover copy from typed FailoverError metadata", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).toContain("afora configure");
  ...
  it("does not suggest re-authentication for typed format failures", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).not.toContain("afora configure");
    }
  });
  ...
  it("falls back to a generic reauth command when the provider in the OAuth error is unsafe", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Model login expired on the gateway. Re-auth with `afora models auth login` in a terminal, then try again.",
      );
    }
  });
});
