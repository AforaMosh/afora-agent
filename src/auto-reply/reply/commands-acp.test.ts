...
vi.mock("../../agents/subagents/spawn/acp-spawn.js", () => ({
  resolveAcpSpawnRuntimePolicyError: (params: { cfg?: AforaConfig }) =>
...
function createDiscordParams(commandBody: string, cfg: AforaConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: AforaConfig = baseCfg) {
...
async function runDiscordAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
...
async function runTelegramDmAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runSlackDmAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
function createMatrixThreadParams(commandBody: string, cfg: AforaConfig = baseCfg) {
...
async function runMatrixAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runMatrixThreadAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(createMatrixThreadParams(commandBody, cfg), true);
}

async function runFeishuDmAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runLineDmAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runIMessageDmAcpCommand(commandBody: string, cfg: AforaConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
describe("/acp command", () => {
  ...
  it("inherits the target agent workspace when /acp spawn omits --cwd", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-"));
    try {
      const cfg = {
        ...baseCfg,
        agents: {
  ...
  it("sanitizes leaked task and runtime details in ACP status output", async () => {
    ...
    expect(result?.reply?.text).not.toContain("Afora runtime context (internal):");
