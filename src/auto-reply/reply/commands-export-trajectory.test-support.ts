...
function makeParams(
  workspaceDir = tempDirs.make("afora-export-command-"),
): HandleCommandsParams {
  return {
    cfg: {
      session: {
        store: "/tmp/afora-sessions.json",
      },
    },
    ctx: {
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "afora sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(reply.text).toContain("https://docs.afora.ai/tools/trajectory");
    ...
    expect(execCall.defaults.sessionStore).toBe("/tmp/afora-sessions.json");
    ...
    expect(command).not.toContain("afora sessions export-trajectory");
    ...
    expect(String(request.workspace)).toContain("afora-export-command-");
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("afora sessions export-trajectory");
