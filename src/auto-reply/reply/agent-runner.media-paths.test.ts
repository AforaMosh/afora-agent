...
describe("runReplyAgent media path normalization", () => {
  ...
  it("reuses the provided media context inside executeAgentTurn", async () => {
    // Regression test for AforaMosh/afora-agent#68056.
  ...
  it("passes current inbound media paths as native Afora images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "afora-native-agent-media-"));
  ...
  it("does not pass recent history images as unlabeled native Afora images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "afora-native-agent-history-"));
    cleanupPaths.push(tmpDir);
  ...
  it("retains resolved current images and skips unresolved attachments", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "afora-native-agent-partial-"));
    cleanupPaths.push(tmpDir);
