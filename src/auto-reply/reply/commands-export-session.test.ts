  ...
  return {
    ...
    writeSessionExportFileMock: vi.fn(
      async (_params: {
        ...
        absolutePath: "/tmp/workspace/afora-session.html",
        displayPath: "afora-session.html",
      }),
    ),
...
describe("buildExportSessionReply", () => {
  ...
  it("injects scripts and session data through the real export template", async () => {
    ...
    expect(html).not.toContain("data-afora-export-placeholder");
  ...
  it("passes the generated HTML and explicit path to the export boundary", async () => {
    ...
    expect(hoisted.writeSessionExportFileMock).toHaveBeenCalledWith({
      ...
      defaultFileName: expect.stringMatching(/^afora-session-session--.+\.html$/),
  ...
  it("preserves replacement text with dollar sequences", async () => {
    hoisted.exportHtmlTemplateContents.set(
      "template.html",
      [
        '<style data-afora-export-placeholder="CSS"></style>',
        '<script id="session-data" type="application/json" data-afora-export-placeholder="SESSION_DATA"></script>',
        '<script data-afora-export-placeholder="MARKED_JS"></script>',
        '<script data-afora-export-placeholder="HIGHLIGHT_JS"></script>',
        '<script data-afora-export-placeholder="JS"></script>',
      ].join(""),
    );
  ...
  it("exports marker-backed sessions by identity without requiring the marker as a file", async () => {
    hoisted.resolveSessionFilePathMock.mockReturnValue(
      "sqlite:target:session-1:/tmp/target-store/afora-agent.sqlite",
    );
    hoisted.loadSessionStoreMock.mockReturnValue({
      "agent:target:session": {
        sessionFile: "sqlite:target:session-1:/tmp/target-store/afora-agent.sqlite",
        sessionId: "session-1",
        updatedAt: 1,
      },
    });
