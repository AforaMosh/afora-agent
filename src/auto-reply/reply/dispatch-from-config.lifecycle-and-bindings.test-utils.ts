...
describe("dispatchReplyFromConfig", () => {
  ...
  it("stages remote iMessage media before plugin-bound inbound claim metadata reaches Codex", async () => {
    ...
    const stagedPath =
      "/tmp/afora-proof/.afora/media/remote-cache/agent-main-imessage/photo.jpg";
    stageSandboxMediaMocks.stageSandboxMedia.mockImplementationOnce(async (paramsUnknown) => {
      ...
      expect(params.workspaceDir).toContain(".afora/workspace");
    ...
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        pluginBindingOwner: "plugin",
        pluginId: "afora-codex-app-server",
        pluginRoot: "/plugins/afora-codex-app-server",
        data: {
          kind: "codex-app-server-session",
          version: 1,
          sessionFile: "/tmp/session.jsonl",
  ...
  it("does not run plugin-owned binding delivery when the caller already aborted", async () => {
    ...
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        pluginBindingOwner: "plugin",
        pluginId: "afora-codex-app-server",
        pluginRoot: "/workspace/afora-app-server",
        data: {
          kind: "codex-app-server-session",
          version: 1,
          sessionFile: "/tmp/session.jsonl",
