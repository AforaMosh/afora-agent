...
const originalStateDirEnv = process.env.AFORA_STATE_DIR;
...
function restoreProcessState() {
  if (originalStateDirEnv === undefined) {
    ...
    setTestEnvValue("AFORA_STATE_DIR", originalStateDirEnv);
  }
}
...
describe("resolveCurrentTurnImages", () => {
  ...
  it("hydrates Telegram-style state-relative media into native prompt images", async () => {
    await withTestDir({ prefix: "afora-current-turn-images-" }, async (base) => {
  ...
  it.each([
    ...
    await withTestDir({ prefix: "afora-current-turn-canonical-kind-" }, async (base) => {
  ...
  it.each([undefined, "application/pdf", "application/octet-stream", "image/png"] as const)(
    ...
    async (contentType) => {
      await withTestDir({ prefix: "afora-current-turn-document-image-" }, async (base) => {
  ...
  it.each([undefined, "application/octet-stream", "binary/octet-stream"] as const)(
    ...
    async (contentType) => {
      await withTestDir({ prefix: "afora-current-turn-unknown-image-" }, async (base) => {
  ...
  it.each(["application/pdf", "application/zip", "text/plain"] as const)(
    ...
    async (contentType) => {
      await withTestDir({ prefix: "afora-current-turn-unknown-document-" }, async (base) => {
        const documentPath = path.join(base, "report.png");
        await fs.writeFile(documentPath, PNG_IMAGE_BYTES);
  ...
  it.each([
    ...
    await withTestDir({ prefix: "afora-current-turn-spoofed-image-" }, async (base) => {
  ...
  it("hydrates AVIF attachments when transport metadata only declares generic bytes", async () => {
    await withTestDir({ prefix: "afora-current-turn-avif-" }, async (base) => {
  ...
  it("does not duplicate a prepared host-staged image during runner hydration", async () => {
    await withTestDir({ prefix: "afora-current-turn-staged-image-" }, async (base) => {
  ...
  it("appends extracted PDF page images without dropping current image attachments", async () => {
    await withTestDir({ prefix: "afora-current-turn-pdf-images-" }, async (base) => {
  ...
  it("orders extracted PDF page images before later current image attachments", async () => {
    await withTestDir({ prefix: "afora-current-turn-pdf-order-" }, async (base) => {
      const imagePath = path.join(base, "photo.png");
  ...
  it("retains resolved native images when current media partially resolves", async () => {
    await withTestDir({ prefix: "afora-current-turn-partial-" }, async (base) => {
  ...
  it("proceeds without native images when current media resolution throws", async () => {
    ...
    await withTestDir({ prefix: "afora-current-turn-throw-" }, async (base) => {
      const imagePath = path.join(base, "present.png");
