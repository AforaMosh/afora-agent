...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-allowlist-config-"));
  const configPath = path.join(dir, "afora.json");
  const previous = process.env.AFORA_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      setTestEnvValue("AFORA_CONFIG_PATH", previous);
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<AforaConfig>(configPath);
      ...
      createTestRegistry([
        {
          ...
          plugin: {
            ...
            config: {
              ...
              defaultAccountId: (cfg: AforaConfig) =>
                (cfg.channels?.telegram as TelegramTestSectionConfig | undefined)?.defaultAccount ??
                DEFAULT_ACCOUNT_ID,
            },
          },
