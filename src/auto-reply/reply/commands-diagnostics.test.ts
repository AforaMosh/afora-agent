...
import type { AforaPluginCommandDefinition, PluginCommandContext } from "../../plugins/types.js";
import { normalizeSessionDeliveryState } from "../../utils/delivery-context.shared.js";

type PluginCommandHandler = AforaPluginCommandDefinition["handler"];
...
function buildDiagnosticsParams(
  ...
  return {
    cfg: { commands: { text: true } } as AforaConfig,
    ctx: {
      Provider: "whatsapp",
      Surface: "whatsapp",
      CommandSource: "text",
...
describe("diagnostics command", () => {
  it("requests Gateway diagnostics approval without a duplicate pending chat reply", async () => {
    ...
    expect(execCall.defaults.approvalWarningText).toContain(
      "https://docs.afora.ai/gateway/diagnostics",
    );
    ...
    expect(command).not.toBe("afora gateway diagnostics export --json");
  });
  ...
  it("falls back to a visible reply when approval cannot be queued", async () => {
    ...
    expect(result?.reply?.text).toContain("https://docs.afora.ai/gateway/diagnostics");
  ...
  it("routes group diagnostics confirmations privately", async () => {
    const commandHandler = vi.fn(async () => ({
      text: [
        "Codex diagnostics sent to OpenAI servers:",
        "- channel whatsapp, Afora session session-1, Codex thread codex-thread-1",
      ].join("\n"),
    }));
    registerHostTrustedReservedCommandForTest({
      name: "codex",
