...
vi.mock("../../agents/harness/builtin-afora.js", () => ({
...
function saveStatusTestAuthProfiles(params: {
  ...
  const agentDir = path.join(params.dir, ".afora", "agents", "main", "agent");
...
describe("buildStatusReply subagent summary", () => {
  ...
  it("does not leak internal runtime context through the task status line", async () => {
    ...
    expect(reply?.text).not.toContain("Afora runtime context (internal):");
  ...
  it("shows the effective non-Afora embedded harness in /status", async () => {
  ...
  it("uses Codex OAuth auth labels for openai models running on the Codex harness", async () => {
    ...
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".afora", "agents", "main", "agent");
        fs.mkdirSync(agentDir, { recursive: true });
        saveAuthProfileStore(
          {
            version: 1,
  ...
  it("uses Codex OAuth auth labels for explicit OpenAI Afora auth order", async () => {
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".afora", "agents", "main", "agent");
        fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("uses workspace-scoped auth evidence in /status auth labels", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-status-auth-label-"));
    const workspaceDir = path.join(tempRoot, "workspace");
    const pluginDir = path.join(workspaceDir, ".afora", "extensions", "workspace-auth-label");
  ...
  it("keeps /status on an explicit Afora runtime override after config changes", async () => {
    registerStatusCodexHarness();
...
async function buildKiraStatusReply(cfg: AforaConfig) {
  return await buildStatusReply({
    cfg,
    command: {
