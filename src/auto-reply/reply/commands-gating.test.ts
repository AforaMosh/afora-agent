...
function buildParams(commandBody: string, cfg: AforaConfig): HandleCommandsParams {
  const ctx = {
    Body: commandBody,
...
describe("command gating", () => {
  ...
  it("blocks disabled bash", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: false } } as AforaConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: { enabled: true, allowed: true, failures: [] },
    });
  ...
  it("blocks bash when elevated access is unavailable", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: true } } as AforaConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: {
        enabled: true,
  ...
  it("redacts secret-shaped fields from full /config show replies", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      parsed: {
        gateway: {
          auth: {
            mode: "token",
            token: "AFORA_CONFIG_SHOW_CANARY_TOKEN_65623",
            password: "AFORA_CONFIG_SHOW_CANARY_PASSWORD_65623",
          },
          bind: "127.0.0.1",
          port: 3210,
        },
        models: {
          providers: {
            openai: {
              apiKey: "AFORA_CONFIG_SHOW_CANARY_API_KEY_65623",
        ...
        browser: {
          cdpUrl:
            "wss://chrome.example.test/devtools?token=AFORA_CONFIG_SHOW_CANARY_CDP_TOKEN_65623&apiKey=AFORA_CONFIG_SHOW_CANARY_CDP_API_KEY_65623",
          profiles: {
            local: {
            ...
            remote: {
              cdpUrl:
                "wss://chrome.remote.example.test/devtools?apiKey=AFORA_CONFIG_SHOW_CANARY_CDP_PROFILE_API_KEY_65623",
            },
          },
        },
        talk: {
    ...
    expect(output).not.toContain("AFORA_CONFIG_SHOW_CANARY_TOKEN_65623");
    expect(output).not.toContain("AFORA_CONFIG_SHOW_CANARY_PASSWORD_65623");
    expect(output).not.toContain("AFORA_CONFIG_SHOW_CANARY_API_KEY_65623");
    expect(output).not.toContain("AFORA_CONFIG_SHOW_CANARY_CDP_TOKEN_65623");
    expect(output).not.toContain("AFORA_CONFIG_SHOW_CANARY_CDP_API_KEY_65623");
    expect(output).not.toContain("AFORA_CONFIG_SHOW_CANARY_CDP_PROFILE_API_KEY_65623");
  ...
  it("redacts secret-shaped values from /config set acknowledgements", async () => {
    ...
    const params = buildParams(
      '/config set gateway.auth.token="AFORA_CONFIG_SET_CANARY_TOKEN_65623"',
      {
        commands: { config: true, text: true },
        channels: { whatsapp: { allowFrom: ["*"] } },
      } as AforaConfig,
    ...
    expect(output).toContain(REDACTED_SENTINEL);
    expect(output).not.toContain("AFORA_CONFIG_SET_CANARY_TOKEN_65623");
  });
  ...
  it("redacts secret-shaped fields from /debug show replies", async () => {
    getConfigOverridesMock.mockReturnValueOnce({
      gateway: {
        auth: {
          token: "AFORA_DEBUG_SHOW_CANARY_TOKEN_65623",
        },
      },
      channels: {
        telegram: {
          botToken: "AFORA_DEBUG_SHOW_CANARY_BOT_TOKEN_65623",
        },
      },
      messages: {
        ackReaction: ":)",
    ...
    expect(output).not.toContain("AFORA_DEBUG_SHOW_CANARY_TOKEN_65623");
    expect(output).not.toContain("AFORA_DEBUG_SHOW_CANARY_BOT_TOKEN_65623");
  });

  it("redacts secret-shaped values from /debug set acknowledgements", async () => {
    const params = buildParams(
      '/debug set gateway.auth.token="AFORA_DEBUG_SET_CANARY_TOKEN_65623"',
      {
        commands: { debug: true, text: true },
        channels: { whatsapp: { allowFrom: ["*"] } },
      } as AforaConfig,
    ...
    expect(output).toContain(REDACTED_SENTINEL);
    expect(output).not.toContain("AFORA_DEBUG_SET_CANARY_TOKEN_65623");
  });
  ...
  it("enforces gateway client permissions for /config commands", async () => {
    const baseCfg = { commands: { config: true, text: true } } as AforaConfig;
