...
const DIAGNOSTICS_DOCS_URL = "https://docs.afora.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "afora gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentAforaCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
async function requestGatewayDiagnosticsExportApproval(
  ...
  try {
    ...
    const result = await execTool.execute("chat-diagnostics-gateway-export", {
      command,
      env: buildCurrentAforaCliExecEnv(),
      security: "allowlist",
      ask: "always",
      background: true,
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this Afora session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an Afora session file.",
    ) === true
  );
}
