...
function createOpenAiFileModelsConfig(): NonNullable<AforaConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime provider and media surfaces", () => {
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-secrets-file-provider-"));
  ...
  it("refreshes provider auth without resolving or republishing gateway state", async () => {
    ...
    const root = autoCleanupTempDirs.make("afora-provider-auth-refresh-");
    const secretsPath = path.join(root, "secrets.json");
    ...
    try {
      ...
      const runtimeSourceConfig: AforaConfig = {
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
  ...
  it("isolates an inherited memory ref to its exact agent owner", async () => {
    ...
    const snapshot = await prepareSecretsRuntimeSnapshot({
      ...
      agentDirs: ["/tmp/afora-agent-cold", "/tmp/afora-agent-healthy"],
      loadAuthStore: () => ({ version: 1, profiles: {} }),
      allowUnavailableSecretOwners: true,
    });
