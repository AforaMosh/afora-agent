...
function writeExternalChannelPlugin(params: { pluginId: string; channelId: string }) {
  const rootDir = makeTrackedTempDir("afora-channel-secret-contract", tempDirs);
  fs.writeFileSync(
...
describe("external channel secret contract api", () => {
  ...
  it("loads dist/ secret-contract-api sidecars for compiled npm-published external channel plugins", () => {
    const rootDir = makeTrackedTempDir("afora-channel-secret-contract-dist", tempDirs);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const rootDir = makeTrackedTempDir("afora-channel-secret-contract-hardlink", tempDirs);
      const outsideDir = makeTrackedTempDir(
        "afora-channel-secret-contract-hardlink-outside",
        tempDirs,
      );
