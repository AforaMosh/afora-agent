...
describe("secret ref resolver", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-secrets-resolve-"));
  ...
  itPosix(
    ...
    async () => {
      ...
      const configFor = (command: string): AforaConfig => ({
        secrets: {
          providers: {
    ...
    await withMockedWindowsAclVerificationUnavailable(
      ...
      async () => {
        ...
        expect(describeSecretResolutionOperatorRecovery(error)).toBe(
          "Restore Windows path security verification, or use an existing secret file whose owner and ACLs Afora can verify",
        );
      },
    );
  });
  ...
  it("fails closed on Windows when exec provider ACL source is unknown", async () => {
    ...
    await withMockedWindowsAclVerificationUnavailable(
      ...
      async () => {
        ...
        expect(describeSecretResolutionOperatorRecovery(error)).toBe(
          "Restore Windows path security verification, or use an existing provider command whose owner and ACLs Afora can verify",
        );
