...
import { isValidAgentId } from "@afora/normalization-core/agent-id";
...
export function parsePluginSecretTargetSpecifier(
  ...
  if (!value.startsWith("auth-profiles:")) {
    return {
      path: value.startsWith("afora:") ? value.slice("afora:".length) : value,
    };
  }
...
function createPluginConfigSecretTarget(params: {
  ...
  const resolved = resolveSecretPlanTargetByPathCore({
    configFile: params.agentId ? "auth-profiles.json" : "afora.json",
    pathSegments,
  });
  if (!resolved) {
    throw new Error(
