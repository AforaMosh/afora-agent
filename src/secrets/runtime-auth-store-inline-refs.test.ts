...
describe("secrets runtime snapshot inline auth-store refs", () => {
    ...
    expect(snapshot.warnings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          path: `/tmp/afora-agent-main.auth-profiles.${profileId}.key`,
        }),
        expect.objectContaining({
          code: "SECRETS_REF_IGNORED_INACTIVE_SURFACE",
          path: `/tmp/afora-agent-main.auth-profiles.${tokenProfileId}.token`,
        }),
      ]),
    );
  ...
  it("isolates an auth profile whose SecretRef provider is absent from copied config", async () => {
    const agentDir = "/tmp/afora-agent-copied-auth-store";
