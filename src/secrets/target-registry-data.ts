...
function createPluginAforaConfigSecretTargetEntry(
  pluginId: string,
  configPath: string,
  ...
  for (const record of plugins) {
    for (const config of WEB_PROVIDER_SECRET_CONFIGS) {
      if (
        ...
        entries.push(createPluginAforaConfigSecretTargetEntry(record.id, config.configPath));
      }
    }
  }
...
function listPluginConfigSecretTargetRegistryEntries(
  ...
  for (const record of plugins) {
    ...
    for (const secretInput of secretInputs) {
      const entry = createPluginAforaConfigSecretTargetEntry(record.id, secretInput.path);
...
export function getSecretTargetRegistry(params?: {
  ...
  if (params?.sourceTree) {
    ...
    return loadSecretTargetRegistryFromPluginMetadata({
      env: {
        ...process.env,
        AFORA_BUNDLED_PLUGINS_DIR: process.env.AFORA_BUNDLED_PLUGINS_DIR ?? "extensions",
      },
