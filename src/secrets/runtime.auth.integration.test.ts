...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("afora-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".afora", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".afora", "agents", "ops", "agent");
