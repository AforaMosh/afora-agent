...
describe("cron failure alert account routing", () => {
  ...
  it.each([
    ...
    const state = createCronServiceState({
      storePath: "/tmp/afora-cron-failure-alert-account-routing.json",
      cronEnabled: true,
      defaultAgentId: "main",
  ...
  it("carries run start time without using it for alert cooldown", () => {
    ...
    const state = createCronServiceState({
      storePath: "/tmp/afora-cron-failure-alert-run-time.json",
      cronEnabled: true,
  ...
  it.each([
    ...
    const sendCronFailureAlert = vi.fn(async () => undefined);
    const state = createCronServiceState({
      storePath: "/tmp/afora-cron-failure-alert-thread-routing.json",
      cronEnabled: true,
