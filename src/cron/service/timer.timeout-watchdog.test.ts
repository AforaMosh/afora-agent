...
type WatchdogScenarioDeps = Pick<CronServiceDeps, "runIsolatedAgentJob"> &
  Partial<
    Pick<
      CronServiceDeps,
      "cleanupTimedOutAgentRun" | "onIsolatedAgentSetupTimeout" | "sendCronFailureAlert"
    >
  >;

async function createWatchdogScenario(params: {
  id: string;
  name: string;
  scheduledAt: string;
  timeoutSeconds: number;
  deps: WatchdogScenarioDeps;
  configureJob?: (job: CronJob) => void;
}) {
  const store = timerRegressionFixtures.makeStorePath();
  const scheduledAt = Date.parse(params.scheduledAt);
  const cronJob = createIsolatedRegressionJob({
    id: params.id,
    name: params.name,
    scheduledAt,
    schedule: { kind: "at", at: new Date(scheduledAt).toISOString() },
    payload: { kind: "agentTurn", message: "work", timeoutSeconds: params.timeoutSeconds },
    state: { nextRunAtMs: scheduledAt },
  });
  params.configureJob?.(cronJob);
...
describe("cron service timer regressions", () => {
  it("outer cron timeout fires at configured timeoutSeconds, not at 1/3 (#29774)", async () => {
    ...
    const scenario = await createWatchdogScenario({
    ...
    const timerPromise = onTimer(scenario.state);
    ...
    const job = scenario.state.store?.jobs.find((entry) => entry.id === "timeout-fraction-29774");
  ...
  it("cleans up timed-out isolated runs even when the runner ignores abort", async () => {
    ...
    const { state, advance } = await createWatchdogScenario({
  ...
  it("does not spend setup timeout while waiting for cron-nested admission", async () => {
    ...
    const onIsolatedAgentSetupTimeout = vi.fn();
    const { state, cronJob, advance } = await createWatchdogScenario({
      id: "isolated-setup-timeout-lane-wait",
      name: "setup timeout lane wait",
      scheduledAt: "2026-05-10T09:01:00.000Z",
      timeoutSeconds: 120,
      deps: {
  ...
  it("does not notify setup timeout for custom-session cron waits", async () => {
    ...
    const { state, advance } = await createWatchdogScenario({
      ...
      scheduledAt: "2026-05-10T09:04:00.000Z",
      timeoutSeconds: 120,
      configureJob: (job) => {
        job.sessionTarget = "session:customCronSession";
      },
      deps: {
        cleanupTimedOutAgentRun: vi.fn(async () => {}),
        onIsolatedAgentSetupTimeout,
  ...
  it("times out isolated agent runs that stall before execution starts (#74803)", async () => {
    ...
    const { state, advance } = await createWatchdogScenario({
      ...
      scheduledAt: "2026-05-10T09:05:00.000Z",
  ...
  it("clears the pre-execution watchdog on explicit execution milestones (#80283)", async () => {
    ...
    const { state, advance } = await createWatchdogScenario({
      ...
      scheduledAt: "2026-05-10T09:10:00.000Z",
      timeoutSeconds: 1_200,
      deps: {
        cleanupTimedOutAgentRun,
        runIsolatedAgentJob: vi.fn(
  ...
  it.each([
    ...
    async ({ phase, phaseText, id, name }) => {
      ...
      const { state, advance } = await createWatchdogScenario({
        id,
        name,
        scheduledAt: "2026-05-13T09:56:00.000Z",
        timeoutSeconds: 1_200,
        deps: {
          cleanupTimedOutAgentRun,
          runIsolatedAgentJob: vi.fn(
  ...
  it("re-arms the pre-execution watchdog when before_agent_reply does not claim (#82811)", async () => {
    ...
    const { state, advance } = await createWatchdogScenario({
      ...
      scheduledAt: "2026-05-17T03:00:00.000Z",
      timeoutSeconds: 1_200,
      configureJob: (job) => {
        job.delivery = {
          mode: "announce",
