      ...
      },
      {
        ...
        plugin: {
          ...
          config: {
            listAccountIds: () => [],
            resolveAccount: () => ({}),
            resolveAllowFrom: ({ cfg }: { cfg: AforaConfig }) =>
...
function makeCfg(overrides?: Partial<AforaConfig>): AforaConfig {
  return {
    bindings: [],
    channels: {},
    ...overrides,
...
function makeForumBoundCfg(accountId = "account-b"): AforaConfig {
  return makeCfg({
    bindings: [
      {
        agentId: AGENT_ID,
...
async function resolveLastTarget(cfg: AforaConfig) {
