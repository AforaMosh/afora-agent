...
describe("runCronIsolatedAgentTurn session identity", () => {
  ...
  it("passes resolved agentDir to runEmbeddedAgent", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call.agentDir).toBe(path.join(home, ".afora", "agents", "main", "agent"));
    });
  });
  ...
  it.each([
    ...
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".afora", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
      ...
      expect(call.sessionTarget).toEqual({
        ...
        storePath: path.join(home, ".afora", "agents", agentId, "sessions", "sessions.json"),
      });
    });
  });
  ...
  it("uses lightweight bootstrap context for command-style cron payloads", async () => {
    await withTempHome(async (home) => {
      await runCronTurn(home, {
        jobPayload: {
          kind: "agentTurn",
          message: "cd /srv/afora && ./scripts/nightly-report.sh",
        },
      });
