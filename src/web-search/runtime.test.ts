...
function getCustomSearchApiKey(config?: AforaConfig): unknown {
...
function createCustomSearchConfig(apiKey: unknown): AforaConfig {
  return {
    plugins: {
      entries: {
        "custom-search": {
...
describe("web search runtime", () => {
  ...
  it("auto-detects a provider from a model-provider auth profile", async () => {
    const agentDir = mkdtempSync(path.join(os.tmpdir(), "afora-web-search-auth-"));
  ...
  it("auto-detects a provider from the active agent auth profile", async () => {
    const defaultAgentDir = mkdtempSync(path.join(os.tmpdir(), "afora-web-search-default-"));
    const activeAgentDir = mkdtempSync(path.join(os.tmpdir(), "afora-web-search-active-"));
  ...
  it("passes the active agentDir into selected provider tools", async () => {
    const activeAgentDir = mkdtempSync(path.join(os.tmpdir(), "afora-web-search-tool-agent-"));
