...
function resolveInstallerVersionCases(params: { stdinCwd: string }): string[] {
  const output = execFileSync(
    ...
    [
      "-c",
      `${versionHelperSource}
fake_afora_decorated() { printf '%s\\n' 'Afora 2026.3.10 (abcdef0)'; }
fake_afora_raw() { printf '%s\\n' "Afora dev's build"; }
AFORA_BIN=fake_afora_decorated resolve_afora_version
AFORA_BIN=fake_afora_raw resolve_afora_version
(
  cd "$1"
  source /dev/stdin <<'AFORA_STDIN_INSTALLER'
${versionHelperSource}
fake_afora_stdin() { printf '%s\\n' 'Afora 2026.3.10 (abcdef0)'; }
AFORA_BIN=fake_afora_stdin
resolve_afora_version
AFORA_STDIN_INSTALLER
)`,
      "afora-version-test",
      params.stdinCwd,
    ],
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const hostileCwd = makeTempDir(tempRoots, "afora-install-stdin-");
...
`,
      ...
      ).toEqual(["2026.3.10", "Afora dev's build", "2026.3.10"]);
    },
  );
});
