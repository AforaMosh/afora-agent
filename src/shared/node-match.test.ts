...
describe("shared/node-match", () => {
  ...
  it("prefers a unique current Afora client over a legacy clawdbot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=afora-macos.*node=third-mac.*client=afora-macos/,
    );
  });
