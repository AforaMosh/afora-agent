...
describe("avatar policy", () => {
  it("classifies avatar URI and path helpers directly", () => {
    ...
    expect(isWindowsAbsolutePath("C:\\\\avatars\\\\afora.png")).toBe(true);
  });
  ...
  it("detects avatar-like path strings", () => {
    expect(looksLikeAvatarPath("avatars/afora.svg")).toBe(true);
    expect(looksLikeAvatarPath("afora.webp")).toBe(true);
