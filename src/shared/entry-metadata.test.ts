...
describe("shared/entry-metadata", () => {
  it("prefers metadata emoji and homepage when present", () => {
    expect(
      resolveEmojiAndHomepage({
        metadata: { emoji: "🦀", homepage: " https://afora.ai " },
        frontmatter: { emoji: "🙂", homepage: "https://example.com" },
      }),
    ).toEqual({
      emoji: "🦀",
      homepage: "https://afora.ai",
    });
  });
  ...
  it("falls back through frontmatter homepage aliases and drops blanks", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { emoji: "🙂", website: " https://docs.afora.ai " },
      }),
    ).toEqual({
      emoji: "🙂",
      homepage: "https://docs.afora.ai",
    });
    expect(
      resolveEmojiAndHomepage({
        metadata: { homepage: "   " },
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { url: " https://afora.ai/install " },
      }),
    ).toEqual({
      homepage: "https://afora.ai/install",
    });
  });
  ...
  it("does not fall back once frontmatter homepage aliases are present but blank", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: {
          homepage: " ",
          website: "https://docs.afora.ai",
          url: "https://afora.ai/install",
        },
      }),
    ).toStrictEqual({});
  });
