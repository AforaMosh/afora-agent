...
// Harness-mirrored subagents (Codex/Copilot native threads) have no Afora child session, so
// they cannot be recovered or cancelled through one. Both halves of the test are load-bearing:
// Afora-owned rows always carry childSessionKey, and only the agent-harness task runtime stamps
