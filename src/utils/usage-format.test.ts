...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["AFORA_AGENT_DIR", "AFORA_STATE_DIR"]);
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-usage-format-"));
  ...
  it("prefers models.json pricing over afora config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to afora config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
