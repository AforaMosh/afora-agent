...
const CAPTURE_MARKER = "__AFORA_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__AFORA_SHELL_SNAPSHOT_ENV__";
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // AFORA_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.AFORA_SHELL === "exec") {
    env.AFORA_SHELL = "exec";
  }
  return env;
}
