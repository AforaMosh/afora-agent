import type { AgentEvent } from "afora-agent/plugin-sdk/agent-core";
...
describe("handleToolExecutionEnd cron mutation tracking", () => {
  ...
  it.each([
    ["exec", "afora cron add --at +1h --message 'follow up' --name reminder"],
    ["exec", "npx afora cron add --at=+1h --message 'follow up'"],
    ["exec", "bunx afora cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm exec afora cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx afora cron add --at +1h --message 'follow up'"],
    ["exec", "npx -y afora cron add --at +1h --message 'follow up'"],
    ["exec", "bunx --bun afora cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx afora-agent@latest cron add --at +1h --message 'follow up'"],
    ["exec", "npx afora-agent@latest cron add --at +1h --message 'follow up'"],
    ["exec", "bunx afora-agent@latest cron add --at +1h --message 'follow up'"],
    ["exec", "/usr/local/bin/afora cron add --at +1h --message 'follow up'"],
    ["bash", "corepack pnpm exec afora cron add --at +1h --message 'follow up'"],
    ["exec", "env AFORA_PROFILE=test afora cron add --at +1h --message 'follow up'"],
    ["exec", "afora cron create --at +1h --message 'follow up'"],
    ["exec", "afora --profile work cron create --at +1h --message 'follow up'"],
    ["exec", "afora --dev cron add --at +1h --message 'follow up'"],
    ["exec", "afora --log-level debug --no-color cron add --at +1h --message 'follow up'"],
    ["exec", "afora --container helper cron add --at +1h --message 'follow up'"],
    ["exec", "afora cron add --at +1h --message 'follow up || wait'"],
    ["exec", "afora cron add --at +1h --message 'follow up' 2>&1"],
  ...
  it("does not increment successfulCronAdds when shell cron add fails", async () => {
    ...
    await executeTool(ctx, {
      ...
      args: {
        command: "afora cron add --at +1h --message 'follow up' --name reminder",
      },
      isError: false,
      result: {
        details: {
  ...
  it.each([
    ["afora cron list --json", "a different cron action"],
    ["echo afora cron add --at +1h", "a command that only mentions cron add"],
    ["afora cron add --at '+1h", "an unterminated shell argument"],
    ["cd /tmp && afora cron add --at +1h", "a compound command"],
    ["afora cron add --help", "the add command help"],
    ["afora cron create -h", "the create alias help"],
    ["afora cron add --bad||true", "a masked cron failure"],
    ["afora cron add --at +1h; true", "a semicolon suffix"],
    ["afora cron add --at +1h | cat", "a pipeline suffix"],
    ["afora cron add --at +1h & true", "a background suffix"],
    ["afora cron add --at +1h\ntrue", "a newline-separated suffix"],
    ["afora cron add --bad # ignored\ntrue", "a comment-masked cron failure"],
    ["npx -y echo afora cron add --at +1h", "a package runner for another executable"],
    ["pnpm afora cron add --at +1h", "a bare pnpm package script"],
    ["corepack pnpm afora cron add --at +1h", "a corepack pnpm package script"],
    ["afora-agent@latest cron add --at +1h", "a package spec without a package runner"],
    ["pnpm exec afora-agent@latest cron add --at +1h", "a package spec passed to pnpm exec"],
    ["afora cron add --bad &>/tmp/cron.log", "a bash-only combined redirection"],
    ["afora cron add --bad &>>/tmp/cron.log", "a bash-only append redirection"],
...
describe("handleToolExecutionEnd timeout metadata", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      args: { command: "git status", workdir: "/Users/agent/Projects/Afora" },
...
describe("handleToolExecutionEnd exec approval prompts", () => {
  ...
  it("emits a deterministic unavailable payload when the initiating surface cannot approve", async () => {
    ...
    expect(text).toContain(
      "Print the Control UI URL with `afora dashboard --no-open`, open it in a browser, then use the approval inbox.",
    );
    expect(text).toContain(
      "Inspect the node's effective exec policy with `afora approvals get --node node-mac-1`.",
    );
