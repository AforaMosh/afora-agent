...
describe("isLiveTestEnabled", () => {
  it("treats LIVE and AFORA_LIVE_TEST as shared live gates", () => {
    expect(isLiveTestEnabled([], { LIVE: "1" })).toBe(true);
    expect(isLiveTestEnabled([], { AFORA_LIVE_TEST: "1" })).toBe(true);
...
describe("isLiveProfileKeyModeEnabled", () => {
  it("only enables profile-key mode for the dedicated flag", () => {
    expect(isLiveProfileKeyModeEnabled({ AFORA_LIVE_REQUIRE_PROFILE_KEYS: "1" })).toBe(true);
    expect(isLiveProfileKeyModeEnabled({ AFORA_LIVE_TEST: "1" })).toBe(false);
