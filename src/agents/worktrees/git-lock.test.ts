...
describe("lockWorktreeForProcess", () => {
  ...
  async function setupWorktree(): Promise<ManagedWorktreeRecord> {
    const root = await fs.realpath(tempDirs.make("afora-git-lock-"));
  ...
  it("reclaims a lock left behind by a dead Afora process", async () => {
    ...
    expect(await lockedReason(record.repoRoot, record.path)).toBe(`afora pid=${process.pid}`);
  });

  it("keeps a lock held by a live Afora process", async () => {
    const record = await setupWorktree();
    ...
    expect(await lockedReason(record.repoRoot, record.path)).toBe(`afora pid=${livePid}`);
  });

  it("keeps a foreign lock that Afora does not own", async () => {
    const record = await setupWorktree();
