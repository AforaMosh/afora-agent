...
function resolveLiveProviderCatalogTimeoutMs(env: NodeJS.ProcessEnv): number | null {
  const live =
    env.AFORA_LIVE_TEST === "1" || env.AFORA_LIVE_GATEWAY === "1" || env.LIVE === "1";
  if (!live) {
    return null;
  }
  const raw = env.AFORA_LIVE_PROVIDER_DISCOVERY_TIMEOUT_MS?.trim();
  if (!raw) {
    return 15_000;
  }
...
function resolveProviderDiscoveryFilter(params: {
  ...
  const rawValues = [
    env.AFORA_LIVE_PROVIDERS?.trim(),
    env.AFORA_LIVE_GATEWAY_PROVIDERS?.trim(),
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("afora.modelsConfigImplicitProvidersTestApi")
  ] = {
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): AforaConfig {
...
): Promise<NonNullable<AforaConfig["models"]>["providers"]> {
