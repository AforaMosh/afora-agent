...
export function findSourceTurnRange(params: {
...
export function findMessageToolCallIndexInSourceTurn(params: {
  messages: readonly unknown[];
...
export function hasSiblingAssistantToolCalls(message: unknown): boolean {
  if (!message || typeof message !== "object" || getMessageRole(message) !== "assistant") {
    return true;
  }
  const content = (message as { content?: unknown }).content;
...
export function findSuccessfulMessageToolResultIndex(params: {
  messages: readonly unknown[];
  sourceTurnRange: { startIndex: number; endIndex: number };
  toolCallId: string;
...
export function canReconcileTerminalDeliveryAtSourceTurnTail(params: {
  messages: readonly unknown[];
  sourceTurnId: string;
  sourceTurnRange: { startIndex: number; endIndex: number };
  toolCallId: string;
...
export function buildRecoveryToolResultIdempotencyKey(
  sourceTurnId: string,
  toolCallId: string,
): string {
