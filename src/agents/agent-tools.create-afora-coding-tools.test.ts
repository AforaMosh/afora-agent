/**
 * Broad coverage for createAforaCodingTools.
...
import type { AgentTool, AgentToolResult } from "afora-agent/plugin-sdk/agent-core";
...
import * as aforaPluginTools from "./afora-plugin-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createAforaCodingTools>) {
...
type AforaCodingTool = ReturnType<typeof createAforaCodingTools>[number];
type AforaToolsOptions = NonNullable<Parameters<typeof createAforaTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: AforaCodingTool[], name: string): AforaCodingTool {
...
function requireToolExecute(tool: AforaCodingTool): NonNullable<AforaCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateAforaToolsOptions(): AforaToolsOptions {
  const calls = vi.mocked(createAforaTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createAforaTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: AforaToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createAforaCodingTools", () => {
  it("forwards the session web-search gate to core tool materialization", () => {
    vi.mocked(createAforaTools).mockClear();
    createAforaCodingTools({ webSearchEnabled: false });

    expect(latestCreateAforaToolsOptions().webSearchEnabled).toBe(false);
  });
  ...
  it("exposes only gateway config reads to owner sessions", () => {
    const tools = createAforaCodingTools({ config: testConfig });
  ...
  it("passes explicit channel and requester facts to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-hook-channel-"));
  ...
  it("binds configured MCP cron authority only to the exact admitted run", async () => {
    ...
    const forgedTools = runWithCronCreatorAuthorityCapabilityResolver({
      ...
      runId: "forged-run",
      resolve,
      run: () => createAforaCodingTools({ runId: "forged-run", senderIsOwner: false }),
    });
    expect(toolNameList(forgedTools)).not.toContain("automations");
    expect(
      vi.mocked(createAforaTools).mock.lastCall?.[0]?.resolveCronCreatorToolAuthority,
    ).toBeUndefined();
    ...
    const activeRun = runWithCronCreatorAuthorityCapability(capability, async () => {
      const wrongRunTools = runWithCronCreatorAuthorityCapabilityResolver({
        ...
        runId: "other-run",
        resolve,
        run: () => createAforaCodingTools({ runId: "admitted-run", senderIsOwner: false }),
      });
      ...
      expect(admittedToolNames).not.toContain("afora");
      retainedResolver =
        vi.mocked(createAforaTools).mock.lastCall?.[0]?.resolveCronCreatorToolAuthority;
    ...
    expect(
      toolNameList(createAforaCodingTools({ runId: "admitted-run", senderIsOwner: false })),
  ...
  it("drops senderless Automations retention when exact authority aborts or errors", async () => {
    ...
    const buildTools = (capability: ReturnType<typeof createCronCreatorAuthorityCapability>) =>
      runWithCronCreatorAuthorityCapabilityResolver({
        ...
        runId: "lifecycle-run",
        resolve,
        run: () => createAforaCodingTools({ runId: "lifecycle-run", senderIsOwner: false }),
      });
    ...
    expect(
      toolNameList(createAforaCodingTools({ runId: "lifecycle-run", senderIsOwner: false })),
    ).not.toContain("automations");
  });
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createAforaTools).mockReturnValueOnce([wrapped as never]);

    const tools = createAforaCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core Afora tools are not materialized", () => {
    const createAforaToolsMock = vi.mocked(createAforaTools);
    createAforaToolsMock.mockClear();
  ...
  it("keeps the injected ring-zero tool under policy and rejects a same-name replacement", () => {
    ...
    vi.mocked(createAforaTools).mockReturnValueOnce([duplicateTool]);

    const tools = runWithAgentRingZeroTools([injectedTool], () =>
      createAforaCodingTools({
        config: { tools: { allow: ["read"], deny: ["afora"] } },
        runtimeToolAllowlist: ["afora"],
        toolConstructionPlan: {
          includeBaseCodingTools: false,
          includeShellTools: false,
          includeChannelTools: false,
    ...
    expect(tools[0]?.name).toBe("afora");
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateAforaToolsOptions();
  ...
  it("does not inherit native-harness bridge runtime allowlists", () => {
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateAforaToolsOptions().pluginToolAllowlist).toEqual([
      "sessions_spawn",
      "memory_search",
    ]);
    expect(latestCreateAforaToolsOptions().inheritedToolAllowlist).toEqual([]);
  });
  ...
  it("inherits embedded runtime toolsAllow when explicitly marked as parent capability", () => {
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateAforaToolsOptions().inheritedToolAllowlist;
  ...
  it("keeps restricted spawn inheritance in the caller-owned runtime snapshot", () => {
    ...
    expect(latestCreateAforaToolsOptions().inheritedToolAllowlist).toBe(
      inheritedToolAllowlistRef,
    );
  ...
  it("does not snapshot additive alsoAllow policies for spawn inheritance", () => {
    ...
    expect(latestCreateAforaToolsOptions().inheritedToolDenylist).toContain("exec");
  });
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:afora"], ["*"]]) {
      const tools = createAforaCodingTools({
        config: { tools: { profile: "minimal" } },
        runtimeToolAllowlist,
        toolConstructionPlan: {
  ...
  it("passes source reply delivery mode to Afora tool construction", () => {
    const createAforaToolsMock = vi.mocked(createAforaTools);
    createAforaToolsMock.mockClear();
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateAforaToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it.each([
    ...
    const storeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-completion-grant-"));
    ...
    const config: AforaConfig = { session: { store: storeTemplate } };
    ...
    try {
      ...
      expect(latestCreateAforaToolsOptions().sourceReplyOnly).toBe(testCase.expected);
    } finally {
      await fs.rm(storeDir, { recursive: true, force: true });
    }
  });

  it("passes configured filesystem policy to Afora tool construction", () => {
    const createAforaToolsMock = vi.mocked(createAforaTools);
    createAforaToolsMock.mockClear();
    ...
    expect(latestCreateAforaToolsOptions().fsPolicy).toEqual({ workspaceOnly: true });
  });
  ...
  it("uses the canonical spawn workspace for follow-up task suggestions", () => {
    ...
    expect(latestCreateAforaToolsOptions()).toMatchObject({
      workspaceDir: "/agent/workspace",
      spawnWorkspaceDir: "/host/project",
      cwd: "/host/project",
    });
  ...
  it("keeps an unsandboxed task repo as the follow-up suggestion cwd", () => {
    ...
    expect(latestCreateAforaToolsOptions().cwd).toBe("/task/repo");
  });
  ...
  it("passes plugin suppression into Afora tool construction plans", () => {
    const createAforaToolsMock = vi.mocked(createAforaTools);
    createAforaToolsMock.mockClear();
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateAforaToolsOptions().disablePluginTools).toBe(true);
  });

  it("forwards trusted conversation recall to Afora tool construction", () => {
    const createAforaToolsMock = vi.mocked(createAforaTools);
    createAforaToolsMock.mockClear();
    ...
    expect(latestCreateAforaToolsOptions().conversationRecall).toEqual(conversationRecall);
  });

  it("keeps plugin-only construction off the Afora core factory", () => {
    const createAforaToolsMock = vi.mocked(createAforaTools);
    createAforaToolsMock.mockClear();
  ...
  it("forwards prepared run facts to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(aforaPluginTools, "resolveAforaPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("forwards the native channel id through standard tool construction", () => {
    ...
    expect(latestCreateAforaToolsOptions().nativeChannelId).toBe("oc_native_chat");
    expect(latestCreateAforaToolsOptions().currentChatType).toBe("group");
    expect(latestCreateAforaToolsOptions().messageActionTurnCapability).toBe(
      "turn-capability-1",
    );
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateAforaToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
  ...
  it("materializes additive runtime tools while preserving normal deny policy", () => {
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateAforaToolsOptions().pluginToolAllowlist).not.toContain(
      "workboard_heartbeat",
    );
    expect(
      isPluginToolAllowed(
        new Set(latestCreateAforaToolsOptions().pluginToolAllowlist),
        "workboard",
        "workboard_heartbeat",
      ),
    ).toBe(true);
    expect(latestCreateAforaToolsOptions()).not.toHaveProperty("runtimePluginToolGrant");
    expectListIncludes(latestCreateAforaToolsOptions().pluginToolDenylist, ["workboard_block"]);
  });

  it("passes explicit denylist entries to Afora tool factory planning", () => {
    const createAforaToolsMock = vi.mocked(createAforaTools);
    createAforaToolsMock.mockClear();
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateAforaToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("removes message from persisted visible child sessions on every turn", async () => {
    const storeDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-visible-subagent-message-"));
    const storeTemplate = path.join(storeDir, "{agentId}", "sessions.json");
  ...
  it("passes inherited allowlist entries to Afora plugin discovery", async () => {
    const createAforaToolsMock = vi.mocked(createAforaTools);
    createAforaToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `afora-session-store-${agentId}-{agentId}.json`,
    );
    await writeSessionStore(storeTemplate, agentId, {
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateAforaToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateAforaToolsOptions().cronCreatorToolAllowlist;
  ...
  it("passes the final unrestricted tool surface to cron-created agent turns", () => {
    ...
    createAforaCodingTools({ config: {} });

    expect(createAforaToolsMock).toHaveBeenCalledTimes(1);
    const cronAllowNames = cronCreatorToolNames(
      latestCreateAforaToolsOptions().cronCreatorToolAllowlist,
    );
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      AforaToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("afora-tools:test-helper")).toBeLessThan(
      stages.indexOf("afora-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createAforaCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createAforaCodingTools({
      config: testConfig,
    ...
    const codexTools = createAforaCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: AforaConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createAforaCodingTools({
    ...
    const anthropicTools = createAforaCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: AforaConfig = {
      tools: {
        exec: {
    ...
    const allowed = createAforaCodingTools({
    ...
    const denied = createAforaCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createAforaCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAforaCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    ...
    expect(latestCreateAforaToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createAforaCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createAforaCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createAforaCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createAforaCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createAforaCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createAforaCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createAforaCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createAforaCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createAforaCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-read-"));
    try {
      ...
      const contents = "Hello from afora read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "afora-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "afora-workspace"),
    ...
    const tools = createAforaCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-canonical-"));
    try {
      const tools = createAforaCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "afora-memory-cwd-"));
  ...
  it("records ordinary write, edit, and apply_patch memory provenance from turn taint", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-memory-write-taint-"));
  ...
  it("records agent provenance after an untainted same-turn delete and recreate", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-memory-recreate-"));
  ...
  it("orders parallel apply_patch delete cleanup before a tainted recreate", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-memory-race-"));
    let recordedOrigin: "agent" | "untrusted" | undefined;
  ...
  it("records sandbox-backed memory writes before mutation", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-memory-sandbox-taint-"));
    const recordWriteProvenance = vi.fn<NonNullable<MemoryFlushPlan["recordWriteProvenance"]>>(
      async () => {},
    );
    registerMemoryCapability("memory-core", {
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-legacy-alias-"));
    try {
      const tools = createAforaCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-structured-write-"));
    try {
      const tools = createAforaCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-structured-edit-"));
    try {
...
describe("createAforaCodingTools read behavior", () => {
  ...
  it("uses host decoding only for host-backed sandbox paths", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-sbx-encoding-"));
  ...
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "afora-outside.txt");
  ...
  it("rejects sandbox directory reads before calling the bridge read operation", async () => {
    const tmpDir = tempDirs.make("afora-sbx-directory-");
  ...
  it("resolves Unicode-equivalent filenames through sandbox operations", async () => {
    const tmpDir = tempDirs.make("afora-sbx-unicode-");
  ...
  it("classifies sandbox AVIF reads from the already-read buffer", async () => {
    const tmpDir = tempDirs.make("afora-sbx-avif-");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-read-autopage-"));
  ...
  it("maps inbound media refs to sandbox-staged media for reads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-read-media-sbx-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-read-cap-"));
  ...
  it("describes explicit offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-read-offset-eof-"));
  ...
  it("ignores a trailing newline when describing offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-read-offset-adaptive-"));
    await fs.writeFile(path.join(tmpDir, "notes.txt"), "one\ntwo\nthree\n", "utf8");
    try {
      const readTool = createSandboxedReadTool({
        root: tmpDir,
  ...
  it("stops adaptive pagination when the current page reaches EOF", async () => {
    ...
    const readTool = createAforaReadTool({
      name: "read",
      label: "read",
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createAforaReadTool(
      baseRead as unknown as Parameters<typeof createAforaReadTool>[0],
    );
...
describe("createAforaCodingTools message provider policy", () => {
