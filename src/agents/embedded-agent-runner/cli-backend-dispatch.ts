 ...
 * afora harness and call the provider API directly with that runtime's
 * credentials (`cli_runtime_passthrough_afora`). Anthropic routes direct
...
import { stripAforaMcpToolPrefix } from "../cli-runner/tool-policy.js";
...
async function runEmbeddedAgentViaCliBackend(
  ...
  const cliToolAvailability = {
    native: [] as [],
    afora: dispatch.toolsAllow,
  };
  const onAgentToolResult = params.onAgentToolResult;
  // The CLI backend writes no Afora session records; mirror the run into
  ...
  const unsubscribe = onAgentEvent((evt) => {
    ...
    const toolName = normalizeToolPolicyName(stripAforaMcpToolPrefix(rawName));
