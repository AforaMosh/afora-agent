...
export async function executePreparedCompactionSession(runtime: PreparedCompactionRuntime) {
  ...
  try {
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Afora
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores Afora runtime's auto-compaction (afora#75799), so re-apply
      ...
      // exact Afora-managed registrations so custom tools survive startup.
