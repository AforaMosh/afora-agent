...
export async function detectAndLoadPromptImages(
  ...
  const unsuppressedImageFactIndexes = normalizeMediaFacts(params.media).flatMap(
    (fact, factIndex) =>
      isImageMediaFact(fact) && !suppressedFactIndexes.has(factIndex) ? [factIndex] : [],
  );
...
export async function hydratePromptMediaMessages(
  ...
  for (const [index, message] of messages.entries()) {
    ...
    if (!resolvedMedia.length && !existingMedia.some((block) => block.type === "video")) {
      continue;
    }
    const persistedImageFactIndexes = readPersistedImageBlockFactIndexes(message);
    const persistedMediaFactIndexes = readPersistedMediaBlockFactIndexes(message);
    const existingMediaFactIndexes =
      persistedMediaFactIndexes?.length === existingMedia.length
        ? persistedMediaFactIndexes
        : readRuntimePromptImageFactIndexes(existingMedia);
    ...
    if (result.media.length > 0) {
      nextMeta.mediaBlockFactIndexes = readRuntimePromptImageFactIndexes(result.media) ?? [];
    } else {
      delete nextMeta.mediaBlockFactIndexes;
    }
    const previousOmissions = Array.isArray(nextMeta.mediaVideoOmissions)
      ? (nextMeta.mediaVideoOmissions as Array<{ reason?: unknown }>)
      : [];
    const omissionTextsToReplace = previousOmissions.flatMap((entry) =>
      typeof entry.reason === "string" &&
      ["unsupported", "unavailable", "invalid", "mime", "item-size", "count", "aggregate"].includes(
        entry.reason,
      )
        ? [formatNativeVideoOmission(entry.reason as NativeVideoOmissionReason)]
        : [],
    );
    const retainedContent = content.filter((block) => {
      ...
      const omissionIndex = omissionTextsToReplace.indexOf(block.text);
      if (omissionIndex < 0) {
        return true;
      }
      omissionTextsToReplace.splice(omissionIndex, 1);
      return false;
    });
    if (result.videoOmissions.length > 0) {
      nextMeta.mediaVideoOmissions = result.videoOmissions.map(
        ({ sourceId, sourceIndex, reason }) => ({
          ...(sourceId ? { sourceId } : {}),
          sourceIndex,
          reason,
        }),
      ...
      delete nextMeta.mediaVideoOmissions;
    }
    hydrated ??= messages.slice();
    const hydratedMessage = {
      ...message,
      content: [...retainedContent, ...result.orderedBlocks],
    } as AgentMessage;
