...
describe("fact-carried image references", () => {
  ...
  it.each([
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-canonical-media-ref-"));
  ...
  it("loads an explicit ref matching a fact sliced into an inline slot", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-inline-explicit-ref-"));
  ...
  it("keeps identity-bearing refs when image order metadata has more inline slots", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-partial-inline-order-"));
  ...
  it("fails an exact inline slot whose image block is missing", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-missing-inline-"));
  ...
  it("hydrates a fact whose only local identity is a file URL", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-image-file-url-"));
    const imagePath = path.join(workspaceDir, "photo.png");
    await fs.writeFile(imagePath, Buffer.from(TINY_PNG_BASE64, "base64"));
  ...
  it("hydrates managed inbound media URIs before workspace path resolution", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-native-image-uri-"));
  ...
  it("hydrates sandbox-staged inbound media URIs", async () => {
    const sandboxRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-native-image-sbx-uri-"));
  ...
  it("allows sandbox-validated host paths outside default media roots", async () => {
    ...
    const sandboxParent = await fs.mkdtemp(path.join(homeDir, "afora-sandbox-image-"));
    try {
