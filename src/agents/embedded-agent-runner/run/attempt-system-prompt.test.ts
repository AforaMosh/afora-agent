  ...
  it("injects workspace identity context", () => {
    ...
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        userDate: "2026-01-05",
        contextFiles: [
          { path: "/tmp/afora/SOUL.md", content: "SOUL_CONTEXT_MARKER" },
          { path: "/tmp/afora/IDENTITY.md", content: "IDENTITY_CONTEXT_MARKER" },
          { path: "/tmp/afora/USER.md", content: "USER_CONTEXT_MARKER" },
        ],
      },
    ...
    expect(result.systemPrompt).toContain("## /tmp/afora/SOUL.md");
    expect(result.systemPrompt).toContain("SOUL_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/afora/IDENTITY.md");
    expect(result.systemPrompt).toContain("IDENTITY_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/afora/USER.md");
  ...
  it("preserves bootstrap Project Context", () => {
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        contextFiles: [
          {
            path: "/tmp/afora/BOOTSTRAP.md",
            content: "Reply with BOOTSTRAP_OK.",
          },
          {
            path: "/tmp/afora/SOUL.md",
            content: "SOUL_CONTEXT_MARKER",
          },
          {
            path: "/tmp/afora/IDENTITY.md",
            content: "IDENTITY_CONTEXT_MARKER",
          },
          {
            path: "/tmp/afora/USER.md",
    ...
    expect(result.systemPrompt).toContain("## /tmp/afora/USER.md");
    expect(result.systemPrompt).toContain("USER_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/afora/BOOTSTRAP.md");
