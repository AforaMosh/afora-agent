...
beforeEach(async () => {
  const rawRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-cli-fault-sequences-"));
...
function fallbackConfig(fallbacks: string[] = []): AforaConfig {
  return {
    agents: {
      defaults: {
...
function buildReusableProcessContext(params: CliBoundaryParams): PreparedCliRunContext {
  ...
  context.aforaHistoryPrompt = RESEED_PROMPT;
...
describe("CLI runner fault sequences", () => {
  ...
  it("stops max-turns after one parsed tool effect without recovery or model replay", async () => {
          ...
          content: [
            {
              type: "mcp_tool_use",
              id: "effect-1",
              name: "mcp__afora__memory_search",
              input: { query: "wings" },
            },
            {
    ...
    expect(toolEvents.filter((event) => event.phase === "result")).toEqual([
      expect.objectContaining({ name: "mcp__afora__memory_search" }),
    ]);
    expect(supervisorSpawnMock).toHaveBeenCalledTimes(1);
    expectCounts({
      childProcesses: 1,
