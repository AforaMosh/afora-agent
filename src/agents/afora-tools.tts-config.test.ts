// Verifies createAforaTools wires shared config and context into the TTS tool.
...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    textToSpeech: vi.fn(async () => ({
      success: true,
      audioPath: "/tmp/afora/tts-config-test.opus",
      provider: "microsoft",
...
vi.mock("./afora-tools.nodes-workspace-guard.js", () => ({
...
describe("createAforaTools TTS config wiring", () => {
  beforeEach(() => {
...
describe("createAforaTools transcript ownership wiring", () => {
  beforeEach(() => {
    mocks.createTranscriptsToolOptions.mockClear();
  });

  it("uses trusted caller authority instead of the delivery account", () => {
    const injectedConfig = { transcripts: { enabled: true } } satisfies AforaConfig;
...
describe("createAforaTools media generation session wiring", () => {
  beforeEach(() => {
    mocks.createImageGenerateToolOptions.mockClear();
    mocks.createMusicGenerateToolOptions.mockClear();
    mocks.createVideoGenerateToolOptions.mockClear();
...
describe("createAforaTools session status route context wiring", () => {
  beforeEach(() => {
...
describe("createAforaTools cron context wiring", () => {
  beforeEach(() => {
    mocks.createCronToolOptions.mockClear();
  });
