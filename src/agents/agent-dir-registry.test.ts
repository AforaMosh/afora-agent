...
describe("agent directory registry", () => {
  it("unregisters only the requested owner", () => {
    const agentDir = path.join("/tmp", `afora-agent-dir-registry-${process.pid}`);
  ...
  it("detects registered ownership on either side of a cleanup boundary", () => {
    const root = path.join("/tmp", `afora-agent-dir-overlap-${process.pid}`);
    ...
    expect(
      isPathOwnedByAnotherRegisteredAgent({
        agentId: "deleted",
        pathname: path.join(agentDir, "afora-agent.sqlite"),
      }),
    ).toBe(true);
    expect(
      isPathOwnedByAnotherRegisteredAgent({
  ...
  it("keeps ownership stable through a symlinked parent after the agent dir is removed", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-agent-dir-registry-"));
