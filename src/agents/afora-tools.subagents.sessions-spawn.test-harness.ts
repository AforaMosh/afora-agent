...
type CreateAforaToolsOpts = Parameters<CreateSessionsSpawnTool>[0];
...
const sessionStorePath = path.join(
  os.tmpdir(),
  `afora-sessions-spawn-test-store-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
);
...
export async function getSessionsSpawnTool(opts: CreateAforaToolsOpts) {
