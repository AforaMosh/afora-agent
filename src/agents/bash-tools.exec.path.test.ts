...
const ENV_KEYS = ["AFORA_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL", "SSLKEYLOGFILE"] as const;
...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("AFORA_SHELL")) {
        input.onStdout?.(env.AFORA_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  it("strips malformed XML arg-value suffixes from exec command and routing options", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-exec-xml-"));
    try {
  ...
  it("fails without running when an explicit workdir is unavailable", async () => {
    const missingWorkdir = path.join(
      os.tmpdir(),
      `afora-missing-workdir-${process.pid}-${Date.now()}`,
    );
  ...
  it("sets AFORA_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-afora-shell", {
      command: 'printf "%s" "${AFORA_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
    const value = normalizeText(result.content.find((c) => c.type === "text")?.text);
  ...
  it("does not apply login-shell PATH when probe rejects unregistered absolute SHELL", async () => {
    ...
    process.env.PATH = "/usr/bin";
    const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-shell-env-"));
...
describe("exec host env validation", () => {
  ...
  it("strips dangerous inherited env vars from host execution", async () => {
    ...
    process.env.SSLKEYLOGFILE = "/tmp/afora-ssl-keys.log";
    try {
      const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
      const result = await tool.execute("call1", {
      ...
      expect(output).not.toContain("/tmp/afora-ssl-keys.log");
    } finally {
  ...
  it.each([
    ...
    "exec -a afora /approve abc123 deny",
    ...
    "sudo AFORA_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root AFORA_APPROVE=1 bash -lc '/approve abc123 allow-once'",
