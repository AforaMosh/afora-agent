...
async function withTempDir<T>(fn: (dir: string) => Promise<T>) {
  ...
  const dir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "afora-patch-")));
  try {
    return await fn(dir);
  } finally {
...
async function withWorkspaceTempDir<T>(fn: (dir: string) => Promise<T>) {
  const dir = await fs.mkdtemp(path.join(process.cwd(), "afora-patch-workspace-"));
  try {
    return await fn(dir);
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
...
*** End Patch`;
  ...
  it("rejects absolute paths outside cwd by default", async () => {
    await withTempDir(async (dir) => {
      const escapedPath = path.join(os.tmpdir(), `afora-apply-patch-${Date.now()}.txt`);
...
*** End Patch`;
  ...
  it("allows deleting a symlink itself even if it points outside cwd", async () => {
    await withTempDir(async (dir) => {
      const outsideDir = await fs.mkdtemp(path.join(path.dirname(dir), "afora-patch-outside-"));
      try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (dir) => {
        ...
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), "afora-patch-outside-"));
