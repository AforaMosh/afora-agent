...
describe("tool display details", () => {
  ...
  it("summarizes tool-search code targets from described tool ids", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: "const tool = await afora.tools.describe('afora:core:exec'); return await afora.tools.call(tool.id, { command: 'echo hi' });",
      }),
    ).toEqual({
      toolName: "afora:core:exec",
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await afora.tools.call("web_search", { query: "Afora", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "Afora",
      count: 1000,
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "Afora docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "Afora docs" (top 3)');
  });
  ...
  it("formats Parallel's native objective + search_queries shape", () => {
    expect(
      ...
      'for "Find the Afora repository on GitHub", "afora github", "afora repository" (top 5)',
    );
  });
  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo pipefail\ngit -C /Users/adityasingh/.afora/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.afora/workspace",
        },
      }),
    );
  ...
  it("uses compact workspace markers for common workspace paths", () => {
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git fetch", workdir: "/Users/peter/mantis-workspace/afora" },
          detailMode: "explain",
        }),
      ),
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git status", workdir: "/Users/peter/Projects/afora" },
          detailMode: "explain",
        }),
      ),
      ...
      formatToolDetail(
        resolveToolDisplay({
          ...
          args: {
            ...
            workdir: "/root/.afora/sandboxes/agent-clawsweeper-sandbox-discor-766423d0",
          },
          detailMode: "explain",
        }),
      ),
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "Afora docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "Afora docs"');
  });
    ...
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          ...
          workdir: "/Users/example/.afora/workspace",
        },
        detailMode: "explain",
      }),
    );
  ...
  it("keeps heredoc body separators out of top-level stage splitting", () => {
    const stages = splitTopLevelStages(
      [
        "mkdir -p .afora/tmp/farm-notices",
        "cat > .afora/tmp/farm-notices/ventura.txt <<'EOF'",
