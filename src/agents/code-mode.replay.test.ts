...
describe("Code Mode restart-safe replay", () => {
  ...
  it("rejects agents.run before dispatch when cold recovery rotates identity", async () => {
    const getSwarmRunByLaunchReplayKey = vi.fn();
    testing.setSwarmDepsForTest({ getSwarmRunByLaunchReplayKey });
    const spawnTool = fakeTool("sessions_spawn", "Spawn");
    const {
      config,
      catalogRef,
      ctx,
    ...
    const sourceReplayId = testing.codeModeReplayIdForToolCall(
      { ...ctx, runId: "run-source" },
      "call_0",
      code,
      "response-source",
    );
    const recoveryReplayId = testing.codeModeReplayIdForToolCall(
      { ...ctx, runId: "run-recovery" },
      "call_0",
      code,
      "response-recovery",
    );
    ...
    expect(recoveryReplayId).not.toBe(sourceReplayId);
    ...
    expect(spawnTool.execute).not.toHaveBeenCalled();
  });
