...
describe("buildAuthProfileUnusableHint", () => {
  it("keeps Claude subscription and Anthropic API-key recovery distinct", () => {
    expect(
      ...
      "claude auth login && afora models auth login --provider anthropic --method cli --profile-id 'anthropic:claude-cli'",
    );
    expect(
      buildAuthProfileUnusableHint({
        kind: "cooldown",
    ...
    ).toContain("afora models auth login --provider anthropic --profile-id 'anthropic:api-key'");
    expect(
      buildAuthProfileUnusableHint({
        kind: "cooldown",
        reason: "auth",
  ...
  it("routes legacy Gemini CLI profiles to supported Google API-key setup", () => {
    ...
    expect(hint).toBe(
      "Gemini CLI OAuth cannot be repaired by Afora. Connect Google with an AI Studio API key using `afora models auth login --provider google`, then select that Google profile for the Gemini CLI runtime.",
    );
...
describe("oauth refresh failure hints", () => {
  ...
  it("includes the profile id in refresh-failure login hints when known", () => {
    ...
    ).toBe("afora models auth login --provider openai --profile-id 'Work Profile'");
  });
  ...
  it("renders login commands containing backticks as valid Markdown code spans", () => {
    ...
    expect(formatOAuthRefreshFailureLoginCommandMarkdown(command)).toBe(
      "``afora models auth login --provider openai --profile-id 'openai:work`slot'``",
    );
  });
    ...
    await withLoopbackServer(
      ...
      async (baseUrl) => {
        ...
        expect(error?.message).toContain(
          "Re-auth with `claude auth login && afora models auth login --provider anthropic --method cli`",
        );
        console.log(
