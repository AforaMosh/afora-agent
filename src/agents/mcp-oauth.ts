...
import {
  type AforaStateLeaseContext,
  withAforaStateLease,
} from "../state/afora-state-lease.js";
import {
  buildMcpHttpFetch,
...
function mcpOAuthAdditionalAuthorizationError(serverName: string): Error {
  return new Error(
    `MCP server "${serverName}" requires additional OAuth authorization. Run afora mcp login ${serverName}.`,
  );
}
  ...
  return await withMcpOAuthLease(
    ...
    async (lease) => {
      ...
      if (!tokens?.access_token) {
        ...
        throw new Error(
          `MCP server "${params.identity.serverName}" requires OAuth authorization. Run afora mcp login ${params.identity.serverName}.`,
        );
      }
      ...
      if (!tokens.refresh_token) {
        throw new Error(
          `MCP server "${params.identity.serverName}" has expired OAuth credentials. Run afora mcp login ${params.identity.serverName}.`,
        );
      }
      ...
      if (result !== "AUTHORIZED" || !refreshedTokens?.access_token) {
        throw new Error(
          `MCP server "${params.identity.serverName}" could not refresh OAuth credentials. Run afora mcp login ${params.identity.serverName}.`,
        );
      }
