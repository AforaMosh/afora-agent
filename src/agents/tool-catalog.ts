...
function buildCoreToolGroupMap() {
  ...
  const aforaTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAforaGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:afora": aforaTools,
