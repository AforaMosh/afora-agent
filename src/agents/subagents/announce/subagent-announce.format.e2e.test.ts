...
let configOverride: AforaConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: AforaConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/AforaMosh/afora-agent/issues/31298
  ...
  afterAll(() => {
    ...
    process.env.AFORA_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // AFORA_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Afora runtime context (internal):");
