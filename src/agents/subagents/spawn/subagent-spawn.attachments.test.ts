...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(async () => {
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `afora-subagent-attachments-${process.pid}-${Date.now()}-`),
    );
  ...
  it("materializes attachments under explicit cwd when native subagent cwd is provided", async () => {
    const explicitWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `afora-subagent-cwd-attachments-${process.pid}-${Date.now()}-`),
    );
    try {
      const { spawnSubagentDirect } = subagentSpawnModule;
      const result = await spawnSubagentDirect(
      ...
      const explicitAttachmentsRoot = path.join(explicitWorkspaceDir, ".afora", "attachments");
      const targetAttachmentsRoot = path.join(workspaceDirOverride, ".afora", "attachments");
  ...
  it("normalizes explicit cwd before materializing native subagent attachments", async () => {
    const homeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `afora-subagent-home-attachments-${process.pid}-${Date.now()}-`),
    );
    ...
    try {
      await withEnvAsync({ HOME: homeDir }, async () => {
        ...
        expect(result.status).toBe("accepted");
        const attachmentsRoot = path.join(expectedCwd, ".afora", "attachments");
