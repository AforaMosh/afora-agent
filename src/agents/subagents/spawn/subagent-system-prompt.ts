...
export function buildSubagentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "5. Descendant completion is push-based; use an available turn-yield tool when needed; never busy-poll. If no turn-yield tool is available, end the turn in words — a completion event resumes you. Never wait inside a shell command (`sleep`, `until`, `while` loops): a quiet process is killed as unresponsive and its work is lost.",
  ...
  if (canSpawn) {
    lines.push(
      ...
      "Push-based: never list histories, sleep, or poll in loops. Use an available turn-yield tool when needed; otherwise end the turn and await the runtime event that resumes you.",
      "Waiting is never a shell command. A blocking `sleep`/`until`/`while` wait produces no output, trips the unresponsive-process kill, and discards the run.",
      ...
      ...(acpEnabled
        ? [
            ...
            "Local subagent list/status tools cover Afora runtime=subagent only; ACP ids come from `acp.allowedAgents`.",
            "Never ask the user for slash/CLI or exec afora/acpx when delegation tools can act.",
