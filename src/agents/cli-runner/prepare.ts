  ...
  try {
    ...
    const mcpClientGrantCapture =
      mcpClientGrant && mcpLoopbackRuntime
        ? (() => {
            // A warm CLI child keeps the bearer it launched with. Track which
            // token this prepared turn currently speaks for so capture fences
            // follow the transfer instead of the retired mint.
