/**
 * Materializes selected Afora skills as a temporary Claude CLI plugin.
 */
...
const AFORA_CLAUDE_PLUGIN_NAME = "afora-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredAforaTmpDir(), "afora-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, AFORA_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: AFORA_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Afora skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
