/**
 * Claude CLI argument helpers for Afora-managed bundle MCP config.
 */
import fs from "node:fs/promises";
import { isRecord } from "@afora/normalization-core/record-coerce";
import { normalizeOptionalString } from "@afora/normalization-core/string-coerce";
import { withAforaMcpCaptureHeader } from "./bundle-mcp-runtime.js";
...
export function findClaudeMcpConfigPaths(args?: string[]): string[] {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--mcp-config") {
      ...
      // as positional prompts after Afora injects its strict overlay.
...
/** Return Claude args with Afora's strict MCP config path injected. */
...
/** Writes the active per-attempt capture token into Afora's generated Claude MCP config. */
export async function writeClaudeMcpCaptureConfig(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      withAforaMcpCaptureHeader(
        raw,
        params.captureKey,
        "Claude MCP capture requires an afora server config",
      ),
      null,
      2,
    )}\n`,
