...
describe("prepareCliBundleMcpConfig", () => {
  it("disables Claude native web search without bundle MCP", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/afora-cli-web-search-disabled",
  ...
  it("serves only the exclusive config, ignoring user and plugin servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "afora-cli-bundle-mcp-exclusive-",
    );
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      exclusiveConfig: {
        mcpServers: { afora: { command: "node", args: ["afora.mjs"] } },
      },
    });
    ...
    expect(raw.mcpServers?.afora?.args).toEqual(["afora.mjs"]);
    expect(prepared.mcpConfigHash).toMatch(/^[0-9a-f]{64}$/);
  ...
  it("applies server disables to exclusive CLI MCP configs", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/afora-cli-bundle-mcp-exclusive-disable",
      exclusiveConfig: { mcpServers: { afora: { command: "node" } } },
      toolOverrides: { mcpServers: { afora: false } },
    });
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "afora-cli-bundle-mcp-workspace-root-",
    );
    const pluginRoot = path.join(workspaceDir, ".afora", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    const otherEnvPrepared = await prepareBundleProbeCliConfig({
      additionalConfig,
      env: {
        AFORA_MCP_TOKEN: "other-loopback-token",
        AFORA_MCP_CLI_CAPTURE_KEY: "",
      },
    });
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      env: {
        AFORA_MCP_TOKEN: "lb-tk-123",
        AFORA_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/afora-bundle-mcp-disabled",
    });
