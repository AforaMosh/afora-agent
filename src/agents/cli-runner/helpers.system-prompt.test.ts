...
describe("buildCliAgentSystemPrompt", () => {
  ...
  it("uses CLI backend tool fallback instead of Afora tool assumptions", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/afora",
      tools: [],
      modelDisplay: "test/model",
    ...
    expect(prompt).not.toContain("Afora lists the standard tools above");
    expect(prompt).not.toContain("This runtime enables:");
    expect(prompt).not.toContain("For long waits, avoid rapid poll loops");
    expect(prompt).not.toContain("Larger work: use `sessions_spawn`");
    expect(prompt).not.toContain("Do not poll `subagents list` / `sessions_list` in a loop");
    expect(prompt).toContain("No Afora tool list is injected");
  });
  ...
  it("uses cwd, not bootstrap workspace, for CLI workspace guidance", () => {
    ...
    expect(prompt).not.toContain("Working directory: /tmp/afora-agent");
  });
  ...
  it("includes CLI-scoped plugin command guidance", () => {
    ...
    // Afora-main command text into external CLI backends.
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "Afora-only command guidance.",
          surfaces: ["afora_main"],
        },
      ],
    ...
    expect(prompt).not.toContain("Afora-only command guidance.");
  });
