...
const LIVE = isLiveTestEnabled(["AFORA_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "afora-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("afora_live_probe", "Afora Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Afora-configured streamable-http server", async () => {
    const geminiCommand = process.env.AFORA_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("aforaLiveProbe");
