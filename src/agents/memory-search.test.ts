...
const asConfig = (cfg: AforaConfig): AforaConfig => ({
  ...cfg,
...
describe("memory search config", () => {
  ...
  function configWithDefaultProvider(provider: string): AforaConfig {
    return asConfig({
      memory: {
        search: {
          provider,
  ...
  it.each([
    ...
    expect(resolveRememberAcrossConversations(asConfig(cfg as AforaConfig), "main")).toBe(
      expected,
    );
  });
  ...
  it("defaults provider to openai when unspecified", () => {
    ...
    expect(resolved?.store.databasePath).toBe(resolveAforaAgentSqlitePath({ agentId: "main" }));
  });
