...
type PluginModelCatalogDatabase = Pick<AforaAgentKyselyDatabase, "cache_entries">;
...
function repairPersistedPluginModelCatalogs(params: {
  ...
  const applied = runAforaAgentWriteTransaction(
    (database) => {
      const kysely = getNodeSqliteKysely<PluginModelCatalogDatabase>(database.db);
...
/** Detects model catalogs generated by Afora rather than user-authored JSON. */
