...
export function formatDiskSpaceErrorCopy(raw: string): string | undefined {
  ...
  return /\benospc\b/i.test(raw) ||
    ...
    ? "Afora could not write local session data because the disk is full. Free some disk space and try again."
    : undefined;
}
...
export function renderMissingApiKeyReplyCopy(params?: {
  ...
  if (provider === "openai") {
    return '⚠️ Missing API key for provider "openai". Run `afora doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `afora models auth login --provider openai` or run `afora configure`.';
  }
...
export function renderCliTimeoutReplyCopy(params: {
  ...
  if (params.replayPrevented) {
    workStatus += " Afora did not replay this turn automatically.";
  }
  return mode === "no-output"
    ...
    : `⚠️ CLI turn${routingSuffix}: timed out after ${seconds}s (overall turn limit). The gateway is unaffected.${workStatus} For long work, use a detached Afora sub-agent (no run timeout by default), or raise \`agents.defaults.timeoutSeconds\`.`;
}
...
const CONTROL_UI_LOG_HINT = "To view logs, run `afora logs --follow` in a terminal.";
