...
describe("MCP OAuth refresh issuer binding", () => {
  beforeEach(() => closeAforaStateDatabaseForTest());
  ...
  it("does not send the stored refresh token to a different issuer from a challenge", async () => {
    await withTempHome(
      ...
      { prefix: "afora-mcp-oauth-issuer-switch-", ...TEMP_HOME_OPTIONS },
    );
  });
  ...
  it("still refreshes when a new metadata URL resolves to the original issuer", async () => {
    await withTempHome(
      ...
      { prefix: "afora-mcp-oauth-issuer-same-", ...TEMP_HOME_OPTIONS },
    );
  });
  ...
  it.each([
    ...
    async ({ issuer }) => {
      await withTempHome(
        ...
        { prefix: "afora-mcp-oauth-issuer-normalization-", ...TEMP_HOME_OPTIONS },
      );
    },
  );
  ...
  it("refreshes an upgraded row whose issuer is recoverable from stored discovery", async () => {
    await withTempHome(
      ...
      { prefix: "afora-mcp-oauth-issuer-upgrade-", ...TEMP_HOME_OPTIONS },
    );
  });
  ...
  it("blocks an issuer-changing challenge on an upgraded row after it refreshed", async () => {
    await withTempHome(
      ...
      { prefix: "afora-mcp-oauth-issuer-upgrade-challenge-", ...TEMP_HOME_OPTIONS },
    );
  });
  ...
  it("binds legacy stored tokens to the pre-challenge issuer before rediscovery", async () => {
    await withTempHome(
      ...
      { prefix: "afora-mcp-oauth-issuer-legacy-", ...TEMP_HOME_OPTIONS },
    );
  });
  ...
  it("fails closed for a token-only legacy store with no recoverable issuer", async () => {
    await withTempHome(
      ...
      { prefix: "afora-mcp-oauth-issuer-tokenonly-", ...TEMP_HOME_OPTIONS },
    );
  });
  ...
  it("persists bound issuer provenance across a store reload", async () => {
    await withTempHome(
      ...
      { prefix: "afora-mcp-oauth-issuer-reload-", ...TEMP_HOME_OPTIONS },
    );
  });
});
