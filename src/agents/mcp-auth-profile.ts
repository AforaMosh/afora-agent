...
/** Returns whether a server needs an Afora-managed bearer projected externally. */
...
function buildTokenEnvVarName(serverName: string): string {
  ...
  return `AFORA_MCP_AUTH_${hash.toUpperCase()}_TOKEN`;
}

function stripAforaOnlyOAuthConfig(server: BundleMcpServerConfig): BundleMcpServerConfig {
  const next = { ...server };
  delete next.auth;
  delete next.oauth;
  return next;
...
export async function resolveMcpBearerBundleConfig(
  ...
  for (const [serverName, server] of Object.entries(params.config.mcpServers)) {
    ...
    nextServers ??= { ...params.config.mcpServers };
    nextServers[serverName] = stripAforaOnlyOAuthConfig({
      ...server,
      headers: {
        ...headers,
