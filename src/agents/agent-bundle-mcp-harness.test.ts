...
function makeRuntime(params: {
  ...
  const staticServerName = "shared-search";
      ...
      ...(params.mixedScope
        ? {
            [staticServerName]: {
              ...
              safeServerName: staticServerName,
              launchSummary: staticServerName,
              toolCount: 1,
            },
          }
        : {}),
              ...
              ? [
                  {
                    ...
                    safeServerName: staticServerName,
                    toolName: "search",
                    description: "search shared records",
                    inputSchema: { type: "object", properties: {} },
                    fallbackDescription: "search shared records",
                  },
                ]
              : []),
          ],
  ...
  return {
    ...
    ...(params.requesterSenderId
      ...
      : {}),
    ...(params.mixedScope
      ? { isRequesterScopedServer: (candidate: string) => candidate === serverName }
      : {}),
    createdAt: Date.now(),
    get lastUsedAt() {
      return lastUsedAt;
...
describe("materializeConfiguredMcpToolsForHarnessRun", () => {
  ...
  it("scopes remembered requester diagnostics without hiding live operator diagnostics", async () => {
    mocks.setResolveImpl(async (params) =>
      makeRuntime({
        sessionId: params.sessionId,
        requesterSenderId: params.requesterSenderId ?? undefined,
    ...
    const result = await materializeConfiguredMcpToolsForHarnessRun({
      sessionId: "session-diagnostic-scope",
      workspaceDir: "/workspace",
      requesterSenderId: "alice",
    });

    expect(mocks.rememberAdvertisedScopedMcpCatalog).toHaveBeenCalledWith(
      "session-diagnostic-scope",
      expect.objectContaining({
        diagnostics: [expect.objectContaining({ serverName: "user-mail" })],
      }),
    );
    expect(result?.diagnostics?.map((diagnostic) => diagnostic.serverName).toSorted()).toEqual([
      "shared-search",
      "user-mail",
    ]);
    await result?.dispose();
