...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("afora-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive Afora channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", async () => {
    await expect(
      detectUnsafeExecControlShellCommand("afora channels login --channel whatsapp"),
    ).resolves.toBe("channel-login");
    expect(
      await detectUnsafeExecControlShellCommand(
        "pnpm exec afora channels login --channel whatsapp --verbose",
      ),
    ).toBe("channel-login");
    await expect(
      detectUnsafeExecControlShellCommand("afora channels status --deep"),
    ).resolves.toBeNull();
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-afora-channel-login", {
        command: "afora channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive Afora channel login commands/);
    await expect(
      tool.execute("call-wrapped-afora-channel-login", {
        command: "sudo -u afora bash -lc 'afora channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Afora channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'afora channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Afora channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env afora channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Afora channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'afora channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Afora channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it.each([
    ...
    await withTempDir("afora-exec-approval-binding-", async (tmp) => {
  ...
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("afora-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("afora-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("afora-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("afora-exec-preflight-win-", async (tmp) => {
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__afora_missing_redos_guard__)\nprintf '%s' '${htmlBlock}' >/dev/null`;
