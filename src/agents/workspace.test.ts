...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses AFORA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      AFORA_HOME: "/srv/afora-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/afora-home"), ".afora", "workspace"));
  });
  ...
  it("roots named profile workspaces inside the profile state directory", () => {
    ...
    expect(dir).toBe(path.join(path.resolve("/srv/afora-home"), ".afora-work", "workspace"));
  });

  it("rejects invalid environment-only profile names", () => {
    expect(() =>
      resolveDefaultAgentWorkspaceDir({
        AFORA_PROFILE: "../escape",
        HOME: "/home/peter",
      } as NodeJS.ProcessEnv),
    ).toThrow('Invalid profile name: "../escape"');
  });

  it("prefers AFORA_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
    ...
    expect(dir).toBe(path.resolve("/srv/afora-workspace"));
  });
});
...
describe("ensureAgentWorkspace", () => {
  ...
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("afora-workspace-");
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not afora state\n";
  ...
  it("requires Doctor before using legacy JSON setup state", async () => {
    ...
    ).rejects.toThrow(/run afora doctor --fix/u);
    await expect(
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".afora"), { recursive: true });
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-workspace-hardlink-"));
    try {
