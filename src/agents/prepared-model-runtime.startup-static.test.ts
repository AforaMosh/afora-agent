...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    planAforaModelsJsonSource: vi.fn(
...
vi.mock("./models-config.js", () => ({
  ensureAforaModelsJson: mocks.ensureAforaModelsJson,
  planAforaModelsJsonSource: mocks.planAforaModelsJsonSource,
}));
...
describe("prepared model runtime Gateway catalog mode", () => {
  it("imports and materializes only configured and auth-candidate providers", async () => {
    ...
    expect(mocks.planAforaModelsJsonSource).toHaveBeenCalledWith(
      config,
  ...
  it("uses live provider catalogs for an explicit read-only list scope", async () => {
    ...
    expect(mocks.planAforaModelsJsonSource).not.toHaveBeenCalledWith(
      expect.anything(),
      expect.anything(),
  ...
  it("publishes exact dynamic configured models without building a live catalog", async () => {
    ...
    expect(mocks.buildPreparedModelCatalogSnapshot).not.toHaveBeenCalled();
    expect(mocks.loadStaticCatalog).not.toHaveBeenCalled();
    expect(mocks.planAforaModelsJsonSource).not.toHaveBeenCalled();
    expect(mocks.ensureAforaModelsJson).not.toHaveBeenCalled();
  });
