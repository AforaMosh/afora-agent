...
describe("project memory bootstrap", () => {
  ...
  const entries = [
    {
      ...
      score: 0.8,
      snippet:
        "Use the release helper. <!-- trigger: release helper --> <!-- importance: 8 --> <!-- project: github.com/AforaMosh/afora-agent -->",
  ...
  async function prepareEntries(
    ...
    activeProjectKeys: string[] = ["github.com/AforaMosh/afora-agent"],
  ): Promise<string[]> {
  ...
  it("includes entries from every project retained in the session active set", async () => {
    const rendered = (
      await prepareEntries(entries, ["github.com/example/other", "github.com/AforaMosh/afora-agent"])
    ).join("\n");
    expect(rendered).toContain("Use the release helper.");
  ...
  it("uses the dedicated curated listing instead of a daily-note-crowded search", async () => {
    ...
    const rendered = (
      await prepareProjectMemoryBootstrap({
        cfg: {},
        agentId: "main",
        activeProjectKeys: ["github.com/AforaMosh/afora-agent"],
      })
    ).join("\n");
    expect(rendered).toContain("Use the release helper.");
  ...
  it("builds scoped write guidance without capturing global memory", () => {
    const instruction = buildProjectMemoryWriteInstruction("github.com/AforaMosh/afora-agent");
    expect(instruction).toContain("<!-- project: github.com/AforaMosh/afora-agent -->");
