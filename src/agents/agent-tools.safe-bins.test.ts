...
let createAforaCodingTools: typeof import("./agent-tools.js").createAforaCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      AFORA_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "afora-test-no-bundled-extensions"),
    },
    async () => {
      ({ createAforaCodingTools } = await import("./agent-tools.js"));
    },
  );
});
...
vi.mock("afora-agent/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
describe("createAforaCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "afora-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "afora-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "afora-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "afora-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "afora-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "afora-safe-bins-grep-",
        safeBins: ["grep"],
