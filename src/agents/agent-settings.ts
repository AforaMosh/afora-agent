...
export function applyAgentCompactionSettingsFromConfig(params: {
  ...
  // Omission preserves embedded/project settings. Afora config reloads create a new
...
export function resolveEffectiveCompactionMode(cfg?: AforaConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers Afora runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (afora#75799).
 *
...
/**
 * Disable Afora runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when Afora or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (afora#75799).
 * Default-mode runs against ordinary providers keep Afora runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores Afora runtime's
