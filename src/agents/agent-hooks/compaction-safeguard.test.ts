...
import type { ExtensionAPI, ExtensionContext } from "afora-agent/plugin-sdk/agent-sessions";
import { createAssistantMessageEventStream, type Model } from "afora-agent/plugin-sdk/llm";
...
describe("compaction-safeguard recent-turn preservation", () => {
  ...
  it("passes compaction instructions to providers and preserves suffix context", async () => {
    ...
    expect(JSON.stringify(providerMessages)).not.toContain("afora.runtime-context");
...
async function expectWorkspaceSummaryEmptyForAgentsAlias(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-compaction-summary-"));
...
describe("readWorkspaceContextForSummary", () => {
  ...
  it("reads workspace context from the configured workspace instead of process cwd", async () => {
    const processRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-compaction-cwd-"));
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-compaction-workspace-"));
