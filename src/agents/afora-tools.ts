...
import { filterToolsByClientCaps } from "./afora-tools.client-caps.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./afora-tools.media-factory-plan.js";
import { createMediaGenerationAsyncStartCallback } from "./afora-tools.media-yield.js";
import type { ModelAwareToolContext } from "./afora-tools.model-context.js";
import { applyNodesToolWorkspaceGuard } from "./afora-tools.nodes-workspace-guard.js";
import {
...
import { createRequesterYieldCallback } from "./afora-tools.requester-yield.js";
import { createAforaSwarmToolGroups } from "./afora-tools.swarm.js";
import { resolveTranscriptsTool } from "./afora-tools.transcripts.js";
...
import { createAforaDelegateToolsForRun } from "./tools/afora-delegate-tool.js";
...
export { filterToolsByClientCaps } from "./afora-tools.client-caps.js";
export function createAforaTools(
  options?: {
  ...
  const swarmToolGroups = createAforaSwarmToolGroups({
    config: resolvedConfig,
  ...
  options?.recordToolPrepStage?.("afora-tools:session-workspace");
  ...
  const imageTool =
    ...
    resolveImageToolFactoryAvailable({
      ...
      : null;
  options?.recordToolPrepStage?.("afora-tools:image-tool");
  ...
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("afora-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("afora-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("afora-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("afora-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("afora-tools:nodes-tool");
  ...
  const progressCardTool = shouldIncludeProgressCardToolForAforaTools({
    config: resolvedConfig,
    ...
    : null;
  const includeAskUserTool = shouldIncludeAskUserToolForAforaTools({
    config: resolvedConfig,
    agentSessionKey: options?.runSessionKey ?? options?.agentSessionKey,
    pluginToolDenylist: options?.pluginToolDenylist,
  });
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentAforaTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentAforaTools([transcriptsTool]),
    ...collectPresentAforaTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool(),
          ...createAforaDelegateToolsForRun({ ...options, sessionAgentId }),
        ]),
    ...
    ...collectPresentAforaTools([progressCardTool]),
    ...
    ...collectPresentAforaTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("afora-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("afora-tools:plugin-tools");
  }

  allTools = filterToolsByClientCaps(allTools, options?.clientCaps);
  options?.recordToolPrepStage?.("afora-tools:client-capabilities");
  ...
  options?.recordToolPrepStage?.("afora-tools:tool-hooks");
  return allTools
    .map((tool) =>
