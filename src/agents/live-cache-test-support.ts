import { parseStrictInteger } from "@afora/normalization-core/number-coercion";
/**
...
export const LIVE_CACHE_TEST_ENABLED =
  isLiveTestEnabled() && isTruthyEnvValue(process.env.AFORA_LIVE_CACHE_TEST);
...
export async function withLiveCacheHeartbeat<T>(
  ...
  const heartbeatMs = Math.max(
    1_000,
    toInt(process.env.AFORA_LIVE_HEARTBEAT_MS, DEFAULT_HEARTBEAT_MS),
  );
...
export async function completeSimpleWithLiveTimeout<TApi extends Api>(
  ...
  timeoutMs = Math.max(
    1_000,
    toInt(process.env.AFORA_LIVE_MODEL_TIMEOUT_MS, DEFAULT_TIMEOUT_MS),
  ),
...
/** Compute cache-hit ratio from Afora usage counters. */
