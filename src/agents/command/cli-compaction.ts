...
import { AFORA_AGENT_RUNTIME_ID, normalizeOptionalAgentRuntimeId } from "../agent-runtime-id.js";
import {
...
type CliCompactionDeps = {
  ...
  resolveContextEngine: (cfg: AforaConfig) => Promise<ContextEngine>;
...
function isNativeHarnessCompactionSession(
  ...
  if (!harnessId || normalizeOptionalAgentRuntimeId(harnessId) === AFORA_AGENT_RUNTIME_ID) {
    return false;
  }
...
export async function runCliTurnCompactionLifecycle(params: {
  ...
  if (
    params.sessionEntry?.modelSelectionLocked === true &&
    lockedHarnessRuntime !== AFORA_AGENT_RUNTIME_ID &&
