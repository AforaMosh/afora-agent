...
const TEST_ENV_VAR = "AFORA_MODELS_CONFIG_TEST_ENV";
...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/afora-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): AforaConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: AforaConfig) {
...
let unauthenticatedProviderWritePlan: Awaited<ReturnType<typeof planAforaModelsJsonWithDeps>>;
...
async function planGoogleVertexProfileCatalog() {
  const agentDir = "/tmp/afora-google-vertex-models-profile";
  try {
...
beforeAll(async () => {
  ...
  unauthenticatedProviderWritePlan = await planAforaModelsJsonWithDeps(
    {
...
describe("models-config", () => {
  it("keeps the implicit provider catalog when explicit baseUrl is blank", async () => {
    let observedConfig: AforaConfig | undefined;
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/afora-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planAforaModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/afora-models-config-env-vars-test",
        env: {},
  ...
  it.each([
    ...
    async ({ pluginIds, expectedRegistry }) => {
      ...
      await planAforaModelsJsonWithDeps(
        {
          cfg: { models: { providers: {} } },
          agentDir: "/tmp/afora-models-config-policy-registry-test",
          env: {},
          existingRaw: "",
          existingParsed: null,
          pluginMetadataSnapshot,
    ...
    const plan = await planAforaModelsJsonWithDeps(
      {
        ...
        agentDir: "/tmp/afora-google-vertex-adc-models",
        env: {},
        existingRaw: "",
        existingParsed: null,
      },
