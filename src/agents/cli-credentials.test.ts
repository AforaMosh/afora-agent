...
describe("cli credentials", () => {
  ...
  it("keeps external CLI credential files anchored to the OS home", () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-os-home-"));
    const aforaHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-effective-home-"));
    ...
    try {
      ...
      const decoys = [
        {
          filePath: path.join(aforaHome, ".claude", ".credentials.json"),
          value: {
            claudeAiOauth: {
        ...
        {
          filePath: path.join(aforaHome, ".codex", "auth.json"),
          value: {
            tokens: {
              access_token: createJwtWithExp(codexExpiry),
        ...
        {
          filePath: path.join(aforaHome, ".minimax", "oauth_creds.json"),
          value: {
        ...
        {
          filePath: path.join(aforaHome, ".gemini", "oauth_creds.json"),
          value: {
      ...
      fs.rmSync(aforaHome, { recursive: true, force: true });
    }
  });
  ...
  it("does not let no-keychain Claude cache misses poison keychain reads", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-claude-cache-"));
    vi.setSystemTime(new Date("2025-01-01T00:00:00Z"));
  ...
  it("attaches the CLI config account email to Claude credentials", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-claude-email-"));
    const expires = Date.parse("2036-04-25T12:00:00Z");
  ...
  it("recognizes Claude Code user apiKeyHelper settings as CLI-managed auth", () => {
    const tempDir = tempDirs.make("afora-claude-settings-");
  ...
  it("prefers Claude Code user apiKeyHelper settings over stored Claude credentials", () => {
    const tempDir = tempDirs.make("afora-claude-helper-first-");
    const settingsDir = path.join(tempDir, ".claude");
    fs.mkdirSync(settingsDir, { recursive: true });
    fs.writeFileSync(
      path.join(settingsDir, "settings.json"),
  ...
  it("reads Codex credentials from keychain when available", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-"));
  ...
  it("does not read stale Codex tokens when auth.json resolves to API-key mode", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-api-key-mode-"));
    process.env.CODEX_HOME = tempHome;
    const expSeconds = Math.floor(Date.parse("2026-03-24T12:34:56Z") / 1000);
    execSyncMock.mockImplementation(() => {
      throw new Error("not found");
  ...
  it("reads API-key auth from the active Codex Keychain store", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-keychain-api-key-"));
    execSyncMock.mockImplementation((command: unknown) =>
  ...
  it("prefers active Codex OAuth over a stale file API key", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-keychain-oauth-"));
    fs.writeFileSync(
  ...
  it("uses the API key that Codex reports active instead of a stale Keychain record", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-default-file-"));
    fs.writeFileSync(
      path.join(tempHome, "auth.json"),
  ...
  it("accepts legacy Codex API-key status only with one readable candidate", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-legacy-status-"));
    fs.writeFileSync(
      path.join(tempHome, "auth.json"),
  ...
  it("treats an empty Codex auth.json API-key field as API-key mode", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-empty-api-key-mode-"));
    process.env.CODEX_HOME = tempHome;
    const expSeconds = Math.floor(Date.parse("2026-03-24T12:34:56Z") / 1000);
    execSyncMock.mockImplementation(() => {
      throw new Error("not found");
  ...
  it("rejects Codex auth.json fallback expiry when stat and process clock are invalid", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-invalid-clock-"));
    process.env.CODEX_HOME = tempHome;
    const authPath = path.join(tempHome, "auth.json");
    fs.mkdirSync(tempHome, { recursive: true, mode: 0o700 });
    fs.writeFileSync(
  ...
  it("uses Codex auth.json fallback expiry when file mtime has fractional milliseconds", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-fractional-mtime-"));
    process.env.CODEX_HOME = tempHome;
    const authPath = path.join(tempHome, "auth.json");
    fs.mkdirSync(tempHome, { recursive: true, mode: 0o700 });
    fs.writeFileSync(
  ...
  it("does not read Codex keychain when keychain prompts are disabled", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-no-prompt-"));
    process.env.CODEX_HOME = tempHome;
    const expSeconds = Math.floor(Date.parse("2026-03-24T12:34:56Z") / 1000);
    const authPath = path.join(tempHome, "auth.json");
    fs.mkdirSync(tempHome, { recursive: true, mode: 0o700 });
  ...
  it("does not let no-keychain Codex cache misses poison keychain reads", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-codex-cache-"));
    process.env.CODEX_HOME = tempHome;
    const expSeconds = Math.floor(Date.parse("2026-03-24T12:34:56Z") / 1000);
  ...
  it("lifts Google account identity from the Gemini id_token", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-gemini-"));
    try {
  ...
  it("reads Gemini credentials without identity fields when id_token is absent", () => {
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "afora-gemini-noid-"));
    try {
      const credPath = path.join(tempHome, ".gemini", "oauth_creds.json");
      fs.mkdirSync(path.dirname(credPath), { recursive: true, mode: 0o700 });
      fs.writeFileSync(
