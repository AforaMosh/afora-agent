/** Tests merging bundled MCP defaults with Afora user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets Afora mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps Afora transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled Afora MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled Afora MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
