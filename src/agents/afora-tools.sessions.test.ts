...
import "./test-helpers/fast-afora-tools-sessions.js";
...
function createAforaTools(options?: {
...
function getSessionTool(
  ...
  options?: Parameters<typeof createAforaTools>[0],
) {
  const tool = createAforaTools(options).find((candidate) => candidate.name === name);
  if (!tool) {
...
describe("sessions tools", () => {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "afora", model: "gateway-injected" }),
    );
    expect(details.messages).toContainEqual(
  ...
  it("keeps scoped sends from creating post-return work or durable watches", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-scoped-session-send-"));
    const storePath = path.join(tmpDir, "sessions.json");
  ...
  it("sessions_send never reroutes an exact-incarnation grant to a Cron parent", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-exact-cron-send-"));
    const storePath = path.join(tmpDir, "sessions.json");
    const requesterKey = "agent:main:main";
