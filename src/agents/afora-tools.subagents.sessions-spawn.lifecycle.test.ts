...
} from "./afora-tools.subagents.sessions-spawn.test-harness.js";
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.AFORA_TEST_FAST;
...
describe("afora-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(async () => {
  ...
  afterAll(() => {
    ...
    process.env.AFORA_TEST_FAST = fastModeEnv.previous;
  });
  ...
  it("sessions_spawn retires bundle MCP runtime when run-mode cleanup completes", async () => {
    ...
    await getOrCreateSessionMcpRuntime({
      ...
      workspaceDir: "/tmp/afora-subagent-mcp-retire",
