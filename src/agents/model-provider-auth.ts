...
const configFingerprintCache = new WeakMap<AforaConfig, string>();
...
function resolveProviderAuthConfigFingerprint(cfg: AforaConfig | undefined): string | null {
  if (!cfg) {
    return null;
  }
...
export function createProviderAuthChecker(params: {
  ...
  const evaluateModelAuth = (
    ...
    const cacheKey = hasRouteFacts
      ? `${key}\0${hashRuntimeConfigValue(ref as unknown as AforaConfig)}`
      : key;
...
function collectProviderAuthWarmRuntimeAuthLookups(cfg: AforaConfig): {
