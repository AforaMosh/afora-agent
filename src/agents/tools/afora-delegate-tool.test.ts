...
import { createAforaDelegateToolsForRun } from "./afora-delegate-tool.js";
...
describe("afora delegation tool", () => {
  it("relays context and surfaces pending approval", async () => {
    ...
    const tool = createAforaDelegateToolsForRun({
      sessionAgentId: "main",
      runSessionKey: "agent:main:dm:one",
      agentChannel: "webchat",
    })[0];
    if (!tool) {
      throw new Error("expected Afora delegation tool");
    }
    ...
    expect(callGateway).toHaveBeenCalledWith("afora.chat", {
  ...
  it("uses the owner-only core gate", () => {
    expect(GATEWAY_OWNER_ONLY_CORE_TOOLS).toContain("afora");
  });
});
