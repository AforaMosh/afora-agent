...
const TEST_AGENT_DIR = "/tmp/afora-pdf-model-config";
...
vi.mock("./model-config.helpers.js", () => ({
  ...
  hasProviderAuthForTool: ({ provider, cfg }: { provider: string; cfg?: AforaConfig }) => {
  ...
  resolveDefaultModelRef: (cfg?: AforaConfig) => {
