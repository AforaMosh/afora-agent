...
describe("web_search shared cache", () => {
  it("keeps cache entries module-local instead of exposing them on a global symbol", async () => {
    ...
    vi.resetModules();
    delete (globalThis as Record<PropertyKey, unknown>)[Symbol.for("afora.web-search.cache")];
    ...
    expect(
      (globalThis as Record<PropertyKey, unknown>)[Symbol.for("afora.web-search.cache")],
    ).toBeUndefined();
  });
});
