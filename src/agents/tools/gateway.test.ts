...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    afora: process.env.AFORA_GATEWAY_TOKEN,
    gatewayUrl: process.env.AFORA_GATEWAY_URL,
  };
  ...
  afterAll(() => {
    releaseTestDelegatedAuthorities();
    if (envSnapshot.afora === undefined) {
      delete process.env.AFORA_GATEWAY_TOKEN;
    } else {
      process.env.AFORA_GATEWAY_TOKEN = envSnapshot.afora;
    }
    if (envSnapshot.gatewayUrl === undefined) {
      delete process.env.AFORA_GATEWAY_URL;
    } else {
      process.env.AFORA_GATEWAY_URL = envSnapshot.gatewayUrl;
    }
  });
  ...
  it("uses AFORA_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    ...
    process.env.AFORA_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
  ...
  it("explains stale gateway cron connection metadata rejections", async () => {
    ...
    await expect(
      ...
      "The running Gateway is from an older Afora build and rejected current agent runtime connection metadata. Restart the Gateway with `afora gateway restart`, then retry.",
    );
  ...
  it("does not send the local approval runtime token to env-selected gateways", async () => {
    process.env.AFORA_GATEWAY_URL = "wss://gateway.example";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
  ...
  it("does not send the local approval runtime token to loopback env-selected gateways", async () => {
    process.env.AFORA_GATEWAY_URL = "ws://127.0.0.1:18789";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
  ...
  it("does not send the local approval runtime token to loopback env-selected gateway paths", async () => {
    process.env.AFORA_GATEWAY_URL = "ws://127.0.0.1:18789/ws";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
