...
function withPdfModel(primary: string): AforaConfig {
  return {
    agents: { defaults: { pdfModel: { primary } } },
  } as AforaConfig;
}

function withDefaultModel(primary: string): AforaConfig {
  return {
...
async function withManagedInboundPdf(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-pdf-managed-inbound-"));
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-pdf-out-"));
      try {
        const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
        const tool = requirePdfTool(
  ...
  it.each(["file:///workspace/doc.pdf", "FILE:/workspace/doc.pdf"])(
    ...
    async (pdf) => {
      await withTempPdfAgentDir(async (agentDir) => {
        const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-pdf-sandbox-"));
        try {
