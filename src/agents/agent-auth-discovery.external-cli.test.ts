...
describe("resolveAgentDiscoveryAuthFacts external CLI scoping", () => {
  ...
  it("threads scoped external CLI discovery into writable auth store loading", () => {
    ...
    resolveAgentDiscoveryAuthFacts("/tmp/afora-agent", {
      config: cfg,
      env: {},
      externalCli,
    });

    expect(storeMocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/afora-agent", {
  ...
  it("merges prepared ambient credentials without repeating ambient discovery", () => {
    ...
    const { credentials } = resolveAgentDiscoveryAuthFacts("/tmp/afora-agent", {
      ambientCredentials: {
