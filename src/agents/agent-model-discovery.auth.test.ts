...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "afora-agent-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("includes env-backed provider auth when no auth profile exists", () => {
    ...
    const previousDisableBundledPlugins = process.env.AFORA_DISABLE_BUNDLED_PLUGINS;
    ...
    try {
      ...
      if (previousDisableBundledPlugins === undefined) {
        ...
        process.env.AFORA_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
      }
    }
  });
