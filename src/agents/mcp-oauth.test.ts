...
import { withTempHome as withBaseTempHome } from "afora-agent/plugin-sdk/test-env";
...
describe("MCP OAuth provider", () => {
  ...
  it("reuses a valid stored session without persisting an authorization redirect", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-existing-session-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  });
    ...
    await withTempHome(
      async () => {
        ...
        await expect(
          ...
          'MCP server "Remote Docs" requires additional OAuth authorization. Run afora mcp login Remote Docs.',
        );
      ...
      {
        prefix: "afora-mcp-oauth-insufficient-scope-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("stops refreshing after a replacement token is rejected twice", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-terminal-rejection-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("keeps a rejected-token challenge for explicit reauthorization after refresh fails", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-rejected-token-challenge-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("uses a persisted challenge when refreshing after Doctor credential import", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-doctor-challenge-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("uses unknown-expiry tokens live but refreshes them before blind projection", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-legacy-token-",
        skipSessionCleanup: true,
        env: {
  ...
  it("requires explicit login when no native OAuth credentials exist", async () => {
    await withTempHome(
      async () => {
        ...
        ).rejects.toThrow("Run afora mcp login Remote Docs.");
        expect(authMock).not.toHaveBeenCalled();
      },
      {
        prefix: "afora-mcp-oauth-missing-token-",
        skipSessionCleanup: true,
        env: {
          AFORA_CONFIG_PATH: undefined,
          AFORA_STATE_DIR: undefined,
  ...
  it("marks challenge-only bootstrap state as safe for Doctor credential import", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-challenge-provenance-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("persists challenge hints without mutating an in-flight PKCE login", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-challenge-bootstrap-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("stores token state only in shared SQLite with restricted permissions", async () => {
    await withTempHome(
      async (home) => {
        ...
        const rows = openAforaStateDatabase()
        ...
        await expect(fs.readdir(`${home}/.afora/mcp-oauth`)).rejects.toMatchObject({
          code: "ENOENT",
        });
  ...
  it("updates provider fields atomically and clears token expiry on invalidation", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-atomic-fields-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("fails closed when canonical SQLite JSON is malformed", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-corrupt-row-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("fails closed when canonical token expiry has no token state", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-orphan-expiry-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("isolates, counts, and clears requester credentials by configured server", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-requesters-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("persists the localhost retry for completion and then clears the session", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-localhost-persist-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("does not retry a code exchange redirect mismatch", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-code-mismatch-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
  ...
  it("does not persist localhost when the fallback attempt fails", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-localhost-failure-",
        skipSessionCleanup: true,
        env: {
          AFORA_CONFIG_PATH: undefined,
          AFORA_STATE_DIR: undefined,
  ...
  it("does not start hidden authorization flows without an authorization callback", async () => {
    await withTempHome(
      async () => {
        ...
        expect(() => provider.state?.()).toThrow("Run afora mcp login Remote Docs.");
        expect(() => provider.saveCodeVerifier?.("verifier")).toThrow(
          "Run afora mcp login Remote Docs.",
        );
        await expect(
      ...
      {
        prefix: "afora-mcp-oauth-noninteractive-",
        skipSessionCleanup: true,
        env: {
          AFORA_CONFIG_PATH: undefined,
          AFORA_STATE_DIR: undefined,
  ...
  it("resumes authorization after restart, retains failures, and supersedes older starts", async () => {
    await withTempHome(
      ...
      {
        prefix: "afora-mcp-oauth-session-",
        skipSessionCleanup: true,
        env: { AFORA_CONFIG_PATH: undefined, AFORA_STATE_DIR: undefined },
      },
    );
