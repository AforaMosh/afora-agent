...
describe.skipIf(process.platform === "win32")("shell snapshot subprocesses", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "AFORA_EXEC_SHELL_SNAPSHOT", "AFORA_STATE_DIR"]);
    spawnMock.mockReset();
  ...
  it("does not create output pipes for status-only shell commands", async () => {
    ...
    const home = tempDirs.make("afora-snapshot-spawn-home-");
    const stateDir = tempDirs.make("afora-snapshot-spawn-state-");
