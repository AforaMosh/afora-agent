...
export const LEGACY_WORKSPACE_STATE_DIRNAME = ".afora";
const LEGACY_WORKSPACE_STATE_FILENAME = "workspace-state.json";
export const LEGACY_WORKSPACE_STATE_CURRENT_FILENAME = "afora-workspace-state.json";
...
export const LEGACY_WORKSPACE_ATTESTATION_HEADER = "afora-workspace-attestation:v1";
...
export function assertNoUnmigratedWorkspaceState(params: { workspaceDir: string }): void {
  ...
  if (hasLegacy) {
    throw new Error(
      `Legacy workspace setup state requires migration for ${identity.workspacePath}; run afora doctor --fix.`,
    );
  }
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("afora.workspaceLegacyStateTestApi")] =
...
export function prepareLegacyWorkspaceStateReset(
  ...
  const candidates = [
    ...
    ...sources.stateDirAttestationPaths.map((sourcePath) => ({
      ...
      sourcePath,
      // Hashed paths inside Afora-owned attestation directories are
    ...
    candidate,
    {
      ...
      // Sibling claims remain outside Afora-owned roots. Renaming a claimed
