...
import { resolveAforaPluginToolInputs } from "./afora-tools.plugin-context.js";
import { applyPluginToolDeliveryDefaults } from "./plugin-tool-delivery-defaults.js";
import type { AnyAgentTool } from "./tools/common.js";

describe("afora plugin tool context", () => {
  it("forwards trusted requester sender identity", () => {
    const result = resolveAforaPluginToolInputs({
      options: {
        config: {} as never,
  ...
  it("defaults missing and unknown conversation-read origins to delegated", () => {
    const missing = resolveAforaPluginToolInputs({
      options: { config: {} as never },
    });
    const unknown = resolveAforaPluginToolInputs({
      options: {
        config: {} as never,
  ...
  it("forwards host-prepared active project keys", () => {
    const activeProjectKeys = ["github.com/AforaMosh/afora-agent"];
    const result = resolveAforaPluginToolInputs({
