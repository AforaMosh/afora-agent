...
describe("provider attribution", () => {
  ...
  it("resolves the canonical Afora product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      AFORA_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Afora runtime wrapper.",
      product: "Afora",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a documented Google Gemini attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      version: "2026.3.22",
      headers: {
        "x-goog-api-client": "afora-agent/2026.3.22",
      },
    });
  });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { AFORA_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(
      resolveProviderAttributionPolicy("openai", { AFORA_VERSION: "2026.3.22" })?.headers,
    ).toEqual({
      originator: "afora",
      version: "2026.3.22",
      "User-Agent": "afora-agent/2026.3.22",
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { AFORA_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
      reviewNote:
        "xAI api.x.ai accepts a standard afora User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
      product: "Afora",
      version: "2026.3.22",
      headers: {
        originator: "afora",
    ...
    expect(
      resolveProviderAttributionPolicy("xai", { AFORA_VERSION: "2026.3.22" })?.headers,
    ).toEqual({
      originator: "afora",
      version: "2026.3.22",
      "User-Agent": "afora-agent/2026.3.22",
  ...
  it("lists the current attribution support matrix", () => {
    ...
    expect(
      listProviderAttributionPolicies({ AFORA_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
