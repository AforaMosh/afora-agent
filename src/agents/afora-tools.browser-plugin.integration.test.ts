// Verifies Afora plugin tools are resolved with browser/runtime context.
...
const TEST_AGENT_DIR = path.join(os.tmpdir(), "afora-plugin-tool-auth-test");
...
function firstResolvePluginToolsParams(): Record<string, unknown> {
  // Captures the plugin runtime contract passed from Afora tool resolution.
...
describe("createAforaTools browser plugin integration", () => {
  afterEach(() => {
  ...
  it("keeps the browser tool returned by plugin resolution", () => {
    ...
    const tools = resolveAforaPluginToolsForOptions({
      options: { config },
  ...
  it("forwards the lifecycle registry to workspace-scoped plugin tools", () => {
    hoisted.resolvePluginTools.mockReturnValue([]);
    const config = { plugins: { enabled: true } } as AforaConfig;
  ...
  it("keeps provider availability and credential resolution aligned for env-only auth", async () => {
    const envName = "AFORA_PLUGIN_TOOL_AUTH_TEST_KEY";
  ...
  it("preserves ungated plugin resolution when no authoritative auth store is supplied", () => {
    hoisted.resolvePluginTools.mockReturnValue([]);
    const config = { plugins: { allow: ["browser"] } } as AforaConfig;
  ...
  it("does not pass a stale active snapshot as plugin runtime config for a resolved run config", () => {
    ...
    let capturedRuntimeConfig: AforaConfig | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      capturedRuntimeConfig = (params as { context?: { runtimeConfig?: AforaConfig } }).context
        ?.runtimeConfig;
      return [];
    });
  ...
  it("does not let a source-less pinned config snapshot override explicit plugin tool config", () => {
    ...
    let capturedRuntimeConfig: AforaConfig | undefined;
    let getRuntimeConfig: (() => AforaConfig | undefined) | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      const context = (
        params as {
          context?: {
  ...
  it("exposes a live runtime config getter to plugin tool factories", () => {
    ...
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      getRuntimeConfig = (
        params as { context?: { getRuntimeConfig?: () => AforaConfig | undefined } }
...
function requirePluginTool(name: string, overrides?: Parameters<typeof createAforaTools>[0]) {
  hoisted.resolvePluginTools.mockReturnValue([
    {
      name,
...
describe("createAforaTools Gateway caller identity", () => {
  afterEach(() => {
