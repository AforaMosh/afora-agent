...
type CreateAforaToolsOptions = Parameters<
  typeof import("./afora-tools.js").createAforaTools
>[0];
let createAforaToolsForTestModule: typeof import("./afora-tools.js").createAforaTools;
let legacyComfyToolNames: string[];

async function createAforaToolsForTest(options?: CreateAforaToolsOptions) {
  return createAforaToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/afora.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function createExplicitMediaModelConfig(): AforaConfig {
  return {
    agents: {
      defaults: {
        mediaModels: {
...
function legacyModelProviderConfig(provider: Record<string, unknown>): AforaConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createAforaTools: createAforaToolsForTestModule } =
      await import("./afora-tools.js"));

    const config = legacyModelProviderConfig({
      workflow: { "1": { inputs: {} } },
      promptNodeId: "1",
    });
    vi.stubEnv("AFORA_BUNDLED_PLUGINS_DIR", path.join(process.cwd(), "extensions"));
    legacyComfyToolNames = (
      await createAforaToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: AforaConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createAforaToolsForTest({
      config,
      agentDir: "/tmp/afora-agent-main",
