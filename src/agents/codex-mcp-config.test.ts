// Covers conversion from Afora bundle-MCP config into Codex app-server
...
describe("buildCodexMcpServersConfig", () => {
  it("normalizes Afora MCP servers into Codex app-server mcp_servers shape", () => {
    ...
    expect(
      ...
      afora: {
        ...
        bearer_token_env_var: "AFORA_MCP_TOKEN",
        http_headers: {
          "x-static": "static-value",
        },
        env_http_headers: {
...
describe("loadCodexBundleMcpThreadConfigCore", () => {
  ...
  it("prepares Agent Plugins data dirs before projecting Codex thread config", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-codex-agent-mcp-"));
