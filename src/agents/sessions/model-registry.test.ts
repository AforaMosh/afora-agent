...
import { getApiProvider } from "@afora/ai/internal/runtime";
...
function writeModelsJson(contents: unknown): string {
  const dir = mkdtempSync(join(tmpdir(), "afora-model-registry-"));
  tempDirs.push(dir);
...
describe("ModelRegistry models.json auth", () => {
  ...
  it("keeps authored provider models available when the plugin catalog database is corrupt", () => {
    ...
    writeFileSync(join(dirname(modelsPath), "afora-agent.sqlite"), "not a SQLite database");
