...
describe("SettingsManager scoped persistence", () => {
  it("preserves external sibling changes while writing global and project scopes", async () => {
    ...
    storage.set("global", {
      ...
      packages: ["npm:@afora/global"],
    });
    storage.set("project", {
      packages: ["npm:@afora/project"],
      skills: ["old-skill"],
    });
    ...
    expect(settingsManager.getPackages()).toEqual(["npm:@afora/external"]);
...
describe("SettingsManager runtime overrides", () => {
  ...
  it("preserves runtime overrides after project setting writes", async () => {
    ...
    settingsManager.setProjectPackages(["npm:@afora/example"]);

    expect(settingsManager.getPackages()).toEqual(["npm:@afora/example"]);
  ...
  it("recursively merges provider retry overrides and replaces arrays", () => {
    const settingsManager = SettingsManager.inMemory({
      ...
      packages: ["npm:@afora/base"],
    });

    settingsManager.applyOverrides({
      retry: { provider: { maxRetries: 5 } },
      packages: ["npm:@afora/override"],
    });
    ...
    expect(settingsManager.getPackages()).toEqual(["npm:@afora/override"]);
  });
});
