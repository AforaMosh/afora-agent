...
beforeAll(async () => {
  ...
  clearExtensionCache();
  const dir = await mkdtemp(join(tmpdir(), "afora-extension-sdk-"));
  tempDirs.push(dir);
  ...
  it("virtualizes scoped and unscoped SDK module ids", async () => {
    ...
    expect(virtualModulesCase.virtualModuleIds).toEqual(
      expect.arrayContaining([
        ...
        "@afora/plugin-sdk/agent-core",
        "afora-agent/plugin-sdk/llm",
        "@afora/plugin-sdk/llm",
        "afora-agent/plugin-sdk/agent-sessions",
        "@afora/plugin-sdk/agent-sessions",
      ]),
    );
  });
});
