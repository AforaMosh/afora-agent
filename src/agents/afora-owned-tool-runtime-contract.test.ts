// Verifies Afora-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "afora-agent/plugin-sdk/agent-core";
import {
...
describe("Afora-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetAforaOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installAforaOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installAforaOwnedToolHooks();
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/afora-reply.png"]);
    expect(
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installAforaOwnedToolHooks({ blockReason: "blocked by policy" });
