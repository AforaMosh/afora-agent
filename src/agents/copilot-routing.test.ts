...
function withDefaultRoster(config: AforaConfig = {}): AforaConfig {
  return {
    ...config,
...
function cfgWithProviderRuntime(id: string): AforaConfig {
...
function cfgWithModelRuntime(modelId: string, id: string): AforaConfig {
  return withDefaultRoster({
    models: {
      providers: {
        "github-copilot": {
