...
describe("isSilentOverflowProneModel", () => {
  ...
  // afora#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable Afora runtime's secondary compaction path;
  // Afora's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of afora#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering Afora runtime's
  ...
  // state.messages intact; Afora's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep Afora runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove Afora runtime's
