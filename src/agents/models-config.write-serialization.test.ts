...
function readRawCatalogCacheRow(
  ...
  const database = new DatabaseSync(path.join(agentDir, "afora-agent.sqlite"), {
    readOnly: true,
  });
  try {
    const row = database
...
const planAforaModelsJsonMock = vi.fn();
...
let planAforaModelsJsonSource: typeof import("./models-config.js").planAforaModelsJsonSource;
...
function planParamsAt(callIndex: number): {
  ...
  // Planner call shape is the contract between ensureAforaModelsJson and planning.
  const call = planAforaModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planAforaModelsJson: (...args: unknown[]) => planAforaModelsJsonMock(...args),
  }));
  ...
  ({ ensureAforaModelsJson, planAforaModelsJsonSource } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("materializes an authoritative plugin catalog replacement without mutating state", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planAforaModelsJsonMock.mockResolvedValue({
        action: "write",
      ...
      const planned = await planAforaModelsJsonSource({}, agentDir, {
        workspaceDir,
  ...
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureAforaModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureAforaModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureAforaModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".afora", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".afora", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("migrates released provider credentials before model planning can regenerate a catalog", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planAforaModelsJsonMock.mockImplementation(async () => {
      ...
      expect(planAforaModelsJsonMock).toHaveBeenCalledOnce();
  ...
  it("refuses to regenerate provider catalogs while released credentials cannot be read", async () => {
    ...
    await withModelsTempHome(async (home) => {
      ...
      try {
        await expect(ensureAforaModelsJson({}, agentDir)).rejects.toThrow(
          "Cannot safely prepare provider models until legacy catalog migration succeeds",
        );
        expect(planAforaModelsJsonMock).not.toHaveBeenCalled();
      } finally {
  ...
  it("never promotes an unmarked cached plugin catalog into model planning", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planAforaModelsJsonMock.mockImplementation(
      ...
      await ensureAforaModelsJson({ models: { providers: {} } }, agentDir, {
  ...
  it("removes stale plugin-owned model catalogs from the agent SQLite cache", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureAforaModelsJson({}, agentDir);
  ...
  it("keeps generated SQLite plugin catalogs on non-authoritative skip plans", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planAforaModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      ...
      expect(planAforaModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureAforaModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureAforaModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planAforaModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planAforaModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureAforaModelsJson(first);
      await ensureAforaModelsJson(second);
      await ensureAforaModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureAforaModelsJson(first),
        ensureAforaModelsJson(second),
      ]);
