...
const TARGET_MODEL_REF =
  process.env.AFORA_LIVE_OPENAI_REASONING_COMPAT_MODEL?.trim() || DEFAULT_TARGET_MODEL_REF;
...
function resolveTargetModelRef(): { provider: string; modelId: string } {
  ...
  if (!provider?.trim() || !modelId) {
    throw new Error(
      `Invalid AFORA_LIVE_OPENAI_REASONING_COMPAT_MODEL: ${JSON.stringify(TARGET_MODEL_REF)}`,
    );
  }
  return {
