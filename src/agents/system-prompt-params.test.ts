...
function buildParams(params: { config?: AforaConfig; workspaceDir?: string; cwd?: string }) {
...
describe("buildSystemPromptParams", () => {
  ...
  it("detects repo root from workspaceDir", async () => {
    const temp = tempDirs.make("afora-workspace-");
  ...
  it("falls back to cwd when workspaceDir has no repo", async () => {
    const temp = tempDirs.make("afora-cwd-");
    const repoRoot = path.join(temp, "repo");
  ...
  it("uses configured repoRoot when valid", async () => {
    const temp = tempDirs.make("afora-config-");
  ...
  it("ignores invalid repoRoot config and auto-detects", async () => {
    ...
    const temp = tempDirs.make("afora-invalid-");
    const repoRoot = path.join(temp, "repo");
  ...
  it("returns undefined when no repo is found", async () => {
    const workspaceDir = tempDirs.make("afora-norepo-");
  ...
  it("does not rediscover the repository after preparation", async () => {
    const workspaceDir = tempDirs.make("afora-prepared-norepo-");
    const repoRoot = tempDirs.make("afora-late-repo-");
