      ...
      return {
        buildReplayPolicy: (context?: {
          ...
          switch (provider) {
            case "env-sensitive":
              return {
                sanitizeToolCallIds: context?.env?.AFORA_TEST_TRANSCRIPT_POLICY === "strict",
                ...(context?.env?.AFORA_TEST_TRANSCRIPT_POLICY === "strict"
...
describe("resolveTranscriptPolicy", () => {
  ...
  it("does not reuse cached replay policies across custom env objects", () => {
    ...
    const strictEnv = {
      ...process.env,
      AFORA_TEST_TRANSCRIPT_POLICY: "strict",
    };
    const looseEnv = {
      ...process.env,
      AFORA_TEST_TRANSCRIPT_POLICY: "loose",
    };
