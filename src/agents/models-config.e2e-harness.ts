...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousAforaAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousAforaAgentDir = process.env.AFORA_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousAforaAgentDir === undefined) {
      delete process.env.AFORA_AGENT_DIR;
    } else {
      process.env.AFORA_AGENT_DIR = previousAforaAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const CUSTOM_PROXY_MODELS_CONFIG: AforaConfig = {
  models: {
    providers: {
      "custom-proxy": {
