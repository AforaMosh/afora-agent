...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-managed-media-"));
  try {
    return await withEnvAsync({ AFORA_STATE_DIR: stateDir }, async () => {
...
async function withOutsideHardlinkInAforaTmp<T>(
  params: {
    aforaTmpDir: string;
  ...
  const hardlinkPath = path.join(params.aforaTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.aforaTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.aforaTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.aforaTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxPath", () => {
  it("keeps home-sibling roots absolute in escape diagnostics", async () => {
    ...
    await withEnvAsync({ HOME: home, AFORA_HOME: undefined }, async () => {
  ...
  it("still shortens roots beneath the home directory", async () => {
    const home = path.join(os.homedir(), "test-home");
    await withEnvAsync({ HOME: home, AFORA_HOME: undefined }, async () => {
      const root = path.join(home, "afora-sandbox");
      const outside = path.dirname(root);

      expect(() => resolveSandboxPath({ filePath: outside, cwd: root, root })).toThrow(
        `Path escapes sandbox root (~${path.sep}afora-sandbox): ${outside}`,
      );
    });
  });
});
...
describe("resolveSandboxedMediaSource", () => {
  const aforaTmpDir = resolvePreferredAforaTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Afora tmp root",
      media: path.join(aforaTmpDir, "image.png"),
      expected: path.join(aforaTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Afora tmp root",
      media: pathToFileURL(path.join(aforaTmpDir, "photo.png")).href,
      expected: path.join(aforaTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Afora tmp root",
      media: path.join(aforaTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(aforaTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(aforaTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside afora tmp root",
      media: path.join(os.tmpdir(), "outside-afora", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Afora tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(aforaTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(aforaTmpDir, { recursive: true });
      const symlinkPath = path.join(aforaTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Afora tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAforaTmp(
      {
        aforaTmpDir,
  ...
  it("rejects symlinked Afora tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInAforaTmp(
