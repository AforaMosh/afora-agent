...
export function buildNativeHookRelayCommandWithStateDatabase(params: {
  ...
  const executable = params.executable ?? resolveAforaCliExecutable();
  const argv =
    executable === "afora"
      ? ["afora"]
...
function resolveAforaCliExecutable(): string {
  const envPath = process.env.AFORA_CLI_PATH?.trim();
  ...
  throw new Error("Cannot resolve Afora CLI executable path for native hook relay");
}
