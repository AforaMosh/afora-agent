...
type NativeHookRelayBridgeRow = AforaStateKyselyDatabase["native_hook_relay_bridges"];
...
export function readNativeHookRelayBridgeRecord(
  ...
  return withAforaStateDatabaseReadOnly(
    (database) =>
...
export function pruneNativeHookRelayBridgeRecords(params: {
  ...
  const database = openAforaStateDatabase({ path: params.stateDbPath });
  const db = getNodeSqliteKysely<NativeHookRelayBridgeDatabase>(database.db);
