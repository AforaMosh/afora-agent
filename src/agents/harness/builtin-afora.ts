/**
 * Built-in Afora harness registration.
 *
 * Harness selection uses this factory to expose the embedded Afora runtime
...
/** Creates the built-in harness backed by the embedded Afora agent runner. */
export function createAforaAgentHarness(): AgentHarnessV2 {
  const harness: AgentHarnessV2 = {
    ...
    runIsolatedCompletionV2: async (params) => {
      if (params.authorization.owner !== "host") {
        throw new Error("The built-in Afora harness requires host-prepared authorization.");
      }
...
export function isBuiltInAforaAgentHarness(harness: AgentHarness): boolean {
  return builtInAforaHarnesses.has(harness);
}
