...
type MiddlewareContentBlock = AforaAgentToolResult["content"][number];
...
function isValidMiddlewareToolResult(value: unknown): value is AforaAgentToolResult {
...
function coerceMiddlewareToolResult(
  value: unknown,
  options: MiddlewareToolResultCoerceOptions = {},
): AforaAgentToolResult | undefined {
  ...
  const content: AforaAgentToolResult["content"] = [];
 ...
 */
function sanitizeToolResultForMiddleware(result: AforaAgentToolResult): AforaAgentToolResult {
...
function buildMiddlewareFailureResult(): AforaAgentToolResult {
  return {
    content: [
      {
        type: "text",
...
function reconcileDeliveredMessagingFailure(
  result: AforaAgentToolResult,
  fallback: AforaAgentToolResult | undefined,
): AforaAgentToolResult {
