// Built-in Afora harness tests cover logical thinking-mode boundaries.
...
describe("createAforaAgentHarness", () => {
  beforeEach(() => {
  ...
  it("brands only host-created instances as the built-in runtime", () => {
    expect(isBuiltInAforaAgentHarness(createAforaAgentHarness())).toBe(true);
    expect(
      isBuiltInAforaAgentHarness({
        id: "afora",
        label: "forged",
  ...
  it("preserves logical Ultra for the embedded attempt", async () => {
    ...
    await createAforaAgentHarness().runAttempt(params);
  ...
  it("enforces a tool-free settled-turn finalization", async () => {
    ...
    const harness = createAforaAgentHarness();
  ...
  it("runs isolated completion through the prepared zero-tool transport", async () => {
    const params = {
      ...
      NonNullable<ReturnType<typeof createAforaAgentHarness>["runIsolatedCompletionV2"]>
    >[0];

    await expect(createAforaAgentHarness().runIsolatedCompletionV2?.(params)).resolves.toEqual({
  ...
  it("rejects harness-owned isolated authorization", async () => {
    ...
    await expect(createAforaAgentHarness().runIsolatedCompletionV2?.(params)).rejects.toThrow(
