...
import { isAforaAbortableWrapper } from "./embedded-agent-runner/run/abortable.js";
import {
  FailoverError,
...
function isTerminalAbortFromError(err: unknown): boolean {
  ...
  return isAforaAbortableWrapper(err) && causeCandidates.some(isTerminalAbortCandidate);
}
...
function isCliAgentRuntime(runtime: string | undefined, cfg: AforaConfig | undefined): boolean {
...
export async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  if (isCliAgentRuntime(runtime, params.cfg)) {
    // CLI runtimes own their transport/auth, so stale Afora provider
