...
async function resolveContext(config: AforaConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): AforaConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
