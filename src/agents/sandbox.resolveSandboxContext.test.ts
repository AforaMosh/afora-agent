...
beforeAll(async () => {
  sandboxFixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-sandbox-context-"));
});
...
describe("resolveSandboxContext", () => {
  ...
  it("uses Podman directly when the Podman backend is configured", async () => {
    ...
    try {
      ...
      const workspaceStat = await fs.stat("/tmp/afora-test");
      const expectedUser =
  ...
  it("materializes skills into a hidden read-only workspace for writable sandboxes", async () => {
      ...
      agents: {
        defaults: {
          sandbox: {
            ...
            scope: "session",
            workspaceAccess: "rw",
            workspaceRoot: path.join(workspaceDir, ".afora", "sandboxes"),
          },
        },
      },
    };
    ...
    expect(syncOptions?.targetWorkspaceDir).toContain(
      path.join(".afora", "sandbox", "skills-workspaces"),
    );
    expect(syncOptions?.targetWorkspaceDir).toMatch(
      /[\\/]workspace-[a-f0-9]{32}[\\/]\.afora[\\/]sandbox-skills$/,
    );
    expect(syncOptions?.targetWorkspaceDir).not.toBe(
      path.join(workspaceDir, ".afora", "sandbox-skills"),
    );
    expect(syncOptions?.targetWorkspaceDir?.startsWith(path.join(workspaceDir, ".afora"))).toBe(
      false,
    );
    expect(syncOptions?.config).toBe(cfg);
    expect(syncOptions?.agentId).toBe("main");
  ...
  it("uses the SSH backend remote workspace for sandbox workspace info", async () => {
    ...
    expect(result?.containerWorkdir).toMatch(
      /^\/remote\/afora\/afora-ssh-workspace-[a-f0-9]{32}\/workspace$/,
    );
  ...
  it("materializes skills for shared writable sandboxes even when roots match", async () => {
    ...
    expect(syncOptions?.targetWorkspaceDir).toMatch(
      /[\\/]shared-[a-f0-9]{8}[\\/]\.afora[\\/]sandbox-skills$/,
    );
    expect(syncOptions?.targetWorkspaceDir).not.toBe(
      path.join(workspaceDir, ".afora", "sandbox-skills"),
    );
