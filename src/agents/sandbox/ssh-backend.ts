...
function resolvePreprovisionedSshRuntimePaths(params: {
  ...
  return {
    ...
    remoteSkillsWorkspaceDir: path.posix.join(remoteWorkspaceDir, ".afora", "sandbox-skills"),
  };
}
...
function buildSshSandboxRuntimeId(scopeKey: string): string {
  ...
  if (/:workspace:[a-f0-9]{32}$/i.test(trimmed)) {
    return `afora-ssh-workspace-${hashTextSha256(trimmed).slice(0, 32)}`;
  }
  ...
  return `afora-ssh-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
