...
function formatSandboxRecreateHint(params: { scope: SandboxScope; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`afora sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`afora sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("afora sandbox recreate --all");
}
