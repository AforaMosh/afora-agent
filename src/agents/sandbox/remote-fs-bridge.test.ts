...
describe("remote sandbox fs bridge", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("afora-remote-fs-create-", async (stateDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("afora-remote-fs-linked-root-", async (stateDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("afora-remote-fs-bridge-", async (stateDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("afora-remote-fs-bounded-read-", async (stateDir) => {
        const workspacePath = path.join(stateDir, "workspace");
        await fs.mkdir(workspacePath, { recursive: true });
        const workspaceDir = await fs.realpath(workspacePath);
        await fs.writeFile(path.join(workspaceDir, "note.txt"), "hello", "utf8");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("afora-remote-fs-copy-", async (stateDir) => {
        const workspacePath = path.join(stateDir, "workspace");
        await fs.mkdir(workspacePath, { recursive: true });
        const workspaceDir = await fs.realpath(workspacePath);
  ...
  it("normalizes stat output locale and saturates unsafe sizes", async () => {
    ...
    await withTempDir("afora-remote-fs-bridge-stat-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("rejects hardlinked files under localized remote shells", async () => {
    await withTempDir("afora-remote-fs-bridge-hardlink-locale-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
      const runtime = createStatRuntime(workspaceDir, {
        hardlinks: (script) =>
  ...
  it("does not reject malformed non-decimal hardlink counts", async () => {
    await withTempDir("afora-remote-fs-bridge-hardlink-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
      const runtime = createStatRuntime(workspaceDir, {
