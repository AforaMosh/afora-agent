...
const runRemoteShellScript: RemoteShellSandboxHandle["runRemoteShellScript"] =
  createLocalRemoteShellScriptRunner({ shellArg0: "afora-test" });

describe("workspace skills bridge mount policy", () => {
  it("resolves workspace skill roots as read-only", async () => {
    await withTempDir("afora-skills-bridge-", async (stateDir) => {
      ...
      expect(resolve(".afora/sandbox-skills/skills/demo/SKILL.md").writable).toBe(false);
      expect(resolve(".afora/sandbox-skills/skills/demo/SKILL.md").hostPath).toBe(
        path.join(skillsWorkspaceDir, "skills", "demo", "SKILL.md"),
      );
      expect(resolve("/workspace/skills/demo/SKILL.md").writable).toBe(false);
      expect(resolve("/workspace/.afora/sandbox-skills/skills/demo/SKILL.md").writable).toBe(
        false,
      );
    });
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("afora-skills-remote-absent-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("afora-skills-remote-only-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const skillsWorkspaceDir = path.join(stateDir, "sandbox-state");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await withTempDir("afora-skills-remote-link-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const remoteWorkspaceDir = path.join(stateDir, "remote-workspace");
        await fs.mkdir(workspaceDir, { recursive: true });
        await fs.mkdir(path.join(remoteWorkspaceDir, "skills", "demo"), { recursive: true });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("afora-skills-remote-cwd-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const remoteWorkspaceDir = path.join(stateDir, "remote-workspace");
        await fs.mkdir(path.join(workspaceDir, "skills", "demo"), { recursive: true });
        await fs.mkdir(path.join(remoteWorkspaceDir, "skills", "demo"), { recursive: true });
