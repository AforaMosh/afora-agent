...
describe("sandbox fs bridge anchored ops", () => {
  ...
  it.each(pinnedReadCases)("$name", async (testCase) => {
    await withTempDir("afora-fs-bridge-contract-read-", async (stateDir) => {
  ...
  it.each([
    ...
    await withTempDir("afora-fs-bridge-bounded-read-", async (stateDir) => {
  ...
  it.each([
    ...
    await withTempDir("afora-fs-bridge-bounded-reject-", async (stateDir) => {
      const { bridge } = await createSeededSandboxFsBridge(stateDir, {
        rootContents: "hello",
      });
  ...
  it("rejects files that grow after the sandbox descriptor is opened", async () => {
    await withTempDir("afora-fs-bridge-bounded-growth-", async (stateDir) => {
  ...
  it.each(pinnedCases)("$name", async (testCase) => {
    ...
    await withTempDir("afora-fs-bridge-contract-write-", async (stateDir) => {
      const { bridge } = await createSeededSandboxFsBridge(stateDir);
  ...
  it("allows dot-dot-prefixed sandbox entries without treating them as parent traversal", async () => {
    await withTempDir("afora-fs-bridge-dot-prefix-", async (stateDir) => {
      const { bridge } = await createSeededSandboxFsBridge(stateDir);
  ...
  it("stat anchors parent + basename", async () => {
    await withTempDir("afora-fs-bridge-contract-stat-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
  ...
  it("runs stat under the C locale so missing-file errors return null", async () => {
    await withTempDir("afora-fs-bridge-stat-missing-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("keeps non-missing stat failures as errors", async () => {
    await withTempDir("afora-fs-bridge-stat-error-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("saturates unsafe stat size output", async () => {
    await withTempDir("afora-fs-bridge-stat-parse-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
