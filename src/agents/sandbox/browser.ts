...
const CDP_SOURCE_RANGE_ENV_KEY = "AFORA_BROWSER_CDP_SOURCE_RANGE";
const CDP_AUTH_TOKEN_ENV_KEY = "AFORA_BROWSER_CDP_AUTH_TOKEN";
const SANDBOX_BROWSER_IMAGE_CONTRACT_LABEL = "org.afora.sandbox-browser.contract";
...
function buildSandboxCdpAuthHeader(token: string): string {
  return `Basic ${Buffer.from(`afora:${token}`).toString("base64")}`;
}
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_AFORA_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_AFORA_BROWSER_PROFILE_NAME,
    extraArgs: [],
    tabCleanup: {
      enabled: true,
      idleMinutes: 120,
...
async function ensureSandboxBrowserContainer(
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "afora.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `afora sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `afora sandbox recreate --browser --agent ${agentId}`;
          }
          return "afora sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        "afora.sandboxBrowser": "1",
        "afora.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `AFORA_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `AFORA_BROWSER_ENABLE_NOVNC=${params.cfg.browser.noVncEnabled ? "1" : "0"}`);
    args.push("-e", `AFORA_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `${CDP_AUTH_TOKEN_ENV_KEY}=${cdpAuthToken}`);
    args.push(
      "-e",
      `AFORA_BROWSER_AUTO_START_TIMEOUT_MS=${params.cfg.browser.autoStartTimeoutMs}`,
    );
    if (cdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
    }
    args.push("-e", `AFORA_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `AFORA_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    args.push("-e", "AFORA_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_AFORA_BROWSER_PROFILE_NAME)
    : null;
