...
} from "./afora-tools.subagents.test-harness.js";
import {
...
describe("afora-tools: subagents scope isolation", () => {
  let storePath = "";
  ...
  beforeEach(() => {
    ...
    storePath = path.join(
      os.tmpdir(),
      `afora-subagents-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
