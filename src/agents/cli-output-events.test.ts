...
describe("createCliJsonlStreamingParser events", () => {
  it("streams Gemini message deltas and tool events", () => {
    ...
    parser.push(
      [
        ...
        JSON.stringify({
          ...
          tool_name: "mcp_afora_create_goal",
          tool_id: "tool-1",
    ...
    expect(starts).toEqual([
      {
        toolCallId: "tool-1",
        name: "mcp_afora_create_goal",
        kind: "tool_use",
        args: { objective: "Update files" },
      },
    ]);
    expect(results).toEqual([
      { toolCallId: "tool-1", name: "mcp_afora_create_goal", isError: false, result: "created" },
    ]);
      ...
      [
        ...
        JSON.stringify({
          ...
          event: {
            type: "content_block_delta",
            index: 0,
            delta: { type: "input_json_delta", partial_json: '{"query":"afora"}' },
          },
        }),
        JSON.stringify({
          type: "stream_event",
  ...
  it.each([
    {
      ...
      name: "web_search",
      input: { query: "afora" },
      result: [{ type: "web_search_result", title: "Afora", url: "https://example.com" }],
      isError: false,
    },
    {
