...
import { resolveAforaMcpTransportAlias } from "../config/mcp-config-normalize.js";
...
type McpServerConnectionResolverEntry = AforaPluginMcpServerConnectionResolver & {
  pluginId: string;
};
...
const MCP_CONNECTION_RESOLVER_TEST_STATE_KEY = Symbol.for(
  "afora.mcpServerConnectionResolverTestState",
);
...
export function applyMcpConnectionOverride(
  ...
  const fromTransport =
    typeof base.transport === "string"
      ? resolveAforaMcpTransportAlias(base.transport)
      : undefined;
  const fromType = resolveAforaMcpTransportAlias(base.type);
...
export const testing = {
  setMcpServerConnectionResolversForTest(
    resolvers?: Iterable<AforaPluginMcpServerConnectionResolver & { pluginId?: string }> | null,
  ): void {
    if (!resolvers) {
