...
import { defaultApiRegistry } from "@afora/ai/internal/runtime";
import { prepareModelForSimpleCompletion } from "@afora/ai/transports";
import { normalizeProviderId } from "@afora/model-catalog-core/provider-id";
import { expectDefined } from "@afora/normalization-core";
import { type Api, completeSimple, type Model } from "afora-agent/plugin-sdk/llm";
...
import { ensureAforaModelsJson } from "./models-config.js";
...
const DIRECT_ENABLED = Boolean(process.env.AFORA_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.AFORA_LIVE_HEARTBEAT_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.AFORA_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
const LIVE_TEST_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.AFORA_LIVE_TEST_TIMEOUT_MS, 60 * 60 * 1000),
);
const DEFAULT_LIVE_MODEL_CONCURRENCY = 20;
const LIVE_MODEL_CONCURRENCY = resolveLiveModelConcurrency(
  process.env.AFORA_LIVE_MODEL_CONCURRENCY,
);
const LIVE_MODELS_JSON_TIMEOUT_MS = resolveLiveModelsJsonTimeoutMs(
  process.env.AFORA_LIVE_MODELS_JSON_TIMEOUT_MS,
);
...
let activeLiveCompletionConfig: AforaConfig | undefined;
...
function applyLiveOllamaProviderEnvCompat(params: {
  ...
  const liveBaseUrl = params.env?.AFORA_LIVE_OLLAMA_BASE_URL?.trim();
...
function resolveLiveOllamaBaseUrl(model: Pick<Model, "baseUrl">, config?: AforaConfig): string {
  return (
...
describe("explicit live model discovery scope", () => {
  ...
  it("merges explicit model providers with AFORA_LIVE_PROVIDERS", () => {
  ...
  it("uses the local Ollama auth marker for self-hosted live env URLs", () => {
    ...
    for (const baseUrl of [
      ...
      const result = applyLiveProviderDiscoveryPluginCompat({
        ...
        providers: ["ollama"],
        env: {
          AFORA_LIVE_OLLAMA_BASE_URL: baseUrl,
        },
      });
  ...
  it("replaces configured Ollama auth when live env redirects to a different local endpoint", () => {
    ...
    const result = applyLiveProviderDiscoveryPluginCompat({
      ...
      providers: ["ollama"],
      env: {
        AFORA_LIVE_OLLAMA_BASE_URL: "http://127.0.0.1:11434",
      },
    });
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.AFORA_LIVE_MODELS?.trim();
      ...
      const providers = parseProviderFilter(process.env.AFORA_LIVE_PROVIDERS);
      ...
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set AFORA_LIVE_MODELS=modern|small|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const perModelTimeoutMs = toInt(process.env.AFORA_LIVE_MODEL_TIMEOUT_MS, 30_000);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via AFORA_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
