...
import { completeSimple, type Model } from "afora-agent/plugin-sdk/llm";
...
const SETUP_TOKEN_MODEL = process.env.AFORA_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "AFORA_LIVE_SETUP_TOKEN did not look like a setup-token. Use AFORA_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set AFORA_LIVE_SETUP_TOKEN_VALUE or AFORA_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureAforaModelsJson(cfg, tokenSource.agentDir);
