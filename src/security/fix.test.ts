...
describe("security fix", () => {
  ...
  const runConfigFixScenario = async (params: {
    ...
    const cfg = JSON.parse(await fs.readFile(configPath, "utf-8")) as AforaConfig;
    return { res, cfg };
  };
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-security-fix-suite-"));
  });
  ...
  it("collects permission targets for credentials + agent auth/sessions + include files", async () => {
    ...
    const authDatabasePath = path.join(agentDir, "afora-agent.sqlite");
