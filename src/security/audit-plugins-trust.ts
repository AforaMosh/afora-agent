...
function readChannelCommandSetting(
  ...
  const channelCfg = cfg.channels?.[channelId as keyof NonNullable<AforaConfig["channels"]>];
  ...
  if (pluginDirs.length > 0) {
    ...
    if (enabledExtensionPluginIds.length > 0) {
      ...
      for (const context of contexts) {
        ...
        const broadPolicy = deps.isToolAllowedByPolicies("__afora_plugin_probe__", policies);
  ...
  if (npmPluginInstalls.length > 0) {
    ...
    if (pluginVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `afora plugins update --all` (or reinstall affected plugins) to refresh install metadata.",
      });
    }
  }
  ...
  if (npmHookInstalls.length > 0) {
    ...
    if (hookVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `afora hooks update --all` (or reinstall affected hooks) to refresh install metadata.",
      });
    }
  }
