...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("afora status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("afora security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set AFORA_GATEWAY_TOKEN/AFORA_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("afora security audit --deep")}".`,
    });
  }
  return findings;
}
