...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: AforaConfig, stateDir: string) => {
  ...
  const writePluginIndexInstallRecords = async (
    ...
    const index: InstalledPluginIndex = {
      ...
      plugins: Object.keys(records).map((pluginId) => ({
        pluginId,
        manifestPath: path.join(stateDir, "extensions", pluginId, "afora.plugin.json"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-security-install-"));
  });
    ...
    }> = [
      {
        ...
        run: async () => {
          ...
          writeHookInstalls(stateDir, {
            "test-hooks": { source: "npm", spec: "@afora/test-hooks" },
          });
        ...
        run: async () => {
          ...
          await writePluginIndexInstallRecords(stateDir, {
            "voice-call": {
              source: "npm",
              spec: "@afora/voice-call",
              resolvedSpec: "@afora/voice-call@1.2.3",
              integrity: "sha512-plugin",
            },
          });
          writeHookInstalls(stateDir, {
            "test-hooks": {
              ...
              resolvedSpec: "@afora/test-hooks@1.2.3",
              integrity: "sha512-hook",
            },
          });
          return runInstallMetadataAudit({}, stateDir);
...
describe("security audit extension tool reachability findings", () => {
  ...
  const runSharedExtensionsAudit = async (config: AforaConfig) => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-security-extensions-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { AFORA_HOME: isolatedHome });
