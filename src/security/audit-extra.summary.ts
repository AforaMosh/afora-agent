...
function summarizeGroupPolicy(cfg: AforaConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: AforaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AforaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AforaConfig): boolean {
...
function isBrowserEnabled(cfg: AforaConfig): boolean {
...
export function collectAttackSurfaceSummaryFindings(cfg: AforaConfig): SecurityAuditFinding[] {
  ...
  const detail =
    ...
    `\n` +
    "trust model: personal assistant (one trusted operator boundary), not hostile multi-tenant on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.afora.ai/gateway/multi-tenant-hosting";
