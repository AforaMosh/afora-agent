...
type CollectDangerousConfigFlags = (cfg: AforaConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.AFORA_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.AFORA_GATEWAY_PASSWORD);
  ...
  if (cfg.mcp?.apps?.enabled === true) {
    findings.push({
      ...
      remediation:
        "Keep this enabled only for MCP servers you trust. Disable with `afora config set mcp.apps.enabled false --strict-json` when it is not needed.",
    });
  }
