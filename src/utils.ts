...
export { sliceUtf16Safe, truncateUtf16Safe } from "@afora/normalization-core/utf16-slice";

/** Resolves the Afora config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".afora");
  try {
...
/** Resolves the effective Afora home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AFORA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AFORA_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$AFORA_HOME`. */
