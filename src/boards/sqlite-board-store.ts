...
import { ensureAforaAgentBoardSchemaInTransaction } from "../state/afora-agent-board-schema.js";
...
>;
type BoardDatabaseHandle = Pick<AforaAgentDatabase, "db" | "path">;
...
// not "no such table".
function boardTablesPresent(database: Pick<AforaAgentDatabase, "db">): boolean {
...
function ensureBoardSchema(database: AforaAgentDatabase): void {
  ...
  runSqliteImmediateTransactionSync(
    database.db,
    () => ensureAforaAgentBoardSchemaInTransaction(database.db),
    {
...
export class SqliteBoardStore implements BoardStore {
  ...
  listSessionsWithBoards(): string[] {
    ...
    const agentIds = new Set(
      listAforaRegisteredAgentDatabases({ env: this.options.env }).map(
    ...
    for (const agentId of agentIds) {
      const canonicalPath =
        ...
        resolveAforaAgentSqlitePath({ agentId, env: this.options.env });
      const result = withAforaAgentDatabaseReadOnly(
        (database) => {
