...
describe("media-understanding missing provider errors", () => {
  it("includes the catalog repair hint for a media provider contract", async () => {
    ...
    expect(message).toMatch(/^Media provider not available: groq .*afora plugins install/);
    expect(message).toContain("@afora/groq-provider");
    expect(message).toContain("afora plugins registry --refresh");
    expect(message).toContain("stop and start the gateway service");
    expect(message).toContain("afora doctor --fix");
  });
