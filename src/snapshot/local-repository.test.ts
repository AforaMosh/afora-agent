...
async function createTempDir(): Promise<string> {
  const tempDir = tempDirs.make("afora-snapshot-repository-");
...
describe("local SQLite snapshot repository", () => {
  ...
  it("sanitizes transient global rows and enforces the global owner", async () => {
    const tempDir = await createTempDir();
    const sourcePath = path.join(tempDir, "afora.sqlite");
    const repositoryPath = path.join(tempDir, "snapshots");
  ...
  it("snapshots agents without requiring legacy lease storage", async () => {
    const tempDir = await createTempDir();
    const sourcePath = path.join(tempDir, "afora-agent.sqlite");
    const repositoryPath = path.join(tempDir, "snapshots");
