...
import type { AssistantMessage, ModelInputContent, ProviderReplayState } from "../llm/types.js";
import {
  isVideoMediaFact,
...
function projectWorkerInputContent(part: ModelInputContent) {
  ...
  return { type: "text" as const, text: WORKER_VIDEO_UNAVAILABLE_TEXT };
}
      ...
      ? {
          diagnostics: message.diagnostics.map((diagnostic) => {
            const details = diagnostic.details
              ? asOptionalRecord(sanitizeWorkerPayload(diagnostic.details))
              : undefined;
            return {
...
export function toWorkerTranscriptMessage(
  ...
  if (message.role === "user") {
    ...
    const mediaBlockCount = Array.isArray(message.content)
      ? message.content.filter((part) => part.type === "image" || part.type === "video").length
      : 0;
  ...
  if (message.role === "toolResult") {
    return {
      ...
      message: {
        ...
        content: message.content.map(projectWorkerInputContent),
        ...(message.details === undefined
          ? {}
          : { details: sanitizeWorkerPayload(message.details) }),
