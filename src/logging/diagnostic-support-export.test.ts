...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "afora.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["afora", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              AFORA_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).not.toContain("__AFORA_REDACTED__");
    ...
    expect(combined).toContain("$AFORA_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/afora-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.AFORA_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("truncates support strings without splitting UTF-16 surrogate pairs", () => {
    ...
    const redactedPathPrefix = `$AFORA_STATE_DIR${path.sep}`;
    expect(
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\afora`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$AFORA_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\afora\\dist\\index.js`,
              "--config",
              `${stateDir}\\afora.json`,
            ...
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\afora\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\afora\\\\dist\\\\index.js");
    expect(serialized).toContain("$AFORA_STATE_DIR\\\\afora.json");
    expect(serialized).toContain("--aws-secret-access-key=<redacted>");
    expect(serialized).toContain("--awsSecretAccessKey");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\afora\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/afora.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/afora-sanitized.jsonl");
