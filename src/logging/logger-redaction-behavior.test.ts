...
const logPathTracker = createSuiteLogPathTracker("afora-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", async () => {
    ...
    withEnv({ AFORA_CONFIG_PATH: configPath, AFORA_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-afora.log")).toBe(
        path.join(home, "custom-afora.log"),
      );
    });
  });
