...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-logging-config-"));
  tempDirs.push(dir);
...
describe("readLoggingConfig", () => {
  ...
  it("reads logging style without a mutating config load for config schema", () => {
    process.argv = ["node", "afora", "config", "schema"];
  ...
  it("supports JSON5 comments and trailing commas", () => {
    const configPath = writeConfig(`{
      // users commonly keep comments in afora.json
      logging: {
        consoleLevel: "warn",
      },
    }`);
  ...
  it("resolves nested includes and environment substitutions for logging style", () => {
    ...
    fs.writeFileSync(
      ...
      `{ consoleStyle: "\${AFORA_TEST_CONSOLE_STYLE}", file: "\${MISSING_LOG_FILE}" }`,
    );

    withEnv(
      {
        AFORA_CONFIG_PATH: configPath,
        AFORA_TEST_CONSOLE_STYLE: "json",
  ...
  it("does not cache a partial style while environment-backed fields are unresolved", () => {
    ...
    withEnv(
      {
        AFORA_CONFIG_PATH: configPath,
        AFORA_TEST_LOG_FILE: "/tmp/afora-env-backed.log",
      },
      () => {
        expect(readLoggingConfig()).toStrictEqual({
          consoleStyle: "json",
          file: "/tmp/afora-env-backed.log",
          level: "debug",
        });
      },
    );
  ...
  it("returns undefined for missing or malformed config files", () => {
    withEnv(
      { AFORA_CONFIG_PATH: path.join(os.tmpdir(), "afora-missing-config.json") },
      () => {
