    ...
    info: {
      ...
      hostRequirements: {
        "agent-run": {
          ...
          unsupportedMessage:
            "Use the native Codex or Afora embedded runtime, or switch contextEngine to legacy.",
        },
      },
    },
    async ingest() {
...
describe("context engine host compatibility", () => {
  ...
  it("allows native Codex and Afora embedded hosts to satisfy pre-prompt assembly", () => {
    ...
    assertContextEngineHostSupport({
      contextEngine: engine,
      operation: "agent-run",
      host: AFORA_EMBEDDED_CONTEXT_ENGINE_HOST,
    });
  });
