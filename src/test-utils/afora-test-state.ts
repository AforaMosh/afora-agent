// Creates isolated Afora state directories for integration-style tests.
...
type AforaTestStateLayout = "home" | "state-only" | "split";

type AforaTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
type AforaTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: AforaTestStateLayout;
  scenario?: AforaTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "afora-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "afora.json"),
...
function scenarioConfig(options: AforaTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "afora-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: AforaTestStateOptions): Record<string, string | undefined> {
...
export async function createAforaTestState(
  options: AforaTestStateOptions = {},
): Promise<AforaTestState> {
  ...
  const state: AforaTestState = {
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed Afora env set, not plugin-provided host env.
        if (value === undefined) {
    ...
    cleanup: async () => {
      ...
      closeAforaStateDatabaseByPath(resolveAforaStateSqlitePath(env));
      state.restoreEnv();
      await fs.rm(root, {
        recursive: true,
        force: true,
...
export async function withAforaTestState<T>(
  options: AforaTestStateOptions,
  fn: (state: AforaTestState) => Promise<T>,
): Promise<T> {
  const state = await createAforaTestState(options);
  try {
