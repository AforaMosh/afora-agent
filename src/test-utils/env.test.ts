...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "AFORA_ENV_TEST_A";
    const keyB = "AFORA_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "AFORA_ENV_TEST_ADDED";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "AFORA_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "AFORA_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "AFORA_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "AFORA_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "AFORA_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    setTestEnvValue(key, "outer");
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    ...
    const previousBundledDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
    setTestEnvValue("AFORA_HOME", "/srv/afora-home");
    setTestEnvValue("AFORA_STATE_DIR", "/srv/afora-state");
    setTestEnvValue("AFORA_BUNDLED_PLUGINS_DIR", "/srv/afora-bundled");
    ...
    try {
      ...
      expect(env.AFORA_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.AFORA_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("AFORA_HOME", previousAforaHome);
      restoreEnvKey("AFORA_STATE_DIR", previousStateDir);
      restoreEnvKey("AFORA_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processAforaHome: process.env.AFORA_HOME,
          processBundledDir: process.env.AFORA_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.AFORA_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processAforaHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.AFORA_HOME).toBe("/srv/afora-home");
    } finally {
      restoreEnvKey("AFORA_HOME", previousAforaHome);
    }
  });
