...
async function runGatewayConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("afora configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("afora config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(ctx.configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("afora setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("afora config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("afora config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runGatewayHealthChecks(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if ((await hasActiveGatewayExecCredential(ctx)) && ctx.options.allowExec !== true) {
    note(
      "Gateway health probes skipped because gateway credentials use an exec SecretRef. Run `afora doctor --allow-exec` to verify Gateway health with exec SecretRefs.",
      "Gateway",
    );
