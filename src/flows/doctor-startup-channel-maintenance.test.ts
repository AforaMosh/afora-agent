...
describe("doctor startup channel maintenance", () => {
  ...
  it("maps channel doctor preview warnings to structured findings", async () => {
    ...
    await expect(
      collectChannelPreviewWarningHealthFindings({
        cfg,
        doctorFixCommand: "afora doctor --fix --dry-run",
        env: { AFORA_TEST: "1" },
        allowExec: true,
      }),
    ).resolves.toEqual([
      {
        ...
        fixHint:
          "Run `afora doctor --fix --dry-run` if the channel warning recommends repair, or update the affected channel config manually.",
      },
    ]);
  ...
  it("runs Matrix startup migration during repair flows", async () => {
    ...
    expect(call.env).toEqual({ AFORA_TEST: "1" });
