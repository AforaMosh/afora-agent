...
type PromptDefaultModelResult = { model?: string; config?: AforaConfig };
...
function resolveConfiguredModelRaw(cfg: AforaConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: AforaConfig): string[] {
...
function resolveModelPickerConfig(cfg: AforaConfig, agentId?: string): AforaConfig {
...
function createModelRouteRuntimeResolver(params: {
  ...
  const cache = new Map<string, "codex" | "afora" | undefined>();
  return (route) => {
    const baseUrlKey =
    ...
    const runtime =
      policy.runtime === "codex" ? "codex" : policy.runtime === "afora" ? "afora" : undefined;
