...
async function makeParentRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "afora-temp-dir-helper-test-"));
...
describe("withTestDir", () => {
  it("removes the cached async prefix root when the case finishes", async () => {
    ...
    await withTestDir({ prefix: "afora-leak-check-", parentDir }, async (dir) => {
  ...
  it("keeps the cached async prefix root while another case is active", async () => {
    ...
    const first = withTestDir({ prefix: "afora-shared-root-", parentDir }, async (dir) => {
      await fs.writeFile(path.join(dir, "first.txt"), "ok");
      await firstCanFinish;
    });

    await withTestDir({ prefix: "afora-shared-root-", parentDir }, async (dir) => {
  ...
  it("removes the cached sync prefix root when the case finishes", async () => {
    ...
    withTempDirSync({ prefix: "afora-leak-check-sync-", parentDir }, (dir) => {
