...
async function importThemeWithEnv(env: ThemeEnvOverrides) {
  if (Object.hasOwn(env, "AFORA_THEME")) {
    if (env.AFORA_THEME === undefined) {
      delete process.env.AFORA_THEME;
    } else {
      process.env.AFORA_THEME = env.AFORA_THEME;
    }
  }
...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      AFORA_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when AFORA_THEME=light", async () => {
    const mod = await importThemeWithEnv({ AFORA_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when AFORA_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ AFORA_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats AFORA_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ AFORA_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("AFORA_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      AFORA_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ AFORA_THEME: "light" });
    const darkMod = await importThemeWithEnv({ AFORA_THEME: "dark" });
