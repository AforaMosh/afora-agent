...
// These cases spawn afora.mjs outside the source TUI runner. CI opts in only
// after the exact head has a complete build, so source-mode PTY smoke must skip them.
const itWithBuiltCli = process.env.AFORA_TUI_PTY_USE_BUILT_CLI === "1" ? it : it.skip;
...
function buildTuiCliScript(args: string[]) {
  ...
  return [
    ...
    `program.parseAsync([process.execPath, "afora", ...${JSON.stringify(args)}], { from: "node" }).catch((error) => {`,
    `  console.error(error);`,
    `  process.exit(1);`,
    `});`,
  ].join("\n");
...
function buildTuiProcessArgs(args: string[]) {
  if (process.env.AFORA_TUI_PTY_USE_BUILT_CLI === "1") {
    return [path.join(process.cwd(), "afora.mjs"), ...args];
  }
  ...
  return {
    ...
    agents: {
      defaults: {
        ...
        models: {
          "tui-pty-mock/gpt-5.5": { agentRuntime: { id: "afora" } },
        },
        skills: [],
        skipBootstrap: true,
      },
...
async function startLocalModeTui(
  ...
  opts: {
    ...
    prepareConfig?: (params: {
      ...
      tempDir: string;
      stateDir: string;
    }) => Promise<AforaConfig> | AforaConfig;
  } = {},
) {
  const replyText = opts.replyText ?? "LOCAL_PTY_RESPONSE";
  const tempDir = await mkdtemp(path.join(tmpdir(), "afora-tui-pty-local-"));
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    AFORA_TUI_LOCAL_RUN_SHUTDOWN_GRACE_MS: "500",
  ...
  let config: AforaConfig = buildLocalModeConfig({
    workspaceDir,
    ...
    agents: {
      defaults: {
        ...
        models: Object.fromEntries(
          modelRefs.map((modelRef) => [modelRef, { agentRuntime: { id: "afora" } }]),
        ),
        skills: [],
        skipBootstrap: true,
      },
...
async function startSharedGatewayFixture(): Promise<SharedGatewayFixture> {
  const tempDir = await mkdtemp(path.join(tmpdir(), "afora-tui-pty-gateway-"));
  let mockModel: MockModelServer | undefined;
  let gateway: AforaTestInstance | undefined;
  ...
  try {
    ...
    gateway = await createAforaTestInstance({
...
async function startIsolatedGatewayPty(params: {
  ...
  const tempDir =
    ...
    (await mkdtemp(path.join(tmpdir(), "afora-tui-pty-gateway-client-")));
  let run: PtyRun;
  try {
    await writeFile(path.join(tempDir, "afora.json"), "{}\n", "utf8");
    ...
    run = startPty(process.execPath, buildTuiProcessArgs(cliArgs), {
      ...
      env: {
        ...
        HOME: tempDir,
        AFORA_HOME: tempDir,
        AFORA_CONFIG_PATH: path.join(tempDir, "afora.json"),
...
describe("TUI PTY real backends", () => {
  for (const alias of ["chat", "terminal"] as const) {
    it(
      `launches afora ${alias} as local mode through a real PTY`,
  ...
  it(
    "sends the initial message supplied to afora tui through a real local PTY",
    async ({ onTestFinished }) => {
  ...
  it(
    ...
    async ({ onTestFinished }) => {
      ...
      try {
        ...
        if (process.env.AFORA_BEHAVIOR_EVIDENCE === "1") {
          console.info(
    ...
    async ({ onTestFinished }) => {
      ...
      try {
        ...
        await fixture.run.write(
          "!node -e \"console.log('T06_STDOUT'); console.error('T06_STDERR'); console.log('T06_ENV='+process.env.AFORA_SHELL); process.exitCode=7\"\r",
        );
        await fixture.run.waitForOutput("Allow local shell commands for this session?");
  ...
  itWithBuiltCli(
    ...
    async ({ onTestFinished }) => {
      ...
      try {
        await fixture.run.waitForOutput("local ready", LOCAL_STARTUP_TIMEOUT_MS);
        const cliPath = path.join(process.cwd(), "afora.mjs");
        ...
        const repaired = JSON.parse(await readFile(fixture.configPath, "utf8")) as AforaConfig;
        ...
        const { stdout } = await runExec(
          ...
          {
            cwd: process.cwd(),
            env: { ...fixture.env, AFORA_TEST_RUNTIME_LOG: "1" },
            logOutput: false,
            timeoutMs: LOCAL_OUTPUT_TIMEOUT_MS,
          },
        );
        ...
        prepareConfig: async ({ config, tempDir }) => {
          ...
          await Promise.all([
            writeFile(
              ...
              `${JSON.stringify(
                {
                  name: "@afora/t05-local-auth-fixture",
                  version: "0.0.0",
                  type: "module",
...
`,
      ...
      try {
        ...
        const sqlitePath = path.join(agentDir, "afora-agent.sqlite");
        ...
        const config = JSON.parse(await readFile(fixture.configPath, "utf8")) as AforaConfig;
  ...
  registerGatewayTest(
    ...
    async ({ onTestFinished }) => {
      ...
      const requestOffset = shared.mockModel.requests(scenario.modelId).length;
      const clientStateDir = await mkdtemp(path.join(tmpdir(), "afora-tui-pty-resume-client-"));
  ...
  describe("with shared Gateway fixture", () => {
    ...
    it("launches afora tui against a real Gateway through a real PTY", async () => {
