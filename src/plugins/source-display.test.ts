...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "afora", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".afora", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".afora", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "afora-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".afora", "extensions"),
      },
    });
  });
});
