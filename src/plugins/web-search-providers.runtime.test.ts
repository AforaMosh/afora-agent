...
let loadAforaPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/afora.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/afora.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
...
function createWebSearchManifestRecord(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/afora.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    syntheticAuthRefs: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadAforaPluginsMock).toHaveBeenCalledTimes(count);
}
...
function expectScopedWebSearchCandidates(pluginIds: readonly string[]) {
  ...
  expect(
    requireLastCallFirstArg(loadAforaPluginsMock, "loadAforaPlugins").onlyPluginIds,
  ).toEqual([...pluginIds]);
}
...
function expectAutoEnabledWebSearchLoad(params: {
  ...
  const loaderParams = requireLastCallFirstArg(loadAforaPluginsMock, "loadAforaPlugins");
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadAforaPluginsMock = vi
      .spyOn(loaderModule, "loadAforaPlugins")
  ...
  it("resolves current config contents when config changes in place", () => {
    ...
    const env = createWebSearchEnv({ AFORA_HOME: "/tmp/afora-home-a" });
  ...
  it("resolves current env contents when env changes in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.AFORA_HOME = "/tmp/afora-home-b";
      },
      expectedLoaderCalls: 2,
    });
  });
  ...
  it("does not reuse snapshot provider loads across host Vitest env changes", () => {
    ...
    expect(loadAforaPluginsMock).toHaveBeenCalledTimes(2);
  });
});
