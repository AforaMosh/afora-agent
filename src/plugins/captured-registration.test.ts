...
import type { AnyAgentTool, AforaPluginApi } from "./types.js";
...
describe("captured plugin registration", () => {
  it("preserves root machine-output metadata", () => {
    ...
    expect(
      descriptor?.machineOutput?.({
        argv: ["node", "afora", "captured-machine"],
        stdoutIsTTY: false,
      }),
    ).toBe(true);
  });
  ...
  it("returns synthetic scheduled-turn ids independent of human-readable names", async () => {
    let scheduleSessionTurn: AforaPluginApi["scheduleSessionTurn"] | undefined;
    let registerSessionSchedulerJob: AforaPluginApi["registerSessionSchedulerJob"] | undefined;
    const captured = capturePluginRegistration({
