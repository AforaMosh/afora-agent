...
const excludeOptionalEnv = { AFORA_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
function createPlugin(
  ...
  params: {
    ...
    packageAfora?: Record<string, unknown>;
  },
) {
  const pluginDir = path.join(repoRoot, "extensions", params.id);
  fs.mkdirSync(pluginDir, { recursive: true });
  writeJson(path.join(pluginDir, "afora.plugin.json"), {
    id: params.id,
  ...
  writeJson(path.join(pluginDir, "package.json"), {
    ...
    ...(params.packageAfora ? { afora: params.packageAfora } : {}),
  });
  return pluginDir;
}
...
function readBundledManifest(repoRoot: string, pluginId: string): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "afora.plugin.json"),
      "utf8",
    ),
...
function createTlonSkillPlugin(repoRoot: string, skillPath = "node_modules/@tloncorp/tlon-skill") {
  return createPlugin(repoRoot, {
    id: "tlon",
    packageName: "@afora/tlon",
    manifest: { skills: [skillPath] },
    packageAfora: { extensions: ["./index.ts"] },
  });
}
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-meta-");
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "afora.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    expect(packageJson.afora?.extensions).toEqual(["./index.js"]);
  });

  it("copies generated bundled channel config schemas into dist manifests", () => {
    const repoRoot = makeRepoRoot("afora-bundled-channel-config-meta-");
    createPlugin(repoRoot, {
      id: "telegram",
      packageName: "@afora/telegram",
      manifest: {
        channels: ["telegram"],
        channelConfigs: {
          telegram: {
  ...
  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-node-modules-");
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-hoisted-skill-");
    const pluginDir = createTlonSkillPlugin(repoRoot);
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-missing-skill-");
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-retry-");
    const pluginDir = createPlugin(repoRoot, {
      id: "diffs",
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "afora.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@afora/google-gemini-cli-auth",
    });
  ...
  it("removes non-packaged private QA plugin metadata unless private QA build is enabled", () => {
    const repoRoot = makeRepoRoot("afora-private-qa-metadata-");
    createPlugin(repoRoot, {
      id: "qa-lab",
      packageName: "@afora/qa-lab",
      packageAfora: { extensions: ["./index.ts"] },
    });
    ...
    copyBundledPluginMetadataWithEnv({
      repoRoot,
      env: { AFORA_BUILD_PRIVATE_QA: "1" } as NodeJS.ProcessEnv,
    });

    expect(fs.existsSync(path.join(staleDistDir, "afora.plugin.json"))).toBe(true);
  ...
  it.each([
    ...
    },
    {
      ...
      packageAfora: {
        extensions: ["./index.ts"],
        install: { npmSpec: "@afora/whatsapp" },
      },
      env: {},
      expectedExists: false,
    },
  ] as const)("$name", ({ pluginId, packageName, packageAfora, env, expectedExists }) => {
    const repoRoot = makeRepoRoot(`afora-bundled-plugin-${pluginId}-`);
    createPlugin(repoRoot, {
      id: pluginId,
      packageName,
      packageAfora,
  ...
  it("removes build-excluded bundled plugin metadata", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-excluded-meta-");
    createPlugin(repoRoot, {
      id: "whatsapp",
      packageName: "@afora/whatsapp",
      packageAfora: {
  ...
  it("preserves isolated source-checkout output for an external plugin", () => {
    const repoRoot = makeRepoRoot("afora-external-plugin-local-dist-meta-");
    createPlugin(repoRoot, {
      id: "sms",
      packageName: "@afora/sms",
      packageAfora: {
        extensions: ["./index.ts"],
    ...
    expect(fs.existsSync(path.join(distPluginDir, "afora.plugin.json"))).toBe(true);
    expect(readBundledPackageJson(repoRoot, "sms").afora?.extensions).toEqual(["./index.js"]);
  });

  it("preserves manifest-less runtime support package outputs and copies package metadata", () => {
    const repoRoot = makeRepoRoot("afora-bundled-runtime-support-");
    const pluginDir = path.join(repoRoot, "extensions", "image-generation-core");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@afora/image-generation-core",
      version: "0.0.1",
      private: true,
      type: "module",
    });
    ...
    expect(
      fs.existsSync(
        path.join(repoRoot, "dist", "extensions", "image-generation-core", "afora.plugin.json"),
      ),
    ).toBe(false);
    expect(
      JSON.parse(
  ...
  it("refuses to remove dist plugin trees through a symlinked dist root", () => {
    const repoRoot = makeRepoRoot("afora-bundled-plugin-meta-symlink-");
    ...
    createPlugin(repoRoot, { id: "acpx", packageName: "@afora/acpx" });
