...
describe("installPluginFromGitSpec", () => {
  ...
  beforeEach(async () => {
    ...
    const globalConfigRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "afora-git-install-npmrc-"),
    );
  ...
  it("does not emit git install success when committing the managed repo fails", async () => {
    const gitRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-git-install-fail-"));
  ...
  it("reports effective install mode for requested git update without an installed target", async () => {
    const gitDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-git-install-mode-"));
    try {
      runCommandWithTimeoutMock
        .mockResolvedValueOnce({ code: 0, stdout: "", stderr: "" })
        .mockResolvedValueOnce({ code: 0, stdout: "abc123\n", stderr: "" })
  ...
  it("stages the clone beside the managed repo so replacement stays on one filesystem (#99885)", async () => {
    const gitDir = trackedTempDirs.make("afora-git-install-stage-");
    try {
      runCommandWithTimeoutMock
        .mockResolvedValueOnce({ code: 0, stdout: "", stderr: "" })
        .mockResolvedValueOnce({ code: 0, stdout: "abc123\n", stderr: "" })
  ...
  it("falls back to the Afora temp root when target workspace creation fails", async () => {
    const gitDir = trackedTempDirs.make("afora-git-install-stage-fallback-");
    runCommandWithTimeoutMock
      .mockResolvedValueOnce({ code: 0, stdout: "", stderr: "" })
      .mockResolvedValueOnce({ code: 0, stdout: "abc123\n", stderr: "" })
      .mockResolvedValueOnce({ code: 0, stdout: "", stderr: "" });
    ...
    try {
      ...
      // withTempDir roots fallback staging at resolvePreferredAforaTmpDir(), which
      // prefers /tmp/afora and only degrades to a uid-scoped os.tmpdir path when
      ...
      expect(path.dirname(expectDefined(fallbackPrefix, "fallbackPrefix test invariant"))).toBe(
        await fs.realpath(resolvePreferredAforaTmpDir()),
      );
  ...
  it("keeps dry-run clone staging out of managed state", async () => {
    const caseDir = trackedTempDirs.make("afora-git-dry-run-stage-");
  ...
  it("uses a credential-free managed repo path for authenticated git URLs", async () => {
    const gitDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-git-install-path-"));
    try {
      runCommandWithTimeoutMock
        .mockResolvedValueOnce({ code: 0, stdout: "", stderr: "" })
        .mockResolvedValueOnce({ code: 0, stdout: "abc123\n", stderr: "" })
  ...
  it("keeps the existing managed repo when replacement install fails", async () => {
    const gitDir = await fs.mkdtemp(path.join(os.tmpdir(), "afora-git-install-preserve-"));
