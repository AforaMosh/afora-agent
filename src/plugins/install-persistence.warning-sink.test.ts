...
describe("plugin install persistence warning audiences", () => {
  ...
  it("reports missing required configuration without forwarding informational logs", async () => {
    ...
    loadPluginManifestRegistryMock.mockReturnValue({
      plugins: [
        recordPluginManifestInstallOwner(
          {
            id: "workboard",
            manifestPath: `${install.installPath}/afora.plugin.json`,
            configSchema: {
              type: "object",
              required: ["token"],
    ...
    expect(warn).toHaveBeenCalledExactlyOnceWith(
      'Installed plugin "workboard" without enabling it because it requires configuration first. Configure it, then run `afora plugins enable workboard`.',
    );
