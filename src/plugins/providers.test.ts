...
type LoadAforaPlugins = typeof import("./loader.js").loadAforaPlugins;
...
const loadAforaPluginsMock = vi.fn<LoadAforaPlugins>();
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadAforaPluginsMock, "Afora plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadAforaPluginsMock, "Afora plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: AforaConfig = {
    plugins: {},
  };
  const autoEnabledConfig: AforaConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadAforaPlugins: (...args: Parameters<LoadAforaPlugins>) =>
        loadAforaPluginsMock(...args),
