...
const resolveAforaPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/afora-root.js", () => ({
  resolveAforaPackageRootSync: (...args: unknown[]) =>
    resolveAforaPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSyncSuiteTempRootTracker("afora-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      afora: params.afora ?? { extensions: ["./dist/index.js"] },
      ...(params.dependency
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootAfora) {
      lockPackages["node_modules/afora"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalAforaPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "afora"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.afora ? { afora: viewPackage.afora } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalAforaPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "afora") {
            const aforaRoot = path.join(npmRoot, "node_modules", "afora");
            fs.mkdirSync(aforaRoot, { recursive: true });
            fs.writeFileSync(
              path.join(aforaRoot, "package.json"),
              JSON.stringify({ name: "afora", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootAfora) {
            ...
            fs.writeFileSync(
              path.join(aforaRoot, "package.json"),
              JSON.stringify({ name: "afora", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "afora") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "afora"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveAforaPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "afora", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveAforaPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "afora-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@afora/pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
  ...
  const dependencySpec = managedManifest.dependencies?.["@afora/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  ...
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@afora/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@afora/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("afora-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@afora/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_afora-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_afora-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@afora/pack-demo";
    const archiveV1Path = path.join(stateDir, "afora-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "afora-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@afora/pack-demo";
    const archivePath = path.join(stateDir, "afora-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .afora/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@afora/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@afora/voice-call@0.0.1");
  ...
  it("rejects npm installs when the installed artifact drifts from verified metadata", async () => {
    ...
    expect(fs.existsSync(path.join(npmProjectRoot, "_afora-quarantined-npm-projects"))).toBe(
      false,
    );
  ...
  it("quarantines incomplete integrity metadata and rebuilds the managed project once", async () => {
    ...
    expect(
      fs.readdirSync(path.join(npmProjectRoot, "_afora-quarantined-npm-projects")),
    ).toHaveLength(1);
  });
  ...
  it("does not restore a quarantined tree when post-recovery validation fails", async () => {
    ...
    const addPeerAfterQuarantine = () => {
      ...
      manifest.afora ??= {};
      manifest.afora.managedPeerDependencies = [addedPeerName];
    ...
    const quarantineParent = path.join(npmProjectRoot, "_afora-quarantined-npm-projects");
    ...
    expect(manifest.afora?.managedPeerDependencies ?? []).not.toContain(addedPeerName);
  });
  ...
  it("repairs omitted current-platform packages with a fresh npm cache", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@afora/codex-fixture";
    ...
    expect(result.ok).toBe(true);
    expect(managedInstallAttempts).toBe(2);
    expect(repairCacheDir).toContain("afora-npm-cache-");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@afora/voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    const stalePackageDir = path.join(npmProjectRoot, "node_modules", "stale-plugin");
    fs.mkdirSync(stalePackageDir, { recursive: true });
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed afora peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "afora")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/afora");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "afora")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their afora peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveAforaPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@afora/codex@2026.5.7",
      packageName: "@afora/codex",
      version: "2026.5.7",
      pluginId: "@afora/codex",
      npmRoot,
      peerDependencies: { afora: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@afora/codex");
    expect(result.error).toContain("plugin-local node_modules/afora link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate afora package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@afora/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@afora/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("AFORA_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@afora/whatsapp@2026.5.27",
      packageName: "@afora/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { afora: ">=2026.5.27" },
      afora: {
        extensions: ["./dist/index.js"],
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@afora", "whatsapp"))).toBe(false);
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@afora/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @afora/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@afora/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@afora/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@afora/msteams@2026.5.28-beta.3",
        packageName: "@afora/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@afora/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @afora/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@afora/msteams",
    ...
    expect(managedManifest.dependencies?.["@afora/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root afora materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "afora"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/afora"]).toBeUndefined();
      expect(
        fs.lstatSync(path.join(requiredPeerPluginDir, "node_modules", "afora")).isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed afora root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
    ...
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "afora"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/afora": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/afora/-/afora-2026.5.4.tgz",
            },
          },
          dependencies: {
            afora: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "afora", "package.json"),
      JSON.stringify({
        name: "afora",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      ...
      pluginId: "discord",
      npmRoot,
      peerDependencies: { afora: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("afora");
    expect(manifest.dependencies?.["@afora/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/afora"]).toBeUndefined();
    expect(lockfile.dependencies?.afora).toBeUndefined();
  });

  it("preserves the active host afora runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "afora");
    ...
    resolveAforaPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/afora-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/afora-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.afora).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/afora-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/afora-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/afora-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/afora-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@afora/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "afora"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "afora")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local afora peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@afora/codex",
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@afora/codex": "0.0.1" },
      packages: [
        {
          packageName: "@afora/codex",
          version: "0.0.1",
    ...
    const peerLink = path.join(installedDir, "node_modules", "afora");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@afora/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
              name: "@afora/codex",
              version: "0.0.2",
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmProjectRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; afora?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            ...
            expect(manifest.afora?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
              "range-target@>1",
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official Afora package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("accepts a trusted manifest-declared plugin id replacement during update", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    mockNpmViewAndInstall({
      spec: "@afora/fish-audio-speech@2026.8.1-beta.0",
      packageName: "@afora/fish-audio-speech",
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@afora/voice-call");
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@afora/voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@afora/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@afora/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@afora/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @afora/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@afora/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@afora/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @afora/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@afora/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@afora/voice-call",
