...
async function withHostHookState(
  ...
  const stateDir = await fs.mkdtemp(path.join(resolvePreferredAforaTmpDir(), prefix));
...
describe("host-hook fixture plugin contract", () => {
  ...
  it("keeps repeated middleware runtime and matcher scopes paired", async () => {
    ...
    registerTestPlugin({
      ...
      record: createPluginRecord({
        ...
        origin: "bundled",
        contracts: { agentToolResultMiddleware: ["afora", "codex"] },
      }),
      register(api) {
    ...
    await registration.handler({ ...event, toolName: "apply_patch" }, { runtime: "afora" });
    await registration.handler({ ...event, toolName: "exec" }, { runtime: "afora" });
  ...
  it("allows the official npm Codex plugin to keep /codex command ownership", () => {
    const { config, registry } = createPluginRegistryFixture();
    const codexRoot = path.join("/tmp", ".afora", "npm", "node_modules", "@afora", "codex");
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("allows the official ClawHub Codex plugin to keep /codex command ownership", () => {
    const { config, registry } = createPluginRegistryFixture();
    const codexRoot = path.join("/tmp", ".afora", "extensions", "codex");
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("scopes trusted policies through canonical Afora tool ids", async () => {
  ...
  it("requires explicit unset to remove plugin session extension state", async () => {
    ...
    await withHostHookState("afora-host-hooks-patch-", async ({ storePath, tempConfig }) => {
  ...
  it("reports duplicate next-turn injections as not newly enqueued", async () => {
    await withHostHookState("afora-host-hooks-injection-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("preserves global enqueue order when draining live next-turn injections", async () => {
    ...
    setActivePluginRegistry(registry);
    await withHostHookState("afora-host-hooks-order-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("cleans plugin-owned session state and lifecycle resources on reset/disable", async () => {
    ...
    await withHostHookState("afora-host-hooks-state-", async ({ tempConfig }) => {
  ...
  it("removes persistent plugin-owned session state and pending injections during cleanup", async () => {
    ...
    await withHostHookState("afora-host-hooks-store-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("does not clear unrelated run context during session-scoped cleanup", async () => {
    ...
    await withHostHookState("afora-host-hooks-run-context-", async ({ tempConfig }) => {
      await runPluginHostCleanup({
        cfg: tempConfig,
        registry,
        reason: "reset",
  ...
  it("preserves durable plugin session state during plugin restart cleanup", async () => {
    ...
    await withHostHookState(
      "afora-host-hooks-restart-state-",
      async ({ storePath, tempConfig }) => {
        await updateSessionStore(storePath, (store) => {
          store["agent:main:main"] = {
            sessionId: "session-1",
  ...
  it("cleans pending injections for plugins that registered no host-hook callbacks", async () => {
    ...
    await withHostHookState(
      "afora-host-hooks-injection-only-",
      async ({ storePath, tempConfig }) => {
        await updateSessionStore(storePath, (store) => {
          store["agent:main:main"] = {
            sessionId: "session-1",
