...
const FORBIDDEN_CONTRACT_MODULE_SPECIFIER_PATTERNS = [
  /^vitest$/u,
  /^afora-agent\/plugin-sdk\/testing$/u,
  /(^|\/)test-api(?:\.[cm]?[jt]s)?$/u,
  /(^|\/)__tests__(\/|$)/u,
  /(^|\/)test-support(\/|$)/u,
  /(^|\/)[^/]*\.test(?:[-.][^/]*)?(?:\.[cm]?[jt]s)?$/u,
...
function analyzeSourceModule(params: { filePath: string; source: string }): {
  ...
  for (const match of source.matchAll(STATIC_FROM_IMPORT_RE)) {
    ...
    if (specifier === "afora-agent/plugin-sdk/core" && importsDefinePluginEntry(importClause)) {
    ...
    for (const plugin of listBundledPluginRoots()) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          afora?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.afora?.extensions) ? pkg.afora.extensions : [];
        if (
          extensions.some(
  ...
  it("follows relative import edges while scanning guarded contract graphs", () => {
    expect(
      analyzeSourceModule({
        ...
        import { y } from "afora-agent/plugin-sdk/core";
      `,
  ...
  it("detects aliased definePluginEntry imports from core", () => {
    expect(
      analyzeSourceModule({
        ...
        source: `
          import { definePluginEntry as dpe } from "afora-agent/plugin-sdk/core";
          import { somethingElse } from "afora-agent/plugin-sdk/core";
        `,
      }).importsDefinePluginEntryFromCore,
    ).toBe(true);
  });
