...
import { FILE_TYPE_SNIFF_MAX_BYTES } from "@afora/media-core/mime";
import { registerTestPlugin } from "afora-agent/plugin-sdk/plugin-test-contracts";
...
async function withSessionStore(
  ...
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredAforaTmpDir(), "afora-session-attachments-"),
  );
...
describe("plugin session attachments", () => {
  afterEach(() => {
    ...
    delete (globalThis as { proofAttachmentApi?: AforaPluginApi }).proofAttachmentApi;
  ...
  it("uses the live runtime config when a captured API sends an attachment", async () => {
    await withSessionStore(async ({ stateDir, storePath, filePath }) => {
      ...
      let capturedApi: AforaPluginApi | undefined;
      registerTestPlugin({
        registry,
