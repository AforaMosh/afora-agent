...
const fixtureRoot = makeTrackedTempDir("afora-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadAforaPlugins, clearPluginLoaderCache } = await import("./loader.test-fixtures.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "afora.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("afora plugins inspect <id> --runtime --json");
    expect(summary).toContain("afora plugins list");
  });
});
