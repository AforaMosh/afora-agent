...
function writeDistPluginFile(repoRoot: string, root: "dist" | "dist-runtime", pluginId: string) {
  ...
  fs.writeFileSync(path.join(pluginDir, "afora.plugin.json"), "{}\n", "utf8");
}
...
describe("pruneDockerPluginDist", () => {
  ...
  it("removes package-excluded plugin runtime artifacts unless Docker explicitly opts it in", () => {
    const repoRoot = makeRepoRoot("afora-docker-plugin-dist-");
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: { AFORA_EXTENSIONS: "diagnostics-otel" } as NodeJS.ProcessEnv,
    });
  ...
  it("honors custom bundled plugin source roots when pruning Docker runtime importers", () => {
    const repoRoot = makeRepoRoot("afora-docker-plugin-source-");
    writeJsonFile(path.join(repoRoot, "package.json"), {
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: {
        AFORA_BUNDLED_PLUGIN_DIR: "plugins",
  ...
  it("removes node_modules dependency closure that only omitted Docker plugins need", () => {
    const repoRoot = makeRepoRoot("afora-docker-plugin-node-modules-");
    writeJsonFile(path.join(repoRoot, "package.json"), {
    ...
    writeNodePackage(repoRoot, "@afora/acpx");
    writeNodePackage(repoRoot, "@afora/codex");
    ...
    const removed = pruneDockerPluginDist({
      repoRoot,
      env: { AFORA_EXTENSIONS: "codex" } as NodeJS.ProcessEnv,
    });

    expect(removed).toEqual([
      "node_modules/@afora/acpx",
