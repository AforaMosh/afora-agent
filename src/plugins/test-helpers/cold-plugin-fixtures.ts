...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/afora-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "afora.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): AforaConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    AFORA_HOME: path.join(homeDir, "home"),
    AFORA_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
