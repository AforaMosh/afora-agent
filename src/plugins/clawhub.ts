...
function isTrustedSourceLinkedOfficialPackage(pkg: NonNullable<ClawHubPackageDetail["package"]>) {
  ...
  return (
    ...
    (sourceRepo === "AforaMosh/afora-agent" ||
      sourceRepo === "github.com/AforaMosh/afora-agent" ||
      sourceRepo === "https://github.com/AforaMosh/afora-agent")
  );
}
...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    ...
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "afora skills install ${installRef}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Afora runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires Afora >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
...
> {
  ...
  try {
    ...
    const clawhubAuthority = isDefaultClawHubBaseUrl(params.baseUrl) ? "afora" : "third-party";
    params.logger?.info?.(
