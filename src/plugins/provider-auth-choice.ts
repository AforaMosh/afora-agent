...
function resolveConfiguredDefaultModelPrimary(cfg: AforaConfig): string | undefined {
...
async function applyDefaultModelFromAuthChoice(params: {
  config: AforaConfig;
  configBeforeProviderAuth?: AforaConfig;
  selectedModel: string;
  selectedModelDisplay?: string;
  preserveExistingDefaultModel: boolean | undefined;
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  workspaceDir?: string;
  runSelectedModelHook: (config: AforaConfig) => Promise<void>;
}): Promise<AforaConfig> {
...
export async function runProviderPluginAuthMethod(params: {
  ...
  allowSecretRefPrompt?: boolean;
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: AforaConfig; defaultModel?: string }> {
  ...
  const persistAuthProfiles = async (profiles = result.profiles) => {
    ...
    for (const profile of profiles) {
      ...
      await upsertAuthProfileWithLockOrThrow({
        ...
        credential,
        agentDir,
        stateDir: params.env?.AFORA_STATE_DIR,
      });
    }
...
export async function prepareAuthChoiceLoadedPluginProvider(
  ...
  const resolveScopedRuntimeProviders = (config: AforaConfig): ProviderPlugin[] =>
    resolvePluginProviders({
      config,
      workspaceDir,
      env: params.env,
