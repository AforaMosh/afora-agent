...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("afora-marketplace-download-"),
    )
...
async function withKnownMarketplaceRegistry<T>(
  ...
  return await withTempDir("afora-marketplace-known-", async (homeDir) => {
    const aforaHome = path.join(homeDir, "afora-home");
    ...
    return await withEnvAsync({ HOME: homeDir, AFORA_HOME: aforaHome }, async () =>
      run(homeDir),
    );
  });
}
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("afora-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("afora-marketplace-test-", async (homeDir) => {
      const aforaHome = path.join(homeDir, "afora-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, AFORA_HOME: aforaHome },
