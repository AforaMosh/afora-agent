...
import { linkAforaPeerDependencies } from "./plugin-peer-link.js";
...
> {
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid afora.plugin.json: ${ocManifestResult.error}`,
  ...
  const compatibilityError = validateAforaPackageInstallCompatibility({
  ...
  const extensionsResult = ensureAforaExtensions({
    manifest,
  });
...
export async function scanAndLinkInstalledPackage(params: {
  ...
  const peerLinkRepair = await linkAforaPeerDependencies({
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedAforaPeerLinkError(params.pluginId),
    };
  }
  return null;
}
