...
import {
  installAforaInternalCorePackageNativeResolver,
  installAforaPluginSdkNativeResolver,
...
function writeFakeAforaPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "afora.mjs"), "#!/usr/bin/env node\n", "utf8");
...
function createInternalCoreAliasFixture(prefix: string): {
  ...
  const { loaderModulePath } = writeFakeAforaPackage(root);
...
describe("installAforaInternalCorePackageNativeResolver", () => {
  it("shares one internal core alias scan between resolver installers", () => {
    const fixture = createInternalCoreAliasFixture("afora-sdk-native-core-cache-");
    ...
    try {
      ...
      const aliases = installAforaInternalCorePackageNativeResolver({
        moduleUrl: fixture.moduleUrl,
      });

      expect(aliases).toContain("@afora/markdown-core/code-spans");
  ...
  it("shares one internal core alias scan across importers from the same host package", () => {
    const fixture = createInternalCoreAliasFixture("afora-sdk-native-core-shared-host-");
    ...
    try {
      installAforaInternalCorePackageNativeResolver({ moduleUrl: fixture.moduleUrl });
      expect(existsSync).toHaveBeenCalledWith(fixture.sourcePath);
      ...
      existsSync.mockClear();
      readFileSync.mockClear();
      installAforaInternalCorePackageNativeResolver({ moduleUrl: secondModuleUrl });
  ...
  it("keeps internal core alias registration isolated between host modules", () => {
    const first = createInternalCoreAliasFixture("afora-sdk-native-core-host-a-");
    const second = createInternalCoreAliasFixture("afora-sdk-native-core-host-b-");
    const existsSync = vi.spyOn(fs, "existsSync");

    try {
      installAforaInternalCorePackageNativeResolver({ moduleUrl: first.moduleUrl });
      existsSync.mockClear();

      installAforaInternalCorePackageNativeResolver({ moduleUrl: second.moduleUrl });

      expect(existsSync).toHaveBeenCalledWith(second.sourcePath);
      expect(
        fs.realpathSync(
          createRequire(first.coreSourceParent).resolve("@afora/markdown-core/code-spans"),
        ),
      ).toBe(fs.realpathSync(first.sourcePath));
      expect(
        fs.realpathSync(
          createRequire(second.coreSourceParent).resolve("@afora/markdown-core/code-spans"),
        ),
  ...
  it("rescans internal core aliases after plugin metadata lifecycle invalidation", () => {
    const fixture = createInternalCoreAliasFixture("afora-sdk-native-core-invalidation-");
    const existsSync = vi.spyOn(fs, "existsSync");
...
describe("installAforaPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeAforaPackage(stableRoot);
    writeFakeAforaPackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.AFORA_DEV_SOURCE_ROOT;
    process.env.AFORA_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installAforaPluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("afora-agent/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("afora-agent/plugin-sdk/agent-runtime"))).toBe(
        fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")),
      );
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.AFORA_DEV_SOURCE_ROOT;
      } else {
        process.env.AFORA_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("afora-agent/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("afora-agent/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeAforaPackage(root);
    ...
    expect(installedAliases).toContain("afora-agent/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("afora-agent/plugin-sdk/channel-outbound"))).toBe(
      fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")),
    );
  });

  it("lets built external plugins resolve Afora SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeAforaPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("afora-agent/plugin-sdk/channel-outbound")),
      ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
      const sdk = requireFromPlugin("afora-agent/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("afora-agent/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installAforaPluginSdkNativeResolver } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"afora-agent/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"afora-agent/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installAforaPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeAforaPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("afora-agent/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("afora-agent/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for Afora-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeAforaPackage(root);
    ...
    expect(installedAliases).toContain("@afora/normalization-core/string-coerce");
    expect(installedAliases).toContain("@afora/normalization-core/boolean-coercion");
    expect(installedAliases).toContain("@afora/normalization-core/result");
    expect(installedAliases).toContain("@afora/normalization-core/agent-id");
    expect(installedAliases).toContain("@afora/media-core/mime");
    expect(installedAliases).toContain("@afora/media-core/attachment-classify");
    expect(installedAliases).toContain("@afora/markdown-core/code-spans");
    expect(installedAliases).toContain("@afora/ai/transports");
    expect(installedAliases).toContain("@afora/ai/internal/openai-responses-payload-policy");
    expect(installedAliases).toContain("@afora/ai/internal/retry-after");
    expect(installedAliases).toContain("@afora/ai/internal/runtime");
    expect(installedAliases).toContain("@afora/acp-core/runtime/types");
    expect(installedAliases).toContain("@afora/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@afora/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(
      fs.realpathSync(
        requireFromCoreSource.resolve("@afora/normalization-core/boolean-coercion"),
      ),
    ).toBe(fs.realpathSync(booleanCoercionSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@afora/normalization-core/result")),
    ).toBe(fs.realpathSync(resultSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@afora/normalization-core/agent-id")),
    ).toBe(fs.realpathSync(agentIdSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@afora/media-core/mime"))).toBe(
      fs.realpathSync(mediaMimeSource),
    );
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@afora/media-core/attachment-classify")),
    ).toBe(fs.realpathSync(mediaAttachmentClassifySource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@afora/markdown-core/code-spans")),
    ).toBe(fs.realpathSync(markdownCoreSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@afora/ai/transports"))).toBe(
      fs.realpathSync(aiTransportsSource),
    );
    expect(
      fs.realpathSync(
        requireFromCoreSource.resolve("@afora/ai/internal/openai-responses-payload-policy"),
      ),
    ).toBe(fs.realpathSync(aiResponsesPayloadPolicySource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@afora/ai/internal/retry-after")),
    ).toBe(fs.realpathSync(aiRetryAfterSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@afora/ai/internal/runtime"))).toBe(
      fs.realpathSync(aiRuntimeSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@afora/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@afora/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@afora/normalization-core/string-coerce")).toThrow();
    expect(() =>
      requireFromPlugin.resolve("@afora/normalization-core/boolean-coercion"),
    ).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/normalization-core/result")).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/media-core/attachment-classify")).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/markdown-core/code-spans")).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/ai/transports")).toThrow();
    expect(() =>
      requireFromPlugin.resolve("@afora/ai/internal/openai-responses-payload-policy"),
    ).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/ai/internal/retry-after")).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/ai/internal/runtime")).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@afora/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeAforaPackage(root);
    ...
    expect(installedAliases).toContain("afora-agent/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("afora-agent/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("afora-agent/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "afora-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeAforaPackage(root);
    ...
    expect(installedAliases).toContain("afora-agent/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("afora-agent/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("afora-agent/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("afora-agent/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("afora-agent/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("afora-agent/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});
