...
function formatPluginInspectCommand(pluginId: string): string {
  return `afora plugins inspect ${quoteCliArg(pluginId)}`;
}
...
export function warnWhenAllowlistIsOpen(params: {
  ...
  const remediation = truncated
    ? "Run 'afora plugins list --enabled --verbose' to enumerate every discovered plugin id, inspect trusted ids with 'afora plugins inspect <id>', and add the ones you trust to plugins.allow in afora.json."
    : `To trust them explicitly, set plugins.allow in afora.json (e.g. "plugins": { "allow": [${autoDiscoverable
        .map((entry) => JSON.stringify(entry.id))
        .join(
          ", ",
        )}] }). Run 'afora plugins list --enabled --verbose' or ${inspectCommands} to confirm plugin ids.`;
