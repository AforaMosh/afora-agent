...
function collectConfiguredPluginEntryIds(config: AforaConfig): string[] {
...
function resolveRegister(mod: AforaPluginModule): {
...
function resolveSetupRegistration(
  ...
  let mod: AforaPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as AforaPluginModule;
  } catch (error) {
  ...
  const resolved = resolveRegister((mod as { default?: AforaPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Afora ignored the plugin setup runtime entry. Remove setup-api/afora.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
