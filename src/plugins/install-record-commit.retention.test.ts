...
describe("retained managed npm record commits", () => {
  it("suppresses recovery when a retained install record is removed", async () => {
    await withAforaTestState({ label: "retained-record-removal" }, async (state) => {
      const packageName = "@afora/retained-demo";
  ...
  it.each(["direct", "symlink"] as const)(
    ...
    async (activePathKind) => {
      await withAforaTestState({ label: `retained-active-${activePathKind}` }, async (state) => {
        const packageName = "@afora/retained-active";
        const installPath = writeManagedNpmPlugin({
          stateDir: state.stateDir,
          packageName,
  ...
  it("does not retire a removed npm record outside the managed npm root", async () => {
    await withAforaTestState({ label: "retained-outside-root" }, async (state) => {
      const outsideRoot = state.path("outside-root");
      try {
        const packageName = "@afora/outside-retained";
        const installPath = writeManagedNpmPlugin({
  ...
  it("keeps npm-to-local source changes cleanup-eligible", async () => {
    await withAforaTestState({ label: "retained-source-change" }, async (state) => {
      const packageName = "@afora/moved-local";
      const installPath = writeManagedNpmPlugin({
        stateDir: state.stateDir,
        packageName,
