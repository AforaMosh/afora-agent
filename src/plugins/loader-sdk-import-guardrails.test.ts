...
const ALLOWED_PLUGIN_SDK_FIXTURE_IMPORTS = new Set([
  ...
  'src/plugins/loader.git-path-regression.test.ts:`import { resolveOutboundSendDep } from "afora-agent/plugin-sdk/channel-outbound";',
  'src/plugins/loader.git-path-regression.test.ts:          "afora-agent/plugin-sdk/channel-outbound": ${JSON.stringify(copiedChannelRuntimeShim)},',
  // Intentional packaged bundled-plugin SDK alias regression tests.
  'src/plugins/loader.test.ts:`import { normalizeLowercaseStringOrEmpty } from "afora-agent/plugin-sdk/string-coerce-runtime";`,',
]);
...
function findLoaderFixtureSdkImports(): string[] {
  ...
  for (const file of LOADER_FIXTURE_TEST_FILES) {
    ...
    for (const line of source.split("\n")) {
      if (
        line.includes('require("afora-agent/plugin-sdk') ||
        (line.includes("import ") && line.includes('"afora-agent/plugin-sdk'))
      ) {
