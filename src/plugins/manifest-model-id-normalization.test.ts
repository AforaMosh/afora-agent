  ...
  writePersistedInstalledPluginIndexSync(
    {
      ...
      plugins: [
        {
          pluginId: "normalizer",
          manifestPath: path.join(params.pluginDir, "afora.plugin.json"),
...
describe("manifest model id normalization", () => {
  ...
  it("reflects manifest edits and state directory changes without a prepared snapshot", () => {
    const stateDirA = tempDirs.make("afora-model-id-normalization-");
    ...
    setTestEnvValue("AFORA_STATE_DIR", stateDirA);
    ...
    const stateDirB = tempDirs.make("afora-model-id-normalization-");
  ...
  it("reuses manifest metadata for the same environment identity", () => {
    ...
    const listNormalizerRecords = () =>
      listAforaPluginManifestMetadata(process.env).filter(
