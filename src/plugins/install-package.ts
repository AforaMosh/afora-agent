...
async function detectNativePackageInstallSource(
  ...
  return manifest && ensureAforaExtensions({ manifest }).ok ? manifest : undefined;
}
...
export async function installPluginFromPath(
  ...
  return {
    ...
    error:
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains afora.plugin.json, or list standalone plugin files in plugins.load.paths.",
  };
}
