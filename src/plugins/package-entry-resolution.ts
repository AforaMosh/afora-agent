...
function runtimeExtensionsLengthMismatchMessage(params: {
  ...
  return (
    `package.json afora.runtimeExtensions length (${params.runtimeExtensionsLength}) ` +
    `must match afora.extensions length (${params.extensionsLength})`
  );
}
...
function resolvePackageRuntimeExtensionEntries(params: {
  ...
  const runtimeExtensionsResult = readPackageManifestStringList({
    fieldName: "afora.runtimeExtensions",
