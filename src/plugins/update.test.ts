...
import { bundledPluginRootAt } from "afora-agent/plugin-sdk/test-fixtures";
...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-afora",
    targetDir: params?.targetDir ?? "/tmp/opik-afora",
...
function createSuccessfulClawHubUpdateResult(params?: {
  ...
  return {
    ...
    targetDir: params?.targetDir ?? "/tmp/afora-plugins/legacy-chat",
...
function createEnabledDemoClawHubInstallConfig(): AforaConfig {
...
function createInstalledPackageDir(params: {
  ...
  const dir =
    params.installPath ?? fs.mkdtempSync(path.join(os.tmpdir(), "afora-plugin-update-test-"));
    ...
    JSON.stringify(
      {
        ...
        ...(params.runnable ? { afora: { extensions: ["./index.js"] } } : {}),
      },
      null,
      2,
    ),
...
function createAforaPeerLinkFixtures(plugins: Array<{ pluginId: string; packageName: string }>) {
  const stateDir = makeTrackedTempDir("afora-plugin-update-owner", tempDirs);
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { afora: ">=2026.5.4" },
...
function mockNpmViewMetadata(params: {
  ...
  runCommandWithTimeoutMock.mockResolvedValueOnce({
    ...
    stdout: JSON.stringify({
      ...
      ...(params.afora ? { afora: params.afora } : {}),
    }),
    stderr: "",
  });
}
...
function createNpmUpdateFixture(params: {
  ...
  registryAfora?: Record<string, unknown>;
  spec?: string;
  resolvedSpec?: string;
  integrity?: string;
  shasum?: string;
  ...
  if (params.registryVersion) {
    mockNpmViewMetadata({
      ...
      ...(params.registryAfora ? { afora: params.registryAfora } : {}),
    });
  }
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@afora/${pluginId}`,
  };
}
...
>[number];
type PluginInstallRecord = NonNullable<NonNullable<AforaConfig["plugins"]>["installs"]>[string];

function createDisabledPluginConfig(install: PluginInstallRecord): AforaConfig {
  return {
    plugins: {
...
function expectCodexAppServerInstallState(params: {
  ...
  const install = params.result.config.plugins?.installs?.["afora-codex-app-server"];
  expect(install?.source).toBe("npm");
  expect(install?.spec).toBe(params.spec);
  expect(install?.installPath).toBe("/tmp/afora-codex-app-server");
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("does not treat inherited prototype names as install records", async () => {
    const config: AforaConfig = { plugins: { installs: {} } };
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-afora",
        spec: "@opik/opik-afora",
        integrity: "sha512-old",
        installPath: "/tmp/opik-afora",
      }),
      pluginIds: ["opik-afora"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-afora",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        ...
        spec: "afora-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/afora-codex-app-server",
      }),
      pluginIds: ["afora-codex-app-server"],
      specOverrides: {
        "afora-codex-app-server": "afora-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "afora-codex-app-server",
        targetDir: "/tmp/afora-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "afora-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      installerResolvedSpec: "@afora/acpx@2026.5.2-beta.2",
    });

    const result = await updatePlugin(config, "acpx", { syncOfficialPluginInstalls: true });

    expect(npmInstallCall()?.spec).toBe("@afora/acpx");
    expect(npmInstallCall()?.expectedPluginId).toBe("acpx");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@afora/acpx@2026.5.2-beta.2");
  });
  ...
  it.each([
    ...
    async ({ channel, configuredChannel, registryVersion, expectedSpec }) => {
      const { config } = createNpmUpdateFixture({
        ...
        installerResolvedSpec: `@afora/codex@${registryVersion}`,
      });
      ...
      expect(result.config.plugins?.installs?.codex?.spec).toBe("@afora/codex");
      expect(result.config.plugins?.installs?.codex?.resolvedSpec).toBe(
        `@afora/codex@${registryVersion}`,
      );
    },
  );
  ...
  it("retains the default fallback for a targeted official beta update", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ...
        error: "No matching version found for @afora/codex@beta",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "codex",
    ...
    expect(npmInstallCall(0)?.spec).toBe("@afora/codex@beta");
    expect(npmInstallCall(1)?.spec).toBe("@afora/codex");
    expect(result.config.plugins?.installs?.codex?.spec).toBe("@afora/codex");
  });
  ...
  it("pins unchanged official npm records during official sync", async () => {
    ...
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@afora/acpx@2026.5.2");
  ...
  it("keeps integrity drift checks for exact official pins during official sync", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      spec: "@afora/acpx@2026.5.2",
      integrity: "sha512-old",
      installerVersion: "2026.5.2",
      installerResolvedSpec: "@afora/acpx@2026.5.2",
    });
  ...
  it("skips integrity checks when official sync may choose a compatible fallback", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      registryVersion: "2026.5.2",
      registryIntegrity: "sha512-old",
      registryAfora: { compat: { pluginApi: ">=9999.0.0" } },
      spec: "@afora/acpx@2026.5.2",
      integrity: "sha512-old",
      installerVersion: "2026.5.1",
      installerResolvedSpec: "@afora/acpx@2026.5.1",
    });
  ...
  it("keeps integrity drift checks for exact prerelease-only official pins", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      registryIntegrity: "sha512-beta",
      spec: "@afora/voice-call@0.0.2-beta.1",
      integrity: "sha512-old",
      installerVersion: "0.0.2-beta.1",
      installerResolvedSpec: "@afora/voice-call@0.0.2-beta.1",
    });
  ...
  it.each([
      ...
      fallbackMetadata: [
        {
          version: "2026.5.2",
          integrity: "sha512-old",
          afora: { compat: { pluginApi: ">=9999.0.0" } },
        },
      ],
      installerVersion: "2026.5.1",
      expectedIntegrity: undefined,
    ...
    for (const metadata of fallbackMetadata) {
      if ("versions" in metadata && metadata.versions) {
        ...
        mockNpmViewMetadata({ name: "@afora/acpx", ...metadata });
      }
    }
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error: "No matching version found for @afora/acpx@beta",
        code: "npm_package_not_found",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@afora/acpx",
            version: installerVersion,
            resolvedSpec: `@afora/acpx@${installerVersion}`,
          },
        }),
      );
    ...
    expect(npmInstallCall(0)?.spec).toBe("@afora/acpx@beta");
    expect(npmInstallCall(0)?.expectedIntegrity).toBeUndefined();
    expect(npmInstallCall(1)?.spec).toBe("@afora/acpx");
  ...
  it("does not skip trusted official default updates when latest resolves to the installed prerelease", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      registryIntegrity: "sha512-beta",
      registryShasum: "beta",
      spec: "@afora/acpx@2026.5.2-beta.2",
  ...
  it("updates trusted official npm plugins when latest resolves to a stable correction release", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      installerResolvedSpec: "@afora/acpx@2026.5.3-1",
    });
      ...
      expect(result.outcomes).toEqual([
        {
          ...
          message:
            ...
            `Pass \`afora plugins update ${overrideSpec}\` to follow that registry line.`,
        },
      ]);
    },
  );
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          ...
          "Pass `afora plugins update @acme/demo@latest` to follow that registry line.",
      },
    ]);
  });
  ...
  it.each([
    ...
    vi.stubEnv("AFORA_COMPATIBILITY_HOST_VERSION", "2026.5.28-beta.3");
    const { config } = createNpmUpdateFixture({
      pluginId: "msteams",
      ...
      registryAfora: { extensions: ["./dist/index.js"], ...compatibility },
      integrity: "sha512-newer",
      shasum: "newer",
      installerVersion: "2026.5.28-beta.3",
      installerResolvedSpec: "@afora/msteams@2026.5.28-beta.3",
    });

    const result = await updatePlugin(config, "msteams");

    expect(npmInstallCall()?.spec).toBe("@afora/msteams");
    ...
    expectRecordFields(result.config.plugins?.installs?.msteams, {
      ...
      resolvedSpec: "@afora/msteams@2026.5.28-beta.3",
    });
    if (assertFullOutcome) {
      expect(result.outcomes).toEqual([
        {
  ...
  it("repairs missing afora peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@afora/codex",
      version: "2026.5.3",
      peerDependencies: { afora: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@afora/codex",
      version: "2026.5.3",
    ...
    expect(npmInstallCall()?.spec).toBe("@afora/codex");
    expect(npmInstallCall()?.mode).toBe("update");
  ...
  it("skips unchanged npm plugins when the afora peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@afora/codex",
      version: "2026.5.3",
      peerDependencies: { afora: ">=2026.5.3" },
    ...
    fs.symlinkSync(
      ...
      path.join(installPath, "node_modules", "afora"),
      "junction",
    );
    mockNpmViewMetadata({
      name: "@afora/codex",
  ...
  it.each(["peerDependencies", "dependencies"] as const)(
    ...
    async (dependencyField) => {
      const stateDir = makeTrackedTempDir("afora-plugin-update-legacy", tempDirs);
      const plugins = ["email", "calendar"].map((pluginId) => {
        ...
        const staleHostDir = path.join(installPath, "node_modules", "afora");
      ...
      const result = await withEnvAsync({ AFORA_STATE_DIR: stateDir }, async () =>
  ...
  it("repairs afora peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@afora/brave-plugin" },
      { pluginId: "codex", packageName: "@afora/codex" },
      { pluginId: "discord", packageName: "@afora/discord" },
    ];
    const { stateDir, installPaths, peerLinkPath, linkPeer } =
      createAforaPeerLinkFixtures(plugins);
  ...
  it("repairs sibling afora peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@afora/brave-plugin" },
      { pluginId: "codex", packageName: "@afora/codex" },
      { pluginId: "discord", packageName: "@afora/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@afora/codex",
            version: "2026.5.5",
            resolvedSpec: "@afora/codex@2026.5.5",
          },
        }),
      );
    });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const copiedHosts = [
        ...
        const copiedHostDir = path.join(installPath, "node_modules", "afora");
        fs.mkdirSync(copiedHostDir, { recursive: true });
        fs.writeFileSync(
          path.join(copiedHostDir, "package.json"),
          JSON.stringify({ name: "afora", version: "2026.4.1" }),
        );
      ...
      for (const copiedHostDir of copiedHosts) {
        ...
        ).toEqual({ name: "afora", version: "2026.4.1" });
      }
    },
  );

  it("continues repairing sibling afora peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@afora/brave-plugin" },
      { pluginId: "codex", packageName: "@afora/codex" },
    ];
      ...
      updateNpmInstalledPlugins({
        config: createPeerLinkInstallConfig({
          ...
          extraInstalls: {
            ...
            broken: {
              source: "npm",
              spec: "@afora/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@afora/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@afora/broken-plugin@2026.5.4",
            },
          },
        }),
        pluginIds: ["codex"],
    ...
    expect(warnMessages).toEqual([
      expect.stringContaining(
        `Could not repair afora peer link at ${malformedInstallPath}: SyntaxError:`,
      ),
      `Skipping afora peerDependency link because ${path.join(brokenInstallPath, "node_modules")} is not a real directory.`,
    ]);
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "afora-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".afora", "extensions", "lossless-claw");
  ...
  it("disables a corrupt installed payload when metadata probing also fails", async () => {
    ...
    const message =
      'Disabled "lossless-claw" after plugin update failure; Afora will continue without it. Failed to check lossless-claw: npm view failed: registry timeout';
  ...
  it("disables a missing plugin payload when metadata probing also fails", async () => {
    const warn = vi.fn();
    const installPath = fs.mkdtempSync(path.join(os.tmpdir(), "afora-plugin-update-missing-"));
    ...
    const message =
      'Disabled "demo" after plugin update failure; Afora will continue without it. Failed to update demo: npm view failed: registry timeout';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.changed).toBe(true);
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@afora/codex",
          version: "2026.5.4",
          resolvedSpec: "@afora/codex@2026.5.4",
        },
      }),
    );
  ...
  it("preserves exact official npm pins on an inferred beta channel", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      installerResolvedSpec: "@afora/codex@2026.5.28",
    });
    const result = await updatePlugin(config, "codex", {
      dryRun: true,
      officialPluginUpdateChannel: "beta",
    ...
    expect(npmInstallCall()?.spec).toBe("@afora/codex@2026.5.28");
    expect(npmInstallCall()?.expectedPluginId).toBe("codex");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.changed).toBe(false);
    expectRecordFields(result.outcomes[0], {
  ...
  it("reinstalls missing exact official npm pins without official install sync", async () => {
    const extensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-missing-plugin-"));
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@afora/codex",
          version: "2026.5.28",
          resolvedSpec: "@afora/codex@2026.5.28",
        },
      }),
    );
  ...
  it.each(["@acme/demo@1.2.3", "@acme/demo@v1.2.3"])(
    ...
    async (spec) => {
      ...
      expectRecordFields(result.outcomes[0], {
        ...
        currentVersion: "1.2.3",
        nextVersion: "1.2.4",
        message: `demo is pinned to ${spec} (installed 1.2.3); registry latest resolves to 1.2.4. Pass \`afora plugins update @acme/demo@latest\` to follow that registry line.`,
      });
    },
  );
  ...
  it("updates disabled trusted official ClawHub installs through the catalog spec", async () => {
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@afora/diagnostics-otel");
  ...
  it("updates bare trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@afora/diagnostics-prometheus",
      }),
    );
        ...
        plugins: {
          installs: {
            "diagnostics-prometheus": {
              source: "clawhub",
              spec: "clawhub:@afora/diagnostics-prometheus@2026.5.3",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@afora/diagnostics-prometheus");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-prometheus"], {
      source: "clawhub",
      spec: "clawhub:@afora/diagnostics-prometheus",
      version: "2026.5.4",
      clawhubPackage: "@afora/diagnostics-prometheus",
      clawhubChannel: "official",
    });
  ...
  it("does not skip a risk-gated ClawHub update when the installed package is missing", async () => {
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:demo");
    const message =
      'Disabled "demo" after plugin update failure; Afora will continue without it. Failed to update demo: Update cancelled; rerun with --acknowledge-clawhub-risk to continue after reviewing the warning. (ClawHub clawhub:demo).';
    expect(result.changed).toBe(true);
    expect(result.config.plugins?.entries?.demo).toEqual({
      enabled: false,
      config: { preserved: true },
  ...
  it("disables a blocked ClawHub plugin without changing trust policy", async () => {
    ...
    const message =
      'Disabled "demo" after plugin update failure; Afora will continue without it. Failed to update demo: ClawHub blocked this release; update was not started. (ClawHub clawhub:demo).';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.outcomes).toEqual([
      {
        pluginId: "demo",
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
      ...
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            ...
            resolvedSpec: "@opik/opik-afora@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-afora@0.2.5",
          };
        }
    ...
    const result = await updatePlugin(config, "opik-afora", { logger: { warn } });

    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-afora" (@opik/opik-afora@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-afora: aborted: npm package integrity drift detected for @opik/opik-afora@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ok: false,
        code: "npm_package_not_found",
        error: "Package not found on npm: @afora/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@afora/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @afora/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedSpec: "afora-codex-app-server@0.2.0-beta.3",
      }),
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "afora-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "afora-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
      ...
      specOverrides: {
        "afora-codex-app-server": "afora-codex-app-server@beta",
      },
      expectedSpec: "afora-codex-app-server@beta",
      expectedRecordSpec: "afora-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "afora-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
  ...
  it("preserves explicit official npm tag overrides during manual updates", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      installerResolvedSpec: "@afora/acpx@2026.5.3-beta.1",
    });
    const result = await updatePlugin(config, "acpx", {
      specOverrides: { acpx: "@afora/acpx@beta" },
    });

    expectNpmUpdateCall({
      spec: "@afora/acpx@beta",
      expectedPluginId: "acpx",
    });
    expectRecordFields(result.config.plugins?.installs?.acpx, {
      spec: "@afora/acpx@beta",
      version: "2026.5.3-beta.1",
      resolvedSpec: "@afora/acpx@2026.5.3-beta.1",
    });
  });
  ...
  it("targets the exact core version for official extended-stable updates and preserves intent", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      installerResolvedSpec: "@afora/acpx@2026.7.33",
    });
    const result = await updatePlugin(config, "acpx", {
      syncOfficialPluginInstalls: true,
    ...
    expectNpmUpdateCall({
      spec: "@afora/acpx@2026.7.33",
      expectedPluginId: "acpx",
    });
    expectRecordFields(result.config.plugins?.installs?.acpx, {
      spec: "@afora/acpx",
      version: "2026.7.33",
      resolvedSpec: "@afora/acpx@2026.7.33",
    });
  });
  ...
  it("lets an explicit bare official spec opt a legacy pin into exact-core tracking", async () => {
    const { config } = createNpmUpdateFixture({
      ...
      registryVersion: "2026.7.33",
      spec: "@afora/acpx@2026.6.21",
      installerVersion: "2026.7.33",
      installerResolvedSpec: "@afora/acpx@2026.7.33",
    });
    const result = await updatePlugin(config, "acpx", {
      specOverrides: { acpx: "@afora/acpx" },
      syncOfficialPluginInstalls: true,
      officialPluginUpdateChannel: "extended-stable",
      coreVersion: "2026.7.33",
    });
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for afora-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "afora-codex-app-server",
          ...
          npmResolution: {
            name: "afora-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "afora-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    const warnMessages: string[] = [];
    const result = await updatePlugin(config, "afora-codex-app-server", {
      updateChannel: "beta",
      logger: { warn: (msg) => warnMessages.push(msg) },
    });

    expect(npmInstallCall(0)?.spec).toBe("afora-codex-app-server@beta");
    expect(npmInstallCall(1)?.spec).toBe("afora-codex-app-server");
    expect(npmInstallCall(1)?.config).toBe(config);
    expect(warnMessages).toEqual([
      'Plugin "afora-codex-app-server" has no beta npm release for afora-codex-app-server@beta; using afora-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "afora-codex-app-server",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated afora-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback used afora-codex-app-server because afora-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback).toEqual({
      requestedSpec: "afora-codex-app-server@beta",
      usedSpec: "afora-codex-app-server",
      requestedLabel: "@beta",
      usedLabel: "@latest",
      reason: "unavailable",
      message:
        "plugin channel fallback: afora-codex-app-server used @latest because @beta was unavailable",
    });
  });
  ...
  it("reports npm beta fallback as tentative during dry-run checks", async () => {
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Would update afora-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback would use afora-codex-app-server because afora-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback?.message).toBe(
      "plugin channel fallback: afora-codex-app-server would use @latest because @beta was unavailable",
    );
  });
  ...
  it("falls back to the default npm spec when the beta package exists but is invalid", async () => {
    ...
    expect(npmInstallCall(1)?.spec).toBe("afora-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "afora-codex-app-server" failed beta npm update for afora-codex-app-server@beta; using afora-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "afora-codex-app-server",
    ...
    expect(result.outcomes[0]?.channelFallback).toMatchObject({
      ...
      usedLabel: "@latest",
      reason: "failed",
      message: "plugin channel fallback: afora-codex-app-server used @latest after @beta failed",
    });
  });
  ...
  it.each(["security_scan_blocked", "security_scan_failed"] as const)(
    ...
    async (code) => {
      ...
      const result = await updatePlugin(
        createCodexAppServerInstallConfig({ spec: "afora-codex-app-server" }),
        "afora-codex-app-server",
        { updateChannel: "beta" },
      );
      ...
      expect(result.outcomes[0]).toMatchObject({
        status: "error",
        message: `Failed to update afora-codex-app-server: install policy returned ${code}`,
      });
    },
  );
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "afora-codex-app-server",
        status: "error",
        message: `Failed to ${action} afora-codex-app-server: npm package not found for afora-codex-app-server.`,
        channelFallback: {
          requestedSpec: "afora-codex-app-server@beta",
          usedSpec: "afora-codex-app-server",
          requestedLabel: "@beta",
          usedLabel: "@latest",
          reason: "failed",
          message: `plugin channel fallback: afora-codex-app-server ${fallbackVerb} @latest after @beta failed`,
        },
      },
    ]);
  });
  ...
  it("preserves explicit npm tags when updating on the beta channel", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createCodexAppServerInstallConfig({
        spec: "afora-codex-app-server@rc",
      }),
      pluginIds: ["afora-codex-app-server"],
      updateChannel: "beta",
      dryRun: true,
  ...
  it("uses the beta core channel for a targeted official ClawHub install with npm-only catalog metadata", async () => {
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@afora/discord@beta");
  ...
  it("does not apply the official beta channel to custom ClawHub provenance", async () => {
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@afora/discord");
  });
  ...
  it("does not fall back to npm for blocked official ClawHub artifact downloads", async () => {
    ...
    installPluginFromClawHubMock.mockResolvedValueOnce({
      ...
      code: "clawhub_download_blocked",
      error:
        'ClawHub blocked artifact download for "@afora/discord@2026.5.16-beta.5"; install was not started. ClawHub /api/v1/packages/%40afora%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded.',
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          'Skipped discord ClawHub update: ClawHub blocked artifact download for "@afora/discord@2026.5.16-beta.5"; install was not started. ClawHub /api/v1/packages/%40afora%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded. Existing installed plugin left unchanged.',
      },
    ]);
  ...
  it("uses the default npm spec when beta ClawHub falls back before an artifact block", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        pluginId: "discord",
        targetDir: "/tmp/afora-plugins/discord",
        version: "2026.5.16",
        npmResolution: {
          name: "@afora/discord",
          version: "2026.5.16",
          resolvedSpec: "@afora/discord@2026.5.16",
        },
      }),
    );
    ...
    expect(clawHubInstallCall(0)?.spec).toBe("clawhub:@afora/discord@beta");
    expect(clawHubInstallCall(1)?.spec).toBe("clawhub:@afora/discord");
    expect(npmInstallCall()?.spec).toBe("@afora/discord");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@afora/discord@2026.5.16",
      installPath: "/tmp/afora-plugins/discord",
      version: "2026.5.16",
    });
    expect(result.outcomes[0]?.message).toBe(
      "Updated discord: 2026.5.12 -> 2026.5.16. (warning: official ClawHub artifact fallback used @afora/discord).",
    );
    expect(warnMessages).toEqual([
      'Plugin "discord" has no beta ClawHub release for clawhub:@afora/discord@beta; using clawhub:@afora/discord instead. Core update can still complete.',
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@afora/discord; using npm @afora/discord instead. Core update can still complete.',
    ]);
  });
  ...
  it("uses exact-core npm when an official ClawHub install falls back on extended-stable", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@afora/discord",
          version: "2026.7.33",
          resolvedSpec: "@afora/discord@2026.7.33",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@afora/discord@2026.7.33");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@afora/discord",
      version: "2026.7.33",
  ...
  it("reports npm dry-run versions for trusted official ClawHub artifact fallback", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@afora/discord",
        version: "2026.5.16-beta.5",
        resolvedSpec: "@afora/discord@2026.5.16-beta.5",
      },
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@afora/discord@beta");
    expect(npmInstallCall()?.dryRun).toBe(true);
    expect(result.outcomes).toEqual([
      {
        pluginId: "discord",
        ...
        message:
          "discord is up to date (2026.5.16-beta.5). (warning: official ClawHub artifact fallback would use @afora/discord@beta).",
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update discord: artifact unavailable (ClawHub clawhub:@afora/discord@beta).",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      targetDir: "/tmp/afora-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@afora/voice-call");
    expect(npmInstallCall()?.expectedPluginId).toBe("voice-call");
    expect(result.config.plugins?.allow).toEqual(["@afora/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@afora/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@afora/voice-call");
    expect(result.config.plugins?.entries?.["@afora/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["@afora/voice-call"], {
      source: "npm",
      spec: "@afora/voice-call",
      installPath: "/tmp/afora-voice-call",
      version: "0.0.2",
    });
  ...
  it.each([
    {
      ...
      expectedInstallSpec: "@afora/fish-audio-speech@beta",
      expectedRecordSpec: "@afora/fish-audio-speech",
    },
    {
      name: "stable",
      params: { officialPluginUpdateChannel: "stable" as const },
      expectedInstallSpec: "@afora/fish-audio-speech",
      expectedRecordSpec: "@afora/fish-audio-speech",
    },
    {
      ...
      expectedInstallSpec: "@afora/fish-audio-speech@2026.8.1",
      expectedRecordSpec: "@afora/fish-audio-speech",
    },
    {
      ...
      params: {
        ...
        specOverrides: { "fish-audio": "@afora/fish-audio-speech@next" },
      },
      expectedInstallSpec: "@afora/fish-audio-speech@next",
      expectedRecordSpec: "@afora/fish-audio-speech@next",
    },
  ])(
    ...
    const config = {
      plugins: {
        installs: {
          ...
          "fish-audio-speech": {
            source: "npm",
            spec: "@afora/fish-audio-speech@2026.8.1-beta.1",
            resolvedName: "@afora/fish-audio-speech",
            resolvedSpec: "@afora/fish-audio-speech@2026.8.1-beta.1",
  ...
  it("keeps authored plugin config shape when only the install key migrates", async () => {
    ...
    expect(result.config.plugins).toEqual({
      installs: {
        "@afora/voice-call": expect.objectContaining({
          source: "npm",
          spec: "@afora/voice-call",
          installPath: "/tmp/afora-voice-call",
        }),
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@afora/context-engine",
      targetDir: "/tmp/afora-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@afora/context-engine");
    expectRecordFields(result.config.plugins?.installs?.["@afora/context-engine"], {
      source: "npm",
      spec: "@afora/context-engine",
      installPath: "/tmp/afora-context-engine",
      version: "0.0.2",
    });
  ...
  it("forwards dangerous force unsafe install to plugin update installers", async () => {
    ...
    expect(npmInstallCall()?.spec).toBe("afora-codex-app-server@beta");
    expect(npmInstallCall()?.dangerouslyForceUnsafeInstall).toBe(true);
    expect(npmInstallCall()?.expectedPluginId).toBe("afora-codex-app-server");
  });

  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/afora/extensions/demo";
    const extensionsDir = "/var/afora/extensions";
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/afora-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/afora-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@afora/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@afora/legacy-chat@2.0.0",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@afora/legacy-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      spec: "@afora/legacy-chat",
      installPath: "/tmp/afora-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@afora/legacy-chat",
  ...
  it("installs an externalized bundled plugin under its renamed package id", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/afora-plugins/afora-qqbot",
        version: "2.0.1",
      }),
    );
    ...
    expect(npmInstallCall()?.expectedPluginId).toBe("afora-qqbot");
    expect(npmInstallCall()?.expectedIntegrity).toBe("sha512-qqbot-catalog-pin");
    expect(result.summary.switchedToNpm).toEqual(["afora-qqbot"]);
    expect(result.config.plugins?.entries?.qqbot).toBeUndefined();
    expect(result.config.plugins?.entries?.["afora-qqbot"]).toEqual({ enabled: true });
    expect(result.config.plugins?.installs?.qqbot).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["afora-qqbot"], {
      source: "npm",
      spec: "@tencent-connect/afora-qqbot@2.0.1",
      installPath: "/tmp/afora-plugins/afora-qqbot",
      version: "2.0.1",
    });
  });
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/afora-plugins/voice-call",
  ...
  it("falls back from ClawHub to npm only when the ClawHub package is absent", async () => {
    ...
    expect(result.summary.warnings).toEqual([
      "ClawHub clawhub:legacy-chat@2026.5.1-beta.2 unavailable for legacy-chat; falling back to npm @afora/legacy-chat.",
    ]);
    expect(result.summary.errors).toStrictEqual([]);
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
  ...
  it("uses exact-core npm when an official ClawHub bridge falls back on extended-stable", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          ...
          version: "2026.7.33",
          resolvedSpec: "@afora/voice-call@2026.7.33",
        },
      }),
    );
    ...
    const result = await syncExternalizedPlugin({
      ...
      bridge: {
        bundledPluginId: "voice-call",
        preferredSource: "clawhub",
        clawhubSpec: "clawhub:@afora/voice-call",
        npmSpec: "@afora/voice-call",
        channelIds: ["voice-call"],
      },
    ...
    expect(npmInstallCall()?.spec).toBe("@afora/voice-call@2026.7.33");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/afora-plugins/default-chat",
        version: "2.0.0",
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@afora/default-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["default-chat"], {
      source: "npm",
      spec: "@afora/default-chat",
      installPath: "/tmp/afora-plugins/default-chat",
      version: "2.0.0",
    });
  });
  ...
  it.each(["constructor", "__proto__"])(
    ...
    async (targetPluginId) => {
      ...
      const npmPackageName = `afora-plugin-${targetPluginId}`;
      resolveBundledPluginSourcesMock.mockReturnValue(new Map());
  ...
  it.each([
    {
      ...
      expectedInstall: { source: "npm", spec: "@afora/legacy-chat" },
      bridge: {},
    ...
    },
    {
      ...
      expectedInstall: { source: "npm", resolvedName: "@afora/legacy-chat" },
      bridge: {},
      expectClawHubNotCalled: false,
    },
    {
      ...
      install: {
        source: "npm",
        spec: "@afora/legacy-chat@1.2.3",
        resolvedSpec: "@afora/legacy-chat@1.2.3",
        installPath: "/tmp/afora-plugins/legacy-chat",
      },
      expectedInstall: { source: "npm", spec: "@afora/legacy-chat@1.2.3" },
      bridge: {},
      expectClawHubNotCalled: false,
    },
    {
