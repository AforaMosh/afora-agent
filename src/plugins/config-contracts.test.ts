...
const mocks = vi.hoisted(() => {
  ...
  return {
    discoverAforaPlugins: vi.fn(() => ({ candidates: [], diagnostics: [] })),
...
vi.mock("./discovery.js", () => ({
  discoverAforaPlugins: mocks.discoverAforaPlugins,
}));
...
function createPluginRecord(
  ...
  return {
    ...
    hooks: [],
    source: `/tmp/${overrides.id}/afora.plugin.json`,
...
describe("resolvePluginConfigContractsById", () => {
  beforeEach(() => {
    mocks.discoverAforaPlugins.mockReset();
    mocks.discoverAforaPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
  ...
  it("uses a supplied manifest registry as the authoritative contract source", () => {
    ...
    expect(mocks.discoverAforaPlugins).not.toHaveBeenCalled();
