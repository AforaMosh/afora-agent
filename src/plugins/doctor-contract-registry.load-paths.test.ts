...
function writeLegacyRuntimeDoctorPlugin(params: {
  ...
  fs.writeFileSync(
    path.join(params.pluginRoot, "afora.plugin.json"),
    JSON.stringify({
      id: params.pluginId,
  ...
  fs.writeFileSync(
    ...
    `import { ${params.importedSymbols.join(", ")} } from "afora-agent/plugin-sdk/runtime-doctor";
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): AforaConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: AforaConfig, pluginId: string): Record<string, unknown> {
...
describe("doctor contract registry load-path plugins", () => {
  it.each([
    ...
    async ({ pluginId, namespace, sourceFile, stateKey, state }) => {
      const stateDir = tempDirs.make("afora-doctor-contract-load-paths-");
      const pluginRoot = tempDirs.make("afora-doctor-contract-load-paths-");
  ...
  it.each([
    ...
    const stateDir = tempDirs.make("afora-doctor-contract-legacy-package-");
    const pluginRoot = tempDirs.make("afora-doctor-contract-legacy-package-");
