...
export type ConfigScopedRuntimeCache<T> = WeakMap<AforaConfig, Map<string, T>>;
...
type ConfigScopedPromiseLoader<T> = {
  load(config?: AforaConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: AforaConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<AforaConfig, Promise<T>>();

  const createPromise = (config?: AforaConfig): Promise<T> => {
  ...
  return {
    async load(config?: AforaConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<AforaConfig, Promise<T>>();
    },
  };
}
