...
export type ProviderPlugin = {
   ...
   *
   * Afora calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Afora performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Afora registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Afora wrappers.
   *
   ...
   *
   * Called after Afora resolves the raw configured credential but before the
   ...
   * hint than Afora's generic auth-store guidance.
   */
   ...
   * Afora's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Afora should surface a provider-specific hint for
   ...
   * merging. Afora deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Afora stores the
   ...
   * without replacing the full Afora-owned system prompt.
   */
   ...
   *
   * Runs after Afora's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Afora's default fallback policy.
   */
   ...
   * rewrites after Afora has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Afora handles
   ...
   *
   * Afora uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `afora doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `afora doctor` should offer.
   *
   ...
   *
   * Afora calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep Afora's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Afora falls back to that stored profile.
   */
