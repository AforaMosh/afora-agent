...
function createBundledWhatsAppRuntimeFixture() {
  const rootDir = makeTrackedTempDir("afora-whatsapp-boundary", tempDirs);
  for (const [relativePath, value] of Object.entries({
    ...
    "dist/connection-controller-registry.js": [
      'const key = Symbol.for("afora.whatsapp.connectionControllerRegistry");',
      "const g = globalThis;",
      "if (!g[key]) {",
...
function createExternalTypeScriptRuntimePackageFixture() {
  const rootDir = makeTrackedTempDir("afora-external-boundary-ts", tempDirs);
  writeRuntimeFixtureText(
    rootDir,
    "package.json",
    JSON.stringify(
      {
        name: "afora-external-ts-runtime",
        type: "module",
      },
      null,
      2,
...
describe("runtime plugin boundary whatsapp seam", () => {
  ...
  it("rejects bundled TypeScript runtime modules instead of using the source loader", () => {
    const rootDir = makeTrackedTempDir("afora-bundled-boundary-ts", tempDirs);
