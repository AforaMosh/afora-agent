...
function createTemporaryRoot(): string {
  const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "afora-plugin-ordering-")));
...
describe("deterministic plugin discovery ownership", () => {
  it("keeps plugin, equal-priority hook, and service order independent of directory enumeration", () => {
    ...
    const registry = loadAforaPlugins({
      ...
      env: { AFORA_STATE_DIR: stateRoot, AFORA_DISABLE_BUNDLED_PLUGINS: "1" },
    });
  ...
  it("keeps configured load-path precedence under deterministic directory discovery", () => {
    ...
    const discovery = discoverAforaPlugins({
      ...
      installRecords: {},
      env: {
        AFORA_STATE_DIR: path.join(stateRoot, "state"),
  ...
  it("orders manifest-only plugin metadata consistently across filesystem enumeration", () => {
    ...
    const extensionRoot = path.join(home, ".afora", "extensions");
    for (const pluginId of ["zeta", "alpha"]) {
      createPlugin({ pluginRoot: path.join(extensionRoot, pluginId), pluginId });
    }
    reverseDirectoryEntries(extensionRoot);
    ...
    const records = listAforaPluginManifestMetadata({
      AFORA_HOME: home,
      AFORA_BUNDLED_PLUGINS_DIR: path.join(root, "no-bundled-plugins"),
    });
