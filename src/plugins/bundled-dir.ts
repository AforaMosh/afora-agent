...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "afora-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "AFORA_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.AFORA_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "afora.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveAforaPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveAforaPackageRootSync({ moduleUrl: import.meta.url });
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveAforaPackageRootSync({ moduleUrl: import.meta.url });
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.AFORA_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.AFORA_BUNDLED_PLUGINS_DIR ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.AFORA_BUNDLED_PLUGINS_DIR?.trim();
