...
const mocks = vi.hoisted(() => {
  const lease = {
    databasePath: "/tmp/afora-plugin-index.sqlite",
...
describe("commitConfigWithPendingPluginInstalls", () => {
  beforeEach(() => {
    ...
    mocks.replaceConfigFile.mockImplementation(async (params: { nextConfig: AforaConfig }) => ({
      path: "/tmp/afora.json",
      previousHash: null,
      snapshot: {} as never,
  ...
  it("handles prototype-named pending records with own-key semantics", () => {
    ...
    const baseConfig = { plugins: { installs: baseInstalls } } satisfies AforaConfig;
    const nextConfig = { plugins: { installs: nextInstalls } } satisfies AforaConfig;
  ...
  it("marks replaced managed npm generations when install records are committed", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-record-commit-"));
  ...
  it("removes a new retirement marker when the leased config commit rolls back", async () => {
    const stateDir = retentionTempDirs.make("afora-record-commit-");
    const installPath = writeManagedNpmPlugin({
      stateDir,
      packageName: "@afora/retained-rollback",
    ...
    const previousInstallRecords: Record<string, PluginInstallRecord> = {
      "retained-rollback": {
        source: "npm",
        spec: "@afora/retained-rollback@1.0.0",
        installPath,
      },
    };
  ...
  it("does not mark arbitrary npm paths outside the managed npm root", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-record-commit-"));
    const outsideRoot = fs.mkdtempSync(path.join(os.tmpdir(), "afora-record-outside-"));
    const previousInstallPath = path.join(
      outsideRoot,
      "npm",
      "projects",
  ...
  it("removes earlier retained markers when a later marker creation fails", async () => {
    ...
    fs.writeFileSync(
      path.join(stateDir, "npm", "projects", "voice-call-v1", ".afora-retained-npm-installs"),
      "not a directory",
      "utf8",
    );
