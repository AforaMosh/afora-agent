...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".afora", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and honors invocation policy", async () => {
    const homeDir = tempDirs.make("afora-bundle-commands-home-");
    const workspaceDir = tempDirs.make("afora-bundle-commands-workspace-");
    await withEnvAsync(
      {
        HOME: homeDir,
        USERPROFILE: homeDir,
  ...
  it("warns and skips oversized bundle commands without dropping siblings", async () => {
    const homeDir = tempDirs.make("afora-bundle-commands-oversized-");
    const workspaceDir = tempDirs.make("afora-bundle-commands-oversized-ws-");
