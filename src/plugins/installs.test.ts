...
describe("resolveNpmInstallRecordSpec", () => {
  it("uses an exact resolved registry spec when managed installs request pinning", () => {
    expect(
      resolveNpmInstallRecordSpec({
        requestedSpec: "@afora/codex",
        resolution: {
          name: "@afora/codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@afora/codex@2026.5.30-beta.1",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@afora/codex@2026.5.30-beta.1");
  });
...
describe("recordPluginInstall", () => {
  ...
  it("replaces local npm-pack provenance on a registry reinstall", () => {
    const localInstall = recordPluginInstall(
      {},
      {
        pluginId: "diffs",
        source: "npm",
        spec: "/tmp/afora-diffs.tgz",
        sourcePath: "/tmp/afora-diffs.tgz",
        installPath: "/tmp/afora/plugins/diffs-local",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmTarballName: "afora-diffs-1.0.0.tgz",
    ...
    const registryInstall = recordPluginInstall(localInstall, {
      ...
      source: "npm",
      spec: "@afora/diffs",
      installPath: "/tmp/afora/plugins/diffs",
      resolvedName: "@afora/diffs",
      resolvedVersion: "1.1.0",
      resolvedSpec: "@afora/diffs@1.1.0",
      installedAt: "2026-05-15T00:00:03.000Z",
    });
  ...
  it("moves an exact prior npm load path on same-version reinstall", () => {
    const previousInstallPath = "/tmp/afora/npm/projects/alpha-v1/node_modules/alpha";
    const nextInstallPath = "/tmp/afora/npm/projects/alpha-v2/node_modules/alpha";
    const customPath = `${previousInstallPath}/custom-child`;
    const adjacentPath = "/tmp/afora/npm/projects/beta/node_modules/beta";
    const existing = {
      plugins: {
