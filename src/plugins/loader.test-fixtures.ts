...
export type PluginLoadConfig = NonNullable<Parameters<typeof loadAforaPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadAforaPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "afora-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.AFORA_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.AFORA_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".afora", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ AFORA_STATE_DIR: stateDir, ...params.env }, () =>
    loadAforaPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.AFORA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.AFORA_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.AFORA_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
