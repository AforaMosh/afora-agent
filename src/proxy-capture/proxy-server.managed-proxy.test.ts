...
async function makeSettings(): Promise<DebugProxySettings> {
  testRoot = await mkdtemp(join(tmpdir(), "afora-debug-proxy-managed-proxy-"));
  const certDir = join(testRoot, "certs");
  process.env.AFORA_STATE_DIR = testRoot;
  return {
    enabled: true,
    required: false,
...
describe("debug proxy managed-proxy direct upstream policy", () => {
  const originalProxyActive = process.env["AFORA_PROXY_ACTIVE"];
  const originalAllowDirect =
    process.env["AFORA_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];

  beforeEach(async () => {
    await cleanupTestDirs();
    delete process.env["AFORA_PROXY_ACTIVE"];
    delete process.env["AFORA_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];
  });
  ...
  afterEach(async () => {
    if (originalProxyActive === undefined) {
      ...
      process.env["AFORA_PROXY_ACTIVE"] = originalProxyActive;
    }
  ...
  it("allows HTTP upstreams with the explicit diagnostic override", async () => {
    ...
    process.env["AFORA_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"] = "1";
  ...
  it("rejects CONNECT upstreams before opening direct sockets while managed proxy mode is active", async () => {
    process.env["AFORA_PROXY_ACTIVE"] = "true";
    const server = await startDebugProxyServer({ settings: await makeSettings() });
    try {
