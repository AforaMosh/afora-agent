...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[afora proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[afora proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`afora proxy coverage\` for details.`,
  );
}
