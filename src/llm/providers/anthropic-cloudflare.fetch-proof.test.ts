import type { Context, Model } from "@afora/ai";
...
// ever reached. This proves the stream facade installs Afora's guarded
...
// Importing the facade installs the Afora AI transport host ports.
import "../stream.js";
...
describe("Anthropic Cloudflare guard-specific SSRF blocking proof", () => {
  ...
  it("blocks a private-IP request before globalThis.fetch is called (guard-specific behavior)", async () => {
    ...
    const { streamAnthropic } = await import("@afora/ai/internal/anthropic");
