...
import { writeTextAtomic } from "@afora/fs-safe/atomic";
...
export async function copyMemoryMigrationFileItem(
  ...
  try {
    ...
    if (replaceExisting) {
      ...
      stagingDir = path.join(".afora-memory-import-staging", crypto.randomUUID());
    ...
    if (stagingDir) {
      ...
      await safeRoot.remove(".afora-memory-import-staging").catch(() => undefined);
    }
