...
const pluginModuleLoaderJitiFactoryOverrideKey = Symbol.for(
  "afora.pluginModuleLoaderJitiFactoryOverride",
);
...
function createBundledChannelEntry(params: {
  importerPath: string;
  pluginId: string;
  registerCliMetadata?: (api: AforaPluginApi) => void;
  registerFull?: (api: AforaPluginApi) => void;
  registerCapabilities?: (api: AforaPluginApi) => void;
}) {
...
describe("defineBundledChannelEntry", () => {
  ...
  it("runs tool registrations without channel sidecar hydration during tool discovery", () => {
    const tempRoot = tempDirs.make("afora-bundled-entry-tools-");
  ...
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = tempDirs.make("afora-bundled-entry-runtime-");
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
...
describe("defineBundledChannelSetupEntry", () => {
  it("exposes setup-runtime registrations without loading the full channel entry", () => {
    const tempRoot = tempDirs.make("afora-bundled-setup-entry-");
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    const setupRuntimeRegister = vi.fn<(api: AforaPluginApi) => void>();
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("AFORA_DIAGNOSTICS", "plugin.load-profile");
  ...
  try {
    ...
    const tempRoot = tempDirs.make("afora-channel-entry-contract-");
...
function runCompiledEsmSidecarFastPathProbe(): SpawnSyncReturns<string> {
  ...
  fs.writeFileSync(path.join(tempRoot, "afora.mjs"), "#!/usr/bin/env node\n", "utf8");
  ...
  fs.writeFileSync(
    ...
    'import { defineChannelMessageAdapter } from "afora-agent/plugin-sdk/channel-outbound";\nexport const sentinel = defineChannelMessageAdapter();\n',
    "utf8",
  );
  ...
  return spawnSync(process.execPath, ["--import", "tsx", probePath], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: { ...process.env, AFORA_DIAGNOSTICS: "plugin.load-profile" },
  });
}
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before module loads them", async () => {
    ...
    vi.doMock("../infra/boundary-file-read.js", () => ({
      openRootFileSync: () => ({
        ok: true,
        path: "C:\\Users\\alice\\afora\\dist\\extensions\\feishu\\helper.ts",
      ...
      try {
        ...
        expect(
          channelEntryContract.loadBundledEntryExportSync<number>(
            "file:///C:/Users/alice/afora/dist/extensions/feishu/index.js",
            {
              specifier: "./helper.ts",
              exportName: "load",
            },
        ...
        expect(jitiLoad).toHaveBeenCalledWith(
          "file:///C:/Users/alice/afora/dist/extensions/feishu/helper.ts",
        );
      } finally {
  ...
  it("transforms Afora SDK dependencies after a native built sidecar load declines", async () => {
  ...
  it("preserves presence-based source fallback disable semantics and cache modes", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"afora-agent"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("AFORA_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "enabled");
    ...
    for (const value of ["", "   ", "off", "no", " ON ", "arbitrary"]) {
      vi.stubEnv("AFORA_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", value);
      expect(loadSecretContract).toThrow(
        `resolved "${path.join(pluginRoot, "src", "secret-contract.js")}"`,
      );
    }
