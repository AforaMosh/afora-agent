...
type ChannelConfigAccessorParams<Config extends AforaConfig = AforaConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends AforaConfig = AforaConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends AforaConfig = AforaConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: AforaConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends AforaConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends AforaConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends AforaConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
