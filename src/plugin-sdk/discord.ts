...
export type { AforaConfig } from "./config-contracts.js";
export type { AforaPluginApi, PluginRuntime } from "./channel-plugin-common.js";
...
/** Discord channel config shape for one account in Afora config. */
export type DiscordAccountConfig = NonNullable<NonNullable<AforaConfig["channels"]>["discord"]>;
...
type DiscordApiFacadeModule = {
  ...
  inspectDiscordAccount: (params: { cfg: AforaConfig; accountId?: string | null }) => unknown;
  listDiscordAccountIds: (cfg: AforaConfig) => string[];
  ...
  resolveDefaultDiscordAccountId: (cfg: AforaConfig) => string;
...
function resolveCompatRuntimeConfig(params: { cfg?: AforaConfig }): AforaConfig {
...
/** List configured Discord account ids from Afora config. */
export function listDiscordAccountIds(cfg: AforaConfig): string[] {
...
export function resolveDefaultDiscordAccountId(cfg: AforaConfig): string {
