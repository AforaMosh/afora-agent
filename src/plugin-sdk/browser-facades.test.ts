...
describe("plugin-sdk browser facades", () => {
  ...
  it("delegates browser profile helpers to the browser facade", async () => {
    ...
    const rootConfig = { gateway: { port: 18789 } } as import("../config/config.js").AforaConfig;

    expect(browserProfiles.resolveBrowserConfig(cfg, rootConfig)).toBe(resolvedConfig);
    expect(browserProfiles.resolveProfile(resolvedConfig, "afora")).toBe(resolvedProfile);
    ...
    expect(resolveProfile).toHaveBeenCalledWith(resolvedConfig, "afora");
  });
  ...
  it("delegates browser control auth helpers to the browser facade", async () => {
    ...
    } as import("../config/config.js").AforaConfig;
