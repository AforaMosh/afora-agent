...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "afora-agent/plugin-sdk/core";
import {
  readFileWithinRoot,
...
} from "afora-agent/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTestDir({ prefix: "afora-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps root-bounded file-access helpers on file-access-runtime", async () => {
    await withTestDir({ prefix: "afora-sdk-file-access-compat-" }, async (root) => {
