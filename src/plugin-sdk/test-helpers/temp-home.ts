// Temp home test helpers create isolated Afora home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked AFORA_HOME.
  deleteTestEnvValue("AFORA_HOME");
  setTestEnvValue("AFORA_STATE_DIR", path.join(base, ".afora"));
...
export async function withTempHomeCore<T>(
  ...
  const prefix = opts.prefix ?? "afora-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".afora", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
