...
const FACADE_LOADER_GLOBAL = "__aforaTestLoadBundledPluginPublicSurfaceModuleSync";
...
function createBundledChannelConfigFixtures(): string {
  const bundledPluginsDir = path.join(
    packageRoot,
    "dist",
    nextTrustedPluginId("afora-channel-config-fixtures-"),
  );
...
describe("plugin-sdk facade loader", () => {
  it("resolves channel config facades lazily from generated plugin fixtures", async () => {
    process.env.AFORA_BUNDLED_PLUGINS_DIR = createBundledChannelConfigFixtures();
  ...
  it("honors trusted bundled plugin dir overrides under the package root", () => {
    const pluginId = nextTrustedPluginId("afora-facade-loader-override-");
    const overrideA = createBundledPluginFixture({
      pluginId,
      kind: "dist",
      prefix: "afora-facade-loader-a-",
      marker: "override-a",
    });
    const overrideB = createBundledPluginFixture({
      pluginId,
      kind: "dist-runtime",
      prefix: "afora-facade-loader-b-",
      marker: "override-b",
    });

    process.env.AFORA_BUNDLED_PLUGINS_DIR = overrideA.bundledPluginsDir;
    ...
    process.env.AFORA_BUNDLED_PLUGINS_DIR = overrideB.bundledPluginsDir;
  ...
  it("falls back to package source surfaces when an override dir lacks a bundled plugin", () => {
    const fixture = createPackageSourcePluginFixture({
      prefix: "afora-facade-loader-source-fallback-",
      marker: "source-fallback",
    });
    process.env.AFORA_BUNDLED_PLUGINS_DIR = createTempDirSync("afora-facade-loader-empty-");
  ...
  it("open failures are not classified as MissingPublicSurfaceError", () => {
    const tempRoot = createTempDirSync("afora-facade-loader-boundary-fail-");
  ...
  it("shares loaded facade ids with facade-runtime", () => {
    const fixture = createBundledPluginFixture({
      prefix: "afora-facade-loader-ids-",
      marker: "identity-check",
    });
    process.env.AFORA_BUNDLED_PLUGINS_DIR = fixture.bundledPluginsDir;
  ...
  it("uses native require for Windows dist facade loads", () => {
    const fixture = createBundledPluginFixture({
      prefix: "afora-facade-loader-windows-",
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const fixture = createCircularPluginFixture("afora-facade-loader-circular-");
    process.env.AFORA_BUNDLED_PLUGINS_DIR = fixture.bundledPluginsDir;
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const fixture = createThrowingPluginFixture("afora-facade-loader-throw-");
    process.env.AFORA_BUNDLED_PLUGINS_DIR = fixture.bundledPluginsDir;
