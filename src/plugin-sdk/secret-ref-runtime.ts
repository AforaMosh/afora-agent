...
function renderSecretRefApplyCommands(
  ...
  const render = (shell: CommandShell, indent = "") => {
    ...
    return [
      `${indent}afora secrets apply --from ${quotedPlanPath} --dry-run --allow-exec`,
      `${indent}afora secrets apply --from ${quotedPlanPath} --allow-exec`,
    ];
  };
  if (platform !== "win32") {
    return render("posix");
...
export function createPluginSecretRefSetupCli(params: PluginSecretRefSetupCliParams) {
  ...
  const inspectProvider = (config: AforaConfig, requestedAlias?: string) => {
  ...
  const parseConfigTargetMappings = (
    ...
    (values ?? []).map((value) => {
      ...
      if (separator <= 0 || separator === value.length - 1) {
        throw new Error(
          `Invalid --target value "${value}". Use <afora-config-path>=<${params.secretIdPlaceholder}>.`,
        );
      }
  ...
  const runSetup = async (options: SecretRefSetupOptions): Promise<void> => {
    ...
    writeSecretRefCliLine("  afora secrets audit --check --allow-exec");
    writeSecretRefCliLine("  afora secrets reload");
  };
