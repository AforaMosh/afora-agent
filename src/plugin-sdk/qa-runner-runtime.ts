...
type QaRunnerTransportAdapterDefinition = {
  ...
  createGatewayConfig: (params: {
    baseUrl: string;
  }) => Pick<AforaConfig, "channels" | "messages">;
  waitReady: (params: {
    gateway: {
      call: (
        method: string,
...
export function listQaRunnerCliContributions(): readonly QaRunnerCliContribution[] {
  ...
  for (const plugin of listDeclaredQaRunnerPlugins(env)) {
    ...
    for (const runner of plugin.qaRunners) {
      ...
      if (!registration) {
        throw new Error(
          `QA runner plugin "${plugin.id}" declared "${runner.commandName}" in afora.plugin.json but did not export a matching CLI registration from its QA runner surface`,
        );
      }
    ...
    for (const commandName of runtimeRegistrationByCommandName?.keys() ?? []) {
      if (!declaredCommandNames.has(commandName)) {
        throw new Error(
          `QA runner plugin "${plugin.id}" exported "${commandName}" from its QA runner surface but did not declare it in afora.plugin.json`,
        );
      }
    }
  }
