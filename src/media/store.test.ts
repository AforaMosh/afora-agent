...
describe("media store", () => {
  ...
  it.each([
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal17, homeLocal4) => {
          ...
          expect(path.normalize(dir)).toContain(`${path.sep}.afora${path.sep}media`);
    ...
    },
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal9, homeInner) => {
          ...
          const targetDir = path.join(homeInner, ".afora", "media", "oversized-stream");
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, homeLocal) => {
      vi.doMock("@afora/media-core/mime", async () => {
        const actual = await vi.importActual<typeof import("@afora/media-core/mime")>(
          "@afora/media-core/mime",
        );
        return {
          ...actual,
      ...
      try {
        ...
        vi.doUnmock("@afora/media-core/mime");
      }
    });
  });
