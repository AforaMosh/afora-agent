...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredAforaTmpDir(), "web-media-core-"));
...
describe("loadWebMedia", () => {
  ...
  it("resolves home-relative local media paths through allowed local roots", async () => {
    await withEnvAsync({ AFORA_HOME: fixtureRoot }, async () => {
  ...
  it("allows trusted generated host-read HTML reports under Afora temp root", async () => {
  ...
  it("allows exact marked outbound HTML bytes and rejects same-size replacements", async () => {
    ...
    try {
      await withEnvAsync({ AFORA_STATE_DIR: stateRoot }, async () => {
  ...
  it("requires a marker when outbound staging is nested under the trusted temp root", async () => {
    const stateRoot = await fs.mkdtemp(
      path.join(resolvePreferredAforaTmpDir(), "web-media-overlap-state-"),
    );
    try {
      await withEnvAsync({ AFORA_STATE_DIR: stateRoot }, async () => {
        const { saveMediaBuffer } = await import("./store.js");
        ...
        expect(path.resolve(saved.path)).toContain(path.resolve(resolvePreferredAforaTmpDir()));
        await expectLoadWebMediaErrorCode(
          loadWebMedia(saved.path, {
            maxBytes: 1024 * 1024,
            localRoots: "any",
  ...
  it("prunes more stale markers than one SQLite parameter batch", async () => {
    ...
    try {
      await withEnvAsync({ AFORA_STATE_DIR: stateRoot }, async () => {
        ...
        const { openAforaStateDatabase, runAforaStateWriteTransaction } =
          await import("../state/afora-state-db.js");
  ...
  it("rejects host-read HTML files outside the trusted Afora temp root", async () => {
  ...
  it("rejects trusted host-read HTML symlinks that resolve outside Afora temp root", async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), "web-media-host-html-"));
  ...
  it("rejects trusted host-read HTML hardlinks to files outside Afora temp root", async () => {
    const outsideRoot = await fs.mkdtemp(
      path.join(path.dirname(resolvePreferredAforaTmpDir()), "web-media-host-html-"),
    );
    const outsideHtml = path.join(outsideRoot, "report.html");
