...
const PIPEWIRE_SINK_NAME = "afora_meeting_audio";
const PIPEWIRE_MONITOR_NAME = `${PIPEWIRE_SINK_NAME}.monitor`;
const PIPEWIRE_SOURCE_NAME = PIPEWIRE_SINK_NAME;
const PIPEWIRE_MEETING_AUDIO_DEVICE_LABEL = "Afora Meeting Audio";
...
export async function ensureMeetingAudioBackend(params: {
  ...
  if (!pulseListContains(sinks.stdout ?? "", PIPEWIRE_SINK_NAME)) {
    ...
    if (loaded.code !== 0) {
      ...
      if (!pulseListContains(sinks.stdout ?? "", PIPEWIRE_SINK_NAME)) {
        assertCommandSucceeded(loaded, "Could not create the Afora PipeWire-Pulse sink");
      }
    }
  }
  ...
  if (!pulseListContains(sources.stdout ?? "", PIPEWIRE_SOURCE_NAME)) {
    ...
    if (loaded.code !== 0) {
      ...
      if (!pulseListContains(sources.stdout ?? "", PIPEWIRE_SOURCE_NAME)) {
        assertCommandSucceeded(loaded, "Could not create the Afora PipeWire-Pulse source");
      }
    }
  }
  sinks = await params.run(["pactl", "list", "short", "sinks"], params.timeoutMs);
  ...
  if (
    ...
    throw new Error("Afora PipeWire-Pulse sink or monitor source was not created.");
  }
}
