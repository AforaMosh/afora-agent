...
describe("meeting browser leave ownership", () => {
  ...
  it("does not close a tab whose page belongs to another session", async () => {
    ...
    expect(result).toEqual({
      left: true,
      note: "Test meeting tab belongs to another Afora meeting session; left its current call untouched.",
    });
  });
  ...
  it("does not report success when page ownership is unverified", async () => {
    ...
    expect(result).toEqual({
      left: false,
      note: "Browser control could not verify that the Test meeting tab still belongs to this Afora meeting session.",
    });
  });
