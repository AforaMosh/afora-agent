...
export function createMeetingStatusCallSource(options: MeetingStatusCallSourceOptions): string {
    ...
    if (media.length > 0) {
      try {
        ...
        if (output?.deviceId) {
          ...
          notes.push("The Afora virtual audio speaker output was not visible to ${options.platform.displayName}.");
        }
      } catch (error) {
        audioOutputRouted = false;
      ...
      if (!audioOutputRouted && audioOutputRouteError) {
        notes.push("Could not route ${options.platform.displayName} speaker output to the Afora virtual audio device: " + audioOutputRouteError);
      }
    } else {
      audioOutputRouted = false;
      try {
  ...
  if (inCall && allowMicrophone && !manualAction) {
    if (audioInputRouted !== true || audioOutputRouted !== true) {
      manualAction = manualActionFor("${options.platform.manualActionReasonPrefix}-audio-choice-required", "Verify the Afora virtual audio device is selected as both the ${options.platform.displayName} microphone and speaker before starting talk-back.");
