  ...
  try {
    ...
    if (leaveResult.sessionMatched === false && leaveResult.ownershipRetained !== true) {
      if (leaveResult.sessionConflict !== true) {
        return {
          left: false,
          note: `Browser control could not verify that the ${params.adapter.browserLabel} tab still belongs to this Afora meeting session.`,
        };
      }
      return {
        left: true,
        note: `${params.adapter.browserLabel} tab belongs to another Afora meeting session; left its current call untouched.`,
      };
    }
...
>(params: {
  ...
  if (snapshot.sessionMatched === false) {
    throw new Error(
      `The tracked ${params.adapter.browserLabel} tab now belongs to another Afora meeting session.`,
    );
  }
  return {
