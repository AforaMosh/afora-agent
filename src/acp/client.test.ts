...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets AFORA_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "afora",
    });

    expect(env.AFORA_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("afora");
  });

  it("overrides pre-existing AFORA_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      AFORA_SHELL: "wrong",
    });
    expect(env.AFORA_SHELL).toBe("acp-client");
  ...
  it("preserves AFORA_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        AFORA_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["AFORA_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Afora bridge", () => {
    ...
    expect(env.AFORA_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.AFORA_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("AFORA_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "afora", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    const dir = tempDirs.make("afora-acp-client-test-");
    const scriptPath = path.join(dir, "afora", "dist", "entry.js");
    const shimPath = path.join(dir, "afora.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\afora\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
  ...
  it("prompts for search when rawInput path escapes cwd", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/afora-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
  ...
  it("auto-approves search when query-like title text contains a path label", async () => {
    await expectAutoAllowWithoutPrompt({
      ...
      cwd: "/tmp/afora-acp-cwd/workspace",
    });
  });
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/afora-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/afora-acp-cwd",
    });
