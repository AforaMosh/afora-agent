...
describe("resolveAcpClientSpawnEnv", () => {
  ...
  it("strips explicit secret keys case-insensitively", () => {
    const env = resolveAcpClientSpawnEnv(
      {
        Skill_Secret: "skill-secret", // pragma: allowlist secret
        OPENAI_API_KEY: "openai-secret", // pragma: allowlist secret
        OPENCLAW_API_KEY: "keep-me",
      },
      { stripKeys: new Set(["SKILL_SECRET"]) },
    );

    expect(env.Skill_Secret).toBeUndefined();
    expect(env.OPENAI_API_KEY).toBe("openai-secret");
    expect(env.OPENCLAW_API_KEY).toBe("keep-me");
    expect(env.OPENCLAW_SHELL).toBe("acp-client");
  });

  it("preserves provider auth env vars for the self-contained agent", () => {
    const env = resolveAcpClientSpawnEnv({
      OPENAI_API_KEY: "openai-secret", // pragma: allowlist secret
...
describe("buildAcpClientStripKeys", () => {
  it("includes only active skill env keys", () => {
