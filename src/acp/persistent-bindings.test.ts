...
type ConfiguredBinding = NonNullable<AforaConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: AforaConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/afora" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
