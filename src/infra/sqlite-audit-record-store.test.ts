...
describe("SQLite audit record store", () => {
  ...
  it("keeps the newest configured number of rows per scope", async () => {
    await withTestDir({ prefix: "afora-audit-store-" }, async (stateDir) => {
  ...
  it("reads bounded newest-first pages by sequence", async () => {
    await withTestDir({ prefix: "afora-audit-store-latest-" }, async (stateDir) => {
      const store = createSqliteAuditRecordStore<{ value: number }>({
        scope: "latest-test",
        maxEntries: 10,
        env: { ...process.env, AFORA_STATE_DIR: stateDir },
  ...
  it("preserves insertion order and prunes the oldest row when timestamps tie", async () => {
    await withTestDir({ prefix: "afora-audit-store-ties-" }, async (stateDir) => {
      const store = createSqliteAuditRecordStore<{ value: number }>({
  ...
  it("prunes by insertion order when wall-clock timestamps move", async () => {
    await withTestDir({ prefix: "afora-audit-store-clock-skew-" }, async (stateDir) => {
      const store = createSqliteAuditRecordStore<{ value: number }>({
        scope: "clock-skew",
        maxEntries: 2,
        env: { ...process.env, AFORA_STATE_DIR: stateDir },
  ...
  it("commits batch inserts with one retention pass", async () => {
    await withTestDir({ prefix: "afora-audit-store-batch-" }, async (stateDir) => {
      const store = createSqliteAuditRecordStore<{ value: number }>({
        scope: "batch-test",
        maxEntries: 2,
        env: { ...process.env, AFORA_STATE_DIR: stateDir },
  ...
  it("keeps keyed mutations atomic without changing insertion age", async () => {
    await withTestDir({ prefix: "afora-audit-store-upsert-" }, async (stateDir) => {
      const store = createSqliteAuditRecordStore<{ value: number }>({
        scope: "upsert-test",
        maxEntries: 2,
        env: { ...process.env, AFORA_STATE_DIR: stateDir },
  ...
  it("orders legacy batches before existing runtime rows", async () => {
    await withTestDir({ prefix: "afora-audit-store-legacy-order-" }, async (stateDir) => {
      const store = createSqliteAuditRecordStore<{ value: number }>({
        scope: "legacy-order",
        maxEntries: 4,
        env: { ...process.env, AFORA_STATE_DIR: stateDir },
