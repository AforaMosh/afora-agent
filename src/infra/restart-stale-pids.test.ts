...
function createAforaBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "afora-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation((command: unknown) => {
    ...
    call += 1;
    if (call === 1) {
      return createAforaBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("finds stale pids when lsof needs seconds to answer", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown, _args: unknown, options: unknown) => {
        ...
        return timeout >= slowLsofMs
          ? createAforaBusyResult(stalePid)
          : createErrnoResult("ETIMEDOUT", "lsof timed out");
      });
      expect(findGatewayPidsOnPortSync(18789)).toStrictEqual([stalePid]);
    });

    it("parses afora-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "afora-gateway" },
          { pid: process.pid, cmd: "afora-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of afora", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/afora/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: afora-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "afora-gateway" },
          { pid: unrelatedStalePid, cmd: "afora-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "afora-gateway" },
            { pid: grandparentPid, cmd: "afora-gateway" },
            { pid: benignStalePid, cmd: "afora-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "afora-gateway" },
            { pid: gatewayGrandparentPid, cmd: "afora-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "afora-gateway" },
              { pid: gatewayGrandparentPid, cmd: "afora-gateway" },
              { pid: benignStalePid, cmd: "afora-gateway" },
            ]),
            stderr: "",
    ...
    it("excludes pids whose command does not include 'afora'", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "afora-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["afora", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple afora pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "afora-gateway" },
          { pid: pid2, cmd: "afora-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-afora pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks afora processes — non-afora listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty afora stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty afora stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + afora pid in stdout — container-restricted lsof reports partial results
          return createAforaBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "afora-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("cleanStaleGatewayProcessesSync", () => {
    ...
    it("continues cleanup and port polling when individual process signals fail", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: termDeniedPid, cmd: "afora-gateway" },
                { pid: killDeniedPid, cmd: "afora-gateway" },
              ]),
            })
          : createLsofResult({ status: 1 });
      });
    ...
    it("does not kill a protected gateway pid after reparenting", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: protectedPid, cmd: "afora-gateway" },
                { pid: stalePid, cmd: "afora-gateway" },
              ]),
            })
          : createLsofResult({ status: 1 });
    ...
    it("refreshes the protected pid after listener enumeration before filtering", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        ...
        if (lsofCall === 1) {
          ...
          return createLsofResult({
            stdout: lsofOutput([
              { pid: replacementPid, cmd: "afora-gateway" },
              { pid: stalePid, cmd: "afora-gateway" },
            ]),
          });
        }
    ...
    it("does not signal the replacement when it is the only enumerated listener", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        ...
        listenerSnapshotCaptured = true;
        return createAforaBusyResult(replacementPid);
      });
      const resolveProtectedPid = vi.fn(() => {
        expect(listenerSnapshotCaptured).toBe(true);
        return replacementPid;
    ...
    it("fails closed when the refreshed protected pid cannot be resolved", () => {
      ...
      mockSpawnSync.mockReturnValue(createAforaBusyResult(listenerPid));
      const resolveProtectedPid = vi.fn(() => {
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'afora'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("afora")
      // inside the mid-loop flush: a non-afora cmd between two entries must not
      // be pushed, but the following afora entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-afora entry first, then afora entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — parse false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid afora entry must still be found.
      const stdout = `p0\ncafora-gateway\np${stalePid}\ncafora-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  describe("pollPortOnce — status 1 + non-empty stdout", () => {
    it("keeps polling when status 1 includes a non-afora listener record", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return lsofCall === 2
              ? createLsofResult({
                  stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "afora-gateway" }]),
                })
              : createLsofResult({ status: 1 });
          }
          return createLsofResult();
