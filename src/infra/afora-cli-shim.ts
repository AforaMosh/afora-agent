...
import {
  ...
  type AforaCliInvocation,
} from "./afora-cli-invocation.js";

const AGENT_CLI_BIN_DIR = path.join("tmp", "agent-cli");
const GATEWAY_AGENT_CLI_STATE_KEY = Symbol.for("afora.gatewayAgentCliShim");
...
function renderPosixShim(invocation: AforaCliInvocation, profile: string | null): string {
...
function renderWindowsShim(invocation: AforaCliInvocation, profile: string | null): string {
  const args = [...invocation.args, ...(profile ? ["--profile", profile] : [])];
...
export async function prepareGatewayAgentCliShim(
  ...
  const invocation = options.invocation ?? resolveCurrentAforaCliInvocation([]);
  ...
  const executablePath = path.join(binDir, platform === "win32" ? "afora.cmd" : "afora");
  const content =
    platform === "win32"
