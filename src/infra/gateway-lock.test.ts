...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "afora-gateway-lock-" });
...
describe("gateway lock", () => {
  ...
  it("blocks concurrent acquisition until release", async () => {
    ...
    const pending = acquireForTest(env, {
      timeoutMs: 15,
      readProcessCmdline: () => ["afora", "gateway", "run"],
    });
  ...
  it("serializes different config paths that resolve to the same state directory", async () => {
    ...
    const envA = {
      ...process.env,
      AFORA_CONFIG_PATH: configA,
      AFORA_STATE_DIR: stateDir,
    };
    const envB = {
      ...process.env,
      AFORA_CONFIG_PATH: configB,
      AFORA_STATE_DIR: stateDir,
    };
  ...
  it("reads the active runtime port across configs that share a state directory", async () => {
    ...
    await fs.writeFile(configB, "{}", "utf8");
    const envB = { ...envA, AFORA_CONFIG_PATH: configB };
    const lock = expectGatewayLock(
      await acquireForTest(envA, {
        platform: "darwin",
        port: 48789,
    ...
    try {
      await expect(
        acquireForTest(env, {
          ...
          port: 18789,
          timeoutMs: 15,
          readProcessCmdline: () => ["afora", "doctor", "--state-sqlite", "compact"],
        }),
      ).rejects.toBeInstanceOf(GatewayLockError);
      expect(connectSpy).not.toHaveBeenCalled();
    } finally {
  ...
  it("keeps lock when configured port is busy and owner pid is alive", async () => {
    ...
    try {
      const pending = acquireForTest(env, {
        ...
        platform: "darwin",
        port: 18789,
        readProcessCmdline: () => ["/usr/local/bin/afora", "gateway", "run"],
        readProcessStartTime: () => 111,
      });
      await expect(pending).rejects.toBeInstanceOf(GatewayLockError);
    } finally {
  ...
  it("keeps state ownership when the config singleton override is enabled", async () => {
    ...
    const lock = expectGatewayLock(
      await acquireGatewayLock({
        allowInTests: true,
        env: { ...env, AFORA_ALLOW_MULTI_GATEWAY: "1", VITEST: "" },
        lockDir: resolveTestLockDir(env),
      }),
    );
  ...
  it("keeps lock on darwin when process cmdline is a gateway", async () => {
    ...
    const pending = acquireForTest(env, {
      ...
      platform: "darwin",
      port: 18789,
      readProcessCmdline: () => ["/usr/local/bin/afora", "gateway", "run", "--port", "18789"],
      readProcessStartTime: () => 111,
    });
    await expect(pending).rejects.toBeInstanceOf(GatewayLockError);
