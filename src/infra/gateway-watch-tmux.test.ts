...
describe("gateway-watch tmux wrapper", () => {
  it("derives stable session names from profile and port", () => {
    expect(resolveGatewayWatchTmuxSessionName({ args: ["gateway", "--force"], env: {} })).toBe(
      "afora-gateway-watch-main",
    );
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--force", "--port", "19001"],
        env: { AFORA_PROFILE: "Dev Profile" },
      }),
    ).toBe("afora-gateway-watch-dev-profile-19001");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["--dev", "gateway", "--port=18789"],
        env: {},
      }),
    ).toBe("afora-gateway-watch-dev");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--dev", "--port=18789"],
        env: {},
      }),
    ).toBe("afora-gateway-watch-main");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--profile", "work", "--port=18789"],
        env: { AFORA_PROFILE: "main" },
      }),
    ).toBe("afora-gateway-watch-work");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--force"],
        env: { AFORA_GATEWAY_PORT: "127.0.0.1:18789" },
      }),
    ).toBe("afora-gateway-watch-main");
  });
  ...
  it("builds a login-shell command that runs the raw watcher in the repo", () => {
    const command = buildGatewayWatchTmuxCommand({
      ...
      cwd: "/repo with spaces/afora",
      env: {
        AFORA_GATEWAY_PORT: "19001",
        AFORA_GATEWAY_RESTART_TRACE: "1",
        AFORA_GATEWAY_STARTUP_TRACE: "1",
        AFORA_GATEWAY_WATCH_AUTO_DOCTOR: "0",
        AFORA_PROFILE: "Dev Profile",
        AFORA_TRACE_SYNC_IO: "0",
        SHELL: "/bin/zsh",
      },
      nodePath: "/opt/node",
      sessionName: "afora-gateway-watch-main",
    });

    expect(command).toContain("exec '/bin/zsh' -lc");
    expect(command).toContain("/repo with spaces/afora");
    expect(command).toContain("'AFORA_GATEWAY_WATCH_TMUX_CHILD=1'");
    expect(command).toContain("'AFORA_GATEWAY_WATCH_SESSION=afora-gateway-watch-main'");
    expect(command).toContain("'\\''-u'\\'' '\\''NO_COLOR'\\''");
    expect(command).toContain("'FORCE_COLOR=1'");
    expect(command).toContain("'AFORA_GATEWAY_PORT=19001'");
    expect(command).toContain("'AFORA_GATEWAY_RESTART_TRACE=1'");
    expect(command).toContain("'AFORA_GATEWAY_STARTUP_TRACE=1'");
    expect(command).toContain("'AFORA_GATEWAY_WATCH_AUTO_DOCTOR=0'");
    expect(command).toContain("'AFORA_PROFILE=Dev Profile'");
    expect(command).toContain("'AFORA_TRACE_SYNC_IO=0'");
    expect(command).toContain("'\\''-u'\\'' '\\''AFORA_SKIP_CHANNELS'\\''");
  ...
  it("stops a loaded service on the watched port with CLI selector precedence", () => {
    ...
    const code = runGatewayWatchServiceHandoff({
      ...
      cwd: "/repo",
      env: { AFORA_GATEWAY_PORT: "19001", AFORA_PROFILE: "main" },
      nodePath: "/opt/node",
      spawnSync,
    ...
    expect(requireRecord(stopOptions.env, "stop env").AFORA_GATEWAY_PORT).toBe("18789");
  ...
  it("consumes benchmark flags and passes the CPU profile dir to the watched child", () => {
    ...
    expect(command).toContain("'AFORA_RUN_NODE_CPU_PROF_DIR=.artifacts/gateway-watch-profiles'");
    expect(command).toContain("'AFORA_RUN_NODE_CPU_PROF_MAX_FILES=40'");
    expect(command).toContain("'AFORA_TRACE_SYNC_IO=0'");
  ...
  it("preserves an explicit benchmark CPU profile retention cap", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--benchmark"],
      cwd: "/repo",
      env: { AFORA_RUN_NODE_CPU_PROF_MAX_FILES: "8", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(code).toBe(0);
    const command = spawnShellCommand(spawnSync);
    expect(command).toContain("'AFORA_RUN_NODE_CPU_PROF_MAX_FILES=8'");
  });
  ...
  it("preserves explicit sync I/O tracing in benchmark mode", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--benchmark"],
      cwd: "/repo",
      env: { AFORA_TRACE_SYNC_IO: "1", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(code).toBe(0);
    const command = spawnShellCommand(spawnSync);
    expect(command).toContain("'AFORA_TRACE_SYNC_IO=1'");
    expect(command).toContain(
      "'AFORA_RUN_NODE_OUTPUT_LOG=.artifacts/gateway-watch-profiles/gateway-watch-output.log'",
    );
    expect(command).toContain("'AFORA_RUN_NODE_FILTER_SYNC_IO_STDERR=1'");
    expect(stderr.chunks.join("")).toContain(
  ...
  it("creates a detached tmux session when none exists", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 0, "tmux", ["has-session", "-t", "afora-gateway-watch-main"])
        .encoding,
    ).toBe("utf8");
    ...
    expect(
      expectSpawn(spawnSync, 5, "tmux", [
        ...
        "-t",
        "afora-gateway-watch-main",
        "AFORA_GATEWAY_WATCH_CWD",
        "/repo",
      ]).encoding,
    ).toBe("utf8");
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch started in tmux session afora-gateway-watch-main",
    );
    expect(stdout.chunks.join("")).toContain("tmux attach -t afora-gateway-watch-main");
    expect(stdout.chunks.join("")).toContain(
      "tmux capture-pane -ep -t afora-gateway-watch-main -S -200",
    );
    expect(stdout.chunks.join("")).toContain(
      "tmux show-options -v -t afora-gateway-watch-main @afora.gateway_watch.cwd",
    );
  });
  ...
  it("auto-attaches in an interactive terminal after creating a session", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 6, "tmux", ["attach-session", "-t", "afora-gateway-watch-main"])
        .stdio,
    ).toBe("inherit");
  ...
  it("switches tmux clients instead of nesting attach when already inside tmux", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 5, "tmux", ["switch-client", "-t", "afora-gateway-watch-main"])
        .stdio,
    ).toBe("inherit");
  });
  ...
  it("respawns the existing tmux pane on repeated runs", () => {
    ...
    expect(String(respawnArgs[6])).toContain("AFORA_GATEWAY_WATCH_AUTO_DOCTOR=0");
    expect(String(respawnArgs[6])).toContain("AFORA_SKIP_CHANNELS");
    expect(requireRecord(respawnCall[2], "spawn options").encoding).toBe("utf8");
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch restarted in tmux session afora-gateway-watch-dev-19001",
    );
  });
  ...
  it("recreates a stale session when its active pane target is missing", () => {
    ...
    expect(
      expectSpawn(spawnSync, 2, "tmux", ["kill-session", "-t", "afora-gateway-watch-main"])
        .encoding,
    ).toBe("utf8");
  ...
  it("runs the raw foreground watcher when tmux mode is disabled", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force"],
      cwd: "/repo",
      env: { AFORA_GATEWAY_WATCH_TMUX: "0" },
      nodePath: "/node",
      spawnSync,
    });
