  ...
  providerRuntimeMock: {
    ...
    resolveProviderUsageAuthWithPlugin: vi.fn(async (params) => {
      ...
      if (params.provider === "anthropic") {
        ...
        if (adminKey) {
          return {
            token: `afora:anthropic-admin:v1:${JSON.stringify({ token: adminKey })}`,
          };
        }
        const candidates =
        ...
        if (storedAdminKey) {
          return {
            token: `afora:anthropic-admin:v1:${JSON.stringify({ token: storedAdminKey })}`,
          };
        }
      ...
      if (params.provider === "openai") {
        ...
        if (adminKey) {
          return { token: `afora:openai-admin:v1:${JSON.stringify({ token: adminKey })}` };
        }
        const oauth = await params.context.resolveOAuthToken();
        if (oauth) {
          return oauth;
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "afora-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".afora");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".afora", "agents", "main", "agent");
  }
  ...
  it("routes the dedicated OpenAI admin key to the provider-owned usage path", async () => {
    ...
    await expectResolvedAuthsFromSuiteHome({
      ...
      expected: [
        {
          provider: "openai",
          token: 'afora:openai-admin:v1:{"token":"env-openai-admin-key"}',
        },
      ],
    });
  });
  ...
  it("routes Anthropic Admin API keys to provider cost usage", async () => {
    await expectResolvedAuthsFromSuiteHome({
      ...
      expected: [
        {
          provider: "anthropic",
          token: 'afora:anthropic-admin:v1:{"token":"sk-ant-admin-status-key"}',
        },
      ],
    });
  });
  ...
  it("selects a stored Anthropic Admin key before coexisting OAuth and API auth", async () => {
    await expectResolvedAuthsFromSuiteHome({
      ...
      expected: [
        {
          provider: "anthropic",
          token: 'afora:anthropic-admin:v1:{"token":"sk-ant-admin-billing"}',
        },
      ],
    });
  });
