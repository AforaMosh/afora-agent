// Tests Afora home directory resolution.
...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers AFORA_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands AFORA_HOME when set to ~", () => {
    const env = {
      AFORA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for AFORA_HOME",
      env: { AFORA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when AFORA_HOME is tilde-only and no fallback home exists",
      env: { AFORA_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
  ...
  it("fails clearly when both home and cwd are unavailable", () => {
    ...
    try {
      expect(() => resolveRequiredHomeDir({}, noHome)).toThrow(/set AFORA_HOME/i);
...
describe("resolveOsHomeDir", () => {
  it("ignores AFORA_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          AFORA_HOME: "/srv/afora-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/afora-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/afora-home " },
      expected: "/srv/afora-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/afora-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of AFORA_HOME", () => {
    expect(
