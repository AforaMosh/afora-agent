...
describe("managed npm root", () => {
  ...
  it("syncs Afora-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads workspace pnpm overrides for managed plugin installs", async () => {
    ...
    await expect(readAforaManagedNpmRootOverrides()).resolves.toEqual(expectedOverrides);
  });
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readAforaManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "managed-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@afora/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@afora/discord/-/discord-2026.5.2.tgz",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for afora@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
  ...
  it("repairs stale managed afora peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    ...
    await fs.writeFile(
      path.join(npmRoot, "node_modules", "afora", "package.json"),
      `${JSON.stringify({ name: "afora", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "afora"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "afora.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "afora.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootAforaPeer({ npmRoot, runCommand })).resolves.toBe(true);
    expect(runCommand).toHaveBeenCalledTimes(1);
    ...
    expect(lockfile.packages?.["node_modules/@afora/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.afora).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "afora"));
    for (const binName of ["afora", "afora.cmd", "afora.ps1"]) {
  ...
  it("does not repair the active Afora host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/afora-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "afora", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "afora-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "afora"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "afora"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "afora.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "afora.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/afora-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "afora"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/afora"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/afora-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.afora).toBeUndefined();
    for (const binName of ["afora", "afora.cmd", "afora.ps1"]) {
