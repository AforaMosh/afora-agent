// Covers preferred Afora temp directory resolution.
import { constants as fsConstants } from "node:fs";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { DEFAULT_POSIX_TMP_ROOT, resolvePreferredAforaTmpDir } from "./tmp-afora-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAforaTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `afora-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredAforaTmpDir({
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredAforaTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredAforaTmpDir", () => {
  it("prefers /tmp/afora when it already exists and is writable", () => {
  ...
  it("prefers /tmp/afora when it does not exist but /tmp is writable", () => {
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/afora when /tmp/afora is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/afora when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/afora exists but is not writable",
    ...
    {
      name: "falls back when /tmp/afora is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/afora is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/afora is group/other writable",
  ...
  it("repairs existing /tmp/afora permissions when they are too broad", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      `[afora] tightened permissions on temp dir: ${DEFAULT_POSIX_TMP_ROOT}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/afora after create when the initial mode stays too broad", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Afora temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "afora");
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[afora] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/afora when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/afora when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === DEFAULT_POSIX_TMP_ROOT && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    ...
    ).toThrow(/Unable to create fallback Afora temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/afora` and routed
    // log files / TTS temp files there instead of `%TEMP%\afora`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "afora-501");
    ...
    const result = resolvePreferredAforaTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
