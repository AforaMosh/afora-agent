...
import { triggerAforaRestart } from "./restart.js";
...
const PNPM_VERSIONED_AFORA_ENTRY_PATTERN =
  /^(.*?)([\\/])node_modules\2\.pnpm\2afora@[^\\/]+\2node_modules\2afora\2.+$/;

function rewritePnpmVersionedAforaEntryPath(entryPath: string): string {
  // pnpm can expose argv[1] as a versioned realpath that self-update removes.
  // Respawn through the stable Afora package wrapper instead.
  return entryPath.replace(
    PNPM_VERSIONED_AFORA_ENTRY_PATTERN,
    "$1$2node_modules$2afora$2afora.mjs",
  );
}
...
function spawnDetachedGatewayProcess(opts: GatewayRespawnOptions = {}): {
  ...
  const args = [
    ...process.execArgv,
    ...(entryArg ? [rewritePnpmVersionedAforaEntryPath(entryArg)] : []),
    ...entryArgs,
  ];
 ...
 * - AFORA_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthyEnvValue(process.env.AFORA_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerAforaRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthyEnvValue(process.env.AFORA_NO_RESPAWN)) {
    return { mode: "disabled", detail: "AFORA_NO_RESPAWN" };
  }
  try {
