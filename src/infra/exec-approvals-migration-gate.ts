 ...
 * `afora doctor --fix` repairs the default root while a scoped install stays blocked.
...
function doctorFixInstruction(filePath: string): string {
  const command = "Run `afora doctor --fix`";
  return process.env.AFORA_STATE_DIR?.trim()
    ? `${command} with AFORA_STATE_DIR set to ${path.dirname(filePath)}`
    : command;
}
