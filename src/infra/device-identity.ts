...
function resolveLegacyDeviceIdentityPath(options: DeviceIdentityStoreOptions = {}): string {
  ...
  return path.join(
    resolveAforaStateDirForDatabasePath(databasePath),
...
function assertNoPendingLegacyIdentity(options: DeviceIdentityStoreOptions): void {
  ...
  if (
    ...
    throw new Error(
      `Legacy device identity exists at ${legacyPath}. Run "afora doctor --fix" before starting the gateway or connecting this client.`,
    );
  }
}
...
function withDeviceIdentityCoordinator<T>(
  ...
  const coordinator = acquireDeviceIdentityCoordinator({
    ...
    stateDir: resolveAforaStateDirForDatabasePath(resolved.databasePath),
  });
  let result: T;
  try {
