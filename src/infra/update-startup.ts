...
function resolveAutoUpdatePolicy(cfg: AforaConfig): AutoUpdatePolicy {
...
export async function refreshGatewayUpdateStatus(cfg: AforaConfig): Promise<void> {
  const channel = normalizeUpdateChannel(cfg.update?.channel) ?? DEFAULT_PACKAGE_CHANNEL;
  if (channel !== "dev") {
    return;
  }
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.AFORA_NO_AUTO_UPDATE);
  ...
  if (isDevGit) {
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by AFORA_NO_AUTO_UPDATE", {
        version: upstreamSha,
        tag: "dev",
      });
    }
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("afora update")}`,
      );
