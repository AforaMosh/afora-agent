// Resolves Afora home and platform-specific config directories.
...
function resolveRawHomeDir(env: NodeJS.ProcessEnv, homedir: () => string): string | undefined {
  const explicitHome = normalize(env.AFORA_HOME);
  if (!explicitHome) {
...
/** Resolves Afora's effective home, honoring AFORA_HOME before OS homes. */
...
/** Resolves the underlying OS user home, ignoring AFORA_HOME overrides. */
...
export function resolveRequiredHomeDir(
  ...
  throw new Error(
    "Unable to resolve an Afora home: set AFORA_HOME, HOME, or USERPROFILE, or run from an existing directory.",
  );
}
...
/** Resolves a user-supplied path against the OS home, ignoring AFORA_HOME. */
