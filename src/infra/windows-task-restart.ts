...
function buildScheduledTaskRestartScript(params: {
  ...
  const lines = [
    ...
    "setlocal",
    ...setupLines,
    `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] afora restart attempt source=windows-task-handoff target=${quotedTaskName}`,
    ...
    "goto retry",
    ":fallback",
    `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] afora restart fallback source=windows-task-handoff`,
  ];
  if (taskScriptPath) {
  ...
  lines.push(
    ":cleanup",
    `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] afora restart finished source=windows-task-handoff`,
    'del "%~f0" >nul 2>&1',
  );
  return lines.join("\r\n");
}
...
export function relaunchGatewayScheduledTask(env: NodeJS.ProcessEnv = process.env): RestartAttempt {
  ...
  const scriptPath = path.join(
    ...
    `afora-schtasks-restart-${randomUUID()}.cmd`,
  );
