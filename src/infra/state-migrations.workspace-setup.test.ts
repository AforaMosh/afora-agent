...
describe("legacy workspace Doctor migration", () => {
  ...
  function setup() {
    const homeDir = tempDirs.make("afora-workspace-migration-home-");
  ...
  it("detects configured and orphan sources only for explicit Doctor repair", async () => {
    const context = setup();
    const setupPath = path.join(context.workspaceDir, "afora-workspace-state.json");
  ...
  it("imports setup and attestation state, records receipts, and removes files", async () => {
    ...
    const db = openAforaStateDatabase({ env: context.env }).db;
    expect(
      db
        .prepare(
  ...
  it("imports the legacy onboarding completion alias", async () => {
    ...
    const identity = resolveWorkspaceStateIdentity(context.workspaceDir);
    expect(
      openAforaStateDatabase({ env: context.env })
  ...
  it("persists the configured symlink alias during Doctor import", async () => {
    ...
    const aliasContext = {
      ...context,
      cfg: { agents: { defaults: { workspace: workspaceAlias } } } satisfies AforaConfig,
      workspaceDir: workspaceAlias,
    };
    const completedAt = "2026-07-15T10:01:00.000Z";
    await fsp.writeFile(
      path.join(workspaceAlias, "afora-workspace-state.json"),
  ...
  it("removes a stale nested setup marker after the root marker wins", async () => {
    const context = setup();
    const identity = resolveWorkspaceStateIdentity(context.workspaceDir);
    const rootPath = path.join(context.workspaceDir, "afora-workspace-state.json");
    const nestedPath = path.join(context.workspaceDir, ".afora", "workspace-state.json");
  ...
  it.each(["hardlink", "oversized"] as const)(
    ...
    async (kind) => {
      const context = setup();
      const attestationPath = `${context.workspaceDir}.attested`;
      const content = `afora-workspace-attestation:v1\n2026-07-15T11:00:00.000Z\n${kind === "oversized" ? "x".repeat(3_000) : ""}`;
      if (kind === "hardlink") {
  ...
  it("rejects a setup source beneath a symlinked workspace subdirectory", async () => {
    ...
    await fsp.symlink(externalDir, path.join(context.workspaceDir, ".afora"));
  ...
  it("merges complementary legacy milestones into unowned SQLite setup state", async () => {
    ...
    const receipt = db
      ...
      .get(path.join(identity.workspacePath, "afora-workspace-state.json")) as {
      report_json: string;
    };
  ...
  it("uses receipts for idempotent cleanup-only retries", async () => {
    ...
    expect(
      db
        ...
        .get(path.join(identity.workspacePath, "afora-workspace-state.json")),
    ).toEqual({
