...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.afora";
...
export function resolveExecApprovalsDisplayPath(): string {
  ...
  const locator = path.join("state", "afora.sqlite#exec_approvals_config");
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.AFORA_STATE_DIR?.trim()
    ? "$AFORA_STATE_DIR/state/afora.sqlite#exec_approvals_config"
    : `${DEFAULT_EXEC_APPROVALS_STATE_DIR}/state/afora.sqlite#exec_approvals_config`;
}
