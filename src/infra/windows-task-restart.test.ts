...
const resolvePreferredAforaTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".afora", "gateway.cmd");
  }),
);
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredAforaTmpDirMock.mockReset();
    resolvePreferredAforaTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ AFORA_PROFILE: "work" });
    const cmdExePath = getWindowsCmdExePath();

    expect(result.ok).toBe(true);
    expect(result.method).toBe("schtasks");
    expect(result.tried).toContain('schtasks /Run /TN "Afora Gateway (work)"');
    ...
    expect(script).toContain(
      'afora restart attempt source=windows-task-handoff target="Afora Gateway (work)"',
    );
    expect(script).toContain(
      `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "$task = Get-ScheduledTask -TaskName 'Afora Gateway (work)' -ErrorAction SilentlyContinue; if ($null -ne $task -and $task.State -eq 'Running') { exit 0 }; exit 1" >nul 2>&1`,
    );
    expect(script).not.toContain("findstr");
    expect(script).toContain('schtasks /Run /TN "Afora Gateway (work)" >>');
    expect(script.indexOf("powershell.exe -NoProfile")).toBeLessThan(
      script.indexOf('schtasks /Run /TN "Afora Gateway (work)"'),
    );
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers AFORA_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
    ...
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Afora Gateway (custom)" >>');
  });
  ...
  it("escapes custom task names in the PowerShell running-task probe", () => {
    ...
    relaunchGatewayScheduledTask({
      AFORA_WINDOWS_TASK_NAME: "Afora Gateway (Bob's work)",
    });
    ...
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain(
      `-Command "$task = Get-ScheduledTask -TaskName 'Afora Gateway (Bob''s work)' -ErrorAction SilentlyContinue; if ($null -ne $task -and $task.State -eq 'Running') { exit 0 }; exit 1"`,
    );
    expect(script).not.toContain("findstr");
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredAforaTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ AFORA_PROFILE: "work" });
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-state-"));
  ...
  const asciiPathEnv = {
    ...
    AFORA_STATE_DIR: "C:\\ocw-test\\state",
  };
  ...
  it("writes marked code-page bytes for CJK task names that decode back exactly", () => {
    ...
    const result = relaunchGatewayScheduledTask({
      ...asciiPathEnv,
      AFORA_WINDOWS_TASK_NAME: "Afora Gateway (隆)",
    });
    ...
    expect(
      raw
        .toString("latin1")
        .startsWith("@chcp 936 >nul\r\n@rem afora-launcher-encoding=gbk\r\n"),
    ).toBe(true);
    ...
    expect(script.startsWith("@echo off\r\n")).toBe(true);
    expect(script).toContain('schtasks /Run /TN "Afora Gateway (隆)" >>');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });
