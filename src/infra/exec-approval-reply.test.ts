...
vi.mock("./exec-approval-surface.js", () => ({
  describeNativeExecApprovalClientSetup: vi.fn(
    (params: {
      ...
      if (channel === "telegram") {
        return `Approve it from the Web UI or terminal UI for now. ${label} supports native exec approvals for this account. Configure \`${accountPrefix}.execApprovals.approvers\`; if you leave it unset, Afora can infer numeric owner IDs from \`${accountPrefix}.allowFrom\` or direct-message \`${accountPrefix}.defaultTo\` when possible. Leave \`${accountPrefix}.execApprovals.enabled\` unset/\`auto\` or set it to \`true\`.`;
      }
      return null;
    },
  ),
...
describe("exec approval reply helpers", () => {
  ...
  it("distinguishes node approval-inbox access from policy inspection", () => {
    ...
    expect(text).toContain(
      "Inspect the node's effective exec policy with `afora approvals get --node mac-1`.",
    );
    expect(text).not.toContain("`afora dashboard --no-open` or `afora approvals get");
