// Creates temporary Afora directories for runtime scratch work.
import { getWorkerDeploySecureTempRoot } from "../worker/worker-deploy-runtime-registry.js";

/** Preferred shared Afora temp root on POSIX systems when ownership and permissions are safe. */
export const DEFAULT_POSIX_TMP_ROOT = "/tmp/afora";
...
export type ResolvePreferredAforaTmpDirOptions = {
...
/** Resolves a safe Afora temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredAforaTmpDir(
  options: ResolvePreferredAforaTmpDirOptions = {},
): string {
  return loadResolveSecureTempRoot()({
    ...options,
    preferredDir: DEFAULT_POSIX_TMP_ROOT,
    fallbackPrefix: "afora",
    warningPrefix: "[afora]",
    unsafeFallbackLabel: "Afora temp dir",
