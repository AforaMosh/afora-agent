// Builds PATH values for Afora child processes.
...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current Afora process is on.
  try {
  ...
  // Bundled macOS app: `afora` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "afora");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.AFORA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin && cwd) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "afora"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `afora`
...
/**
 * Best-effort PATH bootstrap so skills that require the `afora` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAforaCliOnPath(opts: EnsureAforaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AFORA_PATH_BOOTSTRAPPED)) {
    return;
  }
