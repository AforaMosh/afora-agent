...
export function isAforaCorrectionSemver(version: SemVer): boolean {
  return version.prerelease.length === 1 && typeof version.prerelease[0] === "number";
}

function toAforaComparableVersion(version: SemVer): string {
  if (isAforaCorrectionSemver(version)) {
...
/** Compares prereleases, stable releases, then Afora numeric corrections. */
export function compareAforaSemver(left: SemVer, right: SemVer): number {
  return compareBuild(toAforaComparableVersion(left), toAforaComparableVersion(right));
}

/** Converts legacy Afora `1.2.3.beta.N` tags into valid SemVer prereleases. */
