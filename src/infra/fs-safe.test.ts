...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("afora-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("afora-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("afora-fs-safe-");
  ...
  it("writes external command output within an allowed root", async () => {
    const dir = await tempDirs.make("afora-fs-safe-output-");
  ...
  it.each([
    ...
    const dir = await tempDirs.make("afora-fs-safe-output-name-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("afora-fs-safe-root-");
    const sourceDir = await tempDirs.make("afora-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("afora-fs-safe-root-");
    const outside = await tempDirs.make("afora-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    withEnv({ HOME: fakeHome, AFORA_HOME: fakeHome }, () => {
      const result = expandHomePrefix("~/file.txt");
      expect(path.normalize(result)).toBe(path.join(fakeHome, "file.txt"));
    });

    const root = await tempDirs.make("afora-tilde-test-");
    await withEnvAsync({ HOME: root, AFORA_HOME: root }, async () => {
    ...
    const outsideRoot = await tempDirs.make("afora-tilde-outside-");
    await expectRejectCode(
