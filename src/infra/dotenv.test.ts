...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "AFORA_LIVE_ANTHROPIC_KEYS",
  "AFORA_LIVE_GEMINI_KEY",
  "AFORA_LIVE_OPENAI_KEY",
  "AFORA_GATEWAY_TOKEN",
  "AFORA_GATEWAY_PASSWORD",
  "AFORA_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "AFORA_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "AFORA_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "AFORA_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "AFORA_BUNDLED_SKILLS_DIR=./attacker-skills",
...
describe("loadDotEnv", () => {
  it("loads ~/.afora/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.afora/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        setTestEnvValue("HOME", base);
        const defaultStateDir = path.join(base, ".afora");
        setTestEnvValue("AFORA_STATE_DIR", defaultStateDir);
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "afora", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "AFORA_STATE_DIR=./evil-state",
            "AFORA_CONFIG_PATH=./evil-config.json",
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "AFORA_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "AFORA_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "AFORA_LIVE_GEMINI_KEY=sk-gemini-live",
            "AFORA_LIVE_OPENAI_KEY=sk-openai-live",
            "AFORA_GATEWAY_TOKEN=attacker-token",
            "AFORA_GATEWAY_PASSWORD=attacker-password",
            "AFORA_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
      ...
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          ...
          [
            ...
            "AFORA_AGENT_DIR=./evil-agent",
            `AFORA_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "AFORA_OAUTH_DIR=./evil-oauth",
  ...
  it("blocks AFORA_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "AFORA_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.AFORA_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.AFORA_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "AFORA_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'AFORA_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );
        ...
        expect(process.env.AFORA_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.AFORA_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "AFORA_PINNED_PYTHON=./attacker-python",
            "AFORA_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.AFORA_PINNED_PYTHON;
        delete process.env.AFORA_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.AFORA_PINNED_PYTHON).toBeUndefined();
        expect(process.env.AFORA_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "AFORA_PINNED_PYTHON=/trusted/python",
            "AFORA_PINNED_WRITE_PYTHON=/trusted/write-python",
        ...
        expect(process.env.AFORA_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.AFORA_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "AFORA_LIVE_GEMINI_KEY=sk-gemini-live",
            "AFORA_LIVE_OPENAI_KEY=sk-openai-live",
            "AFORA_GATEWAY_TOKEN=trusted-token",
            "AFORA_GATEWAY_PASSWORD=trusted-password",
            "AFORA_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.AFORA_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.AFORA_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.AFORA_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.AFORA_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.AFORA_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.AFORA_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.AFORA_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "AFORA_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks AFORA_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "AFORA_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when AFORA_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        setTestEnvValue("HOME", base);
        setTestEnvValue("AFORA_STATE_DIR", customStateDir);
        await writeEnvFile(
          path.join(base, ".config", "afora", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.AFORA_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "afora-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
  ...
  it("blocks workspace .env takeover vars before loading the global fallback", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        expect(process.env.AFORA_STATE_DIR).toBe(stateDir);
        expect(process.env.AFORA_CONFIG_PATH).toBeUndefined();
        expect(process.env.AFORA_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.NODE_OPTIONS).toBeUndefined();
        expect(process.env.NODE_REDIRECT_WARNINGS).toBeUndefined();
        expect(process.env.NODE_REPL_EXTERNAL_MODULE).toBeUndefined();
        expect(process.env.NODE_REPL_HISTORY).toBeUndefined();
