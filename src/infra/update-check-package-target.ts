import { normalizeNullableString as toOptionalTrimmedString } from "@afora/normalization-core/string-coerce";
import { readProviderJsonResponse } from "../agents/provider-http-errors.js";
import { runCommandWithTimeout } from "../process/exec.js";
import {
  parseAforaSchemaVersions,
  type AforaSchemaVersions,
} from "../state/afora-schema-versions.js";
...
function parseNpmPackageTargetMetadata(raw: string): {
  ...
  const afora = rec.afora && typeof rec.afora === "object" ? rec.afora : null;
  const schemaVersions =
    parseAforaSchemaVersions(rec["afora.schemaVersions"]) ??
    (afora
      ? parseAforaSchemaVersions((afora as Record<string, unknown>).schemaVersions)
      : undefined);
  return {
...
function packageTargetSpec(params: { target: string; spec?: string }): string {
  ...
  return spec || `afora@${params.target.trim() || "latest"}`;
}
...
async function fetchNpmPackageTargetStatusFromRegistry(params: {
  ...
  try {
    ...
    const json = await readProviderJsonResponse<{
      ...
      afora?: { schemaVersions?: unknown };
    }>(res, "npm package target status");
    const schemaVersions = parseAforaSchemaVersions(json.afora?.schemaVersions);
    return {
      target: params.target,
