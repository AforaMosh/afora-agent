...
it("stages prompt blobs after a recreated session directory", async () => {
  await withTestDir({ prefix: "afora-legacy-session-store-" }, async (root) => {
...
it("normalizes file-era rows and drops malformed entries", async () => {
  await withTestDir({ prefix: "afora-legacy-session-normalize-" }, async (root) => {
...
it("normalizes compatibility writes before persistence", async () => {
  await withTestDir({ prefix: "afora-legacy-session-write-" }, async (root) => {
    const storePath = path.join(root, "sessions.json");
    const store = {
      malformed: null,
      "agent:main:main": {
