...
const canCreateDirectorySymlinks = (() => {
  ...
  try {
    probeDir = fsSync.mkdtempSync(
      path.join(os.tmpdir(), "afora-install-safe-dir-symlink-probe-"),
    );
...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@afora/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@afora/matrix", id: "matrix", expected: true },
    { packageName: "@afora/matrix", id: "@afora/matrix", expected: true },
    { packageName: "@afora/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@afora/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@afora__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTestDir({ prefix: "afora-install-safe-" }, async (baseDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTestDir({ prefix: "afora-install-safe-" }, async (baseDir) => {
        await withTestDir({ prefix: "afora-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTestDir({ prefix: "afora-install-safe-" }, async (parentDir) => {
