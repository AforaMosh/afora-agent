...
type AgentCacheDatabase = Pick<AforaAgentKyselyDatabase, "cache_entries">;
...
function readCacheDatabase<T>(
  ...
  try {
    const result = withAforaAgentDatabaseReadOnly(operation, {
...
export function acquireSessionCostUsageRefreshLock(
  ...
  const acquired = runAforaAgentWriteTransaction(
    (database) => {
      const kysely = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
      const currentRaw =
        executeSqliteQuerySync(
