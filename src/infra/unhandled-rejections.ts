...
const EXCEPTION_HANDLERS_GLOBAL_KEY = Symbol.for("afora.uncaughtException.handlers");
...
function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[afora] Unhandled rejection handler failed:",
...
export function isUncaughtExceptionHandled(error: unknown): boolean {
  for (const handler of exceptionHandlers) {
    try {
      ...
      console.error(
        "[afora] Uncaught exception handler failed:",
        err instanceof Error ? (err.stack ?? err.message) : err,
      );
    }
  }
...
export function installUnhandledRejectionHandler(): void {
  ...
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[afora] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[afora] FATAL unhandled rejection:", formatUncaughtError(reason));
      exitWithTerminalRestore("fatal unhandled rejection", reason, "fatal_unhandled_rejection");
      return;
    }

    if (isConfigError(reason)) {
      console.error("[afora] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
      const exitCode =
    ...
    if (isTransientUnhandledRejectionError(reason)) {
      console.warn(
        "[afora] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[afora] Unhandled promise rejection:", formatUncaughtError(reason));
