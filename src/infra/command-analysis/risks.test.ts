...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "AFORA_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "afora", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("afora channels login --channel whatsapp");
  });
