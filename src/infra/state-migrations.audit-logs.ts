  ...
  try {
    ...
    if (params.source.storage === "raw-archive") {
      ...
      if (!scrubbedRecords.ok) {
        ...
        warnings.push(
          `Retained uncheckpointed ${params.source.label} recovery archive; rerun afora doctor --fix`,
        );
        return result(false);
      }
      if (scrubbedRecords.records.length !== 0) {
        warnings.push(
          `A legacy ${params.source.label} writer appended during recovery; rerun afora doctor --fix to import the retained rows`,
        );
        return result(false);
      }
    ...
    if (scrubbedRecords.records.length !== 0) {
      warnings.push(
        `A legacy ${params.source.label} writer appended during migration; rerun afora doctor --fix to import the retained rows`,
      );
      return result(false);
    }
    ...
    if ((await root.exists(sourceRelativePath)) && !params.recreatedSourceScheduled) {
      warnings.push(
        `An old writer recreated ${params.source.label} at ${params.source.logicalSourcePath}; rerun afora doctor --fix to import the retained rows`,
      );
    }
    return result(checkpointed);
  } finally {
