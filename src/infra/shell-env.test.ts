...
describe("shell env fallback", () => {
  ...
  it("is disabled by default", () => {
    ...
    expect(shouldEnableShellEnvFallback({ AFORA_LOAD_SHELL_ENV: "0" })).toBe(false);
    expect(shouldEnableShellEnvFallback({ AFORA_LOAD_SHELL_ENV: "1" })).toBe(true);
  });
  ...
  it("uses the same truthy env parsing for deferred fallback", () => {
    ...
    expect(shouldDeferShellEnvFallback({ AFORA_DEFER_SHELL_ENV_FALLBACK: "false" })).toBe(false);
    expect(shouldDeferShellEnvFallback({ AFORA_DEFER_SHELL_ENV_FALLBACK: "yes" })).toBe(true);
  });
  ...
  it("resolves timeout from env with default fallback", () => {
    ...
    expect(resolveShellEnvFallbackTimeoutMs({ AFORA_SHELL_ENV_TIMEOUT_MS: "42" })).toBe(42);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        AFORA_SHELL_ENV_TIMEOUT_MS: "nope",
      }),
    ).toBe(15000);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        AFORA_SHELL_ENV_TIMEOUT_MS: "42abc",
      }),
    ).toBe(15000);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        AFORA_SHELL_ENV_TIMEOUT_MS: String(Number.MAX_SAFE_INTEGER),
      }),
  ...
  it("imports missing expected keys even when another expected key already exists", () => {
    const env: NodeJS.ProcessEnv = { AFORA_GATEWAY_TOKEN: "set" };
    const exec = vi.fn(() =>
      Buffer.from(
        "AFORA_GATEWAY_TOKEN=from-shell\0TWILIO_ACCOUNT_SID=AC123\0TWILIO_AUTH_TOKEN=secret\0TWILIO_FROM_NUMBER=+15550001234\0",
      ),
    );
    ...
    expect(env.AFORA_GATEWAY_TOKEN).toBe("set");
  ...
  it("prefers the login-shell executable over a daemon PATH candidate when requested", () => {
    ...
    const root = tempDirs.make("afora-shell-path-");
