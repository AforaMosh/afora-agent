...
const SUPERVISOR_HINTS = {
  launchd: ["AFORA_LAUNCHD_LABEL"],
  systemd: ["AFORA_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["AFORA_WINDOWS_TASK_NAME"],
} as const;
...
interface DetectRespawnSupervisorOptions {
  includeLinuxAforaGatewayServiceMarker?: boolean;
}
...
function hasAforaGatewayServiceMarker(env: NodeJS.ProcessEnv): boolean {
  return (
    env.AFORA_SERVICE_MARKER?.trim() === "afora" &&
    env.AFORA_SERVICE_KIND?.trim() === "gateway"
  );
}

function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.AFORA_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "linux") {
    return hasAnyHint(env, SUPERVISOR_HINTS.systemd) ||
      (options.includeLinuxAforaGatewayServiceMarker === true &&
        hasAforaGatewayServiceMarker(env))
      ? "systemd"
      : null;
  }
  if (platform === "win32") {
    ...
    const marker = env.AFORA_SERVICE_MARKER?.trim();
