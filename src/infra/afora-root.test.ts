// Covers Afora package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__afora_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "afora-root");
...
function setPackageRoot(root: string, name = "afora") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./afora-root.js").resolveAforaPackageRootSync,
  asyncResolver: typeof import("./afora-root.js").resolveAforaPackageRoot,
  opts: Parameters<typeof import("./afora-root.js").resolveAforaPackageRootSync>[0],
...
vi.mock("./afora-root.fs.runtime.js", () => ({
  aforaRootFsSync: mockFsModule(),
  aforaRootFs: mockFsPromisesModule(),
}));

describe("resolveAforaPackageRoot", () => {
  let resolveAforaPackageRoot: typeof import("./afora-root.js").resolveAforaPackageRoot;
  let resolveAforaPackageRootSync: typeof import("./afora-root.js").resolveAforaPackageRootSync;

  beforeAll(async () => {
    ({ resolveAforaPackageRoot, resolveAforaPackageRootSync } =
      await import("./afora-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "afora");
        const pkgRoot = path.join(project, "node_modules", "afora");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "afora");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "afora.mjs")));
        setPackageRoot(realPkg);
        return { opts: { argv1: bin }, expected: realPkg };
      },
    },
    {
      name: "prefers a symlink target nested under another afora package",
      setup: () => {
        const sourceRoot = fx("nested-symlink-scenario");
        const bin = path.join(sourceRoot, ".artifacts", "prefix", "bin", "afora");
        ...
        state.realpaths.set(abs(bin), abs(path.join(installedRoot, "afora.mjs")));
    ...
    {
      name: "falls through from a non-afora moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-afora");
    ...
    {
      name: "returns null for non-afora package roots",
      setup: () => {
        const pkgRoot = fx("not-afora");
        setPackageRoot(pkgRoot, "not-afora");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "afora.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "afora");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    {
      name: "still resolves the afora package below a node_modules boundary",
      setup: () => {
