...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.afora.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  ...
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const aforaPayload = requireRecord(payload.afora, "afora payload");
    expectRecordFields(aforaPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof aforaPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.afora.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open Afora to review this request.",
      },
      sound: "default",
      category: "afora.exec-approval",
      "content-available": 1,
    });
    const aforaPayload = requireRecord(payload.afora, "afora payload");
    expectRecordFields(aforaPayload, {
    ...
    expect(typeof aforaPayload.ts).toBe("number");
    expectNoProperties(aforaPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("builds plugin approval alerts with request copy and a bounded body", async () => {
    ...
    expect(payload.aps).toEqual({
      ...
      sound: "default",
      category: "afora.plugin-approval",
      "content-available": 1,
    });
    const aforaPayload = requireRecord(payload.afora, "afora payload");
    expectRecordFields(aforaPayload, {
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    const payload = requirePayload(requireSendRequest(send));
    expectRecordFields(requireRecord(payload.afora, "afora payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
