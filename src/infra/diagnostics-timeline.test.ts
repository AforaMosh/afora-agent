...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "afora-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      AFORA_DIAGNOSTICS: "timeline",
      AFORA_DIAGNOSTICS_RUN_ID: "run-1",
      AFORA_DIAGNOSTICS_ENV: "env-1",
      AFORA_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, AFORA_DIAGNOSTICS: "1" } })).toBe(true);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, AFORA_DIAGNOSTICS: "yes" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, AFORA_DIAGNOSTICS: "on" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, AFORA_DIAGNOSTICS: "all" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, AFORA_DIAGNOSTICS: "*" } })).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, AFORA_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({ env: { ...env, AFORA_DIAGNOSTICS: "telegram.http" } }),
    ).toBe(false);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, AFORA_DIAGNOSTICS: "0" } })).toBe(
      false,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, AFORA_DIAGNOSTICS_TIMELINE_PATH: "" },
      }),
    ).toBe(false);
  });
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.AFORA_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as AforaConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as AforaConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as AforaConfig;
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("afora.diagnostics.v1");
  ...
  it("routes timeline write failures through the captured console boundary once", async () => {
    ...
    const failingEnv = {
      ...env,
      AFORA_DIAGNOSTICS_TIMELINE_PATH: join(blockingFile, "timeline.jsonl"),
    };
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.AFORA_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as AforaConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
