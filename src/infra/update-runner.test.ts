...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "afora-update-" });
...
describe("runGatewayUpdate", () => {
  const preflightPrefixPattern = /(?:afora-update-preflight-|ocu-pf-)/;
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "afora.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "afora.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "afora", version: "1.0.0" };
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        `#!${process.execPath}\n` +
          ...
          `fs.writeFileSync(process.env.AFORA_UPDATE_TEST_CHILD_PID_PATH, String(child.pid));\n` +
          `setInterval(() => {}, 1000);\n`,
        "utf-8",
      );
      ...
      try {
        await withEnvAsync(
          {
            AFORA_UPDATE_TEST_CHILD_PID_PATH: childPidPath,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const packageRoot = path.join(installDir, "node_modules", "afora");
      const storeRoot = path.join(tempDir, "pnpm-home", "store", "v11", "links", "afora");
      await fs.mkdir(path.dirname(packageRoot), { recursive: true });
      await fs.mkdir(storeRoot, { recursive: true });
      await Promise.all([
        fs.writeFile(
          path.join(installDir, "package.json"),
          JSON.stringify({ private: true, dependencies: { afora: "1.0.0" } }),
          "utf8",
        ),
        fs.writeFile(
          path.join(storeRoot, "package.json"),
          JSON.stringify({ name: "afora", version: "1.0.0" }),
          "utf8",
        ),
      ]);
      await Promise.all([
        fs.symlink(storeRoot, packageRoot, "dir"),
        fs.symlink(installDir, path.join(globalRoot, "hash-afora"), "dir"),
      ]);
  ...
  it("skips an unrelated enclosing git root before the Afora checkout", async () => {
  ...
  async function writePreflightPackageManagerFixture(root: string, packageManager = "pnpm@8.0.0") {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "afora", version: "1.0.0", packageManager }),
      "utf-8",
    );
  }
  ...
  async function createDevGitRunner(params?: {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "afora.mjs")} doctor --non-interactive --fix`;
  ...
  async function createTrackedGitFixture(detached: boolean) {
    ...
    await runRealGit(sourceRoot, "config", "user.name", "Afora Test");
    await runRealGit(sourceRoot, "config", "user.email", "afora@example.com");
    await fs.writeFile(
      path.join(sourceRoot, "package.json"),
      JSON.stringify({ name: "afora", version: "1.0.0", packageManager: "pnpm@10.0.0" }),
    );
    await fs.writeFile(path.join(sourceRoot, "afora.mjs"), "export {};\n");
    await fs.writeFile(path.join(sourceRoot, "README.md"), "base\n");
    await runRealGit(sourceRoot, "add", "package.json", "afora.mjs", "README.md");
    ...
    await runRealGit(localRoot, "config", "user.name", "Afora Test");
    await runRealGit(localRoot, "config", "user.email", "afora@example.com");
    if (detached) {
  ...
  const npmGlobalInstallCommand = (spec: string, extraArgs: string[] = []) => {
    const allowScriptsIdentity = spec.toLowerCase().startsWith("afora@") ? "afora" : spec;
    return [
      "npm",
      "i",
      "-g",
    ...
    const { runner, calls } = createRunner({
      ...
      [`git -C ${tempDir} show ${upstreamSha}:package.json`]: {
        stdout: JSON.stringify({
          afora: { schemaVersions: { state: 3, agent: 11 } },
        }),
      },
  ...
  it("uses pnpm highest resolution mode for update installs", async () => {
    ...
    const { runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${doctorNodePath} ${path.join(tempDir, "afora.mjs")} doctor --non-interactive --fix`,
  ...
  it("marks git update doctor passes for configured-plugin repair deferral when requested", async () => {
    ...
    expect(doctorEnv?.AFORA_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART).toBe("1");
    expect(doctorEnv?.AFORA_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR).toBe("1");
    expect(doctorEnv?.AFORA_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION).toBe("1");
  });
  ...
  it("uses the pre-mutation activation decision for the git update doctor pass", async () => {
    ...
    let doctorEnv: NodeJS.ProcessEnv | undefined;
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "afora.mjs")} doctor --non-interactive`;
    const { runCommand } = createGitInstallRunner({
      stableTag,
      installCommand: "pnpm install",
      buildCommand: "pnpm build",
    ...
    expect(result.status).toBe("ok");
    expect(doctorEnv?.AFORA_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR).toBe("0");
    expect(doctorEnv?.AFORA_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION).toBe("0");
    expect(doctorEnv?.AFORA_SERVICE_REPAIR_POLICY).toBeUndefined();
  });
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      "pnpm build": { stdout: "" },
      "pnpm ui:build": { stdout: "" },
      [`${doctorNodePath} ${path.join(tempDir, "afora.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      buildCommand: "pnpm build",
      uiBuildCommand: "pnpm ui:build",
      doctorCommand: `${process.execPath} ${path.join(tempDir, "afora.mjs")} doctor --non-interactive`,
      onCommand: (key, options) => {
        if (key === "pnpm --version") {
          const envPath = options?.env?.PATH ?? options?.env?.Path ?? "";
          if (envPath.includes("afora-update-pnpm-")) {
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    expect(calls).not.toContain("pnpm ui:build");
    expect(pnpmEnvPaths.filter((envPath) => envPath.includes("afora-update-pnpm-"))).not.toEqual(
      [],
    );
  });
  ...
  it("runs dev preflight lint in constrained mode when explicitly enabled", async () => {
    ...
    const result = await withEnvAsync({ AFORA_UPDATE_PREFLIGHT_LINT: "1" }, async () =>
    ...
    expect(lintEnv[0]?.AFORA_LOCAL_CHECK).toBe("1");
    expect(lintEnv[0]?.AFORA_LOCAL_CHECK_MODE).toBe("throttled");
  });
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "afora.mjs")} doctor --non-interactive --fix`;
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "afora", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "afora", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  const createGlobalInstallHarness = (params: {
    ...
    const runCommand = async (argv: string[], options?: { env?: NodeJS.ProcessEnv }) => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          stdout: JSON.stringify([{ filename: "afora-2.0.0.tgz" }]),
          stderr: "",
          code: 0,
        };
      }
        ...
        if (installCommandMatches(params.installCommand, normalizedInstallCommand)) {
          const packageRoot =
            process.platform === "win32"
              ? path.join(installPrefix, "node_modules", "afora")
              : path.join(installPrefix, "lib", "node_modules", "afora");
  ...
  it.each([
    ...
    },
    {
      ...
      expectedInstallCommand: npmGlobalInstallCommand("afora@beta"),
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const sourceSpec = "github:AforaMosh/afora-agent#main";
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: (argv) =>
        argv[0] === "npm" &&
        argv[1] === "i" &&
        argv[2] === "-g" &&
        argv[3] === "--allow-scripts=./afora-2.0.0.tgz" &&
        path.basename(argv[4] ?? "") === "afora-2.0.0.tgz" &&
    ...
    const installCall = calls.find((call) => call.includes("afora-2.0.0.tgz"));
  ...
  it("runs doctor after global npm updates before reporting success", async () => {
    ...
    expect(result.steps.map((step) => step.name)).toContain("afora doctor");
    expect(doctorEnv?.AFORA_UPDATE_IN_PROGRESS).toBe("1");
    expect(doctorEnv?.AFORA_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
    expect(doctorEnv?.AFORA_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART).toBe("1");
    expect(doctorEnv?.AFORA_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR).toBe("1");
    expect(doctorEnv?.AFORA_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION).toBe("0");
    expect(doctorEnv?.AFORA_COMPATIBILITY_HOST_VERSION).toBe("2.0.0");
  });
  ...
  it("fails global npm updates when post-update doctor fails", async () => {
    ...
    expect(lastStep?.name).toBe("afora doctor");
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "afora");
    const staleDir = path.join(nodeModules, ".afora-stale");
    ...
    const { runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async (options) => {
        ...
        if (options?.installPrefix) {
          ...
          await fs.writeFile(path.join(binDir, "afora"), "#!/bin/sh\n", "utf-8");
        }
      },
    });
  ...
  it("uses clean staged npm swaps for pnpm installs that resolve to an npm global root", async () => {
    ...
    const npmPrefixedGlobalInstallCalls = calls.filter((call) =>
      call.startsWith("npm i -g --allow-scripts=afora --prefix "),
    );
  ...
  it("uses AFORA_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = npmGlobalInstallCommand(
      "http://10.211.55.2:8138/afora-next.tgz",
    );
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
    ...
    await withEnvAsync(
      { AFORA_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/afora-next.tgz" },
      async () => {
        const result = await runWithCommand(runCommand, { cwd: pkgRoot });
        expect(result.status).toBe("ok");
      },
  ...
  it("rejects git roots that are not a afora checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(tempDir);
    const { runner, calls } = createRunner({
      [`git -C ${tempDir} rev-parse --show-toplevel`]: { stdout: tempDir },
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-afora-root");
    expect(calls.filter((call) => call.includes("status --porcelain"))).toEqual([]);
  });

  it("fails with a clear reason when afora.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "afora.mjs"), { force: true });
    ...
    expect(result.steps.some((step) => step.name === "afora doctor entry")).toBe(true);
