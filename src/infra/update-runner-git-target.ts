...
import {
  parsePackageAforaSchemaVersions,
...
type GitTargetSchemaMetadata =
  | { status: "ok"; schemaVersions?: AforaSchemaVersions }
...
async function readGitTargetSchemaVersions(params: {
  ...
  try {
    const schemaVersions = parsePackageAforaSchemaVersions(JSON.parse(result.stdout) as unknown);
