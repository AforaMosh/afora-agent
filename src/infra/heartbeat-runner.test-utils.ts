...
export async function withTempHeartbeatSandbox<T>(
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), options?.prefix ?? "afora-hb-"));
  const storePath = path.join(tmpDir, "sessions.json");
  const replySpy = createHeartbeatReplySpy();
  const previousEnv = new Map<string, string | undefined>();
  const envNames = new Set(["AFORA_STATE_DIR", ...(options?.unsetEnvVars ?? [])]);
  for (const envName of envNames) {
    previousEnv.set(envName, process.env[envName]);
    process.env[envName] = envName === "AFORA_STATE_DIR" ? path.join(tmpDir, "state") : "";
  }
