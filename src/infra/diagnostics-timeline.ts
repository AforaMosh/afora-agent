...
const AFORA_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "afora.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.AFORA_DIAGNOSTICS)) &&
    typeof env.AFORA_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.AFORA_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: AFORA_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.AFORA_DIAGNOSTICS_RUN_ID ? { runId: env.AFORA_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.AFORA_DIAGNOSTICS_ENV ? { envName: env.AFORA_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.AFORA_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
