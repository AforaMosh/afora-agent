...
function ruleRow(params?: {
  ...
  return {
    DisplayName: params?.displayName ?? "Afora Gateway",
...
describe("Windows Gateway firewall diagnostics", () => {
  ...
  it.each([
    [
      ...
      "Afora could not quickly inspect Windows Firewall LAN Gateway policy.",
    ],
    [
      "truncated stdout",
    ...
    [
      "malformed JSON",
      async () => ({ code: 0, stdout: "{" }),
      "Afora could not parse Windows Firewall LAN Gateway policy.",
    ],
    [
      "a runner exception",
      async () => {
