...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AFORA_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*afora-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "afora";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "afora");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "afora-gw");
  ...
  records.push(`_afora-gw._tcp IN PTR ${instanceLabel}._afora-gw._tcp`);
  records.push(`${instanceLabel}._afora-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._afora-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; afora-content-hash: ${contentHash}\n${contentBody}`;
}
