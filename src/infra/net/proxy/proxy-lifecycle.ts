...
import { isHttpUrl, isWebSocketUrl } from "@afora/net-policy/url-protocol";
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    AFORA_PROXY_ACTIVE: process.env["AFORA_PROXY_ACTIVE"],
    AFORA_PROXY_LOOPBACK_MODE: process.env["AFORA_PROXY_LOOPBACK_MODE"],
    AFORA_PROXY_CA_FILE: process.env["AFORA_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["AFORA_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["AFORA_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["AFORA_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["AFORA_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or AFORA_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["AFORA_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["AFORA_PROXY_CA_FILE"],
  });
...
export async function startProxy(config: ProxyConfig | undefined): Promise<ProxyHandle | null> {
  if (
    ...
    (!config?.proxyUrl?.trim() && !process.env["AFORA_PROXY_URL"]?.trim())
  ) {
    return null;
  }
 ...
 * loopback CDP probe to a Chromium instance Afora spawned itself.
 *
