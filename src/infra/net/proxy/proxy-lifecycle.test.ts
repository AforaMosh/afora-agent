...
describe("startProxy", () => {
  ...
  function writeTempCa(contents = "proxy-ca"): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), "afora-proxy-lifecycle-ca-"));
    tempDirs.push(dir);
  ...
  it("uses AFORA_PROXY_URL when config proxyUrl is omitted", async () => {
    process.env["AFORA_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("honors an explicit opt-out when AFORA_PROXY_URL is present", async () => {
    process.env["AFORA_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("prefers config proxyUrl over AFORA_PROXY_URL", async () => {
    process.env["AFORA_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("uses HTTPS proxy URLs from AFORA_PROXY_URL", async () => {
    process.env["AFORA_PROXY_URL"] = "https://127.0.0.1:3128";
  ...
  it("passes configured proxy CA trust to Proxyline", async () => {
    ...
    expect(process.env["AFORA_PROXY_CA_FILE"]).toBe(caFile);
    expect(installGlobalProxyMock).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("does not load configured proxy CA files for plain HTTP proxy URLs", async () => {
    const missingCaFile = path.join(os.tmpdir(), "afora-missing-http-proxy-ca.pem");
  ...
  it("loads inherited HTTPS proxy CA trust for child routing", () => {
    ...
    process.env["AFORA_PROXY_CA_FILE"] = caFile;
  ...
  it("sets process proxy env vars for inherited clients", async () => {
    ...
    expect(process.env["AFORA_PROXY_LOOPBACK_MODE"]).toBe("gateway-only");
  });
  ...
  it("persists loopbackMode in env for forked child CLIs", async () => {
    ...
    expect(process.env["AFORA_PROXY_LOOPBACK_MODE"]).toBe("block");
  ...
  it("restores previous proxy env and stops Proxyline on stop", async () => {
    ...
    expect(process.env["AFORA_PROXY_ACTIVE"]).toBeUndefined();
    expect(proxylineStopMock).toHaveBeenCalledOnce();
    expect(forceResetGlobalDispatcherMock).toHaveBeenCalledTimes(2);
  });
