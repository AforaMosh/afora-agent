// End-to-end proxy smoke tests run child Afora modules through a local
...
async function withDiscordTlsFixture<T>(
  ...
  return await withTestDir(
    {
      prefix: "afora-discord-tls-",
      parentDir: resolvePreferredAforaTmpDir(),
    },
    async (dir) => {
  ...
  it("routes normal HTTP and WebSocket egress through an operator-managed proxy even when NO_PROXY includes loopback", async () => {
    ...
    const child = await runNodeModule(
      `
        ...
        try {
          const response = await undiciFetch(process.env.AFORA_TEST_TARGET_URL, {
            signal: AbortSignal.timeout(${PROBE_TIMEOUT_MS}),
          });
          const body = await response.text();
          const globalFetchResponse = await fetch(process.env.AFORA_TEST_GLOBAL_FETCH_TARGET_URL, {
            signal: AbortSignal.timeout(${PROBE_TIMEOUT_MS}),
          });
          const globalFetchBody = await globalFetchResponse.text();
          const nodeHttp = await nodeHttpGet(process.env.AFORA_TEST_NODE_HTTP_TARGET_URL);
          const explicitAgent = await nodeHttpGet(process.env.AFORA_TEST_EXPLICIT_AGENT_TARGET_URL, {
            agent: new http.Agent(),
          });
          await expectFailure("node:https", () =>
            nodeHttpsProbe(process.env.AFORA_TEST_NODE_HTTPS_TARGET_URL),
          );
          await websocketProbe(process.env.AFORA_TEST_WS_TARGET_URL);
          await gatewayLoopbackBypassProbe(process.env.AFORA_TEST_GATEWAY_BYPASS_WS_URL);
      ...
      {
        ...process.env,
        AFORA_PROXY_URL: `http://127.0.0.1:${proxyPort}`,
        AFORA_TEST_TARGET_URL: `http://127.0.0.1:${targetPort}/private-metadata`,
        AFORA_TEST_GLOBAL_FETCH_TARGET_URL: `http://127.0.0.1:${globalFetchTargetPort}/global-fetch-metadata`,
        AFORA_TEST_NODE_HTTP_TARGET_URL: `http://127.0.0.1:${targetPort}/node-http-metadata`,
        AFORA_TEST_EXPLICIT_AGENT_TARGET_URL: `http://127.0.0.1:${targetPort}/explicit-agent`,
        AFORA_TEST_NODE_HTTPS_TARGET_URL: `https://127.0.0.1:${httpsLikeTargetPort}/https-connect-proof`,
        AFORA_TEST_WS_TARGET_URL: `ws://127.0.0.1:${wsTargetPort}/websocket-proxied`,
        AFORA_TEST_GATEWAY_BYPASS_WS_URL: `ws://127.0.0.1:${gatewayBypassWsTargetPort}/gateway-bypass`,
  ...
  it("preserves the target TLS hostname for Node HTTPS requests through the managed proxy", async () => {
    await withDiscordTlsFixture(async (tlsFixture) => {
      ...
      const child = await runNodeModule(
        ...
        try {
          const response = await nodeHttpsGet(process.env.AFORA_TEST_DISCORD_TLS_URL);
      ...
      `,
        {
          ...
          AFORA_PROXY_URL: `http://127.0.0.1:${proxyPort}`,
          AFORA_TEST_DISCORD_TLS_URL: `https://discord.com:${tlsTargetPort}/tls-proxy-proof`,
          NO_PROXY: "127.0.0.1,localhost",
          no_proxy: "localhost",
        },
      );
