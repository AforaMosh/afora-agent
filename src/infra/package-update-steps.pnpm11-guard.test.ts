...
async function writePnpmIsolatedPackage(params: {
  ...
  await fs.writeFile(
    ...
    JSON.stringify({
      private: true,
      dependencies: { afora: params.version, ...params.dependencies },
    }),
    "utf8",
  );
...
describe("pnpm 11 isolated install preflight", () => {
  it("rejects grouped installs before dropping sibling packages", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-group-" }, async (base) => {
  ...
  it("rejects multiple standalone installs before an alias-wide update", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-multiple-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      await writePnpmIsolatedPackage({
        globalRoot,
        installName: "other",
  ...
  it("rejects an orphaned invoking install before manager probes", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-invoking-orphan-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      const packageRoot = path.join(globalRoot, "orphan", "node_modules", "afora");
  ...
  it("rejects an orphan whose package symlink shares the active store target", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-shared-store-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      const activeInstallRoot = path.join(globalRoot, "active");
      const orphanInstallRoot = path.join(globalRoot, "orphan");
      const activePackageRoot = path.join(activeInstallRoot, "node_modules", "afora");
      const orphanPackageRoot = path.join(orphanInstallRoot, "node_modules", "afora");
      const sharedPackageRoot = path.join(base, "store", "afora");
      await Promise.all([
  ...
  it("uses the owner-reported custom bin without changing pnpm command resolution", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-isolated-" }, async (base) => {
      ...
      const oldPackageRoot = path.join(globalRoot, "old", "node_modules", "afora");
      const newPackageRoot = path.join(globalRoot, "new", "node_modules", "afora");
      ...
      await fs.symlink(path.join(globalRoot, "old"), path.join(globalRoot, "hash-afora"), "dir");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd, env }): Promise<PackageUpdateStepResult> => {
        if (name === "global update") {
          ...
          await fs.rm(path.join(globalRoot, "hash-afora"), { force: true });
          ...
          await Promise.all([
            fs.writeFile(
              path.join(newPackageRoot, "dist", "afora-install-guard"),
              "pending\n",
              "utf8",
            ),
            fs.writeFile(
            ...
            fs.writeFile(
              ...
              JSON.stringify({ private: true, dependencies: { afora: "2.0.0" } }),
              "utf8",
            ),
          ]);
          await fs.symlink(
            path.join(globalRoot, "new"),
            path.join(globalRoot, "hash-afora"),
            "dir",
          );
          ...
          await expect(
            fs.readFile(path.join(newPackageRoot, ".afora-lifecycle-pending"), "utf8"),
          ).resolves.toBe("pending\n");
          await fs.rm(path.join(newPackageRoot, "dist", "afora-install-guard"));
      ...
      await expectPathMissing(path.join(newPackageRoot, ".afora-lifecycle-pending"));
  ...
  it("accepts a replacement pnpm project that reuses the same shared-store package", async () => {
    await withTestDir(
      { prefix: "afora-package-update-pnpm-shared-replacement-" },
      async (base) => {
        const globalDir = path.join(base, "pnpm-home", "global");
        const globalRoot = path.join(globalDir, "v11");
        ...
        const oldPackageRoot = path.join(oldInstallRoot, "node_modules", "afora");
        const newPackageRoot = path.join(newInstallRoot, "node_modules", "afora");
        const sharedPackageRoot = path.join(base, "store", "afora");
        const activeLink = path.join(globalRoot, "hash-afora");
        await Promise.all([
  ...
  it("preserves pnpm local specs before mutating from the owner root", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-relative-spec-" }, async (base) => {
      const globalDir = path.join(base, "pnpm-home", "global");
      const globalRoot = path.join(globalDir, "v11");
      const globalBinDir = path.join(base, "pnpm-home", "bin");
      const callerProjectDir = path.join(base, "caller-project");
      ...
      const cases: Array<{
        ...
        {
          installSpec: "afora@link:./candidate",
          expectedInstallSpec: `afora@link:${path.join(callerProjectDir, "candidate")}`,
        },
        {
        ...
        {
          installSpec: "afora@file:./candidate.tar",
          expectedInstallSpec: `afora@file:${candidateTar}`,
        },
        { installSpec: "candidate.tar", expectedInstallSpec: "candidate.tar" },
        { installSpec: "afora@candidate.tar", expectedInstallSpec: "afora@candidate.tar" },
  ...
  it("probes pnpm from its owner root before rejecting a mismatched major", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-major-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      const globalBinDir = path.join(base, "pnpm-home", "bin");
      const { packageRoot } = await writePnpmIsolatedPackage({
        globalRoot,
  ...
  it("rejects a pnpm command that owns another global root", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-root-" }, async (base) => {
      ...
      expect(result.failedStep?.stderrTail).toContain("not the invoking Afora install");
  ...
  it("rejects a pnpm update that leaves only an orphaned old package root", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-orphan-" }, async (base) => {
      const globalRoot = path.join(base, "pnpm-home", "global", "v11");
      const globalBinDir = path.join(base, "pnpm-home", "bin");
  ...
  it("retries interrupted pnpm package lifecycle repair", async () => {
    await withTestDir({ prefix: "afora-package-update-pnpm-lifecycle-" }, async (base) => {
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        if (name === "global update" && firstAttempt) {
          ...
          await Promise.all([
            fs.writeFile(
              path.join(packageRoot, "dist", "afora-install-guard"),
              "pending\n",
              "utf8",
            ),
            fs.writeFile(
          ...
          await fs.rm(path.join(packageRoot, "dist", "afora-install-guard"));
        }
      ...
      await expect(
        fs.readFile(path.join(packageRoot, ".afora-lifecycle-pending"), "utf8"),
      ).resolves.toBe("pending\n");
      ...
      await expectPathMissing(path.join(packageRoot, ".afora-lifecycle-pending"));
    });
  });
});
