...
import { readExistingAgentSchemaMeta } from "../state/afora-agent-db-schema-helpers.js";
...
const LEGACY_AGENT_DATABASE_BASENAME = "afora-agent.sqlite";
...
export async function migrateLegacySessions(
  ...
  if (detected.sessions.targetStoreAliases.hasFinalSymlink) {
    warnings.push(
      `Deferred legacy session migration in final-component symlink store ${detected.sessions.targetStorePath}; configure one canonical session.store path, then rerun afora doctor --fix`,
    );
    return { changes, warnings };
  }
  ...
  if (!targetReadable) {
    if (options.recoverCorruptTargetStore) {
      ...
      warnings.push(
        `Target sessions store unreadable; left untouched to avoid overwriting at ${detected.sessions.targetStorePath}. Run afora doctor --fix to archive it and retry the legacy merge.`,
      );
    }
  }
  ...
  if (
    ...
    if (normalized.rejectedProtectedKeyCount > 0) {
      warnings.push(
        `Refused legacy session migration because normalization rejected ${normalized.rejectedProtectedKeyCount} existing target session ${normalized.rejectedProtectedKeyCount === 1 ? "key" : "keys"}; left ${detected.sessions.targetStorePath} and ${detected.sessions.legacyStorePath} in place. Repair the conflicting rows, then rerun afora doctor --fix.`,
      );
      return { changes, warnings };
    }
