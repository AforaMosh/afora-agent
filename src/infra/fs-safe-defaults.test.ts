// Covers Afora's default fs-safe native helper configuration.
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.AFORA_FS_SAFE_NATIVE_MODE;
    delete process.env.afora_fs_safe_native_mode;
    delete process.env.FS_SAFE_PYTHON_MODE;
    delete process.env.AFORA_FS_SAFE_PYTHON_MODE;
    delete process.env.FS_SAFE_PYTHON;
    delete process.env.AFORA_FS_SAFE_PYTHON;
    delete process.env.AFORA_PINNED_PYTHON;
    delete process.env.AFORA_PINNED_WRITE_PYTHON;
  });

  it("disables the native helper by default in Afora", async () => {
  ...
  it("honors the Afora-specific env mode override", async () => {
    process.env.AFORA_FS_SAFE_NATIVE_MODE = "auto";
  ...
  it("honors case-insensitive mode overrides on Windows", async () => {
    ...
    process.env.afora_fs_safe_native_mode = "require";
  ...
  it("lets fs-safe migrate legacy require mode without overriding it", async () => {
    process.env.AFORA_FS_SAFE_PYTHON_MODE = "require";
  ...
  it("does not treat a retired interpreter path as a native mode override", async () => {
    process.env.AFORA_FS_SAFE_PYTHON = "/usr/bin/python3";
