// Resolves the Afora package root from runtime and package metadata.
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await aforaRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(aforaRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // the target so a launcher nested under another Afora checkout keeps its own package root.
  try {
    const resolved = aforaRootFsSync.realpathSync(normalized);
...
export async function resolveAforaPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
// Every distinct Afora package root among the runtime hints, in candidate order (symlinked
...
export function resolveAforaPackageRootsSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string[] {
...
export function resolveAforaPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
  return resolveAforaPackageRootsSync(opts)[0] ?? null;
}
