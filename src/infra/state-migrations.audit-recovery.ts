...
export async function scrubLegacyAuditRecoveryArchive(params: {
  ...
  try {
    if (!legacyAuditRawCheckpointsMatch(params.expectedSnapshot, writable.stat)) {
      params.warnings.push(
        `Skipped scrubbing changed ${params.label} legacy recovery archive; rerun afora doctor --fix`,
      );
      return undefined;
    }
    await advanceAuditRecoveryWrite({
  ...
  try {
    ...
    params.warnings.push(
      `Changed ${params.label} legacy recovery archive during scrub verification; rerun afora doctor --fix: ${String(error)}`,
    );
    return undefined;
  }
  ...
  if (!scrubbedPrefix.equals(scrubbedContent)) {
    params.warnings.push(
      `Failed verifying scrubbed ${params.label} legacy recovery archive; rerun afora doctor --fix`,
    );
    return undefined;
  }
...
export async function recordLegacyAuditRawCheckpoint(params: {
  ...
  try {
    ...
    if (!legacyAuditRawCheckpointsMatch(checkpoint, params.snapshot)) {
      params.warnings.push(
        `Retained changed legacy audit backup ${params.rawPath}; rerun afora doctor --fix to import its later rows`,
      );
      return false;
    }
