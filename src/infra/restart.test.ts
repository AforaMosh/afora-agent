...
const { triggerAforaRestart } = await import("./restart.js");
...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-afora/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "cafora", `p${stalePidB}`, "cafora-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerAforaRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    setPlatform("darwin");
    withEnv(
      { VITEST: undefined, NODE_ENV: undefined, HOME: "/Users/test", AFORA_PROFILE: "default" },
      () => {
        ...
        const result = triggerAforaRestart();
        ...
        expect(result).toEqual({
          ...
          tried: [
            ...
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.afora.gateway.plist`,
          ],
        });
      },
    );
      ...
      () => {
        ...
        expect(result).toEqual({
          ...
          tried: [
            `launchctl kickstart -k gui/${uid}/ai.afora.gateway`,
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.afora.gateway.plist`,
            `launchctl kickstart gui/${uid}/ai.afora.gateway`,
          ],
        });
      },
    );
