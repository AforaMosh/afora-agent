...
function buildGenericNativeExecApprovalFallbackText(params?: {
  ...
  if (params?.host === "node") {
    ...
    manualRecovery += ` Inspect the node's effective exec policy with \`afora approvals get --node ${nodeId}\`.`;
  }
  return clients
    ? `Approve it from the Web UI or terminal UI, or enable a native chat approval client such as ${clients}. ${manualRecovery} If those accounts already know your owner ID via allowFrom or owner config, Afora can often infer approvers automatically.`
