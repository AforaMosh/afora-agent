...
const LIVE =
  (isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.AFORA_LIVE_TEST)) &&
  isTruthyEnvValue(process.env.AFORA_LIVE_APNS_REACHABILITY);
const describeLive = LIVE ? describe : describe.skip;

function getLiveTimeoutMs(): number {
  const raw = process.env.AFORA_LIVE_APNS_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_TIMEOUT_MS;
  }
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error(`AFORA_LIVE_APNS_TIMEOUT_MS must be a positive number, got ${raw}`);
  }
