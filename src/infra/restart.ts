...
function emitGatewayRestart(reasonOverride?: string, intent?: GatewayRestartIntent): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerAforaRestart() uses schtasks to restart the gateway.
...
export function triggerAforaRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.AFORA_SYSTEMD_UNIT,
      process.env.AFORA_PROFILE,
    );
  ...
  const label =
    process.env.AFORA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AFORA_PROFILE);
