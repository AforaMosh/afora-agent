...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "afora.internal", expected: "afora.internal." },
    { value: "afora.internal.", expected: "afora.internal." },
    { value: "  afora.internal  ", expected: "afora.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { AFORA_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { AFORA_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { AFORA_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("afora.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "afora.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("afora.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN afora.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_afora-gw._tcp IN PTR studio-london._afora-gw._tcp`,
      `studio-london._afora-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Afora)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/afora`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN afora.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._afora-gw._tcp IN TXT`,
        `displayName=Mac Studio (Afora)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("afora.internal."),
      changed: false,
    });
    expect(replaceFileAtomicSyncMock).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(replaceFileAtomicSyncMock).toHaveBeenCalledWith({
      filePath: getWideAreaZonePath("afora.internal."),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "afora-widearea-dns-fault-"));
      const dnsDir = path.join(stateDir, "dns");
      const zonePath = path.join(dnsDir, "afora.internal.db");
      ...
      try {
        ...
        expect(fs.readdirSync(dnsDir)).toEqual(["afora.internal.db"]);
      } finally {
