...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): AforaConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): AforaConfig {
  return {
