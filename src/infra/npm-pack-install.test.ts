  ...
  return {
    ...
    withInstallWorkspace: vi.fn(
      async (_prefix: string, fn: (tmpDir: string) => Promise<unknown>) => {
        return await fn("/tmp/afora-npm-pack-install-test");
      },
    ),
...
describe("installFromNpmSpecArchiveWithInstaller", () => {
  const baseSpec = "@afora/test@1.0.0";
  const baseArchivePath = "/tmp/afora-test.tgz";
  ...
  it("returns pack errors without invoking installer", async () => {
    ...
    expect(withInstallWorkspace).toHaveBeenCalledWith("afora-test-", expect.any(Function));
  });
  ...
  it("returns resolution metadata and installer result on success", async () => {
    mockPackedSuccess({ name: "@afora/test", version: "1.0.0" });
  ...
  it("aborts when integrity drift callback rejects drift", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: "aborted: npm package integrity drift detected for @afora/test@1.0.0",
    });
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("warns and aborts on drift when no callback is configured", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "Integrity drift detected for @afora/test@1.0.0: expected sha512-old, got sha512-new",
    );
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("rejects prerelease resolutions unless explicitly requested", async () => {
    mockPackedSuccess({
      resolvedSpec: "@afora/test@latest",
      version: "1.1.0-beta.1",
    });
    const installFromArchive = vi.fn(async () => ({ ok: true as const }));

    const result = await runInstall({
      spec: "@afora/test@latest",
      installFromArchive,
    });
  ...
  it("passes archive path and installer params to installFromArchive", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/afora-plugin.tgz",
      metadata: {
        resolvedSpec: "@afora/voice-call@1.0.0",
