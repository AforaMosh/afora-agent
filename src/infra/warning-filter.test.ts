...
const warningFilterKey = Symbol.for("afora.warning-filter");
...
describe("warning filter", () => {
  ...
  it("installs once and suppresses known warnings at emit time", async () => {
    ...
    try {
      ...
      emitWarning("Visible warning", { type: "Warning", code: "AFORA_TEST_WARNING" });
      emitWarning(
        Object.assign(new Error("The punycode module is deprecated."), {
          name: "DeprecationWarning",
          code: "DEP0040",
        }),
        { type: "Warning", code: "AFORA_VISIBLE_OVERRIDE" },
      );
      await flushWarnings();
      expect(
        seenWarnings.find((warning) => warning.code === "AFORA_TEST_WARNING"),
      ).toStrictEqual({
        code: "AFORA_TEST_WARNING",
        name: "Warning",
