...
function timestampBasename(tenant: string, nowMs: number): string {
  const stamp = new Date(nowMs)
    ...
    .replaceAll(":", "-")
    .replace(/\.\d{3}Z$/u, "Z");
  return `afora-fleet-backup-${tenant}-${stamp}.tgz`;
}
...
export async function backupFleetCell(params: {
  ...
  if (inspection.kind === "ok") {
    ...
    if (inspection.running) {
      throw new Error(
        `Fleet cell ${params.record.tenantId} is running; stop it first (afora fleet stop ${params.record.tenantId}) so SQLite state is captured consistently.`,
      );
    }
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(tempRoot, "afora-fleet-backup-"));
...
export async function restoreFleetCell(params: {
  ...
  if (inspectionResult.kind === "missing") {
    throw new Error(
      `Fleet cell container is missing for ${params.record.tenantId}; remove the stale registration without purging data (afora fleet rm ${params.record.tenantId} --force), recreate a stopped cell with the intended image (afora fleet create ${params.record.tenantId} --no-start --image <image>), then retry fleet restore.`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      manifest === null ||
      !("kind" in manifest) ||
      manifest.kind !== "afora-fleet-cell-backup" ||
