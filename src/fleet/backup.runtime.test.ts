...
const tempRoot = createSuiteTempRootTracker({ prefix: "afora-fleet-backup-test-" });
...
function inspection(running = false): Extract<FleetContainerInspectResult, { kind: "ok" }> {
  return {
    ...
    labels: {
      ...
      "afora.fleet.owner": cellOwnerId(record.dataDir),
      "afora.fleet.attempt": ATTEMPT,
      "afora.fleet.env-keys": "",
      "afora.fleet.disk-limit": "10g",
    },
    environment: { AFORA_GATEWAY_TOKEN: "old-token" },
...
describe("fleet restore runtime", () => {
  ...
  it("rejects archives inside cell state and foreign cell networks before mutation", async () => {
    ...
    containers.inspectNetwork.mockResolvedValue({
      kind: "ok",
      labels: { "afora.fleet.tenant": "acme", "afora.fleet.owner": "foreign" },
      attachedContainers: [],
      internal: false,
    });
    await expect(restoreFleetCell(restoreParams(containers, archive))).rejects.toThrow(
  ...
  it.each([
    ...
    await fs.writeFile(
      path.join(source, "manifest.json"),
      JSON.stringify({ schemaVersion: 1, kind: "afora-fleet-cell-backup", tenant: "acme" }),
    );
    await mutate(source);
    const archive = path.join(root, `${path.basename(source)}.tgz`);
  ...
  it("fails preflight on a drifted container before any destructive step", async () => {
    ...
    delete drifted.labels["afora.fleet.env-keys"];
  ...
  it("swaps state, repins config, and rotates the token", async () => {
    ...
    const config = JSON.parse(
      await fs.readFile(path.join(record.dataDir, "afora.json"), "utf8"),
    ) as { gateway?: { controlUi?: { allowedOrigins?: string[] } } };
    expect(config.gateway?.controlUi?.allowedOrigins).toContain("http://127.0.0.1:19100");
    expect(containers.run.mock.calls[0]?.[0].environment.AFORA_GATEWAY_TOKEN).toBe("new-token");
    ...
    expect(containers.run.mock.calls[0]?.[0].environment.AFORA_GATEWAY_TOKEN).not.toBe(
      "old-token",
    );
  ...
  it("stops an unhealthy started replacement so its undelivered token cannot serve", async () => {
    ...
    containers.run.mockImplementation(async () => {
      ...
      running.labels["afora.fleet.attempt"] = NEXT_ATTEMPT;
    });
    let clock = 0;
    let message = "";
    try {
