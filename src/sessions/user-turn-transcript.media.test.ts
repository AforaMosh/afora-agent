...
describe("buildPersistedUserTurnMediaInputsFromFields", () => {
  ...
  it("resolves relative canonical paths against each fact workspace", () => {
    const workspaceDir = "/tmp/afora-user-turn-workspace";
    expect(
      buildPersistedUserTurnMediaInputsFromFields({
        __afora: {
...
describe("buildLateMediaAttachedProjection canonical persistence", () => {
  it.each([
    ...
    {
      name: "facts-only",
      message: { __afora: { media: [{ path: "/media/fact.png" }] } },
    ...
    },
    {
      ...
      message: {
        ...
        __afora: { media: [{ path: "/media/equal.png" }] },
      },
    ...
    const metadata = (testCase.message as { __afora?: Record<string, unknown> })["__afora"];
    const projection = buildLateMediaAttachedProjection({
      role: "user",
      content: "",
      ...testCase.message,
      __afora: { ...metadata, lateMedia: true },
    } as never);
...
describe("buildPersistedUserTurnMessage media projection", () => {
  it.each([
    ...
    {
      name: "explicit MIME",
      media: [{ path: "/tmp/blob.bin", contentType: "application/x-afora" }],
      expectedLegacy: {
        MediaPath: "/tmp/blob.bin",
        MediaPaths: ["/tmp/blob.bin"],
        MediaType: "application/x-afora",
        MediaTypes: ["application/x-afora"],
      },
      expectedMedia: [{ path: "/tmp/blob.bin", contentType: "application/x-afora" }],
    },
    {
      name: "bare kind",
      media: [{ kind: "image" }],
    ...
    expect(
      (message as unknown as { __afora?: { media?: unknown } })["__afora"]?.media,
