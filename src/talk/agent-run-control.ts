/**
 * Runtime adapter for realtime voice control of active Afora agent runs.
 *
...
export async function controlRealtimeVoiceAgentRun(
  ...
  if (mode === "cancel") {
    if (!sessionId) {
      return {
        ...
        message: "There is no active Afora run to cancel.",
        speak: true,
        show: true,
        suppress: false,
      };
    ...
    const message = aborted
      ? "Cancelled the active Afora run."
      : "Afora could not cancel the active run.";
    return {
      ok: aborted,
      mode,
      sessionKey,
  ...
  return {
    ...
    message:
      mode === "followup"
        ? "Queued that follow-up for the active Afora run."
