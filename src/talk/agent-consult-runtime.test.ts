...
function requireEmbeddedAgentCall(runEmbeddedAgent: {
  ...
  if (!call) {
    throw new Error("Expected embedded Afora agent call");
  }
  const [params] = call;
  if (typeof params !== "object" || params === null || Array.isArray(params)) {
    throw new Error("Expected embedded Afora agent params to be an object");
  }
...
describe("realtime voice agent consult runtime", () => {
  beforeEach(async () => {
    ...
    testTempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "afora-talk-consult-")),
    );
    setTestEnvValue("AFORA_STATE_DIR", testTempDir);
  });
  ...
  afterEach(async () => {
    ...
    if (tempDir) {
      closeAforaAgentDatabaseByPath(path.join(tempDir, "afora-agent.sqlite"));
