/**
 * Realtime voice tool definition and helpers for delegating work to Afora.
 *
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME = "afora_agent_consult";
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured Afora agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final Afora result before answering with the actual result.`,
  };
}
...
/** Resolve the Afora tool allowlist paired with the consult exposure policy. */
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  if (config.consultPolicy === "always") {
    return [
      "Consult behavior:",
      "- Call afora_agent_consult before every substantive answer.",
  ...
  return [
    ...
    "- Call afora_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's Afora-specific context.",
...
export function buildRealtimeVoiceSessionInstructions(params: {
  ...
  if (params.isAgentProxy) {
    return [
      ...
      "Mode: Afora agent proxy.",
      "You are the realtime voice surface for the same Afora agent the user can message directly.",
      "Do not mention a backend, supervisor, helper, or separate system. Present the result as your own work.",
      "Delegate substantive requests, actions, tool work, current facts, memory, workspace context, and user-specific context with afora_agent_consult.",
      "Do not block, refuse, or downscope at the voice layer. Delegate to Afora and treat its result as authoritative.",
      "Answer directly only for greetings, acknowledgements, brief latency tests, or filler while waiting.",
      'While waiting for Afora data or tool results, use at most one short natural backchannel such as "yeah", "mm-hmm", "got it", or "one sec"; vary it and do not treat it as the final answer.',
      "When Afora sends an internal exact answer to speak, do not call tools. Say only that answer.",
...
/** Build the delegated Afora agent prompt for a live voice consult. */
export function buildRealtimeVoiceAgentConsultPrompt(params: {
  ...
  return [
    ...
    "Act as the configured Afora agent on behalf of this user. Use available tools when the request asks you to do work.",
