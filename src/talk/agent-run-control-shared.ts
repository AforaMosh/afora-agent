/**
 * Shared realtime voice controls for active Afora agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "afora_agent_control";
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Control an active Afora tool-backed voice run. Use this when the caller asks in any language for status/progress, cancellation, a redirect/change to the active work, or a follow-up after the current work. Do not use this for ordinary greetings or chatter unless the caller is asking about the active work.",
  parameters: {
    type: "object",
    properties: {
      text: {
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal Afora voice control result.",
    "Do not call afora_agent_consult or any other tool for this message.",
    "Speak this exact Afora status to the voice call, without adding, removing, or rephrasing words.",
...
export function buildRealtimeVoiceAgentCancelProviderResult(
  message = "Cancelled the active Afora run.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "Afora is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "Afora has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "Afora could not queue that follow-up."
    : "Afora could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "Afora finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `Afora is starting ${name}.` : "Afora is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `Afora finished ${name} and is continuing.`
        : "Afora finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `Afora is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "Afora is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `Afora is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "Afora is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "Afora is working on the current voice request.";
  }
